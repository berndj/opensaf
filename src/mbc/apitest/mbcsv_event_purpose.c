...
uint32_t mbcstm_event_purposes(uint32_t svc_index, uint32_t ssn_index,
			       uint32_t asys, uint32_t bsys,
			       MBCSTM_TEST_ACTION action, uint32_t act_on_sys,
			       uint32_t act_val, MBCSTM_CB_TEST cb_check,
			       ...
			       MBCSTM_FSM_STATES state_check)
{
	...
	if (asys == mbcstm_cb.sys) {
		mbcstm_cb.svces[svc_index].ssns[ssn_index].csi_role =
		    SA_AMF_HA_ACTIVE;
		mbcstm_cb.svces[svc_index].ssns[ssn_index].dest_role =
		    V_DEST_RL_ACTIVE;
	}
	...
	if (act_on_sys == mbcstm_cb.sys) {
		...
		case MBCSTM_TEST_ACTION_ROLE:
			if (mbcstm_cb.svces[svc_index]
				    .ssns[ssn_index]
				    .csi_role == SA_AMF_HA_ACTIVE &&
			    act_val == SA_AMF_HA_STANDBY) {
				mbcstm_cb.svces[svc_index]
				    .ssns[ssn_index]
				    .csi_role = SA_AMF_HA_QUIESCED;
				if (mbcstm_ssn_set_role(svc_index, ssn_index) !=
				    NCSCC_RC_SUCCESS)
					goto final;
			}
			sleep(1);
		...
		case MBCSTM_TEST_ACTION_CLOSE:
			if (mbcstm_ssn_close(svc_index, ssn_index) !=
			    NCSCC_RC_SUCCESS)
				goto final;
			break;
		/*change*/
	...
	if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
		uint32_t index;
		mbcstm_check_inv(MBCSTM_CHECK_PEER_LIST, svc_index, ssn_index,
				 &peers);
		...
		if (mbcstm_cb.svces[svc_index].ssns[ssn_index].cb_test !=
			...
			if (mbcstm_cb.svces[svc_index]
				.ssns[ssn_index]
				.cb_test_result != NCSCC_RC_SUCCESS)
				goto final;
			else
				tet_printf(
				    "test result is going to be SUCCESS");
		}
	}
...
final:
	...
	mbcstm_cb.svces[svc_index].ssns[ssn_index].cb_test_result =
	    NCSCC_RC_FAILURE;
...
void mbcstm_peerup_event_newactive_to_active()
{
	...
	if (mbcstm_event_purposes(
		svc_index, ssn_index, MBCSTM_SVC_INS1, MBCSTM_SVC_INS2,
		MBCSTM_TEST_ACTION_ROLE, MBCSTM_SVC_INS2, SA_AMF_HA_ACTIVE,
		MBCSTM_CB_NO_TEST, 1, 3, MBCSTM_SSN_ANC2,
		MBCSTM_ACT_STATE_MULTIPLE_ACTIVE) != NCSCC_RC_SUCCESS) {
...
void mbcstm_peerup_event_newactive_to_standby()
{
	...
	if (mbcstm_event_purposes(
		svc_index, ssn_index, MBCSTM_SVC_INS2, MBCSTM_SVC_INS2,
		MBCSTM_TEST_ACTION_NO, 0, 0,
		MBCSTM_CB_STANDBY_COLD_ENCODE_CHECK,
		MBCSTM_TIMER_SEND_COLD_SYNC_PERIOD, 3, MBCSTM_SSN_ANC2,
		MBCSTM_STBY_STATE_WAIT_TO_COLD_SYNC) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
	} else {
		final_res = TET_MBCSV_PASS;
...
void mbcstm_peerup_event_newstandby_to_active()
{
	...
	if (mbcstm_event_purposes(
		svc_index, ssn_index, MBCSTM_SVC_INS1, MBCSTM_SVC_INS2,
		MBCSTM_TEST_ACTION_NO, 0, 0, MBCSTM_CB_ACTIVE_COLD_DECODE_FAIL,
		MBCSTM_TIMER_SEND_COLD_SYNC_PERIOD, 3, MBCSTM_SSN_ANC2,
		MBCSTM_ACT_STATE_WAIT_FOR_COLD_WARM_SYNC) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
	} else {
		final_res = TET_MBCSV_PASS;
...
void mbcstm_peerup_event_newstandby_to_standby()
{
	...
	if (mbcstm_event_purposes(svc_index, ssn_index, MBCSTM_SVC_INS2,
				  MBCSTM_SVC_INS3, MBCSTM_TEST_ACTION_NO, 0, 0,
				  MBCSTM_CB_NO_TEST, 1, 3, MBCSTM_SSN_ANC3,
...
void mbcstm_peerdown_event_active()
{
	...
	if (mbcstm_event_purposes(svc_index, ssn_index, MBCSTM_SVC_INS1,
				  MBCSTM_SVC_INS2, MBCSTM_TEST_ACTION_CLOSE,
				  MBCSTM_SVC_INS2, 0, MBCSTM_CB_NO_TEST, 1, 2,
				  0, 0) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
	} else {
		final_res = TET_MBCSV_PASS;
...
void mbcstm_peerdown_event_standby()
{
	...
	if (mbcstm_event_purposes(svc_index, ssn_index, MBCSTM_SVC_INS2,
				  MBCSTM_SVC_INS1, MBCSTM_TEST_ACTION_CLOSE,
				  MBCSTM_SVC_INS2, 0, MBCSTM_CB_NO_TEST, 1, 2,
				  0, 0) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
...
void mbcstm_cngrole_event_newactive_to_standby()
{
	...
	if (mbcstm_event_purposes(
		svc_index, ssn_index, MBCSTM_SVC_INS2, MBCSTM_SVC_INS3,
		MBCSTM_TEST_ACTION_ROLE, MBCSTM_SVC_INS3, SA_AMF_HA_ACTIVE,
		MBCSTM_CB_STANDBY_COLD_ENCODE_CHECK, 1, 3, MBCSTM_SSN_ANC3,
		MBCSTM_STBY_STATE_WAIT_TO_COLD_SYNC) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
	} else {
...
void mbcstm_cngrole_event_newstandby_to_active()
{
	...
	if (mbcstm_event_purposes(
		svc_index, ssn_index, MBCSTM_SVC_INS1, MBCSTM_SVC_INS2,
		MBCSTM_TEST_ACTION_ROLE, MBCSTM_SVC_INS2, SA_AMF_HA_STANDBY,
...
void mbcstm_cngrole_event_newstandby_to_standby()
{
	...
	if (mbcstm_event_purposes(svc_index, ssn_index, MBCSTM_SVC_INS2,
				  MBCSTM_SVC_INS3, MBCSTM_TEST_ACTION_ROLE,
				  MBCSTM_SVC_INS2, SA_AMF_HA_STANDBY,
				  MBCSTM_CB_NO_TEST, 1, 3, MBCSTM_SSN_ANC2,
				  MBCSTM_STBY_STATE_IDLE) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
...
void mbcstm_standby_active_quiesced()
{
	...
	if (mbcstm_event_purposes(svc_index, ssn_index, MBCSTM_SVC_INS2,
				  MBCSTM_SVC_INS2, MBCSTM_TEST_ACTION_ROLE,
				  MBCSTM_SVC_INS2, SA_AMF_HA_QUIESCED,
				  MBCSTM_CB_NO_TEST, 1, 3, MBCSTM_SSN_ANC2,
				  MBCSTM_STBY_STATE_IDLE) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
...
/* NEW */
/* Note: Below two cases are not added in to the arry because behaviour is
...
void mbcstm_multi_multi_active()
{
	...
	if (mbcstm_event_purposes(svc_index, ssn_index, MBCSTM_SVC_INS1,
				  MBCSTM_SVC_INS2, MBCSTM_TEST_ACTION_ROLE,
				  MBCSTM_SVC_INS3, SA_AMF_HA_ACTIVE,
				  MBCSTM_CB_NO_TEST, 1, 3, MBCSTM_SSN_ANC2,
				  MBCSTM_STBY_STATE_IDLE) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
...
void mbcstm_multi_multi_active_standby()
{
	...
	if (mbcstm_event_purposes(svc_index, ssn_index, MBCSTM_SVC_INS1,
				  MBCSTM_SVC_INS3, MBCSTM_TEST_ACTION_ROLE,
				  MBCSTM_SVC_INS1, SA_AMF_HA_STANDBY,
				  MBCSTM_CB_NO_TEST, 1, 3, MBCSTM_SSN_ANC2,
				  MBCSTM_STBY_STATE_IDLE) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
...
1. While acitve in wait for cold sync request , async updates can not be sent,
case need to come.
