	...
	for (svc_count = 1; svc_count < MBCSTM_SVC_MAX; svc_count++) {
      ...
      mbcstm_cb.svces[svc_count].version.majorVersion = 1;
		...
		for (ssn_index = 1; ssn_index < MBCSTM_SSN_MAX; ssn_index++) {
			...
			/* printf("\n ssn_count %d",
			 * mbcstm_cb.svces[svc_count].ssn_count); */
			mbcstm_cb.svces[svc_count].ssns[ssn_count].svc_index =
			    svc_count;
			mbcstm_cb.svces[svc_count].ssns[ssn_count].cb_test =
			    MBCSTM_CB_NO_TEST;
			mbcstm_cb.svces[svc_count].ssns[ssn_count].dest_id =
			    mbcstm_cb.vdests[ssn_index].dest_id;
			mbcstm_cb.svces[svc_count].ssns[ssn_count].anchor =
			    mbcstm_cb.vdests[ssn_index].anchor;
			mbcstm_cb.svces[svc_count].ssns[ssn_count].dest_role =
			    V_DEST_RL_STANDBY;
			/* V_DEST_RL_INIT,V_DEST_RL_ACTIVE,V_DEST_RL_STANDBY */
			mbcstm_cb.svces[svc_count].ssns[ssn_count].csi_role =
			    SA_AMF_HA_STANDBY;
			/* SA_AMF_HA_ACTIVE, SA_AMF_HA_STANDBY,
			 * SA_AMF_HA_QUIESCED, SA_AMF_HA_STOPPING */
			mbcstm_cb.svces[svc_count].ssns[ssn_count].ssn_index =
			    ssn_count;
			mbcstm_cb.svces[svc_count].ssns[ssn_count].ws_flag =
			    true;
			mbcstm_cb.svces[svc_count].ssns[ssn_count].ws_timer =
