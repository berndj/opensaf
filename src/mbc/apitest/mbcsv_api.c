...
uint32_t mbcstm_dest_close()
{
	...
	for (ssn_index = 1; ssn_index <= ssn_count; ssn_index++) {
		...
		if (ncsvda_api(&vda_info) != NCSCC_RC_SUCCESS) {
			mbcstm_print(__FILE__, fun_name, __LINE__,
				     "VDA_DESTROY", NCSCC_RC_FAILURE);
			return NCSCC_RC_FAILURE;
		}
...
uint32_t mbcstm_svc_registration(uint32_t svc_index)
{
	...
	if (NCSCC_RC_SUCCESS != ncs_mbcsv_svc(&mbcsv_arg)) {
		mbcstm_print(__FILE__, fun_name, __LINE__, "REGS",
			     NCSCC_RC_FAILURE);
		return NCSCC_RC_FAILURE;
	}
	...
	if (NCSCC_RC_SUCCESS != mbcstm_ssn_get_select(svc_index)) {
		mbcstm_print(__FILE__, fun_name, __LINE__, "GET_SELECT",
			     NCSCC_RC_FAILURE);
		return NCSCC_RC_FAILURE;
	}
...
uint32_t mbcstm_svc_finalize(uint32_t svc_index)
{
	...
	if (NCSCC_RC_SUCCESS != ncs_mbcsv_svc(&mbcsv_arg)) {
		mbcstm_print(__FILE__, fun_name, __LINE__, "FINALIZE",
			     NCSCC_RC_FAILURE);
		return NCSCC_RC_FAILURE;
	}
...
uint32_t mbcstm_ssn_open(uint32_t svc_index, uint32_t ssn_index)
{
	...
	if (NCSCC_RC_SUCCESS != ncs_mbcsv_svc(&mbcsv_arg)) {
		mbcstm_print(__FILE__, fun_name, __LINE__, "OPEN",
			     NCSCC_RC_FAILURE);
		return NCSCC_RC_FAILURE;
	}
...
uint32_t mbcstm_ssn_set_role(uint32_t svc_index, uint32_t ssn_index)
{
	...
	if (NCSCC_RC_SUCCESS != ncs_mbcsv_svc(&mbcsv_arg)) {
		mbcstm_print(__FILE__, fun_name, __LINE__, "ROLE",
			     NCSCC_RC_FAILURE);
		return NCSCC_RC_FAILURE;
	}
...
uint32_t mbcstm_ssn_close(uint32_t svc_index, uint32_t ssn_index)
{
	...
	if (NCSCC_RC_SUCCESS != ncs_mbcsv_svc(&mbcsv_arg)) {
		mbcstm_print(__FILE__, fun_name, __LINE__, "CLOSE",
			     NCSCC_RC_FAILURE);
		return NCSCC_RC_FAILURE;
	}
...
uint32_t mbcstm_svc_dispatch(uint32_t svc_index)
{
	...
	if (NCSCC_RC_SUCCESS != ncs_mbcsv_svc(&mbcsv_arg)) {
		mbcstm_print(__FILE__, fun_name, __LINE__, "DISPATCH",
			     NCSCC_RC_FAILURE);
		return NCSCC_RC_FAILURE;
	}
...
uint32_t mbcstm_start_process_thread(uint32_t svc_index)
{
	...
	*index = svc_index;
	...
	if (m_NCS_TASK_CREATE((NCS_OS_CB)mbcstm_process_events, index,
			      "MBCSV_DISPATCH", NCS_TASK_PRIORITY_7,
			...
			NCS_MBCSV_OBJ obj)
{
	...
	if (NCSCC_RC_SUCCESS != ncs_mbcsv_svc(&mbcsv_arg)) {
		mbcstm_print(__FILE__, fun_name, __LINE__, "OBJ SET",
			     NCSCC_RC_FAILURE);
		return NCSCC_RC_FAILURE;
	}
...
uint32_t mbcstm_svc_encode_cb(NCS_MBCSV_CB_ARG *arg)
{
	...
	case 1:
		tet_printf("ENCODE MSG TYPE:NCS_MBCSV_MSG_ASYNC_UPDATE");
		printf("\nENCODE MSG TYPE:NCS_MBCSV_MSG_ASYNC_UPDATE");
		break;
	case 2:
		tet_printf("ENCODE MSG TYPE:NCS_MBCSV_MSG_COLD_SYNC_REQ");
		printf("\nENCODE MSG TYPE:NCS_MBCSV_MSG_COLD_SYNC_REQ");
		break;
	case 3:
		tet_printf("ENCODE MSG TYPE:NCS_MBCSV_MSG_COLD_SYNC_RESP");
	...
	case 4:
		...
		break;
	case 5:
		tet_printf("ENCODE MSG TYPE:NCS_MBCSV_MSG_WARM_SYNC_REQ");
		printf("\nENCODE MSG TYPE:NCS_MBCSV_MSG_WARM_SYNC_REQ");
		break;
	case 6:
		tet_printf("ENCODE MSG TYPE:NCS_MBCSV_MSG_WARM_SYNC_RESP");
	...
	case 7:
		...
		break;
	case 8:
		tet_printf("ENCODE MSG TYPE:NCS_MBCSV_MSG_DATA_REQ");
		printf("\nENCODE MSG TYPE:NCS_MBCSV_MSG_DATA_REQ");
		break;
	case 9:
		tet_printf("ENCODE MSG TYPE:NCS_MBCSV_MSG_DATA_RESP");
		printf("\nENCODE MSG TYPE:NCS_MBCSV_MSG_DATA_RESP");
		break;
	case 10:
		tet_printf("ENCODE MSG TYPE:NCS_MBCSV_MSG_DATA_RESP_COMPLETE");
	...
	case NCS_MBCSV_MSG_ASYNC_UPDATE:
		...
		if (arg->info.encode.io_action == NCS_MBCSV_ACT_RMV) {
			/*
			  U r supposed to ask all the peers to REMOVE the data
			  element arg->info.encode.io_reo_hdl from their data
			  inventory remove the last data element
			*/
		...
		if (arg->info.encode.io_action == NCS_MBCSV_ACT_UPDATE) {
			/*
			  U r supposed to ask all the peers to UPDATE the data
			  element arg->info.encode.io_reo_hdl with this new data
			*/
			if (mbcstm_data_encode(arg->info.encode.io_reo_type,
					       arg->info.encode.io_reo_hdl,
					       &arg->info.encode.io_uba,
				   ...
				   NCS_UBAID *uba, MBCSTM_SSN *ssn)
{
	...
	switch (reo_type) {
	case NORMAL_DATA:
		data = ncs_enc_reserve_space(uba, sizeof(MBCSTM_CSI_DATA) +
						      sizeof(uint32_t));
		if (data == NULL) {
			tet_printf("\n fake_encode_elem: DATA NULL");
			return NCSCC_RC_FAILURE;
	...
	case PERFORMANCE_DATA:
		...
		ncs_encode_n_octets_in_uba(uba, (uint8_t *)info->msg,
					   info->length);
		break;
	default:
		return NCSCC_RC_FAILURE;
				   ...
				   MBCSTM_SSN *ssn)
{
	...
	case NORMAL_DATA:
		data = ncs_dec_flatten_space(
		    uba, data_buff, sizeof(MBCSTM_CSI_DATA) + sizeof(uint32_t));
	...
	case PERFORMANCE_DATA:
		...
		ncs_decode_n_octets_from_uba(uba, (uint8_t *)info->msg,
					     info->length);
...
uint32_t mbcstm_svc_decode_cb(NCS_MBCSV_CB_ARG *arg)
{
	...
	case 1:
		tet_printf("DECODE MSG TYPE:NCS_MBCSV_MSG_ASYNC_UPDATE");
		printf("\nDECODE MSG TYPE:NCS_MBCSV_MSG_ASYNC_UPDATE");
		break;
	case 2:
		tet_printf("DECODE MSG TYPE:NCS_MBCSV_MSG_COLD_SYNC_REQ");
		printf("\nDECODE MSG TYPE:NCS_MBCSV_MSG_COLD_SYNC_REQ");
		break;
	case 3:
		tet_printf("DECODE MSG TYPE:NCS_MBCSV_MSG_COLD_SYNC_RESP");
	...
	case 4:
		...
		break;
	case 5:
		tet_printf("DECODE MSG TYPE:NCS_MBCSV_MSG_WARM_SYNC_REQ");
		printf("\nDECODE MSG TYPE:NCS_MBCSV_MSG_WARM_SYNC_REQ");
		break;
	case 6:
		tet_printf("DECODE MSG TYPE:NCS_MBCSV_MSG_WARM_SYNC_RESP");
	...
	case 7:
		...
		break;
	case 8:
		tet_printf("DECODE MSG TYPE:NCS_MBCSV_MSG_DATA_REQ");
		printf("\nDECODE MSG TYPE:NCS_MBCSV_MSG_DATA_REQ");
		break;
	case 9:
		tet_printf("DECODE MSG TYPE:NCS_MBCSV_MSG_DATA_RESP");
		printf("\nDECODE MSG TYPE:NCS_MBCSV_MSG_DATA_RESP");
		break;
	case 10:
		tet_printf("DECODE MSG TYPE:NCS_MBCSV_MSG_DATA_RESP_COMPLETE");
	...
	case NCS_MBCSV_MSG_ASYNC_UPDATE:
		if (ssn->perf_flag == 1)
			mbcstm_data_decode(PERFORMANCE_DATA,
					   &arg->info.decode.i_uba, ssn);
		else {
			if (arg->info.decode.i_action == NCS_MBCSV_ACT_ADD) {
				if (mbcstm_data_decode(
					NORMAL_DATA, &arg->info.decode.i_uba,
					ssn) == NCSCC_RC_FAILURE) {
					perror(
			...
			if (arg->info.decode.i_action == NCS_MBCSV_ACT_RMV) {
				/*
				  u r supposed to remove the data element with
				  index decrement the data_count
				*/
	...
	case NCS_MBCSV_MSG_COLD_SYNC_RESP:
		if (ssn->perf_flag == 1)
			...
			if (mbcstm_data_decode(NORMAL_DATA,
					       &arg->info.decode.i_uba,
					       ssn) == NCSCC_RC_FAILURE) {
				perror("\nCB ERROR: Not able to Decode data\n");
				return NCSCC_RC_FAILURE;
			}
	...
	case NCS_MBCSV_MSG_WARM_SYNC_RESP:
		if (mbcstm_data_decode(NORMAL_DATA, &arg->info.decode.i_uba,
				       ssn) == NCSCC_RC_FAILURE) {
			perror("\nCB ERROR: Not able to Decode data\n");
			return NCSCC_RC_FAILURE;
		}
...
uint32_t mbcstm_print(char *file, char *fun, uint32_t line, char *event,
		      uint32_t status)
{
	if (status == NCSCC_RC_FAILURE) {
		/*tet_printf("\n AT LINE %d IN FUNCTION %s IN FILE %s CALL TO %s
		  FAILED", line, fun, file, event);*/
		...
		/*change
		  ...
		  FILE \ %s CALL TO %s FAILED", line, fun, file, event);*/
	} else {
		/*tet_printf("\n AT LINE %d IN FUNCTION %s IN FILE %s CALL TO %s
		...
		/*change
		  ...
		  IN\ FILE %s CALL TO %s SUCCEED",line, fun, file, event);*/
	}
	/*change*/
	return 1;
}
	...
	for (svc_index = 1; svc_index <= svc_count; svc_index++) {
		...
		for (ssn_index = 1; ssn_index <= ssn_count; ssn_index++) {
			...
			tet_printf(
			    ...
			    ssn->dest_id, ssn->anchor, ssn->pwe_hdl,
			    ssn->ckpt_hdl, ssn->csi_role);
		}
	}
	/*change*/
	return 1;
...
uint32_t mbcstm_sync_point()
{
	...
	if (tet_remsync(0, sys, num_sys, MBCSTM_SYNC_TIMEOUT, TET_SV_NO,
			NULL) != 0)

	{
		mbcstm_print(__FILE__, fun_name, __LINE__, "TET SYNC",
			     NCSCC_RC_FAILURE);
