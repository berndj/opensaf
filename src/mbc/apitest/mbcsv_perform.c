			      ...
			      uint32_t size, uint32_t send_type)
{
	...
	if (mbcstm_svc_cp_send(svc_index, ssn_index, NCS_MBCSV_ACT_ADD,
			       PERFORMANCE_DATA, (long)data,
...
uint32_t mbcstm_verify_sync_msg(SSN_PERF_DATA *data, MBCSTM_SSN *ssn)
{
	...
	if (crc != data->crc) {
		...
		ssn->cb_flag++;
		/* printf("\n crc checking is successfull for msg %d",
		 * ssn->cb_flag);    */
	}
	return NCSCC_RC_SUCCESS;
}
				   ...
				   NCS_MBCSV_MSG_TYPE send_type)
{
	...
	if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
		while (count-- != 0) {
			/* sleep(1); */
			if (mbcstm_perf_sync_msg(svc_index, ssn_index, size,
						 send_type) != NCSCC_RC_SUCCESS)
				goto final;
			size += 500;
		}
		...
		if ((mbcstm_cb.svces[svc_index]
		    ...
		    (mbcstm_cb.svces[svc_index].ssns[ssn_index].cb_flag !=
		     msg_count))
			goto final;
				   ...
				   MBCSTM_CB_TEST sync)
{
	...
	if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
		...
		for (i = 0; i < nrepeat; i++) {
			...
			if (mbcstm_ssn_open(svc_index, ssn_index) !=
			    NCSCC_RC_SUCCESS)
				goto final;
			...
			if (mbcstm_cb.svces[svc_index]
				...
				double t0;
				;
				t1 = mbcstm_cb.svces[svc_index]
					 .ssns[ssn_index]
					 .perf_final_time;
				t0 = (double)open_time.tv_sec +
				     (double)open_time.tv_usec * 1e-6;
				t0 = t1 - t0;
				    ...
				    uint32_t asys)
{
	...
	if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
		double t;
		while (mbcstm_cb.svces[svc_index].ssns[ssn_index].cold_flag !=
...
void mbcstm_test_usrasync_send_messages()
{
	...
	if (mbcstm_msg_check_purposes(svc_index, ssn_index,
