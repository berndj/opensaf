...
Sec : %d Usec : %d",
	    ssn->sync_count, ssn->data_count, data->data_id, data->sec,
	    data->usec);
...
uint32_t mbcstm_print_data_points(uint32_t svc_index, uint32_t ssn_index)
{
	...
	for (i = 1; i <= ssn->data_count; i++) {
		data = &ssn->data[i];
		printf("\n Data id : %d Sec : %d Usec : %d", data->data_id,
		       data->sec, data->usec);
	}
	printf("\n\t-----        END                 -----\n");
	return NCSCC_RC_SUCCESS;
				  ...
				  MBCSTM_CB_TEST sync)
{
	...
	if (mbcstm_cb.sys == MBCSTM_SVC_INS1)
		if (sync != MBCSTM_CB_NO_TEST)
			mbcstm_cb.svces[svc_index].ssns[ssn_index].cb_test =
			    sync;
	...
	if (mbcstm_cb.sys == MBCSTM_SVC_INS1)
		for (index = send_index; index <= send_count; index++) {
			if (mbcstm_svc_cp_send(svc_index, ssn_index, action,
					       NORMAL_DATA, index,
...
void mbcstm_ckpt_sendsync()
{
	...
	if (mbcstm_ckpt_send_purpose(svc_index, ssn_index, MBCSTM_SVC_INS1, 1,
				     1, NCS_MBCSV_ACT_ADD, NCS_MBCSV_SND_SYNC,
...
void mbcstm_ckpt_standby_sendsync()
{
	...
	if (mbcstm_ckpt_send_purpose(svc_index, ssn_index, MBCSTM_SVC_INS2, 1,
				     1, NCS_MBCSV_ACT_ADD, NCS_MBCSV_SND_SYNC,
				     MBCSTM_CB_NO_TEST) == NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_PASS;
		cases_passed++;
