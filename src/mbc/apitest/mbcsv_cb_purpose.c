...
uint32_t mbcstm_sync_purposes(uint32_t svc_index, uint32_t ssn_index,
			      uint32_t asys, MBCSTM_CB_TEST sync, uint32_t warm,
			      uint32_t wait)
{
	/*change*/
...
uint32_t mbcstm_data_purposes(uint32_t svc_index, uint32_t ssn_index,
			      uint32_t asys, uint32_t wait, MBCSTM_CB_TEST sync)
{
	/*change*/
	uint32_t mbcstm_create_data_point(uint32_t, uint32_t);
	uint32_t mbcstm_check_inv(MBCSTM_CHECK, uint32_t, uint32_t, void *);
	...
	if (asys != mbcstm_cb.sys) {
		...
		*/
		if (mbcstm_svc_data_request(svc_index, ssn_index) !=
		    NCSCC_RC_SUCCESS)
			goto final;
		else {
			printf(
...
uint32_t mbcstm_notify_purposes(uint32_t svc_index, uint32_t ssn_index,
				uint32_t asys, uint32_t isys,
				MBCSTM_CB_TEST sync,
		...
		if (ssn->csi_role == SA_AMF_HA_STANDBY) {
			if (arg->info.encode.io_msg_type ==
				...
				if (ssn->cb_flag == 0)
					...
					ssn->cb_test_result = NCSCC_RC_SUCCESS;
					if (pthread_mutex_unlock(&mutex_cb) ==
					    0)
						;
					tet_printf("\n mutex given");
				}
 ...
 WARM_SYNC_RESP_COMPLETE");
	...
	case MBCSTM_CB_ACTIVE_COLD_TIMER_EXP:
		if (ssn->csi_role == SA_AMF_HA_STANDBY) {
			...
			if (arg->info.error.i_code ==
			    NCS_MBCSV_COLD_SYNC_TMR_EXP) {
				gettimeofday(&tv, NULL);
...
void mbcstm_coldsync_standby_encode_fail()
{
	...
	if (mbcstm_sync_purposes(svc_index, ssn_index, MBCSTM_SVC_INS2,
				 MBCSTM_CB_STANDBY_COLD_ENCODE_FAIL, 0,
				 11) != NCSCC_RC_SUCCESS) {
...
void mbcstm_coldsync_coldrequest_noresponse()
{
	...
	if (mbcstm_sync_purposes(svc_index, ssn_index, MBCSTM_SVC_INS1,
				 MBCSTM_CB_ACTIVE_COLD_DECODE_FAIL, 0,
				 11) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
	} else {
...
void mbcstm_coldsync_stdby_decode_fail()
{
	...
	if (mbcstm_sync_purposes(svc_index, ssn_index, MBCSTM_SVC_INS2,
				 MBCSTM_CB_STANDBY_COLD_DECODE_FAIL, 0,
				 11) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
	} else {
...
void mbcstm_coldsync_active_encode_fail()
{
	...
	if (mbcstm_sync_purposes(svc_index, ssn_index, MBCSTM_SVC_INS1,
				 MBCSTM_CB_ACTIVE_COLD_ENCODE_FAIL, 0,
				 11) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
	} else {
...
void mbcstm_coldsync_timer()
{
	...
	if (mbcstm_sync_purposes(svc_index, ssn_index, MBCSTM_SVC_INS2,
				 MBCSTM_CB_ACTIVE_COLD_TIMER_EXP, 0,
...
void mbcstm_warmsync_timer()
{
	...
	if (mbcstm_sync_purposes(svc_index, ssn_index, MBCSTM_SVC_INS2,
				 MBCSTM_CB_ACTIVE_WARM_TIMER_EXP, 0,
				 2 * MBCSTM_TIMER_SEND_WARM_SYNC_PERIOD) !=
	    NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
	} else {
...
void mbcstm_warmsync_complete()
{
	...
	if (mbcstm_data_purposes(
		svc_index, ssn_index, asys, MBCSTM_TIMER_WARM_SYNC_CMPLT_PERIOD,
		MBCSTM_CB_STANDBY_WARM_CMPL_FAIL) != NCSCC_RC_SUCCESS)
...
void mbcstm_data_request_idle()
{
	...
	if (mbcstm_data_purposes(svc_index, ssn_index, asys, 1,
...
void mbcstm_send_notify_idle()
{
	...
	if (mbcstm_notify_purposes(
		svc_index, ssn_index, asys, isys, MBCSTM_CB_PEER_INFO_FAIL,
		NCS_MBCSV_ALL_PEERS, case_disc, len) == NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_PASS;
		cases_passed++;
	} else {
		final_res = TET_MBCSV_FAIL;
...
void mbcstm_notify_send_all()
{
	...
	if (mbcstm_notify_purposes(svc_index, ssn_index, asys, isys,
				   MBCSTM_CB_NO_TEST, NCS_MBCSV_ALL_PEERS,
...
void mbcstm_notify_send_to_active()
{
	...
	if (mbcstm_notify_purposes(svc_index, ssn_index, asys, isys,
				   MBCSTM_CB_NO_TEST, NCS_MBCSV_ACTIVE,
				   case_disc, len) != NCSCC_RC_SUCCESS)
...
void mbcstm_notify_send_to_standby()
{
	...
	if (mbcstm_notify_purposes(svc_index, ssn_index, asys, isys,
				   MBCSTM_CB_NO_TEST, NCS_MBCSV_STANDBY,
				   case_disc, len) != NCSCC_RC_SUCCESS)
