...
static uint32_t mbcstm_get_output_descriptor(MBCSTM_FILE *fd,
					     MBCSTM_FILE *out_fd);
static uint32_t mbcstm_close_output_descriptor(MBCSTM_FILE *fd,
					       MBCSTM_FILE *out_fd);
uint32_t mbcstm_get_info(MBCSTM_FILE *fd, MBCSTM_CHECK check,
			 uint32_t svc_index, uint32_t ssn_index, void *data);
			  ...
			  uint32_t ssn_index, void *data)
{
	...
	if (mbcstm_close_output_descriptor(&my_fd, &out_fd) !=
	    NCSCC_RC_SUCCESS) {
		mbcstm_print(__FILE__, fun_name, __LINE__, "MBCSTM CHECK INV ",
			     NCSCC_RC_FAILURE);
		return NCSCC_RC_FAILURE;
			 ...
			 uint32_t svc_index, uint32_t ssn_index, void *data)
{
	...
	/*
	   if(mbcstm_line_compare(fd,"|               |           NONE        |
	   NONE                   |") == 0)
	   {
	...
	*/
	...
	while (inc) {
		...
		if (svc_id == to_svc_id) {
			...
			uint64_t my_anchor;
			uint32_t in_quiescing, peer_up_sent, ftm_role_set,
			    role_set, data_req_sent;
			uint32_t peer = 1;
			uint32_t ssn = 1;

			inc = 0;
			printf(
			    "sevice id matched going to  get session information");

			if (mbcstm_line_compare(
				fd,
				"|               |           NONE        |           NONE                   |") ==
			    0) {
				printf(
			...
			while (ssn) {
				...
				if (sscanf(
					buf,
					"|               |%8X|%8X| %c |%c|                                  |",
					&pwe_hdl, &ckpt_hdl, &warm_sync_on,
					&role) == 4) {
					len = mbcstm_getline(fd, buf);
					if (sscanf(
						buf,
						"|               |%16llX|%1d%1d%1d%1d%1d |           MY   PEERS              |",
						&my_anchor, &in_quiescing,
						&peer_up_sent, &ftm_role_set,
						&role_set,
						&data_req_sent) == 6) {
						printf("\n My cis data");
					}
				...
				while (peer) {
					...
					char peer_role, peer_state[5],
					    peer_comp[2], peer_cold[2];
					...
					if (sscanf(
						...
						if (sscanf(
							buf,
							"|               |                       |%c|%4s|%c| %c |%1d%1d%1d%1d%1d%1d%1d%1d%1d%1d%1d|%2d%2d%2d |    |",
							&peer_role, peer_state,
							&is_incompatible,
							&is_cold_sync_done,
							&peer_incompatible,
							&warm_sync_sent,
					...
					if (get_peer_info == 1) {
						...
						peers->peers[peer_count].peer_anchor
						= peer_anchor;
						*/
						/*
