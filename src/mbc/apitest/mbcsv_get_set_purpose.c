#include "mbcsv_api.h"
#include "tet_api.h"
uint32_t mbcstm_get_set_purposes(uint32_t svc_index, uint32_t ssn_index,
				 uint32_t asys, uint32_t action,
				 NCS_MBCSV_OBJ obj_type, uint32_t obj_val,
				 ...
				 MBCSTM_FSM_STATES state_check)
{
	...
	if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
		if (action == NCS_MBCSV_OP_OBJ_SET) {
			...
			if (obj_type == NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF)
				old_value = mbcstm_cb.svces[svc_index]
						.ssns[ssn_index]
						.ws_flag;
			else
				old_value = mbcstm_cb.svces[svc_index]
...
void mbcstm_set_warm_sync_off_active()
{
	...
	if (mbcstm_get_set_purposes(
		svc_index, ssn_index, MBCSTM_SVC_INS1, NCS_MBCSV_OP_OBJ_SET,
		NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF, false, MBCSTM_CB_NO_TEST, 1, 3,
		MBCSTM_SSN_ANC2,
...
void mbcstm_set_warm_sync_off_standby()
{
	...
	if (mbcstm_get_set_purposes(
		svc_index, ssn_index, MBCSTM_SVC_INS2, NCS_MBCSV_OP_OBJ_SET,
		NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF, false, MBCSTM_CB_NO_TEST, 1, 3,
		MBCSTM_SSN_ANC2,
		MBCSTM_ACT_STATE_MULTIPLE_ACTIVE) != NCSCC_RC_SUCCESS)
...
void mbcstm_set_warm_sync_on_standby()
{
	...
	if (mbcstm_get_set_purposes(
		svc_index, ssn_index, MBCSTM_SVC_INS2, NCS_MBCSV_OP_OBJ_SET,
		NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF, true, MBCSTM_CB_NO_TEST, 1, 3,
		MBCSTM_SSN_ANC2,
		MBCSTM_ACT_STATE_MULTIPLE_ACTIVE) != NCSCC_RC_SUCCESS)
...
void mbcstm_get_warm_sync_on_off()
{
	...
	if (mbcstm_get_set_purposes(
		svc_index, ssn_index, MBCSTM_SVC_INS2, NCS_MBCSV_OP_OBJ_GET,
		NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF, true, MBCSTM_CB_NO_TEST, 1, 3,
		MBCSTM_SSN_ANC2,
		MBCSTM_ACT_STATE_MULTIPLE_ACTIVE) != NCSCC_RC_SUCCESS)
...
void mbcstm_set_warm_sync_timer_active()
{
	...
	if (mbcstm_get_set_purposes(
		svc_index, ssn_index, MBCSTM_SVC_INS1, NCS_MBCSV_OP_OBJ_SET,
		NCS_MBCSV_OBJ_TMR_WSYNC, 1000, MBCSTM_CB_NO_TEST, 1, 3,
		MBCSTM_SSN_ANC2,
		MBCSTM_ACT_STATE_MULTIPLE_ACTIVE) != NCSCC_RC_SUCCESS) {
		final_res = TET_MBCSV_FAIL;
		cases_failed++;
