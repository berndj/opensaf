...
void tet_mbcsv_op(int choice)
{
	...
	case 1:
		tet_printf(
		    "Not able to Succeed:Supply Invalid OP code to MBCSv ");
	...
	case 2:
		tet_printf(
		    "Not able to Succeed:Supply NULL callback to INITIALIZE ");
		mbcsv_arg.i_op = 0;
		if (ncs_mbcsv_svc(&mbcsv_arg) == NCSCC_RC_SUCCESS)
			FAIL = 1;
		else
...
void tet_mbcsv_initialize(int choice)
{
	...
	case 2:
		tet_printf(
		    "Not able to Finalize an already Finalized MBCSV instance");
		if (mbcstm_svc_finalize(1) == NCSCC_RC_SUCCESS)
			FAIL = 1;
		break;
	case 3:
	...
	*/
	...
	case 8:
		...
		break;
		/*    case 9:
		      tet_printf("Not able to Dispatch ALL with NULL Mbcsv
		   Handle"); mbcstm_cb.svces[1].disp_flags =SA_DISPATCH_ALL;
		      mbcstm_cb.svces[1].task_flag=false;
		      if(mbcstm_svc_registration(1)!=NCSCC_RC_SUCCESS)
		      FAIL=1;
		      if(mbcstm_svc_dispatch(1)!=NCSCC_RC_SUCCESS)
...
void tet_mbcsv_change_role(int choice)
{
	...
	case 11:
		tet_printf(
		    "Not able to Change Role with Invalid Checkpoint handle");
...
void tet_mbcsv_Notify(int choice)
{
	...
	uint32_t mbcstm_svc_send_notify(
	    uint32_t, uint32_t, NCS_MBCSV_NTFY_MSG_DEST, char *, uint32_t);
	...
	case 1:
		...
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			...
			mbcstm_sync_point();
			if (mbcstm_svc_send_notify(1, 1, NCS_MBCSV_STANDBY, str,
	...
	case 2:
		...
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			...
			mbcstm_sync_point();
			if (mbcstm_svc_send_notify(1, 1, NCS_MBCSV_ALL_PEERS,
						   str,
						   len) != NCSCC_RC_SUCCESS) {
				tet_printf(
	...
	case 3:
		...
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			...
			sleep(2);
			mbcstm_sync_point();
			if (mbcstm_svc_send_notify(1, 1, NCS_MBCSV_ACTIVE, str,
						   len) != NCSCC_RC_SUCCESS) {
	...
	case 4:
		...
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			...
			mbcstm_cb.svces[1].ssns[1].csi_role =
			    SA_AMF_HA_QUIESCING;
			if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
				perror("Standby to Active role change Failed");
				FAIL = 1;
	...
	case 7:
		tet_printf(
		    "BIG(1151 Bytes) Notify message to STANDBY from STANDBY");
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			memset(BIG, 'S', 1200);
			BIG[1199] = '\0';
			printf("\n BIG length = %ld\n",
			...
			mbcstm_sync_point();
			if (mbcstm_svc_send_notify(1, 1, NCS_MBCSV_STANDBY, BIG,
						   1150) != NCSCC_RC_SUCCESS) {
				tet_printf("\n Send notify to Standby failed");
				FAIL = 1;
			}
		} else {
	...
	case 8:
		tet_printf(
		    "NOT able to Send Notify message to ACTIVE from ACTIVE");
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			mbcstm_cb.svces[1].ssns[1].csi_role = SA_AMF_HA_ACTIVE;
			if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
				perror("Standby to Active role change Failed");
	...
	case 10:
		tet_printf(
		    "Not able to Send Notify message with invalid Destination");
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			mbcstm_cb.svces[1].ssns[1].csi_role = SA_AMF_HA_ACTIVE;
			if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
				perror("Standby to Active role change Failed");
	...
	case 11:
		tet_printf(
		    "Not able to Send Notify message with invalid Ckpt Handle");
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			mbcstm_cb.svces[1].ssns[1].csi_role = SA_AMF_HA_ACTIVE;
			if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
				perror("Standby to Active role change Failed");
			...
			temp_ckpt_hdl = mbcstm_cb.svces[1].ssns[1].ckpt_hdl;
			mbcstm_cb.svces[1].ssns[1].ckpt_hdl =
			    (NCS_MBCSV_HDL)(long)NULL;
			if (mbcstm_svc_send_notify(1, 1, NCS_MBCSV_STANDBY, str,
						   len) == NCSCC_RC_SUCCESS) {
				tet_printf("\n Send notify to Standby failed");
				FAIL = 1;
	...
	case 12:
		tet_printf(
		    "Not able to Send Notify message to STANDBY from ACTIVE \
when No STANDBY peer available");
	...
	case 13:
		tet_printf(
		    "Not able to Send Notify message to ACTIVE from STANDBY \
when No ACTIVE peer available");
	...
	case 14:
		tet_printf(
		    "Send Notify message to IDLE STANDBY Peer  from IDLE ACTIVE");
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
	...
	case 15:
		tet_printf(
		    "Not able to Send Notify message after closing the session");
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			mbcstm_cb.svces[1].ssns[1].csi_role = SA_AMF_HA_ACTIVE;
			if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
				perror("Standby to Active role change Failed");
	...
	if ((choice == 15) && (mbcstm_cb.sys == MBCSTM_SVC_INS1))
		printf("\n");
	else if (mbcstm_ssn_close(1, 1) != NCSCC_RC_SUCCESS)
		FAIL = 1;
	mbcstm_sync_point();
	mbcstm_cb.svces[1].ssns[1].csi_role = SA_AMF_HA_STANDBY;
...
void tet_get_set_warm_sync(int choice)
{
	...
	*/
	...
	case 2:
		tet_printf(
		    "Check Whether Is_Warm_Sync_ON is populated properly");
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_GET,
			       NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF);
		fflush(stdout);
		sleep(2);
	...
	case 3:
		tet_printf(
		    "Not able to  Enable already enabled Warm Sync Flag");
	...
	case 4:
		...
		mbcstm_cb.svces[1].ssns[1].ws_timer = 1000;
		if (mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
				   NCS_MBCSV_OBJ_TMR_WSYNC) != NCSCC_RC_SUCCESS)
			FAIL = 1;
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_GET,
			       NCS_MBCSV_OBJ_TMR_WSYNC);
		sleep(2);
	...
	case 9:
		...
		mbcsv_arg.info.obj_get.i_ckpt_hdl =
		    (NCS_MBCSV_HDL)(long)NULL; /*wrong valude*/
	...
	case 12:
		...
		mbcsv_arg.info.obj_set.i_ckpt_hdl =
		    (NCS_MBCSV_HDL)(long)NULL; /*wrong valude*/
		if (NCSCC_RC_SUCCESS == ncs_mbcsv_svc(&mbcsv_arg))
			FAIL = 1;
		break;
	...
	case 15:
		tet_printf(
		    "Try to set the same value for WARN SYNC ON flag again");
		mbcstm_cb.svces[1].ssns[1].ws_flag =
		    mbcstm_cb.svces[1].ssns[1].ws_flag;
		if (mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
				   NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF) !=
		    NCSCC_RC_SUCCESS)
		...
		sleep(2);
		tet_printf(
		    "Try to set the same value for WARN SYNC TIMER again");
...
void tet_mbcsv_data_request(int choice)
{
	...
	case 1:
		tet_printf(
		    "Data Req Process: Standby Requesting the Missing DATA");
		mbcstm_cb.svces[1].ssns[1].ws_timer = 1000;
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
			       NCS_MBCSV_OBJ_TMR_WSYNC);
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			...
			if (mbcstm_cb.sys == MBCSTM_SVC_INS2) {
				mbcstm_cb.svces[1].ssns[1].cb_test =
				    MBCSTM_CB_NO_TEST;
				mbcstm_sync_point();
				mbcstm_cb.svces[1].ssns[1].data_req =
				    1; /*dont change*/
				mbcstm_cb.svces[1].ssns[1].data_req_count =
				    DATA_COUNT; /*data count*/
				sleep(2);
				if (mbcstm_svc_data_request(1, 1) !=
				    NCSCC_RC_SUCCESS)
					FAIL = 1;
			} else
				mbcstm_sync_point();
	...
	case 2:
		tet_printf(
		    "Data Req Process: Quiescing Requesting the Missing DATA");
		mbcstm_cb.svces[1].ssns[1].ws_timer = 1000;
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
			       NCS_MBCSV_OBJ_TMR_WSYNC);
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
          ...
          if(mbcstm_cb.sys == MBCSTM_SVC_INS2)
			...
			if (mbcstm_cb.sys == MBCSTM_SVC_INS2) {
				...
				mbcstm_cb.svces[1].ssns[1].csi_role =
				    SA_AMF_HA_QUIESCING;
				if (mbcstm_ssn_set_role(1, 1) !=
				    NCSCC_RC_SUCCESS) {
					perror(
					    "Standby to Active role change Failed");
					FAIL = 1;
				}
				sleep(2);
				mbcstm_sync_point();
	...
	case 3:
		tet_printf(
		    "Standby Requesting the Missing DATA with Invalid Mbcsv Hdl");
		mbcstm_cb.svces[1].ssns[1].ws_timer = 1000;
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
			       NCS_MBCSV_OBJ_TMR_WSYNC);
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			...
			if (mbcstm_cb.sys == MBCSTM_SVC_INS2) {
				...
				*/
				...
				mbcsv_arg.i_mbcsv_hdl =
				    (NCS_MBCSV_HDL)(long)NULL; /*Wrong Value*/

				uba = &mbcsv_arg.info.send_data_req.i_uba;
				if (NCSCC_RC_SUCCESS !=
				    ncs_enc_init_space(uba)) {
					mbcstm_print(__FILE__, fun_name,
						     __LINE__, "UBA_INIT_SPACE",
						     NCSCC_RC_FAILURE);
					FAIL = 1;
				}
				...
				mbcsv_arg.info.send_data_req.i_ckpt_hdl =
				    ssn->ckpt_hdl;
				if (NCSCC_RC_SUCCESS !=
				    ncs_mbcsv_svc(&mbcsv_arg)) {
					mbcstm_print(__FILE__, fun_name,
						     __LINE__, "DATA_REQUEST",
						     NCSCC_RC_FAILURE);
	...
	case 4:
		tet_printf(
		    "Standby Requesting the Missing DATA with Invalid Checkpoint Hdl");
		mbcstm_cb.svces[1].ssns[1].ws_timer = 1000;
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
			       NCS_MBCSV_OBJ_TMR_WSYNC);
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
...
void tet_mbcsv_send_checkpoint(int choice)
{
	...
	*/
	...
	if (mbcstm_ssn_open(1, 1) != NCSCC_RC_SUCCESS)
		FAIL = 1;
	memset(mbcstm_cb.svces[1].ssns[1].data, '\0', sizeof(MBCSTM_CSI_DATA));
	mbcstm_sync_point();
	...
	case 1:
		...
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			...
			mbcstm_create_data_point(1, 1);
			mbcstm_svc_cp_send(
			    1, 1, NCS_MBCSV_ACT_ADD, NORMAL_DATA,
	...
	case 7:
		...
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
			       NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF);
		tet_printf(
		    "Sync Send, USR Async, MBC Async: Update Check Point");
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			mbcstm_cb.svces[1].ssns[1].csi_role = SA_AMF_HA_ACTIVE;
			if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
				perror("Standby to Active role change Failed");
			...
			mbcstm_create_data_point(1, 1);
			mbcstm_svc_cp_send(
			    1, 1, NCS_MBCSV_ACT_UPDATE, NORMAL_DATA,
			    mbcstm_cb.svces[1].ssns[1].data_count,
			    NCS_MBCSV_SND_SYNC);
			sleep(1);
			mbcstm_create_data_point(1, 1);
	...
	case 8:
		...
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			...
			mbcstm_create_data_point(1, 1);
			mbcstm_svc_cp_send(
			    1, 1, NCS_MBCSV_ACT_DONT_CARE, NORMAL_DATA,
			    mbcstm_cb.svces[1].ssns[1].data_count,
			    NCS_MBCSV_SND_SYNC);
			sleep(1);
			mbcstm_create_data_point(1, 1);
	...
	case 10:
		...
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
			       NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF);
		tet_printf(
		    "Not able to checkpoint with invalid Checkpoint Handle");
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			mbcstm_cb.svces[1].ssns[1].csi_role = SA_AMF_HA_ACTIVE;
			if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
				perror("Standby to Active role change Failed");
	...
	case 14:
		...
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
			       NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF);
		tet_printf(
		    "No Standby Available: Active trying to Send Check Point Data");
		mbcstm_cb.svces[1].ssns[1].csi_role = SA_AMF_HA_ACTIVE;
		if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
			perror("Standby to Active role change Failed");
			FAIL = 1;
	...
	case 15:
		...
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			...
			if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
				perror(
				    "Standby to Quiescing role change Failed");
				FAIL = 1;
			}
			mbcstm_sync_point();
	...
	case 16:
		...
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
			       NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF);
		tet_printf(
		    "Client in QUIESCED: Not able to Sync Send: Add Check  point");
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			mbcstm_cb.svces[1].ssns[1].csi_role = SA_AMF_HA_ACTIVE;
			if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
				perror("Standby to Active role change Failed");
			...
			if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
				perror(
				    "Standby to Quiesced role change Failed");
				FAIL = 1;
			}
			mbcstm_sync_point();
			mbcstm_create_data_point(1, 1);
	...
	case 17:
		...
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
			       NCS_MBCSV_OBJ_WARM_SYNC_ON_OFF);
		tet_printf(
		    "Reciever Client in QUIESCING: able to get Sync Send Check  point");
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			mbcstm_cb.svces[1].ssns[1].csi_role = SA_AMF_HA_ACTIVE;
			if (mbcstm_ssn_set_role(1, 1) != NCSCC_RC_SUCCESS) {
				perror("Standby to Active role change Failed");
...
void tet_mbcsv_test()
{
	...
	*/
	...
	case 2:
		...
		break;
	case 4:
		/*Intially Active to Quiesced, a new Active came: print inv ar
		 * Quiesced*/
		tet_printf("Send Notify message to STANDBY from ACTIVE");
		mbcstm_cb.svces[1].ssns[1].ws_flag = 0;
		mbcstm_svc_obj(1, 1, NCS_MBCSV_OP_OBJ_SET,
		...
		if (mbcstm_cb.sys == MBCSTM_SVC_INS1) {
			...
			fflush(stdout);
			/*          mbcstm_cb.svces[1].ssns[1].csi_role =
			   SA_AMF_HA_STANDBY; if(mbcstm_ssn_set_role(1,1) !=
			   NCSCC_RC_SUCCESS)
				{
				perror("Standby to Active role change Failed");
				FAIL=1;
...
count  = %d\n",
		...
		mbcstm_cb.svces[mbcstm_cb.svc_count]
		    .ssns[mbcstm_cb.vdest_count]
		    .pwe_hdl = vda_info.info.vdest_create.o_mds_pwe1_hdl;
...
uint32_t tet_mbcsv_dest_start(void)
{
	...
	for (ssn_index = 1; ssn_index <= ssn_count; ssn_index++) {
		MBCSTM_SET_VDEST_ID_IN_MDS_DEST(
		    dest, mbcstm_cb.vdests[ssn_index].dest_id);
		if (create_vdest(dest) != NCSCC_RC_SUCCESS) {
			printf(
			    "In tet_mbcsv_dest_start function.. VDEST creation failed\n");
			return NCSCC_RC_FAILURE;
		}
...
uint32_t tet_mbcsv_dest_close(void)
{
	...
	for (ssn_index = 1; ssn_index <= ssn_count; ssn_index++) {
		...
		if (destroy_vdest(dest) != NCSCC_RC_SUCCESS) {
			printf(
			    "In tet_mbcsv_dest_start function.. VDEST Destroy failed\n");
			return NCSCC_RC_FAILURE;
		}
		mbcstm_cb.vdests[ssn_index].status = false;
		for (svc_index = 1; svc_index <= mbcstm_cb.svc_count;
		     svc_index++) {
