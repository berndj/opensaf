...
..............................................................................
  ...
  receiving the PEER message MBCA will post this message to correspondig
service's mailbox. On dispatch, following functions will get called to process
the messages received from the peer.
...
PEER_INST *mbcsv_add_new_peer(CKPT_INST *ckpt, MBCSV_ANCHOR anchor)
{
	...
	/* Setup timers */
	m_INIT_NCS_MBCSV_TMR(new_peer, NCS_MBCSV_TMR_SEND_COLD_SYNC,
			     NCS_MBCSV_TMR_SEND_COLD_SYNC_PERIOD);

	m_INIT_NCS_MBCSV_TMR(new_peer, NCS_MBCSV_TMR_SEND_WARM_SYNC,
			     ckpt->warm_sync_time);

	m_INIT_NCS_MBCSV_TMR(new_peer, NCS_MBCSV_TMR_COLD_SYNC_CMPLT,
			     NCS_MBCSV_TMR_COLD_SYNC_CMPLT_PERIOD);

	m_INIT_NCS_MBCSV_TMR(new_peer, NCS_MBCSV_TMR_WARM_SYNC_CMPLT,
			     NCS_MBCSV_TMR_WARM_SYNC_CMPLT_PERIOD);

	m_INIT_NCS_MBCSV_TMR(new_peer, NCS_MBCSV_TMR_DATA_RESP_CMPLT,
			     NCS_MBCSV_TMR_DATA_RESP_CMPLT_PERIOD);

	m_INIT_NCS_MBCSV_TMR(new_peer, NCS_MBCSV_TMR_TRANSMIT,
			     NCS_MBCSV_TMR_TRANSMIT_PERIOD);
...
uint32_t mbcsv_shutdown_peer(PEER_INST *peer_ptr)
{
	...
	TRACE_ENTER2(
	    "stop peer timers, close session for peer anchor = %" PRIu64,
	    peer_ptr->peer_anchor);
	...
	if (SA_AMF_HA_ACTIVE == peer_ptr->my_ckpt_inst->my_role) {
		if (true == peer_ptr->data_resp_process) {
			TRACE(
			    "I'm ACTIVE and was in the middle of data response, \
							 hence invoking error indication calback");
			m_MBCSV_INDICATE_ERROR(
			    peer_ptr, peer_ptr->my_ckpt_inst->client_hdl,
			    NCS_MBCSV_DATA_RESP_TERMINATED, false,
			    peer_ptr->version, peer_ptr->req_context, rc);
		} else if (true == peer_ptr->c_syn_resp_process) {
			TRACE(
			    "I'm ACTIVE and was in the middle of cold sync response, \
							hence invoking error indication calback");
			m_MBCSV_INDICATE_ERROR(
			    peer_ptr, peer_ptr->my_ckpt_inst->client_hdl,
			...
			TRACE(
			    "I'm ACTIVE and was in the middle of warm sync response, \
							hence invoking error indication calback");
			m_MBCSV_INDICATE_ERROR(
			    peer_ptr, peer_ptr->my_ckpt_inst->client_hdl,
	...
	if (NCSCC_RC_SUCCESS != rc)
		TRACE_2(
		    "Error Indication callback returned failure for service Id = %u",
					      ...
					      MBCSV_REG *mbc_reg)
{
	...
	if (NULL != (ckpt = (CKPT_INST *)ncs_patricia_tree_get(
			 &mbc_reg->ckpt_ssn_list,
			 (const uint8_t *)&msg->rcvr_peer_key.pwe_hdl))) {
		switch (msg->info.peer_msg.info.peer_disc.msg_sub_type) {
		case MBCSV_PEER_UP_MSG:
			TRACE("peer version: %hu",
			      msg->info.peer_msg.info.peer_disc.info.peer_up
				  .peer_version);
		...
		case MBCSV_PEER_INFO_MSG:
			TRACE("peer version: %hu",
			      msg->info.peer_msg.info.peer_disc.info.peer_info
				  .peer_version);
		...
		case MBCSV_PEER_INFO_RSP_MSG:
			TRACE("peer version: %hu",
			      msg->info.peer_msg.info.peer_disc.info
				  .peer_info_rsp.peer_version);
			mbcsv_process_peer_info_rsp(msg, ckpt);
			break;

		case MBCSV_PEER_CHG_ROLE_MSG:
			TRACE("eer version: %hu",
			      msg->info.peer_msg.info.peer_disc.info
				  .peer_chg_role.peer_version);
		...
		TRACE_1(
		    "%s, My role: %u, My svc_id: %u, pwe handle:%u, peer role:%u, peer_anchor: %" PRIu64,
		    disc_trace[msg->info.peer_msg.info.peer_disc.msg_sub_type],
		    ckpt->my_role, ckpt->my_mbcsv_inst->svc_id, ckpt->pwe_hdl,
		    msg->info.peer_msg.info.peer_disc.peer_role,
		    msg->rcvr_peer_key.peer_anchor);
	} else {
		TRACE_LEAVE2("Unable to find checkpoint for svc_id = %u",
			     mbc_reg->svc_id);
		return NCSCC_RC_FAILURE;
	}
...
*
*  DESCRIPTION:       This function sets the first state of the peer session
FSM. *                     Then depending on its current role it will start FSM.
*
*  RETURNS:           NONE
*
\*****************************************************************************/
...
void mbcsv_set_up_new_session(CKPT_INST *ckpt, PEER_INST *new_act_peer)
{
	...
	if (new_act_peer->incompatible)
		...
		m_SET_NCS_MBCSV_STATE(new_act_peer,
				      NCS_MBCSV_STBY_STATE_STEADY_IN_SYNC);
		...
		m_MBCSV_SEND_CLIENT_MSG(new_act_peer,
					NCSMBCSV_SEND_WARM_SYNC_REQ,
					NCS_MBCSV_ACT_DONT_CARE);

		new_act_peer->warm_sync_sent = true;

		/* When we get a warm sync response - this timer will be
		 * cancelled */
		ncs_mbcsv_start_timer(new_act_peer,
		...
		m_MBCSV_SEND_CLIENT_MSG(new_act_peer,
					NCSMBCSV_SEND_COLD_SYNC_REQ,
					NCS_MBCSV_ACT_DONT_CARE);

		/* When we get a cold sync response - this timer will be
		 * cancelled */
		ncs_mbcsv_start_timer(new_act_peer,
...
void mbcsv_set_peer_state(CKPT_INST *ckpt, PEER_INST *peer, bool peer_up)
{
	...
	case SA_AMF_HA_ACTIVE: /* ckpt->my_role */
	{
		...
		case SA_AMF_HA_ACTIVE: /*: peer->peer_role */
		{
			...
			m_NCS_MBCSV_FSM_DISPATCH(
			    peer, NCSMBCSV_EVENT_MULTIPLE_ACTIVE, NULL);
		} break;

		/*
		 * If new peer is standby then set the state of new peer to wait
		 * for cold sync if peer is compatible else set to IDLE state.
		 */
		case SA_AMF_HA_STANDBY: /* : peer->peer_role */
		case SA_AMF_HA_QUIESCED: {
			if ((NULL != ckpt->active_peer) &&
			    (peer->peer_anchor ==
			     ckpt->active_peer->peer_anchor)) {
				mbcsv_clear_multiple_active_state(ckpt);
			} else {
				if (peer->incompatible)
					m_SET_NCS_MBCSV_STATE(
					    peer, NCS_MBCSV_ACT_STATE_IDLE);
				else {
					if (peer->cold_sync_done)
						m_SET_NCS_MBCSV_STATE(
						    peer,
	...
	case SA_AMF_HA_QUIESCED: /* ckpt->my_role */
	{
		...
		switch (peer->peer_role) {
		/* If new peer's role is ACTIVE then establish session with this
		   new peer and close old session */
		case SA_AMF_HA_ACTIVE: /* peer->peer_role */
		{
			TRACE(
			    "peer is ACTIVE, clear old session and start new session");
		...
		case SA_AMF_HA_QUIESCED: /* peer->peer_role */
		{
			if ((NULL != ckpt->active_peer) &&
			    (peer->peer_anchor ==
			     ckpt->active_peer->peer_anchor)) {
				/* Our Active peer is changing state, close its
				 * session */
				mbcsv_close_old_session(ckpt->active_peer);

				/* Check if any other peer is ACTIVE if yes then
				 * set-up session with this new peer. */
				if (NULL !=
				    (peer_ptr = mbcsv_my_active_peer(ckpt))) {
					ckpt->active_peer = peer_ptr;
					mbcsv_set_up_new_session(ckpt,
								 peer_ptr);
				} else
					ckpt->active_peer = NULL;
			} else
				m_SET_NCS_MBCSV_STATE(
				    peer, NCS_MBCSV_STBY_STATE_IDLE);
			TRACE_LEAVE();
			return;
		} break;
 ...
 *                     1) Check whether this peer already exist. If YES then
 *drop this message. Else add this peer in peer list. 2) Send back PEER_INFO
 *message to the peer from which PEER_UP is received. 3)
 *Call PEER_INFO call back which will validate the peer
 *version. 4) Initiate a role based actions to set-up a session.
 *
				    ...
				    uint8_t peer_up)
{
	...
	/*
	 * Check If this peer up message is not mine. If it my peer up I don't
	 * need to process this mesage. Also check that my role is set. If no
	 * then drop this request and wait for client to set my role.
	 */
	if ((ckpt->my_anchor == msg->rcvr_peer_key.peer_anchor) ||
	    (ckpt->role_set == false)) {
		TRACE_LEAVE2("peer up message is for self");
		return NCSCC_RC_SUCCESS;
	}

	if (NULL != (peer = mbcsv_search_and_return_peer(
			 ckpt->peer_list, msg->rcvr_peer_key.peer_anchor))) {
		if (peer_up) {
			mbcsv_send_peer_disc_msg(
			    MBCSV_PEER_INFO_MSG, ckpt->my_mbcsv_inst, ckpt,
			    peer, MDS_SENDTYPE_RED,
			    msg->rcvr_peer_key.peer_anchor);
			TRACE_LEAVE();
			return NCSCC_RC_SUCCESS;
		} else {

			peer->version = msg->info.peer_msg.info.peer_disc.info
					    .peer_info.peer_version;
	...
	if (0 != (mbx = mbcsv_get_mbx(msg->rcvr_peer_key.pwe_hdl,
		...
		if (NULL == ncs_patricia_tree_get(&mbcsv_cb.peer_list,
						  (const uint8_t *)&key)) {
	...
	if (NULL ==
	    (peer = mbcsv_add_new_peer(ckpt, msg->rcvr_peer_key.peer_anchor))) {
	...
	if (peer_up)
		peer->version =
		    msg->info.peer_msg.info.peer_disc.info.peer_up.peer_version;
	else
		peer->version = msg->info.peer_msg.info.peer_disc.info.peer_info
				    .peer_version;
	 ...
	 * compatible or not.
	 */
	m_MBCSV_PEER_INFO(peer, ckpt->client_hdl, ckpt->my_mbcsv_inst->svc_id,
			  peer->version, peer->incompatible);
	...
	if (peer_up) {
		...
		mbcsv_send_peer_disc_msg(
		    MBCSV_PEER_INFO_MSG, ckpt->my_mbcsv_inst, ckpt, peer,
		    MDS_SENDTYPE_RED, msg->rcvr_peer_key.peer_anchor);
	} else {
		mbcsv_update_peer_info(msg, ckpt, peer);
	}
...
{
	peer->peer_hdl =
	    msg->info.peer_msg.info.peer_disc.info.peer_info.my_peer_inst_hdl;

	/* In.... */
	peer->incompatible |=
	    msg->info.peer_msg.info.peer_disc.info.peer_info.compatible;

	mbcsv_send_peer_disc_msg(MBCSV_PEER_INFO_RSP_MSG, ckpt->my_mbcsv_inst,
				 ckpt, peer, MDS_SENDTYPE_RED,
				 msg->rcvr_peer_key.peer_anchor);
 ...
 *
 *  DESCRIPTION:       This function processes the PEER_DOWN message received
 *from its peer client.
 *
...
uint32_t mbcsv_process_peer_down(MBCSV_EVT *msg, CKPT_INST *ckpt)
{
	...
	case SA_AMF_HA_QUIESCED: {
		if (role == SA_AMF_HA_ACTIVE) {
			...
			if (act_peer) {
				/* Our Active peer is going down, close its
				 * session */
				ckpt->active_peer = NULL;

				/* Check if anyother peer is ACTIVE if yes then
				 * set-up session with this new peer. */
				if (NULL !=
				    (peer_ptr = mbcsv_my_active_peer(ckpt))) {
					ckpt->active_peer = peer_ptr;
 ...
 *
 *  DESCRIPTION:       This function processes the PEER_INFO_RSP message
 *received from its peer client.
 *
 *  ACTION:            Actions to be taken on receiving PEER_INFO_RSP are:
 *                     1) We are going to receive the peer handle in this
 *message which we will be sending along with all the client messages. so store
 *this handle with you for future refrences.
 *
 *  RETURNS:           SUCCESS - All went well
...
uint32_t mbcsv_process_peer_info_rsp(MBCSV_EVT *msg, CKPT_INST *ckpt)
{
	...
	if (NULL == (peer = mbcsv_search_and_return_peer(
			 ckpt->peer_list, msg->rcvr_peer_key.peer_anchor))) {
		TRACE_LEAVE2("peer does not exist, svc_id: %u",
			     ckpt->my_mbcsv_inst->svc_id);
		return NCSCC_RC_FAILURE;
	}

	peer->peer_hdl = msg->info.peer_msg.info.peer_disc.info.peer_info_rsp
			     .my_peer_inst_hdl;
	peer->incompatible =
	    msg->info.peer_msg.info.peer_disc.info.peer_info_rsp.compatible;
	peer->peer_role = msg->info.peer_msg.info.peer_disc.peer_role;
 ...
 *
 *  DESCRIPTION:       This function processes the PEER_CHG_ROLE message
 *received from its peer client.
 *
...
*
*  DESCRIPTION:       This function composes and then sends the peer disco
message *                     to the intended peer.
*
*  RETURNS:           SUCCESS - All went well
*                     FAILURE - fail to process PEER_UP.
*
				  ...
				  uint32_t mds_send_type, MBCSV_ANCHOR anchor)
{
	...
	case MBCSV_PEER_INFO_MSG:
		TRACE("peer info msg");
		evt.info.peer_msg.info.peer_disc.info.peer_info.peer_version =
		    mbc->version;
		evt.info.peer_msg.info.peer_disc.info.peer_info.compatible =
		    peer->incompatible;
		evt.info.peer_msg.info.peer_disc.info.peer_info
		    .my_peer_inst_hdl = peer->hdl;
		break;

	case MBCSV_PEER_INFO_RSP_MSG:
		TRACE("peer info response msg");
		evt.info.peer_msg.info.peer_disc.info.peer_info_rsp
		    .peer_version = mbc->version;
		evt.info.peer_msg.info.peer_disc.info.peer_info_rsp.compatible =
		    peer->incompatible;
		evt.info.peer_msg.info.peer_disc.info.peer_info_rsp
		    .my_peer_inst_hdl = peer->hdl;
		break;
	...
	case MBCSV_PEER_UP_MSG:
		TRACE("peer up msg");
		evt.info.peer_msg.info.peer_disc.info.peer_up.peer_version =
		    mbc->version;
		break;

	case MBCSV_PEER_CHG_ROLE_MSG:
		TRACE("change role msg");
		evt.info.peer_msg.info.peer_disc.info.peer_chg_role
		    .peer_version = mbc->version;
		break;
	...
	default:
		TRACE_LEAVE2(
		    "Incorrect msg type received in peer discover message");
		return NCSCC_RC_FAILURE;
	}
