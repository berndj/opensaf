...
..............................................................................
  ...
  FUNCTIONS INCLUDED in this module:
   ...
   ncs_mbcsv_svc                    - MBCSV API, processes different MBCSV
client requests. mbcsv_process_initialize_request - Process Initialize request.
...
static const MBCSV_PROCESS_REQ_FUNC_PTR
	...
	mbcsv_process_initialize_request, mbcsv_process_get_sel_obj_request,
	mbcsv_process_dispatch_request,   mbcsv_process_finalize_request,
	mbcsv_process_open_request,       mbcsv_process_close_request,
	mbcsv_process_chg_role_request,   mbcsv_process_snd_ckpt_request,
	mbcsv_process_snd_ntfy_request,   mbcsv_process_snd_data_req,
	mbcsv_process_get_request,	mbcsv_process_set_request};
 ...
 * ========================================================
 * COLD_SYNC_REQUEST		: STANDBY sends to ACTIVE requesting
 *first/initial sync of whole DB. COLD_SYNC_RESPONSE		: ACTIVE
 *responds to STANDBY as 1 message or multiple response messages.
 ...
 *
 * WARM_SYNC_REQUEST		: STANDBY sends to ACTIVE periodically
 *requesting sync status. WARM_SYNC_RESPONSE		: ACTIVE responds to
 *STANDBY as 1 message or multiple response messages.
 ...
 *  REQUEST TYPES :
 *
 *                     NCS_MBCSV_OP_INITIALIZE - put an MBCSv instance in start
 *state NCS_MBCSV_OP_FINALIZE   - Recover MBCSv instance resources
 *                     NCS_MBCSV_OP_SELECT     -  Get SELECT obj to coordinate
 *Dispatch NCS_MBCSV_OP_DISPATCH   - Service any pending Callbacks
 *                     NCS_MBCSV_OP_OPEN       -  Initiate contact with peer
 *entity NCS_MBCSV_OP_CLOSE      - Dis-engage from peer entity relation
 *                     NCS_MBCSV_OP_CHG_ROLE   - Change client's HA
 *state-machine NCS_MBCSV_OP_SEND_CKPT  - Send a Checkpoint to my peer entity
 *                     NCS_MBCSV_OP_SEND_NOTIFY - Send a free form msg to my
 *peer entity NCS_MBCSV_OP_SUB_ONESHOT - Subscribe for ONE occurrence of event
 *                     NCS_MBCSV_OP_SUB_PERSIST - Set up a persistent
 *subscription NCS_MBCSV_OP_SUB_CANCEL  - cancel a subscription
...
uint32_t mbcsv_process_initialize_request(NCS_MBCSV_ARG *arg)
{
	...
	TRACE_ENTER2("Register and obtain an MBCSV handle for svc_id: %u",
		     svc_id);
	...
	/*
	 * Walk through MBCSv reg list and find whether service is already
	 * registered with MBCSv. If yes then return error.
	 */
	if (NULL != (MBCSV_REG *)ncs_patricia_tree_get(
	 ...
	 */
	if (NULL ==
	    (new_reg->mbcsv_cb_func = arg->info.initialize.i_mbcsv_cb)) {
		TRACE_2("Null callback function");
		rc = SA_AIS_ERR_INVALID_PARAM;
		goto err2;
	}

	/* Version and clinet handle information is stored as it is witout
	 * validating */
	...
	/*
	 * Create patricia tree for the checkpoint instance  Handle used for
	 * this tree is the PWE handle.
	 */
	pt_params.key_size = sizeof(uint32_t);

	if (ncs_patricia_tree_init(&new_reg->ckpt_ssn_list, &pt_params) !=
	    NCSCC_RC_SUCCESS) {
	...
	/*
	 * Create Handle manager handle which will later be passed back to
	 * caller
	 */
	 ...
	 */
	if (NCSCC_RC_SUCCESS !=
	    ncs_patricia_tree_add(&mbcsv_cb.reg_list,
...
*           removes the service registration instance from the MBCSv
registration *           list. Clear all the memory allocated for this service
specific *           structure. Clears the mailbox of the service.
*
*
* Input:    arg -  is of type NCS_MBCSV_ARG.
*
...
uint32_t mbcsv_process_get_sel_obj_request(NCS_MBCSV_ARG *arg)
{
	...
	arg->info.sel_obj_get.o_select_obj =
	    (SaSelectionObjectT)m_GET_FD_FROM_SEL_OBJ(
		m_NCS_IPC_GET_SEL_OBJ(&mbc_reg->mbx));
...
*           Open registers the checkpoint instance with the MBCSv. On
registering *           it will advertise the its presence to all its peers.
*
*
* Input:    arg -  is of type NCS_MBCSV_ARG.
*
...
uint32_t mbcsv_process_open_request(NCS_MBCSV_ARG *arg)
{
	...
	/*
	 * Find the service registration instance using the mbc_hdl passed by
	 * the application.
	 */
	if (NULL ==
	    (mbc_reg = (MBCSV_REG *)m_MBCSV_TAKE_HANDLE(arg->i_mbcsv_hdl))) {
	...
	/*
	 * Search checkpoint list for the ckpt instance. If ckpt instance
	 * already exist then the parameter passed are invalid and so reject
	 * request.
	 */
	if (NULL != (CKPT_INST *)ncs_patricia_tree_get(
			&mbc_reg->ckpt_ssn_list, (const uint8_t *)&pwe_hdl)) {
	...
	new_ckpt->fsm =
	    (NCS_MBCSV_STATE_ACTION_FUNC_PTR *)ncsmbcsv_init_state_tbl;
	 ...
	 */
	if (NCSCC_RC_SUCCESS !=
	    ncs_patricia_tree_add(&mbc_reg->ckpt_ssn_list,
	 ...
	 */
	if (NCSCC_RC_SUCCESS !=
	    mbcsv_add_new_mbx(pwe_hdl, mbc_reg->svc_id, mbc_reg->mbx)) {
		TRACE_2("reg list add failed");
		rc = SA_AIS_ERR_FAILED_OPERATION;

		ncs_patricia_tree_del(&mbc_reg->ckpt_ssn_list,
				      (NCS_PATRICIA_NODE *)new_ckpt);
	...
	/*
	 * So we are successful in open call. Return success and send back
	 * checkpoint handle to caller.
	 */
...
*           peers that this checkpoint instance is no longer valid. MBCSv
removes *           the peer information of this checkpoint instance.
*
* Input:    arg -  is of type NCS_MBCSV_ARG.
*
* Returns:  NCSCC_RC_SUCCESSS/NCSCC_RC_FAILURE
...
uint32_t mbcsv_process_close_request(NCS_MBCSV_ARG *arg)
{
	...
	/*
	 * Search for the ckpt instance in the list. If it does not exist then
	 * return error since we don't have anything to close.
	 */
	if (NULL == (ckpt_inst = (CKPT_INST *)m_MBCSV_TAKE_HANDLE(
			 arg->info.close.i_ckpt_hdl))) {
...
uint32_t mbcsv_process_chg_role_request(NCS_MBCSV_ARG *arg)
{
	...
	if ((ckpt_inst->ftm_role_set == false) &&
	    ((arg->info.chg_role.i_ha_state != SA_AMF_HA_ACTIVE) &&
	     (arg->info.chg_role.i_ha_state != SA_AMF_HA_STANDBY))) {
	...
	if (((SA_AMF_HA_ACTIVE == ckpt_inst->my_role) &&
	    ...
	    ((SA_AMF_HA_STANDBY == ckpt_inst->my_role) &&
	     (SA_AMF_HA_QUIESCED == arg->info.chg_role.i_ha_state))) {
	...
	evt->info.peer_msg.info.chg_role.ckpt_hdl =
	    arg->info.chg_role.i_ckpt_hdl;
	evt->info.peer_msg.info.chg_role.new_role =
	    arg->info.chg_role.i_ha_state;

	if (NCSCC_RC_SUCCESS !=
	    m_MBCSV_SND_MSG(&mbc_reg->mbx, evt, NCS_IPC_PRIORITY_NORMAL)) {
...
uint32_t mbcsv_process_snd_ckpt_request(NCS_MBCSV_ARG *arg)
{
	...
	case NCS_MBCSV_SND_USR_ASYNC: {
		mbcsv_send_ckpt_data_to_all_peers(&arg->info.send_ckpt,
						  ckpt_inst, mbc_reg);
	} break;

	case NCS_MBCSV_SND_MBC_ASYNC: {
		TRACE(
		    "send type specified is NCS_MBCSV_SND_MBC_ASYNC: posting to mail box");
		/*
		...
		/* Indicates to check the peer existance while processing this
		 * event */
		evt->info.peer_msg.info.usr_msg_info.io_no_peer = check_peer;

		if (NCSCC_RC_SUCCESS !=
		    m_MBCSV_SND_MSG(&mbc_reg->mbx, evt,
...
uint32_t mbcsv_process_snd_ntfy_request(NCS_MBCSV_ARG *arg)
{
	 ...
	 */
	if (NULL == (ckpt_inst = (CKPT_INST *)m_MBCSV_TAKE_HANDLE(
			 arg->info.send_notify.i_ckpt_hdl))) {
		TRACE_2("checkpoint instance does not exist");
		rc = SA_AIS_ERR_BAD_HANDLE;
		goto err1;
	}
	...
	if (NCSCC_RC_SUCCESS !=
	    mbcsv_send_notify_msg(arg->info.send_notify.i_msg_dest, ckpt_inst,
				  mbc_reg, arg->info.send_notify.i_msg)) {
...
uint32_t mbcsv_process_snd_data_req(NCS_MBCSV_ARG *arg)
{
	 ...
	 */
	if (NULL == (ckpt_inst = (CKPT_INST *)m_MBCSV_TAKE_HANDLE(
			 arg->info.send_data_req.i_ckpt_hdl))) {
		TRACE_2("checkpoint instance does not exist");
		rc = SA_AIS_ERR_BAD_HANDLE;
		goto err1;
	}
	 ...
	 */
	if ((SA_AMF_HA_STANDBY != ckpt_inst->my_role) &&
	    (SA_AMF_HA_QUIESCED != ckpt_inst->my_role)) {
	...
	if (NCSCC_RC_SUCCESS !=
	    mbcsv_send_data_req(&arg->info.send_data_req.i_uba, ckpt_inst,
				mbc_reg)) {
...
uint32_t mbcsv_process_set_request(NCS_MBCSV_ARG *arg)
{
			...
			if (ckpt_inst->my_role == SA_AMF_HA_STANDBY) {
				for (peer_ptr = ckpt_inst->peer_list;
					...
					if ((ckpt_inst->warm_sync_on == true) &&
						...
						TRACE(
						    "turned on warmsync , started warm sync timer");
					} else {
						ncs_mbcsv_stop_timer(
						    peer_ptr,
						    NCS_MBCSV_TMR_SEND_WARM_SYNC);
						TRACE(
						    "stopped warm sync timer");
	...
	case NCS_MBCSV_OBJ_TMR_WSYNC: {
		 ...
		 */
		if ((arg->info.obj_set.i_val <
		     NCS_MBCSV_MIN_SEND_WARM_SYNC_TIME) ||
		    (arg->info.obj_set.i_val >
		     NCS_MBCSV_MAX_SEND_WARM_SYNC_TIME)) {
		...
		if (ckpt_inst->warm_sync_time != arg->info.obj_set.i_val) {
			...
			for (peer_ptr = ckpt_inst->peer_list; peer_ptr != NULL;
			     peer_ptr = peer_ptr->next) {
				peer_ptr->tmr[NCS_MBCSV_TMR_SEND_WARM_SYNC]
				    .period = ckpt_inst->warm_sync_time;
			}
		}
	} break;
