...
static const char *tmr_evt_str[] = {"MBCSV_TMR_SEND_COLD_SYNC",
...
uint32_t mbcsv_process_events(MBCSV_EVT *rcvd_evt, uint32_t mbcsv_hdl)
{
	...
	case MBCSV_EVT_MDS_SUBSCR: {
		...
		case NCSMDS_RED_UP: {
			TRACE("RED_UP event");
			/*
			 * Send PEER_UP message to the peer anchor from which we
			 * have received UP event.
			 */
			if (NULL != (ckpt = (CKPT_INST *)ncs_patricia_tree_get(
					 &mbc_reg->ckpt_ssn_list,
					 (const uint8_t *)&rcvd_evt
					     ->rcvr_peer_key.pwe_hdl))) {
				mbcsv_send_peer_disc_msg(
				    MBCSV_PEER_UP_MSG, mbc_reg, ckpt, peer,
				    MDS_SENDTYPE_RED,
		...
		case NCSMDS_RED_DOWN: {
			...
			if (NULL != (ckpt = (CKPT_INST *)ncs_patricia_tree_get(
					 ...
					 (const uint8_t *)&rcvd_evt
					     ->rcvr_peer_key.pwe_hdl))) {
				/*
				 * Check if any of the peers of my ckpts lies on
				 * this vdest. If yes then remove that peer and
				 * close a session.
				 */
	...
	case MBCSV_EVT_TMR: {
		TRACE("Timer event");
		/*
		 * Send all these timer events to the state machine which will
		 * handle this event for us.
		 */
		if (NULL != (peer = (PEER_INST *)m_MBCSV_TAKE_HANDLE(
				 rcvd_evt->info.tmr_evt.peer_inst_hdl))) {
			hdl_to_give = rcvd_evt->info.tmr_evt.peer_inst_hdl;
			ckpt = peer->my_ckpt_inst;

			TRACE_1(
			    "myrole: %u, svc_id: %u, pwe_hdl: %u, peer_anchor: %" PRIu64
			    ", peer_state: %u, event type:%s",
			    ckpt->my_role, mbc_reg->svc_id, ckpt->pwe_hdl,
			    peer->peer_anchor, peer->state,
			    tmr_evt_str[rcvd_evt->info.tmr_evt.type]);

			m_NCS_MBCSV_FSM_DISPATCH(peer,
						 (NCSMBCSV_EVENT_TMR_MIN +
	...
	case MBCSV_EVT_INTERNAL: {
		...
		case MBCSV_EVT_INTERNAL_CLIENT: {
			...
			if (NULL != (peer = (PEER_INST *)m_MBCSV_TAKE_HANDLE(
					 (uint32_t)rcvd_evt->rcvr_peer_key
					     .peer_inst_hdl))) {
				hdl_to_give =
				    (uint32_t)
					rcvd_evt->rcvr_peer_key.peer_inst_hdl;
				ckpt = peer->my_ckpt_inst;
				m_NCS_MBCSV_FSM_DISPATCH(
				    peer,
				    rcvd_evt->info.peer_msg.info.client_msg.type
					.evt_type,
				    rcvd_evt);
		...
		case MBCSV_EVT_MBC_ASYNC_SEND: {
			...
			if (NULL == (ckpt = (CKPT_INST *)m_MBCSV_TAKE_HANDLE(
					 rcvd_evt->info.peer_msg.info
			...
			mbcsv_send_ckpt_data_to_all_peers(
			    &rcvd_evt->info.peer_msg.info.usr_msg_info, ckpt,
			    mbc_reg);

			m_MBCSV_GIVE_HANDLE(rcvd_evt->info.peer_msg.info
						.usr_msg_info.i_ckpt_hdl);
		} break;
