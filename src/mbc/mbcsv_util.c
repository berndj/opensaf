...
*           list. It frees all the checkpoint instances and free the all
allocated *           memory.
*
...
uint32_t mbcsv_rmv_reg_inst(MBCSV_REG *reg_list, MBCSV_REG *mbc_reg)
{
	...
	/*
	 * Find and remove all the ckpt instance. After removing all the
	 * instances destroy this ckpt list.
	 */
	while (NULL !=
	       (ckpt = (CKPT_INST *)ncs_patricia_tree_getnext(
	 ...
	 */
	if (NCSCC_RC_SUCCESS !=
	    ncs_patricia_tree_del((NCS_PATRICIA_TREE *)reg_list,
				  (NCS_PATRICIA_NODE *)mbc_reg)) {
...
uint32_t mbcsv_remove_ckpt_inst(CKPT_INST *ckpt)
{
	 ...
	 */
	if (NCSCC_RC_SUCCESS !=
	    mbcsv_rmv_entry(ckpt->pwe_hdl, ckpt->my_mbcsv_inst->svc_id)) {
	 ...
	 * of each ckpt instance. It also stops all the timers of the state
	 * machine. It completely free's all the resources curretnly being used
	 * by this MBC client registration instance.
	 */
	 ...
	 */
	if (NCSCC_RC_SUCCESS !=
	    ncs_patricia_tree_del(&ckpt->my_mbcsv_inst->ckpt_ssn_list,
				  (NCS_PATRICIA_NODE *)ckpt)) {
	...
	if (0 == mbcsv_get_next_entry_for_pwe(ckpt->pwe_hdl, &svc_id)) {
		...
		mbcsv_send_peer_disc_msg(MBCSV_PEER_DOWN_MSG,
					 ckpt->my_mbcsv_inst, ckpt, NULL,
					 MDS_SENDTYPE_RBCAST, 0);

	TRACE("The checkpoint instance belonging to pwe_hdl: %u is removed.",
	      ckpt->pwe_hdl);
...
uint32_t mbcsv_process_chg_role(MBCSV_EVT *rcvd_evt, MBCSV_REG *mbc_inst)
{
	 ...
	 */
	if (NULL == (ckpt = m_MBCSV_TAKE_HANDLE(
			 rcvd_evt->info.peer_msg.info.chg_role.ckpt_hdl))) {
	...
	case SA_AMF_HA_ACTIVE: {
		...
		ckpt->fsm = (NCS_MBCSV_STATE_ACTION_FUNC_PTR *)
		    ncsmbcsv_active_state_tbl;

		peer = ckpt->peer_list;
		/*
		 * We are first going to check whether some other ACTIVE peer is
		 * there? If yes then set the state machine of this ACTIVE to
		 * appropriate state.
		 */
		while (NULL != peer) {
			if (peer->incompatible)
				m_SET_NCS_MBCSV_STATE(peer,
						      NCS_MBCSV_ACT_STATE_IDLE);
			else if (SA_AMF_HA_ACTIVE == peer->peer_role) {
				TRACE_1(
				    "changing state to NCS_MBCSV_ACT_STATE_MULTIPLE_ACTIVE");
				m_SET_NCS_MBCSV_STATE(
				    peer, NCS_MBCSV_ACT_STATE_MULTIPLE_ACTIVE);
			} else if (peer->cold_sync_done) {
				TRACE_1(
				    "changing state to NCS_MBCSV_ACT_STATE_KEEP_STBY_IN_SYNC");
				m_SET_NCS_MBCSV_STATE(
				    peer,
				    NCS_MBCSV_ACT_STATE_KEEP_STBY_IN_SYNC);
			} else if (!peer->cold_sync_done) {
				TRACE_1(
				    "changing state to NCS_MBCSV_ACT_STATE_WAIT_FOR_COLD_WARM_SYNC");
				m_SET_NCS_MBCSV_STATE(
				    peer,
	...
	case SA_AMF_HA_QUIESCED: {
		...
		TRACE_1("changing role to %s",
			((ckpt->my_role == SA_AMF_HA_STANDBY) ? "STANDBY"
							      : "QUIESCED"));
		ckpt->in_quiescing = false;
		ckpt->fsm = (NCS_MBCSV_STATE_ACTION_FUNC_PTR *)
		    ncsmbcsv_standby_state_tbl;
		...
		while (NULL != peer) {
			if (old_role == SA_AMF_HA_ACTIVE) {
				TRACE_1(
				    "previous role was ACTIVE, closing old sessions with peer");
	...
	case SA_AMF_HA_QUIESCING: {
		/*
		 * Do Nothing. Just set the varibale in_quiescing which will
		 * tell us that the HA role was set to quiescing some time.
		 */
	...
	if (ckpt->peer_up_sent)
		mbcsv_send_peer_disc_msg(MBCSV_PEER_CHG_ROLE_MSG, mbc_inst,
					 ckpt, NULL, MDS_SENDTYPE_RBCAST, 0);
	else {
		ckpt->peer_up_sent = true;
		mbcsv_send_peer_disc_msg(MBCSV_PEER_UP_MSG, mbc_inst, ckpt,
					 NULL, MDS_SENDTYPE_RBCAST, 0);
	}
					   ...
					   MBCSV_REG *mbc_inst)
{
	...
	if (NULL == ckpt_inst->peer_list) {
		if (msg_to_send->io_no_peer == true) {
			...
			/* Application requested to let it know if the peer
			 * doesn't exist */
			parg.i_client_hdl = ckpt_inst->client_hdl;
			parg.i_ckpt_hdl = ckpt_inst->ckpt_hdl;
			parg.i_op = NCS_MBCSV_CBOP_ENC;
			parg.info.encode.io_msg_type =
			    NCS_MBCSV_MSG_ASYNC_UPDATE;
			...
			if (NCSCC_RC_SUCCESS !=
			    mbc_inst->mbcsv_cb_func(&parg)) {
				TRACE_LEAVE2(
				    "NCS_MBCSV_MSG_ASYNC_UPDATE Encode callback returned failure");
				return NCSCC_RC_FAILURE;
			}
		}
		TRACE_LEAVE();
	...
	while (NULL != peer_ptr) {
		/*
		 * Check whether we have to send the message to this peer. If
		 * not then go to next peer.
		 */
		...
		if (NCSCC_RC_SUCCESS != mbc_inst->mbcsv_cb_func(&parg)) {
			TRACE_LEAVE2(
			    "encode callback returned failure for ASYNC UPDATE");
			return NCSCC_RC_FAILURE;
		}

		/*
		 * Check whether we want to send this encoded message to any
		 * other peer with same software revision. If yes then send
		 * message to all of them. So this encoded checkpoint message
		 * will get sent to all the peers with same software revision.
		...
		evt_msg.info.peer_msg.info.client_msg.type.msg_sub_type =
		    NCS_MBCSV_MSG_ASYNC_UPDATE;
		evt_msg.info.peer_msg.info.client_msg.action =
		    msg_to_send->i_action;
		evt_msg.info.peer_msg.info.client_msg.reo_type =
		    msg_to_send->i_reo_type;
		evt_msg.info.peer_msg.info.client_msg.snd_type =
		    msg_to_send->i_send_type;

		TRACE(
		    "send the encoded message to any other peer with same s/w version");
			       ...
			       MBCSV_REG *mbc_inst, NCSCONTEXT i_msg)
{
	...
	evt_msg.rcvr_peer_key.svc_id = mbc_inst->svc_id;
	evt_msg.info.peer_msg.type = MBCSV_EVT_INTERNAL_CLIENT;
	evt_msg.info.peer_msg.info.client_msg.type.evt_type =
	    NCSMBCSV_EVENT_NOTIFY;
	...
	case NCS_MBCSV_ACTIVE: {
		...
		if (NULL != ckpt_inst->active_peer) {
			...
			m_NCS_MBCSV_FSM_DISPATCH(ckpt_inst->active_peer,
						 NCSMBCSV_SEND_NOTIFY,
						 &evt_msg);

			if (true == ckpt_inst->active_peer->okay_to_send_ntfy) {
				/*
				 * Call encode callback to encode message to be
				 * sent.
				 */
				...
				parg.info.notify.i_peer_version =
				    ckpt_inst->active_peer->version;
				...
				if (NCSCC_RC_SUCCESS !=
					...
					TRACE_LEAVE2(
					    "notify encode callback returned failure");
					return NCSCC_RC_FAILURE;
				}

				evt_msg.rcvr_peer_key.peer_inst_hdl =
				    ckpt_inst->active_peer->peer_hdl;
				evt_msg.info.peer_msg.info.client_msg.uba =
				    *uba;

				m_NCS_MBCSV_MDS_ASYNC_SEND(
				    &evt_msg, ckpt_inst,
				    ckpt_inst->active_peer->peer_anchor);
			}
		}
	} break;
	...
	case NCS_MBCSV_ALL_PEERS: {
		...
		while (peer_count) {
			/* If done with traversing thru peer_list, again set the
			   ptr to start of the peerlist to traverse for another
			   version of peers */
			if (peer == NULL) {
				tmp_peer_version = 0;
			...
			if ((msg_dest == NCS_MBCSV_STANDBY) &&
			    (SA_AMF_HA_STANDBY != peer->peer_role)) {
			...
			m_NCS_MBCSV_FSM_DISPATCH(peer, NCSMBCSV_SEND_NOTIFY,
						 &evt_msg);
			...
			if (true == peer->okay_to_send_ntfy) {
				if (set_peer_version) {
					...
					/*
					 * Call encode callback to encode
					 * message to be sent.
					 */
					uba = &parg.info.notify.i_uba;
					memset(uba, '\0', sizeof(NCS_UBAID));
				...
				if (tmp_peer_version == peer->version) {
					if (NULL == (dup_ub = m_MMGR_DITTO_BUFR(
							 uba->start))) {
						TRACE_LEAVE2(
			     ...
			     MBCSV_REG *mbc_inst)
{
	...
	m_NCS_MBCSV_FSM_DISPATCH(ckpt_inst->active_peer, NCSMBCSV_SEND_DATA_REQ,
				 &evt_msg);
...
uint32_t mbcsv_send_client_msg(PEER_INST *peer, uint8_t evt, uint32_t action)
{
	...
	if (NCSCC_RC_SUCCESS !=
	    m_MBCSV_SND_MSG(&peer->my_ckpt_inst->my_mbcsv_inst->mbx, evt_msg,
...
uint32_t ncs_mbcsv_encode_message(PEER_INST *peer, MBCSV_EVT *evt_msg,
				  uint8_t *io_event, NCS_UBAID *uba)
{
	NCS_MBCSV_CB_ARG parg;
	...
	case NCSMBCSV_EVENT_DATA_RESP:
		TRACE(
		    "set call_again_action for coldsync_resp or warmsync_resp or data_resp");
	...
	*io_event = parg.info.encode.io_msg_type;
	...
	case NCSMBCSV_EVENT_DATA_RESP:
		TRACE(
		    "set-next call_again_action for coldsync_resp or warmsync_resp or data_resp");
...
uint32_t mbcsv_send_msg(PEER_INST *peer, MBCSV_EVT *evt_msg, uint8_t event)
{
	...
	evt_msg->info.peer_msg.info.client_msg.reo_type =
	    peer->call_again_reo_type;
	evt_msg->info.peer_msg.info.client_msg.first_rsp = peer->new_msg_seq;

	if ((event != NCS_MBCSV_MSG_SYNC_SEND_RSP) &&
	    (event != NCS_MBCSV_MSG_DATA_REQ)) {
		if ((ret_val = ncs_mbcsv_encode_message(
			 peer, evt_msg, &event, &uba)) != NCSCC_RC_SUCCESS) {
			TRACE_LEAVE();
			return ret_val;
		}
		...
		evt_msg->info.peer_msg.info.client_msg.type.raw = event;
		evt_msg->info.peer_msg.info.client_msg.action =
		    peer->call_again_action;
	}

	if (NCS_MBCSV_MSG_SYNC_SEND_RSP == event) {
		TRACE("NCS_MBCSV_MSG_SYNC_SEND_RSP event");
		if (m_NCS_MBCSV_SYNC_MDS_RSP_SEND(evt_msg, peer->my_ckpt_inst,
						  peer->peer_anchor) !=
		    NCSCC_RC_SUCCESS) {
			/* Failure implies communications are down; clean up
			 * segmented list */
			TRACE_LEAVE2(
		...
		if (m_NCS_MBCSV_MDS_ASYNC_SEND(evt_msg, peer->my_ckpt_inst,
					       peer->peer_anchor) !=
		    NCSCC_RC_SUCCESS) {
	...
	case NCSMBCSV_EVENT_DATA_RESP:
		TRACE_1(
		    "event is multi-part response(i.e.coldsync resp, warmsync resp or dataresp), call-again");
...
*
* Purpose:  This function removes the message subscription filter from the
message *           subscription informatio.
*
