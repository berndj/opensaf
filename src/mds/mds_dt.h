...
#ifndef MDS_MDS_DT_H_
#define MDS_MDS_DT_H_
...
typedef struct mdtm_reassembly_queue {
  ...
  uint8_t
      tmr_flag; /* If true represents the timer running else timer has stopped
...
#define MDS_HDR_LEN                                                           \
  25 /* Mds_prot-4bit, Mds_version-2bit , Msg prior-2bit, Hdr_len-16bit,      \
        Seq_no-32bit, Enc_dec_type-2bit, Msg_snd_type-6bit, Pwe_id-16bit,     \
        Sndr_vdest_id-16bit, Sndr_svc_id-16bit, Rcvr_vdest_id-16bit,          \
        Rcvr_svc_id-16bit, Exch_id-32bit, App_Vers-16bit  node_name_len-8bit, \
        dynamic value node_name-<_POSIX_HOST_NAME_MAX>bits*/
...
#define SUM_MDS_HDR_PLUS_MDTM_HDR_PLUS_LEN                             \
  ((2 + CHK_SUM_ENB_FLAG_LEN + CHK_SUM_BYTES_LEN + MDTM_FRAG_HDR_LEN + \
    MDS_HDR_LEN)) /*2Len,1chksum_flg,2chksum,Fraghdr,MDS hdr */
#else
#define SUM_MDS_HDR_PLUS_MDTM_HDR_PLUS_LEN \
  ((2 + MDTM_FRAG_HDR_LEN + MDS_HDR_LEN)) /* 2 Len , Frag hdr, MDS hdr */
#endif

#define MDTM_MAX_SEGMENT_SIZE \
...
#define MDS_EVENT_SHIFT_FOR_PWE \
  10 /* As SVCID is 8-MDS Prefix, 8- SVC Inst, 6- PWE, 10 -SVCid */
...
static inline uint32_t m_MDS_CHECK_NCS_NODE_ID_RANGE(NODE_ID node) {
  return m_MDS_CHECK_TIPC_NODE_ID_RANGE(
...
#define m_MDS_CHECK_TIPC_NODE_ID_RANGE(node)                             \
  (((((node) < MDS_TIPC_NODE_ID_MIN) || ((node) > MDS_TIPC_NODE_ID_MAX)) \
        ? NCSCC_RC_FAILURE                                               \
        : NCSCC_RC_SUCCESS))
#define m_MDS_CHECK_NCS_NODE_ID_RANGE(node)                            \
  (((((node) < MDS_NCS_NODE_ID_MIN) || ((node) > MDS_NCS_NODE_ID_MAX)) \
        ? NCSCC_RC_FAILURE                                             \
        : NCSCC_RC_SUCCESS))
#endif
...
#define m_MMGR_ALLOC_REASSEM_QUEUE                                          \
  m_NCS_MEM_ALLOC(sizeof(MDTM_REASSEMBLY_QUEUE), NCS_MEM_REGION_PERSISTENT, \
                  NCS_SERVICE_ID_MDS, MDS_MEM_REASSEM_QUEUE)

#define m_MMGR_FREE_REASSEM_QUEUE(p)                               \
  m_NCS_MEM_FREE(p, NCS_MEM_REGION_PERSISTENT, NCS_SERVICE_ID_MDS, \
                 MDS_MEM_REASSEM_QUEUE)

#define m_MMGR_ALLOC_HDL_LIST                                           \
  m_NCS_MEM_ALLOC(sizeof(MDTM_REF_HDL_LIST), NCS_MEM_REGION_PERSISTENT, \
                  NCS_SERVICE_ID_MDS, MDS_MEM_HDL_LIST)

#define m_MMGR_FREE_HDL_LIST(p)                                    \
  m_NCS_MEM_FREE(p, NCS_MEM_REGION_PERSISTENT, NCS_SERVICE_ID_MDS, \
                 MDS_MEM_HDL_LIST)

#define m_MMGR_ALLOC_CACHED_EVENTS_LIST                                      \
  m_NCS_MEM_ALLOC(sizeof(MDTM_CACHED_EVENT_LIST), NCS_MEM_REGION_PERSISTENT, \
                  NCS_SERVICE_ID_MDS, MDS_MEM_CACHED_EVENTS_LIST)

#define m_MMGR_FREE_CACHED_EVENTS_LIST(p)                          \
  m_NCS_MEM_FREE(p, NCS_MEM_REGION_PERSISTENT, NCS_SERVICE_ID_MDS, \
