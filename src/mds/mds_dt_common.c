...
static uint32_t mdtm_fill_data(MDTM_REASSEMBLY_QUEUE *reassem_queue,
			       uint8_t *buffer, uint16_t len, uint8_t enc_type);
static MDTM_REASSEMBLY_QUEUE *mdtm_check_reassem_queue(uint32_t seq_num,
						       MDS_DEST id);
static MDTM_REASSEMBLY_QUEUE *mdtm_add_reassemble_queue(uint32_t seq_num,
							MDS_DEST id);
...
uint32_t mdtm_process_reassem_timer_event(uint32_t seq_num, MDS_DEST id)
{
	...
	if (reassem_queue == NULL) {
		m_MDS_LOG_DBG(
		    "MDTM: Empty Reassembly queue, No Matching found\n");
		return NCSCC_RC_FAILURE;
	}

	if (reassem_queue->tmr_info != NULL) {
		mdtm_free_reassem_msg_mem(
		    &reassem_queue->recv
			 .msg); /* Found During MSG Size bug Fix */
	...
	if (MDTM_TX_TYPE_TIPC == mdtm_transport) {
		...
		node_status =
		    m_MDS_CHECK_TIPC_NODE_ID_RANGE((uint32_t)(id >> 32));
		...
		m_MDS_LOG_ERR("MDTM: unsupported transport =%d",
			      mdtm_transport);
		abort();
	}
	mds_incr_subs_res_recvd_msg_cnt(
	    reassem_queue->recv.dest_svc_hdl, reassem_queue->recv.src_svc_id,
	    reassem_queue->recv.src_vdest, lcl_adest,
	    reassem_queue->recv.src_seq_num);
	mds_mcm_msg_loss(reassem_queue->recv.dest_svc_hdl, lcl_adest,
			 reassem_queue->recv.src_svc_id,
			 reassem_queue->recv.src_vdest);
...
uint32_t mdtm_del_from_ref_tbl(MDS_SUBTN_REF_VAL ref)
{
	 ...
	 */
	for (back = NULL, mov_ptr = mdtm_ref_hdl_list_hdr; mov_ptr != NULL;
	     back = mov_ptr,
	    mov_ptr =
		 mov_ptr->next) { /* Safe because we quit loop after deletion */
...
*********************************************************/
uint32_t mdtm_process_recv_message_common(uint8_t flag, uint8_t *buffer,
					  uint16_t len,
					  uint64_t transport_adest,
					  ...
					  uint32_t *buff_dump)
{
	...
	if (MDTM_TX_TYPE_TIPC == mdtm_transport) {
		node_status = m_MDS_CHECK_TIPC_NODE_ID_RANGE(
		    (uint32_t)(transport_adest >> 32));
	...
	if (MDTM_DIRECT == flag) {
		...
		/* Check whether total lenght of message is not less than MDS
		 * header length */
		if (len < len_mds_hdr) {
			m_MDS_LOG_ERR(
			    "MDTM: Message recd (Non Fragmented) len is less than the MDS HDR len  Adest = <%" PRId64
			    "> len =%d len_mds_hdr=%d",
		 ...
		 */
		pwe_hdl = m_MDS_GET_PWE_HDL_FROM_VDEST_HDL_AND_PWE_ID(
		    (MDS_VDEST_HDL)dest_vdest_id, pwe_id);

		if (NCSCC_RC_SUCCESS !=
		    mds_svc_tbl_get_svc_hdl(pwe_hdl, dest_svc_id,
					    &dest_svc_hdl)) {
			*buff_dump = 0; /* For future hack */
			m_MDS_LOG_ERR(
			    "MDTM: svc_id = %s(%d) Doesnt exists for the message recd, Adest = <%" PRId64
			    ">\n",
		...
		if (mds_svc_tbl_get_role(dest_svc_hdl) != NCSCC_RC_SUCCESS) {
			...
			case MDS_SENDTYPE_BCAST:
				m_MDS_LOG_ERR(
				    "MDTM: Recd Message SVC is in standby so dropping the message:Dest svc_id = %s(%d), dest_vdest_id = %d\n",
				    get_svc_names(dest_svc_id), dest_svc_id,
				    dest_vdest_id);
				/* Increment the recd counter as this is normal
				 */
				mds_incr_subs_res_recvd_msg_cnt(
				    dest_svc_hdl, src_svc_id, src_vdest_id,
				    adest, svc_seq_num);
				return NCSCC_RC_FAILURE;
				break;
			default:
		...
		if (reassem_queue == NULL) {
			m_MDS_LOG_ERR(
			    "MDTM: Memory allocation failed for reassem_queue\n");
			return NCSCC_RC_FAILURE;
		}
		...
		if (len_mds_hdr > (MDS_HDR_LEN - 1)) {
			 ...
			 * than mds header length of this instance */
			if (len_mds_hdr <
			    (MDS_HDR_LEN +
			     reassem_queue->recv.src_node_name_len)) {
				m_MDS_LOG_ERR(
		...
		MDS_PROCESS_INFO *info =
		    mds_process_info_get(adest, src_svc_id);
		if (info != NULL) {
		...
		m_MDS_LOG_DBG(
		    "MDTM: Recd Unfragmented message with SVC Seq num =%d, from src Adest = <%" PRId64
		    ">",
		...
		if (msg_snd_type == MDS_SENDTYPE_ACK) {
			...
			if (len != len_mds_hdr) {
				/* Size of Payload data in ACK message should be
				 * zero, If not its an error */
				m_MDS_LOG_ERR(
				    "MDTM: ACK message contains payload data, Total Len=%d,  len_mds_hdr=%d",
				    len, len_mds_hdr);
				m_MMGR_FREE_REASSEM_QUEUE(reassem_queue);
				return NCSCC_RC_FAILURE;
			}
			...
			/*if(reassem_queue->recv.msg_fmt_ver!=1)
			   {
			   m_MDS_LOG_ERR("MDTM: Unable to process the recd
			   message due to wrong
			...
			if (NCSCC_RC_SUCCESS !=
			    mdtm_fill_data(reassem_queue, &buffer[len_mds_hdr],
					   (len - (len_mds_hdr)), enc_type)) {
				m_MDS_LOG_ERR(
			...
			/* Depending on msg type if flat or full enc apply dec
			 * space, for setting the uba to decode by user */
			if (reassem_queue->recv.msg.encoding ==
			    MDS_ENC_TYPE_FLAT) {
				ncs_dec_init_space(
				    &reassem_queue->recv.msg.data.flat_uba,
				    reassem_queue->recv.msg.data.flat_uba
					.start);
			} else if (reassem_queue->recv.msg.encoding ==
				   MDS_ENC_TYPE_FULL) {
				ncs_dec_init_space(
				    &reassem_queue->recv.msg.data.fullenc_uba,
				    reassem_queue->recv.msg.data.fullenc_uba
					.start);
			}
		...
		/* Call upper layer */
		m_MDS_LOG_INFO(
		    "MDTM: Sending data to upper layer for a single recd message\n");
		...
		/* Check whether total lenght of message is not less than or
		 * equal to MDS header length and MDTM frag header length */
		if (len <= (len_mds_hdr + MDTM_FRAG_HDR_LEN)) {
			m_MDS_LOG_ERR(
			    "MDTM: Message recd (Fragmented First Pkt) len is less than or equal to \
		...
		data = NULL;
		data =
		    &buffer[MDS_HEADER_MSG_TYPE_POSITION + MDTM_FRAG_HDR_LEN];
		...
		if (NCSCC_RC_SUCCESS !=
			...
			*buff_dump = 0; /* For future hack */
			m_MDS_LOG_ERR(
			    "MDTM: svc_id = %s(%d) Doesnt exists for the message recd\n",
			    get_svc_names(dest_svc_id), dest_svc_id);
			return NCSCC_RC_FAILURE;
		}
		...
		reassem_queue =
		    mdtm_add_reassemble_queue(seq_num, transport_adest);

		if (reassem_queue == NULL) {
			m_MDS_LOG_ERR(
			    "MDTM: New reassem queue creation failed\n");
			return NCSCC_RC_FAILURE;
		}
		...
		m_MDS_LOG_DBG(
		    "MDTM: Recd fragmented message(first frag) with Frag Seqnum=%d SVC Seq num =%d, from src Adest = <%" PRId64
		    ">",
		...
		if ((len - (len_mds_hdr + MDTM_FRAG_HDR_LEN)) > 0) {
			if (NCSCC_RC_SUCCESS !=
			    mdtm_fill_data(
				reassem_queue,
				&buffer[len_mds_hdr + MDTM_FRAG_HDR_LEN],
				(len - (len_mds_hdr + MDTM_FRAG_HDR_LEN)),
				enc_type)) {
				m_MDS_LOG_ERR("MDTM: MDtm fill data failed\n");
				mdtm_del_reassemble_queue(seq_num,
							  transport_adest);
				return NCSCC_RC_FAILURE;
			}
		} else {
			m_MDS_LOG_ERR(
			    "MDTM: No Payload Data present, Total Len=%d, sum of frag_hdr and mds_hdr=%d",
			    len, (len_mds_hdr + MDTM_FRAG_HDR_LEN));
		...
		if (tmr_req_info == NULL) {
			m_MDS_LOG_ERR(
			    "MDTM: Memory allocation for timer request failed\n");
			return NCSCC_RC_FAILURE;
		}
		memset(tmr_req_info, 0, sizeof(MDS_TMR_REQ_INFO));
		tmr_req_info->type = MDS_REASSEMBLY_TMR;
		tmr_req_info->info.reassembly_tmr_info.seq_no =
		    reassem_queue->key.frag_sequence_num;
		...
		reassem_queue->tmr_hdl = ncshm_create_hdl(
		...
		m_MDS_LOG_DBG(
		    "MCM_DB:RecvMessage:TimerStart:Reassemble:Hdl=0x%08x:SrcSvcId=%s(%d):SrcVdest=%d,DestSvcHdl=%" PRId64
		    "\n",
...
uint32_t mds_tmr_callback(NCSCONTEXT tmr_info_hdl)
{
	...
	if ((m_NCS_IPC_SEND(&mdtm_mbx_common, mbx_tmr_info,
			    NCS_IPC_PRIORITY_NORMAL)) != NCSCC_RC_SUCCESS) {
		/* Message Queuing failed, free the msg. In TDS they are
		 * relaseing the task ,releasing IPC and freeing the TDS CB
		 * shall we do that same ....??   */
...
uint32_t mds_tmr_mailbox_processing(void)
{
	...
	mbx_evt_info = (MDS_MBX_EVT_INFO *)(m_NCS_IPC_NON_BLK_RECEIVE(
	    &mdtm_mbx_common, NULL));
	...
	if (mbx_evt_info == NULL) {
		...
		tmr_req_info = (MDS_TMR_REQ_INFO *)ncshm_take_hdl(
		    NCS_SERVICE_ID_COMMON,
		    (uint32_t)(mbx_evt_info->info.tmr_info_hdl));
		if (tmr_req_info == NULL) {
			m_MDS_LOG_INFO(
			    "MDTM: Tmr Mailbox Processing:Handle invalid (=0x%08x)",
			    mbx_evt_info->info.tmr_info_hdl);
			/* return NCSCC_RC_SUCCESS; */ /* Fall through to free
							  memory */
		} else {
			switch (tmr_req_info->type) {
			case MDS_QUIESCED_TMR:
				m_MDS_LOG_DBG(
				    "MDTM: Tmr Mailbox Processing:QuiescedTimer Hdl=0x%08x",
				    mbx_evt_info->info.tmr_info_hdl);
				mds_mcm_quiesced_tmr_expiry(
				    tmr_req_info->info.quiesced_tmr_info
					.vdest_id);
				break;

			case MDS_SUBTN_TMR:
				m_MDS_LOG_DBG(
				    "MDTM: Tmr Mailbox Processing:Subtn Tmr Hdl=0x%08x",
				    mbx_evt_info->info.tmr_info_hdl);
				mds_mcm_subscription_tmr_expiry(
				    tmr_req_info->info.subtn_tmr_info.svc_hdl,
				    tmr_req_info->info.subtn_tmr_info
					.sub_svc_id);
				break;

			case MDS_AWAIT_ACTIVE_TMR:
				m_MDS_LOG_DBG(
				    "MDTM: Tmr Mailbox Processing:Active Tmr Hdl=0x%08x",
				    mbx_evt_info->info.tmr_info_hdl);
				mds_mcm_await_active_tmr_expiry(
				    tmr_req_info->info.await_active_tmr_info
					.svc_hdl,
				    tmr_req_info->info.await_active_tmr_info
					.sub_svc_id,
				    tmr_req_info->info.await_active_tmr_info
			...
			case MDS_REASSEMBLY_TMR:
				...
				mdtm_process_reassem_timer_event(
				    tmr_req_info->info.reassembly_tmr_info
					.seq_no,
				    tmr_req_info->info.reassembly_tmr_info.id);
				break;
			case MDS_DOWN_TMR: {
				MDS_PROCESS_INFO *info = mds_process_info_get(
				    tmr_req_info->info.down_event_tmr_info
					.adest,
				    tmr_req_info->info.down_event_tmr_info
					.svc_id);
				/* only delete if process not exist to avoid
				 * race with a client that re-registers
				 * immediately after unregister */
				if ((info != NULL) &&
				    ((kill(info->pid, 0) == -1) &&
				     (errno != EPERM))) {
					TRACE(
				...
				if (tmr_req_info->info.down_event_tmr_info
					...
					ncs_tmr_free(
					    tmr_req_info->info
						.down_event_tmr_info.tmr_id);
				}
				break;
			}
			default:
				m_MDS_LOG_ERR(
				    "MDTM: Tmr Mailbox Processing:JunkTmr Hdl=0x%08x",
				    mbx_evt_info->info.tmr_info_hdl);
				break;
			}
		...
		/* No destruction processing. Due to already existing
		   implementation, the destroying thread performs the full
		   destruction. This messagae is merely to wake up the MDTM
		   thread so that it may process the destroy-command. We need to
		   acknowledge that this event has been processed */
		m_NCS_SEL_OBJ_IND(&mbx_evt_info->info.destroy_ack_obj);
		status = NCSCC_RC_DISABLED; /* To indicate that thread should
					       destroy itself */
	} else {
		/* Event-type not set. BUG */
		assert(0);
		/* No further processing here. Just fall through and free evt
		 * structure */
	}
	/* Free tmr_req_info */
...
*********************************************************/
uint32_t mdtm_process_recv_data(uint8_t *buffer, uint16_t len,
				uint64_t transport_adest, uint32_t *buff_dump)
{
	/*
	   Get the MDS Header from the data received
	   if Destination SVC exists
	   if the message is a fragmented one
	   reassemble  the recd data(is done based on the received data
	   <Sequence number, transport_adest>) An entry for the matching
	   transport_adest is serached in the MDS_ADEST_LIST

	   if found
	   reasesemble is done on the MDS_ADEST_LIST
	   if not found
	   an entry is created and reasesemble is done on the MDS_ADEST_LIST
	   A timer is also started on the reassembly queue (to prevent large
	   timegaps between the fragemented packets) Send the data to the upper
	   (Check the role of Destination Service
	   for some send types before giving the data to upper layer)
	 */

	/* If the data is recd over here, it means its a fragmented or
	 * non-fragmented pkt) */
	uint16_t pkt_type = 0;
	uint8_t *data;
	...
	if (pkt_type == MDTM_NORMAL_PKT) {
		return mdtm_process_recv_message_common(
		    MDTM_DIRECT, &buffer[MDTM_FRAG_HDR_LEN],
		...
		m_MDS_LOG_DBG(
		    "MDTM: Recd message with Fragment Seqnum=%d, frag_num=%d, from src_id=<0x%08x:%u>",
		    seq_num, frag_num, (uint32_t)(transport_adest >> 32),
		    (uint32_t)(transport_adest));

		/* Checking in reassembly queue */
		reassem_queue =
		    mdtm_check_reassem_queue(seq_num, transport_adest);

		if (reassem_queue != NULL) {
			if (len <= MDTM_FRAG_HDR_LEN) {
				m_MDS_LOG_ERR(
				    "MDTM: No payload data present in fragmented msg or incomplete frag hdr=%d",
				    len);
				return NCSCC_RC_FAILURE;
			}
			...
			if (reassem_queue->to_be_dropped == true) {
				...
				if (((pkt_type & 0x7fff) > 0) &&
				    ((pkt_type & 0x8000) == 0)) {
					/* Last frag in the message recd */

					/* Free memory Allocated to this msg and
					 * MDTM_REASSEMBLY_QUEUE */
					mdtm_free_reassem_msg_mem(
					    &reassem_queue->recv.msg);
					...
					m_MMGR_FREE_TMR_INFO(
					    reassem_queue->tmr_info);

					reassem_queue->tmr_info = NULL;

					/* Delete entry from
					 * MDTM_REASSEMBLY_QUEUE */
					mdtm_del_reassemble_queue(
					    reassem_queue->key
						.frag_sequence_num,
					    reassem_queue->key.id);
				}
				*buff_dump =
				    0; /* For future use. It can be made 1,
					  easily without having to relink etc.
					*/
				m_MDS_LOG_ERR(
				    "MDTM: Message is dropped as msg is out of seq Adest = <%" PRIu64
				    ">\n",
				    transport_adest);
				return NCSCC_RC_FAILURE;
			}
			...
			if (reassem_queue->next_frag_num == frag_num) {
				...
				if (mds_svc_tbl_get_role(
					reassem_queue->recv.dest_svc_hdl) !=
				    NCSCC_RC_SUCCESS)
				/* fUNTION WILL RETURN success when svc is in
				   active or quiesced */
				{
				...
				/* Enqueue the data at the End depending on the
				 * data type */
				if (reassem_queue->recv.msg.encoding ==
				    MDS_ENC_TYPE_FLAT) {
					m_MDS_LOG_INFO(
					    "MDTM: Reassembling in flat UB\n");
					NCS_UBAID ub;
					ncs_enc_init_space_pp(&ub, 0, 0);
					ncs_encode_n_octets_in_uba(
					    &ub, &buffer[MDTM_FRAG_HDR_LEN],
					    (len - MDTM_FRAG_HDR_LEN));

					ncs_enc_append_usrbuf(
					    &reassem_queue->recv.msg.data
						 .flat_uba,
					    ub.start);
				} else if (reassem_queue->recv.msg.encoding ==
					   MDS_ENC_TYPE_FULL) {
					m_MDS_LOG_INFO(
					    "MDTM: Reassembling in FULL UB\n");
					NCS_UBAID ub;
					ncs_enc_init_space_pp(&ub, 0, 0);
					ncs_encode_n_octets_in_uba(
					    &ub, &buffer[MDTM_FRAG_HDR_LEN],
				...
				/* Reassemble and send to upper layer if last
				 * pkt */
				if (((pkt_type & 0x7fff) > 0) &&
				    ((pkt_type & 0x8000) == 0)) {
					/* Last frag in the message recd */
					...
					m_MDS_LOG_INFO(
					    "MDTM: Sending data to upper layer\n");

					/* Depending on the msg type if flat or
					 * full enc apply dec space, for setting
					...
					if (reassem_queue->recv.msg.encoding ==
						...
						ncs_dec_init_space(
						    ...
						    reassem_queue->recv.msg.data
							.flat_uba.start);
					} else if (reassem_queue->recv.msg
						       .encoding ==
						   MDS_ENC_TYPE_FULL) {
						ncs_dec_init_space(
						    &reassem_queue->recv.msg
					...
					/* for direct buff and cpy encoding
					 * modes we do nothig */
					mds_mcm_ll_data_rcv(
					    &reassem_queue->recv);

					/* Now delete this entry from Global
					 * reassembly queue */
					mdtm_del_reassemble_queue(
					    reassem_queue->key
						.frag_sequence_num,
				...
				*buff_dump =
					...
					*/
				m_MDS_LOG_ERR(
				    "MDTM: Frag recd is not next frag so dropping Adest = <%" PRIu64
				    ">\n",
				    transport_adest);
				reassem_queue
				...
				if (((pkt_type & 0x7fff) > 0) &&
					...
					/* Free memory Allocated to UB and
					 * MDTM_REASSEMBLY_QUEUE */
					mdtm_free_reassem_msg_mem(
					    &reassem_queue->recv.msg);
			...
			return mdtm_process_recv_message_common(
			    MDTM_REASSEMBLE, buffer, len, transport_adest,
			    temp_frag_seq_num, buff_dump);
		} else {
			*buff_dump = 0;
			/* Some stale message, Log and Drop */
			m_MDS_LOG_ERR(
			    "MDTM: Some stale message recd, hence dropping Adest = <%" PRIu64
			    ">\n",
			    transport_adest);
			return NCSCC_RC_FAILURE;
		}
			       ...
			       uint8_t *buffer, uint16_t len, uint8_t enc_type)
{
	...
	case MDS_ENC_TYPE_FLAT: {
		ncs_enc_init_space_pp(&reassem_queue->recv.msg.data.flat_uba, 0,
				      0);
		ncs_encode_n_octets_in_uba(
		    &reassem_queue->recv.msg.data.flat_uba, buffer, len);
		return NCSCC_RC_SUCCESS;
	} break;

	case MDS_ENC_TYPE_FULL: {
		ncs_enc_init_space_pp(&reassem_queue->recv.msg.data.fullenc_uba,
				      0, 0);
		ncs_encode_n_octets_in_uba(
		    &reassem_queue->recv.msg.data.fullenc_uba, buffer, len);
		return NCSCC_RC_SUCCESS;
	} break;

	case MDS_ENC_TYPE_DIRECT_BUFF: {
		reassem_queue->recv.msg.data.buff_info.buff =
		    mds_alloc_direct_buff(len);
		memcpy(reassem_queue->recv.msg.data.buff_info.buff, buffer,
		       len);
						       ...
						       MDS_DEST id)
{
	...
	if (reassem_queue == NULL) {
		m_MDS_LOG_DBG(
		    "MDS_DT_COMMON : reassembly queue doesnt exist seq_num=%d, Adest = <0x%08x,%u",
...
{
	/*
	   STEP 1: create an entry in the reassemble queue with parameters as
	   seq_num and id, return the Pointer to the reassembly queue
	 */
	MDTM_REASSEMBLY_QUEUE *reassem_queue = NULL;
	...
	if (reassem_queue == NULL) {
		m_MDS_LOG_ERR(
		    "MDTM: Memory allocation to reassembly queue failed\n");
		return reassem_queue;
	}
...
void mds_buff_dump(uint8_t *buff, uint32_t len, uint32_t max)
{
	...
	if (len > max) {
		m_MDS_LOG_ERR(
		    "DUMP:Changing dump-extent:buff=0x%s:max=%d, len=%d\n",
		    buff, max, len);
		len = max;
	}
	...
	for (offset = 0; (len - offset) > 8; offset += 8) {
		m_MDS_LOG_ERR(
		    ...
		    (uint32_t)(long)buff, offset, offset + 7, buff[offset],
		    buff[offset + 1], buff[offset + 2], buff[offset + 3],
		    buff[offset + 4], buff[offset + 5], buff[offset + 6],
		    buff[offset + 7]);
	}
	...
	m_MDS_LOG_ERR(
	    "DUMP:buff=0x%08x:offset=%3d to %3d:Bytes = 0x%02x 0x%02x 0x%02x 0x%02x : 0x%02x 0x%02x 0x%02x 0x%02x",
	    (uint32_t)(long)buff, offset, len - 1, last_line[0],
	    last_line[0 + 1], last_line[0 + 2], last_line[0 + 3],
	    last_line[0 + 4], last_line[0 + 5], last_line[0 + 6],
	    last_line[0 + 7]);
}
...
uint32_t mds_destroy_event(NCS_SEL_OBJ destroy_ack_obj)
{
	...
	if ((m_NCS_IPC_SEND(&mdtm_mbx_common, mbx_evt_info,
...
bool mdtm_mailbox_mbx_cleanup(NCSCONTEXT arg, NCSCONTEXT msg)
{
	...
	case MDS_MBX_EVT_TIMER_EXPIRY:
		/* freeing of tmr_req_info and handle is done in mcm, where
		 * tmr_running flag is still true */
		break;
	case MDS_MBX_EVT_DESTROY:
		/* Destroy ack object is destroyed by thread posting the
		 * destroy-event */
		break;
	default:
		assert(0);
