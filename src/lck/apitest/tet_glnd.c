...
void tet_glnd_show()
{
	...
	while (1) {
		...
		default:
			printf(
			    "enter appropriate code,codes supported are glnd-n,rsc-r,end-e...\n");
			break;
		}
		if (c == 'e')
			exit(1);
...
void print_glnd_cb()
{
	...
	cb = (GLND_CB *)ncshm_take_hdl(NCS_SERVICE_ID_GLND,
				       m_GLND_RETRIEVE_GLND_CB_HDL);
...
void print_resource_cb_info()
{
	...
	glnd_rsc = (GLND_RESOURCE_INFO *)ncs_patricia_tree_getnext(
