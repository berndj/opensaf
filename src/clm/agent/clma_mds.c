...
{
	return clma_dec_node_get_msg(uba,
				     &msg->info.api_resp_info.param.node_get);
}

static uint32_t
clma_dec_cluster_ntf_buf_msg(NCS_UBAID *uba,
...
static uint32_t clma_mds_dec(struct ncsmds_callback_info *info)
{
	...
	case CLMSV_CLMS_TO_CLMA_API_RESP_MSG: {
		...
		case CLMSV_NODE_GET_RESP:
			if (msg->info.api_resp_info.rc == SA_AIS_OK)
				total_bytes +=
				    clma_dec_node_get_rsp_msg(uba, msg);
			break;
	...
	case CLMSV_CLMS_TO_CLMA_CBK_MSG: {
		...
		case CLMSV_NODE_ASYNC_GET_CBK:
			...
			total_bytes +=
			    clma_dec_node_async_get_cbk_msg(uba, msg);
			break;
		default:
			    ...
			    MDS_SEND_PRIORITY_TYPE prio)
{
	...
	case CLMSV_CLMS_TO_CLMA_IS_MEMBER_MSG: {
		...
		clma_hdl_rec->is_member =
		    clmsv_msg->info.is_member_info.is_member;
	...
	case NCSMDS_UP:
		...
		if (clma_cb.clms_reinit_required == true) {
			for (clma_hdl_rec = clma_cb.client_list;
				...
				NCS_SEL_OBJ sel_obj =
				    m_NCS_IPC_GET_SEL_OBJ(&clma_hdl_rec->mbx);
