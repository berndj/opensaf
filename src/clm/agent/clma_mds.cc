...
static uint32_t clma_dec_node_get_msg(NCS_UBAID *uba,
...
static uint32_t clma_dec_cluster_ntf_buf_msg(
  ...
  for (i = 0; i < param->numberOfItems; i++) {
    ...
    total_bytes += 4;
    total_bytes +=
        clma_dec_node_get_msg(uba, &param->notification[i].clusterNode);
  }
...
static uint32_t clma_dec_track_current_rsp_msg(NCS_UBAID *uba, CLMSV_MSG *msg) {
  ...
  track->notify_info = static_cast<SaClmClusterNotificationBufferT_4 *>(
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
static uint32_t clma_dec_node_async_get_cbk_msg(NCS_UBAID *uba,
                                                CLMSV_MSG *msg) {
  uint8_t *p8;
  uint32_t total_bytes = 0;
  ...
  switch (msg->evt_type) {
    ...
    case CLMSV_CLMS_TO_CLMA_CBK_MSG: {
      ...
      msg->info.cbk_info.type =
          static_cast<CLMSV_CBK_MSG_TYPE>(ncs_decode_32bit(&p8));
    ...
    case CLMSV_CLMS_TO_CLMA_IS_MEMBER_MSG: {
      ...
      msg->info.is_member_info.is_member =
          static_cast<SaBoolT>(ncs_decode_32bit(&p8));
      msg->info.is_member_info.is_configured =
          static_cast<SaBoolT>(ncs_decode_32bit(&p8));
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uint32_t clma_clms_msg_proc(clma_cb_t *cb, CLMSV_MSG *clmsv_msg,
...
static uint32_t clma_mds_rcv(struct ncsmds_callback_info *mds_cb_info) {
  ...
  /* process the message */
  rc = clma_clms_msg_proc(&clma_cb, clmsv_msg,
...
static uint32_t clma_mds_dec_flat(struct ncsmds_callback_info *info) {
  ...
  if (rc != NCSCC_RC_SUCCESS) TRACE("clma_mds_dec rc = %d", rc);
...
uint32_t clma_mds_init(clma_cb_t *cb) {
  ...
  mds_info.info.svc_install.i_svc_cb = clma_mds_callback;
  /* callback */ /*need to do */
  mds_info.info.svc_install.i_mds_q_ownership =
      false; /* CLMA doesn't own the mds queue */
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uint32_t clma_mds_msg_sync_send(clma_cb_t *cb, CLMSV_MSG *i_msg,
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uint32_t clma_mds_msg_async_send(clma_cb_t *cb, CLMSV_MSG *i_msg,
                                 uint32_t prio) {
  NCSMDS_INFO mds_info;
  uint32_t rc = NCSCC_RC_SUCCESS;
  ...
  /* send the message */
  rc = ncsmds_api(&mds_info);
  if (rc != NCSCC_RC_SUCCESS) TRACE("failed");
