...
static unsigned int clma_create(void)
{
	...
	/* register with MDS */
	if ((NCSCC_RC_SUCCESS !=
	     (rc = clma_mds_init(&clma_cb)))) { /* need to do */
		rc = NCSCC_RC_FAILURE;
		goto error;
	}
					...
					SaVersionT *version, uint32_t client_id)
{
	...
	if (0 == (rec->local_hdl =
		      ncshm_create_hdl(NCS_HM_POOL_ID_COMMON,
				       NCS_SERVICE_ID_CLMA, (NCSCONTEXT)rec))) {
	...
	case CLMSV_TRACK_CBK: {
		if (clma_validate_version(hdl_rec->version)) {
			if (reg_cbk->saClmClusterTrackCallback) {
				...
				buf->viewNumber =
				    cbk_info->param.track.buf_info.viewNumber;
				buf->numberOfItems =
				    cbk_info->param.track.buf_info
					.numberOfItems;
				buf->notification =
				    (SaClmClusterNotificationT *)malloc(
					sizeof(SaClmClusterNotificationT) *
					buf->numberOfItems);
				clma_fill_clusterbuf_from_buf_4(
				    buf, &cbk_info->param.track.buf_info);
				reg_cbk->saClmClusterTrackCallback(
				    buf, cbk_info->param.track.mem_num,
		...
		if (clma_validate_version(hdl_rec->version)) {
			if (reg_cbk->saClmClusterNodeGetCallback) {
				SaClmClusterNodeT *node =
				    (SaClmClusterNodeT *)malloc(
					sizeof(SaClmClusterNodeT));
				clma_fill_node_from_node4(
				    node, cbk_info->param.node_get.info);
			...
			if (reg_cbk_4->saClmClusterNodeGetCallback) {
				reg_cbk_4->saClmClusterNodeGetCallback(
				    cbk_info->param.node_get.inv,
					     ...
					     clma_client_hdl_rec_t *hdl_rec)
{
	...
	while (NULL != (cbk_msg = (CLMSV_MSG *)m_NCS_IPC_NON_BLK_RECEIVE(
		...
		if (cbk_msg->info.cbk_info.type == CLMSV_TRACK_CBK ||
			...
			rc = clma_hdl_cbk_rec_prc(cb, cbk_msg,
						  hdl_rec); /*need to do */
					  ...
					  clma_client_hdl_rec_t *hdl_rec)
{
	...
	do {
		...
		if (cbk_msg->info.cbk_info.type == CLMSV_TRACK_CBK ||
			...
			TRACE_2("cbk_msg->info.cbk_info.type = %d",
				cbk_msg->info.cbk_info.type);
...
void clma_msg_destroy(CLMSV_MSG *msg)
{
	...
	if (msg->evt_type == CLMSV_CLMS_TO_CLMA_API_RESP_MSG) {
		if (msg->info.api_resp_info.type == CLMSV_TRACK_CURRENT_RESP) {
			 ...
			 * must be done here
			 */
			if (msg->info.api_resp_info.param.track.notify_info
				->numberOfItems)
				free(msg->info.api_resp_info.param.track
		...
		if (msg->info.cbk_info.type == CLMSV_TRACK_CBK) {
			 ...
			 * must be done here
			 */
			if (msg->info.cbk_info.param.track.buf_info
				.numberOfItems)
				free(msg->info.cbk_info.param.track.buf_info
					 .notification);
