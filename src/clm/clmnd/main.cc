...
static uint32_t clmna_mds_dec(struct ncsmds_callback_info *info) {
   ...
   **/
  if (nullptr ==
      (msg = static_cast<CLMSV_MSG *>(calloc(1, sizeof(CLMSV_MSG))))) {
    TRACE("calloc failed\n");
    return NCSCC_RC_FAILURE;
  }
  ...
  switch (msg->evt_type) {
    ...
    case CLMSV_CLMS_TO_CLMA_API_RESP_MSG: {
      ...
      msg->info.api_resp_info.type =
          static_cast<CLMSV_API_RESP_MSG_TYPE>(ncs_decode_32bit(&p8));
    ...
    case CLMSV_CLMS_TO_CLMNA_ACTION_MSG: {
      ...
      if (msg->info.action_info.node_id == 0 ||
          clmna_cb->node_info.node_id == msg->info.reboot_info.node_id) {
        total_bytes +=
            clmsv_decodeSaNameT(uba, &(msg->info.action_info.action));

        action = osaf_extended_name_borrow(&(msg->info.action_info.action));
...
static void clmna_handle_mds_change_evt(bool caused_by_timer_expiry,
  ...
  if (caused_by_timer_expiry == false &&
    ...
    if (change == NCSMDS_UP) {
      ElectionStarterUpEvent(clmna_cb->election_starter, node_id,
                             svc_id == NCSMDS_SVC_ID_CLMNA
                                 ? ElectionStarterServiceNode
                                 : ElectionStarterServiceController);
    } else {
      ElectionStarterDownEvent(clmna_cb->election_starter, node_id,
                               svc_id == NCSMDS_SVC_ID_CLMNA
                                   ? ElectionStarterServiceNode
                                   : ElectionStarterServiceController);
    }
...
static void clmna_handle_join_response(SaAisErrorT error,
  ...
  if (error == SA_AIS_ERR_NOT_EXIST) {
    ...
    LOG_ER("%s is already up. Specify a unique name in" PKGSYSCONFDIR
           "/node_name",
           node_name->value);
    ...
    if (clmna_cb->server_synced == false) {
      ...
      if (clmna_cb->nid_started &&
          nid_notify("CLMNA", NCSCC_RC_SUCCESS, nullptr) != NCSCC_RC_SUCCESS) {
...
static uint32_t clmna_mds_enc(struct ncsmds_callback_info *info) {
  ...
  if (CLMSV_CLMA_TO_CLMS_API_MSG == msg->evt_type) {
    ...
    if (msg->info.api_info.type == CLMSV_CLUSTER_JOIN_REQ) {
      ...
      ncs_encode_8bit(&p8, msg->info.api_info.param.nodeup_info.ifs);
      ncs_enc_claim_space(uba, 1);
      total_bytes += 1;
      TRACE("Encoded CLM_IFS: [%c] ", msg->info.api_info.param.nodeup_info.ifs);

      p8 = ncs_enc_reserve_space(uba, 8);
      ncs_encode_64bit(&p8, msg->info.api_info.param.nodeup_info.boot_time);
      ncs_enc_claim_space(uba, 8);
      total_bytes += 8;
      TRACE("Encoded boot time: %" PRIu64,
            (uint64_t)msg->info.api_info.param.nodeup_info.boot_time);
      ...
      if (no_of_addresses != 0) {
        ...
        total_bytes += clmsv_encodeNodeAddressT(
            uba, &(msg->info.api_info.param.nodeup_info.address));
      }
    }
  }
...
static uint32_t clmna_mds_callback(struct ncsmds_callback_info *info) {
  ...
  if (info->i_op <= MDS_CALLBACK_SVC_EVENT) {
    ...
    if (rc != NCSCC_RC_SUCCESS) LOG_ER("MDS_CALLBACK_SVC_EVENT not in range");
  }
...
static void get_user_data(NODE_INFO *node) {
  ...
  if (memchr(node->user_data.value, node->ifs, node->user_data.length) !=
      nullptr) {
...
static int get_node_info(NODE_INFO *node) {
  ...
  if (memchr(node->node_name.value, node->ifs, node->node_name.length) !=
      nullptr) {
  ...
  uint64_t boot_time_10us = (osaf_timespec_to_nanos(&boot_time) + 5000) / 10000;
  ...
  if (family != nullptr) {
    ...
    if (errno != 0 || *endptr != '\0' || *family == '\0') family = nullptr;
  }
  if (family != nullptr && value != nullptr &&
      memchr(value, node->ifs, strlen(value)) == nullptr) {
    size_t len = strlen(value);
    if (len > SA_CLM_MAX_ADDRESS_LENGTH) len = SA_CLM_MAX_ADDRESS_LENGTH;
    node->address.family = static_cast<SaClmNodeAddressFamilyT>(family_val);
...
static void clmna_execute_action(const char *action) {
  ...
  if (strchr(action, '/')) {
    syslog(LOG_ERR, "Executing CLM action '%s' contains not allowed sign '/'",
           action);
    return;
  }

  int size = snprintf(str, sizeof(str), PKGCLMSCRIPTDIR "/osafclm_%s", action);
  if (size < 0 || static_cast<size_t>(size) > sizeof(str)) {
    syslog(LOG_ERR, "Path for executing CLM script '%s' is longer than %zu",
           action, sizeof(str));
    return;
  }
  ...
  if (rc == -1) {
    ...
    if (WIFEXITED(rc) && WEXITSTATUS(rc) == 0) {
      syslog(LOG_NOTICE, "CLM script: %s successfully executed", str);
    } else {
      syslog(LOG_CRIT, "CLM script: %s failed, rc = %d", str, rc);
    }
  }
}
...
static void scale_out_tmr_exp(void *arg) {
  ...
  if (clmna_cb->is_scale_out_retry_tmr_running == true) {
    ...
    if (evt != nullptr) {
      ...
      LOG_ER("Could not allocate IPC event: %s", __FUNCTION__);
    }
  } else {
    LOG_ER("Unexpected scale out timer expiration: %s", __FUNCTION__);
  }
...
static void start_scale_out_retry_tmr(int64_t timeout) {
  ...
  if (clmna_cb->scale_out_retry_tmr != nullptr &&
      clmna_cb->is_scale_out_retry_tmr_running == false) {
    clmna_cb->scale_out_retry_tmr =
        ncs_tmr_start(clmna_cb->scale_out_retry_tmr, timeout, scale_out_tmr_exp,
                      nullptr, const_cast<char *>(__FILE__), __LINE__);
    if (clmna_cb->scale_out_retry_tmr != nullptr) {
...
static void clmna_process_dummyup_msg() {
  if (clmna_cb->clms_mds_dest != 0) {
    ...
    msg.info.api_info.param.nodeup_info.boot_time = self_node.boot_time;
    msg.info.api_info.param.nodeup_info.no_of_addresses =
  ...
  switch (msg->type) {
    case CLMNA_EVT_CHANGE_MSG:
      clmna_handle_mds_change_evt(msg->change.caused_by_timer_expiry,
                                  msg->change.change, msg->change.node_id,
                                  msg->change.svc_id);
      break;
...
int main(int argc, char *argv[]) {
  ...
  if (getenv("SA_AMF_COMPONENT_NAME") == nullptr) clmna_cb->nid_started = true;
  ...
  if (clmna_mds_init() != NCSCC_RC_SUCCESS) goto done;
   ...
   */
  if (clmna_cb->nid_started && signal(SIGUSR1, sigusr1_handler) == SIG_ERR) {
  ...
  fds[FD_AMF].fd =
      clmna_cb->nid_started ? usr1_sel_obj.rmv_obj : clmna_cb->amf_sel_obj;
  ...
  while (1) {
    ...
    if (fds[FD_AMF].revents & POLLIN) {
      if (clmna_cb->amf_hdl != 0) {
        if ((error = saAmfDispatch(clmna_cb->amf_hdl, SA_DISPATCH_ALL)) !=
            SA_AIS_OK) {
