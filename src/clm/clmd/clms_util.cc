...
static const SaNameT _clmSvcUsrName = {sizeof("safApp=safClmService") - 1,
                                       "safApp=safClmService"};
...
CLMS_CLUSTER_NODE *clms_node_get_by_name(const SaNameT *name) {
  CLMS_CLUSTER_NODE *clms_node = nullptr;

  TRACE_ENTER2("name input %s length %d", name->value, name->length);

  clms_node = (CLMS_CLUSTER_NODE *)ncs_patricia_tree_get(&clms_cb->nodes_db,
                                                         (uint8_t *)name);
  if (clms_node != nullptr) {
    /* Adjust the pointer */
    clms_node =
        (CLMS_CLUSTER_NODE *)(((char *)clms_node) -
                              (((char *)&(
                                   ((CLMS_CLUSTER_NODE *)0)->pat_node_name)) -
...
CLMS_CLUSTER_NODE *clms_node_get_by_eename(SaNameT *name) {
  ...
  clms_node = (CLMS_CLUSTER_NODE *)ncs_patricia_tree_get(&clms_cb->ee_lookup,
                                                         (uint8_t *)name);
  ...
  if (clms_node != nullptr) {
    ...
    clms_node =
        (CLMS_CLUSTER_NODE *)(((char *)clms_node) -
                              (((char *)&(
                                   ((CLMS_CLUSTER_NODE *)0)->pat_node_eename)) -
                               ((char *)((CLMS_CLUSTER_NODE *)0))));
...
CLMS_CLUSTER_NODE *clms_node_getnext_by_id(SaUint32T node_id) {
  ...
  if (node_id == 0) {
    node = (CLMS_CLUSTER_NODE *)ncs_patricia_tree_getnext(&clms_cb->id_lookup,
                                                          (uint8_t *)0);
  } else
    node = (CLMS_CLUSTER_NODE *)ncs_patricia_tree_getnext(&clms_cb->id_lookup,
                                                          (uint8_t *)&node_id);
  ...
  switch (i) {
    ...
    case 1:
      ...
      if (rc != NCSCC_RC_SUCCESS) {
        ...
        node->pat_node_eename.key_info = nullptr;
        goto done;
      }
      break;
    case 2:
      ...
      if (rc != NCSCC_RC_SUCCESS) {
        ...
        node->pat_node_name.key_info = nullptr;
        goto done;
      }
      break;
...
uint32_t clms_node_delete(CLMS_CLUSTER_NODE *nd, int i) {
  ...
  osafassert(nd != nullptr);

  switch (i) {
    case 0:
      if (nd->node_id != 0) {
        if ((rc = ncs_patricia_tree_del(
                 &clms_cb->id_lookup, &nd->pat_node_id)) != NCSCC_RC_SUCCESS) {
          LOG_WA("ncs_patricia_tree_del FAILED for nodename %s rc %u",
    ...
    case 1:
      ...
      if (clms_cb->reg_with_plm == SA_TRUE) {
        if ((rc = ncs_patricia_tree_del(&clms_cb->ee_lookup,
                                        &nd->pat_node_eename)) !=
            NCSCC_RC_SUCCESS) {
          LOG_WA("ncs_patricia_tree_del FAILED for nodename %s rc %u",
                 nd->node_name.value, rc);
    ...
    case 2:
      if ((rc = ncs_patricia_tree_del(
               &clms_cb->nodes_db, &nd->pat_node_name)) != NCSCC_RC_SUCCESS) {
        LOG_WA("ncs_patricia_tree_del FAILED for nodename %s rc %u",
               nd->node_name.value, rc);
        goto done;
      }
  ...
  if (!strncmp((char *)objName->value, "safNode=", 8)) {
    ...
    if (tmpstr != nullptr)
      if (!strncmp(++tmpstr, "safCluster=", 11)) {
        if (!strncmp(tmpstr, (char *)osaf_cluster->name.value,
                     osaf_cluster->name.length)) {
          TRACE("Node is child of our cluster %s", osaf_cluster->name.value);
          TRACE_LEAVE();
          return NCSCC_RC_SUCCESS;
        } else {
          LOG_ER("Node is not the child of our cluster %s",
...
uint32_t clms_nodedb_lookup(int i) {
  ...
  while (nullptr != (node = clms_node_getnext_by_id(nodeid))) {
    ...
    if (i == 0) {
      if (node->stat_change == SA_TRUE) num_nd_changes++;
    } else {
 ...
 */
SaClmClusterNotificationT_4 *clms_notbuffer_changes_only(
  ...
  while ((nullptr != (node = clms_node_getnext_by_id(nodeid))) && (i < num)) {
    nodeid = node->node_id;
...
uint32_t clms_client_del_trackresp(SaUint32T client_id) {
  CLMS_CLUSTER_NODE *node = nullptr;
  SaUint32T nodeid = 0;
  CLMS_TRACK_INFO *trkrsp_rec = nullptr;
  ...
  while (nullptr != (node = clms_node_getnext_by_id(nodeid))) {
    ...
    if (ncs_patricia_tree_size(&node->trackresp) != 0) {
      trkrsp_rec = (CLMS_TRACK_INFO *)ncs_patricia_tree_getnext(
          &node->trackresp, (uint8_t *)0);
      while (trkrsp_rec != nullptr) {
        ...
        if (trkrsp_rec->client_id == client_id) {
          if (ncs_patricia_tree_size(&node->trackresp) == 1) {
            ...
            if (NCSCC_RC_SUCCESS !=
                ncs_patricia_tree_del(&node->trackresp,
                                      &trkrsp_rec->pat_node)) {
              LOG_ER("ncs_patricia_tree_del FAILED for client_id %u",
                     client_id);
              rc = NCSCC_RC_FAILURE;
              goto done;
            }
...
uint32_t clms_node_trackresplist_empty(CLMS_CLUSTER_NODE *op_node) {
  ...
  trkrec = (CLMS_TRACK_INFO *)ncs_patricia_tree_getnext(&op_node->trackresp,
                                                        (uint8_t *)0);
  while (trkrec != nullptr) {
    inv_id = trkrec->inv_id;
    if (NCSCC_RC_SUCCESS !=
        ncs_patricia_tree_del(&op_node->trackresp, &trkrec->pat_node)) {
...
void clms_clmresp_error_timeout(CLMS_CB *cb, CLMS_CLUSTER_NODE *node) {
  ...
  ckpt_node_rec(node);
  ckpt_cluster_rec();
  immutil_saImmOiAdminOperationResult(cb->immOiHandle, node->curr_admin_inv,
...
uint32_t clms_clmresp_rejected(CLMS_CB *cb, CLMS_CLUSTER_NODE *node,
        ...
        if (client->track_flags & SA_TRACK_VALIDATE_STEP) {
          ais_er =
              saPlmReadinessTrackResponse(cb->ent_group_hdl, node->plm_invid,
                                          SA_PLM_CALLBACK_RESPONSE_REJECTED);
          if (ais_er != SA_AIS_OK)
  ...
  if (rc != NCSCC_RC_SUCCESS) TRACE("clms_node_trackresplist_empty FAILED");
    ...
    case IMM_LOCK: {
      ...
      if (clms_cb->reg_with_plm == SA_TRUE)
        clms_reboot_remote_node(
            node, "client responded with error during admin lock");
      break;
    }
    case IMM_SHUTDOWN: {
      clms_clmresp_error_timeout(cb, node);
      ...
      if (clms_cb->reg_with_plm == SA_TRUE)
        clms_reboot_remote_node(
            node, "client responded with error during admin shutdown");
      break;
    }
    case PLM: {
#ifdef ENABLE_AIS_PLM
...
void *clms_rem_reboot(void *_reboot_info) {
  ...
  opensaf_reboot(rem_reb->node->node_id, (char *)rem_reb->node->ee_name.value,
                 (char *)rem_reb->str);
  free(rem_reb->str);
  free(rem_reb);
...
void clms_reboot_remote_node(CLMS_CLUSTER_NODE *op_node, const char *str) {
  ...
  CLMS_REM_REBOOT_INFO *rem_reb =
      static_cast<CLMS_REM_REBOOT_INFO *>(malloc(sizeof(CLMS_REM_REBOOT_INFO)));
...
uint32_t clms_clmresp_ok(CLMS_CB *cb, CLMS_CLUSTER_NODE *op_node,
  ...
  if (op_node->admin_op == PLM) {
    ...
    if (ncs_patricia_tree_size(&op_node->trackresp) == 0) {
      ...
      op_node->admin_op = ADMIN_OP{};
      op_node->stat_change = SA_FALSE;
      immutil_saImmOiAdminOperationResult(cb->immOiHandle,
                                          op_node->curr_admin_inv, SA_AIS_OK);
      op_node->change = SA_CLM_NODE_NO_CHANGE;

      rc = clms_send_is_member_info(clms_cb, op_node->node_id, op_node->member,
                                    SA_TRUE);
      if (rc != NCSCC_RC_SUCCESS) {
...
void ckpt_node_down_rec(CLMS_CLUSTER_NODE *node) {
  ...
  if (clms_cb->ha_state == SA_AMF_HA_ACTIVE) {
    ...
    if (async_rc != NCSCC_RC_SUCCESS) TRACE("send_async_update FAILED");
  }
  TRACE_LEAVE();
}
...
void ckpt_node_rec(CLMS_CLUSTER_NODE *node) {
  ...
  if (clms_cb->ha_state == SA_AMF_HA_ACTIVE) {
    ...
    memcpy(ckpt.param.node_rec.node_name.value, node->node_name.value,
...
uint32_t clms_send_is_member_info(CLMS_CB *cb, SaClmNodeIdT node_id,
  ...
  while (client != nullptr) {
    ...
    if (local_node_id == node_id) {
      ...
      if (rc != NCSCC_RC_SUCCESS) {
        TRACE_2("clms_mds_msg_send failed for client '%u', rc = %u",
  ...
  TRACE_LEAVE();
  if (send_failed) return NCSCC_RC_FAILURE;
  return rc;
}

/*Walk through the node db to know the number of member nodes*/
uint32_t clms_num_mem_node() {
  SaUint32T node_id = 0;
  SaUint32T i = 0;
...
bool ip_matched(uint16_t family1, uint8_t *ip1, uint16_t family2,
                uint8_t *ip2) {
  return true;
}
