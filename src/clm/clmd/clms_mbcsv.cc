...
static uint32_t ckpt_proc_reg_rec(CLMS_CB *cb, CLMS_CKPT_REC *data) {
  ...
  if (client == nullptr) {
    ...
    if ((client = clms_client_new(param->mds_dest, param->client_id)) ==
        nullptr) {
  ...
  if (node != nullptr) {
    ...
    if (clms_cb->reg_with_plm == SA_TRUE) {
      SaAisErrorT aisrc = saPlmEntityGroupAdd(
          clms_cb->ent_group_hdl, entityNames, 1, SA_PLM_GROUP_SINGLE_ENTITY);
      if (aisrc != SA_AIS_OK) {
        LOG_ER("saPlmEntityGroupAdd FAILED rc = %d", aisrc);
      }
    }
#endif
  }
...
static uint32_t ckpt_proc_node_config_rec(CLMS_CB *cb, CLMS_CKPT_REC *data) {
  ...
  memcpy(node->ee_name.value, param->ee_name.value, param->ee_name.length);
...
static uint32_t ckpt_proc_node_rec(CLMS_CB *cb, CLMS_CKPT_REC *data) {
  ...
  node->node_name.length = param->node_name.length;
  memcpy(node->node_name.value, param->node_name.value, node->node_name.length);
  ...
  if ((ip = (IPLIST *)ncs_patricia_tree_get(
    ...
    if (ip->addr.length) {
      ...
      node->node_addr.family = SA_CLM_AF_INET; /* For backward compatibility */
...
static uint32_t mbcsv_callback(NCS_MBCSV_CB_ARG *arg) {
  ...
  switch (arg->i_op) {
    ...
    case NCS_MBCSV_CBOP_PEER:
      ...
      if (rc != NCSCC_RC_SUCCESS) LOG_ER("ckpt_peer_info_cbk_handler FAILED");
      break;
    case NCS_MBCSV_CBOP_NOTIFY:
      /* NOTIFY info from CLMS peer */
      rc = ckpt_notify_cbk_handler(arg);
      if (rc != NCSCC_RC_SUCCESS) LOG_ER("ckpt_notify_cbk_handler FAILED");
      break;
    case NCS_MBCSV_CBOP_ERR_IND:
      /* Peer error indication info */
      rc = ckpt_err_ind_cbk_handler(arg);
      if (rc != NCSCC_RC_SUCCESS) LOG_ER("ckpt_err_ind_cbk_handler FAILED");
      break;
    default:
      rc = NCSCC_RC_FAILURE;
...
uint32_t encode_node_rec(NCS_UBAID *uba) {
  ...
  CLMSV_CKPT_NODE *node =
      static_cast<CLMSV_CKPT_NODE *>(malloc(sizeof(CLMSV_CKPT_NODE)));
  if (node == nullptr) {
    TRACE("Malloc Failed");
    return 0; /*dude can you return NCSCC_RC_FAILURE */
  }
...
void prepare_ckpt_to_ckpt_config_node(
    CLMSV_CKPT_NODE_CONFIG_REC *node,
...
uint32_t cluster_rec_(NCS_UBAID *uba) {
  ...
  if (enc_mbcsv_cluster_rec_msg(uba, &cluster_rec) == 0) return 0;
  return 1;
}
...
uint32_t enc_mbcsv_node_del_msg(NCS_UBAID *uba,
                                CLMSV_CKPT_NODE_DEL_REC *param) {
  uint32_t total_bytes = 0;
...
static uint32_t ckpt_decode_cold_sync(CLMS_CB *cb, NCS_MBCSV_CB_ARG *cbk_arg) {
  ...
  if (rc != NCSCC_RC_SUCCESS) {
    /* Do not allow standby to get out of sync */
    /*clms_exit("Cold sync failed", SA_AMF_COMPONENT_RESTART); */ /*dude chk it
                                                                     out */
  }
  TRACE_LEAVE();
  return rc;
...
static uint32_t ckpt_decode_async_update(CLMS_CB *cb,
  ...
  CLMS_CKPT_REC *ckpt_msg =
      static_cast<CLMS_CKPT_REC *>(calloc(1, sizeof(CLMS_CKPT_REC)));
  ...
  switch (hdr->type) {
    case CLMS_CKPT_CLIENT_INFO_REC:
      ...
      num_bytes =
          decode_client_msg(&cbk_arg->info.decode.i_uba, ckpt_client_rec);
      if (num_bytes == 0) {
    ...
    case CLMS_CKPT_FINALIZE_REC:
      ...
      num_bytes =
          decode_finalize_msg(&cbk_arg->info.decode.i_uba, ckpt_finalize_rec);
      if (num_bytes == 0) {
    ...
    case CLMS_CKPT_TRACK_CHANGES_REC:
      ...
      ckpt_client_rec = &ckpt_msg->param.client_rec;
      num_bytes = decode_track_changes_msg(&cbk_arg->info.decode.i_uba,
                                           ckpt_client_rec);
      if (num_bytes == 0) {
    ...
    case CLMS_CKPT_NODE_RUNTIME_REC:
      ...
      num_bytes =
          decode_node_rec_msg(&cbk_arg->info.decode.i_uba, ckpt_node_rec);
      if (num_bytes == 0) {
    ...
    case CLMS_CKPT_NODE_REC:
      ...
      num_bytes =
          decode_node_msg(&cbk_arg->info.decode.i_uba, ckpt_csync_node_rec);
      if (num_bytes == 0) {
    ...
    case CLMS_CKPT_NODE_DEL_REC:
      ...
      num_bytes =
          decode_node_del_msg(&cbk_arg->info.decode.i_uba, ckpt_node_del_rec);
      if (num_bytes == 0) {
    ...
    case CLMS_CKPT_AGENT_DOWN_REC:
      ...
      num_bytes =
          decode_agent_down_msg(&cbk_arg->info.decode.i_uba, ckpt_agent_down);
      if (num_bytes == 0) {
    ...
    case CLMS_CKPT_CLUSTER_REC:
      ...
      num_bytes =
          decode_cluster_msg(&cbk_arg->info.decode.i_uba, ckpt_cluster_rec);
      if (num_bytes == 0) {
        TRACE("decode_agent_down_msg FAILED");
        rc = NCSCC_RC_FAILURE;
        goto done;
    ...
    case CLMS_CKPT_NODE_DOWN_REC:
      ...
      num_bytes =
          decode_node_down_msg(&cbk_arg->info.decode.i_uba, ckpt_node_down_rec);
      if (num_bytes == 0) {
...
static uint32_t decode_node_rec_msg(NCS_UBAID *uba,
  ...
  p8 = ncs_dec_flatten_space(uba, local_data, 4);
  param->admin_state = static_cast<SaClmAdminStateT>(ncs_decode_32bit(&p8));
  ncs_dec_skip_space(uba, 4);
  total_bytes += 4;
  ...
  p8 = ncs_dec_flatten_space(uba, local_data, 4);
  param->change = static_cast<SaClmClusterChangesT>(ncs_decode_32bit(&p8));
  ncs_dec_skip_space(uba, 4);
  total_bytes += 4;
  ...
  p8 = ncs_dec_flatten_space(uba, local_data, 4);
  param->ee_red_state =
      static_cast<SaPlmReadinessStateT>(ncs_decode_32bit(&p8));
  ncs_dec_skip_space(uba, 4);
  total_bytes += 4;
#endif
