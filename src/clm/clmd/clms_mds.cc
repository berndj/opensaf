...
static uint32_t clms_enc_node_get_msg(NCS_UBAID *uba,
                                      SaClmClusterNodeT_4 *msg) {
  uint8_t *p8;
  uint32_t total_bytes = 0;
...
static uint32_t clms_enc_cluster_ntf_buf_msg(
    NCS_UBAID *uba, SaClmClusterNotificationBufferT_4 *notify_info) {
  uint8_t *p8;
  ...
  for (i = 0; i < param->numberOfItems; i++) {
    ...
    total_bytes += 4;
    total_bytes +=
        clms_enc_node_get_msg(uba, &param->notification[i].clusterNode);
  }
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
static uint32_t clms_enc_node_async_get_cbk_msg(NCS_UBAID *uba,
                                                CLMSV_MSG *msg) {
  uint8_t *p8;
  uint32_t total_bytes = 0;
...
uint32_t clms_mds_enc(struct ncsmds_callback_info *info) {
  ...
  if (CLMSV_CLMS_TO_CLMNA_REBOOT_MSG == msg->evt_type ||
    ...
    total_bytes += clmsv_encodeSaNameT(uba, &(msg->info.action_info.action));
  } else {
...
uint32_t clms_mds_dec(struct ncsmds_callback_info *info) {
  ...
  if (nullptr == (evt = static_cast<CLMSV_CLMS_EVT *>(
  ...
  info->info.dec.o_msg = reinterpret_cast<uint8_t *>(evt);

  p8 = ncs_dec_flatten_space(uba, local_data, 4);
  evt->info.msg.evt_type = static_cast<CLMSV_MSG_TYPE>(ncs_decode_32bit(&p8));
  ncs_dec_skip_space(uba, 4);
  total_bytes += 4;
  ...
  if (CLMSV_CLMA_TO_CLMS_API_MSG == evt->info.msg.evt_type) {
    ...
    evt->info.msg.info.api_info.type =
        static_cast<CLMSV_API_MSG_TYPE>(ncs_decode_32bit(&p8));
    ncs_dec_skip_space(uba, 4);
    total_bytes += 4;
...
static uint32_t clms_mds_node_event(struct ncsmds_callback_info *mds_info) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  CLMSV_CLMS_EVT *clmsv_evt = nullptr;
  uint32_t node_id = 0;
  IPLIST *ip = nullptr;
  TRACE_ENTER();
...
static uint32_t clms_mds_rcv(struct ncsmds_callback_info *mds_info) {
  ...
  /* Send the message to clms */
  rc = m_NCS_IPC_SEND(
      &clms_cb->mbx, clmsv_evt,
      static_cast<NCS_IPC_PRIORITY>(mds_info->info.receive.i_priority));
  if (rc != NCSCC_RC_SUCCESS) TRACE("IPC send failed %d", rc);
...
static uint32_t clms_mds_quiesced_ack(struct ncsmds_callback_info *mds_info) {
  ...
  if (clms_cb->is_quiesced_set != true) return NCSCC_RC_SUCCESS;
...
static uint32_t clms_mds_svc_event(struct ncsmds_callback_info *info) {
  ...
  if (info->info.svc_evt.i_change == NCSMDS_DOWN) {
    ...
    if (evt->type) {
      /* Push the event and we are done */
      if (m_NCS_IPC_SEND(&clms_cb->mbx, evt, NCS_IPC_PRIORITY_HIGH) !=
          NCSCC_RC_SUCCESS) {
        TRACE("ipc send failed");
        free(evt);
        rc = NCSCC_RC_FAILURE;
...
uint32_t clms_mds_init(CLMS_CB *cb) {
  ...
  /* Set the role of MDS */
  if (cb->ha_state == SA_AMF_HA_ACTIVE) cb->mds_role = V_DEST_RL_ACTIVE;

  if (cb->ha_state == SA_AMF_HA_STANDBY) cb->mds_role = V_DEST_RL_STANDBY;
...
uint32_t clms_mds_msg_send(CLMS_CB *cb, CLMSV_MSG *msg, MDS_DEST *dest,
  ...
  if (nullptr == mds_ctxt || 0 == mds_ctxt->length) {
    /* regular send */
  ...
  /* send the message */
  rc = ncsmds_api(&mds_info);
  if (rc != NCSCC_RC_SUCCESS) LOG_IN("mds send returned: %u", rc);
