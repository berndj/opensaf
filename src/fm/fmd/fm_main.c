...
int main(int argc, char *argv[])
{
	...
	if ((rc = initialize_for_assignment(
		 fm_cb, (SaAmfHAStateT)fm_cb->role)) != NCSCC_RC_SUCCESS) {
	...
	/* USR1/AMF fd */
	fds[FD_AMF].fd =
	    nid_started ? usr1_sel_obj.rmv_obj : fm_cb->fm_amf_cb.amf_fd;
	...
	while (1) {
		...
		if (fds[FD_AMF].revents & POLLIN) {
			if (fm_cb->fm_amf_cb.amf_hdl != 0) {
				SaAisErrorT error;
				TRACE("AMF event rec");
				if ((error = saAmfDispatch(
					 fm_cb->fm_amf_cb.amf_hdl,
...
static SaAisErrorT get_peer_clm_node_name(NODE_ID node_id)
{
	...
	if ((rc = saClmClusterNodeGet_4(fm_cb->clm_hdl, node_id,
					FM_CLM_API_TIMEOUT, &cluster_node)) ==
	    SA_AIS_OK) {
...
static void fm_mbx_msg_handler(FM_CB *fm_cb, FM_EVT *fm_mbx_evt)
{
	...
	case FM_EVT_NODE_DOWN:
		...
		if ((fm_mbx_evt->node_id == fm_cb->peer_node_id)) {
			...
			if ((fm_cb->role == PCS_RDA_STANDBY) &&
				 ...
				 *be set to ACTIVE.
				 */
				if ((SaAmfHAStateT)fm_cb->role !=
				    fm_cb->amf_state)
					TRACE(
					    "Failover processing trigerred in the middle of role change");
				...
				if (!((fm_cb->role == PCS_RDA_ACTIVE) &&
					...
					LOG_NO(
					    "Controller Failover: Setting role to ACTIVE");
	...
	case FM_EVT_PEER_UP:
		...
		if ((fm_cb->role == PCS_RDA_ACTIVE) &&
		    (fm_cb->csi_assigned == false)) {
			LOG_WA(
			    "Two active controllers observed in a cluster, newActive: %x and old-Active: %x",
...
static void fm_evt_proc_rda_callback(FM_CB *cb, FM_EVT *evt)
{
	...
	if (evt->info.rda_info.role == PCS_RDA_ACTIVE &&
	    cb->role != PCS_RDA_ACTIVE && cb->amf_state != SA_AMF_HA_ACTIVE &&
	...
	if ((rc = initialize_for_assignment(
		 cb, (SaAmfHAStateT)evt->info.rda_info.role)) !=
	    NCSCC_RC_SUCCESS) {
		LOG_ER("initialize_for_assignment FAILED %u", (unsigned)rc);
...
static uint32_t fms_fms_exchange_node_info(FM_CB *fm_cb)
{
	...
	if (fm_cb->peer_adest != 0) {
		...
		gfm_msg.info.node_info.node_name.length =
		    fm_cb->node_name.length;
		memcpy(gfm_msg.info.node_info.node_name.value,
		       fm_cb->node_name.value, fm_cb->node_name.length);
