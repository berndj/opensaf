...
void saPlmEntityGroupAdd_01(void)
{
	...
	safassert(saPlmEntityGroupCreate(plmHandle, &entityGroupHandle),
		  SA_AIS_OK);
	rc = saPlmEntityGroupAdd(entityGroupHandle, &f120_slot_1_dn,
...
void saPlmEntityGroupAdd_11(void)
{
	...
	safassert(saPlmEntityGroupCreate(plmHandle, &entityGroupHandle),
		  SA_AIS_OK);
	rc = saPlmEntityGroupAdd(-1, &f120_slot_1_dn, entityNamesNumber,
				 SA_PLM_GROUP_SINGLE_ENTITY);
	test_validate(rc, SA_AIS_ERR_BAD_HANDLE);
	safassert(saPlmFinalize(plmHandle), SA_AIS_OK);
}
...
void saPlmEntityGroupAdd_13(void)
{
	...
	safassert(saPlmEntityGroupCreate(plmHandle, &entityGroupHandle),
		  SA_AIS_OK);
	/* Nonexistent DN*/
	rc = saPlmEntityGroupAdd(entityGroupHandle, &f120_nonexistent,
				 entityNamesNumber, SA_PLM_GROUP_SINGLE_ENTITY);
...
void saPlmEntityGroupAdd_14(void)
{
	...
	safassert(saPlmEntityGroupCreate(plmHandle, &entityGroupHandle),
		  SA_AIS_OK);
	rc = saPlmEntityGroupAdd(entityGroupHandle, &f120_slot_1_eedn,
				 entityNamesNumber,
				 SA_PLM_GROUP_SUBTREE_HES_ONLY);
	test_validate(rc, SA_AIS_OK);
...
void saPlmEntityGroupAdd_16(void)
{
	...
	rc = saPlmEntityGroupAdd(entityGroupHandle, &f120_slot_1_dn,
				 entityNamesNumber, SA_PLM_GROUP_SUBTREE);
	rc = saPlmEntityGroupAdd(entityGroupHandle, &amc_slot_1_dn,
				 entityNamesNumber, SA_PLM_GROUP_SUBTREE);
	test_validate(rc, SA_AIS_ERR_EXIST);
	safassert(saPlmFinalize(plmHandle), SA_AIS_OK);
}
