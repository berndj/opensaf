...
void saPlmEntityGroupRemove_01(void)
{
	...
	safassert(saPlmEntityGroupAdd(entityGroupHandle, &f120_slot_1_dn,
		  ...
		  SA_AIS_OK);
	rc = saPlmEntityGroupRemove(entityGroupHandle, &f120_slot_1_dn,
				    entityNamesNumber);
...
void saPlmEntityGroupRemove_08(void)
{
	...
	safassert(saPlmEntityGroupAdd(entityGroupHandle, &f120_slot_1_dn,
				      ...
				      SA_PLM_GROUP_SINGLE_ENTITY),
		  SA_AIS_OK);
	rc = saPlmEntityGroupRemove(entityGroupHandle, &zerosize_dn,
				    entityNamesNumber);
...
void saPlmEntityGroupRemove_12(void)
{
	...
	safassert(saPlmEntityGroupCreate(plmHandle, &entityGroupHandle),
		  SA_AIS_OK);
	rc = saPlmEntityGroupRemove(entityGroupHandle, &f120_nonexistent,
				    entityNamesNumber);
	test_validate(rc, SA_AIS_ERR_NOT_EXIST);
	safassert(saPlmFinalize(plmHandle), SA_AIS_OK);
}
...
void saPlmEntityGroupRemove_14(void)
{
	...
	safassert(saPlmEntityGroupAdd(entityGroupHandle, &f120_slot_1_dn,
				      entityNamesNumber, SA_PLM_GROUP_SUBTREE),
		  SA_AIS_OK);
	rc = saPlmEntityGroupRemove(entityGroupHandle, &amc_slot_1_dn,
				    entityNamesNumber);
	test_validate(rc, SA_AIS_ERR_INVALID_PARAM);
	safassert(saPlmFinalize(plmHandle), SA_AIS_OK);
}
