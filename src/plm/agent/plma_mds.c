...
uint32_t plma_mds_callback(struct ncsmds_callback_info *info)
{
	...
	case MDS_CALLBACK_ENC_FLAT:
		if (0) {
			...
			rc = plms_mds_enc(&info->info.enc_flat,
					  &plma_cb->edu_hdl);
		}
		break;
	case MDS_CALLBACK_DEC_FLAT:
		if (1) {
			...
			rc = plms_mds_dec(&info->info.dec_flat,
					  &plma_cb->edu_hdl);
		}
		break;
	...
	if (pEvt->req_res == PLMS_REQ) {
		if (pEvt->req_evt.req_type == PLMS_AGENT_TRACK_EVT_T) {
			if (pEvt->req_evt.agent_track.evt_type ==
				...
				grp_info = (PLMA_ENTITY_GROUP_INFO *)
				...
				rc = m_NCS_IPC_SEND(
				    &grp_info->client_info->callbk_mbx,
