...
uint32_t plm_process_single_dispatch(PLMA_CLIENT_INFO *client_info)
{
	...
	if (NULL != (msg = (PLMS_EVT *)m_NCS_IPC_NON_BLK_RECEIVE(
		...
		if ((msg->req_res == PLMS_REQ) &&
			...
			group_info =
			    (PLMA_ENTITY_GROUP_INFO *)ncs_patricia_tree_get(
				&plma_cb->entity_group_info,
				(uint8_t *)&grp_hdl);
			if (!group_info) {
			...
			if (group_info->trk_strt_stop) {
				if (msg->req_evt.agent_track.evt_type ==
					...
					if (client_info->track_callback
						.saPlmReadinessTrackCallback) {
						/** Invoke the callback */
						client_info->track_callback
						    .saPlmReadinessTrackCallback(
							grp_hdl,
							msg->req_evt.agent_track
							    .track_cbk
							    .track_cookie,
							msg->req_evt.agent_track
							    .track_cbk
							...
							SA_AIS_OK);
						TRACE_5(
						    "PLMA: Callback invoked. Dispatch flag : SA_DISPATCH_ONE");
					}
				}
			}
		} else {
				...
				if (msg->req_evt.agent_track.evt_type ==
					...
					if (client_info->track_callback
						...
						client_info->track_callback
							...
							SA_AIS_OK);
						TRACE_5(
						    "PLMA: Callback invoked. Dispatch flag : SA_DISPATCH_ALL");
					}
				}
			}
		} else {
...
uint32_t plm_process_blocking_dispatch(PLMA_CLIENT_INFO *client_info)
{
	...
	/** Receive a message from the mailbox */
	while (NULL != (msg = (PLMS_EVT *)m_NCS_IPC_RECEIVE(
			    &client_info->callbk_mbx, NULL))) {
		/** Process the message here */
		if ((msg->req_res == PLMS_REQ) &&
		    (PLMS_AGENT_TRACK_EVT_T == msg->req_evt.req_type)) {
				...
				if (msg->req_evt.agent_track.evt_type ==
					...
					if (client_info->track_callback
						...
						client_info->track_callback
							...
							SA_AIS_OK);
						TRACE_5(
						    "PLMA: Callback invoked. Dispatch flag : SA_DISPATCH_BLOCKING");
					}
				}
			}
		} else {
...

{
	...
	TRACE_5(" Version received: %c %d %d", version->releaseCode,
	...
	proc_rc = plm_mds_msg_sync_send(plma_cb->mds_hdl, NCSMDS_SVC_ID_PLMA,
					NCSMDS_SVC_ID_PLMS,
					plma_cb->plms_mdest_id, &plm_init_evt,
					&plm_init_resp, PLMS_MDS_SYNC_TIME);
				  ...
				  SaSelectionObjectT *selectionObject)
{
	...
	client_info = (PLMA_CLIENT_INFO *)ncs_patricia_tree_get(
	    &plma_cb->client_info, (uint8_t *)&plmHandle);
	/** Release lock for CB */
...
{
	PLMA_CB *plma_cb = plma_ctrlblk;
	PLMA_ENTITY_GROUP_INFO_LIST *entity_grp_list =
	    client_node->grp_info_list;
	PLMA_ENTITY_GROUP_INFO_LIST *list_next;

	TRACE_ENTER();

	while (entity_grp_list) {
		PLMA_RDNS_TRK_MEM_LIST *start =
		    entity_grp_list->entity_group_info->rdns_trk_mem_list;
...
SaAisErrorT saPlmFinalize(SaPlmHandleT plmHandle)
{
	...
	proc_rc = plm_mds_msg_sync_send(plma_cb->mds_hdl, NCSMDS_SVC_ID_PLMA,
					NCSMDS_SVC_ID_PLMS,
					plma_cb->plms_mdest_id, &plm_fin_evt,
					&plm_fin_resp, PLMS_MDS_SYNC_TIME);

	/** Check if return value is SUCCESS or not */
	if (NCSCC_RC_SUCCESS != proc_rc) {
		LOG_ER(
		    "PLMA : plm_mds_msg_sync_send FOR FINALIZE REQUEST FAILED");
		rc = SA_AIS_ERR_TRY_AGAIN;
		goto end;
	}
	...
	/** Traverse through the group info list and free all entitiy group
	 * info's */
			    ...
			    PLMA_ENTITY_GROUP_INFO *entityNew)
{
	...
	TRACE_ENTER();
	new = (PLMA_ENTITY_GROUP_INFO_LIST *)malloc(
	    sizeof(PLMA_ENTITY_GROUP_INFO_LIST));
	if (!new) {
				   ...
				   SaPlmEntityGroupHandleT *groupHandle)
{
	...
	group_info =
	    (PLMA_ENTITY_GROUP_INFO *)calloc(1, sizeof(PLMA_ENTITY_GROUP_INFO));
	if (!group_info) {
	...
	if (NCSCC_RC_SUCCESS != proc_rc) {
		LOG_ER(
		    "PLMA: plm_mds_msg_sync_send FOR GRP CREATE REQUEST FAILED");
		rc = SA_AIS_ERR_TRY_AGAIN;
		goto mds_fail;
	}
	...
	if (!plm_out_res) {
		LOG_ER(
		    "PLMA: plm_mds_msg_sync_send RETURNS WITH INVALID RESPONSE");
		rc = SA_AIS_ERR_TRY_AGAIN;
		goto resp_err;
	} else {
		/** process the response */
		...
		*groupHandle = plm_out_res->res_evt.hdl;
		...
		add_entity_grp_to_list(&client_info->grp_info_list, group_info);
		...
		group_info->pat_node.key_info =
		    (uint8_t *)&group_info->entity_group_handle;
				...
				SaPlmGroupOptionsT options)
{
		...
		for (ii = 0; ii < entityNamesNumber; ii++) {
			...
			if (i != ii) {
				if ((entityNames[i].length ==
					...
					LOG_ER(
					    "PLMA: ENTITY APPREARS MORE THAN ONCE IN entityNames ARRAY");
					rc = SA_AIS_ERR_EXIST;
					goto end;
				}
			}
	...
	if (NCSCC_RC_SUCCESS != proc_rc) {
		LOG_ER(
		    "PLMA: plm_mds_msg_sync_send FOR GRP ADD REQUEST FAILED");
		rc = SA_AIS_ERR_TRY_AGAIN;
		goto end;
	}
				   ...
				   SaUint32T entityNamesNumber)
{
	...
	if (NCSCC_RC_SUCCESS != proc_rc) {
		LOG_ER(
		    "PLMA: plm_mds_msg_sync_send FOR GRP REM REQUEST FAILED");
		rc = SA_AIS_ERR_TRY_AGAIN;
		goto end;
	}
...
void clean_group_info_node(PLMA_ENTITY_GROUP_INFO *group_info)
{
	...
	PLMA_ENTITY_GROUP_INFO_LIST *grp_info_list =
	    group_info->client_info->grp_info_list;
...
SaAisErrorT saPlmEntityGroupDelete(SaPlmEntityGroupHandleT entityGroupHandle)
{
	...
	if (group_info->is_trk_enabled) {
		LOG_ER(
		    "PLMA: Track stop is not yet called. Can not delete the group.");
		rc = SA_AIS_ERR_BUSY;
		goto end;
	}
	memset(&plm_in_evt, 0, sizeof(PLMS_EVT));
	...
	if (NCSCC_RC_SUCCESS != proc_rc) {
		LOG_ER(
		    "PLMA: plm_mds_msg_sync_send FOR GRP DEL REQUEST FAILED");
		rc = SA_AIS_ERR_TRY_AGAIN;
		goto end;
	}
				...
				SaPlmReadinessTrackedEntityT *entity)
{
	...
	if (!new) {
		...
		LOG_CR(
		    "PLMA : PLMA_RDNS_TRK_MEM_LIST memory alloc failed, error val:%s",
		    strerror(errno));
		return;
	}
	new->entities = entity;
				...
				SaPlmReadinessTrackedEntitiesT *trackedEntities)
{
	...
	if (m_PLM_IS_SA_TRACK_CURRENT_SET(trackFlags)) {
		if (trackedEntities) {
			if ((trackedEntities->entities != NULL) &&
				...
				LOG_ER(
				    "PLMA: INVALID INPUT PARAM. entities pointer != NULL and numberOfEntities = 0 ");
				rc = SA_AIS_ERR_INVALID_PARAM;
				goto end;
			}
		}
	...
	if ((!m_PLM_IS_SA_TRACK_CHANGES_SET(trackFlags) &&
		...
		TRACE_5("Flags after masking the validate and start step : %x",
			trackFlags);
	}
	...
	if ((m_PLM_IS_SA_TRACK_CHANGES_SET(trackFlags) ||
	     m_PLM_IS_SA_TRACK_CHANGES_ONLY_SET(trackFlags))) {
		if (!group_info->client_info->track_callback
			 .saPlmReadinessTrackCallback) {
	...
	plm_in_evt.req_evt.agent_track.grp_handle =
	    group_info->entity_group_handle;
	...
	if (m_PLM_IS_SA_TRACK_CURRENT_SET(trackFlags) && (trackedEntities)) {
		plm_in_evt.req_evt.agent_track.track_start.no_of_entities =
		...
		proc_rc = plm_mds_msg_sync_send(
		    ...
		    plma_cb->plms_mdest_id, &plm_in_evt, &plm_out_res,
		    PLMS_MDS_SYNC_TIME);

		if (NCSCC_RC_SUCCESS != proc_rc) {
			LOG_ER(
			    "PLMA: plm_mds_msg_sync_send FOR READINESS TRACK REQUEST FAILED");
			rc = SA_AIS_ERR_TRY_AGAIN;
			goto end;
		}
		if ((m_PLM_IS_SA_TRACK_CHANGES_SET(trackFlags) ||
		...
		if (!plm_out_res) {
			LOG_ER(
			    "PLMA:INVALID RESPONSE FOR READINESS TRACK REQUEST");
			rc = SA_AIS_ERR_TRY_AGAIN;
			goto end;
		}
		if (plm_out_res->res_evt.error != SA_AIS_OK) {
			if (SA_AIS_ERR_NO_SPACE == plm_out_res->res_evt.error)
				trackedEntities->numberOfEntities =
				    plm_out_res->res_evt.entities
					->numberOfEntities;

			LOG_ER(
			    "PLMA: ERR RESPONSE FOR READINESS TRACK REQUEST");
			rc = plm_out_res->res_evt.error;
			goto end;
		}
	} else { /** for rest of the flags, Get the response in callback */
		proc_rc = plms_mds_normal_send(
		    plma_cb->mds_hdl, NCSMDS_SVC_ID_PLMA, &plm_in_evt,
		    plma_cb->plms_mdest_id, NCSMDS_SVC_ID_PLMS);
		if (NCSCC_RC_SUCCESS != proc_rc) {
			LOG_ER(
			    "PLMA: plms_mds_normal_send FOR READINESS TRACK REQUEST FAILED");
			rc = SA_AIS_ERR_TRY_AGAIN;
			goto end;
		}
		group_info->trk_strt_stop = 1;
	...
	if (trackedEntities) {
		...
		if ((!trackedEntities->entities) && (entities_num)) {
			trackedEntities->entities =
			    malloc(entities_num *
				   sizeof(SaPlmReadinessTrackedEntityT));
			add_entities_to_track_list(
			    &group_info->rdns_trk_mem_list,
			    trackedEntities->entities);
		}
					...
					SaPlmReadinessTrackResponseT response)
{
	...
	/* Search for entity group info using the group handle */
	group_info = (PLMA_ENTITY_GROUP_INFO *)ncs_patricia_tree_get(
	    &plma_cb->entity_group_info, (uint8_t *)&entityGrpHdl);
	m_NCS_UNLOCK(&plma_cb->cb_lock, NCS_LOCK_READ);
	...
	if (NCSCC_RC_SUCCESS != proc_rc) {
		LOG_ER(
		    "PLMA: plms_mds_normal_send FOR READINESS TRACK RESPONSE REQUEST FAILED");
		rc = SA_AIS_ERR_TRY_AGAIN;
		goto end;
	}
end:
...
SaAisErrorT saPlmReadinessTrackStop(SaPlmEntityGroupHandleT entityGroupHandle)
{
	...
	if (!plm_out_res) {
		LOG_ER(
		    "PLMA:INVALID RESPONSE FOR READINESS TRACK STOP REQUEST");
		rc = SA_AIS_ERR_TRY_AGAIN;
		goto end;
	}
	if (plm_out_res->res_evt.error != SA_AIS_OK) {
			       ...
			       SaPlmReadinessTrackedEntityT *entities)
{
	...
	proc_rc =
	    validate_entities_ptr(&group_info->rdns_trk_mem_list, entities);
	if (NCSCC_RC_SUCCESS != proc_rc) {
		LOG_ER("PLMA: INVALID ENTITIES INPUT PARAM");
		rc = SA_AIS_ERR_INVALID_PARAM;
		goto end;
				       ...
				       SaNtfCorrelationIdsT *correlationIds)
{
	...
	if (impactedEntity->length > SA_MAX_NAME_LENGTH) {
		LOG_ER("Invalid length set for the impactedEntity %s",
	...
	plm_in_evt.req_evt.agent_track.readiness_impact.impacted_entity =
	    (SaNameT *)impactedEntity;
	...
	if (NCSCC_RC_SUCCESS != proc_rc) {
		LOG_ER(
		    "PLMA: plm_mds_msg_sync_send FOR READINESS IMPACT FAILED");
		rc = SA_AIS_ERR_TRY_AGAIN;
		goto end;
	}
