...
/*****************************************************************************
  ...
  DESCRIPTION:
  This include file contains PLMS Control block and entity related data
structures

******************************************************************************/
#ifndef PLM_COMMON_PLMS_H_
#define PLM_COMMON_PLMS_H_
...
#define m_PLM_IS_SA_TRACK_CURRENT_SET(attr) (attr & SA_TRACK_CURRENT)
#define m_PLM_IS_SA_TRACK_CHANGES_SET(attr) (attr & SA_TRACK_CHANGES)
#define m_PLM_IS_SA_TRACK_CHANGES_ONLY_SET(attr) (attr & SA_TRACK_CHANGES_ONLY)
#define m_PLM_IS_SA_TRACK_START_STEP_SET(attr) (attr & SA_TRACK_START_STEP)
...
#define m_PLM_IS_SA_PLM_GROUP_SUBTREE_SET(attr) (attr & SA_PLM_GROUP_SUBTREE)
...
#define PLMS_WRT_PLMA_SUBPART_VER_RANGE \
  (PLMS_WRT_PLMA_SUBPART_VER_MIN - PLMS_WRT_PLMA_SUBPART_VER_MAX + 1)
...
#define m_PLMS_GET_AMF_VER(amf_ver) \
...
typedef enum { PLMS_HE_ENTITY, PLMS_EE_ENTITY } PLMS_ENTITY_TYPE;
...
/* PLM_ENTITY is the wrapper structure for SaPlmHeT and SaPlmEeT defined by the
   spec It contains other common fields related to HE and EE. It also contains
   parent_list and child_list which will help in building containment hierarchy,
   dependency_list points to the list of entities that this entity is dependent
   on and rev_dependency_list points to the list of entities that are dependent
   on this entity
*/
struct plms_entity {
  NCS_PATRICIA_NODE pat_node;
...
/* PLMS_CLIENT_INFO holds the database of all clients that have subscribed with
 * PLM */
