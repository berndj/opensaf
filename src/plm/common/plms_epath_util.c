...
static SaUint32T
convert_entity_types(const SaHpiEntityPathT *entity_path, SaInt8T *ent_path_str,
				       ...
				       SaInt8T **ent_path_str)
{
	...
	for (i = 0; i < SAHPI_MAX_ENTITY_PATH; i++) {
		...
		if (entity_path->Entry[i].EntityType <
		...
		if (SAHPI_ENT_RACK == entity_path->Entry[i].EntityType) {
		     ...
		     SaUint32T index_array[SAHPI_MAX_ENTITY_PATH])
{
	...
	for (i = 0; i < SAHPI_MAX_ENTITY_PATH; i++) {
		memcpy(ent_path_str,
		       hpi_ent_type_list[index_array[i]].etype_str,
				  ...
				  SaHpiEntityPathT *epath_ptr)
{
	...
	if (NULL == (epath = (SaInt8T *)malloc(epath_len))) {
		LOG_CR("Failed to allocate memory error: %s", strerror(errno));
		return NCSCC_RC_FAILURE;
	}
	...
	while (tok != NULL) {
		...
		**/
		num_entity_types =
		    sizeof(hpi_ent_type_list) / sizeof(PLMS_ENTITY_TYPE_LIST);
		for (entity_type_index = 0;
		     entity_type_index < num_entity_types;
		     entity_type_index++) {
			if (0 ==
			    strcmp(
				tok,
				hpi_ent_type_list[entity_type_index].etype_str))
				break;
		}
		...
		if (num_entity_types == entity_type_index) {
			...
			entity_type =
			    hpi_ent_type_list[entity_type_index].etype_val;
		}
		...
		*ptr = '\0';
		...
		sscanf(tok, "%d",
		       &epath_ptr->Entry[entity_index].EntityLocation);
		++entity_index;
		if ((tok = ptr + 1) >= end_char)
			break;
