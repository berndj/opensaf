...
Description: This File contains the Common utility routines used by PLMS and
PLMA
	...
	if (evt->req_res == PLMS_REQ) {
		...
		case PLMS_AGENT_TRACK_EVT_T:
			...
			case PLMS_AGENT_TRACK_READINESS_IMPACT_EVT:
				if (evt->req_evt.agent_track.readiness_impact
					.impacted_entity) {
					free(evt->req_evt.agent_track
						 .readiness_impact
						 .impacted_entity);
					/* FIXME see if correlation ids needs to
					 * be freed */
				}
				free(evt);
				break;
			...
			case PLMS_AGENT_TRACK_CBK_EVT:
				if (evt->req_evt.agent_track.track_cbk
					.tracked_entities.entities) {
					free(evt->req_evt.agent_track.track_cbk
						 .tracked_entities.entities);
				}
				free(evt);
				break;
		...
		case PLMS_PLMC_EVT_T:
			...
			case PLMS_PLMC_EE_OS_RESP:
				if (evt->req_evt.plms_plmc_evt.ee_os_info
					.version) {
					free(evt->req_evt.plms_plmc_evt
						 .ee_os_info.version);
				}
				if (evt->req_evt.plms_plmc_evt.ee_os_info
					.product) {
