...
SaUint32T plms_hsm_initialize(PLMS_HPI_CONFIG *hpi_cfg)
{
	...
	cb->extr_pending_timeout =
	    (SaHpiTimeT)hpi_cfg->extr_pending_timeout * 1000000000;

	cb->insert_pending_timeout =
	    (SaHpiTimeT)hpi_cfg->insert_pending_timeout * 1000000000;

	/*Set the auto-insert timeout which is domain specific and applies to
	 * all the reource which support managed hot swap */
	hpirc =
	    saHpiAutoInsertTimeoutSet(session_id, cb->insert_pending_timeout);
	if (SA_OK != hpirc) {
				 ...
				 PLMS_HPI_STATE_MODEL *model)
{
	...
	do {
		...
		SaErrorT hpirc = saHpiGetIdByEntityPath(
		    cb->session_id, *epath_ptr, SAHPI_NO_RECORD, &instanceId,
		    &resourceId, &instrumentId, &rptUpdateCount);
		...
		if (hpirc != SA_OK) {
			...
			if (convert_entitypath_to_string(epath_ptr,
				...
				for (i = 0; i < SAHPI_MAX_ENTITY_PATH; i++) {
					sprintf(&entityPathStr[strlen(
						    entityPathStr)],
						"%i",
						epath_ptr->Entry[i].EntityType);

					sprintf(
					    &entityPathStr[strlen(
						entityPathStr)],
					    ".%i",
		...
		hpirc = saHpiRptEntryGetByResourceId(cb->session_id, resourceId,
						     &rptEntry);
				  ...
				  SaHpiRptEntryT *rpt_entry)
{
	...
	if (event_type == SAHPI_ET_USER) {
		rc = saNtfStateChangeNotificationAllocate(
		    hsmcb->plm_ntf_hdl, &plm_notification,
		    no_of_corr_notifications, 0, 2, 0,
		    SA_NTF_ALLOC_SYSTEM_LIMIT);
	} else {
		rc = saNtfStateChangeNotificationAllocate(
		    hsmcb->plm_ntf_hdl, &plm_notification,
		    no_of_corr_notifications, 0, 3, 0,
		    SA_NTF_ALLOC_SYSTEM_LIMIT);
	}
	if (SA_AIS_OK != rc) {
	...
	*(plm_notification.notificationHeader.eventTime) =
	...
	plm_notification.notificationHeader.additionalInfo[0].infoType =
	    SA_NTF_VALUE_UINT32;
	plm_notification.notificationHeader.additionalInfo[0]
	    .infoValue.uint32Val = cb->domain_id;
	...
	ret = saNtfPtrValAllocate(
	    plm_notification.notificationHandle, sizeof(SaHpiEventT),
	    (void **)&dest_ptr1,
	...
	if (event_type != SAHPI_ET_USER) {
		...
		ret = saNtfPtrValAllocate(
		    plm_notification.notificationHandle, sizeof(SaHpiRptEntryT),
		    (void **)&dest_ptr2,
		...
		if (ret != SA_AIS_OK) {
			saNtfNotificationFree(
			    plm_notification.notificationHandle);
			return ret;
		}
...
static void *plms_hsm(void)
{
	...
	if (!plms_cb->plmc_initialized &&
	    plms_cb->ha_state == SA_AMF_HA_ACTIVE) {
	...
	while (true) {
		ret = saHpiEventGet(cb->session_id, SAHPI_TIMEOUT_BLOCK, &event,
				    &rdr, &rpt_entry, NULL);
		...
		if (rc) {
			LOG_CR(
			    "HSM: Failed to take hsm_ha_state lock,exiting thread, ret value:%d err:%s",
			    rc, strerror(errno));
			assert(0);
		}
		active =
		    (hsm_ha_state.state == SA_AMF_HA_ACTIVE) ? true : false;
		rc = pthread_mutex_unlock(&hsm_ha_state.mutex);
		if (rc) {
			LOG_CR(
			    "HSM:Failed to unlock hsm_ha_state,exiting thread,ret value:%d err:%s",
			    rc, strerror(errno));
			assert(0);
		}
		...
		if (hotswap_state_model == PLMS_HPI_TWO_HOTSWAP_MODEL ||
			...
			/* Get the Power state */
			rc = saHpiResourcePowerStateGet(
			    cb->session_id, rpt_entry.ResourceId, &power_state);
			if (SA_OK != rc) {
				LOG_ER(
				    "HSM1:saHpiResourcePowerStateGet failed for res:%u with ret val:%d",
				    rpt_entry.ResourceId, rc);
				continue;
			}
			TRACE(
			    "HSM:saHpiResourcePowerStateGet succeeded for res:%u with ret val:%d state:%d",
			...
			if (event.EventType == SAHPI_ET_HOTSWAP) {
				state = event.EventDataUnion.HotSwapEvent
					    .HotSwapState;
				...
				if (SA_OK != rc) {
					LOG_ER(
					    "HSM:saHpiHotSwapStateGet failed for res with state model as:%d and ret val:%d",
		...
		if (event.EventType == SAHPI_ET_HOTSWAP) {
			if (hotswap_state_model ==
				...
				if (event.EventDataUnion.HotSwapEvent
					    .HotSwapState ==
					...
					rc = saHpiHotSwapPolicyCancel(
					    cb->session_id,
					    rpt_entry.ResourceId);
					if (SA_OK != rc)
						LOG_ER(
						    "Error taking control of res:%d ret val:%d",
						    rpt_entry.ResourceId, rc);

					/* Set the AutoExtractionTimeout */
					rc = saHpiAutoExtractTimeoutSet(
					    cb->session_id,
					    rpt_entry.ResourceId,
					    cb->extr_pending_timeout);
					if (SA_OK != rc)
						LOG_ER(
						    "AutoExtractTimeoutSet failed for res:%u ret val:%d",
						    rpt_entry.ResourceId, rc);
				}
			}
		 ...
		 * to be set on both active and standby, but anything else is
		 * only done by active
		 */
		if (!active)
			continue;
		...
		if (event.EventType == SAHPI_ET_RESOURCE) {
			 ...
			 */
			if (event.EventDataUnion.ResourceEvent
				.ResourceEventType ==
		...
		if (event.EventType == SAHPI_ET_SENSOR) {
			if (event.EventDataUnion.SensorEvent.SensorType ==
				...
				hsm_send_hotswap_event(
				    &rpt_entry, hotswap_state_model, state,
				    event.EventDataUnion.HotSwapEvent
					.PreviousHotSwapState,
				    retriev_idr_info);
			}
...
static SaUint32T hsm_discover_and_dispatch()
{
	...
	if (SA_OK != rc) {
		...
		if (SA_OK != rc) {
			...
			session, exiting the thread, ret val is:%d",
			       rc);
			assert(0);
		}
	}
	...
	do {
		...
		if (SA_OK != rc) {
			...
			if (SA_ERR_HPI_NOT_PRESENT == rc) {
				if (current == SAHPI_FIRST_ENTRY)
					LOG_ER(
					    "RPT table is eempty ret val is:%d",
					    rc);
				else
					LOG_ER(
					    "saHpiRptEntryGet retruned SA_ERR_HPI_NOT_PRESENT \
			...
			if (SA_ERR_HPI_INVALID_PARAMS == rc) {
				LOG_ER(
				    "Invalid params for saHpiRptEntryGet,breaking from the loop");
				rc = NCSCC_RC_FAILURE;
				break;
			} else {
				/* Reopen the session once to handle session
				 * issues if any */
				if (rpt_retry_count) {
					LOG_ER(
					    "saHpiRptEntryGet failed after reopening the sess retval:%u exiting",
					    rc);
					assert(0);
				}
				hsm_session_reopen();
				...
				if (SA_OK != rc) {
					LOG_ER(
					    "saHpiDiscover failed, exiting fom the thread, retval:%u",
					    rc);
					assert(0);
				}
				next = SAHPI_FIRST_ENTRY;
		...
		if (rpt_entry.ResourceCapabilities &
			...
			if (SA_OK != rc)
				LOG_ER(
				    "saHpiEventLogClear for res: %d failed ret val:%d",
				    rpt_entry.ResourceId, rc);
		}
		...
		if (hotswap_state_model == PLMS_HPI_FULL_FIVE_HOTSWAP_MODEL) {
			...
			rc = saHpiAutoExtractTimeoutSet(
			    cb->session_id, rpt_entry.ResourceId,
			    cb->extr_pending_timeout);
			if (SA_OK != rc) {
		...
		if (hotswap_state_model == PLMS_HPI_TWO_HOTSWAP_MODEL ||
			...
			TRACE(
			    "HSM:saHpiResourcePowerStateGet Succeded for res:%u with ret val:%d state:%d",
			    rpt_entry.ResourceId, rc, power_state);
			...
			if (SA_OK != rc) {
				LOG_ER(
				    "HSM:saHpiHotSwapStateGet failed for res:%u with ret val:%d",
				    rpt_entry.ResourceId, rc);
				continue;
			}
			TRACE("Hotswap state of res:%u is :%u",
			      rpt_entry.ResourceId, state);
		}
		...
		if (SAHPI_LAST_ENTRY == next &&
			...
			if (SA_OK != rc) {
				LOG_ER(
				    "HSM:saHpiDomainInfoGet failed with ret val:%d",
				    rc);
				return rc;
			}
				       ...
				       SaUint32T *hotswap_state_model)
{
	...
	if (rpt_entry->ResourceCapabilities & SAHPI_CAPABILITY_FRU) {
		...
		if (rpt_entry->ResourceCapabilities & SAHPI_CAPABILITY_POWER) {
			...
			*hotswap_state_model = PLMS_HPI_THREE_HOTSWAP_MODEL;
			TRACE(
			    "Resource :%d is not a FRU but has power capability",
			    rpt_entry->ResourceId);
		} else {
			TRACE("Resource :%d is not a FRU",
			      rpt_entry->ResourceId);
			return NCSCC_RC_FAILURE;
		}
	}
	TRACE(
	    "Hotswap State model for res:%u is:%u res_capab is:%x entity_type is:%u",
	    rpt_entry->ResourceId, *hotswap_state_model,
	    rpt_entry->ResourceCapabilities,
	    rpt_entry->ResourceEntity.Entry[0].EntityType);
					...
					SaUint32T retriev_idr_info)
{
	...
	req_evt.hpi_evt.sa_hpi_evt.EventDataUnion.HotSwapEvent.HotSwapState =
	    hotswap_state;
	req_evt.hpi_evt.sa_hpi_evt.EventDataUnion.HotSwapEvent
	    .PreviousHotSwapState = prev_hotswap_state;
					  ...
					  PLMS_INV_DATA *inv_data)
{
	...
	while (area_id != SAHPI_LAST_ENTRY) {
		...
		err = saHpiIdrAreaHeaderGet(cb->session_id,
					    rpt_entry->ResourceId, idr_id,
					    SAHPI_IDR_AREATYPE_UNSPECIFIED,
					    area_id, &next_area, &area_info);

		if (err != SA_OK) {
			LOG_ER(
			    "saHpiIdrAreaHeaderGet failed for res: %d err: %d",
			    rpt_entry->ResourceId, err);
			return NCSCC_RC_FAILURE;
		}
	...
	while ((err == SA_OK) && (fieldId != SAHPI_LAST_ENTRY)) {
		err = saHpiIdrFieldGet(cb->session_id, rpt_entry->ResourceId,
				       idr_id, area_info.AreaId,
				       SAHPI_IDR_FIELDTYPE_UNSPECIFIED, fieldId,
				       &next_field, &thisField);
		if (err == SA_OK) {
			if (thisField.Type ==
				...
				inv_data->chassis_area.part_no.DataLength =
				    thisField.Field.DataLength;
				memcpy(
				    inv_data->chassis_area.part_no.Data,
				    thisField.Field.Data,
				    inv_data->chassis_area.part_no.DataLength);
			}
		} else {
			LOG_ER(
			    "saHpiIdrFieldGet failed for res: %d error val is:%d",
			    rpt_entry->ResourceId, err);
			return NCSCC_RC_FAILURE;
		}
		fieldId = next_field;
	...
	while ((err == SA_OK) && (fieldId != SAHPI_LAST_ENTRY)) {
		...
		if (err == SA_OK) {
			if (thisField.Type ==
				...
				memcpy(
				    inv_data->board_area.manufacturer_name.Data,
				    thisField.Field.Data,
				    thisField.Field.DataLength);
			} else if (thisField.Type ==
				...
				memcpy(inv_data->board_area.fru_field_id.Data,
				       thisField.Field.Data,
				       inv_data->board_area.fru_field_id
					   .DataLength);
			}
		} else {
			LOG_ER(
		...
		if (err == SA_OK) {
			if (thisField.Type ==
				...
				memcpy(inv_data->product_area.product_name.Data,
				       thisField.Field.Data,
				       inv_data->product_area.product_name
					   .DataLength);
			} else if (thisField.Type ==
				   SAHPI_IDR_FIELDTYPE_SERIAL_NUMBER) {
				/* copy product_name into event structure */
				...
				memcpy(inv_data->product_area.serial_no.Data,
				       thisField.Field.Data,
				       inv_data->product_area.serial_no
					   .DataLength);
			} else if (thisField.Type ==
				   SAHPI_IDR_FIELDTYPE_PART_NUMBER) {
				/* copy product_name into event structure */
				...
				inv_data->product_area.part_no.DataLength =
				    thisField.Field.DataLength;
				memcpy(
				    inv_data->product_area.part_no.Data,
				    thisField.Field.Data,
				    inv_data->product_area.part_no.DataLength);
			} else if (thisField.Type ==
				...
				inv_data->product_area.product_version
				    .DataLength = thisField.Field.DataLength;
				memcpy(
				    inv_data->product_area.product_version.Data,
				    thisField.Field.Data,
				    inv_data->product_area.product_version
					.DataLength);
			} else if (thisField.Type ==
				...
				memcpy(inv_data->product_area.asset_tag.Data,
				       thisField.Field.Data,
				       inv_data->product_area.asset_tag
					   .DataLength);
			} else if (thisField.Type ==
				   SAHPI_IDR_FIELDTYPE_FILE_ID) {
				/* copy product_name into event structure */
				...
				memcpy(inv_data->product_area.fru_field_id.Data,
				       thisField.Field.Data,
				       inv_data->product_area.fru_field_id
					   .DataLength);
			}
		} else {
			LOG_ER(
...
SaUint32T hsm_correct_length(SaHpiIdrFieldT *thisfield)
{
	...
	while (isspace(field_ptr[fieldlength]) ||
	       (field_ptr[fieldlength] == '\0')) {
		if (fieldlength == 0) {
...
SaUint32T hsm_get_idr_info(SaHpiRptEntryT *rpt_entry, PLMS_INV_DATA *inv_data)
{
	...
	do {
		...
		if (SA_OK != err) {
			LOG_ER(
			    "HSM:RDR get (saHpiRdrGet) failed for res: %d with err:%d",
			    rpt_entry->ResourceId, err);
...
static SaUint32T hsm_session_reopen()
{
	...
	/* Open a session on the SAHPI_UNSPECIFIED_DOMAIN_ID */
	while (1) {
		err = saHpiSessionOpen(SAHPI_UNSPECIFIED_DOMAIN_ID, &session_id,
				       NULL);
		if (SA_OK != err) {
			LOG_ER("HSM:saHpiSessionOpen failed with err:%d \
                                after retrying %d times",
			       err, retry);
