...
SaUint32T plms_mds_enc_flat(struct ncsmds_callback_info *info,
			    EDU_HDL *edu_hdl);
SaUint32T plms_mds_dec_flat(struct ncsmds_callback_info *info,
			    EDU_HDL *edu_hdl);
...
SaUint32T plms_mds_enc(MDS_CALLBACK_ENC_INFO *enc_info, EDU_HDL *edu_hdl)
{
	...
	if (enc_info->i_to_svc_id == NCSMDS_SVC_ID_PLMA) {
		enc_info->o_msg_fmt_ver = m_NCS_ENC_MSG_FMT_GET(
		    enc_info->i_rem_svc_pvt_ver, PLMS_WRT_PLMA_SUBPART_VER_MIN,
		    PLMS_WRT_PLMA_SUBPART_VER_MAX, PLMS_WRT_PLMA_MSG_FMT_ARRAY);
	}
	if (enc_info->i_to_svc_id == NCSMDS_SVC_ID_PLMS) {
		enc_info->o_msg_fmt_ver = m_NCS_ENC_MSG_FMT_GET(
		    enc_info->i_rem_svc_pvt_ver, PLMA_WRT_PLMS_SUBPART_VER_MIN,
		    PLMA_WRT_PLMS_SUBPART_VER_MAX, PLMA_WRT_PLMS_MSG_FMT_ARRAY);
	}
	...
	rc = m_NCS_EDU_EXEC(edu_hdl, plms_edp_plms_evt, enc_info->io_uba,
			    EDP_OP_TYPE_ENC, evt, &ederror);
...
SaUint32T plms_mds_dec(MDS_CALLBACK_DEC_INFO *dec_info, EDU_HDL *edu_hdl)
{
	...
	rc = m_NCS_EDU_EXEC(edu_hdl, plms_edp_plms_evt, dec_info->io_uba,
			    EDP_OP_TYPE_DEC, (PLMS_EVT **)&dec_info->o_msg,
			    &ederror);
 ...
 * Notes         : None.
 *****************************************************************************/
SaUint32T plms_mds_normal_send(MDS_HDL mds_hdl, NCSMDS_SVC_ID from_svc,
 ...
 * Notes         : None.
 *****************************************************************************/
SaUint32T plms_mds_scoped_send(MDS_HDL mds_hdl, NCSMDS_SCOPE_TYPE scope,
			       NCSMDS_SVC_ID from_svc, NCSCONTEXT evt,
			       NCSMDS_SVC_ID to_svc)
{
 ...
 * Notes         : None.
 *****************************************************************************/
SaUint32T plms_mds_bcast_send(MDS_HDL mds_hdl, NCSMDS_SVC_ID from_svc,
			      NCSCONTEXT evt, NCSMDS_SVC_ID to_svc)
{
...
SaUint32T plm_mds_msg_sync_send(MDS_HDL mds_hdl, uint32_t from_svc,
