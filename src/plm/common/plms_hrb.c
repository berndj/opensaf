...
static SaUint32T hrb_get_resourceid(SaInt8T *epath_str, SaUint32T epath_len,
...
SaUint32T plms_hrb_initialize()
{
	...
	if (rc) {
		LOG_ER("pthread_create FAILED ret code:%d error:%s", rc,
		       strerror(errno));
		return NCSCC_RC_FAILURE;
	}
...
static void *plms_hrb(void)
{
	...
	while (osaf_poll_one_fd(m_GET_FD_FROM_SEL_OBJ(mbx_fd), -1) != -1) {
		...
		while (NULL !=
		       (hpi_req = (PLMS_HPI_REQ *)m_NCS_IPC_NON_BLK_RECEIVE(
			    &cb->mbx, hpi_req))) {
...
static SaUint32T hrb_process_hpi_req(PLMS_HPI_REQ *hpi_req)
{
	...
	TRACE("Processing hpi request for res:%s cmd:%d", hpi_req->entity_path,
	      hpi_req->cmd);
	...
	case PLMS_HPI_CMD_ACTION_REQUEST:
		...
		hpirc = saHpiHotSwapActionRequest(cb->session_id, resourceid,
						  hpi_req->arg);
		break;
	...
	case PLMS_HPI_CMD_RESOURCE_ACTIVE_SET:
		...
		hpirc = saHpiResourceActiveSet(cb->session_id, resourceid);
		break;

	case PLMS_HPI_CMD_RESOURCE_INACTIVE_SET:
		/* Transition from INSERTION PENDING or
		EXTRACTION PENDING state to INACTIVE */
		hpirc = saHpiResourceInactiveSet(cb->session_id, resourceid);
		break;
				    ...
				    SaHpiResourceIdT *resourceid)
{
	...
	if (NCSCC_RC_FAILURE == convert_string_to_epath(epath_str, &epath)) {
	...
	do {
		...
		for (i = 0; i < SAHPI_MAX_ENTITY_PATH; i++) {
			if (epath.Entry[i].EntityType !=
				...
				rpt_entry.ResourceEntity.Entry[i]
				    .EntityLocation) {
				/* not it */
				break;
			} else if (epath.Entry[i].EntityType ==
				       SAHPI_ENT_ROOT &&
				   rpt_entry.ResourceEntity.Entry[i]
