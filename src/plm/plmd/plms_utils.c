...
#include "plm/common/plms_notifications.h"
...
******************************************************************************/
SaUint32T plms_anc_chld_dep_adm_flag_is_set(PLMS_ENTITY *ent,
...
@brief		: Find all the affected entities of the entity ent.
		...
		Similarly if the function is called with is_insvc as 0, then
make sure the function is called only if the ent will move to OOS.
...
void plms_affected_ent_list_get(PLMS_ENTITY *root_ent,
				PLMS_GROUP_ENTITY **ent_list, SaBoolT is_insvc)
...
@brief		:Find out the dependent entities which are affected because
		the root ent(root_ent) is going to insvc/oos(is_insvc). We get
to traverse the rev_dep_list recursively. We also get to see the children of the
ent part of rev_dep_list.
...
@param[in]	: root_ent - Root entity for which the affected entities will be
		calculated.
@param[in]	: Entity for which this function is called. Means we have to
traverse ent`s rev_dep_list.
@param[out]	: ent_list - Head of list of affected entities whose
readiness state is affected.
@param[out]	: ent_fg_list- Head of list of affected entities whose
readiness flag is affected.
@param[in]	: is_insvc - 1 ==> In service
			     0 ==> Out o service.
******************************************************************************/
void plms_aff_dep_ent_list_get(PLMS_ENTITY *root_ent, PLMS_GROUP_ENTITY *ent,
			       ...
			       SaBoolT is_insvc)
{
	...
	plms_aff_dep_ent_list_get(root_ent, ent->next, ent_list, ent_fg_list,
				  is_insvc);
	if (is_insvc) {
		...
		case PLMS_HE_ENTITY:
			...
			if (NULL != ent->plm_entity->parent) {
				...
				if (!(plms_is_rdness_state_set(
					 ...
					 *ent_list, ent->plm_entity->parent) &&
			...
			while (head) {
				if ((SA_PLM_READINESS_IN_SERVICE ==
					 ...
					 .saPlmHEReadinessState) ||
				    (plms_is_ent_in_ent_list(
					*ent_list, head->plm_entity)) ||
				    (0 == (strcmp(head->plm_entity->dn_name_str,
						  root_ent->dn_name_str)))) {
					count++;
				}
				head = head->next;
			}
...
@param[in]	: is_insvc - 1 ==> In service
			     0 ==> Out o service.
******************************************************************************/
void plms_aff_chld_ent_list_get(PLMS_ENTITY *root_ent, PLMS_ENTITY *ent,
				PLMS_GROUP_ENTITY **ent_list,
				PLMS_GROUP_ENTITY **ent_fg_list,
				SaBoolT is_insvc)
{
...
@return		: false - If ent is not a child of root.
		  true - If ent is a child of root.
******************************************************************************/
SaUint32T plms_chld_get(PLMS_ENTITY *ent, PLMS_GROUP_ENTITY **chld_list)
{
	if (NULL == ent) {
					...
					SaBoolT mark_unmark)
{
	...
	plms_aff_dep_list_imnt_failure_get(ent, ent->rev_dep_list, mark_unmark,
					   &dep_aff_list);
					 ...
					 PLMS_GROUP_ENTITY **aff_ent_list)
{
	...
	if (!mark_unmark) { /* Clear the dep-imminent-failure flag.*/
		...
		if (!plms_rdness_flag_is_set(
			ent, SA_PLM_RF_DEPENDENCY_IMMINENT_FAILURE)) {

			plms_aff_chld_list_imnt_failure_get(
			    root_ent, ent->leftmost_child, mark_unmark,
			    aff_ent_list);
			return;
		}
		...
		while (head) {
			...
			if (((PLMS_HE_ENTITY ==
				...
				if (!plms_rdness_flag_is_set(
					...
					count++;
				} else if (plms_is_ent_in_ent_list(
					       *aff_ent_list,
					       head->plm_entity) ||
					   (0 == strcmp(root_ent->dn_name_str,
							head->plm_entity
		...
		if (count >= ent->min_no_dep) {
			...
			if (plms_rdness_flag_is_set(
				ent, SA_PLM_RF_IMMINENT_FAILURE)) {
				return;
			}
		} else {
					...
					PLMS_GROUP_ENTITY **aff_ent_list)
{
	...
	if (!mark_unmark) {
		...
		if (count >= ent->plm_entity->min_no_dep) {
			...
			if (plms_rdness_flag_is_set(
				ent->plm_entity, SA_PLM_RF_IMMINENT_FAILURE)) {
				return;
			}
	...
	plms_aff_chld_list_imnt_failure_get(root_ent,
					    ent->plm_entity->leftmost_child,
					    mark_unmark, aff_ent_list);

	plms_aff_dep_list_imnt_failure_get(
	    root_ent, ent->plm_entity->rev_dep_list, mark_unmark, aff_ent_list);
		...
		if (PLMS_EE_ENTITY == head->plm_entity->entity_type) {
			...
			while (parent) {
				if (plms_is_ent_in_ent_list(aff_he_list,
							    parent)) {
					plms_ent_to_ent_list_add(
					    head->plm_entity, aff_chld_ee_list);
					break;
				} else if (0 == strcmp(parent->dn_name_str,
						       ent->dn_name_str)) {
					plms_ent_to_ent_list_add(
					    head->plm_entity, aff_chld_ee_list);
					break;
				...
				PLMS_ENTITY_GROUP_INFO_LIST **list)
{
	...
	while (ent_grp) {
		...
		/* Ist node. */
		if (NULL == tail) {
			tail = (PLMS_ENTITY_GROUP_INFO_LIST *)calloc(
			    1, sizeof(PLMS_ENTITY_GROUP_INFO_LIST));
			if (NULL == tail) {
			...
			*list = tail;
			...
			prev->next = (PLMS_ENTITY_GROUP_INFO_LIST *)calloc(
			    1, sizeof(PLMS_ENTITY_GROUP_INFO_LIST));
			if (NULL == prev->next) {
				LOG_CR("plms_ent_grp_list_add, calloc FAILED.\
				Error: %s",
...
******************************************************************************/
void plms_ent_to_ent_list_add(PLMS_ENTITY *ent, PLMS_GROUP_ENTITY **list)
{
	...
	if (NULL == tail) {
		tail =
		    (PLMS_GROUP_ENTITY *)calloc(1, sizeof(PLMS_GROUP_ENTITY));
		if (NULL == tail) {
...
plms_inv_to_cbk_in_grp_find(PLMS_INVOCATION_TO_TRACK_INFO *list,
			    SaInvocationT inv)
{
...
@return		: No of entities to be
packed(0/count_changes/count_changes_only).
******************************************************************************/
...
@param[in]	: grp - Group for which the callback will be called.
@param[in]	: aff_ent_list - List of affected entities.
@param[in]	: grp_op - Type of change (group change/readiness status
change).
******************************************************************************/
			   ...
			   SaPlmChangeStepT change_step)
{
	...
	if (m_PLM_IS_SA_TRACK_CHANGES_ONLY_SET(grp->track_flags)) {
		...
		while (head) {
			...
			ent[count].entityName = head->plm_entity->dn_name;

			/* TODO: For the time being UNUSED is filled.
			This has to be the ntf-id */
			ent[count].plmNotificationId = SA_NTF_IDENTIFIER_UNUSED;
			if (PLMS_HE_ENTITY == head->plm_entity->entity_type) {
				   ...
				   SaBoolT mark_unmark)
{
	...
	head = ent_list;
	while (head) {
		TRACE("setting %s to %i", head->plm_entity->dn_name.value,
		      mark_unmark);
					   ...
					   SaUint16T minor_id)
{
	...
	if (PLMS_HE_ENTITY == ent->entity_type) {
		if (PLMS_READINESS_FLAG_RESET !=
			...
			/* Update IMM.*/
			ret_val = plms_attr_imm_update(
			    ent, "saPlmHEReadinessFlags", SA_IMM_ATTR_SAUINT64T,
			    PLMS_READINESS_FLAG_RESET);
		...
		if (PLMS_READINESS_FLAG_RESET !=
			...
			/* Update IMM.*/
			ret_val = plms_attr_imm_update(
			    ent, "saPlmEEReadinessFlags", SA_IMM_ATTR_SAUINT64T,
			    PLMS_READINESS_FLAG_RESET);
			if (NCSCC_RC_SUCCESS != ret_val) {
				LOG_ER("Cleared readiness flag for the ent %s,\
				Updating IMM FAILED",
	...
	if (ntf) {
		...
		ret_val = plms_state_change_ntf_send(
		    cb->ntf_hdl, &(ent->dn_name), &ntf_id, src_ind,
		    1 /*no_of_st_change*/, &ntf_state, minor_id,
		    0 /*no_cor_ids*/, NULL /*cor_ids*/,
...
******************************************************************************/
SaNtfIdentifierT
plms_readiness_flag_mark_unmark(PLMS_ENTITY *ent, SaPlmReadinessFlagsT flag,
				SaBoolT mark_unmark, PLMS_ENTITY *root_ent,
				SaUint16T src_ind, SaUint16T minor_id)
{
	...
	if (ntf) {
		...
		ret_err = plms_state_change_ntf_send(
		    cb->ntf_hdl, &(ent->dn_name), &ntf_id, src_ind,
		    1 /*no_of_st_change*/, &ntf_state, minor_id,
		    0 /*no_cor_ids*/, NULL /*cor_ids*/,
		    (NULL != root_ent) ? (&(root_ent->dn_name)) : NULL);
		...
		if (NCSCC_RC_SUCCESS != ret_err) {
			...
			Flag: %llu",
			       ent->dn_name_str, mark_unmark, flag_imm, flag);
		}
	}
	return ntf_id;
}
					  ...
					  SaUint16T src_ind, SaUint16T minor_id)
{
	...
	if (PLMS_HE_ENTITY == ent->entity_type) {
		if (state != ent->entity.he_entity.saPlmHEReadinessState) {
			...
			/* Update IMM.*/
			ret_err =
			    plms_attr_imm_update(ent, "saPlmHEReadinessState",
		...
		if (state != ent->entity.ee_entity.saPlmEEReadinessState) {
			...
			/* Update IMM.*/
			ret_err =
			    plms_attr_imm_update(ent, "saPlmEEReadinessState",
						 SA_IMM_ATTR_SAUINT32T, state);
			if (NCSCC_RC_SUCCESS != ret_err) {
				LOG_ER("Readiness state updated, Updating IMM \
				FAILED. Ent: %s, New St: %d, Old St: %d",
...
/******************************************************************************
@brief		: Overwrite the expected readiness state of the affected
entities with the current readiness status.
@param[in]	: aff_ent_list - List of affected entities.
******************************************************************************/
void plms_aff_ent_exp_rdness_state_ow(PLMS_GROUP_ENTITY *aff_ent_list)
{
	...
	while (ent_head) {
		if (PLMS_HE_ENTITY == ent_head->plm_entity->entity_type) {
			ent_head->plm_entity->exp_readiness_status
			    .readinessState =
...
SaNtfIdentifierT plms_op_state_set(PLMS_ENTITY *ent,
				   ...
				   PLMS_ENTITY *root_ent, SaUint16T src_ind,
				   SaUint16T minor_id)
{
	SaUint32T ret_err;
	SaPlmNtfStateChangeT ntf_state;
	...
	if (PLMS_HE_ENTITY == ent->entity_type) {
		if (state != ent->entity.he_entity.saPlmHEOperationalState) {
			...
			ret_err =
			    plms_attr_imm_update(ent, "saPlmHEOperationalState",
						 SA_IMM_ATTR_SAUINT32T, state);
			if (NCSCC_RC_SUCCESS != ret_err) {
		...
		if (state != ent->entity.ee_entity.saPlmEEOperationalState) {
			...
			ret_err =
			    plms_attr_imm_update(ent, "saPlmEEOperationalState",
						 SA_IMM_ATTR_SAUINT32T, state);
			if (NCSCC_RC_SUCCESS != ret_err) {
				LOG_ER("Operational state updated, IMM update \
				FAILED. Ent: %s, New St: %d, Old St: %d",
				      ...
				      SaUint16T minor_id)
{
	...
	if (PLMS_HE_ENTITY == ent->entity_type) {
		if ((SaPlmHEAdminStateT)state !=
			...
			/* Update IMM.*/
			ret_err =
			    plms_attr_imm_update(ent, "saPlmHEAdminState",
						 SA_IMM_ATTR_SAUINT32T, state);
			if (NCSCC_RC_SUCCESS != ret_err) {
		...
		if ((SaPlmEEAdminStateT)state !=
			...
			/* Update IMM.*/
			ret_err =
			    plms_attr_imm_update(ent, "saPlmEEAdminState",
						 SA_IMM_ATTR_SAUINT32T, state);
			if (NCSCC_RC_SUCCESS != ret_err) {
				LOG_ER("Admin state updated, IMM update  \
				FAILED. Ent: %s, New St: %d, Old St: %d",
					 ...
					 SaUint16T src_ind, SaUint16T minor_id)
{
	...
	if (PLMS_HE_ENTITY == ent->entity_type) {
		if ((SaPlmHEPresenceStateT)state !=
			...
			/* Update IMM.*/
			ret_err =
			    plms_attr_imm_update(ent, "saPlmHEPresenceState",
						 SA_IMM_ATTR_SAUINT32T, state);
			if (NCSCC_RC_SUCCESS != ret_err) {
		...
		if ((SaPlmEEPresenceStateT)state !=
			...
			/* Update IMM.*/
			ret_err =
			    plms_attr_imm_update(ent, "saPlmEEPresenceState",
						 SA_IMM_ATTR_SAUINT32T, state);
			if (NCSCC_RC_SUCCESS != ret_err) {
				LOG_ER("Presence state updated, IMM update \
				FAILED. Ent: %s, New St: %d, Old St: %d",
			       ...
			       SaImmValueTypeT val_type, SaUint64T val)
{
	...
	err = immutil_saImmOiRtObjectUpdate_2(cb->oi_hdl, &(ent->dn_name),
					      imm_attr);
	if (SA_AIS_OK != err) {
...
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
SaUint32T plms_attr_saname_imm_update(PLMS_ENTITY *ent, SaInt8T *attr_name,
				      SaNameT val,
				      SaImmAttrModificationTypeT mod_type)
{
	SaAisErrorT err;
	SaImmAttrModificationT_2 temp;
	const SaImmAttrModificationT_2 *imm_attr[2];
	...
	if (SA_AIS_OK != err) {
		...
		,ret_val: %d",
		       ent->dn_name_str, attr_name, he_type, err);
		return NCSCC_RC_FAILURE;
	}
...
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
SaUint32T plms_attr_sastring_imm_update(PLMS_ENTITY *ent, SaInt8T *attr_name,
					SaStringT val,
					SaImmAttrModificationTypeT mod_type)
{
	SaAisErrorT err;
...
SaBoolT plms_is_ent_in_ent_list(PLMS_GROUP_ENTITY *list, PLMS_ENTITY *ent)
{
	...
	while (tail) {
		if (0 ==
		    strcmp(tail->plm_entity->dn_name_str, ent->dn_name_str))
			return true;
		tail = tail->next;
	}
	return false;
...
SaUint32T plms_move_ent_to_insvc(PLMS_ENTITY *chld_ent, SaUint8T *is_flag_aff)
{
	*is_flag_aff = 0;
	...
	if ((PLMS_HE_ENTITY == chld_ent->entity_type) &&
		...
		state is neither active nor deacting",
		       chld_ent->dn_name_str);
		return NCSCC_RC_FAILURE;
	...
	if ((NULL != chld_ent->parent) &&
		...
		if (!plms_rdness_flag_is_set(chld_ent, SA_PLM_RF_DEPENDENCY)) {

			plms_readiness_flag_mark_unmark(
			    chld_ent, SA_PLM_RF_DEPENDENCY, 1 /*mark*/, NULL,
	...
	if (plms_min_dep_is_ok(chld_ent)) {
		if (plms_rdness_flag_is_set(chld_ent, SA_PLM_RF_DEPENDENCY)) {
			...
			plms_readiness_flag_mark_unmark(
			    chld_ent, SA_PLM_RF_DEPENDENCY, 0 /*unmark*/, NULL,
			    SA_NTF_OBJECT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);
			*is_flag_aff = 1;
		}
		...
		if (!plms_rdness_flag_is_set(chld_ent, SA_PLM_RF_DEPENDENCY)) {
			...
			plms_readiness_flag_mark_unmark(
			    chld_ent, SA_PLM_RF_DEPENDENCY, 1 /*unmark*/, NULL,
			    SA_NTF_OBJECT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);
			*is_flag_aff = 1;
		}
...
@param[in]	: inst_chld_ee - 1 ==> Instantiate the chld EE if all the
conditions are matched. 0 ==> Do not instantiate chld EE.
@param[in]	: inst_dep_ee - 1 ==> Instantiate the dep EE if all the
conditions are matched. 0 ==> Do not instantiate dep EE.
******************************************************************************/
				 ...
				 SaUint8T inst_chld_ee, SaUint8T inst_dep_ee)
{
	...
	plms_move_chld_ent_to_insvc(chld_ent->right_sibling, ent_list,
	...
	if (plms_min_dep_is_ok(chld_ent)) {
		if (plms_rdness_flag_is_set(chld_ent, SA_PLM_RF_DEPENDENCY)) {
			...
			plms_readiness_flag_mark_unmark(
			    chld_ent, SA_PLM_RF_DEPENDENCY, 0 /* unmark*/,
				...
				SaUint8T inst_dep_ee)
{
	...
	if ((NULL != dep_ent_list->plm_entity->parent) &&
		...
		if (!plms_rdness_flag_is_set(dep_ent_list->plm_entity,
			...
			plms_readiness_flag_mark_unmark(
			    dep_ent_list->plm_entity, SA_PLM_RF_DEPENDENCY,
			    1 /*mark*/, NULL, SA_NTF_OBJECT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);

			plms_ent_to_ent_list_add(dep_ent_list->plm_entity,
						 ent_list);
		}
		LOG_ER("Entity %s can not be moved to insvc, as parent is \
		not in service",
	...
	if ((PLMS_EE_ENTITY == dep_ent_list->plm_entity->entity_type) &&
	    (SA_PLM_EE_PRESENCE_INSTANTIATED !=
	     dep_ent_list->plm_entity->entity.ee_entity.saPlmEEPresenceState) &&
	    (SA_PLM_EE_PRESENCE_TERMINATING !=
	     dep_ent_list->plm_entity->entity.ee_entity.saPlmEEPresenceState)) {
		if (inst_dep_ee && inst) {
	...
	plms_readiness_state_set(
	    dep_ent_list->plm_entity, SA_PLM_READINESS_IN_SERVICE, NULL,
	...
	if (dep_ent_list->plm_entity->rev_dep_list)
		plms_move_dep_ent_to_insvc(
		    dep_ent_list->plm_entity->rev_dep_list, ent_list,
		    inst_dep_ee);
...
/******************************************************************************
@brief		: Call readiness callback for marking/clearing the
					       ...
					       SaNtfCorrelationIdsT *cor_ids)
{
	...
	if (plms_anc_chld_dep_adm_flag_is_set(ent, aff_ent_list)) {
		...
		is in some other operational context.",
		      ent->dn_name_str);
	...
	ntf_id = plms_readiness_flag_mark_unmark(
	    ent, SA_PLM_RF_IMMINENT_FAILURE, 1 /*mark*/, NULL,
	    SA_NTF_OBJECT_OPERATION, SA_PLM_NTFID_STATE_CHANGE_ROOT);

	head = aff_ent_list;
	while (head) {
		plms_readiness_flag_mark_unmark(
		    head->plm_entity, SA_PLM_RF_DEPENDENCY_IMMINENT_FAILURE,
...
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
SaUint32T plms_rdness_imminent_failure_cleared_process(
    PLMS_ENTITY *ent, PLMS_SEND_INFO *snd_info, SaNtfCorrelationIdsT *cor_ids)
{
	...
	ntf_id = plms_readiness_flag_mark_unmark(
	    ent, SA_PLM_RF_IMMINENT_FAILURE, 0 /*unmark*/, NULL,
	    SA_NTF_OBJECT_OPERATION, SA_PLM_NTFID_STATE_CHANGE_ROOT);
				      ...
				      SaNtfCorrelationIdsT *cor_ids)
{
	...
	if (((PLMS_HE_ENTITY == ent->entity_type) &&
		...
		TRACE("Operational state is already disabled. Return.");
		{
			...
			evt.res_evt.ntf_id = ntf_id;
			evt.res_evt.error = SA_AIS_OK;
			ret_err = plm_send_mds_rsp(
			    cb->mds_hdl, NCSMDS_SVC_ID_PLMS, snd_info, &evt);

			if (NCSCC_RC_SUCCESS != ret_err) {
				LOG_ER("Sync resp to PLMA for readiness impact\
				fault clear FAILED. ret_val = %d",
				       ret_err);
			}
		}
		TRACE_LEAVE2("Return Val: %d", NCSCC_RC_SUCCESS);
	...
	if (plms_rdness_flag_is_set(ent, SA_PLM_RF_IMMINENT_FAILURE)) {
		...
		plms_readiness_flag_mark_unmark(
		    ent, SA_PLM_RF_IMMINENT_FAILURE, 0, NULL,
		    SA_NTF_OBJECT_OPERATION, SA_PLM_NTFID_STATE_CHANGE_ROOT);

		/* Also clear dep-imminet-failure flag is set.*/
		plms_readiness_flag_mark_unmark(
		    ent, SA_PLM_RF_DEPENDENCY_IMMINENT_FAILURE, 0, NULL,
		    SA_NTF_OBJECT_OPERATION, SA_PLM_NTFID_STATE_CHANGE_ROOT);
	...
	} else if (plms_rdness_flag_is_set(
		       ent, SA_PLM_RF_DEPENDENCY_IMMINENT_FAILURE)) {
		...
		plms_aff_dep_list_imnt_failure_get(ent, ent->rev_dep_list, 0,
						   &aff_ent_list_flag);
	...
	if (((PLMS_HE_ENTITY == ent->entity_type) &&
		...
		disabled. */
		ntf_id = plms_op_state_set(ent, SA_PLM_OPERATIONAL_DISABLED,
		...
		if (is_flag) {
			/* Overwrite the expected readiness status with the
			 * current.*/
			...
			/* Find out all the groups, all affected entities belong
			to and add the groups to trk_info->group_info_list.
			*/
			if (NULL != aff_ent_list_flag) {
				plms_ent_list_grp_list_add(
				    aff_ent_list_flag,
				    &(trk_info.group_info_list));
			}
			...
			while (log_head_grp) {
				TRACE(
				    "%llu,",
				    log_head_grp->ent_grp_inf->entity_grp_hdl);
				log_head_grp = log_head_grp->next;
			}
		...
		plms_affected_ent_list_get(ent, &aff_ent_list_state,
					   0 /*OOS */);

		if (plms_anc_chld_dep_adm_flag_is_set(ent,
						      aff_ent_list_state)) {
			TRACE("Imminent failure request can not be set as the \
			ent %s is in some other operational context.",
			      ent->dn_name_str);
		...
		while (head) {
			plms_readiness_state_set(
			    head->plm_entity, SA_PLM_READINESS_OUT_OF_SERVICE,
			    ent, SA_NTF_OBJECT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_DEP);

			plms_readiness_flag_mark_unmark(
			    head->plm_entity, SA_PLM_RF_DEPENDENCY, 1 /*mark*/,
			    ent, SA_NTF_OBJECT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_DEP);

			if (PLMS_EE_ENTITY == head->plm_entity->entity_type) {
				ret_err =
				    plms_ee_term(head->plm_entity, false, 0);
					      ...
					      SaNtfCorrelationIdsT *cor_ids)
{
	...
	ntf_id = plms_op_state_set(ent, SA_PLM_OPERATIONAL_ENABLED, NULL,
				   SA_NTF_OBJECT_OPERATION,
				   SA_PLM_NTFID_STATE_CHANGE_ROOT);
	...
	if (NCSCC_RC_SUCCESS != ret_err) {
		...
		/* There is no change in readiness status in this context. Hence
		store the context, and use the cause when the EE/HE is
		instantiated/activated.*/
		trk_info_flt =
		    (PLMS_TRACK_INFO *)calloc(1, sizeof(PLMS_TRACK_INFO));
		if (NULL == trk_info_flt) {
			LOG_CR(
			    "Rdness Impact: calloc of track info failed.error: %s",
			    strerror(errno));
			assert(0);
		}
	...
	if (!is_set) {
		...
		if (NCSCC_RC_SUCCESS != ret_err) {
			LOG_ER(
			    "Sync resp to PLMA for readiness impact fault clear FAILED.ret_val=%d",
			    ret_err);
		}
		return ret_err;
	}
...
SaUint32T plms_readiness_impact_process(PLMS_EVT *evt)
{
	...
	if (NULL == ent) {
		...
		LOG_ER("Entity not found in patricia tree. Ent: %s", imp_ent);
		{
			...
			ret_err =
			    plm_send_mds_rsp(cb->mds_hdl, NCSMDS_SVC_ID_PLMS,
					     snd_info, &evt_resp);
	...
	case SA_PLM_RI_IMMINENT_FAILURE:
		ret_err = plms_rdness_imminent_failure_process(
		    ent, snd_info, rd_impact->correlation_ids);
		break;
	case SA_PLM_RI_FAILURE:
		ret_err = plms_rdness_failure_process(
		    ent, snd_info, rd_impact->correlation_ids);
		break;
	case SA_PLM_RI_IMMINENT_FAILURE_CLEARED:
		ret_err = plms_rdness_imminent_failure_cleared_process(
		    ent, snd_info, rd_impact->correlation_ids);
		break;
	case SA_PLM_RI_FAILURE_CLEARED:
		ret_err = plms_rdness_failure_cleared_process(
		    ent, snd_info, rd_impact->correlation_ids);
		break;
	default:
...
/******************************************************************************
@brief		: Enable the isolated entity. Enabling means
activating/instantiating the entity depending upon the isolation method.
@param[in]	: ent - HE or EE
...
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE.
******************************************************************************/
SaUint32T plms_ent_enable(PLMS_ENTITY *ent, SaUint32T adm_op,
			  SaUint32T mngt_cbk)
{
	SaUint32T ret_err = NCSCC_RC_SUCCESS;
	...
	if (PLMS_HE_ENTITY == ent->entity_type) {
		if (SA_PLM_HE_ADMIN_LOCKED_INACTIVE !=
			...
			if ((PLMS_ISO_DEFAULT == ent->iso_method) ||
				...
				ret_err =
				    plms_he_activate(ent, adm_op, mngt_cbk);
				if (NCSCC_RC_SUCCESS == ret_err) {
					ent->iso_method = PLMS_ISO_DEFAULT;
				} else {
			...
			} else if (PLMS_ISO_HE_RESET_ASSERT ==
				   ent->iso_method) {
		...
		if (SA_PLM_EE_ADMIN_LOCKED_INSTANTIATION !=
			...
			if ((PLMS_ISO_DEFAULT == ent->iso_method) ||
				...
				ret_err =
				    plms_ee_instantiate(ent, adm_op, mngt_cbk);
				if (NCSCC_RC_SUCCESS == ret_err) {
					ent->iso_method = PLMS_ISO_DEFAULT;
				} else {
				...
				TRACE("Isolation method is reset assert.");
				ret_err =
				    plms_he_reset(ent->parent, adm_op, mngt_cbk,
						  SAHPI_RESET_DEASSERT);
				if (NCSCC_RC_SUCCESS == ret_err) {
					ent->iso_method = PLMS_ISO_DEFAULT;
				} else {
				...
				ret_err = plms_he_activate(ent->parent, adm_op,
							   mngt_cbk);
				if (NCSCC_RC_SUCCESS == ret_err) {
					ent->iso_method = PLMS_ISO_DEFAULT;
				} else {
...
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE.
******************************************************************************/
SaUint32T plms_ent_isolate(PLMS_ENTITY *ent, SaUint32T adm_op,
			   SaUint32T mngt_cbk)
{
	...
	if (PLMS_EE_ENTITY == ent->entity_type) {
		...
		if (NCSCC_RC_SUCCESS != ret_err) {
			LOG_ER(
			    "EE Isolation(term) FAILED. Trying parent.Ent: %s",
			    ent->dn_name_str);
			...
			if (ent->parent) {
				...
				if (!can_isolate) {
					LOG_ER(
					    "Not attemptimg EE isolation as dep/child is insvc. Ent: %s",
					    ent->dn_name_str);
				}
			} else {
				LOG_ER(
				    "EE can not be isolated. Parent is domain.  Ent: %s",
				    ent->dn_name_str);
			...
			if (can_isolate) {
				if (ent->parent->entity_type ==
				    PLMS_EE_ENTITY) {
					/* Virtualization. Tell hypervisor to
					 * isolate the EE */
					ret_err = plms_ee_isolate_vm(ent);

					if (NCSCC_RC_SUCCESS != ret_err) {
						LOG_ER(
						    "EE isolation using hypervisor failed");
						plms_readiness_flag_mark_unmark(
						    ent,
				...
				} else if (ent->parent->entity_type ==
					   PLMS_HE_ENTITY) {
					/* assert reset state on parent HE */
					ret_err =
					    plms_he_reset(ent->parent, 0, 1,
							  SAHPI_RESET_ASSERT);

					if (NCSCC_RC_SUCCESS != ret_err) {
						LOG_ER(
						    "EE isolation (Reset assert of parent HE) FAILED. Deactivate the \
						parent HE.  Ent: %s",
						    ent->dn_name_str);
						...
						if (NCSCC_RC_SUCCESS !=
							...
							/* Isolation failed.*/
							LOG_ER(
							    "Deactivation of parent HE FAILED ==> Isolation of the\
							EE failed. Ent: %s",
							    ent->dn_name_str);
							...
							TRACE(
							    "EE isolation (deactivate the parent) SUCCESSFUL. Ent: %s",
							    ent->dn_name_str);
						...
						TRACE(
						    "EE isolation(reset assert of parent) SUCCESSFUL. Ent: %s",
						    ent->dn_name_str);
						ent->iso_method =
				...
				plms_readiness_flag_mark_unmark(
				    ent, SA_PLM_RF_ISOLATE_PENDING, 1, NULL,
				    SA_NTF_OBJECT_OPERATION,
				    SA_PLM_NTFID_STATE_CHANGE_ROOT);

				plms_readiness_flag_mark_unmark(
				    ent, SA_PLM_RF_MANAGEMENT_LOST, 1, NULL,
				    SA_NTF_OBJECT_OPERATION,
				    SA_PLM_NTFID_STATE_CHANGE_ROOT);
			...
			/* Clear isolate pending flag.*/
			plms_readiness_flag_mark_unmark(
			    ent, SA_PLM_RF_ISOLATE_PENDING, 0, NULL,
			    SA_NTF_OBJECT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);

			/* Clear admin pending */
			plms_readiness_flag_mark_unmark(
			    ent, SA_PLM_RF_ADMIN_OPERATION_PENDING, 0, NULL,
			    SA_NTF_OBJECT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);

			/* Clear management lost.*/
			plms_readiness_flag_mark_unmark(
			    ent, SA_PLM_RF_MANAGEMENT_LOST, 0, NULL,
			    SA_NTF_OBJECT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);
		}
	} else {
		...
		if (NCSCC_RC_SUCCESS != ret_err) {
			LOG_ER(
			    "HE isolation(deact) FAILED. Try reset assert. Ent: %s",
			    ent->dn_name_str);
			ret_err =
			    plms_he_reset(ent, adm_op, 1, SAHPI_RESET_ASSERT);

			if (NCSCC_RC_SUCCESS != ret_err) {
				/* Can not isolate.*/
				LOG_ER(
				    "HE isolation(reset assert) FAILED, ISOLATION FAILED. Ent: %s",
				    ent->dn_name_str);
				...
				TRACE(
				    "HE isolation (reset assert) SUCCESSFUL.");
			       ...
			       SaPlmTrackCauseT ope_id)
{
	...
	if ((SA_PLM_CHANGE_VALIDATE == step) ||
	    ((SA_PLM_CHANGE_START == step) &&
	     (SA_PLM_CAUSE_SHUTDOWN == ope_id))) {
		ret_err = plms_mbcsv_send_async_update(&msg, NCS_MBCSV_ACT_ADD);
	} else if (SA_PLM_CHANGE_START == step) {
		ret_err =
		    plms_mbcsv_send_async_update(&msg, NCS_MBCSV_ACT_UPDATE);
	} else {
...
******************************************************************************/
void plms_he_np_clean_up(PLMS_ENTITY *ent,
			 PLMS_EPATH_TO_ENTITY_MAP_INFO **epath_ent)
{
	...
	PLMS_CB *cb = plms_cb;
	SaInt8T ret_err;
	SaUint32T (*hsm_func_ptr)(SaInt8T * epath_str,
				  SaHpiEntityPathT * epath_ptr) = NULL;
	...
	if (NULL == *epath_ent) {
		...
		hsm_func_ptr =
		    dlsym(cb->hpi_intf_hdl, "convert_string_to_epath");
		if (NULL == hsm_func_ptr) {
			LOG_ER("dlsym() of HPI lib failed with error %s",
			       dlerror());
			return;
		}
	...
	/* Update IMM*/
	plms_attr_sastring_imm_update(
	    ent, "saPlmHECurrEntityPath",
	...
	/* Update IMM*/
	ret_err = plms_attr_imm_update(ent, "saPlmHEOperationalState",
...
******************************************************************************/
void plms_dep_immi_flag_check_and_set(PLMS_ENTITY *ent,
				      PLMS_GROUP_ENTITY **aff_ent_list,
				      SaUint32T *is_set)
{
	...
	*is_set = to_b_set;
	...
	if (count >= ent->min_no_dep) {
		...
		while (anc) {
			if (plms_rdness_flag_is_set(
				anc, SA_PLM_RF_IMMINENT_FAILURE) ||
			    plms_rdness_flag_is_set(
				anc, SA_PLM_RF_DEPENDENCY_IMMINENT_FAILURE)) {
				to_b_set = 1;
				break;
			}
			anc = anc->parent;
	...
	if (to_b_set) {
		...
		if (!plms_rdness_flag_is_set(
			...
			*is_set = to_b_set;
			...
			plms_aff_dep_list_imnt_failure_get(
			    ent, ent->rev_dep_list, 1, aff_ent_list);
