...
SaAisErrorT plms_state_change_ntf_send(
    SaNtfHandleT plm_ntf_hdl, SaNameT *object, SaNtfIdentifierT *ntf_id,
    SaUint16T source_indicator, SaUint16T no_of_state_changes,
    SaPlmNtfStateChangeT *state_changes, SaUint16T minor_id,
    SaUint16T no_of_corr_notifications, SaNtfIdentifierT *corr_ids,
    ...
    )
{
	...
	ret = saNtfStateChangeNotificationAllocate(
	    ...
	    add_info_items, no_of_state_changes,
	...
	*(plm_notification.sourceIndicator) = source_indicator;
	...
	for (ii = 0; ii < no_of_state_changes; ii++) {
		...
		plm_notification.changedStates[ii].stateId =
		    tmp_state->state.stateId;
		plm_notification.changedStates[ii].oldStatePresent =
		    tmp_state->state.oldStatePresent;
		plm_notification.changedStates[ii].oldState =
		    tmp_state->state.oldState;
		plm_notification.changedStates[ii].newState =
		    tmp_state->state.newState;
...
SaAisErrorT plms_hpi_evt_ntf_send(SaNtfHandleT plm_ntf_hdl, SaNameT *object,
				  SaUint32T event_type, SaInt8T *entity_path,
				  SaUint32T domain_id, SaHpiEventT *hpi_event,
...
/* FIXME : Need to enable, when the following API is supported */
	...
	ret = saNtfStateChangeNotificationAllocate(
	    ...
	    0, SA_NTF_ALLOC_SYSTEM_LIMIT /* variable data size*/
	    );
	if (SA_AIS_OK != ret) {
				...
				SaNtfIdentifierT *ntf_id)
{
	...
	*(plm_notification.perceivedSeverity) = severity;
	...
	if (event_type == SA_NTF_ALARM_EQUIPMENT) {
		...
		ret = saNtfPtrValAllocate(
		    plm_notification.notificationHandle,
		    strlen(entity_path) + 1, (void **)&dest_ptr,
		    &(plm_notification.notificationHeader.additionalInfo[0]
			  .infoValue));
