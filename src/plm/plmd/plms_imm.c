...
static void plms_handle_om_search_init_try_again_err(
    SaNameT *, SaImmScopeT, SaImmSearchOptionsT,
    const SaImmSearchParametersT_2 *, const SaImmAttrNameT *,
    SaImmSearchHandleT *);
...
static SaAisErrorT plms_imm_ccb_obj_delete_cbk(SaImmOiHandleT, SaImmOiCcbIdT,
					       const SaNameT *);
static SaAisErrorT
plms_imm_ccb_obj_modify_cbk(SaImmOiHandleT, SaImmOiCcbIdT, const SaNameT *,
...
static SaAisErrorT
validate_he_entity_paths_attr_val(PLMS_ENTITY *,
				  const SaImmAttrModificationT_2 *);
...
static SaAisErrorT validate_deletion_of_ee_base_type_obj(SaImmOiCcbIdT,
							 const SaNameT *);
static SaAisErrorT
validate_changes_to_he_type_obj(SaImmOiCcbIdT, const SaNameT *, SaInt32T,
				const SaImmAttrModificationT_2 **);
...
static void plms_reg_with_imm_as_oi(void)
{
	...
	do {
		...
		while (true) {
			...
			rc = immutil_saImmOiImplementerSet(plms_cb->oi_hdl,
							   impl_name);
...
static void plms_handle_om_search_init_try_again_err(
    SaNameT *root_name, SaImmScopeT scope, SaImmSearchOptionsT search_opts,
    const SaImmSearchParametersT_2 *search_param,
    const SaImmAttrNameT *attr_names, SaImmSearchHandleT *search_hdl)
{
	SaAisErrorT error;
	SaUint8T retry_cnt = 0;
	TRACE_ENTER();
	do {
		...
		sleep(1);
		error = saImmOmSearchInitialize_2(
		    plms_cb->imm_hdl, root_name, scope, search_opts,
		    search_param, attr_names, search_hdl);
		if (error != SA_AIS_ERR_TRY_AGAIN) {
			break; /* from the do while */
		}
		++retry_cnt;
...
static void
plms_handle_om_search_next_try_again_err(SaImmSearchHandleT search_hdl,
					 SaNameT *obj_name,
...
static void plms_get_objects_from_imm(SaInt8T *rdn_attr_name, SaInt8T obj_type)
{
	...
	SaImmSearchOptionsT search_opts =
	...
	if (error != SA_AIS_OK) {
		...
		if (error != SA_AIS_ERR_TRY_AGAIN) {
			...
			plms_handle_om_search_init_try_again_err(
			    root_name, scope, search_opts, &search_param,
			    attr_names, &search_hdl);
		}
	}
					       ...
					       const SaImmAttrValuesT_2 **attr)
{
				...
				for (k = 0; k < dep_names_num; k++) {
					...
					if ((dep_name->length ==
					    ...
					    (strncmp((char *)dep_name->value,
						     (char *)parent_name->value,
						     dep_name->length) == 0)) {
						TRACE_LEAVE2(
						    "DepName is same as parent name, \
					...
					for (l = k + 1; l < dep_names_num;
					     l++) {
						next_dep_name = (SaNameT *)*(
						    (attr[j]->attrValues) + l);
						if ((dep_name->length ==
						     next_dep_name->length) &&
						    (strncmp(
							 (char *)
							     dep_name->value,
							 (char *)next_dep_name
					  ...
					  "saPlmDepMinNumber") == 0) {
				dep_min_num =
				    *(SaUint32T *)*((attr[j])->attrValues);
			}
		}
		if (dep_min_num > dep_names_num) {
			TRACE_LEAVE2("min no of dependencies: %u, \
			num_dep_names: %u",
				     dep_min_num, dep_names_num);
			return SA_AIS_ERR_BAD_OPERATION;
		}
	}
	for (j = 0; attr[j] != NULL; j++) {
		if (((attr[j])->attrValues == NULL) ||
		    (*((attr[j])->attrValues) == NULL)) {
			/* Log the error */
			TRACE("attr_val is null for attr_name %s",
			      attr[j]->attrName);
			continue;
		}
		...
		} else if (strcmp((attr[j])->attrName, "saPlmHEEntityPaths") ==
			   0) {
			if ((attr[j])->attrValuesNumber >
			    SAHPI_MAX_ENTITY_PATH) {
				/* Log the error */
				TRACE_LEAVE2("No of values in \
					saPlmHEEntityPaths is %u, which is out \
					of range",
					     (attr[j])->attrValuesNumber);
				return SA_AIS_ERR_BAD_OPERATION;
			}
	...
	if ((parent_name != NULL) && (parent_name->length > 0)) {
		...
		operation->objectName.length =
		    snprintf((SaInt8T *)operation->objectName.value, len,
			     "%s,%s", obj_rdn, (SaInt8T *)parent_name->value);
	} else {
		operation->objectName.length = sprintf(
		    (SaInt8T *)operation->objectName.value, "%s", obj_rdn);
	}
	TRACE_LEAVE();
	return SA_AIS_OK;
}
					       ...
					       const SaNameT *obj_name)
{
	...
	if (strncmp((SaInt8T *)obj_name->value, "safHEType", 9) == 0) {
		plm_he_base_info = (PLMS_HE_BASE_INFO *)ncs_patricia_tree_get(
		    &plms_cb->base_he_info, (SaUint8T *)obj_name);
		...
		if ((plm_ent->entity.he_entity.saPlmHEReadinessState !=
			...
			TRACE_LEAVE2(
			    "readiness state: %u, rev_dep_list: %" PRIdPTR ", \
		...
		plm_ee_base_info = (PLMS_EE_BASE_INFO *)ncs_patricia_tree_get(
		    &plms_cb->base_ee_info, (SaUint8T *)obj_name);
		...
		if (strstr(dn_name, "safHEType") != NULL) {
			rc = validate_changes_to_he_type_obj(
			    ccb_id, obj_name, CCBUTIL_DELETE, NULL);
			if (rc != SA_AIS_OK) {
		...
		if ((strcmp(plms_cb->domain_info.domain.safDomain, rdn_val) !=
			...
			TRACE_LEAVE2(
			    "leftmost_child ptr of domain: %" PRIdPTR,
	...
	} else if (strncmp((SaInt8T *)obj_name->value, "safDependency", 13) ==
		   0) {
		/* This can be allowed to go through, log the same if reqd */
	} else if (memcmp(obj_name->value, "plmVmmConfig",
	...
	} else if (memcmp(obj_name->value, "plmVmConfig",
...
static SaAisErrorT
plms_imm_ccb_obj_modify_cbk(SaImmOiHandleT imm_oi_hdl, SaImmOiCcbIdT ccb_id,
			    const SaNameT *obj_name,
			    const SaImmAttrModificationT_2 **attr_mods)
{
	...
	if (memcmp(obj_name->value, "safHEType", 9) == 0) {
		...
		if (plm_ent->entity.he_entity.saPlmHEPresenceState !=
			...
			TRACE_LEAVE2(
			    "HE's presence state: %u, modify cannot be\
				done",
			    plm_ent->entity.he_entity.saPlmHEPresenceState);
			return SA_AIS_ERR_BAD_OPERATION;
		} else {
		...
		if ((plm_ent->entity.ee_entity.saPlmEEPresenceState !=
			...
			TRACE_LEAVE2(
			    "EE's presence state: %u, modify cannot be\
				done",
			    plm_ent->entity.ee_entity.saPlmEEPresenceState);
			return SA_AIS_ERR_BAD_OPERATION;
		} else {
			...
			for (j = 0; attr_mods[j] != NULL; j++) {
				if (strcmp(attr_mods[j]->modAttr.attrName,
					...
					rc = validate_ee_type_attr_val(
					    ccb_id, &(attr_mods[j]->modAttr));
					if (rc != SA_AIS_OK) {
		...
		if (strstr(dn_name, "safHEType") != NULL) {
			rc = validate_changes_to_he_type_obj(
			    ccb_id, obj_name, CCBUTIL_MODIFY, attr_mods);
			if (rc != SA_AIS_OK) {
			...
			for (j = 0; attr_mods[j] != NULL; j++) {
				if (strcmp(attr_mods[j]->modAttr.attrName,
					...
					attr_value = (SaUint32T *)*(
					    attr_mods[j]->modAttr.attrValues);
					if ((*attr_value <
		...
		for (j = 0; attr_mods[j] != NULL; j++) {
			if (strcmp(attr_mods[j]->modAttr.attrName,
				   "saPlmDepNames") == 0) {
				dep_names_num =
				    attr_mods[j]->modAttr.attrValuesNumber;
				for (k = 0; k < dep_names_num; k++) {
					dep_name = (SaNameT *)*(
					    (attr_mods[j]->modAttr.attrValues) +
					    k);
				...
				if (attr_mods[j]->modType ==
					...
					dep_names_num =
					    cur_names_num - dep_names_num;
				}
			} else if (strcmp(attr_mods[j]->modAttr.attrName,
					  "saPlmDepMinNumber") == 0) {
				dep_min_num = *(SaUint32T *)*(
				    attr_mods[j]->modAttr.attrValues);
			}
		}
		if (dep_min_num > dep_names_num) {
	...
	if ((ccb_util_ccb_data = ccbutil_getCcbData(ccb_id)) != NULL) {
		/* "memorize the request" */
		if (ccbutil_ccbAddModifyOperation(ccb_util_ccb_data, obj_name,
						  attr_mods) != 0) {
				   ...
				   SaImmOiCcbIdT ccb_id)
{
	...
	while ((opdata = ccbutil_getNextCcbOp(ccb_id, opdata)) != NULL) {
		...
		if (opdata->operationType == CCBUTIL_CREATE) {
			plms_create_objects(
			    obj_type, &opdata->objectName,
			    (SaImmAttrValuesT_2 **)(opdata->param.create
							.attrValues));
...
static void plms_imm_admin_op_cbk(SaImmOiHandleT imm_oi_hdl,
				  SaInvocationT inv_id, const SaNameT *obj_name,
				  ...
				  const SaImmAdminOperationParamsT_2 **params)
{
	...
	if (params[0] != NULL) {
		strcpy((SaInt8T *)plms_evt->req_evt.admin_op.option,
		       *(SaStringT *)params[0]->paramBuffer);
		TRACE_2("param buffer: %s",
			*(SaStringT *)params[0]->paramBuffer);
	}
	/* post this to mailbox */
	rc = m_NCS_IPC_SEND(&plms_cb->mbx, (NCS_IPC_MSG *)plms_evt,
...
static SaAisErrorT
validate_he_entity_paths_attr_val(PLMS_ENTITY *plm_ent,
...
static SaAisErrorT
validate_he_base_type_attr_val(SaImmOiCcbIdT ccb_id,
			       const SaImmAttrValuesT_2 *attr)
{
	...
	plm_he_base_info = (PLMS_HE_BASE_INFO *)ncs_patricia_tree_get(
	    &plms_cb->base_he_info, (SaUint8T *)attr_value);
	if (plm_he_base_info == NULL) {
		TRACE_2("he_base_type_obj with the given attr value does not\
			exist in the patricia tree");
		/* Check if it is created in same CCB */
		opdata = NULL;
		while ((opdata = ccbutil_getNextCcbOp(ccb_id, opdata)) !=
		       NULL) {
			if ((opdata->operationType == CCBUTIL_CREATE) &&
			    (memcmp(&opdata->objectName, attr_value,
				    sizeof(SaNameT)) == 0)) {
				break;
			}
		}
					     ...
					     const SaImmAttrValuesT_2 *attr)
{
	...
	if (ee_base_info == NULL) {
		...
		while (ee_type_node != NULL) {
			if (strcmp(ee_type_node->ee_type.safVersion, rdn) ==
			    0) {
				break;
			}
...
static SaAisErrorT
validate_deletion_of_he_base_type_obj(SaImmOiCcbIdT ccb_id,
				      const SaNameT *obj_name)
{
	...
	while (plm_ent != NULL) {
		if ((plm_ent->entity_type == PLMS_HE_ENTITY) &&
		    (memcmp(&plm_ent->entity.he_entity.saPlmHEBaseHEType,
			    obj_name, sizeof(SaNameT)) == 0)) {
...
static SaAisErrorT
validate_deletion_of_ee_base_type_obj(SaImmOiCcbIdT ccb_id,
				      const SaNameT *obj_name)
{
	CcbUtilOperationData_t *opdata = NULL;
	PLMS_ENTITY *plm_ent;
	...
	while (plm_ent != NULL) {
		if (plm_ent->entity_type == PLMS_EE_ENTITY) {
			...
			strncpy(ee_type,
				(SaInt8T *)
				    plm_ent->entity.ee_entity.saPlmEEType.value,
				plm_ent->entity.ee_entity.saPlmEEType.length);
			(void)strtok(ee_type, ",");
			parent_dn = strtok(NULL, "\0");
	...
	while ((opdata = ccbutil_getNextCcbOp(ccb_id, opdata)) != NULL) {
		if ((opdata->operationType == CCBUTIL_CREATE) &&
			...
			attr = (SaImmAttrValuesT_2 **)
				   opdata->param.create.attrValues;
			for (j = 0; *attr != 0; j++) {
				if (strcmp((*attr)->attrName, "saPlmEEType") ==
				    0) {
					attr_val =
					    (SaNameT *)*((*attr)->attrValues);
					memset(ee_type, 0,
...
static SaAisErrorT
validate_changes_to_he_type_obj(SaImmOiCcbIdT ccb_id, const SaNameT *obj_name,
				SaInt32T op_type,
				const SaImmAttrModificationT_2 **attr_mods)
{
	...
	while (plm_ent != NULL) {
		if ((plm_ent->entity_type == PLMS_HE_ENTITY) &&
		    ((memcmp(&plm_ent->entity.he_entity.saPlmHECurrHEType,
			     obj_name, sizeof(SaNameT)) == 0) ||
	...
	if (op_type == CCBUTIL_MODIFY) {
		for (j = 0; attr_mods[j] != NULL; j++) {
			if (strcmp(attr_mods[j]->modAttr.attrName,
				...
				if (attr_mods[j]->modType ==
				    SA_IMM_ATTR_VALUES_ADD) {
					if (he_type_node->he_type.saPlmHetIdr !=
					    NULL) {
						TRACE_LEAVE2(
						    "saPlmHetIdr has one value\
					...
					cannot be modified");
						return SA_AIS_ERR_BAD_OPERATION;
							   ...
							   .attrValues) ==
						   NULL) {
						/* for ADD modify, can this be
						 * null? */
						/* Does IMM not check this..?
						   Log and return error */
						TRACE_LEAVE2(
						    "attrValues is NULL for\
					saPlmHetIdr, cannot be modified");
						return SA_AIS_ERR_BAD_OPERATION;
					}
				} else if (attr_mods[j]->modType ==
					   SA_IMM_ATTR_VALUES_DELETE) {
					/* check if the value is same as the one
					   which exists in the data structure */
					/* IMM does not check this..? */
					attr_val_str = (SaStringT *)*(
					    attr_mods[j]->modAttr.attrValues);
					if (he_type_node->he_type.saPlmHetIdr ==
					    NULL) {
						TRACE_LEAVE2(
					...
					if (strcmp(*attr_val_str,
						   he_type_node->he_type
						       .saPlmHetIdr) != 0) {
						TRACE_LEAVE2(
						    "Given attr value %s is\
				...
				/* Nothing to be validated for REPLACE
				 * modification */
			}
		}
	}
						   ...
						   const SaNameT *obj_name)
{
	...
	while (plm_ent != NULL) {
		if ((plm_ent->entity_type == PLMS_EE_ENTITY) &&
		    (memcmp(&plm_ent->entity.ee_entity.saPlmEEType, obj_name,
			    sizeof(SaNameT)) == 0)) {
				   ...
				   SaImmAttrValuesT_2 **attrs)
{
	...
	for (i = 0; attrs[i] != NULL; i++) {
		...
		if (strcmp(attr_name, "safDomain") == 0) {
			plms_cb->domain_info.domain.safDomain =
			    calloc(SA_MAX_NAME_LENGTH + 1, sizeof(SaInt8T));
			if (plms_cb->domain_info.domain.safDomain == NULL) {
			...
			TRACE_2("RDN attr safDomain value: %s",
				plms_cb->domain_info.domain.safDomain);
				       ...
				       SaImmAttrValuesT_2 **attrs)
{
	...
	for (i = 0; attrs[i] != NULL; i++) {
		...
		if (strcmp(attr_name, "safDependency") == 0) {
			...
			plm_ent->min_no_dep =
			    *(SaUint32T *)*((attrs[i])->attrValues);
			TRACE_2("saPlmDepMinNumber attr value: %u",
				plm_ent->min_no_dep);
		}
	}
					 ...
					 SaImmAttrValuesT_2 **attrs)
{
	...
	TRACE_ENTER();
	he_base_info =
	    (PLMS_HE_BASE_INFO *)calloc(1, sizeof(PLMS_HE_BASE_INFO));
	if (he_base_info == NULL) {
	...
	for (i = 0; attrs[i] != NULL; i++) {
		...
		if (strcmp(attr_name, "safHEType") == 0) {
			he_base_info->he_base_type.safHEType =
			    (SaStringT)calloc(SA_MAX_NAME_LENGTH + 1,
					      sizeof(SaInt8T));
			...
			strcpy(he_base_info->he_base_type.safHEType,
			       *(SaStringT *)*(attrs[i]->attrValues));
			TRACE_2("RDN attr value safHEType: %s",
				he_base_info->he_base_type.safHEType);
		} else if (strcmp(attr_name, "saPlmHetHpiEntityType") == 0) {
			attr_val_len =
			    strlen(*(SaStringT *)*(attrs[i]->attrValues));
			he_base_info->he_base_type.saPlmHetHpiEntityType =
			    (SaInt8T *)calloc(attr_val_len + 1,
					      sizeof(SaInt8T));
			...
			strcpy(he_base_info->he_base_type.saPlmHetHpiEntityType,
			       *(SaStringT *)*(attrs[i]->attrValues));
			TRACE_2(
			    "saPlmHetHpiEntityType attr value: %s",
			    he_base_info->he_base_type.saPlmHetHpiEntityType);
		}
	}
	...
	*/
	...
	for (i = 0; attrs[i] != NULL; i++) {
		...
		if (strcmp(attr_name, "safHE") == 0) {
			...
			strcpy(he->entity.he_entity.safHE,
			       *(SaStringT *)*(attrs[i]->attrValues));
			TRACE_2("RDN attr safHE value: %s",
				he->entity.he_entity.safHE);
			...
			for (j = 0; j < values_num; j++) {
				attr_val =
				    (SaStringT *)*((attrs[i]->attrValues) + j);
				attr_val_len = strlen(*attr_val);
				temp =
				    calloc(attr_val_len + 1, sizeof(SaInt8T));
				if (temp == NULL) {
					LOG_CR("memory allocation failed for \
				...
				he->entity.he_entity.saPlmHEEntityPaths[j] =
				    temp;
			...
			he->entity.he_entity.saPlmHEAdminState =
			    *(SaUint32T *)*((attrs[i])->attrValues);
			TRACE_2("saPlmHEAdminState attr value: %u",
				he->entity.he_entity.saPlmHEAdminState);
	...
	if (strcmp(rdn_name, "safDomain") == 0) {
		...
		if (!parent_ent) {
			...
			if (rc != NCSCC_RC_SUCCESS) {
				LOG_ER(
				    "create_he_obj failed: calling assert now");
				assert(0);
			}
		}
					 ...
					 SaImmAttrValuesT_2 **attrs)
{
	...
	TRACE_ENTER();
	ee_base_info =
	    (PLMS_EE_BASE_INFO *)calloc(1, sizeof(PLMS_EE_BASE_INFO));
	if (ee_base_info == NULL) {
		LOG_CR(
		    "memory allocation failed for ee_base_info, calling assert now");
		TRACE_LEAVE();
		assert(0);
	}
	for (i = 0; attrs[i] != NULL; i++) {
		...
		if (strcmp(attr_name, "safEEType") == 0) {
			ee_base_info->ee_base_type.safEEType =
			    (SaInt8T *)calloc(SA_MAX_NAME_LENGTH + 1,
					      sizeof(SaInt8T));
			...
			strcpy(ee_base_info->ee_base_type.safEEType,
			       *(SaStringT *)*(attrs[i]->attrValues));
			TRACE_2("RDN attr value safEEType: %s",
				ee_base_info->ee_base_type.safEEType);
			...
			strcpy(ee_base_info->ee_base_type.saPlmEetProduct,
			       *(SaStringT *)*(attrs[i]->attrValues));
			TRACE_2("saPlmEetProduct attr value: %s",
				ee_base_info->ee_base_type.saPlmEetProduct);
			...
			ee_base_info->ee_base_type.saPlmEetVendor =
			    (SaInt8T *)calloc(attr_val_len + 1,
					      sizeof(SaInt8T));
			if (ee_base_info->ee_base_type.saPlmEetVendor == NULL) {
				LOG_CR("memory allocation failed for \
			...
			strcpy(ee_base_info->ee_base_type.saPlmEetVendor,
			       *(SaStringT *)*(attrs[i]->attrValues));
			TRACE_2("saPlmEetVendor attr value: %s",
				ee_base_info->ee_base_type.saPlmEetVendor);
			...
			strcpy(ee_base_info->ee_base_type.saPlmEetRelease,
			       *(SaStringT *)*(attrs[i]->attrValues));
			TRACE_2("saPlmEetRelease attr value: %s",
				ee_base_info->ee_base_type.saPlmEetRelease);
		}
	}
	...
	for (i = 0; attrs[i] != NULL; i++) {
		...
		if (strcmp(attr_name, "safEE") == 0) {
			...
			strcpy(ee->entity.ee_entity.safEE,
			       *(SaStringT *)*(attrs[i]->attrValues));
			TRACE_2("RDN attr safEE value: %s",
				ee->entity.ee_entity.safEE);
			...
			memcpy(ee->entity.ee_entity.saPlmEEType.value,
			       ((SaNameT *)*((attrs[i])->attrValues))->value,
			       ee->entity.ee_entity.saPlmEEType.length);
		} else if (strcmp(attr_name, "saPlmEEInstantiateTimeout") ==
			   0) {
				    ...
				    SaImmAttrValuesT_2 **attrs)
{
	...
	ee_base_info = (PLMS_EE_BASE_INFO *)ncs_patricia_tree_get(
	    &plms_cb->base_ee_info, (SaUint8T *)&key_dn);
	ee_type_info =
	    (PLMS_EE_TYPE_INFO *)calloc(1, sizeof(PLMS_EE_TYPE_INFO));
	...
	for (i = 0; attrs[i] != NULL; i++) {
		...
		if (strcmp(attr_name, "safVersion") == 0) {
			ee_type_info->ee_type.safVersion =
			    calloc(SA_MAX_NAME_LENGTH + 1, sizeof(SaInt8T));
				       ...
				       SaImmAttrModificationT_2 **attrs)
{
	...
	for (i = 0; attrs[i] != NULL; i++) {
		...
		if (strcmp(attr_name, "saPlmDepNames") == 0) {
			...
			if (attrs[i]->modType == SA_IMM_ATTR_VALUES_DELETE) {
				for (j = 0; j < attr_val_num; j++) {
					...
					/* remove ent_node from dep_list of
					   plm_ent */
					rem_ent2_from_dep_list_of_ent1(
					    plm_ent, ent_node);
			...
			for (j = 0; j < attr_val_num; j++) {
				...
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER("add_ent2_to_dep_list_of_ent1 \
					returned error %u, calling assert",
					       rc);
					assert(0);
				}
				/* Update the rev_dep_list of ent_node
	...
	if (plms_min_dep_is_ok(plm_ent) != NCSCC_RC_SUCCESS) {
		if (plm_ent->entity.he_entity.saPlmHEReadinessState ==
			...
			/* TODO handle this */
			LOG_ER(
			    "plms_min_dep_is_ok failed for %s: need to do something: probably take the entity OOS",
			    plm_ent->dn_name.value);
		} else {
	...
	for (k = 0; attrs[k] != NULL; k++) {
		...
		if (strcmp(attr_name, "saPlmHEEntityPaths") == 0) {
			if (attrs[k]->modType == SA_IMM_ATTR_VALUES_DELETE) {
				for (i = 0; i < attr_val_num; i++) {
					attr_val = (SaStringT *)*(
					    attrs[k]->modAttr.attrValues + i);
					for (j = 0; j < he->num_cfg_ent_paths;
					     j++) {
			       ...
			       SaImmAttrModificationT_2 **attrs)
{
	...
	for (i = 0; attrs[i] != NULL; i++) {
		...
		if (strcmp(attr_name, "saPlmEEType") == 0) {
			sa_name_ptr =
			    (SaNameT *)*(attrs[i]->modAttr.attrValues);
			ee->entity.ee_entity.saPlmEEType.length =
			    sa_name_ptr->length;
			memcpy(ee->entity.ee_entity.saPlmEEType.value,
		...
		if (strcmp(attr_name, "saPlmHetIdr") == 0) {
			...
			if ((attrs[i]->modType == SA_IMM_ATTR_VALUES_DELETE) ||
				...
				he_type_node->he_type.saPlmHetIdr =
				    realloc(he_type_node->he_type.saPlmHetIdr,
					    strlen(*attr_val) + 1);
...
static void rem_ent2_from_dep_list_of_ent1(PLMS_ENTITY *ent1, PLMS_ENTITY *ent2)
{
				    ...
				    SaStringT *attr_val)
{
	...
	if (idr_area == NULL) {
		...
		/* This is possible if null value is replacing the existing
		 * value */
	...
	do {
		...
		if (strcmp(idr_area, "PRODUCT") == 0) {
			if (strcmp(idr_field, "ASSET_TAG") == 0) {
				memcpy(he_type->inv_data.product_area.asset_tag
					   .Data,
				       idr_field_val, val_len);
				he_type->inv_data.product_area.asset_tag
				    .DataLength = val_len;
			} else if (strcmp(idr_field, "MANUFACTURER") == 0) {
				memcpy(he_type->inv_data.product_area
					   .manufacturer_name.Data,
				       idr_field_val, val_len);
				he_type->inv_data.product_area.manufacturer_name
				    .DataLength = val_len;
			} else if (strcmp(idr_field, "PART_NUMBER") == 0) {
				memcpy(
				    he_type->inv_data.product_area.part_no.Data,
				    idr_field_val, val_len);
				he_type->inv_data.product_area.part_no
				    .DataLength = val_len;
			} else if (strcmp(idr_field, "PRODUCT_NAME") == 0) {
				memcpy(he_type->inv_data.product_area
					   .product_name.Data,
				       idr_field_val, val_len);
				he_type->inv_data.product_area.product_name
				    .DataLength = val_len;
			} else if (strcmp(idr_field, "PRODUCT_VERSION") == 0) {
				memcpy(he_type->inv_data.product_area
					   .product_version.Data,
				       idr_field_val, val_len);
				he_type->inv_data.product_area.product_version
				    .DataLength = val_len;
			} else if (strcmp(idr_field, "SERIAL_NUMBER") == 0) {
				memcpy(he_type->inv_data.product_area.serial_no
					   .Data,
				       idr_field_val, val_len);
				he_type->inv_data.product_area.serial_no
				    .DataLength = val_len;
			} else {
			...
			if (strcmp(idr_field, "CHASSIS_TYPE") == 0) {
				he_type->inv_data.chassis_area.chassis_type =
				    atoi(idr_field_val);
			} else if (strcmp(idr_field, "PART_NUMBER") == 0) {
				memcpy(
				    he_type->inv_data.chassis_area.part_no.Data,
				    idr_field_val, val_len);
				he_type->inv_data.chassis_area.part_no
				    .DataLength = val_len;
			} else if (strcmp(idr_field, "SERIAL_NUMBER") == 0) {
				memcpy(he_type->inv_data.chassis_area.serial_no
					   .Data,
				       idr_field_val, val_len);
				he_type->inv_data.chassis_area.serial_no
				    .DataLength = val_len;
			} else {
				/* Field which we are not storing
				   might have come, log it */
			...
			if (strcmp(idr_field, "MANUFACTURER") == 0) {
				memcpy(he_type->inv_data.board_area
					   .manufacturer_name.Data,
				       idr_field_val, val_len);
				he_type->inv_data.board_area.manufacturer_name
				    .DataLength = val_len;
			} else if (strcmp(idr_field, "PART_NUMBER") == 0) {
				memcpy(
				    he_type->inv_data.board_area.part_no.Data,
				    idr_field_val, val_len);
				he_type->inv_data.board_area.part_no
				    .DataLength = val_len;
			} else if (strcmp(idr_field, "PRODUCT_NAME") == 0) {
				memcpy(he_type->inv_data.board_area.product_name
					   .Data,
				       idr_field_val, val_len);
				he_type->inv_data.board_area.product_name
				    .DataLength = val_len;
			} else if (strcmp(idr_field, "SERIAL_NUMBER") == 0) {
				memcpy(
				    he_type->inv_data.board_area.serial_no.Data,
				    idr_field_val, val_len);
				he_type->inv_data.board_area.serial_no
				    .DataLength = val_len;
			} else {
				/* Field which we are not storing
				   might have come, log it */
...
static void plms_delete_he_type_obj(SaNameT *obj_name)
{
	...
	if (strcmp(rdn, he_type_node->he_type.safVersion) == 0) {
		...
		while (he_type_node->next != NULL) {
			...
			if (strcmp(rdn,
				   next_he_type_node->he_type.safVersion) ==
			    0) {
...
static void plms_delete_ee_type_obj(SaNameT *obj_name)
{
	...
	if (strcmp(rdn, ee_type_node->ee_type.safVersion) == 0) {
		...
		while (ee_type_node->next != NULL) {
			...
			if (strcmp(rdn,
				   next_ee_type_node->ee_type.safVersion) ==
			    0) {
				/* Found the node to be deleted, adjust the
				   pointers */
...
static void plms_delete_dep_obj(SaNameT *obj_name)
{
	...
	remove_ent_from_rev_dep_list_of_others(plm_ent);
	free_dep_list(plm_ent);
	TRACE_LEAVE2("dependency object of %s deleted", plm_ent->dn_name_str);
}
...
void plms_proc_quiesced_standby_role_change()
{
	...
	while ((plm_ent = (PLMS_ENTITY *)ncs_patricia_tree_getnext(
		    &plms_cb->entity_info, (SaUint8T *)0)) != NULL) {
	...
	while ((he_base_info = (PLMS_HE_BASE_INFO *)ncs_patricia_tree_getnext(
		...
		while (he_base_info->he_type_info_list != NULL) {
			he_type_node = he_base_info->he_type_info_list;
			he_base_info->he_type_info_list =
			    he_base_info->he_type_info_list->next;
			plms_free_he_type_obj(he_type_node);
		}
		ncs_patricia_tree_del(&plms_cb->base_he_info,
				      &he_base_info->pat_node);
	...
	while ((ee_base_info = (PLMS_EE_BASE_INFO *)ncs_patricia_tree_getnext(
		    &plms_cb->base_ee_info, (SaUint8T *)0)) != NULL) {
		free(ee_base_info->ee_base_type.safEEType);
		/* Remove all the he_type objects in this node */
		while (ee_base_info->ee_type_info_list != NULL) {
			ee_type_node = ee_base_info->ee_type_info_list;
			ee_base_info->ee_type_info_list =
			    ee_base_info->ee_type_info_list->next;
			plms_free_ee_type_obj(ee_type_node);
		}
		ncs_patricia_tree_del(&plms_cb->base_ee_info,
				      &ee_base_info->pat_node);
	...
	while ((ent_map_info =
		    (PLMS_EPATH_TO_ENTITY_MAP_INFO *)ncs_patricia_tree_getnext(
			&plms_cb->epath_to_entity_map_info, (SaUint8T *)0)) !=
	       NULL) {
	...
	while ((client_info = (PLMS_CLIENT_INFO *)ncs_patricia_tree_getnext(
		    &plms_cb->client_info, (SaUint8T *)0)) != NULL) {
		PLMS_ENTITY_GROUP_INFO_LIST *tmp_ent_grp_ptr1 = NULL,
					    *tmp_ent_grp_ptr2 = NULL;
	...
	while (
	    (ent_grp_info = (PLMS_ENTITY_GROUP_INFO *)ncs_patricia_tree_getnext(
		 &plms_cb->entity_group_info, (SaUint8T *)0)) != NULL) {
...
#endif
static void
free_entity_group_info_list(PLMS_ENTITY_GROUP_INFO_LIST *grp_info_list)
{
...
static void plms_check_n_del_ent_from_ent_groups(PLMS_ENTITY *plm_ent)
{
	...
	while (group_list != NULL) {
		plms_ent_from_grp_ent_list_rem(
		    plm_ent, &group_list->ent_grp_inf->plm_entity_list);
...
static void
plms_ent_from_grp_ent_list_rem(PLMS_ENTITY *plm_ent,
			       PLMS_GROUP_ENTITY_ROOT_LIST **ent_list)
{
	PLMS_GROUP_ENTITY_ROOT_LIST *temp, *trav;
	TRACE_ENTER2("entity: %s", plm_ent->dn_name_str);
	if (strcmp((*ent_list)->plm_entity->dn_name_str,
		   plm_ent->dn_name_str) == 0) {
		...
		*ent_list = (*ent_list)->next;
		...
		while (trav->next != NULL) {
			temp = trav->next;
			if (strcmp(temp->plm_entity->dn_name_str,
				   plm_ent->dn_name_str) == 0) {
...
SaAisErrorT plms_imm_init(void)
{
	...
	do {
		...
		imm_oi_cbks.saImmOiAdminOperationCallback =
		    plms_imm_admin_op_cbk;
		imm_oi_cbks.saImmOiCcbAbortCallback = plms_imm_ccb_abort_cbk;
		imm_oi_cbks.saImmOiCcbApplyCallback = plms_imm_ccb_apply_cbk;
		imm_oi_cbks.saImmOiCcbCompletedCallback =
		    plms_imm_ccb_completed_cbk;
		imm_oi_cbks.saImmOiCcbObjectCreateCallback =
		    plms_imm_ccb_obj_create_cbk;
		imm_oi_cbks.saImmOiCcbObjectDeleteCallback =
		    plms_imm_ccb_obj_delete_cbk;
		imm_oi_cbks.saImmOiCcbObjectModifyCallback =
		    plms_imm_ccb_obj_modify_cbk;
		...
		if ((rc = immutil_saImmOiSelectionObjectGet(
			 plms_cb->oi_hdl, &plms_cb->imm_sel_obj)) !=
		    SA_AIS_OK) {
