					  ...
					  PLMS_PLMC_EE_OS_INFO *os_info)
{
	...
	if ((SA_PLM_EE_ADMIN_LOCKED_INSTANTIATION ==
		...
		LOG_ER(
		    "EE insting ignored, as its state or parent/dep state not proper:%s",
		    ent->dn_name_str);
	...
	if (SA_PLM_EE_PRESENCE_INSTANTIATING !=
		...
		plms_presence_state_set(ent, SA_PLM_EE_PRESENCE_INSTANTIATING,
	...
	if (!(ent->tmr.timer_id)) {
		...
		ret_err = plms_timer_start(
		    &ent->tmr.timer_id, ent,
...
/******************************************************************************
@brief		: Process instantiated event from PLMC. If required  OS
verification is done and the entity is moved to instantiated. But still the EE
is not moved to insvc. Waiting for tcp-connect.
@param[in]	: ent - EE
					 ...
					 PLMS_PLMC_EE_OS_INFO *os_info)
{
	...
	if ((SA_PLM_EE_ADMIN_LOCKED_INSTANTIATION ==
		...
		LOG_ER(
		    "EE insted ignored, as its state or parent/dep state not proper:%s",
		    ent->dn_name_str);
		TRACE_LEAVE2("Return Val: %d", NCSCC_RC_FAILURE);
		return NCSCC_RC_FAILURE;
	}
	...
	/* Verification is successful. Update the presence state.*/
	plms_presence_state_set(ent, SA_PLM_EE_PRESENCE_INSTANTIATED, NULL,
...
SaUint32T plms_plmc_terminating_process(PLMS_ENTITY *ent)
{
	...
	if (SA_PLM_EE_PRESENCE_TERMINATING !=
	    ent->entity.ee_entity.saPlmEEPresenceState) {
		plms_presence_state_set(ent, SA_PLM_EE_PRESENCE_TERMINATING,
					NULL, SA_NTF_OBJECT_OPERATION,
					SA_PLM_NTFID_STATE_CHANGE_ROOT);
	}
...
SaUint32T plms_plmc_tcp_connect_process(PLMS_ENTITY *ent)
{
	...
	if ((SA_PLM_EE_ADMIN_LOCKED_INSTANTIATION ==
		...
		/* Check if the dependency flag can be set. If yes then set the
		dependency flag and call appropiate callback with cause as .*/
		plms_is_dep_set_cbk_call(ent);

		LOG_ER(
		    "Term the entity, as the admin state is lckinst or parent/dep is in OOS. Ent: %s",
		    ent->dn_name_str);
		ret_err = plmc_sa_plm_admin_lock_instantiation(
		    ent->dn_name_str, plms_plmc_tcp_cbk);
	...
	if (SA_PLM_EE_PRESENCE_INSTANTIATED !=
		...
		Presence state to instantiated for ent: %s",
		      ent->dn_name_str);
		plms_presence_state_set(ent, SA_PLM_EE_PRESENCE_INSTANTIATED,
					NULL, SA_NTF_OBJECT_OPERATION,
					SA_PLM_NTFID_STATE_CHANGE_ROOT);
	}
	...
	if ((NULL != ent->trk_info) &&
	    (SA_PLM_CAUSE_FAILURE_CLEARED == ent->trk_info->track_cause)) {
		prev_adm_op = 1;
	...
	if (SA_PLM_EE_ADMIN_UNLOCKED !=
		...
		if (is_set) {
			plms_insted_dep_immi_failure_cbk_call(
			    ent, aff_ent_list_flag);
		}
...
SaUint32T plms_plmc_tcp_disconnect_process(PLMS_ENTITY *ent)
{
	...
	if (SA_PLM_READINESS_OUT_OF_SERVICE ==
		...
		if ((NULL != ent->trk_info) &&
		    (SA_PLM_CAUSE_EE_RESTART == ent->trk_info->track_cause)) {
...
SaUint32T plms_plmc_lock_response(PLMS_ENTITY *ent, SaUint32T lock_rc)
{
	...
	if (PLMS_PLMC_SUCCESS == lock_rc) {
		...
		if (!plms_rdness_flag_is_set(ent, SA_PLM_RF_MANAGEMENT_LOST)) {
			plms_readiness_flag_mark_unmark(
			    ent, SA_PLM_RF_MANAGEMENT_LOST, true, NULL,
			    SA_NTF_MANAGEMENT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);
...
SaUint32T plms_plmc_err_cbk_proc(PLMS_ENTITY *ent, PLMS_PLMC_ERR err_info)
{
	...
	if (PLMC_LIBACT_CLOSE_SOCKET == err_info.acode) {
		if (PLMC_LIBERR_LOST_CONNECTION == err_info.ecode) {
			...
			/* Case of flip-flop. Mark the flag. If the conntection
			is restored or the EE is terminated, clear this flag.*/
			ent->ee_sock_ff = true;
		}
	}
...
******************************************************************************/
SaUint32T plms_plmc_get_prot_ver_response(PLMS_ENTITY *ent, SaUint8T *prot_ver)
{
...
/******************************************************************************
@brief		: Process get_os_info response. Verify the OS info. If
verification is successful, unlock the EE and change the readiness state to
		insvc.
@param[in]	: ent - EE
					 ...
					 PLMS_PLMC_EE_OS_INFO *os_info)
{
	...
	if (NULL != os_info) {
		...
		if (plms_rdness_flag_is_set(ent, SA_PLM_RF_MANAGEMENT_LOST)) {
			if (NCSCC_RC_SUCCESS !=
			    plms_os_info_resp_mngt_flag_clear(ent)) {
				return NCSCC_RC_FAILURE;
			}
		}
		ret_err = plms_ee_verify(ent, os_info);
		...
		if (NCSCC_RC_SUCCESS != ret_err) {
			...
			/* Set the dep-imminet-failure flag if required.*/
			plms_dep_immi_flag_check_and_set(
			    ent, &aff_ent_list_flag, &is_set);

			/* Fault clear case. Admin repair and readiness
			 * impact.*/
			if ((NULL != ent->trk_info) &&
			    (SA_PLM_CAUSE_FAILURE_CLEARED ==
			     ent->trk_info->track_cause)) {
				prev_adm_op = 1;
				trk_info = ent->trk_info;
				ent->trk_info = NULL;
				if (SA_PLM_ADMIN_REPAIRED ==
				    trk_info->imm_adm_opr_id) {
				...
				/* Admin RESTART amd Admin RESET case.*/
				   ...
				   ent->trk_info /* && ent->am_i_aff_ent*/) {
				prev_adm_op = 1;
				trk_info = ent->trk_info;
				ent->trk_info = NULL;
				trk_info->track_count--;
				TRACE(
				    "Perform the in progress admin op for entity: %s, adm-op: %d",
				    ent->dn_name_str,
				    trk_info->root_entity->adm_op_in_progress);
				...
				if (!(trk_info->track_count)) {
					...
					*/
					...
					if (NCSCC_RC_SUCCESS != ret_err) {
						LOG_ER(
						    "Adm operation result sending to IMM failed for ent:\
						%s,imm-ret-val: %d",
						    ent->dn_name_str, ret_err);
					}
					TRACE(
					    "Adm operation result sending to IMM successful for ent: %s",
					    ent->dn_name_str);
				}
			}
...
@brief		:Process osaf_stop response.
		TODO: Not implemented for release-1.
******************************************************************************/
SaUint32T plms_plmc_osaf_stop_response(PLMS_ENTITY *ent, SaUint32T osaf_stop_rc)
{
	TRACE_ENTER2("Not implemented, Entity: %s", ent->dn_name_str);
	TRACE_LEAVE2("Not implemented, Return Val: %d", NCSCC_RC_SUCCESS);
	return NCSCC_RC_SUCCESS;
...
@brief		:Process svc_start response.
		TODO: Not implemented for release-1.
******************************************************************************/
SaUint32T plms_plmc_svc_start_response(PLMS_ENTITY *ent, SaUint32T svc_start_rc)
{
	TRACE_ENTER2("Not implemented, Entity: %s", ent->dn_name_str);
	TRACE_LEAVE2("Not implemented, Return Val: %d", NCSCC_RC_SUCCESS);
	return NCSCC_RC_SUCCESS;
...
@brief		:Process svc_stop response.
		TODO: Not implemented for release-1.
******************************************************************************/
SaUint32T plms_plmc_svc_stop_response(PLMS_ENTITY *ent, SaUint32T svc_stop_rc)
{
	TRACE_ENTER2("Not implemented, Entity: %s", ent->dn_name_str);
	TRACE_LEAVE2("Not implemented, Return Val: %d", NCSCC_RC_SUCCESS);
	return NCSCC_RC_SUCCESS;
...
@brief		:Process get ee_id response.
		TODO: Not implemented for release-1.
******************************************************************************/
SaUint32T plms_plmc_ee_id_response(PLMS_ENTITY *ent, SaUint32T plmd_rest_rc)
{
	TRACE_ENTER2("Not implemented, Entity: %s", ent->dn_name_str);
	TRACE_LEAVE2("Not implemented, Return Val: %d", NCSCC_RC_SUCCESS);
	return NCSCC_RC_SUCCESS;
...
int32_t plms_plmc_connect_cbk(SaInt8T *ee_id, SaInt8T *msg)
{
	...
	if (0 == strcmp(PLMC_CONN_CB_MSG, msg)) {
		...
		LOG_ER("Connect cbk with invalid msg, ent: %s, msg: %s", ee_id,
		       msg);
	}
...
int32_t plms_plmc_udp_cbk(udp_msg *msg)
{
	...
	evt = (PLMS_EVT *)calloc(1, sizeof(PLMS_EVT));
	if (NULL == evt) {
		LOG_CR("udp_cbk, calloc FAILED. Err no: %s", strerror(errno));
		assert(0);
	}
	...
	case EE_INSTANTIATING:
		evt->req_evt.plms_plmc_evt.plmc_evt_type = PLMS_PLMC_EE_INSTING;
		break;

	case EE_TERMINATED:
		evt->req_evt.plms_plmc_evt.plmc_evt_type = PLMS_PLMC_EE_TRMTED;
		break;

	case EE_INSTANTIATED:
		evt->req_evt.plms_plmc_evt.plmc_evt_type = PLMS_PLMC_EE_INSTED;
		break;

	case EE_TERMINATING:
		evt->req_evt.plms_plmc_evt.plmc_evt_type = PLMS_PLMC_EE_TRMTING;
		break;

	default:
		LOG_ER("UDP cbk with invalid msg. ent: %s, msg %s", msg->ee_id,
		       msg->msg);
		free(evt);
		return 1;
	}
	...
	plms_os_info_parse(os_info, &(evt->req_evt.plms_plmc_evt.ee_os_info));
	free(os_info);
	os_info = NULL;
	...
	if (NCSCC_RC_SUCCESS !=
	    m_NCS_IPC_SEND(&cb->mbx, evt, NCS_IPC_PRIORITY_HIGH)) {
		LOG_ER("Posting to MBX FAILED, ent: %s, msg: %s", msg->ee_id,
		       msg->msg);
		TRACE_LEAVE2("Return Val: %d", false);
		return 1;
	}
	TRACE("Posted to MBX SUCCESSFUL, ent: %s, msg: %s", msg->ee_id,
	      msg->msg);
...
static int32_t plms_plmc_tcp_cbk(tcp_msg *msg)
{
	...
	evt = (PLMS_EVT *)calloc(1, sizeof(PLMS_EVT));
	if (NULL == evt) {
		LOG_CR("tcp_cbk, calloc FAILED. Err no: %s", strerror(errno));
		assert(0);
	}
	...
	case PLMC_GET_ID_CMD:
		evt->req_evt.plms_plmc_evt.plmc_evt_type = PLMS_PLMC_EE_ID_RESP;
		break;
	...
	case PLMC_GET_PROTOCOL_VER_CMD:
		...
		evt->req_evt.plms_plmc_evt.ee_ver =
		    (SaUint8T *)calloc(1, sizeof(SaUint8T) * (len + 1));
		memcpy(evt->req_evt.plms_plmc_evt.ee_ver, msg->result, len + 1);
		break;

	case PLMC_GET_OSINFO_CMD:
		evt->req_evt.plms_plmc_evt.plmc_evt_type = PLMS_PLMC_EE_OS_RESP;
	...
	default:
		LOG_ER("tcp_cbk with invalid msg, ent: %s, msg: %s", msg->ee_id,
		       msg->cmd);
		free(evt);
		TRACE_LEAVE2("Return Val: %d", false);
		return 1;
...
@brief		: There are five actions taken by plmc-lib on encountering
		...
		PLMC_LIBACT_EXIT_THREAD and PLMC_LIBACT_DESTROY_LIBRARY are
handled in this function. As the action is exit(0), no need to process these
		events in PLMS MBX context. If situation arises for any other
errors, it will be taken care then.
******************************************************************************/
int32_t plms_plmc_error_cbk(plmc_lib_error *msg)
{
	...
	if (PLMC_LIBACT_DESTROY_LIBRARY == msg->acode) {
		LOG_ER(
		    "PLMS EXIT !!!!!! PLMC lib destroyed, I am helpless. PLMc act: %s",
		    msg->action);
		exit(0);
	}
	if ('\0' == msg->ee_id[0]) {
		TRACE(
		    "Not posting to MBX, as no ee_id. PLMS can not handle it.");
		TRACE_LEAVE2("Return Val: %d", true);
		return 0;
	}
...
SaUint32T plms_plmc_mbx_evt_process(PLMS_EVT *evt)
{
		...
		if (NULL == ent) {
			...
			if (ret_err == NCSCC_RC_SUCCESS) {
				if (evt->req_evt.plms_plmc_evt.plmc_evt_type ==
				    PLMS_PLMC_EE_TCP_CONCTED) {
					TRACE(
					    "sending plmcd restart to %s for scaling",
					    evt->req_evt.plms_plmc_evt.ee_id
						.value);

					ret_err = plmc_plmcd_restart(
					    (char *)evt->req_evt.plms_plmc_evt
						.ee_id.value,
					    plms_plmc_tcp_cbk);
				}
			} else {
				LOG_IN(
				    "scaling not enabled: ignoring unknown EE: %s",
	...
	case PLMS_PLMC_EE_INSTING:
		ret_err = plms_plmc_instantiating_process(
		    ent, &(evt->req_evt.plms_plmc_evt.ee_os_info));

		break;
	case PLMS_PLMC_EE_INSTED:
		ret_err = plms_plmc_instantiated_process(
		    ent, &(evt->req_evt.plms_plmc_evt.ee_os_info));
		break;
	...
	case PLMS_PLMC_EE_VER_RESP:
		ret_err = plms_plmc_get_prot_ver_response(
		    ent, evt->req_evt.plms_plmc_evt.ee_ver);
		break;
	case PLMS_PLMC_EE_OS_RESP:
		ret_err = plms_plmc_get_os_info_response(
		    ent, &evt->req_evt.plms_plmc_evt.ee_os_info);
		break;
	case PLMS_PLMC_EE_LOCK_RESP:
		ret_err = plms_plmc_lock_response(
		    ent, evt->req_evt.plms_plmc_evt.resp);
		break;
	case PLMS_PLMC_EE_UNLOCK_RESP:
		ret_err = plms_plmc_unlock_response(
		    ent, evt->req_evt.plms_plmc_evt.resp);
		break;
	case PLMS_PLMC_EE_LOCK_INST_RESP:
		ret_err = plms_plmc_lckinst_response(
		    ent, evt->req_evt.plms_plmc_evt.resp);
		break;
	case PLMS_PLMC_SVC_START_RESP:
		ret_err = plms_plmc_svc_start_response(
		    ent, evt->req_evt.plms_plmc_evt.resp);
		break;
	case PLMS_PLMC_SVC_STOP_RESP:
		ret_err = plms_plmc_svc_stop_response(
		    ent, evt->req_evt.plms_plmc_evt.resp);
		break;
	case PLMS_PLMC_OSAF_START_RESP:
		ret_err = plms_plmc_osaf_start_response(
		    ent, evt->req_evt.plms_plmc_evt.resp);
		break;
	case PLMS_PLMC_OSAF_STOP_RESP:
		ret_err = plms_plmc_osaf_stop_response(
		    ent, evt->req_evt.plms_plmc_evt.resp);
		break;
	case PLMS_PLMC_PLMD_RESTART_RESP:
		ret_err = plms_plmc_plmd_restart_response(
		    ent, evt->req_evt.plms_plmc_evt.resp);
		break;
	case PLMS_PLMC_EE_ID_RESP:
		ret_err = plms_plmc_ee_id_response(
		    ent, evt->req_evt.plms_plmc_evt.resp);
		break;
	case PLMS_PLMC_EE_RESTART_RESP:
		ret_err = plms_plmc_restart_response(
		    ent, evt->req_evt.plms_plmc_evt.resp);
		break;
	case PLMS_PLMC_ERR_CBK:
		ret_err = plms_plmc_err_cbk_proc(
		    ent, evt->req_evt.plms_plmc_evt.err_info);
		break;
	default:
		break;
	}
...
static SaUint32T plms_ee_verify(PLMS_ENTITY *ent, PLMS_PLMC_EE_OS_INFO *os_info)
{
	...
	dn_ee_type = (SaInt8T *)calloc(
	    1,
	    sizeof(SaInt8T) * (ent->entity.ee_entity.saPlmEEType.length + 1));
	if (NULL == dn_ee_type) {
	...
	ee_base_type_node = (PLMS_EE_BASE_INFO *)ncs_patricia_tree_get(
	    &(cb->base_ee_info), (SaUint8T *)&(dn_ee_base_type_key));
	...
	if (ee_base_type_node->ee_base_type.saPlmEetProduct) {
		if (0 != strcmp(ee_base_type_node->ee_base_type.saPlmEetProduct,
			...
			PLMC-provided: %s",
			       ee_base_type_node->ee_base_type.saPlmEetProduct,
			       os_info->product);
			free(dn_ee_type);
			free(dn_ee_type_ver);
			TRACE_LEAVE2();
	...
	if (ee_base_type_node->ee_base_type.saPlmEetVendor) {
		if (0 != strcmp(ee_base_type_node->ee_base_type.saPlmEetVendor,
			...
			PLMC-provided: %s",
			       ee_base_type_node->ee_base_type.saPlmEetVendor,
			       os_info->vendor);
			free(dn_ee_type);
			free(dn_ee_type_ver);
			TRACE_LEAVE2();
	...
	if (ee_base_type_node->ee_base_type.saPlmEetRelease) {
		if (0 != strcmp(ee_base_type_node->ee_base_type.saPlmEetRelease,
			...
			%s, PLMC-provided: %s",
			       ee_base_type_node->ee_base_type.saPlmEetRelease,
			       os_info->release);
			free(dn_ee_type);
			free(dn_ee_type_ver);
			TRACE_LEAVE2();
...
/******************************************************************************
@brief		: Parse the string os_info and fill evt_os_info. The
string format of the OS info should be mentioned in plmcd.conf as given below
				    ...
				    PLMS_PLMC_EE_OS_INFO *evt_os_info)
{
	...
	temp_os_info =
	    (SaInt8T *)calloc(1, sizeof(SaInt8T) * (strlen(os_info) + 1));
	...
	while (4 >= order) {
		...
		if ((temp_os_info[0] == ';') || ('\0' == temp_os_info[0])) {
			...
			token =
			    (SaInt8T *)calloc(1, sizeof(SaInt8T) * (count + 1));
...
/******************************************************************************
@brief		: Make an attempt to take the EE to in service and if
successful, do the same for its dependent and children. If in the process any
...
static SaUint32T plms_plmc_unlck_insvc(PLMS_ENTITY *ent,
				       PLMS_TRACK_INFO *trk_info,
				       PLMS_GROUP_ENTITY *aff_ent_list_flag,
				       SaUint32T is_set)
{
	SaUint32T ret_err = NCSCC_RC_SUCCESS;
	PLMS_GROUP_ENTITY *aff_ent_list = NULL;
	...
	if (NCSCC_RC_SUCCESS == plms_move_ent_to_insvc(ent, &is_flag_aff)) {
		...
		plms_move_chld_ent_to_insvc(ent->leftmost_child, &aff_ent_list,
					    1, 1);
		TRACE("Make an attempt to make the dependents to insvc.");
		/* Move the dependent entities to InSvc and clear the
		dependency flag.*/
		plms_move_dep_ent_to_insvc(ent->rev_dep_list, &aff_ent_list, 1);
		...
		if (NULL != aff_ent_list_flag) {
			...
			plms_ent_list_grp_list_add(
			    aff_ent_list_flag, &(new_trk_info.group_info_list));
...
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
SaUint32T plms_ee_unlock(PLMS_ENTITY *ent, SaUint32T is_adm_op,
			 SaUint32T mngt_cbk)
{
	SaUint32T ret_err;
	SaUint32T cbk = 0;
...
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
SaUint32T plms_ee_term(PLMS_ENTITY *ent, SaUint32T is_adm_op,
		       SaUint32T mngt_cbk)
{
...
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
SaUint32T plms_ee_lock(PLMS_ENTITY *ent, SaUint32T is_adm_op,
		       SaUint32T mngt_cbk)
{
	SaUint32T ret_err, cbk = 0;
...
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
SaUint32T plms_ee_reboot(PLMS_ENTITY *ent, SaUint32T is_adm_op,
			 SaUint32T mngt_cbk)
{
	SaUint32T ret_err, cbk = 0;
	TRACE_ENTER2("Entity: %s", ent->dn_name_str);
	...
	ret_err =
	    plmc_sa_plm_admin_restart(ent->dn_name_str, plms_plmc_tcp_cbk);
...
@brief		: Instantiate the EE by resetting the parent HE.
		This function takes care of setting admin pending and management
		lost flag and also calling the callback if the instantiate
operation fails.
@param[in]	: ent - EE
@param[in]	: is_adm_op - Is this an admin operation(0/1).
@param[in]	: mngt_cbk - Should callback be called for management lost(0/1).
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
SaUint32T plms_ee_instantiate(PLMS_ENTITY *ent, SaUint32T is_adm_op,
			      SaUint32T mngt_cbk)
{
	...
	if (parent->entity_type == PLMS_EE_ENTITY) {
		/* Virtualization */
		TRACE(
		    "instantiate request for virtualized EE - parent EE is %s",
		    parent->dn_name_str);
		...
		if (NULL == parent->entity.he_entity.saPlmHECurrEntityPath) {
			...
			/* don't reset if this is myself -- obviously we are
			already running; just return success.  PLMCD message
			will handle EE */
			if (parent->entity_type == PLMS_HE_ENTITY &&
			    cb->my_entity_path &&
			    !strcmp(
				parent->entity.he_entity.saPlmHECurrEntityPath,
				cb->my_entity_path)) {
			...
			TRACE("Reset the parent HE %s(ee_inst).",
			      parent->dn_name_str);
			ret_err =
			    plms_he_reset(parent, 1 /*adm_op*/, 1 /*mngt_cbk*/,
					  SAHPI_COLD_RESET);
		}
	}
...
SaUint32T plms_ee_inst_failed_tmr_exp(PLMS_ENTITY *ent)
{
	...
	plms_presence_state_set(ent, SA_PLM_EE_PRESENCE_INSTANTIATION_FAILED,
				NULL, SA_NTF_OBJECT_OPERATION,
				SA_PLM_NTFID_STATE_CHANGE_ROOT);
...
SaUint32T plms_ee_term_failed_tmr_exp(PLMS_ENTITY *ent)
{
	...
	/* Take the ent to instantiation-failed state.*/
	plms_presence_state_set(ent, SA_PLM_EE_PRESENCE_TERMINATION_FAILED,
				NULL, SA_NTF_OBJECT_OPERATION,
				SA_PLM_NTFID_STATE_CHANGE_ROOT);
...
SaUint32T plms_inst_term_failed_isolate(PLMS_ENTITY *ent)
{
	...
	if (ent->parent) {
		...
		head = chld_list;
		while (head) {
			if (0 == strcmp(head->plm_entity->dn_name_str,
					ent->dn_name_str)) {
				head = head->next;
				continue;
			}
	...
	if (can_isolate) {
		if (parent->entity_type == PLMS_EE_ENTITY) {
			...
			if (NCSCC_RC_SUCCESS != ret_err) {
				can_isolate = 0;
				LOG_ER(
				    "EE %s isolation failed unable to terminate virtual machine",
				    ent->dn_name_str);
			} else {
				TRACE(
				    "Isolated the ent %s, Terminating virtual machine successful",
				    ent->dn_name_str);
			...
			ret_err =
			    plms_he_reset(ent->parent, 0 /*adm_op*/,
			...
			if (NCSCC_RC_SUCCESS != ret_err) {
				...
				if (NCSCC_RC_SUCCESS != ret_err) {
					...
					TRACE(
					    "Isolated the ent %s, Deactivate parent HE successfull.",
					    ent->dn_name_str);
					ent->iso_method =
				...
				TRACE(
				    "Isolate the ent %s, Reset assert parent HE successfull.",
				    ent->dn_name_str);
		...
		LOG_ER(
		    "EE's siblings are insvc and, hence the EE cannot be isolated.");
		return NCSCC_RC_FAILURE;
	}
	if (!can_isolate) {
...
static SaUint32T plms_ee_terminated_mngt_flag_clear(PLMS_ENTITY *ent)
{
	...
	plms_readiness_flag_mark_unmark(ent, SA_PLM_RF_ADMIN_OPERATION_PENDING,
					0, NULL, SA_NTF_OBJECT_OPERATION,
					SA_PLM_NTFID_STATE_CHANGE_ROOT);

	/* Clear isolate pending flag.*/
	plms_readiness_flag_mark_unmark(ent, SA_PLM_RF_ISOLATE_PENDING, 0, NULL,
					SA_NTF_OBJECT_OPERATION,
					SA_PLM_NTFID_STATE_CHANGE_ROOT);
...
static SaUint32T plms_tcp_connect_mngt_flag_clear(PLMS_ENTITY *ent)
{
	...
	plms_readiness_flag_mark_unmark(
	    ent, SA_PLM_RF_ISOLATE_PENDING, 0 /*unmark*/, NULL,
	    SA_NTF_OBJECT_OPERATION, SA_PLM_NTFID_STATE_CHANGE_ROOT);
...
/******************************************************************************
@brief		: Clear readiness flags on getting successful lock-inst
response.
@param[in]	: ent - Entity whose readinsess flags are to be cleared.
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
...
/******************************************************************************
@brief		: Clear readiness flags on getting successful get os info
response.
@param[in]	: ent - Entity whose readinsess flags are to be cleared.
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
...
@return		: None.
******************************************************************************/
void plms_insted_dep_immi_failure_cbk_call(PLMS_ENTITY *ent,
...
void plms_is_dep_set_cbk_call(PLMS_ENTITY *ent)
{
	...
	if (((NULL != ent->parent) &&
		...
		if (!plms_rdness_flag_is_set(ent, SA_PLM_RF_DEPENDENCY)) {
			...
			plms_readiness_flag_mark_unmark(
			    ent, SA_PLM_RF_DEPENDENCY, 1 /*mark*/, NULL,
			    SA_NTF_OBJECT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);
