...
PLMS_GROUP_ENTITY *
plm_get_aff_ent_list(PLMS_GROUP_ENTITY_ROOT_LIST *entity_list);
	...
	while (grp_entity_list) {
		...
		while (start) {
			if (grp_info == start->ent_grp_inf) {
				if (NULL == prev) {
					grp_entity_list->plm_entity
					    ->part_of_entity_groups =
					    start->next;
				} else {
...
void plm_free_invocation_list_frm_grp(PLMS_ENTITY_GROUP_INFO *grp_info)
{
	...
	ent = (PLMS_ENTITY *)ncs_patricia_tree_getnext(&cb->entity_info,
						       (SaUint8T *)0);
	while (ent) {
...
plm_get_aff_ent_list(PLMS_GROUP_ENTITY_ROOT_LIST *entity_list)
{
	...
	while (entity_list) {
		...
		if (!node) {
			LOG_CR(
			    "PLMS : PLMS_GROUP_ENTITY memory alloc failed, error val:%s",
			    strerror(errno));
			assert(0);
		}
...
void plms_process_agent_initialize(PLMS_EVT *plm_evt)
{
	...
	if (plm_handle_pool == MAX_PLM_POOL_HANDLES) {
		TRACE_5(
		    "plm_handle_pool reached max limit, hence wrapped around");
		plm_handle_pool = 0;
		/**
...
void plms_process_agent_finalize(PLMS_EVT *plm_evt)
{
	...
	if (cb->ha_state == SA_AMF_HA_ACTIVE) {
		...
		proc_rc =
		    plms_mbcsv_send_async_update(&mbcsv_msg, NCS_MBCSV_ACT_RMV);
		if (proc_rc != NCSCC_RC_SUCCESS) {
			LOG_ER("PLMS : Async update to stdby failed");
			/* FIXME  What to do ?? */
		}
...
send_resp:
	...
	/** send the response */
	plm_send_mds_rsp(cb->mds_hdl, NCSMDS_SVC_ID_PLMS, &plm_evt->sinfo,
			 &plm_resp);
...
void plms_process_trk_start_evt(PLMS_EVT *plm_evt)
{
	...
	no_of_ent_recd =
	    plm_evt->req_evt.agent_track.track_start.no_of_entities;
	...
	no_of_ent_in_grp =
	    plms_get_num_entities_of_grp(grp_info->plm_entity_list);
	...
	if ((m_PLM_IS_SA_TRACK_CURRENT_SET(track_flags)) &&
	    ((plm_evt->sinfo.stype == MDS_SENDTYPE_SNDRSP) ||
	     (plm_evt->sinfo.stype == MDS_SENDTYPE_RSP))) {
		...
		if (!entity_list) {
			rc = SA_AIS_ERR_NO_MEMORY;
			LOG_CR(
			    "PLMS :  memory alloc for entities failed, error val:%s",
			    strerror(errno));
			goto send_resp;
		}
		memset(entity_list, 0,
		       no_of_ent_in_grp * sizeof(SaPlmReadinessTrackedEntityT));
	...
	send_resp:
		...
		if (m_PLM_IS_SA_TRACK_CURRENT_SET(track_flags)) {
			...
			trk_info.track_cause = SA_PLM_CAUSE_STATUS_INFO;
			...
			if (!grp_info_node) {
				LOG_CR(
				    "PLMS :  PLMS_ENTITY_GROUP_INFO_LIST memory alloc failed, error val:%s",
				    strerror(errno));
				return;
			}
			grp_info->track_cookie = plm_evt->req_evt.agent_track
						     .track_start.track_cookie;
			grp_info_node->ent_grp_inf = grp_info;
			grp_info_node->next = NULL;
			trk_info.group_info_list = grp_info_node;
			trk_info.aff_ent_list =
			    plm_get_aff_ent_list(grp_info->plm_entity_list);
			trk_info.grp_op = SA_PLM_GROUP_NO_CHANGE;
			proc_rc = plms_cbk_call(&trk_info, 1);
			free_aff_ent_list(trk_info.aff_ent_list);

			/* If the flag is only current then restore the prev
			 * cookie.*/
			if (!m_PLM_IS_SA_TRACK_CHANGES_SET(track_flags) &&
			    !m_PLM_IS_SA_TRACK_CHANGES_ONLY_SET(track_flags)) {
...
void plms_process_trk_stop_evt(PLMS_EVT *plm_evt)
{
	...
	if (cb->ha_state == SA_AMF_HA_ACTIVE) {
		/* Checkpointing with track_flags & cookie as NULL, so that on
		 * Standy it will be set to 0 */
		memset(&mbcsv_msg, 0, sizeof(PLMS_MBCSV_MSG));
		mbcsv_msg.header.msg_type = PLMS_A2S_MSG_ENT_GRP_INFO;
		mbcsv_msg.header.num_records = 1; /* Since async */
		mbcsv_msg.info.ent_grp_info.entity_group_handle =
		...
		rc = plms_mbcsv_send_async_update(&mbcsv_msg,
						  NCS_MBCSV_ACT_UPDATE);
...
void plm_rem_grp_info_from_client(PLMS_ENTITY_GROUP_INFO *grp_info)
{
	...
	while (grp_info_list) {
		if (grp_info == grp_info_list->ent_grp_inf) {
			 ...
			 SaNameT *ent_names, SaUint32T no_of_ents)
{
	...
	for (ii = 0; ii < no_of_ents; ii++) {
		...
		while (list_start) {
			if ((key_dn->length ==
			     list_start->plm_entity->dn_name.length) &&
			    (strncmp((SaInt8T *)key_dn->value,
				     (SaInt8T *)
					 list_start->plm_entity->dn_name.value,
				     key_dn->length) == 0)) {
				break;
			}
		...
		if (!node) {
			LOG_CR(
			    "PLMS :  PLMS_GROUP_ENTITY_ROOT_LIST memory alloc failed, error val:%s",
			    strerror(errno));
			assert(0);
		}
			 ...
			 PLMS_ENTITY_GROUP_INFO *grp_info)
{
	...
	while (start) {
		if (plm_entity == start->plm_entity) {
			...
			TRACE_5("Entity removed from grp :  %s",
				plm_entity->dn_name.value);
			free(start);
...
plm_get_ckpt_ent_list(PLMS_GROUP_ENTITY_ROOT_LIST *ent_list)
{
	...
	while (list) {
		node = (PLMS_CKPT_ENTITY_LIST *)malloc(
		    sizeof(PLMS_CKPT_ENTITY_LIST));
		if (!node) {
			LOG_CR(
			    "PLMS :  PLMS_CKPT_ENTITY_LIST memory alloc failed, error val:%s",
			    strerror(errno));
			assert(0);
		}
...
void plms_process_grp_rem_evt(PLMS_EVT *plm_evt)
{
	...
	for (ii = 0; ii < plm_evt->req_evt.agent_grp_op.entity_names_number;
	     ii++) {
		...
		while (tmp_entity) {
			if ((key_dn->length ==
			     tmp_entity->plm_entity->dn_name.length) &&
			    (strncmp((SaInt8T *)key_dn->value,
				     (SaInt8T *)
					 tmp_entity->plm_entity->dn_name.value,
				     key_dn->length) == 0)) {
				if ((tmp_entity->plm_entity ==
				     tmp_entity->subtree_root) ||
				    (NULL == tmp_entity->subtree_root)) {

					break;
				} else {
					LOG_ER(
					    "The entity : %s cant be removed",
					    tmp_entity->plm_entity->dn_name
						.value);
					rc = SA_AIS_ERR_INVALID_PARAM;
					goto send_resp;
				}
	...
	/* If the entity to be removed is in mid of any admin context, then
	 * reject and return try_again.*/
	for (ii = 0; ii < plm_evt->req_evt.agent_grp_op.entity_names_number;
	     ii++) {
		key_dn = &plm_evt->req_evt.agent_grp_op.entity_names[ii];
		...
		if (NULL == ent) {
			LOG_ER(
			    "The entity %s does not exist in PLMS database tree.",
			    key_dn->value);
			rc = SA_AIS_ERR_NOT_EXIST;
			goto send_resp;
		}
		...
		if (ent->am_i_aff_ent || ent->adm_op_in_progress) {
			LOG_ER(
			    "The ent: %s is in admin context. Can not be removed.",
			    ent->dn_name_str);
	...
	if (NCSCC_RC_SUCCESS !=
	    plm_rem_entity(&del_list, grp_info->plm_entity_list,
	...
	/** remove the entities from the grp. Before that copy del list to tmp
	 * ptr so that del list it can be used later. */
	...
	for (ii = 0; ii < no_of_ents; ii++) {
		...
		while (added_ent) {
			if (added_ent->plm_entity == new_entity) {
				...
				*entity_list = NULL;
				LOG_ER(
				    "The entity %s is repeated and  cant be added again",
				    new_entity->dn_name.value);
				rc = SA_AIS_ERR_EXIST;
				return rc;
			}
		...
		plms_chld_get(new_entity->leftmost_child, &child_list);
		PLMS_GROUP_ENTITY *start = child_list;
		while (start) {
			PLMS_GROUP_ENTITY_ROOT_LIST *tmp_grp_entity_list =
			    grp_entity_list;
			added_ent = *entity_list;
			/**
			 ...
			 */
			while (added_ent) {
				if (added_ent->plm_entity ==
				    start->plm_entity) {
					TRACE(
					    "The entity %s is added just before this turn",
			...
			prev = node;
			start = start->next;
			TRACE_5("Root Entity %s, child added : %s",
				new_entity->dn_name.value,
				node->plm_entity->dn_name.value);
		}
	}
	return rc;
}
...
void plms_process_grp_add_evt(PLMS_EVT *plm_evt)
{
		...
		while (tmp_entity) {
			if (key_dn->length ==
				...
				if ((strncmp((SaInt8T *)key_dn->value,
					     (SaInt8T *)tmp_entity->plm_entity
						 ->dn_name.value,
					     key_dn->length)) == 0) {
					/* Added as single entity */
					LOG_ER(
					    "PLMS : The entity %s already exists",
					    key_dn->value);
					rc = SA_AIS_ERR_EXIST;
					goto send_resp;
				}
	...
	if (options == SA_PLM_GROUP_SINGLE_ENTITY) {
		...
		/* Add entity referred by entity names to group */
		rc = plm_add_sub_tree(
		    &new_entity_list, grp_info->plm_entity_list,
		    plm_evt->req_evt.agent_grp_op.entity_names,
		    plm_evt->req_evt.agent_grp_op.entity_names_number, options);
		if (rc != SA_AIS_OK) {
			goto send_resp;
		}
	}
	...
	if (NULL == entity_head) {
		...
		TRACE_5(
		    "Num of entities added to grp with grp hdl %Lu are : %u ",
...

		{
			if (client_info) {
				if (SA_AMF_HA_STANDBY == ha_state) {
					...
					if (!client_node) {
						LOG_CR(
						    "PLMS : PLMS_CKPT_CLIENT_INFO_LIST memory alloc failed, error val:%s",
						    strerror(errno));
						assert(0);
					}
					memset(
					 ...
					 */
					plms_mbcsv_send_async_update(
					    &msg, NCS_MBCSV_ACT_UPDATE);
					plm_clean_client_info(client_info);
				}
			}
		}
...
void plms_process_mds_info_event(PLMS_EVT *plm_evt)
{
	...
	if (mds_info->svc_id == NCSMDS_SVC_ID_PLMS)
		...
		LOG_ER("Received a service event for an unknown service %u",
		       mds_info->svc_id);
		return;
	}
	...
	case NCSMDS_DOWN:
		if (mds_info->svc_id == NCSMDS_SVC_ID_PLMA) {
			...
			plms_clean_agent_db(plm_evt->req_evt.mds_info.dest,
					    cb->ha_state);
		}
