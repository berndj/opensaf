...
void plms_cb_dump_routine()
{
	...
	fprintf(fp, "\n AMF Invocation ID: %llu.",
		(unsigned long long)cb->amf_invocation_id);
	...
	fprintf(fp, "\n MBCSV_CKPT handle: %u.",
		(unsigned int)cb->mbcsv_ckpt_hdl);
	fprintf(fp, "\n Health Check Started: %u.",
		(unsigned int)cb->healthCheckStarted);
	...
	while (ent) {
		...
		if (ent->dn_name_str)
			...
			fprintf(
			    fp,
			    "\n\n DN name of the Entity[%d] is not available",
			    count);
		if (ent->entity_type == PLMS_HE_ENTITY)
			...
			fprintf(fp, "\n Entity-Type: NULL");
		fprintf(fp, "\n Number of configured entity path:%d",
			ent->num_cfg_ent_paths);

		if (ent->exp_readiness_status.readinessState ==
		    SA_PLM_READINESS_OUT_OF_SERVICE)
			fprintf(
			    fp,
			    "\n Expected Readiness state:SA_PLM_READINESS_OUT_OF_SERVICE");
		else if (ent->exp_readiness_status.readinessState ==
		...
		group_list =
		    (PLMS_ENTITY_GROUP_INFO_LIST *)ent->part_of_entity_groups;
		if (!group_list)
			fprintf(fp, ":NULL");
		else {
			while (group_list) {
				fprintf(fp, ":%llu",
					(unsigned long long)group_list
					    ->ent_grp_inf->entity_grp_hdl);
		...
		fprintf(fp, "\n First PLM notify id:%llu",
			(unsigned long long)ent->first_plm_ntf_id);
		...
		fprintf(fp, "\n adm_op_in_progress:%u",
			(unsigned int)ent->adm_op_in_progress);
		fprintf(fp, "\n Invocation :%llu",
			(unsigned long long)ent->adm_op_in_progress);
		fprintf(fp, "\n am_i_aff_ent:%u",
			(unsigned int)ent->am_i_aff_ent);

		/*Dependency List */
		fprintf(fp, "\n Minimum number of dependencies :%u",
			(unsigned int)ent->min_no_dep);
		fprintf(fp, "\n Dependency List");
		if (ent->min_no_dep < 1)
			fprintf(fp, ":NULL");
		else {
			...
			while (ent_group) {
				if (ent_group->plm_entity->dn_name_str)
					fprintf(
					    fp, ":%s",
					    ent_group->plm_entity->dn_name_str);
				else
					fprintf(fp, ":NULL");
		...
		fprintf(fp, "\n Management lost trigger:%u",
			(unsigned int)ent->mngt_lost_tri);
		if (ent->parent) {
			if (ent->parent->dn_name_str)
				fprintf(fp,
					"\n DN Name of the Parent entity:%s",
		...
		if (ent->leftmost_child) {
			if (ent->leftmost_child->dn_name_str)
				fprintf(
				    fp,
				    "\n DN Name of the Left most child entity:%s",
				    ent->leftmost_child->dn_name_str);
			else
				fprintf(
				    fp,
		...
		if (ent->right_sibling) {
			if (ent->right_sibling->dn_name_str)
				fprintf(
				    fp,
				    "\n DN Name of the right sibling entity:%s",
				    ent->right_sibling->dn_name_str);
			else
				fprintf(
				    fp,
		...
		if (ent->trk_info) {
			fprintf(
			    fp, "\n\t Imm admin operation id:%llu",
			    (unsigned long long)ent->trk_info->imm_adm_opr_id);
			if (ent->trk_info->root_entity) {
				if (ent->trk_info->root_entity->dn_name_str)
					fprintf(
					    fp,
					    "\n\t PLMS root entity dn name:%s",
			...
			fprintf(fp, "\n\t Track count:%u",
				(unsigned int)ent->trk_info->track_count);
			if (ent->trk_info->change_step ==
			    SA_PLM_CHANGE_VALIDATE)
				fprintf(
				    fp,
				    "\n\t Change step: SA_PLM_CHANGE_VALIDATE");
			else if (ent->trk_info->change_step ==
				 SA_PLM_CHANGE_START)
				fprintf(
				    fp,
			...
			fprintf(fp, "\n\t Track cause:%u",
				(unsigned int)ent->trk_info->track_cause);
			fprintf(fp, "\n\t Root correlation id:%llu",
				(unsigned long long)
				    ent->trk_info->root_correlation_id);
		} else
			fprintf(fp, "NULL");
		...
		if (ent->entity_type == PLMS_HE_ENTITY) {
			...
			if (ent->entity.he_entity.saPlmHEReadinessState ==
			    SA_PLM_READINESS_OUT_OF_SERVICE)
				fprintf(
				    fp,
				    "\n HE Readiness state:SA_PLM_READINESS_OUT_OF_SERVICE");
			else if (ent->entity.he_entity.saPlmHEReadinessState ==
				 SA_PLM_READINESS_IN_SERVICE)
				fprintf(
				    fp,
			...
			fprintf(fp, "\n HE Readiness flags:%llu",
				(unsigned long long)ent->entity.he_entity
				    .saPlmHEReadinessFlags);
			if (ent->entity.he_entity.saPlmHEPresenceState ==
			    SA_PLM_HE_PRESENCE_NOT_PRESENT)
				fprintf(
				    fp,
				    "\n HE Presence state:SA_PLM_HE_PRESENCE_NOT_PRESENT");
			else if (ent->entity.he_entity.saPlmHEPresenceState ==
			...
			if (ent->entity.ee_entity.safEE)
				fprintf(fp, "\n safEE:%s",
					ent->entity.ee_entity.safEE);
			else
				fprintf(fp, "\n safEE:NULL");
			if (ent->entity.ee_entity.saPlmEEType.length)
				fprintf(
				    fp, "\n saPlmEEType:%s",
				    ent->entity.ee_entity.saPlmEEType.value);
			else
				fprintf(fp, "\n saPlmEEType:NULL");
			fprintf(fp, "\n saPlmEEInstantiateTimeout:%lld",
				(long long int)ent->entity.ee_entity
				    .saPlmEEInstantiateTimeout);
			fprintf(fp, "\n saPlmEETerminateTimeout:%lld",
				(long long int)ent->entity.ee_entity
				    .saPlmEETerminateTimeout);
			if (ent->entity.ee_entity.saPlmEEAdminState ==
			    SA_PLM_EE_ADMIN_UNLOCKED)
				fprintf(
				    fp,
				    "\n EE Admin state:SA_PLM_EE_ADMIN_UNLOCKED");
			else if (ent->entity.ee_entity.saPlmEEAdminState ==
				 SA_PLM_EE_ADMIN_LOCKED)
				fprintf(
				    fp,
				    "\n EE Admin state:SA_PLM_EE_ADMIN_LOCKED");
			else if (ent->entity.ee_entity.saPlmEEAdminState ==
				 SA_PLM_EE_ADMIN_LOCKED_INSTANTIATION)
				fprintf(
				    fp,
			...
			if (ent->entity.ee_entity.saPlmEEReadinessState ==
			    SA_PLM_READINESS_OUT_OF_SERVICE)
				fprintf(
				    fp,
				    "\n EE Readiness state:SA_PLM_READINESS_OUT_OF_SERVICE");
			else if (ent->entity.ee_entity.saPlmEEReadinessState ==
				 SA_PLM_READINESS_IN_SERVICE)
				fprintf(
				    fp,
			...
			fprintf(fp, "\n EE Readiness flags:%llu",
				(unsigned long long)ent->entity.ee_entity
				    .saPlmEEReadinessFlags);
			if (ent->entity.ee_entity.saPlmEEPresenceState ==
			    SA_PLM_EE_PRESENCE_UNINSTANTIATED)
				fprintf(
				    fp,
				    "\n EE Presence state:SA_PLM_EE_PRESENCE_UNINSTANTIATED");
			else if (ent->entity.ee_entity.saPlmEEPresenceState ==
			...
			if (ent->entity.ee_entity.saPlmEEOperationalState ==
			    SA_PLM_OPERATIONAL_ENABLED)
				fprintf(
				    fp,
				    "\n EE Operation state:SA_PLM_OPERATIONAL_ENABLED");
			else if (ent->entity.ee_entity
	...
	while (he_base_info) {
		...
		if (!he_type_info) {
			...
			while (he_type_info) {
				if (he_type_info->he_type.safVersion)
					fprintf(
					    fp, "\n HE type safversion:%s",
					    he_type_info->he_type.safVersion);
				else
					fprintf(fp,
						"\n HE type safversion:NULL");
				if (he_type_info->he_type.saPlmHetIdr)
					fprintf(
					    fp, "\n HE type saPlmHetldr:%s",
					    he_type_info->he_type.saPlmHetIdr);
				else
					fprintf(fp,
		...
		count++;
		he_base_info = (PLMS_HE_BASE_INFO *)ncs_patricia_tree_getnext(
		    &cb->base_he_info, (SaUint8T *)&(he_base_info->dn_name));
	}
	...
	while (ee_base_info) {
		...
		if (!ee_type_info) {
			...
			while (ee_type_info) {
				...
				fprintf(
				    fp,
				    "\n EE type saPlmEeDefaultInstantiateTimeout:%lld",
				    (long long int)ee_type_info->ee_type
					.saPlmEeDefaultInstantiateTimeout);
				fprintf(
				    fp,
				    "\n EE type saPlmEeDefTerminateTimeout:%lld",
				    (long long int)ee_type_info->ee_type
		...
		count++;
		ee_base_info = (PLMS_EE_BASE_INFO *)ncs_patricia_tree_getnext(
		    &cb->base_ee_info, (SaUint8T *)&(ee_base_info->dn_name));
	}
	...
	if (!plm_dep) {
		...
		while (plm_dep) {
			...
			fprintf(fp,
				"\n\n DN Name of the dependency list[%d]:%s",
				count, tmp);
			fprintf(fp, "\n Number of dependencies:%d",
				plm_dep->no_of_dep);
			if (plm_dep->dependency.safDependency)
				fprintf(fp, "\n DN Name of its dependency:%s",
					plm_dep->dependency.safDependency);
			count++;
	...
	epath_emap = (PLMS_EPATH_TO_ENTITY_MAP_INFO *)ncs_patricia_tree_getnext(
	    &cb->epath_to_entity_map_info, NULL);
	if (!epath_emap) {
		...
		while (epath_emap) {
			if (epath_emap->entity_path)
				fprintf(
				    fp,
				    "\n Entity path for epath_to_entity_map_info list [%d]:%s",
				    count, epath_emap->entity_path);
			else
				fprintf(
				    fp,
				    "\n Entity path for epath_to_entity_map_info list [%d]:NULL",
				    count);
			if (epath_emap->plms_entity->dn_name_str)
				fprintf(fp, "\n DN Name of PLM entity:%s",
					epath_emap->plms_entity->dn_name_str);
			else
				fprintf(fp, "\n DN Name of PLM entity:NULL");
			count++;
			epath_emap = (PLMS_EPATH_TO_ENTITY_MAP_INFO *)
	...
	if (!client_info) {
		...
		while (client_info) {
			...
			fprintf(fp, "\n Client info handle:%llu",
				(unsigned long long)client_info->plm_handle);
			fprintf(fp, "\n Client info mds dest id:%llu",
				(unsigned long long)client_info->mdest_id);
			fprintf(fp, "\n Client info group list handle id:");
			group_list = (PLMS_ENTITY_GROUP_INFO_LIST *)
					 client_info->entity_group_list;
			while (group_list) {
				fprintf(fp, "\n \t%llu",
					(unsigned long long)group_list
					    ->ent_grp_inf->entity_grp_hdl);
	...
	count = 1;
	group_info = (PLMS_ENTITY_GROUP_INFO *)ncs_patricia_tree_getnext(
	    &cb->entity_group_info, NULL);
	if (!group_info) {
		fprintf(fp, "\n Group info is not available");
	} else {
		while (group_info) {
			fprintf(fp, "\n\n Entity group info: [%d]", count);
			fprintf(fp, "\n Entity group handle: %llu",
				(unsigned long long)group_info->entity_grp_hdl);
			fprintf(fp, "\n Agent mdest id: %llu",
				(unsigned long long)group_info->agent_mdest_id);
			fprintf(fp, "\n PLM entity list");
			ent_group_root = group_info->plm_entity_list;
			while (ent_group_root) {
				if (ent_group_root->plm_entity->dn_name_str)
					fprintf(fp, "\n \t%s",
						ent_group_root->plm_entity
						    ->dn_name_str);
				else
					fprintf(fp, "\n Dn name is NULL");
				if (ent_group_root->subtree_root->dn_name_str)
					fprintf(fp, "\n \t%s",
						ent_group_root->subtree_root
						    ->dn_name_str);
				else
					fprintf(fp,
			...
			fprintf(fp, "\n PLM Group info track flags:%x",
				group_info->track_flags);
			fprintf(fp, "\n PLM Group info track cookie:%llu",
				(unsigned long long)group_info->track_cookie);
			track_info = group_info->invocation_list;
			while (track_info) {
				fprintf(
				    fp, "\n PLM Group info invocation:%llu",
				    (unsigned long long)track_info->invocation);
				fprintf(
				    fp,
				    "\n PLM Group info track info imm_adm_opr_id:%llu",
				    (unsigned long long)
					track_info->track_info->imm_adm_opr_id);
				track_info = track_info->next;
			}
			fprintf(fp, "\n PLM Group info invocation callback:%x",
				group_info->invoke_callback);
			count++;
			client_info =
			    (PLMS_CLIENT_INFO *)ncs_patricia_tree_getnext(
				&cb->client_info,
