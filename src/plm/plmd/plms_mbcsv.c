...
SaUint32T plms_process_mbcsv_data(PLMS_MBCSV_MSG *msg,
...
SaUint32T plms_mbcsv_encode_proc(NCS_MBCSV_CB_ARG *arg)
{
	...
	msg_fmt_version =
	    m_NCS_MBCSV_FMT_GET(arg->info.encode.i_peer_version,
...
SaUint32T plms_mbcsv_decode_proc(NCS_MBCSV_CB_ARG *arg)
{
	...
	msg_fmt_version =
	    m_NCS_MBCSV_FMT_GET(arg->info.decode.i_peer_version,
				PLMS_MBCSV_VERSION, PLMS_MBCSV_VERSION_MIN);
...
SaUint32T plms_edp_mbcsv_msg_header(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
				    ...
				    EDU_ERR *o_err)
{
	...
	EDU_INST_SET plms_mbcsv_msg_header_rules[] = {
	    {EDU_START, plms_edp_mbcsv_msg_header, 0, 0, 0,
	     sizeof(PLMS_MBCSV_HEADER), 0, NULL},
	    {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
	     (long)&((PLMS_MBCSV_HEADER *)0)->msg_type, 0, NULL},
	    {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
	     (long)&((PLMS_MBCSV_HEADER *)0)->num_records, 0, NULL},
	    {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
	     (long)&((PLMS_MBCSV_HEADER *)0)->data_len, 0, NULL},
	...
	rc = m_NCS_EDU_RUN_RULES(edu_hdl, edu_tkn, plms_mbcsv_msg_header_rules,
...
SaUint32T plms_edp_trk_step_info(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
				 NCSCONTEXT ptr, uint32_t *ptr_data_len,
				 EDU_BUF_ENV *buf_env, EDP_OP_TYPE op,
				 EDU_ERR *o_err)
{
	...
	EDU_INST_SET plms_trk_step_info_rules[] = {
	    {EDU_START, plms_edp_trk_step_info, 0, 0, 0,
	     sizeof(PLMS_CKPT_TRACK_STEP_INFO), 0, NULL},
	    {EDU_EXEC, ncs_edp_sanamet, 0, 0, 0,
	     (long)&((PLMS_CKPT_TRACK_STEP_INFO *)0)->dn_name, 0, NULL},
	    {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
	     (long)&((PLMS_CKPT_TRACK_STEP_INFO *)0)->opr_type, 0, NULL},
	    {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
	     (long)&((PLMS_CKPT_TRACK_STEP_INFO *)0)->change_step, 0, NULL},
	    {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
	     (long)&((PLMS_CKPT_TRACK_STEP_INFO *)0)->root_correlation_id, 0,
	     NULL},
	    {EDU_END, 0, 0, 0, 0, 0, 0, NULL},
	};
	...
	rc = m_NCS_EDU_RUN_RULES(edu_hdl, edu_tkn, plms_trk_step_info_rules,
				 struct_ptr, ptr_data_len, buf_env, op, o_err);
	return rc;
}
/****************************************************************************
 ...
 * Notes         : None.
 *****************************************************************************/
SaUint32T plms_edp_entity_list(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
			       NCSCONTEXT ptr, uint32_t *ptr_data_len,
			       EDU_BUF_ENV *buf_env, EDP_OP_TYPE op,
			       EDU_ERR *o_err)
{
	...
	EDU_INST_SET plms_entity_list_rules[] = {
	    {EDU_START, plms_edp_entity_list, EDQ_LNKLIST, 0, 0,
	     sizeof(PLMS_CKPT_ENTITY_LIST), 0, NULL},

	    {EDU_EXEC, ncs_edp_sanamet, 0, 0, 0,
	     (long)&((PLMS_CKPT_ENTITY_LIST *)0)->entity_name, 0, NULL},

	    {EDU_EXEC, ncs_edp_sanamet, 0, 0, 0,
	     (long)&((PLMS_CKPT_ENTITY_LIST *)0)->root_entity_name, 0, NULL},

	    {EDU_TEST_LL_PTR, plms_edp_entity_list, 0, 0, 0,
	     (long)&((PLMS_CKPT_ENTITY_LIST *)0)->next, 0, NULL},
	    {EDU_END, 0, 0, 0, 0, 0, 0, NULL},
	};
	...
	if (op == EDP_OP_TYPE_ENC) {
		...
		if (*d_ptr == NULL) {
			*d_ptr = (PLMS_CKPT_ENTITY_LIST *)malloc(
			    sizeof(PLMS_CKPT_ENTITY_LIST));
			if (*d_ptr == NULL) {
				*o_err = EDU_ERR_MEM_FAIL;
				return NCSCC_RC_FAILURE;
			}
	...
	rc = m_NCS_EDU_RUN_RULES(edu_hdl, edu_tkn, plms_entity_list_rules,
				 struct_ptr, ptr_data_len, buf_env, op, o_err);
	return rc;
}
...
SaUint32T plms_edp_entity_grp_info(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
				   NCSCONTEXT ptr, uint32_t *ptr_data_len,
				   EDU_BUF_ENV *buf_env, EDP_OP_TYPE op,
				   EDU_ERR *o_err)
{
	...
	EDU_INST_SET plms_entity_grp_info_rules[] = {
	    {EDU_START, plms_edp_entity_grp_info, 0, 0, 0,
	     sizeof(PLMS_CKPT_ENTITY_GROUP_INFO), 0, NULL},
	    {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
	     (long)&((PLMS_CKPT_ENTITY_GROUP_INFO *)0)->entity_group_handle, 0,
	     NULL},
	    {EDU_EXEC, ncs_edp_mds_dest, 0, 0, 0,
	     (long)&((PLMS_CKPT_ENTITY_GROUP_INFO *)0)->agent_mdest_id, 0,
	     NULL},
	    {EDU_EXEC, plms_edp_entity_list, EDQ_POINTER, 0, 0,
	     (long)&((PLMS_CKPT_ENTITY_GROUP_INFO *)0)->entity_list, 0, NULL},
	    {EDU_EXEC, ncs_edp_uns8, 0, 0, 0,
	     (long)&((PLMS_CKPT_ENTITY_GROUP_INFO *)0)->track_flags, 0, NULL},
	    {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
	     (long)&((PLMS_CKPT_ENTITY_GROUP_INFO *)0)->track_cookie, 0, NULL},
	    {EDU_END, 0, 0, 0, 0, 0, 0, NULL},
	};
	...
	rc = m_NCS_EDU_RUN_RULES(edu_hdl, edu_tkn, plms_entity_grp_info_rules,
				 struct_ptr, ptr_data_len, buf_env, op, o_err);
	return rc;
}
				    ...
				    EDU_ERR *o_err)
{
	...
	EDU_INST_SET plms_client_info_list_rules[] = {
	    {EDU_START, plms_edp_client_info_list, 0, 0, 0,
	     sizeof(PLMS_CKPT_CLIENT_INFO_LIST), 0, NULL},
	    {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
	     (long)&((PLMS_CKPT_CLIENT_INFO_LIST *)0)->plm_handle, 0, NULL},
	    {EDU_EXEC, ncs_edp_mds_dest, 0, 0, 0,
	     (long)&((PLMS_CKPT_CLIENT_INFO_LIST *)0)->mdest_id, 0, NULL},
	    {EDU_END, 0, 0, 0, 0, 0, 0, NULL},
	};
...
SaUint32T plms_mbcsv_enc_async_update(NCS_MBCSV_CB_ARG *arg)
{
	...
	if (data->header.msg_type == PLMS_A2S_MSG_TRK_STP_INFO) {
		...
		rc = m_NCS_EDU_EXEC(&cb->edu_hdl, plms_edp_trk_step_info,
				    &arg->info.encode.io_uba, EDP_OP_TYPE_ENC,
				    &data->info.step_info, &ederror);
		if (!rc) { /* Encode failed. */
		...
		rc = m_NCS_EDU_EXEC(&cb->edu_hdl, plms_edp_entity_grp_info,
				    &arg->info.encode.io_uba, EDP_OP_TYPE_ENC,
				    &data->info.ent_grp_info, &ederror);
		if (!rc) { /* Encode failed. */
			LOG_ER(
			    "PLMS A2S plms_edp_entity_grp_info encode failed");
			return NCSCC_RC_FAILURE;
		} else
			rc = NCSCC_RC_SUCCESS;
		...
		rc = m_NCS_EDU_EXEC(&cb->edu_hdl, plms_edp_client_info_list,
				    &arg->info.encode.io_uba, EDP_OP_TYPE_ENC,
				    &data->info.ent_grp_info, &ederror);
		if (!rc) { /* Encode failed. */
			LOG_ER(
			    "PLMS A2S plms_edp_client_info_list encode failed");
			return NCSCC_RC_FAILURE;
		} else
			rc = NCSCC_RC_SUCCESS;
	}
...
SaUint32T plms_mbcsv_dec_async_update(NCS_MBCSV_CB_ARG *arg)
{
	...
	/* decode header */
	rc = m_NCS_EDU_EXEC(&cb->edu_hdl, plms_edp_mbcsv_msg_header,
			    &arg->info.decode.i_uba, EDP_OP_TYPE_DEC, &hdr,
			    &ederror);
	if (rc != NCSCC_RC_SUCCESS) {
	...
	if (msg->header.msg_type == PLMS_A2S_MSG_TRK_STP_INFO) {
		...
		rc = m_NCS_EDU_EXEC(&cb->edu_hdl, plms_edp_entity_grp_info,
				    &arg->info.decode.i_uba, EDP_OP_TYPE_DEC,
				    &entity_grp_info, &ederror);
		if (!rc) { /* Decode failed. */
			LOG_ER(
			    "PLMS A2S plms_edp_entity_grp_info decode  failed");
			return NCSCC_RC_FAILURE;
		} else
			rc = NCSCC_RC_SUCCESS;
		...
		if (!rc) { /* Decode failed. */
			LOG_ER(
			    "PLMS A2S plms_edp_client_info_list decode  failed");
			return NCSCC_RC_FAILURE;
		} else
			rc = NCSCC_RC_SUCCESS;
...
SaUint32T plms_edu_enc_trk_step_info_data(NCS_MBCSV_CB_ARG *arg)
{
	...
	/* First see that if there are any records to be check pointed. If there
	 * are no trk step info records and entity grp info records, then just
	 * send the complete response */
	if (!cb->step_info) {
		entity_grp_ptr =
		    (PLMS_ENTITY_GROUP_INFO *)ncs_patricia_tree_getnext(
			&cb->entity_group_info, (uint8_t *)0);
		if (!entity_grp_ptr) {
			/* If there are no entity grp info recs to be
			 * checkpointed close it */
	...
	ckpt_ptr = (PLMS_CKPT_TRACK_STEP_INFO *)malloc(
	    sizeof(PLMS_CKPT_TRACK_STEP_INFO));
	if (!ckpt_ptr) {
	...
	while (ptr) {
		...
		rc = m_NCS_EDU_EXEC(&cb->edu_hdl, plms_edp_trk_step_info, uba,
				    EDP_OP_TYPE_ENC, ckpt_ptr, &ederror);
		if (rc != NCSCC_RC_SUCCESS) {
			free(ckpt_ptr);
			return rc;
		}
	...
	if (num_rec < MAX_NO_OF_TRK_STEP_INFO_RECS)
		/*		cb->prev_trk_step_rec = 0xFFFF; */ /* This is to
								      indicate
								      that we
								      are done
								      with
	...
	entity_grp_ptr = (PLMS_ENTITY_GROUP_INFO *)ncs_patricia_tree_getnext(
	    &cb->entity_group_info, (uint8_t *)0);
...
static SaUint32T plms_clean_mbcsv_database()
{
	...
	while ((client_info = (PLMS_CLIENT_INFO *)ncs_patricia_tree_getnext(
		    &cb->client_info, (SaUint8T *)0)) != NULL) {
				       ...
				       PLMS_CKPT_ENTITY_GROUP_INFO *dst_ptr)
{
	...
	while (tmp_entity_ptr) {
		tmp_ckpt_ent_list_ptr = (PLMS_CKPT_ENTITY_LIST *)malloc(
		    sizeof(PLMS_CKPT_ENTITY_LIST));
		...
		memcpy(&tmp_ckpt_ent_list_ptr->entity_name,
		       &tmp_entity_ptr->plm_entity->dn_name, sizeof(SaNameT));
...
SaUint32T plms_edu_enc_entity_grp_info_data(NCS_MBCSV_CB_ARG *arg)
{
	...
	ckpt_ptr = (PLMS_CKPT_ENTITY_GROUP_INFO *)malloc(
	    sizeof(PLMS_CKPT_ENTITY_GROUP_INFO));
	if (!ckpt_ptr) {
		LOG_ER("Memory Allocation failed");
		return NCSCC_RC_FAILURE;
	}
	...
	if (cb->prev_ent_grp_hdl) {
		ptr = (PLMS_ENTITY_GROUP_INFO *)ncs_patricia_tree_getnext(
		    &cb->entity_group_info, (uint8_t *)&cb->prev_ent_grp_hdl);
	} else {
		ptr = (PLMS_ENTITY_GROUP_INFO *)ncs_patricia_tree_getnext(
		    &cb->entity_group_info, (uint8_t *)0);
	}
	...
	while (ptr) {
		...
		/* Encode the individual record */
		rc = m_NCS_EDU_EXEC(&cb->edu_hdl, plms_edp_entity_grp_info, uba,
				    EDP_OP_TYPE_ENC, ckpt_ptr, &ederror);
		if (rc != NCSCC_RC_SUCCESS) {
			free(ckpt_ptr);
			TRACE_LEAVE();
		...
		ptr = (PLMS_ENTITY_GROUP_INFO *)ncs_patricia_tree_getnext(
		    &cb->entity_group_info, (uint8_t *)&ptr->entity_grp_hdl);
	}
...
SaUint32T plms_edu_enc_client_info_data(NCS_MBCSV_CB_ARG *arg)
{
	...
	ckpt_client_info = (PLMS_CKPT_CLIENT_INFO_LIST *)malloc(
	...
	if (cb->prev_client_info_hdl) {
		client_info = (PLMS_CLIENT_INFO *)ncs_patricia_tree_getnext(
		    &cb->client_info, (uint8_t *)&cb->prev_client_info_hdl);
	} else {
		client_info = (PLMS_CLIENT_INFO *)ncs_patricia_tree_getnext(
		    &cb->client_info, (uint8_t *)0);
		if (!client_info) {
			if (arg->info.encode.io_msg_type ==
			    NCS_MBCSV_MSG_COLD_SYNC_RESP)
				arg->info.encode.io_msg_type =
			...
			async_upd_cnt =
			    ncs_enc_reserve_space(uba, sizeof(uint32_t));
			if (async_upd_cnt == NULL) {
				LOG_ER("Async update count encode Failed");
				return NCSCC_RC_FAILURE;
			}
			ncs_encode_32bit(&async_upd_cnt, cb->async_update_cnt);
			ncs_enc_claim_space(uba, sizeof(uint32_t));
			TRACE("Async update count sent to stdby : %d",
			      cb->async_update_cnt);
			free(ckpt_client_info);
			return NCSCC_RC_SUCCESS;
		}
	...
	while (client_info) {
		...
		/* Encode the individual record */
		rc =
		    m_NCS_EDU_EXEC(&cb->edu_hdl, plms_edp_client_info_list, uba,
				   EDP_OP_TYPE_ENC, ckpt_client_info, &ederror);
		if (rc != NCSCC_RC_SUCCESS) {
			free(ckpt_client_info);
			return rc;
		}
		...
		client_info = (PLMS_CLIENT_INFO *)ncs_patricia_tree_getnext(
		    &cb->client_info, (uint8_t *)&client_info->plm_handle);
	}
	if (client_info != NULL) {
...
SaUint32T plms_mbcsv_decode_cold_sync_data(NCS_MBCSV_CB_ARG *cbk_arg)
{
	...
	while (1) {
		...
		if ((rc = plms_dec_mbcsv_header(&cbk_arg->info.decode.i_uba,
						&data->header)) !=
		    NCSCC_RC_SUCCESS) {
		...
		if (data->header.msg_type == PLMS_A2S_MSG_TRK_STP_INFO) {
			...
			while (num_rec) {
				...
				rc = m_NCS_EDU_EXEC(
				    &cb->edu_hdl, plms_edp_trk_step_info,
				    &cbk_arg->info.decode.i_uba,
				    EDP_OP_TYPE_DEC, &trk_step_info, &ederror);
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "COLD SYNC track step info decode failed");
					free(data);
					return NCSCC_RC_FAILURE;
				}

				rc = plms_process_mbcsv_data(
				    data, cbk_arg->info.decode.i_action);
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "COLD SYNC mbcsv data update failed");
					free(data);
					return NCSCC_RC_FAILURE;
				}

				memset(trk_step_info, 0,
				       sizeof(PLMS_CKPT_TRACK_STEP_INFO));
				--num_rec;
			...
			while (num_rec) {
				...
				rc = m_NCS_EDU_EXEC(&cb->edu_hdl,
						    plms_edp_entity_grp_info,
						    &cbk_arg->info.decode.i_uba,
						    EDP_OP_TYPE_DEC,
						    &entity_grp_info, &ederror);
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "COLD SYNC entity grp info decode failed");
					free(data);
					return NCSCC_RC_FAILURE;
				}
				...
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "COLD SYNC mbcsv data add failed");
					free(data);
					return NCSCC_RC_FAILURE;
				}

				memset(entity_grp_info, 0,
				       sizeof(PLMS_CKPT_ENTITY_GROUP_INFO));
				--num_rec;
			} /* end of while loop.  */
			/* Free the entity list before freeing the group */
			...
			while (num_rec) {
				...
				rc = m_NCS_EDU_EXEC(
				    &cb->edu_hdl, plms_edp_client_info_list,
				    &cbk_arg->info.decode.i_uba,
				    EDP_OP_TYPE_DEC, &client_info, &ederror);
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "COLD SYNC client info decode failed");
					free(data);
					return NCSCC_RC_FAILURE;
				}
		...
		if ((cbk_arg->info.decode.i_msg_type ==
			...
			TRACE("Async update count received : %d",
			      cb->async_update_cnt);
		}
		free(data);
		TRACE_LEAVE();
...
uint32_t plms_mbcsv_add_trk_step_info_rec(PLMS_MBCSV_MSG *msg)
{
	...
	PLMS_CB *cb = plms_cb;
	PLMS_CKPT_TRACK_STEP_INFO *ptr = NULL, *prev_ptr = NULL,
				  *tmp_ptr = NULL;

	ptr = (PLMS_CKPT_TRACK_STEP_INFO *)malloc(
	    sizeof(PLMS_CKPT_TRACK_STEP_INFO));
	if (!ptr) {
	...
	strcpy((SaInt8T *)ptr->dn_name.value,
	       (SaInt8T *)msg->info.step_info.dn_name.value);
...
SaUint32T plms_mbcsv_update_trk_step_info_rec(PLMS_MBCSV_MSG *msg)
{
	...
	tmp_ptr = cb->step_info;
	while (tmp_ptr) {
		if (strncmp((SaInt8T *)tmp_ptr->dn_name.value,
			    (SaInt8T *)msg->info.step_info.dn_name.value,
			    msg->info.step_info.dn_name.length) == 0) {
			/* Changed rec found ?? */
...
SaUint32T plms_mbcsv_rem_trk_step_info_rec(PLMS_MBCSV_MSG *msg)
{
	...
	if (!tmp_ptr) {
		/* Means we didn't find rec with the given name */
		LOG_ER(
		    "Unable to fetch the record to be removed at local database");
		rc = NCSCC_RC_FAILURE;
	} else {
		/* Means everything went fine */
		rc = NCSCC_RC_SUCCESS;
...
SaUint32T plms_mbcsv_add_entity_grp_info_rec(PLMS_MBCSV_MSG *msg)
{
	...
	/* Since the same function is used for cold sync add and async update,
	 * first we see if the entity grp info already exist. if exists, modify
	 * the existing record, if not, create a new record and add it to the
	 * list */

	/* Traverse the list and find out the right group */
	ptr = cb->ckpt_entity_grp_info;
	while (ptr) {
		if (ptr->entity_group_handle ==
		    msg->info.ent_grp_info.entity_group_handle) {
			/* here means we found out the group info structure. Add
			 * the entity names to the entity list */
			msg_list_ptr = msg->info.ent_grp_info.entity_list;

			while (msg_list_ptr) {
				/* First allocate mem for entity list and add it
				 * to group info */
				cur_list_ptr = (PLMS_CKPT_ENTITY_LIST *)malloc(
				    sizeof(PLMS_CKPT_ENTITY_LIST));
				if (!cur_list_ptr) {
					/* FIXME : See if anything needs to
					 * freed*/
					LOG_ER(
					    "Mem alloc failed for entity list");

					return NCSCC_RC_FAILURE;
				}
				memset(cur_list_ptr, 0,
				       sizeof(PLMS_CKPT_ENTITY_LIST));
	...
	if (ptr == NULL) /* Means we didn't find existing entity grp info */
	{
		...
		ptr = (PLMS_CKPT_ENTITY_GROUP_INFO *)malloc(
		    sizeof(PLMS_CKPT_ENTITY_GROUP_INFO));

		if (!ptr) {
			LOG_ER(
			    "Mem Alloc failed for adding entity grp info rec");
			assert(0);
		}
		...
		while (msg_list_ptr) {
			...
			/* First allocate mem for entity list and add it to
			 * group info */
			cur_list_ptr = (PLMS_CKPT_ENTITY_LIST *)malloc(
			    sizeof(PLMS_CKPT_ENTITY_LIST));
			if (!cur_list_ptr) {
			...
			strncpy(str, (SaInt8T *)msg_list_ptr->entity_name.value,
				msg_list_ptr->entity_name.length);

			TRACE("Entity %s is added", str);

			memcpy(&cur_list_ptr->entity_name,
			       &msg_list_ptr->entity_name, sizeof(SaNameT));
...
SaUint32T plms_mbcsv_rmv_entity_grp_info_rec(PLMS_MBCSV_MSG *msg)
{
	...
	while (ptr) {
		if (ptr->entity_group_handle ==
		    msg->info.ent_grp_info.entity_group_handle) {
			/* We found out the entity group, now remove the
			 * entities from the list */
			msg_ptr = msg->info.ent_grp_info.entity_list;
			if (msg_ptr == NULL) {
				/* if there are no entities to be removed means
				 * remove group */
				group_remove = 1;
			}
			while (msg_ptr) {
				...
				while (grp_ptr) {
					if (strncmp(
						(SaInt8T *)
						    grp_ptr->entity_name.value,
						(SaInt8T *)
						    msg_ptr->entity_name.value,
						msg_ptr->entity_name.length) ==
					    0) {
						strncpy(str,
							(SaInt8T *)grp_ptr
							    ->entity_name.value,
						...
						TRACE("Entity %s is removed",
						      str);

						/* means entity needs to be
						 * removed. Here we doesn't
						 * check for subtree flag as it
						 * is already checked at the
						 * active side */
						if (prev_ptr == NULL) {
							ptr->entity_list =
							    grp_ptr->next;
	...
	TRACE("Group with grp hdl : %llu not found",
	      msg->info.ent_grp_info.entity_group_handle);
	m_NCS_UNLOCK(&cb->cb_lock, NCS_LOCK_WRITE);
	TRACE_LEAVE();
	return NCSCC_RC_SUCCESS;
...
SaUint32T plms_mbcsv_add_client_info(PLMS_MBCSV_MSG *msg)
{
	...
	if (NCSCC_RC_SUCCESS !=
	    ncs_patricia_tree_add(&cb->client_info, &client_info->pat_node)) {
...
SaUint32T plms_mbcsv_rmv_client_info(PLMS_MBCSV_MSG *msg)
{
	...
	client_info = (PLMS_CLIENT_INFO *)ncs_patricia_tree_get(
	    &cb->client_info, (uint8_t *)&msg->info.client_info.plm_handle);
	if (!client_info) {
		/* LOG ERROR */
	...
	if ((cb->ha_state == SA_AMF_HA_STANDBY) ||
		...
		if (msg->header.msg_type == PLMS_A2S_MSG_TRK_STP_INFO) {
			if (action == NCS_MBCSV_ACT_ADD) {
				...
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "Unable to add trk step info rec to database");
					return rc;
				}
				...
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "Unable to modify trk step info record database");
					return rc;
				}
				...
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "Unable to remove trk step info rec");
					return rc;
				}
			}
			...
			if (action == NCS_MBCSV_ACT_ADD) {
				...
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "Unable to add entity grp info rec to database");
					return rc;
				}
			} else if (action == NCS_MBCSV_ACT_UPDATE) {
				...
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "Unable to update entity grp info rec ");
					return rc;
				}
			} else if (action == NCS_MBCSV_ACT_RMV) {
				...
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "Unable to remove entity grp info rec");
					return rc;
				}
			}
			...
			if (action == NCS_MBCSV_ACT_ADD) {
				...
				if (rc != NCSCC_RC_SUCCESS) {
					LOG_ER(
					    "Unable to add client info rec to database");
					return rc;
				}
			} else if (action == NCS_MBCSV_ACT_UPDATE) {
