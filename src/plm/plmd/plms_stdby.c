...
SaUint32T plms_perform_pending_admin_lock(PLMS_CKPT_TRACK_STEP_INFO *tack_step,
					  PLMS_ENTITY *ent);
SaUint32T
plms_perform_pending_admin_shutdown(PLMS_CKPT_TRACK_STEP_INFO *tack_step,
				    PLMS_ENTITY *ent);
SaUint32T
plms_perform_pending_admin_deactivate(PLMS_CKPT_TRACK_STEP_INFO *track_step,
				      PLMS_ENTITY *ent);
static SaUint32T
plms_perform_pending_admin_clbk(PLMS_ENTITY_GROUP_INFO_LIST *grp_list,
	...
	while (plm_ent != NULL) {
		if ((plm_ent->entity_type == PLMS_HE_ENTITY) &&
			...
			epath_to_ent->entity_path = (SaInt8T *)calloc(
			    1, strlen(plm_ent->entity.he_entity
					  .saPlmHECurrEntityPath) +
				   1);
			strcpy(epath_to_ent->entity_path,
			       plm_ent->entity.he_entity.saPlmHECurrEntityPath);
			ret_err = (*hsm_func_ptr)(epath_to_ent->entity_path,
						  &epath_to_ent->epath_key);
			if (NCSCC_RC_SUCCESS != ret_err) {
				LOG_ER("function returned the error %d",
				       ret_err);
				return ret_err;
			}
			...
			TRACE(
			    "Entity Path: %s of entity: %s added to epath_to_ent_map tree",
			    plm_ent->entity.he_entity.saPlmHECurrEntityPath,
			    plm_ent->dn_name_str);

			/* get the state model so we can perform HPI operations
			 */
			ret_err = (*hsm_get_state_model)(
			    &epath_to_ent->epath_key, &plm_ent->state_model);
			if (NCSCC_RC_SUCCESS != ret_err) {
...
SaUint32T plms_build_ent_grp_tree()
{
	...
	while (ckpt_grp_info) {
		...
		while (client_info) {
			if (client_info->mdest_id ==
			    ckpt_grp_info->agent_mdest_id)
				break;
			client_info =
		...
		if (NULL == client_info) {
			LOG_ER("Client_info corresponding to mdest: %" PRIu64
			       " is not \
				found",
			       ckpt_grp_info->agent_mdest_id);
			return NCSCC_RC_SUCCESS;
		...
		if (!grp_info) {
			...
			if (ckpt_grp_info->entity_group_handle >
			    entity_grp_hdl_pool)
				entity_grp_hdl_pool =
				    ckpt_grp_info->entity_group_handle;
		...
		if (!grp_node) {
			LOG_CR(
			    "PLMS_ENTITY_GROUP_INFO_LIST memory alloc failed, error val:%s",
			    strerror(errno));
			free(grp_info);
			continue;
		}
		...
		while (entity_list) {
			...
			grp_entity_list = (PLMS_GROUP_ENTITY_ROOT_LIST *)malloc(
			...
			PLMS_ENTITY_GROUP_INFO_LIST *egrp_list =
...
void plms_perform_pending_admin_operation()
{
	...
	while (track_step) {
		...
		ent = (PLMS_ENTITY *)ncs_patricia_tree_get(
		    &(cb->entity_info), (SaUint8T *)&(track_step->dn_name));
		if (NULL == ent) {
		...
		case SA_PLM_CAUSE_LOCK:
			plms_perform_pending_admin_lock(track_step, ent);
			break;
		case SA_PLM_CAUSE_SHUTDOWN:
			plms_perform_pending_admin_shutdown(track_step, ent);
			break;
		case SA_PLM_CAUSE_HE_DEACTIVATION:
			plms_perform_pending_admin_deactivate(track_step, ent);
			break;
		default:
				...
				PLMS_CKPT_TRACK_STEP_INFO *track_step)
{
	...
	while (grp_list) {
		...
		agent_evt.req_evt.agent_track.track_cbk.tracked_entities
		    .entities = (SaPlmReadinessTrackedEntityT *)malloc(
		...
		ret_err = plms_mds_normal_send(
		    cb->mds_hdl, NCSMDS_SVC_ID_PLMS, (void *)(&agent_evt),
 ...
 ***********************************************************************/
SaUint32T
plms_perform_pending_admin_shutdown(PLMS_CKPT_TRACK_STEP_INFO *track_step,
				    PLMS_ENTITY *ent)
{
	...
	plms_ent_list_grp_list_add(aff_ent_list, &group_info_list);
	...
	while (head) {
		plms_readiness_state_set(
		    head->plm_entity, SA_PLM_READINESS_OUT_OF_SERVICE, NULL,
	...
	while (head) {
		plms_readiness_flag_mark_unmark(
		    head->plm_entity, SA_PLM_RF_DEPENDENCY, true,
		    head->plm_entity->parent, SA_NTF_OBJECT_OPERATION,
		    SA_PLM_NTFID_STATE_CHANGE_ROOT);
		head = head->next;
	}
 ...
 *
 ***********************************************************************/
SaUint32T plms_perform_pending_admin_lock(PLMS_CKPT_TRACK_STEP_INFO *track_step,
					  PLMS_ENTITY *ent)
{
	SaUint32T ret_err = NCSCC_RC_FAILURE;
			...
			if (tmp_aff_ent_list->plm_entity) {
				if (PLMS_EE_ENTITY ==
					...
					ret_err = plms_ee_term(
					    tmp_aff_ent_list->plm_entity, false,
					    0);
					if (NCSCC_RC_SUCCESS != ret_err) {
						LOG_ER(
						    "EE %s term FAILED.",
