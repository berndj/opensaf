...
******************************************************************************/
void plms_he_adm_fsm_init(
    PLMS_ADM_FUNC_PTR plm_HE_adm_state_op[][SA_PLM_ADMIN_OP_MAX])
{
	TRACE_ENTER2();
	...
	plm_HE_adm_state_op[SA_PLM_HE_ADMIN_SHUTTING_DOWN]
			   [SA_PLM_ADMIN_SHUTDOWN] =
			       plms_HE_adm_shutdown_state_shutdown_op;
	plm_HE_adm_state_op[SA_PLM_HE_ADMIN_SHUTTING_DOWN]
			   [SA_PLM_ADMIN_ACTIVATE] =
			       plms_HE_adm_shutdown_state_act_op;
	plm_HE_adm_state_op[SA_PLM_HE_ADMIN_SHUTTING_DOWN]
			   [SA_PLM_ADMIN_DEACTIVATE] =
			       plms_HE_adm_shutdown_state_deact_op;

	plm_HE_adm_state_op[SA_PLM_HE_ADMIN_LOCKED_INACTIVE]
			   [SA_PLM_ADMIN_UNLOCK] =
			       plms_HE_adm_lckinact_state_unlock_op;
	plm_HE_adm_state_op[SA_PLM_HE_ADMIN_LOCKED_INACTIVE]
			   [SA_PLM_ADMIN_LOCK] =
			       plms_HE_adm_lckinact_state_lock_op;
	plm_HE_adm_state_op[SA_PLM_HE_ADMIN_LOCKED_INACTIVE]
			   [SA_PLM_ADMIN_SHUTDOWN] =
			       plms_HE_adm_lckinact_state_shutdown_op;
	plm_HE_adm_state_op[SA_PLM_HE_ADMIN_LOCKED_INACTIVE]
			   [SA_PLM_ADMIN_ACTIVATE] =
			       plms_HE_adm_lckinact_state_act_op;
	plm_HE_adm_state_op[SA_PLM_HE_ADMIN_LOCKED_INACTIVE]
			   [SA_PLM_ADMIN_DEACTIVATE] =
			       plms_HE_adm_lckinact_state_deact_op;
...
******************************************************************************/
void plms_ee_adm_fsm_init(
    PLMS_ADM_FUNC_PTR plm_EE_adm_state_op[][SA_PLM_ADMIN_OP_MAX])
{
	TRACE_ENTER2();

	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_UNLOCKED][SA_PLM_ADMIN_UNLOCK] =
	    plms_EE_adm_unlock_state_unlock_op;
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_UNLOCKED][SA_PLM_ADMIN_LOCK] =
	    plms_EE_adm_unlock_state_lock_op;
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_UNLOCKED][SA_PLM_ADMIN_SHUTDOWN] =
	    plms_EE_adm_unlock_state_shutdown_op;
	...
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_LOCKED][SA_PLM_ADMIN_UNLOCK] =
	    plms_EE_adm_locked_state_unlock_op;
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_LOCKED][SA_PLM_ADMIN_LOCK] =
	    plms_EE_adm_locked_state_lock_op;
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_LOCKED][SA_PLM_ADMIN_SHUTDOWN] =
	    plms_EE_adm_locked_state_shutdown_op;
	...
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_SHUTTING_DOWN]
			   [SA_PLM_ADMIN_UNLOCK] =
			       plms_EE_adm_shutdown_state_unlock_op;
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_SHUTTING_DOWN][SA_PLM_ADMIN_LOCK] =
	    plms_EE_adm_shutdown_state_lock_op;
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_SHUTTING_DOWN]
			   [SA_PLM_ADMIN_SHUTDOWN] =
			       plms_EE_adm_shutdown_state_shutdown_op;
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_SHUTTING_DOWN]
			   [SA_PLM_ADMIN_UNLOCK_INSTANTIATION] =
	...
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_LOCKED_INSTANTIATION]
			   [SA_PLM_ADMIN_UNLOCK] =
			       plms_EE_adm_lckinst_state_unlock_op;
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_LOCKED_INSTANTIATION]
			   [SA_PLM_ADMIN_LOCK] =
			       plms_EE_adm_lckinst_state_lock_op;
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_LOCKED_INSTANTIATION]
			   [SA_PLM_ADMIN_SHUTDOWN] =
			       plms_EE_adm_lckinst_state_shutdown_op;
	plm_EE_adm_state_op[SA_PLM_EE_ADMIN_LOCKED_INSTANTIATION]
			   [SA_PLM_ADMIN_UNLOCK_INSTANTIATION] =
...
SaUint32T plms_HE_adm_unlock_state_unlock_op(PLMS_EVT *evt)
{
	...
	ret_err = saImmOiAdminOperationResult(
	    cb->oi_hdl, evt->req_evt.admin_op.inv_id, SA_AIS_ERR_NO_OP);
	if (SA_AIS_OK != ret_err) {
		LOG_ER("Sending response to IMM failed. Ent: %s, \
		Operations: Unlock, ret_err: %d",
		       tmp, ret_err);
	}
...
SaUint32T plms_HE_adm_unlock_state_lock_op(PLMS_EVT *evt)
{
	...
	ent = (PLMS_ENTITY *)ncs_patricia_tree_get(
	    &(cb->entity_info), (SaUint8T *)&(evt->req_evt.admin_op.dn_name));
	if (NULL == ent) {
		LOG_ER("Ent not found in entity_info tree for dn_name: %s",
		       tmp);
		ret_err = saImmOiAdminOperationResult(
	...
	if ((0 == (strcmp(evt->req_evt.admin_op.option,
		...
		if ((SA_PLM_READINESS_OUT_OF_SERVICE ==
			...
			if (ent->am_i_aff_ent) {
				...
				entity for some other operation.",
				       tmp);
				...
				plms_admin_state_set(
				    ent, SA_PLM_HE_ADMIN_LOCKED, NULL,
	...
	if (0 == (strcmp(evt->req_evt.admin_op.option,
		...
		ret_err =
		    plms_try_lock(ent, evt->req_evt.admin_op, aff_ent_list);

	} else if (0 == (strcmp(evt->req_evt.admin_op.option,
				SA_PLM_ADMIN_LOCK_OPTION_FORCED))) {
		ret_err =
		    plms_forced_lock(ent, evt->req_evt.admin_op, aff_ent_list);
		...
		ret_err =
		    plms_default_lock(ent, evt->req_evt.admin_op, aff_ent_list);
	}
	TRACE_LEAVE2("ret_err: %d", ret_err);
	return ret_err;
}
...
SaUint32T plms_HE_adm_unlock_state_act_op(PLMS_EVT *evt)
{
	...
	ret_err = saImmOiAdminOperationResult(
	    cb->oi_hdl, evt->req_evt.admin_op.inv_id, SA_AIS_ERR_BAD_OPERATION);
	if (SA_AIS_OK != ret_err) {
		LOG_ER("Sending response to IMM failed. Ent: %s, \
		Operations: Activate, ret_err: %d",
		       tmp, ret_err);
	}
...
SaUint32T plms_HE_adm_unlock_state_deact_op(PLMS_EVT *evt)
{
	...
	if (SA_AIS_OK != ret_err) {
		LOG_ER("Sending response to IMM failed. Ent: %s, \
		Operations: Deactivate, ret_err: %d",
		       tmp, ret_err);
	}
...
SaUint32T plms_HE_adm_locked_state_lock_op(PLMS_EVT *evt)
{
	...
	plms_get_str_from_dn_name(&(evt->req_evt.admin_op.dn_name), tmp);
	TRACE_ENTER2(
	    "NO Operation performed. Entity: %s. Operation: lock to lock, Variant: %s",
	    tmp, evt->req_evt.admin_op.option);

	ret_err = saImmOiAdminOperationResult(
	    cb->oi_hdl, evt->req_evt.admin_op.inv_id, SA_AIS_ERR_NO_OP);
	if (SA_AIS_OK != ret_err) {
		LOG_ER("Sending response to IMM failed. Ent: %s, \
		Operation: lock to lock, ret_err: %d",
		       tmp, ret_err);
	}
...
SaUint32T plms_HE_adm_locked_state_deact_op(PLMS_EVT *evt)
{
	...
	if (NULL == ent) {
		...
		if (SA_AIS_OK != ret_err) {
			LOG_ER("Sending response to IMM failed. Ent: %s, \
			Operation: Deactivate, ret_err: %d",
			       tmp, ret_err);
		}
		TRACE_LEAVE2("ret_err: %d", ret_err);
		return ret_err;
	...
	if (ent->am_i_aff_ent) {
		...
		entity %s is in another admin context: %d.",
	...
	if (NCSCC_RC_SUCCESS != ret_err) {
		...
		ret_err = saImmOiAdminOperationResult(
		    cb->oi_hdl, evt->req_evt.admin_op.inv_id, SA_AIS_OK);
		if (NCSCC_RC_SUCCESS != ret_err) {
			LOG_ER("Sending response to IMM failed. Ent: \
			%s, Operation: Deactivate, ret_err: %d",
			       tmp, ret_err);
...
SaUint32T plms_HE_adm_lckinact_state_act_op(PLMS_EVT *evt)
{
	...
	if (NCSCC_RC_SUCCESS != ret_err) {
		LOG_ER("HE %s activation failed. ret_err = %d",
...
SaUint32T plms_HE_adm_reset_op(PLMS_EVT *evt)
{
	...
	if (NULL == ent) {
		...
		not found in entity_info tree for dn_name %s",
		       tmp);
	...
	if (plms_anc_chld_dep_adm_flag_is_set(ent, aff_ent_list)) {
		...
		affected entity is in other admin context. Ent: %s",
		       tmp);
	...
	/* Reset the target HE.*/
	ret_err =
	    plms_he_reset(ent, 1 /*adm_op*/, 1 /*mngt_cbk*/, SAHPI_COLD_RESET);
	if (NCSCC_RC_SUCCESS != ret_err) {
	...
	if (plms_is_rdness_state_set(ent, SA_PLM_READINESS_OUT_OF_SERVICE)) {
		TRACE("HE reset: Ent in OOS state. No cbk called. Ent: %s",
		      tmp);
		ret_err = saImmOiAdminOperationResult(
		    cb->oi_hdl, evt->req_evt.admin_op.inv_id, SA_AIS_OK);
		if (SA_AIS_OK != ret_err) {
	...
	plms_ee_chld_of_aff_he_find(ent, aff_he_list, aff_ent_list,
				    &aff_chld_ee_list);
	...
	plms_ee_not_chld_of_aff_he_find(aff_ent_list, aff_chld_ee_list,
					&dep_chld_ee_list);
	...
	while (head) {
		...
		if (NCSCC_RC_SUCCESS != ret_err) {
			...
			head = head->next;
			plms_ent_from_ent_list_rem(temp->plm_entity,
						   &aff_he_list);

		} else {
			plms_presence_state_set(
			    head->plm_entity, SA_PLM_HE_PRESENCE_INACTIVE, ent,
			    SA_NTF_MANAGEMENT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_DEP);
			head = head->next;
		}
	...
	while (head) {
		plms_presence_state_set(
		    head->plm_entity, SA_PLM_EE_PRESENCE_UNINSTANTIATED, ent,
		    SA_NTF_MANAGEMENT_OPERATION, SA_PLM_NTFID_STATE_CHANGE_DEP);
		plms_readiness_flag_mark_unmark(
		    head->plm_entity, SA_PLM_RF_DEPENDENCY, 1 /*mark*/, ent,
		    SA_NTF_MANAGEMENT_OPERATION, SA_PLM_NTFID_STATE_CHANGE_DEP);
	...
	while (head) {
		...
		plms_presence_state_set(
		    head->plm_entity, SA_PLM_HE_PRESENCE_ACTIVE, ent,
		    SA_NTF_MANAGEMENT_OPERATION, SA_PLM_NTFID_STATE_CHANGE_DEP);
		head = head->next;
	}
	...
	if (NCSCC_RC_SUCCESS == plms_move_ent_to_insvc(ent, &is_flag_aff)) {
		...
		dependency flag.*/
		plms_move_dep_ent_to_insvc(ent->rev_dep_list, &act_aff_ent_list,
					   0);
	...
	if ((0 == (strcmp(evt->req_evt.admin_op.option,
		...
		if ((SA_PLM_READINESS_OUT_OF_SERVICE ==
			...
			if (ent->am_i_aff_ent) {
				...
				is already in other operaional context.",
				       tmp);
				...
				plms_admin_state_set(
				    ent, SA_PLM_EE_ADMIN_LOCKED, NULL,
				    SA_NTF_MANAGEMENT_OPERATION,
				    SA_PLM_NTFID_STATE_CHANGE_ROOT);
...
SaUint32T plms_EE_adm_unlock_state_unlckinst_op(PLMS_EVT *evt)
{
	...
	if (SA_AIS_OK != ret_err) {
		LOG_ER("Sending response to IMM failed. Ent: %s, \
		Operations: Unlckinst, ret_err: %d",
		       tmp, ret_err);
	}
...
SaUint32T plms_EE_adm_locked_state_lckinst_op(PLMS_EVT *evt)
{
	...
	if (NULL == ent) {
		...
		tree is not found for dn_name: %s",
		       tmp);
		ret_err = saImmOiAdminOperationResult(
		    cb->oi_hdl, evt->req_evt.admin_op.inv_id,
		    SA_AIS_ERR_NO_RESOURCES);
	...
	none.*/
	/* Mark the admin state to locked-inactive.*/
	plms_admin_state_set(ent, SA_PLM_EE_ADMIN_LOCKED_INSTANTIATION, NULL,
			     SA_NTF_MANAGEMENT_OPERATION,
			     SA_PLM_NTFID_STATE_CHANGE_ROOT);
...
SaUint32T plms_EE_adm_lckinst_state_unlckinst_op(PLMS_EVT *evt)
{
	...
	if (NULL == ent) {
		...
		entity_info tree is not found for dn_name: %s",
		       tmp);
...
SaUint32T plms_EE_adm_restart(PLMS_EVT *evt)
{
	...
	if (NULL == ent) {
		...
		if (SA_AIS_OK != ret_err) {
			LOG_ER("Sending response to IMM failed. Ent: %s, \
			Operation: Admin-restart, ret_err: %d",
			       tmp, ret_err);
		}
	...
	if (SA_PLM_EE_PRESENCE_INSTANTIATED !=
		...
		if (SA_AIS_OK != ret_err) {
			LOG_ER("Sending response to IMM failed. Ent: %s, \
			Operation: Adm-restart, ret_err: %d",
			       tmp, ret_err);
		}
	...
	*/
	if (plms_anc_chld_dep_adm_flag_is_set(ent, aff_ent_list)) {
		...
		the affected entity is in another operational context.",
		       tmp);
	...
	if (0 == strcmp(evt->req_evt.admin_op.option,
		...
		ret_err =
		    plms_ee_abrupt_restart_process(ent, evt, aff_ent_list);

	} else {

		ret_err =
		    plms_ee_graceful_restart_process(ent, evt, aff_ent_list);
	}
...
SaUint32T plms_adm_remove(PLMS_EVT *evt)
{
	...
	PLMS_GROUP_ENTITY *chld_list = NULL, *head, *log_head,
			  *aff_ent_list = NULL;
	PLMS_TRACK_INFO trk_info;
	...
	if (!plms_rdness_flag_is_set(ent, SA_PLM_RF_MANAGEMENT_LOST)) {
		...
		admin remove is not valid. Ent: %s",
		       tmp);
	...
	if (PLMS_HE_ENTITY == ent->entity_type) {
      ...
      release.*/
			...
			plms_readiness_state_set(
			    ent, SA_PLM_READINESS_OUT_OF_SERVICE, NULL,
			    SA_NTF_MANAGEMENT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);
...
SaUint32T plms_adm_repair(PLMS_EVT *evt)
{
	...
	if (((PLMS_HE_ENTITY == ent->entity_type) &&
		...
		is not set. Nothing to do. Return.Ent: %s",
		       ent->dn_name_str);
		ret_err = saImmOiAdminOperationResult(
		    cb->oi_hdl, evt->req_evt.admin_op.inv_id, SA_AIS_ERR_NO_OP);
		if (NCSCC_RC_SUCCESS != ret_err) {
	...
	if (((PLMS_HE_ENTITY == ent->entity_type) &&
		...
		LOG_ER("Operational state is ENABLED and imminent failure flag\
		is set==>Failure is not yet happened.Ent: %s",
		       ent->dn_name_str);
		...
		/* If the operational state is disabled, means imminent failure
		 * flag no set. Hence 1. Mark the operational state to enable.
		 * 2. Clear isolate pending flag.
		 * 3. Clear imminent failure flag(Not required as wont be set).
		 * 4. activate/instantiate the ent (as the entity would have
		 * been isolated  and hence would be in inactive/uninstantiated
		 * state). 5. Clear dependency-imminent-failure flag of all
		 * the aff ents( not needed as wont be set.).
		 */
		...
		plms_readiness_flag_mark_unmark(ent, SA_PLM_RF_ISOLATE_PENDING,
						0 /* unmark*/, NULL,
						SA_NTF_MANAGEMENT_OPERATION,
						SA_PLM_NTFID_STATE_CHANGE_ROOT);

		/* Check is the imminet-failure flag of the entity can be set.*/
		plms_dep_immi_flag_check_and_set(ent, &aff_ent_list_flag,
						 &is_set);
#if 0
		...
		if (NCSCC_RC_SUCCESS != ret_err) {
			...
			if (!is_set) {
				...
				if (NCSCC_RC_SUCCESS != ret_err) {
					LOG_ER(
					    "Sending admin response to IMM failed.IMM Ret code: %d",
					    ret_err);
				} else {
					TRACE(
					    "Sending admin response to IMM successful.");
				}
				return ret_err;
			}
		} else {
			is_enabled = 1;
			/* There is no change in readiness status in this
			context. Hence store the context, and use the cause when
			the EE/HE is instantiated/activated.*/
			trk_info_flt = (PLMS_TRACK_INFO *)calloc(
			    1, sizeof(PLMS_TRACK_INFO));
			if (NULL == trk_info_flt) {
				LOG_CR(
				    "adm_repair, calloc of track info failed.error: %s",
				    strerror(errno));
				assert(0);
			}
			memset(trk_info_flt, 0, sizeof(PLMS_TRACK_INFO));
			trk_info_flt->imm_adm_opr_id =
...
@param[in]	: aff_ent_list - List of affected entities for this lock
operation.
			   ...
			   PLMS_GROUP_ENTITY *aff_ent_list)
{
	...
	*/
	...
	plms_forced_lock_cbk(ent, trk_info, adm_op, is_new_req /* new aff ent*/,
			     1);
				      ...
				      SaUint32T new_aff_ent, SaUint32T term_ee)
{
	...
	while (head) {
		plms_readiness_state_set(
		    ...
		    SA_NTF_MANAGEMENT_OPERATION, SA_PLM_NTFID_STATE_CHANGE_DEP);
		plms_readiness_flag_mark_unmark(
		    head->plm_entity, SA_PLM_RF_DEPENDENCY, true, ent,
		    SA_NTF_MANAGEMENT_OPERATION, SA_PLM_NTFID_STATE_CHANGE_DEP);
	...
	if ((PLMS_EE_ENTITY == ent->entity_type) &&
		...
		ret_err = saImmOiAdminOperationResult(
		    cb->oi_hdl, trk_info->inv_id, SA_AIS_ERR_DEPLOYMENT);
		...
		/* Return OK to IMM. */
		ret_err = saImmOiAdminOperationResult(
		    cb->oi_hdl, trk_info->inv_id, SA_AIS_OK);
...
static SaUint32T plms_ent_unlock_to_shutdown(PLMS_EVT *evt)
{
	...
	if (NULL == ent) {
		...
		found in entity_info tree for the dn_name %s",
		       tmp);
	...
	plms_readiness_state_set(ent, SA_PLM_READINESS_STOPPING, NULL,
				 SA_NTF_MANAGEMENT_OPERATION,
				 SA_PLM_NTFID_STATE_CHANGE_ROOT);
	...
	while (head) {
		...
		if (!plms_is_rdness_state_set(
			...
			plms_readiness_state_set(head->plm_entity,
						 SA_PLM_READINESS_STOPPING, ent,
						 SA_NTF_MANAGEMENT_OPERATION,
						 SA_PLM_NTFID_STATE_CHANGE_DEP);
		} else { /* This entity is not part of affected entity
			    anymore.*/
	...
	if (NULL != to_be_rmv) {
		...
		head = to_be_rmv;
		while (head) {
			plms_ent_from_ent_list_rem(head->plm_entity,
						   &aff_ent_list);
			head = head->next;
		}
...
static SaUint32T plms_ent_shutdown_to_lock(PLMS_EVT *evt)
{
	...
	if (ent->adm_op_in_progress && (NULL != trk_info) &&
		...
		Return TIMEOUT.*/
		ret_err = saImmOiAdminOperationResult(
		    cb->oi_hdl, trk_info->inv_id, SA_AIS_ERR_TIMEOUT);

		if (NCSCC_RC_SUCCESS != ret_err) {
			LOG_ER("Sending admin response for shutdown to IMM\
			failed. IMM Ret code: %d",
			       ret_err);
		} else {
		...
		if (ent->adm_op_in_progress) {
			...
			LOG_ER(
			    "Operation can not be performed. cur_track_cause: %d, req_lock_option: %s",
...
#endif
	ret_err =
	    plms_forced_lock_cbk(ent, trk_info, evt->req_evt.admin_op, 0, 1);
	if (NCSCC_RC_SUCCESS != ret_err) {
	}
...
static SaUint32T plms_ent_shutdown_to_unlock(PLMS_EVT *evt)
{
	...
	if (ent->adm_op_in_progress && (NULL != trk_info) &&
		...
		if (ent->adm_op_in_progress) {
			...
			LOG_ER(
			    "Operation can not be performed. cur_track_change: %d, requested_op: %d",
			    ent->adm_op_in_progress,
			    evt->req_evt.admin_op.operation_id);
...
static SaUint32T plms_ent_locked_to_unlock(PLMS_EVT *evt)
{
	...
	if (PLMS_HE_ENTITY == ent->entity_type) {
		...
		if (plms_rdness_flag_is_set(ent, SA_PLM_RF_DEPENDENCY) ||
			...
			go insvc. Ent: %s",
			      ent->dn_name_str);
			plms_admin_state_set(ent, SA_PLM_EE_ADMIN_UNLOCKED,
					     NULL, SA_NTF_MANAGEMENT_OPERATION,
					     SA_PLM_NTFID_STATE_CHANGE_ROOT);
...
static SaUint32T plms_ent_unlock(PLMS_ENTITY *ent, PLMS_TRACK_INFO *trk_info,
				 PLMS_IMM_ADMIN_OP adm_op, SaUint32T inst_ee,
				 SaUint32T unlck_ee)
{
...
SaUint32T plms_cbk_call(PLMS_TRACK_INFO *trk_info, SaUint8T add_root)
{
	...
	while (grp_list) {
		...
		if (!grp_list->ent_grp_inf->track_flags &&
		    (SA_PLM_CAUSE_STATUS_INFO != trk_info->track_cause)) {
		...
		agent_evt.req_evt.agent_track.track_cbk.invocation_id = inv_id;
		...
		agent_evt.req_evt.agent_track.track_cbk.root_correlation_id =
		...
		agent_evt.req_evt.agent_track.track_cbk.tracked_entities
		    .numberOfEntities = cnt;

		agent_evt.req_evt.agent_track.track_cbk.tracked_entities
		    .entities = (SaPlmReadinessTrackedEntityT *)calloc(
		...
		plms_grp_aff_ent_fill(agent_evt.req_evt.agent_track.track_cbk
					  .tracked_entities.entities,
				      grp_list->ent_grp_inf,
				      trk_info->aff_ent_list, trk_info->grp_op,
				      trk_info->change_step);
		...
		ret_err = plms_mds_normal_send(
		    cb->mds_hdl, NCSMDS_SVC_ID_PLMS, &agent_evt,
		    grp_list->ent_grp_inf->agent_mdest_id, NCSMDS_SVC_ID_PLMA);
		if (NCSCC_RC_SUCCESS != ret_err) {
			...
			if (is_validate) {
				inv_to_trk_info =
				    (PLMS_INVOCATION_TO_TRACK_INFO *)calloc(
					1,
					sizeof(PLMS_INVOCATION_TO_TRACK_INFO));
				...
				plms_inv_to_trk_grp_add(
				    &(grp_list->ent_grp_inf->invocation_list),
				    inv_to_trk_info);
			...
			, no of ent: %d",
			      grp_list->ent_grp_inf->entity_grp_hdl,
			      trk_info->change_step, trk_info->track_cause,
			      cnt);
		}
		/* MDS should have copied the data and hence free
		agent_evt.req_evt.agent_track.track_cbk.
		tracked_entities.entities*/
		free(agent_evt.req_evt.agent_track.track_cbk.tracked_entities
			 .entities);
		agent_evt.req_evt.agent_track.track_cbk.tracked_entities
		    .entities = NULL;
	...
	TRACE("Track_count: %d, Track_cause: %d, Track_step: %d",
	      trk_info->track_count, trk_info->track_cause,
	      trk_info->change_step);
...
SaUint32T plms_cbk_response_process(PLMS_EVT *evt)
{
	...
	*/
	...
	case SA_PLM_CALLBACK_RESPONSE_ERROR:
		...
		plms_inv_to_cbk_in_grp_inv_rmv(
		    grp, res->track_cbk_res.invocation_id);
	...
	case SA_PLM_CALLBACK_RESPONSE_REJECTED:
		...
		if (trk_info->root_entity->adm_op_in_progress ==
			...
			plms_aff_ent_flag_mark_unmark(trk_info->aff_ent_list,
						      false);
		}
		...
		if (SA_PLM_CAUSE_HE_DEACTIVATION ==
			...
			ret_err = plms_he_activate(trk_info->root_entity, false,
						   true);
			if (NCSCC_RC_SUCCESS != ret_err) {
...
/******************************************************************************
@brief		: Process the response from the client for the validate
callback.
...
@param[in]	: trk_info - Carries the required info to call completed
callback.
@return		: NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
...
SaUint32T plms_imm_adm_op_req_process(PLMS_EVT *evt)
{
	...
	if (NULL == ent) {
		...
		LOG_ER("plms_imm_adm_op_req_process:\
			Ent not found for in patricia tree. dn_name: %s",
		       tmp);
		TRACE_LEAVE2("ret_err: %d", ret_err);
		return ret_err;
	}
...
******************************************************************************/
static void plms_lock_start_cbk_call(PLMS_ENTITY *ent,
				     PLMS_TRACK_INFO *trk_info)
{
	trk_info->change_step = SA_PLM_CHANGE_START;
...
@param[in]	: trk_info - Carries required info to call the callback.
******************************************************************************/
static void plms_lock_completed_cbk_call(PLMS_ENTITY *ent,
					 PLMS_TRACK_INFO *trk_info)
{
...
/******************************************************************************
@brief		: Call the completed callback for shutdown operation after
getting response for the start callback from all the clients.
@param[in]	: ent - Entity on which lock operation is performed.
@param[in]	: trk_info - Carries required info to call the callback.
******************************************************************************/
					     ...
					     PLMS_TRACK_INFO *trk_info)
{
	...
	while (head) {
		...
		ret_err = plms_ee_term(head->plm_entity, false, 0);
		if (NCSCC_RC_SUCCESS != ret_err) {
			LOG_ER("EE Term FAILED. Ent: %s", ent->dn_name_str);
		}
...
void plms_deact_completed_cbk_call(PLMS_ENTITY *ent, PLMS_TRACK_INFO *trk_info)
{
	...
	while (head) {
		if ((PLMS_EE_ENTITY == head->plm_entity->entity_type) &&
		    (SA_PLM_EE_PRESENCE_UNINSTANTIATED !=
		     head->plm_entity->entity.ee_entity.saPlmEEPresenceState)) {
	...
	if (PLMS_HE_ENTITY == ent->parent->entity_type) {
		...
		*/
		...
		if (NCSCC_RC_SUCCESS != ret_err) {
			...
			/* Set admin pending and management lost flag fot the
			 * entity.*/
			if (!plms_rdness_flag_is_set(
				ent, SA_PLM_RF_MANAGEMENT_LOST)) {

				/* Set management lost flag.*/
				plms_readiness_flag_mark_unmark(
				    ent, SA_PLM_RF_MANAGEMENT_LOST, 1 /*mark*/,
				    NULL, SA_NTF_MANAGEMENT_OPERATION,
				    SA_PLM_NTFID_STATE_CHANGE_ROOT);

				plms_readiness_flag_mark_unmark(
				    ent, SA_PLM_RF_ADMIN_OPERATION_PENDING,
				    1 /*mark*/, NULL,
				    SA_NTF_MANAGEMENT_OPERATION,
				    SA_PLM_NTFID_STATE_CHANGE_ROOT);
...
******************************************************************************/
static SaUint32T
plms_ee_graceful_restart_process(PLMS_ENTITY *ent, PLMS_EVT *evt,
				 PLMS_GROUP_ENTITY *aff_ent_list)
{
