...
static SaUint32T plms_he_verify(PLMS_HPI_EVT *, SaPlmHETypeT **,
				PLMS_ENTITY **);
...
@return		: None.
******************************************************************************/
void plms_he_pres_fsm_init(
    PLMS_PRES_FUNC_PTR plms_HE_pres_state_op[][SA_PLM_HPI_HE_PRES_STATE_MAX])
{
	TRACE_ENTER2();

	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_NOT_PRESENT]
			     [SAHPI_HS_STATE_INSERTION_PENDING] =
				 plms_HE_inact_np_to_inspending;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_NOT_PRESENT]
			     [SAHPI_HS_STATE_ACTIVE] = plms_HE_inact_np_to_act;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_NOT_PRESENT]
			     [SAHPI_HS_STATE_EXTRACTION_PENDING] =
				 plms_HE_pres_state_invalid;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_NOT_PRESENT]
	...
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_INACTIVE]
			     [SAHPI_HS_STATE_EXTRACTION_PENDING] =
				 plms_HE_inact_to_extpending_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_INACTIVE]
			     [SAHPI_HS_STATE_NOT_PRESENT] =
				 plms_HE_inact_to_np_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_INACTIVE]
			     [SAHPI_HS_STATE_INACTIVE] =
				 plms_HE_inact_to_inact_op;

	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_ACTIVATING]
			     [SAHPI_HS_STATE_ACTIVE] =
				 plms_HE_actving_to_act_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_ACTIVATING]
			     [SAHPI_HS_STATE_EXTRACTION_PENDING] =
				 plms_HE_actving_to_extpending_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_ACTIVATING]
			     [SAHPI_HS_STATE_INACTIVE] =
				 plms_HE_actving_to_inact_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_ACTIVATING]
			     [SAHPI_HS_STATE_NOT_PRESENT] =
				 plms_HE_actving_to_np_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_ACTIVATING]
			     [SAHPI_HS_STATE_INSERTION_PENDING] =
				 plms_HE_pres_state_invalid;
	...
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_ACTIVE]
			     [SAHPI_HS_STATE_EXTRACTION_PENDING] =
				 plms_HE_act_to_extpending_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_ACTIVE]
			     [SAHPI_HS_STATE_INACTIVE] =
				 plms_HE_act_to_inact_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_ACTIVE]
			     [SAHPI_HS_STATE_NOT_PRESENT] =
				 plms_HE_act_to_np_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_ACTIVE]
			     [SAHPI_HS_STATE_INSERTION_PENDING] =
				 plms_HE_act_to_inspending_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_ACTIVE]
			     [SAHPI_HS_STATE_ACTIVE] = plms_HE_act_to_act_op;

	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_DEACTIVATING]
			     [SAHPI_HS_STATE_INACTIVE] =
				 plms_HE_deacting_to_inact_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_DEACTIVATING]
			     [SAHPI_HS_STATE_INSERTION_PENDING] =
				 plms_HE_deacting_to_inspending_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_DEACTIVATING]
			     [SAHPI_HS_STATE_ACTIVE] =
				 plms_HE_deacting_to_act_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_DEACTIVATING]
			     [SAHPI_HS_STATE_NOT_PRESENT] =
				 plms_HE_deacting_to_np_op;
	plms_HE_pres_state_op[SA_PLM_HE_PRESENCE_DEACTIVATING]
			     [SAHPI_HS_STATE_EXTRACTION_PENDING] =
				 plms_HE_pres_state_invalid;
...
/******************************************************************************
@brief		: M0/M1->M2/M3 valid transition for 5 state and 5 partial
state model. 1. Deactivate the board if the admin state is lock-inactive. 2.
Nothing to do for partial 5 state model. 3. Activate the board for full 5 state
model, as HS/HRB would have cancelled the activation timer.
...
@brief		: Valid for
		...
		2. M0/M1->M2/M3->M4 for 5 state and 5 partial state when M2/M3
is missed.

		Deactivate the board if the admin state is lock-inactive.
Otherwise mark the presence state to active and take the board to insvc if other
...
@brief		: M1->M2/M3->M4->M5/M6 valid for 5 state and 5 partial state.
		...
		activated state. But for 5 state model, HSM would have canceled
the extraction timer,just deactivate the HE.
...
/******************************************************************************
@brief		: M1->M1 transition. This is the situation when HPI
rediscovers the resources. PLMS uses this to clear the management lost flag if
set for that HE.
...
@brief		: M4->M5. Valid for both 5 full and partial state model.
		...
		If the entity is moving to M5 because of its parent, then just
change the presence state. As we do not have any control over 5-partial state
model, dont do anything, only change the presence state. For 5-full state model
		make sure the callbacks are called depending upon deactivation
policy.
...
missed. 1. If I am moving to inactive because of my parent, then nothing to do.
		Only change the presence and readiness state.
		2. If the entity is already OOS, then change only the presence
state. 3. If the entity is insvc, then make sure the entity is moved to OOS and
...
@brief		: M4->M0 : For 2 state
		...
		M4->M5/M6->M1->M0 : For 5 state and 5 partial state and M5/M6/M1
are missed. 1. Take the HE to OOS if not in. 2. Clear the readiness flag. 3.
Wipe out current entity path and current HEType.
...
/******************************************************************************
@brief		: M4->M5/M6->M1->M2/M3. Valid only for 5 state and 5 state
partial model and M5/M6/M1 are missed.

		In M4, this HE might be in InSvc. If it is and PLMS misses
		M5/M6/M1 then, PLMS has to move the affected entities to OOS
before activating this HE.
...
/******************************************************************************
@brief		: M4->M4. PLMS gets this for those HE which are in M4
state on hpi rediscovery. PLMS uses this information to clear the management
lost flag, if set for the same HE.
		...
		while (head) {
			...
			if (!plms_rdness_flag_is_set(head->plm_entity,
				...
				plms_readiness_flag_mark_unmark(
				    ...
				    true, ent, SA_NTF_OBJECT_OPERATION,
				    SA_PLM_NTFID_STATE_CHANGE_DEP);
...
static SaUint32T plms_HE_deacting_to_inspending_op(PLMS_EVT *evt)
{
	...
	*/
	if (plms_is_anc_in_deact_cnxt(ent)) {
		TRACE(
		    "The HE %s is moving to activating because of his Parent.",
		    ent->dn_name_str);
		plms_readiness_state_set(ent, SA_PLM_READINESS_OUT_OF_SERVICE,
					 NULL, SA_NTF_OBJECT_OPERATION,
					 SA_PLM_NTFID_STATE_CHANGE_ROOT);
	...
	if (PLMS_HPI_PARTIAL_FIVE_HOTSWAP_MODEL == ent->state_model) {
		...
		/* trk_info is null, prev he_deactivate failed and clean up was
		 * already done? In anycase since the trk_info is freed there is
		 * no list to track against
		 */
		if (NULL == ent->trk_info) {
			TRACE_LEAVE2("Return Val: %d", NCSCC_RC_SUCCESS);
			return NCSCC_RC_SUCCESS;
		...
		case SA_PLM_CHANGE_START:
			...
			Ent: %s, Change step: %d, adm_op: %d",
			      ent->dn_name_str, ent->trk_info->change_step,
		...
		Ent: %s, State model: %d",
		       ent->dn_name_str, ent->state_model);
		return NCSCC_RC_FAILURE;
	}
	...
	*/
	...
	if (PLMS_HPI_PARTIAL_FIVE_HOTSWAP_MODEL == ent->state_model) {
		...
		case SA_PLM_CHANGE_START:
			TRACE("Clean up the previuos admin context.\
			Ent: %s, adm_op_id: %llu",
			      ent->dn_name_str, ent->trk_info->imm_adm_opr_id);
			/* Clean up the inv_to_cbk nodes belongs to
			this trk_info.*/
			head = ent->trk_info->group_info_list;
			while (head) {
...
static SaUint32T plms_he_active_process(PLMS_ENTITY *ent)
{
	...
	if (NCSCC_RC_SUCCESS == plms_move_ent_to_insvc(ent, &is_flag_aff)) {
		...
		trk_info.grp_op = SA_PLM_GROUP_MEMBER_READINESS_CHANGE;
		trk_info.root_correlation_id = SA_NTF_IDENTIFIER_UNUSED;
		if ((NULL != ent->trk_info) && (SA_PLM_CAUSE_FAILURE_CLEARED ==
						ent->trk_info->track_cause)) {
...
SaUint32T plms_he_oos_to_np_process(PLMS_ENTITY *ent)
{
	...
	plms_presence_state_set(ent, SA_PLM_HE_PRESENCE_NOT_PRESENT, NULL,
				SA_NTF_OBJECT_OPERATION,
				SA_PLM_NTFID_STATE_CHANGE_ROOT);
		...
		case PLMS_HE_ENTITY:
			if (SA_PLM_HE_PRESENCE_NOT_PRESENT !=
				...
				plms_presence_state_set(
				    head->plm_entity,
				    SA_PLM_HE_PRESENCE_NOT_PRESENT, NULL,
				    SA_NTF_OBJECT_OPERATION,
				    SA_PLM_NTFID_STATE_CHANGE_DEP);
...
SaUint32T plms_he_insvc_to_np_process(PLMS_ENTITY *ent)
{
	...
	while (head) {
		...
		if (PLMS_EE_ENTITY == head->plm_entity->entity_type) {
			...
			if (!plms_is_chld(ent, head->plm_entity)) {
				...
				plms_presence_state_set(
				    head->plm_entity,
				    SA_PLM_HE_PRESENCE_NOT_PRESENT, ent,
				    SA_NTF_OBJECT_OPERATION,
				    SA_PLM_NTFID_STATE_CHANGE_DEP);
	...
	plms_ent_list_grp_list_add(act_ent_list, &(trk_info.group_info_list));
...
static SaUint32T plms_adm_cnxt_clean_up(PLMS_ENTITY *ent)
{
	...
	if (SA_PLM_CAUSE_LOCK == ent->adm_op_in_progress) {
		...
		case SA_PLM_CHANGE_VALIDATE:
			...
			plms_ent_exp_rdness_state_ow(ent);
			plms_aff_ent_exp_rdness_state_ow(
			    ent->trk_info->aff_ent_list);
			ent->trk_info->track_count = 0;
			plms_cbk_call(ent->trk_info, 1);
		...
		case SA_PLM_CHANGE_START:
			...
			while (aff_head) {
				plms_readiness_state_set(
				    ...
				    SA_PLM_NTFID_STATE_CHANGE_DEP);
				plms_readiness_flag_mark_unmark(
				    aff_head->plm_entity, SA_PLM_RF_DEPENDENCY,
				    true, ent, SA_NTF_MANAGEMENT_OPERATION,
				    SA_PLM_NTFID_STATE_CHANGE_DEP);
...
following things 1. If the HE is verified i.e. the transition is not from M0,
then call the required FSM function. 2. If the HE is yet to be verified and the
hot swap event is M2 or M4, then verify the HE and call the FSM. If verification
fails, ignore the HE.
...
static SaUint32T plms_hpi_hs_evt_process(PLMS_EVT *evt)
{
	...
	TRACE_ENTER2(
	    "Entity: %s, evt_state: %d", hpi_evt->entity_path,
	...
	epath_to_ent = (PLMS_EPATH_TO_ENTITY_MAP_INFO *)ncs_patricia_tree_get(
	    &(cb->epath_to_entity_map_info), (SaUint8T *)&(hpi_evt->epath_key));
	if (NULL == epath_to_ent) {
		...
		if ((SAHPI_HS_STATE_INSERTION_PENDING ==
		    ...
		    (SAHPI_HS_STATE_INACTIVE ==
		     hpi_evt->sa_hpi_evt.EventDataUnion.HotSwapEvent
			 .HotSwapState) ||
		    (SAHPI_HS_STATE_ACTIVE == hpi_evt->sa_hpi_evt.EventDataUnion
						  .HotSwapEvent.HotSwapState)) {
			...
			if (NCSCC_RC_SUCCESS != ret_err) {
				...
				ret_err = plms_alarm_ntf_send(
				    cb->ntf_hdl,
				    /*TODO nearest parent*/
				    &domain_name, SA_NTF_ALARM_EQUIPMENT,
				    hpi_evt->entity_path,
			...
			memcpy(&(cur_he_type_dn
				     .value[strlen(curHEType->safVersion) + 1]),
			...
			/* Update IMM.*/
			plms_attr_saname_imm_update(
			    ent, "saPlmHECurrHEType",
			...
			epath_to_ent = (PLMS_EPATH_TO_ENTITY_MAP_INFO *)calloc(
			    1, sizeof(PLMS_EPATH_TO_ENTITY_MAP_INFO));

			epath_to_ent->entity_path = (SaInt8T *)calloc(
			    1, strlen(hpi_evt->entity_path) + 1);
			...
			epath_to_ent->pat_node.key_info =
			    (SaUint8T *)&(epath_to_ent->epath_key);
			...
			ret_err =
			    ncs_patricia_tree_add(&cb->epath_to_entity_map_info,
			...
			if (NCSCC_RC_SUCCESS != ret_err) {
				...
				ent->entity.he_entity.saPlmHECurrHEType.length =
				    0;
				memset(ent->entity.he_entity.saPlmHECurrHEType
					   .value,
				       0, SA_MAX_NAME_LENGTH);
			...
			Current st: %d, Received Hpi st: %d",
			      ent->dn_name_str,
			      ent->entity.he_entity.saPlmHEPresenceState,
			      hpi_evt->sa_hpi_evt.EventDataUnion.HotSwapEvent
				  .HotSwapState);
			...
			verify the HE.Ignore and return",
			       hpi_evt->entity_path);
		}
		...
		ret_err =
		    plms_HE_pres_state_op[ent->entity.he_entity
					      .saPlmHEPresenceState]
					 [hpi_evt->sa_hpi_evt.EventDataUnion
					      .HotSwapEvent.HotSwapState](evt);
	}
...
static SaUint32T plms_hpi_resource_evt_process(PLMS_EVT *evt)
{
	...
	TRACE_ENTER2(
	    "Entity: %s, resource type: %d", hpi_evt->entity_path,
	...
	do {
		...
		if (NULL == epath_to_ent) {
			LOG_ER(
			    "Received RESOURCE event %i for unknown resource %s",
			    hpi_evt->sa_hpi_evt.EventDataUnion.ResourceEvent
				.ResourceEventType,
			    hpi_evt->entity_path);
		...
		if (hpi_evt->sa_hpi_evt.EventDataUnion.ResourceEvent
			.ResourceEventType == SAHPI_RESE_RESOURCE_FAILURE) {
			plms_readiness_flag_mark_unmark(
			    ent, SA_PLM_RF_MANAGEMENT_LOST, SA_TRUE, NULL,
			    SA_NTF_MANAGEMENT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);
		} else if (hpi_evt->sa_hpi_evt.EventDataUnion.ResourceEvent
			       .ResourceEventType ==
...
@param[out]	: o_he_type - If the verification is successful, this points to
		the matched HEType which will be used to populate current
HEType.
...
@return		: NCSCC_RC_FAILURE/NCSCC_RC_SUCCESS
******************************************************************************/
static SaUint32T plms_he_verify(PLMS_HPI_EVT *hpi_evt, SaPlmHETypeT **o_he_type,
				PLMS_ENTITY **o_ent)
{
	SaUint32T ret_err = NCSCC_RC_FAILURE;
	SaUint32T max;
	...
	while (ent) {
		...
		if (PLMS_HE_ENTITY != ent->entity_type) {
			ent = (PLMS_ENTITY *)ncs_patricia_tree_getnext(
			    &cb->entity_info, (SaUint8T *)&(ent->dn_name));
			continue;
		}
		...
		while (he_type_list) {
			ret_err = plms_inv_data_compare(he_type_list->inv_data,
							hpi_evt->inv_data);
			if (NCSCC_RC_SUCCESS != ret_err) {
	...
	if (!(epath_matched & he_type_idr_matched)) {
		...
		HE. ent_path: %s",
		       epath_matched, he_type_idr_matched,
		       hpi_evt->entity_path);
	}
...
@return		: NCSCC_RC_FAILURE/NCSCC_RC_SUCCESS
******************************************************************************/
static SaUint32T plms_inv_data_compare(PLMS_INV_DATA conf_inv,
				       PLMS_INV_DATA hpi_inv)
{
	...
	if ((conf_inv.board_area.version)) {
		if (conf_inv.board_area.version == hpi_inv.board_area.version) {
		} else {
	...
	if ((conf_inv.board_area.manufacturer_name.DataLength)) {
		...
		if (0 ==
		    memcmp(conf_inv.board_area.manufacturer_name.Data,
	...
	if ((conf_inv.product_area.manufacturer_name.DataLength)) {
		if (conf_inv.product_area.manufacturer_name.DataLength !=
			...
			LOG_ER(
			    "Product_info->manf name did not match.\
			Conf: %d, Published: %d",
		...
		if (0 ==
		    memcmp(
			conf_inv.product_area.manufacturer_name.Data,
	...
	if ((conf_inv.product_area.product_name.DataLength)) {
		...
		if (0 ==
		    memcmp(conf_inv.product_area.product_name.Data,
	...
	if ((conf_inv.product_area.product_version.DataLength)) {
		...
		if (0 ==
		    memcmp(conf_inv.product_area.product_version.Data,
	...
	if ((conf_inv.product_area.fru_field_id.DataLength)) {
		...
		if (0 ==
		    memcmp(conf_inv.product_area.fru_field_id.Data,
...
@brief		: Deactivate the HE,taking the state model into consideration.
		...
		Notes: This also takes care of setting management lost flag,
admin operation pending flag and sending callback for management lost.

@param[in]	: ent - PLMS_ENTITY representation of HE.
@param[in]	: is_adm_op - If adm operation or not. If set to 1 and HE
deactivation fails, then admin pending flag is set.
@param[in]	: mngt_cbk - If set to 1 and HE deactivation fails, then call
the management lost flag.

@return		: NCSCC_RC_FAILURE/NCSCC_RC_SUCCESS
******************************************************************************/
SaUint32T plms_he_deactivate(PLMS_ENTITY *ent, SaUint32T is_adm_op,
			     SaUint32T mngt_cbk)
{
	SaUint32T cbk = 0;
	SaUint32T ret_err = NCSCC_RC_FAILURE;
	...
	case PLMS_HPI_TWO_HOTSWAP_MODEL:
		/* TODO: */
		ret_err = plms_hrb_req(ent, PLMS_HPI_CMD_ACTION_REQUEST,
				       SAHPI_HS_ACTION_EXTRACTION /*arg*/);
		break;
	case PLMS_HPI_THREE_HOTSWAP_MODEL:
		ret_err = plms_hrb_req(ent, PLMS_HPI_CMD_RESOURCE_POWER_OFF,
				       SAHPI_POWER_OFF /*arg*/);
		break;
	...
	case PLMS_HPI_FULL_FIVE_HOTSWAP_MODEL:
		if (SA_PLM_HE_PRESENCE_ACTIVE ==
		    ent->entity.he_entity.saPlmHEPresenceState) {
			if (!ent->deact_in_pro) {
				ret_err = plms_hrb_req(
				    ent, PLMS_HPI_CMD_ACTION_REQUEST,
				    SAHPI_HS_ACTION_EXTRACTION /*arg*/);
				if (NCSCC_RC_SUCCESS == ret_err)
			...
			if (!ent->deact_in_pro) {
				ret_err = plms_hrb_req(
				    ent, PLMS_HPI_CMD_RESOURCE_INACTIVE_SET, 0);
				if (NCSCC_RC_SUCCESS == ret_err)
					ent->deact_in_pro = true;
			} else {
				ret_err = NCSCC_RC_SUCCESS;
	...
	if (NCSCC_RC_SUCCESS != ret_err) {
		if (is_adm_op) {
			if (!plms_rdness_flag_is_set(
				...
				plms_readiness_flag_mark_unmark(
				    ent, SA_PLM_RF_ADMIN_OPERATION_PENDING, 1,
		...
		if (!plms_rdness_flag_is_set(ent, SA_PLM_RF_MANAGEMENT_LOST)) {
			...
			plms_readiness_flag_mark_unmark(
			    ent, SA_PLM_RF_MANAGEMENT_LOST, 1 /*mark*/, NULL,
			    SA_NTF_MANAGEMENT_OPERATION,
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);
			cbk = 1;
		}
...
@param[in]	: ent - PLMS_ENTITY representation of HE.
@param[in]	: is_adm_op - If adm operation or not. If set to 1 and HE
activation fails, then admin pending flag is set.
@param[in]	: mngt_cbk - If set to 1 and HE activation fails, then call the
		management lost flag.

@return		: NCSCC_RC_FAILURE/NCSCC_RC_SUCCESS
******************************************************************************/
SaUint32T plms_he_activate(PLMS_ENTITY *ent, SaUint32T is_adm_op,
			   SaUint32T mngt_cbk)
{
	SaUint32T cbk = 0;
	SaUint32T ret_err = NCSCC_RC_FAILURE;
	...
	case PLMS_HPI_TWO_HOTSWAP_MODEL:
		...
		break;
	case PLMS_HPI_THREE_HOTSWAP_MODEL:
		ret_err = plms_hrb_req(ent, PLMS_HPI_CMD_RESOURCE_POWER_ON,
				       SAHPI_POWER_ON /*arg*/);
		break;
	...
	case PLMS_HPI_FULL_FIVE_HOTSWAP_MODEL:
		if (SA_PLM_HE_PRESENCE_INACTIVE ==
			...
			if (!ent->act_in_pro) {
				ret_err = plms_hrb_req(
				    ent, PLMS_HPI_CMD_RESOURCE_ACTIVE_SET, 0);
				if (NCSCC_RC_SUCCESS == ret_err) {
					ent->act_in_pro = true;
				}
			} else {
...
/******************************************************************************
@brief		: Reset the HE. This also takes care of setting management
lost flag, adm_op_pending flag and calling management lost callback if required.
			...
			SaUint32T mngt_cbk, SaUint32T reset_type)
{
	...
	ret_err =
	    plms_hrb_req(ent, PLMS_HPI_CMD_RESOURCE_RESET, reset_type /*arg*/);
	...
	if (NCSCC_RC_SUCCESS != ret_err) {
		...
		if (!plms_rdness_flag_is_set(ent, SA_PLM_RF_MANAGEMENT_LOST)) {
			...
			plms_readiness_flag_mark_unmark(
			    ent, SA_PLM_RF_MANAGEMENT_LOST, 1 /*mark*/, NULL,
			    (is_adm_op ? SA_NTF_MANAGEMENT_OPERATION
				       : SA_NTF_OBJECT_OPERATION),
			    SA_PLM_NTFID_STATE_CHANGE_ROOT);
			cbk = 1;
		}
...
/******************************************************************************
@brief		: MDS sync request to HRB to execute
cmd(activate/deactivate/reset).
...
static SaUint32T plms_hrb_req(PLMS_ENTITY *ent, PLMS_HPI_CMD cmd, SaUint32T arg)
{
	...
	SaUint32T (*hrb_mds_send_func_ptr)(
	    MDS_HDL mds_hdl, SaUint32T from_svc, SaUint32T to_svc,
	    MDS_DEST to_dest, PLMS_HPI_REQ * i_evt, PLMS_HPI_RSP * *o_evt,
	...
	req.entity_path_len =
	    strlen(ent->entity.he_entity.saPlmHECurrEntityPath);
	req.cmd = cmd;
	req.arg = arg;

	/* Get the hrb Init func ptr */
	hrb_mds_send_func_ptr =
	    dlsym(cb->hpi_intf_hdl, "plms_hrb_mds_msg_sync_send");
	if (NULL == hrb_mds_send_func_ptr) {
		LOG_ER("dlsym()of plms_hrb_mds_msg_sync_send failed, error %s",
		       dlerror());
		TRACE_LEAVE2("Return Val: %d", NCSCC_RC_FAILURE);
		return NCSCC_RC_FAILURE;
	}
	ret_err = (*hrb_mds_send_func_ptr)(cb->mds_hdl, NCSMDS_SVC_ID_PLMS,
					   NCSMDS_SVC_ID_PLMS_HRB, cb->hrb_dest,
					   &req, &resp, PLMS_HRB_MDS_TIMEOUT);
	if (NCSCC_RC_SUCCESS != ret_err) {
		...
		if (NCSCC_RC_SUCCESS != resp->ret_val) {
			LOG_ER("plms_hrb_req FAILED. Ent: %s, cmd: %d\
			resp: %d",
			       ent->dn_name_str, cmd, resp->ret_val);
			ret_err = NCSCC_RC_FAILURE;
		}
	}
...
static SaUint32T plms_deact_resp_mngt_flag_clear(PLMS_ENTITY *ent)
{
	...
	if (plms_rdness_flag_is_set(ent, SA_PLM_RF_ISOLATE_PENDING)) {
		...
		plms_readiness_flag_mark_unmark(
		    ent, SA_PLM_RF_MANAGEMENT_LOST, 0 /*unmark*/, NULL,
		...
		plms_readiness_flag_mark_unmark(
		    ent, SA_PLM_RF_ADMIN_OPERATION_PENDING, 0 /*unmark*/, NULL,
		    SA_NTF_OBJECT_OPERATION, SA_PLM_NTFID_STATE_CHANGE_ROOT);
