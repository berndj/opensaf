...
SaUint32T plms_mds_cpy(struct ncsmds_callback_info *clbk_info)
{
	...
	dst_msg->entity_path =
	    (SaStringT)calloc(1, strlen(src_msg->entity_path) + 1);
	if (NULL == dst_msg->entity_path) {
		LOG_ER("Memory allocation failed ret:%s", strerror(errno));
		assert(0);
	}

	memcpy(dst_msg->entity_path, src_msg->entity_path,
	       src_msg->entity_path_len + 1);
...
static SaUint32T plms_mds_rcv(MDS_CALLBACK_RECEIVE_INFO *rcv_info)
{
	...
	rc =
	    m_NCS_IPC_SEND(&cb->mbx, (NCSCONTEXT)pEvt, NCS_IPC_PRIORITY_NORMAL);
