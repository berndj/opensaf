...
uint32_t smfa_client_info_rmv(SaSmfHandleT hdl)
{
	...
	while (NULL != client_info_list) {
		if (hdl == client_info_list->client_hdl) {
			if (client_info_list->client_hdl ==
			    client_info_prev->client_hdl) {
				/* Deleting the head node.*/
				cb->smfa_client_info_list =
				    client_info_list->next_client;
			} else {
				client_info_prev->next_client =
				    client_info_list->next_client;
			}
...
*****************************************************************************/
SMFA_SCOPE_INFO *smfa_scope_info_get(SMFA_CLIENT_INFO *client_info,
...
*****************************************************************************/
void smfa_scope_info_add(SMFA_CLIENT_INFO *client_info,
...
@brief		: Free the scope info node. This does not delete the scope info
		  ...
		  memory for the filters inside the scope info. After calling
this function, free the scope info node itself.
...
void smfa_scope_info_free(SMFA_SCOPE_INFO *scope_info)
{
	...
	for (no_of_filters = 0;
	     no_of_filters < scope_info->scope_of_interest.filtersNumber;
	     no_of_filters++) {
		if (scope_info->scope_of_interest.filters[no_of_filters]
			.filter.label) {
			free(
			    scope_info->scope_of_interest.filters[no_of_filters]
				.filter.label);
			scope_info->scope_of_interest.filters[no_of_filters]
			    .filter.label = NULL;
		}
...
*****************************************************************************/
uint32_t smfa_scope_info_rmv(SMFA_CLIENT_INFO *client_info,
			     SaSmfCallbackScopeIdT scope_id)
{
	SMFA_SCOPE_INFO *scope_info_list = client_info->scope_info_list;
	...
	while (NULL != scope_info_list) {
		if (scope_id == scope_info_list->scope_id) {
			if (scope_info_prev->scope_id ==
			    scope_info_list->scope_id) {
				/* Deleting the Head node.*/
				client_info->scope_info_list =
				    scope_info_list->next_scope;
			} else {
				/* Deleting the non-head node.*/
				scope_info_prev->next_scope =
				    scope_info_list->next_scope;
			}
...
SaAisErrorT smfa_dispatch_cbk_one(SMFA_CLIENT_INFO *client_info)
{
	...
	TRACE_ENTER();
	if (NULL != (msg = (SMF_EVT *)m_NCS_IPC_NON_BLK_RECEIVE(
			 &client_info->cbk_mbx, NULL))) {
		...
		if (SMF_CLBK_EVT != msg->evt_type) {
			LOG_ER(
			    "SMFA: Wrong evt is posted to client MBX. evt_type: %d",
			    msg->evt_type);
			free(msg);
			TRACE_LEAVE();
			return SA_AIS_OK;
		...
		client_info->reg_cbk.saSmfCampaignCallback(
		    client_info->client_hdl, msg->evt.cbk_evt.inv_id,
		    msg->evt.cbk_evt.scope_id, &msg->evt.cbk_evt.object_name,
		    msg->evt.cbk_evt.camp_phase, &msg->evt.cbk_evt.cbk_label,
...
SaAisErrorT smfa_dispatch_cbk_all(SMFA_CLIENT_INFO *client_info)
{
	...
	TRACE_ENTER();
	while (NULL != (msg = (SMF_EVT *)m_NCS_IPC_NON_BLK_RECEIVE(
			    &client_info->cbk_mbx, NULL))) {
		/* You have reached this far means, everything is fine.
		 1. Client is not yet finalized.
		 2. Cbk is also registered.
...
/***************************************************************************
@brief		: Dispatch block. Block while reading the MBX if there is
no msg in the MBX.
@param[in]	: client_info - Client for which dispatch is called.
@return		: SA_AIS_OK.
*****************************************************************************/
SaAisErrorT smfa_dispatch_cbk_block(SMFA_CLIENT_INFO *client_info)
{
	...
	TRACE_ENTER();
	while (NULL != (msg = (SMF_EVT *)m_NCS_IPC_RECEIVE(
			    &client_info->cbk_mbx, NULL))) {
		/* You have reached this far means, everything is fine.
		 1. Client is not yet finalized.
		 2. Cbk is also registered.
...
@brief		: Preocess the OK response from the client. Agent takes care
		  ...
		  sending the response ti SMFND. This function does the
following. 1. Get to the inv node in cb->cbk_list. 2. If does not match, drop
the msg. If matches, get to the hdl node in cbk_list->hdl_list. 3. If does not
match, drop the msg. If matches, decrement the resp count. 4. If the response
count reaches to zero, implies there are no clients for this handle to respond
and hence delete the hdl node from the cbk_list->hdl_list. If the response count
is not zero, implies that there are responses expected from this hdl and hence
		  return from there.
		  5. After deleting the hdl node from cbk_list->hdl_list, check
if the list is empty. 6. If the list is not empty, implies there are other
clients yet to respond. If the list is empty, implies all the clients have
		  responded for this cbk. And hence remove this inv node from
		  cb->cbk_list and return SUCCESS so that the consolidated
response can be sent to SMFND.
@param[in]	: smfHandle - Handle.
@param[in]	: invocation - Inv id sent in response by the application.
@return		: NCSCC_RC_FAILURE/NCSCC_RC_SUCCESS.
*****************************************************************************/
uint32_t smfa_cbk_ok_resp_process(SaSmfHandleT smfHandle,
			...
			while (hdl_list) {
				...
				if (smfHandle == hdl_list->hdl) {
					...
					if (0 == hdl_list->cnt) {
						...
						/* Still resp expected from
						 * other hdls.*/
						free(hdl_list);
					}
					TRACE_2(
					    "SMFA: Waiting for remaining resp.");
...
@brief		: Process the error response. This function does the following.
		  ...
		  the invocation.
		  2. Make sure all the hdl nodes are freed from
cbk_list->hdl_list before deleting inv node.
@param[in]	: hdl - Handle.
@param[in]	: invocation - Inv id sent in response by the application.
@return		: NCSCC_RC_FAILURE/NCSCC_RC_SUCCESS.
*****************************************************************************/
...
@return		: NCSCC_RC_FAILURE/NCSCC_RC_SUCCESS.
***************************************************************************/
uint32_t smfa_cbk_filter_type_match(SaSmfLabelFilterT *stored_flt,
				    SaSmfCallbackLabelT *passed_flt)
{
	uint32_t rc = NCSCC_RC_FAILURE;

	switch (stored_flt->filterType) {
	case SA_SMF_PREFIX_FILTER:
		/* Match only the string of length registered by the application
		   from the begining of the label.*/
		if (0 != stored_flt->filter.labelSize) {
			if (0 ==
			    strncmp((char *)stored_flt->filter.label,
				    (char *)passed_flt->label,
	...
	case SA_SMF_SUFFIX_FILTER:
		...
		if (0 != stored_flt->filter.labelSize) {
			if (0 ==
			    strncmp((char *)stored_flt->filter.label,
				    (char *)&passed_flt
					->label[passed_flt->labelSize -
						stored_flt->filter.labelSize],
				    (unsigned)stored_flt->filter.labelSize)) {
				rc = NCSCC_RC_SUCCESS;
			}
		}
...
@brief		: 1. If the cb->cbk_list does not have inv node corersponding
		  ...
		  2. If inv node exists, then create only hdl node and add at
the head of the cb->cbk_list->hdl_list.

		  This function will be hit only for the first time when the hdl
node is created and hence optimized taking this into consideration.
...
SMFA_CBK_HDL_LIST *smfa_inv_hdl_add(SaInvocationT inv_id, SaSmfHandleT hdl)
{
	...
	if (NULL == cbk_list) {
		/* First filter matching. 1. Create inv node 2. Create hdl
		 * node.*/
		cbk_list = (SMFA_CBK_LIST *)calloc(1, sizeof(SMFA_CBK_LIST));
		if (!cbk_list) {
			LOG_ER("SMFA: calloc FAILED, error: %s",
			       strerror(errno));
			osafassert(0);
		}
		cbk_list->inv_id = inv_id;
		cbk_list->hdl_list =
		    (SMFA_CBK_HDL_LIST *)calloc(1, sizeof(SMFA_CBK_HDL_LIST));
		...
		/* Inv_id matched ==> this might not the first hdl for this
		   inv_id but first filter matching for this hdl. So create hdl
		   node.*/
		hdl_list =
		    (SMFA_CBK_HDL_LIST *)calloc(1, sizeof(SMFA_CBK_HDL_LIST));
		if (!hdl_list) {
...
@return		: NCSCC_RC_FAILURE/NCSCC_RC_SUCCESS.
***************************************************************************/
uint32_t smfa_cbk_filter_match(SMFA_CLIENT_INFO *client_info,
			       SMF_CBK_EVT *cbk_evt)
{
	...
	while (scope_info) {
		for (no_of_filters = 0;
		     no_of_filters <
		     scope_info->scope_of_interest.filtersNumber;
		     no_of_filters++) {
			if (NCSCC_RC_SUCCESS ==
			    smfa_cbk_filter_type_match(
				&scope_info->scope_of_interest
				...
				evt->evt.cbk_evt.cbk_label.label =
				    (SaUint8T *)calloc(
					1, cbk_evt->cbk_label.labelSize *
					       sizeof(SaUint8T));
				...
				if (cbk_evt->params) {
					evt->evt.cbk_evt.params =
					    (SaStringT)calloc(
						1, cbk_evt->params_len + 1);
					if (NULL == evt->evt.cbk_evt.params) {
						LOG_ER(
						    "SMFA: calloc FAILED, error: %s",
						    strerror(errno));
						osafassert(0);
					}
					strncpy(evt->evt.cbk_evt.params,
						cbk_evt->params,
						cbk_evt->params_len);
					/* calloc above has set the null char at
					 * the end of params */
				}

				evt->evt.cbk_evt.cbk_label.labelSize =
				    cbk_evt->cbk_label.labelSize;
				strncpy(
				    (char *)evt->evt.cbk_evt.cbk_label.label,
				    (char *)cbk_evt->cbk_label.label,
				    cbk_evt->cbk_label.labelSize);

				evt->evt_type = SMF_CLBK_EVT;
				evt->evt.cbk_evt.inv_id = cbk_evt->inv_id;
				evt->evt.cbk_evt.scope_id =
				    scope_info->scope_id;
				evt->evt.cbk_evt.camp_phase =
				    cbk_evt->camp_phase;
...
@brief		: 1. Find out if there is any pending response from this client
		  ...
		  is the last client for which this agent is waiting for the
response. 3. If yes, then send OK response to SMFND.
			...
			if (hdl == hdl_list->hdl) {
				...
				if (prev_hdl->hdl == hdl_list->hdl) {
					...
					if (NULL == cbk_list->hdl_list) {
						...
						{
							/* Populate the resp and
							 * send to SMFND.*/
							memset(
							    &resp_evt, 0,
							    sizeof(SMFSV_EVT));
							resp_evt.type =
							    SMFSV_EVT_TYPE_SMFND;
							resp_evt.info.smfnd
							    .type =
							    SMFND_EVT_CBK_RSP;
							resp_evt.info.smfnd
							...
							if (NCSCC_RC_SUCCESS !=
							    smfa_to_smfnd_mds_async_send(
								(NCSCONTEXT)&resp_evt)) {
								LOG_ER(
								    "SMFA: MDS send to SMFND FAILED.");
								/* TODO: What
								 * can we do??
								 */
							}
...
/***************************************************************************
@brief		: Clean up the MBX before destroying it. Free all the msgs
in the MBX.
@param[in]	: arg -
@param[in]	: msg -
@return		: true
