...
uint32_t smfnd_cmd_asynch_resp(NCS_OS_PROC_EXECUTE_TIMED_CB_INFO *info)
{
	...
	case NCS_OS_PROC_EXIT_WITH_CODE: {
		...
		cmd_result = SMFSV_CMD_RESULT_CODE +
			     info->exec_stat.info.exit_with_code.exit_code;
		break;
	}
	case NCS_OS_PROC_EXIT_ON_SIGNAL: {
		...
		cmd_result = SMFSV_CMD_SIGNAL_TERM +
			     info->exec_stat.info.exit_on_signal.signal_num;
		break;
	}
	default: {
	...
	rc = smfsv_mds_send_rsp(smfnd_cb->mds_handle, response_info->mds_ctxt,
				response_info->fr_svc, response_info->fr_dest,
				NCSMDS_SVC_ID_SMFND, smfnd_cb->mds_dest,
				&result_evt);
...
int smfnd_exec_asynch_cmd(char *i_cmd, NCS_OS_PROC_EXECUTE_CB i_cb,
			  void *i_cb_arg, uint32_t i_timeout)
{
...
static void proc_cmd_req(smfnd_cb_t *cb, SMFSV_EVT *evt)
{
	...
	if ((evt->info.smfnd.event.cmd_req.cmd_len == 0) ||
	    (evt->info.smfnd.event.cmd_req.cmd == NULL)) {
		LOG_ER("SMFND received empty command, return error");
		cmd_result =
		    SMFSV_CMD_EXEC_FAILED +
		    128; /* Send error response so smfd don't hang waiting */
	} else {
		TRACE("Executing command: %s",
		      evt->info.smfnd.event.cmd_req.cmd);
		cmd_result = system(evt->info.smfnd.event.cmd_req.cmd);
		TRACE("Command %s responded %d",
		      evt->info.smfnd.event.cmd_req.cmd, cmd_result);
	}
	...
	rc = smfsv_mds_send_rsp(cb->mds_handle, evt->mds_ctxt, evt->fr_svc,
				evt->fr_dest, NCSMDS_SVC_ID_SMFND, cb->mds_dest,
				&result_evt);
	if (rc != NCSCC_RC_SUCCESS) {
...
static void proc_cmd_req_asynch(smfnd_cb_t *cb, SMFSV_EVT *evt)
{
	...
	if ((evt->info.smfnd.event.cmd_req_asynch.cmd_len == 0) ||
		...
		cmd_result =
		    SMFSV_CMD_EXEC_FAILED +
		    3; /* Send error response so smfd don't hang waiting */
	} else {
		...
		/* We need to save some info that we need when sending the
		 * asynch response */
		response_info->mds_ctxt = evt->mds_ctxt;
		response_info->fr_svc = evt->fr_svc;
		response_info->fr_dest = evt->fr_dest;

		int exec_result = smfnd_exec_asynch_cmd(
		    evt->info.smfnd.event.cmd_req_asynch.cmd,
		    smfnd_cmd_asynch_resp, (void *)response_info,
		    evt->info.smfnd.event.cmd_req_asynch.timeout);

		if (exec_result == 0) {
			LOG_NO(
			    "Successful start of command execution: %s, timeout %u",
			    evt->info.smfnd.event.cmd_req_asynch.cmd,
			    evt->info.smfnd.event.cmd_req_asynch.timeout);
		...
		/* Does error result mean that we will never receive an asynch
		   response ? if we free the memory here and we DO get the
		   response we will crash. by not freeing the memory here we
		   will leak instead i.e. less impact
		   TODO Check if we can get a response in this situation */
		/*free(response_info); */
		cmd_result = SMFSV_CMD_EXEC_FAILED + 4;

		LOG_ER("Failed start of command execution: %s",
		       evt->info.smfnd.event.cmd_req_asynch.cmd);
		/* Send error response*/
	}
 ...
 *****************************************************************************/
uint32_t smfvs_mds_msg_bcast(uint32_t mds_hdl, uint32_t from_svc,
			     uint32_t to_svc, NCSCONTEXT evt, uint32_t priority,
			     uint32_t scope)
{
	NCSMDS_INFO info;
	uint32_t rc;
...
uint32_t smfnd_cbk_req_proc(smfnd_cb_t *cb, SMFSV_EVT *evt)
{
	...
	if (!cb->agent_cnt) {
		...
		/*resp_evt.info.smfd.type = SMFA_EV:sT_RSP;*/ /* chnaging to
								 below line */
		cbk_resp_evt.info.smfd.type =
		    SMFD_EVT_CBK_RSP; /* chnaging to below line */
		...
		cbk_resp_evt.info.smfd.event.cbk_rsp.evt.resp_evt.err =
		    SA_AIS_OK;

		rc = smfsv_mds_msg_send(cb->mds_handle, NCSMDS_SVC_ID_SMFD,
	...
	cbk_resp_evt.info.smfa.event.cbk_req_rsp =
	    evt->info.smfnd.event.cbk_req_rsp;
	cbk_resp_evt.info.smfa.event.cbk_req_rsp.evt_type = SMF_CLBK_EVT;

	if (NCSCC_RC_SUCCESS !=
	    smfvs_mds_msg_bcast(cb->mds_handle, NCSMDS_SVC_ID_SMFND,
				NCSMDS_SVC_ID_SMFA, &cbk_resp_evt,
	...
	inv_node = (SMFND_SMFA_ADEST_INVID_MAP *)calloc(
	    1, sizeof(SMFND_SMFA_ADEST_INVID_MAP));
	if (NULL == inv_node) {
		LOG_ER("SMFND: calloc failed. Err: %s", strerror(errno));
		osafassert(1);
		TRACE_LEAVE();
		return NCSCC_RC_FAILURE; // This line is never executed, but
					 // Coverity tool is happy with a return
					 // after NULL check
	}
	inv_node->inv_id =
	    evt->info.smfnd.event.cbk_req_rsp.evt.resp_evt.inv_id;
	...
	while (inv_node) {
		if (inv_id == inv_node->inv_id) {
			...
			/* Send resp to SMFD.*/
			{
				...
				resp_evt.info.smfd.event.cbk_rsp.evt_type =
				    SMF_RSP_EVT;
 ...
 * Return Values : NCSCC_RC_FAILURE/NCSCC_RC_SUCCESS.
 *****************************************************************************/
uint32_t smfnd_cbk_resp_ok_proc(smfnd_cb_t *cb, SaInvocationT inv_id,
				uint32_t resp)
{
	SMFND_SMFA_ADEST_INVID_MAP *inv_node = cb->cbk_list;
	SMFND_SMFA_ADEST_INVID_MAP *prev_inv = inv_node;
		...
		if (inv_id == inv_node->inv_id) {
			...
			if (0 == inv_node->no_of_cbk) {
				...
				free(inv_node);
				{
					memset(&resp_evt, 0, sizeof(SMFSV_EVT));
					resp_evt.type = SMFSV_EVT_TYPE_SMFD;
					resp_evt.info.smfd.type =
					    SMFD_EVT_CBK_RSP;
					resp_evt.info.smfd.event.cbk_rsp
					    .evt_type = SMF_RSP_EVT;
					resp_evt.info.smfd.event.cbk_rsp.evt
					    .resp_evt.inv_id = inv_id;
					resp_evt.info.smfd.event.cbk_rsp.evt
					    .resp_evt.err = resp;
...
uint32_t smfnd_cbk_resp_proc(smfnd_cb_t *cb, SMFSV_EVT *evt)
{
	...
	if (SA_AIS_ERR_FAILED_OPERATION ==
	    evt->info.smfnd.event.cbk_req_rsp.evt.resp_evt.err) {
		rc = smfnd_cbk_resp_err_proc(
		    cb, evt->info.smfnd.event.cbk_req_rsp.evt.resp_evt.inv_id);
	} else {
		/* Any response other than FAILED_OPERATION is considered as
		 * OK.*/
		rc = smfnd_cbk_resp_ok_proc(
		    cb, evt->info.smfnd.event.cbk_req_rsp.evt.resp_evt.inv_id,
		    evt->info.smfnd.event.cbk_req_rsp.evt.resp_evt.err);
	}
	TRACE_LEAVE();
...
static void proc_cbk_req_rsp(smfnd_cb_t *cb, SMFSV_EVT *evt)
{
	...
	default: {
		LOG_ER("SMFND received unknown event %d",
		       evt->info.smfnd.event.cbk_req_rsp.evt_type);
		break;
	}
	}
	TRACE_LEAVE();
