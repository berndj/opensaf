...
//------------------------------------------------------------------------------
static void GetSingleValue(const immom::ImmOmAccessorGet& obj,
...
//------------------------------------------------------------------------------
static void GetMultipleValue(const immom::ImmOmAccessorGet& obj,
                             const std::string& name) {
...
void TestOmAccessorGetOnSingleValueClass() {
  ...
  immom::ImmOmAccessorHandle accessorhandle{omhandle.GetHandle()};
  if (accessorhandle.InitializeHandle() == false) {
    std::cerr << "OM accessor handle: InitializeHandle() failed: "
              << accessorhandle.ais_error_string() << std::endl;
    return;
  }

  const std::string single_dn = "immWrpTestCfg=testCfg";
  immom::ImmOmAccessorGet accessorget{accessorhandle.GetHandle(), single_dn};
  std::vector<std::string> attrs{};
  for (const auto& a : smapdb) attrs.push_back(a.first);

  accessorget.SetAttributeNames(attrs);
  if (!accessorget.FetchAttributeValue()) {
    std::cerr << "Accessor get : GetAccessor() failed: "
              << accessorget.ais_error_string() << std::endl;
    return;
  }

  // Get value of single-value attributes
  std::cout << "Attribute value of DN: " << single_dn << std::endl;
  std::cout << "---------------------------" << std::endl;
  for (const auto& attr : smapdb) GetSingleValue(accessorget, attr.first);

  auto end = CLOCK_END;
  TIME_ELAPSED(start, end);
}

void TestOmAccessorGetOnMultipleValueClass() {
  auto start = CLOCK_START;
  ...
  const std::string multiple_dn = "immWrpTestMultipleCfg=testMultipleCfg";
  immom::ImmOmAccessorGet accessorget{accessorhandle.GetHandle(), multiple_dn};
  std::vector<std::string> attrs{};
  for (const auto& a : mapdb) attrs.push_back(a.first);
  ...
  // Get value of multiple-value attributes
  std::cout << "Attribute value of DN: " << multiple_dn << std::endl;
  std::cout << "---------------------------" << std::endl;
  for (const auto& attr : mapdb) GetMultipleValue(accessorget, attr.first);
