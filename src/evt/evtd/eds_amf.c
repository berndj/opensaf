			      ...
			      SaAmfCSIDescriptorT csiDescriptor)
{
	...
	if (NULL == (eds_cb = (NCSCONTEXT)ncshm_take_hdl(NCS_SERVICE_ID_EDS,
		...
		if (new_haState == SA_AMF_HA_QUIESCED) {
			TRACE_LEAVE2(
			    "New state is Quiesced, returning immediately");
			return;
		}
		...
		if (role_change == true) {
			if ((rc = eds_mds_change_role(eds_cb)) !=
			    NCSCC_RC_SUCCESS) {
				LOG_ER("MDS role change failed");
			} else {
				/* Declare as Implementer if state changed only
				 * from STANDBY to ACTIVE, Need to check for
				 * transition from QUEISCED back to ACTIVE
				 */
		...
		if (NCSCC_RC_SUCCESS !=
		    (error = eds_mbcsv_change_HA_state(eds_cb))) {
		...
		/* Do some OverHead Processing At New Active For EDA Down Events
		 * Cleanup if those reccords are present */
		if (eds_cb->ha_state == SA_AMF_HA_ACTIVE) {
			...
			while (eda_down_rec) {
				...
				/* Remove this EDA entry from our processing
				 * lists */
				TRACE_1("Processing EDA down for:%" PRIx64
					"at new Active",
					eda_down_rec->mds_dest);
				temp_eda_down_rec = eda_down_rec;
				eds_remove_regid_by_mds_dest(
				    eds_cb, eda_down_rec->mds_dest);
...
static uint32_t eds_healthcheck_start(EDS_CB *eds_cb)
{
	...
	error = saAmfHealthcheckStart(eds_cb->amf_hdl, &eds_cb->comp_name,
...
uint32_t eds_amf_init(EDS_CB *eds_cb)
{
	...
	if (SA_AIS_OK != (rc = saAmfSelectionObjectGet(
			      eds_cb->amf_hdl, &eds_cb->amfSelectionObject))) {
...
uint32_t eds_amf_register(EDS_CB *eds_cb)
{
	...
	error = saAmfComponentRegister(eds_cb->amf_hdl, &eds_cb->comp_name,
				       (SaNameT *)NULL);
	if (error != SA_AIS_OK) {
		m_NCS_UNLOCK(&eds_cb->cb_lock, NCS_LOCK_WRITE);
 ...
 * Returns:   None
 *
 * Notes  : Upon invocation of this callback, if a node joined or left the
 *cluster, EDS shall send an update to all the EDAs on that particular node.
 ...
 * member nodes of the cluster are maintained in this list.
 *SA_AIS_ERR_UNAVAILABLE is retured for an saEvtInitialize() originating from a
 *non member node.
 **************************************************************************/
    ...
    SaUint32T numberOfMembers, SaAisErrorT error)
{
	...
	if (error == SA_AIS_OK) {
		/* Get EDS CB Handle. */
		if (NULL == (cb = (NCSCONTEXT)ncshm_take_hdl(NCS_SERVICE_ID_EDS,
							     gl_eds_hdl))) {
			LOG_ER("Failed to retrieve eds handle: %u", gl_eds_hdl);
			TRACE_LEAVE();
			return;
		...
		if (notificationBuffer != NULL) {
			for (counter = 0;
			     counter < notificationBuffer->numberOfItems;
			     counter++) {
				is_member =
				...
				node_id =
					...
					.clusterNode.nodeId;
				/* Send an update
				 * - if there is 'change' in cluster membership
				 * to an existing node - if its a new node node
				 * joining the cluster
				 */
				if ((cluster_change = update_node_db(
					 cb, node_id, is_member))) {
					...
					if (is_member) {
						cluster_change =
						    SA_CLM_NODE_JOINED;
						TRACE(
						    "CLM Node : %u Joined the cluster",
						    node_id);
					} else {
						cluster_change =
						    SA_CLM_NODE_LEFT;
						TRACE(
						    "CLM Node : %u Left the cluster",
						    node_id);
					}

					/* Send to all EDAs on node_id */
					send_clm_status_change(
					    cb, cluster_change, node_id);
				}
			}
		} else
			TRACE_1("Notification buffer is NULL");

		TRACE("After processing, clust list contains: %d nodes",
		      ncs_patricia_tree_size(&cb->eds_cluster_nodes_list));
		TRACE("=======================================");
		NODE_INFO *tmp = NULL;
		tmp = (NODE_INFO *)ncs_patricia_tree_getnext(
		    &cb->eds_cluster_nodes_list, (uint8_t *)0);
		while (tmp != NULL) {
			TRACE("Cluster Node Id : %u\n", tmp->node_id);
			tmp = (NODE_INFO *)ncs_patricia_tree_getnext(
			    &cb->eds_cluster_nodes_list,
			    (uint8_t *)&tmp->node_id);
		}
		TRACE("=======================================");
		ncshm_give_hdl(gl_eds_hdl);
...
SaBoolT update_node_db(EDS_CB *cb, NODE_ID node_id, SaBoolT is_member)
{
	...
	if (is_member) {
		...
		if ((cn = (NODE_INFO *)ncs_patricia_tree_get(
			 &cb->eds_cluster_nodes_list, (uint8_t *)&node_id)) ==
		    NULL) {
			/* This is a new node joining the cluster. Add it to our
			 * DB */
			TRACE("Node = %u not existing in the DB, adding to DB",
			      node_id);
			...
			if (ncs_patricia_tree_add(&cb->eds_cluster_nodes_list,
				...
				TRACE_LEAVE2(
				    "Node:  %u is a member, num nodes = %d",
				    node_id,
		...
		/* Node left the cluster. Remove from our list if this node is
		 * in our node DB*/
		if ((cn = (NODE_INFO *)ncs_patricia_tree_get(
			 &cb->eds_cluster_nodes_list, (uint8_t *)&node_id)) ==
		    NULL) {
			TRACE_LEAVE2(
			    "Node = %d not in the DB, nothing to be done",
			    node_id);
			return SA_FALSE;
		}
		if (ncs_patricia_tree_del(&cb->eds_cluster_nodes_list,
					  &cn->pat_node) != NCSCC_RC_SUCCESS) {
			LOG_ER("Patricia tree delete failed for node_id = %d",
			       node_id);
			TRACE_LEAVE();
			return SA_FALSE;
		} else {
			...
			TRACE_LEAVE2("Node: %d not a member, deleted from DB",
				     node_id);
			return SA_TRUE;
		}
			    ...
			    NODE_ID node_id)
{
	...
	while (reg_rec != NULL) {
		if (node_id ==
		    (m_EDS_GET_NODE_ID_FROM_ADEST(reg_rec->eda_client_dest))) {
			rc = eds_mds_msg_send(cb, &msg,
					      &reg_rec->eda_client_dest, NULL,
					      MDS_SEND_PRIORITY_MEDIUM);
			if (rc != NCSCC_RC_SUCCESS)
				LOG_ER(
				    "Failed to send cluster change update to EDA. dest: %" PRIx64,
				    reg_rec->eda_client_dest);
			else
				TRACE_1("Sent ClusterNodeUpdate = %u for %u",
...
bool is_node_a_member(EDS_CB *cb, NODE_ID node_id)
{
	...
	if ((cn = (NODE_INFO *)ncs_patricia_tree_get(
		 &cb->eds_cluster_nodes_list, (uint8_t *)&node_id)) == NULL)
		return false;
	else
		return true;
	TRACE_LEAVE();
