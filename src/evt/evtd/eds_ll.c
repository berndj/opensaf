 ...
 *
 ***************************************************************************/
static SaAisErrorT create_runtime_object(char *cname, SaTimeT create_time,
					 SaImmOiHandleT immOiHandle)
{
	...
	if (parent_name != NULL) {
		...
		rdnstr = cname;
	void *arr1[] = {&rdnstr};
	const SaImmAttrValuesT_2 attr_safChnl = {.attrName = "safChnl",
						 .attrValueType =
	...
	const SaImmAttrValuesT_2 *attrValues[] = {
	    &attr_safChnl, &attr_saEvtChannelCreationTimeStamp, NULL};

	if ((rc = immutil_saImmOiRtObjectCreate_2(immOiHandle, "SaEvtChannel",
						  &parentName, attrValues)) !=
	    SA_AIS_OK)
		LOG_ER("saImmOiRtObjectCreate_2 failed. channel: %s. rc = %d",
		       cname, rc);
	free(dndup);
 ...
 *
 ***************************************************************************/
static uint32_t eds_add_subrec_entry(CHAN_OPEN_REC *copen_rec,
				     SUBSC_REC *subrec)
{
	...
	TRACE("reg_id: %u, chan_id: %u, chan_open_id: %u, subscription id: %u",
	      copen_rec->reg_id, copen_rec->chan_id, copen_rec->chan_open_id,
	      subrec->subscript_id);
	if (copen_rec->subsc_rec_head == NULL)
		copen_rec->subsc_rec_head =
		    subrec; /*If this is the first record */
	else {
		copen_rec->subsc_rec_tail->next =
		    subrec; /* Append at the end */
...
static void eds_remove_subrec_entry(EDS_CB *cb, SUBSC_REC **subrec)
{
	...
	if (p->prev == NULL) {		      /* Top entry */
		if (p->next != NULL) {	/* It's not the only element */
			...
			edsv_free_evt_filter_array(
			    (*subrec)->filters);	 /* Free the filters */
...
static uint32_t eds_add_subscription_to_worklist(EDS_CB *cb, SUBSC_REC *subrec)
{
	...
	while (wp) { /* While there are channels... */
		...
		if (wp->chan_id == subrec->chan_id) {
			...
			if (NULL ==
			    (co = (CHAN_OPEN_REC *)ncs_patricia_tree_get(
				 &wp->chan_open_rec,
				 (uint8_t *)&copen_id_Net))) {
				TRACE_LEAVE2(
				    "channel open record not found: %u",
				    subrec->chan_open_id);
			...
			if (co->reg_id != subrec->reg_list->reg_id) {
				TRACE_LEAVE2("reg id not found: %u",
					     subrec->reg_list->reg_id);
				return (NCSCC_RC_BAD_ATTR);
			}
			/* Set parent chan_open_rec pointer so we can remove
			 * root entry later */
	...
	TRACE_LEAVE();
	return (NCSCC_RC_NO_OBJECT); /* Went through the entire list. Channel
					not found. */
}
 ...
 *
 ***************************************************************************/
static uint32_t eds_add_subscription_to_reglist(EDS_CB *cb, uint32_t reg_id,
						SUBSC_REC *subrec)
{
	EDA_REG_REC *reglist = NULL;
	SUBSC_LIST *sublist = NULL;
	CHAN_OPEN_LIST *cl;
	TRACE_ENTER2("reg_id: %u, chan_id:%u, chan_open_id:%u", reg_id,
		     subrec->chan_id, subrec->chan_open_id);
	...
	while (cl) {
		if (cl->chan_id == subrec->chan_id)
			if (cl->chan_open_id == subrec->chan_open_id) {
				...
				sublist = m_MMGR_ALLOC_EDS_SUBLIST(
				    sizeof(SUBSC_LIST));
				if (!sublist) {
					LOG_CR(
					    "malloc failed for subscription list");
					TRACE_LEAVE();
				...
				if (cl->subsc_list_head == NULL)
					cl->subsc_list_head =
					    sublist; /*If this is the first
							record */
				else {
					cl->subsc_list_tail->next =
					    sublist; /* Append at the end */
				}
...
/****************************************************************************
 *
 * eds_remove_cname_rec - Deletes a channelname record from the channelname
 *list.
 *
...
static uint32_t eds_remove_cname_rec(EDS_CB *cb, EDS_WORKLIST *wp)
{
	...
	if (NULL == (rec_to_del = (EDS_CNAME_REC *)ncs_patricia_tree_get(
			 &cb->eds_cname_list, (uint8_t *)&chan_name_del))) {
	...
	if (NCSCC_RC_SUCCESS !=
	    (rc = ncs_patricia_tree_del(&cb->eds_cname_list,
					&rec_to_del->pat_node))) {
...
uint32_t eds_remove_worklist_entry(EDS_CB *cb, uint32_t chan_id)
{
	...
	while (wp) {
		if (wp->chan_id == chan_id) { /* Found the one we want? */
			TRACE("chan record for :%s, found in worklist",
			      wp->cname);
			/*This functionality is there here before the
			   Bugfix:61494 - Now unlinked channels will be deleted
			   from the database */
			/*  eds_remove_cname_rec(cb,wp); */ /* remove the entry
							       from the channel
							       name database */

			if (wp->prev == NULL) { /* Removing 1st element */
				if (wp->next !=
				    NULL) { /* It's not the only element */
					wp->next->prev =
					    NULL; /* Clear back ptr for new 1st
						     element */
					save_next =
					    wp->next; /* Save new 1st ptr */
				} else { /* Removing the only element */

					save_next = NULL; /* Return a NULL ptr
							     to the list */
				}

				m_NCS_LOCK(&cb->cb_lock, NCS_LOCK_WRITE);
				/* Make sure all retained events have been
				 * removed */
				eds_remove_retained_events(
				    wp->ret_evt_list_head,
				    wp->ret_evt_list_tail);
				/* Destroy the patricia tree for channel open
				 * recs */
				ncs_patricia_tree_destroy(&wp->chan_open_rec);
				m_NCS_UNLOCK(&cb->cb_lock, NCS_LOCK_WRITE);
				m_MMGR_FREE_EDS_CHAN_NAME(
				    wp->cname); /* free channelName */
				m_MMGR_FREE_EDS_WORKLIST(
				    cb->eds_work_list); /* free 1st cell */
				cb->eds_work_list =
				    save_next; /* Set new 1st element address */
				TRACE_LEAVE();
				return (NCSCC_RC_SUCCESS);
			} else if (wp->next ==
				   NULL) { /* Removing last element */
				wp->prev
				    ->next = NULL; /* Clear next ptr for new
						      last element */
				m_NCS_LOCK(&cb->cb_lock, NCS_LOCK_WRITE);
				/* Make sure all retained events have been
				 * removed */
				...
				wp->next->prev =
				    wp
					->prev;		   /* Link previous cell to next
							      one */
				wp->prev->next = wp->next; /* Back link next
							      cell to previous
							    */
				m_NCS_LOCK(&cb->cb_lock, NCS_LOCK_WRITE);
				/* Make sure all retained events have been
			...
			/* This is an ordered list, so if a cell has a chan_id
			 * gtr than what we're looking for, it's not there.
			 */
	...
	TRACE_LEAVE();
	return (
	    NCSCC_RC_NO_OBJECT); /* Went through the entire list. Not found. */
}
 ...
 *
 ***************************************************************************/
static bool is_active_channel(EDS_WORKLIST *wp, uint32_t chan_name_len,
			      uint8_t *chan_name)
{
	...
	if (wp->cname_len == chan_name_len) {
		...
		if (memcmp(wp->cname, chan_name, chan_name_len) == 0) {
			...
			if (!(wp->chan_attrib & CHANNEL_UNLINKED)) {
				TRACE_LEAVE2(
				    "true: channel is not marked as unlinked");
				return (true);
			}
		}
	}
 ...
 *
 ***************************************************************************/
static uint32_t eds_add_cname_rec(EDS_CB *cb, EDS_WORKLIST *wp,
				  uint8_t *chan_name, uint16_t chan_name_len)
{
	EDS_CNAME_REC *cn;
	...
	if (NCSCC_RC_SUCCESS !=
	    ncs_patricia_tree_add(&cb->eds_cname_list, &cn->pat_node)) {
		/* Log it */
 ...
 *
 ***************************************************************************/
static uint32_t eds_add_chan_open_rec(EDS_WORKLIST *wp, uint32_t reg_id,
				      uint32_t chan_id, MDS_DEST dest,
				      ...
				      uint32_t open_flags)
{
	...
	if (ha_state == SA_AMF_HA_STANDBY) {
		TRACE(
		    "Called at STANDBY, simply storing the value received from active");
	...
	/* Insert the record into the patricia tree */
	if (NCSCC_RC_SUCCESS !=
	    ncs_patricia_tree_add(&wp->chan_open_rec, &co->pat_node)) {
	...
	*chan_open_id = co->chan_open_id;
	...
	TRACE_LEAVE2("num users: %u, num publishers:%u, num subscribers:%u",
		     wp->chan_row.num_users, wp->chan_row.num_publishers,
 ...
 *
 ***************************************************************************/
static uint32_t eds_add_chan_open_list(EDS_CB *cb, uint32_t reg_id,
				       uint32_t chan_id, uint32_t chan_open_id)
{
	...
	;
	TRACE_ENTER2(
	    "Associate this chan open with reglist: reg_id:%u, chan_id:%u, chan_open_id:%u",
	...
	rp->chan_open_list =
	    m_MMGR_ALLOC_EDS_COPEN_LIST(sizeof(CHAN_OPEN_LIST));
...
static void eds_channel_close_by_regid(EDS_CB *cb, uint32_t reg_id, bool forced)
{
	...
	while (cl) {
		next = cl->next;
		(void)eds_channel_close(cb, cl->reg_id, cl->chan_id,
					cl->chan_open_id, forced);
		cl = next;
	}
	TRACE_LEAVE();
}
...
static uint32_t eds_remove_chan_open_rec(EDS_WORKLIST *wp, CHAN_OPEN_REC *co)
{
	...
	/* Get the record pointer from the patricia tree */
	if (NULL == (rec_to_del = (CHAN_OPEN_REC *)ncs_patricia_tree_get(
			 &wp->chan_open_rec, (uint8_t *)&co->copen_id_Net))) {
	...
	/* Delete this record from the tree */
	if (NCSCC_RC_SUCCESS !=
	    (rc = ncs_patricia_tree_del(&wp->chan_open_rec,
					&rec_to_del->pat_node))) {
	...
	TRACE_LEAVE2("use count: %u, num_publishers:%u, num_subscribers:%u",
		     wp->use_cnt, wp->chan_row.num_publishers,
		     wp->chan_row.num_subscribers);
	return (NCSCC_RC_SUCCESS);
}
 ...
 *
 ***************************************************************************/
static void eds_remove_chan_open_list(EDS_CB *cb, uint32_t reg_id,
				      uint32_t chan_id, uint32_t chan_open_id)
{
	EDA_REG_REC *reglist;
	CHAN_OPEN_LIST *cl;
	CHAN_OPEN_LIST *prev;
	TRACE_ENTER2("reg_id: %u chan_id:%u, chan_open_id:%u", reg_id, chan_id,
		     chan_open_id);
	...
	prev = cl;
	while (cl) {
		if ((cl->chan_id == chan_id) &&
		    (cl->chan_open_id == chan_open_id))
			break;
		prev = cl; /* Save previous pointer if we need to remove next
			      entry */
		cl = cl->next;
	}
	...
	if (cl == reglist->chan_open_list) { /* 1st in the list? */
		if (cl->next == NULL)	/* Only one in the list? */
			...
			reglist->chan_open_list =
			    cl->next; /* Move next one up */
		}
...
uint32_t eds_add_reglist_entry(EDS_CB *cb, MDS_DEST dest, uint32_t reg_id)
{
	...
	/** Insert the record into the patricia tree **/
	if (NCSCC_RC_SUCCESS !=
	    ncs_patricia_tree_add(&cb->eda_reg_list, &rec->pat_node)) {
		LOG_WA(
		    "patricia tree add failed for reg_id: %u, agent dest: %" PRIx64,
		    rec->reg_id, rec->eda_client_dest);
...
static void eds_del_work_list(EDS_CB *cb, EDS_WORKLIST **p_work_list)
{
	...
	while (NULL != (work_list = *p_work_list)) {
		...
		*p_work_list = work_list->next;
		...
		eds_remove_retained_events(work_list->ret_evt_list_head,
					   work_list->ret_evt_list_tail);
...
uint32_t eds_remove_reglist_entry(EDS_CB *cb, uint32_t reg_id, bool remove_all)
{
	...
	if ((reg_id == 0) && (remove_all == true)) {
		...
		rec_to_del = (EDA_REG_REC *)ncs_patricia_tree_getnext(
		    &cb->eda_reg_list, (uint8_t *)0);

		if (rec_to_del) {
			while (rec_to_del) {
				/** Close all open channels (and remove
				 *subscriptions) * for this registration ID.
				 **/
				eds_channel_close_by_regid(
				    cb, rec_to_del->reg_id, remove_all);

				/** delete the node from the tree
				 **/
				ncs_patricia_tree_del(&cb->eda_reg_list,
						      &rec_to_del->pat_node);
				 ...
				 **/
				rec_to_del =
				    (EDA_REG_REC *)ncs_patricia_tree_getnext(
					&cb->eda_reg_list,
					(uint8_t *)&regId_Net);
			}
		}
		 ...
		 **/
		if (NULL == (rec_to_del = (EDA_REG_REC *)ncs_patricia_tree_get(
				 &cb->eda_reg_list, (uint8_t *)&regId_Net))) {
		...
		if (NCSCC_RC_SUCCESS !=
		    (status = ncs_patricia_tree_del(&cb->eda_reg_list,
						    &rec_to_del->pat_node))) {
 ...
 *
 * This routine is typically used to find the validity of the eda down rec from
 *standby EDA_DOWN_LIST as  EDA client has gone away.
 *
 ****************************************************************************/
bool eds_eda_entry_valid(EDS_CB *cb, MDS_DEST mds_dest)
{
	...
	rp = (EDA_REG_REC *)ncs_patricia_tree_getnext(&cb->eda_reg_list,
						      (uint8_t *)0);
	...
	while (rp != NULL) {
		...
		rp = (EDA_REG_REC *)ncs_patricia_tree_getnext(
		    &cb->eda_reg_list, (uint8_t *)&rp->reg_id_Net);
	}
...
uint32_t eds_remove_eda_down_rec(EDS_CB *cb, MDS_DEST mds_dest)
{
	...
	while (eda_down_rec) {
		if (m_NCS_MDS_DEST_EQUAL(&eda_down_rec->mds_dest, &mds_dest)) {
			...
			/* Reset pointers */
			if (eda_down_rec ==
			    cb->eda_down_list_head) { /* 1st in the list? */
				if (eda_down_rec->next ==
				    NULL) { /* Only one in the list? */
					cb->eda_down_list_head =
					    NULL; /* Clear head sublist pointer
						   */
					cb->eda_down_list_tail =
...
EDA_REG_REC *eds_get_reglist_entry(EDS_CB *cb, uint32_t reg_id)
{
	...
	if (NULL == (rp = (EDA_REG_REC *)ncs_patricia_tree_get(
			 &cb->eda_reg_list, (uint8_t *)&regId_Net))) {
		TRACE_LEAVE2("reg record not found");
...
EDS_WORKLIST *eds_get_worklist_entry(EDS_WORKLIST *wp_root, uint32_t chan_id)
{
	...
	while (wp) {
		if (wp->chan_id == chan_id) {
			...
			/* This is an ordered list, so if a cell has a chan_id
			 * gtr than what we're looking for, it isn't there.
			 */
			if (wp->chan_id > chan_id) {
				TRACE_LEAVE2("record not found");
 ...
 *
 ***************************************************************************/
uint32_t eds_remove_subscription(EDS_CB *cb, uint32_t reg_id, uint32_t chan_id,
				 uint32_t chan_open_id, uint32_t sub_id)
{
	uint32_t rs = NCSCC_RC_SUCCESS;
	EDA_REG_REC *reglist;
	SUBSC_LIST *sublist = NULL;
	...
	CHAN_OPEN_LIST *cl;
	TRACE_ENTER2("reg_id:%u, chan_id:%u, chan_open_id:%u, sub_id: %u",
		     reg_id, chan_id, chan_open_id, sub_id);
...
uint32_t eds_copen_patricia_init(EDS_WORKLIST *wp)
{
	...
	if (NCSCC_RC_SUCCESS !=
	    ncs_patricia_tree_init(&wp->chan_open_rec, &param)) {
 ...
 *
 ***************************************************************************/
uint32_t eds_channel_open(EDS_CB *cb, uint32_t reg_id, uint32_t flags,
			  uint16_t chan_name_len, uint8_t *chan_name,
			  MDS_DEST dest, uint32_t *chan_id,
			  uint32_t *chan_open_id, SaTimeT chan_create_time)
{
	uint32_t rc = NCSCC_RC_SUCCESS;
	EDS_WORKLIST *wp = NULL;
	...
	if (wp == NULL) {
		...
		if (!(flags & SA_EVT_CHANNEL_CREATE)) {
			TRACE_LEAVE2(
			    "SA_AIS_ERR_NOT_EXIST: Create flags not specified");
			return (SA_AIS_ERR_NOT_EXIST);
		}
		cb->eds_work_list =
		    m_MMGR_ALLOC_EDS_WORKLIST(sizeof(EDS_WORKLIST));
		...
		if (cb->ha_state == SA_AMF_HA_STANDBY) {
			wp->chan_id = *chan_id;
			TRACE(
			    "At standby, just store the value sent from active, chan_id:%u",
			    wp->chan_id);
		} else {
			wp->chan_id = 1;
			TRACE(
			    "chan_id assigned for this saEvtChannelOpen is: %u",
			    wp->chan_id);
		}
		...
		*(wp->cname + chan_name_len) = '\0';
		...
		/* Update - channel Table objects with default values & creation
		 * time stamp */
		...
		if (cb->ha_state == SA_AMF_HA_ACTIVE) {
			TRACE(
			    "I'm ACTIVE eds, create the IMM object for this channel");
			create_runtime_object((char *)wp->cname,
					      wp->chan_row.create_time,
					      cb->immOiHandle);
		}
		...
		if (eds_copen_patricia_init(wp) != NCSCC_RC_SUCCESS) {
			TRACE_LEAVE2(
			    "SA_AIS_ERR_LIBRARY: channel open patricia tree init failed");
		...
		if (reg_id != 0) {
			...
			/* Assign a new chan_open_id for this open channel
			 * instance */
			rc = eds_add_chan_open_rec(wp, reg_id, wp->chan_id,
						   dest, chan_open_id, ha_state,
						   flags);
			if (rc != NCSCC_RC_SUCCESS) {
				TRACE_LEAVE2(
				    "SA_AIS_ERR_LIBRARY: channel open record add failed");
				return (SA_AIS_ERR_LIBRARY);
			}
			/* Add an entry to the reglist too */
			rc = eds_add_chan_open_list(cb, reg_id, wp->chan_id,
						    *chan_open_id);
			if (rc != NCSCC_RC_SUCCESS) {
				LOG_IN(
				    "channel open to client handle association not found for channel: %s",
				    wp->cname);
				/* Get the chan open rec from the patricia tree
				 */
				...
				if (NULL ==
				    (co =
					 (CHAN_OPEN_REC *)ncs_patricia_tree_get(
					     &wp->chan_open_rec,
					     (uint8_t *)&copen_id_Net))) {
					TRACE_LEAVE2(
					    "chan_open_id: %u not found in pat tree",
					    *chan_open_id);
					return NCSCC_RC_FAILURE;
				}
				...
				TRACE_LEAVE2(
				    "SA_AIS_ERR_LIBRARY: channel open add failed");
				return (SA_AIS_ERR_LIBRARY);
			}
		}
		...
		*chan_id = wp->chan_id; /* Return channel ID #1 */
		...
		eds_add_cname_rec(
		    cb, wp, chan_name,
		    chan_name_len); /* add the channel name to the cname list */
	...
	while (wp) {
		...
		/* Is this element an active (linked) channel with the correct
		 * name? */
	...
	if (!(flags & SA_EVT_CHANNEL_CREATE)) {
		TRACE_LEAVE2(
		    "SA_AIS_ERR_NOT_EXIST. Channel not found and create flag is not specified");
		return (SA_AIS_ERR_NOT_EXIST);
	}
	...
	if (prevp != NULL) {
		...
		*(wp->cname + chan_name_len) = '\0';
		...
		if (cb->ha_state == SA_AMF_HA_STANDBY) {
			...
			wp->chan_id = prevp->chan_id +
				      1; /* New ID is previous entry +1 */
			TRACE(
			    "chan_id assigned for this saEvtChannelOpen is: %u",
			    wp->chan_id);
		}
		...
		*chan_id = wp->chan_id; /* Return the channel ID */
		...
		eds_add_cname_rec(
		    cb, wp, chan_name,
		    chan_name_len); /* add the channel name to cname list */
 ...
 *
 ***************************************************************************/
uint32_t eds_channel_close(EDS_CB *cb, uint32_t reg_id, uint32_t chan_id,
			   uint32_t chan_open_id, bool forced)
{
	uint32_t copen_id_Net;
	EDS_WORKLIST *wp;
	CHAN_OPEN_REC *co;
	...
	SaAisErrorT rc = SA_AIS_OK;
	TRACE_ENTER2(
	    "Closing channel open instance: chan_open_id: %u,chan_id: %u,reg_id: %u",
	    chan_open_id, chan_id, reg_id);
	...
	while (subrec) {	     /* Make sure no subscriptions left */
		next = subrec->next;
		(void)eds_remove_subscription(
		    cb, subrec->reg_list->reg_id, subrec->chan_id,
		    subrec->chan_open_id, subrec->subscript_id);
		subrec = next;
	}

	wp->chan_row.num_users -= 1;

	TRACE("Num users: %u, use count: %u", wp->chan_row.num_users,
	      wp->use_cnt);
	...
	if (wp->use_cnt == 0) {
		...
		if ((wp->chan_attrib & CHANNEL_UNLINKED) || (true == forced)) {
			...
			if ((true == forced) &&
			    (!(wp->chan_attrib & CHANNEL_UNLINKED))) {
				TRACE(
				    "forced flag 'and' CHANNEL is marked as CHANNEL_UNLINKED");
				eds_remove_cname_rec(cb, wp);
			}
			if (cb->ha_state == SA_AMF_HA_ACTIVE) {
				TRACE("Delete run time object");
				if ((rc = immutil_saImmOiRtObjectDelete(
					 cb->immOiHandle, &chan_name)) !=
				    SA_AIS_OK) {
					LOG_ER(
					    "saImmOiRtObjectDelete failed. Channel: %s. rc = %u",
					    chan_name.value, rc);
					return NCSCC_RC_FAILURE;
				}
			}
 ...
 *
 ***************************************************************************/
uint32_t eds_channel_unlink(EDS_CB *cb, uint32_t chan_name_len,
			    uint8_t *chan_name)
{
	EDS_WORKLIST *wp;
	...
	while (wp) {
		...
		if (is_active_channel(wp, chan_name_len, chan_name)) {
			wp->chan_attrib |=
			    CHANNEL_UNLINKED; /* Set the unlink flag */
			TRACE("Setting the unlink flag for this channel");

			/* If no one else interested in this channel, remove it
			 * completely */
			eds_remove_cname_rec(cb, wp);
			...
			if (wp->use_cnt == 0) {
				...
				if (cb->ha_state == SA_AMF_HA_ACTIVE) {
					if ((rc = immutil_saImmOiRtObjectDelete(
						 cb->immOiHandle,
						 &channel_name)) != SA_AIS_OK) {
						LOG_ER(
						    "saImmOiRtObjectDelete failed. Channel: %s. rc = %u",
						    channel_name.value, rc);
						TRACE_LEAVE();
						return NCSCC_RC_FAILURE;
					}
				}
	...
	return (SA_AIS_ERR_NOT_EXIST); /* Went through the entire list. Not
					  found. */
}

static void eds_retd_evt_del(EDS_RETAINED_EVT_REC **, EDS_RETAINED_EVT_REC **,
			     EDS_RETAINED_EVT_REC *, bool);
/****************************************************************************
 *
 ...
 *
 ****************************************************************************/
uint32_t eds_store_retained_event(EDS_CB *cb, EDS_WORKLIST *wp,
				  CHAN_OPEN_REC *co,
				  EDSV_EDA_PUBLISH_PARAM *publish_param,
				  SaTimeT orig_publish_time)
{
	...
	if (retained_evt == NULL) {
		LOG_CR(
		    "malloc failed for retention event record: Number of retained events: %u",
	...
	if (0 == (retained_evt->retd_evt_hdl = ncshm_create_hdl(
		      NCS_HM_POOL_ID_COMMON, NCS_SERVICE_ID_EDS,
		      (NCSCONTEXT)retained_evt))) {
	...
	if (co) {
		...
		retained_evt->retd_evt_chan_open_id =
		    publish_param->chan_open_id;
	}

	/* Copy the publisher name */
	memcpy(retained_evt->publisherName.value,
	       publish_param->publisher_name.value, SA_MAX_NAME_LENGTH);
	...
	if (wp->ret_evt_list_head[retained_evt->priority] == NULL) {
		...
		wp->ret_evt_list_tail[retained_evt->priority]->next =
		    retained_evt;
	}
	...
	if (retained_evt->retentionTime != SA_TIME_MAX)
		error = eds_start_tmr(
		    cb, &retained_evt->ret_tmr, EDS_RET_EVT_TMR,
		    retained_evt->retentionTime, retained_evt->retd_evt_hdl);
	...
	if (error != NCSCC_RC_SUCCESS) {
		...
		eds_retd_evt_del(&wp->ret_evt_list_head[retained_evt->priority],
				 &wp->ret_evt_list_tail[retained_evt->priority],
				 retained_evt, true);
	} else {
		wp->chan_row.num_ret_evts++;
		TRACE("Number of retained events: %u",
		      wp->chan_row.num_ret_evts);
	}
...
/****************************************************************************
  ...
  Notes         :
******************************************************************************/
static void eds_retd_evt_del(EDS_RETAINED_EVT_REC **list_head,
			     EDS_RETAINED_EVT_REC **list_tail,
			     EDS_RETAINED_EVT_REC *rm_node, bool give_hdl)
{
		...
		*list_head = rm_node->next;
		...
		while (NULL != list_iter) {
			if (list_iter->next == rm_node) {
				/* If this is the last node then modify the tail
				 * pointer */
				if (*list_tail == rm_node)
					*list_tail = list_iter;
				...
				/* Free memory associated with this event */
				edsv_free_evt_pattern_array(
				    rm_node->patternArray);
				if (rm_node->data)
					m_MMGR_FREE_EDSV_EVENT_DATA(
					    rm_node->data);
				...
				/* Gotta destroy the hdl anyway */
				ncshm_destroy_hdl(NCS_SERVICE_ID_EDS,
						  rm_node->retd_evt_hdl);
...
/****************************************************************************
  ...
  Notes         :
******************************************************************************/
static EDS_RETAINED_EVT_REC *
eds_find_retd_evt_by_chan_open_id(EDS_WORKLIST *wp, uint32_t chan_open_id,
				  uint32_t event_id)
{
	EDS_RETAINED_EVT_REC *retd_evt;
	SaUint8T list_iter;
	TRACE_ENTER2("chan_name: %s, chan_open_id: %u, event_id: %u", wp->cname,
		     chan_open_id, event_id);

	for (list_iter = SA_EVT_HIGHEST_PRIORITY;
	     list_iter <= SA_EVT_LOWEST_PRIORITY; list_iter++) {
		retd_evt = wp->ret_evt_list_head[list_iter];
		while (retd_evt) {
			if (retd_evt->retd_evt_chan_open_id == chan_open_id &&
			    retd_evt->event_id == event_id) {
				TRACE_LEAVE();
				return retd_evt;
			}
 ...
 *
 ****************************************************************************/
uint32_t eds_clear_retained_event(EDS_CB *cb, uint32_t chan_id,
				  uint32_t chan_open_id, uint32_t event_id,
				  bool give_hdl)
{
	EDS_WORKLIST *wp;
	...
	if (NULL != (retained_evt = eds_find_retd_evt_by_chan_open_id(
			 wp, chan_open_id, event_id))) {
		eds_retd_evt_del(&wp->ret_evt_list_head[retained_evt->priority],
				 &wp->ret_evt_list_tail[retained_evt->priority],
				 retained_evt, give_hdl);
		wp->chan_row.num_ret_evts--;
		TRACE("Number of retained events: %u",
		      wp->chan_row.num_ret_evts);
	} else {
 ...
 *
 ****************************************************************************/
void eds_remove_retained_events(EDS_RETAINED_EVT_REC **p_retd_evt_rec,
				EDS_RETAINED_EVT_REC **list_tail)
{
