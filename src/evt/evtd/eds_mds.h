...
*******************************************************************************/
#ifndef EVT_EVTD_EDS_MDS_H_
#define EVT_EVTD_EDS_MDS_H_
...
uint32_t eds_mds_init(EDS_CB *);
uint32_t eds_mds_vdest_create(EDS_CB *);
uint32_t eds_mds_finalize(EDS_CB *cb);
uint32_t eds_mds_vdest_destroy(EDS_CB *cb);
...
#define m_EDS_EDSV_LIMIT_GET_MSG_FILL(m, rs)                                   \
  do {                                                                         \
    ...
    {                                                                          \
      ...
      (m).info.api_resp_info.param.limit_get_rsp.max_evt_size =                \
          SA_EVENT_DATA_MAX_SIZE;                                              \
      (m).info.api_resp_info.param.limit_get_rsp.max_ptrn_size =               \
          EDSV_MAX_PATTERN_SIZE;                                               \
      (m).info.api_resp_info.param.limit_get_rsp.max_num_ptrns =               \
          EDSV_MAX_PATTERNS;                                                   \
      (m).info.api_resp_info.param.limit_get_rsp.max_ret_time =                \
          EDSV_MAX_RETENTION_TIME;                                             \
    }                                                                          \
  } while (0);
...
#define m_EDS_EDSV_CHAN_RETENTION_TMR_CLEAR_SYNC_MSG_FILL(m, rs) \
  do {                                                           \
    memset(&(m), 0, sizeof(EDSV_MSG));                           \
    (m).type = EDSV_EDA_API_RESP_MSG;                            \
    (m).info.api_resp_info.type.api_rsp =                        \
...
#define m_EDS_EDSV_DELIVER_EVENT_CB_MSG_FILL(m, regid, subid, chanid, copenid, \
                                             ptrns, prio, pub_name, pub_time,  \
                                             ret_time, evid, recoid, buf_len,  \
                                             buf)                              \
  do {                                                                         \
    memset(&(m), 0, sizeof(EDSV_MSG));                                         \
    (m).type = EDSV_EDS_CBK_MSG;                                               \
