...
void tet_saEvtInitialize(SaEvtHandleT *ptrEvtHandle)
{
	...
	WHILE_TRY_AGAIN(gl_rc)
	{
		...
		RETRY_SLEEP;
		gl_rc = saEvtInitialize(ptrEvtHandle, &gl_evtCallbacks,
					&gl_version);
				 ...
				 SaSelectionObjectT *ptrSelectionObject)
{
	...
	WHILE_TRY_AGAIN(gl_rc)
	{
		...
		RETRY_SLEEP;
		gl_rc =
		    saEvtSelectionObjectGet(*ptrEvtHandle, ptrSelectionObject);
		if (gl_rc == SA_AIS_OK)
			  ...
			  SaEvtChannelHandleT *ptrChannelHandle)
{
	static int try_again_count;
	gl_rc =
	    saEvtChannelOpen(*ptrEvtHandle, &gl_channelName,
			     gl_channelOpenFlags, gl_timeout, ptrChannelHandle);
	WHILE_TRY_AGAIN(gl_rc)
	{
		++try_again_count;
		if (try_again_count == MAX_NUM_TRY_AGAINS) {
		...
		RETRY_SLEEP;
		gl_rc = saEvtChannelOpen(*ptrEvtHandle, &gl_channelName,
...
{
	static int try_again_count;
	gl_rc = saEvtChannelOpenAsync(*ptrEvtHandle, gl_invocation,
	...
	WHILE_TRY_AGAIN(gl_rc)
	{
		...
		RETRY_SLEEP;
		gl_rc =
		    saEvtChannelOpenAsync(*ptrEvtHandle, gl_invocation,
					  &gl_channelName, gl_channelOpenFlags);
		if (gl_rc == SA_AIS_OK)
...
void tet_saEvtChannelClose(SaEvtChannelHandleT *ptrChannelHandle)
{
	...
	resultSuccess("saEvtChannelClose() invoked for saEvtChannelOpen",
		      gl_rc);
}
...
void tet_saEvtEventAttributesGet(SaEvtEventHandleT *ptrEventHandle)
{
	...
	ptrPatternArray->patterns = (SaEvtEventPatternT *)malloc(
	...
	while (pCount < gl_allocatedNumber) {
		ptrPatternArray->patterns[pCount].allocatedSize =
		    gl_patternLength;
	...
	gl_rc = saEvtEventAttributesGet(
	    *ptrEventHandle, ptrPatternArray, &gl_priority, &gl_retentionTime,
	    getPublishName, &gl_publishTime, &gl_evtId);
	WHILE_TRY_AGAIN(gl_rc)
	{
		++try_again_count;
		if (try_again_count == MAX_NUM_TRY_AGAINS) {
		...
		RETRY_SLEEP;
		gl_rc = saEvtEventAttributesGet(
		    *ptrEventHandle, ptrPatternArray, &gl_priority,
		    &gl_retentionTime, getPublishName, &gl_publishTime,
		    &gl_evtId);
		if (gl_rc == SA_AIS_OK)
	...
	if (gl_rc == 1) {
		...
		while (pCount < ptrPatternArray->allocatedNumber) {
			printf("Pattern Size: %llu\n",
			       ptrPatternArray->patterns[pCount].patternSize);
			character =
			    ptrPatternArray->patterns[pCount].pattern[length];
			printf("Pattern: ");
			...
			while (
			    ...
			    length <
				ptrPatternArray->patterns[pCount].patternSize) {
				printf("%c", ptrPatternArray->patterns[pCount]
						 .pattern[length]);
				length++;
				if (length < ptrPatternArray->patterns[pCount]
						 .patternSize) {
					character =
					    ptrPatternArray->patterns[pCount]
						.pattern[length];
				}
			}
	...
	printf("\n******************************\n");
	if (gl_b03_flag) {
		gl_rc = saEvtEventPatternFree(*ptrEventHandle,
...
void tet_saEvtEventAttributesSet(SaEvtEventHandleT *ptrEventHandle)
{
	...
	WHILE_TRY_AGAIN(gl_rc)
	{
		...
		gl_rc = saEvtEventAttributesSet(
		    *ptrEventHandle, &gl_patternArray, gl_priority,
...
void tet_saEvtEventDataGet(SaEvtEventHandleT *ptrEventHandle)
{
	...
	gl_rc =
	    saEvtEventDataGet(*ptrEventHandle, &gl_eventData, &tempDataSize);
	WHILE_TRY_AGAIN(gl_rc)
	{
		++try_again_count;
		if (try_again_count == MAX_NUM_TRY_AGAINS) {
		...
		RETRY_SLEEP;
		gl_rc = saEvtEventDataGet(*ptrEventHandle, &gl_eventData,
					  &tempDataSize);
		if (gl_rc == SA_AIS_OK)
...
{
	static int try_again_count;
	gl_rc = saEvtEventPublish(*ptrEventHandle, gl_eventData,
				  gl_eventDataSize, ptrEvtId);
	WHILE_TRY_AGAIN(gl_rc)
	{
		++try_again_count;
		if (try_again_count == MAX_NUM_TRY_AGAINS) {
			     ...
			     SaEvtSubscriptionIdT *ptrSubId)
{
	...
	int length = 0;
	gl_rc =
	    saEvtEventSubscribe(*ptrChannelHandle, &gl_filterArray, *ptrSubId);
	WHILE_TRY_AGAIN(gl_rc)
	{
		++try_again_count;
		if (try_again_count == MAX_NUM_TRY_AGAINS) {
		...
		RETRY_SLEEP;
		gl_rc = saEvtEventSubscribe(*ptrChannelHandle, &gl_filterArray,
					    *ptrSubId);
		if (gl_rc == SA_AIS_OK)
	...
	while (length < gl_filterArray.filtersNumber) {
		printf("\nFilter Type: %d",
		       gl_filterArray.filters[length].filterType);
		printf("\nFilter Size: %llu",
...
void tet_saEvtEventUnsubscribe(SaEvtChannelHandleT *ptrChannelHandle)
{
	...
	WHILE_TRY_AGAIN(gl_rc)
	{
		...
		RETRY_SLEEP;
		gl_rc =
		    saEvtEventUnsubscribe(*ptrChannelHandle, gl_subscriptionId);
		if (gl_rc == SA_AIS_OK)
				      ...
				      SaEvtEventIdT *ptrEvtId)
{
	...
	WHILE_TRY_AGAIN(gl_rc)
	{
		...
		RETRY_SLEEP;
		gl_rc =
		    saEvtEventRetentionTimeClear(*ptrChannelHandle, *ptrEvtId);
		if (gl_rc == SA_AIS_OK)
...
void eda_selection_thread()
{
	...
	WHILE_TRY_AGAIN(gl_rc)
	{
		RETRY_SLEEP;
		gl_rc =
		    saEvtSelectionObjectGet(gl_threadEvtHandle, &gl_selObject);
	}
	if (gl_rc != SA_AIS_OK) {
	...
	while (1) {
		...
		WHILE_TRY_AGAIN(gl_rc)
		{
			RETRY_SLEEP;
			gl_rc =
			    saEvtDispatch(gl_threadEvtHandle, SA_DISPATCH_ALL);
		}
		sleep(1);
	}
}
