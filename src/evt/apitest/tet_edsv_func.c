...
/* Subscriber(3) - Publisher - Subscriber(3) - Subscribe(4) */
{
 ...
 instance***");
	printf("\n\n******Subscriber******");
	tet_printf(
	    " F:2: Channel Open with Single Event Handle and Subscriptions\
with Same Patterns");
	...
	if (gl_eventDataSize == tempDataSize &&
	    gl_subscriptionId == gl_dupSubscriptionId) {
		iCmpCount++;
	}
...
/* Subscriber(3) - Publisher - Subscribe(4) */
{
	...
	printf("\n\n******Subscriber******");
	tet_printf(
	    " F:3: Channel Open with Single Event Handle and Subscriptions\
...
/* Subscriber(I1,3) - Publisher(I2) - Subscriber(I1,4) */
{
 ...
 multiple instances*************");
	printf("\nFirst gl_evtHandle");
	tet_printf(
	    " F:5: Channel Open with Multiple Event Handles and Subscriptions\
with Same Patterns");
...
/* Subscriber(I1,3) - Publisher(I2) - Subscriber(I1,4) */
{
 ...
 different patterns************");
	printf("\nFirst gl_evtHandle");
	tet_printf(
	    " F:6: Channel Open with Multiple Event Handles and Subscriptions\
with Different Patterns");
...
/* Channel Close with subscriber and publisher after close */
{
	...
	j = saEvtEventSubscribe(gl_channelHandle, &gl_filterArray,
				gl_subscriptionId);
	if (j != SA_AIS_OK) {
		gl_error = SA_AIS_OK;
	}
...
/* Channel Unlink being invoked with an existing open instance */
{
	...
	tet_printf(
	    " F:11: Channel Unlink invoked with an Open Channel instance");
...
/* Attributes Set in the deliver callback function */
{
	...
	i = saEvtEventAttributesSet(gl_eventDeliverHandle, &gl_patternArray,
...
/* Event Subscribe with prefix filter */
{
	...
	if (gl_eventDataSize == tempDataSize &&
	    gl_dupSubscriptionId == gl_subscriptionId) {
		iCmpCount++;
	}
	printf("\n RetainedEvent = 1 \n");
	tet_saEvtEventRetentionTimeClear(&gl_channelHandle, &gl_evtId);
...
/* Event Subscribe with less patterns than filters */
{
	SaEvtEventFilterT moreFilters[2] = {
	    {4, {5, 5, (SaUint8T *)"Hello"}}, {4, {4, 4, (SaUint8T *)"Hell"}},
	};
...
/* Event Subscribe with less patterns than filters with filter size 0 */
{
	SaEvtEventFilterT moreFiltersSize[2] = {
	    {4, {5, 5, (SaUint8T *)"Hello"}}, {3, {0, 0, (SaUint8T *)""}},
	};
	SaEvtEventPatternT patternL[1] = {
	    {5, 5, (SaUint8T *)"Hello"},
	};
...
/* Event Subscribe with mutliple filters with patternsize lessthan filtersize*/
{
	...
	SaEvtEventPatternT multiplePattern[5] = {
	    {5, 5, (SaUint8T *)"Hello"}, {4, 4, (SaUint8T *)"Hell"},
	    {4, 4, (SaUint8T *)"Test"},  {4, 4, (SaUint8T *)"News"},
	    {4, 4, (SaUint8T *)"Moto"},
	};
	SaEvtEventFilterT multipleFilter[5] = {
	    {3, {5, 5, (SaUint8T *)"Hello"}}, {3, {4, 4, (SaUint8T *)"Hell"}},
	    {3, {4, 4, (SaUint8T *)"Test"}},  {3, {4, 4, (SaUint8T *)"News"}},
	    {3, {4, 4, (SaUint8T *)"Moto"}},
	};
...
{
	SaEvtEventFilterT prefixFilter[1] = {{1, {3, 3, (SaUint8T *)"Abc"}}};
...
{
	SaEvtEventFilterT suffixFilter[1] = {{2, {3, 3, (SaUint8T *)"Hel"}}};
...
{
	SaEvtEventFilterT exactDiffFilter[1] = {{3, {5, 5, (SaUint8T *)"Hel"}}};
...
/* Event Subscribe with different filter types */
{
	...
	SaEvtEventFilterT diffFilterType[5] = {
	    {3, {5, 5, (SaUint8T *)"Hello"}}, {1, {2, 2, (SaUint8T *)"He"}},
	    {2, {4, 3, (SaUint8T *)"est"}},   {4, {4, 4, (SaUint8T *)"News"}},
	    {3, {4, 4, (SaUint8T *)"Moto"}},
	};
...
{
	SaEvtEventFilterT prefixFilter[1] = {{1, {6, 6, (SaUint8T *)"HelloA"}}};
...
{
	int i;
	SaEvtEventFilterT prefixFilter[1] = {{4, {6, 6, (SaUint8T *)"HelloA"}}};

	printf(
	    "\n\n*************Case 32: Event Publish for NULL pattern array with\
 attributes set*************");
	tet_printf(
	    "F:32: Event Publish for Null Pattern Array with Attributes Set");
...
{
	SaEvtEventPatternT multiplePattern[5] = {
	    {5, 5, (SaUint8T *)"Hello"}, {3, 3, (SaUint8T *)"Hai"},
	    {4, 4, (SaUint8T *)"Good"},  {5, 5, (SaUint8T *)"Better"},
	    {4, 4, (SaUint8T *)"Best"},
	};
...
*******");
	tet_printf(
	    " F:33: Event Subscription with Different Filters for Different\
...
different channel*************");
	tet_printf(
	    "F:34:Multiple Open Channels have Same Publisher and Subscriber");
...
/* Sync and Async channel for subscriber and publisher */
{
 ...
 publisher*************");
	tet_printf(
	    " F:35: Sync Open Channel for Subscriber and Async Open Channel\
 for Publisher");
	...
	gl_rc = saEvtChannelOpenAsync(gl_evtHandle, gl_invocation,
				      &gl_channelName, gl_channelOpenFlags);
...
{
	int i;
	SaEvtEventPatternT duplicate[1] = {{5, 5, (SaUint8T *)"Test"}};
...
/* Mulitple Handle subscribers and publisher */
{
	...
	SaEvtHandleT tempEvtHandle, localEvtHandle;
	SaEvtChannelHandleT tempChannelHandle;
	SaEvtEventFilterT duplicate[1] = {{3, {5, 5, (SaUint8T *)"Hello"}}};
	printf(
...
void tet_EventPublish_EventOrder() /* Event Publish with event order */
{
	...
	tet_printf(
	    " F:40: Sequential Delivery conformance of the Received Events");
	var_initialize();
...
void tet_EventAttributesGet_LessPatternsNumber()
{
 ...
 number*************");
	...
	gl_rc = saEvtEventAttributesGet(
	    gl_eventHandle, &tempPatternArray, &gl_priority, &gl_retentionTime,
...
void tet_Subscribe_before_ChannelOpen()
{
	...
	i = saEvtEventSubscribe(gl_channelHandle, &gl_filterArray,
				gl_subscriptionId);
...
void tet_Allocate_before_ChannelOpen()
{
	...
	tet_printf(
	    " M:9: Not able to Allocate EventHeadr Before Channel Opened");
...
void tet_Allocate_after_ChannelClose()
{
	...
	tet_printf(
	    " M:10: Not able to Allocate EventHeadr After Channel Closed");
...
void tet_Allocate_after_ChannelUnlink()
{
	...
	tet_printf(
	    " M:11: Not able to Allocate EventHeadr After Channel Unlinked");
...
void tet_Allocate_check_DefaultValues()
{
 ...
 AttributeGet");
	...
	if (gl_priority != 3 && gl_retentionTime != 0 &&
	    gl_publisherName.length != 0 && gl_evtId != 0) {
		i = 0;
	}
...
void tet_AttributesSet_before_Allocate()
{
 ...
 EventHeadr");
	...
	gl_allocatedNumber = 2;
	gl_patternLength = 5;
	i = saEvtEventAttributesSet(gl_eventHandle, &gl_patternArray,
				    gl_priority, gl_retentionTime,
				    &gl_publisherName);
...
void tet_AttributesSet_after_EventFree()
{
	...
	tet_printf(
	    "M:15:Not able to Set Event Attributes After Freeing EventHeadr");
...
void tet_AttributesSet_after_ChannelClose()
{
	...
	tet_printf(
	    " M:16: Not able to Set Event Attributes After Channel Closed");
...
void tet_AttributesSet_after_ChannelUnlink()
{
	...
	tet_printf(
	    " M:17: Not able to Set Event Attributes After Channel Unlinked");
...
void tet_Publish_before_Allocate()
{
	...
	tet_printf(
	    "M:19:Not able to Publish an Event Before Allocating EventHeadr");
	...
	i = saEvtEventPublish(gl_eventHandle, gl_eventData, gl_eventDataSize,
			      &gl_evtId);
...
void tet_Publish_after_EventFree()
{
	...
	tet_printf(
	    " M:20: Not able to Publish an Event After Freeing EventHeadr");
...
void tet_Publish_after_ChannelUnlink()
{
	...
	tet_printf(
	    " M:22: Not able to Publish an Event After Channel Unlinked");
...
void tet_AttributesGet_before_Allocate()
{
	...
	tet_printf(
	    "M:28: Not able to Get Attributes Before Allocating EventHeadr");
	...
	i = saEvtEventAttributesGet(
	    gl_eventHandle, &gl_patternArray, &gl_priority, &gl_retentionTime,
	    &gl_publisherName, &gl_publishTime, &gl_evtId);
	printf("\ntry attributes get");
...
void tet_AttributesGet_after_EventFree()
{
	...
	tet_printf(
	    " M:29: Not able to Get Attributes After Freeing EventHeadr");
...
void tet_RetentionTimeClear_before_ChannelOpen()
{
	...
	tet_printf(
	    " M:37: Not able to Clear RetentionTimer Before Channel Opened");
...
void tet_RetentionTimeClear_after_ChannelClose()
{
	...
	tet_printf(
	    " M:38: Not able to Clear RetentionTimer After Channel Closed");
...
void tet_RetentionTimeClear_after_ChannelUnlink()
{
	...
	tet_printf(
	    " M:39: Not able to Clear RetentionTimer After Channel Unlinked");
...
void tet_ChannelOpen_after_ChannelUnlink()
{
	...
	{
		RETRY_SLEEP;
		i = saEvtChannelOpen(gl_evtHandle, &gl_channelName,
				     gl_channelOpenFlags, 1000000000.0,
				     &gl_channelHandle);
	}
...
void tet_DataGet_without_Memory()
{
	...
	tet_printf(
	    " M:58: Not able to Get Event Data with No Memory allocated ");
	...
	gl_eventDataSize = 0;
	i = saEvtEventDataGet(gl_eventDeliverHandle, &gl_eventData,
			      &gl_eventDataSize);
