...
//
AVD_APP_TYPE::AVD_APP_TYPE(const std::string &dn) : name(dn) {}
...
static AVD_APP_TYPE *apptype_create(const std::string &dn,
                                    const SaImmAttrValuesT_2 **attributes) {
  int i = 0;
  unsigned int j;
  ...
  for (j = 0; j < attr->attrValuesNumber; j++) {
    app_type->sgAmfApptSGTypes.push_back(
        Amf::to_string(static_cast<SaNameT *>(attr->attrValues[j])));
  }
  ...
  switch (opdata->operationType) {
    ...
    case CCBUTIL_MODIFY:
      report_ccb_validation_error(opdata,
                                  "Modification of SaAmfAppType not supported");
      break;
    case CCBUTIL_DELETE:
      ...
      if (nullptr != app_type->list_of_app) {
        ...
        if (app_exist == true) {
          report_ccb_validation_error(opdata, "SaAmfAppType '%s' is in use",
                                      app_type->name.c_str());
          goto done;
        }
      }
...
SaAisErrorT avd_apptype_config_get(void) {
  ...
  while (immutil_saImmOmSearchNext_2(
             searchHandle, &dn_name,
             const_cast<SaImmAttrValuesT_2 ***>(&attributes)) == SA_AIS_OK) {
    std::string dn(Amf::to_string(&dn_name));
    if (!is_config_valid(dn, attributes, nullptr)) goto done2;

    if ((app_type = avd_apptype_get(dn)) == nullptr) {
      if ((app_type = apptype_create(dn, attributes)) == nullptr) goto done2;
