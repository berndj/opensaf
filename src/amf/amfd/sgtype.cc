...
void avd_sgtype_add_sg(AVD_SG *sg) { sg->sg_type->list_of_sg.push_back(sg); }
...
void avd_sgtype_remove_sg(AVD_SG *sg) {
  ...
  if (sg_type != nullptr) {
    auto pos =
        std::find(sg_type->list_of_sg.begin(), sg_type->list_of_sg.end(), sg);
...
static int is_config_valid(const std::string &dn,
  ...
  rc = immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfSgtRedundancyModel"),
                       attributes, 0, &value);
  osafassert(rc == SA_AIS_OK);

  if ((value < SA_AMF_2N_REDUNDANCY_MODEL) ||
      (value > SA_AMF_NO_REDUNDANCY_MODEL)) {
    report_ccb_validation_error(opdata,
                                "Invalid saAmfSgtRedundancyModel %u for '%s'",
                                value, dn.c_str());
    return 0;
  }
  ...
  if (j == 0) {
    report_ccb_validation_error(opdata, "No SU types configured for '%s'",
                                dn.c_str());
    return 0;
  }

  if ((immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfSgtDefAutoRepair"),
                       attributes, 0, &abool) == SA_AIS_OK) &&
      (abool > SA_TRUE)) {
    report_ccb_validation_error(
        opdata, "Invalid saAmfSgtDefAutoRepair %u for '%s'", abool, dn.c_str());
    return 0;
  }

  if ((immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfSgtDefAutoAdjust"),
                       attributes, 0, &abool) == SA_AIS_OK) &&
      (abool > SA_TRUE)) {
    report_ccb_validation_error(
        opdata, "Invalid saAmfSgtDefAutoAdjust %u for '%s'", abool, dn.c_str());
    return 0;
  }
...
//
AVD_AMF_SG_TYPE::AVD_AMF_SG_TYPE(const std::string &dn) : name(dn) {}

/**
 * Create a new SG type object and initialize its attributes from the attribute
 * list.
 * @param dn
 * @param attributes
 *
 ...
 */
static AVD_AMF_SG_TYPE *sgtype_create(const std::string &dn,
                                      const SaImmAttrValuesT_2 **attributes) {
  int i = 0, rc = -1;
  unsigned j = 0;
  AVD_AMF_SG_TYPE *sgt;
  ...
  error = immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfSgtRedundancyModel"),
  ...
  while ((attr = attributes[i++]) != nullptr) {
    if (!strcmp(attr->attrName, "saAmfSgtValidSuTypes")) {
      ...
      if (immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfSgtDefAutoRepair"),
                          attributes, 0,
                          &sgt->saAmfSgtDefAutoRepair) != SA_AIS_OK) {
      ...
      if (immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfSgtDefAutoAdjust"),
                          attributes, 0,
                          &sgt->saAmfSgtDefAutoAdjust) != SA_AIS_OK) {
      ...
      if (immutil_getAttr(
              const_cast<SaImmAttrNameT>("saAmfSgtDefCompRestartProb"),
      ...
      if (immutil_getAttr(
              const_cast<SaImmAttrNameT>("saAmfSgtDefCompRestartMax"),
      ...
      if (immutil_getAttr(
              const_cast<SaImmAttrNameT>("saAmfSgtDefSuRestartProb"),
              attributes, 0, &sgt->saAmfSgtDefSuRestartProb) != SA_AIS_OK) {
        LOG_ER("Get saAmfSgtDefSuRestartProb FAILED for '%s'", dn.c_str());
        goto done;
      }

      if (immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfSgtDefSuRestartMax"),
                          attributes, 0,
                          &sgt->saAmfSgtDefSuRestartMax) != SA_AIS_OK) {
...
/**
 * Get configuration for all SaAmfSGType objects from IMM and create AVD
 * internal objects.
 * @param cb
 *
 * @return int
...
SaAisErrorT avd_sgtype_config_get(void) {
  ...
  SaAisErrorT error = immutil_saImmOmSearchInitialize_2(
...
static SaAisErrorT sgtype_ccb_completed_modify_hdlr(
  ...
  while ((attr_mod = opdata->param.modify.attrMods[i++]) != nullptr) {
    ...
    if (!strcmp(attr_mod->modAttr.attrName, "saAmfSgtDefAutoRepair")) {
      ...
      if (sgt_autorepair > SA_TRUE) {
        report_ccb_validation_error(
            opdata, "invalid saAmfSgtDefAutoRepair in:'%s'", sgt->name.c_str());
        rc = SA_AIS_ERR_BAD_OPERATION;
        goto done;
      }
    } else {
      report_ccb_validation_error(
          opdata, "Modification of attribute is Not Supported:'%s' ",
...
static SaAisErrorT sgtype_ccb_completed_cb(CcbUtilOperationData_t *opdata) {
  ...
  switch (opdata->operationType) {
    ...
    case CCBUTIL_DELETE:
      ...
      if (sg_exist == true) {
        report_ccb_validation_error(opdata, "SGs exist of this SG type '%s'",
                                    sgt->name.c_str());
        goto done;
      }
  ...
  while ((attr_mod = opdata->param.modify.attrMods[i++]) != nullptr) {
    ...
    if (!strcmp(attr_mod->modAttr.attrName, "saAmfSgtDefAutoRepair")) {
      ...
      if (value_is_deleted) {
        ...
        sgt->saAmfSgtDefAutoRepair = static_cast<SaBoolT>(
      ...
      if (old_value != sgt->saAmfSgtDefAutoRepair) {
        for (const auto &sg : sgt->list_of_sg) {
          if (!sg->saAmfSGAutoRepair_configured) {
            ...
            amflog(LOG_NOTICE,
                   "%s inherited saAmfSGAutoRepair value changed to %u",
...
static void sgtype_ccb_apply_cb(CcbUtilOperationData_t *opdata) {
  ...
  switch (opdata->operationType) {
    case CCBUTIL_CREATE:
      sgt = sgtype_create(Amf::to_string(&opdata->objectName),
