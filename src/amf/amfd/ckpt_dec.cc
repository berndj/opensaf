...
static uint32_t dec_cs_cb_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                 uint32_t num_of_obj);
static uint32_t dec_cs_cluster_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                      uint32_t num_of_obj);
static uint32_t dec_cs_node_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                   uint32_t num_of_obj);
static uint32_t dec_cs_app_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                  uint32_t num_of_obj);
static uint32_t dec_cs_sg_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                 uint32_t num_of_obj);
static uint32_t dec_cs_su_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                 uint32_t num_of_obj);
static uint32_t dec_cs_si_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                 uint32_t num_of_obj);
static uint32_t dec_cs_sg_su_oper_list(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                       uint32_t num_of_obj);
static uint32_t dec_cs_sg_admin_si(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                   uint32_t num_of_obj);
static uint32_t dec_cs_comp_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                   uint32_t num_of_obj);
static uint32_t dec_cs_siass(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                             uint32_t num_of_obj);
...
static uint32_t dec_cs_async_updt_cnt(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                      uint32_t num_of_obj);
static uint32_t dec_cs_comp_cs_type_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
                                           uint32_t num_of_obj);
...
const AVSV_DECODE_CKPT_DATA_FUNC_PTR avd_dec_data_func_list[] = {
    dec_cb_config, dec_cluster_config, dec_node_config, dec_app_config,
    dec_sg_config, dec_su_config, dec_si_config, dec_oper_su, dec_sg_admin_si,
    dec_comp_config, dec_comp_cs_type_config, dec_siass, dec_si_trans,
    ...
    dec_node_admin_state, dec_node_oper_state, dec_node_up_info, dec_node_state,
    dec_node_rcv_msg_id, dec_node_snd_msg_id,

    /* SG Async Update messages */
    dec_sg_admin_state, dec_sg_su_assigned_num, dec_sg_su_spare_num,
    dec_sg_su_uninst_num, dec_sg_adjust_state, dec_sg_fsm_state,

    /* SU Async Update messages */
    dec_su_preinstan, dec_su_oper_state, dec_su_admin_state,
    dec_su_readiness_state, dec_su_pres_state, dec_su_si_curr_active,
    dec_su_si_curr_stby, dec_su_term_state, dec_su_switch, dec_su_act_state,

    /* SI Async Update messages */
    dec_si_admin_state, dec_si_assignment_state, dec_si_su_curr_active,
    dec_si_su_curr_stby, dec_si_switch, dec_si_alarm_sent,

    /* COMP Async Update messages */
    dec_comp_proxy_comp_name, dec_comp_curr_num_csi_actv,
...
const AVSV_DECODE_COLD_SYNC_RSP_DATA_FUNC_PTR dec_cs_data_func_list[] = {
    dec_cs_cb_config,   dec_cs_cluster_config,      dec_cs_node_config,
    dec_cs_app_config,  dec_cs_sg_config,           dec_cs_su_config,
    dec_cs_si_config,   dec_cs_sg_su_oper_list,     dec_cs_sg_admin_si,
...
void decode_cluster(NCS_UBAID *ub, AVD_CLUSTER *cluster,
                    const uint16_t peer_version) {
  osaf_decode_uint32(
      ub, reinterpret_cast<uint32_t *>(&cluster->saAmfClusterAdminState));
}
...
void decode_node_config(NCS_UBAID *ub, AVD_AVND *avnd,
  ...
  osaf_decode_sanamet(ub, &node_name);
  avnd->name = Amf::to_string(&node_name);
  osaf_decode_bool(ub, reinterpret_cast<bool *>(&avnd->node_info.member));
  osaf_decode_satimet(ub, &avnd->node_info.bootTimestamp);
  osaf_decode_uint64(
      ub, reinterpret_cast<uint64_t *>(&avnd->node_info.initialViewNumber));
  osaf_decode_uint64(ub, &avnd->adest);
  osaf_decode_uint32(ub,
                     reinterpret_cast<uint32_t *>(&avnd->saAmfNodeAdminState));
  osaf_decode_uint32(ub,
                     reinterpret_cast<uint32_t *>(&avnd->saAmfNodeOperState));
...
static uint32_t dec_node_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  switch (dec->i_action) {
    ...
    case NCS_MBCSV_ACT_RMV:
      ...
      SaNameT node_name;
      osaf_decode_sanamet(&dec->i_uba, &node_name);
      avnd.name = Amf::to_string(&node_name);
      osaf_extended_name_free(&node_name);
      break;
  ...
  if (NCSCC_RC_SUCCESS == status) cb->async_updt_cnt.node_updt++;
...
void decode_app(NCS_UBAID *ub, AVD_APP *app) {
  ...
  osaf_decode_sanamet(ub, &app_name);
  app->name = Amf::to_string(&app_name);
  osaf_extended_name_free(&app_name);
...
static uint32_t dec_app_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  if (NCSCC_RC_SUCCESS == status) cb->async_updt_cnt.app_updt++;
...
static void decode_sg(NCS_UBAID *ub, AVD_SG *sg) {
  ...
  osaf_decode_sanamet(ub, &sg_name);
  sg->name = Amf::to_string(&sg_name);
  osaf_extended_name_free(&sg_name);
...
static uint32_t dec_sg_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  if (NCSCC_RC_SUCCESS == status) cb->async_updt_cnt.sg_updt++;
...
static void decode_su(NCS_UBAID *ub, AVD_SU *su, uint16_t peer_version) {
  ...
  osaf_decode_sanamet(ub, &su_name);
  su->name = Amf::to_string(&su_name);
  ...
  osaf_decode_sanamet(ub, &node_name);
  su->saAmfSUHostedByNode = Amf::to_string(&node_name);
  osaf_extended_name_free(&node_name);
...
static uint32_t dec_su_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  if (status == NCSCC_RC_SUCCESS) cb->async_updt_cnt.su_updt++;

  TRACE_LEAVE2("status:%u, su_updt:%d", status, cb->async_updt_cnt.su_updt);
  return status;
}

static void decode_si(NCS_UBAID *ub, AVD_SI *si, const uint16_t peer_version) {
...
#else
  // default case, si_dep_stateavd_peer_ver should not be check pointed for
  ...
  SaNameT si_name;
  SaNameT si_protected_by_sg;

  osaf_decode_sanamet(ub, &si_name);
  si->name = Amf::to_string(&si_name);
  osaf_extended_name_free(&si_name);
  ...
  osaf_decode_sanamet(ub, &si_protected_by_sg);
  si->saAmfSIProtectedbySG = Amf::to_string(&si_protected_by_sg);
  osaf_extended_name_free(&si_protected_by_sg);
...
static uint32_t dec_si_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  if (status == NCSCC_RC_SUCCESS) cb->async_updt_cnt.si_updt++;
...
static uint32_t dec_sg_admin_si(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  /* If update is successful, update async update count */
  if (NCSCC_RC_SUCCESS == status) cb->async_updt_cnt.sg_admin_si_updt++;
...
void decode_siass(NCS_UBAID *ub, AVSV_SU_SI_REL_CKPT_MSG *su_si_ckpt,
                  const uint16_t peer_version) {
...
static uint32_t dec_siass(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  osaf_extended_name_free(&su_si_ckpt.comp_name);
  osaf_extended_name_free(&su_si_ckpt.csi_name);
  osaf_extended_name_free(&su_si_ckpt.si_name);
  osaf_extended_name_free(&su_si_ckpt.su_name);
...
void decode_comp(NCS_UBAID *ub, AVD_COMP *comp) {
  ...
  osaf_decode_uint32(
      ub, reinterpret_cast<uint32_t *>(&comp->saAmfCompReadinessState));
  osaf_decode_uint32(
      ub, reinterpret_cast<uint32_t *>(&comp->saAmfCompPresenceState));
  osaf_decode_uint32(ub, &comp->saAmfCompRestartCount);
  osaf_decode_sanamet(ub, &proxy_name);
  comp->saAmfCompCurrProxyName = Amf::to_string(&proxy_name);
  osaf_extended_name_free(&proxy_name);
}
...
static uint32_t dec_comp_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  /* If update is successful, update async update count */
  if (NCSCC_RC_SUCCESS == status) cb->async_updt_cnt.comp_updt++;
...
static uint32_t dec_oper_su(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  if (NCSCC_RC_SUCCESS == status) cb->async_updt_cnt.sg_su_oprlist_updt++;

  TRACE_LEAVE2("'%s', status '%u', updt %d", osaf_extended_name_borrow(&name),
               status, cb->async_updt_cnt.sg_su_oprlist_updt);
...
static uint32_t dec_node_up_info(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  osaf_decode_bool(&dec->i_uba,
                   reinterpret_cast<bool *>(&avnd.node_info.member));
  osaf_decode_satimet(&dec->i_uba, &avnd.node_info.bootTimestamp);
  osaf_decode_uint64(&dec->i_uba, reinterpret_cast<uint64_t *>(
...
static uint32_t dec_node_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  osaf_extended_name_free(&name);
  osaf_decode_uint32(&dec->i_uba,
                     reinterpret_cast<uint32_t *>(&avnd.node_state));
...
static uint32_t dec_node_rcv_msg_id(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  osaf_decode_uint32(&dec->i_uba, &avnd.node_info.nodeId);
  osaf_decode_uint32(&dec->i_uba,
                     reinterpret_cast<uint32_t *>(&avnd.rcv_msg_id));
...
static uint32_t dec_node_snd_msg_id(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  osaf_decode_uint32(&dec->i_uba, &avnd.node_info.nodeId);
  osaf_decode_uint32(&dec->i_uba,
                     reinterpret_cast<uint32_t *>(&avnd.snd_msg_id));
...
static uint32_t dec_sg_admin_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  if ((sg->sg_ncs_spec == false) &&
      (sg->sg_redundancy_model == SA_AMF_2N_REDUNDANCY_MODEL)) {
  ...
  TRACE_LEAVE2("'%s', saAmfSGAdminState=%u, sg_updt:%d", sg->name.c_str(),
               sg->saAmfSGAdminState, cb->async_updt_cnt.sg_updt);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_sg_su_assigned_num(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSGNumCurrAssignedSUs=%u, sg_updt:%d",
               sg->name.c_str(), sg->saAmfSGNumCurrAssignedSUs,
               cb->async_updt_cnt.sg_updt);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_sg_su_spare_num(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSGNumCurrInstantiatedSpareSUs=%u, sg_updt:%d",
               sg->name.c_str(), sg->saAmfSGNumCurrInstantiatedSpareSUs,
               cb->async_updt_cnt.sg_updt);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_sg_su_uninst_num(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSGNumCurrNonInstantiatedSpareSUs=%u, sg_updt:%d",
               sg->name.c_str(), sg->saAmfSGNumCurrNonInstantiatedSpareSUs,
               cb->async_updt_cnt.sg_updt);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_sg_adjust_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', adjust_state=%u, sg_updt:%d", sg->name.c_str(),
               sg->adjust_state, cb->async_updt_cnt.sg_updt);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_sg_fsm_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', sg_fsm_state=%u, sg_updt:%d", sg->name.c_str(),
               sg->sg_fsm_state, cb->async_updt_cnt.sg_updt);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_su_preinstan(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSUPreInstantiable=%u, su_updt:%d",
               osaf_extended_name_borrow(&name), su->saAmfSUPreInstantiable,
               cb->async_updt_cnt.su_updt);
...
static uint32_t dec_su_oper_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  AVD_SU *su = su_db->find(Amf::to_string(&name));
  if (su == nullptr) {
    TRACE("'%s' does not exist, creating it", osaf_extended_name_borrow(&name));
    su = new AVD_SU(Amf::to_string(&name));
    unsigned int rc = su_db->insert(su->name, su);
  ...
  TRACE_LEAVE2("'%s', saAmfSUOperState=%u, su_updt:%d",
               osaf_extended_name_borrow(&name), su->saAmfSUOperState,
               cb->async_updt_cnt.su_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_su_admin_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSUAdminState=%u, su_updt:%d",
               osaf_extended_name_borrow(&name), su->saAmfSUAdminState,
               cb->async_updt_cnt.su_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_su_readiness_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSuReadinessState=%u, su_updt:%d",
               osaf_extended_name_borrow(&name), su->saAmfSuReadinessState,
               cb->async_updt_cnt.su_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_su_pres_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSUPresenceState=%u, su_updt:%d",
               osaf_extended_name_borrow(&name), su->saAmfSUPresenceState,
               cb->async_updt_cnt.su_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_su_si_curr_active(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSUNumCurrActiveSIs=%u, su_updt:%d",
               osaf_extended_name_borrow(&name), su->saAmfSUNumCurrActiveSIs,
               cb->async_updt_cnt.su_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_su_si_curr_stby(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSUNumCurrStandbySIs=%u, su_updt:%d",
               osaf_extended_name_borrow(&name), su->saAmfSUNumCurrStandbySIs,
               cb->async_updt_cnt.su_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_su_switch(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', su_switch=%u, su_updt:%d",
               osaf_extended_name_borrow(&name), su->su_switch,
               cb->async_updt_cnt.su_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_su_restart_count(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSURestartCount=%u, su_updt:%d",
               osaf_extended_name_borrow(&name), su->saAmfSURestartCount,
               cb->async_updt_cnt.su_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_si_admin_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSIAdminState=%u, si_updt:%d",
               osaf_extended_name_borrow(&name), si->saAmfSIAdminState,
               cb->async_updt_cnt.si_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_si_assignment_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', saAmfSIAssignmentState=%u, si_updt:%d",
               osaf_extended_name_borrow(&name), si->saAmfSIAssignmentState,
               cb->async_updt_cnt.si_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_si_dep_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  AVD_SI *si = si_db->find(Amf::to_string(&name));
  if (si == nullptr) {
    si = avd_si_new(Amf::to_string(&name));
    osafassert(si != nullptr);
    avd_si_db_add(si);
  }
  ...
  TRACE_LEAVE2("'%s', si_dep_state=%u, si_updt:%d",
               osaf_extended_name_borrow(&name), si->si_dep_state,
               cb->async_updt_cnt.si_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_si_su_curr_active(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  osafassert(si != nullptr);
  osaf_decode_uint32(&dec->i_uba,
                     (uint32_t *)&si->saAmfSINumCurrActiveAssignments);

  cb->async_updt_cnt.si_updt++;

  TRACE_LEAVE2("'%s', saAmfSINumCurrActiveAssignments=%u, si_updt:%d",
               osaf_extended_name_borrow(&name),
               si->saAmfSINumCurrActiveAssignments, cb->async_updt_cnt.si_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_si_su_curr_stby(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  osafassert(si != nullptr);
  osaf_decode_uint32(&dec->i_uba,
                     (uint32_t *)&si->saAmfSINumCurrStandbyAssignments);
...
static uint32_t dec_si_switch(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', si_switch=%u, si_updt:%d",
               osaf_extended_name_borrow(&name), si->si_switch,
               cb->async_updt_cnt.si_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_si_alarm_sent(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE_LEAVE2("'%s', alarm_sent=%u, si_updt:%d",
               osaf_extended_name_borrow(&name), si->alarm_sent,
               cb->async_updt_cnt.si_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_comp_proxy_comp_name(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
   ...
   */
  osaf_decode_sanamet(&dec->i_uba, &comp.comp_info.name);
  osaf_decode_sanamet(&dec->i_uba, &proxy_name);
  comp.saAmfCompCurrProxyName = Amf::to_string(&proxy_name);

  if (nullptr ==
      (comp_struct = comp_db->find(Amf::to_string(&comp.comp_info.name))))
    osafassert(0);
  ...
  TRACE_LEAVE();
  osaf_extended_name_free(&comp.comp_info.name);
  osaf_extended_name_free(&proxy_name);
  return NCSCC_RC_SUCCESS;
}
...
static uint32_t dec_comp_oper_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  comp_struct = avd_comp_get_or_create(comp_name);

  /* Update the fields received in this checkpoint message */
  comp_struct->saAmfCompOperState = comp.saAmfCompOperState;
  avd_saImmOiRtObjectUpdate(comp_name, "saAmfCompOperState",
...
static uint32_t dec_comp_readiness_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  osaf_decode_sanamet(&dec->i_uba, &comp.comp_info.name);
  osaf_decode_uint32(
      &dec->i_uba, reinterpret_cast<uint32_t *>(&comp.saAmfCompReadinessState));

  const std::string comp_name(Amf::to_string(&comp.comp_info.name));
  osaf_extended_name_free(&comp.comp_info.name);

  if (nullptr == (comp_struct = comp_db->find(comp_name))) {
    LOG_ER("%s: comp not found, %s", __FUNCTION__, comp_name.c_str());
    return NCSCC_RC_FAILURE;
  }
  ...
  avd_saImmOiRtObjectUpdate(comp_name, "saAmfCompReadinessState",
                            SA_IMM_ATTR_SAUINT32T,
...
static uint32_t dec_comp_pres_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  osaf_decode_sanamet(&dec->i_uba, &comp.comp_info.name);
  osaf_decode_uint32(
      &dec->i_uba, reinterpret_cast<uint32_t *>(&comp.saAmfCompPresenceState));
  ...
  cb->async_updt_cnt.comp_updt++;
  avd_saImmOiRtObjectUpdate(Amf::to_string(&comp_struct->comp_info.name),
...
uint32_t avd_dec_cold_sync_rsp(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
   ...
   */
  ptr = ncs_dec_flatten_space(&dec->i_uba, (uint8_t *)&num_of_obj,
                              sizeof(uint32_t));
...
static uint32_t dec_cs_app_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
  ...
  for (count = 0; count < num_of_obj; count++) {
    ...
    if (status != NCSCC_RC_SUCCESS) {
      ...
      LOG_WA("'%s' update failed", dec_app.name.c_str());
    }
  }
...
static uint32_t dec_cs_sg_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
  ...
  for (count = 0; count < num_of_obj; count++) {
    ...
    if (status != NCSCC_RC_SUCCESS) {
      ...
      LOG_WA("'%s' update failed", dec_sg.name.c_str());
    }
  }
...
static uint32_t dec_cs_su_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
  ...
  for (unsigned i = 0; i < num_of_obj; i++) {
    ...
    if (status != NCSCC_RC_SUCCESS) {
      ...
      LOG_WA("'%s' update failed", su.name.c_str());
    }
  }
...
static uint32_t dec_cs_si_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
  ...
  for (unsigned i = 0; i < num_of_obj; i++) {
    ...
    if (status != NCSCC_RC_SUCCESS) {
      ...
      LOG_WA("'%s' update failed", si.name.c_str());
    }
  }
...
static uint32_t dec_cs_sg_su_oper_list(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
  ...
  for (cs_count = 0; cs_count < num_of_obj; cs_count++) {
    if (NCSCC_RC_SUCCESS != dec_cs_oper_su(cb, dec)) break;
  }
...
static uint32_t dec_cs_siass(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
   ...
   * are sent.We will send the corresponding COMP_CSI relationship for that
   * SU_SI in the same update.
   */
  for (count = 0; count < num_of_obj; count++) {
    decode_siass(&dec->i_uba, &su_si_ckpt, dec->i_peer_version);
...
uint32_t avd_dec_warm_sync_rsp(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  if (0 != memcmp(updt_cnt, &cb->async_updt_cnt, sizeof(AVSV_ASYNC_UPDT_CNT))) {
    ...
    /* We need to figure out when there is out of sync, later on we have to
     * remove it. */

    if (updt_cnt->cb_updt != cb->async_updt_cnt.cb_updt)
      LOG_ER("cb_updt counters mismatch: Active: %u  Standby: %u",
             updt_cnt->cb_updt, cb->async_updt_cnt.cb_updt);
    if (updt_cnt->node_updt != cb->async_updt_cnt.node_updt)
      LOG_ER("node_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->node_updt, cb->async_updt_cnt.node_updt);
    if (updt_cnt->app_updt != cb->async_updt_cnt.app_updt)
      LOG_ER("app_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->app_updt, cb->async_updt_cnt.app_updt);
    if (updt_cnt->sg_updt != cb->async_updt_cnt.sg_updt)
      LOG_ER("sg_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->sg_updt, cb->async_updt_cnt.sg_updt);
    if (updt_cnt->su_updt != cb->async_updt_cnt.su_updt)
      LOG_ER("su_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->su_updt, cb->async_updt_cnt.su_updt);
    if (updt_cnt->si_updt != cb->async_updt_cnt.si_updt)
      LOG_ER("si_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->si_updt, cb->async_updt_cnt.si_updt);
    if (updt_cnt->sg_su_oprlist_updt != cb->async_updt_cnt.sg_su_oprlist_updt)
      LOG_ER("sg_su_oprlist_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->sg_su_oprlist_updt,
             cb->async_updt_cnt.sg_su_oprlist_updt);
    if (updt_cnt->sg_admin_si_updt != cb->async_updt_cnt.sg_admin_si_updt)
      LOG_ER("sg_admin_si_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->sg_admin_si_updt, cb->async_updt_cnt.sg_admin_si_updt);
    if (updt_cnt->siass_updt != cb->async_updt_cnt.siass_updt)
      LOG_ER("siass_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->siass_updt, cb->async_updt_cnt.siass_updt);
    if (updt_cnt->comp_updt != cb->async_updt_cnt.comp_updt)
      LOG_ER("comp_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->comp_updt, cb->async_updt_cnt.comp_updt);
    if (updt_cnt->csi_updt != cb->async_updt_cnt.csi_updt)
      LOG_ER("csi_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->csi_updt, cb->async_updt_cnt.csi_updt);
    if (updt_cnt->compcstype_updt != cb->async_updt_cnt.compcstype_updt)
      LOG_ER("compcstype_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->compcstype_updt, cb->async_updt_cnt.compcstype_updt);
    if (updt_cnt->si_trans_updt != cb->async_updt_cnt.si_trans_updt)
      LOG_ER("si_trans_updt counters mismatch: Active: %u Standby: %u",
             updt_cnt->si_trans_updt, cb->async_updt_cnt.si_trans_updt);
...
void decode_comp_cs_type_config(NCS_UBAID *ub, AVD_COMPCS_TYPE *comp_cs_type) {
  SaNameT name;
  osaf_decode_sanamet(ub, &name);
  comp_cs_type->name = Amf::to_string(&name);
...
static uint32_t dec_comp_cs_type_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  /* If update is successful, update async update count */
  if (NCSCC_RC_SUCCESS == status) cb->async_updt_cnt.compcstype_updt++;
...
static uint32_t dec_cs_comp_cs_type_config(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec,
  ...
  for (count = 0; count < num_of_obj; count++) {
    ...
    if (status != NCSCC_RC_SUCCESS) {
      ...
      LOG_WA("'%s' update failed", comp_cs_type.name.c_str());
    }
  }
  TRACE_LEAVE2("status '%u'", status);
  return status;
...
static uint32_t dec_ng_admin_state(AVD_CL_CB *cb, NCS_MBCSV_CB_DEC *dec) {
  ...
  TRACE("'%s',saAmfNGAdminState:%d", ng->name.c_str(), ng->saAmfNGAdminState);
  ...
  for (std::set<std::string>::const_iterator iter = ng->saAmfNGNodeList.begin();
    ...
    for (const auto &tmp : node->list_of_su) {
      su = tmp;
      if (su->list_of_susi != nullptr) break;
    }
  ...
  TRACE_LEAVE2("'%s', saAmfNGAdminState=%u, ng_updt:%d",
               osaf_extended_name_borrow(&name), ng->saAmfNGAdminState,
               cb->async_updt_cnt.ng_updt);
  osaf_extended_name_free(&name);
  return NCSCC_RC_SUCCESS;
}
