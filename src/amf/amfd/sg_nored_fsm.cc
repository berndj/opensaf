...
AVD_SU *SG_NORED::assign_sis_to_sus() {
  ...
  for (const auto &i_si : list_of_si) {
    ...
    if ((i_si->saAmfSIAdminState != SA_AMF_ADMIN_UNLOCKED) ||
        ...
        (i_si->list_of_csi == nullptr) || (i_si->list_of_sisu != nullptr)) {
      continue;
    }
    ...
    for (; iter != list_of_su.end(); ++iter) {
      ...
      if ((su->saAmfSuReadinessState == SA_AMF_READINESS_IN_SERVICE) &&
        ...
        if (avd_new_assgn_susi(avd_cb, su, i_si, SA_AMF_HA_ACTIVE, false,
                               &tmp) == NCSCC_RC_SUCCESS) {
          su_oper_list_add(su);
          break;
        } else {
  ...
  switch (su->sg_of_su->sg_fsm_state) {
    ...
    case AVD_SG_FSM_SG_REALIGN:
      ...
      if (su->list_of_susi->si == su->sg_of_su->admin_si) {
         ...
         */
        if (su->list_of_susi->si->saAmfSIAdminState ==
    ...
    case AVD_SG_FSM_SI_OPER:
      ...
      if (su->sg_of_su->admin_si == su->list_of_susi->si) {
         ...
         */
        if (su->sg_of_su->admin_si->saAmfSIAdminState ==
            SA_AMF_ADMIN_SHUTTING_DOWN) {
          if (avd_susi_mod_send(su->list_of_susi, SA_AMF_HA_QUIESCED) !=
              NCSCC_RC_SUCCESS)
            return NCSCC_RC_FAILURE;
  ...
  switch (sg_fsm_state) {
    ...
    case AVD_SG_FSM_SU_OPER:
      if (act == AVSV_SUSI_ACT_DEL) {
         ...
         * Remove the SU from the SU operation list. If the SU operation list
         * is empty. Choose and assign unassigned SIs to unassigned in-service
         * SUs .
         */
...
void SG_NORED::node_fail(AVD_CL_CB *cb, AVD_SU *su) {
  ...
  switch (su->sg_of_su->sg_fsm_state) {
    ...
    case AVD_SG_FSM_SG_REALIGN:
       ...
       * the SU operation list or the SUs SI is not part of the SI admin
       * pointer. Remove the SI relationship to this SU.
       */
      l_si = su->list_of_susi->si;
  ...
  switch (su->sg_of_su->sg_fsm_state) {
    case AVD_SG_FSM_STABLE:
      if ((su->saAmfSUAdminState == SA_AMF_ADMIN_LOCKED) ||
        ...
        /* change the  assignment to quiesced. */
        if (avd_susi_mod_send(su->list_of_susi, SA_AMF_HA_QUIESCING) ==
            NCSCC_RC_FAILURE)
          return NCSCC_RC_FAILURE;
  ...
  switch (si->sg_of_si->sg_fsm_state) {
    case AVD_SG_FSM_STABLE:
      if (si->saAmfSIAdminState == SA_AMF_ADMIN_LOCKED) {
         ...
         */
        /* change the  assignment to quiesced. */
        if (avd_susi_mod_send(si->list_of_sisu, SA_AMF_HA_QUIESCED) ==
            NCSCC_RC_FAILURE)
          return NCSCC_RC_FAILURE;
        ...
        if (avd_susi_mod_send(si->list_of_sisu, SA_AMF_HA_QUIESCING) ==
            NCSCC_RC_FAILURE)
          return NCSCC_RC_FAILURE;
