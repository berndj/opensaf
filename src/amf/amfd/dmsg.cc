...
void avd_mds_d_dec(MDS_CALLBACK_DEC_INFO *dec_info) {
  ...
  switch (d2d_msg->msg_type) {
    case AVD_D2D_CHANGE_ROLE_REQ:
      d2d_msg->msg_info.d2d_chg_role_req.cause =
          static_cast<AVD_ROLE_CHG_CAUSE_T>(ncs_decode_32bit(&data));
      d2d_msg->msg_info.d2d_chg_role_req.role =
    ...
    case AVD_D2D_CHANGE_ROLE_RSP:
      if (SA_AMF_HA_ACTIVE == rcv_msg->msg_info.d2d_chg_role_rsp.role) {
        if (NCSCC_RC_SUCCESS == rcv_msg->msg_info.d2d_chg_role_rsp.status) {
           ...
           */
          TRACE(
              "amfd role change rsp stdby -> actv succ, posting to mail box for qsd -> stdby");
          avd_post_amfd_switch_role_change_evt(cb, SA_AMF_HA_STANDBY);
        } else {
          /* Other AMFD rejected the active request, move the local amfd back to
           * Active from qsd */
          TRACE(
              "amfd role change rsp stdby -> actv fail, posting to mail box for qsd -> actv");
          avd_post_amfd_switch_role_change_evt(cb, SA_AMF_HA_ACTIVE);
        }
      } else {
...
void avsv_d2d_msg_free(AVD_D2D_MSG *d2d_msg) { delete d2d_msg; }
