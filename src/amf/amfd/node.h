...
class AVD_AVND {
 public:
  ...
  bool
      clm_change_start_preceded; /* to indicate there was CLM start cbk before
                                    CLM completed cb. */
...
class AVD_AMF_NG {
 public:
  ...
  uint32_t number_nodes() const { return saAmfNGNodeList.size(); }
  ...
  std::set<std::string> node_oper_list; /* list of nodes impacted because of
                                           node group admin op */
  uint32_t oper_list_size() const { return node_oper_list.size(); }
...
extern void avd_node_admin_lock_unlock_shutdown(
    AVD_AVND *node, SaInvocationT invocation,
