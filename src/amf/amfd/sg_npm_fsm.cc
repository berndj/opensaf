...
static AVD_SU *avd_sg_npm_su_next_asgn(AVD_CL_CB *cb, AVD_SG *sg, AVD_SU *su,
  ...
  if (su == nullptr) {
    ...
    iter = std::find(sg->list_of_su.begin(), sg->list_of_su.end(), su);
...
static AVD_SU_SI_REL *avd_sg_npm_su_othr(AVD_CL_CB *cb, AVD_SU *su) {
  ...
  while (i_susi != AVD_SU_SI_REL_NULL) {
    if (i_susi->si->list_of_sisu != i_susi) {
      ...
      if (o_susi->fsm != AVD_SU_SI_STATE_UNASGN) break;
 ...
 * any SUSI relationships whose SI relationship is not with the quiesced SU
 *specified. If yes a remove message will be sent to each of those SUSIs. It
 *then sends a active all message to the standby SU.
 *
 * Input: cb - the AVD control block.
...
static uint32_t avd_sg_npm_su_chk_snd(AVD_CL_CB *cb, AVD_SU *s_su,
                                      AVD_SU *q_su) {
  ...
  if ((q_su->su_on_node->node_state == AVD_AVND_STATE_ABSENT) ||
    ...
    if (avd_sidep_is_su_failover_possible(q_su) == false) {
      TRACE(
          "role modification cannot be done now as Sponsor SI's are not yet assigned");
      goto done;
    }
  }
  ...
  for (i_susi = s_su->list_of_susi; i_susi != AVD_SU_SI_REL_NULL;
       i_susi = i_susi->su_next) {
    if ((i_susi->si->list_of_sisu != i_susi) &&
  ...
  } /* for (i_susi = su->list_of_susi;i_susi != AVD_SU_SI_REL_NULL; i_susi =
       i_susi->su_next) */
  ...
  if (rc == NCSCC_RC_SUCCESS) {
    if ((q_su->su_on_node->node_state == AVD_AVND_STATE_ABSENT) ||
      ...
      for (i_susi = s_su->list_of_susi; i_susi != nullptr;
           i_susi = i_susi->su_next) {
    ...
    LOG_NO(" %s: %u: Active role modification failed for %s ", __FILE__,
           __LINE__, s_su->name.c_str());
  }
  ...
  for (const auto &curr_si : sg->list_of_si) {
    ...
    if (curr_si->list_of_sisu == AVD_SU_SI_REL_NULL) {
      ...
      if (curr_su == nullptr) {
         ...
         */
        TRACE("No more SUs to assign Active HA state: '%s'",
              curr_si->name.c_str());
        break;
      }

      TRACE("Assigning active for SI(%s) to SU(%s)", curr_si->name.c_str(),
            curr_su->name.c_str());

      if (avd_new_assgn_susi(avd_cb, curr_su, curr_si, SA_AMF_HA_ACTIVE, false,
                             &susi) == NCSCC_RC_SUCCESS) {
  ...
  for (const auto &curr_si : sg->list_of_si) {
    ...
    if (curr_si->list_of_sisu->state == SA_AMF_HA_ACTIVE) {
      ...
      if (curr_su == nullptr) {
         ...
         */
        TRACE("No more SUs to assign Standby HA state: '%s'",
              curr_si->name.c_str());
        break;
      }

      TRACE("Assigning standby for SI(%s) to SU(%s)", curr_si->name.c_str(),
            curr_su->name.c_str());

      if (avd_new_assgn_susi(avd_cb, curr_su, curr_si, SA_AMF_HA_STANDBY, false,
                             &susi) == NCSCC_RC_SUCCESS) {
        /* Add the SU to the operation list */
        avd_sg_su_oper_list_add(avd_cb, curr_su, false);
      } else {
 ...
 * complete, search for in-service SUs that can take assignment for this SI and
 *assign this unassigned SIs to them by Sending D2N-INFO_SU_SI_ASSIGN message
 *for the SUs with role active for the SIs. It then adds the Assigning SUs to
 ...
 *if standby assignment exists, if no it identifies the standby SU corresponding
 *to this SU and assigns this SI to the SU in standby role. It then adds the
 *Assigning SUs to the SU operation list. If no assignments happen, it returns
 *nullptr.
 *
 * Input: cb - the AVD control block
 *        sg - The pointer to the service group.
...
static AVD_SU *avd_sg_npm_su_chose_asgn(AVD_CL_CB *cb, AVD_SG *sg) {
  ...
  while ((si_iter != sg->list_of_si.end()) && (su_found == true)) {
    ...
    if (i_su != nullptr) {
      if ((i_su->sg_of_su->saAmfSGMaxActiveSIsperSU == 0) ||
        ...
        if (new_su == false) {
           ...
           * active assignments.
           */
          while ((i_su != nullptr) && (cnt < sg->saAmfSGNumPrefActiveSUs)) {
            ...
            if (i_su != nullptr) {
              ...
              if ((i_su->sg_of_su->saAmfSGMaxActiveSIsperSU == 0) ||
                ...
                su_found = true;
                break;
              } else if (sg->saAmfSGMaxActiveSIsperSU >
                         i_su->saAmfSUNumCurrActiveSIs) {
                load_su_found = true;
              }
            }
    ...
    if (su_found == true) {
      if (avd_new_assgn_susi(cb, i_su, i_si, SA_AMF_HA_ACTIVE, false,
        ...
        LOG_ER("%s:%u: %s (%zu)", __FILE__, __LINE__, i_si->name.c_str(),
               i_si->name.length());
        LOG_ER("%s:%u: %s (%zu)", __FILE__, __LINE__, i_su->name.c_str(),
               i_su->name.length());
      }
  ...
  while (si_iter != sg->list_of_si.end()) {
    ...
    if (false == su_found) {
       ...
       * Standby SU is found but current Standby assignments are equal to
      ...
      if (l_su != nullptr) {
        if ((i_su->sg_of_su->saAmfSGMaxStandbySIsperSU == 0) ||
          ...
          if ((new_su == true) && (cnt < sg->saAmfSGNumPrefStandbySUs)) {
            std::vector<AVD_SU *>::iterator iter =
                std::find(sg->list_of_su.begin(), sg->list_of_su.end(), l_su);
            ...
            if ((su_found == false) && (cnt < sg->saAmfSGNumPrefStandbySUs)) {
              ...
              if (n_su == nullptr) {
                ...
                iter = std::find(sg->list_of_su.begin(), sg->list_of_su.end(),
                                 n_su);
              }
        ...
        if ((su_found == false) && (cnt < sg->saAmfSGNumPrefStandbySUs)) {
          ...
          if (n_su == nullptr) {
            ...
            iter =
                std::find(sg->list_of_su.begin(), sg->list_of_su.end(), n_su);
          }
          while (iter != sg->list_of_su.end()) {
            i_su = *iter;
            if ((i_su->saAmfSuReadinessState == SA_AMF_READINESS_IN_SERVICE) &&
    ...
    if (su_found == true) {
      if (avd_new_assgn_susi(cb, i_su, i_si, SA_AMF_HA_STANDBY, false,
                             &tmp_susi) == NCSCC_RC_SUCCESS) {
        /* Add the SU to the operation list */
        avd_sg_su_oper_list_add(cb, i_su, false);
      } else {
...
uint32_t SG_NPM::su_fault(AVD_CL_CB *cb, AVD_SU *su) {
  ...
  if (su->list_of_susi == AVD_SU_SI_REL_NULL) return NCSCC_RC_SUCCESS;
  ...
  switch (su->sg_of_su->sg_fsm_state) {
    ...
    case AVD_SG_FSM_SU_OPER:
      ...
      if (su_fault_su_oper(cb, su) == NCSCC_RC_FAILURE) return NCSCC_RC_FAILURE;

      break; /* case AVD_SG_FSM_SU_OPER: */
    case AVD_SG_FSM_SI_OPER:

      if (su_fault_si_oper(cb, su) == NCSCC_RC_FAILURE) return NCSCC_RC_FAILURE;
...
uint32_t SG_NPM::susi_sucss_sg_reln(AVD_CL_CB *cb, AVD_SU *su,
  ...
  if (susi == AVD_SU_SI_REL_NULL) {
    ...
    if ((act == AVSV_SUSI_ACT_MOD) && (state == SA_AMF_HA_QUIESCED)) {
      ...
      if (su->sg_of_su->admin_si != AVD_SI_NULL) {
        ...
        } else { /* if ((su->sg_of_su->admin_si->si_switch ==
      ...
      for (i_susi = su->list_of_susi; (i_susi != AVD_SU_SI_REL_NULL) &&
    ...
    if (act == AVSV_SUSI_ACT_DEL) {
      ...
      if (i_susi == AVD_SU_SI_REL_NULL) {
        ...
        if ((nullptr != su->sg_of_su->si_tobe_redistributed) &&
          ...
          /* SI transfe flow, Active assignment to min_assigned_su is suucess.
             Check if max_assigned_su is still In-service if so send delete
             request to it and reset the pointers   */

          if (su->sg_of_su->max_assigned_su->saAmfSuReadinessState ==
              SA_AMF_READINESS_IN_SERVICE) {
            /* Find the SUSI, which is in Quisced state */
            max_su_susi =
                avd_su_susi_find(avd_cb, su->sg_of_su->max_assigned_su,
                                 su->sg_of_su->si_tobe_redistributed->name);
            if (max_su_susi) avd_susi_del_send(max_su_susi);
          }
       ...
       * SI admin pointer.
       */
      if (avd_susi_del_send(susi) == NCSCC_RC_FAILURE) return NCSCC_RC_FAILURE;
...
/**
 * @brief        This routine checks whether the SIs can be redistributed among
 * SUs to achieve equal  distribution, if so finds  the  max assigned SU, min
 * assigned SU and the SI that needs to be transferred
 *
 * @param[in]    avd_sg -  Pointer to Service Group  that is screened for
...
static void avd_sg_npm_screening_for_si_redistr(AVD_SG *avd_sg) {
  ...
  for (const auto &iter : avd_sg->list_of_su) {
    ...
    if (i_su->saAmfSUNumCurrActiveSIs >
        avd_sg->max_assigned_su->saAmfSUNumCurrActiveSIs)
      avd_sg->max_assigned_su = i_su;
  ...
  if ((avd_sg->max_assigned_su->saAmfSUNumCurrActiveSIs -
    ...
    for (const auto &iter : avd_sg->list_of_su) {
      ...
      if (i_su->saAmfSUNumCurrStandbySIs >
          avd_sg->max_assigned_su->saAmfSUNumCurrStandbySIs)
        avd_sg->max_assigned_su = i_su;

      if (i_su->saAmfSUNumCurrStandbySIs <
          avd_sg->min_assigned_su->saAmfSUNumCurrStandbySIs)
        avd_sg->min_assigned_su = i_su;
    }
    if (nullptr != avd_sg->max_assigned_su) {
    ...
    if ((avd_sg->max_assigned_su->saAmfSUNumCurrStandbySIs -
      ...
      TRACE(
          "SIs are assigned equally among Standby SUs, so no need of SI transfer ");
      goto done;
    }
    TRACE(
        "max_assigned_su '%s' no of assignments '%u', min_assigned_su '%s' no of assignments '%u'",
        avd_sg->max_assigned_su->name.c_str(),
        avd_sg->max_assigned_su->saAmfSUNumCurrStandbySIs,
        avd_sg->min_assigned_su->name.c_str(),
        avd_sg->min_assigned_su->saAmfSUNumCurrStandbySIs);
  ...
  /* Find the SI whose assignment has to be changed,figure out less ranked SI if
     SU has more than one assignments Do check that max_assigned_su and
     min_assigned_su, don't share the same SI assignment */
  su_si = avd_sg->max_assigned_su->list_of_susi;
  while (su_si) {
    if (nullptr == i_si) /* first si */
      i_si = su_si->si;
    else {
      if (i_si->saAmfSIRank > su_si->si->saAmfSIRank) i_si = su_si->si;
    }
    su_si = su_si->su_next;
  } /* while (su_si) */
...
/**
 * @brief        This routine transfers the SI assignment from the SU which has
 * maximum assignments to  the SU which has minimum assignments
 *
 ...
 */
static void avd_sg_npm_si_transfer_for_redistr(AVD_SG *avd_sg) {
  TRACE_ENTER2("SG name:%s SI name:%s", avd_sg->name.c_str(),
               avd_sg->si_tobe_redistributed->name.c_str());
  ...
  AVD_SU_SI_REL *susi = avd_su_susi_find(avd_cb, avd_sg->max_assigned_su,
  ...
  if (susi->state == SA_AMF_HA_STANDBY) {
    ...
    /* Reset the SI transfer fields, SG si assignment flow will take care of
       assigning the removed SI to the possible minimum assigned SU*/
    avd_sg->max_assigned_su = nullptr;
    avd_sg->min_assigned_su = nullptr;
    avd_sg->si_tobe_redistributed = nullptr;
    m_AVSV_SEND_CKPT_UPDT_ASYNC_RMV(avd_cb, avd_sg, AVSV_CKPT_AVD_SI_TRANS);
...
uint32_t avd_sg_get_curr_act_cnt(AVD_SG *sg) {
  ...
  for (const auto &i_su : sg->list_of_su) {
    ...
    if (SA_AMF_HA_ACTIVE == i_su->list_of_susi->state) curr_pref_active_sus++;
  }
...
/*
 * @brief        This routine finds the Standby SU for each of the active
 * assignments on this SU, checks the possibility  of assigning Active role to
 * this SU, if so assigns the  Active role to the corresponding
 * susi and remove the other assignments.If it is not possible to
 * assign Active role sends susi  del to all the asignments on the Standby
 * SU.For the assignments that are removed  the default flow takes
 * care of finding and assigning the qualified SUs
 *
...
void avd_sg_npm_stdbysu_role_change(AVD_SU *su) {
  ...
  while (act_susi != AVD_SU_SI_REL_NULL) {
    ...
    if (act_susi->si->list_of_sisu != act_susi) {
      ...
      if ((act_susi->si->list_of_sisu->si_next->fsm !=
      ...
      if (std_susi->su->saAmfSuReadinessState == SA_AMF_READINESS_IN_SERVICE) {
        if (!curr_pref_active_sus) {
          ...
          curr_pref_active_sus =
              avd_sg_get_curr_act_cnt(std_susi->su->sg_of_su);
        }
        /* Check the possibility of assigning Active role to std_susi->su  */
        if (curr_pref_active_sus >=
            std_susi->su->sg_of_su->saAmfSGNumPrefActiveSUs) {
          ...
          /* Check if node failover is undergoing or admin opr is going on the
           * node*/
          if ((su->su_on_node->node_state == AVD_AVND_STATE_ABSENT) ||
            ...
            /* Check if there is any dependency for performing failover for this
             * act_susi */
          ...
          /* Check si_dep_state as well because si dep underprogress SI should
             not be deleted. */
          if ((flag == true) &&
              (act_susi->si->si_dep_state != AVD_SI_FAILOVER_UNDER_PROGRESS)) {
            /* Check if standby SU has standby SI assignment from another SU, if
             * so send remove message for each of those SI assignments
             */
            AVD_SU_SI_REL *susi;
            ...
            /* std_susi->su role changed to Active so increment the
             * curr_pref_active_sus */
            curr_pref_active_sus++;
            ...
            avd_sidep_si_dep_state_set(act_susi->si,
...
uint32_t SG_NPM::susi_success_su_oper(AVD_CL_CB *cb, AVD_SU *su,
                                      AVD_SU_SI_REL *susi, AVSV_SUSI_ACT act,
                                      SaAmfHAStateT state) {
  AVD_SU_SI_REL *i_susi;
  ...
  if (act == AVSV_SUSI_ACT_MOD) {
    if ((state == SA_AMF_HA_QUIESCED) && (su_oper_list_front() == su)) {
      if ((su->sg_of_su->si_tobe_redistributed) &&
          (su->sg_of_su->max_assigned_su == su)) {
        /* SI Transfer flow, got response for max_assigned_su quiesced opr, if
           min_assigned_su is in in_service send Active assignment to it else
           revert back Active assignment to max_assigned_su */
        ...
        if (su->sg_of_su->min_assigned_su->saAmfSuReadinessState ==
          ...
          if (avd_new_assgn_susi(avd_cb, su->sg_of_su->min_assigned_su,
                                 ...
                                 &tmp_susi) == NCSCC_RC_SUCCESS) {
            /* Add the SU to the operation list and set SG FSM to
        ...
        if ((su->sg_of_su->min_assigned_su->saAmfSuReadinessState ==
            ...
            (susi_assgn_failed == true)) {
          /* min_assigne_su is Out of Service or susi assgn failed for
             min_assigned_su So return Active assignment to max_assigned_su */
        ...
        if (susi != AVD_SU_SI_REL_NULL) {
          ...
          for (i_susi = su->list_of_susi;
               (i_susi != AVD_SU_SI_REL_NULL) &&
               (i_susi->fsm == AVD_SU_SI_STATE_ASGND);
               i_susi = i_susi->su_next)
            ;

          if (i_susi != AVD_SU_SI_REL_NULL) return NCSCC_RC_SUCCESS;
        }
...
static uint32_t avd_sg_npm_susi_sucss_si_oper(AVD_CL_CB *cb, AVD_SU *su,
                                              AVD_SU_SI_REL *susi,
                                              AVSV_SUSI_ACT act,
                                              SaAmfHAStateT state) {
  AVD_SU_SI_REL *i_susi, *o_susi;
  ...
  if (susi != AVD_SU_SI_REL_NULL) {
    ...
    if (act == AVSV_SUSI_ACT_DEL) {
       ...
       * for this SI to this SU, Add this SU to the SU oper list.
       * Change admin state to lock for this SI. Identify its standby SU
       * assignment. send D2N-INFO_SU_SI_ASSIGN remove for only this SI and Add
       * the standby SU to the SU oper list. Remove the SI from the SI admin
       * pointer.
       */
      if (avd_susi_del_send(susi) == NCSCC_RC_FAILURE) return NCSCC_RC_FAILURE;
...
uint32_t SG_NPM::susi_success(AVD_CL_CB *cb, AVD_SU *su, AVD_SU_SI_REL *susi,
  ...
  switch (su->sg_of_su->sg_fsm_state) {
    ...
    case AVD_SG_FSM_SI_OPER:
      ...
      if (avd_sg_npm_susi_sucss_si_oper(cb, su, susi, act, state) ==
          NCSCC_RC_FAILURE)
        return NCSCC_RC_FAILURE;
  ...
  switch (su->sg_of_su->sg_fsm_state) {
    ...
    case AVD_SG_FSM_SU_OPER:
      ...
      if ((susi == AVD_SU_SI_REL_NULL) && (act == AVSV_SUSI_ACT_MOD) &&
        ...
        if ((susi != AVD_SU_SI_REL_NULL) && (state == SA_AMF_HA_QUIESCED) &&
            (act == AVSV_SUSI_ACT_MOD)) {
          /* Check if SI transfer is going on */
          if (su->sg_of_su->si_tobe_redistributed) {
            if ((su->sg_of_su->max_assigned_su == su) &&
                (SA_AMF_READINESS_IN_SERVICE ==
    ...
    case AVD_SG_FSM_SI_OPER:
      ...
      if ((susi != AVD_SU_SI_REL_NULL) && (act == AVSV_SUSI_ACT_MOD) &&
        ...
        /* message with modified quiesced for a SI in the admin pointer with
         * admin values lock/shutdown. Send a D2N-INFO_SU_SI_ASSIGN with remove
         * for this SI to this SU. Remove the SI from the admin pointer. Add the
        ...
        su->sg_of_su->set_fsm_state(AVD_SG_FSM_SG_REALIGN);
           ...
           (state == SA_AMF_HA_QUIESCED) && (su->sg_of_su->admin_si !=
           AVD_SI_NULL)
           && (su->sg_of_su->admin_si->si_switch == AVSV_SI_TOGGLE_SWITCH) &&
           (avd_su_susi_struc_find(cb,su,su->sg_of_su->admin_si->name,false)
...
uint32_t SG_NPM::realign(AVD_CL_CB *cb, AVD_SG *sg) {
  ...
  if (sg->sg_fsm_state != AVD_SG_FSM_STABLE) {
    ...
    if ((AVD_SG_FSM_STABLE == sg->sg_fsm_state) &&
        (true == sg->equal_ranked_su) && (SA_TRUE == sg->saAmfSGAutoAdjust)) {
      /* SG fsm is stable, screen for possibility of redistributing SI
      to achieve equal distribution */
...
static void avd_sg_npm_node_fail_sg_relgn(AVD_CL_CB *cb, AVD_SU *su) {
  ...
  if (su->sg_of_su->admin_si != AVD_SI_NULL) {
    ...
    } else if ((su->sg_of_su->admin_si->list_of_sisu->si_next !=
                AVD_SU_SI_REL_NULL) &&
      ...
      if (l_flag == true) {
        ...
        if ((su->list_of_susi->state == SA_AMF_HA_QUIESCED) &&
          ...
          if ((ot_susi != AVD_SU_SI_REL_NULL) &&
              (ot_susi->su->saAmfSuReadinessState ==
               SA_AMF_READINESS_IN_SERVICE) &&
    ...
    if (l_flag == true) {
      ...
      if ((su->list_of_susi->state == SA_AMF_HA_QUIESCED) ||
          ...
          (su->list_of_susi->state == SA_AMF_HA_ACTIVE)) {
        /* the SI relationships to the SU state is active/quiesced/quiescing
         * assigning. */
        ot_susi = avd_sg_npm_su_othr(cb, su);

        if (ot_susi != AVD_SU_SI_REL_NULL) {
          if ((ot_susi->su->saAmfSuReadinessState ==
               SA_AMF_READINESS_IN_SERVICE) &&
              (ot_susi->state == SA_AMF_HA_STANDBY) &&
              (ot_susi->fsm != AVD_SU_SI_STATE_UNASGN)) {
            /* The standby SU w.r.t this SU is inservice has standby
            ...
            if (avd_sg_npm_su_chk_snd(cb, ot_susi->su, su) == NCSCC_RC_SUCCESS)
              avd_sg_su_oper_list_add(cb, ot_susi->su, false);
          } else if (ot_susi->su->saAmfSuReadinessState !=
                     SA_AMF_READINESS_IN_SERVICE) {
            avd_sg_su_oper_list_add(cb, ot_susi->su, false);
          }
        }
...
void SG_NPM::node_fail(AVD_CL_CB *cb, AVD_SU *su) {
  ...
  if (su->list_of_susi == AVD_SU_SI_REL_NULL) return;

  switch (su->sg_of_su->sg_fsm_state) {
    case AVD_SG_FSM_STABLE:

      if (su->list_of_susi->state == SA_AMF_HA_ACTIVE) {
        /* Find Standby SU corresponding to each susi on this SU and do role
         * change */
        avd_sg_npm_stdbysu_role_change(su);

        /* Free all the SI assignments to this SU. */
        su->delete_all_susis();
        /* sg_fsm_state is still STABLE if no standby SU is found to take role
         * change. */
        if (su->sg_of_su->sg_fsm_state == AVD_SG_FSM_STABLE) {
...
SaAisErrorT SG_NPM::si_swap(AVD_SI *si, SaInvocationT invocation) {
  ...
  for (AVD_SU_SI_REL *susi = actv_su->list_of_susi; susi != nullptr;
       susi = susi->su_next) {
    if (susi->si == si) continue;
    if ((rc = susi->si->si_swap_validate()) != SA_AIS_OK) goto done;
  }

  /* After completion of operation, active SU will become standby. As a standby
     SU, it should not cross saAmfSGMaxStandbySIsperSU.
   */
  if (sg->saAmfSGMaxStandbySIsperSU <
      static_cast<SaUint32T>(
          actv_su->hastate_assignments_count(SA_AMF_HA_ACTIVE))) {
    LOG_NO(
        "%s SWAP not allowed as it will violate saAmfSGMaxStandbySIsperSU in '%s'.",
  ...
  /* Check 2: In ideal situation if same SU is standby for all SIs then there
     will not be any change in number of active SUs and number of standby SUs in
     SG. But if all SIs of active SU have their standbys on different standby
     SUs, then si-swap may increase number of active SUs in SG. Check for
     saAmfSGNumPrefActiveSUs.
   */
  for (AVD_SU_SI_REL *susi = actv_su->list_of_susi; susi != nullptr;
       susi = susi->su_next) {
    AVD_SU_SI_REL *tmp_stdby_susi = avd_npm_other_sisu(susi);
    if (tmp_stdby_susi->su != stdby_su) count++;

    // TODO: Will remove this check in the second version for the case when all
    // actives do  not have standbys on same SU.
    if (count > 0) {
      LOG_NO(
          "SWAP not allowed, SIs of SU in which '%s' is active, do not have standbys in same SU.",
          si->name.c_str());
    }
