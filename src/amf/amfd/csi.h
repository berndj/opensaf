...
typedef struct avd_csi_attr_tag {
  ...
  struct avd_csi_attr_tag
      *attr_next; /* the next attribute in the list of attributes in the CSI. */
} AVD_CSI_ATTR;

typedef struct avd_csi_deps_tag {
  std::string csi_dep_name_value; /* CSI dependency name and value */
  struct avd_csi_deps_tag
      *csi_dep_next; /* the next CSI dependency in the list */
} AVD_CSI_DEPS;
...
class AVD_CSI {
 public:
  ...
  AVD_CSI_DEPS
      *saAmfCSIDependencies{}; /* list of all CSI dependencies for this CSI */
  /* Rank is calculated based on CSI dependency. If no dependency configured
     then rank will be 1. Else rank will one more than rank of
     saAmfCSIDependencies. */
  uint32_t rank{}; /* The rank of the CSI in the SI
                    * Checkpointing - Sent as a one time update.
                    */
  bool osafAmfCSICommunicateCsiAttributeChange =
      false; /*To control invocation of INSTANTIATE script of NON PROXIED NPI
             comp upon modification of CSIAttributes of assigned csi.*/
  ...
  AVD_CSI
      *si_list_of_csi_next{}; /* the next CSI in the list of  component service
                               * instances in the Service instance  */
  struct avd_comp_csi_rel_tag
      *list_compcsi{}; /* The list of compcsi relationship
                        * wrt to this CSI. */
  uint32_t compcsi_cnt{}; /* no of comp-csi rels */
  AVD_CSI *csi_list_cs_type_next{};
  AVD_CS_TYPE *cstype{};
  bool assign_flag = false; /* Flag used while assigning. to mark this csi has
                       been assigned a Comp from * current SI being assigned */

  static AVD_COMP *find_assigned_comp(
...
typedef struct avd_comp_csi_rel_tag {
  ...
  struct avd_comp_csi_rel_tag
      *susi_csicomp_next; /* The next element in the list w.r.t to
                           * susi relationship structure */
  struct avd_comp_csi_rel_tag
      *csi_csicomp_next; /* The next element in the list w.r.t to
                          * CSI structure */
} AVD_COMP_CSI_REL;
 ...
 * @param csi
 * @param comp
 * @param create_in_imm set to true if IMM object (CSI assignment) should be
 * created
 *
 * @return AVD_COMP_CSI_REL*
 */
extern AVD_COMP_CSI_REL *avd_compcsi_create(struct avd_su_si_rel_tag *susi,
                                            AVD_CSI *csi, AVD_COMP *comp,
                                            bool create_in_imm);
 ...
 * @return uns32
 */
extern uint32_t avd_compcsi_delete(AVD_CL_CB *cb,
                                   struct avd_su_si_rel_tag *susi, bool ckpt);
...
extern void avd_compcsi_from_csi_and_susi_delete(
    struct avd_su_si_rel_tag *susi, struct avd_comp_csi_rel_tag *comp_csi,
    bool ckpt);
