  ...
  for (const auto &i_si : sg->list_of_si) {
    ...
    for (const auto &value : *sirankedsu_db) {
      ...
      if ((i_su->saAmfSuReadinessState != SA_AMF_READINESS_IN_SERVICE) ||
        ...
        std::vector<AVD_SU *>::iterator iter =
            std::find(i_su->sg_of_su->list_of_su.begin(),
      ...
      if (avd_su_susi_find(cb, i_su, i_si->name) != AVD_SU_SI_REL_NULL) {
    ...
    for (const auto &iter : sg->list_of_su) {
      ...
      if (i_si->pref_active_assignments() <= i_si->curr_active_assignments()) {
        /* The preferred number of active assignments for SI has reached, so
           continue to next SI */
        i_su = nullptr;
        continue;
      }
    ...
    if (true == sg->equal_ranked_su) {
      ...
      while (true) {
        ...
        if ((nullptr == qualified_su) && (false == next_si_tobe_assigned)) {
          l_flag = false; /* All SU are unqualified, no need to next si*/
          break;
        }
        if ((nullptr == qualified_su) && (true == next_si_tobe_assigned)) {
        ...
        if ((nullptr != qualified_su) && (true == next_si_tobe_assigned)) {
          l_flag = true;
          /* found the SU assign the SI to the SU as active */
          if (avd_new_assgn_susi(cb, qualified_su, i_si, SA_AMF_HA_ACTIVE,
                                 false, &tmp_rel) == NCSCC_RC_SUCCESS) {
            /* Add the SU to the operation list */
            ...
            LOG_ER("%s:%u: %s (%zu)", __FILE__, __LINE__,
                   qualified_su->name.c_str(), qualified_su->name.length());
            l_flag = true; /* Go for next assignment */
            break;
          }
        }
...
uint32_t SG_NACV::si_assign(AVD_CL_CB *cb, AVD_SI *si) {
  ...
  if (avd_sg_nacvred_su_chose_asgn(cb, si->sg_of_si) == nullptr) {
  ...
  switch (su->sg_of_su->sg_fsm_state) {
      ...
      if (su->sg_of_su->admin_si != AVD_SI_NULL) {
        ...
        if (nullptr != su->sg_of_su->max_assigned_su) {
          if (su == su->sg_of_su->min_assigned_su) {
            /* Only care for min_assigned_su as it has been given Act assgnmt
               and SU reported OOS, max_assigned_su would be in Quisced state,
               so send Act to max_assigned_su if it is Act.*/
            /* This is the case of max_assigned_su has refused to go to Quisced,
               send Act one to it and reset the pointers as SI Transfer has
               failed.*/
            if (su->sg_of_su->max_assigned_su->saAmfSuReadinessState ==
              ...
              if (avd_susi_mod_send(i_susi, SA_AMF_HA_ACTIVE) ==
                  NCSCC_RC_FAILURE) {
                su->sg_of_su->max_assigned_su = su->sg_of_su->min_assigned_su =
                    nullptr;
            ...
            /* If su is max_assigned_su then let the default path
               be hitted as max_assigned_su would be assigned Quisced, any way
               when min_assigned_su get Act resp then it will check if
               max_assigned_su is OOS and then it wouldn't do anything just
               reset pointers. So, here don't reset pointers.*/
          }
        }
        m_AVD_SU_SI_CHK_QSD_ASGN(su, flag);
        if (flag == false) {
    ...
    case AVD_SG_FSM_SU_OPER:
      ...
      if (su_oper_list_front() == su) {
         ...
         * Add the SU to the operation list and change state to SG_realign
         * state.
         */
    ...
    case AVD_SG_FSM_SG_REALIGN:
      ...
      if (act == AVSV_SUSI_ACT_DEL) {
          ...
          if (su->sg_of_su->admin_si->list_of_sisu != AVD_SU_SI_REL_NULL) {
            if ((su->sg_of_su->admin_si->list_of_sisu->fsm ==
              ...
              avd_sg_su_oper_list_add(
                  cb, su->sg_of_su->admin_si->list_of_sisu->su, false);
              su->sg_of_su->admin_si->set_admin_state(SA_AMF_ADMIN_LOCKED);
              m_AVD_CLEAR_SG_ADMIN_SI(cb, (su->sg_of_su));
            }
          } else {
        ...
        if (flag == true) {
          avd_sg_su_oper_list_del(cb, su, false);
          if (nullptr != su->sg_of_su->max_assigned_su) {
            /* SI Transfer is undergoing, this is the case when min_su has been
               assigned Act, so check still max_su is still IN-SERV, then send
               DEL to it and reset the pointers, the DEL resp would be handled
               in the default scenario as the pointers has been reset to
               indicate that the desired SI transfer has been successful. If
               max_su is OOS, no problem just reset the pointer.*/
            if (su->sg_of_su->max_assigned_su->saAmfSuReadinessState ==
                SA_AMF_READINESS_IN_SERVICE) {
              AVD_SU_SI_REL *i_susi;
              avd_sg_su_oper_list_del(cb, su, false);
          ...
          if ((su->sg_of_su->admin_si == AVD_SI_NULL) &&
              (su->sg_of_su->su_oper_list.empty() == true)) {
            if (avd_sg_nacvred_su_chose_asgn(cb, su->sg_of_su) == nullptr) {
              /* No New assignments are been done in the SG. change the FSM
               * state */
              su->sg_of_su->set_fsm_state(AVD_SG_FSM_STABLE);
              avd_sidep_sg_take_action(su->sg_of_su);
              avd_sg_app_su_inst_func(cb, su->sg_of_su);
      ...
      if (act == AVSV_SUSI_ACT_DEL) {
        ...
        if (nullptr != su->sg_of_su->max_assigned_su) {
          ...
          if (su->sg_of_su->min_assigned_su->saAmfSuReadinessState ==
              SA_AMF_READINESS_IN_SERVICE) {
            if (avd_new_assgn_susi(cb, su->sg_of_su->min_assigned_su,
                                   su->sg_of_su->si_tobe_redistributed,
                                   SA_AMF_HA_ACTIVE, false,
                                   &tmp_susi) == NCSCC_RC_SUCCESS) {
            } else {
              /* log a fatal error */
              LOG_ER("%s:%u: SU '%s' (%zu), SI '%s' (%zu)", __FILE__, __LINE__,
              ...
              /* Act assignment has failed, so let us retrieve back to old state
                 and send Act assignment to the quisced su and reset the ponters
                 as SI transfer has failed.*/
              if (avd_susi_mod_send(susi, SA_AMF_HA_ACTIVE) ==
                  NCSCC_RC_FAILURE) {
                su->sg_of_su->max_assigned_su = su->sg_of_su->min_assigned_su =
                    nullptr;
                su->sg_of_su->si_tobe_redistributed = nullptr;
          ...
          } else { /* Target SU is Out of Service. Return Assignment to
                      max_assigned_su and reset SI Transfer pointers. */
        ...
        } /* if (nullptr != su->sg_of_su->max_assigned_su) */ else {
          if ((susi == AVD_SU_SI_REL_NULL) ||
            ...
            if (su->saAmfSUAdminState == SA_AMF_ADMIN_SHUTTING_DOWN) {
              ...
              ((su->list_of_susi == susi) && (susi->su_next ==
              AVD_SU_SI_REL_NULL))) */
        }    /* else */
      }      /* if (state == SA_AMF_HA_QUIESCED) */
      break; /* case AVD_SG_FSM_SU_OPER: */
  ...
  switch (su->sg_of_su->sg_fsm_state) {
    ...
    case AVD_SG_FSM_SG_REALIGN:
      ...
      if ((act == AVSV_SUSI_ACT_MOD) &&
        ...
        if (susi != AVD_SU_SI_REL_NULL) {
          /* quiesced/quiescing for a single SI. Send a D2N-INFO_SU_SI_ASSIGN
           * with removal for this SU for the SI. If SI in the SI admin pointer
           * and has only this SU assignment, Change SI admin to LOCK. Remove
           * the SI from the SI admin pointer. If the SU is not in
           * the SU operation list, Add the SU to the operation list.
           */
          ...
          /* quiesced all/quiescing all. Send a D2N-INFO_SU_SI_ASSIGN with
           * removal all for this SU. If SI in the SI admin pointer has
           * assignment only to this SU, Change SI admin to LOCK. Remove the SI
           * from the SI admin pointer. the SU is not in the SU operation list,
           * Add the SU to the operation list. If the SU is in
           * the operation list, If SU admin state is shutdown,
           * change SU admin to LOCK.
           */
    ...
    case AVD_SG_FSM_SU_OPER:
      ...
      if ((act == AVSV_SUSI_ACT_MOD) &&
         ...
         * remove all to the SU. Change to SG_realign state.
         */
        /* Check whether SI Transfer is going on, if yes then handle it
           differently. Else follow default path*/
        if ((nullptr != su->sg_of_su->max_assigned_su) &&
          ...
          if ((susi == AVD_SU_SI_REL_NULL) ||
              (nullptr != su->sg_of_su->max_assigned_su) ||
              ((su->list_of_susi == susi) &&
               (susi->su_next == AVD_SU_SI_REL_NULL))) {
            su->sg_of_su->max_assigned_su = su->sg_of_su->min_assigned_su =
                nullptr;
    ...
    case AVD_SG_FSM_SI_OPER:
      ...
      if ((susi != AVD_SU_SI_REL_NULL) && (act == AVSV_SUSI_ACT_MOD) &&
          ((state == SA_AMF_HA_QUIESCED) || (state == SA_AMF_HA_QUIESCING))) {
        /* message with modified quiesced/quiescing for a SI in the admin
         * pointer. Send a D2N-INFO_SU_SI_ASSIGN with remove for this SI to this
         * SU. If the SI has only one SU assignment, Remove the SI from the
         * admin pointer. Add the SU to operation list. If the SI admin state
         * is shutdown change it to LOCK. Change state to SG_realign.
         */
    ...
    case AVD_SG_FSM_SG_ADMIN:
      ...
      if ((act == AVSV_SUSI_ACT_MOD) &&
          ((state == SA_AMF_HA_QUIESCED) || (state == SA_AMF_HA_QUIESCING))) {
        /* If HA state is quiesced/quiescing all, Send a D2N-INFO_SU_SI_ASSIGN
         * with removal all for this SU.
         */
        if (avd_sg_su_si_del_snd(cb, su) == NCSCC_RC_FAILURE) {
          LOG_ER("%s:%u: %s (%zu)", __FILE__, __LINE__, su->name.c_str(),
                 su->name.length());
...
uint32_t SG_NACV::realign(AVD_CL_CB *cb, AVD_SG *sg) {
  ...
  if (avd_sg_nacvred_su_chose_asgn(cb, sg) == nullptr) {
    /* all the assignments have already been done in the SG. */
    set_adjust_state(AVSV_SG_STABLE);
    avd_sg_app_su_inst_func(cb, sg);
    goto done;
...
void SG_NACV::node_fail(AVD_CL_CB *cb, AVD_SU *su) {
  ...
  switch (su->sg_of_su->sg_fsm_state) {
    ...
    case AVD_SG_FSM_SG_REALIGN:
      ...
      if (nullptr != su->sg_of_su->max_assigned_su) {
        if (su == su->sg_of_su->min_assigned_su) {
          /* Only care for min_assigned_su as it has been given Act assgnmt and
             SU reported OOS, max_assigned_su would be in Quisced state, so send
             Act to max_assigned_su if it is IN-SERV.*/
          if (su->sg_of_su->max_assigned_su->saAmfSuReadinessState ==
              SA_AMF_READINESS_IN_SERVICE) {
            AVD_SU_SI_REL *i_susi;
            i_susi =
          ...
          /* If su is max_assigned_su then let the default path
             be hitted as max_assigned_su would be assigned Quisced, any way
             when min_assigned_su get Act resp then it will check if
             max_assigned_su is OOS and then it wouldn't do anything just reset
             pointers. So, here don't reset pointers.*/
        }
      }
      su->delete_all_susis();
    ...
    case AVD_SG_FSM_SU_OPER:
      ...
      if (su_oper_list_front() == su) {
        ...
        avd_sg_su_oper_list_del(cb, su, false);
        if (nullptr != su->sg_of_su->max_assigned_su) {
          /* This is the case when max_assigned_su has been given Quisced during
             SI Transfer and the corresponding node has gone down*/
          /* min_assigned_su is not assgned SI so if
             (su->sg_of_su->su_oper_list.su == su) will fail and it will not
             come till here.  */
          osafassert(su == su->sg_of_su->max_assigned_su);
          su->sg_of_su->max_assigned_su = su->sg_of_su->min_assigned_su =
              nullptr;
    ...
    case AVD_SG_FSM_SI_OPER:
       ...
       * SI admin pointer. Add the SU to operation list. Change state to
       * SG_realign.
       */
      su->delete_all_susis();
  ...
  switch (si->sg_of_si->sg_fsm_state) {
    case AVD_SG_FSM_STABLE:
      if (si->saAmfSIAdminState == SA_AMF_ADMIN_LOCKED) {
         ...
         */
        for (i_susi = si->list_of_sisu; i_susi != AVD_SU_SI_REL_NULL;
         ...
         * to each of the SUs to which it is assigned. Change state to
         * SI_operation state. Add it to admin SI pointer.
         */
        for (i_susi = si->list_of_sisu; i_susi != AVD_SU_SI_REL_NULL;
             i_susi = i_susi->si_next)
    ...
    case AVD_SG_FSM_SI_OPER:
      if ((si->sg_of_si->admin_si == si) &&
        ...
        while (i_susi != AVD_SU_SI_REL_NULL) {
          if ((i_susi->state != SA_AMF_HA_QUIESCED) &&
  ...
  switch (sg->sg_fsm_state) {
    case AVD_SG_FSM_STABLE:
      if (sg->saAmfSGAdminState == SA_AMF_ADMIN_LOCKED) {
        ...
        /* SG shutdown. Identify all the assigned SUs, send
         * D2N-INFO_SU_SI_ASSIGN modify quiescing all for each of the SU. Add
         * them to the SU operation list. Change state to SG_admin. If no
         * assigned SU exist, no action, stay in stable state.
         */
        for (const auto &i_su : sg->list_of_su) {
          if (i_su->list_of_susi != AVD_SU_SI_REL_NULL) {
 ...
 *                            Success.
 * Returns: Qualified SU/nullptr.
 **************************************************************************/
static AVD_SU *avd_get_qualified_su(AVD_SG *sg, AVD_SI *i_si,
                                    bool *next_si_tobe_assigned) {
  AVD_SU *i_su, *pref_su = nullptr, *pre_temp_su = nullptr;
  ...
  if (false == l_flag) {
    /* This means no SU is qualified, so no need to assign SI, so return
     * failure*/
  ...
  if (false == l_flag_1) {
    /* This means all SU is assigned, so try assigning next SI, return nullptr
       but in next_si_tobe_assigned SUCCESS*/
    *next_si_tobe_assigned = true;
    TRACE_LEAVE();
    return nullptr;
  }
  ...
  *next_si_tobe_assigned = true;
  osafassert(pref_su);
  TRACE_LEAVE2("'%s'", pref_su->name.c_str());
  return pref_su;
}
...
void avd_sg_nwayact_screening_for_si_distr(AVD_SG *avd_sg) {
  AVD_SU *i_su;
  AVD_SI *i_si = nullptr;
  AVD_SU_SI_REL *su_si, *i_susi = nullptr;
  ...
  if ((nullptr == avd_sg->max_assigned_su) ||
      (nullptr == avd_sg->min_assigned_su)) {
  ...
  if ((avd_sg->max_assigned_su->saAmfSUNumCurrActiveSIs -
    ...
    /* Need to change assignments. Find the SI whose assignment has to be
       changed, figure out less ranked SI if SU has more than one assignments.
       Do check that max_assigned_su and min_assigned_su don't share the same SI
       assignment(Say SU1 has SI1, SI2, SI3, SI4 and SU2 has SI4, so don't
       select SI4 at least.)*/
    su_si = avd_sg->max_assigned_su->list_of_susi;
    while (nullptr != su_si) {
      /* Find if this is assigned to avd_sg->min_assigned_su. */
      if (avd_su_susi_find(avd_cb, avd_sg->min_assigned_su, su_si->si->name) !=
          AVD_SU_SI_REL_NULL) {
        /* This SU has already a assignment for this SI go to the
         * next SI.
         */
  ...
  TRACE(
      "max_assigned_su '%s' no of assignments '%u', min_assigned_su '%s' no of assignments '%u',"
      "si_tobe_redistributed '%s'",
      avd_sg->max_assigned_su->name.c_str(),
      avd_sg->max_assigned_su->saAmfSUNumCurrActiveSIs,
      avd_sg->min_assigned_su->name.c_str(),
      avd_sg->min_assigned_su->saAmfSUNumCurrActiveSIs,
      avd_sg->si_tobe_redistributed->name.c_str());
  /* Trigger SI redistribution/transfer here. Only one SI getting shifted from
   * one SU to another SU */
...
void SG_NACV::ng_admin(AVD_SU *su, AVD_AMF_NG *ng) {
  ...
  if (avd_sg_su_si_mod_snd(avd_cb, su, ha_state) == NCSCC_RC_FAILURE) {
    LOG_ER("quiescing state transtion failed for '%s'", su->name.c_str());
    return;
  }
