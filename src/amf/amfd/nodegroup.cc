...
static AVD_AMF_NG *ng_create(const std::string &dn,
 ...
 * @return AVD_AMF_NG*
 */
AVD_AMF_NG *avd_ng_get(const std::string &dn) { return nodegroup_db->find(dn); }
...
static int is_config_valid(const std::string &dn,
    ...
    if (node == nullptr) {
      ...
      if (ccbutil_getCcbOpDataByDN(opdata->ccbId, name) == nullptr) {
        report_ccb_validation_error(
            opdata, "'%s' does not exist either in model or CCB",
            osaf_extended_name_borrow(name));
        TRACE_LEAVE();
        return 0;
      }
  ...
  if (!avd_admin_state_is_valid(tmp_ng->saAmfNGAdminState, opdata)) {
    LOG_NO("Incorrect saAmfNGAdminState:'%u' for '%s'",
           tmp_ng->saAmfNGAdminState, tmp_ng->name.c_str());
    delete tmp_ng;
    TRACE_LEAVE();
    return 0;
  }
...
static AVD_AMF_NG *ng_create(const std::string &dn,
  ...
  if ((immutil_getAttrValuesNumber(
           const_cast<SaImmAttrNameT>("saAmfNGNodeList"), attributes,
           &values_number) == SA_AIS_OK) &&
      (values_number > 0)) {
    for (i = 0; i < values_number; i++) {
      if ((node_name = immutil_getNameAttr(attributes, "saAmfNGNodeList", i)) !=
          nullptr) {
...
SaAisErrorT avd_ng_config_get(void) {
  ...
  ** since it is anyway not synced and needs to be rebuilt. */
  ...
  while (immutil_saImmOmSearchNext_2(searchHandle, &dn,
    ...
    if ((ng = ng_create(Amf::to_string(&dn),
                        (const SaImmAttrValuesT_2 **)attributes)) == nullptr) {
      goto done2;
    }
 ...
 * @return bool
 */
static bool su_is_mapped_to_node_via_nodegroup(const AVD_SU *su,
                                               const AVD_AMF_NG *ng) {
...
bool node_in_nodegroup(const std::string &node, const AVD_AMF_NG *ng) {
  ...
  if (iter != ng->saAmfNGNodeList.end()) return true;
 ...
 * @return SaAisErrorT
 */
static SaAisErrorT ng_ccb_completed_modify_hdlr(
    CcbUtilOperationData_t *opdata) {
    ...
    if (mod->modType == SA_IMM_ATTR_VALUES_DELETE) {
      if (add_found) {
        report_ccb_validation_error(opdata,
                                    "ng modify: no support for mixed ops");
        goto done;
      }
      ...
      for (j = 0; j < mod->modAttr.attrValuesNumber; j++) {
        const std::string node_name(
            Amf::to_string(static_cast<SaNameT *>(mod->modAttr.attrValues[j])));
        ...
        if (node_in_nodegroup(node_name, ng) == false) {
          report_ccb_validation_error(
              opdata, "ng modify: node '%s' does not exist in node group",
              node_name.c_str());
          goto done;
        }
        ...
        ++nodes_deleted;
        /* currently, we don't support multiple node deletions from a group in a
         * CCB */
        if (nodes_deleted > 1) {
          report_ccb_validation_error(
              opdata,
    ...
    if (mod->modType == SA_IMM_ATTR_VALUES_ADD) {
      ...
      for (j = 0; j < mod->modAttr.attrValuesNumber; j++) {
        node = avd_node_get(
            Amf::to_string(static_cast<SaNameT *>(mod->modAttr.attrValues[j])));
        if ((node == nullptr) &&
            (ccbutil_getCcbOpDataByDN(opdata->ccbId,
                                      (SaNameT *)mod->modAttr.attrValues[j]) ==
             nullptr)) {
          report_ccb_validation_error(
              opdata, "'%s' does not exist in model or CCB",
              osaf_extended_name_borrow((SaNameT *)mod->modAttr.attrValues[j]));
          goto done;
        }

        TRACE("ADD %s", osaf_extended_name_borrow(static_cast<SaNameT *>(
                            mod->modAttr.attrValues[j])));
      }

      for (j = 0; j < mod->modAttr.attrValuesNumber; j++) {
        if (node_in_nodegroup(Amf::to_string(static_cast<SaNameT *>(
                                  mod->modAttr.attrValues[j])),
                              ng) == true) {
          report_ccb_validation_error(
              opdata,
              ...
              osaf_extended_name_borrow(
                  static_cast<SaNameT *>(mod->modAttr.attrValues[j])));
          goto done;
        }
      }
    }
 ...
 * @return SaAisErrorT
 */
static SaAisErrorT ng_ccb_completed_delete_hdlr(
    CcbUtilOperationData_t *opdata) {
  SaAisErrorT rc = SA_AIS_ERR_BAD_OPERATION;
  AVD_AVND *node;
  ...
  if ((ng->saAmfNGAdminState != SA_AMF_ADMIN_LOCKED) &&
    ...
    report_ccb_validation_error(opdata,
                                "'%s' can be deleted in locked,"
                                "lock-in or unlocked admin state",
                                ng->name.c_str());
    goto done;
  }
  for (iter = ng->saAmfNGNodeList.begin(); iter != ng->saAmfNGNodeList.end();
       ++iter) {
    ...
    */
    for (const auto &su : node->list_of_ncs_su) {
      if (su_is_mapped_to_node_via_nodegroup(su, ng) &&
        ...
        report_ccb_validation_error(
            opdata, "Cannot delete '%s' because '%s' is mapped using it",
...
static void ng_ccb_apply_modify_hdlr(CcbUtilOperationData_t *opdata) {
  ...
  if (ng == nullptr) {
    ...
    if ((ng = ng_create(Amf::to_string(&opdata->objectName), attributes)) ==
        nullptr) {
  ...
  while ((mod = opdata->param.modify.attrMods[i++]) != nullptr) {
    switch (mod->modType) {
      case SA_IMM_ATTR_VALUES_ADD: {
        for (j = 0; j < mod->modAttr.attrValuesNumber; j++) {
          ng->saAmfNGNodeList.insert(
              Amf::to_string((SaNameT *)mod->modAttr.attrValues[j]));
        }
...
static void ng_ccb_apply_delete_hdlr(CcbUtilOperationData_t *opdata) {
  ...
  if (old_admin_state == SA_AMF_ADMIN_LOCKED_INSTANTIATION) {
    ...
    // Instantiate SUs on nodes of NG. AMFD takes care of assignment after
    // instantiation.
...
void avd_ng_admin_state_set(AVD_AMF_NG *ng, SaAmfAdminStateT state) {
  ...
  avd_send_admin_state_chg_ntf(
      ng->name, (SaAmfNotificationMinorIdT)SA_AMF_NTFID_NG_ADMIN_STATE,
 ...
 */
static SaAisErrorT check_node_stability(const AVD_AVND *node,
                                        const AVD_AMF_NG *ng) {
...
void AVD_AMF_NG::assign_unlocked_nodes() const {
  ...
  for (std::set<std::string>::const_iterator iter = saAmfNGNodeList.begin();
     ...
     */
    for (const auto &su : node->list_of_su) su->sg_of_su->su_insvc(avd_cb, su);
  }
  TRACE_LEAVE();
}
...
void ng_node_lock_and_shutdown(AVD_AVND *node) {
  ...
  if (node->saAmfNodeOperState == SA_AMF_OPERATIONAL_DISABLED) return;
  for (const auto &su : node->list_of_su) {
    su->set_readiness_state(SA_AMF_READINESS_OUT_OF_SERVICE);
 ...
 * of new SUs. Also it updates ng->node_oper_list for tracking operation on this
 * node.
 * @param       ptr to Nodegroup (AVD_AMF_NG).
 */
static void ng_unlock(AVD_AMF_NG *ng) {
...
static void ng_admin_unlock_inst(AVD_AMF_NG *ng) {
  ...
  for (std::set<std::string>::const_iterator iter = ng->saAmfNGNodeList.begin();
    ...
    for (const auto &node_su : node->list_of_su) {
      /*Instantiate only those SUs in this SG which are hosted on the Nodes of
         NG. Also honor saAmfSURank while instantating.
       */
    ...
    if (node->su_cnt_admin_oper > 0) ng->node_oper_list.insert(node->name);
  }
 ...
 * @param params
 */
static void ng_admin_op_cb(SaImmOiHandleT immoi_handle,
                           SaInvocationT invocation, const SaNameT *ng_name,
                           SaImmAdminOperationIdT op_id,
                           const SaImmAdminOperationParamsT_2 **params) {
  SaAisErrorT rc = SA_AIS_OK;
  AVD_AMF_NG *ng = avd_ng_get(Amf::to_string(ng_name));
  TRACE_ENTER2("'%s', inv:'%llu', op:'%llu'",
               osaf_extended_name_borrow(ng_name), invocation, op_id);

  switch (op_id) {
    case SA_AMF_ADMIN_LOCK_INSTANTIATION:
      rc = check_ng_stability(ng);
      if (rc != SA_AIS_OK) {
        report_admin_op_error(avd_cb->immOiHandle, invocation, rc, nullptr,
                              "Some entity is unstable, Operation cannot "
                              "be performed on '%s'"
                              "Check syslog for entity details",
                              osaf_extended_name_borrow(ng_name));
        goto done;
      }
      if (ng->saAmfNGAdminState == SA_AMF_ADMIN_LOCKED_INSTANTIATION) {
        report_admin_op_error(
            avd_cb->immOiHandle, invocation, SA_AIS_ERR_NO_OP, nullptr,
      ...
      if (ng->saAmfNGAdminState != SA_AMF_ADMIN_LOCKED) {
        report_admin_op_error(
            avd_cb->immOiHandle, invocation, SA_AIS_ERR_BAD_OPERATION, nullptr,
      ...
      for (std::set<std::string>::const_iterator iter =
        ...
        node->su_cnt_admin_oper = 0;
        node->admin_ng = ng;
        if (node->saAmfNodeAdminState != SA_AMF_ADMIN_LOCKED) continue;
      ...
      for (std::set<std::string>::const_iterator iter =
        ...
        if (node->saAmfNodeAdminState != SA_AMF_ADMIN_LOCKED_INSTANTIATION) {
          LOG_NO("'%s' LOCK_INSTANTIATION: AMF node admin state is not LOCKED",
                 node->name.c_str());
          continue;
        }
      ...
      TRACE("node_oper_list size:%u", ng->oper_list_size());
      if (ng->node_oper_list.empty()) ng_complete_admin_op(ng, SA_AIS_OK);
      break;
    case SA_AMF_ADMIN_UNLOCK_INSTANTIATION:
      ...
      if (ng->saAmfNGAdminState == SA_AMF_ADMIN_LOCKED) {
        report_admin_op_error(avd_cb->immOiHandle, invocation, SA_AIS_ERR_NO_OP,
    ...
    case SA_AMF_ADMIN_LOCK:
      ...
      if (ng->saAmfNGAdminState == SA_AMF_ADMIN_LOCKED_INSTANTIATION) {
        report_admin_op_error(
            avd_cb->immOiHandle, invocation, SA_AIS_ERR_BAD_OPERATION, nullptr,
            "'%s' Invalid Admin Operation LOCK in state %s", ng->name.c_str(),
    ...
    case SA_AMF_ADMIN_UNLOCK:
      ...
      if (ng->saAmfNGAdminState != SA_AMF_ADMIN_LOCKED) {
        report_admin_op_error(
            avd_cb->immOiHandle, invocation, SA_AIS_ERR_BAD_OPERATION, nullptr,
            "'%s' Invalid Admin Operation UNLOCK in state %s", ng->name.c_str(),
            avd_adm_state_name[ng->saAmfNGAdminState]);
        goto done;
      }
      ng->admin_ng_pend_cbk.invocation = invocation;
...
void avd_ng_constructor(void) {
  ...
  avd_class_impl_set("SaAmfNodeGroup", nullptr, ng_admin_op_cb,
                     ng_ccb_completed_cb, ng_ccb_apply_cb);
}
...
void avd_ng_restore_headless_states(AVD_CL_CB *cb,
  ...
  // nodegroup then saAmfSGAdminState also needs to be set.
      ...
      for (const auto &value : *nodegroup_db) {
        ...
        for (std::set<std::string>::const_iterator iter =
          ...
          if (susi->su->su_on_node == node) {
            // Nodegroup was shutting down, also needs to check whether
            // nodegroup operation was borrowing SG ADMIN FSM code (for 2N only)
            if (ng->saAmfNGAdminState == SA_AMF_ADMIN_SHUTTING_DOWN) {
              node->admin_ng = ng;
            ...
            // Node group was LOCKED, AMFD treat nodegroup and node operation in
            // the same view, except nodegroup was borrowing SG ADMIN FSM. This
            // is because the other case that nodegroup operation was only SG
            ...
            TRACE(
                "Restore nodegroup operation, SG(%s), ng_using_saAmfSGAdminState: %u, saAmfSGAdminState:%u",
