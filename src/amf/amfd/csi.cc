    ...
    if (nullptr != (cst = avd_compcstype_find_match(cstype, comp))) {
      if (SA_AMF_HA_ACTIVE == sisu->state) {
        if (cst->saAmfCompNumCurrActiveCSIs < cst->saAmfCompNumMaxActiveCSIs) {
          break;
        } else { /* We can't assign this csi to this comp, so check for another
                    comp */
          continue;
        }
      } else {
...
void csi_cmplt_delete(AVD_CSI *csi, bool ckpt) {
  ...
  if (!ckpt) {
    ...
    for (curr = (AVD_PG_CSI_NODE *)m_NCS_DBLIST_FIND_FIRST(&csi->pg_node_list);
         curr != nullptr; curr = (AVD_PG_CSI_NODE *)m_NCS_DBLIST_FIND_NEXT(
                              &curr->csi_dll_node)) {
...
static int is_config_valid(const std::string &dn,
  ...
  rc = immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfCSType"), attributes, 0,
                       &aname);
  ...
  if ((immutil_getAttrValuesNumber(
           const_cast<SaImmAttrNameT>("saAmfCSIDependencies"), attributes,
    ...
    for (i = 0; i < values_number; i++) {
      ...
      rc = immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfCSIDependencies"),
      ...
      if (dn.compare(csi_dependency) == 0) {
        report_ccb_validation_error(
            opdata,
            "'%s' validation failed - dependency configured to"
            " itself!",
            dn.c_str());
        TRACE_LEAVE();
        return 0;
        ...
        if (opdata == nullptr) {
          ...
          if (object_exist_in_imm(csi_dependency) == false) {
            report_ccb_validation_error(opdata,
                                        "'%s' validation failed - '%s' does not"
                                        " exist",
                                        dn.c_str(), csi_dependency.c_str());
            TRACE_LEAVE();
            return 0;
          }
        } else if (ccbutil_getCcbOpDataByDN(opdata->ccbId, &tmp_dependency) ==
                   nullptr) {
          report_ccb_validation_error(
              opdata,
      ...
      if (parent.compare(dep_parent) != 0) {
        report_ccb_validation_error(
            opdata,
            "'%s' validation failed - dependency to CSI in other"
            " SI is not allowed",
            dn.c_str());
        TRACE_LEAVE();
        return 0;
  ...
  /* Verify that the SI can contain this CSI */
  {
    ...
    if (nullptr != (avd_si = avd_si_get(si_name))) {
      /* Check for any admin operations undergoing. This is valid during dyn
       * add*/
      if ((opdata != nullptr) &&
          (AVD_SG_FSM_STABLE != avd_si->sg_of_si->sg_fsm_state)) {
        report_ccb_validation_error(opdata,
                                    "SG('%s') fsm state('%u') is not in "
                                    "AVD_SG_FSM_STABLE(0)",
...
/**
 * @brief       Check whether the CSI dependency is already existing in the
 * existing list if not adds to the dependencies list
 *
...
static bool csi_add_csidep(AVD_CSI *csi, AVD_CSI_DEPS *new_csi_dep) {
  ...
  for (temp_csi_dep = csi->saAmfCSIDependencies; temp_csi_dep != nullptr;
    ...
    if (new_csi_dep->csi_dep_name_value.compare(
            temp_csi_dep->csi_dep_name_value) == 0) {
      csi_added = true;
    }
  }
  if (!csi_added) {
...
static void csi_remove_csidep(AVD_CSI *csi, const std::string &required_dn) {
  ...
  for (curr = csi->saAmfCSIDependencies; curr != nullptr;
       curr = curr->csi_dep_next) {
...
//
AVD_CSI::AVD_CSI(const std::string &csi_name) : name(csi_name) {}
/**
...
AVD_CSI *csi_create(const std::string &csi_name) {
  ...
  if (csi_db->insert(csi->name, csi) != NCSCC_RC_SUCCESS) osafassert(0);
 ...
 * @return      pointer to AVD_CSI
 */
static void csi_get_attr_and_add_to_model(AVD_CSI *csi,
                                          const SaImmAttrValuesT_2 **attributes,
  ...
  error = immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfCSType"), attributes,
                          0, &cs_type);
    ...
    if (values_number == 0) {
      ...
      for (i = 0; i < values_number; i++) {
        ...
        if (immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfCSIDependencies"),
                            attributes, i, &temp_name) != SA_AIS_OK) {
        ...
        if (found == true) delete new_csi_dep;
      }
    }
  } else {
    csi->rank = 1;
    TRACE_ENTER2("DEP not configured, marking rank 1. Csi'%s', Rank'%u'",
                 csi->name.c_str(), csi->rank);
  }

  if ((immutil_getAttrValuesNumber(
           const_cast<SaImmAttrNameT>(
               "osafAmfCSICommunicateCsiAttributeChange"),
           attributes, &values_number) != SA_AIS_OK)) {
    TRACE("Default for osafAmfCSICommunicateCsiAttributeChange set to false");
    csi->osafAmfCSICommunicateCsiAttributeChange =
        false;  // Default value is always 0.
  }
...
SaAisErrorT avd_csi_config_get(const std::string &si_name, AVD_SI *si) {
  ...
  while (immutil_saImmOmSearchNext_2(searchHandle, &temp_csi_name,
                                     (SaImmAttrValuesT_2 ***)&attributes) ==
         SA_AIS_OK) {
    const std::string csi_name(Amf::to_string(&temp_csi_name));
    if (!is_config_valid(csi_name, attributes, nullptr)) goto done2;
 ...
 *
 **************************************************************************/
static SaAisErrorT csi_ccb_completed_create_hdlr(
    CcbUtilOperationData_t *opdata) {
    ...
    if (nullptr != avd_si->list_of_sisu) {
      ...
      while (t_sisu) {
        if (t_sisu->csi_add_rem == true) {
          LOG_NO(
              "CSI create of '%s' in queue: pending assignment"
              " for '%s'",
      ...
      while (t_sisu) {
        AVD_SU *su = t_sisu->su;
        /* We need to assign this csi if an extra component exists, which is
         * unassigned.*/
        ...
        t_comp = su->find_unassigned_comp_that_provides_cstype(
            Amf::to_string(&cstype_name));

        /* Component not found.*/
        if (nullptr == t_comp) {
          /* This means that all the components are assigned, let us assigned it
             to assigned component.*/
          t_comp = AVD_CSI::find_assigned_comp(Amf::to_string(&cstype_name),
 ...
 *
 **************************************************************************/
static SaAisErrorT csi_ccb_completed_modify_hdlr(
    CcbUtilOperationData_t *opdata) {
  SaAisErrorT rc = SA_AIS_ERR_BAD_OPERATION;
  ...
  while ((attr_mod = opdata->param.modify.attrMods[i++]) != nullptr) {
    if (!strcmp(attr_mod->modAttr.attrName, "saAmfCSType")) {
      ...
      if (SA_AMF_ADMIN_LOCKED != csi->si->saAmfSIAdminState) {
        report_ccb_validation_error(
            opdata, "Parent SI is not in locked state, SI state '%d'",
            csi->si->saAmfSIAdminState);
        goto done;
      }
      if (cstype_db->find(Amf::to_string(&cstype_name)) == nullptr) {
        report_ccb_validation_error(opdata, "CS Type not found '%s'",
                                    osaf_extended_name_borrow(&cstype_name));
        goto done;
      }
      ...
      // Reject replacement of CSI deps, only deletion and addition are
      // supported.
      if (attr_mod->modType == SA_IMM_ATTR_VALUES_REPLACE) {
        report_ccb_validation_error(
            opdata, "'%s' - replacement of CSI dependency is not supported",
            object_name.c_str());
        goto done;
      }
      const std::string required_dn(Amf::to_string(
          static_cast<SaNameT *>(attr_mod->modAttr.attrValues[0])));
      const AVD_CSI *required_csi = csi_db->find(required_dn);

      // Required CSI must exist in current model
      if (required_csi == nullptr) {
        report_ccb_validation_error(opdata, "CSI '%s' does not exist",
                                    required_dn.c_str());
        goto done;
      }
      ...
      if (attr_mod->modType == SA_IMM_ATTR_VALUES_ADD) {
        ...
        if (attr_mod->modAttr.attrValuesNumber > 1) {
          report_ccb_validation_error(opdata,
                                      "only one dep can be added at a time");
          goto done;
        }

        // check cyclic dependencies by scanning the deps of the required CSI
        for (csi_dep = required_csi->saAmfCSIDependencies; csi_dep;
             csi_dep = csi_dep->csi_dep_next) {
          if (csi_dep->csi_dep_name_value.compare(object_name) == 0) {
            // the required CSI requires this CSI
            report_ccb_validation_error(
                opdata, "cyclic dependency between '%s' and '%s'",
                object_name.c_str(), required_dn.c_str());
            goto done;
          }
        }
        ...
        if (csi->name.compare(required_dn.c_str()) == 0) {
          report_ccb_validation_error(opdata, "dependency for '%s' to itself",
                                      csi->name.c_str());
          goto done;
        }
 ...
 *
 **************************************************************************/
static SaAisErrorT csi_ccb_completed_delete_hdlr(
    CcbUtilOperationData_t *opdata) {
  SaAisErrorT rc = SA_AIS_ERR_BAD_OPERATION;
  AVD_CSI *csi;
  AVD_SU_SI_REL *t_sisu;
  ...
  if (avd_cb->avail_state_avd != SA_AMF_HA_ACTIVE) {
    if (csi == nullptr) {
      /* This means that csi has been deleted during checkpointing at STDBY and
         completed callback has arrived delayed.*/
      TRACE("CSI delete completed (STDBY): '%s' does not exist",
            osaf_extended_name_borrow(&opdata->objectName));
    }
    // IMM honors response of completed callback only from active amfd, so reply
    // ok from standby amfd.
    rc = SA_AIS_OK;
  ...
  if (AVD_SG_FSM_STABLE != csi->si->sg_of_si->sg_fsm_state) {
    report_ccb_validation_error(
        opdata, "SG('%s') fsm state('%u') is not in AVD_SG_FSM_STABLE(0)",
  ...
  if (csi->si->saAmfSIAdminState != SA_AMF_ADMIN_LOCKED) {
    if (nullptr == csi->si->list_of_sisu) {
      ...
      /* SI is unlocked and this is the last csi to be deleted, then donot allow
       * it. */
      if (csi->si->list_of_csi->si_list_of_csi_next == nullptr) {
        report_ccb_validation_error(
            opdata,
            " csi('%s') is the last csi in si('%s'). Lock SI and"
            " then delete csi.",
      ...
      while (t_sisu) {
        if (t_sisu->csi_add_rem == true) {
          LOG_NO(
              "CSI remove of '%s' rejected: pending "
              "assignment for '%s'",
    ...
    if (csi->list_compcsi != nullptr) {
      report_ccb_validation_error(opdata, "SaAmfCSI '%s' is in use",
                                  csi->name.c_str());
      rc = SA_AIS_ERR_BAD_OPERATION;
      goto done;
    }
...
static void ccb_apply_delete_hdlr(CcbUtilOperationData_t *opdata) {
  ...
  TRACE_ENTER();
  if (avd_cb->avail_state_avd != SA_AMF_HA_ACTIVE) {
    /* A double check whether csi has been deleted from DB or not and whether
       pointer stored userData is still valid. */
    csi_in_db = csi_db->find(Amf::to_string(&opdata->objectName));
    if ((csi == nullptr) || (csi_in_db == nullptr)) {
      /* This means that csi has been deleted during checkpointing at STDBY and
         delete callback has arrived delayed.*/
  ...
  if ((nullptr != csi->si->list_of_sisu) && (csi->compcsi_cnt != 0)) {
    ...
    /* csi->compcsi_cnt == 0 ==> This means that there is no comp_csi related to
       this csi in the SI. It may happen this csi is not assigned to any CSI
       because of no compcstype match, but its si may have SUSI. This will
       happen in case of deleting one comp from SU in upgrade case Scenario :
       Add one comp1-csi1, comp2-csi2 in upgrade procedure, then delete
       comp1-csi1 i.e. in the end call immcfg -d csi1. Since csi1 will not be
    ...
    while (t_sisu) {
      ...
      if ((SA_AMF_HA_ACTIVE == t_sisu->state) && (true == first_sisu)) {
        first_sisu = false;
        if (avd_snd_susi_msg(avd_cb, t_sisu->su, t_sisu, AVSV_SUSI_ACT_DEL,
                             true, t_csicomp) != NCSCC_RC_SUCCESS) {
          LOG_ER("susi send failure for su'%s' and si'%s'",
                 t_sisu->su->name.c_str(), t_sisu->si->name.c_str());
          goto done;
        }
      }
...
static void csi_ccb_apply_modify_hdlr(struct CcbUtilOperationData *opdata) {
  ...
  while ((attr_mod = opdata->param.modify.attrMods[i++]) != nullptr) {
    if (!strcmp(attr_mod->modAttr.attrName, "saAmfCSType")) {
      ...
      SaNameT cstype_name = *(SaNameT *)attr_mod->modAttr.attrValues[0];
      TRACE("saAmfCSType modified from '%s' to '%s' for Csi'%s'",
            csi->saAmfCSType.c_str(), osaf_extended_name_borrow(&cstype_name),
            csi->name.c_str());
      ...
      avd_cstype_add_csi(csi);
                       ...
                       "osafAmfCSICommunicateCsiAttributeChange")) {
      csi->osafAmfCSICommunicateCsiAttributeChange =
          static_cast<bool>(*((bool *)attr_mod->modAttr.attrValues[0]));
      LOG_NO(
          "Modified osafAmfCSICommunicateCsiAttributeChange for '%s' to '%u'",
      ...
      if (attr_mod->modType == SA_IMM_ATTR_VALUES_ADD) {
        ...
        new_csi_dep->csi_dep_name_value = Amf::to_string(
            static_cast<SaNameT *>(attr_mod->modAttr.attrValues[0]));
        bool already_exist = csi_add_csidep(csi, new_csi_dep);
        if (already_exist) delete new_csi_dep;
...
static void csi_ccb_apply_create_hdlr(struct CcbUtilOperationData *opdata) {
  ...
  csi_get_attr_and_add_to_model(
      csi, opdata->param.create.attrValues,
...
SaAisErrorT csi_assign_hdlr(AVD_CSI *csi) {
  ...
  /* Check whether csi assignment is already in progress and if yes, then
     return. This csi will be assigned after the undergoing csi assignment gets
     over.*/
  if (csi->si->list_of_sisu != nullptr) {
    for (t_sisu = csi->si->list_of_sisu; t_sisu != nullptr;
         t_sisu = t_sisu->si_next) {
      if (t_sisu->csi_add_rem == true) {
  ...
  if (nullptr != csi->si->list_of_sisu) {
    ...
    while (t_sisu) {
      ...
      t_comp = t_sisu->su->find_unassigned_comp_that_provides_cstype(
          csi->saAmfCSType);
      ...
      if (nullptr == t_comp) {
        /* This means that all the components are assigned, let us assigned it
           to assigned component.*/
        t_comp = AVD_CSI::find_assigned_comp(csi->saAmfCSType, t_sisu,
                                             t_sisu->su->list_of_comp);
      }
      if (nullptr == t_comp) {
        LOG_ER(
            "Compcsi doesn't exist or MaxActiveCSI/MaxStandbyCSI have reached for csi '%s'",
            csi->name.c_str());
        goto done;
      }

      if ((compcsi = avd_compcsi_create(t_sisu, csi, t_comp, true)) ==
          nullptr) {
      ...
      if ((SA_AMF_HA_ACTIVE == t_sisu->state) && (true == first_sisu)) {
        first_sisu = false;
        if (avd_snd_susi_msg(avd_cb, t_sisu->su, t_sisu, AVSV_SUSI_ACT_ASGN,
                             true, compcsi) != NCSCC_RC_SUCCESS) {
          /* free all the CSI assignments and end this loop */
          avd_compcsi_delete(avd_cb, t_sisu, true);
          /* Unassign the SUSI */
          avd_susi_update_assignment_counters(t_sisu, AVSV_SUSI_ACT_DEL,
...
static void avd_create_csiassignment_in_imm(SaAmfHAStateT ha_state,
  ...
  const SaImmAttrValuesT_2 attr_safCSIComp = {
      const_cast<SaImmAttrNameT>("safCSIComp"), SA_IMM_ATTR_SANAMET, 1, arr1};
  const SaImmAttrValuesT_2 attr_saAmfCSICompHAState = {
      const_cast<SaImmAttrNameT>("saAmfCSICompHAState"), SA_IMM_ATTR_SAUINT32T,
      1, arr2};
  ...
  const SaImmAttrValuesT_2 *attrValues[] = {
      &attr_safCSIComp, &attr_saAmfCSICompHAState,
      &attr_saAmfCSICompHAReadinessState, nullptr};
...
AVD_COMP_CSI_REL *avd_compcsi_create(AVD_SU_SI_REL *susi, AVD_CSI *csi,
  ...
  for (compcsi = susi->list_of_csicomp; compcsi;
       compcsi = compcsi->susi_csicomp_next) {
    if ((compcsi->comp == comp) && (compcsi->csi == csi)) goto done;
  }
...
uint32_t avd_compcsi_delete(AVD_CL_CB *cb, AVD_SU_SI_REL *susi, bool ckpt) {
  ...
  while (susi->list_of_csicomp != nullptr) {
    ...
    avd_delete_csiassignment_from_imm(
        Amf::to_string(&lcomp_csi->comp->comp_info.name), lcomp_csi->csi->name);
    delete lcomp_csi;
  }
 ...
 *
 **************************************************************************/
void avd_compcsi_from_csi_and_susi_delete(AVD_SU_SI_REL *susi,
  ...
  while (t_compcsi_susi) {
    if (t_compcsi_susi == comp_csi) break;
  ...
  if (!ckpt)
    avd_snd_pg_upd_msg(avd_cb, comp_csi->comp->su->su_on_node, comp_csi,
                       SA_AMF_PROTECTION_GROUP_REMOVED, std::string(""));
  avd_delete_csiassignment_from_imm(
      Amf::to_string(&comp_csi->comp->comp_info.name), comp_csi->csi->name);
  delete comp_csi;
...
bool csi_assignment_validate(AVD_SG *sg) {
  ...
  for (const auto &temp_si : sg->list_of_si)
    for (temp_sisu = temp_si->list_of_sisu; temp_sisu;
         temp_sisu = temp_sisu->si_next)
      if (temp_sisu->csi_add_rem == true) return true;
  return false;
}
...
bool are_sponsor_csis_assigned_in_su(AVD_CSI *csi, AVD_SU *su) {
  for (AVD_CSI_DEPS *spons_csi = csi->saAmfCSIDependencies;
       spons_csi != nullptr; spons_csi = spons_csi->csi_dep_next) {
    ...
    for (AVD_COMP_CSI_REL *compcsi = tmp_csi->list_compcsi; compcsi;
      ...
      if (compcsi->comp->su == su) is_sponsor_assigned = true;
    }
    // Return false if this sponsor is not assigned to this SU.
    if (is_sponsor_assigned == false) return false;
  }
  return true;
}
...
SaAisErrorT avd_compcsi_recreate(AVSV_N2D_ND_CSICOMP_STATE_MSG_INFO *info) {
  ...
  for (comp_state = info->comp_list; comp_state != nullptr;
    ...
    // . saAmfCompCurrProxyName and saAmfCompCurrProxiedNames wouldn't change
    // during headless
...
void avd_compcsi_cleanup_imm_object(AVD_CL_CB *cb) {
  ...
  const SaImmAttrNameT siass_attributes[] = {
      const_cast<SaImmAttrNameT>("safCSIComp"), NULL};
  ...
  while ((rc = immutil_saImmOmSearchNext_2(
    ...
    AVD_CSI *csi =
        csi_db->find(strstr(osaf_extended_name_borrow(&dn), "safCsi"));
    osafassert(csi);
    SaNameT comp_name;
    avsv_sanamet_init_from_association_dn(&dn, &comp_name, "safComp",
                                          csi->name.c_str());
