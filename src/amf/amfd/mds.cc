...
const MDS_CLIENT_MSG_FORMAT_VER avd_avd_msg_fmt_map_table[] = {
    ...
    AVD_AVD_MSG_FMT_VER_4, AVD_AVD_MSG_FMT_VER_5, AVD_AVD_MSG_FMT_VER_6};
...
/**
 * Initialize MDS. Compile the EDP, create vdest, install svc id on vdest,
 * subscribe, ...
 * @param cb
 *
 * @return uns32
  ...
  switch (info->i_op) {
    case MDS_CALLBACK_RECEIVE:
      ...
      if ((info->info.receive.i_fr_svc_id == NCSMDS_SVC_ID_AVND) &&
        ...
        rc = avd_n2d_msg_rcv((AVD_DND_MSG *)info->info.receive.i_msg, node_id,
                             msg_fmt_ver);
       ...
       */
        ...
        LOG_WA("%s: unknown svc id %u", __FUNCTION__,
               info->info.receive.i_fr_svc_id);
        return NCSCC_RC_FAILURE;
      }
      break;
    ...
    case MDS_CALLBACK_ENC_FLAT: {
      if ((info->i_yr_svc_id == NCSMDS_SVC_ID_AVD) &&
        ...
        info->info.enc.o_msg_fmt_ver = m_NCS_ENC_MSG_FMT_GET(
            info->info.enc.i_rem_svc_pvt_ver, AVD_AVD_SUBPART_VER_MIN,
        ...
        if (info->info.enc.o_msg_fmt_ver < AVD_AVD_MSG_FMT_VER_1) {
          LOG_ER("%s: wrong msg fmt ver %u, %u.", __FUNCTION__,
        ...
        info->info.enc.o_msg_fmt_ver = m_NCS_ENC_MSG_FMT_GET(
            info->info.enc.i_rem_svc_pvt_ver, AVD_AVND_SUBPART_VER_MIN,
        ...
        LOG_ER("%s: svc id %u %u", __FUNCTION__, info->i_yr_svc_id,
    ...
    case MDS_CALLBACK_DEC_FLAT: {
      if ((info->i_yr_svc_id == NCSMDS_SVC_ID_AVD) &&
        ...
        if (!m_NCS_MSG_FORMAT_IS_VALID(
                info->info.dec.i_msg_fmt_ver, AVD_AVD_SUBPART_VER_MIN,
                AVD_AVD_SUBPART_VER_MAX, avd_avd_msg_fmt_map_table)) {
          LOG_ER("%s: wrong msg fmt not valid %u", __FUNCTION__,
        ...
        if (!m_NCS_MSG_FORMAT_IS_VALID(
                info->info.dec.i_msg_fmt_ver, AVD_AVND_SUBPART_VER_MIN,
                AVD_AVND_SUBPART_VER_MAX, avd_avnd_msg_fmt_map_table)) {
          LOG_ER("%s: wrong msg fmt not valid %u", __FUNCTION__,
                 info->info.dec.i_msg_fmt_ver);
          return NCSCC_RC_FAILURE;
    ...
    case NCSMDS_UP:
      switch (evt_info->i_svc_id) {
        ...
        case NCSMDS_SVC_ID_AVND: {
          ...
          evt1->info.nd_mds_info.mds_version = evt_info->i_rem_svc_pvt_ver;
          evt1->info.nd_mds_info.node_id =
              m_NCS_NODE_ID_FROM_MDS_DEST(evt_info->i_dest);
          if (m_NCS_IPC_SEND(&cb->avd_mbx, evt1, NCS_IPC_PRIORITY_HIGH) !=
              NCSCC_RC_SUCCESS) {
            LOG_ER("%s: ncs_ipc_send failed", __FUNCTION__);
            delete evt1;
          }
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uint32_t avd_avnd_mds_send(AVD_CL_CB *cb, AVD_AVND *nd_node,
                           AVD_DND_MSG *snd_msg) {
  NCSMDS_INFO snd_mds;
  uint32_t rc;
...
void avd_mds_avd_up_evh(AVD_CL_CB *cb, AVD_EVT *evt) { TRACE_ENTER(); }
...
void avd_mds_avd_down_evh(AVD_CL_CB *cb, AVD_EVT *evt) { TRACE_ENTER(); }
...
void avd_standby_avd_down_evh(AVD_CL_CB *cb, AVD_EVT *evt) { TRACE_ENTER(); }

uint32_t avd_mds_send(MDS_SVC_ID i_to_svc, MDS_DEST i_to_dest,
                      NCSCONTEXT i_msg) {
  uint32_t rc;
  ...
  if ((rc = ncsmds_api(&snd_mds)) != NCSCC_RC_SUCCESS)
    LOG_WA("%s: failed %u, to %u@%" PRIx64, __FUNCTION__, rc, i_to_svc,
           i_to_dest);
