...
typedef struct avnd_cbk_tag {
  ...
  /* Redundant AvND hdl, generated at STDBY AvND. If red_opq_hdl is non-zero,
     then don't destroy opq_hdl, rather destroy red_opq_hdl during
     avnd_comp_cbq_rec_del() */
...
typedef struct avnd_comp_csi_rec {
  ...
  bool pending_removal;       /*Flag is used when AMFND gets removal of assignments
                                from AMFD       when a CSI is still in assigning state. In
                                such case, AMFND can issue       removal callback only after
                                CSI gets assigned. AMFND will mark this flag       so that
                                it can start removal after CSI is assigned.
                               */
  bool suspending_assignment; /* This flag is used when su-si active assignment
                          is on-going, some other csi may get assigned but
                          *this* csi is still in unassigned state. At this
                          point, AMFND receives QUIESCED or QUIESCING assignment
                          from AMFD. This csi has never got active assignment so
                          the quiesced csi callback should not be issued. This
                          flag is true, which indicates this csi is not involved
                          in QUIESCED/QUIESCING assignment loop. Once su-si
                          QUIESCED/ QUIESCING assignment is done, this flag
                          reset to false. */
...
typedef struct avnd_comp_tag {
  ...
  int config_is_valid;         /* Used to indicate that config has to be refreshed from
                                  IMM */
  bool assigned_flag;          /* Used in finding multiple csi for a single comp while
                                  csi mod.*/
...
#define m_AVND_CHECK_FOR_STDBY_FOR_EXT_COMP(cb, ext_comp)                  \
  (((SA_AMF_HA_STANDBY == (cb)->avail_state_avnd) && (true == (ext_comp))) \
       ? NCSCC_RC_SUCCESS                                                  \
       : NCSCC_RC_FAILURE)
...
#define AVND_COMP_FLAG_TERM_FAIL                           \
  0x00001000 /* This flag will be set for a small duration \
...
#define m_AVND_COMP_TYPE_IS_LOCAL_NODE(x) \
  (((x)->comp_type) & AVND_COMP_TYPE_LOCAL_NODE)
#define m_AVND_COMP_TYPE_IS_INTER_NODE(x) \
  (((x)->comp_type) & AVND_COMP_TYPE_INTER_NODE)
#define m_AVND_COMP_TYPE_IS_EXT_CLUSTER(x) \
  (((x)->comp_type) & AVND_COMP_TYPE_EXT_CLUSTER)

#define m_AVND_COMPT_TYPE_SET(x, flag) (((x)->comp_type) |= (flag))
#define m_AVND_COMP_TYPE_SET_LOCAL_NODE(x) \
  m_AVND_COMPT_TYPE_SET(x, AVND_COMP_TYPE_LOCAL_NODE)
#define m_AVND_COMP_TYPE_SET_INTER_NODE(x) \
  m_AVND_COMPT_TYPE_SET(x, AVND_COMP_TYPE_INTER_NODE)
#define m_AVND_COMP_TYPE_SET_EXT_CLUSTER(x) \
  m_AVND_COMPT_TYPE_SET(x, AVND_COMP_TYPE_EXT_CLUSTER)
...
#define m_AVND_COMP_TYPE_LOCAL_SET(x) \
  m_AVND_COMP_TYPE_SET(x, AVND_COMP_TYPE_LOCAL)
#define m_AVND_COMP_TYPE_PROXY_SET(x) \
  m_AVND_COMP_TYPE_SET(x, AVND_COMP_TYPE_PROXY)
#define m_AVND_COMP_TYPE_PROXIED_SET(x) \
  m_AVND_COMP_TYPE_SET(x, AVND_COMP_TYPE_PROXIED)
#define m_AVND_COMP_TYPE_PREINSTANTIABLE_SET(x) \
  m_AVND_COMP_TYPE_SET(x, AVND_COMP_TYPE_PREINSTANTIABLE)
#define m_AVND_COMP_TYPE_SAAWARE_SET(x) \
  m_AVND_COMP_TYPE_SET(x, AVND_COMP_TYPE_SAAWARE)

/* macros for resetting the comp types */
#define m_AVND_COMP_TYPE_RESET(x, bitmap) (((x)->flag) &= ~(bitmap))
#define m_AVND_COMP_TYPE_LOCAL_RESET(x) \
  m_AVND_COMP_TYPE_RESET(x, AVND_COMP_TYPE_LOCAL)
#define m_AVND_COMP_TYPE_PROXY_RESET(x) \
  m_AVND_COMP_TYPE_RESET(x, AVND_COMP_TYPE_PROXY)
#define m_AVND_COMP_TYPE_PROXIED_RESET(x) \
  m_AVND_COMP_TYPE_RESET(x, AVND_COMP_TYPE_PROXIED)
#define m_AVND_COMP_TYPE_PREINSTANTIABLE_RESET(x) \
  m_AVND_COMP_TYPE_RESET(x, AVND_COMP_TYPE_PREINSTANTIABLE)
#define m_AVND_COMP_TYPE_SAAWARE_RESET(x) \
  m_AVND_COMP_TYPE_RESET(x, AVND_COMP_TYPE_SAAWARE)
...
#define m_AVND_COMP_IS_INST_CMD_SUCC(x) \
  (((x)->flag) & AVND_COMP_FLAG_INST_CMD_SUCC)
...
#define m_AVND_COMP_INST_CMD_SUCC_SET(x) \
  (((x)->flag) |= AVND_COMP_FLAG_INST_CMD_SUCC)
...
#define m_AVND_PROXY_FOR_EXT_COMP_SET(x) \
  (((x)->flag) |= AVND_PROXY_FOR_EXT_COMP)
...
#define m_AVND_COMP_INST_CMD_SUCC_RESET(x) \
  (((x)->flag) &= ~AVND_COMP_FLAG_INST_CMD_SUCC)
#define m_AVND_COMP_ALL_CSI_RESET(x) (((x)->flag) &= ~AVND_COMP_FLAG_ALL_CSI)
#define m_AVND_COMP_TERM_FAIL_RESET(x) \
  (((x)->flag) &= ~AVND_COMP_FLAG_TERM_FAIL)
...
#define m_AVND_CSI_COMP_DLL_NODE_OFFSET                  \
  ((uint8_t *)&(AVND_COMP_CSI_REC_NULL->comp_dll_node) - \
   (uint8_t *)AVND_COMP_CSI_REC_NULL)
#define m_AVND_CSI_REC_FROM_COMP_DLL_NODE_GET(x)                  \
  ((x) ? ((AVND_COMP_CSI_REC *)(((uint8_t *)(x)) -                \
                                m_AVND_CSI_COMP_DLL_NODE_OFFSET)) \
       : 0)
...
#define m_AVND_COMPDB_REC_CSI_GET(comp, csi_name) \
  m_AVND_CSI_REC_FROM_COMP_DLL_NODE_GET(          \
      ncs_db_link_list_find(&(comp).csi_list, (uint8_t *)(csi_name)))
  ...
  {                                                                         \
    for ((o_rec) = (comp)->cbk_list;                                        \
         (o_rec) && !((AVSV_AMF_HC == (o_rec)->cbk_info->type) &&           \
                      (0 == m_CMP_HORDER_SAHCKEY(                           \
                                (o_rec)->cbk_info->param.hc.hc_key, hck))); \
         (o_rec) = (o_rec)->next)                                           \
      ;                                                                     \
  }
...
#define m_AVND_COMP_CBQ_START_PUSH(comp, rec)                            \
  {                                                                      \
    ...
    if ((comp)->cbk_list == 0)                                           \
      ...
      for (curr = (comp)->cbk_list; curr; prv = curr, curr = curr->next) \
        ;                                                                \
      prv->next = (rec);                                                 \
      (rec)->next = 0;                                                   \
    }                                                                    \
...
extern uint32_t avnd_comp_clc_fsm_run(struct avnd_cb_tag *, AVND_COMP *,
                                      AVND_COMP_CLC_PRES_FSM_EV);

extern uint32_t avnd_comp_clc_fsm_trigger(struct avnd_cb_tag *, AVND_COMP *,
                                          AVND_COMP_CLC_PRES_FSM_EV);
...
extern AVND_COMP_CBK *avnd_comp_cbq_rec_add(struct avnd_cb_tag *, AVND_COMP *,
                                            ...
                                            SaTimeT);
extern uint32_t avnd_comp_cbq_send(struct avnd_cb_tag *, AVND_COMP *,
                                   MDS_DEST *, SaAmfHandleT,
                                   AVSV_AMF_CBK_INFO *, SaTimeT);

extern void avnd_comp_cbq_rec_del(struct avnd_cb_tag *, AVND_COMP *,
                                  AVND_COMP_CBK *);

extern uint32_t avnd_comp_cbq_rec_send(struct avnd_cb_tag *, AVND_COMP *,
                                       AVND_COMP_CBK *, bool);
...
extern AVND_COMP_CSI_REC *avnd_compdb_csi_rec_get(struct avnd_cb_tag *,
                                                  const std::string &,
                                                  const std::string &);

extern uint32_t avnd_amf_resp_send(struct avnd_cb_tag *, AVSV_AMF_API_TYPE,
                                   SaAisErrorT, uint8_t *, MDS_DEST *,
                                   MDS_SYNC_SND_CTXT *, AVND_COMP *, bool);

extern void avnd_comp_hc_finalize(struct avnd_cb_tag *, AVND_COMP *,
                                  SaAmfHandleT, MDS_DEST *);
extern void avnd_comp_cbq_finalize(struct avnd_cb_tag *, AVND_COMP *,
                                   SaAmfHandleT, MDS_DEST *);

extern void avnd_comp_cbq_csi_rec_del(struct avnd_cb_tag *, AVND_COMP *,
                                      const std::string &);
...
extern uint32_t avnd_comp_csi_assign_done(struct avnd_cb_tag *, AVND_COMP *,
                                          AVND_COMP_CSI_REC *);

extern uint32_t avnd_comp_csi_remove_done(struct avnd_cb_tag *, AVND_COMP *,
                                          AVND_COMP_CSI_REC *);
...
extern uint32_t avnd_comp_pm_rec_add(struct avnd_cb_tag *, AVND_COMP *,
                                     AVND_COMP_PM_REC *);
extern void avnd_comp_pm_rec_del(struct avnd_cb_tag *, AVND_COMP *,
                                 AVND_COMP_PM_REC *);
extern void avnd_comp_pm_rec_del_all(struct avnd_cb_tag *, AVND_COMP *);
extern uint32_t avnd_comp_pm_stop_process(struct avnd_cb_tag *, AVND_COMP *,
                                          AVSV_AMF_PM_STOP_PARAM *,
                                          SaAisErrorT *);
extern uint32_t avnd_comp_pm_start_process(struct avnd_cb_tag *, AVND_COMP *,
                                           AVSV_AMF_PM_START_PARAM *,
                                           SaAisErrorT *);
extern uint32_t avnd_comp_pmstart_modify(struct avnd_cb_tag *,
                                         AVSV_AMF_PM_START_PARAM *,
                                         AVND_COMP_PM_REC *, SaAisErrorT *);
extern AVND_COMP_PM_REC *avnd_comp_new_rsrc_mon(struct avnd_cb_tag *,
                                                AVND_COMP *,
                                                AVSV_AMF_PM_START_PARAM *,
                                                SaAisErrorT *);
extern bool avnd_comp_pm_rec_cmp(AVSV_AMF_PM_START_PARAM *, AVND_COMP_PM_REC *);
extern uint32_t avnd_evt_ava_pm_start_evh(struct avnd_cb_tag *,
                                          struct avnd_evt_tag *);
extern uint32_t avnd_evt_ava_pm_stop_evh(struct avnd_cb_tag *,
                                         struct avnd_evt_tag *);
extern void avnd_comp_pm_param_val(struct avnd_cb_tag *, AVSV_AMF_API_TYPE,
                                   uint8_t *, AVND_COMP **, AVND_COMP_PM_REC **,
                                   SaAisErrorT *);
extern void avnd_comp_pm_finalize(struct avnd_cb_tag *, AVND_COMP *,
                                  SaAmfHandleT);
...
extern uint32_t avnd_comp_am_oper_req_process(struct avnd_cb_tag *,
                                              AVND_COMP *);
extern uint32_t avnd_comp_amstop_clc_res_process(struct avnd_cb_tag *,
                                                 AVND_COMP *,
                                                 NCS_OS_PROC_EXEC_STATUS);
extern uint32_t avnd_comp_amstart_clc_res_process(struct avnd_cb_tag *,
                                                  AVND_COMP *,
                                                  NCS_OS_PROC_EXEC_STATUS);
...
extern uint32_t avnd_comp_hc_rec_start(struct avnd_cb_tag *, AVND_COMP *,
                                       AVND_COMP_HC_REC *);
...
extern uint32_t avnd_comp_unreg_prc(struct avnd_cb_tag *, AVND_COMP *,
                                    AVND_COMP *);
...
extern void comp_reset_restart_count(const struct avnd_cb_tag *cb,
                                     AVND_COMP *comp);
...
extern AVND_COMP *avnd_compdb_rec_get(AmfDb<std::string, AVND_COMP> &compdb,
                                      const std::string &name);
extern AVND_COMP *avnd_compdb_rec_get_next(
    AmfDb<std::string, AVND_COMP> &compdb, const std::string &name);
...
uint32_t avnd_comp_proxy_status_avd_sync(struct avnd_cb_tag *cb,
                                         AVND_COMP *comp);
