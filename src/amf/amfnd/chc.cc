...
static uint32_t avnd_comp_hc_rec_process(AVND_CB *, AVND_COMP *,
                                         AVND_COMP_HC_REC *, AVND_COMP_HC_OP,
                                         SaAisErrorT);

static uint32_t avnd_comp_hc_rec_stop(AVND_CB *, AVND_COMP *,
                                      AVND_COMP_HC_REC *);

static uint32_t avnd_comp_hc_rec_confirm(AVND_CB *, AVND_COMP *,
                                         AVND_COMP_HC_REC *, SaAisErrorT);

static uint32_t avnd_comp_hc_rec_tmr_exp(AVND_CB *, AVND_COMP *,
                                         AVND_COMP_HC_REC *);
...
uint32_t avnd_evt_ava_hc_start_evh(AVND_CB *cb, AVND_EVT *evt) {
  ...
  avnd_comp_hc_param_val(cb, AVSV_AMF_HC_START, (uint8_t *)hc_start, &comp, 0,
                         &amf_rc);
  TRACE_1("sending response:%d", amf_rc);
  /* send the response back to AvA */
  rc = avnd_amf_resp_send(cb, AVSV_AMF_HC_START, amf_rc, 0, &api_info->dest,
                          &evt->mds_ctxt, comp, msg_from_avnd);

  /* now proceeed with the rest of the processing */
  if ((SA_AIS_OK == amf_rc) && (NCSCC_RC_SUCCESS == rc)) {
    if ((0 !=
         (rec = avnd_comp_hc_rec_add(cb, comp, hc_start, &api_info->dest)))) {
      rc = avnd_comp_hc_rec_process(cb, comp, rec, AVND_COMP_HC_START,
                                    static_cast<SaAisErrorT>(0));
    } else
      rc = NCSCC_RC_FAILURE;
  }
  ...
  if (NCSCC_RC_SUCCESS != rc) {
    LOG_ER("avnd_evt_ava_hc_start():%s:Hdl=%llx,InvType: %u, Err Rcvr:%u",
           osaf_extended_name_borrow(&hc_start->comp_name), hc_start->hdl,
           hc_start->inv_type, hc_start->rec_rcvr.raw);
  }
  TRACE_LEAVE();
  return rc;
}
...
uint32_t avnd_evt_ava_hc_stop_evh(AVND_CB *cb, AVND_EVT *evt) {
  ...
  avnd_comp_hc_param_val(cb, AVSV_AMF_HC_STOP, (uint8_t *)hc_stop, &comp, &rec,
                         &amf_rc);

  /* send the response back to AvA */
  rc = avnd_amf_resp_send(cb, AVSV_AMF_HC_STOP, amf_rc, 0, &api_info->dest,
                          &evt->mds_ctxt, comp, msg_from_avnd);

  /* stop the healthcheck */
  if ((SA_AIS_OK == amf_rc) && (NCSCC_RC_SUCCESS == rc))
    rc = avnd_comp_hc_rec_process(cb, comp, rec, AVND_COMP_HC_STOP,
                                  static_cast<SaAisErrorT>(0));

done:
  if (NCSCC_RC_SUCCESS != rc) {
    LOG_ER("avnd_evt_ava_hc_stop():%s : Hdl:%llx",
           osaf_extended_name_borrow(&hc_stop->comp_name), hc_stop->hdl);
  }
...
uint32_t avnd_evt_ava_hc_confirm_evh(AVND_CB *cb, AVND_EVT *evt) {
  ...
  avnd_comp_hc_param_val(cb, AVSV_AMF_HC_CONFIRM, (uint8_t *)hc_confirm, &comp,
                         &rec, &amf_rc);

  /* send the response back to AvA */
  rc = avnd_amf_resp_send(cb, AVSV_AMF_HC_CONFIRM, amf_rc, 0, &api_info->dest,
                          &evt->mds_ctxt, comp, msg_from_avnd);
  ...
  if (NCSCC_RC_SUCCESS != rc) {
    LOG_ER("avnd_evt_ava_hc_confirm():%s: Hdl=%llx, hc_res=%u",
           osaf_extended_name_borrow(&hc_confirm->comp_name), hc_confirm->hdl,
           hc_confirm->hc_res);
  }
...
uint32_t avnd_evt_tmr_hc_evh(AVND_CB *cb, AVND_EVT *evt) {
  ...
  if ((0 == (rec = (AVND_COMP_HC_REC *)ncshm_take_hdl(NCS_SERVICE_ID_AVND,
                                                      tmr->opq_hdl))))
    goto done;
  ...
  rc = avnd_comp_hc_rec_process(cb, rec->comp, rec, AVND_COMP_HC_TMR_EXP,
                                static_cast<SaAisErrorT>(0));
...
/****************************************************************************
  ...
  Notes         : None
******************************************************************************/
void avnd_comp_hc_param_val(AVND_CB *cb, AVSV_AMF_API_TYPE api_type,
                            uint8_t *param, AVND_COMP **o_comp,
  ...
  switch (api_type) {
    case AVSV_AMF_HC_START: {
      ...
      if ((*o_comp = avnd_compdb_rec_get(
               cb->compdb, Amf::to_string(&hc_start->comp_name))) == nullptr) {
      ...
      if (m_AVND_COMP_PRES_STATE_IS_UNINSTANTIATED(*o_comp) ||
        ...
        LOG_ER("Component '%s' is not healthy (pres state '%u')",
               osaf_extended_name_borrow(&hc_start->comp_name),
               (*o_comp)->pres);
      ...
      /* non proxy component should not start/stop/confirm health check for any
       * other component */
      if (avsv_cmp_horder_sanamet(&hc_start->comp_name,
                                  &hc_start->proxy_comp_name)) {
        if (!m_AVND_COMP_TYPE_IS_PROXIED(*o_comp) ||
            (*o_comp)->pxy_comp->name.compare(
                Amf::to_string(&hc_start->proxy_comp_name))) {
          LOG_ER("Component '%s' is not a proxy",
                 osaf_extended_name_borrow(&hc_start->proxy_comp_name));
          *o_amf_rc = SA_AIS_ERR_NOT_EXIST;
          return;
        }
      }
      memset(&hlt_chk, 0, sizeof(AVSV_HLT_KEY));
      osaf_extended_name_lend(osaf_extended_name_borrow(&hc_start->comp_name),
                              &hlt_chk.comp_name);
      ...
      if (0 == avnd_hcdb_rec_get(cb, &hlt_chk)) {
        ...
        if (nullptr == avnd_hctypedb_rec_get(cb, (*o_comp)->saAmfCompType,
                                             &hc_start->hc_key)) {
    ...
    case AVSV_AMF_HC_STOP: {
      ...
      /* get the comp */
      if ((*o_comp = avnd_compdb_rec_get(
               cb->compdb, Amf::to_string(&hc_stop->comp_name))) == nullptr) {
        *o_amf_rc = SA_AIS_ERR_NOT_EXIST;
        return;
      }
      ...
      /* non proxy component should not start/stop/confirm health check for any
       * other component */
      if (avsv_cmp_horder_sanamet(&hc_stop->comp_name,
                                  &hc_stop->proxy_comp_name)) {
        if (!m_AVND_COMP_TYPE_IS_PROXIED(*o_comp) ||
            (*o_comp)->pxy_comp->name.compare(
                Amf::to_string(&hc_stop->proxy_comp_name))) {
          *o_amf_rc = SA_AIS_ERR_NOT_EXIST;
          return;
        }
      }
      memset(&hlt_chk, 0, sizeof(AVSV_HLT_KEY));
      osaf_extended_name_lend(osaf_extended_name_borrow(&hc_stop->comp_name),
                              &hlt_chk.comp_name);
    ...
    case AVSV_AMF_HC_CONFIRM: {
      ...
      /* get the comp */
      if ((*o_comp = avnd_compdb_rec_get(
               cb->compdb, Amf::to_string(&hc_confirm->comp_name))) ==
          nullptr) {
        *o_amf_rc = SA_AIS_ERR_NOT_EXIST;
        return;
      }
      ...
      /* non proxy component should not start/stop/confirm health check for any
       * other component */
      if (avsv_cmp_horder_sanamet(&hc_confirm->comp_name,
                                  &hc_confirm->proxy_comp_name)) {
        if (!m_AVND_COMP_TYPE_IS_PROXIED(*o_comp) ||
            (*o_comp)->pxy_comp->name.compare(
                Amf::to_string(&hc_confirm->proxy_comp_name))) {
          *o_amf_rc = SA_AIS_ERR_NOT_EXIST;
          return;
        }
      }
      memset(&hlt_chk, 0, sizeof(AVSV_HLT_KEY));
      osaf_extended_name_lend(osaf_extended_name_borrow(&hc_confirm->comp_name),
                              &hlt_chk.comp_name);
      l_num = hc_confirm->hc_key.keyLen;
      hlt_chk.key_len = l_num;
      memcpy(hlt_chk.name.key, hc_confirm->hc_key.key,
             hc_confirm->hc_key.keyLen);
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
AVND_COMP_HC_REC *avnd_comp_hc_rec_add(AVND_CB *cb, AVND_COMP *comp,
                                       AVSV_AMF_HC_START_PARAM *hc_start,
                                       MDS_DEST *dest) {
  AVND_COMP_HC_REC *rec = 0;
  AVND_HC *hc = 0;
  ...
  if ((hc = avnd_hcdb_rec_get(cb, &hlt_chk)) != nullptr) {
    ...
    /* HC instance did not exist, look for HC type */
    AVND_HCTYPE *hctype =
        avnd_hctypedb_rec_get(cb, comp->saAmfCompType, &hc_start->hc_key);
    if (hctype != nullptr) {
...
void avnd_comp_hc_rec_del_all(AVND_CB *cb, AVND_COMP *comp) {
  ...
  while (0 !=
         (rec = (AVND_COMP_HC_REC *)m_NCS_DBLIST_FIND_FIRST(&comp->hc_list))) {
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uint32_t avnd_comp_hc_rec_process(AVND_CB *cb, AVND_COMP *comp,
                                  AVND_COMP_HC_REC *rec, AVND_COMP_HC_OP op,
                                  SaAisErrorT res) {
  uint32_t rc = NCSCC_RC_SUCCESS;
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uint32_t avnd_comp_hc_rec_start(AVND_CB *cb, AVND_COMP *comp,
                                AVND_COMP_HC_REC *rec) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER();
  ...
  if (m_AVND_COMP_HC_REC_IS_AMF_INITIATED(rec)) {
    ...
    if (NCSCC_RC_SUCCESS == rc) rec->status = AVND_COMP_HC_STATUS_WAIT_FOR_RESP;
  }

  /* now start the periodic timer (relevant both for comp-initiated &
   * amf-initiated) */
  if (NCSCC_RC_SUCCESS == rc) {
...
void avnd_comp_hc_cmd_restart(AVND_COMP *comp) {
  ...
  /* This may come because of script responding after component was terminated
   * by lock-in. */
  ...
  /*
     ...
     command is in progress then its response may come during the termination
     phase . If comp is not found instantiated then do not start health check
     timer again.  Exit status of any pending Terminate/cleanup scripts will
     take care of comp's fault management.
   */
  if (!m_AVND_COMP_PRES_STATE_IS_INSTANTIATED(comp)) {
    TRACE_1("'%s' not instantiated, not starting Command based HC timer",
            comp->name.c_str());
    rec->status = AVND_COMP_HC_STATUS_STABLE;
    return;
  }
  if (rec->status == AVND_COMP_HC_STATUS_SND_TMR_EXPD) {
    uint32_t rc = avnd_start_tmr(avnd_cb, &rec->tmr, AVND_TMR_HC, rec->period,
                                 rec->opq_hdl);
    osafassert(rc == NCSCC_RC_SUCCESS);
    rec->status = AVND_COMP_HC_STATUS_STABLE;
  } else
...
void avnd_comp_hc_cmd_stop(AVND_CB *cb, AVND_COMP *comp) {
  ...
  if (rec != nullptr) avnd_stop_tmr(cb, &rec->tmr);
}
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uint32_t avnd_comp_hc_rec_confirm(AVND_CB *cb, AVND_COMP *comp,
                                  AVND_COMP_HC_REC *rec, SaAisErrorT res) {
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uint32_t avnd_comp_hc_rec_tmr_exp(AVND_CB *cb, AVND_COMP *comp,
                                  AVND_COMP_HC_REC *rec) {
  AVND_ERR_INFO err_info;
  uint32_t rc = NCSCC_RC_SUCCESS;

  TRACE_ENTER2("%s - %s, sts: %u", comp->name.c_str(), rec->key.key,
               rec->status);
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
void avnd_comp_hc_finalize(AVND_CB *cb, AVND_COMP *comp, SaAmfHandleT hdl,
                           MDS_DEST *dest) {
  ...
  while (curr) {
    ...
    if ((curr->req_hdl == hdl) &&
      ...
      curr =
          (prv)
              ? (AVND_COMP_HC_REC *)m_NCS_DBLIST_FIND_NEXT(&prv->comp_dll_node)
