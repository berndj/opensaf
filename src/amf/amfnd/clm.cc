...
static void clm_node_left(SaClmNodeIdT node_id) {
  ...
  for (comp = avnd_compdb_rec_get_next(avnd_cb->internode_avail_comp_db, name);
       comp != nullptr; comp = avnd_compdb_rec_get_next(
                            avnd_cb->internode_avail_comp_db, name)) {
    name = comp->name;
    /* Check the node id */
      ...
      if (m_AVND_COMP_TYPE_IS_PROXIED(comp)) {
        ...
        while (pxd_rec) {
          ...
          if (NCSCC_RC_SUCCESS != rc) {
            LOG_ER("avnd_evt_avd_node_update_msg:Unreg failed: %s",
                   curr_rec->pxied_comp->name.c_str());
          }
        } /* while */
      }   /* else if(m_AVND_COMP_TYPE_IS_PROXY(comp)) */
...
static void clm_to_amf_node(void) {
  ...
  while (immutil_saImmOmSearchNext_2(searchHandle, &amfdn,
                                     (SaImmAttrValuesT_2 ***)&attributes) ==
         SA_AIS_OK) {
    if (immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfNodeClmNode"),
                        attributes, 0, &clmdn) == SA_AIS_OK) {
      std::string clm_node(Amf::to_string(&clmdn));

      if (clm_node.compare(
              osaf_extended_name_borrow(&avnd_cb->node_info.nodeName)) == 0) {
...
SaAisErrorT avnd_start_clm_init_bg() {
  ...
  if (pthread_create(&thread, &attr, avnd_clm_init_thread, avnd_cb) != 0) {
