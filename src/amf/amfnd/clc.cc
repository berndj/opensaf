...
static uint32_t avnd_comp_clc_restart_restart_hdler(AVND_CB *cb,
                                                    AVND_COMP *comp);
...
uint32_t avnd_comp_clc_st_chng_prc(AVND_CB *, AVND_COMP *, SaAmfPresenceStateT,
                                   SaAmfPresenceStateT);
...
static AVND_COMP_CLC_FSM_FN
    avnd_comp_clc_fsm[][AVND_COMP_CLC_PRES_FSM_EV_MAX - 1] = {
...
static const char *pres_state_evt[] = {"OUT_OF_RANGE",
...
static const char *clc_cmd_type[] = {"OUT_OF_RANGE",
...
uint32_t avnd_evt_clc_resp_evh(AVND_CB *cb, AVND_EVT *evt) {
  ...
  switch (clc_evt->cmd_type) {
    ...
    case AVND_COMP_CLC_CMD_TYPE_AMSTART:
      rc =
          avnd_comp_amstart_clc_res_process(cb, comp, clc_evt->exec_stat.value);
      break;
...
uint32_t avnd_evt_comp_pres_fsm_evh(AVND_CB *cb, AVND_EVT *evt) {
  ...
  /* get the comp */
  comp =
      avnd_compdb_rec_get(cb->compdb, Amf::to_string(&comp_fsm_evt->comp_name));
  if (!comp) goto done;

  if (all_comps_terminated_in_su(comp->su) == true) is_uninst = true;

  /* run the fsm */
  if (AVND_COMP_CLC_PRES_FSM_EV_MAX != comp_fsm_evt->ev)
    rc = avnd_comp_clc_fsm_run(
        cb, comp, static_cast<AVND_COMP_CLC_PRES_FSM_EV>(comp_fsm_evt->ev));

  if ((is_uninst == true) && (comp->pres == SA_AMF_PRESENCE_INSTANTIATING))
...
/****************************************************************************
  ...
  Notes         : Proxied component registration timer is started when AMF finds
                  ...
                  proxied. It can also be triggered when AMF detects proxy
failure.
******************************************************************************/
...
/****************************************************************************
  ...
  Return Values : NCSCC_RC_SUCCESS/NCSCC_RC_FAILURE
******************************************************************************/
static uint32_t comp_clc_resp_callback(
  ...
  /* create the event */
  AVND_EVT *evt = avnd_evt_create(avnd_cb, AVND_EVT_CLC_RESP, 0, 0, 0, clc_evt,
...
******************************************************************************/
uint32_t avnd_comp_clc_fsm_trigger(AVND_CB *cb, AVND_COMP *comp,
  ...
  /* create the event */
  evt =
      avnd_evt_create(cb, AVND_EVT_COMP_PRES_FSM_EV, 0, 0, 0, 0, &comp_fsm_evt);
  if (!evt) {
    rc = NCSCC_RC_FAILURE;
    goto done;
  }
...
static bool all_app_comps_terminated(void) {
  ...
  for (comp = avnd_compdb_rec_get_next(avnd_cb->compdb, ""); comp != nullptr;
       comp = avnd_compdb_rec_get_next(avnd_cb->compdb, comp->name)) {
...
uint32_t avnd_comp_clc_fsm_run(AVND_CB *cb, AVND_COMP *comp,
    ...
    switch (ev) {
      case AVND_COMP_CLC_PRES_FSM_EV_CLEANUP_SUCC:
        ...
        if (all_app_comps_terminated()) {
          ...
          for (tmp_su = avnd_sudb_rec_get_next(cb->sudb, ""); tmp_su != nullptr;
  ...
  if (prv_st == SA_AMF_PRESENCE_INSTANTIATED &&
    ...
    if (comp->is_hc_cmd_configured) avnd_comp_hc_cmd_stop(cb, comp);
  ...
  if (0 != avnd_comp_clc_fsm[prv_st - 1][ev - 1]) {
    ...
    if (NCSCC_RC_SUCCESS != rc) {
      LOG_NO("Component CLC fsm exited with error for comp:%s",
             comp->name.c_str());
      goto done;
    }
  }
  ...
  /*
      ...
      are cleaned up. So after successful cleanup/termination of restarting
     comp, trigger SU FSM to terminate/cleanup more components or instantiat the
     SU.
   */
  if ((prv_st != final_st) ||
...
uint32_t avnd_comp_clc_st_chng_prc(AVND_CB *cb, AVND_COMP *comp,
  ...
  if (SA_AMF_PRESENCE_RESTARTING == final_st && prv_st != final_st &&
  ...
  if ((SA_AMF_PRESENCE_INSTANTIATED == prv_st) &&
     ...
     */
    if (m_AVND_COMP_TYPE_IS_PROXY(comp)) rc = avnd_comp_proxy_unreg(cb, comp);
  }
  ...
  if (comp->su->is_ncs == true) {
    if (SA_AMF_PRESENCE_INSTANTIATION_FAILED == final_st) {
      ...
      if (comp->su->suMaintenanceCampaign.empty()) {
        ...
        LOG_ER(
            "Amfnd is exiting (due to ncs comp inst failed) to aid fast reboot");
        exit(0);
      } else {
    ...
    if (SA_AMF_PRESENCE_TERMINATION_FAILED == final_st) {
      ...
      if (comp->su->suMaintenanceCampaign.empty()) {
        ...
        LOG_ER(
            "Amfnd is exiting (due to ncs comp term failed) to aid fast reboot");
        exit(0);
      } else {
        LOG_NO("not rebooting as maintenance campaign is ongoing");
        goto done;
  ...
  if (m_AVND_SU_IS_PREINSTANTIABLE(comp->su) &&
      m_AVND_COMP_TYPE_IS_PREINSTANTIABLE(comp)) {
    TRACE("SU and Comp Preinst. comp->su->flag '%u', comp->flag '%u'",
          comp->su->flag, comp->flag);
    ...
    if ((prv_st == SA_AMF_PRESENCE_INSTANTIATED) &&
      ...
      /*
         ...
         will have to mark SU's presence state RESTARTING whenever all the
         components are in Restarting state. This can occur in: -confs with
         single restartable component because of fault with comp-restart
               recovery or RESTART admin op on comp. OR
              -confs with all comp restartable when all comps faults with
         comp-restart recovery. So if I am here because of case a) check if I
         can mark SU RESTARTING.
       */
      if ((isRestartSet(comp->su) == false) &&
          ((m_AVND_COMP_IS_FAILED(comp)) || (comp->admin_oper == true)) &&
          (su_evaluate_restarting_state(comp->su) == true)) {
        TRACE_1(
            "Comp RESTARTING due to comp-restart recovery or RESTART admin op");
    ...
    if ((SA_AMF_PRESENCE_RESTARTING == prv_st) &&
      ...
      /* reassign the comp-csis..
         1)if su-restart recovery is not active which means comp was admin
         restarted or faulted with restart recovery. 2)if comp with restartable
         recovery faulted during the reassignment phase of of surestart
         recovery. This can happen when surestart prob timer expires in the
         surestart recovery phase.
       */
      if ((!m_AVND_SU_IS_RESTART(comp->su)) ||
        ...
        if (NCSCC_RC_SUCCESS != rc) goto done;
        /*
           Mark SU Instantiated when atleast one component moves to instantiated
           state. Single comp restarting case or fault of all restartable comps
           with comp-restart recovery.  For more details read in transition from
           INSTANTIATED to RESTARTING.
         */
        if ((comp->su->pres == SA_AMF_PRESENCE_RESTARTING) &&
            (isRestartSet(comp->su) == false)) {
          TRACE_1(
              "Comp INSTANTIATED due to comp-restart recovery or RESTART admin op");
    ...
    if ((SA_AMF_PRESENCE_TERMINATING == prv_st) &&
      ...
      if (m_AVND_COMP_IS_FAILED(comp) && !comp->csi_list.n_nodes &&
        ...
        /* No need to restart component during shutdown, during surestart
           ...
           For surestart recovery, SU FSM will instantiate all comps after
           successful clean up of all of them.*/
        if (!m_AVND_IS_SHUTTING_DOWN(cb) && !sufailover_in_progress(comp->su) &&
          ...
          rc = avnd_comp_clc_fsm_trigger(cb, comp,
                 ...
                 (comp->su->si_list.n_nodes == 0)) {
        m_AVND_COMP_FAILED_RESET(
            comp); /*if we moved from restart -> term
  ...
  if (m_AVND_SU_IS_PREINSTANTIABLE(comp->su) &&
      !m_AVND_COMP_TYPE_IS_PREINSTANTIABLE(comp)) {
    TRACE(
        "SU Preinst and Comp Non-Preinst. comp->su->flag '%u', comp->flag '%u'",
        comp->su->flag, comp->flag);
    /* instantiating -> instantiated */
    if ((SA_AMF_PRESENCE_INSTANTIATING == prv_st) &&
        (SA_AMF_PRESENCE_INSTANTIATED == final_st)) {
      ...
      if (sufailover_in_progress(comp->su) ||
        ...
        if (!m_AVND_COMP_IS_FAILED(comp)) {
          ...
          if (m_AVND_COMP_CSI_CURR_ASSIGN_STATE_IS_ASSIGNING(csi))
            rc = avnd_comp_csi_assign_done(
                cb, comp, m_AVND_COMP_IS_ALL_CSI(comp) ? 0 : csi);
          else if (m_AVND_COMP_CSI_CURR_ASSIGN_STATE_IS_REMOVING(csi))
            rc = avnd_comp_csi_remove_done(
                cb, comp, m_AVND_COMP_IS_ALL_CSI(comp) ? 0 : csi);
          if (NCSCC_RC_SUCCESS != rc) goto done;
        } else {
    ...
    if ((SA_AMF_PRESENCE_INSTANTIATING == prv_st) &&
      ...
      csi = m_AVND_CSI_REC_FROM_COMP_DLL_NODE_GET(
          m_NCS_DBLIST_FIND_FIRST(&comp->csi_list));
      osafassert(csi);
      avnd_di_susi_resp_send(cb, comp->su,
                             m_AVND_COMP_IS_ALL_CSI(comp) ? 0 : csi->si);
    }
  }
  ...
  if (!m_AVND_SU_IS_PREINSTANTIABLE(comp->su) &&
      !m_AVND_COMP_TYPE_IS_PREINSTANTIABLE(comp)) {
    TRACE("SU and Comp Non-Preinst. comp->su->flag '%u', comp->flag '%u'",
          comp->su->flag, comp->flag);
    /* restarting -> instantiated */
    if ((SA_AMF_PRESENCE_RESTARTING == prv_st) &&
        (SA_AMF_PRESENCE_INSTANTIATED == final_st)) {
      ...
      /*
         Mark SU Instantiated when atleast one component moves to instantiated
         state. Single comp restarting case or fault of all restartable comps
         with comp-restart recovery. Please read detailed explanation in state
         transition for PI comp in PI SU from INSTANTIATED to RESTARTING in this
         function.
       */
      if ((comp->su->pres == SA_AMF_PRESENCE_RESTARTING) &&
          (isRestartSet(comp->su) == false)) {
    ...
    if ((prv_st == SA_AMF_PRESENCE_INSTANTIATED) &&
        (final_st == SA_AMF_PRESENCE_RESTARTING)) {
      // Please read detailed explanation in same state transition for PI comp
      // in PI SU.
  ...
  if (m_AVND_SU_IS_PREINSTANTIABLE(comp->su) &&
    ...
    if (SA_AMF_PRESENCE_INSTANTIATION_FAILED == final_st)
      ...
      /* If sufailover flag is enabled, then SU FSM needs to be triggered in
         ...
         restart admin operation on a non restartable component or SU trigger SU
         FSM.
       */
      ev = AVND_SU_PRES_FSM_EV_COMP_UNINSTANTIATED;
    else if ((final_st == SA_AMF_PRESENCE_RESTARTING) &&
             (prv_st == SA_AMF_PRESENCE_RESTARTING) &&
             (comp->su->admin_op_Id == SA_AMF_ADMIN_RESTART))
      /* If a restartable PI SU consists of NPI comp, then trigger SU FSM to
        terminate other components or start instnatiation.*/
...
uint32_t avnd_comp_clc_uninst_inst_hdler(AVND_CB *cb, AVND_COMP *comp) {
  ...
  if (m_AVND_COMP_TYPE_IS_PROXIED(comp)) {
    if (comp->pxy_comp != nullptr) {
      ...
      m_AVND_TMR_PXIED_COMP_INST_START(
          cb, *comp,
          rc); /* start a timer for proxied instantiating timeout duration */
    }
...
uint32_t avnd_comp_clc_insting_inst_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Instantiate event in the Instantiating state",
               comp->name.c_str());
  ...
  if (m_AVND_COMP_TYPE_IS_PREINSTANTIABLE(comp)) {
    ...
    /* call the proxied instantiate callback, start a timer for callback
     * responce */
    rc = avnd_comp_cbk_send(cb, comp, AVSV_AMF_PXIED_COMP_INST, 0, 0);
  } else {
...
uint32_t avnd_comp_clc_insting_instsucc_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Instantiate success event in the Instantiating state",
               comp->name.c_str());
...
uint32_t avnd_comp_clc_xxxing_instfail_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Instantiate fail event in Instantiating/Restarting State",
               comp->name.c_str());

  /* reset the comp-reg & instantiate params */
  if (!m_AVND_COMP_TYPE_IS_PROXIED(comp)) m_AVND_COMP_REG_PARAM_RESET(cb, comp);
  ...
  if (isRestartSet(comp->su)) {  // SU is restarting (RESTART admin op or
    ...
    if (isFailed(comp->su)) {  // SU is failed (case surestart recovery).
      ...
      if ((comp->pres == SA_AMF_PRESENCE_RESTARTING) &&
        ...
        /* Component got cleaned up in RESTARTING state and surestart recovery
           is going on. In surestart recovery component never enters in
           RESTARTING state. This means component was cleaned up in the context
           of comp-restart recovery. Since further escalation has reached to
           surestart, same cleanup can be used and thus comp can be marked
           uninstantiated.*/
        avnd_comp_pres_state_set(avnd_cb, comp, SA_AMF_PRESENCE_UNINSTANTIATED);
        return false;
      }
    } else {  // Case of RESTART admin op or assignment phase of surestart
              // recovery.
      if (isAdminRestarted(comp->su)) {  // RESTART admin op.
        /*During RESTART admin op on SU, after termination of all components,
          amfnd starts instantiation of components. A component may fault at
          this stage. Such a component is eligible for instantiation.*/
        if ((comp->pres == SA_AMF_PRESENCE_RESTARTING) ||
            (comp->pres == SA_AMF_PRESENCE_INSTANTIATING))
          return true;
      } else {  // Assignment phase during RESTART admin op or during surestart
                // recovery.
        /*After successful instantiation of all the components of SU because of
          either su restart recovery or RESTART admin op on SU, amfnd starts
          reassigning the components in SU. A component may fault during
          reassignment. Such a component is eligible for instantiation.*/
        return true;
      }
    }
...
uint32_t avnd_comp_clc_xxxing_cleansucc_hdler(AVND_CB *cb, AVND_COMP *comp) {
  ...
  // Comp instantiation depends on recovery policy. Check for instantiation
  // eligibility.
  if (is_failed_comp_eligible_for_instantiation(comp) == false) goto done;

  TRACE("inst_retry_cnt:%u, inst_retry_max:%u", clc_info->inst_retry_cnt,
        clc_info->inst_retry_max);
...
uint32_t avnd_comp_clc_inst_term_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Terminate event in the Instantiated state",
               comp->name.c_str());
...
uint32_t avnd_comp_clc_inst_clean_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Cleanup event in the instantiated state",
               comp->name.c_str());
  ...
  if (m_AVND_COMP_TYPE_IS_PROXIED(comp)) {
    ...
    if ((m_AVND_SU_IS_RESTART(comp->su) &&
      ...
      /* If proxy is not in good shape, we can't issue callback, so issue
         cleanup (assume it is local component). */
      rc = avnd_comp_clc_cmd_execute(cb, comp, AVND_COMP_CLC_CMD_TYPE_CLEANUP);
    } else {
    ...
    /* if there are still outstanding proxied components we can't terminate
     * right now */
    /* Check if the proxy and proxied components are in the same SUs. */
    ...
    while (rec) {
      if (comp->su == rec->pxied_comp->su) break;
...
uint32_t avnd_comp_clc_inst_restart_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Restart event in the instantiated state",
               comp->name.c_str());
    ...
    if (m_AVND_COMP_TYPE_IS_PROXIED(comp) &&
      ...
      if (m_AVND_COMP_IS_RESTART_DIS(comp) && (comp->csi_list.n_nodes > 0)) {
        /* A non-restartable assigned healthy component(DisableRestart=1) and
           ...
           first perform reassignment for this component to other SU then term
           it. At present assignment of whole SU will be gracefully reassigned
           instead of only this comp. Thus for PI applications modeled on NWay
           and Nway Active model this is spec deviation.
         */
        /*Send amfd to gracefully remove assignments for thus SU.*/
        su_send_suRestart_recovery_msg(comp->su);
        goto done;
...
uint32_t avnd_comp_clc_inst_orph_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Orphaned event in the Instantiated state",
               comp->name.c_str());
  ...
  if (NCSCC_RC_SUCCESS == rc) {
    avnd_comp_pres_state_set(
        cb, comp, static_cast<SaAmfPresenceStateT>(SA_AMF_PRESENCE_ORPHANED));
  }
...
uint32_t avnd_comp_clc_terming_termfail_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Terminate fail event in the terminating state",
               comp->name.c_str());

  if (m_AVND_COMP_TYPE_IS_PROXIED(comp)) avnd_comp_cbq_del(cb, comp, true);
...
uint32_t avnd_comp_clc_terming_cleansucc_hdler(AVND_CB *cb, AVND_COMP *comp) {
  ...
  if (AVND_TERM_STATE_OPENSAF_SHUTDOWN_STARTED == cb->term_state) {
    ...
    if ((!comp->su->is_ncs) && (comp->csi_list.n_nodes > 0) &&
      ...
      for (csi = m_AVND_CSI_REC_FROM_COMP_DLL_NODE_GET(
        ...
        /* Removal of last CSI from this component may lead to SUSI
           assign/remove done indication, which eventually deletes all COMP-CSI
           record. In such a case there will not be any CSI in comp->csi_list,
  ...
  /*
     Cleanup of failed component is over. If there is some pernding
     component-failover report for AMFD, send it.
   */
  if (m_AVND_COMP_IS_FAILED(comp) && m_AVND_SU_IS_FAILED(su) &&
    ...
    // Reset component-failover here. SU failover is reset as part of REPAIRED
    // admin op.
...
uint32_t avnd_comp_clc_restart_instsucc_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Instantiate success event in the restarting state",
               comp->name.c_str());
...
uint32_t avnd_comp_clc_restart_term_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Terminate event in the restarting state",
               comp->name.c_str());
...
uint32_t avnd_comp_clc_restart_termsucc_hdler(AVND_CB *cb, AVND_COMP *comp) {
  ...
  /*A NPI comp in PI SU can get terminated in other than admin restartop on it
    viz. lock on SI, SG, NODE etc. In these cases this component cannot be
    instantiated. In the remaining case of admin restart on it, it will be
    instantiated. This handler is not invoked when such a componet fails with
    comp restart recovery, in that case cleanup success/fail handler will be
    invoked.
   */
  if (!(m_AVND_COMP_TYPE_IS_PREINSTANTIABLE(comp)) &&
      m_AVND_SU_IS_PREINSTANTIABLE(comp->su) && (comp->admin_oper != true))
    goto done;

  // During restart admin operation on su, all components will be instantiated
  // from SU FSM.
  if (m_AVND_SU_IS_RESTART(comp->su)) {
    goto done;
  }
  ...
  if (m_AVND_COMP_TYPE_IS_PROXIED(comp) && comp->pxy_comp != 0 &&
    ...
    rc =
        avnd_comp_clc_cmd_execute(cb, comp, AVND_COMP_CLC_CMD_TYPE_INSTANTIATE);
...
uint32_t avnd_comp_clc_restart_termfail_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("Terminate fail event in the restarting state:'%s'",
               comp->name.c_str());
...
uint32_t avnd_comp_clc_restart_clean_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Cleanup event in the restarting state",
               comp->name.c_str());
...
uint32_t avnd_comp_clc_orph_instsucc_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Instantiate success event in the Orphaned state",
               comp->name.c_str());
...
uint32_t avnd_comp_clc_orph_term_hdler(AVND_CB *cb, AVND_COMP *comp) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  TRACE_ENTER2("'%s': Terminate event in the Orphaned state",
               comp->name.c_str());
...
/****************************************************************************
  ...
  Notes         : For testing purpose (to let the comp know the node-id),
                  ...
                  derived from HPI.
******************************************************************************/
uint32_t avnd_comp_clc_cmd_execute(AVND_CB *cb, AVND_COMP *comp,
  ...
  if (true == comp->su->su_is_external) {
    if (AVND_COMP_CLC_CMD_TYPE_CLEANUP == cmd_type) {
      ...
      LOG_ER(
          "Command other than cleanup recvd for ext comp: Comp and cmd_type are '%s': %u",
  ...
  /*
  ** 4.3 in B.04 states:
  ** "In case of a non-proxied, non-SA-aware component, the Availability
  *Management Framework passes * the name/value pairs of the component service
  *instance as environment variables to each CLC-CLI * command."
  */
  if (!m_AVND_COMP_TYPE_IS_PREINSTANTIABLE(comp) &&
      !m_AVND_COMP_TYPE_IS_PROXIED(comp)) {
    AVND_COMP_CSI_REC *csi;
  ...
  for (count = 1; count < argc; count++)
    TRACE_1("CLC CLI command arguments[%d] ='%s'", count,
            cmd_info.i_argv[count]);
  ...
  for (count = 0; count < cmd_info.i_set_env_args->num_args; count++)
    TRACE_1("CLC CLI command env variable name = '%s': value ='%s'",
            cmd_info.i_set_env_args->env_arg[count].name,
            cmd_info.i_set_env_args->env_arg[count].value);
...
uint32_t avnd_instfail_su_failover(AVND_CB *cb, AVND_SU *su,
  ...
  TRACE_ENTER2(
      "Executing Component Failover: Instantiation failed SU: '%s' : Failed component: '%s'",
