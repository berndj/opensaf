...
/******************************************************************************
  ...
  Description   : This routine delete NODE_ID to MDS_DEST record for a
particular AvND.
...
AVND_COMP *avnd_internode_comp_add(AVND_CB *cb, const std::string &name,
                                   NODE_ID node_id, uint32_t *rc,
                                   bool pxy_for_ext_comp, bool comp_is_proxy) {
  AVND_COMP *comp = 0;

  *rc = SA_AIS_OK;

  /* verify if this component is already present in the db */
  if ((comp = avnd_compdb_rec_get(avnd_cb->internode_avail_comp_db, name)) !=
      nullptr) {
...
/******************************************************************************
  ...
  Description   : This routine deletes an internode component from
internode_avail_comp_db.
