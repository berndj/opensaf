...
typedef struct avnd_cb_tag {
  ...
  AVND_TMR
      hb_duration_tmr; /* The timer for supervision of heart beats from avd. */

  /* error recovery escalation params */
  AVND_ERR_ESC_LEVEL
      node_err_esc_level;    /* curr escalation level of this node */
  ...
  AmfDb<std::string, AVND_COMP>
      internode_avail_comp_db; /* Internode components, whose node is UP */
