...
AVND_EVT *avnd_evt_create(AVND_CB *cb, AVND_EVT_TYPE type,
                          MDS_SYNC_SND_CTXT *mds_ctxt, MDS_DEST *mds_dest,
                          void *info, AVND_CLC_EVT *clc,
  ...
  switch (type) {
    ...
    case AVND_EVT_MDS_AVA_DN:
      evt->priority = NCS_IPC_PRIORITY_NORMAL; /* keep priority as normal so
                                                  that it doesn't */
    ...
    case AVND_EVT_CLC_RESP:
      ...
      osaf_extended_name_alloc(osaf_extended_name_borrow(&clc->comp_name),
                               &evt->info.clc.comp_name);
      break;
    ...
    case AVND_EVT_COMP_PRES_FSM_EV:
      ...
      osaf_extended_name_alloc(osaf_extended_name_borrow(&comp_fsm->comp_name),
                               &evt->info.comp_fsm.comp_name);
      break;
    ...
    case AVND_EVT_IR:
      ...
      osaf_extended_name_alloc(osaf_extended_name_borrow((SaNameT *)info),
                               &evt->info.ir_evt.su_name);
      break;

    case AVND_EVT_PID_EXIT:
      osaf_extended_name_alloc(((AVND_COMP_PM_REC *)info)->comp->name.c_str(),
                               &evt->info.pm_evt.comp_name);
  ...
  if (!evt) LOG_ER("AvND event creation failed for evt type %u", type);
...
void avnd_evt_destroy(AVND_EVT *evt) {
  ...
  switch (type) {
    ...
    case AVND_EVT_AVND_AVND_MSG:
      if (evt->info.avnd) avsv_nd2nd_avnd_msg_free(evt->info.avnd);
      break;
    /* AvA event types */
    case AVND_EVT_AVA_FINALIZE:
    case AVND_EVT_AVA_COMP_REG:
    ...
    case AVND_EVT_AVA_ERR_CLEAR:
    case AVND_EVT_AVA_RESP:
      if (evt->info.ava.msg) avsv_nda_ava_msg_free(evt->info.ava.msg);
      break;
...
uint32_t avnd_evt_ha_state_change_evh(AVND_CB *cb, AVND_EVT *evt) {
  ...
  if (nullptr == ha_state_event) return rc;
...
error:
  TRACE("evt_type:%u, ha_state:%u ,cb->is_quisced_set:%u, rc:%u are", evt->type,
        ha_state_event->ha_state, cb->is_quisced_set, rc);
