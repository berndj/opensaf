	...
	case AVSV_AVND_AMF_API_RESP_MSG:
		if (msg->info.api_resp_info.type == AVSV_AMF_HA_STATE_GET) {
			osaf_extended_name_free(
			    &msg->info.api_resp_info.param.ha_get.comp_name);
			osaf_extended_name_free(
			    &msg->info.api_resp_info.param.ha_get.csi_name);
		}
	default:
		break;
	}
...
/****************************************************************************
  ...
  Notes         : None
******************************************************************************/
uint32_t avsv_ndnd_avnd_msg_copy(AVSV_ND2ND_AVND_MSG *dmsg,
				 AVSV_ND2ND_AVND_MSG *smsg)
{
...
uint32_t avsv_nda_ava_msg_copy(AVSV_NDA_AVA_MSG *dmsg, AVSV_NDA_AVA_MSG *smsg)
{
	...
	case AVSV_AVND_AMF_API_RESP_MSG:
		if (smsg->info.api_resp_info.type == AVSV_AMF_HA_STATE_GET) {
			osaf_extended_name_alloc(
			    osaf_extended_name_borrow(
				&smsg->info.api_resp_info.param.ha_get
				     .comp_name),
	...
	case AVSV_AVND_AMF_CBK_MSG:
		rc = avsv_amf_cbk_copy(&dmsg->info.cbk_info,
				       smsg->info.cbk_info);
		break;
...
uint32_t avsv_amf_cbk_copy(AVSV_AMF_CBK_INFO **o_dcbk, AVSV_AMF_CBK_INFO *scbk)
{
	...
	*o_dcbk = malloc(sizeof(AVSV_AMF_CBK_INFO));
	...
	case AVSV_AMF_HC:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(&scbk->param.hc.comp_name),
		    &(*o_dcbk)->param.hc.comp_name);
		break;

	case AVSV_AMF_COMP_TERM:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(&scbk->param.comp_term.comp_name),
		    &(*o_dcbk)->param.comp_term.comp_name);
		break;

	case AVSV_AMF_CSI_REM:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(&scbk->param.csi_rem.comp_name),
		    &(*o_dcbk)->param.csi_rem.comp_name);
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(&scbk->param.csi_rem.csi_name),
	...
	case AVSV_AMF_PXIED_COMP_INST:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&scbk->param.pxied_comp_inst.comp_name),
	...
	case AVSV_AMF_PXIED_COMP_CLEAN:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&scbk->param.pxied_comp_clean.comp_name),
		    &(*o_dcbk)->param.pxied_comp_clean.comp_name);
		break;

	case AVSV_AMF_PG_TRACK:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(&scbk->param.pg_track.csi_name),
		...
		if (scbk->param.pg_track.buf.numberOfItems) {
			...
			for (i = 0; i < scbk->param.pg_track.buf.numberOfItems;
				...
				osaf_extended_name_alloc(
				    osaf_extended_name_borrow(
					&scbk->param.pg_track.buf
					     .notification[i]
					     .member.compName),
				    &(*o_dcbk)
					 ->param.pg_track.buf.notification[i]
					 .member.compName);
			}
	...
	case AVSV_AMF_CSI_SET:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(&scbk->param.csi_set.comp_name),
		    &(*o_dcbk)->param.csi_set.comp_name);
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&scbk->param.csi_set.csi_desc.csiName),
		...
		if (scbk->param.csi_set.attrs.number) {
			(*o_dcbk)->param.csi_set.attrs.list =
			    malloc(sizeof(AVSV_ATTR_NAME_VAL) *
		...
		if (scbk->param.csi_set.ha == SA_AMF_HA_ACTIVE) {
			...
			osaf_extended_name_alloc(
			    osaf_extended_name_borrow(
				&scbk->param.csi_set.csi_desc.csiStateDescriptor
				     .activeDescriptor.activeCompName),
			    &(*o_dcbk)
				 ->param.csi_set.csi_desc.csiStateDescriptor
				 .activeDescriptor.activeCompName);
		}
		...
		if (scbk->param.csi_set.csi_desc.csiAttr.number) {
			rc = avsv_amf_csi_attr_list_copy(
	...
	case AVSV_AMF_CSI_ATTR_CHANGE:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&scbk->param.csi_attr_change.csi_name),
		...
		if (scbk->param.csi_attr_change.attrs.number > 0) {
			...
			memcpy((*o_dcbk)->param.csi_attr_change.attrs.list,
				   ...
				   scbk->param.csi_attr_change.attrs.number);
			for (i = 0;
			     i < scbk->param.csi_attr_change.attrs.number;
			     i++) {
				osaf_extended_name_alloc(
				    osaf_extended_name_borrow(
					&scbk->param.csi_attr_change.attrs
					     .list[i]
					     .name),
				    &(*o_dcbk)
					 ->param.csi_attr_change.attrs.list[i]
					 .name);
				osaf_extended_name_alloc(
				    osaf_extended_name_borrow(
					&scbk->param.csi_attr_change.attrs
		...
		if (scbk->param.csi_attr_change.csiAttr.number > 0) {
			avsv_amf_csi_attr_list_copy(
			    &(*o_dcbk)->param.csi_attr_change.csiAttr,
...
void avsv_amf_cbk_free(AVSV_AMF_CBK_INFO *cbk_info)
{
	...
	case AVSV_AMF_PG_TRACK:
		...
		if (cbk_info->param.pg_track.buf.numberOfItems) {
			for (i = 0;
			     i < cbk_info->param.pg_track.buf.numberOfItems;
			     i++) {
				osaf_extended_name_free(
				    &cbk_info->param.pg_track.buf
					 .notification[i]
					 .member.compName);
			}
	...
	case AVSV_AMF_CSI_SET:
		...
		if (cbk_info->param.csi_set.attrs.number) {
			if (cbk_info->param.csi_set.attrs.list) {
				for (i = 0;
				     i < cbk_info->param.csi_set.attrs.number;
				     i++) {
					osaf_extended_name_free(
					    &cbk_info->param.csi_set.attrs
						 .list[i]
						 .name);
					osaf_extended_name_free(
					    &cbk_info->param.csi_set.attrs
		...
		avsv_amf_csi_attr_list_free(
		    &cbk_info->param.csi_set.csi_desc.csiAttr);
		break;
	case AVSV_AMF_CSI_ATTR_CHANGE:
		osaf_extended_name_free(
		    &cbk_info->param.csi_attr_change.csi_name);
		/* free the avsv csi attr list */
		if (cbk_info->param.csi_attr_change.attrs.number) {
			if (cbk_info->param.csi_attr_change.attrs.list) {
				for (i = 0; i < cbk_info->param.csi_attr_change
						    .attrs.number;
				     i++) {
					osaf_extended_name_free(
					    &cbk_info->param.csi_attr_change
						 .attrs.list[i]
						 .name);
					osaf_extended_name_free(
					    &cbk_info->param.csi_attr_change
		...
		/* free the amf csi attr list */
		avsv_amf_csi_attr_list_free(
		    &cbk_info->param.csi_attr_change.csiAttr);
		break;
	default:
		break;
	}
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uint32_t avsv_amf_api_copy(AVSV_AMF_API_INFO *d_api_info,
			   AVSV_AMF_API_INFO *s_api_info)
{
	uint32_t rc = NCSCC_RC_SUCCESS;
	...
	case AVSV_AMF_COMP_REG:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(&s_api_info->param.reg.comp_name),
		    &d_api_info->param.reg.comp_name);
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.reg.proxy_comp_name),
	...
	case AVSV_AMF_COMP_UNREG:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.unreg.comp_name),
		    &d_api_info->param.unreg.comp_name);
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.unreg.proxy_comp_name),
	...
	case AVSV_AMF_PM_START:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.pm_start.comp_name),
	...
	case AVSV_AMF_PM_STOP:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.pm_stop.comp_name),
	...
	case AVSV_AMF_HC_START:
		...
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.hc_start.comp_name),
		    &d_api_info->param.ha_get.comp_name);
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.hc_start.proxy_comp_name),
	...
	case AVSV_AMF_HC_STOP:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.hc_stop.comp_name),
		    &d_api_info->param.hc_stop.comp_name);
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.hc_stop.proxy_comp_name),
	...
	case AVSV_AMF_HC_CONFIRM:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.hc_confirm.comp_name),
		    &d_api_info->param.hc_confirm.comp_name);
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.hc_confirm.proxy_comp_name),
	...
	case AVSV_AMF_CSI_QUIESCING_COMPLETE:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.csiq_compl.comp_name),
	...
	case AVSV_AMF_HA_STATE_GET:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.ha_get.comp_name),
		    &d_api_info->param.ha_get.comp_name);
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.ha_get.csi_name),
	...
	case AVSV_AMF_PG_START:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.pg_start.csi_name),
	...
	case AVSV_AMF_PG_STOP:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.pg_stop.csi_name),
	...
	case AVSV_AMF_ERR_REP:
		osaf_extended_name_alloc(
		    osaf_extended_name_borrow(
			&s_api_info->param.err_rep.err_comp),
 ...
 * SaAmfCSIAttributeListT. All memory allocation will be done here.
 *
				 ...
				 AVSV_CSI_ATTRS *avsv_attrs)
{
	...
	amf_attrs->attr =
	    malloc(sizeof(SaAmfCSIAttributeT) * avsv_attrs->number);
	if (!amf_attrs->attr) {
		rc = NCSCC_RC_FAILURE;
		goto done;
	}
	...
	for (cnt = 0; cnt < avsv_attrs->number; cnt++) {
		/* alloc memory for attr name & value */
		amf_attrs->attr[cnt].attrName = malloc(
		    osaf_extended_name_length(&avsv_attrs->list[cnt].name) + 1);
		...
		amf_attrs->attr[cnt].attrValue =
		    malloc(strlen(avsv_attrs->list[cnt].string_ptr) + 1);
