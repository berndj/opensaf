...
{
	SaImmHandleT omHandle;
	SaNameT objectName = {.value = "logConfig=1,safApp=safLogService",
			      .length =
				  strlen("logConfig=1,safApp=safLogService")};
	SaImmAccessorHandleT accessorHandle;
...
static int init_log_open_stream(SaAisErrorT *ais_rc)
{
	...
	SaNameT stream_name = {.value = STREAM_NAME_STR,
...
static int write_logrecord(int rec_cnt, SaAisErrorT *ais_rc)
{
	 ...
	 * 36 10:23:04 10/24/2014 IN safLgStr=logtestfr_stream "logtestfr test
	 * message"
	 */
	SaNameT logSvcUsrName = {.value = STREAM_NAME_STR,
				 .length = sizeof(STREAM_NAME_STR)};
	 ...
	 * and by that also know that log rotation is done and the old file
	 * exist and is closed.
	 */
	for (i = 0; i < rec_cnt; i++) {
		...
		/* Wait until the log record is written before writing the next
		 * one. Wait for selection object and invoke the callback.
		 */
		poll_rc = poll(fds, 1, 10000); /* 10 sec timeout */
		if (poll_rc <= 0) {
			/* An error or timeout. Could be EAGAIN but we handle
			 * this as an error
			 */
			if (poll_rc == -1) {
				...
				printf_v(
				    "%s Poll timeout when writing log record\n",
				    __FUNCTION__);
			}
			rc = -1;
			goto done;
...
static int truncate_filenames(void)
{
	...
	file_cnt =
	    scandir(file_path, &namelist, rotated_files_filter, alphasort);
	if (file_cnt < 0) {
...
int tst_logrotation_truncated_filename(void)
{
	 ...
	 * created each second. File naming is based on time with a resolution
	 * of 1 sec. This means that new file may get the same name as the
	 * previous causing the previous file to be overwritten! To make sure
	 * this will not happen in this test records are written in groups with
	 * a delay in between
	 */
	...
	steptst(
	    "File rotation after truncation shall have been done, stream still open.\n"
	...
	if (rc < 0) {
		perror(
		    "tst_logrotation_truncated_filename() Failed to cleanup after test");
	}
...
static void usage(void)
{
	...
	printf("\nDESCRIPTION\n");
	printf(
	    "\t%s Tests that the log server can handle when log filenames has\n"
	...
	printf(
	    "  -s                    At each test step stop and wait for user to\n"
