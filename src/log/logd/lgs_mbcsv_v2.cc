...
uint32_t edp_ed_write_rec_v2(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn, NCSCONTEXT ptr,
  ...
  EDU_INST_SET ckpt_write_rec_ed_rules[] = {
      {EDU_START, edp_ed_write_rec_v2, 0, 0, 0, sizeof(lgs_ckpt_write_log_v2_t),
       0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_write_log_v2_t *)0)->recordId, 0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_write_log_v2_t *)0)->streamId, 0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_write_log_v2_t *)0)->curFileSize, 0, NULL},
      {EDU_EXEC, ncs_edp_string, 0, 0, 0,
       (long)&((lgs_ckpt_write_log_v2_t *)0)->logFileCurrent, 0, NULL},
      {EDU_EXEC, ncs_edp_string, 0, 0, 0,
       (long)&((lgs_ckpt_write_log_v2_t *)0)->logRecord, 0, NULL},
      {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
       (long)&((lgs_ckpt_write_log_v2_t *)0)->c_file_close_time_stamp, 0, NULL},
      {EDU_END, 0, 0, 0, 0, 0, 0, NULL},
  };

  if (op == EDP_OP_TYPE_ENC) {
    ckpt_write_msg_ptr = static_cast<lgs_ckpt_write_log_v2_t *>(ptr);
  } else if (op == EDP_OP_TYPE_DEC) {
    ckpt_write_msg_dec_ptr = static_cast<lgs_ckpt_write_log_v2_t **>(ptr);
  ...
  rc =
      m_NCS_EDU_RUN_RULES(edu_hdl, edu_tkn, ckpt_write_rec_ed_rules,
                          ckpt_write_msg_ptr, ptr_data_len, buf_env, op, o_err);
...
uint32_t edp_ed_close_stream_rec_v2(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
                                    ...
                                    EDU_ERR *o_err) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  lgs_ckpt_stream_close_v2_t *ckpt_close_stream_msg_ptr = NULL,
                             **ckpt_close_stream_msg_dec_ptr;

  EDU_INST_SET ckpt_close_stream_rec_ed_rules[] = {
      {EDU_START, edp_ed_close_stream_rec_v2, 0, 0, 0,
       sizeof(lgs_ckpt_stream_close_v2_t), 0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_stream_close_v2_t *)0)->streamId, 0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_stream_close_v2_t *)0)->clientId, 0, NULL},
      {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
       (long)&((lgs_ckpt_stream_close_v2_t *)0)->c_file_close_time_stamp, 0,
       NULL},
      {EDU_END, 0, 0, 0, 0, 0, 0, NULL},
  };

  if (op == EDP_OP_TYPE_ENC) {
    ckpt_close_stream_msg_ptr = static_cast<lgs_ckpt_stream_close_v2_t *>(ptr);
  } else if (op == EDP_OP_TYPE_DEC) {
    ckpt_close_stream_msg_dec_ptr =
        static_cast<lgs_ckpt_stream_close_v2_t **>(ptr);
  ...
  rc = m_NCS_EDU_RUN_RULES(edu_hdl, edu_tkn, ckpt_close_stream_rec_ed_rules,
                           ckpt_close_stream_msg_ptr, ptr_data_len, buf_env, op,
                           o_err);
...
uint32_t edp_ed_finalize_rec_v2(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
                                ...
                                EDU_ERR *o_err) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  lgs_ckpt_finalize_msg_v2_t *ckpt_final_msg_ptr = NULL,
                             **ckpt_final_msg_dec_ptr;

  EDU_INST_SET ckpt_final_rec_ed_rules[] = {
      {EDU_START, edp_ed_finalize_rec_v2, 0, 0, 0,
       sizeof(lgs_ckpt_finalize_msg_v2_t), 0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_finalize_msg_v2_t *)0)->client_id, 0, NULL},
      {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
       (long)&((lgs_ckpt_finalize_msg_v2_t *)0)->c_file_close_time_stamp, 0,
       NULL},
      {EDU_END, 0, 0, 0, 0, 0, 0, NULL},
  };

  if (op == EDP_OP_TYPE_ENC) {
    ckpt_final_msg_ptr = static_cast<lgs_ckpt_finalize_msg_v2_t *>(ptr);
  } else if (op == EDP_OP_TYPE_DEC) {
    ckpt_final_msg_dec_ptr = static_cast<lgs_ckpt_finalize_msg_v2_t **>(ptr);
  ...
  rc =
      m_NCS_EDU_RUN_RULES(edu_hdl, edu_tkn, ckpt_final_rec_ed_rules,
                          ckpt_final_msg_ptr, ptr_data_len, buf_env, op, o_err);
...
uint32_t edp_ed_cfg_stream_rec_v2(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
                                  ...
                                  EDU_ERR *o_err) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  lgs_ckpt_stream_cfg_v2_t *ckpt_stream_cfg_msg_ptr = NULL,
                           **ckpt_stream_cfg_msg_dec_ptr;

  EDU_INST_SET ckpt_stream_cfg_rec_ed_rules[] = {
      {EDU_START, edp_ed_cfg_stream_rec_v2, 0, 0, 0,
       sizeof(lgs_ckpt_stream_cfg_v2_t), 0, NULL},
      {EDU_EXEC, ncs_edp_string, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->name, 0, NULL},
      {EDU_EXEC, ncs_edp_string, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->fileName, 0, NULL},
      {EDU_EXEC, ncs_edp_string, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->pathName, 0, NULL},
      {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->maxLogFileSize, 0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->fixedLogRecordSize, 0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->haProperty, 0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->logFullAction, 0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->logFullHaltThreshold, 0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->maxFilesRotated, 0, NULL},
      {EDU_EXEC, ncs_edp_string, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->logFileFormat, 0, NULL},
      {EDU_EXEC, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->severityFilter, 0, NULL},
      {EDU_EXEC, ncs_edp_string, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->logFileCurrent, 0, NULL},
      {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
       (long)&((lgs_ckpt_stream_cfg_v2_t *)0)->c_file_close_time_stamp, 0,
       NULL},
      {EDU_END, 0, 0, 0, 0, 0, 0, NULL},
  };

  if (op == EDP_OP_TYPE_ENC) {
    ckpt_stream_cfg_msg_ptr = static_cast<lgs_ckpt_stream_cfg_v2_t *>(ptr);
  } else if (op == EDP_OP_TYPE_DEC) {
    ckpt_stream_cfg_msg_dec_ptr = static_cast<lgs_ckpt_stream_cfg_v2_t **>(ptr);
...
uint32_t edp_ed_lgs_cfg_rec_v2(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
                               ...
                               EDU_ERR *o_err) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  lgs_ckpt_lgs_cfg_v2_t *ckpt_lgs_cfg_msg_ptr = NULL,
                        **ckpt_lgs_cfg_msg_dec_ptr;

  EDU_INST_SET ckpt_lgs_cfg_rec_ed_rules[] = {
      {EDU_START, edp_ed_lgs_cfg_rec_v2, 0, 0, 0, sizeof(lgs_ckpt_lgs_cfg_v2_t),
       0, NULL},
      {EDU_EXEC, ncs_edp_string, 0, 0, 0,
       (long)&((lgs_ckpt_lgs_cfg_v2_t *)0)->logRootDirectory, 0, NULL},
      {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
       (long)&((lgs_ckpt_lgs_cfg_v2_t *)0)->c_file_close_time_stamp, 0, NULL},
      {EDU_END, 0, 0, 0, 0, 0, 0, NULL},
  };

  if (op == EDP_OP_TYPE_ENC) {
    ckpt_lgs_cfg_msg_ptr = static_cast<lgs_ckpt_lgs_cfg_v2_t *>(ptr);
  } else if (op == EDP_OP_TYPE_DEC) {
    ckpt_lgs_cfg_msg_dec_ptr = static_cast<lgs_ckpt_lgs_cfg_v2_t **>(ptr);
...
uint32_t edp_ed_agent_down_rec_v2(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
                                  ...
                                  EDU_ERR *o_err) {
  uint32_t rc = NCSCC_RC_SUCCESS;
  lgs_ckpt_agent_down_v2_t *ckpt_agent_down_msg_ptr = NULL,
                           **ckpt_agent_down_dec_ptr;

  EDU_INST_SET ckpt_lgs_agent_down_ed_rules[] = {
      {EDU_START, edp_ed_agent_down_rec_v2, 0, 0, 0,
       sizeof(lgs_ckpt_agent_down_v2_t), 0, NULL},
      {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
       (long)&((lgs_ckpt_agent_down_v2_t *)0)->agent_dest, 0, NULL},
      {EDU_EXEC, ncs_edp_uns64, 0, 0, 0,
       (long)&((lgs_ckpt_agent_down_v2_t *)0)->c_file_close_time_stamp, 0,
       NULL},
      {EDU_END, 0, 0, 0, 0, 0, 0, NULL},
  };

  if (op == EDP_OP_TYPE_ENC) {
    ckpt_agent_down_msg_ptr = static_cast<lgs_ckpt_agent_down_v2_t *>(ptr);
  } else if (op == EDP_OP_TYPE_DEC) {
    ckpt_agent_down_dec_ptr = static_cast<lgs_ckpt_agent_down_v2_t **>(ptr);
  ...
  rc = m_NCS_EDU_RUN_RULES(edu_hdl, edu_tkn, ckpt_lgs_agent_down_ed_rules,
                           ckpt_agent_down_msg_ptr, ptr_data_len, buf_env, op,
                           o_err);
  return rc;
}
...
uint32_t edp_ed_ckpt_msg_v2(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn, NCSCONTEXT ptr,
  ...
  EDU_INST_SET ckpt_msg_ed_rules[] = {
      {EDU_START, edp_ed_ckpt_msg_v2, 0, 0, 0, sizeof(lgsv_ckpt_msg_v2_t), 0,
       NULL},
      {EDU_EXEC, edp_ed_header_rec, 0, 0, 0,
       (long)&((lgsv_ckpt_msg_v2_t *)0)->header, 0, NULL},

      {EDU_TEST, ncs_edp_uns32, 0, 0, 0,
       (long)&((lgsv_ckpt_msg_v2_t *)0)->header, 0,
      ...
      /* Lgs cfg */
      {EDU_EXEC, edp_ed_lgs_cfg_rec_v2, 0, 0, static_cast<int>(EDU_EXIT),
  ...
  if (op == EDP_OP_TYPE_ENC) {
    ckpt_msg_ptr = static_cast<lgsv_ckpt_msg_v2_t *>(ptr);
  } else if (op == EDP_OP_TYPE_DEC) {
    ckpt_msg_dec_ptr = static_cast<lgsv_ckpt_msg_v2_t **>(ptr);
