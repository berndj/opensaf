...
static int send_stream_open_msg(uint32_t *lstream_id,
  ...
  TRACE_ENTER();
  osafassert(p_stream->log_stream_name != nullptr &&
             "log_stream_name is nullptr");
...
static void *recovery2_thread(void *dummy) {
  ...
  while (p_client != nullptr && p_client->recovered_flag == false) {
