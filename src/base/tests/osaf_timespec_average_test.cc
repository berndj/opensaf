...
static const uint64_t NANOS_PER_SEC = kNanosPerSec;
...
TEST(OsafTimespecAverage, OneSecond) {
  ...
  osaf_timespec_average(&time1, &time2, &average);
  EXPECT_EQ(average.tv_sec, 0);
  EXPECT_EQ(average.tv_nsec, NANOS_PER_SEC / 2);
}

TEST(OsafTimespecAverage, AlmostOneSecond) {
  struct timespec time1 = {0, NANOS_PER_SEC - 1};
  struct timespec time2 = {0, NANOS_PER_SEC - 1};
  struct timespec average = {1, 1};
  osaf_timespec_average(&time1, &time2, &average);
  EXPECT_EQ(average.tv_sec, 0);
  EXPECT_EQ(average.tv_nsec, NANOS_PER_SEC - 1);
}

TEST(OsafTimespecAverage, AlmostTwoSeconds) {
  struct timespec time1 = {1, NANOS_PER_SEC - 1};
  struct timespec time2 = {0, NANOS_PER_SEC - 1};
  struct timespec average = {1, 1};
  osaf_timespec_average(&time1, &time2, &average);
  uint64_t expected_ns = (NANOS_PER_SEC + 2 * (NANOS_PER_SEC - 1)) / 2;
...
TEST(OsafTimespecAverage, SimpleCase) {
  ...
  uint64_t expected_ns =
      (222 * NANOS_PER_SEC + 111 * NANOS_PER_SEC + 1234 + 5678) / 2;
  struct timespec expected;
  osaf_nanos_to_timespec(expected_ns, &expected);
  EXPECT_EQ(average.tv_sec, expected.tv_sec);
  EXPECT_EQ(average.tv_nsec, expected.tv_nsec);
...
TEST(OsafTimespecAverage, NanosOverflow) {
  struct timespec time1 = {10, NANOS_PER_SEC / 2};
  struct timespec time2 = {11, NANOS_PER_SEC / 2};
  struct timespec average = {1, 1};
  osaf_timespec_average(&time1, &time2, &average);
  uint64_t expected_ns =
      (10 * NANOS_PER_SEC + 11 * NANOS_PER_SEC + NANOS_PER_SEC) / 2;
  struct timespec expected;
  osaf_nanos_to_timespec(expected_ns, &expected);
  EXPECT_EQ(average.tv_sec, expected.tv_sec);
  EXPECT_EQ(average.tv_nsec, expected.tv_nsec);
...
TEST(OsafTimespecAverage, Max) {
  ...
  struct timespec time1 = {max_time, NANOS_PER_SEC - 1};
  struct timespec time2 = {max_time, NANOS_PER_SEC - 1};
  struct timespec average = {1, 1};
  osaf_timespec_average(&time1, &time2, &average);
...
TEST(OsafTimespecAverage, MaxAndOneSecond) {
  ...
  osaf_timespec_average(&time1, &time2, &average);
  EXPECT_EQ(average.tv_sec, max_time / 2 + 1);
  EXPECT_EQ(average.tv_nsec, NANOS_PER_SEC / 2 - 1);
}
