...
#define TMR_DBG_SET(d, f, l)                                                   \
	{                                                                      \
		d.isa = TMR_ISA;                                               \
		d.file = f;                                                    \
		d.line = l;                                                    \
	}
#define TMR_DBG_ASSERT_ISA(d)                                                  \
	{                                                                      \
		if (d.isa != TMR_ISA)                                          \
...
static uint32_t ncs_tmr_add_pat_node(SYSF_TMR *tmr)
{
	...
	temp_tmr_pat_node = (SYSF_TMR_PAT_NODE *)ncs_patricia_tree_get(
	    &gl_tcb.tmr_pat_tree, (uint8_t *)&tmr->key);

	if (temp_tmr_pat_node == (SYSF_TMR_PAT_NODE *)NULL) {
		temp_tmr_pat_node = (SYSF_TMR_PAT_NODE *)m_NCS_MEM_ALLOC(
		    sizeof(SYSF_TMR_PAT_NODE), NCS_MEM_REGION_PERSISTENT,
		    NCS_SERVICE_ID_LEAP_TMR, 0);
		memset(temp_tmr_pat_node, '\0', sizeof(SYSF_TMR_PAT_NODE));
		temp_tmr_pat_node->key = tmr->key;
		temp_tmr_pat_node->pat_node.key_info =
		    (uint8_t *)&temp_tmr_pat_node->key;
	...
	if (temp_tmr_pat_node->tmr_list_start == NULL) {
		temp_tmr_pat_node->tmr_list_end =
		    temp_tmr_pat_node->tmr_list_start = tmr;
	} else {
...
static uint64_t get_time_elapsed_in_ticks(struct timespec *temp_ts_start)
{
	...
	time_elapsed =
	    ((((ts_current.tv_sec - temp_ts_start->tv_sec) * (1000LL)) +
	      ((ts_current.tv_nsec - temp_ts_start->tv_nsec) / 1000000)) /
	     NCS_MILLISECONDS_PER_TICK);
...
static bool sysfTmrExpiry(SYSF_TMR_PAT_NODE *tmp)
{
	...
	while (dead_tmr->next != NULL) { /* process old and new */
		...
		if ((TMR_TEST_STATE(now_tmr, TMR_STATE_DORMANT)) ||
			...
			TMR_SET_STATE(now_tmr,
				      TMR_STATE_DORMANT); /* mark it dormant */
			...
			if (now_tmr->tmrCB != ((TMR_CALLBACK)0x0ffffff)) {
				...
				if (gl_tcb.stats.cnt == 0) {
					syslog(
					    LOG_INFO,
					    "NO Timers Active in Expiry PID %u \n",
					    getpid());
				}
#endif
				now_tmr
	...
	if (start_dead->next != NULL) {
		m_NCS_LOCK(&gl_tcb.safe.free_lock,
			   NCS_LOCK_WRITE); /* critical region START */
		dead_tmr->next =
		    gl_tcb.safe.dmy_free
			.next; /* append old free list to end of that  */
		gl_tcb.safe.dmy_free.next =
		    start_dead->next; /* put start of collected dead in front */
		m_NCS_UNLOCK(&gl_tcb.safe.free_lock,
			     NCS_LOCK_WRITE); /* critical region END */
	}
...
static uint32_t ncs_tmr_engine(struct timeval *tv, uint64_t *next_delay)
{
	...
	uint64_t sum_of_tmr_exp_gaps =
	    0; /* Avg = sum_of_tmr_exp_gaps/tot_tmr_exp */
#endif

	while (true) {
		tmp = (SYSF_TMR_PAT_NODE *)ncs_patricia_tree_getnext(
		    &gl_tcb.tmr_pat_tree, (uint8_t *)NULL);
		...
		if (ticks_elapsed >= next_expiry) {
#if ENABLE_SYSLOG_TMR_STATS
			if (tot_tmr_exp != 0) {
				tmr_exp_curr_start.tv_sec =
				    tmr_exp_curr_start.tv_nsec = 0;
				if (clock_gettime(CLOCK_MONOTONIC,
						  &tmr_exp_curr_start)) {
					perror(
					    "clock_gettime with MONOTONIC Failed \n");
				}
				sum_of_tmr_exp_gaps += TIMESPEC_DIFF_IN_NS(
				    tmr_exp_curr_start, tmr_exp_prev_finish);
				if (tot_tmr_exp >= 100) {
					printf("\nTotal active timers: %d\n",
					       gl_tcb.stats.cnt);
					printf(
					    "Average timer-expiry gap (last %d expiries) = %lld\n",
					    tot_tmr_exp,
					    sum_of_tmr_exp_gaps / tot_tmr_exp);
					tot_tmr_exp = 0;
			...
			if (true ==
			    sysfTmrExpiry(tmp)) { /* call expiry routine */
				return NCSCC_RC_FAILURE;
			}
#if ENABLE_SYSLOG_TMR_STATS
			tot_tmr_exp++;
			tmr_exp_prev_finish.tv_sec =
			    tmr_exp_prev_finish.tv_nsec = 0;
			if (clock_gettime(CLOCK_MONOTONIC,
					  &tmr_exp_prev_finish)) {
				perror(
				    "clock_gettime with MONOTONIC Failed \n");
	...
	tv->tv_usec = (((*next_delay) % (1000 / NCS_MILLISECONDS_PER_TICK)) *
		       (1000 * NCS_MILLISECONDS_PER_TICK));
...
static uint32_t ncs_tmr_wait(void)
{
	...
	while (true) {
		...
		unsigned rc =
		    osaf_ppoll(&set, 1, next_delay != 0 ? &ts : NULL, NULL);
		m_NCS_LOCK(&gl_tcb.safe.enter_lock, NCS_LOCK_WRITE);

		if (rc == 1) {
			if (set.revents != POLLIN)
				osaf_abort(set.revents);
			...
			if (gl_tcb.msg_count == 0) {
				inds_rmvd = m_NCS_SEL_OBJ_RMV_IND(
				    &gl_tcb.sel_obj, true, true);
				if (inds_rmvd <= 0) {
					if (inds_rmvd != -1) {
						/* The object has not been
						   destroyed and it has no
						   indication raised on it
						   inspite of msg_count being
						   non-zero.
						 */
						m_NCS_UNLOCK(
						    &gl_tcb.safe.enter_lock,
						    NCS_LOCK_WRITE);
...
bool sysfTmrCreate(void)
{
	...
	if (m_NCS_TASK_CREATE((NCS_OS_CB)ncs_tmr_wait, 0, (char *)"OSAF_TMR",
...
bool sysfTmrDestroy(void)
{
	...
	while ((tmp = (SYSF_TMR_PAT_NODE *)ncs_patricia_tree_getnext(
		    &gl_tcb.tmr_pat_tree, (uint8_t *)0)) != NULL) {
		ncs_patricia_tree_del(&gl_tcb.tmr_pat_tree,
...
tmr_t ncs_tmr_alloc(char *file, uint32_t line)
{
	...
	if (tmr == NULL) {
		...
		if (tmr == NULL)
			...
			tmr->keep =
			    gl_tcb.safe.dmy_keep.keep; /* put it on keep list */
...
tmr_t ncs_tmr_start(tmr_t tid,
		    int64_t tmrDelay, /* timer period in number of 10ms units */
		    TMR_CALLBACK tmrCB, void *tmrUarg, char *file,
		    uint32_t line)
{
	SYSF_TMR *tmr;
	SYSF_TMR *new_tmr;
	...
	if (((tmr = (SYSF_TMR *)tid) == NULL) ||
	    (tmr_destroying == true)) /* NULL tmrs are no good! */
		return NULL;
	...
	if (TMR_TEST_STATE(
		tmr, TMR_STATE_DORMANT)) { /* If client is re-using timer */
		m_NCS_TMR_CREATE(new_tmr, tmrDelay, tmrCB,
				 tmrUarg); /* get a new one */
		if (new_tmr == NULL) {
			return NULL;
		}
	...
	/*
	   ...
	   3)See whether patricia Node is already present in patricia Tree with
	   the caliculated Key If not present Create patricia Node and add it
	   the patricia tree If present Add the tmr data structure to the double
	   linked list as FIFO. i.e adds at the last of the double linked list.
 ...
 *     NCSCC_RC_SUCCESS    :    This timer START state and is set to DORMANT
 *state NCSCC_RC_TMR_STOPPED:    If the timer is already in DORMANT state
 *
 ****************************************************************************/
uint32_t ncs_tmr_stop_v2(tmr_t tmrID, void **o_tmr_arg)
{
	...
	m_NCS_LOCK(&gl_tcb.safe.enter_lock,
		   NCS_LOCK_WRITE); /* critical region START */
...
static char *gl_tmr_states[] = {"illegal", "CREATE",  "START",
				"illegal", "DORMANT", "illegal",
				"illegal", "illegal", "DESTROY"};
...
uint32_t ncs_tmr_whatsout(void)
{
	...
	free =
	    gl_tcb.safe.dmy_keep.keep; /* get start of keep list; may be NULL */
	while (free != NULL) {
		if (!TMR_TEST_STATE(free,
				    TMR_STATE_DESTROY)) { /* some may be in
							     transition, but
							     report all */
...
uint32_t ncs_tmr_getstats(void) { return NCSCC_RC_SUCCESS; }
