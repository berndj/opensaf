...
static SaAisErrorT sqlite_prepare_ccb(
  ...
  if (!pbeTransStarted()) {
    LOG_ER(
        "sqlite_prepare_ccb invoked when no sqlite transaction has been started.");
    abort();
  }
    ...
    switch (ccbUtilOperationData->operationType) {
      case CCBUTIL_CREATE:
        do {
          ...
          if (!objectToPBE(std::string(osaf_extended_name_borrow(
                           ...
                           ccbId)) {
            LOG_ER(
                "objectToPBE failed in sqlite_prepare_ccb. Handle is closed - exiting");
            exit(1);
          }
        } while (0);
        break;
      ...
      case CCBUTIL_DELETE:
        ...
        objectDeleteToPBE(std::string(osaf_extended_name_borrow(
                              ccbUtilOperationData->param.delete_.objectName)),
                          sDbHandle);
      ...
      case CCBUTIL_MODIFY:
        /* Note: it is important that the code in this MODIFY case follow
           ...
           This is not the only way to solve this. In fact the current solution
           is very unoptimal since it generates possibly several sql commands
           for what could be one. The advantage with the current solution is
           that it follows the logic of the ImmModel and can do so using the
           data provided by the unmodified ccbObjectModify upcall.
         */
        TRACE("Modify of object with DN: %s",
              osaf_extended_name_borrow(
                  ccbUtilOperationData->param.modify.objectName));

        /* Pbe receives canonicalized attrMod for config-object modifications
         * (CCB) but it still receive normal attrMod for runtime-object
         * modifications (updating PRTA). So we have to support all kinds of
         * SaImmAttrModificationTypeT here.
         */
        objName.append(osaf_extended_name_borrow(
            ccbUtilOperationData->param.modify.objectName));
          ...
          switch (attMod->modType) {
            case SA_IMM_ATTR_VALUES_REPLACE:
              objectModifyDiscardAllValuesOfAttrToPBE(
                  sDbHandle, objName, &(attMod->modAttr), ccbId);
            ...
            case SA_IMM_ATTR_VALUES_DELETE:
              ...
              objectModifyDiscardMatchingValuesOfAttrToPBE(
                  sDbHandle, objName, &(attMod->modAttr), ccbId);
...
/* 2PBE: Note potential
   concurrency problem towards sqlite here, between the main thread and the
   runtime-object thread. Sqlite is supposed to be threadsafe, but we dont want
   to rely on that. The actual risk for interference should be small and the
   consequences of a conflict must be non catastrophic. First, the risk is zero
   at the primary PBE because it (should!) only be executing sqlite from the
   main thread. The runtime object thread is hardly used at the primary. At the
   slave, the opposite is (almost) true. The RTO thread will execute all sqlite
   access, except for one important case, explained below. Second,
   pbeBeginTrans() and pbeCommitTrans() implement a spinlock, see
   osaf/libs/common/immsv/immpbe_dump.cc. Thus interference is detected before
   any transactional operations are allowed to begin. Third, at the slave, the
   only sqlite operation performed by the main thread is the commit of an sqlite
   transaction for a regular CCB. The start of the sqlite transaction for a
   regular CCB and all the buildup of the transsaction is handled by the RTO
   thread at the slave. Note the main thread, as an applier will collect all the
   operations into the immutils collection. But actual sqlite processing for a
   CCB is triggered at the slave only when all operations for the CCB have
   arrived.

   The entire prepare processing (start transaction and buildup) is done by the
   RTO thread at the slave. CCB is committed at primary, then in imm-ram.
   Finally the sqlite commit of a ccb at the slave is done by the slave only
   when it receives the completed & apply callbacks in the applier thread.
*/

static bool pbe2_start_prepare_ccb_A_to_B(SaImmOiCcbIdT ccbId,
                                          SaUint32T numOps) {
  TRACE_ENTER();
  bool retval = false;

  /* PBE primary (A) sends the start-prepare order to slave(B). */
  /* Check if slave PBE is down, if so return true if slave PBE is inactive or
   * disabled. */
  ...
  const SaImmAdminOperationParamsT_2 param0 = {ccb_id_string,
                                               SA_IMM_ATTR_SAUINT64T, &ccbId};

  const SaImmAdminOperationParamsT_2 param1 = {num_ops_string,
                                               SA_IMM_ATTR_SAUINT32T, &numOps};
  ...
  do {
    ...
    if (rc2B == SA_AIS_ERR_TRY_AGAIN || rc2B == SA_AIS_ERR_BAD_OPERATION ||
        rc2B == SA_AIS_ERR_NOT_EXIST ||
        (rc2B == SA_AIS_OK && slavePbeRtReply == SA_AIS_ERR_TRY_AGAIN)) {
      usleep(sleep_delay_ms * 1000);
      msecs_waited += sleep_delay_ms;
      LOG_NO(
          "Slave PBE %u or Immsv (%u) replied with transient error on prepare for ccb:%llx/%llu",
          rc2B, slavePbeRtReply, ccbId, ccbId);
    }
    /* Adjust the waiting time, above & below  to be more appropriate ....
       ERR_BAD_OPERATION or ERR_NOT_EXIST from immsv can happen at slave PBE
       startup when slave has not created its RTO, or created RTO but not yet
       set admin-owner for it. */
  } while ((rc2B == SA_AIS_ERR_TRY_AGAIN || rc2B == SA_AIS_ERR_BAD_OPERATION ||
            rc2B == SA_AIS_ERR_NOT_EXIST ||
            (rc2B == SA_AIS_OK && slavePbeRtReply == SA_AIS_ERR_TRY_AGAIN)) &&
           (msecs_waited < 3000));
  ...
  if (rc2B != SA_AIS_OK) {
    if ((rc2B == SA_AIS_ERR_NOT_EXIST) &&
      ...
      LOG_WA(
          "Start prepare for ccb: %llx/%llu towards slave PBE returned: '%u' from Immsv",
          ccbId, ccbId, rc2B);
    }
  } else if (slavePbeRtReply != SA_AIS_OK) {
    LOG_WA(
        "Start prepare for ccb: %llx/%llu towards slave PBE returned: '%u' from standby PBE",
        ccbId, ccbId, slavePbeRtReply);
  } else {
    LOG_IN(
        "Slave PBE replied with OK on attempt to start prepare of ccb:%llx/%llu",
        ccbId, ccbId);
    retval = true;
  }
...
static SaAisErrorT pbe2_ok_to_prepare_ccb_at_B(
    SaImmOiCcbIdT ccbId, SaUint32T expectedNumOps,
  ...
  if (s2PbeBCcbToCompleteAtB == 0) {
    ...
    if (s2PbeBCcbUtilCcbData == NULL) {
      TRACE(
          "First ccb-op not yet received or time-out in waiting on prepare, for ccb:%llu at slave PBE",
          ccbId);
      return SA_AIS_ERR_TRY_AGAIN;
      goto done;
    } else if (s2PbeBCcbUtilCcbData->ccbId != ccbId) {
      /* This should never happen, but since we dont use any locking and since
         the applier thread may mutate ccb-utils concurrently with this lookup
         for a specific ccb-record, there is a non-zero risk that the lookup
         goes wrong. If this happens then it would be incredible if it still
         found the ccb-record with the matching ccb-id. Per definition then the
         lookup did not go wrong. When the pointer to the ccb-record has been
         found *once* here by the runtime thread, the pointer value is assigned
         to a static variable only used by the runtime thread. This ccb-strucure
         will only be removed from immutils by the (the thread that created it)
         in the apply or abort callback. The runtime thread thus reads from the
         immutils structure only from a stable pointer verified to point at a
         ccb record with corrrect ccb-id.
      */
      LOG_WA(
          "Mismatch on record for ccbId:%llx/%llu - thread interference problems ?",
          ccbId, ccbId);
      ...
      TRACE("Slave PBE found record for ccb:%llu received ops:%llu", ccbId,
            numReceivedOps);
    }
  ...
  osafassert(s2PbeBCcbToCompleteAtB ==
             ccbId); /* Checked already by admop-code. */
  ...
  if (s2PbeBCcbOpCountToExpectAtB < s2PbeBCcbOpCountNowAtB) {
    LOG_ER("Current op-count %u for ccb:%llx/%llu is greater than expected: %u",
           s2PbeBCcbOpCountNowAtB, s2PbeBCcbToCompleteAtB,
           s2PbeBCcbToCompleteAtB, s2PbeBCcbOpCountToExpectAtB);
...
static void saImmOiAdminOperationCallback(
  ...
  const SaImmAdminOperationParamsT_2 ccbIdParam = {
      ccb_id_string, SA_IMM_ATTR_SAUINT64T, &ccbId};
  ...
  if (opId == OPENSAF_IMM_PBE_CLASS_CREATE) {
    /* If this case ever replies with error (not OK and not TRY_AGAIN), i.e.
       does not persistify the class, then PBE must also terminate. Class-create
       can not be reverted by the IMMNDs (imm-ram).
     */
    bool schemaChange = false;
    bool persistentExtent =
        false; /* ConfigC or PrtoC. Only relevant if schmaChange == true */
    ...
    do {
      ...
      if (strcmp(param->paramName, (char *)ccb_id_string) == 0) {
        ...
        LOG_WA("Unhandled parameter in PBE class-create:%s - ignoring",
               param->paramName);
      }
      param = params[ix];
      ++ix;
    ...
    if (sPbe2 && !sPbe2B && (sNoStdFlags & OPENSAF_IMM_FLAG_2PBE1_ALLOW)) {
      LOG_IN(
          "PBE is in 1safe2PBE state. Ignoring class create towards slave PBE");
    } else if (sPbe2 && !sPbe2B) {
      /* Primary PBE forwards class create to slave PBE. */
      SaAisErrorT rc2B = SA_AIS_OK;
      SaNameT slavePbeRtObjName;
      osaf_extended_name_lend(OPENSAF_IMM_PBE_RT_OBJECT_DN_B,
                              &slavePbeRtObjName);
      SaAisErrorT slavePbeRtReply = SA_AIS_OK;
      const SaImmAdminOperationParamsT_2 *paramsToSlave[] = {params[0],
                                                             &ccbIdParam, NULL};

      rc2B = saImmOmAdminOperationInvoke_2(
          sOwnerHandle, &slavePbeRtObjName, 0, OPENSAF_IMM_PBE_CLASS_CREATE,
          paramsToSlave, &slavePbeRtReply, SA_TIME_ONE_SECOND * 15);
      if (rc2B != SA_AIS_OK) {
        if (rc2B == SA_AIS_ERR_NOT_EXIST) {
          LOG_IN(
              "Primary PBE got ERR_NOT_EXIST on atempt to create class towards slave PBE - ignoring");
        } else {
          LOG_WA(
              "Primary PBE failed to create class towards slave PBE. Library or immsv replied Rc:%u - ignoring",
              rc2B);
        }
        rc2B = SA_AIS_OK;
      } else if (slavePbeRtReply == SA_AIS_ERR_REPAIR_PENDING) {
        LOG_IN(
            "Slave PBE replied with OK to primary PBE on attempt to create class %s",
            className.c_str());
      } else {
        LOG_WA(
            "PBE got error '%u' from PBE slave on create class. Should not happen.",
            slavePbeRtReply);
        /* The slave behaves just like the primary in this case, if anything
           goes wrong in create class it logs the error and then exits. See the
           actual PBE create class transaction code below. So we should never
           end up here! The primary PBE here ignores the error reported by the
           slave. Either the problem was "local" to the slave or the primary
           will get the same problem below and also exit. But we dont
           immediately exit the primary PBE just because the slave had a problem
           and reported an error. The slave could be broken or inconsistent
           without the primary being so. */
      }
      ...
      if (pbeTransStarted()) {
        ...
        do {
          LOG_IN(
              "Delaying class create at slave PBE due to ongoing commit of ccb:%llx/%llu",
        ...
        LOG_IN(
            "s2PbeBCcbToCompleteAtB==(%llx/%llu) => Class-create "
            "ignoring wait for applyCallback at slave",
            s2PbeBCcbToCompleteAtB, s2PbeBCcbToCompleteAtB);
      }
    }
    ...
    if (sNoStdFlags & OPENSAF_IMM_FLAG_SCHCH_ALLOW) {
      ...
      if (theClass) {
        ...
        if (persistentExtent) {
          instances =
              purgeInstancesOfClassToPBE(pbeOmHandle, className, sDbHandle);
          LOG_IN("PBE removed %u old instances of class %s", instances,
                 className.c_str());
        }
        ...
        /* Note: We dont remove the classname from the opensaf object,
           since we will shortly add the new version of the class, with same
           name.
         */
      }
    }

    /* Note that schema upgrade generates a new PBE class_id for the new version
     * of the class */
    (*sClassIdMap)[className] =
    ...
    LOG_IN("Create of class %s committing with ccbId:%llx", className.c_str(),
           ccbId);
    pbeClosePrepareTrans();
    ...
    /* We only reply with ok result. PBE failed class create handled by timeout,
       no revert in imm-ram. We encode OK result for
       OPENSAF_IMM_PBE_CLASS_CREATE with the arbitrary and otherwise unused (in
       immsv) error code SA_AIS_ERR_REPAIR_PENDING. This way the immnd can tell
       that this is supposed to be an ok reply on PBE_CLASS_CREATE. This usage
       is internal to immsv.
    */
    rc = immutil_saImmOiAdminOperationResult(immOiHandle, invocation,
    ...
    /* If this case ever replies with error (not OK and not TRY_AGAIN), i.e.
       does not persistify the class delete, then PBE must also terminate.
       Class-delete can not be reverted by the IMMNDs (imm-ram).
     */
    int ix = 1;
    std::string className;
    ...
    if (sPbe2 && !sPbe2B && (sNoStdFlags & OPENSAF_IMM_FLAG_2PBE1_ALLOW)) {
      LOG_IN(
          "PBE is in 1safe2PBE state. Ignoring class delete towards slave PBE");
    } else if (sPbe2 && !sPbe2B) {
      ...
      rc2B = saImmOmAdminOperationInvoke_2(
          sOwnerHandle, &slavePbeRtObjName, 0, OPENSAF_IMM_PBE_CLASS_DELETE,
          paramsToSlave, &slavePbeRtReply, SA_TIME_ONE_SECOND * 15);
      if (rc2B != SA_AIS_OK) {
        if (rc2B == SA_AIS_ERR_NOT_EXIST) {
          LOG_NO(
              "Primary PBE Got ERR_NOT_EXIST on atempt to delete class towards slave PBE - ignoring");
        } else {
          LOG_WA(
              "Failed to delete class towards slave PBE. Library or immsv replied Rc:%u - ignoring",
              rc2B);
        }
        rc2B = SA_AIS_OK;
      } else if (slavePbeRtReply == SA_AIS_ERR_NO_SPACE) {
        LOG_NO("Slave PBE replied with OK on attempt to delete class %s",
               className.c_str());
      } else {
        LOG_WA(
            "PBE got error '%u' from PBE slave on attempt to delete class. Should not happen.",
            slavePbeRtReply);
        /* The slave behaves just like the primary in this case, if anything
           goes wrong in delete class it logs the error and then exits. See the
           actual PBE delete class transaction code below. So we should never
           end up here! The primary PBE here ignores the error reported by the
           slave. Either the problem was "local" to the slave or the primary
           will get the same problem below and also exit. But we dont
           immediately exit the primary PBE just because the slave had a problem
      ...
      if (pbeTransStarted()) {
        ...
        do {
          LOG_IN(
              "Delaying class delete at slave PBE due to ongoing commit of ccb:%llx/%llu",
              s2PbeBCcbToCompleteAtB, s2PbeBCcbToCompleteAtB);
          usleep(500000);
          ++try_count;
        } while (pbeTransStarted() && (try_count < max_tries));
        ...
        LOG_IN(
            "s2PbeBCcbToCompleteAtB==(%llx/%llu) => Class-delete "
            "ignoring wait for applyCallback at slave",
            s2PbeBCcbToCompleteAtB, s2PbeBCcbToCompleteAtB);
      }
    }
    ...
    LOG_IN("Delete of class %s committing with ccbId:%llx", className.c_str(),
           ccbId);

    pbeClosePrepareTrans();
    rc = pbeCommitTrans(sDbHandle, ccbId, sEpoch, &sLastCcbCommitTime);
    if (rc != SA_AIS_OK) {
      LOG_WA("PBE failed to commit transaction (ccb:%llx) for class delete",
             ccbId);
      osafassert(rc != SA_AIS_ERR_TRY_AGAIN);
      goto fail;
    }
    ...
    /* We only reply with ok result. PBE failed class delete handled by timeout,
       no revert in imm-ram. We encode OK result for
       OPENSAF_IMM_PBE_CLASS_DELETE with the arbitrary and otherwise unused (in
       immsv) error code SA_AIS_ERR_NO_SPACE. This way the immnd can tell that
       this is supposed to be an ok reply on PBE_CLASS_DELETE. This usage is
       internal to immsv.
    */
    rc = immutil_saImmOiAdminOperationResult(immOiHandle, invocation,
                                             SA_AIS_ERR_NO_SPACE);
    ...
    if (sPbe2 && !sPbe2B && (sNoStdFlags & OPENSAF_IMM_FLAG_2PBE1_ALLOW)) {
      LOG_IN(
          "PBE is in 1safe2PBE state. Ignoring update epoch towards slave PBE");
    } else if (sPbe2 && !sPbe2B) {
      ...
      const SaImmAdminOperationParamsT_2 *paramsToSlave[] = {params[0],
                                                             &ccbIdParam, NULL};
      rc2B = saImmOmAdminOperationInvoke_2(
          sOwnerHandle, &slavePbeRtObjName, 0, OPENSAF_IMM_PBE_UPDATE_EPOCH,
          paramsToSlave, &slavePbeRtReply, SA_TIME_ONE_SECOND * 10);
      if (rc2B != SA_AIS_OK) {
        if (rc2B == SA_AIS_ERR_NOT_EXIST) {
          LOG_IN(
              "Primary PBE got ERR_NOT_EXIST on atempt to update epoch towards slave PBE - ignoring");
        } else {
          LOG_WA(
              "Primary PBE failed to update epoch towards slave PBE. Rc:%u - ignoring",
              rc2B);
        }
      } else if (slavePbeRtReply == SA_AIS_ERR_QUEUE_NOT_AVAILABLE) {
        LOG_IN("Slave PBE replied with OK on attempt to update epoch");
      } else {
        LOG_WA("Slave PBE replied with error '%u' on attempt to update epoch",
               slavePbeRtReply);
    ...
    if (sEpoch >= epoch) {
      TRACE("Current epoch %u already equal or greater than %u ignoring",
            sEpoch, epoch);
      if (sEpoch == epoch) {
        goto reply_ok;
      }
    ...
    if (sPbe2B && s2PbeBCcbToCompleteAtB) {
      ...
      if (pbeTransStarted()) {
        ...
        do {
          LOG_IN(
              "Delaying update epoch at slave PBE due to ongoing commit of ccb:%llx/%llu",
              s2PbeBCcbToCompleteAtB, s2PbeBCcbToCompleteAtB);
          usleep(500000);
          ++try_count;
        } while (pbeTransStarted() && (try_count < max_tries));
        ...
        LOG_IN(
            "s2PbeBCcbToCompleteAtB==(%llx/%llu) => Update-epoch "
            "ignoring wait for applyCallback at slave",
            s2PbeBCcbToCompleteAtB, s2PbeBCcbToCompleteAtB);
      }
    }
    ...
    LOG_NO("Update epoch %u committing with ccbId:%llx/%llu", sEpoch, ccbId,
           ccbId);
    pbeClosePrepareTrans();
    rc = pbeCommitTrans(sDbHandle, ccbId, sEpoch, &sLastCcbCommitTime);
    if (rc != SA_AIS_OK) {
  ...
  /* We only reply with ok result. PBE failed update epoch handled by timeout,
     cleanup. We encode OK result for OPENSAF_IMM_PBE_UPDATE_EPOCH with the
     arbitrary and otherwise unused (in immsv) error code
     SA_AIS_ERR_QUEUE_NOT_AVAILABLE. This way the immnd can tell that this is
     supposed to be an ok reply on PBE_UPDATE_EPOCH. This usage is internal to
     immsv.
  */
  reply_ok:
    rc = immutil_saImmOiAdminOperationResult(immOiHandle, invocation,
    ...
    if (sPbe2 && !sPbe2B && (sNoStdFlags & OPENSAF_IMM_FLAG_2PBE1_ALLOW)) {
      LOG_IN(
          "PBE is in 1safe2PBE state. Ignoring NOST_FLAG_ON towards slave PBE");
    } else if (sPbe2 && !sPbe2B) {
      ...
      SaAisErrorT slavePbeRtReply = SA_AIS_OK;
      rc2B = saImmOmAdminOperationInvoke_2(
          sOwnerHandle, &slavePbeRtObjName, 0, OPENSAF_IMM_NOST_FLAG_ON, params,
          &slavePbeRtReply, SA_TIME_ONE_SECOND * 10);
      if (rc2B != SA_AIS_OK) {
        if (rc2B == SA_AIS_ERR_NOT_EXIST) {
          LOG_IN(
              "Got ERR_NOT_EXIST on atempt to set nostFlags towards slave PBE - ignoring");
          rc = rc2B = SA_AIS_OK;
        } else {
        ...
        LOG_WA("Slave PBE replied with error %u on attempt to set nostFlags",
               slavePbeRtReply);
        rc = slavePbeRtReply;
      }
    }
    ...
    if (sPbe2B && (flagsToSet == OPENSAF_IMM_FLAG_2PBE1_ALLOW)) {
      LOG_WA(
          "Attempt to togle *ON* value x%x in nostdFlags not allowed when PBE slave is running.",
    ...
    if (rc == SA_AIS_OK) {
      ...
      LOG_NO("NOSTD FLAGS value 0x%x switched ON result:0x%x", flagsToSet,
             sNoStdFlags);
      if (!sPbe2B) { /* Only primary PBE updates the cached RTA. */
        ...
        if (rc != SA_AIS_OK) {
          /* Should never get TRY_AGAIN here. RTA update is always accepted,
             Except if local IMMND goes down, but then this PBE will go down
             also.
           */
    ...
    if (!param || (param->paramType != SA_IMM_ATTR_SAUINT32T) ||
        strcmp(param->paramName, (char *)OPENSAF_IMM_ATTR_NOSTD_FLAGS)) {
      LOG_IN(
          "Incorrect parameter to NostdFlags-OFF, should be 'opensafImmNostdFlags:SA_UINT32_T:nnnn'");
      rc = immutil_saImmOiAdminOperationResult(immOiHandle, invocation,
                                               SA_AIS_ERR_INVALID_PARAM);
      goto fail;
    }
    ...
    if (sPbe2 && !sPbe2B && (sNoStdFlags & OPENSAF_IMM_FLAG_2PBE1_ALLOW)) {
      LOG_IN(
          "PBE is in 1safe2PBE state. Ignoring NOST_FLAG_OFF towards slave PBE");
    } else if (sPbe2 && !sPbe2B) {
      ...
      SaAisErrorT slavePbeRtReply = SA_AIS_OK;
      rc2B = saImmOmAdminOperationInvoke_2(
          sOwnerHandle, &slavePbeRtObjName, 0, OPENSAF_IMM_NOST_FLAG_OFF,
          params, &slavePbeRtReply, SA_TIME_ONE_SECOND * 10);
      if (rc2B != SA_AIS_OK) {
        if (rc2B == SA_AIS_ERR_NOT_EXIST) {
          LOG_IN(
              "Got ERR_NOT_EXIST on atempt to un-set nostFlags towards slave PBE - ignoring");
          rc = rc2B = SA_AIS_OK;
        } else {
          if (flagsToUnSet == OPENSAF_IMM_FLAG_2PBE1_ALLOW) {
            LOG_NO(
                "Failed to un-set flag 2PBE1_ALLOW towards slave PBE. Rc:%u - overriding",
                rc2B);
            rc = rc2B = SA_AIS_OK;
          } else {
        ...
        LOG_WA(
            "Slave PBE replied with error '%u' on attempt to un-set nostFlags",
            slavePbeRtReply);
        rc = slavePbeRtReply;
      }
    }
    ...
    if (rc == SA_AIS_OK) {
      ...
      LOG_NO("NOSTD FLAGS value 0x%x switched OFF result:0x%x", flagsToUnSet,
             sNoStdFlags);
      if (!sPbe2B) { /* Only primary PBE updates the cached RTA. */
        SaNameT myObj;
        osaf_extended_name_lend(OPENSAF_IMM_OBJECT_DN, &myObj);
    ...
    do {
      if (strcmp(param->paramName, (char *)ccb_id_string) == 0) {
        ...
        LOG_WA("Unhandled parameter in slave ccb prepare:%s - ignoring",
               param->paramName);
      }
      param = params[ix];
      ++ix;
    ...
    LOG_IN("ccb-prepare received at PBE slave ccbId:%llx/%llu numOps:%u", ccbId,
           ccbId, numOps);

    if ((s2PbeBCcbToCompleteAtB != 0) && (s2PbeBCcbToCompleteAtB != ccbId)) {
      LOG_NO(
          "Prepare ccb:%llx/%llu received at Pbe slave when Prior Ccb %llu still processing",
    ...
    if (rc == SA_AIS_OK) {
      rc = pbeBeginTrans(sDbHandle);
      if (rc != SA_AIS_OK) {
        LOG_WA("PBE failed to start sqlite transaction for CCB %llx/%llu",
               ccbId, ccbId);
        osafassert(rc != SA_AIS_ERR_TRY_AGAIN);
        goto fail;
      }
    } else if (rc != SA_AIS_ERR_TRY_AGAIN) {
      LOG_WA("Prepare for ccb %llx/%llu failed at slave PBE rc:%u", ccbId,
             ccbId, rc);
    }

    if (immutil_saImmOiAdminOperationResult(immOiHandle, invocation, rc) !=
        SA_AIS_OK) {
      LOG_WA(
          "Slave PBE failed in replying to primary on prepare for ccb %llx/%llu.",
          ccbId, ccbId);
      rc = SA_AIS_ERR_NO_RESOURCES;
    }

    if (rc == SA_AIS_OK) {
      /*Now actually do the sqlite slave prepare here instead of in the
        completed callback. This is (A) faster i.e. more tight and in parallel
        with primary. The actuall commit of the ccb sqlite transaction is still
        done by the applier thread. This is to avoid the slave ever committing a
        CCB *before* the primary.
    ...
    rparams = (SaImmAdminOperationParamsT_2 **)realloc(
        NULL, sizeof(SaImmAdminOperationParamsT_2 *));
    rparams[0] = NULL;
    while (params[i]) {
    ...
    if (opName) {
      if ((strcmp(opName, "display") == 0) ||
        ...
        rc = immutil_saImmOiAdminOperationResult_o2(
            immOiHandle, invocation, SA_AIS_OK,
            (const SaImmAdminOperationParamsT_2 **)rparams);
      } else {
        LOG_WA("Invalid operation Name %s for operation ID %llu", opName,
               (SaUint64T)opId);
        rc = immutil_saImmOiAdminOperationResult(immOiHandle, invocation,
                                                 SA_AIS_ERR_INVALID_PARAM);
      }
    } else {
      LOG_WA("Operation Name is not provided for operation ID %llu",
             (SaUint64T)opId);
      rc = immutil_saImmOiAdminOperationResult(immOiHandle, invocation,
                                               SA_AIS_ERR_INVALID_PARAM);
    }
    ...
    while (rparams[i]) {
      resparams = rparams[i];
      if (resparams->paramName) free(resparams->paramName);
...
static SaAisErrorT saImmOiCcbObjectModifyCallback(
  ...
  TRACE_ENTER2("Modify callback for CCB:%llu object:%s", ccbId,
  ...
  if (strncmp(osaf_extended_name_borrow(objectName),
    ...
    LOG_NO("PBE allowing modification to object %s",
           (char *)OPENSAF_IMM_OBJECT_DN);
  }
  ...
  if (sPbe2B) {
    /* Slave PBE waits for prepare for PRTA update (weak ccb) as admop invoked
     * from primary PBE.*/

    while ((msecs_waited < max_waiting_time_ms) &&
           ((s2PbeBCcbToCompleteAtB != ccbId) ||
            (s2PbeBCcbOpCountNowAtB == 0) || (!pbeTransIsPrepared()))) {
      LOG_IN(
          "PBE slave waiting for prepare from primary on PRTA update ccb:%llx",
          ccbId);
      usleep(sleep_delay_ms * 1000);
      msecs_waited += sleep_delay_ms;
    }

    if ((s2PbeBCcbToCompleteAtB != ccbId) || (s2PbeBCcbOpCountNowAtB == 0) ||
        (!pbeTransIsPrepared())) {
      LOG_NO(
          "Slave PBE time-out in waiting on prepare for PRTA update ccb:%llx dn:%s",
          ccbId, osaf_extended_name_borrow(objectName));
      rc = SA_AIS_ERR_NO_RESOURCES;
      goto done;
    }
    ...
    /* 2PBE at primary requests PBE slave to prepare this weak ccb. If prepare
       at slave fails, then the weak ccb (PRTA update) is aborted also here at
       primary. */

    LOG_IN("Starting distributed PBE commit for PRTA update Ccb:%llx/%llu",
           ccbId, ccbId);
    if (!pbe2_start_prepare_ccb_A_to_B(
            ccbId, (SaUint32T)numOps)) { /* Order slave to prepare */
      LOG_WA("PBE-A failed to prepare PRTA update Ccb:%llx/%llu towards PBE-B",
             ccbId, ccbId);
      rc = SA_AIS_ERR_NO_RESOURCES;
      goto done;
    }
  ...
  if (rc != SA_AIS_OK) {
    LOG_WA(
        "PBE failed to start sqlite transaction (ccbId:%llx) for PRT attr update",
        ccbId);
    rc = SA_AIS_ERR_NO_RESOURCES;
    goto done;
  }
  ...
  rc =
      sqlite_prepare_ccb(immOiHandle, ccbId, ccbUtilCcbData->operationListHead);
  if (rc != SA_AIS_OK) {
...
commit_prta_trans:
  ...
  if (rc != SA_AIS_OK) {
    LOG_WA(
        "PBE failed to commit sqlite transaction (ccb:%llx) for PRT attr update",
        ccbId);
    rc = SA_AIS_ERR_NO_RESOURCES;
    goto done;
  }
...
static SaAisErrorT saImmOiCcbCompletedCallback(SaImmOiHandleT immOiHandle,
  ...
  /* The completed callback is actually PREPARE AND COMMIT of the transaction.
     ...
     disk, that is pbeCommitTrans returns SA_AIS_OK, then this callback returns
     OK and the rest of the immsv is obligated to commit. Returning Ok from this
     upcall is then very special and precious.

     If we unambiguously fail to commit, that is if pbeCommitTrans returns
     not SA_AIS_OK, or we fail before that, then this upcall will return with
     failure and the rest of the imsmv is obliged NOT to commit. The failure
     case is the same as for any implementer, returning not OK means this
     implemener vetoes the transaction.

     There is a third case that is non standard.
     If this upcall does not reply (i.e. the PBE crashes or gets stuck in an
     endless loop or gets blocked on disk i/o, or...) then the rest of the immsv
     is obliged to wait until the outcome is resolved. The outcome of a "lost"
     ccb/transaction can be resolved in two ways.
     ...
     The second way is for the IMMSv to overrule the PBE by discarding the
     entire pbe file. This is done by just restarting the pbe in a normal way
     which will cause it to dump the current contents of the immsv, ignoring any
     previous disk rep. Discarding ccbs that could have been commited to disk
     may seem wrong, but as long as no client of the imsmv has seen the result
     of the ccb as commited, there is no problem. But care should be taken to
     discard the old disk rep so that it is not re-used later to see an
     alternative history. If the first solution fails then this solution will be
     used as fallback. getCcbOutcomeFromPbe will then return BAD_OPERATION i.e.
     presumed abort.
   */
  ...
  if ((ccbUtilCcbData = ccbutil_findCcbData(ccbId)) == NULL) {
    LOG_WA(
        "Failed to find CCB object for %llx/%llu - checking DB file for outcome",
        ccbId, ccbId);
    rc = getCcbOutcomeFromPbe(sDbHandle, ccbId,
                              sEpoch); /* rc=NO_RESOURCES or OK */
    (void)ccbutil_getCcbData(ccbId);   /*generate an empty record*/
    goto done;
  }

  if (sPbe2) {
    /* Primary PBE has requested slave PBE to start preparing. If slave replied
       Ok then slave was ready to start prepare and has started its prepare.
    ...
    if (sPbe2B) { /* PbeB == slave */
      if (s2PbeBCcbToCompleteAtB != ccbId) {
        LOG_ER(
            "PBE-B got completed callback for Ccb:%llx/%llu while still bussy "
            "with Ccb:%llx/%llufrom PBE-A",
            ccbId, ccbId, s2PbeBCcbToCompleteAtB, s2PbeBCcbToCompleteAtB);
        rc = SA_AIS_ERR_NO_RESOURCES;
        goto done; /* This ccb has not even prepared yet. Dont abort the prior
                      ccb! */
      }

      if (s2PbeBCcbOpCountToExpectAtB != numOps) {
        LOG_ER(
            "PBE-B got completed callback for Ccb:%llx/%llu but numOps:%llu should be: %u",
            ccbId, ccbId, numOps, s2PbeBCcbOpCountToExpectAtB);
        rc = SA_AIS_ERR_NO_RESOURCES;
        goto abort_trans; /* This ccb has passed prepare, but obj-count is
                             wrong. */
      }
      goto commit_trans; /* Jump over begin-trans & prepare. Done already at
                            PBESlave/B */

    } else { /* PbeA == primary */
      LOG_IN("Starting distributed PBE commit for Ccb:%llx/%llu", ccbId, ccbId);
      if (!pbe2_start_prepare_ccb_A_to_B(
              ccbId, (SaUint32T)numOps)) { /* Order slave to start preparing. */
        LOG_WA("PBE-A failed to prepare Ccb:%llx/%llu towards PBE-B", ccbId,
               ccbId);
        rc = SA_AIS_ERR_NO_RESOURCES;
        goto done;
      }
...
commit_trans:
  ...
  if (rc == SA_AIS_OK) {
    ...
    snprintf(buf, sBufsize,
             "PBE COMMIT of sqlite transaction %llu epoch%u FAILED rc:%d",
             ccbId, sEpoch, rc);
    TRACE("%s", buf);
    saImmOiCcbSetErrorString(immOiHandle, ccbId, buf);
  }
...
abort_trans:
  ...
  if (sPbe2B) { /* PBE-B slave can not abort unilateraly in completed callback.
                 */
    LOG_WA(
        "PBE slave exiting in completed callback for ccb %llx/%llu, file should be regenerated.",
        ccbId, ccbId);
    exit(0);
  }
done:
...
static void saImmOiCcbApplyCallback(SaImmOiHandleT immOiHandle,
  ...
  if ((ccbUtilCcbData = ccbutil_findCcbData(ccbId)) == NULL) {
    LOG_WA("Failed to find CCB object for %llx/%llu - PBE restarted recently?",
           ccbId, ccbId);
    goto done;
  }
...
static void saImmOiCcbAbortCallback(SaImmOiHandleT immOiHandle,
  ...
  if ((ccbUtilCcbData = ccbutil_findCcbData(ccbId)) != NULL) {
    if ((s2PbeBCcbToCompleteAtB == ccbId) && pbeTransStarted() &&
        !pbeTransIsPrepared()) {
      osafassert(sPbe2 && sPbe2B); /* Must be slave PBE */
      /* Avoid aborting ccb in main-thread while on-going prepare on same ccb in
       * rt-thread */
      /* Extra wait for max 3 seconds */
      unsigned int try_count = 0;
      ...
      do {
        LOG_IN(
            "Delaying ccb abort at slave PBE due to ongoing prepare of ccb:%llx/%llu",
            s2PbeBCcbToCompleteAtB, s2PbeBCcbToCompleteAtB);
        usleep(50000);
        ++try_count;
      ...
      if (!pbeTransIsPrepared()) {
        LOG_WA(
            "Slave PBE main thread blocked in abort of ccb:%llx/%llu - exit/restart",
            s2PbeBCcbToCompleteAtB, s2PbeBCcbToCompleteAtB);
        exit(1);
      }
    }
  ...
  if (s2PbeBCcbToCompleteAtB == ccbId) {
    ...
    if (pbeTransStarted()) {
      ...
      LOG_IN(
          "saImmOiCcbAbortCallback: aborting sqlite transaction for CCB %llu",
          ccbId);
      pbeAbortTrans(sDbHandle);
    }
    s2PbeBCcbToCompleteAtB = 0;
...
static SaAisErrorT saImmOiCcbObjectCreateCallback(
  ...
  if (parentName != NULL && !osaf_is_extended_name_empty(parentName)) {
    TRACE_ENTER2("CREATE CALLBACK CCB:%llu class:%s parent:%s", ccbId,
                 className, osaf_extended_name_borrow(parentName));
  } else {
    TRACE_ENTER2("CREATE CALLBACK CCB:%llu class:%s ROOT OBJECT (no parent)",
                 ccbId, className);
  }
  if (!classInfo) {
    snprintf(buf, sBufsize,
             "PBE Internal error: class '%s' not found in classIdMap",
             className);
    LOG_ER("%s", buf);
    saImmOiCcbSetErrorString(immOiHandle, ccbId, buf);
    rc = SA_AIS_ERR_BAD_OPERATION;
  ...
  if ((ccbUtilCcbData = ccbutil_findCcbData(ccbId)) == NULL) {
    if ((ccbUtilCcbData = ccbutil_getCcbData(ccbId)) == NULL) {
      snprintf(buf, sBufsize,
               "PBE Internal error: Failed to get CCB objectfor %llu", ccbId);
      LOG_ER("%s", buf);
      saImmOiCcbSetErrorString(immOiHandle, ccbId, buf);
      rc = SA_AIS_ERR_NO_MEMORY;
      goto done;
  ...
  if (strncmp((char *)className, (char *)OPENSAF_IMM_CLASS_NAME,
              strlen(className)) == 0) {
    snprintf(buf, sBufsize,
             "PBE: will not allow creates of instances of class %s",
             (char *)OPENSAF_IMM_CLASS_NAME);
    LOG_NO("%s", buf);
    saImmOiCcbSetErrorString(immOiHandle, ccbId, buf);
    rc = SA_AIS_ERR_BAD_OPERATION;
    /* We will actually get invoked twice on this create, both as normal
       implementer and as PBE the response on the create upcall from PBE is
       discarded, but not for the regular implementer UC.
     */
    goto done;
  }
  ...
  /* Find the RDN attribute, construct the object DN and store it with the
   * object. */
  while ((attrValue = attr[i++]) != NULL) {
    ...
    if (attrFlags & SA_IMM_ATTR_RDN) {
      ...
      TRACE("Extracted DN: %s(%zu)", objectDn.c_str(),
            static_cast<size_t>(objectDn.size()));
    }
  }

  if (!rdnFound) {
    snprintf(buf, sBufsize, "PBE: Could not find Rdn attribute for class '%s'!",
             className);
    LOG_ER("%s", buf);
    saImmOiCcbSetErrorString(immOiHandle, ccbId, buf);
    rc = SA_AIS_ERR_BAD_OPERATION;
  ...
  if (sPbe2B) {
    /* Slave PBE waits for prepare for PRTO create (weak ccb) as admop invoked
     * from primary PBE.*/
    ...
    while ((msecs_waited < max_waiting_time_ms) &&
           ((s2PbeBCcbToCompleteAtB != ccbId) ||
            (s2PbeBCcbOpCountNowAtB == 0) || (!pbeTransIsPrepared()))) {
      LOG_IN(
          "PBE slave waiting for prepare from primary on PRTO create ccb:%llx",
          ccbId);
      usleep(sleep_delay_ms * 1000);
      msecs_waited += sleep_delay_ms;
    }
    ...
    if ((s2PbeBCcbToCompleteAtB != ccbId) || (s2PbeBCcbOpCountNowAtB == 0) ||
        (!pbeTransIsPrepared())) {
      LOG_NO(
          "Slave PBE time-out in waiting on prepare for PRTO create ccb:%llx dn:%s",
          ccbId, osaf_extended_name_borrow(&operation->objectName));
      rc = SA_AIS_ERR_NO_RESOURCES;
      goto done;
    }
    ...
    /* 2PBE at primary requests PBE slave to prepare this weak ccb. If prepare
       at slave fails, then the weak ccb (PRTO create) is aborted also here at
       primary  */

    LOG_IN("Starting distributed PBE commit for PRTO create Ccb:%llx/%llu",
           ccbId, ccbId);
    if (!pbe2_start_prepare_ccb_A_to_B(
            ccbId, (SaUint32T)numOps)) { /* Order slave to prepare */
      LOG_WA("PBE-A failed to prepare PRTO create Ccb:%llx/%llu towards PBE-B",
             ccbId, ccbId);
      rc = SA_AIS_ERR_NO_RESOURCES;
      goto done;
    }
  ...
  TRACE("Create of PERSISTENT runtime object with DN: %s", objectDn.c_str());

  rc = pbeBeginTrans(sDbHandle);
  if (rc != SA_AIS_OK) {
    LOG_WA("PBE failed to start sqlite transaction (ccbId:%llx)for PRTO create",
           ccbId);
    rc = SA_AIS_ERR_NO_RESOURCES;
    goto done;
  }
...
commit_prto_trans:
  ...
  if (rc != SA_AIS_OK) {
    LOG_WA(
        "PBE failed to commit sqlite transaction (ccbId:%llx) for PRTO create",
        ccbId);
    rc = SA_AIS_ERR_NO_RESOURCES;
    goto done;
  }
...
static SaAisErrorT saImmOiCcbObjectDeleteCallback(SaImmOiHandleT immOiHandle,
  ...
  TRACE_ENTER2("DELETE CALLBACK CCB:%llu object:%s", ccbId,
               osaf_extended_name_borrow(objectName));

  if ((ccbUtilCcbData = ccbutil_findCcbData(ccbId)) == NULL) {
    if ((ccbUtilCcbData = ccbutil_getCcbData(ccbId)) == NULL) {
      snprintf(buf, sBufsize, "PBE: Failed to get CCB objectfor %llx/%llu",
               ccbId, ccbId);
      LOG_WA("%s", buf);
      saImmOiCcbSetErrorString(immOiHandle, ccbId, buf);
      rc = SA_AIS_ERR_NO_MEMORY;
  ...
  if (strcmp(osaf_extended_name_borrow(objectName), OPENSAF_IMM_OBJECT_DN) ==
      0) {
    snprintf(buf, sBufsize, "PBE: will not allow delete of object %s",
             OPENSAF_IMM_OBJECT_DN);
    LOG_NO("%s", buf);
    saImmOiCcbSetErrorString(immOiHandle, ccbId, buf);
    rc = SA_AIS_ERR_BAD_OPERATION;
    /* We will actually get invoked twice on this object, both as normal
       implementer and as PBE the response on the delete upcall from PBE is
       discarded, but not for the regular implemener.
     */
  } else {
...
static SaAisErrorT saImmOiRtAttrUpdateCallback(
  ...
  TRACE_ENTER2("RT ATTR UPDATE CALLBACK PBE2B:%u DN:%s A:%s", sPbe2B,
               osaf_extended_name_borrow(objectName),
               OPENSAF_IMM_PBE_RT_OBJECT_DN_A);

  if ((sPbe2B && strcmp(osaf_extended_name_borrow(objectName),
                        OPENSAF_IMM_PBE_RT_OBJECT_DN_B) == 0) ||
      (!sPbe2B && strcmp(osaf_extended_name_borrow(objectName),
                         OPENSAF_IMM_PBE_RT_OBJECT_DN_A) == 0)) {
    ...
    SaImmAttrValuesT_2 attr1 = {(char *)OPENSAF_IMM_ATTR_PBE_RT_EPOCH,
                                SA_IMM_ATTR_SAUINT32T, 1, attrUpdateValues1};

    SaImmAttrValuesT_2 attr2 = {(char *)OPENSAF_IMM_ATTR_PBE_RT_CCB,
                                SA_IMM_ATTR_SAUINT64T, 1, attrUpdateValues2};

    SaImmAttrValuesT_2 attr3 = {(char *)OPENSAF_IMM_ATTR_PBE_RT_TIME,
                                SA_IMM_ATTR_SATIMET, 1, attrUpdateValues3};
    ...
    const SaImmAttrModificationT_2 *attrMods[] = {&attrMod1, &attrMod2,
                                                  &attrMod3, NULL};
...
SaAisErrorT pbe_daemon_imm_init(SaImmHandleT immHandle) {
  ...
  const SaStringT rdnStr =
      (SaStringT)((sPbe2B) ? "osafImmPbeRt=B" : "osafImmPbeRt=A");
  const SaImmAttrValueT nameValues[] = {(SaImmAttrValueT)&rdnStr};
  const SaImmAttrValuesT_2 v1 = {(SaImmAttrNameT)OPENSAF_IMM_ATTR_PBE_RT_RDN,
                                 SA_IMM_ATTR_SASTRINGT, 1, (void **)nameValues};
  ...
  if (sPbe2) {
    ...
    if (rc != SA_AIS_OK) {
      LOG_ER("saImmOiImplementerSet for %s failed %u", pbeRtImplName.c_str(),
             rc);
      return rc;
    }
    ...
    msecs_waited = 0;
    do {
      rc = saImmOiRtObjectCreate_2(
          pbeOiRtHandle, (SaImmClassNameT)OPENSAF_IMM_PBE_RT_CLASS_NAME,
          &myParent, attrValues);
      ...
      if (rc == SA_AIS_ERR_TRY_AGAIN) {
        ...
        msecs_waited += sleep_delay_ms;
             ...
             (msecs_waited < max_waiting_time_ms * 12)); /* 60 secs */
    ...
    if (sPbe2B) {
      ...
      for (ci = sClassIdMap->begin(); ci != sClassIdMap->end(); ++ci) {
        ...
        rc =
            saImmOmClassDescriptionGet_2(immHandle, (char *)(ci)->first.c_str(),
  ...
  if (!sPbe2B) { /* Not pbe2B i.e. pbe2A or not 2PBE at all. */
    rc =
        saImmOiClassImplementerSet(pbeOiHandle, (char *)OPENSAF_IMM_CLASS_NAME);
    msecs_waited = 0;
    while ((rc == SA_AIS_ERR_TRY_AGAIN) &&
           (msecs_waited < max_waiting_time_ms)) {
      usleep(sleep_delay_ms * 1000);
  ...
  if (rc != SA_AIS_OK) {
    LOG_WA(
        "PBE failed to fetch initial value for NoStdFlags, assuming all flags are zero");
  }
  TRACE_LEAVE();
...
void pbeDaemon(SaImmHandleT immHandle, void *dbHandle,
  ...
  while (immOiSelectionObject) {
    ...
    if (immHandle && (fds[FD_IMM_PBE_OM].revents & POLLIN)) {
      ...
      if (error != SA_AIS_OK) {
        LOG_WA(
            "saImmOmDispatch returned %u PBE lost contact with IMMND - exiting",
            error);
        if (sPbe2) {
