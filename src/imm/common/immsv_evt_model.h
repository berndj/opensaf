...
typedef struct ImmsvOmCcbObjectCreate {
  ...
  IMMSV_OCTET_STRING className;
  IMMSV_OCTET_STRING
      parentOrObjectDn;  // used for parent name and object name in _o3
...
typedef struct ImmsvOmCcbObjectDelete {
  SaUint32T ccbId;
  SaUint32T adminOwnerId;  // Rename? used for both adminOwnerId &
                           // continuationId
  IMMSV_OCTET_STRING objectName;
...
#define m_IMMSV_UNPACK_HANDLE_LOW(imm_handle) \
  (SaUint32T)((imm_handle)&0x00000000ffffffff)

#ifdef __cplusplus
}
#endif

#endif  // IMM_COMMON_IMMSV_EVT_MODEL_H_
