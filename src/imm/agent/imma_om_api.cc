...
/****************************************************************************
  ...
  Notes         :
******************************************************************************/
static SaAisErrorT initialize_common(SaImmHandleT *immHandle,
...
SaAisErrorT saImmOmInitialize_o2(SaImmHandleT *immHandle,
  ...
  if ((requested_version.releaseCode != 'A') ||
    ...
    TRACE_2(
        "ERR_VERSION: THIS SHOULD BE A VERSION A.2.11 initialize but claims to be"
        "%c %u %u",
        requested_version.releaseCode, requested_version.majorVersion,
  ...
  if (out_evt) {
    ...
    if ((value = getenv("IMMA_MAX_OPEN_SEARCHES_PER_HANDLE"))) {
      ...
      if (*value && !*endptr)
        ...
        LOG_WA(
            "IMMA_MAX_OPEN_SEARCHES_PER_HANDLE contains non-valid number value. Set default value (100)");
    }
...
SaAisErrorT saImmOmSelectionObjectGet(SaImmHandleT immHandle,
  ...
  *selectionObject = (-1); /* Ensure non valid descriptor in case of failure. */
  ...
  if (cl_node->isImmA2bCbk) { /* cl_node->isImmA2bCbk is true only if there is
                                 an A.2.11 callback */
    if (cl_node->o.mCallbkA2b.saImmOmAdminOperationInvokeCallback == NULL) {
      TRACE_2(
          "ERR_INVALID_PARAM: saImmOmSelectionObjectGet not allowed "
  ...
  if (cl_node->stale) {
    ...
    if (!resurrected || !cl_node || !(cl_node->isOm) || cl_node->stale) {
      TRACE_3("ERR_BAD_HANDLE: Reactive ressurect of handle %llx failed",
              immHandle);
...
SaAisErrorT saImmOmDispatch(SaImmHandleT immHandle,
  ...
  if (cl_node->stale) {
    ...
    if (!imma_om_resurrect(cb, cl_node, &locked)) {
      TRACE_3(
          "ERR_BAD_HANDLE: Failed to resurrect stale OM handle <c:%u, n:%x>",
    ...
    if (!(cl_node && cl_node->isOm)) {
      TRACE_3(
          "ERR_BAD_HANDLE: client_node_get failed AFTER successful resurrect!");
      rc = SA_AIS_ERR_BAD_HANDLE;
      goto fail;
    }
    ...
    if (cl_node->stale) {
      TRACE_3(
          "ERR_BAD_HANDLE: client became stale AGAIN after successful resurrect!");
      cl_node->exposed = true;
      rc = SA_AIS_ERR_BAD_HANDLE;
      goto fail;
    }
...
SaAisErrorT saImmOmFinalize(SaImmHandleT immHandle) {
  ...
  */
  ...
  if (cl_node->stale) {
    ...
    cl_node->exposed =
        true; /* But dont allow anyone else to resurrect it either */
    goto stale_handle;
  }
...
SaAisErrorT saImmOmAdminOwnerInitialize(
  ...
  if (nameLen >= IMMSV_MAX_ADMO_NAME_LENGTH) {
    TRACE_2("ERR_INVALID_PARAM: Admin owner name too long, size: %u max:%u",
            nameLen, IMMSV_MAX_ADMO_NAME_LENGTH - 1);
    return SA_AIS_ERR_INVALID_PARAM;
  }
...
mds_send_fail:
  ...
  if (cl_node->stale) {
    if (isExposed(cb, cl_node)) {
      TRACE_3("ERR_BAD_HANDLE: Handle %llx became stale and exposed",
              immHandle);
      rc = SA_AIS_ERR_BAD_HANDLE;
    } else {
      TRACE_3(
          "ERR_TRY_AGAIN: Handle %llx became stale but possible to resurrect",
          immHandle);
  ...
  do {
    ...
    if (proc_rc != NCSCC_RC_SUCCESS) {
      ...
      imma_admin_owner_node_get(&cb->admin_owner_tree,
                                &(ao_node->admin_owner_hdl), &ao_node_tmp);
      if (!ao_node_tmp) {
      ...
      TRACE_4(
          "Duplicate admin owner handle %llu (poor clock resolution) adjusting it to %llu",
...
static SaAisErrorT imma_newCcbId(IMMA_CB *cb, IMMA_CCB_NODE *ccb_node,
  ...
  TRACE("Sending request for new ccbid with admin OwnerId:%u", adminOwnerId);
...
mds_send_fail:
  ...
  *locked = true;
  ...
  if (!ccb_node) {
    TRACE_4(
        "ERR_LIBRARY: Ccb node gone, ccb_node->mExclusive not respected by "
        "another thread ?");
    rc = SA_AIS_ERR_LIBRARY;
    goto fail;
  }
  ...
  *ccbHandle = 0LL; /* User may have forgotten to initialize to zero */
  ...
  if (cl_node->stale) {
    ...
    /* Look up admin owner again since successful resurrect implies
       new admin-owner-id ! */
    imma_admin_owner_node_get(&cb->admin_owner_tree, &adminOwnerHandle,
                              &ao_node);
    if (!ao_node) {
  ...
  if (ccbFlags) {
    ...
    if (ccbFlagsTmp & SA_IMM_CCB_REGISTERED_OI) {
      ...
      if (ccbFlagsTmp & SA_IMM_CCB_ALLOW_NULL_OI) {
        ...
        if (!(cl_node->isImmA2b)) {
          TRACE(
              "ERR_VERSION: SA_IMM_CCB_ALLOW_NULL_OI"
  ...
  do {
    ...
    if (proc_rc != NCSCC_RC_SUCCESS) {
      ...
      ccb_node->ccb_hdl++;
      TRACE_4(
          "Duplicate ccb handle %llu (poor clock resolution) adjusting it to %llu",
...
/****************************************************************************
  ...
  Return Values :  Refer to SAI-AIS specification for various return values.
******************************************************************************/
static SaAisErrorT ccb_object_create_common(
    SaImmCcbHandleT ccbHandle, const SaImmClassNameT className,
    const SaNameT *parentName, const SaConstStringT objectName,
...
SaAisErrorT saImmOmCcbObjectCreate_2(SaImmCcbHandleT ccbHandle,
  ...
  return ccb_object_create_common(ccbHandle, className, parentName, NULL,
                                  attrValues);
}
...
SaAisErrorT saImmOmCcbObjectCreate_o3(SaImmCcbHandleT ccbHandle,
  ...
  return ccb_object_create_common(ccbHandle, className, NULL, objectName,
                                  attrValues);
}
...
static SaAisErrorT ccb_object_create_common(
  ...
  if (ccb_node->mExclusive) {
    rc = SA_AIS_ERR_TRY_AGAIN;
    TRACE_3(
        "ERR_TRY_AGAIN: Ccb-id %u being created or in critical phase, in another thread",
        ccb_node->mCcbId);
    goto done;
  }
  ...
  if (objectName && !cl_node->isImmA2f) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmCcbObjectCreate_o3 only supported for "
        "A.02.15 and above");
    goto done;
  }
  if (cl_node->isImmA2x12 && cl_node->clmExposed) {
  ...
  if (cl_node->stale) {
    ...
    if (!(ccb_node->mApplied)) {
      TRACE_3(
          "ERR_FAILED_OPERATION: IMMND DOWN discards ccb "
    ...
    if (!ccb_node) {
      rc = SA_AIS_ERR_BAD_HANDLE;
      TRACE_3(
          "ERR_BAD_HANDLE: Ccb handle not valid after successful resurrect - "
  ...
  imma_admin_owner_node_get(&cb->admin_owner_tree, &(ccb_node->mAdminOwnerHdl),
                            &ao_node);
  if (!ao_node) {
    rc = SA_AIS_ERR_LIBRARY;
  ...
  if (parentNameLength > 0) {
    ...
    if (!osaf_is_extended_names_enabled() &&
        objectNameLength >= SA_MAX_UNEXTENDED_NAME_LENGTH) {
      rc = SA_AIS_ERR_INVALID_PARAM;
  ...
  if (rc == SA_AIS_OK) {
    if (cl_node->stale) {
      ...
      */
      ...
      TRACE_3(
          "ERR_FAILED_OPERATION: Handle %llx became stale "
          "during the down-call",
          immHandle);
      ccb_node->mAborted = true;
      rc = SA_AIS_ERR_FAILED_OPERATION;
    }
     ...
     */
    TRACE_3(
        "ERR_FAILED_OPERATION: Converting TRY_AGAIN to "
...
clm_left:
  ...
  imma_free_errorStrings(
      newErrorStrings); /* In case of failed resurrect only */
...
/****************************************************************************
  ...
  Return Values :  Refer to SAI-AIS specification for various return values.
******************************************************************************/
static SaAisErrorT ccb_object_modify_common(
    SaImmCcbHandleT ccbHandle, SaConstStringT objectName,
    const SaImmAttrModificationT_2 **attrMods, bool bUseString);

SaAisErrorT saImmOmCcbObjectModify_2(
    SaImmCcbHandleT ccbHandle, const SaNameT *objectName,
...
SaAisErrorT saImmOmCcbObjectModify_o3(
    SaImmCcbHandleT ccbHandle, SaConstStringT objectName,
    const SaImmAttrModificationT_2 **attrMods) {
...
static SaAisErrorT ccb_object_modify_common(
  ...
  if (bUseString && !cl_node->isImmA2f) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmCcbObjectModify_o3 only supported for "
        "A.02.15 and above");
    goto done;
  }
  if (cl_node->isImmA2x12 && cl_node->clmExposed) {
  ...
  if (cl_node->stale) {
    ...
    if (ccb_node->mExclusive) {
      rc = SA_AIS_ERR_TRY_AGAIN;
      TRACE_3(
          "ERR_TRY_AGAIN: Ccb-id %u being created or in critical phase, "
          "in another thread",
          ccb_node->mCcbId);
      goto done;
    }
...
/****************************************************************************
  ...
  Return Values :  Refer to SAI-AIS specification for various return values.
******************************************************************************/
static SaAisErrorT ccb_object_delete_common(SaImmCcbHandleT ccbHandle,
...
SaAisErrorT saImmOmCcbObjectDelete_o3(SaImmCcbHandleT ccbHandle,
                                      SaConstStringT objectName) {
...
static SaAisErrorT ccb_object_delete_common(SaImmCcbHandleT ccbHandle,
  ...
  if (ccb_node->mAborted) {
    TRACE_2("ERR_FAILED_OPERATION: CCB %u already aborted", ccb_node->mCcbId);
    rc = SA_AIS_ERR_FAILED_OPERATION;
    goto done;
  }
  ...
  if (bUseString && !cl_node->isImmA2f) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmCcbObjectDelete_o3 only supported for "
        "A.02.15 and above");
    goto done;
  }
  if (cl_node->isImmA2x12 && cl_node->clmExposed) {
  ...
  if (cl_node->stale) {
    ...
    if (ccb_node->mAborted) {
      rc = SA_AIS_ERR_FAILED_OPERATION;
      TRACE_3("ERR_FAILED_OPERATION: Ccb-id %u was already aborted",
              ccb_node->mCcbId);
      goto done;
    }
  }
  ...
  if (rc == SA_AIS_OK) {
     ...
     */
    TRACE_3(
        "ERR_FAILED_OPERATION: Converting TRY_AGAIN to FAILED_OPERATION "
...
SaAisErrorT imma_applyCcb(SaImmCcbHandleT ccbHandle, bool onlyValidate) {
  ...
  TRACE("CCb node found for ccbhandle %llx ccbid:%u", ccbHandle,
        ccb_node->mCcbId);
  ...
  if (cl_node->stale || !(cb->is_immnd_up)) {
    TRACE_3(
        "ERR_FAILED_OPERATION: IMMND DOWN and/or IMM Handle %llx "
  ...
  if (ccb_node->mAugCcb) {
    if (onlyValidate) {
      LOG_IN(
          "ERR_BAD_OPERATION: saImmOmCcbValidate() not allowed on augmented ccbs");
      rc = SA_AIS_ERR_BAD_OPERATION;
    } else {
  ...
  evt.type = IMMSV_EVT_TYPE_IMMND;
  evt.info.immnd.type =
      (onlyValidate) ? IMMND_EVT_A2ND_CCB_VALIDATE : IMMND_EVT_A2ND_CCB_APPLY;
  ...
  if (out_evt) {
    ...
    if (rc != SA_AIS_OK) {
      TRACE_4(
          "CCB-APPLY - Error return from fevs!:%u, "
          "yet reply message also received (?) with error %u",
          rc, out_evt->info.imma.info.errRsp.error);
    ...
    TRACE_4(
        "CCB_APPLY - ERR_TIMEOUT: Got OK from fake_evs but no "
  ...
  if (!(cl_node && cl_node->isOm)) {
    ...
    if (rc == SA_AIS_OK) {
      ...
      */
      TRACE_3(
          "CCB APPLY - OK reply from IMMND on ccb-apply, but "
    ...
    if (cl_node->stale) {
      ...
      if (rc == SA_AIS_OK) {
        ...
        */
        TRACE_3(
            "CCB APPLY - Ok reply from IMMND overrides"
            "stale handle");
    ...
    if (ccb_node) {
      ...
      if (rc == SA_AIS_OK) {
        ...
        TRACE_1("CCB APPLY - Successful Apply for ccb id %u", ccb_node->mCcbId);
      } else {
        ccb_node->mErrorStrings = newErrorStrings;
       ...
       */
      TRACE_4(
          "CCB APPLY - Ccb-handle closed on return from "
          "ccb-apply down-call. Should not be possible!");
      if (rc == SA_AIS_OK) {
        TRACE_3(
            "CCB APPLY - Ok reply from server on ccb-apply, "
            "but ccbHandle was closed on return. OK on CCB "
            "overrides ccbHandle problem.");
      } else {
        TRACE_3(
            "CCB APPLY - CcbHandle no longer valid on "
            "return, Error %u already set",
            rc);
      }
    }
  }
  ...
  if (rc == SA_AIS_ERR_TRY_AGAIN) {
    TRACE_3(
        "CCB APPLY: Error in send/receive of ccb-apply message! "
    ...
    */
    ...
    if (ccb_node) {
      ...
      rc = SA_AIS_ERR_LIBRARY;
      TRACE_4(
          "CCB APPLY - .... ERR_LIBRARY: ccb-node removed during ccb-apply, "
  ...
  if (rc == SA_AIS_ERR_BAD_HANDLE) {
    TRACE_3(
        "CCB APPLY - ERR_TIMEOUT: Error during execution of "
        "ccb-apply! converting ERR_BAD_HANDLE to ERR_TIMEOUT");
    /* For the special case of ccb-apply we convert ERR_BAD_HANDLE to
       ERR_TIMEOUT. This to converge all error handling for UNKNOWN OUTCOME of a
       ccb to a single error code. Unknown result for a ccb is what the user
       least of all wants to deal with from apply, but it can happen.
  ...
  if (rc == SA_AIS_ERR_TIMEOUT) {
    ...
    */
    TRACE_3("client_node %p exposed :%u", cl_node,
    ...
    if (rc == SA_AIS_OK) {
      TRACE_3(
          "CCB APPLY - SA_AIS_OK: CCB result recovery SUCCEEDED - "
          "Apply/Commit for ccb id %u result was OK",
          ccbId);
      if (ccb_node) {
        ccb_node->mApplied = true;
      }
    ...
    if (rc == SA_AIS_ERR_TIMEOUT) {
      TRACE_4(
          "CCB APPLY: CCB result recovery FAILED for "
          "ccb id %u returning ERR_TIMEOUT",
          ccbId);
      if (ccb_node) {
        ccb_node->mExclusive = true;
    ...
    TRACE_3(
        "CCB APPLY: CCB result recovery SUCCEEDED - "
        "ABORT for ccb id %u",
        ccbId);
...
ccb-handle. Also resetts a ccbHandle that has previously received the abort
                   return code SA_AIS_ERR_FAILED_OPERATION.

                   Previously it was only possible to explicitly abort an active
ccb by invoking saImOmCcbFinalize() which also closes the ccb-handle.

                   Previously it was also not possible to reset a ccbHandle that
had received the ccb-aborted return code: SA_AIS_ERR_FAILED_OPERATION.
...
A.2.14. SA_AIS_ERR_BAD_OPERATION - saImmOmCcbAbort not allowed in ccb
                                              augmentation
...
additional ccb-operations can be added to the ccb until the current set of
...
state that only accepts one of:

                      - saImmOmCcbApply(ccbHandle);  - Only applicable if
saImmOmCcbValidate returned SA_AIS_OK, SA_AIS_ERR_TIMEOUT, or
SA_AIS_ERR_TRY_AGAIN (validation not done).

                      - saImmOmCcbAbort(ccbHandle);    - Always applicable if
handle is valid. - saImmOmCcbFinalize(ccbHandle); - Always applicable if handle
is valid.

  Arguments     :  ccbHandle - Ccb Handle

  Return Values :  Same return values as saImmOmCcbApply with the following
adjustment: SA_AIS_OK - Validation succeeded. Ccb has not been
committed/applied. This ccb can now be aborted or an attempt may be made to
                               apply/commit.

                   Refer to SAI-AIS IMM A.2.1 specification for the other return
values and to the OpenSAF_IMMSv_PR (programmers reference) for implementation
                   specific details.

                   Note that just as for regular saImm*OmCcbApply an abort is
signalled by: SA_AIS_ERR_FAILED_OPERATION - Ccb is aborted possibly due to
failed validation.
...
static SaAisErrorT admin_op_invoke_common(
    ...
    SaImmAdminOperationParamsT_2 ***returnParams, bool isA2bCall,
    bool bUseString);
...
SaAisErrorT saImmOmAdminOperationInvoke_o2(
  ...
  rc = admin_op_invoke_common(ownerHandle, objectNameStr, continuationId,
                              operationId, params, operationReturnValue,
                              timeout, returnParams, true, false);
...
SaAisErrorT saImmOmAdminOperationInvoke_o3(
    SaImmAdminOwnerHandleT ownerHandle, SaConstStringT objectName,
    SaImmContinuationIdT continuationId, SaImmAdminOperationIdT operationId,
    const SaImmAdminOperationParamsT_2 **params,
    SaAisErrorT *operationReturnValue, SaTimeT timeout,
  ...
  return admin_op_invoke_common(ownerHandle, objectName, continuationId,
                                operationId, params, operationReturnValue,
                                timeout, returnParams, true, true);
}
...
static SaAisErrorT admin_op_invoke_common(
  ...
  if (timeout < 0) {
    TRACE_4(
        "saImmOmAdminOperationInvoke: failed invalid timeout return value:%d,immHandle:%llx",
    ...
    TRACE_4(
        "saImmOmAdminOperationInvoke: timeout>MDS_MAX_TIMEOUT setting to max :%lld, immHandle:%llx",
  ...
  *operationReturnValue =
      SA_AIS_ERR_NO_SECTIONS; /* Set to bad value to prevent user mistakes. */
  ...
  if (!ao_node) {
    TRACE_2(
        "ERR_BAD_HANDLE: No admin owner associated with admin owner handle!");
    rc = SA_AIS_ERR_BAD_HANDLE;
    goto ao_not_found;
  }
  ...
  if (ao_node->mAugCcb) {
    TRACE_2(
        "ERR_NO_RESOURCES: Augmented CCB AdminOwner handle not allowed here");
    rc = SA_AIS_ERR_NO_RESOURCES;
    goto ao_not_found;
  }
  ...
  if (bUseString && !cl_node->isImmA2f) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmAdminOperationInvoke_o3 only supported for "
        "A.02.15 and above");
    goto client_not_found;
  }
  if (cl_node->isImmA2x12 && cl_node->clmExposed) {
  ...
  for (i = 0; params[i]; ++i) {
    ...
    if (!opNamePar) {
      opNamePar =
          opIdEsc && (strcmp(param->paramName, SA_IMM_PARAM_ADMOP_NAME) == 0);
  ...
  if (opIdEsc && !opNamePar && cl_node->isImmA2b) {
    TRACE_2("ERR_INVALID_PARAM: Op-id > %llx requires param %s",
            (long long unsigned int)SA_IMM_PARAM_ADMOP_ID_ESC,
            SA_IMM_PARAM_ADMOP_NAME);
    rc = SA_AIS_ERR_INVALID_PARAM;
    goto mds_send_fail;
  }
    ...
    if (out_evt->info.imma.type == IMMA_EVT_ND2A_IMM_ERROR) {
      ...
      *operationReturnValue = out_evt->info.imma.info.admOpRsp.result;
      ...
      if ((out_evt->info.imma.type == IMMA_EVT_ND2A_ADMOP_RSP_2) &&
        ...
        if (returnParams) {
          *returnParams =
              imma_proc_get_params(out_evt->info.imma.info.admOpRsp.parms);
          // imma_proc_free_pointers(cb, &(out_evt->info.imma)); ABT: Crashes,
          // figure out why!

          /* Check SaNameT values if the client side does not support extended
           * names. In this case if everything is ok, but the client does not
           * support extended names, operationReturnValue should be set to
           * SA_AIS_ERR_NAME_TOO_LONG, and SaNameT values should be cleared */
          if (!osaf_is_extended_names_enabled()) {
            i = 0;
            while ((*returnParams)[i]) {
              if ((*returnParams)[i]->paramType == SA_IMM_ATTR_SANAMET &&
                  !osaf_is_extended_name_valid(
                      (SaNameT *)(*returnParams)[i]->paramBuffer)) {
...
lock_fail1:
  if (out_evt && rc == SA_AIS_ERR_LIBRARY && returnParams &&
      *returnParams != NULL) {
...
SaAisErrorT saImmOmAdminOperationMemoryFree(
  ...
  if (!(cl_node->isImmA2b)) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmAdminOperationMemoryFree only supported for "
        "A.02.11 and above");
    rc = SA_AIS_ERR_VERSION;
    goto done;
  }
                                          ...
                                          SaInvocationT userInvoc)  // in
{
  ...
  cr = (IMMA_CONTINUATION_RECORD *)malloc(sizeof(IMMA_CONTINUATION_RECORD));
...
/****************************************************************************
  ...
  Notes         :
******************************************************************************/
static SaAisErrorT admin_op_invoke_async_common(
    ...
    const SaImmAdminOperationParamsT_2 **params, bool bUseString);
...
SaAisErrorT saImmOmAdminOperationInvokeAsync_2(
  ...
  rc = admin_op_invoke_async_common(ownerHandle, userInvocation, objectNameStr,
                                    continuationId, operationId, params, false);
...
SaAisErrorT saImmOmAdminOperationInvokeAsync_o3(
    SaImmAdminOwnerHandleT ownerHandle, SaInvocationT userInvocation,
    SaConstStringT objectName, SaImmContinuationIdT continuationId,
    SaImmAdminOperationIdT operationId,
    const SaImmAdminOperationParamsT_2 **params) {
  return admin_op_invoke_async_common(ownerHandle, userInvocation, objectName,
...
static SaAisErrorT admin_op_invoke_async_common(
  ...
  if (bUseString && !cl_node->isImmA2f) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmAdminOperationInvokeAsync_o3 only supported for "
        "A.02.15 and above");
    goto client_not_found;
  }
  if (cl_node->isImmA2x12 && cl_node->clmExposed) {
  ...
  if (cl_node->isImmA2bCbk) { /* cl_node->isImmA2bCbk is true only if there is
    ...
    if (cl_node->o.mCallbkA2b.saImmOmAdminOperationInvokeCallback == NULL) {
      rc = SA_AIS_ERR_INIT;
      TRACE_2(
          "ERR_INIT: The SaImmOmAdminOperationInvokeCallbackT_o2 "
    ...
    if (cl_node->o.mCallbk.saImmOmAdminOperationInvokeCallback == NULL) {
      rc = SA_AIS_ERR_INIT;
      TRACE_2(
          "ERR_INIT: The SaImmOmAdminOperationInvokeCallbackT "
          "function was not set in the initialization of the handle "
          "=> saImmOmAdminOperationInvokeAsync can not reply");
      goto no_callback;
    }
  ...
  if (!push_async_adm_op_continuation(cb,
    ...
    TRACE_2(
        "ERR_INVALID_PARAM: Provided invocation id (%llx) "
        "is not unique, not even in this client instance",
        userInvocation);
    rc = SA_AIS_ERR_INVALID_PARAM;
    goto mds_send_fail;
  }
  ...
  if (rc != SA_AIS_OK) {
    ...
    if (!imma_popAsyncAdmOpContinuation(cb,
                                        evt.info.immnd.info.admOpReq.invocation,
                                        &immHandle, &userInvocation)) {
      TRACE_3(
          "Missmatch on continuation for saImmOmAdminOperationInvokeAsync_2");
    }
  }
...
SaAisErrorT saImmOmAdminOperationContinue(SaImmAdminOwnerHandleT ownerHandle,
  ...
  TRACE_2(
      "saImmOmAdminOperationContinue not implemented use saImmOmAdminOperationContinueAsync instead");
  return SA_AIS_ERR_NOT_SUPPORTED;
}

SaAisErrorT saImmOmAdminOperationContinue_o2(
    SaImmAdminOwnerHandleT ownerHandle, const SaNameT *objectName,
    SaImmContinuationIdT continuationId, SaAisErrorT *operationReturnValue,
    SaImmAdminOperationParamsT_2 ***returnParams) {
  TRACE_2(
      "saImmOmAdminOperationContinue_o2 not implemented use "
      "saImmOmAdminOperationContinueAsync instead");
  return SA_AIS_ERR_NOT_SUPPORTED;
}

SaAisErrorT saImmOmAdminOperationContinue_o3(
    SaImmAdminOwnerHandleT ownerHandle, SaConstStringT objectName,
    SaImmContinuationIdT continuationId, SaAisErrorT *operationReturnValue,
    SaImmAdminOperationParamsT_2 ***returnParams) {
  TRACE_2(
      "saImmOmAdminOperationContinue_o3 not implemented use "
      "saImmOmAdminOperationContinueAsync instead");
  return SA_AIS_ERR_NOT_SUPPORTED;
}

SaAisErrorT saImmOmAdminOperationContinueAsync(
    SaImmAdminOwnerHandleT ownerHandle, SaInvocationT invocation,
    const SaNameT *objectName, SaImmContinuationIdT continuationId) {
  TRACE_2("saImmOmAdminOperationContinueAsync not yet implemented");
  return SA_AIS_ERR_NOT_SUPPORTED;
}

SaAisErrorT saImmOmAdminOperationContinueAsync_o3(
    SaImmAdminOwnerHandleT ownerHandle, SaInvocationT invocation,
    SaConstStringT objectName, SaImmContinuationIdT continuationId) {
...
SaAisErrorT saImmOmAdminOperationContinuationClear_o3(
    SaImmAdminOwnerHandleT ownerHandle, SaConstStringT objectName,
    SaImmContinuationIdT continuationId) {
...
SaAisErrorT saImmOmClassCreate_2(
  ...
  for (i = 0; attr != 0; attr = attrDefinitions[++i]) {
    ...
    if (attr->attrFlags & SA_IMM_ATTR_RDN) {
      if (((attr->attrValueType != SA_IMM_ATTR_SANAMET) &&
        ...
        TRACE(
            "ERR_INVALID_PARAM: RDN '%s' must be of type SaNameT or SaStringT",
            attr->attrName);
        TRACE_LEAVE();
        return SA_AIS_ERR_INVALID_PARAM;
      }
      ...
      if (attr->attrFlags & SA_IMM_ATTR_MULTI_VALUE) {
        TRACE("ERR_INVALID_PARAM: RDN '%s' can not be multivalued",
              attr->attrName);
        TRACE_LEAVE();
        return SA_AIS_ERR_INVALID_PARAM;
      }
      ...
      if (attr->attrFlags & SA_IMM_ATTR_NO_DANGLING) {
        TRACE("ERR_INVALID_PARAM: RDN '%s' can not have NO_DANGLING flag",
              attr->attrName);
        TRACE_LEAVE();
        return SA_AIS_ERR_INVALID_PARAM;
      }
      ...
      if (attr->attrFlags & SA_IMM_ATTR_DN) {
        TRACE("ERR_INVALID_PARAM: RDN '%s' can not have SA_IMM_ATTR_DN flag",
              attr->attrName);
        TRACE_LEAVE();
        return SA_AIS_ERR_INVALID_PARAM;
      }
      ...
      if (attr->attrFlags & SA_IMM_ATTR_STRONG_DEFAULT) {
        TRACE("ERR_INVALID_PARAM: RDN '%s' can not have STRONG_DEFAULT flag",
              attr->attrName);
        TRACE_LEAVE();
        return SA_AIS_ERR_INVALID_PARAM;
      }
    ...
    if (attr->attrFlags & SA_IMM_ATTR_NO_DANGLING) {
      if (classCategory == SA_IMM_CLASS_RUNTIME) {
        TRACE(
            "ERR_INVALID_PARAM: NO_DANGLING attribute '%s' cannot be defined for runtime class",
            attr->attrName);
        TRACE_LEAVE();
        return SA_AIS_ERR_INVALID_PARAM;
      }
      ...
      if ((attr->attrValueType != SA_IMM_ATTR_SANAMET) &&
          !((attr->attrFlags & SA_IMM_ATTR_DN) &&
            (attr->attrValueType == SA_IMM_ATTR_SASTRINGT))) {
        TRACE(
            "ERR_INVALID_PARAM: Attribute '%s' is flagged NO_DANGLING, must be of type SaNameT or SaStringT with DN flag",
            attr->attrName);
        TRACE_LEAVE();
        return SA_AIS_ERR_INVALID_PARAM;
      }
      ...
      if (attr->attrFlags & SA_IMM_ATTR_RUNTIME) {
        TRACE(
            "ERR_INVALID_PARAM: Runtime attribute '%s' cannot have NO_DANGLING flag",
            attr->attrName);
        TRACE_LEAVE();
        return SA_AIS_ERR_INVALID_PARAM;
      }
  ...
  for (i = 0; attr != 0; attr = attrDefinitions[++i]) {
    ...
    if ((attr->attrFlags & SA_IMM_ATTR_STRONG_DEFAULT) &&
        !(cl_node->isImmA2x11)) {
      TRACE_2(
          "SA_IMM_ATTR_STRONG_DEFAULT flag is supported in version A.02.17 or higher");
      rc = SA_AIS_ERR_VERSION;
      goto mds_send_fail;
    }
    ...
    if (!imma_proc_is_valid_type((SaImmValueTypeT)p->d.attrValueType)) {
      rc = SA_AIS_ERR_INVALID_PARAM;
      TRACE("Unknown type not allowed for attr:%s class%s", p->d.attrName.buf,
            evt.info.immnd.info.classDescr.className.buf);
    ...
    if (attr->attrDefaultValue) {
      ...
      /*alloc-4.2 */
      imma_copyAttrValue(p->d.attrDefaultValue, attr->attrValueType,
                         attr->attrDefaultValue);
    }

    p->next =
        evt.info.immnd.info.classDescr.attrDefinitions; /*NULL initially */
...
SaAisErrorT saImmOmClassDescriptionGet_2(
  ...
  if ((className == NULL) || (classCategory == NULL) ||
      (attrDefinition == NULL)) {
    TRACE_LEAVE();
    return SA_AIS_ERR_INVALID_PARAM;
  }
  ...
  if (out_evt) {
    ...
    if (out_evt->info.imma.type == IMMA_EVT_ND2A_IMM_ERROR) {
      ...
      *classCategory =
          (SaImmClassCategoryT)out_evt->info.imma.info.classDescr.classCategory;
      ...
      for (; i < noOfAttributes; i++) {
        ...
        if (!q->attrDefaultValue) {
          ...
          switch (q->attrValueType) {
            ...
            case SA_IMM_ATTR_SANAMET:
              namep = (SaNameT *)copyv;
              osaf_extended_name_alloc(q->attrDefaultValue->val.x.buf,
                                       namep); /*alloc-5 */
              break;
            ...
            case SA_IMM_ATTR_SAANYT:
              ...
              anyp->bufferAddr =
                  (SaUint8T *)malloc(anyp->bufferSize); /*alloc-5 */
          ...
          immsv_evt_free_att_val(q->attrDefaultValue,
...
SaAisErrorT saImmOmClassDescriptionMemoryFree_2(
    SaImmHandleT immHandle, SaImmAttrDefinitionT_2 **attrDefinition) {
  IMMA_CB *cb = &imma_cb;
  IMMA_CLIENT_NODE *cl_node = NULL;
  TRACE_ENTER();
...
SaAisErrorT saImmOmAccessorInitialize(SaImmHandleT immHandle,
  ...
  if (cl_node->searchHandleSize >= cl_node->maxSearchHandles) {
    TRACE_4(
        "ERR_NO_RESOURCES: Too many search handles (%u) on OM handle - probable resource leak",
  ...
  do {
    ...
    if (proc_rc != NCSCC_RC_SUCCESS) {
      ...
      imma_search_node_get(&cb->search_tree, &(search_node->search_hdl),
                           &search_node_tmp);
      if (!search_node_tmp) {
      ...
      TRACE_4(
          "Duplicate search handle %llu (poor clock resolution) adjusting it to %llu",
...
SaAisErrorT saImmOmAccessorFinalize(SaImmAccessorHandleT accessorHandle) {
  ...
  if (proc_rc != NCSCC_RC_SUCCESS) {
    ...
    if (cl_node
            ->isOm) { /* TODO: Is osafassert(cl_node && cl_node->isOm) better
                         solution */
...
static SaAisErrorT accessor_get_common(SaImmAccessorHandleT accessorHandle,
                                       ...
                                       bool bUseString, SaUint32T ccbId);
...
static SaAisErrorT accessor_get_common(SaImmAccessorHandleT accessorHandle,
  ...
  if (bUseString && !cl_node->isImmA2f) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmAccessorGet_o3 only supported for "
        "A.02.15 and above");
    goto release_lock;
  }
  ...
  if (ccbId && !cl_node->isImmA2x11) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: Safe read only supported for "
        "A.02.17 and above");
    goto release_lock;
  }
  ...
  if (attributeNames) {
    ...
    if (*namev && (*(namev + 1)) == NULL &&
      ...
      */
      ...
      if (!(cl_node->isImmA2b)) {
        TRACE(
            "ERR_VERSION: search option SA_IMM_SEARCH_GET_CONFIG_ATTR "
            "requires IMM version A.02.11");
        rc = SA_AIS_ERR_VERSION;
        goto mds_send_fail;
      }
    ...
    for (; *namev; namev++) {
      IMMSV_ATTR_NAME_LIST *p = (IMMSV_ATTR_NAME_LIST *)malloc(
          sizeof(IMMSV_ATTR_NAME_LIST)); /* alloc-2 */
    ...
    /* Exclude RDN for saImmOmAccessorGet_o3 when SA_IMM_SEARCH_GET_ALL_ATTR is
     * set */
    if (bUseString) {
  ...
  if (out_evt) {
    ...
    if (attributes) {
      ...
      p = attrValueList;
      attr = (SaImmAttrValuesT_2 **)calloc(
          noOfAttributes + 1, sizeof(SaImmAttrValuesT_2 *)); /* alloc-1 */
      for (i = 0; i < noOfAttributes; i++, p = p->next) {
        ...
        if (q->attrValuesNumber) {
          attr[i]->attrValues = (SaImmAttrValueT *)calloc(
              1, q->attrValuesNumber * sizeof(SaImmAttrValueT)); /*alloc-4 */
          /*alloc-5 */
          ...
          if (q->attrValuesNumber > 1) {
            ...
            for (ix = 1; ix < q->attrValuesNumber; ++ix) {
              osafassert(r);
              attr[i]->attrValues[ix] = imma_copyAttrValue3(
                  (SaImmValueTypeT)q->attrValueType, &(r->n)); /*alloc-5 */
              r = r->next;
            }
          }
        }
...
mds_send_fail:
  ...
  if (cl_node->stale) {
    if (isExposed(cb, cl_node)) {
      ...
      TRACE_2(
          "ERR_TRY_AGAIN: Handle %llx is stale after down call but "
    ...
    if (out_evt->info.imma.info.searchNextRsp) {
      if (out_evt->info.imma.info.searchNextRsp->attrValuesList) {
        immsv_free_attrvalues_list(
            out_evt->info.imma.info.searchNextRsp->attrValuesList);
      }
...
static unsigned int get_attr_list_size(IMMSV_EDU_ATTR_VAL_LIST *al,
                                       const SaImmValueTypeT t) {
  unsigned int size = 0;
  while (al) {
...
static unsigned int get_obj_size(const IMMSV_OM_OBJECT_SYNC *batch) {
  ...
  while (avl) {
    obj_size +=
        (avl->n.attrName.size + 4 + 1); /* string + size + next-marker */
...
object transactionally. The API works exactly the same way as
                   saImmOmAccessorGet, except that

                   a) The API takes a SaImmCcbHandleT instead of a
SaImmAccessorHandleT

                   b) The values returned for the objects config attributes are
from a version of the object consistent with the CCB/transaction. This means
either the latest applied version or a newer version created in the same CCB but
not yet applied.

                   c) Access to an object that has been deleted in the same CCB
but not applied is rejected with ERR_NOT_EXIST.

                   d) Access to an object that has been created in the same CCB
but not applied is allowed, providing the latest version of the config
attributes in that CCB.

                   e) Safe read is not allowed using a runtime object as target.

                   Runtime attributes residing in a config object are handled
exactly the same as for saImmOmAccessorGet. The reason a safe-read call is not
allowed on a runtime *object* is that a runtime object *only* contains runtime
attributes. Performing a safe-read on a runtime object makes no sense.
...
to exceptions in stated in the description
...
SaAisErrorT saImmOmCcbObjectRead(SaImmCcbHandleT ccbHandle,
  ...
  bool locked = false;
  SaImmAccessorHandleT accessorHandle =
      0LL;  // handle value will be copied from ccb-node
  IMMA_ADMIN_OWNER_NODE *ao_node = NULL;
  IMMA_CCB_NODE *ccb_node = NULL;
  SaImmHandleT immHandle = 0LL;
  IMMA_CLIENT_NODE *cl_node = NULL;
  ...
  if (!cl_node->isImmA2x11) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmCcbObjectRead only supported for "
        "A.02.17 and above");
    goto done;
  }

  /* Stale check and possible resurrect done here for the complete ccb-handle
     level. The stale check later done in the call via accessorGet wil be
     redundant. But the handle should never be stale there if call arrived from
     here.
   */
  if (cl_node->stale) {
    TRACE_1("IMM Handle %llx is stale", immHandle);
  ...
  if (rc == SA_AIS_ERR_INTERRUPT) {
    ...
    memset(&evt, 0, sizeof(IMMSV_EVT));
    evt.type = IMMSV_EVT_TYPE_IMMND;
    /* This is a hack. Re-using the SAFE_READ msg type here only to ackquire
     * lock. */
...
SaAisErrorT immsv_sync(SaImmHandleT immHandle, const SaImmClassNameT className,
                       const SaNameT *objectName,
                       const SaImmAttrValuesT_2 **attrValues, void **batchp,
  ...
  TRACE_ENTER2("remainingSpace %d objsInBatch:%u", *remainingSpacep,
               objsInBatch);
  ...
  if ((className == NULL) || (objectName == NULL) ||
      (attrValues == NULL && batchp == NULL)) {
    LOG_ER(
        "(className == NULL) || (objectName == NULL) || (attrValues == NULL && batchp == NULL)");
    abort();
  }

  /*
    Cases:
    (A) (attrValues == NULL && batchp != NULL) => send the batch. Client
    iteration complete.

    (B) (attrValues != NULL && batchp == NULL) => send a single object (old
    nonbatched variant).

    (C) (attrValues != NULL && batchp != NULL) => add to the batch and send if
    batch full.
  */
  ...
  osafassert(!osaf_is_extended_name_empty(objectName) &&
  ...
  evt.info.immnd.info.obj_sync.objectName.buf =
      (char *)malloc(evt.info.immnd.info.obj_sync.objectName.size);
  ...
  evt.info.immnd.info.obj_sync.objectName
      .buf[evt.info.immnd.info.obj_sync.objectName.size - 1] = '\0';
  ...
  for (i = 0; attrValues[i]; ++i) {
    ...
    p->n.attrName.size = strlen(attr->attrName) + 1;
    if (p->n.attrName.size >= IMMSV_MAX_ATTR_NAME_LENGTH) {
      TRACE_2("ERR_INVALID_PARAM: Attribute name too long: %u",
              p->n.attrName.size);
      rc = SA_AIS_ERR_INVALID_PARAM;
      free(p);
      p = NULL;
  ...
  if (batchp) {
    ...
    if ((*remainingSpacep) > 0) {
       ...
       */
      IMMSV_OM_OBJECT_SYNC *tmp =
          (IMMSV_OM_OBJECT_SYNC *)calloc(1, sizeof(IMMSV_OM_OBJECT_SYNC));
      tmp->next = batch;
      (*batchp) = tmp;
...
static SaAisErrorT search_init_common(
    SaImmHandleT immHandle, SaConstStringT rootName, SaImmScopeT scope,
...
SaAisErrorT saImmOmSearchInitialize_2(
  ...
  rc = search_init_common(immHandle, rootNameStr, scope, searchOptions,
                          searchParam, attributeNames, searchHandle, false);
...
SaAisErrorT saImmOmSearchInitialize_o3(
    SaImmHandleT immHandle, SaConstStringT rootName, SaImmScopeT scope,
    SaImmSearchOptionsT searchOptions,
    const SaImmSearchParametersT_2 *searchParam,
    const SaImmAttrNameT *attributeNames, SaImmSearchHandleT *searchHandle) {
  return search_init_common(immHandle, rootName, scope, searchOptions,
                            searchParam, attributeNames, searchHandle, true);
}
...
static SaAisErrorT search_init_common(
  ...
  bool isNoDanglingSearch =
      searchOptions & SA_IMM_SEARCH_NO_DANGLING_DEPENDENTS;
  IMMA_CB *cb = &imma_cb;
  IMMSV_EVT evt;
  IMMSV_EVT *out_evt = NULL;
  IMMA_CLIENT_NODE *cl_node = NULL;
  ...
  if ((searchOptions & SA_IMM_SEARCH_GET_SOME_ATTR) &&
      (attributeNames == NULL)) {
    TRACE_2(
        "ERR_INVALID_PARAM: SA_IMM_SEARCH_GET_SOME_ATTR is set & "
  ...
  if (bUseString && !cl_node->isImmA2f) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmSearchInitialize_o3 only supported for "
        "A.02.15 and above");
    goto release_lock;
  }
  if (cl_node->isImmA2x12 && cl_node->clmExposed) {
  ...
  *searchHandle = 0LL;
  ...
  if (attributeNames && (!(searchOptions & SA_IMM_SEARCH_GET_SOME_ATTR))) {
    TRACE_2(
        "ERR_IVALID_PARAM: attributeNames != NULL yet searchOptions set to IMM_SEARCH_GET_SOME_ATTR");
    rc = SA_AIS_ERR_INVALID_PARAM;
    goto release_lock;
  }
  ...
  if (isNoDanglingSearch) {
    if (!cl_node->isImmA2d) {
      TRACE(
          "ERR_VERSION: search option SA_IMM_SEARCH_NO_DANGLING_DEPENDENTS "
    ...
    if (!searchParam || !searchParam->searchOneAttr.attrValue) {
      TRACE(
          "ERR_INVALID_PARAM: DN of an object must be specified "
    ...
    if (searchParam->searchOneAttr.attrValueType != SA_IMM_ATTR_SANAMET &&
      ...
      TRACE(
          "ERR_INVALID_PARAM: attrValueType must be SA_IMM_ATTR_SANAMET or "
  ...
  /* Exclude RDN from saImmOmSearchNext_o3 when SA_IMM_SEARCH_GET_ALL_ATTR is
   * set */
  ...
  if (!searchParam ||
      (!searchParam->searchOneAttr.attrName && !isNoDanglingSearch)) {
    if (osaf_is_extended_names_enabled()) {
      req->searchParam.present = ImmOmSearchParameter_PR_NOTHING;
    } else {
      req->searchParam.present =
    ...
    if (searchParam->searchOneAttr.attrName) {
      ...
      req->searchParam.choice.oneAttrParam.attrName
          .buf[req->searchParam.choice.oneAttrParam.attrName.size - 1] = 0;
    } else {
  ...
  if (attributeNames && !isAccGetConfigAttrs) {
    ...
    for (namev = attributeNames; *namev; namev++) {
      IMMSV_ATTR_NAME_LIST *p = (IMMSV_ATTR_NAME_LIST *)malloc(
          sizeof(IMMSV_ATTR_NAME_LIST)); /*alloc-4 */
      p->name.size = strlen(*namev) + 1;
...
SaAisErrorT saImmOmSearchNext_o3(SaImmSearchHandleT searchHandle,
                                 SaStringT *objectName,
                                 SaImmAttrValuesT_2 ***attributes) {
...
static SaAisErrorT search_next_common(SaImmSearchHandleT searchHandle,
  ...
  if (bUseString && !cl_node->isImmA2f) {
    ...
    TRACE_2(
        "ERR_VERSION: saImmOmSearchNext_o3 only supported for "
        "A.02.15 and above");
    goto release_lock;
  }
  ...
  if (cl_node->stale) {
    ...
    /*
       ...
       We can compare with CCBs and ccbHandles. For a ccb operation we can
       always return FAILED_OPERATION which just indicates that the CCB is
       failed/aborted, without giving away the whole game that the immHandle may
       be stale. But there is no corresponding return code for searchNext.

       For searchInitialize and accessor operations, we can hide the stalenes
       of a handle by returning TRY_AGAIN, which gives resurrect a chance to
       make a retry truly possible. But we can not return a TRY_AGAIN on a
       searchNext to mask a stale handle, because a searchNext is a continuation
       operation. What we need is a return code telling the user to restart the
       iteration from scratch (re initialize). There is no such return code.

       TIMEOUT and NO_RESOURCES could be returned here, but the user is not
       likely to react to these codes by restarting the iteration, despite that
       this would be an allowed reaction and quite likley to succeed when it was
       just a stale immHandle that was resurrect'able.
    */
  }
...
searchresult:
  if (out_evt && error == SA_AIS_OK) {
    ...
    if (out_evt->info.imma.type == IMMA_EVT_ND2A_SEARCHNEXT_RSP)
      ...
      search_node->searchBundle =
          out_evt->info.imma.info.searchBundleNextRsp; /* Borrow the pointer */
      out_evt->info.imma.info.searchBundleNextRsp = NULL;
      res_body = search_node->searchBundle->searchResult[0];
    }
  } else if (search_node->searchBundle &&
             error == SA_AIS_OK) { /* Read from buffer */
    res_body =
        search_node->searchBundle->searchResult[search_node->searchIndex++];
  }
...
release_evt:
  ...
  if (out_evt) {
    ...
    if (out_evt->info.imma.type == IMMA_EVT_ND2A_SEARCHNEXT_RSP &&
      ...
      free(out_evt->info.imma.info.searchNextRsp);
      out_evt->info.imma.info.searchNextRsp = NULL;
    } else if (out_evt->info.imma.type == IMMA_EVT_ND2A_SEARCHBUNDLENEXT_RSP &&
               out_evt->info.imma.info.searchBundleNextRsp) {
  ...
  if (search_node && search_node->searchBundle &&
      search_node->searchIndex == search_node->searchBundle->resultSize) {
    bFreeSearchBundle = true;
      ...
      if (search_node) {
        if (imma_search_node_delete(cb, search_node) == NCSCC_RC_SUCCESS) {
          ...
          if (cl_node && cl_node->isOm) {
            ...
            TRACE_2(
                "ERR_LIBRARY: Invalid SaImmHandleT related to search handle");
            error = SA_AIS_ERR_LIBRARY;
          }
        }
        search_node = NULL;
...
static SaAisErrorT admin_owner_set_common(
    SaImmAdminOwnerHandleT adminOwnerHandle, SaConstStringT *objectNames,
    SaImmScopeT scope, bool bUseString);
...
SaAisErrorT saImmOmAdminOwnerSet(SaImmAdminOwnerHandleT adminOwnerHandle,
  ...
  rc = admin_owner_set_common(adminOwnerHandle,
                              (SaConstStringT *)objectNamesStr, scope, false);
...
static SaAisErrorT admin_owner_set_common(
  ...
  if (!ao_node) {
    TRACE_2("ERR_BAD_HANDLE: Admin owner node is missing or %llx",
            adminOwnerHandle);
    rc = SA_AIS_ERR_BAD_HANDLE;
    goto done;
  }
  ...
  if (ao_node->mAugCcb) {
    ...
    IMMA_CCB_NODE *ccb_node = NULL;
    SaImmCcbHandleT ccbHandle =
        ao_node->mImmHandle; /* Same handle value used */
    imma_ccb_node_get(&cb->ccb_tree, &ccbHandle, &ccb_node);
    if (!ccb_node || !(ccb_node->mAugCcb) || ccb_node->mApplied ||
        ccb_node->mAborted) {
      TRACE_2(
          "ERR_NO_RESOURCES: AdminOwner handle obtained from "
  ...
  if (bUseString && !cl_node->isImmA2f) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmAdminOwnerSet_o3 only supported for "
        "A.02.15 and above");
    goto done;
  }
  if (cl_node->isImmA2x12 && cl_node->clmExposed) {
  ...
  for (i = 0; objectNames[i]; ++i) {
    IMMSV_OBJ_NAME_LIST *ol =
        (IMMSV_OBJ_NAME_LIST *)calloc(1, sizeof(IMMSV_OBJ_NAME_LIST)); /*a */
    ol->name.size = strlen(objectNames[i]) + 1;
    ol->name.buf = (char *)objectNames[i];
    ol->next =
        admo_set_evt.info.immnd.info.admReq.objectNames; /*null initially */
    admo_set_evt.info.immnd.info.admReq.objectNames = ol;
  }

  rc = imma_evt_fake_evs(cb, &admo_set_evt, &out_evt, timeout, cl_node->handle,
                         &locked, true);
  cl_node = NULL;
  ...
  while (admo_set_evt.info.immnd.info.admReq.objectNames != NULL) {
    ...
    admo_set_evt.info.immnd.info.admReq.objectNames =
        admo_set_evt.info.immnd.info.admReq.objectNames->next;
...
static SaAisErrorT admin_owner_release_common(
    SaImmAdminOwnerHandleT adminOwnerHandle, SaConstStringT *objectNames,
    SaImmScopeT scope, bool bUseString);
...
SaAisErrorT saImmOmAdminOwnerRelease(SaImmAdminOwnerHandleT adminOwnerHandle,
  ...
  rc = admin_owner_release_common(
      adminOwnerHandle, (SaConstStringT *)objectNamesStr, scope, false);
...
static SaAisErrorT admin_owner_release_common(
  ...
  if (ao_node->mAugCcb) {
    TRACE_2(
        "ERR_BUSY: Admin owner handle from augmented CCB cant be "
        "used for release.");
    rc = SA_AIS_ERR_BUSY;
    goto done;
  }
  ...
  if (bUseString && !cl_node->isImmA2f) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmAdminOwnerRelease_o3 only supported for "
        "A.02.15 and above");
    goto done;
  }
...
SaAisErrorT saImmOmAdminOwnerClear(SaImmHandleT immHandle,
  ...
  rc = admin_owner_clear_common(immHandle, (SaConstStringT *)objectNamesStr,
                                scope, false);
...
SaAisErrorT saImmOmAdminOwnerClear_o3(SaImmHandleT immHandle,
                                      SaConstStringT *objectNames,
                                      SaImmScopeT scope) {
...
static SaAisErrorT admin_owner_clear_common(SaImmHandleT immHandle,
  ...
  if (bUseString && !cl_node->isImmA2f) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmAdminOwnerClear_o3 only supported for "
        "A.02.15 and above");
    goto done;
  }
  if (cl_node->isImmA2x12 && cl_node->clmExposed) {
...
SaAisErrorT saImmOmAdminOwnerFinalize(SaImmAdminOwnerHandleT adminOwnerHandle) {
  ...
  rc = imma_evt_fake_evs(cb, &finalize_evt, &out_evt, timeout, cl_node->handle,
                         &locked, false);
  cl_node = NULL;
  ao_node = NULL;
...
static SaAisErrorT imma_finalizeCcb(SaImmCcbHandleT ccbHandle,
  ...
  SaUint32T adminOwnerId = 0;
  SaTimeT timeout = 0;
  SaImmAccessorHandleT safeReadAccessorHandle =
      0LL;  // Copied from ccb_node later.
  TRACE_ENTER();
  ...
  TRACE_1("CCb node found for ccbhandle %llx ccbid:%u", ccbHandle,
          ccb_node->mCcbId);
  if (ccb_node->mExclusive) {
    rc = SA_AIS_ERR_TRY_AGAIN;
    TRACE_3(
  ...
  if (ccb_node->mAugCcb) {
    if (keepCcbHandleOpen) {
      LOG_IN(
          "ERR_BAD_OPERATION: saImmOmCcbAbort() not allowed inside a ccb augmentation");
      rc = SA_AIS_ERR_BAD_OPERATION;
      goto done;
    }
  ...
  safeReadAccessorHandle =
      ccb_node->mCcbObjectReadAccessorHandle; /* Safe read handle */
  ...
  if (keepCcbHandleOpen && !(cl_node->isImmA2e)) {
    ccb_node->mExclusive = false;
    LOG_IN(
        "ERR_VERSION: saImmOmCcbAbort() only supported in version A.02.14 or higher");
    rc = SA_AIS_ERR_VERSION;
    goto done;
  }
  ...
  if (ccb_node) {
    ...
    if (keepCcbHandleOpen) { /* saImmOmCcbAbort */
      ...
      /* Get the Admin Owner info  */
      imma_admin_owner_node_get(&cb->admin_owner_tree, &adminOwnerHdl,
                                &ao_node);
      if (!ao_node) {
        rc = SA_AIS_ERR_BAD_HANDLE;
        TRACE_3(
            "ERR_BAD_HANDLE: Amin-Owner node associated with Ccb is missing");
        goto done;
      }
      ...
      if ((rc = imma_proc_increment_pending_reply(cl_node, true)) !=
          SA_AIS_OK) {
        TRACE_4(
            "ERR_LIBRARY: Overlapping use of IMM handle by multiple threads");
        goto done;
      }
      ...
      if (!(cl_node && cl_node->isOm)) {
        rc = SA_AIS_ERR_BAD_HANDLE;
        TRACE_3("ERR_BAD_HANDLE: Client node not found after down call");
        imma_ccb_node_delete(cb,
                             ccb_node); /*Remove node from tree and free it. */
        ccb_node = NULL;
        goto done;
      }
...
*/
static bool imma_re_initialize_admin_owners(IMMA_CB *cb,
                                            SaImmHandleT immHandle) {
  SaUint32T proc_rc = NCSCC_RC_SUCCESS;
    ...
    if (adm_node->mImmHandle == immHandle) {
      ...
      if (adm_found_node) {
        ...
        if (adm_node->mReleaseOnFinalize) {
           ...
           */
          TRACE_1(
              "Cant re-attach admin-owner %s because "
              "releaseOnFinalize was set",
              adm_node->mAdminOwnerName);
          goto fail;
        }
      }
    }
  ...
  TRACE_1(
      "Successfully re-initialized AdminOwner %s AOhandle %llx with "
      "id %u for resurrected client handle %llx",
      adm_found_node->mAdminOwnerName, temp_hdl, adm_found_node->mAdminOwnerId,
      immHandle);
...
int imma_om_resurrect(IMMA_CB *cb, IMMA_CLIENT_NODE *cl_node, bool *locked) {
  ...
  *locked = false;
  ...
  TRACE_3(
      "Failed to re-initialize admin-owner for resurrected "
...
/****************************************************************************
  ...
  Description   :  Allows the OM client to fetch error strings related to a
                   ...
                   This is only of interest when the down-call returned an
error, such as FAILED_OPERATION or BAD_OPERATION. That is, an error that could
result from one or more OIs rejecting the ccb call.

  Arguments     :  ccbHandle    - The ccb handle.
                   errorStrings - Pointer to a pointer to a NULL terminated
array of SaStringT. The list of strings is owned by the imma om library and is
cleared/deallocated in conjunction with the next downcall using the ccb-handle.
...

{
  ...
  if (!(cl_node->isImmA2b)) {
    rc = SA_AIS_ERR_VERSION;
    TRACE_2(
        "ERR_VERSION: saImmOmCcbGetErrorStrings only supported for "
        "A.02.11 and above");
    goto client_not_found;
  }
...
*/
SaAisErrorT immsv_om_augment_ccb_initialize(SaImmOiHandleT privateOmHandle,
                                            SaUint32T ccbId,
                                            SaUint32T adminOwnerId,
                                            SaImmCcbHandleT *ccbHandle,
                                            SaImmAdminOwnerHandleT *ownerHandle)

{
  ...
  TRACE("ccbHandle(%p) && ccbId(%u) && adminOwnerId(%u)", ccbHandle, ccbId,
        adminOwnerId);
  ...
  if (!(cl_node->isImmA2b)) {
    TRACE(
        "ERR_VERSION: immsv_om_augment_ccb_initialize "
  ...
  */
  ...
  if (ao_node) {
    ...
    if (ccb_node) {
      ...
      TRACE_1(
          "Admo handle found but ccb handle non existent -"
          " should not happen");
  ...
  */
  ...
  if (proc_rc != NCSCC_RC_SUCCESS) {
    ...
    imma_ccb_node_delete(cb,
                         ccb_node); /* Removes node from tree and frees it. */
    ccb_node = NULL;
    goto done;
  }
...
re_used:
  ...
  *ccbHandle = ccb_node->ccb_hdl;
  ...
  TRACE(
      "Augmented CCB CcbId:%u admin ownerId:%u ccb-handle:%llx admo-handle:%llx\n",
      ccb_node->mCcbId, adminOwnerId, *ccbHandle,
...
*/
SaAisErrorT immsv_om_augment_ccb_get_result(SaImmOiHandleT privateOmHandle,
                                            SaUint32T ccbId)
...
SaAisErrorT immsv_om_augment_ccb_get_admo_name(SaImmHandleT privateOmHandle,
                                               SaStringT object,
...

{
  ...
  rc = saImmOmAccessorGet_o3(acHdl, (SaConstStringT)object, attributeNames,
                             &attributes);
  if (rc != SA_AIS_OK) {
    goto finalize;
  }
  ...
  *admoNameOut =
      (SaStringT)malloc(strlen(*(SaStringT *)attrVal->attrValues[0]) + 1);
...
SaAisErrorT immsv_om_handle_initialize(SaImmHandleT *privateOmHandle,
                                       SaVersionT *version) {
  TRACE_ENTER();
...
SaAisErrorT immsv_om_admo_handle_initialize(
    ...
    SaImmAdminOwnerHandleT *adminOwnerHandle) {
  TRACE_ENTER();
  return saImmOmAdminOwnerInitialize(immHandle, adminOwnerName, SA_TRUE,
                                     adminOwnerHandle);
}
