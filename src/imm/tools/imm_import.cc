...
// redefine logtrace macros local for this file (instead of logtrace.h) because
...
static SaImmValueTypeT getClassAttrValueType(ParserState *, const char *,
                                             const char *);
...
typedef struct ParserStateStruct1 {
        ...
        std::list<SaImmAttrDefinitionT_2> attrDefinitions;
        std::map<std::string, std::map<std::string, SaImmValueTypeT> >
classAttrTypeMap; std::set<std::string> adminOwnerSetSet;
...
static void free_parserState(ParserState *state) {
...

  {
    ...
    for (it = state->attrValueBuffers.begin();
         it != state->attrValueBuffers.end(); ++it)
      free(*it);
...
void setAdminOwnerHelper(ParserState *state, SaNameT *parentOfObject) {
  ...
  if (set_it != state->adminOwnerSetSet.end()) {
    LOG_IN(
        "  Already called saImmOmAdminOwnerSet on parent (or parent of parent) '%s'",
        tmpStr);
    return;
  }
  ...
  if (!state->validation) {
    ...
    SaAisErrorT errorCode = immutil_saImmOmAdminOwnerSet(
        state->ownerHandle, (const SaNameT **)objectNames, SA_IMM_ONE);

    if (SA_AIS_OK != errorCode) {
      LOG_ER("Failed to call saImmOmAdminOwnerSet on parent: '%s' , rc =  %s",
             tmpStr, saf_error(errorCode));
      stopParser(state);
...
/*
 * This function is inherited from an existing utility to import additional
 * classes/objects If the class definition has not been read from the input we
 * try to read the class and it's RDN from the IMM.
 */
static void getClassFromImm(ParserState *state,
  ...
  if (err != SA_AIS_OK) {
    LOG_ER(
        "Failed to fetch class description for %s loaded in IMM, "
        "error code:%u",
        className, err);
    stopParser(state);
    state->parsingStatus = 1;
  ...
  LOG_IN("Read class description from IMM: %s", className);
...
static SaImmAttrValuesT_2 *attrvalue_get(SaImmAttrValuesT_2 **attrValues,
  ...
  for (i = 0; attrValues[i] != NULL; i++)
    if (strcmp(attrValues[i]->attrName, attrName) == 0) return attrValues[i];
...
static void createImmObject(ParserState *state) {
  ...
  LOG_IN("CREATE IMM OBJECT %s, %s", state->objectClass, state->objectName);
 ...
 * @return unsigned int
 */
static unsigned int attrdef_array_size(
    SaImmAttrDefinitionT_2 **attrDefinitions) {
  int i, imm_added = 0;

  for (i = 0; attrDefinitions[i] != NULL; i++) {
    if (strncmp(attrDefinitions[i]->attrName, "SaImmAttr", 9) == 0) imm_added++;
  }
 ...
 */
static SaImmAttrDefinitionT_2 *attrdef_get(
    SaImmAttrDefinitionT_2 **attrDefinitions, SaImmAttrNameT attrName) {
  int i;
...
static bool attrvalue_is_equal(SaImmValueTypeT valueType, SaImmAttrValueT val1,
                               SaImmAttrValueT val2) {
  switch (valueType) {
    ...
    case SA_IMM_ATTR_SAANYT: {
      ...
      if (at1->bufferSize != at2->bufferSize) return false;
...
static bool attrdefs_are_equal(SaImmAttrDefinitionT_2 *attrDef1,
  ...
  if (((attrDef1->attrDefaultValue == NULL) &&
       (attrDef2->attrDefaultValue != NULL)) ||
      ((attrDef1->attrDefaultValue != NULL) &&
       (attrDef2->attrDefaultValue == NULL))) {
  ...
  if ((attrDef1->attrDefaultValue != NULL) &&
      !attrvalue_is_equal(attrDef1->attrValueType, attrDef1->attrDefaultValue,
                          attrDef2->attrDefaultValue)) {
    fprintf(stderr, "FAILED: attrDefaultValue mismatch for attribute '%s'\n",
            attrDef1->attrName);
    return false;
  }
...
static void createImmClass(ParserState *state) {
  ...
  LOG_IN("CREATING IMM CLASS %s", state->className);
  ...
  if (!state->validation) {
    ...
    new_attrDefinitions = (SaImmAttrDefinitionT_2 **)calloc(
        (state->attrDefinitions.size() + 1), sizeof(SaImmAttrDefinitionT_2 *));
    ...
    errorCode = immutil_saImmOmClassCreate_2(
        state->immHandle, className, classCategory,
        (const SaImmAttrDefinitionT_2 **)new_attrDefinitions);
    ...
    if (SA_AIS_OK != errorCode) {
      if (SA_AIS_ERR_EXIST == errorCode) {
        ...
        errorCode = immutil_saImmOmClassDescriptionGet_2(
            state->immHandle, className, &existing_classCategory,
            &existing_attrDefinitions);
        if (SA_AIS_OK != errorCode) {
          fprintf(stderr,
                  "FAILED to get IMM class description for '%s', rc = %s\n",
                  className, saf_error(errorCode));
          stopParser(state);
          state->parsingStatus = 1;
        } else {
          if (existing_classCategory != classCategory) {
            ...
            // If there are more (or less) attributes in the new class
            // description than in the existing one, this check will catch it.
            fprintf(stderr,
                    "FAILED: Number of attribute definitions differ for '%s'\n",
                    className);
            stopParser(state);
            state->parsingStatus = 1;
          } else {
            for (i = 0; existing_attrDefinitions[i] != NULL; i++) {
              ...
              new_attrDef =
                  attrdef_get(new_attrDefinitions, existing_attrDef->attrName);
              if (new_attrDef == NULL) {
                fprintf(stderr,
                        "FAILED: Attribute '%s' missing in loaded class '%s'\n",
...
static void getDNForClass(ParserState *state, const SaImmClassNameT className,
  ...
  if (state->classRDNMap.find(classNameString) == state->classRDNMap.end()) {
...
static void endElementHandler(void *userData, const xmlChar *name) {
  ...
  if (strcmp((const char *)name, "value") == 0) {
    if (state->attrValueBuffers.empty()) {
      ...
      if (newlen == -1) {
        free(newvalue);
        LOG_ER("Failed to decode base64 value in attribute %s (value)",
               state->attrName);
        stopParser(state);
        state->parsingStatus = 1;
        return;
    ...
    if (state->attrDefaultValueBuffer == NULL || !state->attrDefaultValueSet) {
      ...
      if (newlen == -1) {
        free(newvalue);
        LOG_ER(
            "Failed to decode base64 default value in attribute %s (default-value)",
            state->attrName);
        stopParser(state);
        state->parsingStatus = 1;
        return;
    ...
    if (!state->doneParsingClasses && !state->validation) {
      ...
      errorCode = immutil_saImmOmAdminOwnerInitialize(
          state->immHandle, imm_import_adminOwnerName, SA_TRUE,
          &state->ownerHandle);
      ...
      errorCode = immutil_saImmOmCcbInitialize(
          state->ownerHandle,
          imm_import_ccb_safe
...
static void addObjectAttributeDefinition(ParserState *state) {
  ...
  for (iter = state->attrValues.begin(); iter != state->attrValues.end();
    ...
    if (!strcmp(state->attrName, (*iter).attrName)) {
      LOG_ER("Attribute '%s' is defined more than once in object '%s'",
             state->attrName, state->objectName);
      stopParser(state);
      state->parsingStatus = 1;
      return;
    }
  ...
  if (state->validation) {
    ...
    attrValues.attrValueType =
        getClassAttrValueType(state, state->objectClass, state->attrName);
    if (state->ctxt->instate == XML_PARSER_EOF) return;
  }
...
static SaImmValueTypeT getClassAttrValueType(ParserState *state,
  ...
  if (state->classAttrTypeMap.find(classNameString) ==
    ...
    if (state->ctxt->instate == XML_PARSER_EOF) return (SaImmValueTypeT)-1;
    if (state->classAttrTypeMap.find(classNameString) ==
        state->classAttrTypeMap.end()) {
      stopParser(state);
      state->parsingStatus = 1;
...
static void addClassAttributeDefinition(ParserState *state) {
  ...
  if (state->attrName != NULL) {
    std::list<SaImmAttrDefinitionT_2>::iterator it;
    for (it = state->attrDefinitions.begin();
         it != state->attrDefinitions.end(); ++it) {
      if (!strcmp(state->attrName, (*it).attrName)) {
        LOG_ER("Attribute '%s' is defined more than once in class '%s'",
               state->attrName, state->className);
        stopParser(state);
        state->parsingStatus = 1;
        return;
      }
  ...
  if (state->state[state->depth] == RDN) {
    ...
    */
    ...
    attrDefinition.attrValueType = state->attrValueType;
    TRACE_8("ATTR %s, %d", attrDefinition.attrName,
 ...
 */
static int charsToValueHelper(SaImmAttrValueT *value, SaImmValueTypeT type,
                              const char *str, bool strictParse) {
  size_t len;
  unsigned int i;
  char byte[5];
  char *endMark;
...
int loadImmXML(const char *xmlfile, int strictParse) {
  ...
  if (file) {
    ...
    if (rd > 0) {
      state.ctxt =
          xmlCreatePushParserCtxt(&my_handler, &state, buf, rd, xmlfile);
      while (state.ctxt->instate != XML_PARSER_EOF &&
             (rd = fread(buf, 1, 1024, file)) > 0) {
        if (xmlParseChunk(state.ctxt, buf, rd, 0)) break;
      }
  ...
  if (!transaction_mode) {
    ...
    if (state.adminInit) {
      ...
      if (SA_AIS_OK != errorCode) {
        LOG_WA("Failed on saImmOmAdminOwnerFinalize (%s)",
               saf_error(errorCode));
      }
    }
    ...
    /* Save handles */
    if (state.immInit) *imm_import_immHandle = state.immHandle;
    if (state.adminInit) *imm_import_ownerHandle = state.ownerHandle;
    if (state.ccbInit) *imm_import_ccbHandle = state.ccbHandle;
  }

  if (state.attrName) free(state.attrName);
  if (state.className) free(state.className);
...
//  original as possible to ease a future refactoring towards common codebase
int importImmXML(char *xmlfileC, char *adminOwnerName, int verbose,
                 int ccb_safe, SaImmHandleT *immHandle,
                 SaImmAdminOwnerHandleT *ownerHandle,
                 SaImmCcbHandleT *ccbHandle, int mode, const char *xsdPath,
...
int validateImmXML(const char *xmlfile, int verbose, int mode,
  ...
  if (file) {
    ...
    LOG_ER("Cannot open '%s'", xmlfile);
    if (!transaction_mode) exit(EXIT_FAILURE);
  }
