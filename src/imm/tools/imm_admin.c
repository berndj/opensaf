 ...
 * Example: immadm -o 3 -p saAmfNodeSuFailoverMax:SA_INT32_T:7
...
static void usage(const char *progname)
{
	...
	printf("\t%s -o 2 safAmfNode=SC-2,safAmfCluster=myAmfCluster\n",
	       progname);
	printf(
	    "\t%s -o 1 -p lockOption:SA_STRING_T:trylock  safEE=SC-1,safDomain=domain_1\n",
	    progname);
}
...
static int init_param(SaImmAdminOperationParamsT_2 *param, char *arg)
{
	...
	/* get the attrValue. Also account for the 2 colons used to separate the
	 * parameters */
...
void print_param(SaImmAdminOperationParamsT_2 *param)
{
	...
	case SA_IMM_ATTR_SAINT32T:
		printf("%-50s %-12s %d (0x%x)\n", param->paramName,
		       "SA_INT32_T", (*((SaInt32T *)param->paramBuffer)),
		       (*((SaInt32T *)param->paramBuffer)));
		break;
	case SA_IMM_ATTR_SAUINT32T:
		printf("%-50s %-12s %u (0x%x)\n", param->paramName,
		       "SA_UINT32_T", (*((SaUint32T *)param->paramBuffer)),
		       (*((SaUint32T *)param->paramBuffer)));
		break;
	case SA_IMM_ATTR_SAINT64T:
		printf("%-50s %-12s %lld (0x%llx)\n", param->paramName,
		       "SA_INT64_T", (*((SaInt64T *)param->paramBuffer)),
		       (*((SaInt64T *)param->paramBuffer)));
		break;
	case SA_IMM_ATTR_SAUINT64T:
		printf("%-50s %-12s %llu (0x%llx)\n", param->paramName,
		       "SA_UINT64_T", (*((SaUint64T *)param->paramBuffer)),
		       (*((SaUint64T *)param->paramBuffer)));
		break;
	case SA_IMM_ATTR_SATIMET: {
		char buf[32];
		const time_t time =
		    *((SaTimeT *)param->paramBuffer) / SA_TIME_ONE_SECOND;
	...
	case SA_IMM_ATTR_SANAMET:
		printf("%-50s %-12s %s\n", param->paramName, "SA_NAME_T",
		       saAisNameBorrow((SaNameT *)param->paramBuffer));
		break;
	case SA_IMM_ATTR_SAFLOATT:
		printf("%-50s %-12s %f\n", param->paramName, "SA_FLOAT_T",
		       (*((SaFloatT *)param->paramBuffer)));
		break;
	case SA_IMM_ATTR_SADOUBLET:
		printf("%-50s %-12s %lf\n", param->paramName, "SA_DOUBLE_T",
		       (*((SaDoubleT *)param->paramBuffer)));
		break;
	case SA_IMM_ATTR_SASTRINGT:
		printf("%-50s %-12s %s\n", param->paramName, "SA_STRING_T",
		       (*((SaStringT *)param->paramBuffer)));
		break;
	case SA_IMM_ATTR_SAANYT:
		printf("%-50s %-12s %s\n", param->paramName, "SA_ANY_T",
		       "<Not shown>");
		break;
	default:
		printf("%-50s <%-12s (%d)>\n", param->paramName,
		       "Unknown value type", (int)param->paramType);
		exit(EXIT_FAILURE);
	}
}
...
int main(int argc, char *argv[])
{
	...
	if (operationId == -1) {
		fprintf(stderr,
			"error - must specify admin operation ID %llx\n",
			operationId);
		exit(EXIT_FAILURE);
	}
	...
	if ((optind < argc) && (!explicitAdmo)) {
		...
		if (strcmp(saAisNameBorrow(&objectName),
			   OPENSAF_IMM_OBJECT_DN) == 0) {
			releaseAdmo = false;
			adminOwnerName = (SaImmAdminOwnerNameT)malloc(
			    strlen(OPENSAF_IMM_SERVICE_NAME) + 1);
	...
	error = immutil_saImmOmAdminOwnerInitialize(
	    immHandle, adminOwnerName, releaseAdmo ? SA_TRUE : SA_FALSE,
	    &ownerHandle);
	if (error != SA_AIS_OK) {
		fprintf(stderr,
	...
	/* Remaining arguments should be object names on which the admin op
	 * should be performed. */
	while (optind < argc) {
		saAisNameLend(argv[optind], &objectName);
		...
		if (error != SA_AIS_OK) {
			if (error == SA_AIS_ERR_NOT_EXIST) {
				if (strcmp(adminOwnerName,
					   saAisNameBorrow(&objectName)) == 0) {
					/* AdminOwnerName == ImplementerName -
					 * Could be direct admin-op on OI */
					goto retry;
				}
				fprintf(
				    stderr,
				    "error - saImmOmAdminOwnerSet - object '%s' does not exist\n",
	...
	retry:
		error = immutil_saImmOmAdminOperationInvoke_o2(
		    ownerHandle, &objectName, 0, operationId, params,
		    &operationReturnValue, SA_TIME_ONE_SECOND * timeoutVal,
		    &out_params);

		if (error != SA_AIS_OK) {
			fprintf(
			    stderr,
			    "error - saImmOmAdminOperationInvoke_2 FAILED: %s\n",
			    saf_error(error));
			exit(EXIT_FAILURE);
		}
		...
		if (operationReturnValue != SA_AIS_OK) {
			...
			fprintf(
			    stderr,
			    "error - saImmOmAdminOperationInvoke_2 admin-op RETURNED: %s\n",
			    saf_error(operationReturnValue));

			while (out_params && out_params[ix]) {
				if (strcmp(out_params[ix]->paramName,
					   SA_IMM_PARAM_ADMOP_ERROR) == 0) {
					assert(out_params[ix]->paramType ==
					       SA_IMM_ATTR_SASTRINGT);
					SaStringT errStr =
					    (*((SaStringT *)out_params[ix]
						   ->paramBuffer));
			...
			/* After printing error string, print all returned
			 * parameters */
		...
		if (((opName && (strncmp(opName, "display", 7) == 0)) ||
		     verbose) &&
		...
		error =
		    saImmOmAdminOperationMemoryFree(ownerHandle, out_params);
		if (error != SA_AIS_OK) {
			fprintf(
			    stderr,
			    "error - saImmOmAdminOperationMemoryFree FAILED: %s\n",
			    saf_error(error));
			exit(EXIT_FAILURE);
		}
