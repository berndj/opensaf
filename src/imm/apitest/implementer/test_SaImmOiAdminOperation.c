...
static SaNameT rdn = {sizeof("Obj3"), "Obj3"};
...
static SaImmAdminOperationParamsT_2 adminOperationNameParam = {
    SA_IMM_PARAM_ADMOP_NAME, SA_IMM_ATTR_SASTRINGT, &adminOperationName};
...
/*
 * If opId is SA_IMM_PARAM_ADMOP_ID_ESC, then the callback returns
 * SA_AIS_ERR_BAD_OPERATION and set the admin-operation name to return parameter
 * SA_IMM_PARAM_ADMOP_ERROR. If adminOperationErrorString is not NULL, then the
 * return parameter is adminOperationErrorParams. Otherwise, the return
 * parameter is params.
 */
static void saImmOiAdminOperationCallback_o2(
    SaImmOiHandleT immOiHandle, SaInvocationT invocation,
    const SaNameT *objectName, SaImmAdminOperationIdT opId,
    const SaImmAdminOperationParamsT_2 **params)
{
	...
	if (opId == SA_IMM_PARAM_ADMOP_ID_ESC) {
		...
		while (params[ix]) {
			if (!strcmp(params[ix]->paramName,
				    SA_IMM_PARAM_ADMOP_NAME)) {
				TRACE("Admin operation name: %s",
				      params[ix]->paramName);
			} else if (!param) {
				param =
				    (SaImmAdminOperationParamsT_2 *)params[ix];
			}
			ix++;
		}
	...
	} else if (saImmOiAdminOperationCallback_response !=
		   SA_AIS_ERR_TIMEOUT) {
...
static void saImmOiAdminOperationCallback_o2_copyParams(
    SaImmOiHandleT immOiHandle, SaInvocationT invocation,
    const SaNameT *objectName, SaImmAdminOperationIdT opId,
    const SaImmAdminOperationParamsT_2 **params)
{
...
static const SaImmOiCallbacksT_2 oiCallbacks_o2_copyParams = {
	...
	saImmOiAdminOperationCallback_o2_copyParams};
...
static const SaImmCallbacksT_o2 omCallbacks = {
    saImmOmAdminOperationInvokeCallback};

static const SaImmAdminOwnerNameT adminOwnerName =
...
static SaAisErrorT om_admin_exec(SaAisErrorT *imm_rc, const SaNameT *objectName,
				 SaAisErrorT in_rc)
{
	SaAisErrorT rc;
	SaImmHandleT handle;
	...
	SaImmAdminOperationParamsT_2 param = {"TEST", SA_IMM_ATTR_SAUINT64T,
					      &value};
	...
	*imm_rc = immutil_saImmOmAdminOperationInvoke_2(ownerHandle, &localRdn, 0,
	...
	saImmOmAdminOwnerFinalize(
	    ownerHandle); /* Tend to get timeout here with PBE */
...
void SaImmOiAdminOperation_09(void)
{
	...
	while (returnParams[ix]) {
		if (!strcmp(returnParams[ix]->paramName,
	...
	/* Not exactly nice validation because test_validate validates
	   SaAisErrorT errors. Here test_validate is used to track test case
	   counters, and it's possible to use it in the way below */
	test_validate(strcmp(*(SaStringT *)(returnParams[ix]->paramBuffer),
			     adminOperationErrorString),
		      0);
	...
	safassert(saImmOmAdminOperationMemoryFree(ownerHandle, returnParams),
		  SA_AIS_OK);
...
void SaImmOiAdminOperation_10(void)
{
	...
	const SaImmAdminOperationParamsT_2 *params[] = {
	    &param, &adminOperationNameParam, NULL};
	SaImmAdminOperationParamsT_2 **returnParams = NULL;
	SaAisErrorT operationReturnValue;
	...
	/* Not exactly nice validation because test_validate validates
	   SaAisErrorT errors. Here test_validate is used to track test case
	   counters, and is possible to use it in the way below */
	test_validate(strcmp(*(SaStringT *)(returnParams[ix]->paramBuffer),
			     (char *)&value),
		      0);
...
void SaImmOiAdminOperation_11(void)
{
	...
	SaImmAdminOperationParamsT_2 param = {"TEST", SA_IMM_ATTR_SAUINT64T,
					      &value};
	const SaImmAdminOperationParamsT_2 *params[] = {
	    &adminOperationNameParam, &param, NULL};
...
__attribute__((constructor)) static void saImmOiInitialize_2_constructor(void)
{
	...
	test_case_add(5, SaImmOiAdminOperation_01,
		      "SaImmOiAdminOperation - SA_AIS_OK");
	test_case_add(5, SaImmOiAdminOperation_02,
		      "SaImmOiAdminOperation - SA_AIS_ERR_BAD_HANDLE");
	test_case_add(5, SaImmOiAdminOperation_03,
		      "SaImmOiAdminOperation - SA_AIS_ERR_INVALID_PARAM");
	test_case_add(5, SaImmOiAdminOperation_04,
		      "SaImmOiAdminOperation - SA_AIS_ERR_BAD_OPERATION");
	test_case_add(5, SaImmOiAdminOperation_05,
		      "SaImmOiAdminOperation - SA_AIS_ERR_TIMEOUT");
	test_case_add(5, SaImmOiAdminOperation_06,
		      "SaImmOiAdminOperation - SA_AIS_ERR_NOT_EXIST");
	test_case_add(5, SaImmOiAdminOperation_07,
		      "SaImmOiAdminOperation - SA_AIS_OK, OM async");
	test_case_add(
	    5, SaImmOiAdminOperation_08,
	    "SaImmOiAdminOperation - SA_AIS_OK, OM sync with saImmOmAdminOperationMemoryFree");
	test_case_add(
	    5, SaImmOiAdminOperation_09,
	    "SaImmOiAdminOperation - SA_AIS_ERR_BAD_OPERATION, SaImmAdminOperationError");
	test_case_add(
	    5, SaImmOiAdminOperation_10,
	    "SaImmOiAdminOperation - SA_AIS_ERR_BAD_OPERATION, SaImmAdminOperationName");
	test_case_add(
	    5, SaImmOiAdminOperation_11,
	    "SaImmOiAdminOperation - SA_AIS_OK, SaImmAdminOperationName (first param) - async");
	test_case_add(
	    5, SaImmOiAdminOperation_12,
	    "SaImmOiAdminOperation - SA_AIS_OK, SaImmAdminOperationName (first param) - sync");
	test_case_add(
	    5, SaImmOiAdminOperation_13,
	    "SaImmOiAdminOperation - SA_AIS_OK, Same as 5 1 but with implementername as target");
}
