...
static char *classes[] = {"TestClassConfig", "TestClassRuntime",
			  "SaAmfComp_TEST", NULL};
...
static void cleanup()
{
	...
	rc = immutil_saImmOmAdminOwnerInitialize(immHandle, "immoitest",
						 SA_TRUE, &ownerHandle);
	assert(rc == SA_AIS_OK);
	...
	while (*obj) {
		if (osaf_is_extended_names_enabled() ||
			...
			rc = immutil_saImmOmCcbObjectDelete(ccbHandle,
							    &objectName);
			if (rc != SA_AIS_OK && rc != SA_AIS_ERR_NOT_EXIST)
				fprintf(
				    stderr,
				    "Failed to delete object '%s' with error code: %d\n",
				    *obj, rc);
	...
	while (*cls) {
		...
		if (rc == SA_AIS_ERR_BUSY)
			fprintf(stderr,
				"Class '%s' contains object instances\n", *cls);
		assert(rc == SA_AIS_OK || rc == SA_AIS_ERR_NOT_EXIST);
