...
/*
 * This file contains a command line utility to populate IMM with configuration
objects. * A sample class to load using immcfg -f could be:
 *

<?xml version="1.0" encoding="utf-8"?>
<imm:IMM-contents xmlns:imm="http://www.saforum.org/IMMSchema"
xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
xsi:noNamespaceSchemaLocation="SAI-AIS- IMM-XSD-A.01.01.xsd"> <class
name="TestClass"> <category>SA_CONFIG</category> <rdn> <name>rdn</name>
...
static void usage(const char *progname)
{
	...
	printf("\nDESCRIPTION\n");
	printf(
	    "\t%s is an IMM OM test client used to populate IMM with a specified number of object(s)\n",
	    progname);
	printf(
	    "\tThe objects created are all of the same class and their DNs are automatically created.");

	printf("\nOPTIONS\n");
	printf(
	    "\t-c, --class                   output a suitable class definition to stdout\n");
	printf("\t-h, --help                    this help\n");
	printf("\t-v, --verbose                 enable debug printouts\n");
	printf(
	    "\t-p, --populate <cardinality>  create <cardinality> number of objects of class <class name>\n");
...
static range_obj_t *gen_pop_tree(unsigned int level, range_obj_t *rootObj,
				 unsigned int rdn, unsigned int base,
				 unsigned int reminder,
				 SaImmAttrNameT rdnAttName)
{
	int ix;
	...
	if (level == 0) {
		...
		strncat((char *)rangeObj->parentDn.value, "0",
			SA_MAX_NAME_LENGTH);
		rangeObj->parentDn.length =
		    strlen((char *)rangeObj->parentDn.value);
	} else {
		rangeObj->parentDn.length =
		    snprintf((char *)rangeObj->parentDn.value,
	...
	VERBOSE_INFO("Generated level %u rangeObj(%u) %u<-->%u %s\n", level,
		     rdn, rangeObj->botRdn, rangeObj->topRdn,
	...
	while (reminder && (ix < 100)) {
		...
		unsigned int subreminder =
		    (reminder < partition) ? reminder : partition;
		rangeObj->sublevel[ix] =
		    gen_pop_tree(level + 1, rangeObj, /* rdn: */ base + ix,
				 /*new base:*/ base + 100 + (ix)*partition,
				 subreminder, rdnAttName);
...
static void ccb_create_obj(SaNameT *parentDn, SaNameT *rdnVal,
			   SaImmCcbHandleT ccbHandle,
			   const SaImmClassNameT className,
			   SaImmAttrNameT rdnAttName,
			   SaImmValueTypeT rdnAttType)
{
	...
	if (err != SA_AIS_OK) {
		...
		fprintf(
		    stderr,
		    "error - Failed to create object parent%s rdn:%s error:%u\n",
		    (parentDn && parentDn->length) ? (char *)parentDn->value
						   : NULL,
...
static void generate_pop(range_obj_t *rootObj, SaImmCcbHandleT ccbHandle,
			 const SaImmClassNameT className,
			 SaImmAttrNameT rdnAttName, SaImmValueTypeT rdnAttType,
			 SaImmAdminOwnerHandleT ownerHandle)
{
	...
	for (ix = 0; ix < 100 && rdn <= rootObj->topRdn; ++ix, ++rdn) {
		rdnAttrVal.length =
		    snprintf((char *)rdnAttrVal.value, sizeof(rdnAttrVal.value),
			     "%s=%u", rdnAttName, rdn);

		ccb_create_obj(&rootObj->parentDn, &rdnAttrVal, ccbHandle,
			       className, rdnAttName, rdnAttType);
	}
	...
	if (err != SA_AIS_OK) {
		fprintf(
		    stderr,
		    "error - Failed to apply ccb for parent:%s range %u<->%u, error:%s\n",
		    (char *)rootObj->parentDn.value, rootObj->botRdn,
		    rootObj->topRdn, saf_error(err));
		exit(1);
	}
	...
	for (ix = 0; ix < 100 && rootObj->sublevel[ix]; ++ix) {
		generate_pop(rootObj->sublevel[ix], ccbHandle, className,
...
static int populate_imm(const SaImmClassNameT className, unsigned int pop,
			SaImmAdminOwnerHandleT ownerHandle,
			SaImmHandleT immHandle)
{
	...
	for (i = 0, att = attrDefinitions[i]; att != NULL;
	     att = attrDefinitions[++i]) {
		if (att->attrFlags & SA_IMM_ATTR_RDN) {
			...
			fprintf(
			    stderr,
			    "error - Attribute %s has INITIALIZED flag, cant handle\n",
			    att->attrName);
			goto done;
		}
	...
	VERBOSE_INFO("Rdn attrName:%s type:%s\n", rdnAttName,
		     (rdnAttType == SA_IMM_ATTR_SASTRINGT)
			 ? "SA_STRINGT"
	...
	ccb_create_obj(NULL, &rootObj->parentDn, ccbHandle, className,
		       rdnAttName, rdnAttType);
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		int option_index = 0;
		c = getopt_long(argc, argv, "chp:v", long_options,
				&option_index);
