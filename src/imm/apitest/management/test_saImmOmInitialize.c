...
void saImmOmInitialize_04(void)
{
	...
	if (version.releaseCode != immVersion.releaseCode) {
		printf(
		    "\tRelease code not set correctly as out param expected %c got %c\n",
		    immVersion.releaseCode, version.releaseCode);
		rc = SA_AIS_ERR_FAILED_OPERATION;
	}
	if (version.majorVersion != immVersion.majorVersion) {
		printf(
		    "\tMajor version not set correctly as out param expected %u got %u\n",
	...
	if (version.minorVersion != immVersion.minorVersion) {
		printf(
		    "\tMinor version not set correctly as out param expected %u got %u\n",
...
void saImmOmInitialize_09(void)
{
	...
	test_validate(rc, SA_AIS_ERR_VERSION); /* Version A.1.1 is explicitly
						  NOT supported. */
}
...
__attribute__((constructor)) static void saImmOmInitialize_constructor(void)
{
	...
	test_case_add(
	    1, saImmOmInitialize_02,
	    "saImmOmInitialize - SA_AIS_OK - NULL pointer to callbacks");
	test_case_add(
	    1, saImmOmInitialize_03,
	    "saImmOmInitialize - SA_AIS_ERR_INVALID_PARAM - uninitialized handle");
	test_case_add(
	    1, saImmOmInitialize_04,
	    "saImmOmInitialize - SA_AIS_ERR_VERSION - uninitialized version");
	test_case_add(
	    1, saImmOmInitialize_05,
	    "saImmOmInitialize - SA_AIS_ERR_VERSION - too high release level");
	test_case_add(1, saImmOmInitialize_06,
		      "saImmOmInitialize - SA_AIS_OK - minor version set to 2");
	test_case_add(
	    1, saImmOmInitialize_07,
	    "saImmOmInitialize - SA_AIS_ERR_VERSION - major version set to 3");
	test_case_add(
	    1, saImmOmInitialize_08,
	    "saImmOmInitialize - SA_AIS_ERR_VERSION - B.3.99 requested");
	test_case_add(
	    1, saImmOmInitialize_09,
	    "saImmOmInitialize - SA_AIS_ERR_VERSION - A.1.1 requested");
	test_case_add(1, saImmOmInitialize_10,
		      "saImmOmInitialize - SA_AIS_OK - A.2.99 requested");
	test_case_add(
	    1, saImmOmInitialize_11,
	    "saImmOmInitialize - SA_AIS_ERR_VERSION - A.2.1 requested on _o2");

	test_case_add(
	    1, saImmOmSelectionObjectGet_01,
	    "saImmOmSelectionObjectGet - SA_AIS_ERR_INVALID_PARAM - no callback defined");
	test_case_add(
	...
	test_case_add(1, saImmOmDispatch_02,
		      "saImmOmDispatch - SA_AIS_OK SA_DISPATCH_ONE");
	test_case_add(
	    1, saImmOmDispatch_03,
	...
	test_case_add(
	    ...
	    "IMM OM Thread Interference - SA_AIS_ERR_LIBRARY - error library");
}
