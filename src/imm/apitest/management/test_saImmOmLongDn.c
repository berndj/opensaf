	...
	if (error == SA_AIS_OK) {
		...
		while (attrDef[i]) {
			if (!strcmp("attr", attrDef[i]->attrName)) {
				SaNameT *testDn =
				    (SaNameT *)attrDef[i]->attrDefaultValue;
				assert(testDn != NULL);
				assert(strcmp(longDn, osaf_extended_name_borrow(
							  testDn)) == 0);
				break;
			}
			i++;
	...
	if (rc == SA_AIS_OK) {
		...
		for (i = 0; attributes[i]; i++) {
			if (!strcmp(attributes[i]->attrName, "attr8")) {
				...
				assert(
				    strcmp(osaf_extended_name_borrow((
					       SaNameT *)(attributes[i]
							      ->attrValues[0])),
					   longDn) == 0);
				break;
			}
...
void addOmLongDnTestCases()
{
	test_case_add(
	    8, saImmOmLongDn_01,
	    "SA_AIS_OK - saImmOmClassCreate_2 with long DN as default value");
	test_case_add(
	    8, saImmOmLongDn_02,
	    "SA_AIS_OK - saImmOmClassDescriptionGet_2 with long DN as default value");
	test_case_add(8, saImmOmLongDn_03,
		      "SA_AIS_OK - saImmOmCcbObjectCreate_2 - long RDN");
	test_case_add(8, saImmOmLongDn_04,
		      "SA_AIS_OK - saImmOmCcbObjectModify_2 - long DN");
	test_case_add(8, saImmOmLongDn_05,
		      "SA_AIS_OK - saImmOmCcbObjectDelete - long DN");
	test_case_add(
	    8, saImmOmLongDn_06,
	    "SA_AIS_OK - saImmOmSearchInitialize_2 and saImmOmSearchNext_2");
