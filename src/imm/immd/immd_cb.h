...
#define IMMD_WRT_IMMND_SUBPART_VER_RANGE \
  (IMMD_WRT_IMMND_SUBPART_VER_MAX - IMMD_WRT_IMMND_SUBPART_VER_MIN + 1)
...
typedef struct immd_cb_tag {
  ...
  IMMD_IMMND_DETACHED_NODE
      *detached_nodes; /* IMMD SBY list of recently departed payloads */
  const char *mDir;    /* The directory where imm.xml & pbe files reside */
  const char *mFile;   /* The imm.xml file to start from */
  const char
      *mPbeFile;     /* If the pbe feature is configured, without 2PBE suffix */
  ...
  uint32_t
      mScAbsenceVeteranMaxWait; /* Amount of seconds IMMD waits for veteran
                                   IMMDs after sc absence */
...
/*uint32_t immd_immnd_info_node_add(NCS_PATRICIA_TREE *immnd_tree,
 * IMMD_IMMND_INFO_NODE *immnd_info_node);*/
...
void immd_clm_cluster_track_cb(
