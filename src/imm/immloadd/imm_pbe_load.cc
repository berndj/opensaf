...
void *checkPbeRepositoryInit(std::string dir, std::string file) {
  ...
  */
  ...
  if (rc) {
    LOG_ER("Can't open sqlite pbe file '%s', cause:%s", filename.c_str(),
           sqlite3_errmsg((sqlite3 *)dbHandle));
    goto load_from_xml_file;
  }
  ...
  if (rpi == SA_IMM_INIT_FROM_FILE) {
    LOG_NO(
        "saImmRepositoryInit: SA_IMM_INIT_FROM_FILE - falling back to loading from imm.xml");
    goto load_from_xml_file;
  } else if (rpi == SA_IMM_KEEP_REPOSITORY) {
    LOG_IN(
        "saImmRepositoryInit: SA_IMM_KEEP_REPOSITORY - loading from repository");
  } else {
    LOG_ER(
        "saImmRepositoryInit: Not a valid value (%u) - falling back to loading from imm.xml",
        rpi);
    goto load_from_xml_file;
  }
  ...
  rc = sqlite3_get_table((sqlite3 *)dbHandle, sql2, &result, &nrows, &ncols,
                         &zErr);
  if (rc) {
    LOG_IN(
        "Could not access table 'pbe_rep_version', error:%s. Assuming version <0,0>",
        zErr);
    sqlite3_free(zErr);
    ...
    if (nrows == 0) {
      ...
      if (nrows > 1) {
        LOG_WA(
            "pbe_rep_version has %u tuples, should "
  ...
  LOG_IN("PBE repository of rep-version <%u, %u>", pbe_ver_major,
         pbe_ver_minor);

  if (pbe_ver_major > 1) {
    LOG_ER("Can not handle rep-version with major version %u greater than 1",
           pbe_ver_major);
    goto load_from_xml_file;
  }
  ...
  for (int i = 0; i < SQL_STMT_SIZE; i++) {
    rc = sqlite3_prepare_v2((sqlite3 *)dbHandle, preparedSql[i], -1,
                            &(preparedStmt[i]), NULL);
    if (rc != SQLITE_OK) {
...
bool loadClassFromPbe(void *pbeHandle, SaImmHandleT immHandle,
                      const char *className, const char *class_id,
                      SaImmClassCategoryT classCategory, ClassInfo *class_info,
                      bool *pbeCorrupted) {
  ...
  *pbeCorrupted = true;
  ...
  if (rc != SQLITE_ROW && rc != SQLITE_DONE) {
    LOG_IN("Could not access table 'attr_def', error:%s",
  ...
  while (rc == SQLITE_ROW) {
    ...
    if (!(attrFlags & SA_IMM_ATTR_INITIALIZED)) {
      ...
      if (sqlite3_bind_text(stmtDflt, 2, attrName.c_str(), -1, NULL) !=
          SQLITE_OK) {
      ...
      if (rc == SQLITE_DONE) {
        ...
        LOG_IN("Could not access table 'attr_dflt', error:%s",
               sqlite3_errmsg(dbHandle));
        goto bailout;
      } else {
        ...
        if (val) {
          ...
          if (rc == SQLITE_ROW) {
            ...
            TRACE("ABT Default value for %s is %s", attrName.c_str(),
                  (char *)attDflt);
          }
        } else {
          TRACE("ABT No default for %s", attrName.c_str());
    ...
    addClassAttributeDefinition((SaImmAttrNameT)strdup(attrName.c_str()),
                                attrValueType, attrFlags, attDflt, &attrDefs);
...
bailout:
  if (stmtDflt) sqlite3_reset(stmtDflt);
  sqlite3_reset(stmt);
  TRACE_LEAVE();
  return false;
}

bool loadClassesFromPbe(void *pbeHandle, SaImmHandleT immHandle,
                        ClassInfoMap *classInfoMap, bool *pbeCorrupted) {
  sqlite3 *dbHandle = (sqlite3 *)pbeHandle;
  ...
  *pbeCorrupted = true;
  ...
  TRACE_2("Successfully accessed 'classes' table. Rows:%u cols:%u", nrows,
          ncols);
  for (r = 0; r <= nrows; ++r) {
    ...
    if (!loadClassFromPbe(pbeHandle, immHandle, class_name, class_id,
                          class_category, class_info, pbeCorrupted)) {
      sqlite3_free_table(result);
      goto bailout;
    }
...
bool loadObjectFromPbe(void *pbeHandle, SaImmHandleT immHandle,
                       SaImmCcbHandleT ccbHandle, const char *object_id,
                       ClassInfo *class_info, const char *dn,
                       bool *pbeCorrupted) {
  sqlite3 *dbHandle = (sqlite3 *)pbeHandle;
  ...
  *pbeCorrupted = true;
  ...
  while (it != class_info->attrInfoVector.end()) {
    ...
    if (attr_is_multi && !attr_is_pure_rt) {
      ...
      if (sqlite3_bind_text(stmt, 2, (*it)->attrName.c_str(), -1, NULL) !=
          SQLITE_OK) {
      ...
      if (rc != SQLITE_DONE && rc != SQLITE_ROW) {
        LOG_IN("Could not access table multi table, error:%s",
               sqlite3_errmsg(dbHandle));
        goto bailout;
      }
...
bool loadObjectsFromPbe(void *pbeHandle, SaImmHandleT immHandle,
  ...
  *pbeCorrupted = true;
  ...
  TRACE_2("Successfully accessed 'objects' table. Rows:%u cols:%u", nrows,
          ncols);
...
int loadImmFromPbe(void *pbeHandle, bool preload, bool *pbeCorrupted) {
  ...
  *pbeCorrupted = true;
  ...
  if (rc != SQLITE_OK) {
    LOG_ER(
        "Repeated tries to start sqlite loading transaction failed, database locked? rc(%d)",
        rc);
    /* In this case do not escalate the error, because that will assume the
       imm.db is corrupt. The problem here is more likely a locking conflict on
       imm.db, meaning that this loading attempt is being made while another
       user is concurrently claiming exclusive use of the imm.db. This is likely
       to be caused by a poorly coordinated cluster restart. The "other user" is
       likely to be a lingering PBE (on the other SC) or a lingering POSIX file
       lock in an NFS client cache.
     */
    *pbeCorrupted = false;
    goto bailout;
  }
  ...
  if (preload) { /* Break out this into a separate function. */
    ...
    std::string sqlLowCommits(
        "SELECT * FROM ccb_commits WHERE ccb_id < 4294967297");
    std::string sqlCcbId(
        "SELECT MAX(ccb_id) FROM ccb_commits WHERE ccb_id < 4294967297");
    std::string sqlCommitTime(
        "SELECT MAX(commit_time) FROM ccb_commits WHERE ccb_id < 4294967297");
    std::string sqlHighCommits(
        "SELECT * FROM ccb_commits WHERE ccb_id > 4294967297");
    std::string sqlWeakCcbId(
        "SELECT MAX(ccb_id) FROM ccb_commits WHERE ccb_id > 4294967296");
    std::string sqlWeakCommitTime(
        "SELECT MAX(commit_time) FROM ccb_commits WHERE ccb_id > 4294967296");
    ...
    rc = sqlite3_get_table(dbHandle, sqlEpoch.c_str(), &result, &nrows, &ncols,
                           &qErr);
    if (rc) {
    ...
    rc = sqlite3_get_table(dbHandle, sqlLowCommits.c_str(), &result, &nrows,
                           &ncols, &qErr);
    if (rc) {
      LOG_ER("SQL statement ('%s') failed because:\n %s", sqlLowCommits.c_str(),
             qErr);
      sqlite3_free(qErr);
      goto bailout;
    }
    ...
    if (nrows == 0) {
      ...
      rc = sqlite3_get_table(dbHandle, sqlCcbId.c_str(), &result, &nrows,
                             &ncols, &qErr);
      if (rc) {
        LOG_ER("SQL statement ('%s') failed because:\n %s", sqlCcbId.c_str(),
               qErr);
    ...
    rc = sqlite3_get_table(dbHandle, sqlHighCommits.c_str(), &result, &nrows,
                           &ncols, &qErr);
    if (rc) {
      LOG_ER("SQL statement ('%s') failed because:\n %s",
             sqlHighCommits.c_str(), qErr);
      sqlite3_free(qErr);
      goto bailout;
    }
    ...
    if (nrows == 0) {
      ...
      /* Get max weak ccb-id from local imm.db (PRTops, class-create/delete
       * epoch-update) */
      rc = sqlite3_get_table(dbHandle, sqlWeakCcbId.c_str(), &result, &nrows,
                             &ncols, &qErr);
      if (rc) {
        LOG_ER("SQL statement ('%s') failed because:\n %s",
               sqlWeakCcbId.c_str(), qErr);
        sqlite3_free(qErr);
        goto bailout;
      }
      ...
      /* Get max weak commit-time from local imm.db (PRTops, class-create/delete
       * epoch-update) */
      rc = sqlite3_get_table(dbHandle, sqlWeakCommitTime.c_str(), &result,
                             &nrows, &ncols, &qErr);
      if (rc) {
        LOG_ER("SQL statement ('%s') failed because:\n %s",
  ...
  errorCode =
      saImmOmClassDelete(immHandle, (char *)OPENSAF_IMM_PBE_RT_CLASS_NAME);
  ...
  if (preload) {
    exit(0);
  } /* dont delay termination of preloader.*/
...
void escalatePbe(std::string dir, std::string file) {
  ...
  if (rename(filename.c_str(), newFilename.c_str()) != 0) {
    ...
    LOG_NO("Renamed %s to %s to prevent cyclic reload.", filename.c_str(),
           newFilename.c_str());
  }
...
bool isValidationAborted(SaImmCcbHandleT ccbHandle) {
  ...
  errorCode =
      saImmOmCcbGetErrorStrings(ccbHandle, (const SaStringT **)&errorStrings);
  ...
  while (*errorStrings) {
    ...
    if (!strncmp((const char *)*errorStrings, "IMM: Validation abort: ",
                 strlen("IMM: Validation abort: "))) {
      return true;
    }
    errorStrings++;
  }
