...
#define NOTIF_OBJECT_TEST2                                                     \
	"safComp=CompT_SC_FAKE,safSu=SuT_SC_FAKE,safNode=SC_2_2"
#define NOTIF_OBJECT_TEST3                                                     \
	"safComp=CompT_SC_ANOTHER_FAKE,safSu=SuT_SC_FAKE,safNode=SC_2_1"
#define NOTIF_OBJECT_TEST4                                                     \
	"safComp=CompT_SC_FAKE,safSu=SuT_SC_JOKE,safNode=SC_2_1"
#define NOTIF_OBJECT_NO_EXIST                                                  \
	"safComp=CompT_SC_FAKE,safSu=SuT_SC_NOTEXIST,safNode=SC_2_1"
...
static struct not_idsT received_ids = {
    0,
};
static struct not_idsT ok_ids = {
    0,
};
...
void headerTest(void)
{
	...
	safassert(saNtfAlarmNotificationFilterAllocate(
		      ntfHandle, &myAlarmFilter, 1, 1, 1, 1, 0, 0, 0),
		  SA_AIS_OK);
	...
	*myAlarmNotification.trend = SA_NTF_TREND_MORE_SEVERE;
	...
	myAlarmNotification.thresholdInformation->thresholdValue.uint32Val =
	    600;
	myAlarmNotification.thresholdInformation->thresholdHysteresis
	    .uint32Val = 100;
	...
	*(head->eventType) = SA_NTF_ALARM_EQUIPMENT;
	...
	poll_until_received(
	    ntfHandle, *myAlarmNotification.notificationHeader.notificationId);
...
void headerTest2(void)
{
	...
	safassert(saNtfAlarmNotificationFilterAllocate(
		      ntfHandle, &myAlarmFilter, 2, 0, 0, 0, 0, 0, 0),
		  SA_AIS_OK);
	/* only this eventType should be received */
	myAlarmFilter.notificationFilterHeader.eventTypes[0] =
	    SA_NTF_ALARM_EQUIPMENT;
	myAlarmFilter.notificationFilterHeader.eventTypes[1] =
...
void headerTest3(void)
{
	...
	safassert(saNtfAlarmNotificationFilterAllocate(
		      ntfHandle, &myAlarmFilter, 0, 2, 0, 0, 0, 0, 0),
		  SA_AIS_OK);
	/* only these su objects should be received */
	saNameSet(
	    &myAlarmFilter.notificationFilterHeader.notificationObjects[0],
	    NOTIF_OBJECT_TEST_SU);
	saNameSet(
	    &myAlarmFilter.notificationFilterHeader.notificationObjects[1],
	    NOTIF_OBJECT_TEST_SU2);
...
void headerTest4(void)
{
	...
	safassert(saNtfAlarmNotificationFilterAllocate(
		      ntfHandle, &myAlarmFilter, 0, 0, 2, 0, 0, 0, 0),
		  SA_AIS_OK);
	/* only these su objects should be received */
	saNameSet(&myAlarmFilter.notificationFilterHeader.notifyingObjects[0],
		  NOTIF_OBJECT_TEST_SU);
	saNameSet(&myAlarmFilter.notificationFilterHeader.notifyingObjects[1],
		  NOTIF_OBJECT_TEST_SU2);
...
void headerTest5(void)
{
	...
	safassert(saNtfAlarmNotificationFilterAllocate(
		      ntfHandle, &myAlarmFilter, 0, 0, 0, 2, 0, 0, 0),
		  SA_AIS_OK);
	...
	myAlarmFilter.notificationFilterHeader.notificationClassIds[1]
	    .vendorId = 144;
	myAlarmFilter.notificationFilterHeader.notificationClassIds[1].majorId =
	    44;
	myAlarmFilter.notificationFilterHeader.notificationClassIds[1].minorId =
	    22;
...
void alarmNotificationFilterTest(void)
{
	...
	safassert(saNtfAlarmNotificationFilterAllocate(
		      ntfHandle, &myAlarmFilter, 0, 0, 0, 0, 1, 1, 1),
		  SA_AIS_OK);
...
void alarmNotificationFilterTest2(void)
{
	...
	safassert(saNtfAlarmNotificationFilterAllocate(
		      ntfHandle, &myAlarmFilter, 0, 0, 0, 0, 3, 0, 0),
		  SA_AIS_OK);
	/* Set perceived severities */
	myAlarmFilter.probableCauses[0] = SA_NTF_BANDWIDTH_REDUCED;
...
void alarmNotificationFilterTest3(void)
{
	...
	safassert(saNtfAlarmNotificationFilterAllocate(
		      ntfHandle, &myAlarmFilter, 0, 0, 0, 0, 0, 2, 0),
		  SA_AIS_OK);
	/* Set perceived severities */
...
void alarmNotificationFilterTest4(void)
{
	...
	safassert(saNtfAlarmNotificationFilterAllocate(
		      ntfHandle, &myAlarmFilter, 0, 0, 0, 0, 0, 0, 2),
		  SA_AIS_OK);
	/* Set perceived severities */
	myAlarmFilter.trends[0] = SA_NTF_TREND_MORE_SEVERE;
...
void objectCreateDeleteNotificationFilterTest(void)
{
	...
	if ((rc = saNtfObjectCreateDeleteNotificationFilterAllocate(
		 ntfHandle, &myFilter, 1, 1, 1, 1, 2)) == SA_AIS_OK) {
		...
		*(myObjCrDelNotification.sourceIndicator) =
		...
		safassert(saNtfNotificationFree(
			      myObjCrDelNotification.notificationHandle),
			  SA_AIS_OK);
...
void stateChangeNotificationFilterTest(void)
{
	...
	if ((rc = saNtfStateChangeNotificationFilterAllocate(
		 ntfHandle, &myFilter, 1, 1, 1, 1, 2, 0)) == SA_AIS_OK) {
		myFilter.sourceIndicators[0] = SA_NTF_OBJECT_OPERATION;
		myFilter.sourceIndicators[1] = SA_NTF_MANAGEMENT_OPERATION;
		fillFilterHeader(&myFilter.notificationFilterHeader);
		myFilter.notificationFilterHeader.eventTypes[0] =
...
void stateChangeNotificationFilterTest2(void)
{
	...
	if ((rc = saNtfStateChangeNotificationFilterAllocate(
		 ntfHandle, &myFilter, 1, 1, 1, 1, 2, 2)) == SA_AIS_OK) {
		myFilter.sourceIndicators[0] = SA_NTF_OBJECT_OPERATION;
		myFilter.sourceIndicators[1] = SA_NTF_MANAGEMENT_OPERATION;
		fillFilterHeader(&myFilter.notificationFilterHeader);
		myFilter.notificationFilterHeader.eventTypes[0] =
		    SA_NTF_OBJECT_STATE_CHANGE;
		/* only filtering on stateId is used when filtering,
		 * SaNtfStateChangeNotificationFilterT is changed in
		 * NTF-A.02.01*/
...
void securityAlarmNotificationFilterTest2(void)
{
	...
	if ((rc = saNtfSecurityAlarmNotificationFilterAllocate(
		 ntfHandle, &myFilter, 0, 0, 0, 0, 0, 2, 0, 0, 0)) ==
	    SA_AIS_OK) {
...
void securityAlarmNotificationFilterTest3(void)
{
	...
	if ((rc = saNtfSecurityAlarmNotificationFilterAllocate(
		 ntfHandle, &myFilter, 0, 0, 0, 0, 0, 0, 3, 0, 0)) ==
	    SA_AIS_OK) {
		...
		myNotificationFilterHandles.securityAlarmFilterHandle =
		    myFilter.notificationFilterHandle;
		myNotificationFilterHandles.stateChangeFilterHandle = 0;
		myFilter.securityAlarmDetectors[0].valueType =
		    SA_NTF_VALUE_UINT32;
		myFilter.securityAlarmDetectors[0].value.uint32Val =
		    4008640511u; /* 0xeeeeffff */
		myFilter.securityAlarmDetectors[1].valueType =
		    SA_NTF_VALUE_UINT64;
		myFilter.securityAlarmDetectors[1].value.uint64Val =
		    1311879807313575935llu; /* 0x1234bbbbeeeeffff */
		myFilter.securityAlarmDetectors[2].valueType =
		    SA_NTF_VALUE_UINT8;
...
void securityAlarmNotificationFilterTest4(void)
{
	...
	if ((rc = saNtfSecurityAlarmNotificationFilterAllocate(
		 ntfHandle, &myFilter, 0, 0, 0, 0, 0, 0, 0, 3, 0)) ==
	    SA_AIS_OK) {
		...
		myFilter.serviceUsers[0].value.uint32Val =
		    4008640511u; /* 0xeeeeffff */
		myFilter.serviceUsers[1].valueType = SA_NTF_VALUE_UINT64;
		myFilter.serviceUsers[1].value.uint64Val =
		    1311879807313575935llu; /* 0x1234bbbbeeeeffff */
...
void securityAlarmNotificationFilterTest5(void)
{
	...
	if ((rc = saNtfSecurityAlarmNotificationFilterAllocate(
		 ntfHandle, &myFilter, 0, 0, 0, 0, 0, 0, 0, 0, 3)) ==
	    SA_AIS_OK) {
		...
		myFilter.serviceProviders[0].value.uint32Val =
		    4008640511u; /* 0xeeeeffff */
		myFilter.serviceProviders[1].valueType = SA_NTF_VALUE_UINT64;
		myFilter.serviceProviders[1].value.uint64Val =
		    1311879807313575935llu; /* 0x1234bbbbeeeeffff */
...
void combinedNotificationFilterTest(void)
{
	...
	if ((rc = saNtfAlarmNotificationFilterAllocate(
		 ntfHandle, &myAlarmFilter, 1, 1, 1, 1, 1, 1, 1)) ==
	    SA_AIS_OK) {
		fillFilterHeader(&myAlarmFilter.notificationFilterHeader);
		myAlarmFilter.perceivedSeverities[0] = SA_NTF_SEVERITY_WARNING;
		myAlarmFilter.probableCauses[0] = SA_NTF_BANDWIDTH_REDUCED;
		...
		if ((rc = saNtfObjectCreateDeleteNotificationFilterAllocate(
			 ntfHandle, &myObjCrDeFilter, 1, 1, 1, 1, 1)) ==
		    SA_AIS_OK) {
			fillFilterHeader(
			    &myObjCrDeFilter.notificationFilterHeader);
			myObjCrDeFilter.notificationFilterHeader.eventTypes[0] =
			    SA_NTF_OBJECT_CREATION;
			myObjCrDeFilter.sourceIndicators[0] =
			    SA_NTF_MANAGEMENT_OPERATION;

			if ((rc =
				 saNtfAttributeChangeNotificationFilterAllocate(
				     ntfHandle, &myAttrChangeFilter, 1, 1, 1, 1,
				     1)) == SA_AIS_OK) {
				fillFilterHeader(
				    &myAttrChangeFilter
					 .notificationFilterHeader);
				myAttrChangeFilter.notificationFilterHeader
				    .eventTypes[0] = SA_NTF_ATTRIBUTE_CHANGED;
				myAttrChangeFilter.sourceIndicators[0] =
				    SA_NTF_OBJECT_OPERATION;

				if ((rc =
					 saNtfSecurityAlarmNotificationFilterAllocate(
					     ntfHandle, &mySecAlarmFilter, 1, 1,
					     1, 1, 1, 1, 1, 1, 1)) ==
				    SA_AIS_OK) {
					fillFilterHeader(
					    &mySecAlarmFilter
					...
					mySecAlarmFilter.serviceUsers[0]
					    .valueType = SA_NTF_VALUE_INT32;
					mySecAlarmFilter.serviceUsers[0]
					    .value.int32Val = 468;
					mySecAlarmFilter.serviceProviders[0]
					    .valueType = SA_NTF_VALUE_INT32;
					mySecAlarmFilter.serviceProviders[0]
					    .value.int32Val = 789;
					mySecAlarmFilter
					    .securityAlarmDetectors[0]
					    .valueType = SA_NTF_VALUE_UINT64;
					mySecAlarmFilter
					    .securityAlarmDetectors[0]
					    .value.uint64Val = 123412341234llu;
					mySecAlarmFilter.severities[0] =
					    SA_NTF_SEVERITY_MAJOR;
					mySecAlarmFilter.probableCauses[0] =
					...
					if ((rc =
						...
						myStateChangeFilter
						    ...
						    .eventTypes[0] =
						    SA_NTF_OBJECT_STATE_CHANGE;
						/* only filtering on stateId is
						 * used when filtering,
						 * SaNtfStateChangeNotificationFilterT
						 * is changed in NTF-A.02.01*/
						myStateChangeFilter
						    .changedStates[0]
						    .stateId = 77;
						myStateChangeFilter
						    .changedStates[0]
						    .oldStatePresent = SA_FALSE;
						/*	 myStateChangeFilter->changedStates[0].newState
						 * = 3;*/
						/*	 myStateChangeFilter->changedStates[0].oldState
						 * = 5;*/
						...
						*myAlarmNotification
						 ...
						 * 5  */
						/* should be caught */
						createObjectCreateDeleteNotification(
						    ntfHandle,
						    &myObjCrDelNotification);
						*myObjCrDelNotification
						     .sourceIndicator =
						...
						*myAttrChangeNotification
						     .sourceIndicator =
						    SA_NTF_OBJECT_OPERATION;
						head = &myAttrChangeNotification
							    .notificationHeader;
						head->notificationObject
						    ->length =
						    strlen(NOTIF_OBJECT_TEST);
						(void)memcpy(
						    head->notificationObject
							->value,
						    NOTIF_OBJECT_TEST,
						...
						*myStateChangeNotification
						...
						/* === Security alarm  send   */
						createSecurityAlarmNotification(
						    ntfHandle,
						    &mySecAlarmNotification);
...
notificationFilterVerification_constructor(void)
{
	...
	test_case_add(31, alarmNotificationFilterTest,
		      "AlarmNotificationFilter all set");
	test_case_add(31, alarmNotificationFilterTest2,
		      "AlarmNotification probableCause");
	test_case_add(31, alarmNotificationFilterTest3,
		      "AlarmNotification perceivedSeverities");
	test_case_add(31, alarmNotificationFilterTest4,
		      "AlarmNotification trends");
	test_case_add(31, objectCreateDeleteNotificationFilterTest,
		      "ObjectCreateDeleteNotificationFilter sourceIndicators");
	test_case_add(31, attributeChangeNotificationFilterTest,
		      "AttributeChangeNotificationFilter sourceIndicators");
	test_case_add(31, stateChangeNotificationFilterTest,
		      "StateChangeNotificationFilter sourceIndicators");
	test_case_add(31, stateChangeNotificationFilterTest2,
		      "StateChangeNotificationFilter stateIds");
	test_case_add(31, securityAlarmNotificationFilterTest,
		      "SecurityAlarmNotificationFilter probableCauses");
	test_case_add(31, securityAlarmNotificationFilterTest2,
		      "SecurityAlarmNotificationFilter severities");
	test_case_add(31, securityAlarmNotificationFilterTest3,
		      "SecurityAlarmNotificationFilter securityAlarmDetectors");
	test_case_add(31, securityAlarmNotificationFilterTest4,
		      "SecurityAlarmNotificationFilter serviceUsers");
	test_case_add(31, securityAlarmNotificationFilterTest5,
		      "SecurityAlarmNotificationFilter serviceProviders");
	test_case_add(31, combinedNotificationFilterTest,
