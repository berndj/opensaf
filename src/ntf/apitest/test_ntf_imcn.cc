...
#include "ntf/apitest/test_ntf_imcn.h"
...
void saNtfNotificationCallback(
  ...
  if (notification->notificationType ==
    ...
    rec_notif_data
        .c_d_notif_ptr =
            const_cast<SaNtfObjectCreateDeleteNotificationT *>(
    ...
    rec_notif_data.evType = *(
        rec_notif_data.c_d_notif_ptr->notificationHeader.eventType);
  } else if (notification->notificationType ==
       SA_NTF_TYPE_ATTRIBUTE_CHANGE) {
    rec_notif_data.a_c_notif_ptr =
        const_cast<SaNtfAttributeChangeNotificationT *>(
            &notification->notification.attributeChangeNotification);
    rec_notif_data.nHandle =
        rec_notif_data.a_c_notif_ptr->notificationHandle;
    rec_notif_data.evType = *(
        rec_notif_data.a_c_notif_ptr->notificationHeader.eventType);
  } else if (notification->notificationType == SA_NTF_TYPE_STATE_CHANGE) {
    rec_notif_data.o_s_c_notif_ptr =
        const_cast<SaNtfStateChangeNotificationT *>(
            &notification->notification.stateChangeNotification);
    rec_notif_data.nHandle =
        rec_notif_data.o_s_c_notif_ptr->notificationHandle;
    rec_notif_data.evType = *(rec_notif_data.o_s_c_notif_ptr
 ...
 */
static SaBoolT bufs_equal(const SaUint8T *b1,
                          const SaUint8T *b2, SaUint16T ln) {
  SaBoolT rc = SA_TRUE;
  int i = 0;
  for (i = 0; i < ln; i++) {
    if (b1[i] != b2[i]) {
...
/**
 * Special case comparison (used when inputting buffer data with immcfg
 * command).
 */
static SaBoolT bufs_equal1(const SaUint8T *recbuf,
                           const SaUint8T *expbuf,
                           SaUint16T rec_ln) {
  SaBoolT rc = SA_TRUE;
  int i = 0;
  for (i = 0; i < rec_ln; i++) {
...
static void print_notif(NotifData *n) {
  ...
  printf(
      "\n-------------------------------------------------------------\n");
  printf("                 %s\n",
         (n == &rec_notif_data) ? "RECEIVED notif" : "EXPECTED notif");
  printf(
      "-------------------------------------------------------------\n");

  if (n->evType == SA_NTF_OBJECT_CREATION)
    snprintf(tmp, sizeof(tmp), "SA_NTF_OBJECT_CREATION");
  else if (n->evType == SA_NTF_OBJECT_DELETION)
    snprintf(tmp, sizeof(tmp), "SA_NTF_OBJECT_DELETION");
  else if (n->evType == SA_NTF_ATTRIBUTE_CHANGED)
    snprintf(tmp, sizeof(tmp), "SA_NTF_ATTRIBUTE_CHANGED");
  else if (n->evType == SA_NTF_OBJECT_STATE_CHANGE)
    snprintf(tmp, sizeof(tmp), "SA_NTF_OBJECT_STATE_CHANGE");
  ...
  if (tmpi == SA_NTF_OBJECT_OPERATION)
    snprintf(tmp, sizeof(tmp), "SA_NTF_OBJECT_OPERATION");
  else if (tmpi == SA_NTF_MANAGEMENT_OPERATION)
    snprintf(tmp, sizeof(tmp), "SA_NTF_MANAGEMENT_OPERATION");
  else
    snprintf(tmp, sizeof(tmp), "SA_NTF_UNKNOWN_OPERATION");
  ...
  for (i = 0; i < nHeader->numAdditionalInfo; i++) {
    safassert(saNtfPtrValGet(
        n->nHandle, &nHeader->additionalInfo[i].infoValue,
  ...
  if (n->evType == SA_NTF_ATTRIBUTE_CHANGED) {
    ...
    for (i = 0; i < n->c_d_notif_ptr->numAttributes; i++) {
      if (n->c_d_notif_ptr->objectAttributes[i]
        .attributeType == SA_NTF_VALUE_STRING) {
        safassert(saNtfPtrValGet(
            n->nHandle, &n->c_d_notif_ptr->objectAttributes[i].attributeValue,
            reinterpret_cast<void **>(&str), &numElem), SA_AIS_OK);
        printf("[%d]: Id:%d Type:%d Value:(%d)%s\n", i,
               n->c_d_notif_ptr->objectAttributes[i]
             .attributeId,
               n->c_d_notif_ptr->objectAttributes[i]
             .attributeType,
               numElem, str);
      } else if (n->c_d_notif_ptr->objectAttributes[i]
               .attributeType == SA_NTF_VALUE_UINT32) {
        printf("[%d]: Id:%d Type:%d Value:%u\n", i,
               n->c_d_notif_ptr->objectAttributes[i]
             .attributeId,
  ...
  for (i = 0; i < nHeader_rec->numAdditionalInfo; i++) {
    ...
    for (j = 0; j < nHeader_exp->numAdditionalInfo; j++) {
      if (nHeader_exp->additionalInfo[j].infoType ==
        ...
        SA_NTF_VALUE_STRING) {
        safassert(saNtfPtrValGet(
            n_exp->nHandle, &nHeader_exp->additionalInfo[j].infoValue,
            reinterpret_cast<void **>(&exp_str), &exp_ln), SA_AIS_OK);
        safassert(saNtfPtrValGet(
            n_rec->nHandle, &nHeader_rec->additionalInfo[i].infoValue,
            reinterpret_cast<void **>(&rec_str), &rec_ln), SA_AIS_OK);
        if (exp_ln == rec_ln &&
            !strcmp(exp_str, rec_str)) {
          /*
           * CCBId varies between different test
           * runs so it must be handled as a
           * special case so that a mismatch is
          ...
          infoIdVec[0]
             [nHeader_exp->additionalInfo[j]
                  .infoId] = 1;
          infoIdVec[1]
             [nHeader_rec->additionalInfo[i]
                  .infoId] = 1;
          rc = SA_TRUE;
          break;
        }
 ...
 */
static SaBoolT is_equal_ch_attr(const NotifData *n_exp,
                                const NotifData *n_rec) {
  SaBoolT rc = SA_TRUE;
  int i = 0;
  int j = 0;
  char *exp_str = NULL;
    ...
    for (j = 0; j < n_exp->a_c_notif_ptr->numAttributes; j++) {
      if ((n_exp->a_c_notif_ptr->changedAttributes[j]
        ...
        if (n_exp->a_c_notif_ptr->changedAttributes[j]
           ...
           * varying ccb id value doesn't cause a
           * mismatch.
           */
          if (n_exp->ccbInfoId ==
            n_exp->a_c_notif_ptr
 ...
 */
static SaBoolT is_equal_obj_attr(const NotifData *n_exp,
                                 const NotifData *n_rec) {
  SaBoolT rc = SA_TRUE;
  int i = 0;
  int j = 0;
  ...
  for (i = 0; i < n_rec->c_d_notif_ptr->numAttributes; i++) {
    ...
    for (j = 0; j < n_exp->c_d_notif_ptr->numAttributes; j++) {
      if (n_exp->c_d_notif_ptr->objectAttributes[j]
            .attributeType ==
        n_rec->c_d_notif_ptr->objectAttributes[i]
            .attributeType &&
          elemVerified[j] == 0) {
        if (n_rec->c_d_notif_ptr->objectAttributes[i]
          .attributeType == SA_NTF_VALUE_STRING) {
          char *exp_str = NULL;
          char *rec_str = NULL;
          safassert(saNtfPtrValGet(
 ...
 */
static SaBoolT compare_notifs(NotifData *n_exp,
                              NotifData *n_rec) {
  SaBoolT rc = SA_FALSE;
  SaNtfNotificationHeaderT *nHeader_exp =
      (n_exp->c_d_notif_ptr != NULL)
  ...
  } else if (nHeader_exp->notificationObject->length !=
     nHeader_rec->notificationObject->length) {
  } else if (strcmp(
      reinterpret_cast<char *>(nHeader_exp->notificationObject->value),
      reinterpret_cast<char *>(nHeader_rec->notificationObject->value))) {
  } else if (nHeader_exp->notifyingObject->length !=
     nHeader_rec->notifyingObject->length) {
  } else if (strcmp(
      reinterpret_cast<char *>(nHeader_exp->notifyingObject->value),
      reinterpret_cast<char *>(nHeader_rec->notifyingObject->value))) {
  } else if (nHeader_exp->notificationClassId->vendorId !=
     nHeader_rec->notificationClassId->vendorId) {
  } else if (nHeader_exp->notificationClassId->majorId !=
     nHeader_rec->notificationClassId->majorId) {
  } else if (nHeader_exp->notificationClassId->minorId !=
     nHeader_rec->notificationClassId->minorId) {
  ...
  } else if (nHeader_exp->numAdditionalInfo !=
     nHeader_rec->numAdditionalInfo) {
  ...
  (n_exp->c_d_notif_ptr != NULL) ? free(n_exp->c_d_notif_ptr)
               : free(n_exp->a_c_notif_ptr);
  return rc;
}
...
static SaAisErrorT subscribe_notifications() {
  ...
  SaAisErrorT errorCode = saNtfAttributeChangeNotificationFilterAllocate(
      ntfHandle, &attChFilter, 1, 0, 0, 1, 0);
  if (errorCode == SA_AIS_OK) {
    notifFilterHandles.attributeChangeFilterHandle =
        attChFilter.notificationFilterHandle;
    attChFilter.notificationFilterHeader.eventTypes[0] =
        SA_NTF_ATTRIBUTE_CHANGED;
    attChFilter.notificationFilterHeader.notificationClassIds
        ->vendorId = 32993;
    attChFilter.notificationFilterHeader.notificationClassIds
        ->majorId = SA_SVC_IMMS;
    attChFilter.notificationFilterHeader.notificationClassIds
    ...
    errorCode = saNtfObjectCreateDeleteNotificationFilterAllocate(
        ntfHandle, &objCrDelFilter, 2, 0, 0, 1, 0);
    if (errorCode == SA_AIS_OK) {
      notifFilterHandles.objectCreateDeleteFilterHandle =
          objCrDelFilter.notificationFilterHandle;
      objCrDelFilter.notificationFilterHeader.eventTypes[0] =
          SA_NTF_OBJECT_CREATION;
      objCrDelFilter.notificationFilterHeader.eventTypes[1] =
          SA_NTF_OBJECT_DELETION;
      objCrDelFilter.notificationFilterHeader
          .notificationClassIds->vendorId = 32993;
      objCrDelFilter.notificationFilterHeader
          .notificationClassIds->majorId = SA_SVC_IMMS;
      objCrDelFilter.notificationFilterHeader
          .notificationClassIds->minorId = 0;

      errorCode = saNtfStateChangeNotificationFilterAllocate(
          ntfHandle, &objChangeFilter, 1, 0, 0, 1, 0, 0);
      if (errorCode == SA_AIS_OK) {
        notifFilterHandles.stateChangeFilterHandle =
            objChangeFilter.notificationFilterHandle;
        objChangeFilter.notificationFilterHeader
            .eventTypes[0] = SA_NTF_OBJECT_STATE_CHANGE;
        objChangeFilter.notificationFilterHeader
            .notificationClassIds->vendorId = 32993;
        objChangeFilter.notificationFilterHeader
            .notificationClassIds->majorId =
            SA_SVC_IMMS;
        objChangeFilter.notificationFilterHeader
            .notificationClassIds->minorId = 0;
...
static SaAisErrorT unsub_notifications() {
  ...
  int tryCnt = 0;
  while ((errorCode = saNtfNotificationUnsubscribe(1)) ==
 ...
 */
static SaAisErrorT set_ntf(
    NotifData *n_exp, SaNtfEventTypeT ntfEventType,
  ...
  if (n_exp->evType == SA_NTF_OBJECT_CREATION ||
    ...
    if ((error = saNtfObjectCreateDeleteNotificationAllocate(
       ...
       numAttr, SA_NTF_ALLOC_SYSTEM_LIMIT)) == SA_AIS_OK) {
      nHeader = &n_exp->c_d_notif_ptr->notificationHeader;
      n_exp->nHandle =
          n_exp->c_d_notif_ptr->notificationHandle;
      *n_exp->c_d_notif_ptr->sourceIndicator =
          (configObj) ? SA_NTF_MANAGEMENT_OPERATION
    ...
    if ((error = saNtfAttributeChangeNotificationAllocate(
      ...
      n_exp->nHandle =
          n_exp->a_c_notif_ptr->notificationHandle;
      *n_exp->a_c_notif_ptr->sourceIndicator =
          (configObj) ? SA_NTF_MANAGEMENT_OPERATION
          : SA_NTF_OBJECT_OPERATION;
    }
  ...
  if (nHeader != NULL) {
    if (configObj) {
      snprintf(reinterpret_cast<char *>(nHeader->notificationObject->value),
               SA_MAX_NAME_LENGTH, "%s", dn);
    } else {
      snprintf(reinterpret_cast<char *>(nHeader->notificationObject->value),
               SA_MAX_NAME_LENGTH, "%s", strchr(dn, '=') + 1);
    }
    nHeader->notificationObject->length =
       strlen(reinterpret_cast<char *>(nHeader->notificationObject->value)) + 1;
    snprintf(reinterpret_cast<char *>(nHeader->notifyingObject->value),
             SA_MAX_NAME_LENGTH, "safApp=OpenSaf");
    nHeader->notifyingObject->length =
        strlen(reinterpret_cast<char *>(nHeader->notifyingObject->value)) + 1;
 ...
 */
static SaAisErrorT set_add_info(
    NotifData *n_exp, SaUint16T idx,
    SaNtfElementIdT infoId, const char *infoValue) {
  SaUint8T *temp = NULL;

  SaNtfNotificationHeaderT *nHeader =
      (n_exp->c_d_notif_ptr != NULL)
    ? &n_exp->c_d_notif_ptr->notificationHeader
    : &n_exp->a_c_notif_ptr->notificationHeader;

  SaAisErrorT error = saNtfPtrValAllocate(n_exp->nHandle, strlen(infoValue) + 1,
 ...
 */
static SaAisErrorT set_attr_str(
    NotifData *n_exp, SaUint16T idx,
  ...
  if (n_exp->c_d_notif_ptr != NULL) {
    error = saNtfPtrValAllocate(
        n_exp->nHandle, strlen(attrValue) + 1, reinterpret_cast<void **>(&temp),
 ...
 */
static SaAisErrorT set_attr_scalar(
    NotifData *n_exp, SaUint16T idx,
    SaNtfElementIdT attrId,
    SaNtfValueTypeT valType, void *attrValue) {
  SaAisErrorT error = SA_AIS_OK;
  ...
  if (valType == SA_NTF_VALUE_UINT32) {
    n_exp->c_d_notif_ptr->objectAttributes[idx]
        .attributeValue.uint32Val = *reinterpret_cast<SaUint32T *>(attrValue);
  } else if (valType == SA_NTF_VALUE_INT32) {
    n_exp->c_d_notif_ptr->objectAttributes[idx]
        .attributeValue.int32Val = *reinterpret_cast<SaInt32T *>(attrValue);
  } else if (valType == SA_NTF_VALUE_UINT64) {
    n_exp->c_d_notif_ptr->objectAttributes[idx]
        .attributeValue.uint64Val = *reinterpret_cast<SaUint64T *>(attrValue);
  } else if (valType == SA_NTF_VALUE_INT64) {
    n_exp->c_d_notif_ptr->objectAttributes[idx]
        .attributeValue.int64Val = *reinterpret_cast<SaInt64T *>(attrValue);
  } else if (valType == SA_NTF_VALUE_FLOAT) {
    n_exp->c_d_notif_ptr->objectAttributes[idx]
        .attributeValue.floatVal = *reinterpret_cast<SaFloatT *>(attrValue);
  } else if (valType == SA_NTF_VALUE_DOUBLE) {
    n_exp->c_d_notif_ptr->objectAttributes[idx]
        .attributeValue.doubleVal = *reinterpret_cast<SaDoubleT *>(attrValue);
  } else {
    error = SA_AIS_ERR_FAILED_OPERATION;
  }
  return error;
 ...
 */
static SaAisErrorT set_attr_buf(
    NotifData *n_exp, SaUint16T idx,
    SaNtfElementIdT attrId, SaNtfValueTypeT valType,
    void *attrValue) {
  SaAisErrorT error = SA_AIS_OK;
  SaUint16T numAlloc = (valType == SA_NTF_VALUE_BINARY)
         ? (reinterpret_cast<SaAnyT *>(attrValue))->bufferSize
         : (reinterpret_cast<SaNameT *>(attrValue))->length;
  SaUint8T *temp = NULL;
  SaUint8T *srcPtr = (valType == SA_NTF_VALUE_BINARY)
             ? (reinterpret_cast<SaAnyT *>(attrValue))->bufferAddr
             : (reinterpret_cast<SaNameT *>(attrValue))->value;
...
static SaAisErrorT set_attr_extended_name(
    NotifData *n_exp, SaUint16T idx,
    SaNtfElementIdT attrId,
  ...
  SaAisErrorT error = SA_AIS_OK;
  SaUint16T numAlloc = strlen(
      saAisNameBorrow(reinterpret_cast<SaNameT *>(attrValue)));
  SaUint8T *temp = NULL;
 ...
 */
static SaAisErrorT set_attr_change_buf(
    NotifData *n_exp, SaUint16T idx,
    SaNtfElementIdT attrId,
    SaNtfValueTypeT valType,
    void *newValue) {
  ...
  SaUint16T numAlloc = (valType == SA_NTF_VALUE_BINARY)
         ...
         : (reinterpret_cast<SaNameT *>(newValue))->length;
  SaUint8T *temp = NULL;
  SaUint8T *srcPtr = (valType == SA_NTF_VALUE_BINARY)
...
static SaAisErrorT set_attr_change_extended_name(
    NotifData *n_exp,
    SaUint16T idx,
    SaNtfElementIdT attrId,
    SaNtfValueTypeT valType,
 ...
 */
static SaAisErrorT set_attr_change_scalar(
    NotifData *n_exp, SaUint16T idx,
    SaNtfElementIdT attrId,
    SaNtfValueTypeT valType,
    void *newValue) {
 ...
 */
static void create_rt_test_object(
    const SaImmClassNameT cname, const char *dn,
    SaInt32T int32Var, SaUint32T uint32Var,
    SaInt64T int64Var, SaUint64T uint64Var,
    SaTimeT *timeVar, SaNameT *nameVar,
 ...
 */
static void modify_rt_test_object(
    const char *dn, SaInt32T modType,
  ...
  while (attr[i] != NULL) {
    ...
    attrMod[i]->modType = static_cast<SaImmAttrModificationTypeT>(
        modType);
    attrMod[i]->modAttr.attrName = const_cast<char *>(attr[i]->attrName);
...
static int isLongDnsAllowed() {
  ...
  for (i = 0; attributes[i]; ++i) {
    if (!strcmp(attrName, attributes[i]->attrName) &&
      ...
      longDnsAllowed = *reinterpret_cast<int *>(attributes[i]->attrValues[0]);
      break;
    }
  }
...
void objectCreateTest_01(void) {
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_OBJECT_CREATION, DNTESTRT, 12, 12),
        SA_AIS_OK);
    safassert(
    ...
    safassert(set_attr_scalar(&n_exp, 8, 8, SA_NTF_VALUE_DOUBLE,
            &doubleVar),
        SA_AIS_OK);
    safassert(set_attr_buf(&n_exp, 9, 9, SA_NTF_VALUE_LDAP_NAME,
               &nameVar),
        SA_AIS_OK);
    safassert(set_attr_scalar(&n_exp, 10, 10, SA_NTF_VALUE_INT64,
        ...
        SA_AIS_OK);
    safassert(
        set_attr_buf(&n_exp, 11, 11, SA_NTF_VALUE_BINARY, &anyVar),
        SA_AIS_OK);
...
/**
 * Modify a runtime test object and verify correctness of generated
 * notification.
 */
void objectModifyTest_02(void) {
  ...
  void *v1[] = {&var1};
  void *v2[] = {&var2};
  TestAttributeValue att1 = {.attrName = "testUint32",
           .attrType = SA_IMM_ATTR_SAUINT32T,
           .attrValues = v1};
  TestAttributeValue att2 = {.attrName = "testFloat",
           .attrType = SA_IMM_ATTR_SAFLOATT,
           .attrValues = v2};
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTRT, 4, 4),
        SA_AIS_OK);
    safassert(
        set_add_info(&n_exp, 0, 0, "SaImmAttrImplementerName"),
        SA_AIS_OK);
    ...
    safassert(set_attr_change_scalar(&n_exp, 2, 2,
             SA_NTF_VALUE_UINT32, &var1),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 3, 3,
             SA_NTF_VALUE_FLOAT, &var2),
        SA_AIS_OK);
...
void objectModifyTest_03(void) {
  ...
  void *v1[] = {&var1};
  void *v2[] = {&var2};
  TestAttributeValue att1 = {.attrName = "testName",
           .attrType = SA_IMM_ATTR_SANAMET,
           .attrValues = v1};
  TestAttributeValue att2 = {.attrName = "testAny",
           .attrType = SA_IMM_ATTR_SAANYT,
           .attrValues = v2};
  TestAttributeValue *attrs[] = {&att1, &att2, NULL};
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_attr_change_buf(&n_exp, 2, 2,
        ...
        SA_AIS_OK);
    safassert(set_attr_change_buf(&n_exp, 3, 3, SA_NTF_VALUE_BINARY,
                &var2),
        SA_AIS_OK);
...
void objectModifyTest_04(void) {
  ...
  TestAttributeValue att = {.attrName = "testInt32",
          .attrType = SA_IMM_ATTR_SAINT32T,
          .attrValues = v};
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTRT, 3, 4),
        SA_AIS_OK);
    safassert(
        set_add_info(&n_exp, 0, 0, "SaImmAttrImplementerName"),
        SA_AIS_OK);
    ...
    safassert(set_attr_change_scalar(&n_exp, 2, 2,
        ...
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 3, 2,
...
void objectModifyTest_05(void) {
  ...
  void *v[] = {&addvar};
  TestAttributeValue att = {.attrName = "testUint32",
          .attrType = SA_IMM_ATTR_SAUINT32T,
          .attrValues = v};
  TestAttributeValue *attrs[] = {&att, NULL};
...
void objectModifyTest_06(void) {
  ...
  void *v[] = {&addvar};
  TestAttributeValue att = {.attrName = "testInt64",
          .attrType = SA_IMM_ATTR_SAINT64T,
          .attrValues = v};
  TestAttributeValue *attrs[] = {&att, NULL};
...
void objectModifyTest_07(void) {
  ...
  void *v[] = {&addvar};
  TestAttributeValue att = {.attrName = "testUint64",
          .attrType = SA_IMM_ATTR_SAUINT64T,
          .attrValues = v};
  TestAttributeValue *attrs[] = {&att, NULL};
...
void objectModifyTest_08(void) {
  ...
  void *v[] = {&addvar};
  TestAttributeValue att = {.attrName = "testFloat",
          .attrType = SA_IMM_ATTR_SAFLOATT,
          .attrValues = v};
  TestAttributeValue *attrs[] = {&att, NULL};
...
void objectModifyTest_09(void) {
  ...
  void *v[] = {&addvar};
  TestAttributeValue att = {.attrName = "testDouble",
          .attrType = SA_IMM_ATTR_SADOUBLET,
          .attrValues = v};
  TestAttributeValue *attrs[] = {&att, NULL};
...
void objectModifyTest_10(void) {
  ...
  void *v[] = {&addvar};
  TestAttributeValue att = {.attrName = "testTime",
          .attrType = SA_IMM_ATTR_SATIMET,
          .attrValues = v};
  TestAttributeValue *attrs[] = {&att, NULL};
...
void objectModifyTest_11(void) {
  ...
  void *v[] = {&addvar};
  TestAttributeValue att = {.attrName = "testString",
          .attrType = SA_IMM_ATTR_SASTRINGT,
          .attrValues = v};
  TestAttributeValue *attrs[] = {&att, NULL};
...
void objectModifyTest_12(void) {
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_attr_change_buf(&n_exp, 2, 2,
        ...
        SA_AIS_OK);
    safassert(set_attr_change_buf(&n_exp, 3, 2,
...
void objectModifyTest_13(void) {
  ...
  SaAnyT oldvar = {.bufferSize = sizeof(BUF2),
       .bufferAddr = const_cast<SaUint8T *>(BUF2)};
  SaAnyT addvar = {.bufferSize = sizeof(BUF3),
       .bufferAddr = const_cast<SaUint8T *>(BUF3)};
  void *v[] = {&addvar};
  TestAttributeValue att = {.attrName = "testAny",
          .attrType = SA_IMM_ATTR_SAANYT,
          .attrValues = v};
  TestAttributeValue *attrs[] = {&att, NULL};
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_attr_change_str(&n_exp, 1, 1, "OsafNtfCmTestRT"),
        SA_AIS_OK);
    safassert(set_attr_change_buf(&n_exp, 2, 2, SA_NTF_VALUE_BINARY,
                &oldvar),
        SA_AIS_OK);
    safassert(set_attr_change_buf(&n_exp, 3, 2, SA_NTF_VALUE_BINARY,
                &addvar),
        SA_AIS_OK);
...
void objectModifyTest_14(void) {
  ...
  void *v1[] = {&var1};
  void *v2[] = {&var2};
  TestAttributeValue att1 = {.attrName = "testString",
           .attrType = SA_IMM_ATTR_SASTRINGT,
           .attrValues = v1};
  TestAttributeValue att2 = {.attrName = "testDouble",
           .attrType = SA_IMM_ATTR_SADOUBLET,
           .attrValues = v2};
  TestAttributeValue *attrs[] = {&att1, &att2, NULL};
...
void objectModifyTest_15(void) {
  ...
  void *v1[] = {&var1};
  TestAttributeValue att1 = {.attrName = "testTime",
           .attrType = SA_IMM_ATTR_SATIMET,
           .attrValues = v1};
...
void objectModifyTest_16(void) {
  ...
  TestAttributeValue att1 = {.attrName = "testString",
           .attrType = SA_IMM_ATTR_SASTRINGT,
           .attrValues = v1};
  TestAttributeValue att2 = {.attrName = "testInt32",
           .attrType = SA_IMM_ATTR_SAINT32T,
           .attrValues = v2};
  TestAttributeValue *attrs[] = {&att1, &att2, NULL};
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTRT, 4, 7),
        SA_AIS_OK);
    safassert(
        set_add_info(&n_exp, 0, 0, "SaImmAttrImplementerName"),
        SA_AIS_OK);
    ...
    safassert(set_attr_change_scalar(&n_exp, 4, 3,
             SA_NTF_VALUE_INT32, &ivar1),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 5, 3,
             SA_NTF_VALUE_INT32, &ivar2),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 6, 3,
             SA_NTF_VALUE_INT32, &ivar3),
        SA_AIS_OK);
...
void objectModifyTest_17(void) {
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTRT, 4, 5),
        SA_AIS_OK);
    safassert(
        set_add_info(&n_exp, 0, 0, "SaImmAttrImplementerName"),
        SA_AIS_OK);
    ...
    safassert(set_attr_change_scalar(&n_exp, 3, 3,
             SA_NTF_VALUE_INT32, &var3),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 4, 3,
             SA_NTF_VALUE_INT32, &var2),
        SA_AIS_OK);
...
void objectModifyTest_18(void) {
  ...
  SaAnyT var10 = {.bufferSize = sizeof(BUF1),
      .bufferAddr = const_cast<SaUint8T *>(BUF1)};
  SaAnyT var100 = {.bufferSize = sizeof(BUF2),
       .bufferAddr = const_cast<SaUint8T *>(BUF2)};
  SaAnyT var1000 = {.bufferSize = sizeof(BUF3),
        .bufferAddr = const_cast<SaUint8T *>(BUF3)};
  void *v[] = {&var1,  &var2,  &var333, &var444, &var5,
         &var66, &var77, &var888, &var9,   &var10};

  TestAttributeValue att1 = {.attrName = "testInt32",
           .attrType = SA_IMM_ATTR_SAINT32T,
           .attrValues = &v[0]};
  TestAttributeValue att2 = {.attrName = "testUint32",
           .attrType = SA_IMM_ATTR_SAUINT32T,
           .attrValues = &v[1]};
  TestAttributeValue att3 = {.attrName = "testInt64",
           .attrType = SA_IMM_ATTR_SAINT64T,
           .attrValues = &v[2]};
  TestAttributeValue att4 = {.attrName = "testUint64",
           .attrType = SA_IMM_ATTR_SAUINT64T,
           .attrValues = &v[3]};
  TestAttributeValue att5 = {.attrName = "testFloat",
           .attrType = SA_IMM_ATTR_SAFLOATT,
           .attrValues = &v[4]};
  TestAttributeValue att6 = {.attrName = "testDouble",
           .attrType = SA_IMM_ATTR_SADOUBLET,
           .attrValues = &v[5]};
  TestAttributeValue att7 = {.attrName = "testTime",
           .attrType = SA_IMM_ATTR_SATIMET,
           .attrValues = &v[6]};
  TestAttributeValue att8 = {.attrName = "testString",
           .attrType = SA_IMM_ATTR_SASTRINGT,
           .attrValues = &v[7]};
  TestAttributeValue att9 = {.attrName = "testName",
           .attrType = SA_IMM_ATTR_SANAMET,
           .attrValues = &v[8]};
  TestAttributeValue att10 = {.attrName = "testAny",
            .attrType = SA_IMM_ATTR_SAANYT,
            .attrValues = &v[9]};
  TestAttributeValue *attrs[] = {&att1, &att2,  &att3, &att4,
               &att5, &att6,  &att7, &att8,
               &att9, &att10, NULL};
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTRT, 12, 29),
        SA_AIS_OK);
    safassert(
        set_add_info(&n_exp, 0, 0, "SaImmAttrImplementerName"),
        SA_AIS_OK);
    ...
    safassert(set_attr_change_str(&n_exp, 1, 1, "OsafNtfCmTestRT"),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 2, 2,
             SA_NTF_VALUE_INT32, &var1),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 3, 2,
             SA_NTF_VALUE_INT32, &var11),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 4, 2,
             SA_NTF_VALUE_INT32, &var111),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 5, 3,
             SA_NTF_VALUE_UINT32, &var2),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 6, 3,
             SA_NTF_VALUE_UINT32, &var22),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 7, 3,
             SA_NTF_VALUE_UINT32, &var222),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 8, 4,
             SA_NTF_VALUE_INT64, &var3),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 9, 4,
             SA_NTF_VALUE_INT64, &var33),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 10, 4,
             SA_NTF_VALUE_INT64, &var333),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 11, 5,
             SA_NTF_VALUE_UINT64, &var4),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 12, 5,
             SA_NTF_VALUE_UINT64, &var44),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 13, 5,
             SA_NTF_VALUE_UINT64, &var444),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 14, 6,
             SA_NTF_VALUE_FLOAT, &var5),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 15, 6,
             SA_NTF_VALUE_FLOAT, &var55),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 16, 6,
             SA_NTF_VALUE_FLOAT, &var555),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 17, 7,
             SA_NTF_VALUE_DOUBLE, &var66),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 18, 7,
             SA_NTF_VALUE_DOUBLE, &var666),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 19, 8,
             SA_NTF_VALUE_INT64, &var77),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 20, 8,
             SA_NTF_VALUE_INT64, &var777),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 21, 9, var88), SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 22, 9, var888),
        SA_AIS_OK);
    safassert(set_attr_change_buf(&n_exp, 23, 10,
                SA_NTF_VALUE_LDAP_NAME, &var9),
        SA_AIS_OK);
    safassert(set_attr_change_buf(&n_exp, 24, 10,
                SA_NTF_VALUE_LDAP_NAME, &var99),
        SA_AIS_OK);
    safassert(set_attr_change_buf(&n_exp, 25, 10,
                SA_NTF_VALUE_LDAP_NAME, &var999),
        SA_AIS_OK);
    safassert(set_attr_change_buf(&n_exp, 26, 11,
                SA_NTF_VALUE_BINARY, &var10),
        SA_AIS_OK);
    safassert(set_attr_change_buf(&n_exp, 27, 11,
                SA_NTF_VALUE_BINARY, &var100),
        SA_AIS_OK);
    safassert(set_attr_change_buf(&n_exp, 28, 11,
                SA_NTF_VALUE_BINARY, &var1000),
        SA_AIS_OK);
...
/**
 * Delete a runtime test object and verify correctness of generated
 * notification.
 */
void objectDeleteTest_19(void) {
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_OBJECT_DELETION, DNTESTRT, 0, 0),
        SA_AIS_OK);
...
void objectCreateTest_20(void) {
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(
        set_ntf(&n_exp, SA_NTF_OBJECT_CREATION, DNTESTCFG, 14, 14),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 0, 0, "SaImmAttrAdminOwnerName"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 1, 1, "SaImmAttrClassName"),
        SA_AIS_OK);
    ...
    safassert(set_attr_scalar(&n_exp, 10, 10, SA_NTF_VALUE_DOUBLE,
            &doubleVar),
        SA_AIS_OK);
    safassert(set_attr_buf(&n_exp, 11, 11, SA_NTF_VALUE_LDAP_NAME,
               &nameVar),
        SA_AIS_OK);
    safassert(set_attr_scalar(&n_exp, 12, 12, SA_NTF_VALUE_INT64,
        ...
        SA_AIS_OK);
    safassert(
        set_attr_buf(&n_exp, 13, 13, SA_NTF_VALUE_BINARY, &anyVar),
        SA_AIS_OK);
...
void objectModifyTest_21(void) {
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTCFG, 6, 6),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 0, 0, "SaImmAttrAdminOwnerName"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 1, 1, "SaImmAttrClassName"),
...
void objectModifyTest_23(void) {
  ...
  SaInt32T addvar = INT32VAR2;
  SaInt32T oldvar = INT32VAR1;
  snprintf(command, MAX_DATA, "immcfg -t 20 -a testInt32Cfg+=%d %s",
           addvar, DNTESTCFG);
  assert(system(command) != -1);
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTCFG, 5, 6),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 0, 0, "SaImmAttrAdminOwnerName"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 1, 1, "SaImmAttrClassName"),
    ...
    safassert(set_add_info(&n_exp, 2, 2, "SaImmOiCcbIdT"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 3, 3, "ccbLast"), SA_AIS_OK);
    safassert(set_add_info(&n_exp, 4, 4, "testInt32Cfg"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 0, 0, "immcfg_xxx"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 1, 1, "OsafNtfCmTestCFG"),
...
void objectModifyTest_24(void) {
  ...
  SaUint32T oldvar = UINT32VAR2;
  snprintf(command, MAX_DATA, "immcfg -t 20 -a testUint32Cfg+=%u %s",
           addvar, DNTESTCFG);
  assert(system(command) != -1);
...
void objectModifyTest_25(void) {
  ...
  SaInt64T addvar = INT64VAR2;
  SaInt64T oldvar = INT64VAR1;
  snprintf(command, MAX_DATA, "immcfg -t 20 -a testInt64Cfg+=%lld %s",
           addvar, DNTESTCFG);
  assert(system(command) != -1);
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_add_info(&n_exp, 2, 2, "SaImmOiCcbIdT"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 3, 3, "ccbLast"), SA_AIS_OK);
    safassert(set_add_info(&n_exp, 4, 4, "testInt64Cfg"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 0, 0, "immcfg_xxx"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 1, 1, "OsafNtfCmTestCFG"),
...
void objectModifyTest_26(void) {
  ...
  SaUint64T addvar = UINT64VAR2;
  SaUint64T oldvar = UINT64VAR1;
  snprintf(command, MAX_DATA, "immcfg -t 20 -a testUint64Cfg+=%llu %s",
           addvar, DNTESTCFG);
  assert(system(command) != -1);
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_add_info(&n_exp, 2, 2, "SaImmOiCcbIdT"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 3, 3, "ccbLast"), SA_AIS_OK);
    safassert(set_add_info(&n_exp, 4, 4, "testUint64Cfg"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 0, 0, "immcfg_xxx"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 1, 1, "OsafNtfCmTestCFG"),
...
void objectModifyTest_27(void) {
  ...
  SaFloatT addvar = FLOATVAR3;
  SaFloatT oldvar = FLOATVAR2;
  snprintf(command, MAX_DATA, "immcfg -t 20 -a testFloatCfg+=%f %s",
           addvar, DNTESTCFG);
  assert(system(command) != -1);
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_add_info(&n_exp, 2, 2, "SaImmOiCcbIdT"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 3, 3, "ccbLast"), SA_AIS_OK);
    safassert(set_add_info(&n_exp, 4, 4, "testFloatCfg"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 0, 0, "immcfg_xxx"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 1, 1, "OsafNtfCmTestCFG"),
...
void objectModifyTest_28(void) {
  ...
  SaDoubleT addvar = DOUBLEVAR2;
  SaDoubleT oldvar = DOUBLEVAR1;
  snprintf(command, MAX_DATA, "immcfg -t 20 -a testDoubleCfg+=%lf %s",
           addvar, DNTESTCFG);
  assert(system(command) != -1);
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_add_info(&n_exp, 2, 2, "SaImmOiCcbIdT"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 3, 3, "ccbLast"), SA_AIS_OK);
    safassert(set_add_info(&n_exp, 4, 4, "testDoubleCfg"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 0, 0, "immcfg_xxx"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 1, 1, "OsafNtfCmTestCFG"),
...
void objectModifyTest_30(void) {
  ...
  SaStringT addvar = STRINGVAR2;
  SaStringT oldvar = STRINGVAR1;
  snprintf(command, MAX_DATA, "immcfg -t 20 -a testStringCfg+=%s %s",
           addvar, DNTESTCFG);
  assert(system(command) != -1);
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_add_info(&n_exp, 2, 2, "SaImmOiCcbIdT"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 3, 3, "ccbLast"), SA_AIS_OK);
    safassert(set_add_info(&n_exp, 4, 4, "testStringCfg"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 0, 0, "immcfg_xxx"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 1, 1, "OsafNtfCmTestCFG"),
...
void objectModifyTest_33(void) {
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_add_info(&n_exp, 4, 4, "testStringCfg"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 5, 5, "testDoubleCfg"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 0, 0, "immcfg_xxx"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 1, 1, "OsafNtfCmTestCFG"),
...
void objectModifyTest_34(void) {
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTCFG, 5, 5),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 0, 0, "SaImmAttrAdminOwnerName"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 1, 1, "SaImmAttrClassName"),
...
void objectModifyTest_35(void) {
  ...
  snprintf(command, MAX_DATA,
    "immcfg -t 20 -a testStringCfg+=%s -a testInt32Cfg+=%d %s",
    saddvar, iaddvar, DNTESTCFG);
  assert(system(command) != -1);
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTCFG, 6, 9),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 0, 0, "SaImmAttrAdminOwnerName"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 1, 1, "SaImmAttrClassName"),
...
void objectModifyTest_36(void) {
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTCFG, 6, 7),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 0, 0, "SaImmAttrAdminOwnerName"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 1, 1, "SaImmAttrClassName"),
...
void objectModifyTest_37(void) {
  ...
  /* modify an object */
  SaInt32T i32var1 = INT32VAR1, i32var11 = INT32VAR2,
     i32var111 = INT32VAR3;
  SaUint32T ui32var2 = UINT32VAR1, ui32var22 = UINT32VAR2,
      ui32var222 = UINT32VAR3;
  SaInt64T i64var3 = INT64VAR1, i64var33 = INT64VAR2,
     i64var333 = INT64VAR3;
  SaUint64T ui64var4 = UINT64VAR1, ui64var44 = UINT64VAR2,
      ui64var444 = UINT64VAR3;
  ...
  SaAnyT avar10 = {.bufferSize = sizeof(BUF1),
       .bufferAddr = const_cast<SaUint8T *>(BUF1)};
  SaAnyT avar100 = {.bufferSize = sizeof(BUF2),
        .bufferAddr = const_cast<SaUint8T *>(BUF2)};
  SaAnyT avar1000 = {.bufferSize = sizeof(BUF3),
         .bufferAddr = const_cast<SaUint8T *>(BUF3)};

  snprintf(command, MAX_DATA,
    "immcfg -t 20 -a testInt32Cfg+=%d -a testUint32Cfg+=%u"
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_add_info(&n_exp, 4, 4, "testInt32Cfg"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 5, 5, "testUint32Cfg"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 6, 6, "testInt64Cfg"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 7, 7, "testUint64Cfg"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 8, 8, "testFloatCfg"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 9, 9, "testDoubleCfg"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 10, 10, "testTimeCfg"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 11, 11, "testStringCfg"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 12, 12, "testNameCfg"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 13, 13, "testAnyCfg"),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 0, 0, "immcfg_xxx"),
    ...
    safassert(set_attr_change_scalar(
        ...
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 16, 8,
...
void objectMultiCcbTest_38(void) {
  ...
  safassert(
      immutil_saImmOmAdminOwnerSet(ownerHandle, objName, SA_IMM_ONE),
      SA_AIS_OK);
  safassert(immutil_saImmOmCcbInitialize(ownerHandle, 0, &immCcbHandle),
      SA_AIS_OK);
  safassert(immutil_saImmOmCcbObjectModify_2(
          immCcbHandle, (const SaNameT *)&objectName,
          (const SaImmAttrModificationT_2 **)attrMod1),
      SA_AIS_OK);
  safassert(immutil_saImmOmCcbObjectModify_2(
          immCcbHandle, (const SaNameT *)&objectName,
          (const SaImmAttrModificationT_2 **)attrMod2),
      SA_AIS_OK);

  safassert(immutil_saImmOmCcbApply(immCcbHandle), SA_AIS_OK);
  safassert(immutil_saImmOmCcbFinalize(immCcbHandle), SA_AIS_OK);
  safassert(
      immutil_saImmOmAdminOwnerRelease(ownerHandle, objName, SA_IMM_ONE),
      SA_AIS_OK);
  ...
  while (noOfNotifs < 2) {
    ...
    if (rec_notif_data.populated && noOfNotifs == 0) {
      ...
      safassert(set_add_info(&n_exp, 0, 0,
          ...
          SA_AIS_OK);
      safassert(
          set_add_info(&n_exp, 1, 1, "SaImmAttrClassName"),
          SA_AIS_OK);
      safassert(set_add_info(&n_exp, 2, 2, "SaImmOiCcbIdT"),
          SA_AIS_OK);
      safassert(set_add_info(&n_exp, 3, 3, "ccbLast"),
          SA_AIS_OK);
      safassert(set_add_info(&n_exp, 4, 4, "testUint32Cfg"),
          SA_AIS_OK);
      safassert(set_add_info(&n_exp, 5, 5, "testInt32Cfg"),
      ...
      safassert(set_add_info(&n_exp, 4, 4, "testUint64Cfg"),
          SA_AIS_OK);
      safassert(set_add_info(&n_exp, 5, 5, "testInt64Cfg"),
          SA_AIS_OK);
      safassert(
          set_attr_change_str(&n_exp, 0, 0, "multiCcbOwner"),
          SA_AIS_OK);
...
void objectMultiCcbTest_39(void) {
  ...
  safassert(immutil_saImmOmCcbObjectModify_2(
          immCcbHandle, (const SaNameT *)&objectName,
          (const SaImmAttrModificationT_2 **)attrMod3),
      SA_AIS_OK);
  ...
  while (noOfNotifs < 3) {
    ...
    if (rec_notif_data.populated && noOfNotifs == 0) {
      ...
      /*
       * the non-multivalue attributes testUint64Cfg and
       * testInt64Cfg was deleted (see earlier TCs), therefor
       * no attribute values exist, only additional info.
       */
      safassert(set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED,
            DNTESTCFG, 8, 7),
          SA_AIS_OK);
      ...
      safassert(set_add_info(&n_exp, 5, 5, "testInt64Cfg"),
          SA_AIS_OK);
      safassert(set_add_info(&n_exp, 6, 6, "testFloatCfg"),
          SA_AIS_OK);
      safassert(set_add_info(&n_exp, 7, 7, "testDoubleCfg"),
          SA_AIS_OK);
      safassert(
          set_attr_change_str(&n_exp, 0, 0, "multiCcbOwner"),
          SA_AIS_OK);
...
void objectDeleteTest_40(void) {
  ...
  /* delete object */
  snprintf(command, MAX_DATA, "immcfg -t 20 -d %s", DNTESTCFG);
  assert(system(command) != -1);
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_OBJECT_DELETION, DNTESTCFG, 3, 3),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 0, 0, "SaImmAttrAdminOwnerName"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 1, 1, "SaImmOiCcbIdT"),
        SA_AIS_OK);
    safassert(set_add_info(&n_exp, 2, 2, "ccbLast"), SA_AIS_OK);
    safassert(set_attr_str(&n_exp, 0, 0, "immcfg_xxx"), SA_AIS_OK);
    SaUint64T ccidVar = 3;
    safassert(set_attr_scalar(&n_exp, 1, 1, SA_NTF_VALUE_UINT64,
            &ccidVar),
        SA_AIS_OK);
    SaUint32T ccbLast = 1;
    safassert(set_attr_scalar(&n_exp, 2, 2, SA_NTF_VALUE_UINT32,
...
void objectCreateTest_3401(void) {
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_OBJECT_CREATION, DNTESTRT1, 12, 12),
        SA_AIS_OK);
    safassert(
        set_add_info(&n_exp, 0, 0, "SaImmAttrImplementerName"),
        SA_AIS_OK);
    ...
    safassert(set_attr_str(&n_exp, 0, 0, IMPLEMENTERNAME_RT),
        SA_AIS_OK);
    safassert(set_attr_str(&n_exp, 1, 1, "OsafNtfCmTestRT1"),
        SA_AIS_OK);
    safassert(set_attr_scalar(&n_exp, 2, 2, SA_NTF_VALUE_UINT32,
            &uint32Var),
        SA_AIS_OK);
...
void objectModifyTest_3402(void) {
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTRT1, 4, 4),
        SA_AIS_OK);
    safassert(
        set_add_info(&n_exp, 0, 0, "SaImmAttrImplementerName"),
        SA_AIS_OK);
    ...
    safassert(set_attr_change_str(&n_exp, 0, 0, IMPLEMENTERNAME_RT),
        SA_AIS_OK);
    safassert(set_attr_change_str(&n_exp, 1, 1, "OsafNtfCmTestRT1"),
        SA_AIS_OK);
    safassert(set_attr_change_scalar(&n_exp, 2, 2,
             SA_NTF_VALUE_UINT32, &var1),
        SA_AIS_OK);
...
void objectModifyTest_3403(void) {
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_ATTRIBUTE_CHANGED, DNTESTRT1, 3, 4),
        SA_AIS_OK);
    safassert(
        set_add_info(&n_exp, 0, 0, "SaImmAttrImplementerName"),
        SA_AIS_OK);
...
void objectDeleteTest_3404(void) {
  ...
  if (rec_notif_data.populated) {
    NotifData n_exp;
    safassert(
        set_ntf(&n_exp, SA_NTF_OBJECT_DELETION, DNTESTRT1, 0, 0),
        SA_AIS_OK);
...
void objectCreateTest_3501(void) {
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_attr_scalar(&n_exp, 8, 8, SA_NTF_VALUE_DOUBLE,
            &doubleVar),
        SA_AIS_OK);
    safassert(set_attr_extended_name(
            &n_exp, 9, 9, SA_NTF_VALUE_LDAP_NAME, &nameVar),
        SA_AIS_OK);
    safassert(set_attr_scalar(&n_exp, 10, 10, SA_NTF_VALUE_INT64,
            &timeVar),
        SA_AIS_OK);
...
void objectModifyTest_3502(void) {
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_attr_change_extended_name(
            &n_exp, 2, 2, SA_NTF_VALUE_LDAP_NAME, &var1),
        SA_AIS_OK);
    safassert(set_attr_change_buf(&n_exp, 3, 3, SA_NTF_VALUE_BINARY,
                &var2),
        SA_AIS_OK);
...
void objectModifyTest_3503(void) {
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_attr_change_str(&n_exp, 1, 1, "OsafNtfCmTestRT"),
        SA_AIS_OK);
    safassert(set_attr_change_extended_name(
            &n_exp, 2, 2, SA_NTF_VALUE_LDAP_NAME, &oldvar),
        SA_AIS_OK);
    safassert(set_attr_change_extended_name(
            &n_exp, 3, 2, SA_NTF_VALUE_LDAP_NAME, &addvar),
        SA_AIS_OK);
...
void objectCreateTest_3505(void) {
  ...
  if (rec_notif_data.populated) {
    ...
    safassert(set_attr_scalar(&n_exp, 10, 10, SA_NTF_VALUE_DOUBLE,
        ...
        SA_AIS_OK);
    safassert(set_attr_extended_name(
            &n_exp, 11, 11, SA_NTF_VALUE_LDAP_NAME, &nameVar),
        SA_AIS_OK);
    safassert(set_attr_scalar(&n_exp, 12, 12, SA_NTF_VALUE_INT64,
            &timeVar),
        SA_AIS_OK);
...
void objectModifyTest_3507(void) {
  ...
  SaNameT addvar;
  saAisNameLend((SaConstStringT)&extended_name_string_01, &addvar);
  snprintf(command, MAX_DATA, "immcfg -a testNameCfg+=%s %s",
...
__attribute__((constructor)) static void ntf_imcn_constructor(void) {
  ...
  if (stat("//tmp//ntfsv_test_classes.xml", &buf) != 0) {
    ...
    if (rc == 0) {
      ...
      if (f != NULL) {
        ...
        if (fclose(f) != 0) {
          // coverity requires checking returns.
        }
      }
    }
  }
  ...
  test_case_add(32, objectModifyTest_02,
          "runtime, attr ch, REPLACE (UINT32, FLOAT)");
  test_case_add(32, objectModifyTest_03,
          "runtime, attr ch, REPLACE (NAME, ANY)");
  test_case_add(32, objectModifyTest_04, "runtime, attr ch, ADD (INT32)");
  test_case_add(32, objectModifyTest_05,
          "runtime, attr ch, ADD (UINT32)");
  test_case_add(32, objectModifyTest_06, "runtime, attr ch, ADD (INT64)");
  test_case_add(32, objectModifyTest_07,
          "runtime, attr ch, ADD (UINT64)");
  test_case_add(32, objectModifyTest_08, "runtime, attr ch, ADD (FLOAT)");
  test_case_add(32, objectModifyTest_09,
          "runtime, attr ch, ADD (DOUBLE)");
  ...
  test_case_add(32, objectModifyTest_15,
          "runtime, attr ch, REPLACE (TIME)");
  test_case_add(32, objectModifyTest_16,
          "runtime, attr ch, ADD (STRING, INT32)");
  test_case_add(32, objectModifyTest_17,
  ...
  test_case_add(32, objectCreateTest_20,
          "CREATE, config (OsafNtfCmTestCFG) object");
  test_case_add(32, objectModifyTest_21,
          "config, attr ch, REPLACE (UINT32, FLOAT)");
  test_case_add(32, objectModifyTest_22,
          "config, attr ch, REPLACE (NAME, ANY)");
  ...
  test_case_add(32, objectModifyTest_33,
          "config, attr ch, REPLACE (STRING, DOUBLE)");
  test_case_add(32, objectModifyTest_34,
          "config, attr ch, REPLACE (TIME)");
  test_case_add(32, objectModifyTest_35,
          "config, attr ch, ADD (STRING, INT32)");
  test_case_add(32, objectModifyTest_36,
          "config, attr ch, DELETE (STRING, INT32)");
  test_case_add(32, objectModifyTest_37,
          "config, attr ch, ADD (ALL TYPES)");
  test_case_add(32, objectMultiCcbTest_38,
          "config, multiple op in ccb, 2 REPLACE");
  test_case_add(32, objectMultiCcbTest_39,
          "config, multiple op in ccb, ADD, REPLACE, DELETE");
  test_case_add(32, objectDeleteTest_40,
          "DELETE, config (OsafNtfCmTestCFG) object");

  test_suite_add(34, "CM notifications test, persistent runtime");
  test_case_add(34, objectCreateTest_3401,
          "CREATE, runtime (OsafNtfCmTestRT1) object");
  test_case_add(34, objectModifyTest_3402,
          "runtime, attr ch, REPLACE (UINT32, FLOAT)");
  test_case_add(34, objectModifyTest_3403,
          "runtime, attr ch, ADD (INT32)");
