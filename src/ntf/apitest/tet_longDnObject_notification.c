...
static int setVal(imminfo_t *info)
{
	...
	while (s) {
		if (tmp->val_is_num) {
			rc = snprintf(f, MAX_DATA, "-a %s=%s ", tmp->name,
				      tmp->val);
		} else {
			rc = snprintf(f, MAX_DATA, "-a %s='%s' ", tmp->name,
				      tmp->val);
...
// Going to change `saLogStreamFixedLogRecordSize` and
// `saLogStreamLogFileFormat` of alarm stream before testing Long DN. Then, need
...
static attrinfo_t g_config[] = {{"logMaxLogrecsize", "1024", 1}};
 ...
 */
void extCreateObjectCreateDeleteNotification(
 ...
 */
void extCreateAttributeChangeNotification(
    SaNtfHandleT ntfHandle,
 ...
 */
void extCreateStateChangeNotification(
    SaNtfHandleT ntfHandle,
 ...
 */
void extCreateSecurityAlarmNotification(
    SaNtfHandleT ntfHandle,
...
void extAdditionalInfoTest(void)
{
	...
	safassert(saNtfAlarmNotificationAllocate(
		      ...
		      (SaUint16T)(strlen(DEFAULT_ADDITIONAL_TEXT) + 1), 5, 0, 0,
...
void extObjectCreateDeleteNotificationTest(void)
{
	...
	*(myObjCrDelNotification.sourceIndicator) = SA_NTF_OBJECT_OPERATION;
	safassert(
	    saNtfNotificationSend(myObjCrDelNotification.notificationHandle),
	    SA_AIS_OK);
	...
	*(myObjCrDelNotification.sourceIndicator) = SA_NTF_MANAGEMENT_OPERATION;
	...
	if (ntfRecieved.alarmFilterHandle != 0 ||
		...
		safassert(SA_AIS_ERR_BAD_FLAGS, SA_AIS_OK);
	safassert(
	    saNtfNotificationFree(myObjCrDelNotification.notificationHandle),
	    SA_AIS_OK);
	safassert(
	    saNtfNotificationFilterFree(myFilter.notificationFilterHandle),
	    SA_AIS_OK);
...
void extAttributeChangeNotificationTest(void)
{
	...
	*(myAttrChangeNotification.sourceIndicator) = SA_NTF_OBJECT_OPERATION;
	safassert(
	    saNtfNotificationSend(myAttrChangeNotification.notificationHandle),
	    SA_AIS_OK);
	/* this notification should be caught*/
	ok_ids.ids[ok_ids.length++] =
	...
	*(myAttrChangeNotification.sourceIndicator) =
	...
	if (ntfRecieved.alarmFilterHandle != 0 ||
		...
		safassert(SA_AIS_ERR_BAD_FLAGS, SA_AIS_OK);
	safassert(
	    saNtfNotificationFree(myAttrChangeNotification.notificationHandle),
	    SA_AIS_OK);
	safassert(
	    saNtfNotificationFilterFree(myFilter.notificationFilterHandle),
	    SA_AIS_OK);
...
void extStateChangeNotificationTest(void)
{
	...
	safassert(saNtfStateChangeNotificationFilterAllocate(
		      ntfHandle, &myFilter, 1, 1, 1, 1, 2, 0),
		  SA_AIS_OK);
...
void extSecurityAlarmNotificationTest(void)
{
	...
	safassert(
	    saNtfNotificationSend(mySecAlarmNotification.notificationHandle),
	    SA_AIS_OK);
	...
	*mySecAlarmNotification.probableCause = SA_NTF_ADAPTER_ERROR;
	...
	if (ntfRecieved.alarmFilterHandle != 0 ||
		...
		safassert(SA_AIS_ERR_BAD_FLAGS, SA_AIS_OK);
	safassert(
	    saNtfNotificationFree(mySecAlarmNotification.notificationHandle),
	    SA_AIS_OK);
	safassert(
	    saNtfNotificationFilterFree(myFilter.notificationFilterHandle),
	    SA_AIS_OK);
...
longDnObject_notification_constructor(void)
{
	...
	test_case_add(
	    36, extObjectCreateDeleteNotificationTest,
