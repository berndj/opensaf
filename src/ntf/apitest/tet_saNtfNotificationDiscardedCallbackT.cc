...
static void
saNtfDiscardedCallback(SaNtfSubscriptionIdT subscriptionId,
           SaNtfNotificationTypeT notificationType,
           SaUint32T numberDiscarded,
           const SaNtfIdentifierT *discardedNotificationIdentifiers) {
  numDiscarded += numberDiscarded;
}

static SaNtfCallbacksT ntfCbTest = {saNtfNotificationCallbackT,
            saNtfDiscardedCallback};

static SaNtfAlarmNotificationT myAlarmNotification;

/* OBS! TODO: this test case is incomplete. Due to currently no limit in
 * allocating */
...
void saNtfNotificationDiscardedCallbackT_01(void) {
  ...
  SaNtfIdentifierT myNotId[NUM_NOTIF] = {0, 0};
  ...
  safassert(
      saNtfNotificationSubscribe(&myNotificationFilterHandles, my_subid),
      SA_AIS_OK);
...
saNtfNotificationDiscardedCallbackT_constructor(void) {
  test_suite_add(23, "Consumer operations - discarded notification");
  test_case_add(23, saNtfNotificationDiscardedCallbackT_01,
          "saNtfNotificationDiscardedCallbackT Ok");
}
