...
static void dummyCallback(SaNtfSubscriptionIdT subscriptionId,
...
static SaNtfCallbacksT dummyCallbacks = {dummyCallback, NULL};
...
void saNtfNotificationSend_01(void) {
  ...
  *(myAlarmNotification.probableCause) =
  ...
  rc = NtfTest::saNtfNotificationSend(myAlarmNotification.notificationHandle);
...
void saNtfNotificationSend_02(void) {
  ...
  *myNotification.sourceIndicator =
  ...
  rc = NtfTest::saNtfNotificationSend(myNotification.notificationHandle);
  free(myNotificationParams.additionalText);
...
void saNtfNotificationSend_03(void) { attr_ch_send(0, SA_AIS_OK); }

SaAisErrorT send_st_ch(
    saNotificationAllocationParamsT *myNotificationAllocationParams,
...
void sec_al_send(int wrongValueType, SaAisErrorT expectedStatus) {
  ...
  /* Event type */
  *(myNotification.notificationHeader.eventType) =
      myNotificationParams.securityAlarmEventType;
  ...
  *(myNotification.probableCause) = myNotificationParams.probableCause;
  ...
  /* Set service user */
  myNotification.serviceUser->valueType =
      myNotificationParams.serviceUser.valueType;
  myNotification.serviceUser->value.int32Val =
      myNotificationParams.serviceUser.value.int32Val;

  /* Set service provider */
  myNotification.serviceProvider->valueType =
      myNotificationParams.serviceProvider.valueType;
  myNotification.serviceProvider->value.int32Val =
      myNotificationParams.serviceProvider.value.int32Val;

  /* Set alarm detector */
  myNotification.securityAlarmDetector->valueType =
      myNotificationParams.securityAlarmDetector.valueType;
  myNotification.securityAlarmDetector->value.int32Val =
...
void saNtfNotificationSend_05(void) { sec_al_send(0, SA_AIS_OK); }
...
static SaAisErrorT send_obj_cr_del(
    saNotificationAllocationParamsT *myNotificationAllocationParams,
    saNotificationFilterAllocationParamsT *myNotificationFilterAllocationParams,
    saNotificationParamsT *myNotificationParams) {
  struct pollfd fds[1];
  ...
  safassert(NtfTest::saNtfInitialize(&ntfHandle, &dummyCallbacks, &ntfVersion),
      SA_AIS_OK);
  safassert(saNtfSelectionObjectGet(ntfHandle, &selectionObject),
      SA_AIS_OK);

  safassert(saNtfObjectCreateDeleteNotificationFilterAllocate(
          ntfHandle, &myObjCreDelFilter, 0, 0, 0, 0, 0),
      SA_AIS_OK);
  ...
  *myNotification.sourceIndicator =
  ...
  if (rc == SA_AIS_OK) {
    ...
    ret = poll(fds, 1, 10000);
    assert(ret > 0);
    safassert(NtfTest::saNtfDispatch(ntfHandle, SA_DISPATCH_ONE), SA_AIS_OK);
  }
  free(myNotificationParams->additionalText);
...
void send_mismatch_addtext(void) {
  ...
  safassert(NtfTest::saNtfInitialize(&ntfHandle, NULL, &ntfVersion), SA_AIS_OK);
  safassert(saNtfObjectCreateDeleteNotificationAllocate(
          ntfHandle, /* handle to Notification Service instance */
          &myNotification,
          /* number of correlated notifications */
...
__attribute__((constructor)) static void saNtfNotificationSend_constructor(
  ...
  test_case_add(8, saNtfNotificationSend_01,
          "saNtfNotificationSend Alarm");
