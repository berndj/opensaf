...
static void saNtfNotificationCallbackT(SaNtfSubscriptionIdT subscriptionId,
  ...
  notId = *notification->notification.alarmNotification.notificationHeader
         .notificationId;
  ...
  assert(
      saNtfNotificationFree(notification->notification.alarmNotification
              .notificationHandle) == SA_AIS_OK);
}

static void saNtfNotificationCallbackT2(SaNtfSubscriptionIdT subscriptionId,
          const SaNtfNotificationsT *notification) {
  SaNtfIdentifierT notId;
...
static SaNtfCallbacksT ntfCbTest2 = {saNtfNotificationCallbackT2, NULL};

static void *dispatchTread(void *arg) {
  /* TODO: stop this thread */
  safassert(NtfTest::saNtfDispatch(ntfHandle, SA_DISPATCH_BLOCKING),
            SA_AIS_OK);
  exit(0);
  return nullptr;
}
...
void saNtfNotificationCallbackT_01(void) {
  ...
  safassert(NtfTest::saNtfNotificationSubscribe(
      ...
      myNotificationParams.subscriptionId[0]), SA_AIS_OK);
  ...
  *(myAlarmNotification.probableCause) =
  ...
  if (verbose) {
    tet_printf(
        "%-30s cb_notId[0]: %llu, notid: %llu", __FUNCTION__,
        cb_notId[0],
...
void saNtfNotificationCallbackT_03(void) {
  ...
  safassert(NtfTest::saNtfInitialize(&ntfHandle, &ntfCbTest2, &ntfVersion),
      SA_AIS_OK);
  safassert(saNtfSelectionObjectGet(ntfHandle, &selectionObject),
      SA_AIS_OK);
  ...
  *(myAlarmNotification.probableCause) =
  ...
  safassert(NtfTest::saNtfNotificationUnsubscribe(
      myNotificationParams.subscriptionId[0]), SA_AIS_OK);
  assert(NtfTest::saNtfFinalize(ntfHandle) == SA_AIS_OK);
  /* Allocated in fillInDefaultValues() */
  free(myNotificationParams.additionalText);
  test_validate(rc, SA_AIS_OK);
}
...
saNtfNotificationCallbackT_constructor(void) {
  test_suite_add(14, "Consumer operations - callback");
  test_case_add(14, saNtfNotificationCallbackT_01,
          "saNtfNotificationCallbackT SA_DISPATCH_ONE");
  test_case_add(14, saNtfNotificationCallbackT_02,
          "saNtfNotificationCallbackT SA_DISPATCH_ALL");
  /*    test_case_add(14, saNtfNotificationCallbackT_03,
   * "saNtfNotificationCallbackT SA_DISPATCH_BLOCKING");
   */
}
