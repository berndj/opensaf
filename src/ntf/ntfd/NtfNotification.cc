 ...
 *
 * @param clientId Node-wide unique id of a client who has a matching
 * subscription.
 * @param subscriptionId
 *                 Client-wide unique id of a matching subscription.
 */
void NtfNotification::storeMatchingSubscription(
  ...
  TRACE_1(
      "Subscription %u added to list in notification %llu client %u"
      ", subscriptionList size is %u",
      subscriptionId, notificationId_, clientId,
      (unsigned int)subscriptionList.size());
}

/**
 * This method is called when sending the notification to a certain subscription
 * is confirmed.
 *
 ...
 * notification.
 * @param subscriptionId
 *                 Client-wide unique id of the subscription that matched the
 * delivered notification.
 */
void NtfNotification::notificationSentConfirmed(
    unsigned int clientId, SaNtfSubscriptionIdT subscriptionId) {
...
void NtfNotification::notificationLoggedConfirmed() { this->logged = true; }
...
/**
 * This method is called to check whether the list of matching subscriptions is
 * empty.
 *
...
/**
 * This method is called when subscriptions of a certain client should be
 * removed.
 *
 ...
 *
 * @param clientId Node-wide unique id of the client whose subscription was
 * removed.
 * @param subscriptionId
...
void NtfNotification::removeSubscription(unsigned int clientId,
  ...
  while (pos != subscriptionList.end()) {
    TRACE_1(
        "Searching for subscription for client %u, subscription %d in notification %llu",
...
void NtfNotification::syncRequest(NCS_UBAID* uba) {
  TRACE_1(
      "NtfNotification::syncRequest received"
      " in notification %llu",
      notificationId_);
  int retval = sendNewNotification(0, sendNotInfo_, uba);
  if (retval != 1) {
    LOG_ER(
        "NtfNotification::syncRequest sendNewNotification was not sent,"
        " error code is %d",
        retval);
    osafassert(0);
  }
  ...
  for (pos = subscriptionList.begin(); pos != subscriptionList.end(); pos++) {
    TRACE_1(
        "Matching subscription found, client %u,"
...
void NtfNotification::printInfo() {
  ...
  // scan until the end of the list
  while (pos != subscriptionList.end()) {
    TRACE("  clientId %u, subscriptionId: %u", pos->clientId,
          pos->subscriptionId);
    pos++;
  }
}
