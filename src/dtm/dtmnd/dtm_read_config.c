...
char *dtm_validate_listening_ip_addr(DTM_INTERNODE_CB *config)
{
	...
	if (0 == getifaddrs(&if_addrs)) {
		for (if_addr = if_addrs; if_addr != NULL;
			...
			TRACE("IP addr : %s",
			      inet_ntop(if_addr->ifa_addr->sa_family, tmp,
			...
			if (strcmp(match_ip, config->ip_addr) == 0) {
				...
				if (if_addr->ifa_addr->sa_family == AF_INET) {
					...
					TRACE(
					    "DTM:  %s scope_link : %d    IP address : %s  sa_family : %d ",
					    if_addr->ifa_name,
					    config->scope_link, match_ip,
					    config->i_addr_family);
				}
				TRACE(
				    "DTM:  %s Validate  IP address : %s  Bcast address : %s sa_family : %d ",
				    if_addr->ifa_name, match_ip,
...
int dtm_read_config(DTM_INTERNODE_CB *config, char *dtm_config_file)
{
	...
	while (fgets(line, DTM_MAX_TAG_LEN, dtm_conf_file)) {
		...
		if ((tag == 0) || (strncmp(line, "DTM_", 4) == 0)) {
			...
			if (strncmp(line, "DTM_CLUSTER_ID=",
				    strlen("DTM_CLUSTER_ID=")) == 0) {
				tag_len = strlen("DTM_CLUSTER_ID=");
				config->cluster_id = atoi(&line[tag_len]);
				if (config->cluster_id < 1) {
					LOG_ER(
					    "DTM:cluster_id must be a positive integer");
			...
			if (strncmp(line, "DTM_NODE_IP=",
				    strlen("DTM_NODE_IP=")) == 0) {
			...
			if (strncmp(line, "DTM_MCAST_ADDR=",
				    strlen("DTM_MCAST_ADDR=")) == 0) {
			...
			if (strncmp(line, "DTM_TCP_LISTENING_PORT=",
				...
				if (config->stream_port < 1) {
					LOG_ER(
					    "DTM:stream_port  must be a positive integer");
					fclose(dtm_conf_file);
					return -1;
				}
			...
			if (strncmp(line, "DTM_BCAST_FRE_MSECS=",
				...
				if (config->bcast_msg_freq < 1) {
					LOG_ER(
					    "DTM:bcast_msg_freq  must be a positive integer");
					fclose(dtm_conf_file);
					return -1;
				}
			...
			if (strncmp(line, "DTM_INI_DIS_TIMEOUT_SECS=",
				...
				if (config->initial_dis_timeout < 1) {
					LOG_ER(
					    "DTM:initial_dis_timeout must be a positive integer");
					fclose(dtm_conf_file);
					return -1;
				}
			...
			if (strncmp(line, "DTM_TCP_KEEPIDLE_TIME=",
				...
				if (config->comm_keepidle_time < 1) {
					LOG_ER(
					    "DTM:comm_keepidle_time must be a positive integer");
					fclose(dtm_conf_file);
					return -1;
				}
			...
			if (strncmp(line, "DTM_TCP_KEEPALIVE_INTVL=",
				...
				if (config->comm_keepalive_intvl < 1) {
					LOG_ER(
					    "DTM:comm_keepalive_intvl must be a positive integer");
					fclose(dtm_conf_file);
					return -1;
				}
			...
			if (strncmp(line, "DTM_TCP_KEEPALIVE_PROBES=",
				...
				if (config->comm_keepalive_probes < 1) {
					LOG_ER(
					    "DTM:comm_keepalive_probes must be a positive integer");
					fclose(dtm_conf_file);
					return -1;
				}
			...
			if (strncmp(line, "DTM_TCP_USER_TIMEOUT=",
				...
				if (config->comm_user_timeout < 0) {
					LOG_ER(
					    "DTM:comm_user_timeout must be zero or a positive integer");
					fclose(dtm_conf_file);
					return -1;
				}
			...
			if (strncmp(line, "DTM_SOCK_SND_RCV_BUF_SIZE=",
				    strlen("DTM_SOCK_SND_RCV_BUF_SIZE=")) ==
			    0) {
			...
			if (strncmp(line, "DTM_INTRANODE_MAX_PROCESSES=",
				    strlen("DTM_INTRANODE_MAX_PROCESSES=")) ==
			    0) {
				tag_len =
				    strlen("DTM_INTRANODE_MAX_PROCESSES=");
	...
	if ((config->cluster_id) == 0) {
		LOG_ER(
		    "DTM: dtm_read_config: cluster_id is missing in conf file");
		fieldmissing = 1;
	} else if ((config->node_id) == 0) {
		LOG_ER(
		    "DTM: dtm_read_config: node_id is missing in configuration");
		fieldmissing = 1;
	} else if ((config->dgram_port_sndr) == 0) {
		LOG_ER(
		    "DTM: dtm_read_config: dgram_port_sndr is missing in conf file");
		fieldmissing = 1;
	} else if ((config->dgram_port_rcvr) == 0) {
		LOG_ER(
		    "DTM: dtm_read_config: dgram_port_rcvr is missing in conf file");
		fieldmissing = 1;
	} else if ((config->stream_port) == 0) {
		LOG_ER(
		    "DTM: dtm_read_config: stream_port is missing in conf file");
		fieldmissing = 1;
		...
		LOG_ER(
		    "DTM: dtm_read_config: node_name is missing in conf file");
		fieldmissing = 1;
	}
