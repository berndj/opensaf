...
struct sockaddr *bcast_dest_address =
    reinterpret_cast<struct sockaddr *>(&bcast_dest_storage);
...
static uint32_t dgram_join_mcast_group(DTM_INTERNODE_CB *dtms_cb,
  ...
  if (mcast_receiver_addr->ai_family == AF_INET6) {
    ...
    if (setsockopt(dtms_cb->dgram_sock_rcvr, IPPROTO_IPV6, IPV6_JOIN_GROUP,
    ...
    if (setsockopt(dtms_cb->dgram_sock_rcvr, IPPROTO_IP, IP_ADD_MEMBERSHIP,
                   &join_request, sizeof(join_request)) < 0) {
...
uint32_t dtm_dgram_sendto_bcast(DTM_INTERNODE_CB *dtms_cb, const void *buffer,
  ...
  ssize_t num_bytes =
      sendto(dtms_cb->dgram_sock_sndr, buffer, buffer_len, 0,
             reinterpret_cast<struct sockaddr *>(bcast_dest_address),
             bcast_sen_addr_size);
  if (num_bytes < 0) {
    LOG_ER("DTM :sendto() failed err :%s", strerror(errno));
    TRACE_LEAVE2("rc :%d", NCSCC_RC_FAILURE);
...
static uint32_t dgram_set_mcast_ttl(DTM_INTERNODE_CB *dtms_cb, int mcast_ttl) {
  ...
  if (mcast_sender_addr->ai_family == AF_INET6) {  // v6-specific
    ...
    if (setsockopt(dtms_cb->dgram_sock_sndr, IPPROTO_IPV6, IPV6_MULTICAST_HOPS,
    ...
    if (setsockopt(dtms_cb->dgram_sock_sndr, IPPROTO_IPV6, IPV6_MULTICAST_IF,
    ...
    int mcTTL = mcast_ttl;
    if (setsockopt(dtms_cb->dgram_sock_sndr, IPPROTO_IP, IP_MULTICAST_TTL,
                   &mcTTL, sizeof(mcTTL)) < 0) {
...
uint32_t dtm_stream_nonblocking_listener(DTM_INTERNODE_CB *dtms_cb) {
  ...
  if (dtms_cb->scope_link == true) {
    ...
    rv = getaddrinfo(dtms_cb->ip_addr, local_port_str, &addr_criteria,
                     &addr_list);
...
uint32_t dtm_dgram_mcast_listener(DTM_INTERNODE_CB *dtms_cb) {
  ...
  if (dtms_cb->scope_link == true) {
    ...
    rv = getaddrinfo(dtms_cb->mcast_addr, local_port_str, &addr_criteria,
                     &addr_list);
    TRACE("DTM :mcast_addr : %s local_port_str :%s", dtms_cb->mcast_addr,
          local_port_str);
  }
  if (rv != 0) {
    LOG_ER("DTM:Unable to getaddrinfo() rtn_val :%d err :%s", rv,
...
uint32_t dtm_dgram_bcast_listener(DTM_INTERNODE_CB *dtms_cb) {
  ...
  if (dtms_cb->i_addr_family == DTM_IP_ADDR_TYPE_IPV4) {
    if ((rv = getaddrinfo(dtms_cb->bcast_addr, local_port_str, &addr_criteria,
                          &addr_list)) != 0) {
      LOG_ER("DTM:Unable to getaddrinfo() rtn_val :%d err :%s", rv,
             strerror(errno));
      TRACE_LEAVE2("rc :%d", NCSCC_RC_FAILURE);
  ...
  for (; p; p = p->ai_next) {
    ...
    int smode = 1;
    if ((setsockopt(dtms_cb->dgram_sock_rcvr, SOL_SOCKET, SO_REUSEADDR, &smode,
                    sizeof(smode)) == -1)) {
      LOG_ER("DTM : Error setsockpot: err :%s", strerror(errno));
      TRACE_LEAVE2("rc :%d", NCSCC_RC_FAILURE);
      return NCSCC_RC_FAILURE;
    ...
    if (p->ai_family == AF_INET6) {
      ...
      if (setsockopt(dtms_cb->dgram_sock_rcvr, IPPROTO_IPV6, IPV6_MULTICAST_IF,
                     &ifindex, sizeof(ifindex)) < 0) {
...
uint32_t dtm_dgram_bcast_sender(DTM_INTERNODE_CB *dtms_cb) {
  ...
  if (dtms_cb->i_addr_family == DTM_IP_ADDR_TYPE_IPV6) {
    ...
    if (setsockopt(dtms_cb->dgram_sock_sndr, SOL_SOCKET, SO_REUSEADDR, &yes,
                   sizeof yes) < 0) {
    ...
    maddr.ipv6mr_interface = if_nametoindex(dtms_cb->ifname);
    if (setsockopt(dtms_cb->dgram_sock_sndr, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP,
...
ssize_t dtm_dgram_recv_bmcast(DTM_INTERNODE_CB *dtms_cb, void *buffer,
  ...
  ssize_t rtn;
  do {
    rtn = recv(dtms_cb->dgram_sock_rcvr, buffer, buffer_len, MSG_DONTWAIT);
