				...
				DTM_INTRANODE_PID_INFO *pid_node)
{
	...
	if (NULL == hdr) {
		...
		int send_len = 0;
		send_len =
		    send(pid_node->accepted_fd, buffer, len, MSG_NOSIGNAL);
		...
		if (send_len == len) {
			...
			dtm_intranode_set_poll_fdlist(pid_node->accepted_fd,
						      POLLOUT);
			TRACE_LEAVE();
			return NCSCC_RC_SUCCESS;
		}
					     ...
					     int fd)
{
	...
	while (NULL != unsent_msg) {
		...
		int send_len = 0;
		send_len =
		    send(fd, unsent_msg->buffer, unsent_msg->len, MSG_NOSIGNAL);
