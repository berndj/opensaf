...
static uint32_t dtm_lib_msg_snd_common(uint8_t *buffer, uint32_t pid,
                                       uint16_t msg_size);
static uint32_t dtm_intranode_del_svclist_from_pid_tree(
    DTM_INTRANODE_PID_INFO *pid_node, DTM_PID_SVC_INSTALLED_INFO *del_info);

static uint32_t dtm_intranode_add_svclist_to_svc_tree(
    DTM_SVC_INSTALL_INFO *svc_node, DTM_SVC_LIST *add_list);
static uint32_t dtm_intranode_add_svclist_to_pid_tree(
    DTM_INTRANODE_PID_INFO *pid_node, DTM_PID_SVC_INSTALLED_INFO *add_info);
...
static uint32_t dtm_intranode_del_svclist_from_svc_tree(
    DTM_SVC_INSTALL_INFO *svc_node, DTM_SVC_LIST *del_list);
static uint32_t dtm_internode_del_svclist_from_svc_tree(
    DTM_INTRANODE_NODE_DB *node_info, DTM_SVC_INSTALL_INFO *svc_node,
    DTM_SVC_LIST *del_list);
static uint32_t dtm_intranode_del_subscrlist_from_subscr_tree(
    DTM_SVC_SUBSCR_INFO *subscr_node, uint64_t ref_val, uint32_t pid);
static uint32_t dtm_intranode_add_subscrlist_to_subscr_tree(
    DTM_SVC_SUBSCR_INFO *subscr_node, DTM_SUBSCRIBER_LIST *add_info);
static uint32_t dtm_add_to_node_subscr_list(
    DTM_NODE_SUBSCR_INFO *node_subscr_info);
...
static uint32_t dtm_intranode_process_svc_down_common(
    DTM_INTRANODE_NODE_DB *node_info);
static DTM_SVC_INSTALL_INFO *dtm_intranode_getnext_remote_install_svc(
    DTM_INTRANODE_NODE_DB *node_info, uint32_t server);
static uint32_t dtm_intranode_del_subscr_from_pid_info(
    DTM_INTRANODE_PID_INFO *pid_node, DTM_PID_SVC_SUSBCR_INFO *data);
static uint32_t dtm_intranode_add_subscr_to_pid_info(
    DTM_INTRANODE_PID_INFO *pid_node, DTM_PID_SVC_SUSBCR_INFO *data);

static DTM_PID_SVC_SUSBCR_INFO *dtm_intranode_get_subscr_from_pid_info(
    DTM_INTRANODE_PID_INFO *pid_node, uint64_t ref_val);
...
uint32_t dtm_intranode_process_pid_down(int fd) {
  ...
  if (nullptr == pid_node) {
    ...
    while (nullptr != subscr_data) {
      ...
      if (nullptr == subscr_tmp) {
        ...
        dtm_intranode_del_subscrlist_from_subscr_tree(
            subscr_tmp, subscr_data->ref_hdl, pid_node->pid);
      }
      free(subscr_data);
    ...
    if (nullptr != node_subscr) {
      dtm_del_from_node_subscr_list(node_subscr->process_id,
    ...
    while (nullptr != svc_list) {
      ...
      if (svc_list->install_scope > DTM_SVC_INSTALL_SCOPE_NODE)
        dtm_del_from_svc_dist_list(svc_list->server_type,
      ...
      if (nullptr != subscr_node) {
        ...
        while (nullptr != subscr_list) {
          if (subscr_list->pid != pid_node->pid) {
            ...
            if (nullptr == (snd_buf = static_cast<uint8_t *>(
                                calloc(1, DTM_LIB_DOWN_MSG_SIZE_FULL)))) {
    ...
    while (pid_node->msgs_hdr != nullptr) {
      ...
      if (tmp->buffer != nullptr) free(tmp->buffer);
...
uint32_t dtm_intranode_process_bind_msg(uint8_t *buff, int fd) {
  ...
  if (nullptr == pid_node) {
    ...
    if (nullptr ==
        (svc_install_info = static_cast<DTM_PID_SVC_INSTALLED_INFO *>(
             calloc(1, sizeof(DTM_PID_SVC_INSTALLED_INFO))))) {
    ...
    if (nullptr == svc_node) {
      if (nullptr == (svc_node = static_cast<DTM_SVC_INSTALL_INFO *>(
                          calloc(1, sizeof(DTM_SVC_INSTALL_INFO))))) {
    ...
    if (nullptr == (svc_list = static_cast<DTM_SVC_LIST *>(
...
uint32_t dtm_intranode_process_unbind_msg(uint8_t *buff, int fd) {
  ...
  if (nullptr == pid_node) {
    ...
    if (install_scope == DTM_SVC_INSTALL_SCOPE_NONE) {
      /* Message should be sent to other nodes as well */
      dtm_del_from_svc_dist_list(server_type, svc_list.server_inst_lower,
                                 pid_node->pid);
    }
    /* Only local node  */
...
uint32_t dtm_intranode_process_subscribe_msg(uint8_t *buff, int fd) {
  ...
  if (nullptr == pid_node) {
    ...
    uint32_t server_inst_lower = 0, server_inst_higher = 0;
    DTM_SVC_INSTALL_INFO *svc_node = nullptr;
    DTM_SVC_LIST *svc_list = nullptr;
    DTM_PID_SVC_SUSBCR_INFO *subscr_data = nullptr;

    DTM_SVC_SUBSCR_SCOPE subscr_scope =
        static_cast<DTM_SVC_SUBSCR_SCOPE>(ncs_decode_8bit(&data) + 1);
    uint32_t server_type = ncs_decode_32bit(&data);

    DTM_SVC_SUBSCR_INFO *subscr_node =
        dtm_intranode_get_subscr_node(server_type);

    if (nullptr == subscr_node) {
      if (nullptr == (subscr_node = static_cast<DTM_SVC_SUBSCR_INFO *>(
                          calloc(1, sizeof(DTM_SVC_SUBSCR_INFO))))) {
    ...
    if (nullptr == (subscr_info = static_cast<DTM_SUBSCRIBER_LIST *>(
                        calloc(1, sizeof(DTM_SUBSCRIBER_LIST))))) {
      TRACE("DTM :calloc failed for DTM_SUBSCRIBER_LIST");
      // cppcheck-suppress memleak
      return NCSCC_RC_FAILURE;
    }
    if (nullptr == (subscr_data = static_cast<DTM_PID_SVC_SUSBCR_INFO *>(
    ...
    TRACE_1(
        ...
        server_type, server_inst_lower, server_inst_higher, subscr_scope,
        pid_node->pid);
...
uint32_t dtm_intranode_process_unsubscribe_msg(uint8_t *buff, int fd) {
  ...
  if (nullptr == pid_node) {
    ...
    if (nullptr == data_subscr) {
      ...
      TRACE_1("DTM: INTRA: svc unsubscribe type=%d, ref_hdl =%" PRIu64
              " pid=%d",
    ...
    if (nullptr == subscr_node) {
      ...
      /* Now Delete the entry */
      dtm_intranode_del_subscrlist_from_subscr_tree(subscr_node, ref_val,
                                                    pid_node->pid);
    }
  }
  TRACE_LEAVE();
...
uint32_t dtm_intranode_process_node_subscribe_msg(uint8_t *buff, int fd) {
  ...
  if (nullptr == pid_node) {
    ...
    if (nullptr == (node_subscr_info = static_cast<DTM_NODE_SUBSCR_INFO *>(
    ...
    while (nullptr != node_db) {
      ...
      if (nullptr == (buffer = static_cast<uint8_t *>(
        ...
        dtm_lib_msg_snd_common(buffer, node_subscr_info->process_id,
...
*********************************************************/
uint32_t dtm_intranode_add_svclist_to_pid_tree(
    DTM_INTRANODE_PID_INFO *pid_node, DTM_PID_SVC_INSTALLED_INFO *add_info) {
  ...
  while (nullptr != (svc_info = dtm_intranode_getnext_remote_install_svc(
    ...
    while (nullptr != svc_list) {
      ...
      if (nullptr != local_svc_info) {
        ...
        dtm_intranode_del_svclist_from_svc_tree(local_svc_info,
                                                &local_svc_list);
      }
...
uint32_t dtm_intranode_process_node_up(NODE_ID node_id, char *node_name,
  ...
  TRACE_ENTER();
  if (nullptr == (node_db_info = static_cast<DTM_INTRANODE_NODE_DB *>(
                      calloc(1, sizeof(DTM_INTRANODE_NODE_DB))))) {
    return NCSCC_RC_FAILURE;
  }
...
uint32_t dtm_intranode_process_node_down(NODE_ID node_id) {
  ...
  while (nullptr != node_subscr_info) {
    uint8_t *snd_buf = nullptr;
    if (nullptr == (snd_buf = static_cast<uint8_t *>(
                        calloc(1, DTM_LIB_NODE_DOWN_MSG_SIZE_FULL)))) {
...
static DTM_SVC_INSTALL_INFO *dtm_intranode_getnext_remote_install_svc(
    DTM_INTRANODE_NODE_DB *node_info, uint32_t server) {
  DTM_SVC_INSTALL_INFO *node = reinterpret_cast<DTM_SVC_INSTALL_INFO *>(
      ncs_patricia_tree_getnext(&node_info->dtm_rem_node_svc_tree,
                                reinterpret_cast<uint8_t *>(&server)));

  /* Adjust the pointer */
  if (nullptr != node) {
    node = reinterpret_cast<DTM_SVC_INSTALL_INFO *>(
...
*********************************************************/
static DTM_SVC_INSTALL_INFO *dtm_intranode_get_remote_install_svc(
    DTM_INTRANODE_NODE_DB *node_info, uint32_t server) {
  DTM_SVC_INSTALL_INFO *node = reinterpret_cast<DTM_SVC_INSTALL_INFO *>(
      ncs_patricia_tree_get(&node_info->dtm_rem_node_svc_tree,
                            reinterpret_cast<uint8_t *>(&server)));
  ...
  if (nullptr == node_info) {
    ...
    while (0 != num_of_elements) {
      ...
      DTM_SVC_INSTALL_INFO *svc_info =
          dtm_intranode_get_remote_install_svc(node_info, server_type);

      if (nullptr == svc_info) {
        if (nullptr == (svc_info = static_cast<DTM_SVC_INSTALL_INFO *>(
                            calloc(1, sizeof(DTM_SVC_INSTALL_INFO))))) {
          TRACE(
      ...
      if (nullptr == local_svc_info) {
        if (nullptr == (local_svc_info = static_cast<DTM_SVC_INSTALL_INFO *>(
                            calloc(1, sizeof(DTM_SVC_INSTALL_INFO))))) {
          TRACE("DTM :calloc failed for DTM_SVC_INSTALL_INFO");
          return NCSCC_RC_FAILURE;
        }
      ...
      if (nullptr == (svc_list_local = static_cast<DTM_SVC_LIST *>(
                          calloc(1, sizeof(DTM_SVC_LIST))))) {
        // cppcheck-suppress memleak
        return NCSCC_RC_FAILURE;
      }
...
DTM_INTRANODE_PID_INFO *dtm_intranode_get_pid_info_using_fd(int fd) {
  DTM_INTRANODE_PID_INFO *node = reinterpret_cast<DTM_INTRANODE_PID_INFO *>(
      ncs_patricia_tree_get(&dtm_intranode_cb->dtm_intranode_fd_list,
                            reinterpret_cast<uint8_t *>(&fd)));
  /* Adjust the pointer */
  if (nullptr != node) {
...
*********************************************************/
static DTM_SVC_SUBSCR_INFO *dtm_intranode_get_subscr_node(
    uint32_t server_type) {
  DTM_SVC_SUBSCR_INFO *node = reinterpret_cast<DTM_SVC_SUBSCR_INFO *>(
      ncs_patricia_tree_get(&dtm_intranode_cb->dtm_svc_subscr_list,
                            reinterpret_cast<uint8_t *>(&server_type)));
  /* Adjust the pointer */
  if (nullptr != node) {
    node = reinterpret_cast<DTM_SVC_SUBSCR_INFO *>(
        reinterpret_cast<char *>(node) -
...
static DTM_SVC_INSTALL_INFO *dtm_intranode_get_svc_node(uint32_t server_type) {
  DTM_SVC_INSTALL_INFO *node = reinterpret_cast<DTM_SVC_INSTALL_INFO *>(
      ncs_patricia_tree_get(&dtm_intranode_cb->dtm_svc_install_list,
                            reinterpret_cast<uint8_t *>(&server_type)));
  /* Adjust the pointer */
  if (nullptr != node) {
    node = reinterpret_cast<DTM_SVC_INSTALL_INFO *>(
