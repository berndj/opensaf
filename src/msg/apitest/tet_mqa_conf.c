		 ...
		 SaMsgQueueOpenFlagsT op_flgs)
{
	...
	if (cr_attr) {
		...
		if (cr_attr->creationFlags < 2)
			m_TET_MQSV_PRINTF(" creation flags  : %s\n",
					  saMsgQueueCreationFlags_string
	...
	if (op_flgs >= 0 && op_flgs < 8)
		m_TET_MQSV_PRINTF(" open flags      : %s",
				  saMsgQueueOpenFlags_string[op_flgs]);
	else
...
void groupTrackInfo(SaMsgQueueGroupNotificationBufferT *buffer)
{
	...
	if (!buffer->notification)
		...
		for (i = 0; i < buffer->numberOfItems; i++, notification++) {
			...
			m_TET_MQSV_PRINTF(
			    " change          : %s\n",
			    saMsgQueueGroupChanges_string[notification
							      ->change]);
		}
		free(q_name);
	}
...
int tet_test_msgQueueOpen(int i, MQSV_CONFIG_FLAG flg)
{
	...
	print_qinfo(API_Mqsv_QueueOpen[i].queueName,
		    API_Mqsv_QueueOpen[i].creationAttributes,
...
int tet_test_red_msgQueueOpen(int i, MQSV_CONFIG_FLAG flg)
{
	...
	do {
		rc = saMsgQueueOpen(*API_Mqsv_QueueOpen[i].msgHandle,
				    API_Mqsv_QueueOpen[i].queueName,
				    API_Mqsv_QueueOpen[i].creationAttributes,
				    API_Mqsv_QueueOpen[i].openFlags,
				    API_Mqsv_QueueOpen[i].timeout,
				    API_Mqsv_QueueOpen[i].queueHandle);
...
int tet_test_msgQueueOpenAsync(int i, MQSV_CONFIG_FLAG flg)
{
	...
	print_qinfo(API_Mqsv_QueueOpenAsync[i].queueName,
		    API_Mqsv_QueueOpenAsync[i].creationAttributes,
		    API_Mqsv_QueueOpenAsync[i].openFlags);
...
int tet_test_red_msgQueueOpenAsync(int i, MQSV_CONFIG_FLAG flg)
{
	...
	do {
		rc = saMsgQueueOpenAsync(
		    *API_Mqsv_QueueOpenAsync[i].msgHandle,
		    API_Mqsv_QueueOpenAsync[i].invocation,
		    API_Mqsv_QueueOpenAsync[i].queueName,
		    API_Mqsv_QueueOpenAsync[i].creationAttributes,
		    API_Mqsv_QueueOpenAsync[i].openFlags);
...
int tet_test_msgQueueStatusGet(int i, MQSV_CONFIG_FLAG flg)
{
	...
	if (API_Mqsv_QueueStatusGet[i].queueName)
		m_TET_MQSV_PRINTF("\n Queue Name      : %s",
				  API_Mqsv_QueueStatusGet[i].queueName->value);
	else
		m_TET_MQSV_PRINTF("\n Queue Name      : NULL");
...
int tet_test_red_msgQueueStatusGet(int i, MQSV_CONFIG_FLAG flg)
{
	...
	do {
		rc =
		    saMsgQueueStatusGet(*API_Mqsv_QueueStatusGet[i].msgHandle,
					API_Mqsv_QueueStatusGet[i].queueName,
					API_Mqsv_QueueStatusGet[i].queueStatus);
...
int tet_test_msgQueueUnlink(int i, MQSV_CONFIG_FLAG flg)
{
	...
	if (flg != TEST_CLEANUP_MODE ||
		...
		if (API_Mqsv_QueueUnlink[i].queueName)
			m_TET_MQSV_PRINTF(
			    "\n Queue Name      : %s",
			    API_Mqsv_QueueUnlink[i].queueName->value);
		else
			m_TET_MQSV_PRINTF("\n Queue Name      : NULL");
	}
...
int tet_test_msgGroupCreate(int i, MQSV_CONFIG_FLAG flg)
{
	...
	if (API_Mqsv_GroupCreate[i].queueGroupName)
		m_TET_MQSV_PRINTF(
		    "\n Group Name      : %s",
		    API_Mqsv_GroupCreate[i].queueGroupName->value);
	else
		m_TET_MQSV_PRINTF("\n Group Name      : NULL");
	...
	if (API_Mqsv_GroupCreate[i].queueGroupPolicy > 0 &&
		...
		m_TET_MQSV_PRINTF(
		    "\n Group Policy    : %s",
		    saMsgQueueGroupPolicy_string[API_Mqsv_GroupCreate[i]
						     .queueGroupPolicy]);
	else
		m_TET_MQSV_PRINTF("\n Group Policy    : %d",
				  API_Mqsv_GroupCreate[i].queueGroupPolicy);
...
int tet_test_red_msgGroupCreate(int i, MQSV_CONFIG_FLAG flg)
{
	...
	do {
		rc = saMsgQueueGroupCreate(
		    *API_Mqsv_GroupCreate[i].msgHandle,
		    API_Mqsv_GroupCreate[i].queueGroupName,
		    API_Mqsv_GroupCreate[i].queueGroupPolicy);
...
int tet_test_msgGroupInsert(int i, MQSV_CONFIG_FLAG flg)
{
	...
	if (API_Mqsv_GroupInsert[i].queueGroupName)
		m_TET_MQSV_PRINTF(
		    " Group Name      : %s",
		    API_Mqsv_GroupInsert[i].queueGroupName->value);
	else
...
int tet_test_red_msgGroupInsert(int i, MQSV_CONFIG_FLAG flg)
{
	...
	do {
		rc = saMsgQueueGroupInsert(
		    *API_Mqsv_GroupInsert[i].msgHandle,
		    API_Mqsv_GroupInsert[i].queueGroupName,
		    API_Mqsv_GroupInsert[i].queueName);
...
int tet_test_msgGroupRemove(int i, MQSV_CONFIG_FLAG flg)
{
	...
	if (API_Mqsv_GroupRemove[i].queueGroupName)
		m_TET_MQSV_PRINTF(
		    " Group Name      : %s",
		    API_Mqsv_GroupRemove[i].queueGroupName->value);
	else
		m_TET_MQSV_PRINTF(" Group Name      : NULL");
...
int tet_test_red_msgGroupRemove(int i, MQSV_CONFIG_FLAG flg)
{
	...
	do {
		rc = saMsgQueueGroupRemove(
		    *API_Mqsv_GroupRemove[i].msgHandle,
		    API_Mqsv_GroupRemove[i].queueGroupName,
		    API_Mqsv_GroupRemove[i].queueName);
...
int tet_test_msgGroupDelete(int i, MQSV_CONFIG_FLAG flg)
{
	...
	if (flg != TEST_CLEANUP_MODE ||
		...
		if (API_Mqsv_GroupDelete[i].queueGroupName)
			m_TET_MQSV_PRINTF(
			    " Group Name      : %s",
			    API_Mqsv_GroupDelete[i].queueGroupName->value);
		else
			m_TET_MQSV_PRINTF(" Group Name      : NULL");
	}
...
int tet_test_msgGroupTrack(int i, MQSV_CONFIG_FLAG flg)
{
	...
	m_TET_MQSV_PRINTF(
	    " Track Flag      : %s",
	    saMsgGroupTrackFlags_string[API_Mqsv_GroupTrack[i].trackFlags]);

	if ((API_Mqsv_GroupTrack[i].trackFlags & SA_TRACK_CURRENT) &&
	    (rc == SA_AIS_OK)) {
		if (API_Mqsv_GroupTrack[i].buffer) {
			m_TET_MQSV_PRINTF("\n Notification Buffer \n");
			m_TET_MQSV_PRINTF(" No of Items     : %u\n",
					  numOfItems);
			if (notification)
				m_TET_MQSV_PRINTF(" Notification    : %p",
						  notification);
			else
				m_TET_MQSV_PRINTF(" Notification    : NULL");
		} else
...
int tet_test_msgGroupTrackStop(int i, MQSV_CONFIG_FLAG flg)
{
	...
	if (flg != TEST_CLEANUP_MODE ||
		...
		if (API_Mqsv_GroupTrackStop[i].queueGroupName)
			m_TET_MQSV_PRINTF(
			    " Group Name      : %s",
			    API_Mqsv_GroupTrackStop[i].queueGroupName->value);
		else
			m_TET_MQSV_PRINTF(" Group Name      : NULL");
	}
...
int tet_test_msgMessageSendAsync(int i, MQSV_CONFIG_FLAG flg)
{
	...
	if (API_Mqsv_MessageSendAsync[i].ackFlags >= 0 &&
		...
		m_TET_MQSV_PRINTF(
		    " AckFlags        : %s",
		    saMsgAckFlags_string[API_Mqsv_MessageSendAsync[i]
					     .ackFlags]);
	else
		m_TET_MQSV_PRINTF(" AckFlags        : %u",
				  API_Mqsv_MessageSendAsync[i].ackFlags);
...
int tet_test_red_msgMessageSendAsync(int i, MQSV_CONFIG_FLAG flg)
{
	...
	do {
		rc = saMsgMessageSendAsync(
		    *API_Mqsv_MessageSendAsync[i].msgHandle,
		    API_Mqsv_MessageSendAsync[i].invocation,
		    API_Mqsv_MessageSendAsync[i].destination,
		    API_Mqsv_MessageSendAsync[i].message,
		    API_Mqsv_MessageSendAsync[i].ackFlags);
...
int tet_test_red_msgMessageGet(int i, MQSV_CONFIG_FLAG flg)
{
	...
	do {
		rc = saMsgMessageGet(*API_Mqsv_MessageGet[i].queueHandle,
				     API_Mqsv_MessageGet[i].message,
				     API_Mqsv_MessageGet[i].sendTime,
				     API_Mqsv_MessageGet[i].senderId,
				     API_Mqsv_MessageGet[i].timeout);
...
int tet_test_msgMessageSendReceive(int i, MQSV_CONFIG_FLAG flg)
{
	...
	if (API_Mqsv_MessageSendReceive[i].destination)
		m_TET_MQSV_PRINTF(
		    "\n Destination     : %s",
		    API_Mqsv_MessageSendReceive[i].destination->value);
	else
		m_TET_MQSV_PRINTF("\n Destination     : NULL");
...
int tet_test_msgMessageReplyAsync(int i, MQSV_CONFIG_FLAG flg)
{
	...
	if (API_Mqsv_MessageReplyAsync[i].ackFlags >= 0 &&
		...
		m_TET_MQSV_PRINTF(
		    " AckFlags        : %s",
		    saMsgAckFlags_string[API_Mqsv_MessageReplyAsync[i]
					     .ackFlags]);
	else
		m_TET_MQSV_PRINTF(" AckFlags        : %u",
				  API_Mqsv_MessageReplyAsync[i].ackFlags);
...
int tet_test_red_msgMessageReplyAsync(int i, MQSV_CONFIG_FLAG flg)
{
	...
	do {
		rc = saMsgMessageReplyAsync(
		    *API_Mqsv_MessageReplyAsync[i].msgHandle,
		    API_Mqsv_MessageReplyAsync[i].invocation,
		    API_Mqsv_MessageReplyAsync[i].replyMessage,
		    API_Mqsv_MessageReplyAsync[i].senderId,
		    API_Mqsv_MessageReplyAsync[i].ackFlags);
...
void mqsv_create_sendrecvthread(MSG_MESSAGE_SEND_RECV_TC_TYPE *tc)
{
	...
	rc = m_NCS_TASK_CREATE((NCS_OS_CB)mqsv_sendrecv, (NCSCONTEXT)tc,
			       "mqa_sendrcv_test", 5, 8000, &thread_handle);
...
void mqsv_createcancelthread(SaMsgQueueHandleT *queueHandle)
{
	...
	rc = m_NCS_TASK_CREATE((NCS_OS_CB)mqsv_cancel_msg,
			       (NCSCONTEXT)queueHandle, "mqa_cancelest", 5,
			       8000, &thread_handle);
	if (rc != NCSCC_RC_SUCCESS) {
		m_TET_MQSV_PRINTF(" Failed to create thread\n");
		return;
...
void mqsv_createthread(SaMsgHandleT *msgHandle)
{
	...
	rc = m_NCS_TASK_CREATE((NCS_OS_CB)mqsv_selection_thread_blocking,
			       (NCSCONTEXT)msgHandle, "mqa_asynctest_blocking",
...
void mqsv_createthread_one(SaMsgHandleT *msgHandle)
{
	...
	rc = m_NCS_TASK_CREATE((NCS_OS_CB)mqsv_selection_thread_one,
			       (NCSCONTEXT)msgHandle, "mqa_asynctest_one", 5,
			       8000, &thread_handle);
	if (rc != NCSCC_RC_SUCCESS) {
		m_TET_MQSV_PRINTF(" Failed to create thread\n");
		return;
...
void mqsv_createthread_all(SaMsgHandleT *msgHandle)
{
	...
	rc = m_NCS_TASK_CREATE((NCS_OS_CB)mqsv_selection_thread_all,
			       (NCSCONTEXT)msgHandle, "mqa_asynctest_all", 5,
			       8000, &thread_handle);
	if (rc != NCSCC_RC_SUCCESS) {
		m_TET_MQSV_PRINTF(" Failed to create thread\n");
		return;
...
void mqsv_createthread_all_loop()
{
	...
	rc = m_NCS_TASK_CREATE((NCS_OS_CB)mqsv_selection_thread_all_loop,
			       (NCSCONTEXT)NULL, "mqa_asynctest_loop", 5, 8000,
			       &thread_handle);
	if (rc != NCSCC_RC_SUCCESS) {
		m_TET_MQSV_PRINTF(" Failed to create thread\n");
		return;
...
void mqsv_restore_params(MQSV_RESTORE_OPT opt)
{
	...
	case MSG_RESTORE_INIT_BAD_VERSION_T:
		mqsv_fill_msg_version(&gl_mqa_env.inv_params.inv_version, 'C',
				      0, 1);
		break;

	case MSG_RESTORE_INIT_BAD_REL_CODE_T:
		mqsv_fill_msg_version(
		    &gl_mqa_env.inv_params.inv_ver_bad_rel_code, '\0', 1, 0);
		break;

	case MSG_RESTORE_INIT_BAD_MAJOR_VER_T:
		mqsv_fill_msg_version(&gl_mqa_env.inv_params.inv_ver_not_supp,
				      'B', 3, 0);
		break;
	...
	case MSG_RESTORE_GROUP_TRACK_CURRENT_T:
		mqsv_fill_grp_notif_buffer(&gl_mqa_env.buffer_null_notif, 3,
					   NULL);
		break;

	case MSG_RESTORE_GROUP_TRACK_CURRENT_NON_NULL_NOTIF_T:
		memset(&gl_mqa_env.buffer_non_null_notif, '\0',
	...
	case MSG_RESTORE_MESSAGE_GET_NULL_SNDR_NAME_T:
		gl_mqa_env.sender_id = 0;
		gl_mqa_env.send_time = 0;
		memset(&gl_mqa_env.rcv_msg_null_sndr_name, '\0',
		       sizeof(SaMsgMessageT));
		mqsv_fill_rcv_message(&gl_mqa_env.rcv_msg_null_sndr_name, NULL,
				      0, NULL);
	...
	case MSG_RESTORE_MESSAGE_GET_ERR_NO_SPACE_T:
		...
		gl_mqa_env.no_space_rcv_msg.senderName =
		    &gl_mqa_env.rcv_sender_name;
	...
	case MSG_RESTORE_MESSAGE_SEND_RECV_SUCCESS_MSG2_T:
		gl_mqa_env.reply_send_time = 0;
		memset(&gl_mqa_env.reply_msg_null_sndr_name, '\0',
		       sizeof(SaMsgMessageT));
		mqsv_fill_rcv_message(&gl_mqa_env.reply_msg_null_sndr_name,
				      NULL, 0, NULL);
	...
	case MSG_RESTORE_MESSAGE_SEND_RECV_ERR_NO_SPACE_T:
		...
		gl_mqa_env.no_space_reply_msg.senderName =
		    &gl_mqa_env.rcv_sender_name;
	}
}
...
void mqsv_clean_clbk_params()
{
	...
	memset(&gl_mqa_env.track_clbk_notif, '\0',
	       sizeof(SaMsgQueueGroupNotificationBufferT));
}
...
void mqsv_init_red_cleanup(MQSV_INIT_CLEANUP_OPT opt)
{
	...
	case MSG_CLEAN_INIT_NULL_CBKS_T:
	case MSG_CLEAN_INIT_NULL_CLBK_PARAM_T:
		result = tet_test_red_msgFinalize(MSG_FINALIZE_SUCCESS_T,
						  TEST_CLEANUP_MODE);
		break;

	case MSG_CLEAN_INIT_SUCCESS_HDL2_T:
		result = tet_test_red_msgFinalize(MSG_FINALIZE_SUCCESS_HDL2_T,
						  TEST_CLEANUP_MODE);
		break;

	case MSG_CLEAN_INIT_NULL_CBKS2_T:
		result = tet_test_red_msgFinalize(MSG_FINALIZE_SUCCESS_HDL3_T,
						  TEST_CLEANUP_MODE);
		break;

	case MSG_CLEAN_INIT_NULL_RCV_CBK_T:
		result = tet_test_red_msgFinalize(MSG_FINALIZE_SUCCESS_HDL4_T,
						  TEST_CLEANUP_MODE);
		break;
	}
...
void mqsv_q_red_cleanup(MQSV_Q_CLEANUP_OPT opt)
{
	...
	case MSG_CLEAN_QUEUE_OPEN_EMPTY_CREATE_T:
	case MSG_CLEAN_QUEUE_OPEN_PERS_RECV_CLBK_SUCCESS_T:
		result = tet_test_red_msgQueueUnlink(
		    MSG_QUEUE_UNLINK_SUCCESS_Q1_T, TEST_CLEANUP_MODE);
		break;
	...
	case MSG_CLEAN_QUEUE_OPEN_NON_PERS_RECV_CLBK_SUCCESS_T:
		result = tet_test_red_msgQueueUnlink(
		    MSG_QUEUE_UNLINK_SUCCESS_Q2_T, TEST_CLEANUP_MODE);
		break;

	case MSG_CLEAN_QUEUE_OPEN_PERS_RECV_CLBK2_T:
		result = tet_test_red_msgQueueUnlink(
		    MSG_QUEUE_UNLINK_SUCCESS_Q3_T, TEST_CLEANUP_MODE);
		break;
	...
	case MSG_CLEAN_QUEUE_OPEN_NPERS_RECV_CLBK2_T:
		result = tet_test_red_msgQueueUnlink(
		    MSG_QUEUE_UNLINK_SUCCESS_Q4_T, TEST_CLEANUP_MODE);
		break;

	case MSG_CLEAN_QUEUE_OPEN_ASYNC_ZERO_SIZE_T:
	case MSG_CLEAN_QUEUE_OPEN_ZERO_SIZE_T:
		result = tet_test_red_msgQueueUnlink(
		    MSG_QUEUE_UNLINK_SUCCESS_Q5_T, TEST_CLEANUP_MODE);
		break;

	case MSG_CLEAN_QUEUE_OPEN_AFTER_FINALIZE:
		result = tet_test_red_msgInitialize(MSG_INIT_SUCCESS_T,
						    TEST_CLEANUP_MODE);
		result = tet_test_red_msgQueueUnlink(
		    MSG_QUEUE_UNLINK_SUCCESS_Q1_T, TEST_CLEANUP_MODE);
		result = tet_test_red_msgFinalize(MSG_FINALIZE_SUCCESS_T,
...
void mqsv_q_grp_red_cleanup(MQSV_Q_GRP_CLEANUP_OPT opt)
{
	...
	case MSG_CLEAN_GROUP_CREATE_SUCCESS_GR2_T:
	case MSG_CLEAN_GROUP_CREATE_BROADCAST2_T:
		result = tet_test_red_msgGroupDelete(
		    MSG_GROUP_DELETE_SUCCESS_GR2_T, TEST_CLEANUP_MODE);
		break;
	}
