...
static uint32_t mqa_create(NCS_LIB_CREATE *create_info) {
  ...
  if (create_info == NULL) return NCSCC_RC_FAILURE;
  ...
  if (!(cb->agent_handle_id = ncshm_create_hdl(cb->pool_id, NCS_SERVICE_ID_MQA,
                                               (NCSCONTEXT)cb))) {
  ...
  if ((rc = m_NCS_LOCK_INIT(&cb->cb_lock)) != NCSCC_RC_SUCCESS) {
    TRACE_2("Cotnrol Block Lock Initialisation Failed eith return value %d",
            rc);
    goto error1;
  }
...
bool mqa_is_track_enabled(MQA_CB *mqa_cb, SaNameT *queueGroupName) {
  ...
  while ((client_info = (MQA_CLIENT_INFO *)ncs_patricia_tree_getnext(
              &mqa_cb->mqa_client_tree, (uint8_t *const)temp_ptr))) {
    track_info =
        mqa_track_tree_find_and_add(client_info, queueGroupName, false);
    if (!track_info) {
      temp_hdl = client_info->msgHandle;
      temp_ptr = &temp_hdl;
      continue;
...
MQA_CLIENT_INFO *mqa_client_tree_find_and_add(MQA_CB *mqa_cb,
  ...
  if (flag == true) {
    ...
    if (!client_info) {
      ...
      if ((rc = ncs_patricia_tree_add(&mqa_cb->mqa_client_tree,
                                      &client_info->patnode)) !=
          NCSCC_RC_SUCCESS) {
      ...
      if (ncs_patricia_tree_init(&client_info->mqa_track_tree, &param) !=
          NCSCC_RC_SUCCESS) {
...
******************************************************************************/
MQA_CLIENT_INFO *mqa_client_tree_find_next(MQA_CB *mqa_cb,
                                           SaMsgHandleT hdl_id) {
  MQA_CLIENT_INFO *client_info = NULL;
...
MQA_QUEUE_INFO *mqa_queue_tree_find_and_add(MQA_CB *mqa_cb,
  ...
  queue_info = (MQA_QUEUE_INFO *)ncs_patricia_tree_get(&mqa_cb->mqa_queue_tree,
                                                       (uint8_t *)&hdl_id);
  ...
  if (flag == true) {
    ...
    if (!queue_info) {
      ...
      if ((rc = ncs_patricia_tree_add(&mqa_cb->mqa_queue_tree,
                                      &queue_info->patnode)) !=
          NCSCC_RC_SUCCESS) {
