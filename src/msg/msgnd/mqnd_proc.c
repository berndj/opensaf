...
static uint32_t mqnd_existing_queue_open(MQND_CB *cb, MQSV_SEND_INFO *sinfo,
					 MQP_OPEN_REQ *open,
...
uint32_t mqnd_evt_proc_mqp_qtransfer_complete(MQND_CB *cb, MQSV_EVT *req)
{
	...
	/*Timer to keep track of Q which is being transfered can be stopped as
	 * we got the response */
	...
	if (req->msg.mqp_req.info.transferComplete.error == NCSCC_RC_SUCCESS) {
		...
		/*Q Timer expired on N1 so Trans Complete req with error code
		   set was sent either from N1 or Timer Expiry routine running
		   on N2 */
		...
		opr.info.msg.req.info.reg.queue.owner =
		    MQSV_QUEUE_OWN_STATE_ORPHAN;
		opr.info.msg.req.info.reg.queue.retentionTime =
		    qnode->qinfo.queueStatus.retentionTime;
		opr.info.msg.req.info.reg.queue.status =
		    qnode->qinfo.sendingState;
		opr.info.msg.req.info.reg.queue.creationFlags =
		...
		if (rc == SA_AIS_ERR_TIMEOUT) {
			/*This is the case when REG request is assumed to be
			   lost or not proceesed at MQD becoz of failover.So to
			   compensate for the lost request we send an async REG
			   request to MQD */
			/*Request the ASAPi (at MQD) for queue REG */
			TRACE_1(
			    "UNLINK TIMEOUT:ASYNC REG CHANGE IN STATUS TO ORPHAN QUEUE");
			memset(&opr, 0, sizeof(ASAPi_OPR_INFO));
			opr.type = ASAPi_OPR_MSG;
			opr.info.msg.opr = ASAPi_MSG_SEND;
			...
			opr.info.msg.req.msgtype = ASAPi_MSG_REG;
			opr.info.msg.req.info.reg.objtype = ASAPi_OBJ_QUEUE;
			opr.info.msg.req.info.reg.queue.name =
			    qnode->qinfo.queueName;
			opr.info.msg.req.info.reg.queue.addr = cb->my_dest;
			opr.info.msg.req.info.reg.queue.hdl =
			...
			memcpy(opr.info.msg.req.info.reg.queue.size,
			       qnode->qinfo.size,
			       sizeof(SaSizeT) *
				   (SA_MSG_MESSAGE_LOWEST_PRIORITY + 1));
			rc = asapi_opr_hdlr(&opr); /*May be insert a log */
		} else {
			if (rc != SA_AIS_OK || (!opr.info.msg.resp) ||
			    (opr.info.msg.resp->info.rresp.err.flag)) {
				LOG_ER(
				    "Asapi Opr handler for Queue Register Failed with return code %u",
				    rc);
				return rc;
			}
		}
		...
		if (rc == SA_AIS_OK) {
			...
			memset(&queue_ckpt_node, 0,
			       sizeof(MQND_QUEUE_CKPT_INFO));
			mqnd_cpy_qnodeinfo_to_ckptinfo(cb, qnode,
						       &queue_ckpt_node);

			rc = mqnd_ckpt_queue_info_write(
			    cb, &queue_ckpt_node, qnode->qinfo.shm_queue_index);
			if (rc != SA_AIS_OK) {
				LOG_ER("Ckpt Overwrite Failed");
				return rc;
			}
			/* Immsv Runtime Object Create  */
			error = mqnd_create_runtime_MsgQobject(
			    (char *)qnode->qinfo.queueName.value,
			    qnode->qinfo.creationTime, qnode, cb->immOiHandle);

			if (error != SA_AIS_OK) {
				LOG_ER(
				    "Create MsgQobject %s Failed with error: %u",
				    qnode->qinfo.queueName.value, error);
				return NCSCC_RC_FAILURE;
			}

			error = mqnd_create_runtime_MsgQPriorityobject(
			    (char *)qnode->qinfo.queueName.value, qnode,
			    cb->immOiHandle);

			if (error != SA_AIS_OK) {
				LOG_ER(
				    "Create MsgQPriorityobject %s Failed : %u",
				    qnode->qinfo.queueName.value, error);
				return NCSCC_RC_FAILURE;
			}
		}
...
uint32_t mqnd_evt_proc_mqp_qtransfer(MQND_CB *cb, MQSV_EVT *req)
{
	...
	if (qnode->qinfo.owner_flag != MQSV_QUEUE_OWN_STATE_ORPHAN) {
		LOG_ER("%s:%u: ERR_BUSY: MSGQ is Busy (Opened by someone)",
		       __FILE__, __LINE__);
		err = SA_AIS_ERR_BUSY;
		goto send_rsp;
	}
	...
	if (m_NCS_OS_POSIX_MQ(&qreq) != NCSCC_RC_SUCCESS) {
		LOG_ER(
		    "ERR_RESOURCES: Unable to get the queue attributes from the queue");
		err = SA_AIS_ERR_NO_RESOURCES;
		goto send_rsp;
	}

	num_messages = qreq.info.attr.o_attr.mq_curmsgs;
	transfer_rsp.msg.mqp_rsp.info.transferRsp.creationTime =
	    qnode->qinfo.creationTime;
	if ((qreq.info.attr.o_attr.mq_curmsgs == 0) ||
	    (req->msg.mqp_req.info.transferReq.empty_queue)) {
		...
		mqsv_message =
		    m_MMGR_ALLOC_MQND_DEFAULT(qreq.info.attr.o_attr.mq_msgsize);
		...
		transfer_rsp.msg.mqp_rsp.info.transferRsp.mqsv_message =
		    mqsv_message;
	}
	...
	while (num_messages) {
		...
		if (mqnd_mq_rcv(qhdl) != NCSCC_RC_SUCCESS) {
			TRACE_2(
			    "ERR_RESOURCES: Unable to receive the message from the Queue");
			err = SA_AIS_ERR_NO_RESOURCES;
			goto send_rsp;
		}
		...
		size = (uint32_t)(sizeof(MQSV_MESSAGE) +
				  tmp_msg->info.msg.message.size);
	...
	/* Put the messages back in queue to take care of queue transfer failure
	 * scenario */
	/* Also, convert the messages in buffer to Network order before sending
	 * the transfer response */
	offset = 0;
	for (i = 0; i < transfer_rsp.msg.mqp_rsp.info.transferRsp.msg_count;
	     i++) {

		tmp_msg = (MQSV_MESSAGE *)&transfer_rsp.msg.mqp_rsp.info
			      .transferRsp.mqsv_message[offset];
...
reg_req:
	...
	/*Before sending the transfer response start the timer to check the
	 * Queue Transfer progress */
	qnode->qinfo.qtransfer_complete_tmr.type =
	...
	if (rc == NCSCC_RC_SUCCESS) {
		...
		rc = mqnd_ckpt_queue_info_write(cb, &queue_ckpt_node,
						qnode->qinfo.shm_queue_index);
		if (rc != SA_AIS_OK) {
			TRACE_2("Ckpt Overwrite Failed with return code %u",
				rc);
			return rc;
		}
	...
	/* Register with MQD the change in status to
	...
	rc = asapi_opr_hdlr(&opr);
	if (rc == SA_AIS_ERR_TIMEOUT) {
		/*This is the case when REG request is assumed to be lost or not
		   proceesed at MQD becoz of failover.So to compensate for the
		   lost request we send an async REG request to MQD */
		/*Request the ASAPi (at MQD) for queue REG */
		...
		if (rc != SA_AIS_OK || (!opr.info.msg.resp) ||
		    (opr.info.msg.resp->info.rresp.err.flag)) {
			LOG_ER(
			    "ERR_LIBRARY: Asapi Opr handler for Queue Register Failed");
			qnode->qinfo.owner_flag = MQSV_QUEUE_OWN_STATE_ORPHAN;
...
send_rsp:
	...
	transfer_rsp.msg.mqp_rsp.info.transferRsp.old_queueHandle =
	    req->msg.mqp_req.info.transferReq.old_queueHandle;
	transfer_rsp.msg.mqp_rsp.info.transferRsp.openReq =
	    req->msg.mqp_req.info.transferReq.openReq;
	transfer_rsp.msg.mqp_rsp.info.transferRsp.invocation =
	    req->msg.mqp_req.info.transferReq.invocation;
	transfer_rsp.msg.mqp_rsp.info.transferRsp.rcvr_mqa_sinfo =
	    req->msg.mqp_req.info.transferReq.rcvr_mqa_sinfo;
	transfer_rsp.msg.mqp_rsp.info.transferRsp.openType =
	    req->msg.mqp_req.info.transferReq.openType;
					     ...
					     MQP_REQ_TYPE openType)
{
	...
	if (mqp_req->type == MQP_EVT_OPEN_REQ) {
		...
		timeout = open->timeout - 100; /*Start a timer with reduced
						  magnitude to compensate for
						  the req to reach mqnd */
		...
		transfer_req.msg.mqp_req.info.transferReq.invocation =
		    mqp_req->info.openAsyncReq.invocation;
		timeout = MQND_QTRANSFER_REQ_TIMER; /*Async req the timer at mqa
						       is 10 secs so start one
						       here of 9.99 secs */
	}
	...
	transfer_req.msg.mqp_req.info.transferReq.rcvr_mqa_sinfo =
	    *rcvr_mqa_sinfo;
	...
	if (!qevt_node) {
		LOG_CR("Queue Transfer Event node Alloc Failed for handle %llu",
		       old_hdl);
		goto free_mem;
	}
	...
	if (rc == NCSCC_RC_SUCCESS)
		TRACE_1("QTransfer Tmr on Node 1 started for handle %llu",
			old_hdl);
	else {
		LOG_ER("QTransfer Tmr on Node 1 failed for handle %llu",
		       old_hdl);
		goto free_mem;
	}
	...
	if (rc != NCSCC_RC_SUCCESS) {
		TRACE_2(
		    "Adding the Q Trans Evt node to the DB failed for handle %llu",
		    old_hdl);
		goto qevt_fail;
	}
	return rc;
...
uint32_t mqnd_evt_proc_mqp_qtransfer_response(MQND_CB *cb, MQSV_EVT *evt)
{
	...
	/*Received Transfer Req Response so in case of Async open req remove the
	 * event node,stop the timer and free the memory */
	...
	/* Create the queue locally and set the ownership flag from PROGRESS to
	 * OWNED */
	rc = mqnd_queue_create(cb, &transfer_rsp->openReq,
			       &transfer_rsp->rcvr_mqa_sinfo.dest, &qhdl,
			       transfer_rsp, &err);
	if (rc != NCSCC_RC_SUCCESS) {
	...
	transfer_complete.msg.mqp_req.info.transferComplete.queueHandle =
	    transfer_rsp->old_queueHandle;
	transfer_complete.msg.mqp_req.info.transferComplete.error =
	    NCSCC_RC_SUCCESS;
	rc = mqnd_mds_send(cb, NCSMDS_SVC_ID_MQND, transfer_rsp->old_owner,
			   &transfer_complete);
	if (rc != NCSCC_RC_SUCCESS) {
		TRACE_2(
	...
	mqnd_send_mqp_open_rsp(cb, &transfer_rsp->rcvr_mqa_sinfo, &mqp_req, err,
			       qhdl, transfer_rsp->msg_count);
...
uint32_t mqnd_fill_queue_from_transfered_buffer(MQND_CB *cb,
						...
						MQP_TRANSFERQ_RSP *transfer_rsp)
{
	...
	if ((transfer_rsp->openReq.openFlags & SA_MSG_QUEUE_EMPTY) ||
	    (transfer_rsp->msg_count == 0)) {
		mqnd_reset_queue_stats(cb, qnode->qinfo.shm_queue_index);
		return rc;
	}

	/* Resizze the new queue so that its able to hold all the incoming
	 * messages */
	...
	/* Increase queue size so that its able to hold all messages + their
	 * MQSV + LEAP headers */
	info.info.resize.i_newqsize =
	    transfer_rsp->msg_bytes +
	    (transfer_rsp->msg_count *
	     (sizeof(MQSV_MESSAGE) + sizeof(NCS_OS_MQ_MSG_LL_HDR)));
	...
	for (i = 0; i < transfer_rsp->msg_count; i++) {
		...
		mqsv_message =
		    (MQSV_MESSAGE *)&transfer_rsp->mqsv_message[offset];
		...
		mqnd_send_msg_update_stats_shm(
		    cb, qnode, mqsv_message->info.msg.message.size,
		    mqsv_message->info.msg.message.priority);

		rc = mqnd_mq_msg_send(qnode->qinfo.queueHandle, mqsv_message,
				      (uint32_t)size);
...
uint32_t mqnd_existing_queue_open(MQND_CB *cb, MQSV_SEND_INFO *sinfo,
				  MQP_OPEN_REQ *open,
				  ASAPi_NRESOLVE_RESP_INFO *qinfo,
				  SaAisErrorT *err,
				  uint32_t *existing_msg_count)
{
	...
	*existing_msg_count = 0;
	...
	if (open->openFlags & SA_MSG_QUEUE_EMPTY) {
		...
		for (priority = SA_MSG_MESSAGE_HIGHEST_PRIORITY;
		     priority <= SA_MSG_MESSAGE_LOWEST_PRIORITY; priority++) {
			*existing_msg_count +=
	...
	/* Create a listener queue and empty it if the RECEIVED_CALLBACK is
	 * enabled for the queue */
	if (open->openFlags & SA_MSG_QUEUE_RECEIVE_CALLBACK) {
		if (mqnd_listenerq_create(&qnode->qinfo) != NCSCC_RC_SUCCESS) {
			TRACE_4(
			    "ERR_RESOURCES: Listener Queue creation Failed");
			*err = SA_AIS_ERR_NO_RESOURCES;
			goto error1;
		}
	}
	...
	/* Emptying the queue is to be done as last step coz if done earlier and
	   if so other error occurrs, we will need to put in an elaborate logic
	   in place to refill the queue with the messages that were deleted */
	if (open->openFlags & SA_MSG_QUEUE_EMPTY) {
		if (mqnd_mq_empty(qinfo->oinfo.qparam->hdl) !=
		    NCSCC_RC_SUCCESS) {
	...
	/*MQND Restart. Here the owner of the queue is updated. Update the
	 * checkpoint for this record. */
			      ...
			      ASAPi_NRESOLVE_RESP_INFO *qinfo)
{
	...
	if (is_qexists) {
		...
		if (m_MQND_IS_SA_MSG_QUEUE_CREATE_SET(open->openFlags)) {
			/*Match the received creation attributes with the
			   attributes of the existing message queue If not
			   matching, a queue of the same name but with different
			   attributes already exists Therefore, send
			   SA_AIS_ERR_EXIST message back to MQA */
			if ((mqnd_compare_create_attr(&open->creationAttributes,
						      &cre_attr)) == false) {
				LOG_ER(
				    "ERR_EXIST: Comparison between the Receive attributes and"
				    "existing queue attributes Failed");
				err = SA_AIS_ERR_EXIST;
				goto error;
			}
		} else
			/* If create flag is not set, update the creation
			 * attributes from the creation attributes in the
			 * database */
			open->creationAttributes = cre_attr;

		/* If queue is opened by some body, return SA_AIS_ERR_BUSY to
		 * MQA */
		...
		if (qinfo->oinfo.qparam->owner != MQSV_QUEUE_OWN_STATE_ORPHAN) {
			LOG_ER(
			    "%s:%u: ERR_BUSY: MSGQ is Busy (Opened by someone)",
			    __FILE__, __LINE__);
			err = SA_AIS_ERR_BUSY;
			goto error;
		}

		/* If NON-LOCAL queue, initiate transfer of queue from current
		 * owner MQND */
		if (m_NCS_MDS_DEST_EQUAL(&qinfo->oinfo.qparam->addr,
					 &cb->my_dest) == 0) {
			qhdl = qinfo->oinfo.qparam->hdl;
			/* Send the transfer request to remote MQND */
			rc = mqnd_send_transfer_owner_req(
			    cb, mqp_req, sinfo, &qinfo->oinfo.qparam->addr,
			    qinfo->oinfo.qparam->hdl, mqp_req->type);
			if (rc != NCSCC_RC_SUCCESS)
				TRACE_2(
				    "The Queue Transfer owner request Failed");
			return rc;
		} else { /* LOCAL queue */
			rc = mqnd_existing_queue_open(
			    cb, sinfo, open, qinfo, &err, &existing_msg_count);

			if (rc != NCSCC_RC_SUCCESS) {
				TRACE_2(
				    "Attempt to open the existing local message queue failed with %u",
				    rc);
				goto error;
			}
...
error:
	/* Send response back to MQA */
	mqnd_send_mqp_open_rsp(cb, sinfo, mqp_req, err, qhdl,
			       existing_msg_count);
...
uint32_t mqnd_send_mqp_open_rsp(MQND_CB *cb, MQSV_SEND_INFO *sinfo,
				...
				uint32_t qhdl, uint32_t existing_msg_count)
{
	MQSV_EVT rsp_evt;
	uint32_t rc = NCSCC_RC_SUCCESS;
	...
	if (mqp_req->type == MQP_EVT_OPEN_REQ) {
		...
		rsp_evt.msg.mqp_rsp.info.openRsp.msgHandle =
		    open_req->msgHandle;
		rsp_evt.msg.mqp_rsp.info.openRsp.queueName =
		    open_req->queueName;

		if (err == SA_AIS_OK) {
			rsp_evt.msg.mqp_rsp.info.openRsp.queueHandle = qhdl;
			rsp_evt.msg.mqp_rsp.info.openRsp.listenerHandle =
			    qnode->qinfo.listenerHandle;
			rsp_evt.msg.mqp_rsp.info.openRsp.existing_msg_count =
			    existing_msg_count;
		}
		...
		rsp_evt.msg.mqp_async_rsp.params.qOpen.invocation =
		    mqp_req->info.openAsyncReq.invocation;
		rsp_evt.msg.mqp_async_rsp.params.qOpen.openFlags =
		    open_req->openFlags;

		if (err == SA_AIS_OK) {
			rsp_evt.msg.mqp_async_rsp.params.qOpen.queueHandle =
			    qhdl;
			rsp_evt.msg.mqp_async_rsp.params.qOpen.listenerHandle =
			    qnode->qinfo.listenerHandle;
			rsp_evt.msg.mqp_async_rsp.params.qOpen
...
uint32_t mqnd_proc_queue_close(MQND_CB *cb, MQND_QUEUE_NODE *qnode,
			       SaAisErrorT *err)
{
	uint32_t rc = NCSCC_RC_SUCCESS;
	ASAPi_OPR_INFO opr;
	...
	*err = SA_AIS_OK;
	...
	timeout = m_NCS_CONVERT_SATIME_TO_TEN_MILLI_SEC(
	    qnode->qinfo.queueStatus.retentionTime);
	...
	if ((qnode->qinfo.sendingState == MSG_QUEUE_UNAVAILABLE) ||
		...
		if (qnode->qinfo.sendingState == MSG_QUEUE_AVAILABLE) {
			...
			rc = asapi_opr_hdlr(&opr);
			if (rc == SA_AIS_ERR_TIMEOUT) {
				/*This is the case when DEREG request is assumed
				   to be lost or not proceesed at MQD becoz of
				   failover.So to compensate for the lost
				   request we send an async request to MQD to
				   process it once again to be in sync with
				   database at MQND */
				/* Deregister the Queue with ASAPi */
				TRACE_1(
				    "CLOSE TIMEOUT:ASYNC DEREG TO DELETE QUEUE");
				memset(&opr, 0, sizeof(ASAPi_OPR_INFO));
				opr.type = ASAPi_OPR_MSG;
				opr.info.msg.opr = ASAPi_MSG_SEND;
				...
				opr.info.msg.req.msgtype = ASAPi_MSG_DEREG;
				opr.info.msg.req.info.dereg.objtype =
				    ASAPi_OBJ_QUEUE;
				opr.info.msg.req.info.dereg.queue =
				    qnode->qinfo.queueName;

				rc = asapi_opr_hdlr(
				    &opr); /*May be insert a log */
			} else {
				if ((rc != SA_AIS_OK) || (!opr.info.msg.resp) ||
				    (opr.info.msg.resp->info.dresp.err.flag)) {
					LOG_ER(
					    "Asapi Opr Handler for Queue Dereg Failed");
					return NCSCC_RC_FAILURE;
				}
			}
		...
		if (rc == SA_AIS_ERR_TIMEOUT) {
			...
			if ((rc != SA_AIS_OK) || (!opr.info.msg.resp) ||
			    (opr.info.msg.resp->info.rresp.err.flag)) {
				qnode->qinfo.owner_flag =
				    MQSV_QUEUE_OWN_STATE_OWNED;
				LOG_ER(
				    "ERR_RESOURCES: Asapi Opr handler for Queue Register Failed");
				*err = SA_AIS_ERR_NO_RESOURCES;
				return NCSCC_RC_FAILURE;
			}
		}
		...
		/* Update checkpoint section. Queue closeTime needs to be
		 * checkpointed with new value */
		memset(&queue_ckpt_node, 0, sizeof(MQND_QUEUE_CKPT_INFO));
		mqnd_cpy_qnodeinfo_to_ckptinfo(cb, qnode, &queue_ckpt_node);
		...
		if (rc != SA_AIS_OK) {
			...
			qnode->qinfo.owner_flag = MQSV_QUEUE_OWN_STATE_OWNED;
			LOG_ER(
			    "ERR_RESOURCES: Checkpointing queue info to local shared memory of MQND");
			*err = SA_AIS_ERR_NO_RESOURCES;
			return NCSCC_RC_FAILURE;
		}
		...
		if (!(qnode->qinfo.queueStatus.creationFlags &
			...
			TRACE_1(
			    "Retention timer has been strted for the queue");
		}
	}
...
uint32_t mqnd_send_mqp_close_rsp(MQND_CB *cb, MQSV_SEND_INFO *sinfo,
...
uint32_t mqnd_send_mqp_ulink_rsp(MQND_CB *cb, MQSV_SEND_INFO *sinfo,
	...
	SaAisErrorT error)
{
	...
	if (cb == NULL) {
		LOG_ER(
		    "Getting the callback from CLM for the cluster track failed");
		return;
	} else {
