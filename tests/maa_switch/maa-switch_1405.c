...
int maa_get_status(void);
int maa_get_active_node(void);
...
uns32 maa_fail_lock(int node , int lock_val)
{
   ...
   if((node != 7) && (node != 8))
   {
      printf("\n FAILOVERS CAN BE DONE ONLY FOR NODES (7 & 8 )\n");
      return NCSCC_RC_FAILURE;
   }
...
void maa_unlck_nod(int prev_act_node)
{
   ...
   if((prev_act_node != 7) && (prev_act_node != 8))
   {
...
int maa_get_status()
{
   char su1_index[]="safSu=SuT_NCS_SCXB,safNode=SC_2_7";
   char si1_index[]="safSi=Si_SCXB";
   char su2_index[]="safSu=SuT_NCS_SCXB,safNode=SC_2_8";
...
{
   char comp_index[]="safComp=CompT_MAS,safSu=SuT_NCS_SCXB,safNode=SC_2_7";
