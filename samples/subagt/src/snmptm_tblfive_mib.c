...
/*****************************************************************************
..............................................................................
  MODULE NAME: SNMPTM_TBLFIVE_MIB.C

..............................................................................
  
  DESCRIPTION:  This module contains functions used by the SNMPTM Subsystem for
                SNMP operations on the SNMPTM Entp TBLFIVE table.
..............................................................................

  snmptm_tblfive_tbl_req .......... Process the Request of an entp tblfive table
  ncstesttablefiveentry_get ....... Get the object from the tblfive table
  ncstesttablefiveentry_set ....... Set the object in the tblfive table
  ncstesttablefiveentry_next ...... Get the next object from the tblfive table
  ncstesttablefiveentry_setrow .... Set the complete row of the tblfive table
...
#ifndef SNMPTM_TBLFIVE_TBL_INST_LEN
#define SNMPTM_TBLFIVE_TBL_INST_LEN   4 /*  ip_addr (4)  */
#endif

/* function proto-types */
static uns32 get_tblfive_entry(SNMPTM_CB *snmptm,
                               NCSMIB_ARG *arg, 
                               SNMPTM_TBLFIVE **tblfive);
static uns32 get_next_tblfive_entry(SNMPTM_CB   *snmptm,
                                    NCSMIB_ARG  *arg,
...
/****************************************************************************
  ...
  Notes         : 
****************************************************************************/
uns32 snmptm_tblfive_tbl_req(struct ncsmib_arg *args)
{
...
/****************************************************************************
  ...
  Description   :  From the instance ids provided, prepares the key for 
                   searching the TBLFIVE Patricia Tree.
...
void make_key_from_instance_id(SNMPTM_TBL_KEY *key, const uns32 *inst_id, uns32 inst_len)
{
   ...
   /* All in network address in the tblfive key */
...
/****************************************************************************
  ...
  Description   :  From the key in the tblfive form the instance id  
...
/****************************************************************************
 Name          :  get_tblfive_entry

 Description   :  Get the Instance of tblfive requested 
 ...
 Notes         :   
****************************************************************************/
uns32 get_tblfive_entry(SNMPTM_CB  *snmptm, NCSMIB_ARG  *arg, SNMPTM_TBLFIVE  **tblfive)
{
   ...
   /* Can't be NULL */
   if(tblfive_key.ip_addr.info.v4 == (NCS_IPV4_ADDR) NULL)
      return NCSCC_RC_NO_INSTANCE;
   
   /* The tblfive is numbered, do a get based on address */
   if(((*tblfive = (SNMPTM_TBLFIVE*) ncs_patricia_tree_get(&snmptm->tblfive_tree,
         (uns8*) &tblfive_key)) == NULL) ||
         ((*tblfive)->tblfive_key.ip_addr.info.v4 !=
                             tblfive_key.ip_addr.info.v4))
         return NCSCC_RC_NO_INSTANCE;

   printf("TBLFIVE_ENTRY key address is: 0x%x\n", (*tblfive)->tblfive_key.ip_addr.info.v4);
...
/****************************************************************************
  Name          :  get_next_tblfive_entry
 
  Description   :  Find the Instance of tblfive requested 
  ...
  Notes         :  
****************************************************************************/               
uns32 get_next_tblfive_entry(SNMPTM_CB *snmptm,
                            NCSMIB_ARG  *arg, 
                            uns32  *io_next_inst_id,
                            SNMPTM_TBLFIVE  **tblfive)
{
   ...
   {
      /* First tblfive entry is requested */
      if ((*tblfive = (SNMPTM_TBLFIVE*)ncs_patricia_tree_getnext(&snmptm->tblfive_tree, 
                                                      (uns8*)0)) == NULL) 
         return NCSCC_RC_NO_INSTANCE;
   }
   else
   { 
      ...
      /* Prepare the tblfive key from the instant ID */
      make_key_from_instance_id(&tblfive_key, arg->i_idx.i_inst_ids, arg->i_idx.i_inst_len);
      ...
      /* The tblfive is numbered, do a getnext based on address */
      if (((*tblfive = (SNMPTM_TBLFIVE*) ncs_patricia_tree_getnext(&snmptm->tblfive_tree,
            (uns8*) &tblfive_key)) == NULL)) 
         return NCSCC_RC_NO_INSTANCE;
   }
  
   printf("TBLFIVE_ENTRY IP address is: 0x%x\n", (*tblfive)->tblfive_key.ip_addr.info.v4);
   /* Make an instance id from tblfive_key */
   make_instance_id_from_key(&((*tblfive)->tblfive_key), io_next_inst_id); 
...
/****************************************************************************
  Name          :  ncstesttablefiveentry_set
  
  Description   :  Set function for SNMPTM tblfive table objects 
  ...
  Notes         : 
****************************************************************************/
uns32 ncstesttablefiveentry_set(NCSCONTEXT cb, 
                               NCSMIB_ARG *arg, 
                               ...
                               NCS_BOOL test_flag)
{
   ...
   /* Validate row status */
   if (i_set_req->i_param_val.i_param_id == ncsTestTableFiveRowStatus_ID)
   {
      uns32  rc = NCSCC_RC_SUCCESS;
      ...
      temp_mib_req.info.i_val_status_util_info.row = tblfive;
   ...
   /* If the corresponding entry is not exists, and if the object is of 
      read-create type then create a tblfive entry. */
   if ((tblfive == NULL) && (test_flag != TRUE))
   {
      if (var_info->access == NCSMIB_ACCESS_READ_CREATE) 
      {
         if ((i_set_req->i_param_val.i_param_id != ncsTestTableFiveRowStatus_ID) ||
             ((i_set_req->i_param_val.i_param_id == ncsTestTableFiveRowStatus_ID)
              && (create_flag != FALSE)))
         {
             tblfive = snmptm_create_tblfive_entry(snmptm, &tblfive_key);
   ...
   if (test_flag != TRUE) 
   {
      ...
      {
         /* Delete the entry from the TBLFIVE */
         snmptm_delete_tblfive_entry(snmptm, tblfive);
      }
      else
      {
         m_NCS_OS_MEMSET(&temp_mib_req, 0, sizeof(NCSMIBLIB_REQ_INFO)); 
         ...
         temp_mib_req.info.i_set_util_info.data = tblfive;
...
/****************************************************************************
  Name          : ncstesttablefiveentry_extract 
  
  Description   :  Get function for SNMPTM tblfive table objects 
  ...
  Notes         : 
****************************************************************************/
uns32 ncstesttablefiveentry_extract(NCSMIB_PARAM_VAL *param, 
                                   NCSMIB_VAR_INFO  *var_info,
                                   NCSCONTEXT data,
                                   NCSCONTEXT buffer)
{
...
/****************************************************************************
  Name          :  ncstesttablefiveentry_get 
  ...
  Notes         : 
****************************************************************************/
uns32  ncstesttablefiveentry_get(NCSCONTEXT cb, 
                                NCSMIB_ARG *arg, 
                                NCSCONTEXT* data)
{
   SNMPTM_CB     *snmptm = (SNMPTM_CB *)cb;
   ...
   if((ret_code = get_tblfive_entry(snmptm, arg, &tblfive)) != NCSCC_RC_SUCCESS)
       return ret_code;
...
/****************************************************************************
  Name          :  ncstesttablefiveentry_next
  
  Description   :  Next function for SNMPTM tblfive table 
  ...
  Notes         : 
****************************************************************************/
uns32 ncstesttablefiveentry_next(NCSCONTEXT snmptm, 
                                NCSMIB_ARG *arg, 
                                NCSCONTEXT* data, 
                                uns32* next_inst_id,
                                uns32* next_inst_id_len) 
{
   ...
   if ((ret_code = get_next_tblfive_entry(snmptm, arg, next_inst_id, &tblfive))
       != NCSCC_RC_SUCCESS)
       return ret_code;

   *data = tblfive;
   *next_inst_id_len = SNMPTM_TBLFIVE_TBL_INST_LEN;
...
/****************************************************************************
  Name          :  ncstesttablefiveentry_setrow
  
  Description   :  Setrow function for SNMPTM tblfive table 
  ...
  Notes         : 
****************************************************************************/
uns32 ncstesttablefiveentry_setrow(NCSCONTEXT cb, 
                                  NCSMIB_ARG* arg,
                                  NCSMIB_SETROW_PARAM_VAL* tblfive_param,
                                  ...
                                  NCS_BOOL testrow_flag)
{
   ...
   tblfive = (SNMPTM_TBLFIVE*)ncs_patricia_tree_get(&snmptm->tblfive_tree,
                                                  (uns8*)&tblfive_key);
   /* Validate row status */
   if (tblfive_param[ncsTestTableFiveRowStatus_ID - 1].set_flag == TRUE)
   { 
      uns32  rc = NCSCC_RC_SUCCESS;
  
      temp_mib_req.req = NCSMIBLIB_REQ_VALIDATE_STATUS_UTIL_OP; 
      temp_mib_req.info.i_val_status_util_info.row_status =
             &(tblfive_param[ncsTestTableFiveRowStatus_ID - 1].param.info.i_int);
      temp_mib_req.info.i_val_status_util_info.row = tblfive;
   ...
   if(testrow_flag != TRUE)
   {
      ...
      for(param_id = ncsTestTableFiveIpAddress_ID; 
          param_id < ncsTestTableFiveEntryMax_ID; 
          param_id++)
          {  
             if(param_id == ncsTestTableFiveIpAddress_ID)
                continue;
