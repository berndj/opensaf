...
static uns32 
cli_apps_cefs_load(uns8 *file_name, uns32 what_to_do); 
...
uns32 cli_timer_start(CLI_CB *cliCb)
{
    ...
    m_NCS_TMR_START(timer_data->timer_id,
                      (timer_data->period - timer_data->grace_period),
                      cli_timer_cb,
                      (void*)(timer_data));
    if(timer_data->timer_id == TMR_T_NULL)
...
static uns32 cli_dflt_reg(uns32 hdl)
{
   ...
   /*Register the default commands with CLI*/
    ...
    data.i_cmd_count = 4;
    data.i_command_list[0].i_cmdstr = "configure!Enter configuration mode! @root/exec/config@ NCSCLI_STRING!terminal!";    
    data.i_command_list[1].i_cmdstr = "help!Description of the interactive help system!";         
...
void cli_destroy(CLI_CB **io_cli)
{
   ...
   if(pCli->subsys_cb.i_cef_data) m_MMGR_FREE_CLI_DEFAULT_VAL(pCli->subsys_cb.i_cef_data);   
   if(pCli->subsys_cb.i_cef_mode) m_MMGR_FREE_CLI_DEFAULT_VAL(pCli->subsys_cb.i_cef_mode);
...
cli_apps_cefs_load(uns8 *file_name, uns32 what_to_do)
{
    ...
    m_LOG_NCSCLI_STR(NCSFL_LC_DATA, NCSFL_SEV_NOTICE, 
          ...
          (nargs != EOF))
    {
        ...
        if( strcmp(arg1,"CLI_IDLE_TIME")  == 0 )
        {
            ...
            if( (cli_idle_time != NULL) && (cliidletime = atoi(cli_idle_time)) )
            {
                    ...
                    if(cliidletime >= (m_CLI_MIN_IDLE_TIME_IN_SEC * 100)&& cliidletime <= (m_CLI_MAX_IDLE_TIME_IN_SEC * 100) )
