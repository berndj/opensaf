...
uns32 cli_check_number(int8 *i_str)
{
   ...
   sscanf(i_str, "%u", &num);
   if(num) {
      if(0 == (buffer = (int8 *)m_MMGR_ALLOC_CLI_DEFAULT_VAL(m_NCS_OS_STRLEN(i_str)+1)))
         return NCSCC_RC_FAILURE;
      
      m_NCS_OS_MEMSET(buffer, 0, m_NCS_OS_STRLEN(i_str));
      sysf_sprintf(buffer, "%d", num);      
      if(m_NCS_OS_STRLEN(buffer) == m_NCS_OS_STRLEN(i_str)) {
...
cli_check_ipv4addr (int8 *i_str)
{
   ...
   for(;;) {
      m_NCS_OS_MEMSET (ipbuf,'\0', sizeof (ipbuf));
      ptr = i_str;
...
cli_matchtoken(int8 *i_str1, int8 *i_str2, CLI_CMD_ERROR *o_cli_cmd_error)
{
    ...
    if(!*s1 && !*s2) {
        /* reset error string */
        m_NCS_OS_MEMSET(o_cli_cmd_error->errstring, '\0', sizeof(o_cli_cmd_error->errstring));    
    ...
    if(!*s2) {
        /* reset error string */
        m_NCS_OS_MEMSET(o_cli_cmd_error->errstring, 0, 
...
uns32 cli_set_arg(int8 * i_arg, NCSCLI_ARG_SET *o_cmd_tokens, CLI_CB *pCli) /* Fxi for the bug 59854 , CLI_CB* extra argument passed */
{
   ...
   if(NCSCLI_NUMBER == o_cmd_tokens->i_arg_record[count].i_arg_type) {
      ...
      if(longnum > hvalue)
      {
          /*pCli->outOfBoundaryToken = (int8 *)malloc(m_NCS_STRLEN(i_arg)); */
      ...
      m_NCS_OS_MEMSET(o_cmd_tokens->i_arg_record[count].cmd.strval, '\0', len);
   ...
   if(NCSCLI_OPTIONAL == pCli->ctree_cb.cmdElement->tokType) {
      ...
      if(NCSCLI_STRING == i_argval->i_arg_type || NCSCLI_NUMBER == i_argval->i_arg_type) {
         if(NCSCLI_NUMBER == i_argval->i_arg_type) 
         {
           ...
           m_NCS_OS_MEMSET(i_argval->cmd.strval,'\0',len+1); 
                              ...
                              uns32 i_index)
{
   ...
   */
   ...
   if(NCSCLI_NUMBER == o_cmd_tokens->i_arg_record[i_index].i_arg_type) {
      ...
      len = m_NCS_OS_STRLEN(i_cmdElement->defVal) + 1;
      ...
      m_NCS_OS_MEMSET(o_cmd_tokens->i_arg_record[i_index].cmd.strval, '\0', len);
      ...
      if(CLI_NO_MATCH == ret || CLI_PARTIAL_MATCH == ret) {
         ...
         */
         ...
         if(partialmatch) {
            ...
            len = m_NCS_OS_STRLEN(pCli->ctree_cb.cmdElement->tokName) + 1;
            ...
            m_NCS_OS_MEMSET(io_cmd_tokens->i_arg_record[i].cmd.strval, '\0', len);               
               ...
               */
               ...
               if((CLI_SUCCESSFULL_MATCH == ret || CLI_PASSWORD_MATCHED == ret) &&
                  ...
                  if(help_str) {
                     ...
                     m_NCS_OS_MEMSET(io_cmd_tokens->i_arg_record[0].cmd.strval, '\0', len);               
...
int32 cli_find_errpos(int8 *i_actualstr, int8 *i_errstr, int32 i_errpos)
{
   ...
   if(!m_NCS_OS_STRLEN(s2) || !m_NCS_OS_STRLEN(s1)) return 0;
                          ...
                          CLI_SESSION_INFO  *o_session)
{
    ...
    m_NCS_OS_MEMSET(&ctok, 0, sizeof(NCSCLI_ARG_SET));
    m_NCS_OS_MEMSET(&cdata, 0, sizeof(cdata));
    ...
    */
    ...
    /* m_NCS_OS_MEMSET tab string */
    m_NCS_OS_MEMSET(io_param->o_hotkey.tabstring, '\0', CLI_ARG_LEN);    
                ...
                CLI_SESSION_INFO   *io_session)
{
   ...
   */   
   ...
   m_NCS_OS_MEMSET(&io_param->o_hotkey.hlpstr, '\0', sizeof(io_param->o_hotkey.hlpstr));
                      ...
                      CLI_SESSION_INFO  *io_session)
{
   ...
   */
   cmdlen = m_NCS_OS_STRLEN(io_cmd_tokens->i_arg_record[i-1].cmd.strval);
   io_param->o_status = cli_check_syntax(pCli, io_cmd_tokens, io_param, &cli_cmd_error);
...
static void convert_to_community(NCSCLI_ARG_VAL *arg_val, int8* str)
{
   ...
   m_NCS_OS_MEMSET(buffer, 0, sizeof(buffer));
   m_NCS_OS_MEMSET(ipbuff, 0, sizeof(ipbuff));
...
void cli_update_cmd_arg(NCSCLI_ARG_SET *ctok, CLI_EXECUTE_PARAM *io_param)
{
   ...
   m_NCS_OS_MEMSET(buffer, 0, sizeof(buffer));
   m_NCS_OS_MEMSET(wldcbuff, 0, sizeof(wldcbuff));
