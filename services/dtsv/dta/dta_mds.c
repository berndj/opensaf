...
uns32 dta_mds_install_and_subscribe(void)
{
   ...
   {
      return m_DTA_DBG_SINK(NCSCC_RC_FAILURE, "dta_mds_install_and_subscribe: DTA : MDS install and subscribe failed.");
   }
...
uns32 dta_mds_callback(NCSMDS_CALLBACK_INFO *cbinfo)
{
    ...
    case MDS_CALLBACK_SVC_EVENT:
        dta_mds_evt(cbinfo->info.svc_evt,
                    cbinfo->i_yr_svc_hdl);
...
uns32 dta_svc_reg_config(DTA_CB *inst, DTSV_MSG *msg)
{
    ...
    if((svc = (REG_TBL_ENTRY *)ncs_patricia_tree_get(&inst->reg_tbl, (const 
        uns8*)&((DTSV_MSG*)msg)->data.data.reg_conf.msg_fltr.svc_id)) ==NULL)
    {
...
uns32 dta_mds_rcv(MDS_CLIENT_HDL yr_svc_hdl, NCSCONTEXT msg)
{
   ...
   if(inst->created == FALSE)
   {
      ...
      return m_DTA_DBG_SINK(NCSCC_RC_FAILURE,
      "dta_mds_rcv: DTA does not exist. First create DTA.");
   }
   ...
   case DTS_SVC_MSG_FLTR:
       {
         ...
         */ 
         ...
         if((svc = (REG_TBL_ENTRY *)ncs_patricia_tree_get(&inst->reg_tbl, 
         (const uns8*)&((DTSV_MSG*)msg)->data.data.msg_fltr.svc_id)) == NULL)
         {
            status = m_DTA_DBG_SINK(NCSCC_RC_FAILURE,
   ...
   case DTS_FAIL_OVER:
      {
         ...
         if(data == NULL)
         {
            ...
            m_MMGR_FREE_DTSV_MSG((DTSV_MSG*)msg);
            m_DTA_UNLK(&inst->lock);
            return m_DTA_DBG_SINK(NCSCC_RC_FAILURE, "dta_mds_rcv :ncs_dec_flatten_space returns NULL");
         }
 ...
 * Notes         : None.
\*****************************************************************************/
void dta_mds_evt(MDS_CALLBACK_SVC_EVENT_INFO svc_info,
    ...
    case DTA_REGISTER_SVC:
       {
          ...
          if(*mm->data.data.reg.svc_name != '\0')
          {
             ...
             if(data == NULL)
             {
                return m_DTA_DBG_SINK(NCSCC_RC_FAILURE, "dta_mds_enc: Reserve space failed while encoding service registration message");
             }
    ...
    case DTA_LOG_DATA:
      {
         ...
         {
               return m_DTA_DBG_SINK(NCSCC_RC_FAILURE, "dta_mds_enc_v2: Log msg with format type 'D/N/U/X' dropped, because Active DTS version has changed to 1.");
         }
         else
         {
             /*****************************************************\
...
uns32 dta_mds_cpy(MDS_CLIENT_HDL yr_svc_hdl, NCSCONTEXT msg, 
  ...
  *cpy = mm;
  ...
  if (mm->msg_type == DTA_LOG_DATA)
  {
      ...
      if((remote_ver == 1) && (mm->data.data.msg.msg_fmat_ver == 2))
      {
            return m_DTA_DBG_SINK(NCSCC_RC_FAILURE, "dta_mds_cpy: Log msg with format type 'D' dropped, because Active DTS version has changed to 1.");
      }
      else
      {
          /*****************************************************\
      ...
      ncs_dec_init_space(&mm->data.data.msg.log_msg.uba, 
                         mm->data.data.msg.log_msg.uba.start);
   }
...
#endif
{
   ...
   if (ncsmds_api(&mds_info) == NCSCC_RC_SUCCESS)
   {
      ...
      {   
         ...
         status = dta_svc_reg_config(&dta_cb, 
             (DTSV_MSG *)mds_info.info.svc_send.info.sndrsp.o_rsp);
