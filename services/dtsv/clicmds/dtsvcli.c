...
static uns32 dtsv_cli_cmds_reg(NCSCLI_BINDERY *pBindery)
{
   ...
   NCSCLI_CMD_DESC dtsv_cli_cmds[] =
   {
      ...
      }, 
      {
          ...
          "print !Prints! \
           config !current config data for DTS, file stored in /var/opt/opensaf/log/DTS_<date>.config!"
      },
      {
...
*****************************************************************************/
uns32
dtsv_cef_set_logging_level(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   ...
   if(m_NCS_STRCMP(pArgv->cmd.strval,"global") == 0)
   {
      ...
      rc = dtsv_set_mib_int(&mib, ncsDtsvGlobalMessageLogging_ID, 0,
   ...
   else if(m_NCS_STRCMP(pArgv->cmd.strval,"node") == 0)
   {
      ...
      dtsv_cfg_mib_arg(&mib, &oid, NODE_INST_ID_LEN, 
                    NCSMIB_TBL_DTSV_NODE, cli_hdl, dtsv_hdl, NULL);
      
      if((rc = dtsv_get_mib_int(&mib,ncsDtsvNodeMessageLogging_ID,&get_value,reqfnc,MIB_VERR)) == NCSCC_RC_SUCCESS) 
      {
         dtsv_cfg_mib_arg(&mib, &oid, NODE_INST_ID_LEN,NCSMIB_TBL_DTSV_NODE, cli_hdl, dtsv_hdl, NULL);
         if(get_value == 0)
            rc = dtsv_set_mib_int(&mib, ncsDtsvNodeMessageLogging_ID, ncsDtsvNodeRowStatus_ID, NCS_SNMP_TRUE, reqfnc, MIB_VERR);
         else
            rc = dtsv_set_mib_int_val(&mib, ncsDtsvNodeMessageLogging_ID, NCS_SNMP_TRUE, reqfnc, MIB_VERR);   
      }
   }
   else
      rc = NCSCC_RC_FAILURE;
...
*****************************************************************************/
uns32
dtsv_cef_reset_logging_level(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG     mib;   
   uns32          cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32          dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   else if(m_NCS_STRCMP(pArgv->cmd.strval,"node") == 0)
   {
      ...
      if((rc = dtsv_get_mib_int(&mib,ncsDtsvNodeMessageLogging_ID,&get_value,reqfnc,MIB_VERR)) == NCSCC_RC_SUCCESS)
      {
         dtsv_cfg_mib_arg(&mib, &oid, NODE_INST_ID_LEN,NCSMIB_TBL_DTSV_NODE, cli_hdl, dtsv_hdl, NULL);
         if(get_value == 0)
            rc = dtsv_set_mib_int(&mib, ncsDtsvNodeMessageLogging_ID, ncsDtsvNodeRowStatus_ID, NCS_SNMP_FALSE, reqfnc, MIB_VERR);
         else
            rc = dtsv_set_mib_int_val(&mib, ncsDtsvNodeMessageLogging_ID, NCS_SNMP_FALSE, reqfnc, MIB_VERR);                   
      }
   }
   else
      rc = NCSCC_RC_FAILURE;
...
*****************************************************************************/
uns32
dtsv_cef_set_logging_device(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   if(m_NCS_STRCMP(pArgv->cmd.strval,"global") == 0)
   {
                         ...
                         &get_val, &length, reqfnc, MIB_VERR) != NCSCC_RC_FAILURE)
     {
        ...
        rc = dtsv_set_mib_oct(&mib, ncsDtsvGlobalLogDevice_ID, 0,
   ...
   else if(m_NCS_STRCMP(pArgv->cmd.strval,"service") == 0)
   {
      ...
      if(rc != NCSCC_RC_FAILURE)
      {
         ...
         if(rc == NCSCC_RC_NO_INSTANCE)
           rc = dtsv_set_mib_oct(&mib, ncsDtsvServiceLogDevice_ID, ncsDtsvServiceRowStatus_ID, &device, 1, reqfnc, MIB_VERR);
         else
           rc = dtsv_set_mib_oct(&mib, ncsDtsvServiceLogDevice_ID, 0,  &device, 1, reqfnc, MIB_VERR);  
      }
...
*****************************************************************************/
uns32
dtsv_cef_reset_logging_device(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
...
*****************************************************************************/
uns32
dtsv_cef_conf_log_device(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   else
   {
      ...
      if (pArgv->i_arg_type == NCSCLI_KEYWORD)
      {
         ...
         if (m_NCS_STRCMP(pArgv->cmd.strval,"file") == 0)
         {
            ...
            if((rc = dtsv_get_mib_int(&mib,ncsDtsvNodeLogFileSize_ID,&get_value,reqfnc,MIB_VERR)) == NCSCC_RC_SUCCESS)
            { 
               dtsv_cfg_mib_arg(&mib, oids, NODE_INST_ID_LEN,NCSMIB_TBL_DTSV_NODE, cli_hdl, dtsv_hdl, NULL);
               if(get_value == 0)
               {
                  if ((rc = dtsv_set_mib_int(&mib, ncsDtsvNodeLogFileSize_ID, ncsDtsvNodeRowStatus_ID, size, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                  {
               ...
               {
                   if ((rc = dtsv_set_mib_int_val(&mib, ncsDtsvNodeLogFileSize_ID, size, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                   {
                      dtsv_cli_display(cli_hdl, "\nDTSV: node file size configuration failed");
                      return rc;
                   }
            ...
            if (arg_list->i_pos_value & 0x80)
            {
               ...
               if((rc = dtsv_get_mib_int(&mib, ncsDtsvNodeFileLogCompFormat_ID, &get_value, reqfnc, MIB_VERR)) == NCSCC_RC_SUCCESS)
               {
                  dtsv_cfg_mib_arg(&mib, oids, NODE_INST_ID_LEN, NCSMIB_TBL_DTSV_NODE, cli_hdl, dtsv_hdl, NULL);
                  if(get_value == 0)
                  {
                     if ((rc = dtsv_set_mib_int(&mib, ncsDtsvNodeFileLogCompFormat_ID, ncsDtsvNodeRowStatus_ID, format, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                  ...
                  {
                     if ((rc = dtsv_set_mib_int_val(&mib, ncsDtsvNodeFileLogCompFormat_ID, format, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                       dtsv_cli_display(cli_hdl, "\nDTSV: node file log format configuration failed");
                       return rc;
                     }
         ...
         else if (m_NCS_STRCMP(pArgv->cmd.strval,"buffer") == 0)
         {
            ...
            if((rc = dtsv_get_mib_int(&mib,ncsDtsvNodeCircularBuffSize_ID,&get_value,reqfnc,MIB_VERR)) == NCSCC_RC_SUCCESS)
            {
               dtsv_cfg_mib_arg(&mib, oids, NODE_INST_ID_LEN,NCSMIB_TBL_DTSV_NODE, cli_hdl, dtsv_hdl, NULL);
               if(get_value == 0)
               {
                  if ((rc = dtsv_set_mib_int(&mib, ncsDtsvNodeCircularBuffSize_ID, ncsDtsvNodeRowStatus_ID, size, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                  {
               ...
               {
                  if ((rc = dtsv_set_mib_int_val(&mib, ncsDtsvNodeCircularBuffSize_ID, size, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                  {
                    dtsv_cli_display(cli_hdl, "\nDTSV: node buffer size configuration failed");
                      return rc;
                  }
            ...
            if (arg_list->i_pos_value & 0x80)
            {
               ...
               if((rc = dtsv_get_mib_int(&mib,ncsDtsvNodeCirBuffCompFormat_ID,&get_value,reqfnc,MIB_VERR)) == NCSCC_RC_SUCCESS)
               {
                  dtsv_cfg_mib_arg(&mib, oids, NODE_INST_ID_LEN,NCSMIB_TBL_DTSV_NODE, cli_hdl, dtsv_hdl, NULL);
                  if(get_value == 0)
                  {
                     if ((rc = dtsv_set_mib_int(&mib, ncsDtsvNodeCirBuffCompFormat_ID, ncsDtsvNodeRowStatus_ID, format, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                  ...
                  {
                     if ((rc = dtsv_set_mib_int_val(&mib, ncsDtsvNodeCirBuffCompFormat_ID, format, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                        dtsv_cli_display(cli_hdl, "\nDTSV: node buffer log format configuration failed");
                        return rc;
                     }
      ...
      else
      {
         ...
         if (m_NCS_STRCMP(pArgv->cmd.strval,"file") == 0)
         {
            ...
            if((rc = dtsv_get_mib_int(&mib,ncsDtsvServiceLogFileSize_ID,&get_value,reqfnc,MIB_VERR)) == NCSCC_RC_SUCCESS)
               {
                  dtsv_cfg_mib_arg(&mib, oids, SVC_INST_ID_LEN,NCSMIB_TBL_DTSV_SVC_PER_NODE, cli_hdl, dtsv_hdl, NULL);
                  if(get_value == 0)
                  {
                     if ((rc = dtsv_set_mib_int(&mib, ncsDtsvServiceLogFileSize_ID, ncsDtsvServiceRowStatus_ID, size, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                  ...
                  {
                     if ((rc = dtsv_set_mib_int_val(&mib, ncsDtsvServiceLogFileSize_ID, size, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                        dtsv_cli_display(cli_hdl, "\nDTSV: service file size configuration failed");
                        return rc;
                     }
            ...
            if (arg_list->i_pos_value & 0x80)
            {
               ...
               if((rc = dtsv_get_mib_int(&mib,ncsDtsvServiceFileLogCompFormat_ID,&get_value,reqfnc,MIB_VERR)) == NCSCC_RC_SUCCESS)
               {
                  dtsv_cfg_mib_arg(&mib, oids, SVC_INST_ID_LEN,NCSMIB_TBL_DTSV_SVC_PER_NODE, cli_hdl, dtsv_hdl, NULL);
                  if(get_value == 0)
                  {
                     if ((rc = dtsv_set_mib_int(&mib, ncsDtsvServiceFileLogCompFormat_ID, ncsDtsvServiceRowStatus_ID, format, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                  ...
                  {
                     if ((rc = dtsv_set_mib_int_val(&mib, ncsDtsvServiceFileLogCompFormat_ID, format, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                        dtsv_cli_display(cli_hdl, "\nDTSV: service file log format configuration failed");
                        return rc;
                     }
         ...
         else if (m_NCS_STRCMP(pArgv->cmd.strval,"buffer") == 0)
         {
            ...
            if((rc = dtsv_get_mib_int(&mib,ncsDtsvServiceCircularBuffSize_ID,&get_value,reqfnc,MIB_VERR)) == NCSCC_RC_SUCCESS)
               {
                  dtsv_cfg_mib_arg(&mib, oids, SVC_INST_ID_LEN,NCSMIB_TBL_DTSV_SVC_PER_NODE, cli_hdl, dtsv_hdl, NULL);
                  if(get_value == 0)
                  {
                     if ((rc = dtsv_set_mib_int(&mib, ncsDtsvServiceCircularBuffSize_ID, ncsDtsvServiceRowStatus_ID, size, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                  ...
                  {
                     if ((rc = dtsv_set_mib_int_val(&mib, ncsDtsvServiceCircularBuffSize_ID, size, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                        dtsv_cli_display(cli_hdl, "\nDTSV: service buffer size configuration failed");
                        return rc;
                     }
            ...
            if (arg_list->i_pos_value & 0x80)
            {
               ...
               if((rc = dtsv_get_mib_int(&mib,ncsDtsvServiceCirBuffCompFormat_ID,&get_value,reqfnc,MIB_VERR)) == NCSCC_RC_SUCCESS)
               {
                  dtsv_cfg_mib_arg(&mib, oids, SVC_INST_ID_LEN,NCSMIB_TBL_DTSV_SVC_PER_NODE, cli_hdl, dtsv_hdl, NULL);
                  if(get_value == 0)
                  {
                     if ((rc = dtsv_set_mib_int(&mib, ncsDtsvServiceCirBuffCompFormat_ID, ncsDtsvServiceRowStatus_ID, format, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                  ...
                  {
                     if ((rc = dtsv_set_mib_int_val(&mib, ncsDtsvServiceCirBuffCompFormat_ID, format, reqfnc, MIB_VERR)) != NCSCC_RC_SUCCESS)
                     {
                        dtsv_cli_display(cli_hdl, "\nDTSV: service buffer log format configuration failed");
                        return rc;
                     }
...
*****************************************************************************/
uns32
dtsv_cef_enable_logging(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   if (pArgv->i_arg_type == NCSCLI_KEYWORD)
   {
      ...
      if (pArgv->i_arg_type == NCSCLI_KEYWORD)
      {
         ...
         rc = dtsv_set_mib_int(&mib, ncsDtsvGlobalLoggingState_ID, 0,
            NCS_SNMP_TRUE, reqfnc, MIB_VERR);
      }
      else
      {
   ...
   else
   {
      ...
      if (pArgv->i_arg_type == NCSCLI_KEYWORD)
      {
         ...
         if((rc = dtsv_get_mib_int(&mib,ncsDtsvNodeMessageLogging_ID,&get_value,reqfnc,MIB_VERR) == NCSCC_RC_SUCCESS))          
         {
            dtsv_cfg_mib_arg(&mib, oids, NODE_INST_ID_LEN, NCSMIB_TBL_DTSV_NODE, cli_hdl, dtsv_hdl, NULL);
            if(get_value == 0)
               rc = dtsv_set_mib_int(&mib, ncsDtsvNodeLoggingState_ID, ncsDtsvNodeRowStatus_ID, NCS_SNMP_TRUE, reqfnc, MIB_VERR);
            else
               rc = dtsv_set_mib_int_val(&mib, ncsDtsvNodeLoggingState_ID, NCS_SNMP_TRUE, reqfnc, MIB_VERR);
         }
      ...
      else
      {
         ...
         if((rc = dtsv_get_mib_int(&mib,ncsDtsvServiceLoggingState_ID,&get_value,reqfnc,MIB_VERR) == NCSCC_RC_SUCCESS))
         {
            dtsv_cfg_mib_arg(&mib, oids, SVC_INST_ID_LEN,NCSMIB_TBL_DTSV_SVC_PER_NODE, cli_hdl, dtsv_hdl, NULL);
            if(get_value == 0)
               rc = dtsv_set_mib_int(&mib, ncsDtsvServiceLoggingState_ID, ncsDtsvServiceRowStatus_ID, NCS_SNMP_TRUE, reqfnc, MIB_VERR);
            else
               rc = dtsv_set_mib_int_val(&mib, ncsDtsvServiceLoggingState_ID, NCS_SNMP_TRUE, reqfnc, MIB_VERR);
         }
      }
   }
...
*****************************************************************************/
uns32
dtsv_cef_disable_logging(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   else
   {
      ...
      if (pArgv->i_arg_type == NCSCLI_KEYWORD)
      {
         ...
         dtsv_cfg_mib_arg(&mib, oids, NODE_INST_ID_LEN, 
          ...
          if((rc = dtsv_get_mib_int(&mib,ncsDtsvNodeLoggingState_ID,&get_value,reqfnc,MIB_VERR) == NCSCC_RC_SUCCESS))  
          { 
             dtsv_cfg_mib_arg(&mib, oids, NODE_INST_ID_LEN, NCSMIB_TBL_DTSV_NODE, cli_hdl, dtsv_hdl, NULL);
             if(get_value == 0)
                rc = dtsv_set_mib_int(&mib, ncsDtsvNodeLoggingState_ID, ncsDtsvNodeRowStatus_ID, NCS_SNMP_FALSE, reqfnc, MIB_VERR);
             else
                rc = dtsv_set_mib_int_val(&mib, ncsDtsvNodeLoggingState_ID, NCS_SNMP_FALSE, reqfnc, MIB_VERR);
          }
      }
      else
      {
         ...
         { 
            dtsv_cfg_mib_arg(&mib, oids, SVC_INST_ID_LEN, NCSMIB_TBL_DTSV_SVC_PER_NODE, cli_hdl, dtsv_hdl, NULL);
            if(get_value == 0)
               rc = dtsv_set_mib_int(&mib, ncsDtsvServiceLoggingState_ID, ncsDtsvServiceRowStatus_ID, NCS_SNMP_FALSE, reqfnc, MIB_VERR);
            else
               rc = dtsv_set_mib_int_val(&mib, ncsDtsvServiceLoggingState_ID, NCS_SNMP_FALSE, reqfnc, MIB_VERR);
         }
      }
   }
...
*****************************************************************************/
uns32
dtsv_cef_cnf_category(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   if (pArgv->i_arg_type == NCSCLI_KEYWORD)
   {
      ...
      if (pArgv->i_arg_type == NCSCLI_KEYWORD)
      {
         ...
         else 
         {
             ...
             if (dtsv_get_mib_oct(&mib, ncsDtsvGlobalCategoryBitMap_ID, &get_val, &length, reqfnc, MIB_VERR) != NCSCC_RC_FAILURE)
             {
   ...
   else
   {
      ...
      if (pArgv->i_arg_type == NCSCLI_KEYWORD)
      {
         ...
         if (pArgv->i_arg_type == NCSCLI_KEYWORD)
         {
             ...
             bit_map = (bit_set)?CAT_FILTER_SET_ALL_VAL:CAT_FILTER_NOSET_ALL_VAL;
             bit_map = htonl(bit_map);
             rc = dtsv_get_mib_oct(&mib, ncsDtsvNodeCategoryBitMap_ID, &get_val, &length, reqfnc, MIB_VERR);
             if(rc != NCSCC_RC_FAILURE)
             {
                get_val = ntohl(get_val);
                dtsv_cfg_mib_arg(&mib, oids, NODE_INST_ID_LEN, NCSMIB_TBL_DTSV_NODE, cli_hdl, dtsv_hdl, NULL);
                ...
                if(rc == NCSCC_RC_NO_INSTANCE)
                    rc = dtsv_set_mib_oct(&mib, ncsDtsvNodeCategoryBitMap_ID, ncsDtsvNodeRowStatus_ID,(uns8 *)&bit_map, 4, reqfnc, MIB_VERR);
                else
                   rc = dtsv_set_mib_oct(&mib, ncsDtsvNodeCategoryBitMap_ID, 0, (uns8 *)&bit_map, 4, reqfnc, MIB_VERR);
             }
         }
         else
         {
      ...
      else
      {
         ...
         if (pArgv->i_arg_type == NCSCLI_KEYWORD)
         {
             ...
             bit_map = (bit_set)?CAT_FILTER_SET_ALL_VAL:CAT_FILTER_NOSET_ALL_VAL;
             bit_map = htonl(bit_map);
             rc = dtsv_get_mib_oct(&mib, ncsDtsvServiceCategoryBitMap_ID, &get_val, &length, reqfnc, MIB_VERR);
             if (rc!= NCSCC_RC_FAILURE)
             {
                get_val = ntohl(get_val);
                dtsv_cfg_mib_arg(&mib, oids, SVC_INST_ID_LEN,  NCSMIB_TBL_DTSV_SVC_PER_NODE, cli_hdl, dtsv_hdl, NULL);
                if(rc == NCSCC_RC_NO_INSTANCE)
                    rc = dtsv_set_mib_oct(&mib, ncsDtsvServiceCategoryBitMap_ID, ncsDtsvServiceRowStatus_ID,(uns8 *)&bit_map, 4, reqfnc, MIB_VERR);
                else
                    rc = dtsv_set_mib_oct(&mib, ncsDtsvServiceCategoryBitMap_ID, 0, (uns8 *)&bit_map, 4, reqfnc, MIB_VERR);
             }
         }
         else
         {
...
*****************************************************************************/
uns32
dtsv_cef_cnf_severity(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   else
   {
      ...
      if (pArgv->i_arg_type == NCSCLI_KEYWORD)
      {
         ...
         if (rc != NCSCC_RC_FAILURE)
         {
            dtsv_cfg_mib_arg(&mib, oids, NODE_INST_ID_LEN, NCSMIB_TBL_DTSV_NODE, cli_hdl, dtsv_hdl, NULL);
            if(rc == NCSCC_RC_NO_INSTANCE)
              rc = dtsv_set_mib_oct(&mib, ncsDtsvNodeSeverityBitMap_ID, ncsDtsvNodeRowStatus_ID, (uns8 *)&bit_map, 1, reqfnc, MIB_VERR);
            else
              rc = dtsv_set_mib_oct(&mib, ncsDtsvNodeSeverityBitMap_ID, 0, (uns8 *)&bit_map, 1, reqfnc, MIB_VERR);
         }
      }
      else
      {
         ...
         pArgv = &arg_list->i_arg_record[5];
         m_DTS_CLI_SEV_FLTR(pArgv, bit_map);
         rc = dtsv_get_mib_oct(&mib, ncsDtsvServiceSeverityBitMap_ID,  &get_val, &length, reqfnc, MIB_VERR);
         if (rc != NCSCC_RC_FAILURE)
         {
            dtsv_cfg_mib_arg(&mib, oids, SVC_INST_ID_LEN, NCSMIB_TBL_DTSV_SVC_PER_NODE, cli_hdl, dtsv_hdl, NULL);
            if(rc == NCSCC_RC_NO_INSTANCE)
               rc = dtsv_set_mib_oct(&mib, ncsDtsvServiceSeverityBitMap_ID, ncsDtsvServiceRowStatus_ID, (uns8 *)&bit_map, 1, reqfnc, MIB_VERR);
            else
                rc = dtsv_set_mib_oct(&mib, ncsDtsvServiceSeverityBitMap_ID, 0, (uns8 *)&bit_map, 1, reqfnc, MIB_VERR);
         }
...
*****************************************************************************/
uns32
dtsv_cef_cnf_num_log_files(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   rc = dtsv_set_mib_int(&mib, ncsDtsvGlobalNumOfLogFiles_ID, 0,
      pArgv->cmd.intval, reqfnc, MIB_VERR);
...
*****************************************************************************/
uns32
dtsv_cef_enable_sequencing(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   rc = dtsv_set_mib_int(&mib, ncsDtsvGlobalLogMsgSequencing_ID, 0,
      NCS_SNMP_TRUE, reqfnc, MIB_VERR);
...
*****************************************************************************/
uns32
dtsv_cef_disable_sequencing(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
...
*****************************************************************************/
uns32
dtsv_cef_close_open_files(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   rc = dtsv_set_mib_int(&mib, ncsDtsvGlobalCloseOpenFiles_ID, 0,
      NCS_SNMP_TRUE, reqfnc, MIB_VERR);
...
*****************************************************************************/
uns32
dtsv_cef_dump_buff_log(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   oids[1] = 1;
   dtsv_cfg_mib_arg(&mib, oids, SVC_INST_ID_LEN, 
                NCSMIB_TBL_DTSV_CIRBUFF_OP, cli_hdl, dtsv_hdl, NULL);
...
*****************************************************************************/
uns32
dtsv_cef_set_dflt_plcy(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
   NCSMIB_ARG               mib;
   uns32                    cli_hdl  = cef_data->i_bindery->i_cli_hdl;
   uns32                    dtsv_hdl = cef_data->i_bindery->i_mab_hdl;
   ...
   if(m_NCS_STRCMP(pArgv->cmd.strval,"node") == 0)
   {
      ...
      rc = dtsv_set_mib_int_val(&mib, ncsDtsvNodeRowStatus_ID,
   ...
   else if(m_NCS_STRCMP(pArgv->cmd.strval,"service") == 0)
   {
      ...
      rc = dtsv_set_mib_int_val(&mib, ncsDtsvServiceRowStatus_ID,
                            NCSMIB_ROWSTATUS_DESTROY, reqfnc, MIB_VERR);
...
/*****************************************************************************
  ...
  NOTES:
*****************************************************************************/
uns32 dtsv_set_mib_int(NCSMIB_ARG       *mib,
                       uns32           pid,
                       uns32           r_status,
                       uns32           val,
...
/*****************************************************************************
  ...
  NOTES:
*****************************************************************************/
uns32 dtsv_set_mib_int_val(NCSMIB_ARG       *mib,
                       uns32           pid,
                       uns32           val,
                       NCSMIB_REQ_FNC   reqfnc,
                       char            *err)
...
/****************************************************************************
  ...
  Notes         :
****************************************************************************/
uns32 dtsv_send_cmd_mib_req(NCSMIB_ARG *mib, 
...
*****************************************************************************/
uns32
dtsv_cli_display(uns32 cli_hdl, char *str)
{
   NCSCLI_OP_INFO req;
   uns32 rc = NCSCC_RC_SUCCESS;
...
*****************************************************************************/
uns32
dtsv_cli_done(uns8 *string, uns32 status, uns32 cli_hdl)
{
   NCSCLI_OP_INFO req;
   uns32 rc = NCSCC_RC_SUCCESS;
...
/*****************************************************************************
  ...
  NOTES:
*****************************************************************************/
uns32 dtsv_get_mib_oct(NCSMIB_ARG      *mib,
                       uns32           pid,
                       NCSCONTEXT      val,
                       uns32           *len,
                       NCSMIB_REQ_FNC  reqfnc,
...
uns32 dtsv_chk_mib_rsp(char *err, NCSMIB_ARG *arg, uns32 cli_hdl)
{
   ...
   if(arg->rsp.i_status != NCSCC_RC_SUCCESS)
   {
      ...
      {
      case NCSCC_RC_NO_SUCH_TBL:      mib_err = "No such table"          ; break;     
      case NCSCC_RC_NO_OBJECT:        mib_err = "No object"              ; break;        
      case NCSCC_RC_NO_INSTANCE:      mib_err = "No instance"            ; break;      
      case NCSCC_RC_INV_VAL:          mib_err = "Invalid value"          ; break;          
      case NCSCC_RC_INV_SPECIFIC_VAL: mib_err = "Invalid specific value" ; break; 
      case NCSCC_RC_REQ_TIMOUT:       mib_err = "Request timeout"        ; break;       
      case NCSCC_RC_FAILURE:          mib_err = "Failure"                ; break;          
      default:                       break;
      }
