...
static uns32  dtsv_encode_cold_sync_null(DTS_CB *cb,
                                         NCS_MBCSV_CB_ENC *enc, 
                                         uns32 *num_of_obj);
static uns32  dtsv_encode_cold_sync_rsp_dts_svc_reg_tbl_config(DTS_CB *cb,
                                                      NCS_MBCSV_CB_ENC *enc, 
                                                      uns32 *num_of_obj);
static uns32  dtsv_encode_cold_sync_rsp_dta_dest_list_config(DTS_CB *cb,
                                                        NCS_MBCSV_CB_ENC *enc,
                                                        uns32 *num_of_obj);
static uns32  dtsv_encode_cold_sync_rsp_global_policy_config(DTS_CB *cb,
                                                      NCS_MBCSV_CB_ENC *enc,
                                                      uns32 *num_of_obj);
static uns32  dtsv_encode_cold_sync_rsp_dts_async_updt_cnt(DTS_CB *cb, 
                                                           NCS_MBCSV_CB_ENC *enc, 
                                                           uns32 *num_of_obj);
 ...
 */
const DTSV_ENCODE_CKPT_DATA_FUNC_PTR 
          dtsv_enc_ckpt_data_func_list[DTSV_CKPT_MSG_MAX] = 
{
 ...
 */
const DTSV_ENCODE_COLD_SYNC_RSP_DATA_FUNC_PTR 
          dtsv_enc_cold_sync_rsp_data_func_list[] = 
{
...
{
   return m_DTS_DBG_SINK(NCSCC_RC_SUCCESS, "dtsv_encode_cold_sync_null: Received encode for reo_type of 0. Do nothing");
}
...
static uns32  dtsv_encode_ckpt_dts_svc_reg_tbl_config(DTS_CB *cb, NCS_MBCSV_CB_ENC *enc)
{
   ...
   case NCS_MBCSV_ACT_ADD:
      /* Send data only*/
      status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_dts_svc_reg_tbl_config, &enc->io_uba, EDP_OP_TYPE_ENC, (NCSCONTEXT*)(long)enc->io_reo_hdl, &ederror); 
      break;
   ...
   case NCS_MBCSV_ACT_RMV:
      ...
      if(!encoded_dta_dest)
      {
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_ckpt_fls_svc_reg_tbl_config: Encode(DTA MDS_DEST) failed");
      }
   ...
   case NCS_MBCSV_ACT_UPDATE:
      ...
      if(!encoded_cli_map)
      {
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_ckpt_dts_svc_reg_tbl_config: Encode(cli_bit_map) failed");
      }
...
static uns32  dtsv_encode_ckpt_dta_dest_list_config(DTS_CB *cb, NCS_MBCSV_CB_ENC *enc)
{
   ...
   if(enc_svc == NULL)
     return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_ckpt_dta_dest_list_config: ncs_enc_reserve_space returns NULL");
    ...
    * matter.
    */
   if(ncs_encode_n_octets_in_uba(&enc->io_uba, (char *)cb->last_spec_loaded.svc_name, DTSV_SVC_NAME_MAX) != NCSCC_RC_SUCCESS)
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_ckpt_dta_dest_list_config: ncs_encode_n_octets_in_uba returns NULL");
   ...
   status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_dta_dest_list_config, &enc->io_uba, EDP_OP_TYPE_ENC, (NCSCONTEXT*)(long)enc->io_reo_hdl, &ederror);
...
static uns32  dtsv_encode_ckpt_dts_log_file_ckpt_config(DTS_CB *cb, NCS_MBCSV_CB_ENC *enc)
{
   ...
   status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_dts_log_ckpt_data_config, &enc->io_uba, EDP_OP_TYPE_ENC, (NCSCONTEXT*)(long)enc->io_reo_hdl, &ederror); 
   if(status != NCSCC_RC_SUCCESS)
   {
      /* Encode failed!!! */
      m_LOG_DTS_CHKOP(DTS_ASYNC_FAILED);
...
static uns32  dtsv_encode_ckpt_global_policy_config(DTS_CB *cb, NCS_MBCSV_CB_ENC *enc)
{
   ...
   if(!encoded_cli_map)
   {
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_ckpt_global_policy_config: Encode(cli_bit_map) failed");
   }   
   ncs_enc_claim_space(&enc->io_uba, sizeof(uns8));
   ...
   case NCS_MBCSV_ACT_UPDATE:
      /* Send entire data */
      status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_global_policy_config, &enc->io_uba, EDP_OP_TYPE_ENC, (NCSCONTEXT*)(long)enc->io_reo_hdl, &ederror); 
      break;
                     ...
                     NCS_MBCSV_CB_ENC *enc, uns32 *num_of_obj)
{
   ...
   while(dts_svc_reg_ptr != NULL) 
   {
      ...
      status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_dts_svc_reg_tbl_config, &enc->io_uba, EDP_OP_TYPE_ENC, dts_svc_reg_ptr, &ederror); 

      if( status != NCSCC_RC_SUCCESS )
      {
        /* Encode failed!!! */
        m_LOG_DTS_CHKOP(DTS_CSYNC_ENC_FAILED);
        return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_cold_sync_rsp_dts_svc_reg_tbl_config: Encode Failed");
      }
      file_list = &dts_svc_reg_ptr->device.log_file_list;
      status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_dts_file_list_config, &enc->io_uba, EDP_OP_TYPE_ENC, file_list, &ederror);
                 ...
                 NCS_MBCSV_CB_ENC *enc, uns32 *num_of_obj)
{
   ...
   while (dta_dest != NULL) 
   {
      ...
      status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_dta_dest_list_config, &enc->io_uba, EDP_OP_TYPE_ENC, dta_dest, &ederror);

      if( status != NCSCC_RC_SUCCESS )
      {
         /* Encode failed!!! */
         m_LOG_DTS_CHKOP(DTS_CSYNC_ENC_FAILED);
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_cold_sync_rsp_dta_dest_list_config: Encode failed");
      }
      /* Now encode the num of svcs associated with the DTA */
      enc_data = ncs_enc_reserve_space(&enc->io_uba, sizeof(uns32));
      if(enc_data == NULL)
        return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_cold_sync_rsp_dta_dest_list_config: ncs_enc_reserve_space returns NULL");
      ...
      while(svc_entry != NULL)
      {
         ...
         if(spec_entry != NULL)
         {
            ...
            if(ncs_encode_n_octets_in_uba(&enc->io_uba, spec_entry->svc_name, DTSV_SVC_NAME_MAX) != NCSCC_RC_SUCCESS)
               m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_cold_sync_rsp_dta_dest_list_config: ncs_encode_n_octets_in_uba returns failure");
         }
         else
         {
                  ...
                  NCS_MBCSV_CB_ENC *enc, uns32 *num_of_obj)
{
   ...
   if(enc_data == NULL)
     return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_cold_sync_rsp_global_policy_config: ncs_enc_reserve_space returns NULL");
   ...
   status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_global_policy_config, &enc->io_uba,  EDP_OP_TYPE_ENC, gp, &ederror); 

   if( status != NCSCC_RC_SUCCESS )
   {
       /* Encode failed!!! */
       m_LOG_DTS_CHKOP(DTS_CSYNC_ENC_FAILED);
       return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_cold_sync_rsp_global_policy_config: Encode failed");
   }
    ...
    if( status != NCSCC_RC_SUCCESS )
   {
       /* Encode failed!!! */
       m_LOG_DTS_CHKOP(DTS_CSYNC_ENC_FAILED);
       return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_encode_cold_sync_rsp_global_policy_config: Encode DTS_FILE_LIST failed");
   }
