   ...
   case NCS_MBCSV_ACT_ADD:
      {
         ...
         if((svc = (DTS_SVC_REG_TBL *)ncs_patricia_tree_get(&cb->svc_tbl, (const uns8*)&nt_key)) == NULL)
         {
            ...
            return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_dta_dest: No service entry in Patricia Tree");
         ...
         if((to_reg = (DTA_DEST_LIST *)ncs_patricia_tree_get(&cb->dta_list, (const uns8*)&dta_key)) == NULL)
         {
             ...
             if (ncs_patricia_tree_add(&cb->dta_list, (NCS_PATRICIA_NODE *)&to_reg->node) != NCSCC_RC_SUCCESS)
             {
                m_DTS_UNLK(&cb->lock);
                m_LOG_DTS_LOCK(DTS_LK_UNLOCKED,&cb->lock); 
                m_LOG_DTS_EVT(DTS_EV_DTA_DEST_ADD_FAIL, 0, m_NCS_NODE_ID_FROM_MDS_DEST(dta_key), (uns32)dta_key);
                if (NULL != to_reg)
                   m_MMGR_FREE_VCARD_TBL(to_reg);
                return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_dta_dest: Failed to add DTA list in Patricia tree");
             }
             m_LOG_DTS_EVT(DTS_EV_DTA_DEST_ADD_SUCC, 0, m_NCS_NODE_ID_FROM_MDS_DEST(dta_key), (uns32)dta_key);
         }
         else
         {
         ...
         if((spec_entry = (SYSF_ASCII_SPECS *)ncs_patricia_tree_get(&cb->svcid_asciispec_tree, (const uns8*)&spec_key)) == NULL)
         {
            ...
            fflush(stdout); 
            m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_dta_dest: ASCII_SPEC library couldn't be loaded. Check service name & version no. across ASCII_SPEC tables, registration parameters & library name.");
         }
         else
         {
   ...
   case NCS_MBCSV_ACT_UPDATE:
      {
         ...
         if((to_reg = (DTA_DEST_LIST *)ncs_patricia_tree_get(&cb->dta_list, (const uns8*)&dta_key)) == NULL)
         {
             m_DTS_UNLK(&cb->lock);
             m_LOG_DTS_LOCK(DTS_LK_UNLOCKED,&cb->lock);
             return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_dta_dest: NCS_MBCSV_ACT_UPDATE - DTA entry does not exist in Patricia Tree");
         }
         to_reg = (DTA_DEST_LIST *)((long)to_reg - DTA_DEST_LIST_OFFSET);
         to_reg->dta_addr = dtadest->dta_addr;
         to_reg->dta_up = dtadest->dta_up;
         ...
         if((to_reg = (DTA_DEST_LIST *)ncs_patricia_tree_get(&cb->dta_list, (const uns8*)&dta_key)) != NULL)
         {
            ...
            while(svc_entry != NULL)
            {
               ...
               if((svc = (DTS_SVC_REG_TBL *)ncs_patricia_tree_get(&cb->svc_tbl, (const uns8*)&nt_key)) == NULL)
               {
                  m_DTS_UNLK(&cb->lock);
                  m_LOG_DTS_LOCK(DTS_LK_UNLOCKED,&cb->lock);
                  m_LOG_DTS_EVT(DTS_EV_SVC_REG_NOTFOUND, svc_key.ss_svc_id, svc_key.node, (uns32)to_reg->dta_addr);
                  return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_dta_dest: Service entry not found in Patricia tree");
               } 
               /* remove dta from svc->v_cd_list */
               if((del_reg = (DTA_DEST_LIST *)dts_dequeue_dta(svc, to_reg)) == NULL) 
               {
                  m_LOG_DTS_EVT(DTS_EV_SVC_DTA_RMV_FAIL, svc_key.ss_svc_id, svc_key.node, (uns32)to_reg->dta_addr);
                  /* Don't return failure, continue rmeoving service frm dta*/
                  m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_dta_dest: Failed to remove dta entry frm svc->v_cd_list");
               }
               else
                  m_LOG_DTS_EVT(DTS_EV_SVC_DTA_RMV, svc_key.ss_svc_id, svc_key.node, (uns32)to_reg->dta_addr);
               ...
               {
                   m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_dta_dest: Unable to remove spec entry");                  
               }
               ...
               {
                  m_LOG_DTS_EVT(DTS_EV_DTA_SVC_RMV_FAIL, svc_key.ss_svc_id, svc_key.node, (uns32)to_reg->dta_addr); 
                  /* Don't return, continue*/
                  m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_dta_dest: Failed to delete svc from dta->svc_list"); 
               }
               else
                  m_LOG_DTS_EVT(DTS_EV_DTA_SVC_RMV, svc_key.ss_svc_id, svc_key.node, (uns32)to_reg->dta_addr);
               ...
               if((svc->dta_count == 0) && (svc->row_exist == FALSE))
               {
                  ...
                  m_LOG_DTS_EVT(DTS_EV_SVC_REG_ENT_RMVD, svc_key.ss_svc_id, svc_key.node, (uns32)to_reg->dta_addr); 
                  if(svc != NULL)
            ...
            if(to_reg->svc_list == NULL)
            {
               ...
               m_LOG_DTS_EVT(DTS_EV_DTA_DEST_RMV_SUCC, 0, m_NCS_NODE_ID_FROM_MDS_DEST(to_reg->dta_addr), (uns32)to_reg->dta_addr); 
               if (NULL != to_reg)
                 m_MMGR_FREE_VCARD_TBL(to_reg);
            }
         }
 ...
 *        in its v_cd_list. 
\**************************************************************************/
uns32  dtsv_ckpt_add_rmv_updt_svc_reg(DTS_CB *cb, DTS_SVC_REG_TBL *svcreg, DTS_FILE_LIST *file_list, NCS_MBCSV_ACT_TYPE action) 
{
   uns32             status = NCSCC_RC_SUCCESS;
   ...
   case NCS_MBCSV_ACT_ADD:
      {
             ...
             * fill the datastructure*/
             if((node_reg_ptr = (DTS_SVC_REG_TBL *)ncs_patricia_tree_get(&cb->svc_tbl, (const uns8*)&nt_key)) == NULL)
             {
                 ...
                 else
                 {
                  ...
                  /*  node_reg_ptr->device.cur_file_size = 0;  */
                 ...
                 if (ncs_patricia_tree_add(&cb->svc_tbl, (NCS_PATRICIA_NODE *)node_reg_ptr) != NCSCC_RC_SUCCESS)
                 {
                     ...
                     m_DTS_UNLK(&cb->lock);
                     m_LOG_DTS_LOCK(DTS_LK_UNLOCKED, &cb->lock);
                     m_LOG_DTS_SVCREG_ADD_FAIL(DTS_EV_SVC_REG_ENT_ADD_FAIL, key.ss_svc_id, key.node, 0);
                     m_LOG_DTS_EVT(DTS_EV_SVC_REG_FAILED, key.ss_svc_id, key.node, 0);
                     return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_svc_reg : Failed to add node registration in Patricia tree");
                 }
             ...
             else
             {
                ...
                if(ncs_patricia_tree_add(&cb->svc_tbl, (NCS_PATRICIA_NODE *)svc_ptr) != NCSCC_RC_SUCCESS)
                {
                   /* Attempt to add node was unsuccessful */
                   if(NULL != svc_ptr)
                   ...
                   m_LOG_DTS_SVCREG_ADD_FAIL(DTS_EV_SVC_REG_ENT_ADD_FAIL, key.ss_svc_id, key.node, 0);
                   m_LOG_DTS_EVT(DTS_EV_SVC_REG_FAILED, key.ss_svc_id, key.node, 0);
                   return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_svc_reg : Failed to add service entry in patricia tree");
                }
                m_LOG_DTS_EVT(DTS_EV_SVC_REG_ENT_ADD, key.ss_svc_id, key.node, 0);
                /*
                 Do not update the file list as an impact of R&A changes
                */
#if 0
                         ...
                         (svc_ptr->my_key.node == key.node))
                  {
                     switch(cb->cli_bit_map)
                     {
                        ...
                        case ncsDtsvNodeRowStatus_ID:
                           svc_ptr->svc_policy.category_bit_map = svcreg->svc_policy.category_bit_map;
                           svc_ptr->svc_policy.enable = svcreg->svc_policy.enable;
                           svc_ptr->svc_policy.severity_bit_map = svcreg->svc_policy.severity_bit_map;
                           break; 
                     ...
                     svc_ptr = (DTS_SVC_REG_TBL *)dts_get_next_svc_entry(&cb->svc_tbl, (SVC_KEY *)&nt_key);
                  } /*end of while */
   ...
   case NCS_MBCSV_ACT_RMV:
      {
         ...
         if (NULL != svcreg)
         {
            ...
            if((svc_ptr = (DTS_SVC_REG_TBL *)ncs_patricia_tree_get(&cb->svc_tbl, (const uns8*)&nt_key)) == NULL)
            {
               ...
               return m_DTS_DBG_SINK(NCSCC_RC_SUCCESS, "dtsv_ckpt_add_rmv_updt_svc_reg : Service entry not found in patricia tree");  
            }
            ...
            if(cb->svc_rmv_mds_dest == 0)
            {
               ...
               if((svc_ptr->dta_count == 0) && 
                  ((svc_ptr->row_exist == FALSE) ||(cb->cli_bit_map == ncsDtsvServiceRowStatus_ID) || (cb->cli_bit_map == ncsDtsvNodeRowStatus_ID)))
               {
               ...
               else
               {
                  ...
                  m_DTS_UNLK(&cb->lock);
                  m_LOG_DTS_LOCK(DTS_LK_UNLOCKED, &cb->lock);
                  return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_svc_reg: Either MDS_DEST for async update is wrong or database is inconsistent");
               }
            ...
            { 
               ...
               while(dta_entry != NULL)
               {
                  ...
                  {
                     if((svc_ptr = (DTS_SVC_REG_TBL *)dts_del_svc_frm_dta(to_reg, svc_ptr)) == NULL) 
                     {
                        m_DTS_UNLK(&cb->lock);
                        m_LOG_DTS_LOCK(DTS_LK_UNLOCKED, &cb->lock);
                        m_LOG_DTS_EVT(DTS_EV_DTA_SVC_RMV_FAIL, key.ss_svc_id, key.node, (uns32)to_reg->dta_addr);
                        m_LOG_DTS_EVT(DTS_EV_SVC_DEREG_FAILED, key.ss_svc_id, key.node, (uns32)to_reg->dta_addr);
                        return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_svc_reg: Unable to remove svc reg entry from dta's list");
                     }
                     m_LOG_DTS_EVT(DTS_EV_DTA_SVC_RMV, key.ss_svc_id, key.node, (uns32)to_reg->dta_addr);                  

                     /* Remove dta entry frm svc_ptr->v_cd_list */
                     if((to_reg = (DTA_DEST_LIST *)dts_dequeue_dta(svc_ptr, to_reg)) == NULL)
                     {
                        m_DTS_UNLK(&cb->lock);
                        m_LOG_DTS_LOCK(DTS_LK_UNLOCKED, &cb->lock);
                        m_LOG_DTS_EVT(DTS_EV_SVC_DTA_RMV_FAIL, key.ss_svc_id, key.node, (uns32)to_reg->dta_addr);
                        m_LOG_DTS_EVT(DTS_EV_SVC_DEREG_FAILED, key.ss_svc_id, key.node, (uns32)to_reg->dta_addr);
                        return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_svc_reg: Unable to remove adest entry");
                     }
                     ...
                     {
                        m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_svc_reg: Unable to remove spec entry");
                     }
...
uns32  dtsv_ckpt_add_rmv_updt_dts_log(DTS_CB *cb, DTS_LOG_CKPT_DATA *data, NCS_MBCSV_ACT_TYPE action)
{
   ...
   else if(data->key.ss_svc_id == 0) /*node policy device*/
   {
      ...
      { 
         m_DTS_UNLK(&cb->lock);
         m_LOG_DTS_LOCK(DTS_LK_UNLOCKED, &cb->lock);
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_dts_log: Node entry doesn't exist");
      }
      device = &svc->device;
   }
   else  /*service policy device*/
   {
      if((svc = (DTS_SVC_REG_TBL *)ncs_patricia_tree_get(&cb->svc_tbl, (const uns8*)&nt_key)) == NULL)
      {
         m_DTS_UNLK(&cb->lock);
         m_LOG_DTS_LOCK(DTS_LK_UNLOCKED, &cb->lock);
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_dts_log: Service entry doesn't exist");
      }
      device = &svc->device; 
   }
 ...
 * 
\**************************************************************************/
uns32  dtsv_ckpt_add_rmv_updt_global_policy(DTS_CB *cb, GLOBAL_POLICY *gp, DTS_FILE_LIST *file_list, NCS_MBCSV_ACT_TYPE action)
{
   uns32        status = NCSCC_RC_SUCCESS;
/*   OP_DEVICE    *device = NULL;
   DTS_LL_FILE  *file_node = NULL;*/
   ...
   case NCS_MBCSV_ACT_UPDATE:
      ...
      /*      cb->g_policy.device.cur_file_size = 0; */
                ...
                switch(cb->cli_bit_map)
                {
                   ...
                   case ncsDtsvGlobalSeverityBitMap_ID:
                        ...
                        if(NCSCC_RC_SUCCESS != dts_stby_global_filtering_policy_change(cb))
                        {
                           m_DTS_UNLK(&cb->lock);
                           m_LOG_DTS_LOCK(DTS_LK_UNLOCKED, &cb->lock);
                           return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_ckpt_add_rmv_updt_global_policy: Unable to apply global policy changes to all services");
                        }
                        break;
