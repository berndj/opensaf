...
extern const DTSV_ENCODE_CKPT_DATA_FUNC_PTR
          dtsv_enc_ckpt_data_func_list[DTSV_CKPT_MSG_MAX];

extern const DTSV_DECODE_CKPT_DATA_FUNC_PTR
...
uns32  dts_role_change(DTS_CB  *cb, SaAmfHAStateT haState)
{
    ...
    */
   if(((prev_haState == SA_AMF_HA_ACTIVE)&&(haState == SA_AMF_HA_STANDBY)) || ((prev_haState == SA_AMF_HA_STANDBY)&&(haState == SA_AMF_HA_QUIESCED)))
   {
   ...
   if((prev_haState == SA_AMF_HA_ACTIVE) && (haState == SA_AMF_HA_QUIESCED))
   {
       ...
       if(dts_mds_change_role(cb, haState) != NCSCC_RC_SUCCESS)
       {
           ...
           ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_MDS_EVT, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TIL", DTS_MDS_VDEST_CHG, haState);
        ...
        (prev_haState == SA_AMF_HA_QUIESCED)) && (haState == SA_AMF_HA_ACTIVE))
   {
      ...
      if(cb->in_sync != TRUE)
      {
         ...
         if(saAmfComponentErrorReport(cb->amf_hdl, &cb->comp_name, 0, SA_AMF_COMPONENT_RESTART, 0) != SA_AIS_OK)
            return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_role_change: Failed to send Error report to AMF for Active role assignment during Cold-Sync");
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_role_change: Active role assignment during Cold-Sync of Standby received");
      }
...
static uns32  dtsv_mbcsv_callback(NCS_MBCSV_CB_ARG *arg)
{
   ...
   if(cb->created != TRUE)
   {
      ...
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_mbcsv_cb: Failed to get cb handle. cb->created is FALSE ") ; 
   }
   ...
   case NCS_MBCSV_MSG_ASYNC_UPDATE:
      {
         ...
         status = 
    dtsv_enc_ckpt_data_func_list[arg->info.encode.io_reo_type](cb, &arg->info.encode);
      }
      break;
...
send_error:
      ...
      if(saAmfComponentErrorReport(cb->amf_hdl, &cb->comp_name, 0, SA_AMF_COMPONENT_RESTART, 0) != SA_AIS_OK)
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_mbcsv_process_err_ind: Failed to send Error report to AMF");
      break;
...
static uns32 dtsv_mbcsv_open_ckpt(DTS_CB  *cb)
{
   ...
   mbcsv_arg.info.open.i_client_hdl = cb->oac_hdl;
...
static uns32 dtsv_get_mbcsv_sel_obj(DTS_CB  *cb)
{
   ...
   if (NCSCC_RC_SUCCESS != ncs_mbcsv_svc(&mbcsv_arg))
   {
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_get_mbcsv_sel_obj: MBCSV get selection object operation failed");
   }
