...
static uns32  dtsv_decode_cold_sync_null(DTS_CB *cb,
                                         NCS_MBCSV_CB_DEC *dec, 
                                         uns32 num_of_obj);
static uns32  dtsv_decode_cold_sync_rsp_dts_svc_reg_tbl_config(DTS_CB *cb, 
                                                      NCS_MBCSV_CB_DEC *dec,
                                                      uns32 num_of_obj);
static uns32  dtsv_decode_cold_sync_rsp_dta_dest_list_config(DTS_CB *cb,
                                                        NCS_MBCSV_CB_DEC *dec,
                                                        uns32 num_of_obj);
static uns32  dtsv_decode_cold_sync_rsp_global_policy_config(DTS_CB *cb,
                                                      NCS_MBCSV_CB_DEC *dec,
                                                      uns32 num_of_obj);
static uns32  dtsv_decode_cold_sync_rsp_dts_async_updt_cnt(DTS_CB *cb,
                                                           NCS_MBCSV_CB_DEC *dec,
                                                           uns32 num_of_obj);
...
const DTSV_DECODE_CKPT_DATA_FUNC_PTR 
          dtsv_dec_ckpt_data_func_list[DTSV_CKPT_MSG_MAX] = 
{
 ...
 */
const DTSV_DECODE_COLD_SYNC_RSP_DATA_FUNC_PTR 
          dtsv_dec_cold_sync_rsp_data_func_list[] = 
{
...
static uns32  dtsv_decode_ckpt_dts_svc_reg_tbl_config(DTS_CB *cb, NCS_MBCSV_CB_DEC *dec)
{
   ...
   case NCS_MBCSV_ACT_ADD:
      /* Decode data only*/
      status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_dts_svc_reg_tbl_config,  &dec->i_uba, EDP_OP_TYPE_DEC, (DTS_SVC_REG_TBL**)&svc_reg_ptr, &ederror);
      break;
   ...
   case NCS_MBCSV_ACT_RMV:
      ...
      if(ptr == NULL)
      {
          ...
          return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_decode_ckpt_dts_svc_reg_tbl_config: Decode flatten space failed");
      }
...
static uns32  dtsv_decode_ckpt_dta_dest_list_config(DTS_CB *cb, NCS_MBCSV_CB_DEC *dec)
{
   ...
   case NCS_MBCSV_ACT_UPDATE:
      /* Send entire data */
      status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_dta_dest_list_config,  &dec->i_uba, EDP_OP_TYPE_DEC, (DTA_DEST_LIST**)&dta_dest_ptr, &ederror);
      break;
   ...
   ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_ASYNC_UPDT, DTS_FC_UPDT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLLL", DTS_ASYNC_DTA_DEST, dec->i_action, svc_key.node, svc_key.ss_svc_id, (uns32)dta_dest_ptr->dta_addr);
...
static uns32  dtsv_decode_ckpt_global_policy_config(DTS_CB *cb, NCS_MBCSV_CB_DEC *dec)
{
   ...
   {
       m_LOG_DTS_CHKOP(DTS_ASYNC_FAILED);
       return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_decode_ckpt_global_policy_config: Decode flatten space failed");
   }
   cb->cli_bit_map = ncs_decode_8bit(&ptr);
   ncs_dec_skip_space(&dec->i_uba, sizeof(uns8));
   ...
   case NCS_MBCSV_ACT_UPDATE:
      /* Send entire data */
      status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_global_policy_config, &dec->i_uba, EDP_OP_TYPE_DEC, (GLOBAL_POLICY**)&gp_ptr, &ederror);
      break;
   ...
   ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_ASYNC_UPDT, DTS_FC_UPDT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLLL", DTS_ASYNC_GLOBAL_POLICY, dec->i_action, 0, 0, 0);
...
static uns32  dtsv_decode_ckpt_log_file_ckpt_config(DTS_CB *cb, NCS_MBCSV_CB_DEC *dec)
{
   ...
   status = m_NCS_EDU_EXEC(&cb->edu_hdl, dtsv_edp_ckpt_msg_dts_log_ckpt_data_config, &dec->i_uba, EDP_OP_TYPE_DEC, (DTS_LOG_CKPT_DATA**)&data_ptr, &ederror);

   ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_ASYNC_UPDT, DTS_FC_UPDT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLLL", DTS_ASYNC_LOG_FILE, dec->i_action, data_ptr->key.node, data_ptr->key.ss_svc_id, 0);
...
uns32  dtsv_decode_cold_sync_rsp(DTS_CB *cb, NCS_MBCSV_CB_DEC *dec)
{
   ...
   status = 
      dtsv_dec_cold_sync_rsp_data_func_list[dec->i_reo_type](cb, dec, num_of_obj);
                 ...
                 NCS_MBCSV_CB_DEC *dec, uns32 num_of_obj)
{
   ...
   for (count = 0; count < num_of_obj; count++)
   {
      ...
      if( status != NCSCC_RC_SUCCESS )
      {
         /* Decode failed!!! */
         m_LOG_DTS_CHKOP(DTS_CSYNC_DEC_FAILED);
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE,"dtsv_decode_cold_sync_rsp_dta_dest_list_config: Decode failed");
      }
      ...
      {
         m_LOG_DTS_CHKOP(DTS_CSYNC_DEC_FAILED);
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_decode_cold_sync_rsp_dta_dest_list_config: Decode flatten space failed");
      }
      ...
      for(i = 0; i < svc_count; i++)
      {
         ...
         if(spec_present != 0)
         {
            ...
            dec_svc = ncs_dec_flatten_space(&dec->i_uba, (uns8*)&cb->last_spec_loaded.version, sizeof(uns16));
            if(dec_svc == NULL)
            {
               m_LOG_DTS_CHKOP(DTS_CSYNC_DEC_FAILED);
               return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_decode_cold_sync_rsp_dta_dest_list_config: Decode flatten space failed");
            ...
            if(ncs_decode_n_octets_from_uba(&dec->i_uba, (char*)&cb->last_spec_loaded.svc_name, DTSV_SVC_NAME_MAX) != NCSCC_RC_SUCCESS)
               return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_decode_cold_sync_rsp_dta_dest_list_config: ncs_decode_n_octets_from_uba failed");
         }
         status = dtsv_ckpt_add_rmv_updt_dta_dest(cb, dta_dest_ptr, 
                  ...
                  NCS_MBCSV_CB_DEC *dec, uns32 num_of_obj)
{
   ...
   {
      m_LOG_DTS_CHKOP(DTS_CSYNC_DEC_FAILED);
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_decode_cold_sync_rsp_global_policy_config: Decode flatten space failed");
   }
