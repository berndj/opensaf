...
uns32 dts_queue_seq_msg(DTS_CB *inst, DTSV_MSG *msg)
{
    ...
    */
    ...
    {
        return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_queue_seq_msg: Sequence buffer pointer is NULL. Unable to queue message");
    }  
    
    inst->s_buffer.arr_ptr[inst->s_buffer.num_msgs].ms_time.seconds 
        = msg->data.data.msg.log_msg.hdr.time.seconds;
    ...
    */
    if (++inst->s_buffer.num_msgs >= SEQ_ARR_MAX_SIZE)
    {
        if (dts_dump_seq_msg(inst, FALSE) != NCSCC_RC_SUCCESS)
            return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_queue_seq_msg: Failed to dump messages in the sequencing buffer");
    }
...
uns32  dts_dump_seq_msg(DTS_CB *inst, NCS_BOOL all)
{
   ...
   if (inst->s_buffer.arr_ptr == NULL)
   {
       ...
       return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_dump_seq_msg: Sequenc buffer pointer is NULL. Failed to dump messages");
   } 
...
uns32 dts_enable_sequencing(DTS_CB *inst)
{
   ...
   if (inst->s_buffer.arr_ptr == NULL)
   {
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_enable_sequencing: Sequenc buffer pointer is NULL. Failed to dump messages");
   }
