      ...
      if(count > 0)
      {
         ...
         if (m_NCS_SEL_OBJ_ISSET(ncs_amf_sel_obj, &readfds))
         {
            ...
            if (saf_status != SA_AIS_OK)
            {
               ...
               "dts_do_evts: AMF Dispatch failing");
            }
...
{  
       ...
       (SA_AMF_HA_QUIESCED == inst->ha_state))*/
   {
      ...
      case DTA_FLOW_CONTROL:
         ...
         {
            m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_do_evt: Failed to send ACK for Flow control message");
         }
         break;

      case DTS_CONGESTION_HIT:
         ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_FLOW_UP, 0, NCSFL_LC_HEADLINE, NCSFL_SEV_WARNING, "TLL", m_NCS_NODE_ID_FROM_MDS_DEST(msg->dest_addr), (uns32)msg->dest_addr);  
         break;

      case DTS_CONGESTION_CLEAR:
         ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_FLOW_DOWN, 0, NCSFL_LC_HEADLINE, NCSFL_SEV_WARNING, "TLL", m_NCS_NODE_ID_FROM_MDS_DEST(msg->dest_addr), (uns32)msg->dest_addr);
         break;
#endif

      default:
         rc = m_DTS_DBG_SINK(NCSCC_RC_FAILURE, 
            "dts_do_evt: Wrong message type received");
         break;
...
uns32 dts_handle_dta_event (DTSV_MSG *msg)
{
   ...
   {
   case NCSMDS_DOWN:
      m_LOG_DTS_DBGSTRLL(DTS_SERVICE, "Received DTA down event for :", 
          msg->node, (uns32)msg->dest_addr);
      dts_set_dta_up_down(msg->node, msg->dest_addr, FALSE);
      break;
      
   case NCSMDS_UP  :
       m_LOG_DTS_DBGSTRLL(DTS_SERVICE, "Received DTA up event for :", 
          msg->node, (uns32)msg->dest_addr);
...
uns32 dts_register_service (DTSV_MSG *msg)
{
   ...
   m_LOG_DTS_EVT(DTS_EV_SVC_REG_REQ_RCV,
                 msg->data.data.reg.svc_id, msg->node, (uns32)msg->dest_addr);
   ...
   if ((node_reg = (DTS_SVC_REG_TBL *)ncs_patricia_tree_get(&inst->svc_tbl, (const uns8*)&nt_key)) == NULL)
   {
       ...
       if (ncs_patricia_tree_add(&inst->svc_tbl, (NCS_PATRICIA_NODE *)node_reg) != NCSCC_RC_SUCCESS)
       {
          ...
          m_LOG_DTS_EVT(DTS_EV_SVC_REG_FAILED,
                         key.ss_svc_id, key.node, (uns32)msg->dest_addr);
          ...
          m_DTS_UNLK(&inst->lock);
          m_LOG_DTS_LOCK(DTS_LK_UNLOCKED,&inst->lock);
          return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_register_service: Failed to add node registration in Patricia tree");
       }
   }
   ...
   { 
       ...
       if (dts_find_dta(svc, &dta_key) != NULL)
       {
           m_DTS_UNLK(&inst->lock);
           m_LOG_DTS_LOCK(DTS_LK_UNLOCKED,&inst->lock);
           m_LOG_DTS_EVT(DTS_EV_SVC_ALREADY_REG,
                 key.ss_svc_id, key.node, (uns32)msg->dest_addr);
       ...
       if(dta_present == FALSE)
       {
          ...
          if (ncs_patricia_tree_add(&inst->dta_list, (NCS_PATRICIA_NODE *)&to_reg->node) != NCSCC_RC_SUCCESS)
          {
             ...
             m_DTS_UNLK(&inst->lock);
             m_LOG_DTS_LOCK(DTS_LK_UNLOCKED,&inst->lock);
             return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_register_service: Failed to add DTA list in Patricia tree");
          }
   ...
   else
   {
      ...
      if(dta_present == FALSE)
      {
         ...
         if (ncs_patricia_tree_add(&inst->dta_list, (NCS_PATRICIA_NODE *)&to_reg->node) != NCSCC_RC_SUCCESS)
          {
             m_LOG_DTS_EVT(DTS_EV_DTA_DEST_ADD_FAIL, key.ss_svc_id, key.node, (uns32)to_reg->dta_addr);
             m_LOG_DTS_EVT(DTS_EV_SVC_REG_FAILED, key.ss_svc_id, key.node, (uns32)msg->dest_addr);
             if (NULL != to_reg)
                m_MMGR_FREE_VCARD_TBL(to_reg);
      ...
      if (ncs_patricia_tree_add(&inst->svc_tbl, (NCS_PATRICIA_NODE *)svc) != NCSCC_RC_SUCCESS)
      {
          ...
          if(dts_del_svc_frm_dta(to_reg, svc) == NULL)
             m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_register_service: No service entry in dta's service list");
       ...
       if(dtsv_send_ckpt_data(inst, NCS_MBCSV_ACT_ADD, (MBCSV_REO_HDL)(long)svc, DTSV_CKPT_DTS_SVC_REG_TBL_CONFIG, NCS_MBCSV_SND_SYNC) != NCSCC_RC_SUCCESS)
          m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_service_register: MDS Sync send to Stby failed for Service registration");
   ...
   if(dtsv_send_ckpt_data(inst, NCS_MBCSV_ACT_ADD, (MBCSV_REO_HDL)(long)to_reg, DTSV_CKPT_DTA_DEST_LIST_CONFIG, NCS_MBCSV_SND_SYNC) != NCSCC_RC_SUCCESS)
    ...
    */
   if((spec_entry = (SYSF_ASCII_SPECS *)ncs_patricia_tree_get(&dts_cb.svcid_asciispec_tree, (const uns8*)&spec_key)) != NULL)
   {
   ...
   {
      m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_register_service: ASCII_SPEC with given svc name & version couldn't be associated properly. Cross-check the svc_name and version used in registration, ASCII_SPEC table and library name");
   }
   ...
   /* Now configure the service which is just registered with DTS */
    ...
    m_LOG_DTS_EVT(DTS_EV_SVC_REG_SUCCESSFUL,
                 key.ss_svc_id, key.node, (uns32)dta_key);
...
uns32 dts_unregister_service (DTSV_MSG *msg)
{
   ...
   m_LOG_DTS_EVT(DTS_EV_SVC_DE_REG_REQ_RCV,
                 msg->data.data.reg.svc_id, msg->node, (uns32)msg->dest_addr);

   if (((node = (DTS_SVC_REG_TBL *)ncs_patricia_tree_get(&inst->svc_tbl, 
         (const uns8*)&nt_key)) == NULL) || (node->dta_count == 0))
   {
       ...
       m_DTS_UNLK(&inst->lock);
       m_LOG_DTS_LOCK(DTS_LK_UNLOCKED,&inst->lock);
       return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_unregister_service: Service does not exist in the service registration table");
   }
   ...
   if((to_reg = (DTA_DEST_LIST *)dts_find_dta(node, &vkey)) == NULL)
   {
      ...
      m_DTS_UNLK(&inst->lock);
      m_LOG_DTS_LOCK(DTS_LK_UNLOCKED,&inst->lock);
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_unregister_service: DTA adest entry doesn't exist in the svc->v_cd_list"); 
   }
   ...
   if((svc = (DTS_SVC_REG_TBL *)dts_del_svc_frm_dta(to_reg, node)) == NULL)  
   {
      ...
      m_DTS_UNLK(&inst->lock);
      m_LOG_DTS_LOCK(DTS_LK_UNLOCKED,&inst->lock);
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_unregister_service: Unable to remove svc reg entry from dta's list");
   }
   ...
   if(to_reg->svc_list == NULL)
   {
      if(ncs_patricia_tree_del(&inst->dta_list, (NCS_PATRICIA_NODE *)&to_reg->node) != NCSCC_RC_SUCCESS)
      {
         ...
         m_DTS_UNLK(&inst->lock);
         m_LOG_DTS_LOCK(DTS_LK_UNLOCKED,&inst->lock);
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_unregister_service: Unable to remove DTA entry frm patricia tree");  
      }
   ...
   m_LOG_DTS_EVT(DTS_EV_SVC_DEREG_SUCCESSFUL,
                 key.ss_svc_id, key.node, (uns32)vkey);
...
uns32 dts_log_data (DTSV_MSG *msg)
{
       ...
       (msg->seq_msg == FALSE))
   {
       if (NCSCC_RC_SUCCESS != dts_queue_seq_msg(inst, msg))
       {
           ...
           return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_log_data: Unable to Queue message into sequencing buffer");
       }
   ...
   if((node = (DTS_SVC_REG_TBL *)ncs_patricia_tree_get(&inst->svc_tbl, (const uns8*)&nt_key)) == NULL)
   {
      ...
      ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_EVT, DTS_FC_EVT,
        NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLL", DTS_EV_LOG_SVC_KEY_WRONG, msg->data.data.msg.log_msg.hdr.ss_id, msg->node, (uns32)msg->dest_addr); 
   ...
   if(spec == NULL)
   {
      ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_LOG_ERR2, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILL", DTS_SPEC_NOT_REG, 0, msg->data.data.msg.log_msg.hdr.ss_id);
      m_MMGR_FREE_OCT(msg->data.data.msg.log_msg.hdr.fmat_type);
      return NCSCC_RC_FAILURE;
   }
   ...
   {
      rc = dtsv_log_msg(msg, &inst->g_policy.g_policy, 
                        &inst->g_policy.device, GLOBAL_FILE, spec);
   }
   else
   {
...
/**************************************************************************\
 ...
 Notes:  
\**************************************************************************/
uns32 dtsv_log_msg(DTSV_MSG *msg, 
                   POLICY *policy, 
                   OP_DEVICE *device, 
                   uns8 file_type,
   ...
   if ((policy->log_dev & LOG_FILE) == LOG_FILE)
   {
      ...
      if ((device->cur_file_size == 0) || 
          ...
          (device->cur_file_size > 
           (policy->log_file_size * 1024)))
      {
         m_DTS_ADD_NEW_FILE(device);
         if (0 == (device->cur_file_size = dts_new_log_file_create(m_DTS_LOG_FILE_NAME(device), &key, file_type))) 
            return  m_DTS_DBG_SINK(NCSCC_RC_FAILURE, 
         ...
         m_DTSV_SEND_CKPT_UPDT_ASYNC(&dts_cb, NCS_MBCSV_ACT_ADD, (MBCSV_REO_HDL)(long)&data, DTSV_CKPT_DTS_LOG_FILE_CONFIG);

         if(m_DTS_NUM_LOG_FILES(device) > dts_cb.g_policy.g_num_log_files)
         {
            /*delete oldest file */
            m_DTS_RMV_FILE(device);
            m_DTSV_SEND_CKPT_UPDT_ASYNC(&dts_cb, NCS_MBCSV_ACT_RMV, (MBCSV_REO_HDL)(long)&data, DTSV_CKPT_DTS_LOG_FILE_CONFIG);
         } 
         /* 
   ...
   if ((policy->log_dev & CIRCULAR_BUFFER) == CIRCULAR_BUFFER)
   {
       ...
       if(device->cir_buffer.buff_allocated == FALSE)
       {
          if (dts_circular_buffer_alloc(&device->cir_buffer, policy->cir_buff_size) == NCSCC_RC_SUCCESS)
             ...
             return  m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_log_msg: Circular buffer unallocated. Unable to allocate buffer according to policy settings");
       }
...
void dts_global_policy_set(GLOBAL_POLICY *gpolicy)
{
   ...
   {
       if (dts_circular_buffer_alloc(&gpolicy->device.cir_buffer,
           GLOBAL_CIR_BUFF_SIZE) == NCSCC_RC_SUCCESS)
           m_LOG_DTS_CBOP(DTS_CBOP_ALLOCATED, 0 , 0);
   }
   else
...
/**************************************************************************\
 ...
 Notes:  
\**************************************************************************/
void dts_default_node_policy_set(POLICY *npolicy, 
                                 OP_DEVICE *device, 
                                 uns32 node_id)
{
   ...
   if (npolicy->log_dev == CIRCULAR_BUFFER)
   {
       if (dts_circular_buffer_alloc(&device->cir_buffer, 
           NODE_CIR_BUFF_SIZE) == NCSCC_RC_SUCCESS)
...
/**************************************************************************\
 ...
 Notes:  
\**************************************************************************/
uns32 dts_send_filter_config_msg(DTS_CB *inst, 
                                 DTS_SVC_REG_TBL  *svc, 
                                 DTA_DEST_LIST  *dta)
{
    DTSV_MSG               msg;
    if(dta == NULL)
        return m_DTS_DBG_SINK(NCSCC_RC_FAILURE,
       "dts_send_filter_config_msg: NULL pointer passed");
    ...
    if (dts_mds_send_msg(&msg, dta->dta_addr, inst->mds_hdl) != NCSCC_RC_SUCCESS)
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, 
       "dts_send_filter_config_msg: MDS send message failed.");
...
uns32 dts_close_opened_files (void)
{
    ...
    m_DTSV_SEND_CKPT_UPDT_ASYNC(inst, NCS_MBCSV_ACT_UPDATE, (MBCSV_REO_HDL)(long)&inst->g_policy, DTSV_CKPT_GLOBAL_POLICY_CONFIG);
...
\**************************************************************************/
uns32
dts_create_new_pat_entry(DTS_CB       *inst,
                         ...
                         uns8         log_level)
{
    ...
    *node = m_MMGR_ALLOC_SVC_REG_TBL;
    ...
    else
    {
        ...
        {
           m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_create_new_pat_entry: No node entry exists for service entry");
           (*node)->my_node = NULL;
        }
        else
...
static uns32 dts_stby_initialize(DTS_CB *cb)
{
    ...
    else
    {
       ...
       while(node != NULL)
       {
          ...
          {
             if((cb->dflt_plcy.node_dflt.per_node_logging == NCS_SNMP_TRUE) &&
                (key.ss_svc_id == 0))
             {
    ...
    while(dta_reg != NULL)
    {
       ...
       if(dta_reg->svc_list != NULL)
       {
          ...
          if(dts_fail_over_enc_msg(msg) == NCSCC_RC_FAILURE)
          {
             ...
             if(NULL != msg)
                m_MMGR_FREE_DTSV_MSG(msg);
             return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_stby_update_dta_config: Failed to encode msg for fail-over");
          }
...
NCSCONTEXT dts_del_svc_frm_dta(DTA_DEST_LIST *dta, DTS_SVC_REG_TBL *svc)
{
   ...
   if((svc_entry->svc->my_key.node == svc->my_key.node)&&(svc_entry->svc->my_key.ss_svc_id ==  svc->my_key.ss_svc_id))  
   {
   ...
   while((svc_entry != NULL) && (svc_entry->next_in_dta_entry != NULL)) 
   {
       ...
       if((tmp->svc->my_key.node == svc->my_key.node) && (tmp->svc->my_key.ss_svc_id ==  svc->my_key.ss_svc_id))
       {
          /* Store the svc_reg ptr for return */
          svc_ptr = tmp->svc;
          /*Adjust the linked list*/
...
/**************************************************************************\
 ...
 Notes:    None.
\**************************************************************************/
uns32 dts_del_spec_frm_svc(DTS_SVC_REG_TBL *svc, MDS_DEST dta_addr, 
                           SPEC_CKPT *ver)
{
...
void dts_print_dta_dest_pat(void)
{
   ...
   while(dta_dest != NULL)
   {
      ...
      printf("\nDta entry : dta_addr={with node_id=%d, dta_key=%d}, dta_up=%d, updt_req=%d, dta_num_svcs=%d}\n", m_NCS_NODE_ID_FROM_MDS_DEST(dta_dest->dta_addr), (uns32)dta_dest->dta_addr, dta_dest->dta_up, dta_dest->updt_req, dta_dest->dta_num_svcs);
...
void dts_print_svc_reg_pat(DTS_CB *cb, FILE *fp)
{
   ...
   while(svc != NULL)
   {
      ...
      fprintf(fp, "\nPer_node_logging: %d, RowStatus: %d, Row Exists: %d", svc->per_node_logging, svc->row_status, svc->row_exist);

      fprintf(fp, "\nPolicy info:\n------------------");
      fprintf(fp, "\n\tLog Dev: %d\n\tLog File Size: %u\n\tLog File format: %d\n\tCircular Buffer Size: %u\n\tBuff_Log_Fmt: %d\n\tLogging Enable(1-TRUE, 2-FALSE): %d\n\tCategory_Bit_Map:%u\n\tSeverity_Bit_Map:%u", svc->svc_policy.log_dev, svc->svc_policy.log_file_size,  svc->svc_policy.file_log_fmt, svc->svc_policy.cir_buff_size, svc->svc_policy.buff_log_fmt, svc->svc_policy.enable, svc->svc_policy.category_bit_map, svc->svc_policy.severity_bit_map);

      fprintf(fp, "\nDevice info:\n--------------");
      fprintf(fp, "\n\tFile_Open(0-FALSE, 1-TRUE): %d\n\tCurrent File Size: %u\n\tNew File: %d\n\tlast_rec_id: %d\n\tfile_log_fmt_change: %d\n\tbuff_log_fmt_change: %d\n", svc->device.file_open, svc->device.cur_file_size, svc->device.new_file, svc->device.last_rec_id, svc->device.file_log_fmt_change, svc->device.buff_log_fmt_change);
      ...
      {
         fprintf(fp, "\n\tDTA Info:\n\t------------"); 
         fprintf(fp, "\n\tdta_addr:{node_id= %d, dta_key= %d}", m_NCS_NODE_ID_FROM_MDS_DEST(dta_entry->dta->dta_addr), (uns32)dta_entry->dta->dta_addr);
         dta_entry = dta_entry->next_in_svc_entry;
      }
...
void dts_print_cb(DTS_CB *cb, FILE *fp)
{
   ...
   fprintf(fp, "\nPolicy info:\n------------------"); 
   fprintf(fp, "\n\tLog Dev: %d\n\tLog File Size: %u\n\tLog File format: %d\n\tCircular Buffer Size: %u\n\tBuff_Log_Fmt: %d\n\tLogging Enable(1-TRUE, 2-FALSE): %d\n\tCategory_Bit_Map:%u\n\tSeverity_Bit_Map:%u", cb->g_policy.g_policy.log_dev, cb->g_policy.g_policy.log_file_size,  cb->g_policy.g_policy.file_log_fmt, cb->g_policy.g_policy.cir_buff_size, cb->g_policy.g_policy.buff_log_fmt, cb->g_policy.g_policy.enable, cb->g_policy.g_policy.category_bit_map, cb->g_policy.g_policy.severity_bit_map);

   fprintf(fp, "\nDevice info:\n--------------");
   fprintf(fp, "\n\tFile_Open(0-FALSE, 1-TRUE): %d\n\tCurrent File Size: %u\n\tNew File: %d\n\tlast_rec_id: %d\n\tfile_log_fmt_change: %d\n\tbuff_log_fmt_change: %d\n", cb->g_policy.device.file_open, cb->g_policy.device.cur_file_size, cb->g_policy.device.new_file, cb->g_policy.device.last_rec_id, cb->g_policy.device.file_log_fmt_change, cb->g_policy.device.buff_log_fmt_change);
   ...
   while(lib_entry != NULL)
   {
      ...
      lib_entry =  (ASCII_SPEC_LIB *)ncs_patricia_tree_getnext(&cb->libname_asciispec_tree, lib_entry->lib_name);
   }
   ...
   {
      fprintf(fp, "\nSvc-id: %d, version: %d, use_count: %d spec_name: %s", spec_struct->key.svc_id, spec_struct->key.ss_ver, spec_struct->use_count, spec_struct->ss_spec->svc_name);
      spec_struct =  (SYSF_ASCII_SPECS *)ncs_patricia_tree_getnext(&cb->svcid_asciispec_tree, (const uns8*)&spec_struct->key);
   }
