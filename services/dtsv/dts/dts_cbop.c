...
static uns32 
dtsv_cirbuff_op_set_obj(NCSCONTEXT cb, NCSMIB_ARG *arg, 
                           NCSMIB_VAR_INFO *var_info, NCS_BOOL flag);
static uns32 
dtsv_cirbuff_op_get_obj(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data);

static uns32 
dtsv_extract_cirbuff_op_obj(NCSMIB_PARAM_VAL *param_val, NCSMIB_VAR_INFO *var_info,
                      NCSCONTEXT data, NCSCONTEXT buffer);
static uns32 
dtsv_next_cirbuff_op_obj(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data, 
...
static uns32 dump_global_buffer_log(DTS_CB  *inst, uns8 device)
{
   ...
   {
       if(dts_dump_log_to_op_device(&inst->g_policy.device.cir_buffer, 
           device, NULL) == NCSCC_RC_SUCCESS)
       {
           m_LOG_DTS_CBOP(DTS_CBOP_DUMPED, 0 , 0);
           return NCSCC_RC_SUCCESS;
       }
...
uns32 dts_dump_log_to_op_device(CIR_BUFFER *cir_buff, uns8 device, char *file)
{
   ...
   if (found == FALSE)
      return  m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_dump_log_to_op_device: You should not come to this place. SERIOUS issue. No buffer found to be in use");
   ...
   for (i = 0; i < NUM_BUFFS; i++)
   {
      ...
      else if (device == OUTPUT_CONSOLE)
      {
          ...
          return  m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_dump_log_to_op_device: Device type received is not correct");
...
uns32 dts_circular_buffer_alloc(CIR_BUFFER *cir_buff, uns32 buffer_size)
{
    ...
    cir_buff->buff_part[0].cir_buff_ptr = 
        m_MMGR_ALLOC_CIR_BUFF(buffer_size * 1024);
    ...
    for (i = 0; i < NUM_BUFFS; i++)
    {
        cir_buff->buff_part[i].cir_buff_ptr = 
            (cir_buff->buff_part[0].cir_buff_ptr + (size * i));
...
/**************************************************************************\
 ...
 Notes:  
\**************************************************************************/
uns32 dts_dump_buffer_to_buffer(CIR_BUFFER *src_cir_buff, 
                                ...
                                uns32  number)
{
   ...
   if ((src_cir_buff->buff_allocated == FALSE) ||
       (dst_cir_buff->buff_allocated == FALSE))
        return NCSCC_RC_FAILURE;
