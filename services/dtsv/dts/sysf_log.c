...
uns32 dts_ascii_spec_register (NCSFL_ASCII_SPEC* spec)
{
   ...
   {
      ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_SPEC_ERR_EVT, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLC", DTS_SPEC_SVC_NAME_NULL, 0, spec->ss_id, NULL);
      ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_SPEC_REG, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILCL", DTS_SPEC_REG_FAIL, spec->ss_id, spec->svc_name, spec->ss_ver);
      return m_DTS_DBG_SINK_SVC(NCSCC_RC_FAILURE,"dts_ascii_spec_register: Service Name string is NULL. Check your service name", spec->ss_id);
   }

   /* Check for the string length for svc_name, 
      should not be more than 15 chars*/ 
   if((m_NCS_STRLEN(spec->svc_name)+1)>DTSV_SVC_NAME_MAX)
   {
      ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_SPEC_ERR_EVT, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLC", DTS_SPEC_SVC_NAME_ERR, 0, spec->ss_id, NULL);
      ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_SPEC_REG, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILCL", DTS_SPEC_REG_FAIL , spec->ss_id, NULL, spec->ss_ver);
      return m_DTS_DBG_SINK_SVC(NCSCC_RC_FAILURE,"dts_ascii_spec_register: Service Name string size exceeds limits. Check your service name", spec->ss_id);
   ...
   for (i = 0; set[i].set_vals != NULL; i++)
   {
      if (set[i].set_id != i)
      {
         ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_SPEC_ERR_EVT, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLC", DTS_SPEC_SET_ERR, i, spec->ss_id, spec->svc_name);
         ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_SPEC_REG, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILCL", DTS_SPEC_REG_FAIL , spec->ss_id, spec->svc_name, spec->ss_ver);
         return m_DTS_DBG_SINK_SVC_NAME(NCSCC_RC_FAILURE,
     "dts_ascii_spec_register: String set ID mismatch. Check your strung set ID's", spec->svc_name);
      ...
      for (j = 0; str[j].str_val != NULL; j++)
      {
         if (str[j].str_id != j)
         {
            ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_SPEC_ERR_EVT, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLC", DTS_SPEC_STR_ERR, j, spec->ss_id, spec->svc_name);
            ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_SPEC_REG, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILCL", DTS_SPEC_REG_FAIL , spec->ss_id, spec->svc_name, spec->ss_ver);
            return m_DTS_DBG_SINK_SVC_NAME(NCSCC_RC_FAILURE,
            "dts_ascii_spec_register: String Index ID mismatched. Check your string index ID's", spec->svc_name);
   ...
   for (i = 0; fmat[i].fmat_str != NULL; i++)
   {
      if (fmat[i].fmat_id != i)
      {
         ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_SPEC_ERR_EVT, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLC", DTS_SPEC_FMAT_ERR, i, spec->ss_id, spec->svc_name);
         ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_SPEC_REG, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILCL", DTS_SPEC_REG_FAIL , spec->ss_id, spec->svc_name, spec->ss_ver);
         return m_DTS_DBG_SINK_SVC_NAME(NCSCC_RC_FAILURE,
         "dts_ascii_spec_register: String format ID mismatch. Check your string format ID's", spec->svc_name);
      }
   }
   spec->fmat_set_cnt = i;
   ...
   if(ncs_patricia_tree_add(&dts_cb.svcid_asciispec_tree, (NCS_PATRICIA_NODE *)spec_entry) != NCSCC_RC_SUCCESS)
   {
      ...
      return m_DTS_DBG_SINK_SVC_NAME(NCSCC_RC_FAILURE, "dts_ascii_spec_register: Failed to add patricia tree entry", spec->svc_name);
   }
   ...
   ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_SPEC_REG, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILCL", DTS_SPEC_REG_SUCC , spec->ss_id, spec->svc_name, spec->ss_ver);
...
*****************************************************************************/
NCSCONTEXT dts_ascii_spec_load(char *svc_name, uns16 version, 
   ...
   if (reg_unreg_routine != NULL)
   {
      ...
      else /* Reg/De-reg successful */
      {
         if(action == DTS_SPEC_LOAD) /* Reg event */
         {
            ...
            if (ncs_patricia_tree_add(&dts_cb.libname_asciispec_tree, (NCS_PATRICIA_NODE *)lib_entry) != NCSCC_RC_SUCCESS)
            {
               m_MMGR_FREE_DTS_LIBNAME(lib_entry);
               m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_ascii_spec_load: Failed to add node to paticia tree");
               return NULL;
            }
            lib_entry->use_count++;
            return lib_entry;
...
uns32 dts_log_msg_to_str (DTA_LOG_MSG* logmsg, char *str, NODE_ID node, uns32 proc_id, uns32 *len, NCSFL_ASCII_SPEC *spec)
{
   ...
   if (spec == NULL)
   {
      ...
      ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_LOG_ERR2, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILL", DTS_SPEC_NOT_REG, 0, msg->hdr.ss_id);
      return NCSCC_RC_FAILURE; 
   }
   ...
   sysf_sprintf((asc_tod + m_NCS_STRLEN(asc_tod)) ,".%.3d",msg->hdr.time.millisecs);
   ...
   {
      m_MMGR_FREE_OCT(msg->hdr.fmat_type);
      ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_LOG_ERR1, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLLC", DTS_INVALID_FMAT, msg->hdr.fmat_id, spec->fmat_set_cnt, msg->hdr.ss_id, spec->svc_name);
      return NCSCC_RC_FAILURE; 
   }
   ...
   {
      m_MMGR_FREE_OCT(msg->hdr.fmat_type);
      ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_LOG_ERR1, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLLC", DTS_FMAT_ID_MISMATCH, msg->hdr.fmat_id, fmat->fmat_id, msg->hdr.ss_id, spec->svc_name);
      return NCSCC_RC_FAILURE;
   }
   ...
   {
      return m_DTS_DBG_SINK_SVC_NAME(NCSCC_RC_FAILURE, 
     "dts_log_msg_to_str: Format string is NULL in ASCII_SPEC table for the received format ID.", spec->svc_name);
   }
   ...
   {
   case NCSFL_SEV_EMERGENCY: sev = "EMRGCY : "; break;
   case NCSFL_SEV_ALERT    : sev = "ALERT! : "; break;
   case NCSFL_SEV_CRITICAL : sev = "CRITCL : "; break;
   case NCSFL_SEV_ERROR    : sev = "ERROR  : "; break;
   case NCSFL_SEV_WARNING  : sev = "WRNING : "; break;
   case NCSFL_SEV_NOTICE   : sev = "NOTICE : "; break;
   case NCSFL_SEV_INFO     : sev = "INFORM : "; break;
   case NCSFL_SEV_DEBUG    : sev = "DEBUG  : "; break;
   default:
      m_DTS_DBG_SINK_SVC_NAME(NCSCC_RC_FAILURE, "dts_log_msg_to_str: Message Severity is wrong", spec->svc_name);
      sev = "SUBSYS ERR"; break;
   }
...
****************************************************************************/
   if (NCSCC_RC_SUCCESS != dts_get_and_return_val(temp_str, 
          fmat->fmat_type, asc_tod, &msg->uba, spec, set, args, &log_msg_len,
          logmsg->msg_fmat_ver))
   {
       ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_LOG_ERR, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLC", DTS_LOG_CONV_FAIL, 0, msg->hdr.ss_id, spec->svc_name);
       return NCSCC_RC_FAILURE; 
   }
      ...
      case 'I':
         {
            ...
            {
               ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_LOG_ERR1, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLLC", DTS_LOG_SETIDX_ERR, m_S_SET(idx), spec->str_set_cnt, spec->ss_id, spec->svc_name);
               return NCSCC_RC_FAILURE; 
            } 
            if(m_S_STR(idx) >= spec->str_set[m_S_SET(idx)].set_cnt) 
            {
               ncs_logmsg(NCS_SERVICE_ID_DTSV, DTS_LID_LOG_ERR1, DTS_FC_EVT, NCSFL_LC_EVENT, NCSFL_SEV_NOTICE, "TILLLC", DTS_LOG_STRIDX_ERR, m_S_STR(idx), spec->str_set[m_S_SET(idx)].set_cnt, spec->ss_id, spec->svc_name);
               return NCSCC_RC_FAILURE;
            }
