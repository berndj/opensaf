...
static uns32 dts_register_table(DTS_CB * inst, 
                                uns32 table_id, 
                                uns32 (*reg_table)(), 
                                uns32 *tbl_hdl);
...
*****************************************************************************/
uns32
ncs_dtsv_mib_request(NCSMIB_ARG *mib_args)
{
    DTS_CB *     inst = &dts_cb;
    uns32          rc = NCSCC_RC_SUCCESS;   
   
   if((mib_args->i_tbl_id >= NCSMIB_TBL_DTSV_BASE) &&
      (mib_args->i_tbl_id <= NCSMIB_TBL_DTSV_END))
   {
...
{   
    ...
    if (dts_register_table(inst, NCSMIB_TBL_DTSV_SCLR_GLOBAL, 
        dtsv_global_policy_table_register, 
        &inst->global_tbl_row_hdl) != NCSCC_RC_SUCCESS)
        return NCSCC_RC_FAILURE;

    /*  REGISTER NODE POLICY TABLE    */
    if (dts_register_table(inst, NCSMIB_TBL_DTSV_NODE, 
        dtsv_node_policy_table_register,
        &inst->node_tbl_row_hdl) != NCSCC_RC_SUCCESS)
        return NCSCC_RC_FAILURE;
    
    /*  REGISTER SERVICE PER NODE POLICY TABLE    */
    if (dts_register_table(inst, NCSMIB_TBL_DTSV_SVC_PER_NODE,
        dtsv_svc_policy_table_register,
        &inst->svc_tbl_row_hdl)  != NCSCC_RC_SUCCESS)
        return NCSCC_RC_FAILURE;
    
    /*  REGISTER CIRCULAR BUFFER OPERATION TABLE    */
    if (dts_register_table(inst, NCSMIB_TBL_DTSV_CIRBUFF_OP,
        dtsv_cirbuff_op_table_register,
        &inst->cirbuff_op_tbl_row_hdl) != NCSCC_RC_SUCCESS)
        return NCSCC_RC_FAILURE;

    /*  REGISTER CLI OP REQ COMMAND TABLE   */
    if(dts_register_table(inst, NCSMIB_TBL_DTSV_CMD, 
       NULL, /*Don't have any MIBLIB to be registered*/ 
...
uns32 dts_unregister_tables(DTS_CB * inst)
{
    ...
    if (dts_unregister_table(inst, NCSMIB_TBL_DTSV_SCLR_GLOBAL, 
        &inst->global_tbl_row_hdl) != NCSCC_RC_SUCCESS)
        return NCSCC_RC_FAILURE;

    /*  UN-REGISTER NODE POLICY TABLE    */
    if (dts_unregister_table(inst, NCSMIB_TBL_DTSV_NODE, 
        &inst->node_tbl_row_hdl) != NCSCC_RC_SUCCESS)
        return NCSCC_RC_FAILURE;

    /*  UN-REGISTER SERVICE PER NODE POLICY TABLE    */
    if (dts_unregister_table(inst, NCSMIB_TBL_DTSV_SVC_PER_NODE,
        &inst->svc_tbl_row_hdl) != NCSCC_RC_SUCCESS)
        return NCSCC_RC_FAILURE;

    /*  UN-REGISTER CIRCULAR BUFFER OPERATION TABLE    */
    if (dts_unregister_table(inst, NCSMIB_TBL_DTSV_CIRBUFF_OP,
        &inst->cirbuff_op_tbl_row_hdl) != NCSCC_RC_SUCCESS)
        return NCSCC_RC_FAILURE;

    /*  UN-REGISTER CLI OP REQ COMMAND TABLE   */
    if(dts_unregister_table(inst, NCSMIB_TBL_DTSV_CMD,
       &inst->cmd_op_tbl_row_hdl) != NCSCC_RC_SUCCESS)
       return NCSCC_RC_FAILURE;
...
/****************************************************************************
  ...
  NOTES:
*****************************************************************************/
uns32 dts_register_table(DTS_CB * inst, 
                         uns32 table_id, 
                         uns32 (*reg_table)(), 
                         uns32 *tbl_hdl)
{
    ...
    */
    if((table_id >= NCSMIB_TBL_DTSV_SCLR_GLOBAL) && 
       (table_id <= NCSMIB_TBL_DTSV_SVC_PER_NODE))
    {
   ...
   if(reg_table != NULL)
    ...
    if(ncsoac_ss(&dts_arg) != NCSCC_RC_SUCCESS)
        return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_register_table: Failed NCSOAC_SS_OP_ROW_OWNED for dtsv policy table");
...
uns32 dts_unregister_table(DTS_CB * inst, uns32 table_id, uns32 *tbl_hdl)
{
    ...
    if (*tbl_hdl != 0)
    {
        ...
        if(ncsoac_ss(&dts_arg) != NCSCC_RC_SUCCESS)
            return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_unregister_table: Failed to deregister dtsv policy tables");
...
**************************************************************************/
uns32
dtsv_policy_tbl_req(NCSMIB_ARG *args)
{
           ...
           case dtsvAsciiSpecReload:
              {
                 ...
                 if(m_DTS_SND_MSG(&gl_dts_mbx, msg,
                                  NCS_IPC_PRIORITY_LOW) != NCSCC_RC_SUCCESS)
                 {
                    m_DTS_UNLK(&inst->lock);
                    m_LOG_DTS_LOCK(DTS_LK_UNLOCKED,&inst->lock);
                    if(msg != NULL)
                       m_MMGR_FREE_DTSV_MSG(msg);
                     return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_policy_tbl_req: IPC send failed for spec reload"); 
                 }
           ...
           case dtsvPrintCurrConfig:
              {
                                  ...
                                  NCS_IPC_PRIORITY_LOW) != NCSCC_RC_SUCCESS)
                 {
                    ...
                    if(msg != NULL)
                       m_MMGR_FREE_DTSV_MSG(msg);
                     return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_policy_tbl_req: IPC send failed for display config");
                 }
                 status = NCSCC_RC_SUCCESS;
              }
              break;
...
/**************************************************************************
 ...
 Notes:  
**************************************************************************/
void dts_log_policy_change(DTS_SVC_REG_TBL *node,
                           POLICY * old_plcy,
                           POLICY * new_plcy)
{
...
/**************************************************************************
 ...
 Notes:  
**************************************************************************/
void dts_filter_policy_change(DTS_SVC_REG_TBL *node,
                           POLICY * old_plcy,
                           POLICY * new_plcy)
{
   DTS_CB  *inst = &dts_cb;
   ...
   if(node->my_key.ss_svc_id == 0)
   {
      ...
      while (((service = (DTS_SVC_REG_TBL *)dts_get_next_svc_entry(&inst->svc_tbl, &nt_key)) != NULL) && (service->my_key.node == node->my_key.node)) 
      {
...
**************************************************************************/
uns32 
dts_log_device_set(POLICY        *policy,
                   OP_DEVICE     *device,
                   uns8          old_value)
{   
    uns32   rc = NCSCC_RC_SUCCESS;
    ...
    */
    if ((policy->log_dev & CIRCULAR_BUFFER) &&
        (!(old_value & CIRCULAR_BUFFER)))
    {
        rc = dts_circular_buffer_alloc(&device->cir_buffer, 
               policy->cir_buff_size);
    }
    else if ((old_value & CIRCULAR_BUFFER) &&
             (!(policy->log_dev & CIRCULAR_BUFFER)))
    {
    ...
    */
    ...
    else if ((!(policy->log_dev & LOG_FILE)) &&
        (old_value & LOG_FILE))
    {
        device->new_file = TRUE;
        /*if (TRUE == device->file_open)
...
**************************************************************************/
uns32 
dts_buff_size_set(POLICY        *policy,
                  OP_DEVICE     *device,
                  uns32          old_value)
{   
    ...
    else 
    {
        ...
        {
            if (dts_buff_size_increased(&device->cir_buffer, 
                policy->cir_buff_size) != NCSCC_RC_SUCCESS)
                return NCSCC_RC_FAILURE;
        }
        else
        {
            if (dts_buff_size_decreased(&device->cir_buffer, 
                policy->cir_buff_size) != NCSCC_RC_SUCCESS)
                return NCSCC_RC_FAILURE;
        }
    }
