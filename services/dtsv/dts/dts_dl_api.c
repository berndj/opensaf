 ...
 *****************************************************************************/
uns32
dts_lib_req (NCS_LIB_REQ_INFO *req_info)
{
 ...
 *****************************************************************************/
uns32 
dts_lib_init (NCS_LIB_REQ_INFO *req_info)
{
   ...
   if(m_NCS_IPC_CREATE(&gl_dts_mbx) != NCSCC_RC_SUCCESS)
   {
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, 
         "dts_lib_init: Failed to create DTS mail box");  
   }
...

   {
      ...
      if(dts_lm( &arg ) != NCSCC_RC_SUCCESS)
      {
         ...
         m_NCS_IPC_RELEASE(&gl_dts_mbx, NULL);
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE,
            "dts_lib_init: Failed to create and init DTS CB");
      }
   }
                        ...
                        &task_handle) !=  NCSCC_RC_SUCCESS)
   {
      ...
      m_NCS_IPC_RELEASE(&gl_dts_mbx, NULL);
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, 
         "dts_lib_init: Failed to create DTS thread.");
   }
   ...
   if(m_NCS_TASK_START(task_handle) != NCSCC_RC_SUCCESS)
   {
      ...
      m_NCS_IPC_RELEASE(&gl_dts_mbx, NULL);
      return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, 
         "dts_lib_init: Failed to start DTS task");
   }
...
/*#if (DTS_SIM_TEST_ENV == 1)
   ...
   {*/
      ...
      {
         return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, 
            "dts_lib_destroy: Failed to destroy DTS CB"); 
      }
   /*}
#endif*/
...
\****************************************************************************/
uns32 
dts_apps_ascii_spec_load(uns8 *file_name, uns32 what_to_do)
{
    ...
    if (fp == NULL)
    {
        ...
        return m_DTS_DBG_SINK(NCSCC_RC_FAILURE,
            "dts_apps_ascii_spec_load: Unable to load the file");
    }

    /* continue till the you reach the end of the file */
    while (((nargs=fscanf(fp,"%s %s",lib_name, func_name))==2) && 
          (nargs != EOF))
    {
        ...
        if (reg_unreg_routine != NULL)
        {
            ...
            else
            {
                ...
                if (ncs_patricia_tree_add(&dts_cb.libname_asciispec_tree, (NCS_PATRICIA_NODE *)lib_entry) != NCSCC_RC_SUCCESS)
                {
                   ...
                   return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dts_apps_ascii_spec_load: Failed to add node to paticia tree");
                }
            }/*end of else*/ 
    ...
    { 
        /* log the error */ 
        m_DTS_DBG_SINK(NCSCC_RC_FAILURE,
                    "dts_apps_ascii_spec_load: Config file is corrupted.");
...
int32
dts_app_signal_install(int i_sig_num, SIG_HANDLR i_sig_handler)
{
