...
static uns32 mbcsv_enc_async_update(IMMD_CB *cb,NCS_MBCSV_CB_ARG *arg)
{
    ...
    immd_type_ptr = ncs_enc_reserve_space(&arg->info.encode.io_uba,
                                          sizeof(uns8));
...
static uns32 mbcsv_enc_msg_resp(IMMD_CB *cb,NCS_MBCSV_CB_ARG *arg)
{
    ...
    if ((cb->immd_sync_cnt==0)&&(cb->fevsSendCount==0))
    { /*initial start*/
    ...
    */
    ...
    sync_cnt_ptr= 
        ncs_enc_reserve_space(&arg->info.encode.io_uba,sizeof(uns32));
    assert(sync_cnt_ptr);
    ncs_enc_claim_space(&arg->info.encode.io_uba,sizeof(uns32));
