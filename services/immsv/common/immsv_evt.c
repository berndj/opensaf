...
static const char *immd_evt_names[] =
{
    ...
    "IMMD_EVT_ND2D_ADMO_HARD_FINALIZE2",
    "IMMD_EVT_CB_DUMP",
...
static void
immsv_evt_enc_att_val(NCS_UBAID* o_ub, IMMSV_EDU_ATTR_VAL* v, 
		      SaImmValueTypeT t)
{
    uns8* p8;
    uns32 helper32;
...
static void
immsv_evt_dec_att_val(NCS_UBAID* i_ub, IMMSV_EDU_ATTR_VAL* v, 
		      SaImmValueTypeT t)
{
    uns8* p8;
    uns32 helper32;
...
void
immsv_evt_free_att_val(IMMSV_EDU_ATTR_VAL* p, SaImmValueTypeT t)
{
    switch(t)
    {
        case SA_IMM_ATTR_SAINT32T: 
        ...
        default:
            TRACE_1("Incorrect value for SaImmValueTypeT:%u. "
                ...
                "SaImmAttrValuesT value ?", t);
            assert(0);
    }
}

static void
immsv_evt_enc_attr_def(NCS_UBAID* o_ub, IMMSV_ATTR_DEF_LIST* ad)
{
    if(!ad) return;
    uns8* p8;
...
static void
immsv_evt_enc_attr_mod(NCS_UBAID* o_ub, IMMSV_ATTR_MODS_LIST* p)
{
    if(!p) return;
    uns8* p8;
...
static void
immsv_evt_enc_attribute(NCS_UBAID* o_ub, IMMSV_ATTR_VALUES_LIST* p)
{
    if(!p) return;
    uns8* p8;
...
void
immsv_free_attr_list(IMMSV_EDU_ATTR_VAL_LIST* al, const SaImmValueTypeT avt)
{
    while(al) {
...
void 
immsv_free_attrmods(IMMSV_ATTR_MODS_LIST* p)
{
    while(p) {
        ...
        if(tmp->attrValue.attrValuesNumber) {
            ...
            if(tmp->attrValue.attrValuesNumber > 1) {
                immsv_free_attr_list(tmp->attrValue.attrMoreValues, 
                    tmp->attrValue.attrValueType);
            }
        }
        free(tmp);
...
static void
immsv_evt_dec_attrmods(NCS_UBAID* i_ub, IMMSV_ATTR_MODS_LIST** p)
{
    int depth=1;
    uns8 c8;
    ...
    do {
        ...
        *p = (IMMSV_ATTR_MODS_LIST *) calloc(1, sizeof(IMMSV_ATTR_MODS_LIST));
        ...
        if(attrValuesNumber) {
            immsv_evt_dec_att_val(i_ub, &((*p)->attrValue.attrValue), 
			    (*p)->attrValue.attrValueType);
            --attrValuesNumber;
...
static void
immsv_evt_dec_attributes(NCS_UBAID* i_ub, IMMSV_ATTR_VALUES_LIST** p)
{
    int depth=1;
    uns8 c8;
...
static uns32
immsv_evt_dec_name_list(NCS_UBAID* i_ub, IMMSV_OBJ_NAME_LIST** p)
{
    int depth=1;
    uns8* p8;
    uns8 local_data[8];
...
void
immsv_evt_free_name_list(IMMSV_OBJ_NAME_LIST* p)
{
    while(p) {
...
static void
immsv_evt_enc_class(NCS_UBAID* o_ub, IMMSV_CLASS_LIST* r)
{
    if(!r) return;
    uns8* p8;
...
static uns32
immsv_evt_dec_class(NCS_UBAID* i_ub, IMMSV_CLASS_LIST** r)
{
    int depth=1;
    uns8 c8;
...
void
immsv_evt_free_classList(IMMSV_CLASS_LIST* p)
{
    while(p) {
...
static void
immsv_evt_enc_impl(NCS_UBAID* o_ub, IMMSV_IMPL_LIST* q)
{
    if(!q) return;
    uns8* p8;
...
static uns32
immsv_evt_dec_impl(NCS_UBAID* i_ub, IMMSV_IMPL_LIST** q)
{
    int depth=1;
    uns8 c8;
...
void
immsv_evt_free_impl(IMMSV_IMPL_LIST* q)
{
    while(q) {
...
static uns32
immsv_evt_dec_admo(NCS_UBAID* i_ub, IMMSV_ADMO_LIST** p)
...
void
immsv_evt_free_admo(IMMSV_ADMO_LIST* p)
{
    while(p) {
...
static void
immsv_evt_enc_attrName(NCS_UBAID* o_ub, IMMSV_ATTR_NAME_LIST* p)
...
static uns32
immsv_evt_dec_attrNames(NCS_UBAID* i_ub, IMMSV_ATTR_NAME_LIST** p)
...
void
immsv_evt_free_attrNames(IMMSV_ATTR_NAME_LIST* p)
{
    while(p) {
...
static void
immsv_evt_enc_admop_param(NCS_UBAID* o_ub, IMMSV_ADMIN_OPERATION_PARAM* op)
{
    if(!op) return;
    uns8* p8;
...
void
immsv_free_attrdefs_list(IMMSV_ATTR_DEF_LIST* adp)
{
    while(adp) {
...
static uns32
immsv_evt_dec_attr_def(NCS_UBAID* i_ub, IMMSV_ATTR_DEF_LIST** adp)
{
    int depth=1;
    uns8 c8;
        ...
        *adp = (IMMSV_ATTR_DEF_LIST *) calloc(1, sizeof(IMMSV_ATTR_DEF_LIST));
        ...
        if(c8) {
            (*adp)->d.attrDefaultValue = 
                (IMMSV_EDU_ATTR_VAL *) calloc(1, sizeof(IMMSV_EDU_ATTR_VAL));
...
static uns32
immsv_evt_dec_admop_param(NCS_UBAID* i_ub, IMMSV_ADMIN_OPERATION_PARAM** opp)
{
    int depth=1;
    uns8 c8;
...
\*****************************************************************************/
static uns32 
immsv_evt_enc_sublevels(IMMSV_EVT *i_evt, NCS_UBAID *o_ub)
{
    ...
    if(i_evt->type == IMMSV_EVT_TYPE_IMMA)
    {
        ...
        if(i_evt->type == IMMSV_EVT_TYPE_IMMD)
        {
            ...
            if(i_evt->type == IMMSV_EVT_TYPE_IMMND)
            {
                ...
                } else if(i_evt->info.immnd.type == IMMND_EVT_A2ND_OBJ_SYNC)
                {
                    ...
                    /*Encode the list of attributes*/
                    IMMSV_ATTR_VALUES_LIST* p = 
                        i_evt->info.immnd.info.obj_sync.attrValues;
                ...
                } else if(i_evt->info.immnd.type == IMMND_EVT_A2ND_SEARCHINIT)
                {
                    ...
                    if(i_evt->info.immnd.info.searchInit.searchParam.present ==
                        ...
                        os= &(i_evt->info.immnd.info.
                            searchInit.searchParam.choice.oneAttrParam.attrName);
                        if(os->size) {
                            immsv_evt_enc_inline_string(o_ub, os);
                        }
                        immsv_evt_enc_att_val(o_ub, 
                            &(i_evt->info.immnd.info.searchInit.
                                searchParam.choice.oneAttrParam.attrValue), 
                            i_evt->info.immnd.info.searchInit.
                ...
                } else if(i_evt->info.immnd.type == IMMND_EVT_A2ND_RT_ATT_UPPD_RSP)
                {
                    ...
                    /*Encode the list of attribute names*/
                    IMMSV_ATTR_NAME_LIST* p = 
                        i_evt->info.immnd.info.rtAttUpdRpl.sr.attributeNames;
...
\*****************************************************************************/
static uns32 
immsv_evt_dec_sublevels(NCS_UBAID *i_ub, IMMSV_EVT *o_evt)
{
                ...
                (o_evt->info.immnd.type == IMMND_EVT_D2ND_GLOB_FEVS_REQ))
            {
                    ...
                    (o_evt->info.immnd.type == IMMND_EVT_A2ND_OI_OBJ_IMPL_REL))
                {
                    ...
                    } else if(o_evt->info.immnd.type == IMMND_EVT_A2ND_OBJ_SYNC) 
                    {
                        /*Decode the className*/
                        IMMSV_OCTET_STRING* os = 
                            &(o_evt->info.immnd.info.obj_sync.className);
                        immsv_evt_dec_inline_string(i_ub, os);
                    ...
                    {
                        immsv_evt_dec_name_list(i_ub, 
                            &(o_evt->info.immnd.info.admReq.objectNames));
                    ...
                    } else if(o_evt->info.immnd.type == IMMND_EVT_A2ND_RT_ATT_UPPD_RSP)
                    {
                        ...
                        /*Decode the list of attribute names*/
                        IMMSV_ATTR_NAME_LIST* p = 
                            o_evt->info.immnd.info.rtAttUpdRpl.sr.attributeNames;
                        if(p) {
                            immsv_evt_dec_attrNames(i_ub, &p);
                        ...
                        IMMND_EVT_ND2ND_SEARCH_REMOTE_RSP)
                    {
                        ...
                        /* Decode object name*/
                        IMMSV_OCTET_STRING* os = 
                            &(o_evt->info.immnd.info.rspSrchRmte.runtimeAttrs.objectName);
                        p8 = ncs_dec_flatten_space(i_ub, local_data, 4);
                        os->size = ncs_decode_32bit(&p8);
                        ncs_dec_skip_space(i_ub, 4);
                        immsv_evt_dec_inline_string(i_ub, os);
                        ...
                        if(c8) {
                            ...
                            IMMSV_ATTR_VALUES_LIST* p = NULL;
                            immsv_evt_dec_attributes(i_ub, &p);
                            o_evt->info.immnd.info.rspSrchRmte.runtimeAttrs.attrValuesList 
                                = p;
                        } else {
                            o_evt->info.immnd.info.rspSrchRmte.runtimeAttrs.attrValuesList 
                                = NULL;
        ...
        } else if(o_evt->type == IMMSV_EVT_TYPE_IMMND)
        {
            ...
            switch(immndevt->type) {
                ...
                case IMMND_EVT_A2ND_CCB_OBJ_DELETE_RSP: /*CcbObjDelete local Reply*/
                    ...
                    p8 = ncs_dec_flatten_space(i_ub, local_data, 4);
                    immndevt->info.ccbUpcallRsp.inv = 
                        ncs_decode_32bit(&p8);
                    ncs_dec_skip_space(i_ub, 4);
                ...
                case IMMND_EVT_ND2ND_SEARCH_REMOTE_RSP:/*Forward search req from impl ND*/
                    ...
                    p8 = ncs_dec_flatten_space(i_ub, local_data, 4);
                    immndevt->info.rspSrchRmte.runtimeAttrs.objectName.size = 
                        ncs_decode_32bit(&p8);
                    ncs_dec_skip_space(i_ub, 4);
