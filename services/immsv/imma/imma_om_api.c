                                        ...
                                        SaImmAdminOwnerHandleT* adminOwnerHandle)
{
    ...
    ao_node = 
        (IMMA_ADMIN_OWNER_NODE *) calloc(1, sizeof(IMMA_ADMIN_OWNER_NODE));
    if (ao_node == NULL)
    {
        rc = SA_AIS_ERR_NO_MEMORY;
 ...
 *       parameter.
 *******************************************************************/
static SaAisErrorT imma_newCcbId(IMMA_CB *cb, 
                                  ...
                                  SaImmCcbHandleT* ccbHandle)
{
    ...
    rc = imma_admin_owner_node_get(&cb->admin_owner_tree, &adminOwnerHandle, 
                                   &ao_node);
    if (!ao_node)
    {
        rc = SA_AIS_ERR_BAD_HANDLE;
    ...
    rc = imma_client_node_get(&cb->client_tree, &(ao_node->mImmHandle), 
                              &cl_node);
    if (!(cl_node&&cl_node->isOm))
    {
        rc = SA_AIS_ERR_BAD_HANDLE;
...
{
    return saImmOmCcbObjectCreate_2(ccbHandle, className, parentName, 
                                     ...
                                     const SaImmAttrValuesT_2** attrValues)
{
    ...
    /* Get the CCB info */
    proc_rc = imma_ccb_node_get(&cb->ccb_tree, &ccbHandle, 
                                &ccb_node);
    if (!ccb_node)
    {
        rc = SA_AIS_ERR_BAD_HANDLE;
    ...
    proc_rc = imma_admin_owner_node_get(&cb->admin_owner_tree, 
                                        &(ccb_node->mAdminOwnerHdl),
                                        &ao_node);
    if (!ao_node)
    {
    ...
    proc_rc = imma_client_node_get(&cb->client_tree, &(ccb_node->mImmHandle), 
                                   &cl_node);
    if (!(cl_node&&cl_node->isOm))
    {
        rc = SA_AIS_ERR_BAD_HANDLE;
    ...
    if (parentName && parentName->length)
    {
        ...
        if(evt.info.immnd.info.objCreate.parentName.size > SA_MAX_NAME_LENGTH){
            rc = SA_AIS_ERR_INVALID_PARAM;
            TRACE_2("Parent name too long for SaNameT: %u", 
                evt.info.immnd.info.objCreate.parentName.size);
            goto mds_send_fail;
                                   ...
                                   const SaImmAttrModificationT **attrMods)
{
    ...
    return saImmOmCcbObjectModify_2(ccbHandle, objectName, 
                                        ...
                                        SaTimeT timeout)
{
    ...
    return saImmOmAdminOperationInvoke_2(ownerHandle, objectName, 0, operationId,
        (const SaImmAdminOperationParamsT_2 **) params, operationReturnValue, timeout);
}
#endif
                                          ...
                                          SaTimeT timeout)
{
    ...
    *operationReturnValue = SA_AIS_OK; 
    ...
    /* Get the Admin Owner info  */
    proc_rc= imma_admin_owner_node_get(&cb->admin_owner_tree, &ownerHandle,
                                       &ao_node);
    if (!ao_node)
    {
        rc = SA_AIS_ERR_BAD_HANDLE;
    ...
    proc_rc= imma_client_node_get(&cb->client_tree, &(ao_node->mImmHandle),
                                  &cl_node);
    if (!(cl_node&&cl_node->isOm))
    {
        rc = SA_AIS_ERR_BAD_HANDLE;
    ...
    evt.info.immnd.info.admOpReq.objectName.size= 
        strnlen((char *) objectName->value, objectName->length)+1;
    if (objectName->length +1 < 
        evt.info.immnd.info.admOpReq.objectName.size)
    {
        evt.info.immnd.info.admOpReq.objectName.size = 
    ...
    if (evt.info.immnd.info.admOpReq.objectName.buf)
    { /*free-1*/
...
static int push_async_adm_op_continuation(
                                          IMMA_CB* cb,            //in
                                               ...
                                               const SaImmAdminOperationParamsT_2 **params)
{
    ...
    /* Get the Admin Owner info  */
    imma_admin_owner_node_get(&cb->admin_owner_tree, &ownerHandle,
                              &ao_node);
    if (!ao_node)
    {
        rc = SA_AIS_ERR_BAD_HANDLE;
    ...
    rc = imma_evt_fake_evs(cb, &evt, NULL, 0, cl_node->handle,
                           &locked, FALSE);
    TRACE("Fevs send RETURNED:%u", rc);
...
SaAisErrorT saImmOmAdminOperationContinue(SaImmAdminOwnerHandleT ownerHandle,
                                          ...
                                          SaAisErrorT *operationReturnValue)
{
    TRACE_2("saImmOmAdminOperationContinue not implemented "
           "use saImmOmAdminOperationContinueAsync instead");
...
#ifdef IMM_A_01_01
SaAisErrorT saImmOmClassCreate (
                               ...
                               const SaImmAttrDefinitionT** attrDefinitions)
{
    ...
    const SaImmAttrDefinitionT_2** attrDefinitions2 = 
    ...
    rc = saImmOmClassCreate_2(immHandle, className, classCategory, 
                              attrDefinitions2);

    for (attr2c = attrDefinitions2[0], ix=0; attr2c != 0;
        attr2c = attrDefinitions2[++ix])
    {
                                 ...
                                 const SaImmAttrDefinitionT_2** attrDefinitions)
{
    ...
    evt.info.immnd.info.classDescr.className.size = 
        strlen(className) +1;
...
#ifdef IMM_A_01_01
SaAisErrorT saImmOmClassDescriptionGet (
                                       SaImmHandleT immHandle,
                                       const SaImmClassNameT className,
                                       SaImmClassCategoryT* classCategory,
                                       SaImmAttrDefinitionT*** attrDefinition)
{
    SaImmAttrDefinitionT_2** attrDefinitions2;
    SaAisErrorT     rc = SA_AIS_OK;  

    rc = saImmOmClassDescriptionGet_2(immHandle, className, classCategory,
                                      &attrDefinitions2);
    ...
    if (rc == SA_AIS_OK)
    {
        ...
        for (attr2c = attrDefinitions2[0];attr2c; 
            attr2c = attrDefinitions2[++ix])
        {
            ++nrofAttrDefs;
        }
        ...
        for (attr2c = attrDefinitions2[0], ix=0; attr2c;
            attr2c = attrDefinitions2[++ix])
        {
                                         ...
                                         SaImmAttrDefinitionT_2*** attrDefinition)
{
 ...
 mds_send_fail:
    if (evt.info.immnd.info.classDescr.className.buf)
    {/*free-0*/
...
#ifdef IMM_A_01_01
SaAisErrorT saImmOmClassDescriptionMemoryFree (SaImmHandleT immHandle,
                                               SaImmAttrDefinitionT** attrDefinition)
{
    IMMA_CB         *cb = &imma_cb;
    IMMA_CLIENT_NODE      *cl_node = NULL;
...
SaAisErrorT saImmOmAccessorFinalize (SaImmAccessorHandleT accessorHandle)
{
    ...
    imma_search_node_get(&cb->search_tree, &accessorHandle, 
                         &search_node);
...
{
    return saImmOmAccessorGet_2(accessorHandle, objectName, attributeNames,
                                 ...
                                 SaImmAttrValuesT_2            ***attributes)
{
    ...
    proc_rc = imma_search_node_get(&cb->search_tree, &accessorHandle, 
                                   &search_node);
    ...
    */
    ...
    rc = saImmOmSearchInitialize_2(immHandle, 
                       ...
                       const SaImmAttrValuesT_2 **attrValues)
{
    ...
    proc_rc = imma_client_node_get(&cb->client_tree, &immHandle, 
                                   &cl_node);
    if (!(cl_node&&cl_node->isOm))
    {
        rc = SA_AIS_ERR_BAD_HANDLE;
    ...
    {
        IMMSV_ATTR_VALUES_LIST* p = 
            evt.info.immnd.info.obj_sync.attrValues;
...
SaAisErrorT immsv_finalize_sync(SaImmHandleT immHandle)
{
    ...
    rc = imma_client_node_get(&cb->client_tree, &immHandle, 
                              &cl_node);
    ...
    /* send the request to the IMMND */
    proc_rc = imma_mds_msg_sync_send(
                                     cb->imma_mds_hdl, 
                                     &(cb->immnd_mds_dest), 
                                    ...
                                    SaImmSearchHandleT* searchHandle)
{
    ...
    if(searchParam) {
        searchParam2.searchOneAttr.attrName = 
            searchParam->searchOneAttr.attrName;
        searchParam2.searchOneAttr.attrValueType = 
            searchParam->searchOneAttr.attrValueType;
        searchParam2.searchOneAttr.attrValue = 
                                      ...
                                      SaImmSearchHandleT* searchHandle)
{
    ...
    if ((scope == SA_IMM_ONE) && (*searchHandle) && !searchParam && 
        !searchOptions)
    { /*Special ACCESSOR case*/
        TRACE("Special accessor case:%llu\n", *searchHandle);
        isAccessor=TRUE;
        searchOptions = SA_IMM_SEARCH_ONE_ATTR |
                        (attributeNames?SA_IMM_SEARCH_GET_SOME_ATTR:
                         SA_IMM_SEARCH_GET_ALL_ATTR);

        /*Look up search handle*/
        proc_rc = imma_search_node_get(&cb->search_tree, searchHandle, 
                                       &search_node);
    ...
    if (rootName && rootName->length &&
        (rootName->length < SA_MAX_NAME_LENGTH))
    {
    ...
    else
    {
        ...
        req->searchParam.choice.oneAttrParam.
            attrName.buf[req->searchParam.choice.oneAttrParam.attrName.size-1]=0;
    ...
    /* send the request to the IMMND */
    proc_rc = imma_mds_msg_sync_send(
                                     cb->imma_mds_hdl, &cb->immnd_mds_dest, 
                                     &evt, &out_evt, IMMSV_WAIT_TIME);
    ...
    if (out_evt)
    {
        ...
        {
            search_node->mSearchId = 
                out_evt->info.imma.info.searchInitRsp.searchId;
            if (*searchHandle)
            {
                             ...
                             SaImmAttrValuesT***     attributes)
{
    ...
    error = saImmOmSearchNext_2(searchHandle, objectName, 
                                (SaImmAttrValuesT_2 ***) attributes);
                               ...
                               SaImmAttrValuesT_2***   attributes)
{
    ...
    proc_rc = imma_client_node_get(&cb->client_tree, &search_node->mImmHandle, 
                                   &cl_node);
    ...
    if (out_evt)
    {
        ...
        for (;i<noOfAttributes; i++, p = p->next)
        {
            ...
            if (q->attrValuesNumber)
            {
                ...
                */
                attr[i]->attrValues = calloc(1, q->attrValuesNumber * 
                                  ...
                                  SaImmScopeT            scope)
{
    ...
    admo_set_evt.info.immnd.info.admReq.adm_owner_id = 
    ...
    rc = imma_evt_fake_evs(cb, &admo_set_evt, &out_evt, IMMSV_WAIT_TIME, 
...
SaAisErrorT saImmOmAdminOwnerFinalize (SaImmAdminOwnerHandleT adminOwnerHandle)
{
    ...
    finalize_evt.info.immnd.info.admFinReq.adm_owner_id = 
        ao_node->mAdminOwnerId;

    rc = imma_evt_fake_evs(cb, &finalize_evt, &out_evt, IMMSV_WAIT_TIME, 
...
SaAisErrorT saImmOmCcbFinalize (SaImmCcbHandleT ccbHandle)
{
    ...
    /*Get the CCB info*/
    imma_ccb_node_get(&cb->ccb_tree, &ccbHandle, 
                      &ccb_node);
    if (!ccb_node)
    {
        rc = SA_AIS_ERR_BAD_HANDLE;
    ...
    if (!ccb_node->mTerminated)
    {
        ...
        imma_client_node_get(&cb->client_tree, &(ccb_node->mImmHandle), 
                             &cl_node);
        if (!(cl_node&&cl_node->isOm))
        {
            rc = SA_AIS_ERR_BAD_HANDLE;
        ...
        rc = imma_evt_fake_evs(cb, &evt, &out_evt, IMMSV_WAIT_TIME, 
                               cl_node->handle, &locked, FALSE);
