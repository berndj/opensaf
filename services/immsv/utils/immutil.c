...
static SaImmClassNameT dupSaImmClassNameT(
    struct Chunk* clist, const SaImmClassNameT original);
static const SaImmAttrValuesT_2** dupSaImmAttrValuesT_array(
    struct Chunk* clist, const SaImmAttrValuesT_2** original);
static const SaImmAttrModificationT_2** dupSaImmAttrModificationT_array(
    struct Chunk* clist, const SaImmAttrModificationT_2** original);
...
struct CcbUtilCcbData* ccbutil_findCcbData(SaImmOiCcbIdT id)
{
    ...
    while (ccbitem != NULL) {
	if (ccbitem->id == id) return ccbitem;
...
struct CcbUtilCcbData* ccbutil_getCcbData(SaImmOiCcbIdT id)
{
    ...
    if (ccbitem == NULL) ccbitem = ccbutil_createCcbData(id);
    return ccbitem;
}
...
static struct CcbUtilOperationData* newOperationData(
    struct CcbUtilCcbData* ccb,
    enum CcbUtilOperationType type)
{
    struct Chunk* clist = (struct Chunk*)ccb->memref;
...
void ccbutil_ccbAddCreateOperation(
    struct CcbUtilCcbData* ccb,
...
void ccbutil_ccbAddDeleteOperation(
    struct CcbUtilCcbData* ccb,
    ...
    const SaImmAttrModificationT_2 **attrMods)
{
    struct Chunk* clist = (struct Chunk*)ccb->memref;
    struct CcbUtilOperationData* operation = 
	newOperationData(ccb, CCBUTIL_MODIFY);
...
char const* immutil_getClassName(
    struct CcbUtilCcbData* ccb,
    ...
    const SaNameT *objectName)
{
    ...
    if (immutil_saImmOmAccessorInitialize(
	    immHandle, &accessorHandle) != SA_AIS_OK) return NULL;

    if (immutil_saImmOmAccessorGet_2(
	    accessorHandle, objectName, classNameAttr, &attributes) 
	!= SA_AIS_OK) goto finalize;
    if (attributes == NULL || *attributes == NULL) goto finalize;

    cnameattr = *attributes;
    if (strcmp(cnameattr->attrName, SA_IMM_ATTR_CLASS_NAME)!= 0) goto finalize;
...
const SaNameT* immutil_getNameAttr(
    const SaImmAttrValuesT_2 **attr, char const* name, unsigned int index)
{
    unsigned int i;
    if (attr == NULL || attr[0] == NULL) return NULL;
...
char const* immutil_getStringAttr(
    const SaImmAttrValuesT_2 **attr, char const* name, unsigned int index)
{
    unsigned int i;
    if (attr == NULL || attr[0] == NULL) return NULL;
...
const SaUint32T* immutil_getUint32Attr(
    const SaImmAttrValuesT_2 **attr, char const* name, unsigned int index)
{
    unsigned int i;
    if (attr == NULL || attr[0] == NULL) return NULL;
...
SaAisErrorT immutil_update_one_rattr(
...
SaAisErrorT immutil_get_attrValueType(
    SaNameT *objectName, SaImmAttrNameT attrName, SaImmValueTypeT *attrValueType)
{
...
static char* dupStr(
    struct Chunk* clist, const char* original)
{
    unsigned int len;
...
static const SaNameT* dupSaNameT(
    struct Chunk* clist, const SaNameT* original)
{
    SaNameT* copy;
    if (original == NULL) return NULL;
...
static void copySaImmAttrValuesT(
    struct Chunk* clist, SaImmAttrValuesT_2* copy, 
...
static const SaImmAttrModificationT_2* dupSaImmAttrModificationT(
    struct Chunk* clist, const SaImmAttrModificationT_2* original)
{
...
static struct Chunk* newChunk(struct Chunk* next, size_t size)
{
    ...
    if (chunk == NULL) immutilError("Out of memory");
    chunk->next = next;
    chunk->capacity = size;
    chunk->free = size;
    return chunk;
...
SaAisErrorT immutil_saImmOiSelectionObjectGet(
    SaImmOiHandleT immOiHandle,
...
SaAisErrorT immutil_saImmOiClassImplementerSet(
    SaImmOiHandleT immOiHandle,
    const SaImmClassNameT className)
{
...
SaAisErrorT immutil_saImmOiClassImplementerRelease(
    SaImmOiHandleT immOiHandle,
    const SaImmClassNameT className)
{
...
SaAisErrorT immutil_saImmOiImplementerSet(
    SaImmOiHandleT immOiHandle,
...
SaAisErrorT immutil_saImmOiRtObjectDelete(
    SaImmOiHandleT immOiHandle,
    const SaNameT *objectName)
{
...
SaAisErrorT immutil_saImmOiRtObjectUpdate_2(
    SaImmOiHandleT immOiHandle,
    const SaNameT *objectName,
    const SaImmAttrModificationT_2 **attrMods)
{
...
SaAisErrorT immutil_saImmOiAdminOperationResult(
    SaImmOiHandleT immOiHandle,
...
SaAisErrorT immutil_saImmOmInitialize(
    SaImmHandleT *immHandle,
...
SaAisErrorT immutil_saImmOmFinalize(
    SaImmHandleT immHandle)
{
...
SaAisErrorT immutil_saImmOmAccessorInitialize(
    SaImmHandleT immHandle,
    SaImmAccessorHandleT *accessorHandle)
{
...
SaAisErrorT immutil_saImmOmAccessorGet_2(
    SaImmAccessorHandleT        accessorHandle,
    const SaNameT               *objectName,
    const SaImmAttrNameT        *attributeNames,
    SaImmAttrValuesT_2          ***attributes)
{
    SaAisErrorT rc = saImmOmAccessorGet_2(
	accessorHandle, objectName, attributeNames, attributes);
    unsigned int nTries = 1;
    while(rc == SA_AIS_ERR_TRY_AGAIN && nTries < immutilWrapperProfile.nTries){
	usleep(immutilWrapperProfile.retryInterval * 1000);
...
SaAisErrorT immutil_saImmOmAccessorFinalize(
    SaImmAccessorHandleT accessorHandle)
{
    ...
    SaImmSearchHandleT*		searchHandle)
{
    ...
    if (rc == SA_AIS_ERR_NOT_EXIST) return rc;
    if (rc != SA_AIS_OK && immutilWrapperProfile.errorsAreFatal)
	immutilError("saImmOmSearchInitialize FAILED, rc = %d", (int)rc);
    return rc;
}

SaAisErrorT immutil_saImmOmSearchFinalize(
     SaImmSearchHandleT		searchHandle)
{
...
SaAisErrorT immutil_saImmOmSearchNext_2(
     SaImmSearchHandleT	searchHandle,
     SaNameT*		objectName,
     SaImmAttrValuesT_2 ***attributes)
{
...
SaAisErrorT immutil_saImmOmAdminOwnerClear (
    SaImmHandleT immHandle,
