    ...
    const SaImmAdminOperationParamsT_2 **params)
{
    ...
    {
        LOG_ER("Invalid operation ID, should be %d (one) for change filter",
    ...
    SaImmOiCcbIdT ccbId)
{
    ...
    */
    ...
    {
        int i = 0;
        TRACE("Change stream %s",
            ccbUtilOperationData->param.modify.objectName->value);

        if ((stream = log_stream_get_by_name(
            (char *) ccbUtilOperationData->param.modify.objectName->value)) == NULL)
        {
            LOG_ER("Stream %s not found",
                ccbUtilOperationData->param.modify.objectName->value);
            rc = SA_AIS_ERR_BAD_OPERATION;
            goto done;
        }
    ...
    SaImmOiCcbIdT ccbId)
{
    ...
    {
        int i = 0;
        TRACE("Apply changes to stream %s",
              ccbUtilOperationData->param.modify.objectName->value);
...
static SaAisErrorT stream_create_and_configure(
    const char *dn, log_stream_t **in_stream, int stream_id)
{
    SaAisErrorT rc = SA_AIS_OK;
