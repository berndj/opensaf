...
uns32 mqd_asapi_evt_hdlr(MQSV_EVT *pEvt, MQD_CB *pMqd)
{
   ...
   if(pMqd->active) {      
      ...
      pMsg = pEvt->msg.asapi;
      if(!pMsg) return NCSCC_RC_FAILURE;

      /* Check the message type and handle the message */
      if(ASAPi_MSG_REG == pMsg->msgtype) {
         /* Handle the ASAPi Registration request */
         m_LOG_MQSV_D(MQD_ASAPi_REG_MSG_RCV,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
         ...
         m_LOG_MQSV_D(MQD_ASAPi_DEREG_MSG_RCV,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
         ...
         m_LOG_MQSV_D(MQD_ASAPi_NRESOLVE_MSG_RCV,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
         ...
         m_LOG_MQSV_D(MQD_ASAPi_GETQUEUE_MSG_RCV,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
         ...
         m_LOG_MQSV_D(MQD_ASAPi_TRACK_MSG_RCV,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
...
\****************************************************************************/
static uns32 
mqd_asapi_reg_hdlr(MQD_CB *pMqd, ASAPi_REG_INFO *reg, MQSV_SEND_INFO *info)
{
   uns32             rc = NCSCC_RC_SUCCESS;
   ASAPi_MSG_INFO    msg;  
   ...
   if(pMqd->ha_state == SA_AMF_HA_ACTIVE) {
      ...
      if(rc == NCSCC_RC_SUCCESS) {
         ...
         if(reg->objtype == ASAPi_OBJ_QUEUE)
            m_LOG_MQSV_D(MQD_REG_HDLR_DB_UPDATE_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
         else 
            m_LOG_MQSV_D(MQD_REG_HDLR_DB_UPDATE_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
      }
      else {
         if(reg->objtype == ASAPi_OBJ_QUEUE)
            m_LOG_MQSV_D(MQD_REG_HDLR_DB_UPDATE_FAILED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
         else 
            m_LOG_MQSV_D(MQD_REG_HDLR_DB_UPDATE_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
...
send_resp:
  ...
  { 
   ...
   { 
    ...
    {
      if(reg->objtype == ASAPi_OBJ_QUEUE)
         m_LOG_MQSV_D(MQD_ASAPi_REG_RESP_MSG_ERR,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else 
         m_LOG_MQSV_D(MQD_ASAPi_REG_RESP_MSG_ERR,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      return rc;
    }
    if(reg->objtype == ASAPi_OBJ_QUEUE)
      m_LOG_MQSV_D(MQD_ASAPi_REG_RESP_MSG_SENT,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
    else 
      m_LOG_MQSV_D(MQD_ASAPi_REG_RESP_MSG_SENT,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
   }
  }
    return rc;
...
\****************************************************************************/
uns32 
mqd_asapi_dereg_hdlr(MQD_CB *pMqd, ASAPi_DEREG_INFO *dereg, MQSV_SEND_INFO *info)
{
   uns32             rc = NCSCC_RC_SUCCESS;
   ASAPi_MSG_INFO    msg;  
   ...
   if(pMqd->ha_state == SA_AMF_HA_ACTIVE) {
      ...
      if(rc == NCSCC_RC_SUCCESS)
      {
         ...
         if(dereg->objtype == ASAPi_OBJ_QUEUE)
           m_LOG_MQSV_D(MQD_DEREG_HDLR_DB_UPDATE_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
         else 
           m_LOG_MQSV_D(MQD_DEREG_HDLR_DB_UPDATE_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
      }
      else
      {
        if(dereg->objtype == ASAPi_OBJ_QUEUE)
           m_LOG_MQSV_D(MQD_DEREG_HDLR_DB_UPDATE_FAILED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
        else 
            m_LOG_MQSV_D(MQD_DEREG_HDLR_DB_UPDATE_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      } 
   }
   else {
   ...
   if(info)
   {
    ...
    if(info->stype == MDS_SENDTYPE_RSP) 
    {
      ...
      {
          if(dereg->objtype == ASAPi_OBJ_QUEUE)
             m_LOG_MQSV_D(MQD_ASAPi_DEREG_RESP_MSG_ERR,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
          else 
             m_LOG_MQSV_D(MQD_ASAPi_DEREG_RESP_MSG_ERR,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
           return rc;
      }   
      if(dereg->objtype == ASAPi_OBJ_QUEUE)
        m_LOG_MQSV_D(MQD_ASAPi_DEREG_RESP_MSG_SENT,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
      else 
        m_LOG_MQSV_D(MQD_ASAPi_DEREG_RESP_MSG_SENT,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
    }
   }  
   return rc;
...
\****************************************************************************/
uns32 
mqd_asapi_dereg_db_upd(MQD_CB *pMqd, ASAPi_DEREG_INFO *dereg, ASAPi_MSG_INFO *msg)
{
      ...
      case ASAPi_OBJ_BOTH:
         {
            ...
            if ((!(qgrpexist = mqd_asapi_obj_validate(pMqd, &dereg->group, &pObjNode))) 
               ...
               m_LOG_MQSV_D(MQD_DB_DEL_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
               goto resp_send;
            }
            ...
            if(!pOelm) { /* Object doesn't exist ... */
              ...
              if(pMqd->ha_state == SA_AMF_HA_ACTIVE) {
               ...
               if(pOelm) {
                  ...
                  m_LOG_MQSV_D(MQD_GROUP_REMOVE_QUEUE_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
               }
            }
         }
         break;
      ...
      case ASAPi_OBJ_GROUP:
         {
            ...
            while((pOelm = ncs_dequeue(&pObjNode->oinfo.ilist))) {
             ...
             m_LOG_MQSV_D(MQD_GROUP_DELETE_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
         }
         break;
      ...
      case ASAPi_OBJ_QUEUE:
         {
            ...
            if (!mqd_asapi_obj_validate(pMqd, &dereg->queue, &pObjNode)) {
               ...
               msg->info.dresp.err.flag = TRUE;
               msg->info.dresp.err.errcode = rc;
               m_LOG_MQSV_D(MQD_DB_DEL_FAILED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
               goto resp_send;
            }
            ...
            /* Destroy the Node */
            mqd_db_node_del(pMqd, pObjNode);
            m_LOG_MQSV_D(MQD_QUEUE_DELETE_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
         }
         break;
...
\****************************************************************************/
static uns32 
mqd_asapi_nresolve_hdlr(MQD_CB *pMqd, ASAPi_NRESOLVE_INFO *nresolve, 
                        MQSV_SEND_INFO *info)
{
   ...
   { /* Object doesn't exist ... */
    ...
    /* Set the error flag and fill the description */
      ...
      m_LOG_MQSV_D(MQD_NRESOLV_HDLR_DB_ACCESS_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
      goto send_resp;
   }
...
send_resp:
   ...
   rc = mqd_asapi_resp_send(&msg, info);
   if(NCSCC_RC_SUCCESS != rc) {
     m_LOG_MQSV_D(MQD_ASAPi_NRESOLVE_RESP_MSG_ERR,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
   }
   else {
     m_LOG_MQSV_D(MQD_ASAPi_NRESOLVE_RESP_MSG_SENT,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
   }
...
\****************************************************************************/
static uns32 
mqd_asapi_getqueue_hdlr(MQD_CB *pMqd, ASAPi_GETQUEUE_INFO *getqueue, 
                        MQSV_SEND_INFO *info)
{
   NCS_BOOL             exist = FALSE;
   MQD_OBJ_NODE         *pObjNode = 0;
   ...
   if(pObjNode->oinfo.type== MQSV_OBJ_QUEUE) {
      if(pObjNode->oinfo.info.q.is_mqnd_down) {
         ...
         m_LOG_MQSV_D(MQD_DB_UPD_MQND_DOWN,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
         goto send_resp;
      }
   }           
...
send_resp:
   ...
   rc = mqd_asapi_resp_send(&msg, info);    
   if(NCSCC_RC_SUCCESS != rc) {
      m_LOG_MQSV_D(MQD_ASAPi_GETQUEUE_RESP_MSG_ERR,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      return rc; 
   }
...
\****************************************************************************/
static uns32 
mqd_asapi_track_hdlr(MQD_CB *pMqd, ASAPi_TRACK_INFO *track, MQSV_SEND_INFO *info)
{
   NCS_BOOL          exist = FALSE;
   MQD_OBJ_NODE      *pObjNode = 0;
   uns32             rc = NCSCC_RC_SUCCESS;
   ...
   if(m_ASAPi_TRACK_IS_ENABLE(track->val)) {
      ...
      if(NCSCC_RC_SUCCESS != rc) {
         ...
         if(pObjNode->oinfo.type==ASAPi_OBJ_QUEUE)
           m_LOG_MQSV_D(MQD_ASAPi_QUEUE_MAKE_FAILED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
         else
           m_LOG_MQSV_D(MQD_ASAPi_QUEUE_MAKE_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      }
      else
      {
          if(pObjNode->oinfo.type==ASAPi_OBJ_QUEUE)
           m_LOG_MQSV_D(MQD_ASAPi_QUEUE_MAKE_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
         else
           m_LOG_MQSV_D(MQD_ASAPi_QUEUE_MAKE_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
     }
   }
...
\****************************************************************************/
 uns32 
mqd_asapi_track_db_upd(MQD_CB *pMqd, ASAPi_TRACK_INFO *track, MQSV_SEND_INFO *info,MQD_OBJ_NODE **onode)
{
   NCS_BOOL          exist = FALSE;
   MQD_OBJ_NODE      *pObjNode = 0;
   uns32             rc = NCSCC_RC_SUCCESS;
   ...
   if(m_ASAPi_TRACK_IS_ENABLE(track->val)) 
   {
      ...
      {
        if(pObjNode->oinfo.type==ASAPi_OBJ_QUEUE)
           m_LOG_MQSV_D(MQD_GROUP_TRACK_DB_UPDATE_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
        else
           m_LOG_MQSV_D(MQD_GROUP_TRACK_DB_UPDATE_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
      }
      else
      {
        if(pObjNode->oinfo.type==ASAPi_OBJ_QUEUE)
           m_LOG_MQSV_D(MQD_GROUP_TRACK_DB_UPDATE_FAILURE,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
        else
           m_LOG_MQSV_D(MQD_GROUP_TRACK_DB_UPDATE_FAILURE,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      }
   }
   else if(m_ASAPi_TRACK_IS_DISABLE(track->val)) 
         {
           ...
           { 
              if(pObjNode->oinfo.type==ASAPi_OBJ_QUEUE)
                 m_LOG_MQSV_D(MQD_GROUP_TRACKSTOP_DB_UPDATE_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
              else 
                 m_LOG_MQSV_D(MQD_GROUP_TRACKSTOP_DB_UPDATE_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
           }
           else
           {    
              if(pObjNode->oinfo.type==ASAPi_OBJ_QUEUE)
                 m_LOG_MQSV_D(MQD_GROUP_TRACKSTOP_DB_UPDATE_FAILURE,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
              else
                 m_LOG_MQSV_D(MQD_GROUP_TRACKSTOP_DB_UPDATE_FAILURE,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
           }
         }   
  *onode= pObjNode; 
   return rc; 
...
static uns32 mqd_asapi_track_ntfy_send(MQD_OBJ_INFO *pObjInfo, ASAPi_OBJECT_OPR opr) 
{
   ...
   while((pTrack = (MQD_TRACK_OBJ *)ncs_walk_items(&pObjInfo->tlist, &itr))) {
      ...
      if(NCSCC_RC_SUCCESS != rc) {
         if(pObjInfo->type==ASAPi_OBJ_QUEUE)
            m_LOG_MQSV_D(MQD_ASAPi_TRACK_NTFY_MSG_ERR,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
         else
            m_LOG_MQSV_D(MQD_ASAPi_TRACK_NTFY_MSG_ERR,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
         if(cons_rc == NCSCC_RC_SUCCESS)
            cons_rc  = rc; 
         continue;  /* Continue sending to the other dests */
      }
      if(pObjInfo->type==ASAPi_OBJ_QUEUE)
            m_LOG_MQSV_D(MQD_ASAPi_TRACK_NTFY_MSG_SENT,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
      else
            m_LOG_MQSV_D(MQD_ASAPi_TRACK_NTFY_MSG_SENT,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
   }
...
static uns32 mqd_asapi_resp_send(ASAPi_MSG_INFO *msg, MQSV_SEND_INFO *info)
{
   ...
   if(NCSCC_RC_SUCCESS != rc) return SA_AIS_ERR_FAILED_OPERATION;
...
\****************************************************************************/
static uns32 
mqd_asapi_queue_make(MQD_OBJ_INFO *pObjInfo, ASAPi_QUEUE_PARAM **o_queue, 
                     uns16 *o_cnt, NCS_BOOL select)
{
   uns16             qcnt = 0;
   ASAPi_QUEUE_PARAM *pQueue = 0;
   MQD_OBJECT_ELEM   *pOelm = 0;
   ...
   *o_queue = 0;
   ...
   if(MQSV_OBJ_QUEUE == pObjInfo->type) {         
      ...
      if(!pQueue) {
         if(pObjInfo->type==ASAPi_OBJ_QUEUE)
              m_LOG_MQSV_D(MQD_MEMORY_ALLOC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,SA_AIS_ERR_NO_MEMORY,__FILE__,__LINE__);
           else
              m_LOG_MQSV_D(MQD_MEMORY_ALLOC_FAIL,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,SA_AIS_ERR_NO_MEMORY,__FILE__,__LINE__);
   ...
   else return SA_AIS_ERR_INVALID_PARAM;
...
\****************************************************************************/
 uns32 
mqd_asapi_db_upd(MQD_CB *pMqd, ASAPi_REG_INFO *reg, MQD_OBJ_NODE **onode, 
      ...
      case ASAPi_OBJ_BOTH:
           {
              ...
              if(!pOelm) {
                 ...
                 *opr = ASAPi_QUEUE_ADD;
                 ...
                 m_LOG_MQSV_D(MQD_DB_ADD_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
              }
              else {
                /* The queue is already the member of the group, identified by the
                 * group name */
                 m_LOG_MQSV_D(MQD_REG_DB_UPD_ERR_EXIST,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
                 return SA_AIS_ERR_EXIST;
              }
              m_LOG_MQSV_D(MQD_REG_DB_GRP_INSERT_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
           } 
           break;
      ...
      case ASAPi_OBJ_GROUP:
           {
              ...
              *opr = ASAPi_GROUP_ADD; /* This is Group create */
              ...
              if(NCSCC_RC_SUCCESS != rc) {
                    m_LOG_MQSV_D(MQD_REG_DB_QUEUE_GROUP_CREATE_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
              }
              else
                 m_LOG_MQSV_D(MQD_REG_DB_QUEUE_GROUP_CREATE_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
           }
         break;
      ...
      case ASAPi_OBJ_QUEUE:
           {
              ...
              if (mqd_asapi_obj_validate(pMqd, &reg->queue.name, &pObjNode)) {
                 ...
                 if (mqd_check_for_namespace_collision(pObjNode, MQSV_OBJ_QGROUP)) {
                    m_LOG_MQSV_D(MQD_REG_DB_UPD_ERR_EXIST,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
                    rc = SA_AIS_ERR_EXIST;
                    return rc;
                 }
                 ...
                 *opr = ASAPi_QUEUE_UPD;
                 ...
                 m_LOG_MQSV_D(MQD_REG_DB_QUEUE_UPDATE_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
              }
              else {
                 ...
                 if(NCSCC_RC_SUCCESS != rc)
                 {
                    m_LOG_MQSV_D(MQD_MEMORY_ALLOC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
                    return SA_AIS_ERR_NO_MEMORY;
                 }
                 ...
                 *opr = ASAPi_QUEUE_ADD;
                 ...
                 rc = mqd_db_node_add(pMqd, pObjNode);
                 if(NCSCC_RC_SUCCESS != rc) {
                    m_LOG_MQSV_D(MQD_REG_DB_QUEUE_CREATE_FAILED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
                 }
                 else
                    m_LOG_MQSV_D(MQD_REG_DB_QUEUE_CREATE_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
              }
           }
         break;
...
\****************************************************************************/
static NCS_BOOL 
mqd_asapi_obj_validate(MQD_CB *pMqd, SaNameT *name, MQD_OBJ_NODE **o_node)
{
   MQD_OBJ_NODE   *pObjNode = 0;
