...
static SaAisErrorT mqa_send_to_destination(MQA_CB *mqa_cb, MDS_DEST *mqnd_mds_dest, 
                            ...
                            SaTimeT timeout, uns32 length);
static SaAisErrorT mqa_send_to_destination_async (MQA_CB *mqa_cb, MDS_DEST *mqnd_mds_dest, 
                            MQSV_DSEND_EVT *qsend_evt, uns32 length);
static uns32 mqa_send_to_group(MQA_CB *mqa_cb, ASAPi_OPR_INFO    *asapi_or, MQSV_DSEND_EVT *qsend_evt,
...
static SaAisErrorT mqa_reply_to_destination(MQA_CB *mqa_cb, MDS_DEST *mqnd_mds_dest, 
                            MQSV_DSEND_EVT *qsend_evt, SaMsgAckFlagsT ackFlags, 
                            SaTimeT timeout, MDS_SYNC_SND_CTXT *context, uns32 length);
static SaAisErrorT mqa_reply_to_destination_async (MQA_CB *mqa_cb, MDS_DEST *mqnd_mds_dest, 
                            MQSV_DSEND_EVT *qsend_evt, MDS_SYNC_SND_CTXT *context, uns32 length);
                ...
                SaVersionT *version)
{
  ...
  if(!mqa_cb->is_mqd_up || !mqa_cb->is_mqnd_up)
  {
   ...
   switch (mds_rc)
   {
       ...
       case NCSCC_RC_REQ_TIMOUT:
           m_LOG_MQSV_A(MQA_MDS_SEND_TIMEOUT,NCSFL_LC_MQSV_INIT,NCSFL_SEV_INFO,m_NCS_NODE_ID_FROM_MDS_DEST(mqa_cb->mqa_mds_dest),__FILE__,__LINE__);
           rc = SA_AIS_ERR_TIMEOUT;
           goto final2;
       case NCSCC_RC_FAILURE:
           m_LOG_MQSV_A(MQA_MDS_SEND_FAILURE,NCSFL_LC_MQSV_INIT,NCSFL_SEV_INFO,m_NCS_NODE_ID_FROM_MDS_DEST(mqa_cb->mqa_mds_dest),__FILE__,__LINE__);
           rc = SA_AIS_ERR_TRY_AGAIN;
            goto final2;
       default:
           m_LOG_MQSV_A(MQA_MDS_SEND_FAILURE,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,m_NCS_NODE_ID_FROM_MDS_DEST(mqa_cb->mqa_mds_dest),__FILE__,__LINE__);
  ...
  if(rc == SA_AIS_OK)
  {
      ...
      if (mqsv_mqa_callback_queue_init(client_info) != NCSCC_RC_SUCCESS)
      {
         ...
         rc = SA_AIS_ERR_NO_RESOURCES;
         m_LOG_MQSV_A(MQA_CLBK_QUEUE_INIT_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
...
final1:
   if (rc != SA_AIS_OK)
   {
      if(rc != SA_AIS_ERR_TRY_AGAIN)
              m_LOG_MQSV_A(MQA_API_MSG_INITIALIZE_FAIL,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else
   ...
   {
       m_LOG_MQSV_A(MQA_API_MSG_INITIALIZE_SUCCESS,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
   }    
...
done0:
   ...
   {
      if(rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQA_API_MSG_SELECTION_OBJECT_FAIL,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else
         m_LOG_MQSV_A(MQA_API_MSG_SELECTION_OBJECT_FAIL,NCSFL_LC_MQSV_INIT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
   else
      m_LOG_MQSV_A(MQA_API_MSG_SELECTION_OBJECT_SUCCESS,NCSFL_LC_MQSV_INIT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
...
saMsgDispatch(SaMsgHandleT msgHandle, SaDispatchFlagsT dispatchFlags)
{
  ...
  /* get the client_info*/
   ...
   { 
      if(rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQA_API_MSG_DISPATCH_FAIL,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else
         m_LOG_MQSV_A(MQA_API_MSG_DISPATCH_FAIL,NCSFL_LC_MQSV_INIT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
   else
...
saMsgFinalize(SaMsgHandleT msgHandle)
{
   ...
   {  
      ...
      if (queue_info->client_info == client_info)
      {
         ...
         if(rc2 != SA_AIS_OK)
         {
             m_LOG_MQSV_A(MQA_FINALIZE_CLOSE_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc2,__FILE__,__LINE__);
             rc = rc2;
...
lock_fail:
   ...
   {
      if(rc != SA_AIS_ERR_TRY_AGAIN)
        m_LOG_MQSV_A(MQA_API_MSG_FINALIZE_FAIL,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else
        m_LOG_MQSV_A(MQA_API_MSG_FINALIZE_FAIL,NCSFL_LC_MQSV_INIT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
   else
   {
      ncs_agents_shutdown(argc, argv);
               ...
               SaMsgQueueHandleT *queueHandle)
{
   ...
   if (creationAttributes)
   {
      ...
      if ( (creationAttributes->creationFlags != 0) && (creationAttributes->creationFlags != SA_MSG_QUEUE_PERSISTENT) )
      {  
        rc = SA_AIS_ERR_BAD_FLAGS;
        m_LOG_MQSV_A(MQA_INVALID_PARAM,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
        return rc;
   ...
   {
      rc = SA_AIS_ERR_INIT; 
      m_LOG_MQSV_A(MQA_RECEIVE_CALLBACK_MISSING,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      goto done;
   }
   ...
   /* send the event */
   mds_rc = mqa_mds_msg_sync_send(mqa_cb->mqa_mds_hdl, &(mqa_cb->mqnd_mds_dest),
                                  &qopen_evt,&out_evt,(uns32)timeout) ;
   switch (mds_rc)
   {
       case NCSCC_RC_SUCCESS:
           break;
   ...
   if (rc == SA_AIS_OK)
   {
      *queueHandle = out_evt->msg.mqp_rsp.info.openRsp.queueHandle;
      ...
      if ((queue_info = mqa_queue_tree_find_and_add(mqa_cb, *queueHandle, TRUE, client_info, openFlags)) == NULL)
      {
         rc = SA_AIS_ERR_BAD_HANDLE;
         m_LOG_MQSV_A(MQA_CLIENT_TREE_ADD_FAILED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
         goto done;
      }
     ...
     /* Start a thread to notify when there is a message in the queue.
      ...
      if(queue_info->openFlags & SA_MSG_QUEUE_RECEIVE_CALLBACK)
      {
          ...
          if (!openRsp) {
             ...
             rc = SA_AIS_ERR_NO_MEMORY;
             m_LOG_MQSV_A(MQP_OPEN_RSP_ALLOC_FAILED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
             goto done;
          }
          ...
          *openRsp = out_evt->msg.mqp_rsp.info.openRsp;
          ...
          {
             rc = SA_AIS_ERR_NO_RESOURCES;
             m_LOG_MQSV_A(MQA_QUEUE_READER_TASK_CREATE_FAILED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
             mqa_queue_tree_delete_node(mqa_cb, *queueHandle);
             goto done;
          }
          ...
          if (rc != NCSCC_RC_SUCCESS)
          {
             ...
             rc = SA_AIS_ERR_NO_RESOURCES;
             m_LOG_MQSV_A(MQA_QUEUE_READER_TASK_START_FAILED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
             mqa_queue_tree_delete_node(mqa_cb, *queueHandle);
             goto done;
          }
   ...
   {
      if(rc != SA_AIS_ERR_TRY_AGAIN)
        m_LOG_MQSV_A(MQA_API_QUEUE_OPEN_SYNC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else
        m_LOG_MQSV_A(MQA_API_QUEUE_OPEN_SYNC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
   else
      m_LOG_MQSV_A(MQA_API_QUEUE_OPEN_SYNC_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
                    ...
                    SaMsgQueueOpenFlagsT openFlags)
{
   ...
   {
      rc = SA_AIS_ERR_NO_RESOURCES;
      m_LOG_MQSV_A(MQA_CREATE_AND_START_TIMER_FAILED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      goto done1;
   }
   ...
   /* send the event */
   if(mqa_mds_msg_async_send((mqa_cb->mqa_mds_hdl), &(mqa_cb->mqnd_mds_dest), &qopen_evt, NCSMDS_SVC_ID_MQND) != NCSCC_RC_SUCCESS)
   {
...
done1:
   ...
   {
     if(rc != SA_AIS_ERR_TRY_AGAIN)
       m_LOG_MQSV_A(MQA_API_QUEUE_OPEN_ASYNC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
     else
       m_LOG_MQSV_A(MQA_API_QUEUE_OPEN_ASYNC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
   else
      m_LOG_MQSV_A(MQA_API_QUEUE_OPEN_ASYNC_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
...
saMsgQueueClose(SaMsgQueueHandleT queueHandle)
{
   ...
   if (rc == SA_AIS_OK)
   {
      ...
      {
         rc = SA_AIS_ERR_LIBRARY;
         m_LOG_MQSV_A(MQA_QUEUE_TREE_DEL_FAILED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      }
   }
   ...
   {
      if (rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQA_API_QUEUE_CLOSE_SYNC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else
         m_LOG_MQSV_A(MQA_API_QUEUE_CLOSE_SYNC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
   else
...
saMsgQueueStatusGet(SaMsgHandleT msgHandle, 
                    const SaNameT *queueName, SaMsgQueueStatusT *queueStatus)
{
   MQA_CB            *mqa_cb=0;
   MQA_CLIENT_INFO   *client_info=0;
   SaAisErrorT          rc;
   ...
   mds_rc = mqa_mds_msg_sync_send(mqa_cb->mqa_mds_hdl,  &mqnd_mds_dest,&qstatus_evt,
                                  &out_evt,(uns32)mqa_timeout);
   switch (mds_rc)
   {
       case NCSCC_RC_SUCCESS:
   ...
   {
      if(rc != SA_AIS_ERR_TRY_AGAIN)
          m_LOG_MQSV_A(MQA_API_QUEUE_STATUS_GET_SYNC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else
          m_LOG_MQSV_A(MQA_API_QUEUE_STATUS_GET_SYNC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
   else 
      m_LOG_MQSV_A(MQA_API_QUEUE_STATUS_GET_SYNC_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
...
saMsgQueueUnlink(SaMsgHandleT msgHandle, const SaNameT *queueName)
{
  ...
  /* Check if mqd, mqnd are up */ 
   ...
   mds_rc = mqa_mds_msg_sync_send(mqa_cb->mqa_mds_hdl, &mqnd_mds_dest,&qunlink_evt,
                                  &out_evt,(uns32)mqa_timeout);
   switch (mds_rc)
   {
       case NCSCC_RC_SUCCESS:
   ...
   {
      if(rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQA_API_QUEUE_UNLINK_SYNC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
     else
         m_LOG_MQSV_A(MQA_API_QUEUE_UNLINK_SYNC_FAIL,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   } 
   else
      m_LOG_MQSV_A(MQA_API_QUEUE_UNLINK_SYNC_SUCCESS,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
                            ...
                            SaTimeT timeout, uns32 length)
{
   ...
   mds_rc = mqa_mds_msg_sync_send_direct(mqa_cb->mqa_mds_hdl, mqnd_mds_dest, qsend_evt,
                                     &out_evt,(uns32)mqa_timeout, length);
   switch (mds_rc) {
       case NCSCC_RC_SUCCESS:
           break;
       case NCSCC_RC_REQ_TIMOUT:
           m_LOG_MQSV_A(MQA_MDS_SEND_TIMEOUT,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,m_NCS_NODE_ID_FROM_MDS_DEST(mqa_cb->mqa_mds_dest),__FILE__,__LINE__);
           rc = SA_AIS_ERR_TIMEOUT;
           goto done;
       case NCSCC_RC_FAILURE:
           m_LOG_MQSV_A(MQA_MDS_SEND_FAILURE,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,m_NCS_NODE_ID_FROM_MDS_DEST(mqa_cb->mqa_mds_dest),__FILE__,__LINE__);
           rc = SA_AIS_ERR_TRY_AGAIN;
            goto done;
       default:
           m_LOG_MQSV_A(MQA_MDS_SEND_FAILURE,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,m_NCS_NODE_ID_FROM_MDS_DEST(mqa_cb->mqa_mds_dest),__FILE__,__LINE__);
           rc = SA_AIS_ERR_NO_RESOURCES;
           goto done;
   }
...
/****************************************************************************
  ...
  Notes         : None
******************************************************************************/ 
uns32 mqa_send_to_group(MQA_CB *mqa_cb, ASAPi_OPR_INFO    *asapi_or, MQSV_DSEND_EVT *qsend_evt,
                        SaMsgAckFlagsT ackFlags, MQA_SEND_MESSAGE_PARAM *param, uns32 length)
{
   ...
   num_queues = asapi_or->info.dest.o_cache->info.ginfo.qlist.count;
    ...
    if(num_queues == 0) {
       ...
       m_LOG_MQSV_A(MQA_QUEUES_NOT_AVLBL_IN_GROUP,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
       return rc;
    }
    ...
    if (unicast) {
       ...
       if (!param->async_flag) {
          ...
          rc = mqa_send_to_destination(mqa_cb, &destination_mqnd, qsend_evt, ackFlags, param->info.timeout, length);
          if(rc != NCSCC_RC_SUCCESS) 
        ...
        do {
            ...
            if (!param->async_flag)
                status = mqa_send_to_destination(mqa_cb, &destination_mqnd, qsend_evt, ackFlags, param->info.timeout, length);
            else
                status = mqa_send_to_destination_async(mqa_cb, &destination_mqnd, qsend_evt, length);
            if(status != NCSCC_RC_SUCCESS)
                m_LOG_MQSV_A(MQA_MDS_SEND_FAILURE,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,status,__FILE__,__LINE__);
                 ...
                 MQA_SEND_MESSAGE_PARAM *param, MQA_CB *mqa_cb)
{
  ...
  /* Check if mqd, mqnd are up */ 
   ...
   {
      rc = SA_AIS_ERR_INIT;
      m_LOG_MQSV_A(MQA_DELIVERED_CALLBACK_MISSING,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      goto done;
   }
   ...
   {
      rc = SA_AIS_ERR_NOT_EXIST;
      m_LOG_MQSV_A(MQA_ASAPi_GETDEST_CACHE_NOT_EXIST,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      goto done;
   }
                 ...
                 SaTimeT timeout)
{
   ...
   {  
     if(rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQA_API_QUEUE_SEND_SYNC_FAIL,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
     else
         m_LOG_MQSV_A(MQA_API_QUEUE_SEND_SYNC_FAIL,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
     
    if(rc == SA_AIS_ERR_TRY_AGAIN)
     MQA_TRY_AGAIN_WAIT;
   }
   else
      m_LOG_MQSV_A(MQA_API_QUEUE_SEND_SYNC_SUCCESS,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
                      ...
                      SaMsgAckFlagsT ackFlags)
{
   ...
   if ( ackFlags & SA_MSG_MESSAGE_DELIVERED_ACK) 
   {
      ...
      if (mqa_create_and_start_timer(&mqp_async_rsp, mqp_async_rsp.params.msgDelivered.invocation) != NCSCC_RC_SUCCESS)
      {
         rc = SA_AIS_ERR_NO_RESOURCES;
         m_LOG_MQSV_A(MQA_CREATE_AND_START_TIMER_FAILED,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
         goto done;
      }
   } 
   ...
   {
     if(rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQA_API_QUEUE_SEND_ASYNC_FAIL,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
     else
         m_LOG_MQSV_A(MQA_API_QUEUE_SEND_ASYNC_FAIL,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
    if(rc == SA_AIS_ERR_TRY_AGAIN)
     MQA_TRY_AGAIN_WAIT;
   }

   else
      m_LOG_MQSV_A(MQA_API_QUEUE_SEND_ASYNC_SUCCESS,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
                ...
                SaTimeT timeout)
{
  ...
  /* get the client_info*/
   ...
   {
     m_MQSV_MQA_GIVEUP_MQA_CB;
     m_LOG_MQSV_A(MQA_LOCK_WRITE_FAILED,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,SA_AIS_ERR_LIBRARY,__FILE__,__LINE__);
     return SA_AIS_ERR_LIBRARY;
   }
  ...
  /* Check if mqd, mqnd are up */ 
   ...
   /* Incriment the msg_get_count. This is used later during cancel request to 
...
again:
   ...
   if (ncs_os_posix_mq(&mq_req) != NCSCC_RC_SUCCESS)
   {
      if (timeout == 0) {
         ...
         rc = SA_AIS_ERR_NO_RESOURCES;
         m_LOG_MQSV_A(MQA_MSG_GET_NATIVE_QUEUE_ERROR,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
         goto done;
      } 
      else { 
      ...
      if (is_timer_present) {
         if (timer_arg == NULL) { 
            ...
            if (ncs_os_posix_mq(&mq_req_snd) != NCSCC_RC_SUCCESS) {
               rc = SA_AIS_ERR_NO_RESOURCES;
               m_LOG_MQSV_A(MQA_MSG_UNABLE_TO_PUT_GENUINE_MESSAGE,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
               goto done;
            }
   ...
   if ((mqsv_message->type == MQP_EVT_CANCEL_REQ)&&(mqsv_message->info.cancel_req.timerId != 0)) {
      ...
      /* The timer of another saMsgMessageGet call expired due to which the posix call unblocked. The
         ...
         if (ncs_os_posix_mq(&mq_req_snd) != NCSCC_RC_SUCCESS) {
            rc = SA_AIS_ERR_NO_RESOURCES;
            m_LOG_MQSV_A(MQA_MSG_UNABLE_TO_PUT_STOP_TIMER_MESSAGE,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
            goto done;
         }
   ...
   if (!message->data)
   {
      ...
      {
         message->data = (void *)malloc((uns32)mqsv_message->info.msg.message.size);
         if (!message->data)
         {
            rc = SA_AIS_ERR_NO_MEMORY;
   ...
   mds_rc = mqa_mds_msg_sync_send_direct(mqa_cb->mqa_mds_hdl, &mqa_cb->mqnd_mds_dest, stats,
                                  &statsrsp,(uns32)MQSV_WAIT_TIME, sizeof(MQSV_DSEND_EVT));
...
check:
   ...
   if (mqsv_message->info.msg.message_info.sendReceive == SA_TRUE)
   {
      ...
      sender_info->sender_context.sender_dest  = mqsv_message->info.msg.message_info.sender.sender_context.sender_dest;
      sender_info->sender_context.src_dest_version  = mqsv_message->info.msg.\
                                                        message_info.sender.sender_context.src_dest_version;
      ...
      if (ncs_enqueue(&(mqa_cb->mqa_senderid_list), (void *)sender_info) != NCSCC_RC_SUCCESS) {
         rc = SA_AIS_ERR_NO_SPACE;
         m_LOG_MQSV_A(MQA_SENDERID_LIST_INSERTION_FAILED,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
         goto done;
      }
                ...
                SaTimeT timeout)
{
   ...
   {
     if(rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQA_API_QUEUE_MESSAGE_GET_FAIL,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
     else
         m_LOG_MQSV_A(MQA_API_QUEUE_MESSAGE_GET_FAIL,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
   else
      m_LOG_MQSV_A(MQA_API_QUEUE_MESSAGE_GET_SUCCESS,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   return rc;
...
/* Send a shutdown messages = number of saMsgMessageGet() calls invoked, to the queue */
   ...
   if (m_NCS_LOCK(&mqa_cb->cb_lock, NCS_LOCK_WRITE) != NCSCC_RC_SUCCESS)
   {
      m_LOG_MQSV_A(MQA_LOCK_WRITE_FAILED,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,SA_AIS_ERR_BAD_HANDLE,__FILE__,__LINE__);
      m_MQSV_MQA_GIVEUP_MQA_CB;
      return SA_AIS_ERR_LIBRARY;
   }
   ...
   if(!mqa_cb->is_mqd_up || !mqa_cb->is_mqnd_up)
   {
      m_LOG_MQSV_A(MQA_MQD_OR_MQND_DOWN,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,SA_AIS_ERR_TRY_AGAIN,__FILE__,__LINE__);
      m_NCS_UNLOCK(&mqa_cb->cb_lock, NCS_LOCK_WRITE);
      m_MQSV_MQA_GIVEUP_MQA_CB;
      return SA_AIS_ERR_TRY_AGAIN;
   }
   ...
   if ( (queue_node = mqa_queue_tree_find_and_add(mqa_cb, queueHandle, FALSE, NULL, 0)) == NULL)
   {
      m_LOG_MQSV_A(MQA_QUEUE_TREE_FIND_FAILED,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,SA_AIS_ERR_BAD_HANDLE,__FILE__,__LINE__);
      m_NCS_UNLOCK(&mqa_cb->cb_lock, NCS_LOCK_WRITE);
      m_MQSV_MQA_GIVEUP_MQA_CB;
      return SA_AIS_ERR_BAD_HANDLE;
   }
   ...
   for (i=0; i<cancel_message_count;i++)
   {
      ...
      {
         rc = SA_AIS_ERR_TRY_AGAIN;
         m_LOG_MQSV_A(MQA_MSG_UNABLE_TO_PUT_CANCEL_MESSAGE,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      }
   }
   ...
   {
      if(rc != SA_AIS_ERR_TRY_AGAIN)
          m_LOG_MQSV_A(MQA_API_QUEUE_MESSAGE_CANCEL_FAIL,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else
          m_LOG_MQSV_A(MQA_API_QUEUE_MESSAGE_CANCEL_FAIL,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
   else
      m_LOG_MQSV_A(MQA_API_QUEUE_MESSAGE_CANCEL_SUCCESS,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
                            ...
                            SaTimeT timeout, uns32 length)
{
   ...
   mds_rc = mqa_mds_msg_sync_send_direct(mqa_cb->mqa_mds_hdl, mqnd_mds_dest, qsend_evt,
                                  qreply_evt,(uns32)mqa_timeout, length);
   switch (mds_rc)
   {
       case NCSCC_RC_SUCCESS:
           rc = SA_AIS_OK;   
                        ...
                        SaTimeT timeout)
{
   ...
   uns32             length, to_dest_ver, o_msg_fmt_ver ;
  ...
  /* Check if mqd, mqnd are up */ 
   ...
   if (!receiveMessage->data) {
      if (reply_msgsize != 0) {
         receiveMessage->data = (void *)malloc((uns32)reply_msgsize);
         if (!receiveMessage->data) {
            rc = SA_AIS_ERR_NO_MEMORY;
            m_LOG_MQSV_A(MQA_MEMORY_ALLOC_FAILED,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
...
send_del_callback: 
   ...
   if(rc != SA_AIS_OK) {
      if(rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQA_API_QUEUE_SENDRCV_FAIL,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else
      {
        m_LOG_MQSV_A(MQA_API_QUEUE_SENDRCV_FAIL,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
        MQA_TRY_AGAIN_WAIT;
      } 
      ...
      m_LOG_MQSV_A(MQA_API_QUEUE_SENDRCV_SUCCESS,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
                            ...
                            SaTimeT timeout, MDS_SYNC_SND_CTXT *context, uns32 length)
{
   ...
   mds_rc = mqa_mds_msg_sync_reply_direct(mqa_cb->mqa_mds_hdl, mqnd_mds_dest, qsend_evt,
                                     (uns32)mqa_timeout, context, length);
   switch (mds_rc) {
       case NCSCC_RC_SUCCESS:
          break;
       case NCSCC_RC_REQ_TIMOUT:
...
SaAisErrorT mqa_reply_to_destination_async(MQA_CB *mqa_cb, MDS_DEST *mqnd_mds_dest, 
                            MQSV_DSEND_EVT *qsend_evt, MDS_SYNC_SND_CTXT *context, uns32 length)
{
   if(mqa_mds_msg_async_reply_direct(mqa_cb->mqa_mds_hdl, mqnd_mds_dest, qsend_evt, context, length) != NCSCC_RC_SUCCESS)
   {
      m_LOG_MQSV_A(MQA_MDS_SEND_FAILURE,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,m_NCS_NODE_ID_FROM_MDS_DEST(mqa_cb->mqa_mds_dest),__FILE__,__LINE__);
      return SA_AIS_ERR_NOT_EXIST;
   }
                 ...
                 MQA_SEND_MESSAGE_PARAM *param, MQA_CB *mqa_cb)
{
   ...
   time_t               tmp_time;
   uns32                length, destination_mqa_ver, o_msg_fmt_ver;

   if ( !message  || !senderId || !(*senderId))
   {
      m_LOG_MQSV_A(MQA_INVALID_PARAM,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,SA_AIS_ERR_INVALID_PARAM,__FILE__,__LINE__);
  ...
  /* Check if mqd, mqnd are up */ 
   ...
   {
      rc = SA_AIS_ERR_NOT_EXIST;
      m_LOG_MQSV_A(MQA_SENDER_INFO_DOES_NOT_EXIST,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      goto done;
   }
                  ...
                  SaTimeT timeout)
{
   ...
   if(m_MQSV_CONVERT_SATIME_TEN_MILLI_SEC(timeout) < NCS_SAF_MIN_ACCEPT_TIME) {
      rc = SA_AIS_ERR_TIMEOUT;
      m_LOG_MQSV_A(MQA_INVALID_PARAM,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,SA_AIS_ERR_TIMEOUT,__FILE__,__LINE__);
      goto done;
   }
   ...
   if(rc ==SA_AIS_OK)
      m_LOG_MQSV_A(MQA_API_QUEUE_REPLY_SYNC_SUCCESS,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   else
   { 
      if(rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQA_API_QUEUE_REPLY_SYNC_FAILED,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else 
         m_LOG_MQSV_A(MQA_API_QUEUE_REPLY_SYNC_FAILED,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
                       ...
                       SaMsgAckFlagsT ackFlags)
{
   ...
   if(rc ==SA_AIS_OK)
      m_LOG_MQSV_A(MQA_API_QUEUE_REPLY_ASYNC_SUCCESS,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   else 
   {
     if(rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQA_API_QUEUE_REPLY_ASYNC_FAILED,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
    else
         m_LOG_MQSV_A(MQA_API_QUEUE_REPLY_ASYNC_FAILED,NCSFL_LC_MQSV_SEND_RCV,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
  }
                      ...
                      SaMsgQueueGroupPolicyT queueGroupPolicy)
{
   ...
   if (!queueGroupName) 
   {
     m_LOG_MQSV_A(MQA_INVALID_PARAM,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,SA_AIS_ERR_INVALID_PARAM,__FILE__,__LINE__);
     return SA_AIS_ERR_INVALID_PARAM;
   }
   ...
   {
     m_LOG_MQSV_A(MQA_INVALID_GROUP_POLICY,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,SA_AIS_ERR_INVALID_PARAM,__FILE__,__LINE__);
     return SA_AIS_ERR_INVALID_PARAM;
   }
   ...
   {
     m_LOG_MQSV_A(MQA_INVALID_GROUP_POLICY,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,SA_AIS_ERR_NOT_SUPPORTED,__FILE__,__LINE__);
  ...
  /* Check if mqd, mqnd are up */ 
   ...
   {
      rc =  SA_AIS_ERR_TRY_AGAIN;
      m_LOG_MQSV_A(MQA_MQD_OR_MQND_DOWN,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,SA_AIS_ERR_TRY_AGAIN,__FILE__,__LINE__);
      m_NCS_UNLOCK(&mqa_cb->cb_lock, NCS_LOCK_WRITE);
      goto done;
   }
   ...
   {
     if (rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQD_API_GROUP_CREATE_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
     else
         m_LOG_MQSV_A(MQD_API_GROUP_CREATE_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
   else
      m_LOG_MQSV_A(MQD_API_GROUP_CREATE_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
              ...
              const SaNameT *queueGroupName)
{
   ...
   {
      if(rc != SA_AIS_ERR_TRY_AGAIN)
          m_LOG_MQSV_A(MQA_API_MSG_GROUP_DEL_FAIL,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else
          m_LOG_MQSV_A(MQA_API_MSG_GROUP_DEL_FAIL,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }  
   else
      m_LOG_MQSV_A(MQA_API_MSG_GROUP_DEL_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
                      ...
                      const SaNameT *queueName)
{
  ...
  /* Check if mqd, mqnd are up */ 
   ...
   if ( (rc = asapi_opr_hdlr(&asapi_or)) != SA_AIS_OK )
   {
      m_LOG_MQSV_A(MQD_ASAPi_GROUP_INSERT_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
   } 
   else
   {
      if (asapi_or.info.msg.resp)
   ...
   {
      if(rc != SA_AIS_ERR_TRY_AGAIN)
         m_LOG_MQSV_A(MQA_API_MSG_GROUP_INSERT_FAIL,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      else
         m_LOG_MQSV_A(MQA_API_MSG_GROUP_INSERT_FAIL,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   }
   else
      m_LOG_MQSV_A(MQA_API_MSG_GROUP_INSERT_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
...
saMsgQueueGroupRemove(SaMsgHandleT msgHandle, 
              const SaNameT *queueGroupName, const SaNameT *queueName)
{
   ASAPi_OPR_INFO    asapi_or;
   MQA_CB            *mqa_cb;
   MQA_CLIENT_INFO   *client_info;
   ...
   if ( (rc = asapi_opr_hdlr(&asapi_or)) != SA_AIS_OK )
   {
      m_LOG_MQSV_A(MQD_ASAPi_GROUP_REMOVE_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
   ...
   {
     if(rc != SA_AIS_ERR_TRY_AGAIN)
          m_LOG_MQSV_A(MQA_API_MSG_GROUP_REMOVE_FAIL,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
     else
          m_LOG_MQSV_A(MQA_API_MSG_GROUP_REMOVE_FAIL,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   } 
   else
      m_LOG_MQSV_A(MQA_API_MSG_GROUP_REMOVE_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
                          ...
                          
{
   ...
   {  
     m_LOG_MQSV_A(MQA_INVALID_TRACK_FLAGS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,SA_AIS_ERR_BAD_FLAGS,__FILE__,__LINE__);
     return SA_AIS_ERR_BAD_FLAGS;
   }
...
/* Checking is postponed for the B0104 changes */
   ...
   if ( (rc = asapi_opr_hdlr(&asapi_or)) != SA_AIS_OK )
   {
      m_LOG_MQSV_A(MQA_ASAPi_GETDEST_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      goto done;
   } 
   ...
   if (num_queues != 0)
   {
      temp_notificationBuffer.notification = m_MMGR_ALLOC_MQA_TRACK_BUFFER_INFO((uns32)num_queues);

      if (!temp_notificationBuffer.notification)
      {
         m_LOG_MQSV_A(MQA_TRACK_BUFFER_INFO_ALLOC_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,SA_AIS_ERR_NO_MEMORY,__FILE__,__LINE__);
         return SA_AIS_ERR_NO_MEMORY;
      }
   ...
   if (trackFlags & SA_TRACK_CURRENT)
   {
      if (!notificationBuffer) 
      {
         ...
         {
            rc = SA_AIS_ERR_NO_MEMORY;
            m_LOG_MQSV_A(MQP_ASYNC_RSP_MSG_ALLOC_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
            goto done;
         }
         ...
         if (num_queues != 0)
         {
            track_current_callback->params.qGrpTrack.notificationBuffer.notification =
                      m_MMGR_ALLOC_MQA_TRACK_BUFFER_INFO((uns32)num_queues);
         ...
         {
            rc = SA_AIS_ERR_LIBRARY;
            m_LOG_MQSV_A(MQA_CLBK_QUEUE_WRITE_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,SA_AIS_ERR_LIBRARY,__FILE__,__LINE__);
            goto done;
         }
      }   /* if (!notificationBuffer) */
      else
          ...
          if(!notificationBuffer->notification)
          {
             if (num_queues != 0)
             {
                ...
                notificationBuffer->notification =
                   (SaMsgQueueGroupNotificationT *)malloc((uns32)num_queues * sizeof(SaMsgQueueGroupNotificationT));
   ...
   if((trackFlags & SA_TRACK_CHANGES) || (trackFlags & SA_TRACK_CHANGES_ONLY))
   {
        ...
        {
           rc = SA_AIS_ERR_NO_SPACE;
           m_LOG_MQSV_A(MQA_TRACK_TREE_ADD_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,SA_AIS_ERR_NO_SPACE,__FILE__,__LINE__);
           m_NCS_UNLOCK(&mqa_cb->cb_lock, NCS_LOCK_WRITE);
           goto done;
        }
   ...
   {
      if(rc != SA_AIS_ERR_TRY_AGAIN)
           m_LOG_MQSV_A(MQA_API_MSG_GROUP_TRACK_FAIL,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
     else
           m_LOG_MQSV_A(MQA_API_MSG_GROUP_TRACK_FAIL,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
   } 
   else
       m_LOG_MQSV_A(MQA_API_MSG_GROUP_TRACK_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
                         ...
                         const SaNameT *queueGroupName)
{
  ...
  /* Check if mqd, mqnd are up */ 
   ...
   {
      rc = SA_AIS_ERR_NOT_EXIST;
      m_LOG_MQSV_A(MQA_TRACK_TREE_FIND_FAILED,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      m_NCS_UNLOCK(&mqa_cb->cb_lock, NCS_LOCK_WRITE);
      goto done;
   }
   ...
   if (rc != SA_AIS_OK)
   {
    if(rc == SA_AIS_ERR_TRY_AGAIN || rc == SA_AIS_ERR_TIMEOUT)
     {
      m_LOG_MQSV_A(MQA_API_MSG_GROUP_TRACK_STOP_FAIL,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
     ...
     m_LOG_MQSV_A(MQA_API_MSG_GROUP_TRACK_STOP_FAIL,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
   }
   else
     m_LOG_MQSV_A(MQA_API_MSG_GROUP_TRACK_STOP_SUCCESS,NCSFL_LC_MQSV_QGRP_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
