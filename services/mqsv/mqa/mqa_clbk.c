...
static uns32  mqa_notify_changes(MQA_CLIENT_INFO *client_info, 
                           ...
                           MQP_ASYNC_RSP_MSG   *callback)
{
   ...
   if (!client_info)
   {
      m_LOG_MQSV_A(MQA_CLIENT_TREE_FIND_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__);
      m_MMGR_FREE_MQP_ASYNC_RSP_MSG(callback);
   ...
   case MQP_ASYNC_RSP_OPEN:
      {
         ...
         if (param->error == SA_AIS_OK) {
            ...
            queue_info = mqa_queue_tree_find_and_add(cb, param->queueHandle, TRUE, client_info, param->openFlags);
             ...
             */
             if(queue_info && (queue_info->openFlags & SA_MSG_QUEUE_RECEIVE_CALLBACK)) {          
                ...
                if (!openRsp) {
                   m_LOG_MQSV_A(MQP_OPEN_RSP_ALLOC_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__);
                ...
                {
                   m_LOG_MQSV_A(MQA_QUEUE_READER_TASK_CREATE_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
                ...
                else 
                {
                   ...
                   if (rc != NCSCC_RC_SUCCESS)
                   {
                      m_LOG_MQSV_A(MQA_QUEUE_READER_TASK_START_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
   ...
   case MQP_ASYNC_RSP_MSGRECEIVED:
      {
         ...
         {
            m_LOG_MQSV_A(MQA_QUEUE_TREE_FIND_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__);
            m_NCS_UNLOCK(&cb->cb_lock, NCS_LOCK_WRITE);
            break;
         }
...
/****************************************************************************
  ...
  Notes         : None
******************************************************************************/
uns32 mqa_hdl_callbk_dispatch_one (MQA_CB            *cb, 
...
uns32  mqa_notify_changes(MQA_CLIENT_INFO *client_info, MQA_TRACK_INFO *track_info, ASAPi_MSG_INFO *asapi_msg)
{
   ...
   track_current_callback->params.qGrpTrack.notificationBuffer.queueGroupPolicy = asapi_msg->info.tntfy.oinfo.policy;
   ...
   if (asapi_msg->info.tntfy.opr != ASAPi_GROUP_DEL) {
      if ( !(callback_buffer = m_MMGR_ALLOC_MQA_TRACK_BUFFER_INFO((track_index+1)))) {
         m_LOG_MQSV_A(MQA_TRACK_BUFFER_INFO_ALLOC_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__);
         rc = NCSCC_RC_FAILURE;
         goto done;
      }
   ...
   switch (asapi_msg->info.tntfy.opr)
   {
      case ASAPi_GROUP_DEL:
         ...
         if (mqa_track_tree_find_and_del(client_info, &track_info->queueGroupName) != TRUE) {
            m_LOG_MQSV_A(MQA_TRACK_TREE_FIND_AND_DEL_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__);
            rc = NCSCC_RC_FAILURE;
            goto done;
         } 
         break;
      ...
      case ASAPi_QUEUE_ADD:
         ...
         track_info->notificationBuffer.notification = m_MMGR_ALLOC_MQA_TRACK_BUFFER_INFO((uns32)(track_index+1));
         if(!track_info->notificationBuffer.notification) {
            m_LOG_MQSV_A(MQA_TRACK_BUFFER_INFO_ALLOC_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__);
            rc = NCSCC_RC_FAILURE;
            goto done;
   ...
   if (mqsv_mqa_callback_queue_write(mqa_cb, client_info->msgHandle, track_current_callback) != NCSCC_RC_SUCCESS) {
       m_LOG_MQSV_A(MQA_CLBK_QUEUE_WRITE_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__);
       rc = NCSCC_RC_FAILURE;
       goto done1;
   }
...
uns32  mqa_notify_changes_only(MQA_CLIENT_INFO *client_info, MQA_TRACK_INFO *track_info, ASAPi_MSG_INFO *asapi_msg)
{
   ...
   switch (asapi_msg->info.tntfy.opr)
   {
      case ASAPi_QUEUE_ADD:
      case ASAPi_QUEUE_DEL:
         track_current_callback->params.qGrpTrack.notificationBuffer.notification->member.queueName = asapi_msg->info.tntfy.oinfo.qparam->name;
         track_current_callback->params.qGrpTrack.error= SA_AIS_OK;
         ...
         if (asapi_msg->info.tntfy.opr == ASAPi_QUEUE_ADD)
            track_current_callback->params.qGrpTrack.notificationBuffer.notification->change = SA_MSG_QUEUE_GROUP_ADDED;
         else
            track_current_callback->params.qGrpTrack.notificationBuffer.notification->change = SA_MSG_QUEUE_GROUP_REMOVED;
      ...
      case ASAPi_GROUP_ADD:
         track_current_callback->params.qGrpTrack.notificationBuffer.notification->change = SA_MSG_QUEUE_GROUP_STATE_CHANGED;
         goto done;
         break;
   ...
   if (rc != NCSCC_RC_SUCCESS) {
       m_LOG_MQSV_A(MQA_NOTIFY_CHANGES_ONLY_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
       if(track_current_callback)
       {  
        m_MMGR_FREE_MQP_ASYNC_RSP_MSG(track_current_callback);
       }
...
uns32 mqsv_mqa_callback_queue_init (MQA_CLIENT_INFO  *client_info)
{
   ...
   m_LOG_MQSV_A(MQA_CLBK_QUEUE_INIT_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__);
   return NCSCC_RC_FAILURE;
}
...
void mqa_queue_reader (NCSCONTEXT arg)
{
   ...
   if ( (queue_node = mqa_queue_tree_find_and_add(mqa_cb, queueHandle, FALSE, NULL, 0)) == NULL) {
     m_LOG_MQSV_A(MQA_QUEUE_TREE_ADD_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__);
     m_NCS_UNLOCK(&mqa_cb->cb_lock, NCS_LOCK_WRITE);
     m_MQSV_MQA_GIVEUP_MQA_CB;
     return ;
   }
   ...
   while (1) {
      ...
      if (existing_msg_count) existing_msg_count--;
