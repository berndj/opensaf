...
uns32 mqa_mds_register (MQA_CB *cb)
{
   ...
   {
      m_LOG_MQSV_A(MQA_MDS_SUBSCRIPTION_ND_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      goto error;
   }
   ...
   if( (rc = ncsmds_api(&svc_info)) == NCSCC_RC_FAILURE)
   {
      m_LOG_MQSV_A(MQA_MDS_SUBSCRIPTION_D_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      goto error;
   }
  ...
  /* check the event type */
   if(evt->type == MQSV_EVT_MQA_CALLBACK)
   {
      ...
      if((rc = mqa_stop_and_delete_timer(mqa_callbk_info) )==NCSCC_RC_SUCCESS)
      {
        ...
        m_LOG_MQSV_A(MQA_STOP_DELETE_TMR_SUCCESS,NCSFL_LC_MQSV_INIT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
      }
      else
      {
        m_LOG_MQSV_A(MQA_STOP_DELETE_TMR_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
        if (mqa_callbk_info)
   ...
   else if(MQSV_EVT_ASAPI == evt->type) 
   {
      ...
      if(rc != NCSCC_RC_SUCCESS)
         m_LOG_MQSV_A(MQA_ASAPi_MSG_RECEIVE_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
     if(evt)
        m_MMGR_FREE_MQA_EVT(evt);
     return rc;
  ...
  uns32 to_dest_slotid, o_msg_fmt_ver;
   ...
   case NCSMDS_DOWN:
      ...
      case NCSMDS_SVC_ID_MQND:
         ...
         m_LOG_MQSV_A(MQA_MQND_DOWN,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,m_NCS_NODE_ID_FROM_MDS_DEST(svc_evt->i_dest),__FILE__,__LINE__);
   ...
   case NCSMDS_UP:
      ...
      case NCSMDS_SVC_ID_MQND:
        ...
        if(!o_msg_fmt_ver)
         ...
         if(m_NCS_NODE_ID_FROM_MDS_DEST(cb->mqa_mds_dest) == m_NCS_NODE_ID_FROM_MDS_DEST(svc_evt->i_dest))
         {
         ...
         m_LOG_MQSV_A(MQA_MQND_UP,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,m_NCS_NODE_ID_FROM_MDS_DEST(svc_evt->i_dest),__FILE__,__LINE__);
         break;
                             ...
                             uns32    length)
{
   ...
   if ( rc == NCSCC_RC_SUCCESS) {
      ...
      if (pEvt->endianness != endianness) {
         ...
         if (pEvt->type == MQSV_EVT_MQP_RSP) 
          {
            /* saMsgMessageSend response from MQND */
            pEvt->info.sendMsgRsp.error = m_MQSV_REVERSE_ENDIAN_L(&pEvt->info.sendMsgRsp.error, endianness);
             ...
             switch (pEvt->type) {
              ...
              case MQP_EVT_REPLY_MSG:
              {
                 ...
                 pEvt->info.replyMsg.message.type = 
                     m_MQSV_REVERSE_ENDIAN_L(&pEvt->info.replyMsg.message.type, endianness);

                 pEvt->info.replyMsg.message.version = 
                     m_MQSV_REVERSE_ENDIAN_L(&pEvt->info.replyMsg.message.version, endianness);

                 pEvt->info.replyMsg.message.size = 
                     m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.replyMsg.message.size, endianness);
 
                 pEvt->info.replyMsg.message.senderName.length = 
                     m_MQSV_REVERSE_ENDIAN_S(&pEvt->info.replyMsg.message.senderName.length, endianness);

                 pEvt->info.replyMsg.msgHandle = 
                     m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.replyMsg.msgHandle, endianness);

                 pEvt->info.replyMsg.messageInfo.sendReceive = 
                     m_MQSV_REVERSE_ENDIAN_L(&pEvt->info.replyMsg.messageInfo.sendReceive, endianness);

                 pEvt->info.replyMsg.messageInfo.sender.senderId = 
                     m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.replyMsg.messageInfo.sender.senderId, endianness);
 
                 pEvt->info.replyMsg.messageInfo.sendTime = 
                     m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.replyMsg.messageInfo.sendTime, endianness);
              }
              break;
              ...
              case MQP_EVT_REPLY_MSG_ASYNC:
              {
                 pEvt->info.replyAsyncMsg.reply.ackFlags = 
                     m_MQSV_REVERSE_ENDIAN_L(&pEvt->info.replyAsyncMsg.reply.ackFlags, endianness);

                 pEvt->info.replyAsyncMsg.invocation = 
                     m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.replyAsyncMsg.invocation, endianness);

                 pEvt->info.replyAsyncMsg.reply.message.type = 
                     m_MQSV_REVERSE_ENDIAN_L(&pEvt->info.replyAsyncMsg.reply.message.type, endianness);

                 pEvt->info.replyAsyncMsg.reply.message.version =
                     m_MQSV_REVERSE_ENDIAN_L(&pEvt->info.replyAsyncMsg.reply.message.version, endianness);

                 pEvt->info.replyAsyncMsg.reply.message.size = 
                     m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.replyAsyncMsg.reply.message.size, endianness);

                 pEvt->info.replyAsyncMsg.reply.message.senderName.length =
                     m_MQSV_REVERSE_ENDIAN_S(&pEvt->info.replyAsyncMsg.reply.message.senderName.length, endianness);

                 pEvt->info.replyAsyncMsg.reply.msgHandle = 
                     m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.replyAsyncMsg.reply.msgHandle, endianness);

                 pEvt->info.replyAsyncMsg.reply.messageInfo.sendReceive =
                     m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.replyAsyncMsg.reply.messageInfo.sendReceive, endianness);
      
                 pEvt->info.replyAsyncMsg.reply.messageInfo.sender.senderId = 
                     m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.replyAsyncMsg.reply.messageInfo.sender.senderId, endianness);

                 pEvt->info.replyAsyncMsg.reply.messageInfo.sendTime = 
                     m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.replyAsyncMsg.reply.messageInfo.sendTime, endianness);
              } 
              break;
              default:
                return NCSCC_RC_FAILURE;
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
static uns32 mqa_mds_msg_sync_reply (uns32 mqa_mds_hdl,
                             MDS_DEST  *destination,
                             MQSV_EVT   *i_evt,
                             uns32  timeout,
                             MDS_SYNC_SND_CTXT *context)
{
   ...
   /* Before entering any mds send function, the caller locks the control block
    * with LOCK_WRITE.
    * unlock the control block before MDS send and lock it after we receive the reply
    * from MDS.
    */
   ...
   mds_info.info.svc_send.i_msg = (NCSCONTEXT)i_evt;
   mds_info.info.svc_send.i_priority = MDS_SEND_PRIORITY_MEDIUM;
   mds_info.info.svc_send.i_to_svc = NCSMDS_SVC_ID_MQA;
   mds_info.info.svc_send.i_sendtype = MDS_SENDTYPE_SNDRACK;
   
   /* fill the send rack strcuture */
   mds_info.info.svc_send.info.sndrack.i_time_to_wait = timeout; /* timeto wait in 10ms */
   mds_info.info.svc_send.info.sndrack.i_sender_dest = *destination;
   mds_info.info.svc_send.info.sndrack.i_msg_ctxt = *context;
                             ...
                             uns32 length)
{
   ...
   if(!mqa_cb) {
      m_LOG_MQSV_A(MQA_CB_RETRIEVAL_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__);
      mds_free_direct_buff((MDS_DIRECT_BUFF)i_evt);
      return NCSCC_RC_FAILURE;
   }
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 mqa_mds_msg_async_send (uns32 mqa_mds_hdl, 
                              MDS_DEST *destination,
                              MQSV_EVT   *i_evt,
                              uns32    to_svc
                              )
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
static uns32 mqa_mds_msg_async_reply (uns32 mqa_mds_hdl,
                             MDS_DEST  *destination,
                             MQSV_EVT   *i_evt,
                             MDS_SYNC_SND_CTXT *context)
{
   ...
   mds_info.info.svc_send.info.rsp.i_sender_dest = *destination;
