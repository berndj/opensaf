...
uns32 mqa_lib_req(NCS_LIB_REQ_INFO *req_info)
{
   ...
   case NCS_LIB_REQ_CREATE:
      ...
      if(rc == NCSCC_RC_SUCCESS)
         m_LOG_MQSV_A(MQA_SE_API_CREATE_SUCCESS,NCSFL_LC_MQSV_INIT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
      else
         m_LOG_MQSV_A(MQA_SE_API_CREATE_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      break;
      
   case NCS_LIB_REQ_DESTROY:
      rc = mqa_destroy(&req_info->info.destroy);
      if(rc == NCSCC_RC_SUCCESS)
         m_LOG_MQSV_A(MQA_SE_API_DESTROY_SUCCESS,NCSFL_LC_MQSV_INIT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
      else
         m_LOG_MQSV_A(MQA_SE_API_DESTROY_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      break;
...
static uns32 mqa_create (NCS_LIB_CREATE *create_info)
{
   ...
   {
      m_LOG_MQSV_A(MQA_REGISTER_WITH_ASAPi_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      goto error8;
   }
...
static void mqa_client_tree_destroy(MQA_CB *mqa_cb)
{
   ...
   {
      m_LOG_MQSV_A(MQA_CLIENT_TREE_DESTROY_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      return;
   }   
...
MQA_CLIENT_INFO *mqa_client_tree_find_and_add(MQA_CB *mqa_cb, SaMsgHandleT hdl_id, NCS_BOOL flag)
{
   ...
   if(flag == TRUE)
   {
      ...
      if(!client_info)
      {
         ...
         if((rc = ncs_patricia_tree_add (&mqa_cb->mqa_client_tree, &client_info->patnode)) != NCSCC_RC_SUCCESS)
         {
   ...
   if(flag == TRUE)
   {
      ...
      if(!track_info)
      {
        ...
        if(( rc = ncs_patricia_tree_add (&client_info->mqa_track_tree, 
                                  &track_info->patnode)) != NCSCC_RC_SUCCESS)
         {
            m_LOG_MQSV_A(MQA_TRACK_TREE_ADD_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
            if (track_info->notificationBuffer.notification)
                m_MMGR_FREE_MQA_TRACK_BUFFER_INFO(track_info->notificationBuffer.notification);
            m_MMGR_FREE_MQA_TRACK_INFO(track_info);
            return NULL;
...
******************************************************************************/
NCS_BOOL mqa_track_tree_find_and_del(MQA_CLIENT_INFO *client_info,
                                          SaNameT *group)
{
   MQA_TRACK_INFO  *track_info=NULL;
   uns32           rc = NCSCC_RC_SUCCESS;
...
static void mqa_queue_tree_destroy(MQA_CB *mqa_cb)
{
   ...
   if ((rc = (m_NCS_LOCK(&mqa_cb->cb_lock, NCS_LOCK_WRITE))) != NCSCC_RC_SUCCESS)
   {
      m_LOG_MQSV_A(MQA_QUEUE_TREE_DESTROY_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
      return;
   }   
                                            ...
                                            SaMsgQueueOpenFlagsT openFlags)
{
   ...
   if(flag == TRUE)
   {
      ...
      if(!queue_info)
      {
         ...
         if((rc = ncs_patricia_tree_add (&mqa_cb->mqa_queue_tree, &queue_info->patnode)) != NCSCC_RC_SUCCESS)
         {
            m_LOG_MQSV_A(MQA_QUEUE_TREE_ADD_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
            m_MMGR_FREE_MQA_QUEUE_INFO(queue_info);
