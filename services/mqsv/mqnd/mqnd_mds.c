...
static uns32 mqnd_mds_callback(struct ncsmds_callback_info *info)
{
   ...
   if( rc == NCSCC_RC_SUCCESS)
      m_LOG_MQSV_ND(MQND_MDS_CLBK_COMPLETE,NCSFL_LC_MQSV_INIT,NCSFL_SEV_INFO,info->i_op,__FILE__,__LINE__);
   else
      m_LOG_MQSV_ND(MQND_MDS_CLBK_COMPLETE,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,info->i_op,__FILE__,__LINE__);
...
static uns32 mqnd_mds_rcv(MQND_CB *pMqnd, MDS_CALLBACK_RECEIVE_INFO *rcv_info)
{
   ...
   if(NCSCC_RC_SUCCESS != rc) {
     m_LOG_MQSV_ND(MQND_MDS_SND_TO_MAILBOX_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
   }   
   return rc;
}
         ...
         case MQP_EVT_SEND_MSG:
           {
              ...
              if (pEvt->info.snd_msg.messageInfo.sendReceive == SA_FALSE) {
                 pEvt->info.snd_msg.messageInfo.sender.senderId = 
                      m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.snd_msg.messageInfo.sender.senderId, endianness);
              }
              else {
                 pEvt->info.snd_msg.messageInfo.sender.sender_context.sender_dest = 
                      m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.snd_msg.messageInfo.sender.sender_context.sender_dest, 
                            endianness);

                 pEvt->info.snd_msg.messageInfo.sender.sender_context.reply_buffer_size = 
                      m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.snd_msg.messageInfo.sender.sender_context.reply_buffer_size, endianness);
              }
         ...
         case MQP_EVT_SEND_MSG_ASYNC:
           {
              ...
              pEvt->info.sndMsgAsync.SendMsg.destination.length = 
                      m_MQSV_REVERSE_ENDIAN_S(&pEvt->info.sndMsgAsync.SendMsg.destination.length, endianness);
              ...
              pEvt->info.sndMsgAsync.SendMsg.messageInfo.sendTime = 
                      m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.sndMsgAsync.SendMsg.messageInfo.sendTime, endianness);

              pEvt->info.sndMsgAsync.SendMsg.messageInfo.sendReceive = 
                      m_MQSV_REVERSE_ENDIAN_L(&pEvt->info.sndMsgAsync.SendMsg.messageInfo.sendReceive, endianness);

              pEvt->info.sndMsgAsync.SendMsg.messageInfo.sender.senderId = 
                      m_MQSV_REVERSE_ENDIAN_LL(&pEvt->info.sndMsgAsync.SendMsg.messageInfo.sender.senderId, endianness);
              ...
              pEvt->info.sndMsgAsync.SendMsg.message.version = 
                      m_MQSV_REVERSE_ENDIAN_L(&pEvt->info.sndMsgAsync.SendMsg.message.version, endianness);
              ...
              pEvt->info.sndMsgAsync.SendMsg.message.senderName.length = 
                      m_MQSV_REVERSE_ENDIAN_S(&pEvt->info.sndMsgAsync.SendMsg.message.senderName.length, endianness);
   ...
   case NCSMDS_DOWN:
      if(svc_evt->i_svc_id == NCSMDS_SVC_ID_MQD)
      {
         if(cb->is_mqd_up == TRUE)
         {
            ...
            cb->is_mqd_up = FALSE;
            m_LOG_MQSV_ND(MQND_MQD_SERVICE_WENT_DOWN,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
            return NCSCC_RC_SUCCESS;
         }
      }
      else if(svc_evt->i_svc_id == NCSMDS_SVC_ID_MQA)
      {
          ...
          m_LOG_MQSV_ND(MQND_MQA_SERVICE_WENT_DOWN,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,m_NCS_NODE_ID_FROM_MDS_DEST(svc_evt->i_dest),__FILE__,__LINE__);
        ...
        case NCSMDS_SVC_ID_MQD:
         {
           ...
           m_LOG_MQSV_ND(MQND_MQD_SERVICE_CAME_UP,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__);
        ...
        case NCSMDS_SVC_ID_MQA:
         {
          ...
          /* Post the event to Update the MQA list */
            ...
            m_LOG_MQSV_ND(MQND_MQA_CAME_UP,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,m_NCS_NODE_ID_FROM_MDS_DEST(svc_evt->i_dest),__FILE__,__LINE__);
...
uns32 mqnd_mds_send_rsp_direct(MQND_CB *cb, MQSV_DSEND_INFO *s_info, MQSV_DSEND_EVT *evt)
{
   ...
   if(rc != NCSCC_RC_SUCCESS)
   {
      m_LOG_MQSV_ND(MQND_MDS_SND_RSP_DIRECT_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
   }
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 mqnd_mds_send (MQND_CB *cb, uns32 to_svc, 
