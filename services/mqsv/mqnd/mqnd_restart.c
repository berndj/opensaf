   ...
   if(cb->is_create_ckpt == FALSE)
     {
      ...
      if(rc != NCSCC_RC_SUCCESS)
       {
        m_LOG_MQSV_ND(MQND_RESTART_BUILD_DB_FROM_CKPTSVC_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
...
static void mqnd_fill_queue_node(MQND_QUEUE_CKPT_INFO *ckpt_queue_info,MQND_QUEUE_INFO *queue_info)
{
  ...
  for(i=SA_MSG_MESSAGE_HIGHEST_PRIORITY; i<=SA_MSG_MESSAGE_LOWEST_PRIORITY; i++)
  {
    ...
    queue_info->queueStatus.saMsgQueueUsage[i].queueSize = ckpt_queue_info->QueueStatsShm.saMsgQueueUsage[i].queueSize;
  }

  if(ckpt_queue_info->qtransfer_complete_tmr.type == MQND_TMR_TYPE_NODE2_QTRANSFER && ckpt_queue_info->qtransfer_complete_tmr.is_active &&                       ckpt_queue_info->qtransfer_complete_tmr.tmr_id!=NULL)
   {
...
static uns32 mqnd_restart_queue_node_add(MQND_CB *cb, MQND_QUEUE_NODE* qnode)
{
  ...
  if(1)
  {
    ...
    if(!(qnode->qinfo.sendingState == SA_MSG_QUEUE_UNAVAILABLE))
    {
       ...
       {
          rc = mqsv_reg_mqndmib_queue_priority_tbl_row(cb,qnode->qinfo.queueName,counter,&qnode->qinfo.mab_rec_priority_row_hdl[counter]);
          if(rc != NCSCC_RC_SUCCESS)
          {
            if(pnode)
           {
   ...
   if(qnode->qinfo.qtransfer_complete_tmr.type == MQND_TMR_TYPE_NODE2_QTRANSFER &&                                                 qnode->qinfo.qtransfer_complete_tmr.is_active && qnode->qinfo.qtransfer_complete_tmr.tmr_id!=TMR_T_NULL)
    {
     rc =  mqnd_tmr_start(&qnode->qinfo.qtransfer_complete_tmr, MQND_QTRANSFER_REQ_TIMER);  
     if(rc == NCSCC_RC_SUCCESS)
       m_LOG_MQSV_ND(MQND_QTRANSFER_NODE2_TMR_STARTED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_INFO,                                                          qnode->qinfo.qtransfer_complete_tmr.qhdl,__FILE__,__LINE__);
     else
       m_LOG_MQSV_ND(MQND_QTRANSFER_NODE2_TMR_STARTED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_ERROR,                                                         qnode->qinfo.qtransfer_complete_tmr.qhdl,__FILE__,__LINE__);
    } 
         ...
         (qnode->qinfo.queueStatus.closeTime))  
   {
      ...
      if((qnode->qinfo.queueStatus.retentionTime) > (SA_TIME_ONE_SECOND*(presentTime - qnode->qinfo.queueStatus.closeTime)))
      { 
       timeout = (qnode->qinfo.queueStatus.retentionTime) - (SA_TIME_ONE_SECOND*(presentTime - qnode->qinfo.queueStatus.closeTime));  
      ...
      if(timeout) 
      {
        /* Retention timer started before restart, not yet expired by now */
         ...
         m_LOG_MQSV_ND(MQND_RETENTION_TMR_STARTED,NCSFL_LC_MQSV_Q_MGMT,NCSFL_SEV_INFO,rc,__FILE__,__LINE__);
...
void mqnd_cpy_qnodeinfo_to_ckptinfo(MQND_CB *cb, MQND_QUEUE_NODE *queue_info,MQND_QUEUE_CKPT_INFO *ckpt_queue_info)
{
   ...
   for(i=SA_MSG_MESSAGE_HIGHEST_PRIORITY; i<=SA_MSG_MESSAGE_LOWEST_PRIORITY; i++)
   {
      ...
      ckpt_queue_info->QueueStatsShm.saMsgQueueUsage[i].queueSize = queue_info->qinfo.queueStatus.saMsgQueueUsage[i].queueSize;
      /*For persistent queue the queue used stats are to be retained */
      ckpt_queue_info->QueueStatsShm.saMsgQueueUsage[i].queueUsed = shm_base_addr[offset].QueueStatsShm.saMsgQueueUsage[i].queueUsed;              
      ckpt_queue_info->QueueStatsShm.saMsgQueueUsage[i].numberOfMessages = shm_base_addr[offset].QueueStatsShm.saMsgQueueUsage[i].numberOfMessages;
   }
      /*For persistent queue the queue used stats are to be retained */
   ckpt_queue_info->QueueStatsShm.totalNumberOfMessages = shm_base_addr[offset].QueueStatsShm.totalNumberOfMessages;
   ckpt_queue_info->QueueStatsShm.totalQueueUsed = shm_base_addr[offset].QueueStatsShm.totalQueueUsed;

   if(queue_info->qinfo.qtransfer_complete_tmr.type == MQND_TMR_TYPE_NODE2_QTRANSFER && queue_info->qinfo.qtransfer_complete_tmr.is_active &&                      queue_info->qinfo.qtransfer_complete_tmr.tmr_id!=TMR_T_NULL)
    {  
