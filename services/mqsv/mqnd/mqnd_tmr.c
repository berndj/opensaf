...
static void mqnd_timer_expiry (NCSCONTEXT uarg)
{
   ...
   if (tmr != NULL)
   {
      ...
      {         
         ...
         {
            m_LOG_MQSV_ND(MQND_MQA_TMR_EXPIRED,NCSFL_LC_TIMER,NCSFL_SEV_NOTICE,NCSCC_RC_SUCCESS,__FILE__,__LINE__);
         }
         evt = m_MMGR_ALLOC_MQSV_EVT(NCS_SERVICE_ID_MQND);
         if (evt == NULL)
         {
            m_LOG_MQSV_ND(MQND_EVT_ALLOC_FAILED,NCSFL_LC_TIMER,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__);
            return;
         }
 ...
 *****************************************************************************/
uns32
mqnd_tmr_start (MQND_TMR *tmr, SaTimeT duration)
{
