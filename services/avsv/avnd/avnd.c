...
uns32 avnd_lib_req (NCS_LIB_REQ_INFO *req_info)
{
   ...
   case NCS_LIB_REQ_CREATE:
      ...
      {
         m_AVND_LOG_SEAPI(AVSV_LOG_SEAPI_CREATE, AVSV_LOG_SEAPI_SUCCESS, 
                          NCSFL_SEV_INFO);
      }
      else
      {
         m_AVND_LOG_SEAPI(AVSV_LOG_SEAPI_CREATE, AVSV_LOG_SEAPI_FAILURE, 
                          NCSFL_SEV_CRITICAL);
      }
      break;
...
uns32 avnd_destroy ()
{
   ...
   /* return avnd cb */
   if (cb) ncshm_give_hdl((uns32)gl_avnd_hdl);
...
AVND_CB *avnd_cb_create ()
{
   ...
   if ( (0 == (cb->cb_hdl = ncshm_create_hdl(cb->pool_id, NCS_SERVICE_ID_AVND, 
                                        (NCSCONTEXT)cb))) )
   {
...
err:
   /* destroy the task */
   if (gl_avnd_task_hdl) avnd_task_destroy();
