...
uns32 avnd_evt_avd_verify_message (AVND_CB *cb, AVND_EVT *evt)
{
   ...
   m_AVND_LOG_FOVER_EVTS(NCSFL_SEV_NOTICE, 
      AVND_LOG_FOVR_VERIFY_MSG_RCVD, NCSCC_RC_SUCCESS);
   ...
   m_AVND_LOG_FOVER_EVTS(NCSFL_SEV_DEBUG, 
      AVND_LOG_FOVR_AVD_SND_ID_CNT, info->snd_id_cnt);

   m_AVND_LOG_FOVER_EVTS(NCSFL_SEV_DEBUG, 
      AVND_LOG_FOVR_AVND_RCV_ID_CNT, cb->rcv_msg_id);
    ...
    */
   m_AVND_LOG_FOVER_EVTS(NCSFL_SEV_DEBUG, 
      AVND_LOG_FOVR_AVD_RCV_ID_CNT, info->rcv_id_cnt);

   m_AVND_LOG_FOVER_EVTS(NCSFL_SEV_DEBUG, 
      AVND_LOG_FOVR_AVND_SND_ID_CNT, cb->snd_msg_id);
   ...
   if ((cb->snd_msg_id != info->rcv_id_cnt) &&
       (msg_found == FALSE))
   {
      /* Log error, seems to be some problem. */
      m_AVND_LOG_FOVER_EVTS(NCSFL_SEV_EMERGENCY, 
            AVND_LOG_FOVR_REC_NOT_FOUND, info->rcv_id_cnt);
