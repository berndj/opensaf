...
{ 
   ...
   trk_info = avnd_clm_trkinfo_list_find(cb, api_info->param.track_start.hdl,
                                         &cla_evt.mds_dest);
   if(trk_info)
   {
...
{ 
   ...
   trk_info = avnd_clm_trkinfo_list_find(cb, api_info->param.track_start.hdl,
                                         &cla_evt.mds_dest);
   if( (trk_info) && 
       ( m_AVND_CLM_IS_TRACK_CHANGES(trk_info)  ||
         m_AVND_CLM_IS_TRACK_CHANGES_ONLY(trk_info) )
     )
   {
...
uns32 avnd_evt_avd_clm_node_on_fover (AVND_CB *cb, AVND_EVT *evt)
{
   ...
   m_AVND_LOG_FOVER_EVTS(NCSFL_SEV_NOTICE,
      AVND_LOG_FOVR_CLM_NODE_INFO, NCSCC_RC_SUCCESS);
...
uns32 avnd_evt_avd_node_update_msg (AVND_CB *cb, AVND_EVT *evt)
{
   ...
   if (SA_TRUE == info->clm_info.member) /* => node addition */
   {
      ...
      if (rec) avnd_clm_snd_track_changes(cb, rec, SA_CLM_NODE_JOINED);
   }
   else /* => node deletion */
   {
...
uns32 avnd_evt_avd_node_up_msg (AVND_CB *cb, AVND_EVT *evt)
{
   ...
   for ( curr = info->list_of_nodes; curr; curr = curr->next)
   {
      ...
      if (rec->info.node_id == cb->clmdb.node_info.nodeId)
      {
         ...
         cb->clmdb.node_info.nodeName.length = 
             m_NCS_OS_NTOHS(cb->clmdb.node_info.nodeName.length);
      }
   }
...
******************************************************************************/
void avnd_clm_snd_track_changes(AVND_CB *cb, 
                                AVND_CLM_REC *current_rec, 
   ...
   while(trk_info)
   {
      if(m_AVND_CLM_IS_TRACK_CHANGES_ONLY(trk_info))
      {
         ...
         notify->clusterNode.nodeName.length = 
            m_NCS_OS_NTOHS( current_rec->info.node_name_net.length );
