...
static uns32 avnd_err_escalate (AVND_CB *, AVND_SU *, AVND_COMP *, 
                                AVSV_ERR_RCVR *);

static uns32 avnd_err_recover (AVND_CB *, AVND_SU *, AVND_COMP *, 
                               AVSV_ERR_RCVR);
...
static uns32 avnd_err_restart_esc_level_0(AVND_CB *, AVND_SU *, AVND_ERR_ESC_LEVEL  *,
                                                                          AVSV_ERR_RCVR  *);
static uns32 avnd_err_restart_esc_level_1(AVND_CB *, AVND_SU *, AVND_ERR_ESC_LEVEL  *,
                                                                          AVSV_ERR_RCVR  *);
...
uns32 avnd_evt_ava_err_rep (AVND_CB *cb, AVND_EVT *evt)
{
  ...
  if(FALSE == msg_from_avnd)
  {
   ...
   /* get the comp */
   comp = m_AVND_COMPDB_REC_GET(cb->compdb, err_rep->err_comp_net);
   ...
   if((TRUE == comp->su->su_is_external) && ((TRUE == msg_from_avnd)))
   {
     /* This request has come from other AvND and it is for external component.
        ...
        if((SA_AMF_NODE_SWITCHOVER == err_rep->rec_rcvr) ||
           ...
           (SA_AMF_NODE_FAILFAST == err_rep->rec_rcvr) ||
           (SA_AMF_CLUSTER_RESET == err_rep->rec_rcvr)) 
           amf_rc = SA_AIS_ERR_INVALID_PARAM;
   }
  ...
  if(NCSCC_RC_SUCCESS != rc)
  {
   m_AVND_AVND_ERR_LOG("avnd_evt_ava_err_rep():Comp,Hdl and rec_rcvr are",
                       &err_rep->err_comp_net,err_rep->hdl,err_rep->rec_rcvr, 0,0);
  }
...
uns32 avnd_evt_ava_err_clear (AVND_CB *cb, AVND_EVT *evt)
{
  ...
  if(FALSE == msg_from_avnd)
  {
   ...
   /* get the comp */
   comp = m_AVND_COMPDB_REC_GET(cb->compdb, err_clear->comp_name_net);
  ...
  if(NCSCC_RC_SUCCESS != rc)
  {
   m_AVND_AVND_ERR_LOG("avnd_evt_ava_err_clear():Comp and Hdl are",
                       &err_clear->comp_name_net,err_clear->hdl,0,0,0);
   }
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 avnd_err_process (AVND_CB       *cb,
                        AVND_COMP     *comp,
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 avnd_err_escalate (AVND_CB   *cb, 
                         AVND_SU   *su, 
                         AVND_COMP *comp, 
                         AVSV_ERR_RCVR *io_esc_rcvr)
{
   uns32 rc = NCSCC_RC_SUCCESS;
   if(comp->pres == NCS_PRES_UNINSTANTIATED ||
         comp->pres == NCS_PRES_INSTANTIATIONFAILED||
         comp->pres == NCS_PRES_TERMINATIONFAILED)
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 avnd_err_recover (AVND_CB   *cb, 
                        AVND_SU   *su, 
                        AVND_COMP *comp, 
                        AVSV_ERR_RCVR rcvr)
{
   ...
   if(comp->pres == NCS_PRES_INSTANTIATING)
   {
   ...
   if((su->pres == NCS_PRES_INSTANTIATIONFAILED) &&
      (comp->pres == NCS_PRES_TERMINATING) &&
   ...
   if((comp->pres == NCS_PRES_TERMINATING) && (rcvr == SA_AMF_COMPONENT_RESTART))
   {
      rc = avnd_comp_clc_fsm_run(cb, comp, AVND_COMP_CLC_PRES_FSM_EV_CLEANUP);
      return rc;
   }
    ...
    */
   if((su->pres == NCS_PRES_TERMINATING ) && (rcvr != SA_AMF_NODE_FAILOVER)
...
uns32 avnd_err_rcvr_comp_restart (AVND_CB *cb, AVND_COMP *comp)
{
   ...
   if (!m_AVND_COMP_TYPE_IS_PREINSTANTIABLE(comp))
   {
      ...
      if(m_AVND_COMP_CSI_CURR_ASSIGN_STATE_IS_ASSIGNED(csi) || m_AVND_COMP_CSI_CURR_ASSIGN_STATE_IS_RESTARTING(csi))
      {
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 avnd_err_rcvr_su_restart (AVND_CB   *cb, 
                                AVND_SU   *su, 
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 avnd_err_rcvr_su_failover (AVND_CB   *cb, 
                                 AVND_SU   *su, 
                                 AVND_COMP *failed_comp)
{
   uns32 rc = NCSCC_RC_SUCCESS;
   ...
   if(su->pres == NCS_PRES_RESTARTING || m_AVND_SU_IS_RESTART(su))
   {
...
******************************************************************************/
uns32 avnd_err_rcvr_node_failover (AVND_CB   *cb,
                                   AVND_SU   *failed_su,
                                   AVND_COMP *failed_comp)
{
   uns32 rc = NCSCC_RC_SUCCESS;
  ...
  /*
   ...
   /* We are now in the context of failover, forget the restart */
   if(failed_su->pres == NCS_PRES_RESTARTING || m_AVND_SU_IS_RESTART(failed_su))
   {
...
uns32 avnd_err_su_repair (AVND_CB *cb, AVND_SU *su)
{
   ...
   m_AVSV_ASSERT(m_AVND_SU_IS_FAILED(su));
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 avnd_err_esc_comp_restart (AVND_CB *cb, AVND_SU *su,
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 avnd_err_restart_esc_level_0(AVND_CB *cb, AVND_SU *su, 
                      AVND_ERR_ESC_LEVEL  *esc_level, AVSV_ERR_RCVR  *esc_rcvr)
{

   uns32 rc = NCSCC_RC_SUCCESS;
   *esc_rcvr = AVSV_AMF_COMPONENT_RESTART;
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 avnd_err_restart_esc_level_1(AVND_CB *cb, AVND_SU *su, 
                      AVND_ERR_ESC_LEVEL  *esc_level, AVSV_ERR_RCVR  *esc_rcvr)
{
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 avnd_err_restart_esc_level_2(AVND_CB *cb, AVND_SU *su, 
                      AVND_ERR_ESC_LEVEL  *esc_level, AVSV_ERR_RCVR  *esc_rcvr)
{
   uns32 rc = NCSCC_RC_SUCCESS;
...
******************************************************************************/
uns32 avnd_err_rcvr_node_failfast (AVND_CB   *cb,
                                   AVND_SU   *failed_su,
                                   AVND_COMP *failed_comp)
{
   uns32 rc = NCSCC_RC_SUCCESS;
