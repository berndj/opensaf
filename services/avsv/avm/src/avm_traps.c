                   ...
                   )
{
   ...
   if(NCSCC_RC_SUCCESS != (status = avm_publish_trap(avm_cb, &snmptm_avm_trap, avm_switch_trap_pattern_array, AVM_SWITCH_TRAP_PATTERN_ARRAY_LEN)))
   {
      m_AVM_LOG_INVALID_VAL_ERROR(status);
   }
             ...
             )  
{
   ...
   saStatus = saEvtEventAttributesSet(eventHandle, &patternArray, priority, retention_time, &avm_cb->publisherName);
