...
void avd_node_up_func(AVD_CL_CB *cb,AVD_EVT *evt)
{
   ...
   if ((avnd = 
   avd_msg_sanity_chk(cb,evt,n2d_msg->msg_info.n2d_clm_node_up.node_id,AVSV_N2D_CLM_NODE_UP_MSG))
   == AVD_AVND_NULL)
   {
   ...
   if ((avnd->node_info.nodeId == cb->node_id_avd) ||
      (avnd->node_info.nodeId == cb->node_id_avd_other))
   {
...
void avd_nd_heartbeat_msg_func(AVD_CL_CB *cb,AVD_EVT *evt)
{
   ...
   if ((avnd = 
   avd_msg_sanity_chk(cb,evt,n2d_msg->msg_info.n2d_hrt_bt.node_id,AVSV_N2D_HEARTBEAT_MSG))
   == AVD_AVND_NULL)
   {
      /* sanity failed return */
      avsv_dnd_msg_free(n2d_msg);
...
void avd_nd_reg_comp_evt_hdl(AVD_CL_CB *cb,AVD_AVND *avnd)
{
      ...
      if (avnd->node_info.nodeId == cb->node_id_avd)
      {
         ...
         cb->cluster_init_time = ((uns64)boot_timestamp.seconds * 
                                    (uns64)(AVSV_NANOSEC_TO_LEAPTM) * (uns64)100)
                                             +
                                 ((uns64)boot_timestamp.millisecs * 
                                    (uns64)(AVSV_NANOSEC_TO_LEAPTM/10));         
   ...
   {
      if ((ncs_su->row_status != NCS_ROW_ACTIVE) ||
         (ncs_su->num_of_comp != ncs_su->curr_num_comp))
      {
...
void avd_fail_over_event(AVD_CL_CB *cb)
{
      ...
      (AVD_AVND *)ncs_patricia_tree_getnext(&cb->avnd_anchor,(uns8*)&node_id)))
   {
       ...
       */
      if ((AVD_AVND_STATE_ABSENT == avnd->node_state ) ||
          ((node_id == cb->node_id_avd_other) && 
           cb->role_switch == FALSE))
      {
         continue;
      }
...
void avd_ack_nack_event(AVD_CL_CB *cb, AVD_EVT *evt)
{
   ...
   if (NULL != (node_fovr = 
      ...
      (uns8*)&evt->info.avnd_msg->msg_info.n2d_ack_nack_info.node_id)))
   {
    ...
    */
   if (NULL == (avnd = avd_avnd_struc_find_nodeid(cb, 
   ...
   if (FALSE == evt->info.avnd_msg->msg_info.n2d_ack_nack_info.ack)
   {
       ...
       */
      for (su_ptr = avnd->list_of_ncs_su; su_ptr != NULL;
       ...
       */
      for (su_ptr = avnd->list_of_su; su_ptr != NULL;
      su_ptr = su_ptr->avnd_list_su_next)
      {
