...
uns32 avd_snd_node_update_msg(AVD_CL_CB *cb,AVD_AVND *avnd)
{
   ...
   d2n_msg->msg_info.d2n_clm_node_update.clm_info.node_id =
   ...
   if(avnd->node_info.member == SA_TRUE)
   {
      ...
      d2n_msg->msg_info.d2n_clm_node_update.clm_info.node_address = 
         avnd->node_info.nodeAddress;
      d2n_msg->msg_info.d2n_clm_node_update.clm_info.node_name_net =
...
static uns32 avd_prep_node_info(AVD_CL_CB *cb,AVD_AVND *avnd,
                                AVD_DND_MSG *nodeup_msg, NCS_BOOL verify)
{
   ...
   node_info = 
   (AVSV_CLM_INFO_MSG *) m_MMGR_ALLOC_AVSV_DND_MSG_INFO(sizeof(AVSV_CLM_INFO_MSG));
   ...
   {
      node_info->next = 
         nodeup_msg->msg_info.d2n_clm_node_fover.list_of_nodes;
   ...
   {
      node_info->next = 
         nodeup_msg->msg_info.d2n_clm_node_up.list_of_nodes;
...
uns32 avd_snd_node_up_msg(AVD_CL_CB *cb,AVD_AVND *avnd,uns32 msg_id_ack)
{
   ...
   d2n_msg->msg_info.d2n_clm_node_up.su_failover_prob = 
...
uns32 avd_snd_hbt_info_msg(AVD_CL_CB *cb)
{
   ...
   d2n_msg->msg_info.d2n_info_hrt_bt.snd_hb_intvl =
      cb->snd_hb_intvl;
...
static uns32 avd_prep_comp_info(AVD_CL_CB *cb,AVD_COMP *comp,
                                 AVD_DND_MSG *comp_msg)
{
   ...
   comp_info = 
       (AVSV_COMP_INFO_MSG *) m_MMGR_ALLOC_AVSV_DND_MSG_INFO(sizeof(AVSV_COMP_INFO_MSG));
   if (comp_info == NULL)
   {
...
uns32 avd_snd_su_comp_msg(AVD_CL_CB *cb,
                          AVD_AVND *avnd,
                          NCS_BOOL *comp_sent,
                          NCS_BOOL fail_over)
{
   ...
   *comp_sent = FALSE;
   ...
   su_msg->msg_info.d2n_reg_su.nodeid =
   comp_msg->msg_info.d2n_reg_comp.node_id = avnd->node_info.nodeId;

   su_msg->msg_info.d2n_reg_su.msg_on_fover =
   comp_msg->msg_info.d2n_reg_comp.msg_on_fover = fail_over;
...
static uns32 avd_prep_csi_attr_info(AVD_CL_CB *cb,AVSV_SUSI_ASGN *compcsi_info,
                       AVD_COMP_CSI_REL *compcsi)
{
   ...
   while ((attr_ptr != NULL) && 
      (compcsi_info->attrs.number < compcsi->csi->num_active_params))
   {
...
uns32 avd_snd_susi_msg(AVD_CL_CB *cb,AVD_SU *su,
                       AVD_SU_SI_REL *susi,AVSV_SUSI_ACT actn)
{
      ...
      while (l_compcsi != AVD_COMP_CSI_REL_NULL)
      {
            ...
            (susi_msg->msg_info.d2n_su_si_assign.ha_state == SA_AMF_HA_ACTIVE)))
         {
            ...
            }else
            {
               ...
               if ((trans_dsc == SA_AMF_CSI_QUIESCED) &&
                  (l_compcsi->csi_csicomp_next->comp->oper_state
                     == NCS_OPER_STATE_DISABLE))
               {
                  compcsi_info->active_comp_dsc = SA_AMF_CSI_NOT_QUIESCED;
               }
 ...
 * 
 **************************************************************************/
static uns32 avd_prep_pg_mem_list(AVD_CL_CB *cb, 
                                  AVD_CSI   *csi, 
 ...
 * NOTES:    None.
 **************************************************************************/
uns32 avd_snd_pg_resp_msg(AVD_CL_CB *cb, 
                          AVD_AVND  *node,
                          AVD_CSI   *csi,
