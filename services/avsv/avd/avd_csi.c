...
static AVD_CSI_PARAM * avd_csi_param_crt(AVD_CSI *csi, SaNameT param_name)
{
   ...
   while ((i_param != AVD_CSI_PARAM_NULL) &&
          (m_CMP_HORDER_SANAMET(i_param->param.name, param->param.name) < 0))
   {
      ptr = i_param;
...
static AVD_CSI_PARAM * 
avd_csi_find_next_param(AVD_CL_CB *cb, AVD_CSI **csi_ptr, SaNameT csi_name, SaNameT param_name)
{
...
uns32 saamfcsitableentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, 
                                  NCSCONTEXT* data)
{
...
uns32 saamfcsitableentry_extract(NCSMIB_PARAM_VAL* param, 
...
uns32 saamfcsitableentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, 
...
uns32 saamfcsinamevaluetableentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, 
                                  NCSCONTEXT* data)
{
   AVD_CL_CB     *avd_cb = (AVD_CL_CB *)cb;
   AVD_CSI       *csi;
...
uns32 saamfcsinamevaluetableentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, 
                         NCSMIB_VAR_INFO* var_info, NCS_BOOL test_flag)
{
   AVD_CL_CB     *avd_cb = (AVD_CL_CB *)cb;
   AVD_CSI       *csi;
   ...
   }else /* if (param == AVD_CSI_PARAM_NULL)*/
   {
      ...
      if(arg->req.info.set_req.i_param_val.i_param_id == saAmfCSINameValueRowStatus_ID)
      {
         ...
         case NCS_ROW_ACTIVE:
            ...
            if(avd_cb->init_state != AVD_CFG_READY)
            {
               if(avd_cs_type_param_find_match(avd_cb, csi->csi_type, param->param.name) == NCSCC_RC_FAILURE)
               {
                  return NCSCC_RC_INV_VAL;
               }
            }
...
{
   AVD_CS_TYPE_PARAM *type_param = AVD_CS_TYPE_PARAM_NULL;
   type_param = (AVD_CS_TYPE_PARAM *)ncs_patricia_tree_getnext(&cb->cs_type_param_anchor,
                                                (uns8*)&indx);
...
uns32 saamfcstypeparamentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg,
                                  NCSCONTEXT* data)
{
   AVD_CL_CB     *avd_cb = (AVD_CL_CB *)cb;
   AVD_CS_TYPE_PARAM_INDX  indx;
...
uns32 saamfcstypeparamentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg,
                         NCSMIB_VAR_INFO* var_info, NCS_BOOL test_flag)
{
   AVD_CL_CB     *avd_cb = (AVD_CL_CB *)cb;
   AVD_CS_TYPE_PARAM_INDX  indx;
   ...
   }else  /* The record is already available */
   {
      if(arg->req.info.set_req.i_param_val.i_param_id == saAmfCSTypeParamRowStatus_ID)
      {
         ...
         case NCS_ROW_NOT_IN_SERVICE:
            ...
            {
               m_AVSV_SEND_CKPT_UPDT_ASYNC_RMV(cb, type_param, AVSV_CKPT_AVD_CS_TYPE_PARAM_CONFIG);
            }
