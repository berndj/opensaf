...
static AVD_SU *avd_sg_nored_su_chose_asgn(AVD_CL_CB *cb,AVD_SG *sg)
{
   ...
   while ((i_si != AVD_SI_NULL) && (i_su != AVD_SU_NULL))
   {
      ...
      l_flag = TRUE;
      while ((i_su != AVD_SU_NULL) && (l_flag == TRUE))
      {
      ...
      {
         /* log a fatal error */               
         m_AVD_LOG_INVALID_VAL_ERROR(((uns32)i_si));
...
{   
   ...
   if (avd_sg_nored_su_chose_asgn(cb,si->sg_of_si) == AVD_SU_NULL)
   {
...
uns32 avd_sg_nored_su_fault_func(AVD_CL_CB *cb,AVD_SU *su)
{
   ...
   case AVD_SG_FSM_STABLE:
      ...
      {
         /* log a fatal error*/
         m_AVD_LOG_INVALID_VAL_ERROR(((uns32)su->list_of_susi));
         m_AVD_LOG_INVALID_NAME_NET_VAL_ERROR(su->name_net.value,su->name_net.length);
         m_AVD_LOG_INVALID_NAME_NET_VAL_ERROR(su->list_of_susi->si->name_net.value,su->list_of_susi->si->name_net.length);
   ...
   case AVD_SG_FSM_SG_REALIGN:
      ...
      if (su->list_of_susi->si == su->sg_of_su->admin_si)
      {
          ...
          */
         if(su->list_of_susi->si->admin_state == NCS_ADMIN_STATE_SHUTDOWN)
         {
            old_state = su->list_of_susi->state;
            old_fsm_state = su->list_of_susi->fsm;
            su->list_of_susi->state = SA_AMF_HA_QUIESCED;
            ...
            m_AVD_SET_SI_ADMIN(cb,(su->list_of_susi->si),NCS_ADMIN_STATE_LOCK);
         }
...
{ 
   ...
   if (avd_sg_nored_su_chose_asgn(cb,su->sg_of_su) == AVD_SU_NULL)
   {
                                ...
                                AVSV_SUSI_ACT act, SaAmfHAStateT state)
{
   ...
   case AVD_SG_FSM_SG_REALIGN: 
      ...
      else if (state == SA_AMF_HA_QUIESCED)
      {
         ...
         m_AVSV_SEND_CKPT_UPDT_ASYNC_UPDT(cb, (su->list_of_susi), AVSV_CKPT_AVD_SU_SI_REL);
         if(avd_snd_susi_msg(cb,su,su->list_of_susi,AVSV_SUSI_ACT_DEL) == NCSCC_RC_FAILURE)
         {
            /* log a fatal error*/
            m_AVD_LOG_INVALID_VAL_ERROR(((uns32)su->list_of_susi));
            m_AVD_LOG_INVALID_NAME_NET_VAL_ERROR(su->name_net.value,su->name_net.length);
   ...
   case AVD_SG_FSM_SU_OPER:
      ...
      else if (state == SA_AMF_HA_QUIESCED)
      {
         ...
         {
            /* log a fatal error*/
            m_AVD_LOG_INVALID_VAL_FATAL(((uns32)su->list_of_susi));
            m_AVD_LOG_INVALID_NAME_NET_VAL_FATAL(su->name_net.value,su->name_net.length);
            m_AVD_LOG_INVALID_NAME_NET_VAL_FATAL(su->list_of_susi->si->name_net.value,su->list_of_susi->si->name_net.length);
...
uns32 avd_sg_nored_realign_func(AVD_CL_CB *cb,AVD_SG *sg)
{
   ...
   m_AVD_LOG_FUNC_ENTRY("avd_sg_nored_realign_func");
   ...
   if (avd_sg_nored_su_chose_asgn(cb,sg) == AVD_SU_NULL)
   {
      /* all the assignments have already been done in the SG. */
      m_AVD_SET_SG_ADJUST(cb,sg,AVSV_SG_STABLE);
      avd_sg_app_su_inst_func(cb, sg);
...
void avd_sg_nored_node_fail_func(AVD_CL_CB *cb,AVD_SU *su)
{
   ...
   case AVD_SG_FSM_SG_REALIGN:
      ...
      {
         m_AVD_CLEAR_SG_ADMIN_SI(cb,(su->sg_of_su));
         m_AVD_SET_SI_ADMIN(cb,l_si,NCS_ADMIN_STATE_LOCK);
      }else
      {
...
uns32 avd_sg_nored_si_admin_down(AVD_CL_CB *cb,AVD_SI *si)
{
   ...
   case AVD_SG_FSM_STABLE:
      if (si->admin_state == NCS_ADMIN_STATE_LOCK)
      {
         ...
         if(avd_snd_susi_msg(cb,si->list_of_sisu->su,si->list_of_sisu,AVSV_SUSI_ACT_MOD) == NCSCC_RC_FAILURE)
         {
            /* log a fatal error*/
            m_AVD_LOG_INVALID_VAL_ERROR(((uns32)si->list_of_sisu));
            m_AVD_LOG_INVALID_NAME_NET_VAL_ERROR(si->list_of_sisu->su->name_net.value,si->list_of_sisu->su->name_net.length);
      ...
      if (sg->admin_state == NCS_ADMIN_STATE_LOCK)
      {
         ...
         while (i_su != AVD_SU_NULL)
         {
            if (i_su->list_of_susi != AVD_SU_SI_REL_NULL)
            {
               ...
               m_AVSV_SEND_CKPT_UPDT_ASYNC_UPDT(cb, (i_su->list_of_susi), AVSV_CKPT_AVD_SU_SI_REL);
         ...
         if (sg->su_oper_list.su != AVD_SU_NULL)
         {
            ...
            while (l_suopr != AVD_SG_OPER_NULL)
            {
                  ...
                  (l_suopr->su->list_of_susi->fsm == AVD_SU_SI_STATE_MODIFY))
               {
                  ...
                  m_AVSV_SEND_CKPT_UPDT_ASYNC_UPDT(cb, (l_suopr->su->list_of_susi), AVSV_CKPT_AVD_SU_SI_REL);
                  avd_gen_su_ha_state_changed_trap(cb, l_suopr->su->list_of_susi);
                  avd_snd_susi_msg(cb,l_suopr->su,l_suopr->su->list_of_susi,AVSV_SUSI_ACT_MOD);
               }
