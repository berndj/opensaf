 ...
 /*  printf("ENC ASYNC COUNT %d\n",cb->cpd_sync_cnt);  */
   ...
   switch(arg->info.encode.io_reo_type)
   {
      ...
      case CPD_A2S_MSG_CKPT_CREATE:
         ...
         rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPD_A2S_CKPT_CREATE),&arg->info.encode.io_uba,EDP_OP_TYPE_ENC,&cpd_msg->info.ckpt_create,&ederror);
         if(rc != NCSCC_RC_SUCCESS)
         {
      ...
      case CPD_A2S_MSG_CKPT_UNLINK:      
         cpd_msg = (CPD_MBCSV_MSG *)NCS_INT64_TO_PTR_CAST(arg->info.encode.io_reo_hdl);
         rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPD_A2S_CKPT_UNLINK),&arg->info.encode.io_uba,EDP_OP_TYPE_ENC,&cpd_msg->info.ckpt_ulink,&ederror); 
         if(rc != NCSCC_RC_SUCCESS)
         {
      ...
      case CPD_A2S_MSG_CKPT_RDSET:
         cpd_msg = (CPD_MBCSV_MSG *)NCS_INT64_TO_PTR_CAST(arg->info.encode.io_reo_hdl);
         rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPSV_CKPT_RDSET),&arg->info.encode.io_uba,EDP_OP_TYPE_ENC,&cpd_msg->info.rd_set,&ederror); 
         if(rc != NCSCC_RC_SUCCESS)
         {
      ...
      case CPD_A2S_MSG_CKPT_AREP_SET:
         cpd_msg = (CPD_MBCSV_MSG *)NCS_INT64_TO_PTR_CAST(arg->info.encode.io_reo_hdl);
         rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPSV_CKPT_DEST_INFO),&arg->info.encode.io_uba,EDP_OP_TYPE_ENC,&cpd_msg->info.arep_set,&ederror); 
         if(rc != NCSCC_RC_SUCCESS)
         {
      ...
      case CPD_A2S_MSG_CKPT_DEST_ADD:
         cpd_msg = (CPD_MBCSV_MSG *)NCS_INT64_TO_PTR_CAST(arg->info.encode.io_reo_hdl);
         rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPSV_CKPT_DEST_INFO),&arg->info.encode.io_uba,EDP_OP_TYPE_ENC,&cpd_msg->info.dest_add,&ederror);
         if(rc != NCSCC_RC_SUCCESS)
         {
      ...
      case CPD_A2S_MSG_CKPT_DEST_DEL:
         cpd_msg = (CPD_MBCSV_MSG *)NCS_INT64_TO_PTR_CAST(arg->info.encode.io_reo_hdl);
         rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPSV_CKPT_DEST_INFO),&arg->info.encode.io_uba,EDP_OP_TYPE_ENC,&cpd_msg->info.dest_del,&ederror);
         if(rc != NCSCC_RC_SUCCESS)
         {
      ...
      case CPD_A2S_MSG_CKPT_USR_INFO:
         cpd_msg = (CPD_MBCSV_MSG *)NCS_INT64_TO_PTR_CAST(arg->info.encode.io_reo_hdl);
         rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPD_A2S_CKPT_USR_INFO),&arg->info.encode.io_uba,EDP_OP_TYPE_ENC,&cpd_msg->info.usr_info,&ederror);
         if(rc != NCSCC_RC_SUCCESS)
         {
      ...
      case CPD_A2S_MSG_CKPT_DEST_DOWN:
         cpd_msg = (CPD_MBCSV_MSG *)NCS_INT64_TO_PTR_CAST(arg->info.encode.io_reo_hdl);
         rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPSV_CKPT_DEST_INFO),&arg->info.encode.io_uba,EDP_OP_TYPE_ENC,&cpd_msg->info.dest_down,&ederror);
         if(rc != NCSCC_RC_SUCCESS)
         {
             /* LOG */
            rc = NCSCC_RC_FAILURE;
 ...
 /*  cb->prev_ckpt_id = 0; */
  ...
  /* First reserve space to store the number of checkpoints that will be sent */
   ...
   while(ckpt_node)
   {
     ...
     /* DO THE EDU EXEC */  
     rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPD_A2S_CKPT_CREATE),&arg->info.encode.io_uba,EDP_OP_TYPE_ENC, &ckpt_create,&ederror);
     if(rc != NCSCC_RC_SUCCESS)
     {
 ...
 /*  printf("DEC ASYNC UPDATE %d\n",cb->sync_upd_cnt); */
   ...
   switch(evt_type)
   {
       case CPD_A2S_MSG_CKPT_CREATE:
          ...
          rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPD_A2S_CKPT_CREATE),&arg->info.decode.i_uba,EDP_OP_TYPE_DEC,&ckpt_create,&ederror);
          if(rc != NCSCC_RC_SUCCESS)
          {
             if(ckpt_create->dest_list)
       ...
       case CPD_A2S_MSG_CKPT_UNLINK:
          ...
          rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPD_A2S_CKPT_UNLINK),&arg->info.decode.i_uba,EDP_OP_TYPE_DEC,&ckpt_unlink,&ederror);
          if(rc != NCSCC_RC_SUCCESS)
          {
             m_LOG_CPD_CL(EDU_EXEC_ASYNC_UNLINK_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,__FILE__,__LINE__);
             rc = NCSCC_RC_FAILURE;
       ...
       case CPD_A2S_MSG_CKPT_RDSET:
          ...
          rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPSV_CKPT_RDSET),&arg->info.decode.i_uba,EDP_OP_TYPE_DEC,&ckpt_rdset,&ederror);
          if(rc != NCSCC_RC_SUCCESS)
          {
             m_LOG_CPD_CL(EDU_EXEC_ASYNC_RDSET_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,__FILE__,__LINE__);
             rc = NCSCC_RC_FAILURE;
       ...
       case CPD_A2S_MSG_CKPT_AREP_SET:
          ...
          rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPSV_CKPT_DEST_INFO),&arg->info.decode.i_uba,EDP_OP_TYPE_DEC,&ckpt_arep,&ederror);   
          if(rc != NCSCC_RC_SUCCESS)
          {
              m_LOG_CPD_CL(EDU_EXEC_ASYNC_AREP_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,__FILE__,__LINE__);  
              rc = NCSCC_RC_FAILURE;
       ...
       case CPD_A2S_MSG_CKPT_DEST_ADD:
          ...
          rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPSV_CKPT_DEST_INFO),&arg->info.decode.i_uba,EDP_OP_TYPE_DEC,&ckpt_dest_add,&ederror);
          if(rc != NCSCC_RC_SUCCESS)
          {
              m_LOG_CPD_CL(EDU_EXEC_ASYNC_DESTADD_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,__FILE__,__LINE__);
              rc = NCSCC_RC_FAILURE;
       ...
       case CPD_A2S_MSG_CKPT_DEST_DEL:
          ...
          rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPSV_CKPT_DEST_INFO),&arg->info.decode.i_uba,EDP_OP_TYPE_DEC,&ckpt_dest_del,&ederror);
          if(rc != NCSCC_RC_SUCCESS)
          {
              m_LOG_CPD_CL(EDU_EXEC_ASYNC_DESTDEL_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,__FILE__,__LINE__);
              rc = NCSCC_RC_FAILURE;
       ...
       case CPD_A2S_MSG_CKPT_USR_INFO:
          ...
          rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPD_A2S_CKPT_USR_INFO),&arg->info.decode.i_uba,EDP_OP_TYPE_DEC,&ckpt_usr_info,&ederror);
          if(rc != NCSCC_RC_SUCCESS)
          {
              m_LOG_CPD_CL(EDU_EXEC_ASYNC_DESTDEL_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,__FILE__,__LINE__);
              rc = NCSCC_RC_FAILURE;
       ...
       case CPD_A2S_MSG_CKPT_DEST_DOWN:
           ...
           rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPSV_CKPT_DEST_INFO),&arg->info.decode.i_uba,EDP_OP_TYPE_DEC,&ckpt_dest_down,&ederror);
           if(rc != NCSCC_RC_SUCCESS)
           {
              m_LOG_CPD_CL(EDU_EXEC_ASYNC_DESTDEL_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,__FILE__,__LINE__);
              rc = NCSCC_RC_FAILURE;
...
uns32  cpd_mbcsv_dec_sync_resp(CPD_CB *cb,NCS_MBCSV_CB_ARG *arg)
{
  ...
  /* Decode the data */
   ...
   {
      rc = m_NCS_EDU_EXEC(&cb->edu_hdl,FUNC_NAME(CPD_A2S_CKPT_CREATE),&arg->info.decode.i_uba,EDP_OP_TYPE_DEC, &ckpt_data, &ederror);
      if(rc != NCSCC_RC_SUCCESS)
      {
