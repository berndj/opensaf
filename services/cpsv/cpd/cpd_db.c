...
/****************************************************************************
  ...
  Notes         : The caller takes the cb lock before calling this function                  
******************************************************************************/
uns32 cpd_ckpt_node_add(NCS_PATRICIA_TREE *ckpt_tree,  
                          CPD_CKPT_INFO_NODE *ckpt_node)
{
   ...
   {
      m_LOG_CPD_FCL(CPD_CKPT_INFO_NODE_ADD_FAILED,CPD_FC_HDLN,NCSFL_SEV_ERROR,ckpt_node->ckpt_id,__FILE__,__LINE__);
                           ...
                           CPD_CKPT_REPLOC_INFO **ckpt_reploc_node)
{
   if(key_info)
   {
      key_info->ckpt_name.length = m_NCS_OS_HTONS(key_info->ckpt_name.length);
      key_info->node_name.length = m_NCS_OS_HTONS(key_info->node_name.length);
      *ckpt_reploc_node = (CPD_CKPT_REPLOC_INFO *)ncs_patricia_tree_getnext(ckpt_reploc_tree, (uns8 *)key_info);
   }
   else
   {
...
/****************************************************************************
  ...
  Notes         : The caller takes the cb lock before calling this function                  
******************************************************************************/
uns32 cpd_ckpt_reploc_node_add(NCS_PATRICIA_TREE *ckpt_reploc_tree,
                          CPD_CKPT_REPLOC_INFO *ckpt_reploc_node)
{
...
/****************************************************************************
  ...
  Notes         : The caller takes the cb lock before calling this function                  
******************************************************************************/
uns32 cpd_ckpt_map_node_add(NCS_PATRICIA_TREE *ckpt_map_tree,  
                          CPD_CKPT_MAP_INFO *ckpt_map_node)
{
   ...
   if(ncs_patricia_tree_add (ckpt_map_tree, &ckpt_map_node->patnode) != NCSCC_RC_SUCCESS)
   {
      ...
      m_LOG_CPD_CCL(CPD_CKPT_MAP_INFO_ADD_FAILED,CPD_FC_HDLN, NCSFL_SEV_ERROR,ckpt_map_node->ckpt_name.value,__FILE__,__LINE__);
...
uns32 cpd_ckpt_map_node_delete(CPD_CB *cb, CPD_CKPT_MAP_INFO *ckpt_map_node)
{
   ...
   {
      m_LOG_CPD_CCL(CPD_MAP_NODE_DELETE_FAILED,CPD_FC_HDLN, NCSFL_SEV_ERROR,ckpt_map_node->ckpt_name.value,__FILE__,__LINE__);
      rc =   NCSCC_RC_FAILURE;
   }
...
/****************************************************************************
  ...
  Notes         : The caller takes the cb lock before calling this function                  
******************************************************************************/
uns32 cpd_cpnd_info_node_add(NCS_PATRICIA_TREE *cpnd_tree,  
                          CPD_CPND_INFO_NODE *cpnd_info_node)
{
   /* Store the client_info pointer as msghandle. */
   NODE_ID key;
   ...
   *cpnd_info_node = (CPD_CPND_INFO_NODE *)
   ...
   if((*cpnd_info_node == NULL) && (*add_flag == TRUE))
   {
    ...
    /*  (*cpnd_info_node)->patnode.key_info = (uns8*)&(m_NCS_NODE_ID_FROM_MDS_DEST((*cpnd_info_node)->cpnd_dest)); */
      ...
      if(ncs_patricia_tree_add (cpnd_tree, &(*cpnd_info_node)->patnode) != NCSCC_RC_SUCCESS)
      {
         m_LOG_CPD_FCL(CPD_CPND_INFO_NODE_FAILED,CPD_FC_HDLN,NCSFL_SEV_ERROR,*dest,__FILE__,__LINE__);
         return NCSCC_RC_FAILURE;
      }
      *add_flag = FALSE;
   }
 ...
 *
 *****************************************************************************/
void cpd_node_ref_info_add(CPD_CKPT_INFO_NODE *ckpt_node, 
