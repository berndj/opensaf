...
/*  msg->info.ckpt_create.ckpt_name.length = m_NCS_OS_NTOHS(msg->info.ckpt_create.ckpt_name.length);     */
   ...
   {   
      m_LOG_CPD_FCL(CPD_STANDBY_CREATE_EVT_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,map_info->ckpt_id,__FILE__,__LINE__);
   ...
   {     
      ...
      if(!node_info)
      {
         m_LOG_CPD_FCL(CPD_STANDBY_CREATE_EVT_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,msg->info.ckpt_create.dest_list[count].dest,__FILE__,__LINE__);
         proc_rc = NCSCC_RC_OUT_OF_MEM;
         if(nref_info)
      ...
      {
         proc_rc = NCSCC_RC_FAILURE;
         m_LOG_CPD_LCL(CPD_STANDBY_CREATE_EVT_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,node_id,__FILE__,__LINE__);
   ...
   {
      m_LOG_CPD_FCL(CPD_STANDBY_CREATE_EVT_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,ckpt_node->ckpt_id,__FILE__,__LINE__);
      goto cpd_ckpt_node_add_fail;
   }


   m_LOG_CPD_CFCL(CPD_STANDBY_CREATE_EVT,CPD_FC_DB,NCSFL_SEV_NOTICE,map_info->ckpt_name.value,\
...
uns32 cpd_sb_proc_ckpt_dest_del(CPD_CB *cb,CPD_MBCSV_MSG *msg)
{
   ...
   if(ckpt_node == NULL)
   {
      m_LOG_CPD_FCL(CPD_STANDBY_DESTDEL_EVT_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,msg->info.dest_del.ckpt_id,__FILE__,__LINE__);
      proc_rc = NCSCC_RC_FAILURE;
      goto fail;
   }    
  ...
  /*    ckpt_node->ckpt_name.length = m_NCS_OS_NTOHS(ckpt_node->ckpt_name.length); */
   ...
   {
      m_LOG_CPD_FCL(CPD_STANDBY_DESTDEL_EVT_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,msg->info.dest_del.mds_dest,__FILE__,__LINE__); 
      proc_rc = NCSCC_RC_FAILURE;
      goto fail;
   }
   if(ckpt_node)
...
uns32 cpd_sb_proc_ckpt_dest_add(CPD_CB *cb,CPD_MBCSV_MSG *msg)
{
   ...
   {
       m_LOG_CPD_FCL(CPD_STANDBY_DESTADD_EVT_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,msg->info.dest_add.ckpt_id,__FILE__,__LINE__);
   ...
   if(!node_info)
   {
      m_LOG_CPD_FCL(CPD_STANDBY_DESTADD_EVT_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,msg->info.dest_add.mds_dest,__FILE__,__LINE__);
      proc_rc = NCSCC_RC_OUT_OF_MEM;
      goto free_mem;
   }
   ...
   {
      proc_rc = NCSCC_RC_FAILURE;
      m_LOG_CPD_LCL(CPD_STANDBY_DESTADD_EVT_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,node_id,__FILE__,__LINE__);
      goto free_mem;
   }
...
uns32 cpd_sb_proc_ckpt_usrinfo(CPD_CB *cb,CPD_MBCSV_MSG *msg)
{
    ...
    if(ckpt_node == NULL)
    {
       m_LOG_CPD_FCL(CPD_CKPT_INFO_NODE_GET_FAILED,CPD_FC_HDLN,NCSFL_SEV_ERROR,msg->info.dest_add.ckpt_id,__FILE__,__LINE__);
       return NCSCC_RC_FAILURE;
    }
