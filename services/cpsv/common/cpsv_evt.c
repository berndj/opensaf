...
static SaCkptSectionIdT *
cpsv_evt_dec_sec_id(NCS_UBAID *i_ub,uns32 svc_id);
static uns32
cpsv_evt_enc_sec_id(NCS_UBAID *o_ub,SaCkptSectionIdT *sec_id);
...
uns32 cpsv_data_access_rsp_encode(CPSV_ND2A_DATA_ACCESS_RSP *data_rsp , NCS_UBAID* io_uba)
{
   ...
   else if((data_rsp->type == CPSV_DATA_ACCESS_LCL_READ_RSP) || (data_rsp->type == CPSV_DATA_ACCESS_RMT_READ_RSP))
   {
...
uns32 cpsv_evt_enc_flat(EDU_HDL *edu_hdl, CPSV_EVT *i_evt, NCS_UBAID *o_ub)
{
   ...
   if(i_evt->type == CPSV_EVT_TYPE_CPA)
   {
      if(i_evt->info.cpa.type == CPA_EVT_ND2A_CKPT_DATA_RSP)
      {
         ...
         if((data_rsp->type == CPSV_DATA_ACCESS_LCL_READ_RSP) || (data_rsp->type == CPSV_DATA_ACCESS_RMT_READ_RSP))
         {
            ...
            else
            {
               if (data_rsp->size > 0)
               {
                  ...
                  for(;iter < data_rsp->size;iter++)
                  {
                     ...
                     ncs_encode_n_octets_in_uba(o_ub,(uns8*)&data_rsp->info.read_data[iter]
                               ,size);
                     if (data_rsp->info.read_data[iter].read_size > 0)
                     {
                        size=data_rsp->info.read_data[iter].read_size;
                        ncs_encode_n_octets_in_uba(o_ub,(uns8*)data_rsp->info. \
   ...
   else if(i_evt->type == CPSV_EVT_TYPE_CPND)
   {
      ...
      else if (i_evt->info.cpnd.type == CPND_EVT_D2ND_CKPT_CREATE)
      {
         ...
         {
            size = i_evt->info.cpnd.info.ckpt_create.ckpt_info.dest_cnt * sizeof(CPSV_CPND_DEST_INFO);
            ncs_encode_n_octets_in_uba(o_ub, (uns8*)dest_list, size);
         }
      }
       ...
       case CPA_EVT_ND2A_CKPT_DATA_RSP:
       {
          ...
          if ( data_rsp->num_of_elmts == -1) 
          ...
          else
           {
            data_rsp->info.read_data =
               m_MMGR_ALLOC_CPSV_ND2A_READ_DATA(data_rsp->num_of_elmts, NCS_SERVICE_ID_CPA);
            if(data_rsp->info.read_data)
             {
              uns32 iter=0;
              for(;iter<data_rsp->size;iter++)
               {
                size = sizeof(CPSV_ND2A_READ_DATA);
                ncs_decode_n_octets_from_uba(i_ub, (uns8 *)&data_rsp->info.read_data[iter], size);
                if( data_rsp->info.read_data[iter].read_size > 0)
                 {
                  size=data_rsp->info.read_data[iter].read_size;
                  data_rsp->info.read_data[iter].data= \
                  m_MMGR_ALLOC_CPSV_DEFAULT_VAL(size,NCS_SERVICE_ID_CPA);
                  if(data_rsp->info.read_data[iter].data)
                   {
                    ncs_decode_n_octets_from_uba(i_ub,\
                    data_rsp->info.read_data[iter].data,size);
                   }
                 }
               }
             }
         ...
         else if(data_rsp->type == CPSV_DATA_ACCESS_WRITE_RSP)
         {
            ...
            else
            {
              ...
              m_MMGR_ALLOC_CPSV_SaUint32T(data_rsp->num_of_elmts, NCS_SERVICE_ID_CPA);
            if(data_rsp->info.write_err_index)
               ncs_decode_n_octets_from_uba(i_ub,(uns8 *)data_rsp->info.write_err_index, size);
            }
         }
        break;
       }
    ...
    switch(o_evt->info.cpnd.type)
     {
      ...
      case CPSV_EVT_ND2ND_CKPT_SECT_CREATE_REQ:
      {
         ...
         if(create->sec_attri.sectionId) 
         {
            create->sec_attri.sectionId=cpsv_evt_dec_sec_id(i_ub, \
                  NCS_SERVICE_ID_CPND);
         }

         if (create->init_size) {
            create->init_data=(void *)m_MMGR_ALLOC_CPSV_DEFAULT_VAL(create->init_size, NCS_SERVICE_ID_CPND);
            ncs_decode_n_octets_from_uba(i_ub,(uns8 *)create->init_data, create->init_size);
         } 
         else
      ...
      case CPSV_EVT_ND2ND_CKPT_SECT_ACTIVE_CREATE_RSP:
      {
         ...
         if (sec_id)
         {
            if(sec_id->idLen)
            {
                sec_id->id = m_MMGR_ALLOC_CPSV_DEFAULT_VAL(sec_id->idLen,NCS_SERVICE_ID_CPND);
                if(sec_id->id )
                ncs_decode_n_octets_from_uba(i_ub,(uns8 *)sec_id->id,sec_id->idLen);
            }
            else
              sec_id->id = NULL;
         }
      ...
      case CPND_EVT_A2ND_CKPT_SECT_CREATE:
      {
         ...
         size = create->init_size;
         if (size!=0) {
            create->init_data=(void *)m_MMGR_ALLOC_CPSV_DEFAULT_VAL(size, NCS_SERVICE_ID_CPND);
