 ...
 * Notes         : None.
 *****************************************************************************/
uns32 cpnd_ckpt_client_add(CPND_CKPT_NODE *cp_node, 
                           CPND_CKPT_CLIENT_NODE *cl_node)
{
   ...
   m_NCS_OS_MEMSET(clist_node,'\0',sizeof(CPND_CKPT_CLLIST_NODE));
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 cpnd_ckpt_client_find(CPND_CKPT_NODE *cp_node,
                            CPND_CKPT_CLIENT_NODE *cl_node)
{
   CPND_CKPT_CLLIST_NODE *ptr_cl_node=cp_node->clist;
   uns32 rc = NCSCC_RC_SUCCESS;
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 cpnd_ckpt_client_del(CPND_CKPT_NODE *cp_node, 
                           CPND_CKPT_CLIENT_NODE *cl_node)
{
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 cpnd_client_ckpt_info_add(CPND_CKPT_CLIENT_NODE *cl_node,
                              CPND_CKPT_NODE *cp_node)
{
   uns32       rc=NCSCC_RC_SUCCESS;
   
   CPND_CKPT_CKPT_LIST_NODE *cplist;
   CPND_CKPT_CKPT_LIST_NODE *ptr_cp_node=cl_node->ckpt_list;

   cplist=m_MMGR_ALLOC_CPND_CKPT_LIST_NODE;
   
   /* need to check */
   m_NCS_OS_MEMSET(cplist,'\0',sizeof(CPND_CKPT_CKPT_LIST_NODE));
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 cpnd_client_ckpt_info_del(CPND_CKPT_CLIENT_NODE *cl_node,
                                            CPND_CKPT_NODE *cp_node)
{
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 cpnd_client_ckpt_info_delete(CPND_CKPT_CLIENT_NODE *cl_node,
                                            CPND_CKPT_NODE *cp_node)
{
...
uns32 cpnd_ckpt_replica_create(CPND_CB *cb,CPND_CKPT_NODE *cp_node)
{
   ...
   /* size of chkpt */
   m_NCS_MEMSET(&cp_node->replica_info.open,'\0',sizeof(cp_node->replica_info.open));

   cp_node->replica_info.open.type=NCS_OS_POSIX_SHM_REQ_OPEN;
   cp_node->replica_info.open.info.open.i_size=sizeof(CPSV_CKPT_HDR)+cp_node->create_attrib.maxSections*(sizeof(CPSV_SECT_HDR)+cp_node->create_attrib.maxSectionSize);
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 cpnd_ckpt_remote_cpnd_add(CPND_CKPT_NODE *cp_node, 
                           MDS_DEST mds_info)
{
   ...
   m_NCS_OS_MEMSET(cpnd_mdest,'\0',sizeof(CPSV_CPND_DEST_INFO));
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 cpnd_ckpt_remote_cpnd_del(CPND_CKPT_NODE *cp_node, 
                              MDS_DEST mds_info)
{
...
uns32 cpnd_ckpt_sec_write(CPND_CKPT_NODE *cp_node,CPND_CKPT_SECTION_INFO \
                        *sec_info,const void *data,uns32 size,\
                         uns32 offset,uns32 type) /* for sync type=2 */
{
   uns32 rc=NCSCC_RC_SUCCESS;
   ...
   write_req.info.write.i_addr = (void *) ((char *)cp_node->replica_info.open.info.open.o_addr + \
               sizeof(CPSV_CKPT_HDR)+ ((sec_info->lcl_sec_id+1) *sizeof(CPSV_SECT_HDR))+(sec_info->lcl_sec_id * cp_node->create_attrib.maxSectionSize));
  ...
  /* if ( type == 0) Needs to be cleaned up later TBD 
   ...
   if ( type == 0)
   {
      ...
      {
         cp_node->replica_info.mem_used = cp_node->replica_info.mem_used+((offset+size) - sec_info->sec_size);
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 cpnd_ckpt_read_replica(CPND_CB *cb,CPND_CKPT_NODE *cp_node ,\
            CPSV_CKPT_ACCESS *read_data,CPSV_EVT *evt)
{
   ...
   {
       m_LOG_CPND_CL(CPND_CPSV_ND2A_READ_DATA_ALLOC_FAIL,CPND_FC_MEMFAIL,NCSFL_SEV_ERROR,__FILE__,__LINE__);
   ...
   m_NCS_MEMSET(evt->info.cpa.info.sec_data_rsp.info.read_data,'\0', \
   ...
   for(; i< read_data->num_of_elmts ;i++,data=data->next)
   {
      ...
      if(((data->dataOffset + data->dataSize) > (cp_node->create_attrib.maxSectionSize))||(data->dataOffset > sec_info->sec_size))
      {
 ...
 *****************************************************************************/
CPSV_CKPT_DATA *
cpnd_ckpt_generate_cpsv_ckpt_access_evt(CPND_CKPT_NODE *cp_node)
{
 ...
 *****************************************************************************/
void
cpnd_proc_free_cpsv_ckpt_data(CPSV_CKPT_DATA *data)
{
 ...
 *****************************************************************************/
uns32
cpnd_allrepl_write_evt_node_free(CPSV_CPND_ALL_REPL_EVT_NODE *evt_node)
{
...
void  cpnd_proc_gen_mapping(CPND_CKPT_NODE *cp_node,CPSV_CKPT_ACCESS *ckpt_read,CPSV_EVT *evt)
{
   ...
   m_NCS_MEMSET(evt->info.cpa.info.sec_data_rsp.info.read_mapping,'\0', \
   ...
   for(iter=0;iter < ckpt_read->num_of_elmts;iter++,sec_data=sec_data->next)
   {
      ...
      if (sec_info == NULL )
      {
         ...
         cp_node->ckpt_id,__FILE__,__LINE__); 
         evt->info.cpa.info.sec_data_rsp.info.\
                 read_mapping[iter].offset_index =-1;
         continue;
      }

      if(((sec_data->dataOffset + sec_data->dataSize) > (cp_node->create_attrib.maxSectionSize))||(sec_data->dataOffset > sec_info->sec_size))
      {
         m_LOG_CPND_CL(CPND_SECTION_BOUNDARY_VIOLATION,CPND_FC_GENERIC,NCSFL_SEV_ERROR,__FILE__,__LINE__);
         evt->info.cpa.info.sec_data_rsp.num_of_elmts=-1;
         evt->info.cpa.info.sec_data_rsp.error= SA_AIS_ERR_INVALID_PARAM; 
      ...
      evt->info.cpa.info.sec_data_rsp.info.read_mapping[iter].offset_index = \
                sec_info->lcl_sec_id;
      evt->info.cpa.info.sec_data_rsp.info.read_mapping[iter].read_size = \
                read_size;
   }
}
                          ...
                          CPSV_EVT *out_evt,CPSV_SEND_INFO *sinfo)
{
   ...
   if (m_CPND_IS_ALL_REPLICA_ATTR_SET(cp_node->create_attrib.creationFlags) == TRUE )
   {
      if(cp_node->cpnd_dest_list != NULL )
      {
...
#if 0
         if (in_evt->type == CPSV_EVT_ND2ND_CKPT_SECT_DATA_ACCESS_REQ)
         {
            send_evt.info.cpnd.info.ckpt_nd2nd_data=in_evt->info.ckpt_nd2nd_data;
            datasize = in_evt->info.ckpt_nd2nd_data.num_of_elmts * cp_node->create_attrib.maxSectionSize; 
         ...
         m_NCS_OS_MEMSET(all_repl_evt,'\0',sizeof(CPSV_CPND_ALL_REPL_EVT_NODE));
         ...
         while(tmp!=NULL)
         {
           ...
           m_NCS_MEMSET(new, 0, sizeof(CPSV_CPND_UPDATE_DEST));
           new->dest = tmp->dest;
           if(head == NULL)
           {
              head = new;
         ...
         rc = cpnd_tmr_start(&all_repl_evt->write_rsp_tmr, m_CPSV_CONVERT_SATIME_TEN_MILLI_SEC(timeout));
         ...
         while(head != NULL)
         {
            ...
            if (rc != NCSCC_RC_SUCCESS)
            {
                 m_LOG_CPND_FFFLCL(CPND_ACTIVE_TO_REMOTE_MDS_SEND_FAIL,CPND_FC_MDSFAIL,NCSFL_SEV_ERROR,\
                                   cb->cpnd_mdest_id,head->dest,cp_node->ckpt_id,rc,__FILE__,__LINE__);
            }
            else
            {
               ...
               (m_CPND_IS_ACTIVE_REPLICA_WEAK_ATTR_SET(cp_node->create_attrib.creationFlags) == TRUE ))
   {
      ...
      if(cp_node->cpnd_dest_list != NULL )
      {
         ...
         while(tmp != NULL)
         {
            ...
            rc=cpnd_mds_msg_send(cb,NCSMDS_SVC_ID_CPND,tmp->dest,\
                         &send_evt);
            if (rc == NCSCC_RC_FAILURE) 
            {
...
#if 0
   if(sinfo->to_svc == NCSMDS_SVC_ID_CPND)  /* fill the response for other CPND */
   {
          out_evt->type=CPSV_EVT_TYPE_CPND;
          out_evt->info.cpnd.type= \
                     CPSV_EVT_ND2ND_CKPT_SECT_ACTIVE_DATA_ACCESS_RSP;

       switch(in_evt->info.ckpt_nd2nd_data.type)
       {
          case CPSV_CKPT_ACCESS_WRITE:

             out_evt->info.cpnd.info.ckpt_nd2nd_data_rsp.type= \
                   CPSV_DATA_ACCESS_WRITE_RSP;
             out_evt->info.cpnd.info.ckpt_nd2nd_data_rsp.num_of_elmts= \
                   in_evt->info.ckpt_nd2nd_data.num_of_elmts;
             out_evt->info.cpnd.info.ckpt_nd2nd_data_rsp.size=0;
             out_evt->info.cpnd.info.ckpt_nd2nd_data_rsp.info. \
                   write_err_index=NULL;
             out_evt->info.cpnd.info.ckpt_nd2nd_data_rsp.error=SA_AIS_OK;
             break;
    ...
    else  /* Fill the response for CPA */
#endif
    {
   ...
   if (mds_failure == TRUE)
   {
      switch(in_evt->info.ckpt_write.type)
      {
         case CPSV_CKPT_ACCESS_WRITE:
            ...
            { 
        #if 0
               out_evt->info.cpnd.type=CPSV_EVT_ND2ND_CKPT_SECT_ACTIVE_DATA_ACCESS_RSP;
               out_evt->info.cpnd.info.ckpt_nd2nd_data_rsp.type= \
                              CPSV_DATA_ACCESS_WRITE_RSP;
               out_evt->info.cpnd.info.ckpt_nd2nd_data_rsp.num_of_elmts= -1;
               out_evt->info.cpnd.info.ckpt_nd2nd_data_rsp.info.write_err_index=NULL;
               out_evt->info.cpnd.info.ckpt_nd2nd_data_rsp.error=SA_AIS_ERR_LIBRARY;
        #endif

                out_evt->type=CPSV_EVT_TYPE_CPA;
                out_evt->info.cpa.type=CPA_EVT_ND2A_CKPT_DATA_RSP;
                out_evt->info.cpa.info.sec_data_rsp.type = CPSV_DATA_ACCESS_WRITE_RSP;
                out_evt->info.cpa.info.sec_data_rsp.error = SA_AIS_ERR_NO_RESOURCES;
            ...
            else
            {
             ...
             out_evt->info.cpnd.info.ckpt_nd2nd_data_rsp.info. \
                   write_err_index=NULL;
             out_evt->info.cpnd.info.ckpt_nd2nd_data_rsp.error=SA_AIS_ERR_NO_RESOURCES;
         ...
         case CPSV_CKPT_ACCESS_OVWRITE:
            if (sinfo == NULL)
            {
               out_evt->info.cpa.type=CPA_EVT_ND2A_CKPT_DATA_RSP;
               out_evt->info.cpa.info.sec_data_rsp.type=CPSV_DATA_ACCESS_OVWRITE_RSP;
               out_evt->info.cpa.info.sec_data_rsp.info.ovwrite_error.error=SA_AIS_ERR_NO_RESOURCES;
            }
            else
            {
               out_evt->type=CPSV_EVT_TYPE_CPND;
 ...
 *****************************************************************************/
uns32 
cpnd_proc_sec_expiry(CPND_CB *cb,CPND_TMR_INFO *tmr_info)
{

   CPND_CKPT_NODE *cp_node;
   CPND_CKPT_SECTION_INFO *pSec_info;
   ...
   send_evt.info.cpd.info.ckpt_sec_info.ckpt_id = tmr_info->ckpt_id;
 ...
 *****************************************************************************/
uns32
cpnd_all_repl_rsp_expiry(CPND_CB *cb,CPND_TMR_INFO *tmr_info)
{
   uns32 rc=NCSCC_RC_SUCCESS;
   CPND_CKPT_NODE *cp_node=NULL;
   ...
   m_NCS_MEMSET(&rsp_evt , 0 , sizeof(CPSV_EVT));
   ...
   if(cp_node && evt_node)
   ...
   else
    {
      ...
      /*log the error*/
     if(!cp_node)
      m_NCS_CONS_PRINTF("ckpt node not found for ckpt_id=%llu\n",tmr_info->ckpt_id);
     if(!evt_node)
       m_NCS_CONS_PRINTF("evt node not found for dest=%llu\n",tmr_info->agent_dest);
     if(cp_node && evt_node &&  !(cp_node->ckpt_id == evt_node->ckpt_id))
      m_NCS_CONS_PRINTF("ckptnode ckptid=%llu evtnode ckptid=%llu\n",cp_node->ckpt_id ,evt_node->ckpt_id);
#endif
    }
   return rc;
} 
...
uns32 
cpnd_proc_fill_sec_desc(CPND_CKPT_SECTION_INFO *pTmpSecPtr, \
                        SaCkptSectionDescriptorT *sec_des)
{

#if 0
  
   sec_des->sectionId.idLen = pTmpSecPtr->sec_id.idLen;
   /* need to allocate memory */
   m_NCS_MEMCPY(sec_des->sectionId.id,pTmpSecPtr->sec_id.id, \
                 pTmpSecPtr->sec_id.idLen); 
#endif
...
uns32 cpnd_ckpt_hdr_update(CPND_CKPT_NODE *cp_node)
{
    ...
    m_NCS_OS_MEMSET(&write_req,'\0',sizeof(write_req));
    m_NCS_OS_MEMSET(&ckpt_hdr,'\0',sizeof(CPSV_CKPT_HDR));
...
uns32 cpnd_sec_hdr_update(CPND_CKPT_SECTION_INFO *sec_info,CPND_CKPT_NODE *cp_node)
{
   ...
   write_req.type   = NCS_OS_POSIX_SHM_REQ_WRITE;
   write_req.info.write.i_addr = (void *) ((char *)cp_node->replica_info.open.info.open.o_addr + sizeof(CPSV_CKPT_HDR));
   write_req.info.write.i_from_buff  = (CPSV_SECT_HDR *)&sec_hdr;
   write_req.info.write.i_offset =sec_info->lcl_sec_id * (sizeof(CPSV_SECT_HDR)+cp_node->create_attrib.maxSectionSize);
