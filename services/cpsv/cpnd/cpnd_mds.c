...
uns32 cpsv_ckpt_access_decode(CPSV_CKPT_ACCESS  *ckpt_data, NCS_UBAID *io_uba )
{
   ...
   ncs_dec_skip_space(io_uba, space);
   ncs_decode_n_octets_from_uba(io_uba, (uns8 *)ckpt_data->ckpt_sync.cpa_sinfo.ctxt.data , (uns32)MDS_SYNC_SND_CTXT_LEN_MAX);
     ...
     if( msg_ptr->type == CPSV_EVT_TYPE_CPND)  /* Replaced EDU with encode/decode Routines for Performance Improvement*/
     {
         ...
         { 
          ...
          case CPND_EVT_ND2ND_CKPT_ACTIVE_SYNC: /* Sync Event */
             ncs_dec_skip_space(dec_info->io_uba, 12);
             rc = cpsv_ckpt_access_decode(&msg_ptr->info.cpnd.info.ckpt_nd2nd_sync, dec_info->io_uba );
             goto free;
         
          case CPSV_EVT_ND2ND_CKPT_SECT_ACTIVE_DATA_ACCESS_REQ: /* Write Event ND2ND*/
             ncs_dec_skip_space(dec_info->io_uba, 12);
             rc = cpsv_ckpt_access_decode(&msg_ptr->info.cpnd.info.ckpt_nd2nd_data, dec_info->io_uba );
             goto free;
...
static uns32 cpnd_mds_rcv(CPND_CB *cb, MDS_CALLBACK_RECEIVE_INFO *rcv_info)
{
   ...
   if ((rcv_info->i_fr_svc_id == NCSMDS_SVC_ID_CPND) && (rcv_info->i_rsp_reqd)) {
      ...
      if(!node) {
         m_NCS_UNLOCK(&cb->cpnd_sync_send_lock, NCS_LOCK_WRITE);
         m_LOG_CPND_CL(CPND_SYNC_SEND_NODE_ALLOC_FAILED,CPND_FC_MEMFAIL,NCSFL_SEV_ERROR,__FILE__,__LINE__);
         return NCSCC_RC_FAILURE;
      }
      ...
      if (ncs_enqueue(&cb->cpnd_sync_send_list, (void *)node) != NCSCC_RC_SUCCESS) { 
         m_LOG_CPND_CL(CPND_NCS_ENQUEUE_EVT_FAILED,CPND_FC_GENERIC,NCSFL_SEV_ERROR,__FILE__,__LINE__);
      }
   }
 ...
 *****************************************************************************/
uns32
cpnd_mds_bcast_send (CPND_CB *cb,
                     CPSV_EVT *evt,
   ...
   if(svc_evt->i_svc_id == NCSMDS_SVC_ID_CPD)   
   {
      ...
      case NCSMDS_DOWN:
         if(cb->is_cpd_up == TRUE)
         {
            ...
            cb->is_cpd_up = FALSE;
            m_LOG_CPND_CL(CPND_CPD_SERVICE_WENT_DOWN,CPND_FC_HDLN,NCSFL_SEV_ALERT,__FILE__,__LINE__);
...
uns32 cpnd_mds_send_rsp(CPND_CB *cb, CPSV_SEND_INFO *s_info, CPSV_EVT *evt)
{
   ...
   if(rc != NCSCC_RC_SUCCESS)
      m_LOG_CPND_FCL(CPND_MDS_SEND_FAIL,CPND_FC_HDLN,NCSFL_SEV_ERROR,s_info->dest,\
       __FILE__,__LINE__);
                             ...
                             uns32  timeout)
{
   ...
   if (to_svc == NCSMDS_SVC_ID_CPND) {
      ...
      if (!node) {
         ...
         m_NCS_UNLOCK(&cb->cpnd_sync_send_lock, NCS_LOCK_WRITE);
         m_LOG_CPND_FCL(CPND_MDS_SEND_FAIL,CPND_FC_HDLN,NCSFL_SEV_INFO,to_dest,\
                        __FILE__,__LINE__);
         return NCSCC_RC_FAILURE;
      }
   ...
   {
      m_LOG_CPND_FLCL(CPND_MDS_SEND_FAIL,CPND_FC_HDLN,NCSFL_SEV_ERROR,to_dest,\
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 cpnd_mds_msg_send (CPND_CB *cb, uns32 to_svc, 
                          MDS_DEST  to_dest, CPSV_EVT   *evt)
{
   ...
   if ( rc != NCSCC_RC_SUCCESS)
   {
      m_LOG_CPND_FCL(CPND_MDS_SEND_FAIL,CPND_FC_HDLN,NCSFL_SEV_ERROR,\
        to_dest,__FILE__,__LINE__);
   }
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 cpnd_mds_msg_sync_ack_send(CPND_CB *cb, uns32 to_svc,
                    MDS_DEST to_dest, CPSV_EVT *i_evt, uns32  timeout)
{
   NCSMDS_INFO                mds_info;
   uns32                      rc;
