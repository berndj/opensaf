...
eds_add_subrec_entry(CHAN_OPEN_REC *copen_rec, SUBSC_REC *subrec)
{
   ...
   {
      m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,NCSCC_RC_BAD_ATTR,__FILE__,__LINE__,0);
 ...
 ***************************************************************************/
static void
eds_remove_subrec_entry(EDS_CB *cb, SUBSC_REC **subrec)
{
   SUBSC_REC  *p;
 ...
 ***************************************************************************/
static uns32
eds_add_subscription_to_worklist(EDS_CB *cb, SUBSC_REC *subrec)
{
   ...
   while(wp)    /* While there are channels... */
   {
      ...
      if (wp->chan_id == subrec->chan_id)
      {
         ...
         if (NULL == (co = (CHAN_OPEN_REC *)ncs_patricia_tree_get(&wp->chan_open_rec,
                                                             (uns8*)&copen_id_Net)))
         {
            m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__,subrec->chan_open_id);
         ...
         {
            m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,co->reg_id,__FILE__,__LINE__,subrec->reg_list->reg_id);
            return(NCSCC_RC_BAD_ATTR);
         }   
   ...
   m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,NCSCC_RC_NO_OBJECT,__FILE__,__LINE__,0);
 ...
 ***************************************************************************/
static uns32
eds_add_subscription_to_reglist(EDS_CB *cb, uns32 reg_id,
                                SUBSC_REC *subrec)
{
   ...
   if (reglist == NULL)
   {
      m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,NCSCC_RC_NO_OBJECT,__FILE__,__LINE__,reg_id);
 ...
 ***************************************************************************/
static uns32
eds_remove_cname_rec(EDS_CB *cb, EDS_WORKLIST *wp)
{
   uns32          rc;
   ...
   if (NULL == (rec_to_del =
                (EDS_CNAME_REC *)ncs_patricia_tree_get(&cb->eds_cname_list,
                                                       (uns8*)&chan_name_del)))
   {
      /* Log It */
      m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__,0);
      return(NCSCC_RC_FAILURE);
   }

   /* Delete this record from the tree */
   if (NCSCC_RC_SUCCESS != (rc = ncs_patricia_tree_del(&cb->eds_cname_list,
                                                       &rec_to_del->pat_node)))
   {
      /* Log it*/
 ...
 ***************************************************************************/
uns32
eds_remove_worklist_entry(EDS_CB *cb, uns32 chan_id)
{
 ...
 ***************************************************************************/
static NCS_BOOL
is_active_channel(EDS_WORKLIST *wp, uns32 chan_name_len, uns8 *chan_name)
{
 ...
 ***************************************************************************/
static uns32
eds_add_cname_rec (EDS_CB *cb, EDS_WORKLIST *wp, uns8 *chan_name, uns16 chan_name_len) 
 ...
 ***************************************************************************/
static uns32
eds_add_chan_open_list(EDS_CB *cb, uns32 reg_id, uns32 chan_id,
                       uns32 chan_open_id)
{
   EDA_REG_REC     *rp;
 ...
 ***************************************************************************/
static void
eds_channel_close_by_regid(EDS_CB *cb, uns32 reg_id, NCS_BOOL forced)
{
   uns32            rs;
   EDA_REG_REC     *reglist;
   CHAN_OPEN_LIST  *cl;
 ...
 ***************************************************************************/
static uns32
eds_remove_chan_open_rec(EDS_WORKLIST *wp, CHAN_OPEN_REC *co)
{
   uns32          rc;
   ...
   /* Get the record pointer from the patricia tree */
   if (NULL == (rec_to_del = 
                (CHAN_OPEN_REC *)ncs_patricia_tree_get(&wp->chan_open_rec, 
   ...
   /* Delete this record from the tree */
   if (NCSCC_RC_SUCCESS != (rc = ncs_patricia_tree_del(&wp->chan_open_rec, 
                                                       &rec_to_del->pat_node)))
   {
      m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__,0);
      return(NCSCC_RC_FAILURE);
 ...
 ***************************************************************************/
static void
eds_remove_chan_open_list(EDS_CB *cb, uns32 reg_id, uns32 chan_id,
                          uns32 chan_open_id)
{
   EDA_REG_REC     *reglist;
   CHAN_OPEN_LIST  *cl;
 ...
 ***************************************************************************/
uns32
eds_add_reglist_entry (EDS_CB *cb, MDS_DEST dest, uns32 reg_id)
{
   uns32          rs = NCSCC_RC_SUCCESS;
   EDA_REG_REC    *rec;
 ...
 ****************************************************************************/
static void
eds_del_work_list(EDS_CB *cb, EDS_WORKLIST **p_work_list)
{
 ...
 ***************************************************************************/
uns32
eds_remove_reglist_entry (EDS_CB *cb, uns32 reg_id, NCS_BOOL remove_all)
{
   ...
   if ((reg_id == 0) && (remove_all == TRUE))
   {
      ...
      if (rec_to_del)
      {
         while(rec_to_del)
         {
            ...
            **/
            rec_to_del = 
               (EDA_REG_REC *)ncs_patricia_tree_getnext(&cb->eda_reg_list,
               (uns8 *)&regId_Net);
         }
      }
   ...
   else
   {
       ...
       **/
      if (NULL == (rec_to_del = 
                     (EDA_REG_REC *)ncs_patricia_tree_get(&cb->eda_reg_list, 
                                    (uns8*)&regId_Net)))
      {
         m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__,0);
         return NCSCC_RC_FAILURE;
      ...
      if (NCSCC_RC_SUCCESS != (status = ncs_patricia_tree_del(&cb->eda_reg_list, 
                                                         &rec_to_del->pat_node)))
      {
         m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,status,__FILE__,__LINE__,0);
         return status;
      }
 ...
 ****************************************************************************/
NCS_BOOL 
eds_eda_entry_valid(EDS_CB *cb, MDS_DEST mds_dest)
{
   EDA_REG_REC    *rp = NULL;
   ...
   while (rp != NULL)
   {
      ...
      rp = (EDA_REG_REC *)ncs_patricia_tree_getnext
               (&cb->eda_reg_list,(uns8 *)&rp->reg_id_Net);
   }
 ...
 ****************************************************************************/
uns32 
eds_remove_eda_down_rec(EDS_CB *cb , MDS_DEST mds_dest)
{
 ...
 ****************************************************************************/
uns32
eds_remove_regid_by_mds_dest(EDS_CB *cb, MDS_DEST mds_dest)
{
   ...
   while (rp != NULL)
   {
      ...
      rp = (EDA_REG_REC *)ncs_patricia_tree_getnext
               (&cb->eda_reg_list,(uns8 *)&regId_Net);
   }
 ...
 ***************************************************************************/
EDA_REG_REC *
eds_get_reglist_entry (EDS_CB *cb, uns32 reg_id)
{
   EDA_REG_REC *rp;
   uns32        regId_Net;
 ...
 ***************************************************************************/
EDS_WORKLIST *
eds_get_worklist_entry (EDS_WORKLIST *wp_root, uns32 chan_id)
{
   EDS_WORKLIST *wp;
   ...
   while(wp)
   {
      ...
      else
      {
         ...
         if (wp->chan_id > chan_id)
         {
            ...
            m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__,chan_id);
 ...
 ***************************************************************************/
uns32
eds_add_subscription(EDS_CB *cb, uns32 reg_id, SUBSC_REC *subrec)
{
   uns32  rs = NCSCC_RC_SUCCESS;
 ...
 ***************************************************************************/
uns32
eds_remove_subscription(EDS_CB *cb, uns32 reg_id, uns32 chan_id,
                        uns32 chan_open_id, uns32 sub_id)
{
   uns32            rs = NCSCC_RC_SUCCESS;
   EDA_REG_REC     *reglist;
   SUBSC_LIST      *sublist = NULL;
 ...
 ****************************************************************************/
uns32
eds_copen_patricia_init(EDS_WORKLIST *wp)
{
                 ...
                 uns32 *chan_id, uns32 *chan_open_id, SaTimeT chan_create_time)
{
  ...
  /* time_t          time_of_day;*/
   ...
   if (wp == NULL)
   {
      ...
      {
         m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,SA_AIS_ERR_NOT_EXIST,__FILE__,__LINE__,0);
...
/*      wp->chan_attrib|=flags; */
      ...
      *(wp->cname+chan_name_len) = '\0';
      ...
      {
         m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,SA_AIS_ERR_LIBRARY,__FILE__,__LINE__,0);
   ...
   while(wp)
   {
      ...
      if (is_active_channel(wp, chan_name_len, chan_name))
      {
         ...
         if (rc != NCSCC_RC_SUCCESS)
         {
            m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,SA_AIS_ERR_LIBRARY,__FILE__,__LINE__,wp->chan_id);
            return(SA_AIS_ERR_LIBRARY);
         }
         /* Add an entry to the reglist too */
         rc = eds_add_chan_open_list(cb, reg_id, wp->chan_id, *chan_open_id);
         if (rc != NCSCC_RC_SUCCESS)
         {
              ...
              if (NULL == (co = (CHAN_OPEN_REC *)ncs_patricia_tree_get(&wp->chan_open_rec,
                                                          (uns8*)&copen_id_Net)))
              {
                 m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_DATA,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__,wp->chan_id);
                 return NCSCC_RC_FAILURE;
              }
 ...
 ***************************************************************************/
uns32
eds_channel_unlink(EDS_CB *cb, uns32 chan_name_len, uns8 *chan_name)
{
   EDS_WORKLIST   *wp;
   ...
   m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_CONTROL,NCSFL_SEV_ERROR,SA_AIS_ERR_NOT_EXIST,__FILE__,__LINE__,0);
   return(SA_AIS_ERR_NOT_EXIST);   /* Went through the entire list. Not found. */
}

static void
eds_retd_evt_del (EDS_RETAINED_EVT_REC **,
                  EDS_RETAINED_EVT_REC **,
                  EDS_RETAINED_EVT_REC *,
                  NCS_BOOL               );
/****************************************************************************
                         ...
                         SaTimeT orig_publish_time)
{
   ...
   if (0 == (retained_evt->retd_evt_hdl = 
                  ncshm_create_hdl(NCS_HM_POOL_ID_COMMON, 
                     NCS_SERVICE_ID_EDS,(NCSCONTEXT)retained_evt)))
   {
      m_LOG_EDSV_S(EDS_LL_PROCESING_FAILURE,NCSFL_LC_EDSV_CONTROL,NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__,0);
   ...
   if(retained_evt->retentionTime != SA_TIME_MAX)
      error=eds_start_tmr(cb,
        ...
        EDS_RET_EVT_TMR,
        retained_evt->retentionTime,
        retained_evt->retd_evt_hdl);
   ...
   if(error != NCSCC_RC_SUCCESS)
   {
     ...
     eds_retd_evt_del(&wp->ret_evt_list_head[retained_evt->priority], &wp->ret_evt_list_tail[retained_evt->priority],
                         retained_evt, TRUE);
   }
   else
...
eds_find_retd_evt_by_chan_open_id(EDS_WORKLIST *wp, 
                                  uns32 chan_open_id, 
                                  uns32 event_id)
{
 ...
 ****************************************************************************/
uns32
eds_clear_retained_event(EDS_CB *cb, uns32 chan_id,
                         uns32 chan_open_id, 
                         uns32 event_id, 
                         NCS_BOOL give_hdl)
{
   ...
   if (NULL != (retained_evt = 
      eds_find_retd_evt_by_chan_open_id(wp, chan_open_id, event_id)))
   {
      eds_retd_evt_del(&wp->ret_evt_list_head[retained_evt->priority], &wp->ret_evt_list_tail[retained_evt->priority],
 ...
 ****************************************************************************/
void
eds_remove_retained_events(EDS_RETAINED_EVT_REC **p_retd_evt_rec,EDS_RETAINED_EVT_REC **list_tail)
{
