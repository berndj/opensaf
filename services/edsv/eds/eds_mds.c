...
******************************************************************************/
static uns32
eds_dec_initialize_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8        *p8;
   uns32       total_bytes = 0;
...
******************************************************************************/
static uns32
eds_dec_finalize_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8        *p8;
   uns32       total_bytes = 0;
...
******************************************************************************/
static uns32
eds_dec_chan_open_sync_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8        *p8;
   uns32       total_bytes = 0;
...
******************************************************************************/
static uns32
eds_dec_chan_open_async_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8        *p8;
   uns32       total_bytes = 0;
...
******************************************************************************/
static uns32
eds_dec_chan_close_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8        *p8;
   uns32       total_bytes = 0;
...
******************************************************************************/
static uns32
eds_dec_chan_unlink_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8        *p8;
   uns32       total_bytes = 0;
...
eds_dec_subscribe_msg(NCS_UBAID *uba, long msg_hdl, uns8 ckpt_flag)
{
   ...
   if(num_filters)
   {
      param->filter_array->filters = m_MMGR_ALLOC_EVENT_FILTERS((uns32)num_filters);
   ...
   for (x=0; x< (uns32)param->filter_array->filtersNumber; x++)
   {
      ...
      else
      {
         ...
         filter_ptr->filter.pattern =
            m_MMGR_ALLOC_EDSV_EVENT_DATA((uns32)filter_ptr->filter.patternSize);
...
******************************************************************************/
static uns32
eds_dec_unsubscribe_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8         *p8;
   uns32        total_bytes = 0;
...
******************************************************************************/
static uns32
eds_dec_retention_time_clr_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8         *p8;
   uns32        total_bytes = 0;
...
******************************************************************************/
static uns32
eds_enc_initialize_rsp_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8        *p8;
   uns32       total_bytes = 0;
...
******************************************************************************/
static uns32
eds_enc_chan_open_sync_rsp_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8        *p8;
   uns32       total_bytes = 0;
...
******************************************************************************/
static uns32
eds_enc_chan_open_cbk_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8        *p8;
   uns32       total_bytes = 0;
...
******************************************************************************/
static uns32
eds_enc_delv_evt_cbk_msg(NCS_UBAID *uba,  EDSV_MSG *msg)
{
   uns8        *p8;
   uns32       x;
   uns32       total_bytes = 0;
   ...
   ncs_encode_32bit(&p8, (uns32)param->eda_event_id);
   /* event_hdl is skipped purposely */
   ncs_encode_32bit(&p8, (uns32)param->ret_evt_ch_oid);
...
static uns32
eds_mds_cpy(struct ncsmds_callback_info *info)
{
   /* TODO; */
...
static uns32
eds_mds_enc(struct ncsmds_callback_info *info)
{
   EDSV_MSG   *msg;
   NCS_UBAID  *uba;
   uns8       *p8;
...
static uns32
eds_mds_dec(struct ncsmds_callback_info *info)
{
   uns8          *p8;
   EDSV_EDS_EVT  *evt;
...
static uns32
eds_mds_enc_flat(struct ncsmds_callback_info *info)
{
...
static uns32
eds_mds_dec_flat(struct ncsmds_callback_info *info)
{
   uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32
eds_mds_rcv(struct ncsmds_callback_info *mds_info)
{
   ...
   edsv_evt->cb_hdl        = (uns32)mds_info->i_yr_svc_hdl;
   ...
   if(NCSCC_RC_FAILURE == 
        (rc =  m_NCS_IPC_SEND(&eds_cb->mbx, edsv_evt, mds_info->info.receive.i_priority) ))
   {
      m_LOG_EDSV_S(EDS_MDS_FAILURE,NCSFL_LC_EDSV_INIT,NCSFL_SEV_ERROR,edsv_evt->evt_type,__FILE__,__LINE__,mds_info->info.receive.i_node_id);
      eds_evt_destroy(edsv_evt);
      return NCSCC_RC_FAILURE;
   }
   
   /* return EDA CB */
   ncshm_give_hdl((uns32)mds_info->i_yr_svc_hdl);
...
static uns32
eds_mds_svc_event(struct ncsmds_callback_info *info)
{
   uns32        eds_cb_hdl;
   EDS_CB       *eds_cb=NULL;
   ...
   {
      m_LOG_EDSV_S(EDS_MDS_FAILURE,NCSFL_LC_EDSV_INIT,NCSFL_SEV_ERROR,info->info.svc_evt.i_your_id,__FILE__,__LINE__,0);
      rc = NCSCC_RC_FAILURE;
      goto give_hdl;
   }
   ...
   if (info->info.svc_evt.i_svc_id == NCSMDS_SVC_ID_EDA)
   {
      if(info->info.svc_evt.i_change == NCSMDS_DOWN)
      {
         ...
         {
             rc = NCSCC_RC_FAILURE;
             m_LOG_EDSV_S(EDS_MEM_ALLOC_FAILED,NCSFL_LC_EDSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__,0);
             goto give_hdl;
         }
         ...
         {
            m_LOG_EDSV_S(EDS_MDS_FAILURE,NCSFL_LC_EDSV_INIT,NCSFL_SEV_ERROR,info->info.receive.i_node_id,__FILE__,__LINE__,evt->evt_type);
            eds_evt_destroy(evt);
            rc = NCSCC_RC_FAILURE;
            goto give_hdl;
         }
...
give_hdl:      
   /* Give the hdl back */
   ncshm_give_hdl((uns32)eds_cb_hdl);
...
static uns32
eds_mds_sys_event(struct ncsmds_callback_info *mds_info)
{
...
static uns32
eds_mds_quiesced_ack(struct ncsmds_callback_info *mds_info)
{
   uns32        eds_cb_hdl;
   EDS_CB       *eds_cb=NULL;
   ...
   {  
      ...
      **/
      ...
      /* Push the event and we are done */
      if(NCSCC_RC_FAILURE == 
         m_NCS_IPC_SEND(&eds_cb->mbx, edsv_evt,NCS_IPC_PRIORITY_NORMAL))
      {
         m_LOG_EDSV_S(EDS_MDS_FAILURE,NCSFL_LC_EDSV_INIT,NCSFL_SEV_ERROR,edsv_evt->evt_type,__FILE__,__LINE__,0);
         ncshm_give_hdl((uns32)mds_info->i_yr_svc_hdl);
         eds_evt_destroy(edsv_evt);
         return NCSCC_RC_FAILURE;
      }
      ...
      /* return EDA CB */
    ncshm_give_hdl((uns32)mds_info->i_yr_svc_hdl);
    m_LOG_EDSV_S(EDS_MDS_SUCCESS,NCSFL_LC_EDSV_INIT,NCSFL_SEV_NOTICE,edsv_evt->evt_type,__FILE__,__LINE__,0);
  }
  else
  {
    ncshm_give_hdl((uns32)mds_info->i_yr_svc_hdl);
...
static uns32 eds_mds_callback(struct ncsmds_callback_info *info)
{
   uns32  rc = NCSCC_RC_SUCCESS;
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 eds_mds_vdest_create(EDS_CB *eds_cb)
{
 ...
 /*  vda_info.info.vdest_create.i_create_type = NCSVDA_VDEST_CREATE_SPECIFIC; */
   ...
   {
      m_LOG_EDSV_S(EDS_MDS_VDEST_CREATE_FAILED,NCSFL_LC_EDSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__,0);
      return rc;
   }
...
uns32 eds_mds_init(EDS_CB *cb)
{
   ...
   {
      m_NCS_CONS_PRINTF(" eds_mds_init: named vdest create FAILED\n");
      return rc;
   }
   m_NCS_CONS_PRINTF(" eds_mds_init: named vdest create SUCCESS\n");
   ...
   {
      m_LOG_EDSV_S(EDS_MDS_INIT_ROLE_CHANGE_FAILED,NCSFL_LC_EDSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__,0);
      m_NCS_CONS_PRINTF(" eds_mds_init: MDS role change to %d FAILED\n",cb->mds_role);
      return rc;
   }
   ...
   {
      m_NCS_CONS_PRINTF(" eds_mds_init: MDS Install FAILED\n");
   ...
   if (rc != NCSCC_RC_SUCCESS)
   {
      m_NCS_CONS_PRINTF(" eds_mds_init: MDS subscribe FAILED\n");
   ...
   m_NCS_CONS_PRINTF(" eds_mds_init: MDS subscribe SUCCESS\n");
   return rc;
}
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 eds_mds_vdest_destroy (EDS_CB *eds_cb)
{
   NCSVDA_INFO    vda_info;
   uns32          rc;
   ...
   if(NCSCC_RC_SUCCESS != ( rc = ncsvda_api(&vda_info)))
   {
      m_LOG_EDSV_S(EDS_MDS_VDEST_DESTROY_FAILED,NCSFL_LC_EDSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__,0);
      return rc;
   }
                        ...
                        MDS_SEND_PRIORITY_TYPE prio)
{
   ...
   rc = ncsmds_api(&mds_info);
   if(rc != NCSCC_RC_SUCCESS)
      m_LOG_EDSV_S(EDS_MDS_FAILURE,NCSFL_LC_EDSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__,m_NCS_NODE_ID_FROM_MDS_DEST(*dest));
   return rc;
}
...
uns32 eds_mds_ack_send (EDS_CB   *cb,
                         EDSV_MSG *msg,
                         MDS_DEST dest,
                         uns32    timeout,
                         uns32    prio)
{
   ...
   if (NCSCC_RC_SUCCESS != (rc = ncsmds_api(&mds_info)))
   {
      m_LOG_EDSV_S(EDS_MDS_FAILURE,NCSFL_LC_EDSV_INIT,NCSFL_SEV_ERROR,msg->type,__FILE__,__LINE__,m_NCS_NODE_ID_FROM_MDS_DEST(dest));
   }
