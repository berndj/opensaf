...
uns32 ncs_spir_api(NCS_SPIR_REQ_INFO *info)
{
      ...
      (info->type != NCS_SPIR_REQ_LOOKUP_NEXT_INST))
   {
      ...
      rc = m_NCS_SPRR_DBG_SINK(NCSCC_RC_FAILURE, "Null environment-id for a per-environment Service-Provider");
      goto quit;
   }
   ...
   case NCS_SPIR_REQ_ADD_INST:
      ...
      spir_entry->use_count = 1; splr_entry->inst_count++; /*Note:Splr-entry */
   ...
   case NCS_SPIR_REQ_RMV_INST:
      /* Check for presence of such an entry */
      spir_entry = (NCS_SPIR_ENTRY *)ncs_patricia_tree_get( &sprr_cb->spir_cb.spi_list, 
         (uns8*)&spir_key);
   ...
   case NCS_SPIR_REQ_LOOKUP_NEXT_INST:
      /* Check for presence of such an entry */
      spir_entry = (NCS_SPIR_ENTRY *)ncs_patricia_tree_getnext( &sprr_cb->spir_cb.spi_list, 
         (uns8*)&spir_key);

      if ((spir_entry == NULL) ||
          (0 != strcmp(spir_entry->key.sp_abstract_name, spir_key.sp_abstract_name)))
      {
         /* There is no such entry. Reject this request */
         rc = NCSCC_RC_NO_OBJECT;
         goto quit;
   ...
   case NCS_SPIR_REQ_LOOKUP_CREATE_INST:
      ...
      if (spir_entry == NULL)
      {
         ...
         if ((*splr_entry->inst_api)(&lib_req) != NCSCC_RC_SUCCESS)
         {
            ...
            m_NCSSPRR_TRACE_ARG4("SPIR:LOOKUP_CREATE_INST:FAILURE(instantiation):ab-name=%s:instance_name=%s:env-id=%d",
               spir_key.sp_abstract_name, spir_key.instance_name.value, spir_key.environment_id);
            goto quit;
         }
      ...
      spir_entry->use_count++; splr_entry->inst_count++; /*Note:Splr-entry */
