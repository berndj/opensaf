...
uns32 gla_mds_get_handle(GLA_CB *cb)
{
   ...
   {
      m_LOG_GLA_HEADLINE(GLA_MDS_GET_HANDLE_FAILED,NCSFL_SEV_ERROR);
      return rc;
   }
...
uns32 gla_mds_callback(struct ncsmds_callback_info *info)
{
   ...
   if(rc != NCSCC_RC_SUCCESS)
   {
      m_LOG_GLA_HEADLINE(GLA_MDS_CALLBK_FAILURE ,NCSFL_SEV_INFO);
   }
...
static uns32 gla_mds_enc_flat(GLA_CB *cb, MDS_CALLBACK_ENC_FLAT_INFO *info)
{
  ...
  /* Get the Msg Format version from the SERVICE_ID & RMT_SVC_PVT_SUBPART_VERSION */
   ...
   {
    /* Drop The Message */
    m_LOG_GLA_HEADLINE(GLA_MSG_FRMT_VER_INVALID, NCSFL_SEV_ERROR);
    m_NCS_CONS_PRINTF("gla_mds_enc_flat:INVALID MSG FORMAT %d\n",info->o_msg_fmt_ver);
    return NCSCC_RC_FAILURE;
   }
}
...
static uns32 gla_mds_enc(GLA_CB *cb, MDS_CALLBACK_ENC_INFO *info)
{
   ...
   if (uba == NULL)
   {
    m_LOG_GLA_HEADLINE(GLA_MDS_ENC_FLAT_FAILURE ,NCSFL_SEV_ERROR);
    return NCSCC_RC_FAILURE;
   }
...
static uns32 gla_mds_dec_flat(GLA_CB *cb, MDS_CALLBACK_DEC_FLAT_INFO *info)
{
   ...
   else
   {
      if(!is_valid_msg_fmt)
       {    
        m_LOG_GLA_HEADLINE(GLA_MSG_FRMT_VER_INVALID, NCSFL_SEV_ERROR);
        m_NCS_CONS_PRINTF("gla_mds_dec_flat:INVALID MSG FORMAT %d\n",is_valid_msg_fmt);
       }
      m_LOG_GLA_HEADLINE(GLA_MDS_DEC_FLAT_FAILURE ,NCSFL_SEV_ERROR);
      return NCSCC_RC_FAILURE;
   }
}
   ...
   case NCSMDS_UP:
      ...
      case NCSMDS_SVC_ID_GLND:
         ...
         if(cb->glnd_svc_up == FALSE)
         {
            ...
            m_LOG_GLA_HEADLINE(GLA_GLND_SERVICE_UP ,NCSFL_SEV_INFO);
            if(gla_agent_register(cb) != NCSCC_RC_SUCCESS)
            {
               m_LOG_GLA_HEADLINE(GLA_AGENT_REGISTER_FAILURE ,NCSFL_SEV_ERROR);
            }
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 gla_mds_msg_sync_send (GLA_CB *cb, 
                             GLSV_GLND_EVT   *i_evt,
                             GLSV_GLA_EVT    **o_evt,
                             uns32  timeout)
{
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 gla_mds_msg_async_send (GLA_CB *cb, 
                              GLSV_GLND_EVT   *i_evt)
{
   NCSMDS_INFO                mds_info;
...
static uns32 glsv_gla_dec_callbk_evt(NCS_UBAID *uba,GLSV_GLA_CALLBACK_INFO *evt)
{
 ...
 if(!p8)
 {
  m_LOG_GLA_HEADLINE(GLA_MDS_DEC_FAILURE, NCSFL_SEV_ERROR);
  return NCSCC_RC_FAILURE;
 }
