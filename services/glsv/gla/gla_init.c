...
uns32 gla_lib_req(NCS_LIB_REQ_INFO *req_info)
{
   ...
   case NCS_LIB_REQ_CREATE:
      ...
      if(rc == NCSCC_RC_SUCCESS)
         m_LOG_GLA_HEADLINE(GLA_SE_API_CREATE_SUCCESS ,NCSFL_SEV_INFO);
      else
         m_LOG_GLA_HEADLINE(GLA_SE_API_CREATE_FAILED ,NCSFL_SEV_ERROR);
      break;
      
   case NCS_LIB_REQ_DESTROY:
      rc = gla_destroy(&req_info->info.destroy);
      if(rc == NCSCC_RC_SUCCESS)
         m_LOG_GLA_HEADLINE(GLA_SE_API_DESTROY_SUCCESS ,NCSFL_SEV_INFO);
      else
         m_LOG_GLA_HEADLINE(GLA_SE_API_DESTROY_FAILED ,NCSFL_SEV_ERROR);
      break;
...
uns32 gla_create (NCS_LIB_CREATE *create_info)
{
   ...
   {
      m_LOG_GLA_MEMFAIL(GLA_CB_ALLOC_FAILED);
      rc = NCSCC_RC_FAILURE;
      goto error1;
   }
   ...
   if ( !(cb->agent_handle_id = ncshm_create_hdl(cb->pool_id, 
                                                 ...
                                                 (NCSCONTEXT)cb)) )
   {
      m_LOG_GLA_HEADLINE(GLA_CREATE_HANDLE_FAILED,NCSFL_SEV_ERROR);
      rc = NCSCC_RC_FAILURE;
      goto error2;
   }
   ...
   {
      m_LOG_GLA_LOCKFAIL(GLA_CB_LOCK_INIT_FAILED ,NCSFL_SEV_ERROR);
      rc = NCSCC_RC_FAILURE;
      goto error3;
   }
   ...
   {
      m_LOG_GLA_HEADLINE(GLA_CLIENT_TREE_INIT_FAILED ,NCSFL_SEV_ERROR);
      rc = NCSCC_RC_FAILURE;
      goto error4;
   }
   ...
   {
      m_LOG_GLA_HEADLINE(GLA_MDS_REGISTER_FAILED ,NCSFL_SEV_ERROR);
      rc = NCSCC_RC_FAILURE;
      goto error5;
   } 
   ...
   m_LOG_GLA_HEADLINE(GLA_MDS_REGISTER_SUCCESS ,NCSFL_SEV_INFO);
...
GLA_CLIENT_INFO *gla_client_tree_find_and_add(GLA_CB *gla_cb, SaLckHandleT hdl_id, NCS_BOOL flag)
{
   ...
   if(flag == TRUE)
   {
      ...
      if(!client_info)
      {
         ...
         /* create the association with hdl-mngr */
         if ( 0 == (client_info->lcl_lock_handle_id = ncshm_create_hdl(NCS_HM_POOL_ID_EXTERNAL1,NCS_SERVICE_ID_GLA,(NCSCONTEXT)client_info)))
         {
               m_MMGR_FREE_GLA_CLIENT_INFO(client_info);
               return NULL;
         } 
         ...
         {
            m_LOG_GLA_HEADLINE(GLA_CLIENT_TREE_ADD_FAILED,NCSFL_SEV_ERROR);
            ncshm_destroy_hdl(NCS_SERVICE_ID_GLA,  client_info->lcl_lock_handle_id);
            m_MMGR_FREE_GLA_CLIENT_INFO(client_info);
            m_NCS_UNLOCK(&gla_cb->cb_lock, NCS_LOCK_WRITE);
            return NULL;
...
uns32 gla_client_info_send(GLA_CB *gla_cb)
{
   ...
   while(client_info)
   {
      ...
      lck_clbk |= (uns16)((client_info->lckCallbk.saLckResourceUnlockCallback)?GLSV_LOCK_UNLOCK_CBK_REG:0);
      ...
      if((ret = gla_mds_msg_async_send(gla_cb,&restart_client_info_evt)) != NCSCC_RC_SUCCESS)
      {
          ...
          m_NCS_NODE_ID_FROM_MDS_DEST(gla_cb->gla_mds_dest),GLSV_GLND_EVT_CLIENT_INFO);
          return NCSCC_RC_FAILURE;
      }

      /* Send resource info for this client */
      gla_resource_info_send(gla_cb,client_info->lock_handle_id);
      client_info = (GLA_CLIENT_INFO *)ncs_patricia_tree_getnext(&gla_cb->gla_client_tree, (uns8 *)&client_info->lock_handle_id);
  }
  return NCSCC_RC_SUCCESS;
}
/****************************************************************************
...
static uns32 gla_resource_info_send(GLA_CB *gla_cb, SaLckHandleT  hdl_id)
{
   ...
   while(res_info)
   {
      ...
      res_info = (GLA_RESOURCE_ID_INFO*)ncs_patricia_tree_getnext(&gla_cb->gla_resource_id_tree, (uns8 *)&res_info->lcl_res_id);
...
uns32 gla_client_tree_delete_node(GLA_CB *gla_cb, GLA_CLIENT_INFO *client_info, NCS_BOOL give_hdl)
{
   ...
   if(ncs_patricia_tree_del(&gla_cb->gla_client_tree,&client_info->patnode)!=NCSCC_RC_SUCCESS)
      m_LOG_GLA_HEADLINE(GLA_CLIENT_TREE_DEL_FAILED,NCSFL_SEV_ERROR);
...
static uns32 gla_client_res_tree_cleanup(GLA_CLIENT_INFO *client_info)
{
   ...
   /* scan the entire handle db & delete each record */
   while(( client_res_info = (GLA_CLIENT_RES_INFO *)ncs_patricia_tree_getnext(&client_info->client_res_tree,(uns8 *)&res_id)))
   {
      /* delete the client info */
...
/****************************************************************************
  ...
  Notes         : None
******************************************************************************/
void gla_res_tree_cleanup_client_down(GLA_CB *gla_cb,
                                      SaLckHandleT handle)
{
   GLA_RESOURCE_ID_INFO  *res_id_info;
   SaLckResourceHandleT res_id = 0;   
...
/****************************************************************************
  ...
  Notes         : None
******************************************************************************/
void gla_lock_tree_cleanup_client_down(GLA_CB *gla_cb,
                                       SaLckHandleT handle)
{
   GLA_LOCK_ID_INFO *lock_id_info = NULL;
   SaLckLockIdT      lock_id = 0;
...
/****************************************************************************
  ...
  Notes         : None
******************************************************************************/
void gla_res_lock_tree_cleanup_client_down(GLA_CB *gla_cb, GLA_RESOURCE_ID_INFO *res_info,
                                       SaLckHandleT handle)
{
   GLA_LOCK_ID_INFO *lock_id_info = NULL;
   SaLckLockIdT      lock_id = 0;
