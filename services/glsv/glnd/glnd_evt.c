...
static uns32 glnd_process_non_master_lck_req_status(GLND_CB *glnd_cb, GLND_RESOURCE_INFO  *res_node, GLND_RES_LOCK_LIST_INFO  *lock_list_info,
...
static uns32 glnd_process_gla_resource_open(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   else
   {
      ...
      res_req_node = glnd_resource_req_node_add(glnd_cb,rsc_info,&evt->mds_context,rsc_info->lcl_resource_id);
...
static uns32 glnd_process_gla_client_info(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   if(restart_client_info.resource_id)
   {
    /* Add res_node to the glnd_res_tree of glnd_cb */
       res_node = (GLND_RESOURCE_INFO *)glnd_restart_client_resource_node_add(glnd_cb,restart_client_info.resource_id);
...
static uns32 glnd_process_gla_resource_close(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   if(res_node)
   {
      ...
      glnd_client_node_lcl_resource_del(glnd_cb,client_info,res_node,rsc_info->lcl_resource_id,rsc_info->lcl_resource_id_count,&resource_del);
      ...
      if(resource_del == TRUE )
      {
        if( res_node->lcl_ref_cnt != 0 && rsc_info->lcl_resource_id_count == 1 )
        {
            ...
            if(res_node->lcl_ref_cnt == 0 && glnd_resource_grant_list_orphan_locks(res_node,&mode) == FALSE)
            {
...
static uns32 glnd_process_check_master_and_non_master_status(GLND_CB *glnd_cb, GLND_RESOURCE_INFO  *res_info)
{
   ...
   {
      if( lock_list_info->lock_info.lock_type == SA_LCK_EX_LOCK_MODE && lock_list_info->non_master_status == GLND_RESTART_STATE )
         return SA_AIS_ERR_TRY_AGAIN;
   }
   return SA_AIS_OK;
}
...
static uns32 glnd_process_gla_resource_lock(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   if( rsc_lock_info->lock_type == SA_LCK_EX_LOCK_MODE &&
      glnd_client_node_resource_lock_find_duplicate_ex(client_info,res_node->resource_id,rsc_lock_info->lcl_resource_id) == NCSCC_RC_FAILURE)
   {
   ...
   if(res_node->status == GLND_RESOURCE_ACTIVE_MASTER)
   {
      ...
      lck_list_info = glnd_resource_master_process_lock_req(glnd_cb,res_node,lck_info,glnd_cb->glnd_mdest_id, rsc_lock_info->lcl_resource_id,rsc_lock_info->lcl_lockid);
   ...
   {
        lck_list_info = glnd_resource_non_master_lock_req(glnd_cb,res_node,lck_info,rsc_lock_info->lcl_resource_id,rsc_lock_info->lcl_lockid);
        if(!lck_list_info)
        {
...
static uns32 glnd_process_gla_resource_unlock(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   if(res_node->status == GLND_RESOURCE_ACTIVE_MASTER)
   {
      ...
      if(m_info)
      {
         ...
         if(rsc_unlock_info->call_type == GLSV_SYNC_CALL)
         {
            ...
            /* send the evt to GLA */
            glnd_mds_msg_send_rsp_gla(glnd_cb,&gla_evt,m_info->lock_info.agent_mds_dest,&evt->mds_context);
         }
         else
         {
         ...
         if(glnd_client_node_resource_lock_req_find_and_del(client_info,
            res_node->resource_id,m_info->lock_info.lockid,m_info->lcl_resource_id) == NCSCC_RC_SUCCESS)
         {
   ...
   {
      /* non-master stuff */
      m_info = glnd_resource_non_master_unlock_req(glnd_cb,res_node,lck_info,rsc_unlock_info->lcl_resource_id,rsc_unlock_info->lcl_lockid);
      if(!m_info)
      {
         error = SA_AIS_ERR_INVALID_PARAM;
         goto err;
...
static uns32 glnd_process_glnd_lck_req(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   if( rc == SA_AIS_ERR_TRY_AGAIN )
   {
        ...
        m_GLND_RESOURCE_NODE_LCK_INFO_FILL(glnd_evt,GLSV_GLND_EVT_LCK_RSP,
               res_node->resource_id,lck_req->lcl_resource_id,lck_req->client_handle_id,
               lck_req->lockid,
               lck_req->lock_type,
               lck_req->lockFlags,
               0,
               0,0,SA_AIS_ERR_TRY_AGAIN,
   ...
   {
      /* request for the lock */
      lck_list_info = glnd_resource_master_process_lock_req(glnd_cb,res_node,lck_info,lck_req->glnd_mds_dest,lck_req->lcl_resource_id,lck_req->lcl_lockid);
      if(lck_list_info)
      {
         ...
         case SA_LCK_LOCK_ORPHANED:
            /* send the status to the non-master glnd */
            m_GLND_RESOURCE_NODE_LCK_INFO_FILL(glnd_evt,GLSV_GLND_EVT_LCK_RSP,
                  res_node->resource_id,lck_req->lcl_resource_id,lck_info.handleId,
                  lck_list_info->lock_info.lockid,
                  lck_info.lock_type,
                  lck_info.lockFlags,
                  lck_list_info->lock_info.lockStatus,
...
static uns32 glnd_process_glnd_unlck_req(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   lck_list_info = glnd_resource_master_unlock_req(glnd_cb,res_node,lck_info,lck_req->glnd_mds_dest,lck_req->lcl_resource_id);
...
/*   err: */
    /* send back the non-master glnd about the failure */
     m_GLND_RESOURCE_NODE_LCK_INFO_FILL(glnd_evt,GLSV_GLND_EVT_UNLCK_RSP,
                  ...
                  0,
                  SA_LCK_LOCK_NO_MORE,
                  0,0,error,lck_req->lcl_lockid,0);
...
static uns32 glnd_process_glnd_lck_rsp(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   /* get the lock node */
   lck_list_info = glnd_resource_local_lock_req_find(res_node,lck_rsp->lcl_lockid,lck_rsp->client_handle_id,lck_rsp->lcl_resource_id);
   if(!lck_list_info)
   {
   ...
   if( lck_rsp->error == SA_AIS_ERR_TRY_AGAIN )
   {
      ...
      if( glnd_client_node_resource_lock_req_find_and_del(client_info,
            res_node->resource_id,lck_list_info->lock_info.lockid,lck_list_info->lcl_resource_id) == NCSCC_RC_SUCCESS )
      {
   ...
   if(lck_rsp->lockStatus == SA_LCK_LOCK_GRANTED)
      glnd_restart_res_lock_list_ckpt_overwrite(glnd_cb,lck_list_info,lck_list_info->res_info->resource_id,0,1);
...
static uns32 glnd_process_glnd_lck_req_cancel(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   lck_list_info = glnd_resource_remote_lock_req_find(res_node,lck_req_cancel->lcl_lockid,
      lck_req_cancel->client_handle_id,lck_req_cancel->glnd_mds_dest,lck_req_cancel->lcl_resource_id);
...
static uns32 glnd_process_glnd_lck_req_orphan(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   lck_list_info = glnd_resource_remote_lock_req_find(res_node,lck_req_orphan->lockid,
      lck_req_orphan->client_handle_id,lck_req_orphan->glnd_mds_dest,lck_req_orphan->lcl_resource_id);
...
static uns32 glnd_process_glnd_lck_waiter_clbk(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   /* get the lock node */
    lck_list_info = glnd_resource_local_lock_req_find(res_node,waiter_clbk->lcl_lockid,waiter_clbk->client_handle_id,
...
static uns32 glnd_process_glnd_fwd_dd_probe(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   GLND_RES_LOCK_LIST_INFO       *wait_list = NULL, 
                                 *grant_list = NULL;
   GLSV_GLND_EVT                 glnd_evt;
   ...
   while (grant_list != NULL)
   {
      ...
      while(fwd_dd_info_list != NULL)
      {
          ...
          if(fwd_dd_info_list->next != NULL)
          {
             ...
             dd_info_list->next = m_MMGR_ALLOC_GLSV_GLND_DD_INFO_LIST(sizeof(GLSV_GLND_DD_INFO_LIST),NCS_SERVICE_ID_GLND);
         ...
         if (lck_req_info->lck_req->lock_info.lockStatus != SA_LCK_LOCK_GRANTED)
         {
          ...
          while(fwd_dd_info_list != NULL)
           ...
           m_NCS_OS_MEMSET(dd_info_list, 0, sizeof(GLSV_GLND_DD_INFO_LIST));
            ...
            {
               if(glnd_mds_msg_send_glnd(glnd_cb,&snd_probe_evt,client_res_list->rsc_info->master_mds_dest) != NCSCC_RC_SUCCESS)
               {
                   /* GLSV_ADD_LOG_HERE MDS send failed */
               }
               dd_info_list = snd_probe_evt.info.dd_probe_info.dd_info_list;
      ...
      do
      {
         ...
         if(res_info)
         {
            ...
            glnd_client_node_resource_add(
                     glnd_client_node_find(glnd_cb,res_list_node->client_handle_id),res_info);
   ...
   if(m_GLND_IS_LOCAL_NODE(&glnd_cb->glnd_mdest_id,&new_master_info->master_dest_id)==0)
   {
     if(new_master_info->status == GLND_RESOURCE_ELECTION_IN_PROGESS )
     {
      if(res_node->status == GLND_RESOURCE_ACTIVE_NON_MASTER)
      {
         ...
         if(new_master_info->orphan == TRUE && new_master_info->orphan_lck_mode == SA_LCK_PR_LOCK_MODE)
            res_node->lck_master_info.pr_orphaned = TRUE;
         else if(new_master_info->orphan == TRUE && new_master_info->orphan_lck_mode == SA_LCK_EX_LOCK_MODE)
            res_node->lck_master_info.ex_orphaned = TRUE;
   ...
   else
   {
     if(new_master_info->status == GLND_RESOURCE_ELECTION_IN_PROGESS )
     {
         ...
         res_node->status = GLND_RESOURCE_ELECTION_IN_PROGESS;
          ...
          (uns32) res_node->resource_id, (uns32)m_NCS_NODE_ID_FROM_MDS_DEST(res_node->master_mds_dest));
...
static uns32 glnd_process_tmr_resource_req_timeout(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   if(res_req_info)
   {
      ...
      if(res_req_info->call_type == GLSV_SYNC_CALL)
      {
         gla_evt.type = GLSV_GLA_API_RESP_EVT;
         gla_evt.info.gla_resp_info.type = GLSV_GLA_LOCK_RES_OPEN;
         glnd_mds_msg_send_rsp_gla(glnd_cb,&gla_evt,res_req_info->agent_mds_dest,&res_req_info->glnd_res_mds_ctxt);         
      }
      else
      {
         gla_evt.type = GLSV_GLA_CALLBK_EVT;
...
static uns32 glnd_process_tmr_nm_resource_lock_req_timeout(GLND_CB *glnd_cb, GLSV_GLND_EVT *evt)
{
   ...
   if( glnd_evt_backup_queue_delete_lock_req(glnd_cb,lck_list_info->lock_info.
       handleId,lck_list_info->res_info->resource_id,lck_list_info->lock_info.lock_type) != NCSCC_RC_SUCCESS)
   {
            ...
            if(node_id == m_NCS_NODE_ID_FROM_MDS_DEST(temp_info->req_mdest_id))
            {
               if(node_status->status == GLND_OPERATIONAL_STATE )
               {
                  if(temp_info->non_master_status == GLND_RESTART_STATE)
                  {
                     ...
                     temp_info->lock_info.handleId,
                     temp_info->lock_info.lockid,
                     temp_info->lock_info.lock_type,
                     temp_info->lock_info.lockFlags,
                     temp_info->lock_info.lockStatus,
                     0,0,SA_AIS_OK,temp_info->lock_info.lcl_lockid,0);
                     glnd_evt.info.node_lck_info.glnd_mds_dest = glnd_cb->glnd_mdest_id;

                     /* send the response evt to GLND */
                     glnd_mds_msg_send_glnd(glnd_cb,&glnd_evt,node_status->dest_id);

                  }

               }
               glnd_process_non_master_lck_req_status(glnd_cb, res_node, temp_info, node_status);
            }
         }
                                                    ...
                                                    GLND_EVT_GLND_NON_MASTER_STATUS *node_status)
{
     ...
     else
     {
       ...
       glnd_restart_res_lock_list_ckpt_overwrite(glnd_cb,lock_list_info,lock_list_info->res_info->resource_id,0,2);
     }
     return NCSCC_RC_SUCCESS;
}
