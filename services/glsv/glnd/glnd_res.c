...
static uns32 glnd_initiate_deadlock_algorithm(GLND_CB             *cb,
                                      GLND_RESOURCE_INFO  *res_info,
...
GLND_RESOURCE_INFO *glnd_resource_node_find_by_name(GLND_CB *glnd_cb, SaNameT res_name)
{
   ...
   while(res_info)
   {
      ...
      res_info = (GLND_RESOURCE_INFO *)ncs_patricia_tree_getnext(&glnd_cb->glnd_res_tree,(uns8 *)&prev_rsc_info);
   }
   return NULL;
}
...
/*****************************************************************************
  ...
  NOTES         : Decrements the reference count and deletes the node when it reaches
                  zero.
*****************************************************************************/
uns32 glnd_set_orphan_state(GLND_CB            *glnd_cb,
                             GLND_RESOURCE_INFO *res_info)
{
...
/*****************************************************************************
  ...
  NOTES         : Decrements the reference count and deletes the node when it reaches
                  zero.
*****************************************************************************/
uns32 glnd_resource_node_destroy(GLND_CB *glnd_cb, 
                                 GLND_RESOURCE_INFO *res_info)
{
   GLSV_GLD_EVT                evt; 
   ...
   while(client_info)
   {
      ...
      for(res_list = client_info->res_list;res_list!= NULL;)
      {
       ...
       client_info = (GLND_CLIENT_INFO *)ncs_patricia_tree_getnext(&glnd_cb->glnd_client_tree,(uns8 *)&prev_app_handle_id);
    }
                                    ...
                                    GLND_RES_LOCK_LIST_INFO *lck_list_info)
{
   ...
   if(res_info->lck_master_info.grant_list == lck_list_info)
   {
         res_info->lck_master_info.grant_list = lck_list_info->next;
   }
   else
   if(res_info->lck_master_info.wait_read_list == lck_list_info)
   {
       res_info->lck_master_info.wait_read_list = lck_list_info->next;
   }
   else
   if(res_info->lck_master_info.wait_exclusive_list == lck_list_info)
   {
   ...
   m_GLSV_DEBUG_CONS_PRINTF("\n  GLND_RESOURCE_LOCK_REQ_DESTROY res - %d lockid- %d \n",
...
/*****************************************************************************
    ...
    NOTES         : None
*****************************************************************************/
uns32 glnd_initiate_deadlock_algorithm(GLND_CB             *cb,
                                      GLND_RESOURCE_INFO  *res_info,
                                      GLSV_LOCK_REQ_INFO  lock_info,
                                      GLND_RES_LOCK_LIST_INFO   *lck_list_info
)
{
     ...
     else
     {
            ...
            dd_info_list->blck_dest_id = glnd_evt.info.dd_probe_info.dest_id
...
SaLckLockIdT      lcl_lock_id)
{
   ...
   case SA_LCK_EX_LOCK_MODE:
         ...
         glnd_resource_grant_list_orphan_locks(res_info,&mode) == FALSE)
      {
         ...
         m_GLSV_DEBUG_CONS_PRINTF("\n LOCK_GRANTED handle - %d res - %d lockid- %d \n",
            (uns32)lock_info.handleId,(uns32)res_info->resource_id,(uns32)lck_list_info->lock_info.lockid);
   ...
   case SA_LCK_PR_LOCK_MODE:
      ...
      else
      {
         ...
         m_GLSV_DEBUG_CONS_PRINTF("\n LOCK_QUEUED handle - %d res - %d lockid- %d \n",
            (uns32)lock_info.handleId,(uns32)res_info->resource_id,(uns32)lck_list_info->lock_info.lockid);
                                                         ...
                                                         
{
   ...
   if(res_info->status == GLND_RESOURCE_ACTIVE_MASTER)
   {
      ...
      lock_list_info = glnd_resource_grant_lock_req_find(res_info,lock_info,req_mds_dest,
                                                          lcl_resource_id);
      if(!lock_list_info)
      {
      ...
      {
         m_GLSV_DEBUG_CONS_PRINTF("\n UNLOCK_DONE handle - %d res - %d lockid- %d \n",
                                                            ...
                                                            SaLckResourceIdT  lcl_resource_id,SaLckLockIdT      lcl_lock_id)
{
   ...
   /* search for the lock info in the queues */
   lck_list_info = glnd_resource_local_lock_req_find(res_info,lock_info.lcl_lockid,lock_info.handleId,lcl_resource_id);
   if(lck_list_info == NULL)
   {
...
/*****************************************************************************
  ...
  NOTES         : None
*****************************************************************************/
NCS_BOOL glnd_resource_grant_list_orphan_locks(GLND_RESOURCE_INFO  *res_info,
                                               SaLckLockModeT      *mode) 
{
         ...
         if(client_info)
         {
            if((client_info->cbk_reg_info & GLSV_LOCK_WAITER_CBK_REG) == GLSV_LOCK_WAITER_CBK_REG)
            {
                m_GLND_RESOURCE_ASYNC_LCK_WAITER_FILL(gla_evt,
                 lock_list_info->lock_info.lockid,lock_list_info->lcl_resource_id,
   ...
   if(m_GLND_IS_LOCAL_NODE(&lock_list_node->req_mdest_id,&glnd_cb->glnd_mdest_id) == 0)
   {
      ...
      if(lock_list_node->lock_info.call_type == GLSV_SYNC_CALL)
      {
         ...
         glnd_mds_msg_send_rsp_gla(glnd_cb,&gla_evt,
            lock_list_node->lock_info.agent_mds_dest,&lock_list_node->glnd_res_lock_mds_ctxt);
      }
      else
      {
...
/*****************************************************************************
  ...
  NOTES         : None
*****************************************************************************/
static void glnd_resource_master_move_pr_locks_to_grant_list(GLND_CB *glnd_cb, 
                                                             GLND_RESOURCE_INFO *res_info)
{
...
/*****************************************************************************
  ...
  NOTES         : None
*****************************************************************************/
static void glnd_resource_master_move_ex_locks_to_grant_list(GLND_CB *glnd_cb, 
                                                             GLND_RESOURCE_INFO *res_info)
{
...
void glnd_resource_convert_nonmaster_to_master(GLND_CB *glnd_cb,GLND_RESOURCE_INFO  *res_node)
{
   ...
   for(lck_list_nm_info= res_node->lcl_lck_req_info;lck_list_nm_info!=NULL;)
   {
       ...
       if(remaining_time>0)
       {
          ...
          if(lck_list_m_info->timeout_tmr.type == GLND_TMR_RES_NM_LOCK_REQ_TIMEOUT)
          {
            ...
            glnd_start_tmr(glnd_cb,&lck_list_m_info->timeout_tmr,
               GLND_TMR_RES_LOCK_REQ_TIMEOUT,
               time,
               (uns32)lck_list_m_info->lck_info_hdl_id); 
      ...
      if(lck_list_m_info->lock_info.lockStatus == SA_LCK_LOCK_GRANTED)
      {
         ...
         {
            glnd_resource_lock_req_set_orphan(glnd_cb,res_node,lck_list_m_info->lock_info.lock_type);
         }
      }
...
void glnd_resource_resend_nonmaster_info_to_newmaster(GLND_CB *glnd_cb,GLND_RESOURCE_INFO  *res_node)
{
   ...
   for(lck_list_nm_info= res_node->lcl_lck_req_info;lck_list_nm_info!=NULL;lck_list_nm_info = lck_list_nm_info->next)
   {
                               ...
                               GLSV_EVT_GLND_DD_PROBE_INFO  *dd_probe)
{
   ...
   NCS_BOOL                            deadlock_present = FALSE,
                                       ignore_probe = FALSE;
   GLSV_GLA_EVT                        gla_evt;
   GLSV_GLND_EVT                       glnd_evt;
            ...
            if (client_res_list->rsc_info->resource_id == dd_info_list->rsc_id) 
            {
               ...
               while(lck_req_info != NULL)
               {
                  if (lck_req_info->lck_req->lock_info.lockid == dd_info_list->lck_id ) 
                  {
                     ...
                     {
                        lck_req_info->lck_req->lock_info.lockStatus = SA_LCK_LOCK_DEADLOCK;
   ...
   if (deadlock_present)
   {
      ...
      if (lck_req_info->lck_req->lock_info.call_type == GLSV_SYNC_CALL)
      {
         ...
         glnd_mds_msg_send_rsp_gla(glnd_cb,&gla_evt,
            client_info->agent_mds_dest,&lck_req_info->lck_req->glnd_res_lock_mds_ctxt);
      }
      else
      {
         m_GLND_RESOURCE_ASYNC_LCK_GRANT_FILL(gla_evt,SA_AIS_OK,
