                                             ...
                                             MDS_DEST     agent_mds_dest)
{
   ...
   while(client_info)
   {
      if(m_NCS_MEMCMP(&client_info->agent_mds_dest,&agent_mds_dest,sizeof(MDS_DEST)) == 0)
         ...
         (uns8 *)&client_info->app_handle_id);
   }
   return NULL;
...
/*****************************************************************************
  ...
  NOTES         : None
*****************************************************************************/
uns32  glnd_client_node_del(GLND_CB          *glnd_cb, 
                            GLND_CLIENT_INFO *client_info)
{
   ...
   for(res_list = client_info->res_list;res_list!= NULL;)
   {
      ...
      if(res_info)
      {
        ...
        if(res_info->lcl_ref_cnt == 0 && glnd_resource_grant_list_orphan_locks(res_info,&mode) == FALSE)
        {
   ...
   if(resource_list)
   {
      ...
      for(lock_req_list = resource_list->lck_list;lock_req_list!=NULL;)
      {
         ...
         else
         {
            ...
            if((lock_req_list->lck_req->lock_info.lockFlags & SA_LCK_LOCK_ORPHAN) == SA_LCK_LOCK_ORPHAN)
       ...
       if(lock_req_list->lck_req)
       {
        if( lock_req_list->lck_req->lcl_resource_id == lcl_resource_id )
        {
         if(lock_req_list->lck_req->res_info->status == GLND_RESOURCE_ACTIVE_NON_MASTER)
         {
           ...
           /* send request to orphan the lock */
            m_GLND_RESOURCE_NODE_LCK_INFO_FILL(glnd_evt, GLSV_GLND_EVT_LCK_REQ_ORPHAN,
               lock_req_list->lck_req->res_info->resource_id,
               lock_req_list->lck_req->lcl_resource_id,
               lock_req_list->lck_req->lock_info.handleId,
               lock_req_list->lck_req->lock_info.lockid,
               lock_req_list->lck_req->lock_info.lock_type,
...
glnd_client_node_resource_lock_req_find(GLND_CLIENT_INFO *client_info,
                                                       ...
                                                       SaLckResourceIdT  res_id, SaLckResourceIdT  lcl_res_id) 
{
   ...
   if(resource_list)
   {
      ...
      {
         if(lck_req_list->lck_req->lock_info.lock_type == SA_LCK_EX_LOCK_MODE && lck_req_list->lck_req->lcl_resource_id == lcl_res_id && lck_req_list->lck_req->lock_info.lockStatus == SA_LCK_LOCK_GRANTED)
            return NCSCC_RC_FAILURE;
      }
   }
   return NCSCC_RC_SUCCESS;
