...
static uns32 glsv_gld_standby_rsc_open(GLSV_GLD_A2S_CKPT_EVT* async_evt)
{
   ...
   rsc_info = gld_find_add_rsc_name(gld_cb, async_evt->info.rsc_open_info.rsc_name, async_evt->info.rsc_open_info.rsc_id,0,&error);
   if (rsc_info == NULL)
   {
...
static uns32 glsv_gld_standby_rsc_set_orphan(GLSV_GLD_A2S_CKPT_EVT* async_evt)
{
   ...
   if( gld_rsc_ref_set_orphan(node_details, async_evt->info.rsc_details.rsc_id,
        async_evt->info.rsc_details.orphan , async_evt->info.rsc_details.lck_mode) == NCSCC_RC_SUCCESS)
   {
...
static uns32 glsv_gld_standby_mds_glnd_down(GLSV_GLD_A2S_CKPT_EVT* async_evt)
{
   ...
   glnd_rsc = (GLSV_GLD_GLND_RSC_REF *)ncs_patricia_tree_getnext
                                            (&node_details->rsc_info_tree, (uns8 *)0);
   if(glnd_rsc)
   {
...
uns32 gld_sb_proc_data_rsp(GLSV_GLD_CB *gld_cb,GLSV_GLD_A2S_RSC_DETAILS *rsc_details )
{
  ...
  if(rsc_details)
  {
    rsc_info = (GLSV_GLD_RSC_INFO *)ncs_patricia_tree_get(&gld_cb->rsc_info_id, (uns8*)&rsc_details->rsc_id);
    if(rsc_info == NULL)
           rsc_info =  gld_add_rsc_info(gld_cb,rsc_details->resource_name,rsc_details->rsc_id,&ret_error);
    if(rsc_info == NULL)
         return NCSCC_RC_FAILURE;
    else
