   ...
   m_NCS_MEMSET(a2s_evt,0,sizeof(GLSV_GLD_A2S_CKPT_EVT));
    ...
    switch(evt_type)
    {
       case GLSV_GLD_EVT_RSC_OPEN:
          ...
          rc = m_NCS_EDU_EXEC(&gld_cb->edu_hdl,glsv_edp_gld_a2s_evt_rsc_open_info,&arg->info.decode.i_uba,EDP_OP_TYPE_DEC, &rsc_info,&ederror);
          if(rc != NCSCC_RC_SUCCESS)
          {
       ...
       case GLSV_GLD_EVT_SET_ORPHAN:
          rsc_details  = &a2s_evt->info.rsc_details;
          /* The contents are decoded from i_uba to a2s_evt */
          rc = m_NCS_EDU_EXEC(&gld_cb->edu_hdl,glsv_edp_gld_a2s_evt_rsc_details,&arg->info.decode.i_uba,EDP_OP_TYPE_DEC,
 &rsc_details,&ederror);
          if(rc != NCSCC_RC_SUCCESS)
          {
       ...
       case GLSV_GLD_EVT_GLND_OPERATIONAL:
          node_details = &a2s_evt->info.glnd_mds_info; 
            /* The contents are decoded from i_uba to a2s_evt */
          rc = m_NCS_EDU_EXEC(&gld_cb->edu_hdl,glsv_edp_gld_a2s_evt_glnd_mds_info,&arg->info.decode.i_uba,EDP_OP_TYPE_DEC,&node_details,&ederror);
          if(rc != NCSCC_RC_SUCCESS)
          {
...
static uns32  glsv_gld_mbcsv_dec_sync_resp(GLSV_GLD_CB *gld_cb,NCS_MBCSV_CB_ARG *arg)
{
  ...
  /* Decode the data */
   ...
   {
      rc = m_NCS_EDU_EXEC(&gld_cb->edu_hdl,glsv_edp_gld_evt_a2s_rsc_details,&arg->info.decode.i_uba,EDP_OP_TYPE_DEC, &rsc_info, &ederror);
      if(rc != NCSCC_RC_SUCCESS)
      {
         return rc;
      }
   ...
   else
   {
    while(rsc_info != NULL)
    {
     ...
     if(node_list != NULL)
     {
      ...
      /* DO THE EDU EXEC */
      rc = m_NCS_EDU_EXEC(&gld_cb->edu_hdl,glsv_edp_gld_evt_a2s_rsc_details,&arg->info.encode.io_uba,EDP_OP_TYPE_ENC, &rsc_details,&ederror); 
      if(rc != NCSCC_RC_SUCCESS)
      {
