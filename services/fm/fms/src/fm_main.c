...
uns32 fm_tmr_start(FM_TMR *,SaTimeT);
void fm_tmr_stop(FM_TMR *);
void fm_tmr_exp(void *);
...
int main (int    argc, char * argv[])
{
    ...
    m_NCS_MEMSET(fm_cb, 0, sizeof(FM_CB));
...
static uns32 fm_get_args(FM_CB *fm_cb)
{
    ...
    while(NULL!=(fgets(readline, 100, fp)))
    {
        ...
        if (strcmp(substr, "controller_1") == 0)
        {
            ...
            if (!temp_slot_1)
            {
                m_NCS_SYSLOG(NCS_LOG_INFO,"Invalid slot_ids given \n");
                fclose(fp);
                return NCSCC_RC_FAILURE;
            }
            substr=strtok(NULL, " /");
            ...
            if (!temp_sub_slot_1)
            {
                m_NCS_SYSLOG(NCS_LOG_INFO,"Invalid sub_slot_ids given \n");
                fclose(fp);
                return NCSCC_RC_FAILURE;
            }
        }
        ...
        {
            m_NCS_SYSLOG(NCS_LOG_INFO,"Invalid token mentioned in the configuration file: %s\n", substr);
            fclose(fp);
            return NCSCC_RC_FAILURE;
        }
        token = NULL;
            ...
            fm_cb->role, fm_mbx_evt->slot, fm_mbx_evt->sub_slot);
                if (fm_cb->role == PCS_RDA_STANDBY)
                {
                            ...
                            fm_mbx_evt->sub_slot == fm_cb->peer_sub_slot)
                    {
                        ...
                        m_NCS_SYSLOG(NCS_LOG_INFO,"Promote active timer started, peer controller slot_id=%d \n",fm_cb->peer_slot);
                    }
                }
                else if (fm_cb->role == PCS_RDA_ACTIVE)
                {
                    /* Shelf id of Peer and Host are same */
                    fm_conv_shelf_slot_to_entity_path(entity_path, fm_cb->peer_shelf, fm_mbx_evt->slot, fm_mbx_evt->sub_slot);
#if (HW_MGMT==1)
...
uns32 fm_tmr_start (FM_TMR *tmr,SaTimeT period)
{
    ...
    if (tmr->tmr_id == NULL)
    {
        ...
        {
                /* TODO: Log Timer already running */
                return NCSCC_RC_FAILURE;
        }
...
void fm_tmr_exp (void *fm_tmr)
{
    ...
    if(fm_cb == NULL)
    {
        m_NCS_SYSLOG(NCS_LOG_INFO,"Taking handle failed in timer expiry \n");
        return;
    }
    ...
    if (evt != NULL)
    {
        m_NCS_MEMSET(evt,'\0',sizeof(FM_EVT));
        ...
        if (m_NCS_IPC_SEND(&fm_cb->mbx,evt,NCS_IPC_PRIORITY_HIGH)
             != NCSCC_RC_SUCCESS)
        {
            m_NCS_SYSLOG(NCS_LOG_INFO,"IPC send failed in timer expiry \n");
...
static uns32 fm_conv_shelf_slot_to_entity_path(uns8 *o_ent_path, 
                                                    uns8 shelf, 
                                                    uns8 slot,
                                                    uns8 sub_slot)
{
    if (sub_slot == 0)
        ...
        /* MAHESH: Embedding subslot into the entity path in string format */
...
static uns32 fms_fma_node_reset_intimate(FM_CB *fm_cb, uns32 slot_id, uns32 sub_slot_id)
{
    ...
    {
            m_NCS_SYSLOG(NCS_LOG_INFO,"Intimation node reset to FMA failed\n");
    }
    m_NCS_SYSLOG(NCS_LOG_INFO,
...
static uns32 fms_reset_peer(FM_CB *fm_cb)
{
    ...
    if(fm_cb->peer_adest != 0)
    {
        ...
        {
            m_NCS_SYSLOG(NCS_LOG_INFO,"Sending node-reset message to peer fms failed\n");
            return NCSCC_RC_FAILURE;
        }
        return NCSCC_RC_SUCCESS;
    }
...
static uns32 fm_create_pidfile(void)
{
    ...
    {
        m_NCS_SYSLOG(NCS_LOG_INFO,"FM PID file open failed \n");   
        return NCSCC_RC_FAILURE;
    }
   return NCSCC_RC_SUCCESS;
}
