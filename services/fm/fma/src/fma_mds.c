...
const MDS_CLIENT_MSG_FORMAT_VER fma_fm_msg_fmt_map_table [FMA_SUBPART_VER_MAX] =
                                                        {FMA_FM_MSG_FMT_VER_1};
...
static uns32 fma_mds_svc_evt(FMA_CB *fma_cb, MDS_CALLBACK_SVC_EVENT_INFO *svc_evt)
{
   ...
   case NCSMDS_DOWN:
       ...
       case NCSMDS_SVC_ID_GFM:
           /* reset the fm_dest */
           if(m_MDS_DEST_IS_AN_ADEST(svc_evt->i_dest) == TRUE && 
              fma_cb->my_node_id == node_id)
           {
              fma_cb->fm_up = FALSE;
              fma_cb->fm_dest = 0;
           }
