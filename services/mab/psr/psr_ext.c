    ...
    while(bind_evt != NULL)
    {
        ...
        while(tbl_list != NULL)
        {
            ...
            { 
               ...
               trec = pss_find_table_tree(pwe_cb, client_node, oaa_node, 
                        tbl_list->tbl_id, TRUE, NULL);
               if(trec == NULL)
               {
            ...
            {
               m_LOG_PSS_TBL_BIND_EVT(NCSFL_SEV_ERROR, PSS_TBL_BIND_MIB_DESC_NULL, msg->data.data.oac_pss_tbl_bind.pcn_list.pcn, tbl_list->tbl_id);
            }
...
uns32 pss_process_tbl_unbind(MAB_MSG * msg)
{
    ...
    {
       m_LOG_PSS_TBL_UNBIND_EVT(NCSFL_SEV_ERROR, PSS_TBL_UNBIND_IS_NOT_PERSISTENT_TABLE, msg->data.data.oac_pss_tbl_unbind.tbl_id);
       return NCSCC_RC_SUCCESS;
    }
    ...
    while(oaa_clt_node != NULL)
    {
        ...
        {
            m_LOG_PSS_TBL_UNBIND_EVT(NCSFL_SEV_INFO, PSS_TBL_UNBIND_FND_OAA_CLT_NODE, msg->data.data.oac_pss_tbl_unbind.tbl_id);
            break;
        }
    ...
    /* Get PCN from the client_table */
    client_node = pss_find_client_entry(pwe_cb, 
        (char*)&del_trec->pss_client_key->pcn, FALSE);
    if(client_node == NULL)
    {
    ...
    while(trec != NULL)
    {
        if(trec->tbl_id == msg->data.data.oac_pss_tbl_unbind.tbl_id)
        {
            ...
            if(client_node->tbl_cnt == 0)
            {
                ...
                m_LOG_PSS_CLIENT_ENTRY(NCSFL_SEV_INFO, PSS_CLIENT_ENTRY_DEL, (char*)&trec->pss_client_key->pcn, pwe_cb->pwe_id);
    ...
    if (retval != NCSCC_RC_SUCCESS)
        ...
        m_LOG_PSS_TBL_UNBIND_EVT(NCSFL_SEV_NOTICE, PSS_TBL_UNBIND_SUCCESS, msg->data.data.oac_pss_tbl_unbind.tbl_id);
...
*****************************************************************************/
PSS_SPCN_LIST *pss_findadd_entry_frm_spcnlist(PSS_CB *inst, char *p_pcn,
                                              NCS_BOOL add)
{
    ...
    {
        m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_SPCN_LIST_ALLOC_FAIL,
                                         ...
                                         NCS_UBAID *uba)
{
    ...
    for (i = 0; i < MAB_MIB_ID_HASH_TBL_SIZE; i++)
    {
        ...
        while(rec != NULL)
        {
           ...
           {
              m_LOG_PSS_WBREQ_II(NCSFL_SEV_INFO, PSS_WBREQ_II_SORT_FNC_TBL_DATA_AVAILABLE, rec->tbl_id);
           ...
           else
           {
              ...
              {
                 m_LOG_PSS_WBREQ_II(NCSFL_SEV_INFO, PSS_WBREQ_II_SORT_FNC_TBL_DATA_NOT_AVAILABLE, rec->tbl_id);
              }
           }
           rec = rec->next;
        }
                                ...
                                NCS_UBAID *uba)
{
    ...
    for(wbreq = wbreq_head; wbreq != NULL; wbreq = wbreq->next)
    {
        ...
        for(rec = client_node->hash[wbreq->tbl_id % MAB_MIB_ID_HASH_TBL_SIZE];
            rec != NULL; rec = rec->next)
        {
            if(rec->tbl_id == wbreq->tbl_id)
            {
                m_LOG_PSS_WBREQ_II(NCSFL_SEV_DEBUG, PSS_WBREQ_II_SORT_FNC_INPUT_TBL_FND_IN_PCN_NODE, wbreq->tbl_id);
                break;
            }
        }
        if(rec != NULL)
        {
           if(pss_data_available_for_table(pwe_cb, (char*)&client_node->key.pcn, 
                   rec) == TRUE)
           {
              m_LOG_PSS_WBREQ_II(NCSFL_SEV_DEBUG, PSS_WBREQ_II_SORT_FNC_TBL_DATA_AVAILABLE, wbreq->tbl_id);
              if((entry = m_MMGR_ALLOC_PSS_WBSORT_ENTRY) == NULL)
              {
                 m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_PSS_WBSORT_ENTRY_ALLOC_FAIL,
           ...
           else
           {
              ...
              if(spcn_entry != NULL)
              {
                 m_LOG_PSS_WBREQ_II(NCSFL_SEV_DEBUG, PSS_WBREQ_II_SORT_FNC_SPCN_TBL_DATA_NOT_AVAILABLE, wbreq->tbl_id);
                 *snd_evt_to_bam = TRUE;
                 if(uba->start != NULL)
                 {
                    m_MMGR_FREE_BUFR_LIST(uba->start);
              ...
              {
                 m_LOG_PSS_WBREQ_II(NCSFL_SEV_DEBUG, PSS_WBREQ_II_SORT_FNC_TBL_DATA_NOT_AVAILABLE, wbreq->tbl_id);
              }
           }
        }
        else
        {
            m_LOG_PSS_WBREQ_II(NCSFL_SEV_ERROR, PSS_WBREQ_II_SORT_FNC_INPUT_TBL_NOT_FND_IN_PCN_NODE, wbreq->tbl_id);
        }
    }
    if(p8_cnt != NULL)
       ncs_encode_16bit(&p8_cnt, (uns16)sortdb->num_entries);
...
*****************************************************************************/
uns32 pss_sort_odreq_instore_tables_with_rank(PSS_PWE_CB *pwe_cb,
                ...
                oaa_clt_entry = oaa_clt_entry->next)
            {
                ...
                if(pwe_cb->p_pss_cb->mib_tbl_desc[rec->tbl_id] != NULL) 
                {
                    ...
                    else
                    {
                        ...
                        {
                            m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_PSS_ODSORT_ENTRY_ALLOC_FAIL,
                        ...
                        if(ncs_patricia_tree_add(&sortdb->sort_db, &entry->pat_node) !=
                                NCSCC_RC_SUCCESS)
                        {
                            m_LOG_PSS_HEADLINE(NCSFL_SEV_CRITICAL, PSS_HDLN_ODSORTDB_ADD_ENTRY_FAIL);
                            m_MMGR_FREE_PSS_WBSORT_ENTRY(entry);
                            return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
                        }
                    }
                    ...
                    {
                        m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_PSS_ODSORT_TBL_REC_ALLOC_FAIL,
                                "pss_sort_odreq_instore_tables_with_rank()");
                        return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
                    }
...
*****************************************************************************/
uns32 pss_ondemand_playback_for_sorted_list(PSS_PWE_CB *pwe_cb,
                                         PSS_ODPLAYBACK_SORT_TABLE *sortdb,
                                         uns8* profile)
{
    PSS_ODSORT_KEY          lcl_key;
    ...
    while( (pNode = ncs_patricia_tree_getnext(&sortdb->sort_db,
                        (const uns8 *)&lcl_key)) != NULL )
    {
        ...
        for(od_tblrec = entry->tbl_rec_list, sort_cnt = sortdb->num_entries;
            od_tblrec != NULL;
            od_tblrec = od_tblrec->next, --sort_cnt)
        {
            ...
            if(pwe_cb->p_pss_cb->mib_tbl_desc[rec->tbl_id] != NULL) 
            {
                ...
                else
                {
                    ...
                    if(pss_playback_process_tbl(pwe_cb, profile, rec,
                        &add_q, &chg_q, 
#if (NCS_PSS_RED == 1)
                        &lcl_cont_ssn,
#endif
            ...
            {
               if((lcl_cont_ssn == FALSE) &&
                  (pwe_cb->curr_plbck_ssn_info.tbl_id == rec->tbl_id))
               {
...
*****************************************************************************/
uns32 pss_flush_mib_entries_from_curr_profile(PSS_PWE_CB *pwe_cb,
                                        PSS_SPCN_LIST *spcn_entry,
                                   ...
                                   PSS_CLIENT_ENTRY *node, char *p_pcn)
{
    ...
    m_LOG_PSS_WBREQ_I(NCSFL_SEV_INFO, PSS_WBREQ_I_PROCESS_SORTED_LIST_START,
         p_pcn, pwe_cb->pwe_id);
    ...
    while (sort_db->num_entries != 0)
    {
        ...
        {
            m_LOG_PSS_WBREQ_I(NCSFL_SEV_INFO, PSS_WBREQ_I_PROCESS_SORTED_LIST_END,
               p_pcn, pwe_cb->pwe_id);
        ...
        if(pwe_cb->p_pss_cb->mib_tbl_desc[entry->tbl_rec->tbl_id] != NULL) 
        {
           if(pwe_cb->p_pss_cb->mib_tbl_desc[entry->tbl_rec->tbl_id]->ptbl_info->table_of_scalars)
              ...
              retval = pss_oac_warmboot_process_tbl(pwe_cb, p_pcn, entry->tbl_rec, 
                            node,
#if (NCS_PSS_RED == 1)
                            &lcl_cont_ssn,
#endif
        ...
        {
           if((lcl_cont_ssn == FALSE) &&
              (pwe_cb->curr_plbck_ssn_info.tbl_id == entry->tbl_rec->tbl_id))
           {
              lcl_cont_ssn = TRUE; /* This is the table which was last completed in the previous playback session */
           }
        }
...
*****************************************************************************/
NCS_BOOL pss_data_available_for_table(PSS_PWE_CB *pwe_cb, char *p_pcn,
                                      PSS_TBL_REC *trec)
{
    ...
    if(in_buf == NULL)
    {
        m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_OCT_ALLOC_FAIL,
            "pss_data_available_for_table()");
        m_NCS_PSSTS_FILE_CLOSE(pwe_cb->p_pss_cb->pssts_api, pwe_cb->p_pss_cb->pssts_hdl,
            retval, file_hdl);
        return FALSE;
    }
    ...
    m_LOG_PSS_STORE(NCSFL_SEV_DEBUG,PSS_MIB_EXISTS_ON_PERSISTENT_STORE,
       pwe_cb->pwe_id, p_pcn, trec->tbl_id);
...
uns32 pss_handle_oaa_down_event(PSS_PWE_CB *pwe_cb, MDS_DEST *fr_card)
{
    ...
    ncs_logmsg(NCS_SERVICE_ID_PSS,  PSS_LID_NO_CB, PSS_FC_HDLN,
               ...
               NCSFL_TYPE_TIC, PSS_HDLN_OAC_DATA_DELETING, addr_str);
...
uns32 pss_process_bam_conf_done(MAB_MSG *msg)
{
    ...
    m_LOG_PSS_CONF_DONE(NCSFL_SEV_NOTICE, PSS_CONF_DONE_RCVD, 
        msg->data.data.bam_conf_done.pcn_list.pcn);
    pwe_cb = (PSS_PWE_CB*)msg->yr_hdl;
    if(pwe_cb == NULL)
    {
    ...
    if((spcn_entry = pss_findadd_entry_frm_spcnlist(pwe_cb->p_pss_cb, 
          (char*)msg->data.data.bam_conf_done.pcn_list.pcn, FALSE)) == NULL)
    {
...
*****************************************************************************/
PSS_OAA_ENTRY *pss_findadd_entry_in_oaa_tree(PSS_PWE_CB *pwe_cb, 
                                           MDS_DEST *mdest, NCS_BOOL add)
{
...
/*****************************************************************************
  ...
  RETURNS:           Pointer to PSS_OAA_CLT_ID
                     NULL, if not found or error encountered.
*****************************************************************************/
PSS_OAA_CLT_ID *pss_add_tbl_in_oaa_node(PSS_PWE_CB *pwe_cb, PSS_OAA_ENTRY *node, 
                                           PSS_TBL_REC *trec)
{
    PSS_OAA_CLT_ID     *oaa_clt_node = NULL, *prv_oaa_clt_node = NULL;
    ...
    {
        m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_OAA_CLT_ID_ALLOC_FAIL,
            "pss_add_tbl_in_oaa_node()");
...
/*****************************************************************************
  ...
  RETURNS:           Pointer to PSS_OAA_CLT_ID
                     NULL, if not found or error encountered.
*****************************************************************************/
PSS_OAA_CLT_ID *pss_find_tbl_in_oaa_node(PSS_PWE_CB *pwe_cb, PSS_OAA_ENTRY *node, 
                                           uns32 tbl_id)
{
...
#define m_RE_REMROW_REQUEST(pwe_cb) \
        { \
           ...
           m_NCS_OS_MEMCPY(pinst_re_ids, idx->i_inst_ids, tbl_info->num_inst_ids * sizeof(uns32)); \
                                                 ...
                                                 NCS_BOOL cur_file_exists)
{
    ...
    while ((pTree->n_nodes != 0) && (pNode != NULL) && (cur_rows_left > 0))
    {
        ...
        {
            cur_data_valid = TRUE;
            m_NCS_MEMCPY(cur_data, cur_ptr, tbl_info->max_row_length);
        ...
        else if (res == 0)
        {
            ...
            {
                cur_data_valid = TRUE;
                m_NCS_MEMCPY(cur_data, pData->data, tbl_info->max_row_length);
            }
            cur_ptr += tbl_info->max_row_length;
            cur_rows_left--;
        ...
        {
            idx->i_inst_len = pss_get_inst_ids_from_data(tbl_info,
                                                        (uns32 *) idx->i_inst_ids,
                                                        cur_data);
            if (idx->i_inst_len == 0)
                return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
            ...
            if(inst->mib_tbl_desc[tbl]->ptbl_info->status_field == 0)
            {
                ...
                if (*rra_inited == FALSE)
                {
                    ...
                    *rra_inited = TRUE;
                    ...
                    m_NCS_MEMCPY((uns32 *)first_idx->i_inst_ids, idx->i_inst_ids,
...
*****************************************************************************/
uns32 pss_playback_process_tbl(PSS_PWE_CB *pwe_cb, uns8 *profile,
                               ...
                               NCS_BOOL last_plbck_evt)
{
    ...
    if ((alt_file_exists == FALSE) &&
        ...
        ((pTree == NULL) ||
        (ncs_patricia_tree_size(pTree) == 0)))
    {
#if (NCS_PSS_RED == 1)
        if(pwe_cb->processing_pending_active_events == TRUE)
        {
    ...
    if (alt_file_exists == TRUE)
    {
        m_NCS_PSSTS_FILE_OPEN(pwe_cb->p_pss_cb->pssts_api, pwe_cb->p_pss_cb->pssts_hdl,
            retval, profile, pwe_cb->pwe_id, p_pcn, tbl, NCS_PSSTS_FILE_PERM_READ, alt_file_hdl);
        if (retval != NCSCC_RC_SUCCESS)
        {
            m_LOG_PSS_STORE(NCSFL_SEV_ERROR,PSS_MIB_OPEN_FAIL,pwe_cb->pwe_id,
                p_pcn,tbl);
            retval = m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
            goto cleanup;
        }
    ...
    while (((pNode != NULL) || (cur_rows_left > 0) || (cur_data_valid == TRUE)) &&
           (alt_rows_left > 0))
    {
            ...
            while(((pTree->n_nodes != 0) && (pNode != NULL)) || (cur_rows_left > 0))
            {
                while ((pTree->n_nodes != 0) && (pNode != NULL) && (cur_rows_left > 0))
                {
                    ...
                    if (res < 0)
                    {
                        ...
                        {
                            m_NCS_MEMCPY(cur_data, pData->data, tbl_info->max_row_length);
                            m_NCS_MEMCPY(cur_key, pData->key, tbl_info->max_key_length);
                            cur_data_valid = TRUE;
                            pss_delete_inst_node_from_tree(pTree, pNode);
    ...
    if((pwe_cb->p_pss_cb->mib_tbl_desc[tbl]->ptbl_info->status_field == 0) &&
       (rra_inited == TRUE))
    {
        m_REMROW_REQUEST(pwe_cb);
#if (NCS_PSS_RED == 1)
                                 ...
                                 NCS_QUEUE *add_queue, NCS_QUEUE *diff_queue)
{
       ...
       while (elem != NULL)
       {
          while (elem != NULL)
          {
            ...
            for (j = 0; j < tbl_info->ptbl_info->num_objects; j++)
            {
                ...
                case NCSMIB_FMAT_INT:
                    ...
                    case sizeof(uns8):
                        ...
                        break;
                    case sizeof(uns16):
                        m_NCS_OS_MEMCPY(&data16, elem->data + param_offset, sizeof(uns16));
                        pv.info.i_int = (uns32) data16;
                        break;
                    case sizeof(uns32):
                        m_NCS_OS_MEMCPY(&pv.info.i_int, elem->data + param_offset, sizeof(uns32));
                        break;
                    default:
                        return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
                    }
                ...
                case NCSMIB_FMAT_BOOL:
                    {
                        ...
                        pv.i_fmat_id = NCSMIB_FMAT_INT;
                        pv.i_param_id = tbl_info->pfields[j].var_info.param_id;
                        m_NCS_OS_MEMCPY(&data_bool, elem->data + param_offset, sizeof(NCS_BOOL));
                ...
                case NCSMIB_FMAT_OCT:
                    {
                        ...
                        {
                            m_NCS_MEMCPY(&pv.i_length, elem->data + param_offset, sizeof(uns16));
                ...
                if ((is_add == TRUE) &&
                    (tbl_info->ptbl_info->status_field == tbl_info->pfields[j].var_info.param_id))
                {
...
#endif
                {
                        ...
                        ((is_add == FALSE) && (diff_cnt == 1))) )
                   {
                      ...
                      m_LOG_PSS_LAST_UPDT_INFO(NCSFL_SEV_DEBUG, PSS_LAST_UPDT_IS_SET, " " /* Not utilized */, pwe_cb->pwe_id, tbl);
                   }
                   ...
                   /* if(pwe_cb->p_pss_cb->ha_state == SA_AMF_HA_ACTIVE) */
                   {
                      ...
                      pwe_cb->curr_plbck_ssn_info.tbl_id = tbl;
                      pwe_cb->curr_plbck_ssn_info.mib_idx.i_inst_len = idx.i_inst_len;
                      m_NCS_OS_MEMCPY(pinst_re_ids, inst_ids, tbl_info->num_inst_ids * sizeof(uns32));
                      pwe_cb->curr_plbck_ssn_info.mib_idx.i_inst_ids = pinst_re_ids;
                      pwe_cb->curr_plbck_ssn_info.mib_obj_id = pv.i_param_id;
                      m_PSS_RE_PLBCK_SSN_INFO(pwe_cb, &pwe_cb->curr_plbck_ssn_info);
                   }
...
#endif
            {
                ...
                {
                   mib_arg.i_policy |= NCSMIB_POLICY_PSS_LAST_PLBCK_TRIGGER;
                   m_LOG_PSS_LAST_UPDT_INFO(NCSFL_SEV_DEBUG, PSS_LAST_UPDT_IS_SETROW, " " /* Not utilized */, pwe_cb->pwe_id, tbl);
                }
                mib_arg.i_rsp_fnc = NULL;
                mib_arg.i_tbl_id = tbl;
                mib_arg.i_usr_key = pwe_cb->p_pss_cb->hm_hdl;
                ...
                else
                {
                    ...
                    if(mib_arg.req.info.setrow_req.i_usrbuf != NULL)
                        m_MMGR_FREE_BUFR_LIST(\
                            mib_arg.rsp.info.setrow_rsp.i_usrbuf);
                }
            }
    ...
    if((tbl_info->capability == NCSMIB_CAPABILITY_SETALLROWS) && (ra_inited == TRUE))
    {
       ...
       /* Now send the mib request */
       {
          ...
          {
             mib_arg.i_policy |= NCSMIB_POLICY_PSS_LAST_PLBCK_TRIGGER;
             m_LOG_PSS_LAST_UPDT_INFO(NCSFL_SEV_DEBUG, PSS_LAST_UPDT_IS_SETALLROWS, " " /* Not utilized */, pwe_cb->pwe_id, tbl);
          }
          mib_arg.i_rsp_fnc = NULL;
          mib_arg.i_tbl_id = tbl;
          mib_arg.i_usr_key = pwe_cb->p_pss_cb->hm_hdl;
...
*****************************************************************************/
uns32 pss_add_to_diff_q (NCS_QUEUE * diff_q, uns32 tbl,
                         char *p_pcn, uns8 * data, uns32 len)
{
    ...
    if (elem == NULL)
    {
        m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_PSS_QELEM_ALLOC_FAIL,
            "pss_add_to_diff_q()");
        return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
    }
    ...
    m_NCS_MEMCPY(elem->data, data, len);
...
*****************************************************************************/
uns32 pss_send_remrow_request(PSS_PWE_CB *pwe_cb, USRBUF * ub, 
...
*****************************************************************************/
uns32 pss_send_remove_request(PSS_PWE_CB *pwe_cb, 
...
uns32 pss_process_display_mib_entries(PSS_CB * inst, NCSMIB_ARG * arg)
{
   ...
   m_NCS_PSSTS_PROFILE_EXISTS(inst->pssts_api, inst->pssts_hdl, 
                                      retval, (char*)&profile_name, profile_exists);
   if((retval != NCSCC_RC_SUCCESS) || (profile_exists == FALSE))
   {
      m_LOG_PSS_HEADLINE(NCSFL_SEV_ERROR, PSS_HDLN_INVALID_PROFILE_SPCFD);
   ...
   for(i = 0; i < pwe_cnt; i++)
   {
      ...
      /* 3.0.a addition */
         snprintf(dest_file_path, sizeof(dest_file_path)-1, "%s%d/%s/%d/%s/%d_tbl_details",
                               NCS_PSS_DEF_PSSV_ROOT_PATH, PSS_PS_FORMAT_VERSION, profile_name, pwe_id, pcn_name, tbl_id);
         ...
         if(inst_file.info.file_exists.o_file_exists == TRUE)
         {
            ...
            m_NCS_MEMCPY(&ps_file_record.profile, &profile_name, sizeof(profile_name));
            ps_file_record.pwe_id = pwe_id;
            m_NCS_MEMCPY(&ps_file_record.pcn, &pcn_name, sizeof(pcn_name));
...
uns32 pss_dump_sclr_tbl(PSS_CB *inst, char *profile, uns16 pwe_id, char *pcn, 
                        uns32 tbl_id, NCSMIB_ARG *arg, FILE *fh)
{
 ...
 /* 3.0.b addition */
   ...
   /* Filling the current persistent format version, profile, pwe_id, pcn and table_id in ps_file_record */
   ps_file_record.ps_format_version = PSS_PS_FORMAT_VERSION;
   m_NCS_MEMCPY(&ps_file_record.profile, &profile, NCS_PSS_MAX_PROFILE_NAME);
   ps_file_record.pwe_id = pwe_id;
   m_NCS_MEMCPY(&ps_file_record.pcn, &pcn, NCSMIB_PCN_LENGTH_MAX);
   ps_file_record.tbl_id = tbl_id;
...
uns32 pss_dump_tbl(PSS_CB *inst, char *profile, uns16 pwe_id, char *pcn, 
                   uns32 tbl_id, NCSMIB_ARG *arg, FILE *fh)
{
 ...
 /* End of 3.0.b addition */
   ...
   m_NCS_PSSTS_FILE_SIZE(inst->pssts_api, inst->pssts_hdl, retval,
       profile, pwe_id, pcn, tbl_id, file_size);
   if(retval != NCSCC_RC_SUCCESS)
   {
...
uns32 pss_dup_re_wbreq_info(MAB_PSS_WARMBOOT_REQ *src, MAB_PSS_WARMBOOT_REQ *dst)
{
   ...
   *dst = *src;
   if(src->pcn_list.pcn != NULL)
   {
      ...
      for(p_tbl = src->pcn_list.tbl_list; p_tbl != NULL; 
          p_tbl = p_tbl->next, pp_tbl = &(*pp_tbl)->next)
      {
         if((*pp_tbl = m_MMGR_ALLOC_MAB_PSS_TBL_LIST) == NULL)
         {
...
uns32 pss_sync_mib_req(NCSMIB_ARG *marg, NCSMIB_REQ_FNC mib_fnc, uns32 sleep_dur, NCSMEM_AID *mma)
{
   ...
   if((lcl_ret == NCSCC_RC_NO_SUCH_TBL) ||
      (lcl_ret == NCSCC_RC_NO_MAS))
   {
...
*****************************************************************************/
void pss_send_eop_status_to_oaa(MAB_MSG *mm, uns32 status, uns32 mib_key,
                                uns32 wbreq_hdl, NCS_BOOL send_all)
{
   ...
   else
   {
      ...
      for(wbreq = &mm->data.data.oac_pss_warmboot_req; wbreq != NULL;
        wbreq = wbreq->next)
      {
