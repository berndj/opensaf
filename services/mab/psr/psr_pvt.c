           ...
           if(pwe_cb->p_pss_cb->ha_state == SA_AMF_HA_STANDBY)
           {
              ...
              if(NCSCC_RC_SUCCESS != status)
              {
                 ...
                 ncs_logmsg(NCS_SERVICE_ID_PSS,  PSS_LID_HDLN_C, PSS_FC_HDLN,
                            ...
                            NCSFL_TYPE_TIC, PSS_HDLN_STDBY_OAA_DOWN_LIST_DELETE_FAILED, addr_str);
              }
              else
              {
                 if (m_NCS_NODE_ID_FROM_MDS_DEST(msg->fr_card) == 0)
                 ...
                 ncs_logmsg(NCS_SERVICE_ID_PSS,  PSS_LID_HDLN_C, PSS_FC_HDLN,
                            NCSFL_LC_HEADLINE, NCSFL_SEV_NOTICE,
                            NCSFL_TYPE_TIC, PSS_HDLN_STDBY_OAA_DOWN_LIST_DELETE_SUCCESS, addr_str);
              }
          }
...
uns32 pss_process_snmp_request(MAB_MSG * msg)
{
        ...
        (inst->mib_tbl_desc[tbl_id] == NULL))
    {
        ...
        if ((tbl_id == NCSMIB_TBL_PSR_PROFILES) ||
            (tbl_id == NCSMIB_SCLR_PSR_TRIGGER))
            return NCSCC_RC_FAILURE;
...
#endif
    {
           ...
           wbreq = wbreq->next)
       {
          ...
          if (client_node == NULL)
          {
             ...
             pss_send_eop_status_to_oaa(msg, NCSCC_RC_FAILURE, wbreq->mib_key, wbreq->wbreq_hdl, FALSE);
             continue;
          }
...
#endif
             /* Send EOP to all requests that are queued here. */
             pss_send_eop_status_to_oaa(msg, NCSCC_RC_FAILURE, wbreq->mib_key, wbreq->wbreq_hdl, TRUE);
             return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
          }
          if(wbreq->pcn_list.tbl_list == NULL)
          {
             ...
             if(spcn_entry != NULL)
             {
                ...
                {
                    m_LOG_PSS_RCVD_WBREQ_EVT(NCSFL_SEV_DEBUG, PSS_WBREQ_RCVD_PLBCK_OPTION_SET_TO_BAM, 
                       wbreq->pcn_list.pcn, pwe_cb->pwe_id,  
                       wbreq->is_system_client, 0);
                    pss_flush_mib_entries_from_curr_profile(pwe_cb, spcn_entry, client_node);
                ...
                {
                    m_LOG_PSS_RCVD_WBREQ_EVT(NCSFL_SEV_DEBUG, PSS_WBREQ_RCVD_PLBCK_OPTION_SET_TO_PSS, 
                       wbreq->pcn_list.pcn, pwe_cb->pwe_id,  
                       wbreq->is_system_client, 0);
                    add_entry = TRUE;
                }
             ...
             if(add_entry)
             {
                retval = pss_sort_wbreq_instore_tables_with_rank(pwe_cb, 
                   &lcl_sort_db, client_node, spcn_entry, &snd_evt_to_bam, &lcl_uba);
          ...
          else
          {
             if((snd_evt_to_bam == FALSE) || (spcn_entry == NULL))
             {
                ...
                retval = pss_wb_playback_for_sorted_list(pwe_cb, &lcl_sort_db, client_node, p_pcn);
                /* Send EOP only to this particular request. */
                pss_send_eop_status_to_oaa(msg, retval, wbreq->mib_key, wbreq->wbreq_hdl, FALSE);
...
uns32 pss_process_set_request(MAB_MSG * msg)
{
    ...
    if(p_pcn == NULL)
    {
        ...
        if (m_NCS_NODE_ID_FROM_MDS_DEST(msg->fr_card) == 0)
           sprintf(addr_str, "From VDEST:%d,For Tbl:%d",m_MDS_GET_VDEST_ID_FROM_MDS_DEST(msg->fr_card),tbl_id);
        else
    ...
    if(tbl_rec->is_scalar)
    {
        ...
        if(pss_find_scalar_node(pwe_cb, tbl_rec, p_pcn, 
                    tbl_id, TRUE, &created_new) != NCSCC_RC_SUCCESS)
         return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
        ...
        {
            retval = pss_save_to_sclr_store(pwe_cb, tbl_rec, 
                NULL, p_pcn, tbl_id);
            if (retval != NCSCC_RC_SUCCESS)
                return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
        }
        else
        {
            ...
            {
               retval = pss_save_to_sclr_store(pwe_cb, tbl_rec, NULL, 
                  p_pcn, tbl_id);
               if (retval != NCSCC_RC_SUCCESS)
                  return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
            }
    ...
    else
    {
        ...
        {
           m_LOG_PSS_SNMP_REQ(NCSFL_SEV_ERROR, PSS_SNMP_REQ_ZERO_INST_LEN_OR_NULL_INST_IDS, pwe_cb->pwe_id, arg->i_tbl_id);
           return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
        }
        tbl_info = pwe_cb->p_pss_cb->mib_tbl_desc[tbl_id];
        ...
        if(retval != NCSCC_RC_SUCCESS)
        {
           m_LOG_PSS_SNMP_REQ(NCSFL_SEV_ERROR, PSS_SNMP_REQ_PARAMVAL_VALIDATE_FAIL, pwe_cb->pwe_id, arg->i_tbl_id);
           return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
        }
        ...
        retval = pss_apply_changes_to_node(tbl_info, tbl_data,
            &arg->rsp.info.set_rsp.i_param_val);
        if (retval != NCSCC_RC_SUCCESS)
            return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
...
uns32 pss_process_setrow_request(MAB_MSG * msg, NCS_BOOL is_move_row)
{
    ...
    /* Get the patricia node containing the data for this inst ids */
    tbl_info = pwe_cb->p_pss_cb->mib_tbl_desc[tbl_id];
    if((tbl_info->ptbl_info->table_of_scalars == FALSE) && ((arg->i_idx.i_inst_len == 0) || (arg->i_idx.i_inst_ids == NULL)))
    {
       m_LOG_PSS_SNMP_REQ(NCSFL_SEV_ERROR, PSS_SNMP_REQ_ZERO_INST_LEN_OR_NULL_INST_IDS, pwe_cb->pwe_id, arg->i_tbl_id);
       return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
    }
    ...
    tbl_data = (PSS_MIB_TBL_DATA *)pNode;
    {
        ...
        while((pa.cnt > 0) && (pa.len > 0))
        {
            ...
            {
                m_LOG_PSS_SNMP_REQ(NCSFL_SEV_ERROR, PSS_SNMP_REQ_PARAMVAL_VALIDATE_FAIL, pwe_cb->pwe_id, arg->i_tbl_id);
                PSS_FREE_ERROR_COND_FOR_SETROW
                return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
            }
...
uns32 pss_process_setallrows_request(MAB_MSG * msg)
{
    ...
    if((tbl_info->ptbl_info->table_of_scalars == TRUE) || (arg->i_idx.i_inst_len == 0) || (arg->i_idx.i_inst_ids == NULL))
    {
       m_LOG_PSS_SNMP_REQ(NCSFL_SEV_ERROR, PSS_SNMP_REQ_ZERO_INST_LEN_OR_NULL_INST_IDS, pwe_cb->pwe_id, arg->i_tbl_id);
       return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
    }
    ...
    for (i = 0; i < num_rows; i++)
    {
        ...
        {
           m_LOG_PSS_SNMP_REQ(NCSFL_SEV_ERROR, PSS_SNMP_REQ_INDEX_VALIDATE_FAIL, pwe_cb->pwe_id, arg->i_tbl_id);
           PSS_FREE_ERROR_COND_FOR_SETALLROWS
           return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
        }
...
uns32 pss_process_removerows_request(MAB_MSG * msg)
{
    ...
    for (i = 0; i < num_rows; i++)
    {
        ...
        /* Get the patricia node containing the data for this inst ids */
        pNode = pss_find_inst_node(pwe_cb, pTree, p_pcn, tbl_id,
            idx.i_inst_len, (uns32 *)idx.i_inst_ids, TRUE);
...
*****************************************************************************/
PSS_CLIENT_ENTRY * pss_find_client_entry(PSS_PWE_CB *pwe_cb, char *p_pcn, 
                                         NCS_BOOL create)
{
    ...
    {
        m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_CLIENT_ENTRY_ALLOC_FAIL,
            "pss_find_client_entry()");
        return (PSS_CLIENT_ENTRY *)m_MAB_DBG_SINK(NULL);
    }
...
PSS_TBL_REC * pss_find_table_tree(PSS_PWE_CB *pwe_cb, PSS_CLIENT_ENTRY *client_node,
                                  PSS_OAA_ENTRY *oaa_node, uns32 tbl_id, NCS_BOOL create,
                                  PSS_RET_VAL *o_prc)
{
    ...
    if(tbl_rec == NULL)
    {
        m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_TBL_REC_ALLOC_FAIL,
            "pss_find_table_tree()");
        return (PSS_TBL_REC *) m_MAB_DBG_SINK(NULL);
    }

    m_NCS_MEMSET(tbl_rec, '\0', sizeof(PSS_TBL_REC));
    tbl_rec->dirty = FALSE;
    ...
    {
        tbl_rec->info.other.params.key_size = 
            pwe_cb->p_pss_cb->mib_tbl_desc[tbl_id]->max_key_length;
        retval = ncs_patricia_tree_init(&tbl_rec->info.other.data, 
            &tbl_rec->info.other.params);
        if(retval != NCSCC_RC_SUCCESS)
        {
...
void pss_client_entry_init(PSS_CLIENT_ENTRY *client_node)
{
    ...
    m_NCS_MEMSET(client_node, '\0', sizeof(PSS_CLIENT_ENTRY));
                                      ...
                                      NCS_BOOL create)
{
    ...
    m_NCS_MEMSET((uns8*)&pkey, '\0', sizeof(pkey));
    ...
    if (pNode == NULL)
    {
        m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_MIB_TBL_DATA_ALLOC_FAIL,
            "pss_find_inst_node()");
        return (NCS_PATRICIA_NODE *)m_MAB_DBG_SINK(NULL);
    }

    m_NCS_MEMSET(tbl_data, '\0', sizeof(PSS_MIB_TBL_DATA));
    ...
    m_NCS_MEMSET(tbl_data->data, 0, tbl_info->max_row_length);
    ...
    retval = pss_read_from_store(pwe_cb, profile_name, tbl_data->key,
                                 p_pcn, tbl_id, pNode, &entry_found);
                           ...
                           NCS_BOOL create, NCS_BOOL *created_new)
{
    ...
    *created_new = FALSE;
    ...
    if(tbl_rec->info.scalar.data == NULL)
    {
       ...
       if(tbl_rec->info.scalar.data == NULL)
       {
          m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_SCLR_MIB_ROW_ALLOC_FAIL,
...
*****************************************************************************/
uns32 pss_apply_changes_to_node(PSS_MIB_TBL_INFO * tbl_info,
                                ...
                                NCSMIB_PARAM_VAL * param)
{
    ...
    case NCSMIB_FMAT_OCT:
        if(var_info->var_info.obj_type == NCSMIB_OCT_OBJ_TYPE)
        {
           if(var_info->var_info.obj_spec.stream_spec.max_len < param->i_length)
           {
              m_LOG_PSS_HEADLINE2(NCSFL_SEV_ERROR, PSS_HDLN2_SCLR_SET_OCT_OBJECT_LENGTH_MISMATCH_WITH_STREAM_SPEC_MAX_LEN);
              return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
           }
           max_len = var_info->var_info.obj_spec.stream_spec.max_len;
        }
        else  /* Discrete octet strings */
        {
           ...
           if(max_len < param->i_length)
           {
              m_LOG_PSS_HEADLINE2(NCSFL_SEV_ERROR, PSS_HDLN2_SCLR_SET_OCT_OBJECT_LENGTH_MISMATCH_WITH_DISCRETE_OCT_STR_MAX_LEN);
              return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
           }
        }
        if (var_info->var_length == TRUE)
...
*****************************************************************************/
uns32 pss_apply_inst_ids_to_node(PSS_MIB_TBL_INFO * tbl_info,
                                 PSS_MIB_TBL_DATA * tbl_data,
                                 uns32 num_ids, uns32 * inst_ids)
{
    ...
    for (i = 0; i < tbl_info->ptbl_info->num_objects; i++)
    {
        ...
        if (var_info->var_info.is_index_id == TRUE)
        {
            ...
            pss_set_bit_for_param(tbl_data->data, (tbl_info->ptbl_info->objects_local_to_tbl[var_info->var_info.param_id]+1));
        }
    }
                          ...
                          NCS_BOOL * entry_found)
{
  ...
  /* End of 3.0.b addition */
    ...
    m_NCS_MEMSET(pkey, '\0', tbl_info->max_key_length);
    if (pkey == NULL)
    {
        m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_OCT_ALLOC_FAIL,
            "pss_read_from_store()");
                          ...
                          uns32 tbl_id, NCS_BOOL * entry_found)
{
   ...
   /* End of 3.0.b addition */
           ...
           m_NCS_PSSTS_FILE_OPEN(pwe_cb->p_pss_cb->pssts_api, pwe_cb->p_pss_cb->pssts_hdl,
                       retval, profile_name, pwe_cb->pwe_id, p_pcn, tbl_id, NCS_PSSTS_FILE_PERM_READ, file_hdl);
           if (retval != NCSCC_RC_SUCCESS)
           {
               m_LOG_PSS_STORE(NCSFL_SEV_ERROR,PSS_MIB_OPEN_FAIL,
                   pwe_cb->pwe_id,p_pcn,tbl_id);
...
*****************************************************************************/
uns32 pss_get_inst_ids_from_data(PSS_MIB_TBL_INFO * tbl_info,
                                uns32 * pinst_ids,
                                uns8 * buffer)
{
    PSS_VAR_INFO * var_info;
...
*****************************************************************************/
uns32 pss_delete_inst_node_from_tree(NCS_PATRICIA_TREE * pTree,
                                     NCS_PATRICIA_NODE * pNode)
{
    uns32 retval;
    PSS_MIB_TBL_DATA * tbl_data = (PSS_MIB_TBL_DATA *)pNode;
...
*****************************************************************************/
uns32 pss_oac_warmboot_process_tbl(PSS_PWE_CB *pwe_cb, char *p_pcn,
                                   PSS_TBL_REC *tbl_rec,
                                   ...
                                   NCS_BOOL is_end_of_playback)
{
    ...
    m_NCS_MEMSET(pinst_ids, '\0', tbl_info->num_inst_ids * sizeof(uns32));
    m_NCS_MEMSET(pinst_ids_first, '\0', tbl_info->num_inst_ids * sizeof(uns32));
#if (NCS_PSS_RED == 1)
        ...
        for (i = 0; i < num_items; i++)
        {
            ...
            for (j = 0; j < tbl_info->ptbl_info->num_objects; j++)
            {
...
#endif
                {
                    ...
                    if(is_end_of_playback && is_last_chunk && (i == (num_items - 1)) && (lcl_param_cnt == 1))
                    {
                        /* This is the last event of the last MIB row */
                        mib_arg.i_policy |= NCSMIB_POLICY_PSS_LAST_PLBCK_TRIGGER;
                        m_LOG_PSS_LAST_UPDT_INFO(NCSFL_SEV_DEBUG, PSS_LAST_UPDT_IS_SET, p_pcn, pwe_cb->pwe_id, tbl_rec->tbl_id);
                    }
...
#endif
            {
                ...
                {
                    mib_arg.i_policy |= NCSMIB_POLICY_PSS_LAST_PLBCK_TRIGGER;
                    m_LOG_PSS_LAST_UPDT_INFO(NCSFL_SEV_DEBUG, PSS_LAST_UPDT_IS_SETROW, p_pcn, pwe_cb->pwe_id, tbl_rec->tbl_id);
                }
                mib_arg.i_rsp_fnc = NULL;
                mib_arg.i_tbl_id = tbl_rec->tbl_id;
                mib_arg.i_xch_id = pwe_cb->p_pss_cb->xch_id++;
    ...
    if(inst->mib_tbl_desc[tbl_rec->tbl_id]->capability == NCSMIB_CAPABILITY_SETALLROWS)
    {
       ...
       {
          mib_arg.i_policy |= NCSMIB_POLICY_PSS_LAST_PLBCK_TRIGGER;
          m_LOG_PSS_LAST_UPDT_INFO(NCSFL_SEV_DEBUG, PSS_LAST_UPDT_IS_SETALLROWS, p_pcn, pwe_cb->pwe_id, tbl_rec->tbl_id);
       }
       mib_arg.i_tbl_id = tbl_rec->tbl_id;
...
*****************************************************************************/
uns32 pss_oac_warmboot_process_sclr_tbl(PSS_PWE_CB *pwe_cb, char *pcn,
                                        PSS_TBL_REC *tbl_rec, 
                                        PSS_CLIENT_ENTRY *node,
#if (NCS_PSS_RED == 1)
                                        NCS_BOOL *i_continue_session,
                                        ...
                                        NCS_BOOL is_end_of_playback)
{
    ...
    if((pwe_cb->processing_pending_active_events == TRUE) &&
       ((*i_continue_session) == FALSE) && 
       (pwe_cb->curr_plbck_ssn_info.tbl_id == 0))
    {
       (*i_continue_session) = TRUE; /* This is the first table in the playback session */
    }
    if((*i_continue_session) == TRUE)
    ...
    {
        m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_OCT_ALLOC_FAIL,
            "pss_oac_warmboot_process_sclr_tbl()");
        retval = m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
        goto cleanup;
    }
    ...
    for (j = 0; j < tbl_info->ptbl_info->num_objects; j++)
    {
...
#endif
        {
            ...
            if(is_end_of_playback && (lcl_params_max == 1))
            {
                ...
                mib_arg.i_policy |= NCSMIB_POLICY_PSS_LAST_PLBCK_TRIGGER;
                m_LOG_PSS_LAST_UPDT_INFO(NCSFL_SEV_DEBUG, PSS_LAST_UPDT_IS_SET, pcn, pwe_cb->pwe_id, tbl_rec->tbl_id);
            }
            mib_arg.i_rsp_fnc = NULL;
            mib_arg.i_tbl_id = tbl_rec->tbl_id;
            mib_arg.i_usr_key = pwe_cb->hm_hdl;
...
*****************************************************************************/
uns32 pss_save_to_store(PSS_PWE_CB *pwe_cb, NCS_PATRICIA_TREE * pTree,
                        uns8 *profile_name, char *p_pcn, uns32 tbl_id)
{
    ...
    m_NCS_MEMSET(out_buf, '\0', buf_size);
    out_ptr = out_buf;
    ...
    m_NCS_MEMSET(key, '\0', tbl_info->max_key_length);
...
*****************************************************************************/
uns32 pss_save_to_sclr_store(PSS_PWE_CB *pwe_cb, PSS_TBL_REC * tbl_rec,
                        uns8 * profile_name, char *p_pcn, uns32 tbl_id)
{
    PSS_CB            *inst = pwe_cb->p_pss_cb;
    PSS_MIB_TBL_INFO * tbl_info;
...
*****************************************************************************/
uns32 pss_set_key_from_inst_ids(PSS_MIB_TBL_INFO * tbl_info, uns8 * key,
                                uns32 num_inst_ids, uns32 * inst_ids)
{
    uns32  i, j, counter;
    uns16  len16;
    ...
    counter = 0;
    m_NCS_MEMSET(key, 0, tbl_info->max_key_length);
    /*m_NCS_MEMSET(key, '\0', NCS_PATRICIA_MAX_KEY_SIZE);*/
    for (i = 0; i < tbl_info->ptbl_info->num_objects; i++)
    {
        var_info = &(tbl_info->pfields[i]);
        if (var_info->var_info.is_index_id == FALSE)
        ...
        case NCSMIB_FMAT_INT:
            {
                ...
                case sizeof(uns32):
                    ...
                    break;
                default:
                    m_LOG_PSS_HEADLINE2(NCSFL_SEV_ERROR, PSS_HDLN2_SET_KEY_FROM_INST_IDS_FMAT_INT_NOT_SUPPORTED);
                    return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
                }
                counter++;
            }
...
*****************************************************************************/
uns32 pss_get_key_from_data(PSS_MIB_TBL_INFO * tbl_info,
                            uns8 * pkey, uns8 * buffer)
{
    ...
    m_NCS_MEMSET(ptr, 0, tbl_info->max_key_length);
    for (i = 0; i < tbl_info->ptbl_info->num_objects; i++)
    {
        var_info = &(tbl_info->pfields[i]);
        if (var_info->var_info.is_index_id == FALSE)
...
*****************************************************************************/
void pss_get_profile_from_inst_ids(uns8 * profile, uns32 num_inst_ids,
...
*****************************************************************************/
uns32 pss_playback_process_sclr_tbl(PSS_PWE_CB *pwe_cb, uns8 *profile,
                                    PSS_TBL_REC * tbl_rec,
#if (NCS_PSS_RED == 1)
                                    NCS_BOOL *i_continue_session,
#endif
                                    NCS_BOOL last_plbck_evt)
{
    ...
    {
        m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_OCT_ALLOC_FAIL,
            "pss_playback_process_sclr_tbl()");
        retval = m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
        goto cleanup;
    }
    m_NCS_MEMSET(alt_data, '\0', tbl_info->max_row_length);

    /* Now open the files and read in data from them */
    m_NCS_PSSTS_FILE_OPEN(pwe_cb->p_pss_cb->pssts_api, pwe_cb->p_pss_cb->pssts_hdl,
        retval, profile, pwe_cb->pwe_id, p_pcn, tbl_rec->tbl_id, NCS_PSSTS_FILE_PERM_READ, alt_file_hdl);
    if (retval != NCSCC_RC_SUCCESS)
    {
        retval = m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
        goto cleanup;
...
uns32 pss_add_to_refresh_tbl_list(PSS_PWE_CB *pwe_cb, PSS_TBL_REC *tbl_rec)
{
   ...
   if(pwe_cb->refresh_tbl_list != NULL)
   {
      ...
      m_NCS_MEMSET(prv_tmp->next, '\0', sizeof(PSS_RFRSH_TBL_LIST));
   ...
   m_NCS_MEMSET(tmp, '\0', sizeof(PSS_RFRSH_TBL_LIST));
 ...
 * Notes:
 *****************************************************************************/
uns32 pss_validate_set_mib_obj(PSS_MIB_TBL_INFO* tbl_obj,
                               NCSMIB_PARAM_VAL* param)
{
    uns32 index = 0;
 ...
 * Notes:
 *****************************************************************************/
uns32 pss_validate_param_val(NCSMIB_VAR_INFO* var_info,
                             NCSMIB_PARAM_VAL* param)
{
 ...
 * Notes:       .
 *****************************************************************************/
uns32 pss_validate_index(PSS_MIB_TBL_INFO* tbl_obj,
                         NCSMIB_IDX *idx)
{
    uns16  i, index;
...
uns32 pss_stdby_oaa_down_list_update(MDS_DEST oaa_addr,NCSCONTEXT yr_hdl,PSS_STDBY_OAA_DOWN_BUFFER_OP buffer_op)
{
    ...
    if(PSS_STDBY_OAA_DOWN_BUFFER_ADD == buffer_op)
    {
        ...
        m_NCS_MEMSET(new_node, '\0', sizeof(PSS_STDBY_OAA_DOWN_BUFFER_NODE));
