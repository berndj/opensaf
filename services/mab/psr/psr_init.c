...
uns32 pss_lib_init()
{
   ...
   if(rc != NCSCC_RC_SUCCESS) 
   {
      ...
      m_LOG_PSS_ERROR_NO_DATA(NCSFL_SEV_CRITICAL, 
                 NCSFL_LC_SVC_PRVDR, PSS_ERR_IPC_CREATE_FAILED); 
   ...
   if(m_NCS_IPC_ATTACH(&gl_pss_amf_attribs.mbx_details.pss_mbx) != NCSCC_RC_SUCCESS) 
   {
      /* log the error */ 
      m_LOG_PSS_ERROR_NO_DATA(NCSFL_SEV_CRITICAL, 
                   NCSFL_LC_SVC_PRVDR, PSS_ERR_IPC_ATTACH_FAILED); 
   ...
   if ((ncs_app_signal_install(SIGUSR1, pss_amf_sigusr1_handler)) == -1) 
   {
      /* log the error */ 
      m_LOG_PSS_ERROR_NO_DATA(NCSFL_SEV_CRITICAL, 
           NCSFL_LC_FUNC_RET_FAIL, PSS_ERR_SIG_INSTALL_FAILED); 
   ...
   if(rc != NCSCC_RC_SUCCESS)
   {
      /* log the error */ 
      m_LOG_PSS_ERROR_I(NCSFL_SEV_ERROR, 
            PSS_ERR_DEF_ENV_CREATE_FAILED, rc);
 ...
 /* End of 3.0.a addition */
   ...
   if(m_NCS_TASK_CREATE((NCS_OS_CB)pss_mbx_amf_process,
      &gl_pss_amf_attribs.mbx_details.pss_mbx,
      NCS_MAB_PSR_TASKNAME, /* "PSS" */
      NCS_MAB_PSR_PRIORITY, 
      NCS_MAB_PSR_STACKSIZE,/* 8192 */
      &gl_pss_amf_attribs.mbx_details.pss_task_hdl) != NCSCC_RC_SUCCESS) 
   {
      /* log the error */ 
      m_LOG_PSS_ERROR_NO_DATA(NCSFL_SEV_CRITICAL, 
            NCSFL_LC_SVC_PRVDR, PSS_ERR_TASK_CREATE_FAILED); 
   ...
   if(m_NCS_TASK_START(gl_pss_amf_attribs.mbx_details.pss_task_hdl) != NCSCC_RC_SUCCESS) 
   {
      /* log the error */ 
      m_LOG_PSS_ERROR_NO_DATA(NCSFL_SEV_CRITICAL, 
            NCSFL_LC_SVC_PRVDR, PSS_ERR_TASK_START_FAILED); 
...
uns32 pss_ts_create(NCSPSS_CREATE *pwe, uns32 ps_format_version)
{
   ...
   if (rc != NCSCC_RC_SUCCESS)
   {
      /* log the failure */ 
      m_LOG_PSS_ERROR_I(NCSFL_SEV_ERROR, 
            PSS_ERR_TGT_SVSC_LM_CREATE_FAILED, rc);
      return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
   }
   ...
   m_LOG_PSS_HDLN_I(NCSFL_SEV_INFO,
               PSS_HDLN_TGT_SVCS_CREATE_SUCCESS, pwe->i_pssts_hdl); 
...
uns32 pss_ts_destroy(uns32 pss_ts_hdl)
{
   ...
   {
      /* log the error */ 
      m_LOG_PSS_ERROR_I(NCSFL_SEV_CRITICAL, 
            PSS_ERR_TGT_SVCS_DESTROY_FAILED, rc);
      return m_MAB_DBG_SINK(rc);
   }

   /* mark the success */
   m_LOG_PSS_HDLN_I(NCSFL_SEV_INFO,
               PSS_HDLN_TGT_SVCS_DESTROY_SUCCESS, pss_ts_hdl); 
...
uns32 pss_create(NCSPSS_CREATE *pwe)
{
   ...
   if (rc != NCSCC_RC_SUCCESS) 
   {
      /* log the failure */ 
      m_LOG_PSS_ERROR_I(NCSFL_SEV_CRITICAL, 
            PSS_ERR_VDEST_CREATE_FAILED, rc);
      return m_LEAP_DBG_SINK(rc);
   }
   ...
   if (pss_def_csi == NULL)
   {
        ...
        m_LOG_PSS_ERROR_II(NCSFL_LC_FUNC_RET_FAIL, NCSFL_SEV_CRITICAL, 
                           ...
                           m_PSS_DEFAULT_ENV_ID, lcl_ha_role); 
   ...
   m_LOG_PSS_HDLN_II(NCSFL_SEV_INFO, PSS_HDLN_DEF_ENV_INIT_SUCCESS, 
                     m_PSS_DEFAULT_ENV_ID, lcl_ha_role); 
