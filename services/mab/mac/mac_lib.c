...
static uns32
maclib_mac_create_ipc_task(); 

/* instantiate MAA in a particular PWE */ 
static uns32 
maclib_mac_instantiate(NCS_LIB_REQ_INFO  *req_info); 

/* un-instantiate MAA in a particular PWE */ 
static uns32    
maclib_mac_uninstantiate(PW_ENV_ID  env_id, 
                         SaNameT    i_inst_name,
                         uns32      i_mac_handle, 
                         NCS_BOOL   i_spir_cleanup); 

/* to add to the global list of instances */
static uns32 
mac_inst_list_add(PW_ENV_ID env_id, uns32 i_mac_hdl, SaNameT i_inst_name); 

/* to delete and release a particular MAA instance from the list */ 
static void 
mac_inst_list_release(PW_ENV_ID env_id); 

/* callback to inform the creator about a failure in MAA */
static uns32 
mac_evt_cb(MAB_LM_EVT* evt);
...
maclib_mac_create(NCS_LIB_REQ_INFO * req_info)
{
    ...
    if (status != NCSCC_RC_SUCCESS)
    {
        ...
        m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR, 
                        MAB_MAC_ERR_SPLR_REG_FAILED, status, 0);
        /* unbind with DTS */
        mab_log_unbind(); 
        return status; 
    }
...
maclib_mac_create_ipc_task()
{
    ...
    status = m_NCS_TASK_CREATE ((NCS_OS_CB)mac_do_evts,
        ...
        NCS_MAB_MAC_PRIORITY,
        NCS_MAB_MAC_STACKSIZE,
        &gl_mac_mbx.mac_mbx_hdl); 
    if (status != NCSCC_RC_SUCCESS)
    {
        /* log the failure */ 
        m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, 
...
maclib_mac_instantiate(NCS_LIB_REQ_INFO  *req_info)
{
    ...
    m_NCS_OS_MEMCPY(&arg.info.create.i_inst_name, 
                         ...
                         NCS_BOOL   i_spir_cleanup)
{
    ...
    if (status != NCSCC_RC_SUCCESS)
    {
        /* log the failure */ 
        m_LOG_MAB_ERROR_II(NCSFL_LC_FUNC_RET_FAIL, NCSFL_SEV_ERROR, 
                        MAB_OAC_ERR_DESTROY_FAILED, status, env_id);
        return status; 
    }
    ...
    if (i_spir_cleanup == TRUE)
    {
        ...
        m_NCS_OS_MEMCPY(&spir_info.i_instance_name, &i_inst_name, sizeof(SaNameT));
...
maclib_mac_destroy(NCS_LIB_REQ_INFO *req_info)
{
    ...
    if (status != NCSCC_RC_SUCCESS)
    {
        /* log the failure */ 
        m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR, 
                        MAB_MAC_ERR_SPLR_DEREG_FAILED, status, 0);
        return status; 
    }
