...
..............................................................................
  ...
  $Header: /ncs/software/release/UltraLinq/MAB/MAB1.0/base/products/mab/mac/mac_api.c 31    3/28/01 2:12p Questk $ 
...
MABMAC_API uns32 ncsmac_mib_request(NCSMIB_ARG* req)
{
   ...
   */
  {
    ...
    m_MAC_UNLK(&inst->lock, NCS_LOCK_READ);
    m_LOG_MAB_LOCK(MAB_LK_MAC_UNLOCKED,&inst->lock);
    code = mab_mds_snd(tx_mds_hdl, &msg, 
                       NCSMDS_SVC_ID_MAC, NCSMDS_SVC_ID_MAS, inst->mas_vcard);
...
uns32 mac_svc_create(NCSMAC_CREATE* create)
{
    ...
    inst->hm_hdl    = ncshm_create_hdl(inst->hm_poolid, NCS_SERVICE_ID_MAB,
                                   (NCSCONTEXT)inst);
    ...
    if (status != NCSCC_RC_SUCCESS)
    {
        /* log the failure */ 
        m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR, 
                           MAB_MAC_ERR_MDS_INSTALL_FAILED, status,
                           create->i_vrid); 
        m_MAC_UNLK(&inst->lock, NCS_LOCK_WRITE);
        /* free the allocated MAA CB */ 
        m_MMGR_FREE_MAC_INST(inst); 
...
uns32 mac_svc_destroy(NCSMAC_DESTROY * destroy)
{
    ...
    if (status != NCSCC_RC_SUCCESS)
    {
        ...
        /* log the failure */ 
        m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR, 
                           MAB_MAC_ERR_MDS_UNINSTALL_FAILED, status, 
                           destroy->i_env_id); 
        m_MAB_DBG_VOID;
    }
