...
static uns32 oac_add_node_to_wbreq_hdl_list(OAC_TBL *inst,
                                     uns32      wbreq_hdl,
                                     NCSOAC_EOP_USR_IND_FNC func);
static void oac_del_node_from_wbreq_hdl_list(OAC_TBL *inst,
                                     uns32      wbreq_hdl);
...
uns32 ncsoac_ss( NCSOAC_SS_ARG* arg )
{
  ...
  case NCSOAC_SS_OP_TBL_OWNED:
    return oac_ss_tbl_reg(&arg->info.tbl_owned, arg->i_tbl_id, 
                arg->i_oac_hdl);

  case NCSOAC_SS_OP_ROW_OWNED:
    return oac_ss_row_reg(&arg->info.row_owned, arg->i_tbl_id, 
        arg->i_oac_hdl);

  case NCSOAC_SS_OP_ROW_GONE:
    return oac_ss_row_unreg(&arg->info.row_gone, arg->i_tbl_id, 
        arg->i_oac_hdl);

  case NCSOAC_SS_OP_TBL_GONE:
    return oac_ss_tbl_unreg(&arg->info.tbl_gone, arg->i_tbl_id, 
        arg->i_oac_hdl);
...
uns32 oac_ss_row_move(NCSOAC_ROW_MOVE*  row_move, uns32 oac_hdl)
  ...
  if ((se = ncsstack_push( &row_move->i_move_req->stack, 
    ...
    (uns8)sizeof(NCS_SE_MIB_ORIG))) == NULL) 
    {
...
uns32 oac_ss_push_mibarg_data_to_pssv(NCSOAC_SS_ARG *arg)
{
  ...
  if(((tbl_rec = oac_tbl_rec_find(inst, arg->i_tbl_id)) == NULL) ||
     ...
     (arg->info.push_mibarg_data.arg->i_tbl_id != arg->i_tbl_id))
  {
     m_OAC_UNLK(&inst->lock);
     ncshm_give_hdl(oac_hdl);
     return m_MAB_DBG_SINK(NCSCC_RC_FAILURE); 
...
uns32 oac_ss_warmboot_req_to_pssv(NCSOAC_SS_ARG*  arg)
  ...
  m_LOG_OAA_WARMBOOTREQ_INFO_I(NCSFL_SEV_NOTICE, MAB_OAA_WARMREQ_INFO_I_PSS_MAS_STATUS, inst->psr_here, inst->mas_here);
  {
     ...
     if(tmp != NULL) 
     {
        while(tmp != NULL)
        {
           m_LOG_OAA_WARMBOOTREQ_INFO_II(NCSFL_SEV_NOTICE, MAB_OAA_WARMREQ_INFO_II_RCVD_REQ, arg->info.warmboot_req.i_pcn, \
  ...
  if(arg->info.warmboot_req.i_tbl_list == NULL)
  {
     ...
     if((inst->psr_here == FALSE) || (inst->mas_here == FALSE) || (inst->is_active == FALSE))
     {
        ...
        code = oac_add_warmboot_req_in_wbreq_list(inst, &arg->info.warmboot_req, 
                      &p_wbreq_node);
        if(code != NCSCC_RC_SUCCESS)
        {
            m_LOG_MAB_HEADLINE(NCSFL_SEV_ERROR, MAB_HDLN_OAC_ADD_WBREQ_TO_PEND_LIST_FAIL);
            m_LOG_MAB_HEADLINE(NCSFL_SEV_ERROR, MAB_HDLN_OAC_TERMINATING_WARMBOOT_REQ_API_PROCESSING);
            ncshm_give_hdl(oac_hdl);
            m_OAC_UNLK(&inst->lock);
            return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
        }
      ...
      if((inst->psr_here == FALSE) || (inst->mas_here == FALSE) || (inst->is_active == FALSE))
      {
         ...
         while(l_ptr != NULL)
         {
            ...
            if(tbl_rec  != NULL)
            {
               ...
               { 
                  m_LOG_OAA_PCN_INFO_I(NCSFL_SEV_ERROR, MAB_OAA_PCN_INFO_TBL_IN_WARMBOOT_REQ_NOT_PERSISTENT, l_ptr->tbl_id);
                  m_LOG_MAB_HEADLINE(NCSFL_SEV_ERROR, MAB_HDLN_OAC_TERMINATING_WARMBOOT_REQ_API_PROCESSING);
                  ncshm_give_hdl(oac_hdl);
                  m_OAC_UNLK(&inst->lock);
                  return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
               ...
               if(p_pcn == NULL)
               {
                  m_LOG_OAA_PCN_INFO_I(NCSFL_SEV_ERROR, MAB_OAA_PCN_INFO_WARMBOOT_REQ_TBL_NOT_FND_IN_PCN_LIST, l_ptr->tbl_id);
                  m_LOG_MAB_HEADLINE(NCSFL_SEV_ERROR, MAB_HDLN_OAC_TERMINATING_WARMBOOT_REQ_API_PROCESSING);
                  ncshm_give_hdl(oac_hdl);
                  m_OAC_UNLK(&inst->lock);
                  return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
  ...
  if(send_req == TRUE)
  {
     ...
     msg.data.data.oac_pss_warmboot_req = wbreq;
     {
        ...
        if(tmp != NULL) 
        {
           while(tmp != NULL)
           {
              m_LOG_OAA_WARMBOOTREQ_INFO_III(NCSFL_SEV_NOTICE, MAB_OAA_WARMREQ_INFO_III_REQ_MDS_EVT_TO_PSS_COMPOSED, wbreq.pcn_list.pcn, wbreq.is_system_client, tmp->tbl_id, wbreq.wbreq_hdl);
              tmp = tmp->next;
           }
        }
        else
        {
           m_LOG_OAA_WARMBOOTREQ_INFO_III(NCSFL_SEV_NOTICE, MAB_OAA_WARMREQ_INFO_III_REQ_MDS_EVT_TO_PSS_COMPOSED, wbreq.pcn_list.pcn, wbreq.is_system_client, 0, 0);
        }
     }
     ...
     if(inst->is_active == TRUE)
     {
        ...
        if (ret_val != NCSCC_RC_SUCCESS)
        {
           ...
           m_LOG_MAB_SVC_PRVDR(NCSFL_SEV_ERROR, MAB_SP_OAC_MDS_SND_MSG_FAILED, 
               NCSMDS_SVC_ID_OAC, code);
           m_LOG_MAB_HDLN_I(NCSFL_SEV_ERROR,MAB_HDLN_OAC_WARMBOOT_REQ_TO_PSS_MDS_SEND_FAIL, msg.data.seq_num);
           m_OAC_UNLK(&inst->lock);
           ncshm_give_hdl(oac_hdl);
           return NCSCC_RC_SUCCESS;
        }
...
*****************************************************************************/
OAA_PCN_LIST *oac_findadd_pcn_in_list(OAC_TBL* inst, char *pcn, 
                              NCS_BOOL is_system_client, uns32 tbl_id,
                              NCS_BOOL add)
{
   ...
   {
       m_LOG_MAB_MEMFAIL(NCSFL_SEV_CRITICAL, MAB_MF_OAC_PCN_LIST_ALLOC_FAIL,
  ...
  *o_wbreq_node = NULL;
  ...
  while(list != NULL)
  {
       ...
       (m_NCS_MEMCMP(wbr->i_pcn, list->pcn, lcl_len) == 0))
    {
       ...
       NCSOAC_PSS_TBL_LIST *oac_tbl_elem = NULL,
                           *prv_oac_tbl_elem = NULL, *tmp_elem = NULL;
                                     ...
                                     NCSOAC_EOP_USR_IND_FNC func)
{
   ...
   for(list = inst->wbreq_hdl_list;
       ((list != NULL) && (list->wbreq_hdl != wbreq_hdl));
  ...
  if(req_head != NULL)
  {
     ...
     for(req = req_head; req != NULL; req = req->next)
     {
        ...
        if(tmp != NULL)
        {
           while(tmp != NULL)
           {
              m_LOG_OAA_WARMBOOTREQ_INFO_III(NCSFL_SEV_NOTICE, MAB_OAA_WARMREQ_INFO_III_REQ_MDS_EVT_TO_PSS_COMPOSED, req->pcn_list.pcn, req->is_system_client, tmp->tbl_id, req->wbreq_hdl);
              tmp = tmp->next;
           }
        }
        else
        {
           m_LOG_OAA_WARMBOOTREQ_INFO_III(NCSFL_SEV_NOTICE, MAB_OAA_WARMREQ_INFO_III_REQ_MDS_EVT_TO_PSS_COMPOSED, req->pcn_list.pcn, req->is_system_client, 0, 0);
        }
     }
 
     msg.data.seq_num = inst->seq_num;
     ++ inst->seq_num;
     retval = mab_mds_snd(inst->mds_hdl, &msg, NCSMDS_SVC_ID_OAC, 
               NCSMDS_SVC_ID_PSS, inst->psr_vcard);
     if(retval != NCSCC_RC_SUCCESS)
     {
         m_LOG_MAB_SVC_PRVDR(NCSFL_SEV_ERROR, MAB_SP_OAC_MDS_SND_MSG_FAILED, 
             NCSMDS_SVC_ID_OAC, retval);
         m_LOG_MAB_HDLN_I(NCSFL_SEV_ERROR,MAB_HDLN_OAC_WARMBOOT_REQ_TO_PSS_MDS_SEND_FAIL, msg.data.seq_num);
         oac_free_wbreq(req_head);
         return m_MAB_DBG_SINK(NCSCC_RC_FAILURE); 
     }
    ...
    if(in_wbreq->i_pcn != NULL)
    {
      ...
      while(tlist != NULL)
      {
        ...
        if((tbl_rec = oac_tbl_rec_find(inst, tlist->tbl_id)) != NULL)
        {
          if(!tbl_rec->is_persistent)
          {
             m_LOG_OAA_PCN_INFO_II(NCSFL_SEV_ERROR, MAB_OAA_PCN_INFO_WARMBOOT_REQ_FOR_NON_PERSISTENT_TABLE_RCVD, in_wbreq->i_pcn, tlist->tbl_id);
             is_wrong_tbl = TRUE;
          }
        }
        else
        {
          m_LOG_OAA_PCN_INFO_II(NCSFL_SEV_ERROR, MAB_OAA_PCN_INFO_WARMBOOT_REQ_FOR_NON_BOUND_TABLE_RCVD, in_wbreq->i_pcn, tlist->tbl_id);
          is_wrong_tbl = TRUE;
        }
        if(is_wrong_tbl)
        {
      ...
      if((in_wbreq->i_tbl_list != NULL) &&
         (invalid_tbl_cnt == total_tbl_cnt))
      {
         m_LOG_MAB_HEADLINE(NCSFL_SEV_ERROR, MAB_HDLN_OAC_WARMBOOT_REQ_REJECTED_DUE_TO_INVALID_TBLS_SPCFD);
         return NCSCC_RC_FAILURE; 
      }
      req->pcn_list.pcn = 
...
*****************************************************************************/
uns32 oac_ss_tbl_reg(NCSOAC_TBL_OWNED* tbl_owned,uns32 tbl_id,
                     uns32 oac_hdl)
  {
  OAC_TBL*     inst;
  OAC_TBL_REC* tbl_rec;
  ...
  if(tbl_owned->is_persistent == TRUE)
  {
    ...
    {
      OAA_PCN_LIST *p_pcn = NULL;
      p_pcn = oac_findadd_pcn_in_list(inst, tbl_owned->i_pcn,
                 FALSE, /* Don't bother about this */
                 tbl_id, TRUE);
      if(p_pcn == NULL)
      {
      ...
      {
        m_LOG_OAA_PCN_INFO_I(NCSFL_SEV_INFO, MAB_OAA_PCN_INFO_PSS_NOT_PRESENT_DURING_TBL_REG, tbl_id);
      }
    }
  }
...
uns32 oac_psr_send_bind_req(OAC_TBL *inst, MAB_PSS_TBL_BIND_EVT *bind_req, NCS_BOOL free_head)
{
   ...
   if (code != NCSCC_RC_SUCCESS)
   {
       ...
       while(list != NULL)
       {
          m_LOG_OAA_PCN_INFO_II(NCSFL_SEV_ERROR, MAB_OAA_PCN_INFO_BIND_REQ_TO_PSS_MDS_SND_FAIL, bind_req->pcn_list.pcn, list->tbl_id);
          list = list->next;
       }
...
/* #if (MAB_LOG == 1) */
   {
       ...
       while(list != NULL)
       {
          m_LOG_OAA_PCN_INFO_II(NCSFL_SEV_NOTICE, MAB_OAA_PCN_INFO_BIND_REQ_TO_PSS_MDS_SND_SUCCESS, bind_req->pcn_list.pcn, list->tbl_id);
...
*****************************************************************************/
uns32 oac_ss_tbl_unreg(NCSOAC_TBL_GONE* tbl_gone,uns32 tbl_id,
                       uns32 oac_hdl)
  {
  OAC_TBL*     inst;
  OAC_TBL_REC* tbl_rec;
...
void oac_del_pcn_from_list(OAC_TBL *inst, uns32 tbl_id)
{
   ...
   for(tmp = inst->pcn_list; tmp != NULL; prv_tmp = tmp, tmp = tmp->next)
   {
      ...
      {
        m_LOG_OAA_PCN_INFO_I(NCSFL_SEV_INFO, MAB_OAA_PCN_INFO_TBL_FND_IN_PCN_LIST_DURING_UNBIND_OP, tbl_id);
  ...
  if(tbl_rec->fltr_list != NULL)
    ...
    if((fltr->fltr.type != row_owned->i_fltr.type) && (row_owned->i_fltr.type != NCSMAB_FLTR_DEFAULT))
      ...
      return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);       
           ...
           ((row_owned->i_fltr.type != NCSMAB_FLTR_RANGE)&&
            (row_owned->i_fltr.type != NCSMAB_FLTR_EXACT)))
    {
    MAB_LM_EVT          mle;
    MAB_LM_FLTR_TYPE_MM ftm;
    ftm.i_xst_type  = fltr->fltr.type;
      ...
      for(fltr = tbl_rec->fltr_list;fltr != NULL;fltr = fltr->next)
        ...
        if(fltr->fltr.is_move_row_fltr == TRUE)
          ...
          if((row_owned->i_fltr.fltr.range.i_bgn_idx == fltr->fltr.fltr.range.i_bgn_idx) &&
             (row_owned->i_fltr.fltr.range.i_idx_len == fltr->fltr.fltr.range.i_idx_len) &&
             (memcmp(row_owned->i_fltr.fltr.range.i_min_idx_fltr,
...
uns32 oac_svc_create(NCSOAC_CREATE* create)
{
    ...
    if (status != NCSCC_RC_SUCCESS)
    {
        /* log the error */ 
        m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR, 
                           MAB_OAC_ERR_MDS_INSTALL_FAILED, status,
                           create->i_vrid); 
        /* free the allocated OAA instance */ 
        m_OAC_UNLK(&inst->lock);
        m_MMGR_FREE_OAC_TBL(inst);
    ...
    else
    {
       ...
       if (status != NCSCC_RC_SUCCESS)
       {
          /* log the failure */
          m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR, 
                           MAB_OAC_ERR_PWE_QUERY_FAILED, status,
                           create->i_vrid); 
          /* free the allocated OAA instance */ 
          m_OAC_UNLK(&inst->lock);
          m_MMGR_FREE_OAC_TBL(inst);
    ...
    if (status != NCSCC_RC_SUCCESS)
    {
        /* log the error */ 
        m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR, 
                           MAB_OAC_ERR_MDS_SUBSCRB_FAILED, status,
                           create->i_vrid); 
        /* free the allocated OAA instance */ 
        m_OAC_UNLK(&inst->lock);
        m_MMGR_FREE_OAC_TBL(inst);
    ...
    if (status != NCSCC_RC_SUCCESS)
    {
        /* log the error */
        m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR,
                           MAB_OAC_ERR_MDS_OAA_SUBSCRB_FAILED, status,
                           create->i_vrid);
        /* free the allocated OAA instance */
        m_OAC_UNLK(&inst->lock);
        m_MMGR_FREE_OAC_TBL(inst);
...
uns32 oac_svc_destroy(NCSOAC_DESTROY *destroy)
{
    ...
    if (status != NCSCC_RC_SUCCESS)
    {
        /* log the failure */ 
        m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR, 
                           MAB_OAC_ERR_MDS_UNINSTALL_FAILED, status, 
                           destroy->i_env_id); 
        m_MAB_DBG_VOID;
...
uns32 oac_svc_get(NCSOAC_GET* get)
{
  ...
  switch(get->i_obj_id)
  {
  case NCSOAC_OBJID_LOG_VAL    : *pval = inst->log_bits; break;
  case NCSOAC_OBJID_LOG_ON_OFF : *pval = inst->log_enbl; break;
  case NCSOAC_OBJID_SUBSYS_ON  : *pval = inst->oac_enbl; break;
...
uns32 oac_svc_set(NCSOAC_SET* set)
{
  ...
  switch(set->i_obj_id)
  {
  case NCSOAC_OBJID_LOG_VAL    : inst->log_bits = set->i_obj_val; break;
  case NCSOAC_OBJID_LOG_ON_OFF : inst->log_enbl = set->i_obj_val; break;
  case NCSOAC_OBJID_SUBSYS_ON  : inst->oac_enbl = set->i_obj_val; break;
