 ...
 *                 Scope from Configuration file in /etc/opt/opensaf/ncs_ifsv_drv_conf.
 *
...
NCS_IFSV_SUBSCR_SCOPE  ifsv_drv_scope_update(uns8 *name)
{
   ...
   fp = fopen("/etc/opt/opensaf/ncs_ifsv_drv_conf","r");
   if (fp == NULL)
   {
      m_NCS_CONS_PRINTF("\n /etc/opt/opensaf/ncs_ifsv_drv_conf file failed to open!!DoesntExist ..\n");
      return 0;
   }
