...
uns32 mbcsv_process_events (MBCSV_EVT   *rcvd_evt, uns32 mbcsv_hdl)
{
   ...
   if (NULL == (mbc_reg = (MBCSV_REG *)m_MBCSV_TAKE_HANDLE(mbcsv_hdl)))
   {
      return m_MBCSV_DBG_SINK(SA_AIS_ERR_BAD_HANDLE, 
         "mbcsv_process_events: Bad handle received.");
   }
         ...
         case NCSMDS_RED_UP:
            {
               ...
               if (NULL != (ckpt = (CKPT_INST*)ncs_patricia_tree_get(&mbc_reg->ckpt_ssn_list, 
                  (const uns8 *)&rcvd_evt->rcvr_peer_key.pwe_hdl)))
               {
                  mbcsv_send_peer_disc_msg(MBCSV_PEER_UP_MSG,
                                           mbc_reg, ckpt, peer, 
                                           MDS_SENDTYPE_RED, 
   ...
   case MBCSV_EVT_INTERNAL:
      {
         ...
         case MBCSV_EVT_INTERNAL_CLIENT:
            {
               ...
               if (NULL != (peer = (PEER_INST*)m_MBCSV_TAKE_HANDLE((uns32)rcvd_evt->rcvr_peer_key.peer_inst_hdl)))
               {
                  hdl_to_give = (uns32)rcvd_evt->rcvr_peer_key.peer_inst_hdl;
                  ckpt = peer->my_ckpt_inst;

                  m_LOG_MBCSV_FSM_EVT(ckpt->my_role, mbc_reg->svc_id, ckpt->pwe_hdl,
                     peer->peer_anchor, peer->state, (MBCSV_FSM_EV_DBG_NONE + 
                     rcvd_evt->info.peer_msg.info.client_msg.msg_sub_type));

                  m_NCS_MBCSV_FSM_DISPATCH(peer, 
                     rcvd_evt->info.peer_msg.info.client_msg.msg_sub_type, rcvd_evt);
         ...
         case MBCSV_EVT_MBC_ASYNC_SEND:
            {
               if (NULL == (ckpt = 
                  (CKPT_INST *) m_MBCSV_TAKE_HANDLE(rcvd_evt->info.peer_msg.info.usr_msg_info.i_ckpt_hdl)))
               {
                  m_MBCSV_DBG_SINK_SVC(SA_AIS_ERR_BAD_HANDLE, 
                     "NCS_MBCSV_OP_SEND_CKPT: CKPT instance does exist.", 0);
                  goto pr_done;
               }
