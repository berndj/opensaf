...
uns32 mbcsv_mds_reg (uns32 pwe_hdl, uns32 svc_hdl, MBCSV_ANCHOR * anchor, MDS_DEST *vdest)
{
   ...
   if (NCSCC_RC_SUCCESS != mbcsv_query_mds(pwe_hdl, anchor, vdest))
   {
      ...
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
         "mbcsv_mds_reg: Invalid handle passed.");
   }
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 mbcsv_mds_send_msg(uns32            send_type,
                         MBCSV_EVT       *msg,
                         CKPT_INST       *ckpt,
                         MBCSV_ANCHOR    anchor)
{
   ...
   default:
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
         "mbcsv_mds_send_msg : Send type not supported by MBCSv");
   }
   ...
   {
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
         "mbcsv_mds_send_msg : MDS send failed");
   }
}
...
uns32 mbcsv_mds_callback(NCSMDS_CALLBACK_INFO * cbinfo)
{
    ...
    case MDS_CALLBACK_SVC_EVENT:
        status = mbcsv_mds_evt(cbinfo->info.svc_evt,
                    cbinfo->i_yr_svc_hdl);
        break;
...
uns32 mbcsv_mds_rcv(NCSMDS_CALLBACK_INFO * cbinfo)
{
   ...
   if (NULL == msg)
       return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE,
       "mbcsv_mds_rcv: NULL message received.");
                              ...
                              msg->rcvr_peer_key.svc_id)))
   {
      ...
      if (NCSCC_RC_SUCCESS != m_MBCSV_SND_MSG(&mbx, msg, send_pri))
      {
         ...
         return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE,
            "mbcsv_mds_rcv: Message Send failed.");
      }
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 mbcsv_mds_evt(MDS_CALLBACK_SVC_EVENT_INFO svc_info,
                    MDS_CLIENT_HDL yr_svc_hdl)
{
   ...
   {
      m_LOG_MBCSV_SVC_PRVDR(svc_info.svc_pwe_hdl, 
         svc_info.i_anc, MBCSV_SP_MDS_RCV_EVT_UP);
   ...
   {
      m_LOG_MBCSV_SVC_PRVDR(svc_info.svc_pwe_hdl, 
         svc_info.i_anc, MBCSV_SP_MDS_RCV_EVT_DN);
                    ...
                    MDS_CLIENT_MSG_FORMAT_VER *msg_fmt_ver)
{
   ...
   if(uba == NULL)
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
             "mbcsv_mds_enc: User buff is NULL");
   ...
   {
     char str[200];
     sprintf(str,"Peer MDS Subpart version:%d not supported, message to svc-id:%d dropped",rem_svc_pvt_ver,to_svc);
   ...
   *msg_fmt_ver = msg_fmt_version;
   ...
   if(mm == NULL)
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
             "mbcsv_mds_enc: Message to be encoded is NULL");
         ...
         case MBCSV_PEER_UP_MSG:
            {
               ...
               mbcsv_encode_version(uba,
                  mm->info.peer_msg.info.peer_disc.info.peer_up.peer_version);
         ...
         case MBCSV_PEER_INFO_RSP_MSG:
            {
               ...
               mbcsv_encode_version(uba,
                  mm->info.peer_msg.info.peer_disc.info.peer_info_rsp.peer_version);
   ...
   default:
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
            "mbcsv_mds_enc: Incorrect message type.");
  }
                    ...
                    MDS_CLIENT_MSG_FORMAT_VER msg_fmat_ver)
{
   ...
   {
     char str[200];
     sprintf(str,"Msg format version:%d not supported, message from svc-id:%d dropped",msg_fmat_ver,to_svc);
     return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE,str);
   }

   if(uba == NULL)
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
      "mbcsv_mds_dec : User buffer is NULL");

   if(msg == NULL)
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
      "mbcsv_mds_dec : Message is NULL");
   ...
   *msg = mm;
         ...
         case MBCSV_PEER_INFO_MSG:
            {
               ...
               mm->info.peer_msg.info.peer_disc.info.peer_info.my_peer_inst_hdl = ncs_decode_32bit(&data);
               mm->info.peer_msg.info.peer_disc.peer_role= ncs_decode_32bit(&data);
               mm->info.peer_msg.info.peer_disc.info.peer_info.compatible= ncs_decode_8bit(&data);
         ...
         case MBCSV_PEER_INFO_RSP_MSG:
            {
               ...
               mm->rcvr_peer_key.peer_inst_hdl = ncs_decode_32bit(&data);
               mm->info.peer_msg.info.peer_disc.info.peer_info_rsp.my_peer_inst_hdl = ncs_decode_32bit(&data);
               mm->info.peer_msg.info.peer_disc.peer_role = ncs_decode_32bit(&data);
               mm->info.peer_msg.info.peer_disc.info.peer_info_rsp.compatible = ncs_decode_8bit(&data);
   ...
   default:
      m_MMGR_FREE_MBCSV_EVT(mm);
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE,
                  "mbcsv_mds_dec: Incorrect message type");
  }
                  ...
                  MDS_CLIENT_MSG_FORMAT_VER* msg_fmt_ver)
{
   ...
   if(msg == NULL)
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
      "mbcsv_mds_cpy: msg received is null");
...
uns32 mbcsv_encode_version(NCS_UBAID* uba, uns16 version)
{
   ...
   if(uba == NULL)
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
             "mbcsv_encode_version: User buff is NULL");

   data = ncs_enc_reserve_space(uba, MBCSV_MSG_VER_SIZE);

   if(data == NULL)
    return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
    "mbcsv_encode_version: ncs_enc_reserve_space returns NULL");
...
uns32 mbcsv_decode_version(NCS_UBAID* uba, uns16 *version)
{
   ...
   if(uba == NULL)
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
      "mbcsv_decode_version : User buffer is NULL");

   data = ncs_dec_flatten_space(uba,data_buff, MBCSV_MSG_VER_SIZE);

   if(data == NULL)
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
      "mbcsv_decode_version :ncs_dec_flatten_space returns NULL");
