...
uns32 mbcsv_add_new_mbx(uns32 pwe_hdl, SS_SVC_ID svc_id, SYSF_MBX mbx)
{
   ...
   if (NULL != ncs_patricia_tree_get(&mbcsv_cb.mbx_list, (const uns8 *)&key))
   {
      ...
      "Unable to add new mailbox entry for service", svc_id);
      goto done;
   }
   ...
   if (NULL == (new_entry = m_MMGR_ALLOC_MBX_INFO))
   {
      ...
      "mbcsv_add_new_mbx: Memory allocation failed");
      goto done;
   }
   ...
   if (NCSCC_RC_SUCCESS != ncs_patricia_tree_add(&mbcsv_cb.mbx_list, 
      (NCS_PATRICIA_NODE *)new_entry))
   {
      m_MMGR_FREE_MBX_INFO(new_entry);
      rc = m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
      "mbcsv_add_new_mbx: Failed to add new mbx in tree");
      goto done;
   }
...
uns32 mbcsv_rmv_entry(uns32 pwe_hdl, SS_SVC_ID svc_id)
{
   ...
   if (NULL == (tree_entry = (MBCSV_MBX_INFO *)ncs_patricia_tree_get(&mbcsv_cb.mbx_list,
                                          (const uns8 *)&key)))
   {
      rc = m_MBCSV_DBG_SINK_SVC(NCSCC_RC_FAILURE, 
...
uns32 mbcsv_destroy_mbx_list(void)
{
   ...
   while (NULL != (tree_entry = 
      (MBCSV_MBX_INFO *)ncs_patricia_tree_get(&mbcsv_cb.mbx_list,
                                          (const uns8 *)&key)))
   {
      key = tree_entry->key;
