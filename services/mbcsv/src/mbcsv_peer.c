...
PEER_INST *mbcsv_search_and_return_peer(PEER_INST* peer_list, MBCSV_ANCHOR anchor)
{
   ...
   for (peer = peer_list; peer != NULL; peer = peer->next)
      if (peer->peer_anchor == anchor) return peer;
...
PEER_INST *mbcsv_add_new_peer(CKPT_INST *ckpt, MBCSV_ANCHOR anchor)
{
   ...
   {
      m_MBCSV_DBG_SINK(NULL, 
         "mbcsv_add_new_peer: Mem alloc failed.");
      return NULL;
   }
...
void mbcsv_clear_multiple_active_state(CKPT_INST *ckpt)
{
   ...
   if (NULL != (peer = mbcsv_my_active_peer(ckpt)))
   {
      ...
      m_NCS_MBCSV_FSM_DISPATCH(peer, 
            NCSMBCSV_EVENT_MULTIPLE_ACTIVE, &rcvd_evt);
   ...
   while (peer != NULL)
   {
      ...
      else
      {
         if (peer->cold_sync_done)
            m_NCS_MBCSV_FSM_DISPATCH(peer, 
            NCSMBCSV_EVENT_STATE_TO_KEEP_STBY_SYNC, &rcvd_evt);
         else
            m_NCS_MBCSV_FSM_DISPATCH(peer, 
            NCSMBCSV_EVENT_STATE_TO_WAIT_FOR_CW_SYNC, &rcvd_evt);
      }
...
void mbcsv_set_up_new_session(CKPT_INST *ckpt, PEER_INST *new_act_peer)
{
   ...
   else if (new_act_peer->cold_sync_done)
   {
      ...
      m_MBCSV_SEND_CLIENT_MSG(new_act_peer, 
         NCSMBCSV_SEND_WARM_SYNC_REQ, NCS_MBCSV_ACT_DONT_CARE);
   ...
   else 
   {
      ...
      m_MBCSV_SEND_CLIENT_MSG(new_act_peer, 
         NCSMBCSV_SEND_COLD_SYNC_REQ, NCS_MBCSV_ACT_DONT_CARE);
...
void mbcsv_set_peer_state(CKPT_INST *ckpt, PEER_INST *peer, NCS_BOOL peer_up)
{
   ...
   case SA_AMF_ACTIVE: /* ckpt->my_role */
      {
         ...
         case SA_AMF_ACTIVE: /*: peer->peer_role */
            {
               ...
               m_NCS_MBCSV_FSM_DISPATCH(peer, 
                   NCSMBCSV_EVENT_MULTIPLE_ACTIVE, NULL);
            }
            break;
         ...
         default:/* peer->peer_role */
            ...
            "mbcsv_set_peer_state: Invalid PEER state.");
            return;
         }
      }
      break;
   ...
   default:
      m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE, 
            "mbcsv_set_peer_state: Invalid HA state.");
      return;
...
uns32 mbcsv_process_peer_up_info(MBCSV_EVT *msg, CKPT_INST *ckpt, uns8 peer_up)
{
   ...
   if (NULL != (peer = mbcsv_search_and_return_peer(ckpt->peer_list, 
    ...
    * compatible or not.
    */
   m_MBCSV_PEER_INFO(peer, ckpt->client_hdl, 
      ckpt->my_mbcsv_inst->svc_id, peer->version, 
      peer->incompatible);
...
uns32 mbcsv_process_peer_info_rsp(MBCSV_EVT *msg, CKPT_INST *ckpt)
{
   ...
   peer->peer_hdl =
      msg->info.peer_msg.info.peer_disc.info.peer_info_rsp.my_peer_inst_hdl;
   peer->incompatible =
                               ...
                               PEER_INST  *peer, uns32 mds_send_type, MBCSV_ANCHOR anchor)
{
   ...
   default:
      return m_MBCSV_DBG_SINK(NCSCC_RC_FAILURE,
         "Incorrect message type received in peer disco message.");
   } 
