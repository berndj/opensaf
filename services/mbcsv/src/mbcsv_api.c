...
#if 0
/**************************************************************************\
* PROCEDURE: mbcsv_process_sub_osh_request
*
* Purpose:  Process MBCSv subscribe one shot request. 
*           Add this message subscription request to the subscription list.
*
*
* Input:    arg -  is of type NCS_MBCSV_ARG.
*
...
uns32 mbcsv_process_sub_osh_request(NCS_MBCSV_ARG* arg)
{
   ...
   {
      return m_MBCSV_DBG_SINK_SVC(SA_AIS_ERR_BAD_HANDLE, 
         "NCS_MBCSV_OP_SUB_ONESHOT: Bad handle received with this API", 0);
   }
    ...
    * error.
    */
   if (NULL == (ckpt_inst = (CKPT_INST *)m_MBCSV_TAKE_HANDLE(arg->info.sub_oneshot.i_ckpt_hdl)))
   {
      rc = m_MBCSV_DBG_SINK_SVC(SA_AIS_ERR_BAD_HANDLE, 
         "NCS_MBCSV_OP_SUB_ONESHOT: CKPT instance does exist.", mbc_reg->svc_id);
      goto err1;
   }

   m_LOG_MBCSV_API(ckpt_inst->my_role, mbc_reg->svc_id, 
      ckpt_inst->pwe_hdl, MBCSV_API_OP_SUB_ONESH);

   if (NULL == (arg->info.sub_oneshot.o_sub_hdl 
                     = mbcsv_subscribe_oneshot(&arg->info.sub_oneshot.i_fltr,
                                               arg->info.sub_oneshot.i_time_10ms)))
   {
      rc = m_MBCSV_DBG_SINK_SVC(SA_AIS_ERR_FAILED_OPERATION, 
         "NCS_MBCSV_OP_SUB_ONESHOT: Failed to subscribe", 0);
   }

   m_MBCSV_GIVE_HANDLE(arg->info.sub_oneshot.i_ckpt_hdl);
...
/**************************************************************************\
* PROCEDURE: mbcsv_process_sub_per_request
*
* Purpose:  Process MBCSv subscribe persistant request. 
*           Add this message subscription request to the subscription list.
*
*
* Input:    arg -  is of type NCS_MBCSV_ARG.
...
uns32 mbcsv_process_sub_per_request(NCS_MBCSV_ARG* arg)
{
   ...
   {
      return m_MBCSV_DBG_SINK_SVC(SA_AIS_ERR_BAD_HANDLE, 
         "NCS_MBCSV_OP_SUB_PERSIST: Bad handle received with this API", 0);
   }
    ...
    * error.
    */
   if (NULL == (ckpt_inst = (CKPT_INST *) m_MBCSV_TAKE_HANDLE(arg->info.sub_persist.i_ckpt_hdl)))
   {
      rc = m_MBCSV_DBG_SINK_SVC(SA_AIS_ERR_BAD_HANDLE, 
         "NCS_MBCSV_OP_SUB_PERSIST: CKPT instance does exist.", 0);
      goto err1;
   }

   m_LOG_MBCSV_API(ckpt_inst->my_role, mbc_reg->svc_id, 
      ckpt_inst->pwe_hdl, MBCSV_API_OP_SUB_ONEPE);

   if (NULL == (arg->info.sub_oneshot.o_sub_hdl 
                     = mbcsv_subscribe_persist(&arg->info.sub_persist.i_fltr)))
   {
      rc = m_MBCSV_DBG_SINK_SVC(SA_AIS_ERR_FAILED_OPERATION, 
         "NCS_MBCSV_OP_SUB_PERSIST: Failed to subscribe", 0);
   }

   m_MBCSV_GIVE_HANDLE(arg->info.sub_persist.i_ckpt_hdl);
...
/**************************************************************************\
* PROCEDURE: mbcsv_process_sub_cancel_request
*
* Purpose:  Process MBCSv subscribe cancel request. 
*           Cancel message subscription. Remove this message subscription 
...
uns32 mbcsv_process_sub_cancel_request(NCS_MBCSV_ARG* arg)
{
   ...
   {
      return m_MBCSV_DBG_SINK_SVC(SA_AIS_ERR_BAD_HANDLE, 
         "NCS_MBCSV_OP_SUB_CANCEL: Bad handle received with this API", 0);
   }
    ...
    * error.
    */
   if (NULL == (ckpt_inst = (CKPT_INST *) m_MBCSV_TAKE_HANDLE(arg->info.sub_cancel.i_ckpt_hdl)))
   {
      rc = m_MBCSV_DBG_SINK_SVC(SA_AIS_ERR_BAD_HANDLE, 
         "NCS_MBCSV_OP_SUB_CANCEL: CKPT instance does exist.", 0);
      goto err1;
   }

   m_LOG_MBCSV_API(ckpt_inst->my_role, mbc_reg->svc_id, 
      ckpt_inst->pwe_hdl, MBCSV_API_OP_SUB_CANCEL);

   if (NULL == (arg->info.sub_oneshot.o_sub_hdl 
                     = mbcsv_subscribe_cancel(arg->info.sub_cancel.i_sub_hdl)))
   {
      rc = m_MBCSV_DBG_SINK_SVC(SA_AIS_ERR_FAILED_OPERATION, 
         "NCS_MBCSV_OP_SUB_CANCEL: Failed to cancel subscription", 0);
   }

   m_MBCSV_GIVE_HANDLE(arg->info.sub_cancel.i_ckpt_hdl);
