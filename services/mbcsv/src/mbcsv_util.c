...
uns32 mbcsv_rmv_reg_inst(MBCSV_REG *reg_list, MBCSV_REG *mbc_reg)
{
                          ...
                          (const uns8 *)&pwe_hdl)))
   {
      ...
      if (NCSCC_RC_SUCCESS != mbcsv_remove_ckpt_inst(ckpt))
      {
         rc = m_MBCSV_DBG_SINK_SVC(NCSCC_RC_FAILURE,
           "Failed to remove this ckpt instance",
           mbc_reg->svc_id);
         goto func_return;
      }
   }
    ...
    */
   if (NCSCC_RC_SUCCESS != ncs_patricia_tree_del((NCS_PATRICIA_TREE *)reg_list, 
...
uns32 mbcsv_remove_ckpt_inst(CKPT_INST *ckpt)
{
    ...
    */
   if (NCSCC_RC_SUCCESS != mbcsv_rmv_entry(ckpt->pwe_hdl,
    ...
    */
   if (NCSCC_RC_SUCCESS != ncs_patricia_tree_del(&ckpt->my_mbcsv_inst->ckpt_ssn_list,
   ...
   if (0 == mbcsv_get_next_entry_for_pwe(ckpt->pwe_hdl, &svc_id))
   {
      ...
      mbcsv_send_peer_disc_msg(MBCSV_PEER_DOWN_MSG, ckpt->my_mbcsv_inst, 
...
uns32 mbcsv_process_chg_role(MBCSV_EVT   *rcvd_evt, MBCSV_REG *mbc_inst)
{
   ...
   default:
      rc = m_MBCSV_DBG_SINK_SVC(NCSCC_RC_FAILURE,
           "Illigal ha role specified", mbc_inst->svc_id);
      goto done;
   }
   ...
   else
   {
      ...
      mbcsv_send_peer_disc_msg(MBCSV_PEER_UP_MSG, mbc_inst, 
                  ckpt, NULL, MDS_SENDTYPE_RBCAST, 0);
   }
...
*
\**************************************************************************/
uns32 mbcsv_send_notify_msg(uns32 msg_dest, CKPT_INST *ckpt_inst,
            ...
            if (NULL != ckpt_inst->active_peer)
            {
               ...
               if (TRUE == ckpt_inst->active_peer->okay_to_send_ntfy)
               {
                  ...
                  if (NCSCC_RC_SUCCESS != mbc_inst->mbcsv_cb_func(&parg))
                  {
                     ...
                     return m_MBCSV_DBG_SINK_SVC(NCSCC_RC_FAILURE,
                             "Notify callback returned failure for ", mbc_inst->svc_id);
                  }
...
uns32 mbcsv_send_data_req(NCS_UBAID *uba, CKPT_INST *ckpt_inst, MBCSV_REG *mbc_inst)
{
   ...
   m_NCS_MBCSV_FSM_DISPATCH(ckpt_inst->active_peer, 
      NCSMBCSV_SEND_DATA_REQ, &evt_msg);
...
*
\**************************************************************************/
uns32 ncs_mbcsv_encode_message(PEER_INST *peer, MBCSV_EVT *evt_msg,
                               uns8 *io_event, NCS_UBAID *uba)
{
   NCS_MBCSV_CB_ARG    parg;
