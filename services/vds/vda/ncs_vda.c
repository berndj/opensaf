...
uns32 ncsvda_api(NCSVDA_INFO *vda_info)
{
    ...
    case NCSVDA_VDEST_CREATE:
            ...
            NCSVDA_VDEST_CREATE_SPECIFIC)
        {
            ...
            m_MDS_FIXED_VDEST_TO_INST_NAME(
                m_MDS_GET_VDEST_ID_FROM_MDS_DEST(vda_info->info.vdest_create.info.specified.i_vdest),
        ...
        else
        {
            ...
            {/* VDEST to be created is named */
               ...
               if ( vda_info->info.vdest_create.i_persistent == TRUE)
                   return vda_info->o_result = m_LEAP_DBG_SINK(NCSCC_RC_FAILURE);
            }
            spir_req.i_instance_name = vda_info->info.vdest_create.info.named.i_name;
        }
            ...
            NCSVDA_VDEST_CREATE_SPECIFIC)
        {
           vda_info->info.vdest_create.info.named.o_vdest = 
                svc_info.info.query_pwe.info.virt_info.o_vdest;
        }
        ...
        {/* VDEST to be created is named */
            vda_info->info.vdest_create.o_mds_vdest_hdl = 
                     (MDS_HDL)vda_info->info.vdest_create.info.named.o_vdest;  
        }
        else
    ...
    case NCSVDA_VDEST_DESTROY:
        ...
        if (vda_info->info.vdest_destroy.i_create_type == 
            NCSVDA_VDEST_CREATE_SPECIFIC)
        {
            /* Check if the VDEST is in allowed range */
            ...
            m_MDS_FIXED_VDEST_TO_INST_NAME(
                m_MDS_GET_VDEST_ID_FROM_MDS_DEST(vda_info->info.vdest_destroy.i_vdest),
                &spir_req.i_instance_name);
        }
        else
        {
    ...
    case NCSVDA_PWE_CREATE:
        ...
        spir_req.i_sp_abstract_name = m_MDS_SP_ABST_NAME;
        m_MDS_FIXED_VDEST_TO_INST_NAME(m_MDS_GET_VDEST_ID_FROM_MDS_DEST(vda_info->info.pwe_create.i_mds_vdest_hdl), &spir_req.i_instance_name);
   ...
   case NCSVDA_PWE_DESTROY:
        ...
        */
        ...
        m_MDS_FIXED_VDEST_TO_INST_NAME(
           m_MDS_GET_VDEST_ID_FROM_PWE_HDL(vda_info->info.pwe_destroy.i_mds_pwe_hdl),
           &spir_req.i_instance_name);
...
static uns32 vda_instantiate(NCS_LIB_REQ_INFO *req)
{
    ...
    {
        if(0 == memcmp(req->info.inst.i_inst_attrs.attr[attr_num].attrName, m_SPRR_VDEST_POLICY_ATTR_NAME, strlen(m_SPRR_VDEST_POLICY_ATTR_NAME)))
        {
   ...
   else
   {
       ...
       /* Voila! The response from VDS is embedded in "vda_info" that was
       ** sent over MDS! 
       */
       new_vdest = vda_info.info.vdest_create.info.named.o_vdest;
   }

   /* STEP : Create VDEST locally.*/
   if (vda_create_vdest_locally(
       policy,
       &new_vdest, 
       &new_vdest_hdl) 
       != NCSCC_RC_SUCCESS)
...
static uns32 vda_uninstantiate(NCS_LIB_REQ_INFO *req)
{
    ...
    if (is_named_vdest) 
    {
       ...
       svc_info.info.svc_send.i_sendtype = MDS_SENDTYPE_SNDRSP;
       svc_info.info.svc_send.i_to_svc = NCSMDS_SVC_ID_VDS;
       svc_info.info.svc_send.info.sndrsp.i_time_to_wait = 1000; /* 10s */
       svc_info.info.svc_send.info.sndrsp.i_to_dest = gl_vda_info.vds_vdest;
...
\*-------------------------------------------------------------------------*/
uns32 vda_create_vdest_locally(uns32       i_pol,
                               MDS_DEST    *i_vdest,
...
\*-------------------------------------------------------------------------*/
uns32 vda_chg_role_vdest(MDS_DEST    *i_vdest,
...
static uns32 vda_mds_cb_enc      (struct ncsmds_callback_info *info)
{
   ...
   case NCSVDA_VDEST_CREATE:
      ...
      /* ENCODE: String name */
      vda_util_enc_vdest_name(uba, &vda_info->info.vdest_create.info.named.i_name);
      break;
...
static uns32 vda_mds_cb_dec      (struct ncsmds_callback_info *info)
{
   ...
   case NCSVDA_VDEST_CREATE:
      /* DECODE: vdest */
      vda_util_dec_vdest(uba, &vda_info->info.vdest_create.info.named.o_vdest);
      break;
