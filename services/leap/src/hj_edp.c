...
/*****************************************************************************
  ...
  PROCEDURE NAME:   ncs_edp_long

  DESCRIPTION:      EDU program handler for "long" data. This function
                    is invoked by EDU for performing encode/decode operation
                    on "long" data.
...
#if 0 
/* For 64bit compatibility , we need to eliminate the usage of ncs_edp_long so
 we recommend to use ncs_edp_uns32 instead of this */

uns32 ncs_edp_long(EDU_HDL *hdl, EDU_TKN *edu_tkn, NCSCONTEXT ptr, 
                    uns32 *ptr_data_len, EDU_BUF_ENV *buf_env, 
                    EDP_OP_TYPE op, EDU_ERR *o_err)
{
uns8    *p8, *src_p8;
    ...
    case EDP_OP_TYPE_ENC:
        ...
        {
            p8 = ncs_enc_reserve_space(buf_env->info.uba, 2);
            ncs_encode_16bit(&p8, (uns16)(sizeof(long)));
            ncs_enc_claim_space(buf_env->info.uba, 2);
        }

        if(ptr_data_len != NULL)
        {
            *ptr_data_len = sizeof(long);
        }
        src_p8 = (uns8 *)ptr;
        if(buf_env->is_ubaid)
        {
            ncs_encode_n_octets_in_uba(buf_env->info.uba, src_p8, (uns32)(sizeof(long)));
        }
        else
        {
            p8 = (uns8*)buf_env->info.tlv_env.cur_bufp;
            ncs_encode_tlv_n_octets(&p8, src_p8, (uns32)(sizeof(long)));
            ncs_edu_skip_space(&buf_env->info.tlv_env, (uns32)(EDU_TLV_HDR_SIZE + (uns32)(sizeof(long))));
        }

#if(NCS_EDU_VERBOSE_PRINT == 1)
        m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
        sysf_sprintf(gl_log_string, 
            "Encoded long: len = 0x%x : ", sizeof(long));
        ncs_edu_log_msg(gl_log_string);
        for(cnt=0;cnt<sizeof(long);cnt++)
        {
            m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
            if(cnt == (sizeof(long)-1))
    ...
    case EDP_OP_TYPE_DEC:
        ...
        if(buf_env->is_ubaid)
        {
            p8 = ncs_dec_flatten_space(buf_env->info.uba, (uns8*)ptr_data_len, 2);
            *ptr_data_len = ncs_decode_16bit(&p8);
            ncs_dec_skip_space(buf_env->info.uba, 2);
        }
        else
        {
            uns16 lcl_len = 0x0000;
            ...
            lcl_len = (uns16)*(p8)++ << 8;
            lcl_len |= (uns16)*(p8)++;
            *ptr_data_len = (uns32)lcl_len;
            if(lcl_len == 0)
               ncs_edu_skip_space(&buf_env->info.tlv_env, (uns32)(EDU_TLV_HDR_SIZE));
        }
        ...
        if(*ptr_data_len != 0)
        {
            if(*(long**)ptr == NULL)
            {
                /* Since "ncs_edp_long" is the responsibility of LEAP, LEAP
                   is supposed to malloc this memory. */
                (*(long**)ptr) = uptr = m_MMGR_ALLOC_EDP_LONG;
                if(uptr == NULL)
                {
                    /* Memory failure. */
                    *o_err = EDU_ERR_MEM_FAIL;
                ...
                m_NCS_MEMSET(uptr, '\0', (*ptr_data_len));
            }
            else
            {
                uptr = *((long**)ptr);
            ...
            {
                if(ncs_decode_n_octets_from_uba(buf_env->info.uba, 
                    (uns8*)uptr, *ptr_data_len) != NCSCC_RC_SUCCESS)
                {
                    *o_err = EDU_ERR_MEM_FAIL;
                    return NCSCC_RC_FAILURE;
                }
            ...
            {
                p8 = (uns8*)buf_env->info.tlv_env.cur_bufp;
                ncs_decode_tlv_n_octets(p8, (uns8*)uptr, *ptr_data_len);
                ncs_edu_skip_space(&buf_env->info.tlv_env, (uns32)(EDU_TLV_HDR_SIZE + *ptr_data_len));
            }

#if(NCS_EDU_VERBOSE_PRINT == 1)
            m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
            sysf_sprintf(gl_log_string, 
                "Decoded long: len = 0x%x : ", *ptr_data_len);
            ncs_edu_log_msg(gl_log_string);
            src_p8 = (uns8 *)uptr;
            for(cnt=0;cnt<sizeof(long);cnt++)
            {
    ...
    case EDP_OP_TYPE_PP:
        {
            ...
            if(len != 0)
            {
                ...
                else
                {
                    p8 = (uns8*)buf_env->info.tlv_env.cur_bufp;
                    ncs_decode_tlv_n_octets(p8, (uns8*)uptr, len);
                    ncs_edu_skip_space(&buf_env->info.tlv_env, (uns32)(EDU_TLV_HDR_SIZE + len));
                }

                src_p8 = (uns8 *)uptr;
                m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
                sysf_sprintf(gl_log_string, 
                    "PP long: len = 0x%x : ", len);
                ncs_edu_log_msg(gl_log_string);
                for(cnt=0;cnt<sizeof(long);cnt++)
                {
                    m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
                ...
                if(ptr != NULL)
                {
                    ...
                    m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
                    sysf_sprintf(gl_log_string, 
                        "Populated long in PPDB: len = 0x%x : ", len);
                    ncs_edu_log_msg(gl_log_string);
                    for(cnt=0;cnt<sizeof(long);cnt++)
                    {
                        m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
            ...
            {
                m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
                sysf_sprintf(gl_log_string, "PP long: len = 0x%x \n", len);
                ncs_edu_log_msg(gl_log_string);
            }
        }
        break;
...
/*****************************************************************************
  ...
  PROCEDURE NAME:   ncs_edp_double

  DESCRIPTION:      EDU program handler for "double" data. This function
                    is invoked by EDU for performing encode/decode operation
                    on "double" data.
...
*****************************************************************************/
#if 0  /* ncs_edp_double is mapped to ncs_edp_int64 IR00084181 */
       /* ncs_edp_int64 is suitable for encoding and decoding of double */
uns32 ncs_edp_double(EDU_HDL *hdl, EDU_TKN *edu_tkn, NCSCONTEXT ptr, 
                    uns32 *ptr_data_len, EDU_BUF_ENV *buf_env, 
                    EDP_OP_TYPE op, EDU_ERR *o_err)
{
    uns8    *p8, *src_p8;
    ...
    case EDP_OP_TYPE_ENC:
        ...
        {
            p8 = ncs_enc_reserve_space(buf_env->info.uba, 2);
            ncs_encode_16bit(&p8, (uns16)sizeof(double));
            ncs_enc_claim_space(buf_env->info.uba, 2);
        }

        if(ptr_data_len != NULL)
        {
            *ptr_data_len = sizeof(double);
        }
        src_p8 = (uns8 *)ptr;
        if(buf_env->is_ubaid)
        {
            ncs_encode_n_octets_in_uba(buf_env->info.uba, src_p8, (uns32)sizeof(double));
        }
        else
        {
            p8 = (uns8*)buf_env->info.tlv_env.cur_bufp;
            ncs_encode_tlv_n_octets(&p8, src_p8, (uns8)sizeof(double));
            ncs_edu_skip_space(&buf_env->info.tlv_env, EDU_TLV_HDR_SIZE + sizeof(double));
        }

#if(NCS_EDU_VERBOSE_PRINT == 1)
        m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
        sysf_sprintf(gl_log_string, 
            "Encoded double: len = 0x%x : ", sizeof(double));
        ncs_edu_log_msg(gl_log_string);
        for(cnt=0;cnt<sizeof(double);cnt++)
        {
            m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
    ...
    case EDP_OP_TYPE_DEC:
        ...
        if(*ptr_data_len != 0)
        {
            if(*(double**)ptr == NULL)
            {
                /* Since "ncs_edp_double" is the responsibility of LEAP, LEAP
                   is supposed to malloc this memory. */
                (*(double**)ptr) = uptr = m_MMGR_ALLOC_EDP_DOUBLE;
                if(uptr == NULL)
                {
                    /* Memory failure. */
                    *o_err = EDU_ERR_MEM_FAIL;
            ...
            m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
            sysf_sprintf(gl_log_string, 
                "Decoded double: len = 0x%x : ", *ptr_data_len);
            ncs_edu_log_msg(gl_log_string);
            src_p8 = (uns8 *)uptr;
            for(cnt=0;cnt<sizeof(double);cnt++)
            {
    ...
    case EDP_OP_TYPE_PP:
        {
            ...
            if(len != 0)
            {
                ...
                if(ptr != NULL)
                {
                    ...
                    m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
                    sysf_sprintf(gl_log_string, 
                        "Populated double in PPDB: len = 0x%x : ", len);
                    ncs_edu_log_msg(gl_log_string);
                    for(cnt=0;cnt<sizeof(double);cnt++)
                    {
                        m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
            ...
            {
                m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
                sysf_sprintf(gl_log_string, "PP double: len = 0x%x \n", len);
                ncs_edu_log_msg(gl_log_string);
            }
        }
        break;
...
/*****************************************************************************
  ...
  PROCEDURE NAME:   ncs_edp_float

  DESCRIPTION:      EDU program handler for "float" data. This function
                    is invoked by EDU for performing encode/decode operation
                    on "float" data.
...
*****************************************************************************/
#if 0 /* mcs_edp_float is mapped to ncs_edp_int32 IR00084181  */
      /* ncs_edp_int32 is suitable for encoding and decoding of float  */   

uns32 ncs_edp_float(EDU_HDL *hdl, EDU_TKN *edu_tkn, NCSCONTEXT ptr, 
                    uns32 *ptr_data_len, EDU_BUF_ENV *buf_env, 
                    EDP_OP_TYPE op, EDU_ERR *o_err)
{
    uns8 *p8, *src_p8;
    ...
    case EDP_OP_TYPE_ENC:
        ...
        {
            p8 = ncs_enc_reserve_space(buf_env->info.uba, 2);
            ncs_encode_16bit(&p8, (uns16)(sizeof(float)));
            ncs_enc_claim_space(buf_env->info.uba, 2);
        }
        ...
        if(*ptr_data_len != 0)
        {
            ...
            if(buf_env->is_ubaid)
            {
                ncs_encode_n_octets_in_uba(buf_env->info.uba, src_p8, (uns32)*ptr_data_len);
            }
            else
            {
                p8 = (uns8*)buf_env->info.tlv_env.cur_bufp;
                ncs_encode_tlv_n_octets(&p8, src_p8, (uns8)*ptr_data_len);
                ncs_edu_skip_space(&buf_env->info.tlv_env, EDU_TLV_HDR_SIZE + *ptr_data_len);
            }
#if(NCS_EDU_VERBOSE_PRINT == 1)
            m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
            sysf_sprintf(gl_log_string, 
                "Encoded float: len = 0x%x : %f\n", *ptr_data_len, *(float*)ptr);
            ncs_edu_log_msg(gl_log_string);
#endif
        }
        break;
    case EDP_OP_TYPE_DEC:
        ...
        if(*ptr_data_len != 0)
        {
            ...
            if(*(float**)ptr == NULL)
            {
                /* Since "ncs_edp_float" is the responsibility of LEAP, LEAP
                   is supposed to malloc this memory. */
                (*(float**)ptr) = uptr = m_MMGR_ALLOC_EDP_FLOAT;
                if(uptr == NULL)
                {
                    /* Memory failure. */
                    *o_err = EDU_ERR_MEM_FAIL;
            ...
            m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
            sysf_sprintf(gl_log_string, 
                "Decoded float: len = 0x%x : %f\n", *ptr_data_len, *(float*)uptr);
            ncs_edu_log_msg(gl_log_string);
#endif
        }
        break;
    ...
    case EDP_OP_TYPE_PP:
        {
            ...
            if(len != 0)
            {
                ...
                m_NCS_MEMSET(uptr, '\0', sizeof(float));
                if(buf_env->is_ubaid)
                {
                    if(ncs_decode_n_octets_from_uba(buf_env->info.uba, 
                        (uns8*)uptr, len) != NCSCC_RC_SUCCESS)
                ...
                m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
                sysf_sprintf(gl_log_string, "PP float: len = 0x%x : %f\n", 
                    len, *(float*)uptr);
                ncs_edu_log_msg(gl_log_string);
                if(ptr != NULL)
                {
...
/*****************************************************************************
  ...
  PROCEDURE NAME:   ncs_edp_ncsfloat32

  DESCRIPTION:      EDU program handler for "ncsfloat32" data. This function
                    is invoked by EDU for performing encode/decode operation
                    on "ncsfloat32" data.
...
*****************************************************************************/
#if 0  /* ncs_edp_ncsfloat32 is mapped to ncs_edp_int32 IR00084181 */
       /* ncs_edp_int32 is suitable for encoding and decoding of ncsfloat32 */
uns32 ncs_edp_ncsfloat32(EDU_HDL *hdl, EDU_TKN *edu_tkn, NCSCONTEXT ptr, 
                    uns32 *ptr_data_len, EDU_BUF_ENV *buf_env, 
                    EDP_OP_TYPE op, EDU_ERR *o_err)
{
    uns8 *p8, *src_p8;
    ...
    case EDP_OP_TYPE_ENC:
        ...
        if(ptr_data_len != NULL)
        {
            *ptr_data_len = sizeof(ncsfloat32);
        }
        ...
        {
            p8 = ncs_enc_reserve_space(buf_env->info.uba, 2);
            ncs_encode_16bit(&p8, (uns16)(sizeof(ncsfloat32)));
            ncs_enc_claim_space(buf_env->info.uba, 2);
        }
        ...
        if(*ptr_data_len != 0)
        {
            ...
            m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
            sysf_sprintf(gl_log_string, 
                "Encoded ncsfloat32: len = 0x%x : %f\n", 
                *ptr_data_len, *(ncsfloat32*)ptr);
            ncs_edu_log_msg(gl_log_string);
#endif
        }
        break;
    case EDP_OP_TYPE_DEC:
        ...
        if(*ptr_data_len != 0)
        {
            ncsfloat32   *uptr = NULL;

            if(*(ncsfloat32**)ptr == NULL)
            {
                /* Since "ncs_edp_ncsfloat32" is the responsibility of LEAP, LEAP
                   is supposed to malloc this memory. */
                (*(ncsfloat32**)ptr) = uptr = m_MMGR_ALLOC_EDP_NCSFLOAT32;
                if(uptr == NULL)
                {
                    /* Memory failure. */
                    *o_err = EDU_ERR_MEM_FAIL;
            ...
            m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
            sysf_sprintf(gl_log_string, 
                "Decoded ncsfloat32: len = 0x%x : %f\n", 
                *ptr_data_len, *(ncsfloat32*)uptr);
            ncs_edu_log_msg(gl_log_string);
#endif
        }
        break;
    ...
    case EDP_OP_TYPE_PP:
        {
            ...
            if(len != 0)
            {
                uptr = m_MMGR_ALLOC_EDP_NCSFLOAT32;
                if(uptr == NULL)
                {
                    /* Memory failure. */
                    *o_err = EDU_ERR_MEM_FAIL;
                ...
                m_NCS_MEMSET(uptr, '\0', sizeof(ncsfloat32));
                if(buf_env->is_ubaid)
                {
                    if(ncs_decode_n_octets_from_uba(buf_env->info.uba, 
                        (uns8*)uptr, len) != NCSCC_RC_SUCCESS)
                ...
                m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
                sysf_sprintf(gl_log_string, "PP ncsfloat32: len = 0x%x : %f\n", 
                    len, *(float*)uptr);
                ncs_edu_log_msg(gl_log_string);
                if(ptr != NULL)
                {
                    ...
                    m_NCS_MEMSET(&gl_log_string, '\0', GL_LOG_STRING_LEN);
                    sysf_sprintf(gl_log_string, 
                        "Populated ncsfloat32 in PPDB: %f \n", 
                        *(ncsfloat32*)uptr);
                    ncs_edu_log_msg(gl_log_string);
                }
                else
...
/*****************************************************************************
  ...
  PROCEDURE NAME:   ncs_edp_mds_dest

  DESCRIPTION:      EDU program handler for "MDS_DEST" data. This function
                    is invoked by EDU for performing encode/decode operation
                    on "MDS_DEST" data.
...
*****************************************************************************/
uns32 ncs_edp_mds_dest(EDU_HDL *hdl, EDU_TKN *edu_tkn, 
                                NCSCONTEXT ptr, uns32 *ptr_data_len, 
                                EDU_BUF_ENV *buf_env, EDP_OP_TYPE op, 
                                EDU_ERR *o_err)
{
    uns32       rc = NCSCC_RC_SUCCESS;
    MDS_DEST    *struct_ptr = NULL, **d_ptr = NULL;

    EDU_INST_SET mds_dest_rules[ ] = {
        {EDU_START, ncs_edp_mds_dest, 0, 0, 0, sizeof(MDS_DEST), 0, NULL},
        {EDU_EXEC, ncs_edp_uns32, 0, 0, 0, 
            (uns32)&((MDS_DEST*)0)->node_id, 0, NULL},
        {EDU_TEST, ncs_edp_uns32, 0, 0, 0, 
            (uns32)&((MDS_DEST*)0)->node_id, 0, ncs_edu_mds_dest_test_nodeid_fnc},
        {EDU_EXEC, ncs_edp_uns16, 0, 0, 0, 
            (uns32)&((MDS_DEST*)0)->info.v1.pad16, 0, NULL},
        {EDU_EXEC, ncs_edp_uns16, 0, 0, EDU_EXIT, 
            (uns32)&((MDS_DEST*)0)->info.v1.vcard, 0, NULL},
        {EDU_EXEC, ncs_edp_uns32, 0, 0, EDU_EXIT, 
            (uns32)&((MDS_DEST*)0)->info.proc_v2, 0, NULL},
        {EDU_END, 0, 0, 0, 0, 0, 0, NULL},
    };

    if(op == EDP_OP_TYPE_ENC)
    {
        struct_ptr = (MDS_DEST *)ptr;
    }
    else if(op == EDP_OP_TYPE_DEC)
    {
        ...
        if(*d_ptr == NULL)
        {
            *d_ptr = m_NCS_MEM_ALLOC(sizeof(MDS_DEST),\
            ...
            m_NCS_MEMSET(*d_ptr, '\0', sizeof(MDS_DEST));
        }
        struct_ptr = *d_ptr;
    }
    else
    ...
    rc = m_NCS_EDU_RUN_RULES(hdl, edu_tkn, mds_dest_rules, struct_ptr, 
...
/*****************************************************************************
  ...
  PROCEDURE NAME:   ncs_edu_mds_dest_test_nodeid_fnc

  DESCRIPTION:      Test function, that gets invoked from the EDP
                    function "ncs_edp_mds_dest".

  RETURNS:          char *, denoting the label to execute next.

*****************************************************************************/
int ncs_edu_mds_dest_test_nodeid_fnc(NCSCONTEXT arg)
{
    typedef enum {
        LCL_TEST_JUMP_OFFSET_ZERO_VALUE = 1,
        LCL_TEST_JUMP_OFFSET_NONZERO_VALUE = 3
    ...
    nodeid = *(NODE_ID*)arg;
    if(nodeid == 0)
        return LCL_TEST_JUMP_OFFSET_ZERO_VALUE;

    return LCL_TEST_JUMP_OFFSET_NONZERO_VALUE;
}
#endif
...
#if 0
        LCL_TEST_JUMP_OFFSET_FMT_HDL = NCS_FMT_HDL,
#endif
...
#if 0
    case NCS_FMT_HDL:
       return LCL_TEST_JUMP_OFFSET_FMT_HDL;
#endif
    case NCS_FMT_STR:
