...
$Header: /ncs/software/leap/base/common/src/ncs_mtree.c 14    7/26/01 5:37p Agranos $
 ...
 ***************************************************************/
static unsigned int
mtree_p32_init(NCS_MTREE                 *const pTree)
{
...
/* Initialize the root node, which is actually part of the tree structure. */
   ...
   pTree->Style.p32.Root.Style.p32.pLeft =
   pTree->Style.p32.Root.Style.p32.pRight = 
   &pTree->Style.p32.Root;
   pTree->Style.p32.Root.Style.p32.pDownToMe =
   pTree->Style.p32.Root.Style.p32.pMoreSpecific =
   pTree->Style.p32.Root.Style.p32.pLessSpecific =
   NCS_MTREE_NODE_NULL;
...
static void
mtree_p32_dest(NCS_MTREE *const pTree)
{
}
...
mtree_p32_findadd(NCS_MTREE         *const pTree,
                  ...
                  void             *const Cookie)
{
   ...
   if (m_MTREE_P32_KEY_CMP(pKey, pSrch->pKey))
   {
      ...
      if ((pNode = (*pFunc)(pKey, KeyBitSize, Cookie)) != NCS_MTREE_NODE_NULL)
      {
         ...
         */
         ...
         pNode->Style.p32.pLeft =
         pNode->Style.p32.pDownToMe =
         NCS_MTREE_NODE_NULL;
   ...
   if ((pNode = (*pFunc)(pKey, KeyBitSize, Cookie)) != NCS_MTREE_NODE_NULL)
   {
      ...
      */
      ...
      pNode->Style.p32.pLessSpecific =
      pNode->Style.p32.pMoreSpecific =
      NCS_MTREE_NODE_NULL;
 ...
 ******************************************************************/
static void
mtree_p32_del(NCS_MTREE      *const pTree,
              NCS_MTREE_NODE *const pNode)
{
   ...
   pLegDownToNode = ((pPrev->Style.p32.pLeft == pNode) ? 
                     &pPrev->Style.p32.pLeft :
                     &pPrev->Style.p32.pRight);
   ...
   do 
   {
      ...
      pNextLeg = ((UpWentRight) ?
                  &pNext->Style.p32.pRight :
                  &pNext->Style.p32.pLeft);
      pTemp = *pNextLeg;
 ...
 *******************************************************************/
static NCS_MTREE_NODE* 
mtree_p32_best(NCS_MTREE *const pTree,
               const uns32 *const  pKey,
               short      KeyBitSize,
               int  *const   pMoreSpecificExists)
{
   ...
   *pMoreSpecificExists = FALSE;  /* Assume not. */
   ...
   do
   {
               ...
               */
                  ...
                  )
               {
                  ...
                  if ((pSrchSave->Style.p32.Bit > pSrch->Style.p32.Bit) &&
                      (m_MTREE_P32_KEY_MASK_CMP(Target, *(const uns32*)pSrchSave->pKey, KeyBitSize)))
                  {
                     *pMoreSpecificExists = TRUE;
                  }
               }
       ...
       */
      if (!m_MTREE_P32_KEY_MASK_CMP(Target,
                                    *(const uns32*)pSrch->pKey,
                                    pSrch->Style.p32.Bit))
      {
      ...
      if ((pSrch->KeyBitSize > KeyBitSize) &&
          (m_MTREE_P32_KEY_MASK_CMP(Target, *(const uns32*)pSrch->pKey,
                                    KeyBitSize))
         )
      {
         *pMoreSpecificExists = TRUE;
 ...
 *******************************************************************/
static NCS_MTREE_NODE*
mtree_p32_nextbest(NCS_MTREE *const pTree,
 ...
 ***********************************************************************/
static NCS_MTREE_NODE *
mtree_p32_next(NCS_MTREE               *const pTree,
               const uns32 *const       pKey,
               short                    KeyBitSize)
{
   uns32 Target;
         ...
         do
         {
            ...
            if (pSrch == pPrev->Style.p32.pLeft)
            {
               ...
               Target = (((*(uns32*)pSrch->pKey) & 
                          P32Masks[pSrch->Style.p32.Bit]) | 
                         P32Bits[1 + pSrch->Style.p32.Bit]) ;
               break;
            }
            else
            {
               ...
               Target = htonl(ntohl(((*(uns32*)pPrev->pKey) | ~P32Masks[pPrev->Style.p32.Bit])) + 1);
...
{
      NCS_MTREE_NODE *testNode;
      testNode = xmtree_nextbest(pTree,
                   pBetterNode);
    return (testNode);
...
static NCS_MTREE_NODE *
PrivateCallback(const void* pKey,
                short       KeyBitSize,
                void*       pNode)
{
             ...
             NCS_MTREE_NODE *const pNode)
{
   ...
   pRetNode = ncs_mtree_find_add(pTree,
                                pNode->pKey,
                                pNode->KeyBitSize,
                                PrivateCallback,
                                pNode);
 ...
 *******************************************************************/
NCS_MTREE_NODE *
ncs_mtree_find_add(NCS_MTREE         *const pTree,
...
ncs_mtree_find_best(NCS_MTREE *const pTree,
                   ...
                   int  *const   pMoreSpecificExists)
{
   return ((*MtreeActions[pTree->Cfg.Style].pBestFunc)(pTree, 
                                                       pKey, 
                                                       KeyBitSize, 
                                                       pMoreSpecificExists));
}
