...
uns32 sm_mem_rpt_ssup(NCSSYSM_MEM_RPT_SSUP*  info)
   ...
   if((to_std == TRUE) || (to_file == TRUE))
      ...
      for (i= 0,mp = mmgr.mpools;i < bkt_cnt; mp++,i++)
         ...
         sprintf(output_string, "%13d%13d%13d%15d\n",
   ...
   if((to_std == TRUE) || (to_file == TRUE))
      ...
      for (mp = gl_sysmon.mmgr->mpools;; mp++)
         ...
         for(me = mp->inuse,j = 0;me != NULL;me = me->next)
            ...
            sprintf (output_string, "%4d%5d%5d%12s%6d%12s%9d%4d%4d%11x%5d \n",
               j + 1,                                /* Nmber */
               me->age,                            /* age */
               me->line,                           /* Line  */
           ...
           if(0 == strcmp(pfile, mmgr.stktrace_info[index].file))
           {
               ...
               if(NULL != ste)
               {
                   do
                   {
                       ...
                       {
                           info->o_records++;
                           sprintf((char *)outstr+loutlen, "| Allocation: %d (ste=0x%08X)\tTicks: %d\n", info->o_records, (uns32)ste, ticks_diff);
                           loutlen = strlen((const char *)outstr);
