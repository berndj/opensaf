 ...
 *******************************************************************/
NCS_HASH_TABLE * 
ncs_hash_init (NCS_HASH_TABLE *pHashInitTable)
{
    ...
    pHashTable->ppHashTablePtr = (NCS_DB_LINK_PTR)m_NCS_HASH_ALLOC(
                                    pHashTable->sizeOfHashTable *
    ...
    m_NCS_MEM_SET (pHashTable->ppHashTablePtr,0,(pHashTable->sizeOfHashTable *
                                    sizeof(NCS_DB_LINK_PTR)));
    return (pHashTable);
}
 ...
 *******************************************************************/
NCS_HASH_KEY 
ncs_hash_key_generate (NCS_HASH_TABLE *pHashTable, void *pDataPtr)
{
 ...
 *******************************************************************/
int 
ncs_hash_insert_data (NCS_HASH_TABLE *pHashTable, NCS_HASH_KEY hashKey,
                     void *pDataPtr)
{
    uns32 bucketOffset;
    ...
    if (pHashTable->pHashKeyInsertProc != NULL)
    {
        ...
        if (pDbLinkList == NULL)
        {
            *(pHashTable->ppHashTablePtr + bucketOffset) = 
                                        (NCS_HASH_DB_LINK_LIST *)pDataPtr;
        } else
        {
 ...
 *******************************************************************/
void *
ncs_hash_key_lookup (NCS_HASH_TABLE *pHashTable, NCS_HASH_KEY hashKey, 
                    void *pDataKey)
{
 ...
 *******************************************************************/
void *
ncs_hash_key_get_next (NCS_HASH_TABLE *pHashTable, NCS_HASH_KEY *hashKey, 
                      void *pDataPtr)
{
    uns32 bucketOffset;
    ...
    if (pDataPtr == NULL)
    {
        ...
        do {
             ...
             */
            if ((pDbLinkList != NULL) || (hashBucketCount >= 
                pHashTable->sizeOfHashTable))
            {
    ...
    } else 
    {
        ...
        if (pTempDbLinkList == NULL)
        {
             ...
             */
            do {
                if ((pTempDbLinkList != NULL) || (hashBucketCount >= 
                    pHashTable->sizeOfHashTable))
                {
                    *hashKey  = (hashBucketCount-1);
 ...
 *******************************************************************/
int 
ncs_hash_key_delete (NCS_HASH_TABLE *pHashTable, NCS_HASH_KEY hashKey, 
                    void *pDataKey)
{
    uns32 bucketOffset;
 ...
 *******************************************************************/
int 
ncs_hash_delete_table (NCS_HASH_TABLE *pHashTable, NCS_BOOL freeUserData)
{
    uns32 tempVar;
    NCS_HASH_DB_LINK_LIST *pDbLinkList;
    NCS_HASH_DB_LINK_LIST *pTempDbLinkList;
