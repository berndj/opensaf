...
void
ncs_add_stats_element( struct ipc_stats_elem *statEnvelope, void *resource)
{
    /*
...
void
ncs_rem_stats_element(struct ipc_stats_elem  *statEnvelope)
{
    /*
...
void
printInternalMsgDump( int cnt, void *msg )
{
    /*
...
NCS_BOOL
internalDumpMsg( void *arg, void *msg )
{
...
int
ncs_dumpIpcStatsToConsole( void )
{
    int       ret = 1;
#if (NCS_IPCQUE_DEBUG != 0)
 ...
 while( curr )
 {
     m_NCS_CONS_PRINTF("%4d%4d%7d%8d%7d%5d %5d%11d%11x\n",
       ...
       curr->failureCnt,
       curr->hiPriCnt,
       curr->currDepth - curr->hiPriCnt,
       now - curr->creationTime,
       curr->resource );
     curr = curr->next;
 }
...
int
ncs_dumpIpcStatsToFile( char * filename )
{
    int ret = 0;
    USE(filename);
#if (NCS_IPCQUE_DEBUG != 0)
...
int
ncs_walkAndDumpIpcQue( int id, int cnt )
{
    /*
