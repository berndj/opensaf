...
uns32
ubsar_init(NCS_UBSAR_INIT* init)
{
 NCS_UBSAR*   ubs;
...
uns32 
ubsar_segment( NCS_UBSAR_SEGMENT* seg)
  {
  NCS_UBSAR*      ubs;
  ...
  seg->o_cnt = m_MMGR_FRAG_BUFR( seg->i_bigbuf, 
                               (uns32)(ubs->max_size - NCS_UBSAR_CPLT_TRLR_SIZE), 
                                ubs->ubq_ubf);
...
uns32
ubsar_get_app_trlr(NCS_UBSAR_TRLR* trlr)
{
 uns32   ttl_len;
...
uns32
ubsar_assemble (NCS_UBSAR_ASSEMBLE* ass)
  {
 uns32       ttl_len;
...
uns32
ubsar_destroy(NCS_UBSAR_DESTROY* dest)
{
 NCS_UBSAR*   ubs;
...
*****************************************************************************/
uns32 
ncs_ubsar_request(NCS_UBSAR_ARGS *args)
  {
  ...
  switch (args->op)
    ...
    case NCS_UBSAR_OP_INIT:
      rc = ubsar_init        (&args->info.init);      break;
      
    case NCS_UBSAR_OP_SGMNT:
      rc = ubsar_segment     (&args->info.segment);   break;
      
    case NCS_UBSAR_OP_ASMBL:
      rc = ubsar_assemble    (&args->info.assemble);  break;
      
    case NCS_UBSAR_OP_GETAPT:
      rc = ubsar_get_app_trlr(&args->info.app_trlr); break;
      
    case NCS_UBSAR_OP_DESTROY:
      rc = ubsar_destroy     (&args->info.destroy);   break;
