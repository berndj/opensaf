...
**************************************************************************/
uns32
rp_tmr_time_left_in_sec(time_t tmr_trig_at, uns32 tmr_value)
{
   uns32 tmr_left = 0;
   time_t now;
...
**************************************************************************/
NCS_RP_TMR_CB * 
ncs_rp_tmr_init(NCS_RP_TMR_INIT *tmr_init_info)
{
   ...
   m_NCS_TMR_CREATE(tmr_cb->tmr_id,m_RP_TMR_DEF_PERIOD,tmr_cb->tmr_callback,
      tmr_cb->callback_arg);
   return (tmr_cb);
}
...
**************************************************************************/
uns32 
ncs_rp_tmr_start (NCS_RP_TMR_CB *tmr_cb, NCS_RP_TMR_HDL tmr_id, uns32 period, 
                 RP_TMR_CALLBACK callbk,
                 void *arg)
{
   NCS_RP_TMR_INFO *tmr_info;
   ...
   {      
      ...
      } else
      {
          ...
          * received timer
          */
         left_sec = rp_tmr_time_left_in_sec(tmr_list->tmr_trig_at, 
            tmr_list->tmr_value);
...
**************************************************************************/
uns32
ncs_rp_tmr_destory(NCS_RP_TMR_CB **pptmr_cb)
{
   NCS_RP_TMR_INFO *tmr_info;
...
ncs_rp_tmr_exp (NCS_RP_TMR_CB *tmr_cb)
{
   ...
   do
   {
      ...
      for (; tmr_list != NULL; )
      {         
         tmr_diff = rp_tmr_time_left_in_sec(tmr_list->tmr_trig_at, 
            tmr_list->tmr_value);         
