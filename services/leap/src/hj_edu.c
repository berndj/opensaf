...
static void 
edu_populate_ppdb_key(EDU_PPDB_KEY *key, 
                      EDU_PROG_HANDLER  parent_edp, 
                      EDU_PROG_HANDLER  self_edp, 
                      uns32   offset);
static EDU_PPDB_NODE_INFO *
edu_ppdb_node_findadd(EDU_PPDB *ppdb, 
                      EDU_PROG_HANDLER  parent_edp,
                      EDU_PROG_HANDLER  self_edp,
                      uns32   offset,
                      NCS_BOOL add);
static void edu_ppdb_node_del(EDU_PPDB *ppdb, 
                              EDU_PPDB_NODE_INFO *node);
...
static uns32 
ncs_edu_get_size_of_var_len_data(EDU_PROG_HANDLER edp, NCSCONTEXT cptr, 
                                 uns32 *p_data_len, EDU_ERR *o_err);

static int edu_chk_ver_ge (EDU_MSG_VERSION *local_version, EDU_MSG_VERSION *to_version, 
                                              int skip_cnt_if_ne);
                   ...
                   EDP_OP_TYPE op, NCSCONTEXT arg, EDU_ERR *o_err, EDU_MSG_VERSION to_version ,uns8 var_cnt, ...)
{
    ...
    if(op == EDP_OP_TYPE_ENC)
    {
        rc = ncs_edu_perform_enc_op(edu_hdl, edp, &lcl_edu_buf, &lcl_cnt, arg, o_err,
                                    var_cnt, var_array);
        if(rc != NCSCC_RC_SUCCESS)
        {
            m_LEAP_DBG_SINK_VOID(NCSCC_RC_FAILURE);
    ...
    {
        rc = ncs_edu_perform_dec_op(edu_hdl, edp, &lcl_edu_buf, &lcl_cnt, arg, o_err,
                                    var_cnt, var_array);
        if(rc != NCSCC_RC_SUCCESS)
        {
            m_LEAP_DBG_SINK_VOID(NCSCC_RC_FAILURE);
...
*****************************************************************************/
uns32 ncs_edu_run_edp(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn, EDU_INST_SET *rule,
                      EDU_PROG_HANDLER edp, NCSCONTEXT ptr, uns32 *dcnt, 
                      EDU_BUF_ENV *buf_env, EDP_OP_TYPE optype, 
                      EDU_ERR *o_err)
{
    NCSCONTEXT              lcl_ptr = NULL;
    EDU_HDL_NODE            *lcl_hdl_node = NULL;
    uns32                   lcl_rc = NCSCC_RC_SUCCESS, 
                            dtype_attrb = 0x00000000;
    ...
    else
    {
        ...
        lcl_rc = edp(edu_hdl, edu_tkn, ptr, dcnt, buf_env, 
                                       optype, o_err);
    }
...
*****************************************************************************/
uns32 ncs_edu_run_rules(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
...
/*****************************************************************************
  ...
  RETURNS:          uns32(depicting the label to execute next)
                    ...
                    - else, (uns32), i.e., label of next executable 
                      instruction.
                        ...
                        EDP_OP_TYPE optype, EDU_ERR *o_err)
{
    ...
    case EDU_TEST:
        ...
        if(optype == EDP_OP_TYPE_PP)
        {
            ...
            rc_lbl = m_NCS_EDU_RUN_TEST_CONDITION(edu_hdl, rule, 
                                          ptr, buf_env, optype, o_err);
    ...
    case EDU_VER_GE:
        {
              rc_lbl = m_NCS_EDU_RUN_VER_GE(edu_hdl, rule,
                                           ptr, buf_env, optype, o_err);
        }
    ...
    case EDU_VER_USR:
        {
              ...
              rc_lbl = m_NCS_EDU_RUN_VER_USR(edu_hdl, rule,
                                           ptr, buf_env, optype, o_err);
        }   
        break;
    case EDU_EXEC_EXT:
...
*****************************************************************************/
uns32 ncs_edu_perform_exec_action(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
                                  ...
                                  EDU_BUF_ENV *buf_env, EDU_ERR *o_err)
{
    ...
    if((rule->fld2 & EDQ_POINTER) == EDQ_POINTER)
    {
        ...
        else if(optype == EDP_OP_TYPE_PP)
        {
            ...
            */
            uns32 *p_data_len = ptr_data_len;
            NCS_BOOL    lcl_is_tested = FALSE;
            EDU_PPDB_NODE_INFO *ppdb_node = NULL;
            uns32   refcount = 0;

            refcount = 
                m_NCS_EDU_GET_REFCOUNT_OF_TESTABLE_FIELD(hdl_node->test_instr_store,
                rule);
            if(refcount != 0)
            {
                lcl_is_tested = TRUE;
        ...
        NCSCONTEXT ptr, uns32 *ptr_data_len, EDU_BUF_ENV *buf_env, EDU_ERR *o_err)
{
    ...
    if(optype == EDP_OP_TYPE_ENC)
    {
        rc = ncs_edu_prfm_enc_on_non_ptr(edu_hdl, edu_tkn, hdl_node, rule, ptr, 
                                         ptr_data_len, buf_env, o_err);
    }
    else if(optype == EDP_OP_TYPE_DEC)
    {
        rc = ncs_edu_prfm_dec_on_non_ptr(edu_hdl, edu_tkn, hdl_node, rule, ptr, 
                                         ptr_data_len, buf_env, o_err);
    }
#if (NCS_EDU_VERBOSE_PRINT == 1)
    else if(optype == EDP_OP_TYPE_PP)
    {
        rc = ncs_edu_prfm_pp_on_non_ptr(edu_hdl, edu_tkn, hdl_node, rule, ptr, 
                                         ptr_data_len, buf_env, o_err);
    }
#endif
...
*****************************************************************************/
uns32 ncs_edu_prfm_enc_on_non_ptr(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn, 
                                  EDU_HDL_NODE *hdl_node, 
            ...
            if(is_builtin_edp)
            {
                if(buf_env->is_ubaid)
                {
                   ...
                   if((l_size != 0) && ((rule->fld1 == ncs_edp_char) || (rule->fld1 == ncs_edp_uns8)))
                   {
                      ...
                      if(lcl_cnt != 0)
                      {
                        rc = ncs_encode_n_octets_in_uba(buf_env->info.uba, newptr, lcl_cnt);
                        if(rc != NCSCC_RC_SUCCESS)
                           return rc;
                      }
                   }
...
*****************************************************************************/
uns32 ncs_edu_prfm_dec_on_non_ptr(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
                                  EDU_HDL_NODE *hdl_node, 
                                  EDU_INST_SET *rule, NCSCONTEXT ptr, 
                                  uns32 *ptr_data_len, EDU_BUF_ENV *buf_env, 
                                  EDU_ERR *o_err)
{
    NCSCONTEXT      newptr = NULL;
    EDU_HDL_NODE    *lcl_hdl_node = NULL;
    uns32           rc = NCSCC_RC_SUCCESS, lcnt = 0, lcl_cnt = 0;
    uns32           l_size = 0;
    ...
    if((rule->fld2 & EDQ_VAR_LEN_DATA) == EDQ_VAR_LEN_DATA)
    {
        ...
        if((*ptr_data_len) != 0)
        {
            ...
            lcl_mem_ptr = m_NCS_MEM_ALLOC(l_size * (*ptr_data_len),\
                                NCS_MEM_REGION_PERSISTENT,\
                                next_rule->fld2, /* Svc-ID */
                ...
                if((l_size != 0) && ((rule->fld1 == ncs_edp_char) || (rule->fld1 == ncs_edp_uns8)))
                {
                   ...
                   if(lcl_cnt != 0)
                   {
                     if(ncs_decode_n_octets_from_uba(buf_env->info.uba,
                        (uns8*)newptr, lcl_cnt) != NCSCC_RC_SUCCESS)
                     {
                        *o_err = EDU_ERR_MEM_FAIL;
                        return m_LEAP_DBG_SINK(NCSCC_RC_FAILURE);
                     }
    ...
    else
    {
        newptr = (NCSCONTEXT)((long)ptr + (long)rule->fld5);
        rc = m_NCS_EDU_RUN_EDP(edu_hdl, edu_tkn, rule, rule->fld1, 
                (NCSCONTEXT)&newptr, &lcl_cnt, buf_env, EDP_OP_TYPE_DEC, 
                o_err);
        if(rc != NCSCC_RC_SUCCESS)
            return rc;
        if((rule->fld2 & EDQ_POINTER) == EDQ_POINTER)
...
*****************************************************************************/
#if (NCS_EDU_VERBOSE_PRINT == 1)
uns32 ncs_edu_prfm_pp_on_non_ptr(EDU_HDL *edu_hdl, EDU_TKN *edu_tkn,
                                 EDU_HDL_NODE *hdl_node, 
                                 EDU_INST_SET *rule, NCSCONTEXT ptr, 
                                 uns32 *ptr_data_len, EDU_BUF_ENV *buf_env, 
                                 EDU_ERR *o_err)
{
    ...
    uns32       lcl_offset = 0, lcl_cnt = 0, lcnt = 0;
    uns32       refcount = 0,rc = NCSCC_RC_SUCCESS;
    uns32       l_size = 0;   
    ...
    if((rule->fld2 & EDQ_VAR_LEN_DATA) == EDQ_VAR_LEN_DATA)
    {
        ...
        /* The length of variable-sized-data is passed in the 
           ...
           "fld3"). */
        rc = ncs_edu_get_size_of_var_len_data(rule->fld3, ppdb_node->data_ptr,
                    &lcl_cnt, o_err);
        if(rc != NCSCC_RC_SUCCESS)
        {
            return m_LEAP_DBG_SINK(rc);
...
/*****************************************************************************
  ...
  RETURNS:          uns32, denoting the EDU return values.
                                    ...
                                    int instr_count)
{
          ...
          (prog[cur_inst_indx].instr != EDU_END))
    {
        ...
        if((rc_lbl == 0) || (rc_lbl == EDU_NEXT))
        {
            ...
            if((prog[cur_inst_indx].fld2 & EDQ_VAR_LEN_DATA) == EDQ_VAR_LEN_DATA)
            {
...
*****************************************************************************/
uns32 ncs_edu_run_rules_for_compile(EDU_HDL *edu_hdl, EDU_HDL_NODE *hdl_node,
                                    EDU_INST_SET *prog, NCSCONTEXT ptr, 
                                    uns32 *ptr_data_len, EDU_ERR *o_err, 
                                    int instr_count)
{
    uns32               l_size = 0;
    int                 i = 0;
    NCS_BOOL            start_fnd = FALSE, end_fnd = FALSE, 
                        testll_fnd = FALSE;
    NCS_BOOL            ptr_set = FALSE, ll_set = FALSE, arr_set = FALSE,
                        var_len_set = FALSE;
    EDU_HDL_NODE        *lcl_hdl_node = NULL;
    ...
    /* Look for duplicate "EDU_START", "EDU_END" and "EDU_TEST_LL_PTR" 
       ...
       "labels" list. */
    for(i = 0;
        ((i != instr_count) && (prog[i].instr != EDU_END));
        i++)
    {
        ...
        else if(prog[i].instr == EDU_TEST_LL_PTR)
        {
            ...
            if((prog[0].fld2 & EDQ_LNKLIST) == EDQ_LNKLIST)
            {
                ...
                else
                {
                    testll_fnd = TRUE;
                    if((prog[i].fld5 == 0) ||
                        ((prog[i].fld5 + sizeof(uns32*)) > hdl_node->size))
                    {
            ...
            if(hdl_node->size != 0)
            {
                ...
                if(arr_set || ptr_set || var_len_set)
                {
                    ...
                    if(l_size != 0)
                    {
                        ...
                        {
                            if((prog[i].fld5 + sizeof(uns32*)) > hdl_node->size)
                            {
                                /* This is another failure case. */
                                *o_err = EDU_ERR_FIELD_OFFSET_EXCEEDS_EDP_SIZE;
                                return m_LEAP_DBG_SINK(NCSCC_RC_FAILURE);
                            ...
                            if(var_len_set)
                            {
                                    ...
                                    &t_size))
                                {
                                    ...
                                    *o_err = EDU_ERR_VAR_LEN_PARAMETER_NOT_BASIC_EDP_TYPE;
                                    return m_LEAP_DBG_SINK(NCSCC_RC_FAILURE);
                                }
                            }
                        }   /* if(ptr_set) */
...
*****************************************************************************/
uns32 ncs_edu_get_refcount_of_testable_field(EDP_TEST_INSTR_REC *inst_store, 
                                EDU_INST_SET *rule)
{
    ...
    {
        if((lrec->edp == rule->fld1) &&
            (lrec->offset == rule->fld5))
        {
...
*****************************************************************************/
uns32 ncs_edu_run_edcompile_on_edp(EDU_HDL *edu_hdl, 
                                   EDU_HDL_NODE *hdl_node, EDU_ERR *o_err)
{
    ...
    return hdl_node->edp(edu_hdl, &edu_tkn, (NCSCONTEXT)&admin_op, NULL, NULL, 
...
*****************************************************************************/
uns32 ncs_edu_validate_and_gen_test_instr_rec_list(EDP_TEST_INSTR_REC **head, 
    ...
    for(i = 0; ((i != instr_count) && (rules_head[i].instr != EDU_END)); i++)
    {
        if(rules_head[i].instr == EDU_TEST)
        {
            ...
            if(!already_added)
            {
                ...
                for(j = 0; 
                    ((j != instr_count) && (rules_head[j].instr != EDU_END));
                    j++)
                {
        ...
        {
            NCS_BOOL    already_added = FALSE, 
                        lcl_to_be_added = FALSE;

            for(j = 0;
                (((j != instr_count) && rules_head[j].instr != EDU_END)); j++)
            {
...
static void ncs_edu_ppdb_destroy(EDU_PPDB *ppdb)
{
   ...
   if(ppdb->is_up)
   {
      for (;;)
      {
         if ((pnode = ncs_patricia_tree_getnext(&ppdb->tree, 
                        (const uns8 *)key)) != 0)
         {
                             ...
                             int *var_array)
{
    ...
    if(buf_env->is_ubaid && 
       ((buf_env->info.uba->start != NULL) || (buf_env->info.uba->max == 0)))
    {
        /* Init only if it is not already done. */
        if(buf_env->info.uba->start != NULL)
    ...
    *(*stream)++ = (uns8)lcnt;         /* length */
    if(count != 0)
    {
        if(val == NULL)
        {
            ...
            return (uns32) EDU_TLV_HDR_SIZE;
        }
            
        for (i=0; i<count; i++)
            *(*stream)++ = *val++;
    }
    return (uns32) (EDU_TLV_HDR_SIZE + count); 
}
...

{
    if(edp == ncs_edp_uns8)
        ...
        *p_data_len = (uns32)(*(int*)cptr);
    else if(edp == ncs_edp_uns32)
        *p_data_len = (uns32)(*(uns32*)cptr);
    else if(edp == ncs_edp_int32)
        *p_data_len = (uns32)(*(int32*)cptr);
    else if(edp == ncs_edp_double)
        *p_data_len = (uns32)(*(double*)cptr);
    else if(edp == ncs_edp_uns64)
        *p_data_len = (uns32)(*(uns64*)cptr);   /* Downsizing 64-bit to 32-bit */
    else if(edp == ncs_edp_int64)
        *p_data_len = (uns32)(*(int64*)cptr);   /* Downsizing 64-bit to 32-bit */
    else
    {
        /* Failure. */
