...
USRBUF *
ncs_encode_n_octets(USRBUF *u, uns8 *os, unsigned int count)
{ 
   uns8           *p;
...
USRBUF *
ncs_encode_uns8(USRBUF *u, uns8 val8)
  {
  uns8   *p8;
...
USRBUF *
ncs_encode_uns16(USRBUF *u, uns16 val16)
  {
  uns8   *p16;
...
USRBUF *
ncs_encode_uns32(USRBUF *u, uns32 val32)
  {
  uns8   *p32;
...
USRBUF *
ncs_encode_uns64(USRBUF *u, uns64 val64)
  {
  uns8   *p64;
...
USRBUF *
ncs_prepend_n_octets(USRBUF *pbuf, uns8 *os, unsigned int length)
  {
  uns8         *pch;
...
USRBUF *
ncs_prepend_uns16( USRBUF *pbuf, uns16 val16 )
  {
  uns8         *p16;
...
USRBUF *
ncs_prepend_uns32( USRBUF *pbuf, uns32 val32 )
  {
  uns8         *p32;
...
USRBUF *
ncs_prepend_uns64( USRBUF *pbuf, uns64 val64 )
  {
  uns8         *p64;
...
uns64
ncs_encode_64bit( uns8 **stream, uns64 val)
  {
...
uns32
ncs_encode_32bit( uns8 **stream, uns32 val)
  {
  *(*stream)++ = (uns8)(val>>24);
  *(*stream)++ = (uns8)(val>>16);
  *(*stream)++ = (uns8)(val>>8);
...
uns32
ncs_encode_24bit( uns8 **stream, uns32 val)
  {
  *(*stream)++ = (uns8)(val>>16);
  *(*stream)++ = (uns8)(val>>8);
  *(*stream)++ = (uns8)(val);
...
uns32
ncs_encode_16bit( uns8 **stream, uns32 val)
  {
  *(*stream)++ = (uns8)(val>>8);
  *(*stream)++ = (uns8)(val);
  return 2;
...
uns32
ncs_encode_8bit( uns8 **stream, uns32 val)
  {
  *(*stream)++ = (uns8)(val);
  return 1; 
  }
...
uns32
ncs_encode_key(uns8 **stream, NCS_KEY* key)
  {
  uns8 len;
...
uns32
ncs_encode_octets( uns8 **stream, uns8 *val, uns32 count)
  {
  uns32 i;
  for (i=0; i<count; i++)
    *(*stream)++ = *val++;
