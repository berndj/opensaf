...
/***************************************************************************
  ...
  H J R W G   P r i v a t e   D a t a   S t r u c t u r e s

  R W G : Reader Writer Guard

  NOTE: SMM wrote structure and corresponding code for a real 
        'multiple READERs, exclusive WRITER scheme' but its too late in this
        release cycle to deploy.

        MDS was the package targeted for its use.

 ***************************************************************************/
/************************************************************************/
/* NCSRWG_OBJ - this structure is embedded in known, persistent thing    */
/************************************************************************/

typedef struct ncsrwg_obj
  {
  NCS_BOOL      open;   /* Is the object (still) open/available       */
  uns32        reader; /* use-count of clients 'inside' object now   */
  uns32        writer; /* use-count of clients 'inside' object now   */
  uns32        waiting;/* num of readers waiting for writer to leave */ 
  NCS_BOOL      mutex;  /* Mutex currently in use                     */
  void*        semaph; /* Counting semephore....                     */
  NCS_LOCK      lock;   /* let READ-ONLY list fight over whose next   */

  } NCSRWG_OBJ; /* Reader Writer Guard */

/***************************************************************************
 *
 * P u b l i c    R e a d e r  W r i t e r  G u a r d   A P I s
 *
 ***************************************************************************/
EXTERN_C uns32      ncsrwg_give     (NCSRWG_OBJ* pg,
                                    uns32      ret);
EXTERN_C uns32      ncsrwg_give_mtx (NCSRWG_OBJ* pg,
                                    uns32      ret);

EXTERN_C NCS_BOOL    ncsrwg_take     (NCSRWG_OBJ* pg);
EXTERN_C NCS_BOOL    ncsrwg_take_mtx (NCSRWG_OBJ* pg,
                                    uns32      cnt);

EXTERN_C uns32      ncsrwg_create   (NCSRWG_OBJ* pg);
EXTERN_C NCS_BOOL    ncsrwg_destroy  (NCSRWG_OBJ* pg);
