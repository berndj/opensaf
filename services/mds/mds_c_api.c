...
/* STEP 2: IF Policy is MxN *
    ...
    if (info->info.vdest_create.i_policy == NCS_VDEST_TYPE_MxN)
    {
        ...
        if (status != NCSCC_RC_SUCCESS)
        {
            ...
            m_MDS_LOG_ERR("MCM_API : vdest_create : VDEST id = %d can't subscribe : MDTM Returned Failure ",vdest_id);
            m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_vdest_create");
            return NCSCC_RC_FAILURE;
        }
...
/* STEP 3: Call mdtm_vdest_unsubscribe(vdest_id)
    ...
    if (local_vdest_policy == NCS_VDEST_TYPE_MxN)
    {
        ...
        if (status != NCSCC_RC_SUCCESS)
        {
            ...
            m_MDS_LOG_ERR("MCM_API : vdest_create : VDEST id = %d can't Unsubscribe : MDTM Returned Failure",local_vdest_id);
        }
        ...
        if (local_vdest_role == V_DEST_RL_ACTIVE)
        {
            ...
            if (status != NCSCC_RC_SUCCESS)
            {
                ...
                m_MDS_LOG_ERR("MCM_API : vdest_create : VDEST id = %d can't UnInstall : MDTM Returned Failure",local_vdest_id);
            }
        }
...
/* STEP 4: Delete DEST from VDEST-table entry */
    ...
    status = mds_vdest_tbl_del 
            (m_MDS_GET_VDEST_ID_FROM_VDEST_HDL(info->info.vdest_destroy.i_vdest_hdl));
...
*/
        ...
        info->info.vdest_config.i_new_role == V_DEST_RL_QUIESCED)
    {
        ...
        m_MDS_LOG_NOTIFY("MCM_API : vdest_chg_role: VDEST id = %d : Role Changed : Active -> Quiesced", local_vdest_id);
    }
        ...
        info->info.vdest_config.i_new_role == V_DEST_RL_ACTIVE)
    {
        ...
        m_MDS_LOG_NOTIFY("MCM_API : vdest_chg_role: VDEST id = %d : Role Changed : Quiesced -> Active", local_vdest_id);
              ...
              info->info.vdest_config.i_new_role == V_DEST_RL_STANDBY)
    {
        ...
        m_MDS_LOG_NOTIFY("MCM_API : vdest_chg_role : VDEST id = %d : Role Changed : Quiesced -> Standby", local_vdest_id);

    }
    else if (current_role == V_DEST_RL_STANDBY && 
              info->info.vdest_config.i_new_role == V_DEST_RL_QUIESCED)
    {
        m_MDS_LOG_ERR("MCM_API : vdest_chg_role: VDEST id = %d : Role Changed : Standby -> Quiesced : NOT POSSIBLE", local_vdest_id);
        m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_vdest_chg_role");
        return NCSCC_RC_FAILURE;
    }
    else if (current_role == V_DEST_RL_STANDBY && 
            info->info.vdest_config.i_new_role == V_DEST_RL_ACTIVE)
    {
        ...
        m_MDS_LOG_NOTIFY("MCM_API : vdest_chg_role : VDEST id = %d : Role Changed : Standby -> Active", local_vdest_id);
    ...
    {       
        ...
        /* Update the role */
        mds_vdest_tbl_update_role (local_vdest_id, 
                                    info->info.vdest_config.i_new_role, TRUE);
        ...
        m_MDS_LOG_NOTIFY("MCM_API : vdest_chg_role : VDEST id = %d : Role Changed : Active -> Standby", local_vdest_id);

    }
    else if (current_role == info->info.vdest_config.i_new_role)
    {
        m_MDS_LOG_NOTIFY("MCM_API : vdest_chg_role : Role Changed FAILED for VDEST id = %d : OLD_ROLE = NEW_ROLE = %s", local_vdest_id,
            (current_role == V_DEST_RL_STANDBY)?"standby":((current_role == V_DEST_RL_ACTIVE)?"active":"quieced"));
    }
...
uns32 mds_mcm_pwe_create (NCSMDS_ADMOP_INFO * info)
{
    ...
    {   
                        ...
                        info->info.pwe_create.i_pwe_id) == NCSCC_RC_SUCCESS)
        {
            ...
            m_MDS_LOG_ERR("MCM_API : pwe_create : FAILED : PWE id = %d Already exist on ADEST",info->info.pwe_create.i_pwe_id);
            m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_pwe_create");
            return NCSCC_RC_FAILURE;
        }
        else
        {
            ...
            m_MDS_LOG_NOTIFY("MCM_API : PWE id = %d Created Successfully on ADEST",info->info.pwe_create.i_pwe_id);
...
uns32 mds_mcm_pwe_destroy (NCSMDS_ADMOP_INFO * info)
{
    ...
    status = mds_pwe_tbl_query
            (m_MDS_GET_VDEST_HDL_FROM_VDEST_ID(vdest_id), pwe_id);
    if (status == NCSCC_RC_FAILURE)
    {
        /* Pwe doesn't exist */
...
uns32 mds_mcm_adm_pwe_query(NCSMDS_ADMOP_INFO * info)
{
    ...
    status = mds_pwe_tbl_query
            (m_MDS_GET_VDEST_HDL_FROM_PWE_HDL(info->info.pwe_query.i_local_dest_hdl), 
    ...
    { 
        /* It is PWE hdl(pwe+vdest) */
        /* ERROR ERROR ERROR */ 
        m_MDS_LOG_ERR("MCM_API : pwe_query : PWE Query Failed for PWE id = %d as DEST hdl = %llx passed is PWE_HDL",
...
uns32 mds_mcm_svc_install (NCSMDS_INFO *info)
{
    ...
    {
       m_MDS_LOG_ERR("MCM_API : svc_install : FAILED : svc id = %d , Should use the proper scope of installation",info->i_svc_id);
       m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_svc_install");
       return NCSCC_RC_FAILURE;
    }
    ...
    if(info->info.svc_install.i_mds_q_ownership==FALSE)
    {
       ...
       {
          m_MDS_LOG_ERR("MCM_API : svc_install : FAILED : svc id = %d , Should use the MDS Q Ownership model",info->i_svc_id);
          m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_svc_install");
          return NCSCC_RC_FAILURE;
       }
    }
...
/*    STEP 3: Call mdtm_svc_install(svc_hdl) */
    ...
    mds_vdest_tbl_get_role (m_MDS_GET_VDEST_ID_FROM_VDEST_HDL(info->i_mds_hdl), 
                                        &local_vdest_role);
    mds_vdest_tbl_get_policy(m_MDS_GET_VDEST_ID_FROM_VDEST_HDL(info->i_mds_hdl),
                                        &local_vdest_policy);
    ...
    if (local_vdest_role == V_DEST_RL_ACTIVE && local_vdest_id != m_VDEST_ID_FOR_ADEST_ENTRY)
    {
        ...
        if (status != NCSCC_RC_SUCCESS)
        {
            ...
            m_MDS_LOG_ERR("MCM_API : svc_install : Second install for : SVC id = %d on VDEST id = %d FAILED : MDTM returned Failure",
                            info->i_svc_id, m_MDS_GET_VDEST_ID_FROM_PWE_HDL(info->i_mds_hdl));
        }
    }
    ...
    m_MDS_LOG_NOTIFY("MCM_API : SVC id = %d on VDEST id = %d, SVC_PVT_VER = %d Install Successfull",
                    info->i_svc_id, m_MDS_GET_VDEST_ID_FROM_PWE_HDL(info->i_mds_hdl), info->info.svc_install.i_mds_svc_pvt_ver);                                    
...
/* STEP 3: mdtm_svc_uninstall(vdest-id, pwe_id, svc_id, svc_hdl) */
    ...
    if (vdest_role == V_DEST_RL_ACTIVE && local_vdest_id != m_VDEST_ID_FOR_ADEST_ENTRY)
    {
        ...
        {
            /* MDTM can't unsubscribe the service */
            m_MDS_LOG_ERR("MCM_API : svc_install : Second Uninstall for : SVC id = %d on VDEST id = %d FAILED : MDTM returned Failure",
                            info->i_svc_id, m_MDS_GET_VDEST_ID_FROM_PWE_HDL(info->i_mds_hdl));
        }
    }
...
/* STEP 1: Validation pass: 
    ...
    mds_svc_tbl_get_install_scope((MDS_PWE_HDL)info->i_mds_hdl, 
                                            info->i_svc_id,
                                            &install_scope);
    if ((info->info.svc_subscribe.i_scope > install_scope) || (info->info.svc_subscribe.i_scope< NCSMDS_SCOPE_INTRANODE))
    {
        /* Subscription scope is bigger than Install scope */

        m_MDS_LOG_ERR("MCM_API : svc_subscribe : SVC id = %d on VDEST id = %d FAILED : Subscripton Scope Mismatch",
                        info->i_svc_id, m_MDS_GET_VDEST_ID_FROM_PWE_HDL(info->i_mds_hdl));
        m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_svc_subscribe");
        return NCSCC_RC_FAILURE;
    }
...
*/
    ...
    {
        m_MDS_LOG_ERR("MCM_API : svc_subscribe : SVC id = %d on VDEST id = %d gave non zero count and NULL Subscription array",
                  info->i_svc_id, m_MDS_GET_VDEST_ID_FROM_PWE_HDL(info->i_mds_hdl));
        return NCSCC_RC_FAILURE;
    }
    ...
    for (i=0; i<info->info.svc_subscribe.i_num_svcs; i++)
    {
        ...
        if (status == NCSCC_RC_SUCCESS)
        {
            m_MDS_LOG_ERR("MCM_API : svc_subscribe : SVC id = %d on VDEST id = %d Subscription to SVC id = %d FAILED | ALREADY EXIST",
                      info->i_svc_id, m_MDS_GET_VDEST_ID_FROM_PWE_HDL(info->i_mds_hdl),
                        info->info.svc_subscribe.i_svc_ids[i]);
            m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_svc_subscribe");
            return NCSCC_RC_FAILURE;
    ...
    for (i=0; i<info->info.svc_subscribe.i_num_svcs; i++)
    {
        ...
        if (status == NCSCC_RC_NO_CREATION)
        {
            ...
            status = mds_subtn_tbl_change_explicit 
                        (svc_hdl, info->info.svc_subscribe.i_svc_ids[i], (info->i_op) == MDS_SUBSCRIBE ? MDS_VIEW_NORMAL : MDS_VIEW_RED);
            m_MDS_LOG_NOTIFY("MCM_API : svc_subscribe :SVC id = %d on VDEST id = %d Subscription to SVC id = %d : Already Exist Implicitly : Changed to Explicit",
                      info->i_svc_id, m_MDS_GET_VDEST_ID_FROM_PWE_HDL(info->i_mds_hdl),
                        info->info.svc_subscribe.i_svc_ids[i]);
        }
        else
        { /* status = NCSCC_RC_FAILURE (It can't be SUCCESS, as it is verified in Validation) */ 
            /* Add new Subscription entry */
            status = mds_mcm_subtn_add (svc_hdl, 
                                info->info.svc_subscribe.i_svc_ids[i],
                                info->info.svc_subscribe.i_scope,
                                view,
                                MDS_SUBTN_EXPLICIT);
            if (status != NCSCC_RC_SUCCESS)
            {
                /* There was some problem while subscribing so rollback all previous subscriptions */
                m_MDS_LOG_ERR("MCM_API : svc_subscribe :SVC id = %d on VDEST id = %d Subscription to SVC id = %d Failed",
                      info->i_svc_id, m_MDS_GET_VDEST_ID_FROM_PWE_HDL(info->i_mds_hdl),
                        info->info.svc_subscribe.i_svc_ids[i]);
        ...
        m_MDS_LOG_NOTIFY("MCM_API : svc_subscribe :SVC id = %d on VDEST id = %d Subscription to SVC id = %d Successful",
                      info->i_svc_id, m_MDS_GET_VDEST_ID_FROM_PWE_HDL(info->i_mds_hdl),
                        info->info.svc_subscribe.i_svc_ids[i]);
    }
...
uns32 mds_mcm_svc_unsubscribe (NCSMDS_INFO *info)
{
    ...
    for (i=0; i<info->info.svc_cancel.i_num_svcs; i++)
    {
        ...
        if (status == NCSCC_RC_FAILURE)
        {
            m_MDS_LOG_ERR("MCM_API : svc_unsubscribe : SVC id = %d on VDEST id = %d Unsubscription to SVC id = %d FAILED : Not Subscribed",
                      info->i_svc_id, m_MDS_GET_VDEST_ID_FROM_PWE_HDL((MDS_PWE_HDL)info->i_mds_hdl),
    ...
    for (i=0; i<info->info.svc_cancel.i_num_svcs; i++)
    {
        ...
        status = NCSCC_RC_SUCCESS;
        status = mds_subtn_tbl_get_ref_hdl(svc_hdl, info->info.svc_cancel.i_svc_ids[i],
                                           &subscr_req_hdl);
        if (status == NCSCC_RC_FAILURE)
        {
        ...
        m_MDS_LOG_NOTIFY("MCM_API : svc_unsubscribe : SVC id = %d on VDEST id = %d Unsubscription to SVC id = %d Successful",
                      info->i_svc_id, m_MDS_GET_VDEST_ID_FROM_PWE_HDL((MDS_PWE_HDL)info->i_mds_hdl),
                        info->info.svc_cancel.i_svc_ids[i]);
    }    
...
uns32 mds_mcm_dest_query (NCSMDS_INFO *info)
{
    ...
    if (info->info.query_dest.o_node_id == 0)
    {/* Destination is VDEST */
        ...
        if (info->info.query_dest.i_query_for_role == TRUE)
        {/* Return Role given Anchor */
            status = mds_subtn_res_tbl_get_by_adest (local_svc_hdl, info->info.query_dest.i_svc_id,
                                                    ...
                                                    &subtn_result_info);
            if (status == NCSCC_RC_FAILURE)
            {/* No such subscription result present */
                m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_dest_query");
                return NCSCC_RC_FAILURE;
            }
        }
        else
        {/* Return Anchor given Role */
            ...
            while (status != NCSCC_RC_FAILURE)
            {
                ...
                {/* Subscription result table entry is VDEST entry */

                    if (subtn_result_info->key.vdest_id == (MDS_VDEST_ID)info->info.query_dest.i_dest &&
                        subtn_result_info->info.vdest_inst.role == info->info.query_dest.info.query_for_anc.i_vdest_rl)
                    {
                        info->info.query_dest.info.query_for_anc.o_anc = (V_DEST_QA)subtn_result_info->key.adest;
                        break;
                    }
                }
                status = mds_subtn_res_tbl_getnext_any (local_svc_hdl, info->info.query_dest.i_svc_id, 
            ...
            if (status == NCSCC_RC_FAILURE)
            {/* while terminated as it didnt find any result */
                m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_dest_query");
                return NCSCC_RC_FAILURE;
            }
        }
        ...
        m_MDS_LOG_NOTIFY("MCM_API : dest_query : Successful for DEST = %llx",
...
uns32 mds_mcm_pwe_query (NCSMDS_INFO *info)
{
    ...
    info->info.query_pwe.o_pwe_id = 
             (PW_ENV_ID)m_MDS_GET_PWE_ID_FROM_PWE_HDL((MDS_PWE_HDL)info->i_mds_hdl);
    ...
    m_MDS_LOG_NOTIFY("MCM_API : query_pwe : Successful for PWE hdl = %lx",
                                            info->i_mds_hdl);
                             ...
                             
{
    ...
    m_MDS_LOG_DBG("MCM_API : REMOTE SVC INFO : SVC id = %d | PWE id = %d | VDEST id = %d | VDEST_POLICY = %d | INSTALL SCOPE = %d | ROLE = %d | MY_PCON = %d |",
...
*/
    ...
    status = mds_subtn_tbl_get_details (local_svc_hdl, svc_id, 
    ...
    if (status == NCSCC_RC_FAILURE)
    {/* Subscription result tabel entry doesn't exist */
        ...
        {/* Remote svc is on ADEST */
            ...
            mds_subtn_res_tbl_add (local_svc_hdl, svc_id, 
                                            ...
                                            scope, NCS_VDEST_TYPE_N_WAY_ROUND_ROBIN,
                                            svc_sub_part_ver,
                                            archword_type);
            ...
            m_MDS_LOG_NOTIFY("MCM_API : svc_up : SVC id = %d on DEST id = %d got UP for SVC id = %d on ADEST <0x%08x, %u>, rem_svc_pvt_ver=%d, rem_svc_archword=%d",
                            m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl),
                            m_MDS_GET_VDEST_ID_FROM_SVC_HDL(local_svc_hdl),
                            svc_id, m_MDS_GET_NODE_ID_FROM_ADEST(adest), m_MDS_GET_PROCESS_ID_FROM_ADEST(adest),svc_sub_part_ver,archword_type);
        }
        else
            ...
            if (vdest_policy == NCS_VDEST_TYPE_MxN)
            {
                if (role == V_DEST_RL_ACTIVE)
                {
                    ...
                    {/* No active present */
                        ...
                        m_MDS_LOG_NOTIFY("MCM_API : svc_up : SVC id = %d on DEST id = %d got UP for SVC id = %d on VDEST id = %d ADEST = <0x%08x, %u>, rem_svc_pvt_ver=%d, rem_svc_archword=%d",
                                m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl),
                                m_MDS_GET_VDEST_ID_FROM_SVC_HDL(local_svc_hdl),
                        ...
                        if (local_subtn_view == MDS_VIEW_RED)
                        {
                            status = NCSCC_RC_SUCCESS;
                            status = mds_mcm_user_event_callback(local_svc_hdl, pwe_id, svc_id, 
                                                                 role, vdest_id, adest, NCSMDS_RED_UP,
                                                                 svc_sub_part_ver,
                                                                 archword_type);
                            if (status != NCSCC_RC_SUCCESS)
                            {
                                /* Callback failure */
                                m_MDS_LOG_ERR("MCM_API : svc_up : RED_UP Callback Failure for SVC id = %d", 
                                                    m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl));
                                m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_svc_up");
                                return NCSCC_RC_FAILURE;

                            }
                            m_MDS_LOG_NOTIFY("MCM_API : svc_up : SVC id = %d on DEST id = %d got RED_UP for SVC id = %d on VDEST id = %d ADEST = <0x%08x, %u>, rem_svc_pvt_ver=%d, rem_svc_archword=%d",
                                    m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl),
                                    m_MDS_GET_VDEST_ID_FROM_SVC_HDL(local_svc_hdl),
                                    svc_id, vdest_id, m_MDS_GET_NODE_ID_FROM_ADEST(adest), m_MDS_GET_PROCESS_ID_FROM_ADEST(adest),svc_sub_part_ver,archword_type);
                        }
                    ...
                    {/* Active/Await-active already present */
                        ...
                        if (tmr_running == TRUE)
                        {
                            ...
                            {
                                /* Callback failure */
                                m_MDS_LOG_ERR("MCM_API : svc_up : NEW_ACTIVE Callback Failure for SVC id = %d",
                                                        m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl));
                                m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_svc_up");
                                return NCSCC_RC_FAILURE;

                            }
                            m_MDS_LOG_NOTIFY("MCM_API : svc_up : SVC id = %d on DEST id = %d got NEW_ACTIVE for SVC id = %d on VDEST id = %d ADEST = <0x%08x, %u>, rem_svc_pvt_ver=%d, rem_svc_archword=%d",
                                    m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl),
                                    m_MDS_GET_VDEST_ID_FROM_SVC_HDL(local_svc_hdl),
                                    svc_id, vdest_id, m_MDS_GET_NODE_ID_FROM_ADEST(adest), m_MDS_GET_PROCESS_ID_FROM_ADEST(adest),svc_sub_part_ver,archword_type);
                        }
                ...
                {/* role == V_DEST_RL_STANDBY */
                    ...
                    {/* local_subtn_view = MDS_VIEW_RED */
                        ...
                        {
                            /* Callback failure */
                            m_MDS_LOG_ERR("MCM_API : svc_up : RED_UP Callback Failure for SVC_ID = %d", 
                                                        m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl));
                            m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_svc_up");                            
                            return NCSCC_RC_FAILURE;
            ...
            if (vdest_policy == NCS_VDEST_TYPE_MxN)
            {
                if (role == V_DEST_RL_ACTIVE)
                {
                    ...
                    {/* No other active entry exist */
                        ...
                        if (dest_role == V_DEST_RL_STANDBY)
                        {
                            ...
                            if (local_subtn_view == MDS_VIEW_RED)
                            {
                                status = NCSCC_RC_SUCCESS;
                                status = mds_mcm_user_event_callback(local_svc_hdl, pwe_id, svc_id, 
                                                                     role, vdest_id, adest, NCSMDS_CHG_ROLE,
                                                                     svc_sub_part_ver,
                                                                     archword_type);
                                if (status != NCSCC_RC_SUCCESS)
                                {
                                    /* Callback failure */
                                    m_MDS_LOG_ERR("MCM_API : svc_up : CHG_ROLE Callback Failure for SVC id = %d",
                                                        m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl));
                                    m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_svc_up");
                                    return NCSCC_RC_FAILURE;

                                }
                                m_MDS_LOG_NOTIFY("MCM_API : svc_up : SVC id = %d on DEST id = %d got CHG_ROLE for SVC id = %d on VDEST id = %d ADEST = <0x%08x, %u>, rem_svc_pvt_ver=%d, rem_svc_archword=%d",
                                        m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl),
                                        m_MDS_GET_VDEST_ID_FROM_SVC_HDL(local_svc_hdl),
                                        svc_id, vdest_id, m_MDS_GET_NODE_ID_FROM_ADEST(adest), m_MDS_GET_PROCESS_ID_FROM_ADEST(adest),svc_sub_part_ver,archword_type);
                            }
                    ...
                    {/* Active or Await Active entry exist */
                        ...
                        {/* Some other Active or Await entry exist */
                            ...
                            if ((tmr_running == TRUE)||(local_subtn_view == MDS_VIEW_NORMAL))
                            {
                                /* Call user callback UP */
                                status = NCSCC_RC_SUCCESS;
                                status = mds_mcm_user_event_callback(local_svc_hdl, pwe_id, svc_id, 
                ...
                {/* role == V_DEST_RL_STANDBY */
                    ...
                    {/* This entry is going down */
                        ...
                        {
                            /* Callback failure */
                            m_MDS_LOG_ERR("MCM_API : svc_up : NO_ACTIVE Callback Failure for SVC id = %d",
                                                        m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl));
                            m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_svc_up");
                            return NCSCC_RC_FAILURE;

                        }
                        m_MDS_LOG_NOTIFY("MCM_API : svc_up : SVC id = %d on DEST id = %d got NO_ACTIVE for SVC id = %d on VDEST id = %d ADEST = <0x%08x, %u>, rem_svc_pvt_ver=%d",
                                m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl),
                                m_MDS_GET_VDEST_ID_FROM_SVC_HDL(local_svc_hdl),
                                svc_id, vdest_id, m_MDS_GET_NODE_ID_FROM_ADEST(adest), m_MDS_GET_PROCESS_ID_FROM_ADEST(adest),svc_sub_part_ver);
                ...
                if (role == V_DEST_RL_ACTIVE)
                {
                    ...
                    {/* dest_role == V_DEST_RL_STANDBY */
                        ...
                        {/* Active or Await active entry exists */
                            ...
                            if (tmr_running == TRUE)
                            {/* If Await active exist then point to this one as active */
                ...
                {/* role == V_DEST_RL_STANDBY */
                    ...
                    {/* dest_role == V_DEST_RL_ACTIVE */
                        ...
                        {/* Active or Await active entry exists */
                            ...
                            if (tmr_running == TRUE)
                            {/* Timer is running so active entry points to NULL */
                                /* Do nothing */
                            }
                            else
                            {/* Active entry exist */
                                ...
                                if (active_subtn_result_info == subtn_result_info)
                                {
                                    ...
                                    status = mds_subtn_res_tbl_query_next_active(local_svc_hdl,
                                           svc_id, vdest_id, subtn_result_info, &next_active_result_info);

                                    if (subtn_result_info == next_active_result_info)
                                    {
...
*/
    ...
    {/* Entry exist in subscription result table */
        ...
        if (vdest_id == m_VDEST_ID_FOR_ADEST_ENTRY)
        {
            ...
            m_MDS_LOG_NOTIFY("MCM_API : svc_down : SVC id = %d on DEST id = %d got DOWN for SVC id = %d on ADEST = <0x%08x, %u>, rem_svc_pvt_ver=%d",
                    m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl),
                    m_MDS_GET_VDEST_ID_FROM_SVC_HDL(local_svc_hdl),
                    svc_id,  m_MDS_GET_NODE_ID_FROM_ADEST(adest), m_MDS_GET_PROCESS_ID_FROM_ADEST(adest),svc_sub_part_ver);
        ...
        {/* Remote svc is on VDEST */
            ...
            {/* Role Mismatch */
                ...
                /* so Discard */
                m_MDS_LOG_INFO("MCM_API : svc_down : ROLE MISMATCH : SVC id = %d on DEST id = %d got DOWN for SVC id = %d on VDEST id = %d on ADEST = <0x%08x, %u>, rem_svc_pvt_ver=%d, rem_svc_archword=%d",
                                m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl),
                                m_MDS_GET_VDEST_ID_FROM_SVC_HDL(local_svc_hdl),
                                svc_id, vdest_id, m_MDS_GET_NODE_ID_FROM_ADEST(adest), m_MDS_GET_PROCESS_ID_FROM_ADEST(adest),svc_sub_part_ver,archword_type);
            ...
            if (role == V_DEST_RL_ACTIVE)
            {
                ...
                if (active_adest == adest)
                {
                    if (vdest_policy == NCS_VDEST_TYPE_MxN)
                    {
                        ...
                        {
                            /* Callback failure */
                            m_MDS_LOG_ERR("MCM_API : svc_down : NO_ACTIVE Callback Failure for SVC id = %d",
                                                        m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl));
                            m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_svc_down");
                            return NCSCC_RC_FAILURE;
                        ...
                        m_MDS_LOG_NOTIFY("MCM_API : svc_down : SVC id = %d on DEST id = %d got NO_ACTIVE for SVC id = %d on VDEST id = %d ADEST = <0x%08x, %u>, rem_svc_pvt_ver=%d",
                                m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl),
                                m_MDS_GET_VDEST_ID_FROM_SVC_HDL(local_svc_hdl),
                                svc_id, vdest_id, m_MDS_GET_NODE_ID_FROM_ADEST(adest), m_MDS_GET_PROCESS_ID_FROM_ADEST(adest),svc_sub_part_ver);
                ...
                if (local_subtn_view == MDS_VIEW_RED)
                {
                    ...
                    {
                        /* Callback failure */
                        m_MDS_LOG_ERR("MCM_API : svc_down : RED_DOWN Callback Failure for SVC id = %d",
                                                        m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl));
                        m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_svc_down");
                        return NCSCC_RC_FAILURE;

                    }
                    m_MDS_LOG_NOTIFY("MCM_API : svc_down : SVC id = %d on DEST id = %d got RED_DOWN for SVC id = %d on VDEST id = %d on ADEST = <0x%08x, %u>, rem_svc_pvt_ver=%d",
                            m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl),
                            m_MDS_GET_VDEST_ID_FROM_SVC_HDL(local_svc_hdl),
                            svc_id, vdest_id, m_MDS_GET_NODE_ID_FROM_ADEST(adest), m_MDS_GET_PROCESS_ID_FROM_ADEST(adest),svc_sub_part_ver);
                }
            ...
            {/* role == V_DEST_RL_STANDBY */
                ...
                {/* local_subtn_view = MDS_VIEW_RED */
                    ...
                    m_MDS_LOG_NOTIFY("MCM_API : svc_down : SVC id = %d on DEST id = %d got RED_DOWN for SVC id = %d on VDEST id = %d ADEST = <0x%08x, %u>, rem_svc_pvt_ver=%d",
                            m_MDS_GET_SVC_ID_FROM_SVC_HDL(local_svc_hdl),
                            m_MDS_GET_VDEST_ID_FROM_SVC_HDL(local_svc_hdl),
                            svc_id, vdest_id, m_MDS_GET_NODE_ID_FROM_ADEST(adest), m_MDS_GET_PROCESS_ID_FROM_ADEST(adest),svc_sub_part_ver);
                }
...
uns32 mds_mcm_vdest_up(MDS_VDEST_ID vdest_id, MDS_DEST adest)
{
    ...
    if (adest == gl_mds_mcm_cb->adest)
    {/* Got up from this vdest itself */
        /* Discard */
                                    ...
                                    MDS_SVC_ARCHWORD_TYPE    archword_type )
{
    ...
    if (local_subtn_info->tmr_flag == TRUE && 
        (event_type == NCSMDS_UP || event_type == NCSMDS_RED_UP)) 
    {/* Subscription timer is running */
        ...
        while (curr_queue_element != NULL)
        {
                ...
                curr_queue_element->send_type != MDS_SENDTYPE_RBCAST)
            {
                ...
                {/* We got UP from ADEST so check if any send to svc on this ADEST is remaining */
                    ...
                    if (curr_queue_element->vdest == m_VDEST_ID_FOR_ADEST_ENTRY)
                    {/* Blocked send is to send to ADEST */

                        if (adest == curr_queue_element->adest)
                        {/* Raise selection object for this entry as we got up from that adest */
                            m_NCS_SEL_OBJ_IND(curr_queue_element->sel_obj);    
                            m_MDS_LOG_INFO("MCM_API : event_callback : Raising SEL_OBJ for Send to ADEST <0x%08x, %u>",
                                curr_queue_element->vdest, m_MDS_GET_NODE_ID_FROM_ADEST(curr_queue_element->adest), 
                                                            m_MDS_GET_PROCESS_ID_FROM_ADEST(curr_queue_element->adest));
                        }
                    }
                }
                else
                {/* We got up for VDEST so check if any send to svc on this VDEST is remaining */

                    if (curr_queue_element->adest == 0 && curr_queue_element->vdest == vdest_id 
                                                && event_type == NCSMDS_UP)
                    {/* Blocked send is send to VDEST and we got atleast one ACTIVE */
                        m_NCS_SEL_OBJ_IND(curr_queue_element->sel_obj);  
                        m_MDS_LOG_INFO("MCM_API : event_callback : Raising SEL_OBJ for Send to VDEST id = %d on ADEST <0x%08x, %u>",
                            curr_queue_element->vdest, m_MDS_GET_NODE_ID_FROM_ADEST(curr_queue_element->adest), 
                                                        m_MDS_GET_PROCESS_ID_FROM_ADEST(curr_queue_element->adest));
                    }
                }
...
*********************************************************/
uns32 mds_mcm_await_active_tmr_expiry(MDS_SVC_HDL svc_hdl, MDS_SVC_ID sub_svc_id, 
                                                MDS_VDEST_ID vdest_id)
{
    ...
    mds_await_active_tbl_del
        (active_subtn_result_info->info.active_vdest.active_route_info->await_active_queue);
    ...
    status = mds_mcm_user_event_callback(svc_hdl, m_MDS_GET_PWE_ID_FROM_SVC_HDL(svc_hdl), 
                                ...
                                vdest_id, 0, NCSMDS_DOWN,active_subtn_result_info->rem_svc_sub_part_ver, MDS_SVC_ARCHWORD_TYPE_UNSPECIFIED);
    if (status != NCSCC_RC_SUCCESS)
    {
        /* Callback failure */
        m_MDS_LOG_ERR("MCM_API : await_active_tmr_expiry : DOWN Callback Failure for SVC id = %d subscribed to SVC id = %d on VDEST id = %d",
            m_MDS_GET_SVC_ID_FROM_SVC_HDL(svc_hdl),
            m_MDS_GET_VDEST_ID_FROM_SVC_HDL(svc_hdl),
            sub_svc_id, vdest_id);
    }

    m_MDS_LOG_NOTIFY("MCM_API : svc_down : await_active_tmr_expiry : SVC id = %d on DEST id = %d got DOWN for SVC id = %d on VDEST id = %d",
            m_MDS_GET_SVC_ID_FROM_SVC_HDL(svc_hdl),
            m_MDS_GET_VDEST_ID_FROM_SVC_HDL(svc_hdl),
            sub_svc_id, vdest_id);
    ...
    /* Destroy timer */
    m_NCS_TMR_DESTROY
        (active_subtn_result_info->info.active_vdest.active_route_info->await_active_tmr);
                                ...
                                MDS_VIEW view, MDS_SUBTN_TYPE subtn_type)
{
    ...
    if (status != NCSCC_RC_SUCCESS)
    {
        ...
        m_MDS_LOG_ERR("MCM_API : mcm_subtn_add : Can't Subscribe from SVC id = %c on DEST = %d to SVC id = %d : MDTM Returned Failure",
                            m_MDS_GET_SVC_ID_FROM_SVC_HDL(svc_hdl), 
                            m_MDS_GET_VDEST_ID_FROM_SVC_HDL(svc_hdl) , subscr_svc_id);
                            ...
                            MDS_DEST remote_adest, MDS_SVC_ID remote_svc_id, NCS_BOOL my_pcon)
{
    ...
    case NCSMDS_SCOPE_NONE:
        ...
        {
            m_MDS_LOG_NOTIFY("MCM_API : svc_up : NONE or CHASSIS Scope Mismatch (remote scope = NODE) for SVC id = %d",remote_svc_id);
            m_MDS_LOG_DBG("MCM_API : Leaving : F : mds_mcm_validate_scope");
            return NCSCC_RC_FAILURE;
        }
        break;
