...
static uns32 mdtm_process_recv_message_common(uns8 flag, uns8 *buffer, uns16 len, uns64 tipc_id, uns32 seq_num_check, uns32 *buff_dump);
...
uns32 mdtm_tipc_destroy(void)
{
    ...
    reassem_queue = (MDTM_REASSEMBLY_QUEUE *)ncs_patricia_tree_getnext
                (&mdtm_reassembly_list,(uns8 *)NULL);
...
/* As the timeout passed in infinite, No need to check for the Timeout*/
...
#endif
            {
...
#if MDS_TIPC_1_5
                if (recd_bytes == 0)
                {  /* As we had disabled the feature of receving the bounced messages, recd_bytes==0 indicates a fatal condition so assert and exit*/
                   m_MDS_LOG_CRITICAL("MDTM: recd bytes=0 on receive sock, fatal condition. Exiting the process");
                   assert(0);
                   exit(1);
                }
#endif
                ...
                {
                    m_MDS_LOG_ERR("MDTM: Error Recd:tipc_id=<0x%08x:%u>:errno=0x%08x",client_addr.addr.id.node,client_addr.addr.id.ref, errno);
                }
#else
                /* Added for the rebounced message print */
                if ((pfd[1].revents & TIPC_UNDELIVERED_MSG) == TIPC_UNDELIVERED_MSG)
                {
                    m_MDS_LOG_ERR("MDTM: Recd the bounced message from:tipc_id=<0x%08x:%u>:errno=0x%08x",client_addr.addr.id.node,client_addr.addr.id.ref, errno);
                }
#endif
                else if(recd_buf_len==recd_bytes)
                {
                   ...
                   {                  
                       ...
                       {
                          m_MDS_LOG_ERR("CHECKSUM-MISMATCH:recvd_on_sock=%d, TIPC-ID=0x%016llx, ADEST=<%08x,%u>", recd_bytes, tipc_id,m_MDS_GET_NCS_NODE_ID_FROM_TIPC_NODE_ID(client_addr.addr.id.node),client_addr.addr.id.ref);
                       ...
                       if (buff_dump)
                       {
                          m_MDS_LOG_ERR("RECV_DATA_PROCESS:recvd_on_sock=%d, TIPC-ID=0x%016llx, ADEST=<%08x,%u>", recd_bytes, tipc_id,m_MDS_GET_NCS_NODE_ID_FROM_TIPC_NODE_ID(client_addr.addr.id.node),client_addr.addr.id.ref);
                          mds_buff_dump(inbuf, recd_bytes, 100);
                       }
                   }
                   else
                ...
                else
                {
                   ...
                   m_MDS_LOG_ERR("LEN-MISMATCH:recvd_on_sock=%d, size_in_mds_hdr=%d,  TIPC-ID=0x%016llx, ADEST=<%08x,%u>", 
                                                  recd_bytes, recd_buf_len, tipc_id,m_MDS_GET_NCS_NODE_ID_FROM_TIPC_NODE_ID(client_addr.addr.id.node),client_addr.addr.id.ref);
                   mds_buff_dump(inbuf, recd_bytes, 100);
                }
            }
    ...
    */
    ...
    switch (inst_type)
    {
        case MDS_SVC_INST_TYPE:
        {
            ...
            archword_type=(MDS_SVC_ARCHWORD_TYPE)((event.found_lower&MDS_ARCHWORD_MASK)>>(LEN_4_BYTES-MDS_ARCHWORD_BITS_LEN)); 
            svc_sub_part_ver=(MDS_SVC_PVT_SUB_PART_VER)((event.found_lower&MDS_VER_MASK)>>(LEN_4_BYTES-MDS_VER_BITS_LEN-MDS_ARCHWORD_BITS_LEN)); 
            pwe_id=(event.s.seq.type>>MDS_EVENT_SHIFT_FOR_PWE)&MDS_EVENT_MASK_FOR_PWE;
            policy=(event.found_lower&MDS_POLICY_MASK)>>(LEN_4_BYTES-MDS_ARCHWORD_BITS_LEN-MDS_VER_BITS_LEN-VDEST_POLICY_LEN);
            role=(event.found_lower&MDS_ROLE_MASK)>>(LEN_4_BYTES-MDS_ARCHWORD_BITS_LEN-MDS_VER_BITS_LEN-VDEST_POLICY_LEN-ACT_STBY_LEN);
            scope=(event.found_lower&MDS_SCOPE_MASK)>>(LEN_4_BYTES-MDS_ARCHWORD_BITS_LEN-MDS_VER_BITS_LEN-VDEST_POLICY_LEN-ACT_STBY_LEN-MDS_SCOPE_LEN);
            ...
            {
               m_MDS_LOG_ERR("MDTM: Dropping  the svc event for SVC id = %d, subscribed by SVC id = %d as the TIPC NODEid is not in the prescribed range=0x%08x, SVC Event type=%d",svc_id, m_MDS_GET_SVC_ID_FROM_SVC_HDL(svc_hdl), event.port.node, discovery_event); 
                return NCSCC_RC_FAILURE;
            }
            ...
            {
               m_MDS_LOG_INFO("MDTM: Raising the svc up event for SVC id = %d, subscribed by SVC id = %d",
            ...
            {
               m_MDS_LOG_INFO("MDTM: Raising the svc down event for SVC id = %d, subscribed by SVC id = %d\n",
                                   svc_id, m_MDS_GET_SVC_ID_FROM_SVC_HDL(svc_hdl));
            ...
            {
                m_MDS_LOG_INFO("MDTM: TIPC EVENT UNSUPPORTED for SVC (other than Publish and Withdraw)\n");
                return NCSCC_RC_FAILURE;
            }
        }
        break;
        ...
        case MDS_VDEST_INST_TYPE:
        {
            ...
            {
               m_MDS_LOG_ERR("MDTM: Dropping  the vdest event, vdest id = %d, as the TIPC NODEid is not in the prescribed range=0x%08x,  Event type=%d",vdest_id, event.port.node, discovery_event); 
                return NCSCC_RC_FAILURE;
            }
            ...
            if (TIPC_PUBLISHED == discovery_event)
            {
               ...
               {
                  m_MDS_LOG_ERR("MDTM: VDEST UP event processing failed for VDEST id = %d", vdest_id);
                  return NCSCC_RC_FAILURE;
               }
              /* Return from here */
               return NCSCC_RC_SUCCESS;
        ...
        default:
        {
           m_MDS_LOG_ERR("MDTM: TIPC EVENT UNSUPPORTED (default). If this case comes this should assert as there no other events being processed");
           return NCSCC_RC_FAILURE;
        }
        break;
    }
...
static uns32 mdtm_process_recv_data(uns8 *buffer, uns16 len, uns64 tipc_id, uns32 *buff_dump)
{
    ...
    */
    ...
    {
       return mdtm_process_recv_message_common(MDTM_DIRECT, &buffer[MDTM_FRAG_HDR_LEN], len-MDTM_FRAG_HDR_LEN, tipc_id, temp_frag_seq_num, buff_dump);
    }
    else
    {
        ...
        m_MDS_LOG_DBG("MDTM: Recd message with Fragment Seqnum=%d, frag_num=%d, from src_Tipc_id=<0x%08x:%u>", seq_num,frag_num,id.node,id.ref);
        ...
        if(reassem_queue!=NULL)
        {
            ...
            {
               m_MDS_LOG_ERR("MDTM: No payload data present in fragmented msg or incomplete frag hdr=%d",len);
               return NCSCC_RC_FAILURE;
            }
            ...
            if (reassem_queue->to_be_dropped==TRUE) 
            {
                ...
                if(((pkt_type&0x7fff)>0) && ((pkt_type&0x8000)==0))
                {
                    ...
                    mdtm_del_reassemble_queue(reassem_queue->key.frag_sequence_num,reassem_queue->key.tipc_id);
                }
            ...
            if(reassem_queue->next_frag_num==frag_num)
            {
                                    ...
                                    /* fUNTION WILL RETURN success when svc is in active or quiesced*/
                {
                    switch(reassem_queue->recv.snd_type)
                    {
                        ...
                        case MDS_SENDTYPE_BCAST:
                        {
                            ...
                            m_MDS_LOG_ERR("MDTM: Message is dropped as msg is destined to standby svc\n");
                            return NCSCC_RC_FAILURE;
                        }
                            break;
                        default:
                ...
                if (reassem_queue->recv.msg.encoding==MDS_ENC_TYPE_FLAT )
                {
                    ...
                    ncs_encode_n_octets_in_uba(&ub, &buffer[MDTM_FRAG_HDR_LEN], (len-MDTM_FRAG_HDR_LEN));
                ...
                if(((pkt_type&0x7fff)>0) && ((pkt_type&0x8000)==0))
                {
                    ...
                    if(reassem_queue->recv.msg.encoding==MDS_ENC_TYPE_FLAT)
                    {
                       ncs_dec_init_space(&reassem_queue->recv.msg.data.flat_uba, reassem_queue->recv.msg.data.flat_uba.start);
                    }
                    else if(reassem_queue->recv.msg.encoding==MDS_ENC_TYPE_FULL)
                    {
                       ncs_dec_init_space(&reassem_queue->recv.msg.data.fullenc_uba, reassem_queue->recv.msg.data.fullenc_uba.start);
                    }
        ...
        {
            return mdtm_process_recv_message_common(MDTM_REASSEMBLE, buffer, len, tipc_id, temp_frag_seq_num, buff_dump);
        }
        else
        {                  
            *buff_dump = 0;
...
static uns32 mdtm_process_recv_message_common(uns8 flag, uns8 *buffer, uns16 len, uns64 tipc_id, uns32 seq_num_check, uns32 *buff_dump)
{
    ...
    if(NCSCC_RC_SUCCESS==node_status)
    {
       adest=((((uns64)(m_MDS_GET_NCS_NODE_ID_FROM_TIPC_NODE_ID((NODE_ID)(tipc_id>>32))))<<32)|(uns32)(tipc_id));
    }
    else
    {
      m_MDS_LOG_ERR("MDTM: Dropping  the recd message, as the TIPC NODEid is not in the prescribed range=0x%08x",(NODE_ID)(tipc_id>>32)); 
      return NCSCC_RC_FAILURE;
    }
...
/* Protocol check is also ignored, as in future this field might be used for some other purpose */
        ...
        if(len<len_mds_hdr)
        {
           m_MDS_LOG_ERR("MDTM: Message recd (Non Fragmented) len is less than the MDS HDR len  TIPC_ID=<0x%llx>", tipc_id);
           return NCSCC_RC_FAILURE;
        }
        /* Check whether mds header length received is not less than mds header length of this instance */
        if(len_mds_hdr<MDS_HDR_LEN)
        {
           m_MDS_LOG_DBG("MDTM:Mds hdr len of recd msg (Non frag) = %d is less than local mds hdr len = %d", len_mds_hdr, MDS_HDR_LEN);
           return NCSCC_RC_FAILURE;
        }
        ...
        if(mds_svc_tbl_get_role(dest_svc_hdl)!=NCSCC_RC_SUCCESS)
        {
            switch(msg_snd_type)
            {
                ...
                case MDS_SENDTYPE_SNDACK:
                case MDS_SENDTYPE_BCAST:
                    m_MDS_LOG_ERR("MDTM: Recd Message SVC is in standby so dropping the message:Dest-Svc = %d,%d\n",dest_svc_id, dest_vdest_id);
                    return NCSCC_RC_FAILURE;
                    break;
                default:
                    break;
        ...
        m_MDS_LOG_DBG("MDTM: Recd Unfragmented message with SVC Seq num =%d, from src_Tipc_id=<%llx>", svc_seq_num,tipc_id);
        ...
        if(msg_snd_type==MDS_SENDTYPE_ACK)
        {
            ...
            if (len != len_mds_hdr)
            {
                ...
                m_MDS_LOG_ERR("MDTM: ACK message contains payload data, Total Len=%d,  len_mds_hdr=%d",len,len_mds_hdr);
                m_MMGR_FREE_REASSEM_QUEUE(reassem_queue);
                return NCSCC_RC_FAILURE;
            }
        }
        else
        {
           ...
           if(NCSCC_RC_SUCCESS!=mdtm_fill_data(reassem_queue, &buffer[len_mds_hdr], (len-(len_mds_hdr)), enc_type))
           {
              m_MDS_LOG_ERR("MDTM: Unable to process the recd message due to prob in mdtm_fill_data\n");
              m_MMGR_FREE_REASSEM_QUEUE(reassem_queue);
              return NCSCC_RC_FAILURE;
           }
...
/* Protocol check is also ignored, as in future this field might be used for some other purpose */
        ...
        {
           m_MDS_LOG_ERR("MDTM: Message recd (Fragmented First Pkt) len is less than or equal to the sum of (len_mds_hdr+MDTM_FRAG_HDR_LEN) len  TIPC_ID=<0x%llx>", tipc_id);
           return NCSCC_RC_FAILURE;
        }
        /* Check whether mds header length received is not less than mds header length of this instance */
        if(len_mds_hdr<MDS_HDR_LEN)
        {
           m_MDS_LOG_DBG("MDTM:Mds hdr len of recd msg(Frag first pkt) = %d is less than local mds hdr len = %d", len_mds_hdr, MDS_HDR_LEN);
           return NCSCC_RC_FAILURE;
        }
        ...
        m_MDS_LOG_DBG("MDTM: Recd fragmented message(first frag) with Frag Seqnum=%d SVC Seq num =%d, from src_Tipc_id=<%llx>", seq_num, svc_seq_num,tipc_id);
        ...
        {
           m_MDS_LOG_ERR("MDTM: No Payload Data present, Total Len=%d, sum of frag_hdr and mds_hdr=%d",len,(len_mds_hdr+MDTM_FRAG_HDR_LEN));
           mdtm_del_reassemble_queue(seq_num,id);
           return NCSCC_RC_FAILURE;
        }
        ...
        reassem_queue->tmr_hdl=ncshm_create_hdl(NCS_HM_POOL_ID_COMMON,NCS_SERVICE_ID_COMMON, (NCSCONTEXT)(reassem_queue->tmr_info)); 
        
        m_NCS_TMR_START(reassem_queue->tmr, MDTM_REASSEMBLE_TMR_VAL,
                                        (TMR_CALLBACK)mds_tmr_callback, 
                                        (void *)(long)(reassem_queue->tmr_hdl));
        m_MDS_LOG_DBG("MCM_DB:RecvMessage:TimerStart:Reassemble:Hdl=0x%08x:SrcSvcId=%d:SrcVdest=%d,DestSvcHdl=0x%08x\n",
...
static MDTM_REASSEMBLY_QUEUE *mdtm_check_reassem_queue(uns32 seq_num, struct tipc_portid id)
{
    ...
    */
    ...
    reassem_queue = (MDTM_REASSEMBLY_QUEUE *)ncs_patricia_tree_get
                (&mdtm_reassembly_list,(uns8 *)&reassembly_key);

    if (reassem_queue == NULL)
    {
        m_MDS_LOG_DBG("MDS_DT_TIPC : reassembly queue doesnt exist seq_num=%d, tipc_id=<0x%08x,%u",seq_num, id.node, id.ref);
...
static MDTM_REASSEMBLY_QUEUE *mdtm_add_reassemble_queue(uns32 seq_num, struct tipc_portid id)
{
    ...
    */
    ...
    ncs_patricia_tree_add (&mdtm_reassembly_list, 
                              (NCS_PATRICIA_NODE *)&reassem_queue->node);
    return reassem_queue;
}
...
static uns32  mdtm_del_from_ref_tbl(MDS_SUBTN_REF_VAL ref)
{
   ...
   */
   for (back = NULL, mov_ptr=mdtm_ref_hdl_list_hdr;
   mov_ptr != NULL;
   back = mov_ptr, mov_ptr = mov_ptr->next) /* Safe because we quit loop after deletion */
   {
...
uns32 mds_mdtm_send_tipc(MDTM_SEND_REQ *req)
{
    ...
    */
    ...
    else
    {
       ...
       if(NCSCC_RC_SUCCESS==node_status)
       {
          tipc_id.node=m_MDS_GET_TIPC_NODE_ID_FROM_NCS_NODE_ID(m_MDS_GET_NODE_ID_FROM_ADEST(req->adest));
       ...
       if(req->snd_type==MDS_SENDTYPE_ACK || req->snd_type==MDS_SENDTYPE_RACK)
       {
          ...
          m_MDS_LOG_DBG("MDTM:Sending message with Service Seqno=%d, TO Dest_Tipc_id=<0x%08x:%u> ",req->svc_seq_num, tipc_id.node,tipc_id.ref);
       ...
       switch(req->msg.encoding)
       {
          ...
          case MDS_ENC_TYPE_FULL:
          {
              ...
              else
              {
                 ...
                 if(NCSCC_RC_SUCCESS!=mdtm_add_frag_hdr(body,(len+SUM_MDS_HDR_PLUS_MDTM_HDR_PLUS_LEN), frag_seq_num, 0))
                 {
                 ...
                 if(NCSCC_RC_SUCCESS!=mdtm_sendto(body, (len+SUM_MDS_HDR_PLUS_MDTM_HDR_PLUS_LEN), tipc_id))
                 {
           ...
           case MDS_ENC_TYPE_DIRECT_BUFF:
           {
              if(req->msg.data.buff_info.len>(MDTM_MAX_DIRECT_BUFF_SIZE-SUM_MDS_HDR_PLUS_MDTM_HDR_PLUS_LEN))
              {
                  m_MDS_LOG_CRITICAL("MDTM: Passed pkt len is more than the single send direct buff\n");
              ...
              if(NCSCC_RC_SUCCESS!=mdtm_add_frag_hdr(body, req->msg.data.buff_info.len+SUM_MDS_HDR_PLUS_MDTM_HDR_PLUS_LEN,frag_seq_num, 0))
              {
                 m_MDS_LOG_ERR("MDTM: Unable to add the frag Hdr to the send msg\n");
                 mds_free_direct_buff(req->msg.data.buff_info.buff);
                 return NCSCC_RC_FAILURE;
...
static uns32 mdtm_frag_and_send(MDTM_SEND_REQ *req, uns32 seq_num, struct tipc_portid id)
{
   ...
   {
      m_MDS_LOG_CRITICAL("MDTM: App. is trying to send data more than MDTM Can fragment and send, Max size is =%d\n",32767*MDTM_NORMAL_MSG_FRAG_SIZE);
      m_MMGR_FREE_BUFR_LIST(usrbuf);
      return NCSCC_RC_FAILURE;
   }
   ...
   while(len!=0)
   {
      ...
      }
      {
         ...
         if(i==1)
         {
            p8=(uns8 *)m_MMGR_DATA_AT_START(usrbuf,(len_buf-SUM_MDS_HDR_PLUS_MDTM_HDR_PLUS_LEN),
            ...
            m_MDS_LOG_DBG("MDTM:Sending message with Service Seqno=%d, Fragment Seqnum=%d, frag_num=%d, TO Dest_Tipc_id=<0x%08x:%u>",req->svc_seq_num,seq_num,frag_val, id.node,id.ref);
...
uns32 mds_tmr_mailbox_processing(void)
{
    ...
    {
        tmr_req_info = (MDS_TMR_REQ_INFO *)ncshm_take_hdl(NCS_SERVICE_ID_COMMON,(uns32)(mbx_evt_info->info.tmr_info_hdl));
        if (tmr_req_info == NULL)
        {
            m_MDS_LOG_NOTIFY("MDTM: Tmr Mailbox Processing:Handle invalid (=0x%08x)", mbx_evt_info->info.tmr_info_hdl);
        ...
        else
        {
            switch(tmr_req_info->type)
            {
                case  MDS_QUIESCED_TMR:
                    m_MDS_LOG_DBG("MDTM: Tmr Mailbox Processing:QuiescedTimer Hdl=0x%08x", mbx_evt_info->info.tmr_info_hdl);
                    mds_mcm_quiesced_tmr_expiry(tmr_req_info->info.quiesced_tmr_info.vdest_id);
                    break;

                case  MDS_SUBTN_TMR:
                    m_MDS_LOG_DBG("MDTM: Tmr Mailbox Processing:Subtn Tmr Hdl=0x%08x", mbx_evt_info->info.tmr_info_hdl);
                ...
                case MDS_AWAIT_ACTIVE_TMR:
                    m_MDS_LOG_DBG("MDTM: Tmr Mailbox Processing:Active Tmr Hdl=0x%08x", mbx_evt_info->info.tmr_info_hdl);
                ...
                case MDS_REASSEMBLY_TMR:
                    m_MDS_LOG_DBG("MDTM: Tmr Mailbox Processing:Reassemble Tmr Hdl=0x%08x", mbx_evt_info->info.tmr_info_hdl);
                    mdtm_process_reassem_timer_event(tmr_req_info->info.reassembly_tmr_info.seq_no,
                                                      tmr_req_info->info.reassembly_tmr_info.id);
                    break;

                default:
                    m_MDS_LOG_ERR("MDTM: Tmr Mailbox Processing:JunkTmr Hdl=0x%08x", mbx_evt_info->info.tmr_info_hdl);
                    break;
            }
...
static void mds_buff_dump( uns8* buff, uns32 len, uns32 max)
{
    ...
    {
        m_MDS_LOG_ERR("DUMP:buff=0x%08x:offset=%3d to %3d:Bytes = 0x%02x 0x%02x 0x%02x 0x%02x : 0x%02x 0x%02x 0x%02x 0x%02x", 
                      (uns32)(long)buff, offset,  offset+7,
    ...
    m_MDS_LOG_ERR("DUMP:buff=0x%08x:offset=%3d to %3d:Bytes = 0x%02x 0x%02x 0x%02x 0x%02x : 0x%02x 0x%02x 0x%02x 0x%02x", 
                       (uns32)(long)buff, offset, len - 1,
                       last_line[0], last_line[0+1], last_line[0+2], last_line[0+3],
