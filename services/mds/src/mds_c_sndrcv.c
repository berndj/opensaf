...
static uns32 mds_mcm_add_bcast_list(SEND_MSG *msg, MDS_BCAST_ENUM bcast_enum, USRBUF *usr_buf, MDS_SVC_PVT_SUB_PART_VER rem_svc_sub_ver,
                                      MDS_CLIENT_MSG_FORMAT_VER  msg_fmt_ver, MDS_SVC_ARCHWORD_TYPE rem_svc_arch_word);

static uns32 mds_mcm_search_bcast_list(SEND_MSG *msg, MDS_BCAST_ENUM bcast_enum, MDS_SVC_PVT_SUB_PART_VER rem_svc_sub_ver,
                                         MDS_SVC_ARCHWORD_TYPE rem_svc_arch_word, MDS_BCAST_BUFF_LIST **mds_mcm_bcast_ptr, NCS_BOOL flag);
...
static uns32 mds_mcm_do_decode_full_or_flat(MDS_SVC_INFO *svccb, NCSMDS_CALLBACK_INFO *cbinfo, MDS_DATA_RECV *recv_msg, NCSCONTEXT orig_msg);
...
static uns32 mds_mcm_process_disc_queue_checks_redundant(MDS_SVC_INFO *svc_cb, MDS_SVC_ID dest_svc_id,
                                                           MDS_VDEST_ID dest_vdest_id, V_DEST_QA anchor, MDS_SEND_INFO *req);
...
static uns32 mcm_pvt_get_sync_send_entry(MDS_SVC_INFO *svc_cb, MDS_DATA_RECV *recv, MDS_MCM_SYNC_SEND_QUEUE  **sync_queue);
static uns32 mcm_pvt_del_sync_send_entry(MDS_PWE_HDL env_hdl, MDS_SVC_ID fr_svc_id, uns32 xch_id, MDS_SENDTYPES snd_type , MDS_DEST adest);
...
static uns32 mds_mcm_send_msg_enc(uns8 to, MDS_SVC_INFO *svc_cb, SEND_MSG *to_msg,
                                      ...
                                      MDS_SEND_INFO *req, uns32 xch_id,
                                    ...
                                    MDS_SEND_PRIORITY_TYPE pri)
{
   ...
   {
       m_MDS_LOG_ERR(
            "MDS_SND_RCV: Normal send Message sent Failed from svc_id= %d, to svc_id=%d\n",fr_svc_id,to_svc_id);
                                                ...
                                                MDS_SEND_PRIORITY_TYPE pri, uns32 xch_id)
{
    ...
    if(tx_send_hdl==NULL)
    {
        ...
        {
           m_MDS_LOG_ERR("MDS_SND_RCV:No Route Found from SVC id = %d to SVC id = %d on ADEST <0x%08x, %u>",fr_svc_id, to_svc_id, m_MDS_GET_NODE_ID_FROM_ADEST(to_dest), m_MDS_GET_PROCESS_ID_FROM_ADEST(to_dest));
           if(to_msg.msg_type==MSG_DIRECT_BUFF)
           {
              return MDS_INT_RC_DIRECT_SEND_FAIL; /* This is as the direct buff is freed at a common location */
           }
    ...
    /* Route Exists active or timer running*/
        ...
        status =mcm_process_await_active(svc_cb, tx_send_hdl, to_svc_id, &to_msg, dest_vdest_id, req->i_sendtype, xch_id, pri);
        return status;
    }
...
SEND_NOW:
                                            ...
                                            dest, &role_ret, &subs_result_hdl))
    {
        ...
        m_MDS_LOG_ERR("MDS_SND_RCV: Query for Destination failed, This case cannot exist as this has been validated before Src_svc=%d, Dest_scv=%d, vdest=%d, ADEST=%llx",svc_cb->svc_id, to_svc_id, dest_vdest_id, dest);
          return NCSCC_RC_FAILURE;
    }
    ...
    else if ((to==DESTINATION_OFF_NODE) || (to==DESTINATION_ON_NODE))
    {
        ...
        if(to_msg->msg_type==MSG_DIRECT_BUFF)
        {
            ...
            return mcm_msg_direct_send_buff(to, to_msg->data.info, to_svc_id,
                                          svc_cb, dest, dest_vdest_id,req->i_sendtype, xch_id, pri, to_msg->msg_fmt_ver);
        }
...
static uns32 mcm_msg_cpy_send(uns8 to, MDS_SVC_INFO *svc_cb, SEND_MSG *to_msg,
                                      MDS_SVC_ID to_svc_id, MDS_VDEST_ID dest_vdest_id,
                                      MDS_SEND_INFO *i_req, uns32 xch_id,
                                      MDS_DEST dest,MDS_SEND_PRIORITY_TYPE pri)
{
    ...
    else if(MSG_NCSCONTEXT==to_msg->msg_type)
    {
       ...
       {
          case MDS_SENDTYPE_RSP:case MDS_SENDTYPE_SNDRACK:
          case MDS_SENDTYPE_RRSP:case MDS_SENDTYPE_REDRACK:
          recv.exchange_id = xch_id;
          recv.snd_type = i_req->i_sendtype;
          
          orig_sender_pwe_hdl = m_MDS_GET_PWE_HDL_FROM_PWE_ID_AND_VDEST_ID(
              m_MDS_GET_PWE_ID_FROM_SVC_HDL(svc_cb->svc_hdl), 
              dest_vdest_id); 
          if(NCSCC_RC_SUCCESS != (mds_svc_tbl_get(orig_sender_pwe_hdl, to_svc_id,((NCSCONTEXT) &orig_sender_cb))))
          {
       ...
       cbinfo.info.cpy.i_last = last;
          ...
          if(NCSCC_RC_SUCCESS!=svc_cb->cback_ptr(&cbinfo))
          {
             if((i_req->i_sendtype== MDS_SENDTYPE_BCAST) || (i_req->i_sendtype== MDS_SENDTYPE_RBCAST))
             {
                 m_MDS_LOG_ERR("MDS_C_SNDRCV; ENCode full callback  failed(broadcast) svc_id=%d\n",to_svc_id);
             }
             else
             {
          ...
          if((i_req->i_sendtype== MDS_SENDTYPE_BCAST) || (i_req->i_sendtype== MDS_SENDTYPE_RBCAST))
          {
             ...
             if(NCSCC_RC_FAILURE==mds_mcm_add_bcast_list(to_msg, BCAST_ENC, req.msg.data.fullenc_uba.start, to_msg->rem_svc_sub_part_ver,
                                                           cbinfo.info.enc.o_msg_fmt_ver, to_msg->rem_svc_arch_word))
             {
                                  ...
                                  uns32 snd_type, uns32 xch_id, MDS_SEND_PRIORITY_TYPE pri)
{
   ...
   if((snd_type== MDS_SENDTYPE_BCAST) || (snd_type== MDS_SENDTYPE_RBCAST))  
   {
      ...
      {
         if(NCSCC_RC_SUCCESS==(mds_mcm_search_bcast_list(to_msg, BCAST_ENC_FLAT, to_msg->rem_svc_sub_part_ver,
                                             to_msg->rem_svc_arch_word, &bcast_ptr, 1)))
         {
      ...
      {
         if(NCSCC_RC_SUCCESS==(mds_mcm_search_bcast_list(to_msg, BCAST_ENC, to_msg->rem_svc_sub_part_ver,
                                             to_msg->rem_svc_arch_word, &bcast_ptr, 1)))
         {
   ...
   if (status != NCSCC_RC_SUCCESS)
   {
      m_MDS_LOG_ERR("MDS_SND_RCV: Encode callback of  Dest =%d, Adest=%llx, svc-id=%d failed while sending to svc=%d)", dest_vdest_id, adest, svc_cb->svc_id, to_svc_id);
   ...
   if(to==DESTINATION_OFF_NODE)
   {
      ...
      {
         m_MDS_LOG_ERR("MDS_SND_RCV: Empty and NULL message from svc-id=%d to svc-id=%d", svc_cb->svc_id, to_svc_id);
         m_MDS_LOG_DBG("MDS_SND_RCV : Leaving mcm_msg_encode_full_or_flat_and_send\n");
         return NCSCC_RC_FAILURE;
      }
      else if((snd_type== MDS_SENDTYPE_BCAST) || (snd_type== MDS_SENDTYPE_RBCAST))
      {
         if(NCSCC_RC_FAILURE==mds_mcm_add_bcast_list(to_msg, BCAST_ENC, msg_send.msg.data.fullenc_uba.start, to_msg->rem_svc_sub_part_ver,
                                                               cbinfo.info.enc.o_msg_fmt_ver, to_msg->rem_svc_arch_word))
         {
            m_MDS_LOG_ERR("MDS_C_SNDRCV: Addition to bcast list failed in enc case");
            return NCSCC_RC_FAILURE;
   ...
   else
   {
      ...
      {
         if(NCSCC_RC_FAILURE==mds_mcm_add_bcast_list(to_msg, BCAST_ENC_FLAT, msg_send.msg.data.flat_uba.start, to_msg->rem_svc_sub_part_ver,
                                                               cbinfo.info.enc_flat.o_msg_fmt_ver, to_msg->rem_svc_arch_word))
         {
                                       ...
                                       MDS_MCM_SYNC_SEND_QUEUE  **sync_queue)
{
    ...
    while(queue!=NULL)
    {
        if(recv->exchange_id==queue->txn_id)
        {
           ...
           else if((queue->msg_snd_type==MDS_SENDTYPE_SNDRACK)||(queue->msg_snd_type==MDS_SENDTYPE_REDRACK))
           {
           ...
           else if((queue->msg_snd_type==MDS_SENDTYPE_SNDACK) || (queue->msg_snd_type==MDS_SENDTYPE_REDACK))
           {
              if(recv->snd_type==MDS_SENDTYPE_ACK)
              {
                 *sync_queue=queue;
   ...
   */
   ...
   if(NCSCC_RC_SUCCESS != disc_rc)
   {
      ...
      if (NCSCC_RC_REQ_TIMOUT == disc_rc)
      {
          ...
          m_MDS_LOG_ERR("MDS_SND_RCV:No Route Found from SVC id = %d to SVC id = %d on ADEST <0x%08x, %u>",svc_cb->svc_id, dest_svc_id, m_MDS_GET_NODE_ID_FROM_ADEST(i_dest), m_MDS_GET_PROCESS_ID_FROM_ADEST(i_dest));
      }
   ...
   else
   {
      if(dest_vdest_id==m_VDEST_ID_FOR_ADEST_ENTRY)
      {
         ...
         /* Query one type to get the tx_send_hdl(results) */
         if(NCSCC_RC_SUCCESS!=mds_subtn_res_tbl_query_by_adest(svc_cb->svc_hdl, dest_svc_id, dest_vdest_id, anchor))
         {
      ...
      else
      {
                                                            ...
                                                            o_dest, timer_running, &t_send_hdl, flag_t))
         {
             ...
             m_MDS_LOG_ERR("MDS_SND_RCV: Destination Route not found even after the DISCOVERY Timer timeout\n");
             m_MDS_LOG_DBG("MDS_SND_RCV :L  mds_mcm_process_disc_queue_checks\n");
             return NCSCC_RC_FAILURE;
         }
 ...
 ***************************************************************************/
static uns32 mds_subtn_tbl_add_disc_queue(MDS_SUBSCRIPTION_INFO *sub_info, MDS_SEND_INFO *req,
                                                MDS_VDEST_ID dest_vdest_id, MDS_DEST dest, MDS_HDL env_hdl, MDS_SVC_ID fr_svc_id)
{
   NCS_SEL_OBJ sel_obj;
...
static uns32  mcm_process_await_active(MDS_SVC_INFO *svc_cb, MDS_SUBSCRIPTION_RESULTS_INFO *send_hdl,
                                                         ...
                                                         MDS_VDEST_ID to_vdest, uns32 snd_type,
                                                         uns32 xch_id, MDS_SEND_PRIORITY_TYPE pri)
{
   ...
   else if(to_msg->msg_type==MSG_NCSCONTEXT)
   {
      ...
      if(((snd_type== MDS_SENDTYPE_BCAST) || (snd_type== MDS_SENDTYPE_RBCAST))&&
                                          ((NCSCC_RC_SUCCESS==(mds_mcm_search_bcast_list(to_msg, BCAST_ENC, to_msg->rem_svc_sub_part_ver,
                                             MDS_SVC_ARCHWORD_TYPE_UNSPECIFIED, &bcast_ptr, 1)))))
      {
         if (ncs_enc_init_space_pp(&req.msg.data.fullenc_uba,NCSUB_MDS_POOL,0) != NCSCC_RC_SUCCESS)
         {
            m_MDS_LOG_ERR("MDS_SND_RCV: encode full init failed svc_id=%d\n",svc_cb->svc_id);
      ...
      else
      {
         ...
         cbinfo.info.enc.io_uba =NULL;
         cbinfo.info.enc.io_uba =&req.msg.data.fullenc_uba;
         cbinfo.info.enc.i_rem_svc_pvt_ver=send_hdl->info.active_vdest.active_route_info->last_active_svc_sub_part_ver; 
         ...
         {
             if(NCSCC_RC_FAILURE==mds_mcm_add_bcast_list(to_msg, BCAST_ENC, req.msg.data.fullenc_uba.start, to_msg->rem_svc_sub_part_ver,
                                                           cbinfo.info.enc.o_msg_fmt_ver, MDS_SVC_ARCHWORD_TYPE_UNSPECIFIED))
             {
               m_MDS_LOG_ERR("MDS_C_SNDRCV: Addition to bcast list failed in enc case");
               return NCSCC_RC_FAILURE;
             }
                                 ...
                                 MDS_SEND_PRIORITY_TYPE pri)
{
   ...
   {
       m_MDS_LOG_ERR(
            "MDS_SND_RCV: RED send Message sent Failed from svc_id= %d, to svc_id=%d\n",fr_svc_id,to_svc_id);
       m_MDS_ERR_PRINT_ADEST(to_dest);
                                                ...
                                                MDS_SEND_PRIORITY_TYPE pri, uns32 xch_id, V_DEST_QA anchor)
{
    ...
    if(NCSCC_RC_SUCCESS!=mds_subtn_res_tbl_query_by_adest(svc_cb->svc_hdl, dest_svc_id, dest_vdest_id,dest))
    {
       /* Destination Route Not Found, still some validations required */
       /* Check in subscriptions whether this exists*/
        if(NCSCC_RC_SUCCESS!=mds_mcm_process_disc_queue_checks_redundant(svc_cb, dest_svc_id, dest_vdest_id, dest, req))
        {
                                                           ...
                                                           MDS_VDEST_ID dest_vdest_id, V_DEST_QA anchor, MDS_SEND_INFO *req)
{
   ...
   */
   ...
   if(NCSCC_RC_SUCCESS != disc_rc)
   {
      ...
      {
          /* We timed out waiting for a route */
          m_MDS_LOG_ERR("MDS_SND_RCV:No Route Found from SVC id = %d to SVC id = %d to VDEST id = %d on  ADEST <0x%08x, %u>",svc_cb->svc_id, dest_svc_id, dest_vdest_id, m_MDS_GET_NODE_ID_FROM_ADEST(anchor), m_MDS_GET_PROCESS_ID_FROM_ADEST(anchor));
      }
      return NCSCC_RC_FAILURE;
   }
   else
                                       ...
                                       MDS_SEND_PRIORITY_TYPE pri)
{
    ...
    if(NCSCC_RC_SUCCESS!=mcm_pvt_create_sync_send_entry(env_hdl, fr_svc_id, req->i_sendtype, xch_id, &sync_queue, msg))
    {
    ...
    if(NCSCC_RC_SUCCESS!=status)
    {
       ...
       m_MDS_LOG_ERR("MDS_SND_RCV: Normal sndrsp mesg SEND Failed from svc_id= %d, to svc_id=%d to_dest=%llx\n",fr_svc_id,to_svc_id,to_dest);
       return status;
    }
    else
    {
       ...
       if(NCSCC_RC_SUCCESS!=mds_mcm_time_wait(sel_obj,req->info.sndrsp.i_time_to_wait))
       {
          ...
          mds_await_active_tbl_del_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id,
                                                    to_svc_id, m_MDS_GET_INTERNAL_VDEST_ID_FROM_MDS_DEST(to_dest), req->i_sendtype);
        ...
        else
        {
            while(mov_ptr!=NULL)
            {
                       ...
                       (mov_ptr->req.dest_svc_id==to_svc_id))
                {
                    if(bk_ptr==NULL)
                    {
                        result->info.active_vdest.active_route_info->await_active_queue=mov_ptr->next_msg;
                    }
                    else
                    {
...
static uns32 mcm_pvt_del_sync_send_entry(MDS_PWE_HDL env_hdl, MDS_SVC_ID fr_svc_id, uns32 xch_id, MDS_SENDTYPES snd_type , MDS_DEST adest)
{
   ...
   for (prev_ptr = NULL, q_ptr=svc_cb->sync_send_queue;
        q_ptr!=NULL;
        prev_ptr = q_ptr, q_ptr = q_ptr->next_send) /* Safe because we quit after deletion */
   {
                                        ...
                                        MDS_SEND_PRIORITY_TYPE pri, V_DEST_QA anchor)
{
    ...
    if(NCSCC_RC_SUCCESS!=mds_subtn_res_tbl_query_by_adest(svc_cb->svc_hdl, to_svc_id, dest_vdest_id,dest))
    {
       /* Check in subscriptions whether this exists*/
        if(NCSCC_RC_SUCCESS!=mds_mcm_process_disc_queue_checks_redundant(svc_cb, to_svc_id, dest_vdest_id, dest,req))
        {
           m_MDS_LOG_ERR("MDS_SND_RCV: Nwo_desto Route Found from svc_id=%d, to svc_id= %d", svc_cb->svc_id, to_svc_id);
           m_MDS_ERR_PRINT_ADEST(to_dest);
           if(to_msg.msg_type==MSG_DIRECT_BUFF)
           {
              return MDS_INT_RC_DIRECT_SEND_FAIL; /* This is as the direct buff is freed at a common location */
                                       ...
                                       MDS_SEND_PRIORITY_TYPE pri)
{
    ...
    */
    ...
    if(NCSCC_RC_SUCCESS!=status)
    {
       ...
       mcm_pvt_del_sync_send_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id,  req->i_sendtype, 0);
       m_MDS_LOG_ERR(
              "MDS_SND_RCV: Normal sndack message SEND Failed from svc_id= %d, to svc_id=%d\n",fr_svc_id,to_svc_id);
       m_MDS_ERR_PRINT_ADEST(to_dest);
       return status;
    }
    else
    {
       ...
       if(NCSCC_RC_SUCCESS!=mds_mcm_time_wait(sel_obj,req->info.sndack.i_time_to_wait))
       {
          ...
          m_MDS_LOG_ERR("MDS_SND_RCV: Timeout occured on sndack message from svc_id=%d, to svc_id=%d", fr_svc_id, to_svc_id);
          m_MDS_ERR_PRINT_ADEST(to_dest);
          mds_await_active_tbl_del_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id,
                                                    to_svc_id, m_MDS_GET_INTERNAL_VDEST_ID_FROM_MDS_DEST(to_dest), req->i_sendtype);
          mcm_pvt_del_sync_send_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id,  req->i_sendtype, 0);
                                        ...
                                        MDS_SEND_PRIORITY_TYPE pri)
{
   ...
   status=mcm_pvt_red_snd_process_common(env_hdl, fr_svc_id, send_msg, to_dest, to_svc_id, req, pri, xch_id, adest);
    ...
    {
        m_MDS_LOG_ERR(
             "MDS_SND_RCV: Normal rsp Message sent Failed from svc_id= %d, to svc_id=%d\n",fr_svc_id,to_svc_id);
        m_MDS_ERR_PRINT_ADEST(to_dest);
                                    ...
                                    MDS_SEND_PRIORITY_TYPE pri)
{
    ...
    else
    {
       ...
       if(NCSCC_RC_SUCCESS!=mds_mcm_time_wait(sel_obj,req->info.redrsp.i_time_to_wait))
       {
          ...
          m_MDS_LOG_ERR("MDS_SND_RCV: Timeout occured on red sndrsp message from svc_id=%d, to svc_id=%d", fr_svc_id,to_svc_id);
          m_MDS_ERR_PRINT_ADEST(to_dest);
          m_MDS_ERR_PRINT_ANCHOR(anchor);
                                     ...
                                     MDS_SEND_PRIORITY_TYPE pri)
{
   ...
   status=mcm_pvt_red_snd_process_common(env_hdl, fr_svc_id, send_msg, to_dest, to_svc_id, req, pri, xch_id, anchor_1);
   ...
   {
       m_MDS_LOG_ERR(
            "MDS_SND_RCV: RED rsp Message sent Failed from svc_id= %d, to svc_id=%d\n",fr_svc_id,to_svc_id);
       m_MDS_ERR_PRINT_ADEST(to_dest);
       m_MDS_ERR_PRINT_ANCHOR(anchor);
       return status;
   }
                                       ...
                                       uns8 flag /* For normal=0, red=1*/)
{
   ...
   if (sub_info==NULL)
   {
      ...
      mds_mcm_subtn_add (svc_cb->svc_hdl, to_svc_id, svc_cb->install_scope,
                           MDS_VIEW_RED/* redundantview */, MDS_SUBTN_IMPLICIT);

      if(NCSCC_RC_SUCCESS!=mds_subtn_tbl_get(svc_cb->svc_hdl, to_svc_id, &sub_info))
      {
         m_MDS_LOG_ERR("MDS_SND_RCV: Broadcast :Subscription made but no pointer got after subscription\n");
         if(to_msg.msg_type==MSG_DIRECT_BUFF)
         {
            return MDS_INT_RC_DIRECT_SEND_FAIL; /* This is as the direct buff is freed at a common location */
         }
      ...
      else
      {
         ...
         if(NCSCC_RC_SUCCESS != disc_rc)
         {
            ...
            {
               /* We timed out waiting for a route */
               m_MDS_LOG_ERR("MDS_SND_RCV: Broadcast : No Route Found from SVC id = %d to SVC id = %d",svc_cb->svc_id, to_svc_id);
            }
            if(to_msg.msg_type==MSG_DIRECT_BUFF)
            {
               return MDS_INT_RC_DIRECT_SEND_FAIL; /* This is as the direct buff is freed at a common location */
   ...
   else if(sub_info->tmr_flag==TRUE)
   {
    ...
    while(1)
    {
        if(flag==0)
        {
            if(NCSCC_RC_SUCCESS!=mds_subtn_res_tbl_getnext_active(svc_cb->svc_hdl, to_svc_id, &info_result))
            {
                break;
            }
        }
        ...
        if(flag==0)
        {
            ...
            if(info_result->key.adest==0)
            {
                ...
                if(NCSCC_RC_SUCCESS!=mcm_process_await_active(svc_cb, info_result, to_svc_id,
                                           &to_msg, info_result->key.vdest_id, req->i_sendtype, 0, pri))
                {
    ...
    switch(recv->snd_type)
    {
        ...
        case MDS_SENDTYPE_SND:
        {
            if(mds_svc_tbl_get_role(svccb->svc_hdl)!=NCSCC_RC_SUCCESS)
            {
                ...
                mds_mcm_free_msg_memory(recv->msg);
                /* log: MESSAGE Dropped*/
                m_MDS_LOG_ERR("MDS_SND_RCV: Message is being dropped as the destination Service is in standby mode=%d\n",svccb->svc_id);
                return NCSCC_RC_FAILURE;
            }
                                 ...
                                 MDS_SEND_PRIORITY_TYPE pri, MDS_CLIENT_MSG_FORMAT_VER msg_fmt_ver)
{
    ...
    return  mcm_pvt_red_snd_process_common(env_hdl, fr_svc_id, send_msg, to_dest, to_svc_id, req ,pri, xch_id, anchor);
}
                                       ...
                                       MDS_SEND_PRIORITY_TYPE pri, MDS_CLIENT_MSG_FORMAT_VER msg_fmt_ver)
{
    ...
    /* First create a sync send entry */
    if(NCSCC_RC_SUCCESS!=mcm_pvt_create_sync_send_entry(env_hdl, fr_svc_id, req->i_sendtype,xch_id, &sync_queue, NULL))
    {
    ...
    {
       /* delete the created the sync send entry */
       m_MDS_LOG_ERR("MDS_SND_RCV: Normal sndrsp direct message SEND Failed from svc_id =%d to svc_id =%d", fr_svc_id, to_svc_id);
       m_MDS_ERR_PRINT_ADEST(to_dest);
       mcm_pvt_del_sync_send_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id, req->i_sendtype, 0);
       return ret_status;
    }
    else
    {
       ...
       if(NCSCC_RC_SUCCESS!=mds_mcm_time_wait(sel_obj, req->info.sndrsp.i_time_to_wait))
       {
          ...
          m_MDS_LOG_ERR("MDS_SND_RCV: Timeout occured on  Direct sndrsp direct message from svc_id =%d to svc_id =%d", fr_svc_id, to_svc_id);
          m_MDS_ERR_PRINT_ADEST(to_dest);
          mds_await_active_tbl_del_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id,
                                                    to_svc_id, m_MDS_GET_INTERNAL_VDEST_ID_FROM_MDS_DEST(to_dest), req->i_sendtype);
          mcm_pvt_del_sync_send_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id, req->i_sendtype, 0);
                                       ...
                                       MDS_SEND_PRIORITY_TYPE pri, MDS_CLIENT_MSG_FORMAT_VER msg_fmt_ver)
{
    ...
    {
       /* delete the created the sync send entry */
       m_MDS_LOG_ERR("MDS_SND_RCV: Normal sndack direct message SEND Failed from svc_id=%d to svc_id=%d", fr_svc_id, to_svc_id);
       m_MDS_ERR_PRINT_ADEST(to_dest);
       mcm_pvt_del_sync_send_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id, req->i_sendtype, 0);
       return ret_status;
    }
    else
    {
       ...
       if(NCSCC_RC_SUCCESS!=mds_mcm_time_wait(sel_obj,req->info.sndack.i_time_to_wait))
       {
          ...
          m_MDS_LOG_ERR("MDS_SND_RCV: Timeout occured on sndack direct message from svc_id=%d to svc_id=%d", fr_svc_id, to_svc_id);
          m_MDS_ERR_PRINT_ADEST(to_dest);
          mds_await_active_tbl_del_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id,
                                                    to_svc_id, m_MDS_GET_INTERNAL_VDEST_ID_FROM_MDS_DEST(to_dest), req->i_sendtype);
          mcm_pvt_del_sync_send_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id, req->i_sendtype, 0);
                                        ...
                                        MDS_SEND_PRIORITY_TYPE pri, MDS_CLIENT_MSG_FORMAT_VER msg_fmt_ver)
{
    ...
    if(NCSCC_RC_SUCCESS!=ret_status)
    {
       m_MDS_LOG_ERR("MDS_SND_RCV: Normal sndrack direct message SEND Failed from svc_id=%d to svc_id=%d", fr_svc_id, to_svc_id);
       m_MDS_ERR_PRINT_ADEST(to_dest);
       mcm_pvt_del_sync_send_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id, req->i_sendtype, msg_dest_adest);
       return ret_status;
    }
    else
    {
       ...
       if(NCSCC_RC_SUCCESS!=mds_mcm_time_wait(sel_obj,req->info.sndrack.i_time_to_wait))
       {
          ...
          m_MDS_LOG_ERR("MDS_SND_RCV: Timeout occured on Normal sndrack direct messagefrom svc_id=%d to svc_id=%d", fr_svc_id, to_svc_id);
          m_MDS_ERR_PRINT_ADEST(to_dest);
          mds_await_active_tbl_del_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id,
                                                    to_svc_id, m_MDS_GET_INTERNAL_VDEST_ID_FROM_MDS_DEST(to_dest), req->i_sendtype);
          mcm_pvt_del_sync_send_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id, req->i_sendtype, msg_dest_adest);
                                        ...
                                        MDS_SEND_PRIORITY_TYPE pri, MDS_CLIENT_MSG_FORMAT_VER msg_fmt_ver)
{
  ...
  send_msg.msg_fmt_ver=msg_fmt_ver; 
   ...
   return mcm_pvt_red_snd_process_common(env_hdl, fr_svc_id, send_msg, to_dest, to_svc_id, req, pri, xch_id, adest);
}
                                    ...
                                    MDS_SEND_PRIORITY_TYPE pri, MDS_CLIENT_MSG_FORMAT_VER msg_fmt_ver)
{
    ...
    {
       /* delete the created the sync send entry */
       m_MDS_LOG_ERR("MDS_SND_RCV: RED sndrsp direct message SEND Failed from svc_id=%d to svc_id=%d", fr_svc_id, to_svc_id);
       m_MDS_ERR_PRINT_ADEST(to_dest);
       m_MDS_ERR_PRINT_ANCHOR(anchor);
       mcm_pvt_del_sync_send_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id, req->i_sendtype, 0);
       return ret_status;
    ...
    else
    {
       ...
       if(NCSCC_RC_SUCCESS!=mds_mcm_time_wait(sel_obj,req->info.redrsp.i_time_to_wait))
       {
          ...
          m_MDS_LOG_ERR("MDS_SND_RCV: Timeout occured on RED sndrsp direct message Failed from svc_id=%d to svc_id=%d", fr_svc_id, to_svc_id);
          m_MDS_ERR_PRINT_ADEST(to_dest);
          m_MDS_ERR_PRINT_ANCHOR(anchor);
          mds_await_active_tbl_del_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id,
                                                    to_svc_id, m_MDS_GET_INTERNAL_VDEST_ID_FROM_MDS_DEST(to_dest), req->i_sendtype);
                                     ...
                                     MDS_SEND_PRIORITY_TYPE pri, MDS_CLIENT_MSG_FORMAT_VER msg_fmt_ver)
{
    ...
    if(NCSCC_RC_SUCCESS!=ret_status)
    {
       m_MDS_LOG_ERR("MDS_SND_RCV: RED sndrack direct message SEND Failed Failed from svc_id=%d to svc_id=%d", fr_svc_id, to_svc_id);
       m_MDS_ERR_PRINT_ADEST(to_dest);
       m_MDS_ERR_PRINT_ANCHOR(anchor);
       mcm_pvt_del_sync_send_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id, req->i_sendtype, msg_dest_adest);
       return ret_status;
    ...
    else
    {
       ...
       if(NCSCC_RC_SUCCESS!=mds_mcm_time_wait(sel_obj,req->info.redrack.i_time_to_wait))
       {
          ...
          m_MDS_LOG_ERR("MDS_SND_RCV: Timeout occured on  RED sndrack direct message Failed from svc_id=%d to svc_id=%d", fr_svc_id, to_svc_id);
          m_MDS_ERR_PRINT_ADEST(to_dest);
          m_MDS_ERR_PRINT_ANCHOR(anchor);
          mds_await_active_tbl_del_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id,
                                                    to_svc_id, m_MDS_GET_INTERNAL_VDEST_ID_FROM_MDS_DEST(to_dest), req->i_sendtype);
                                    ...
                                    MDS_SEND_PRIORITY_TYPE pri, MDS_CLIENT_MSG_FORMAT_VER msg_fmt_ver)
{
    ...
    {
       /* delete the created the sync send entry */
       m_MDS_LOG_ERR("MDS_SND_RCV: RED sndack direct message SEND Failed from svc_id=%d to svc_id=%d", fr_svc_id, to_svc_id);
       m_MDS_ERR_PRINT_ADEST(to_dest);
       m_MDS_ERR_PRINT_ANCHOR(anchor);
       mcm_pvt_del_sync_send_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id, req->i_sendtype, 0);
       return ret_status;
    ...
    else
    {
       ...
       if(NCSCC_RC_SUCCESS!=mds_mcm_time_wait(sel_obj,req->info.redack.i_time_to_wait))
       {
          ...
          m_MDS_LOG_ERR("MDS_SND_RCV: Timeout occured on RED sndack direct  message from svc_id=%d to svc_id=%d", fr_svc_id, to_svc_id);
          m_MDS_ERR_PRINT_ADEST(to_dest);
          m_MDS_ERR_PRINT_ANCHOR(anchor);
          mds_await_active_tbl_del_entry((MDS_PWE_HDL)env_hdl, fr_svc_id, xch_id,
                                                    to_svc_id, m_MDS_GET_INTERNAL_VDEST_ID_FROM_MDS_DEST(to_dest), req->i_sendtype);
                                     ...
                                     MDS_SEND_PRIORITY_TYPE pri, MDS_CLIENT_MSG_FORMAT_VER msg_fmt_ver)
{
   ...
   return mcm_pvt_red_snd_process_common(env_hdl, fr_svc_id, send_msg, to_dest, to_svc_id, req, pri, xch_id, anchor_1);
}
...
uns32 mds_retrieve( NCSMDS_INFO * info)
{
        ...
        while (TRUE)
        {
            ...
            else
            {
                ...
                if(NCSCC_RC_SUCCESS!=mds_svc_tbl_get((MDS_PWE_HDL)(info->i_mds_hdl), svc_id, &hdl))
                {
                    ...
                    m_MDS_LOG_INFO("MDS_SND_RCV: Service doesnt exists : But Message received in IPC mailbox\n");
                ...
                {
                    m_MDS_LOG_INFO("MDS_SND_RCV: Service doesnt exists : New Mailbox created, means service unistalled and installed again\n");
                    m_MMGR_FREE_MSGELEM(msgelem);
                    return NCSCC_RC_SUCCESS;
                }
                if (svc_cb->q_ownership != TRUE)
                {
                    m_MDS_LOG_INFO("MDS_SND_RCV: Service doesnt exists : Service uninstalled and installed again without mds Q-Ownership model\n");
                    m_MMGR_FREE_MSGELEM(msgelem);
                    return NCSCC_RC_SUCCESS;
                }
                /* IR 82530 Fix*/
    ...
    else if (info->info.retrieve_msg.i_dispatchFlags == SA_DISPATCH_ALL)
    {
        while((msgelem = (MDS_MCM_MSG_ELEM*)m_NCS_IPC_NON_BLK_RECEIVE(&local_mbx, NULL))!=NULL)
        {
           ...
           if(mds_mailbox_proc(msgelem, svc_cb)==NCSCC_RC_NO_OBJECT)
           {
              m_MDS_LOG_NOTIFY("MDS_SND_RCV: Svc doesnt exists after calling the mailbox_proc, when called(MDS Q-Ownership,DIS-ALL), svc_id=%d",svc_id);
              return NCSCC_RC_FAILURE;
           }
           msgelem=NULL;
        }
...
static uns32 mds_mailbox_proc(MDS_MCM_MSG_ELEM *msgelem, MDS_SVC_INFO *svc_cb)
{
    ...
    if(msgelem->type==MDS_DATA_TYPE)
    {
        ...
        switch(msgelem->info.data.enc_msg.encoding)
        {
            ...
            case MDS_ENC_TYPE_FULL:
            {
                ...
                if(mds_validate_svc_cb(svc_cb, svc_hdl, svc_id)!=NCSCC_RC_SUCCESS)
                {
                   ...
                   m_MDS_LOG_NOTIFY("MDS_SND_RCV: Service doesnt exists : after calling the callback dec_full(MDS Q-Ownership), svc_id=%d",svc_id);
                   goto out2;
                }
            ...
            case MDS_ENC_TYPE_FLAT:
            {
                ...
                if(mds_validate_svc_cb(svc_cb, svc_hdl, svc_id)!=NCSCC_RC_SUCCESS)
                {
                   ...
                   status=NCSCC_RC_NO_OBJECT;
                   m_MDS_LOG_NOTIFY("MDS_SND_RCV: Service doesnt exists : after calling the callback dec_flat(MDS Q-Ownership), svc_id=%d",svc_id);
                   goto out2;
                }
        ...
        switch(msgelem->info.data.enc_msg.encoding)
        {
            ...
            {
                cbinfo.i_op=MDS_CALLBACK_RECEIVE;
                if( msgelem->info.data.snd_type==MDS_SENDTYPE_SNDRSP || msgelem->info.data.snd_type==MDS_SENDTYPE_REDRSP )
                {
                    uns8 *data;
                    cbinfo.info.receive.i_rsp_reqd   = TRUE;
                    cbinfo.info.receive.i_msg_ctxt.length=MDS_SYNC_SND_CTXT_LEN_MAX;
...
out2:       
        ...
        if(mds_validate_svc_cb(svc_cb, svc_hdl, svc_id)!=NCSCC_RC_SUCCESS)
        {
           m_MDS_LOG_NOTIFY("MDS_SND_RCV: Service doesnt exists : after calling the callback rec or direct_rec(MDS Q-Ownership), svc_id=%d",svc_id);
           status=NCSCC_RC_NO_OBJECT;
        }
    ...
    else
    {
        ...
        if(mds_validate_svc_cb(svc_cb, svc_hdl, svc_id)!=NCSCC_RC_SUCCESS)
        {
           m_MDS_LOG_NOTIFY("MDS_SND_RCV: Service doesnt exists : after calling the callback for the Events(MDS Q-Ownership), svc_id=%d",svc_id);
           status=NCSCC_RC_NO_OBJECT;
        }
...
{ 
   ...
   {
      m_MDS_LOG_INFO("MDS_SND_RCV: Service Uninstalled while calling the callback's in MDS Q-Ownership model, svc_id=%d",svc_id);
      return NCSCC_RC_FAILURE;
   }
                                      ...
                                      MDS_CLIENT_MSG_FORMAT_VER  msg_fmt_ver, MDS_SVC_ARCHWORD_TYPE rem_svc_arch_word)
{
   ...
   status=mds_mcm_search_bcast_list(msg, bcast_enum, rem_svc_sub_ver,
                                         ...
                                         ,MDS_SVC_ARCHWORD_TYPE rem_svc_arch_word, MDS_BCAST_BUFF_LIST **mds_mcm_bcast_ptr, NCS_BOOL flag)
{
   ...
   {
      if((rem_svc_arch_word == tmp_bcast_ptr->rem_svc_arch_word) && (rem_svc_sub_ver==tmp_bcast_ptr->rem_svc_sub_part_ver))
      {
