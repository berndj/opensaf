...
uns32 mds_vdest_tbl_update_role (MDS_VDEST_ID vdest_id, V_DEST_RL role, NCS_BOOL del_tmr_info)
{
    ...
    {
        if (role == V_DEST_RL_QUIESCED)
        {/* New role is quiesced */
        ...
        else if (vdest_info->role == V_DEST_RL_QUIESCED)
        {/* Old role is quiesced */
...
uns32 mds_vdest_tbl_get_first (MDS_VDEST_ID vdest_id, MDS_PWE_HDL *first_pwe_hdl)
{
    ...
    else
    {
        ...
        {
            *first_pwe_hdl = (MDS_PWE_HDL)NULL;
            m_MDS_LOG_DBG("MCM_DB : Leaving : F : mds_vdest_tbl_get_first : VDEST present but no PWE on this VDEST");
            return NCSCC_RC_FAILURE;
        }
        else
        {
...
uns32 mds_vdest_tbl_get_vdest_info_cb(MDS_VDEST_ID vdest_id, MDS_VDEST_INFO **vdest_info)
{
    ...
    *vdest_info = (MDS_VDEST_INFO *)ncs_patricia_tree_get
                    (&gl_mds_mcm_cb->vdest_list,(uns8 *)&vdest_id);
...
uns32 mds_pwe_tbl_add (MDS_VDEST_HDL vdest_hdl, PW_ENV_ID pwe_id, MDS_PWE_HDL *pwe_hdl)
{
    ...
    else
    {
        if (mds_pwe_tbl_query(vdest_hdl,pwe_id) == NCSCC_RC_SUCCESS)
        {
            ...
            m_MDS_LOG_DBG("MCM_DB : Leaving : F : mds_pwe_tbl_add  : VDEST present, but PWE on this VDEST not present");
            return NCSCC_RC_FAILURE;
        }
        else
        {
...
*********************************************************/
uns32 mds_pwe_tbl_del (MDS_PWE_HDL pwe_hdl) /* mds_hdl is stored as back ptr in PWE_INFO */
{
    MDS_VDEST_INFO *vdest_info = NULL;
    MDS_VDEST_ID vdest_id;
    MDS_PWE_ID pwe_id;
...
*********************************************************/
uns32 mds_svc_tbl_get_install_scope(MDS_PWE_HDL pwe_hdl, MDS_SVC_ID svc_id, 
...
*********************************************************/
uns32 mds_svc_tbl_get_svc_hdl(MDS_PWE_HDL pwe_hdl, MDS_SVC_ID svc_id,
                                        MDS_SVC_HDL *svc_hdl)
{
...
*********************************************************/
uns32 mds_svc_tbl_get_first_subscription (MDS_SVC_HDL svc_hdl, 
    ...
    else
    {
        ...
        else
        {
            *first_subscription = NULL;
            m_MDS_LOG_DBG("MCM_DB : Leaving : F : mds_svc_tbl_get_first_subscription : No Subscription present");
            return NCSCC_RC_FAILURE;            
        }
    }
}
...
*********************************************************/
uns32 mds_svc_tbl_getnext_on_vdest (MDS_VDEST_ID vdest_id, MDS_SVC_HDL current_svc_hdl,
                                MDS_SVC_INFO **svc_info)
{
    ...
    temp_svc_info = (MDS_SVC_INFO *)ncs_patricia_tree_getnext(&gl_mds_mcm_cb->svc_list,
                                            (uns8 *)&current_svc_hdl);
                                ...
                                MDS_VIEW view, MDS_SUBTN_TYPE subtn_type)
{
    ...
    if (status == NCSCC_RC_SUCCESS || status ==  NCSCC_RC_NO_CREATION)
    {
        ...
        m_MDS_LOG_NOTIFY("MCM_DB : subtn_tbl_add : IMPLICIT SUBSCRIPTION of SVC id = %d to SVC id = %d ALREADY EXIST");
...
uns32 mds_subtn_tbl_del (MDS_SVC_HDL svc_hdl, uns32 subscr_svc_id)
{
    ...
    m_MDS_LOG_DBG("MCM_DB : Leaving : F : mds_subtn_tbl_del : Subscription not present, Reached till end of function");
    return NCSCC_RC_FAILURE;
}
...
uns32 mds_subtn_tbl_change_explicit (MDS_SVC_HDL svc_hdl, MDS_SVC_ID subscr_svc_id, MDS_VIEW subtn_view_type)
{
    ...
    while (temp_subtn_info != NULL)
    {
        if (temp_subtn_info->sub_svc_id == subscr_svc_id)
        {
            ...
            {
                if (temp_subtn_result_info->key.adest == 0)
                {/* This is active entry for a VDEST */

                    if (temp_subtn_result_info->info.active_vdest.active_route_info->tmr_running == TRUE)
...
*********************************************************/
uns32 mds_subtn_tbl_update_ref_hdl (MDS_SVC_HDL svc_hdl, MDS_SVC_ID subscr_svc_id,
...
*********************************************************/
uns32 mds_subtn_tbl_get (MDS_SVC_HDL svc_hdl, MDS_SVC_ID subscr_svc_id,
...
*********************************************************/
uns32 mds_subtn_tbl_get_details (MDS_SVC_HDL svc_hdl, MDS_SVC_ID subscr_svc_id,
...
*********************************************************/
uns32 mds_block_snd_req_tbl_add (MDS_SVC_HDL svc_hdl, MDS_SYNC_TXN_ID txn_id,
...
*********************************************************/
uns32 mds_block_snd_req_tbl_get(MDS_SVC_HDL svc_hdl, MDS_SYNC_TXN_ID txn_id,
                                           MDS_MCM_SYNC_SEND_QUEUE *result)
{
         return NCSCC_RC_SUCCESS;
}
                             ...
                             MDS_SVC_ARCHWORD_TYPE    archword_type)
{
    ...
    {/* Entry to add is VDEST entry */
        ...
        if (role == V_DEST_RL_ACTIVE)
        {
            ...
            {/* Active entry Or Await active entry exist*/
                
                if (active_subtn_res_info->info.active_vdest.active_route_info->tmr_running == TRUE)
                {/* Present entry is Await Active Entry */
                    ...
                    /* Destroy Handle */
                    ncshm_destroy_hdl(NCS_SERVICE_ID_COMMON, 
                        (uns32)active_subtn_res_info->info.active_vdest.active_route_info->tmr_req_info_hdl);

                    /* Stop Timer */
                    m_NCS_TMR_STOP
                    (active_subtn_res_info->info.active_vdest.active_route_info->await_active_tmr);
                    /* Free tmr_req_info */
                    ...
                    active_subtn_res_info->info.active_vdest.active_route_info->await_active_queue = NULL;
                    active_subtn_res_info->info.active_vdest.active_route_info->last_active_svc_sub_part_ver= svc_sub_part_ver;
                             ...
                             MDS_SVC_ARCHWORD_TYPE    archword_type)
{
    ...
    subtn_res_info = (MDS_SUBSCRIPTION_RESULTS_INFO *)ncs_patricia_tree_get(&gl_mds_mcm_cb->subtn_results,(uns8 *)&subtn_res_key);
    if (subtn_res_info == NULL)
    {
...
*********************************************************/
uns32 mds_subtn_res_tbl_query (MDS_SVC_HDL svc_hdl, MDS_SVC_ID subscr_svc_id, 
                                      MDS_VDEST_ID vdest_id)
{
    uns32 status = NCSCC_RC_SUCCESS;
                                       ...
                                       MDS_VDEST_ID vdest_id, MDS_DEST adest)
{
    ...
    {
        /* Subscription result entry doesn't exist */
       m_MDS_LOG_DBG("MCM_DB : Leaving : F : mds_subtn_res_tbl_query_by_adest : Subscription Result not present");
        return NCSCC_RC_FAILURE;
    }
    else
    {
                                    ...
                                    MDS_SVC_ARCHWORD_TYPE    archword_type )
{
    ...
    if (subtn_res_info == NULL)
    {
        ...
        m_MDS_LOG_DBG("MCM_DB : Leaving : F : mds_subtn_res_tbl_change_active : Subscription Result not present");
        return NCSCC_RC_FAILURE;
    }
    else
    {/* Active or Await active entry exists */
        ...
        {/* Present entry is Await Active Entry */
            ...
            subtn_res_info->info.active_vdest.active_route_info->last_active_svc_sub_part_ver=svc_sub_part_ver;
...
*********************************************************/
uns32 mds_subtn_res_tbl_remove_active (MDS_SVC_HDL svc_hdl, MDS_SVC_ID subscr_svc_id, 
                                      MDS_VDEST_ID vdest_id)
{
    ...
    {
        /* Subscription result entry doesn't exist for active result*/
        m_MDS_LOG_DBG("MCM_DB : Leaving : F : mds_subtn_res_tbl_remove_active : Subscription Result not present");
        return NCSCC_RC_FAILURE;
    }
    else
    {
       ...
       m_MDS_LOG_DBG("MCM_DB:RemoveActive:TimerStart:AwaitActiveTmri:Hdl=0x%08x:SvcHdl=0x%08x:sbscr-svcid=%d,vdest=%d\n",
           subtn_res_info->info.active_vdest.active_route_info->tmr_req_info_hdl, 
                                    ...
                                    NCS_BOOL call_ref_val)
{
    ...
    else
    {
        active_subtn_res_info = 
            subtn_res_info->info.active_vdest.active_route_info->next_active_in_turn;
            ...
            subtn_res_info->info.active_vdest.active_route_info->tmr_running == FALSE)
        {
            ...
            else
            {
                ...
                mds_subtn_res_tbl_change_active(svc_hdl, subscr_svc_id, vdest_id,
                                            next_active_result_info,subtn_res_info->rem_svc_sub_part_ver,subtn_res_info->rem_svc_arch_word);
            }
                                ...
                                MDS_SUBSCRIPTION_RESULTS_INFO **result)
{
    ...
    {
        /* Subscription result entry doesn't exist for active result*/
        m_MDS_LOG_DBG("MCM_DB : Leaving : F : mds_subtn_res_tbl_get_by_adest : Subscription Result not present");
        return NCSCC_RC_FAILURE;
    }
    else
    {
                                        ...
                                        MDS_SUBSCRIPTION_RESULTS_INFO **result)/* use for bcast */
{
    ...
    while (subtn_res_info != NULL)
    {
            ...
            V_DEST_RL_ACTIVE == subtn_res_info->info.vdest_inst.role)
        {
            ...
            {/* Entry is Active/Await Active result entry */
                if (subtn_res_info->info.active_vdest.active_route_info->tmr_running == TRUE)
                {/* It is Await Active entry */
                    *result = subtn_res_info;
        ...
        subtn_res_info = (MDS_SUBSCRIPTION_RESULTS_INFO *)ncs_patricia_tree_getnext(&gl_mds_mcm_cb->subtn_results,
...
uns32 mds_subtn_res_tbl_getnext_by_adest (MDS_DEST adest, MDS_SUBSCRIPTION_RESULTS_KEY *key, MDS_SUBSCRIPTION_RESULTS_INFO **ret_result) 
{
                                    ...
                                    MDS_SUBSCRIPTION_RESULTS_INFO **next_active_result)
{
    ...
    if (current_active_result == NULL)
    {/* Searching started from first node of tree itself */
...
void ncsmds_pp()
{
    ...
    while (vdest_info != NULL)
    {
        ...
        printf("|  %5d   |  %10lld  |    %c   |   %c  |    %5d    |\n",
            vdest_info->vdest_id, (long long)(vdest_info->subtn_ref_val), policy, role, vdest_info->tmr_running);
        printf("\n|-----------------------------------------------------------------\n");
            ...
            while (svc_info != NULL)
            {
                    ...
                    m_MDS_GET_VDEST_ID_FROM_SVC_HDL(svc_info->svc_hdl) == vdest_info->vdest_id)
                {
                    ...
                    printf("|  %5d | %20lld |        %c      |  %8x  | %5d |  %5d |\n",
                        svc_info->svc_id, svc_info->svc_hdl, install_scope, (int)svc_info->yr_svc_hdl, 
                            svc_info->q_ownership, svc_info->seq_no);
                    ...
                    while (subtn_info != NULL)
                    {
                        ...
                        while (subtn_res_info != NULL)
                        {
                            ...
                            if (subtn_res_info->key.adest == 0)
                            {/* This is active instance entry */
                                if (subtn_res_info->info.active_vdest.active_route_info->tmr_running == FALSE)
                                {
                                    printf("|CUR_ACTIVE| 0x%08x | %10d |######|\n",
                                    m_MDS_GET_NODE_ID_FROM_ADEST(subtn_res_info->info.active_vdest.active_route_info->next_active_in_turn->key.adest),
                                    m_MDS_GET_PROCESS_ID_FROM_ADEST(subtn_res_info->info.active_vdest.active_route_info->next_active_in_turn->key.adest));
                                }
                            }
                            else if (subtn_res_info->key.vdest_id == m_VDEST_ID_FOR_ADEST_ENTRY)
                            {/* This is ADEST entry */
                                printf("|  ADEST   | 0x%08x | %10d |   A  |\n", 
                                        m_MDS_GET_NODE_ID_FROM_ADEST(subtn_res_info->key.adest), 
                                        m_MDS_GET_PROCESS_ID_FROM_ADEST(subtn_res_info->key.adest));
                            }
                            else
                            {/* This is VDEST entry */
                                
                                if (subtn_res_info->info.vdest_inst.role == V_DEST_RL_ACTIVE)
                                    subtn_res_role = 'A';
                                else
                                    subtn_res_role = 'S';
