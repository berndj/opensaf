...
NID_SCAP_DAEMON_STARTED=1
NID_SCAP_DAEMON_NOT_FND=2
NID_SCAP_DAEMON_START_FAILED=3
...
start()
{
    ...
    if [ ! -x $BINPATH/$DAEMON ]; then
       ...
       echo "$NID_MAGIC:$SERVICE_CODE:$NID_SCAP_DAEMON_NOT_FND" > $NIDFIFO
       exit 1
    fi
    killall ncs_scap
	...
	echo "$NID_MAGIC:$SERVICE_CODE:$NID_SCAP_DAEMON_START_FAILED" > $NIDFIFO
	exit 0
     else
