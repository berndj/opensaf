...
NID_EDS_DAEMON_STARTED=1
NID_EDS_DAEMON_NOT_FND=2
NID_EDS_DAEMON_START_FAILED=3
...
start()
{
    ...
    if [ ! -x $BINPATH/$DAEMON ]; then
       ...
       echo "$NID_MAGIC:$SERVICE_CODE:$NID_EDS_DAEMON_NOT_FND" > $NIDFIFO
       exit 1
    fi
    killall ncs_eds
	...
	echo "$NID_MAGIC:$SERVICE_CODE:$NID_EDS_DAEMON_START_FAILED" > $NIDFIFO
	exit 0
     else
