...
uns32 mbcsv_lib_init (NCS_LIB_REQ_INFO *req_info)
{
   ...
   {
      return m_MBCSV_DBG_SINK(SA_AIS_ERR_INIT,
         "Lib init request failed: MBCA already created.");
   }
...
uns32 mbcsv_lib_destroy (void)
{
   ...
   {
      return m_MBCSV_DBG_SINK(SA_AIS_ERR_EXIST,
   ...
   */
      ...
      (const uns8 *)&svc_id)))
   {
          ...
          mbcsv_rmv_reg_inst((MBCSV_REG *)&mbcsv_cb.reg_list, mbc_reg))
      {
         ...
         m_MBCSV_DBG_SINK_SVC(SA_AIS_ERR_LIBRARY,
           "Failed to remove this service instance",
           mbc_reg->svc_id);
      }
   }
   ...
   if (NCSCC_RC_SUCCESS != mbcsv_log_unbind())
   {
      /* Not required to return for failure, log the err message and go 
         ahead with cleanup */
      m_MBCSV_DBG_SINK(SA_AIS_ERR_FAILED_OPERATION,
         "Lib init request failed.");
   }
#endif
