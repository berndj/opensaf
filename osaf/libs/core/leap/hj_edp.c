			       ...
			       EDU_BUF_ENV *buf_env, EDP_OP_TYPE op, EDU_ERR *o_err)
{
	...
	if (op == EDP_OP_TYPE_ENC) {
		...
		memset(*d_ptr, '\0', sizeof(NCSMIB_PARAM_VAL));
		struct_ptr = *d_ptr;
	} else {
		struct_ptr = ptr;
	}
	rc = m_NCS_EDU_RUN_RULES(hdl, edu_tkn, ncsmib_param_val_rules, struct_ptr, ptr_data_len, buf_env, op, o_err);
	return rc;
}
			 ...
			 NCSCONTEXT ptr, uns32 *ptr_data_len, EDU_BUF_ENV *buf_env, EDP_OP_TYPE op, EDU_ERR *o_err)
{
	...
	if (op == EDP_OP_TYPE_ENC) {
		...
		memset(*d_ptr, '\0', sizeof(NCSMIB_IDX));
		struct_ptr = *d_ptr;
	} else {
		struct_ptr = ptr;
	}
	rc = m_NCS_EDU_RUN_RULES(hdl, edu_tkn, ncsmib_idx_rules, struct_ptr, ptr_data_len, buf_env, op, o_err);
	return rc;
}
...
ncs_edp_ncs_trap_varbind(EDU_HDL *edu_hdl,
			 EDU_TKN *edu_tkn,
			 NCSCONTEXT ptr,
			 uns32 *ptr_data_len, EDU_BUF_ENV *buf_env, EDP_OP_TYPE operation, EDU_ERR *o_err)
{
	...
	/* decode operation */
		...
		if (*d_trap_varbind == NULL) {
			...
			*d_trap_varbind = m_MMGR_NCS_TRAP_VARBIND_ALLOC;
			...
			memset(*d_trap_varbind, 0, sizeof(NCS_TRAP_VARBIND));
		}
	...
	status = m_NCS_EDU_RUN_RULES(edu_hdl, edu_tkn, trap_varbind_rules,
				     trap_varbind, ptr_data_len, buf_env, operation, o_err);
		       ...
		       NCSCONTEXT ptr, uns32 *ptr_data_len, EDU_BUF_ENV *buf_env, EDP_OP_TYPE op, EDU_ERR *o_err)
{
	...
	if (op == EDP_OP_TYPE_ENC) {
		...
		memset(*d_ptr, '\0', sizeof(NCS_TRAP));
		struc_ptr = *d_ptr;
	} else {
		struc_ptr = ptr;
	}
	rc = m_NCS_EDU_RUN_RULES(edu_hdl, edu_tkn, ncs_edu_trap_rules, struc_ptr, ptr_data_len, buf_env, op, o_err);
	return rc;
}
