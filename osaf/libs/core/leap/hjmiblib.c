 ...
 */
EXTERN_C uns32 miblib_validate_mib_obj(NCSMIB_OBJ_INFO *val_obj, NCSMIB_ARG *arg);
EXTERN_C uns32 miblib_validate_set_mib_obj(NCSMIB_OBJ_INFO *val_obj, NCSMIB_PARAM_VAL *param);

EXTERN_C uns32 miblib_validate_row_status(uns32 *row_status, NCSCONTEXT row);

EXTERN_C uns32 miblib_process_getrow_nextrow(NCSCONTEXT cb,
					     NCSMIB_ARG *args, uns32 *next_inst_id, uns32 *next_inst_id_len);
...
EXTERN_C uns32 miblib_process_setrow_testrow(NCSCONTEXT cb, NCSMIB_ARG *args);
...
EXTERN_C uns32 miblib_set_obj_value(NCSMIB_PARAM_VAL *param_val,
				    NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCS_BOOL *val_same_flag);

EXTERN_C uns32 miblib_validate_index(NCSMIB_OBJ_INFO *val_obj, NCSMIB_IDX *idx, NCS_BOOL next_flg);

EXTERN_C uns32 miblib_validate_param_val(NCSMIB_VAR_INFO *var_info, NCSMIB_PARAM_VAL *param, NCS_BOOL next_flg);
...
uns32 ncsmiblib_get_obj_val(NCSMIB_PARAM_VAL *param_val, NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
			   ...
			   NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCS_BOOL *val_same_flag)
{
	...
	case NCSMIB_FMAT_BOOL:
		switch (var_info->len) {
		case sizeof(uns8):
			if (*(uns8 *)((uns8 *)data + var_info->offset) == (uns8)(param_val->info.i_int))
				*val_same_flag = TRUE;
			else
				*(uns8 *)((uns8 *)data + var_info->offset) = (uns8)(param_val->info.i_int);
			break;

		case sizeof(uns16):
			if (*(uns16 *)((uns8 *)data + var_info->offset) == (uns16)param_val->info.i_int)
				*val_same_flag = TRUE;
			else
				*(uns16 *)((uns8 *)data + var_info->offset) = (uns16)param_val->info.i_int;
			break;

		case sizeof(uns32):
			if (*(uns32 *)((uns8 *)data + var_info->offset) == (uns32)param_val->info.i_int)
				*val_same_flag = TRUE;
			else
				*(uns32 *)((uns8 *)data + var_info->offset) = (uns32)param_val->info.i_int;
			break;
	...
	case NCSMIB_FMAT_OCT:
		...
		if (param_val->i_length == 0) {
			memset(((uns8 *)data + var_info->offset), 0, max_len);
			*val_same_flag = FALSE;
			break;
		}

		if (memcmp(((uns8 *)data + var_info->offset), param_val->info.i_oct, param_val->i_length) == 0) {
			uns32 i;
			...
			if (i == max_len)
				*val_same_flag = TRUE;
				...
				memset(((uns8 *)data + var_info->offset), 0, max_len);
				memcpy(((uns8 *)data + var_info->offset), param_val->info.i_oct, param_val->i_length);
			}
		} else {
			memset(((uns8 *)data + var_info->offset), 0, max_len);
			memcpy(((uns8 *)data + var_info->offset), param_val->info.i_oct, param_val->i_length);
 ...
 * Notes:       .
 *****************************************************************************/
uns32 miblib_validate_index(NCSMIB_OBJ_INFO *val_obj, NCSMIB_IDX *idx, NCS_BOOL next_flg)
{
	uns16 i;
 ...
 * Notes:       
 *****************************************************************************/
uns32 miblib_validate_param_val(NCSMIB_VAR_INFO *var_info, NCSMIB_PARAM_VAL *param, NCS_BOOL next_flg)
{
 ...
 *              which this object belongs.
 *****************************************************************************/
uns32 miblib_validate_set_mib_obj(NCSMIB_OBJ_INFO *val_obj, NCSMIB_PARAM_VAL *param)
{
...
/*===========================================================================
  ...
  Notes    :  
===========================================================================*/
uns32 miblib_process_getrow_nextrow(NCSCONTEXT cb, NCSMIB_ARG *args, uns32 *next_inst_id, uns32 *next_inst_id_len)
{
	...
	if (tbl_info->row_in_one_data_struct == TRUE) {
		...
		if (args->i_op == NCSMIB_OP_REQ_GETROW) {
			...
			ret_code = (tbl_info->next) (cb, args, &data, next_inst_id, next_inst_id_len);
		}
		...
		for (param_id = 1; param_id <= tbl_info->num_objects; param_id++) {
			...
			if ((ret_code = (tbl_info->extract) (&param_val, param_var_info, data, buffer))
			    != NCSCC_RC_SUCCESS) {
				return ret_code;
			}
...
uns32 miblib_process_setrow_testrow(NCSCONTEXT cb, NCSMIB_ARG *args)
{
	...
	ret_code = (tbl_info->setrow) (cb, args, setrow_params, miblib_obj_info[args->i_tbl_id], test_flag);
...
uns32 miblib_process_register_req(NCSMIB_OBJ_INFO *obj_info)
{
	...
	if (obj_info->tbl_info.table_id >= MIB_UD_TBL_ID_END) {
		...
		if ((obj_info->tbl_info.extract == NULL) ||
		    ...
		    (obj_info->tbl_info.next == NULL) || (obj_info->tbl_info.setrow == NULL)) {
			return NCSCC_RC_FAILURE;
		}
...
uns32 miblib_process_setallrows(NCSCONTEXT cb, NCSMIB_ARG *args)
{
	...
	for (j = 1; j <= row_cnt; j++) {
		...
		ret_code = (tbl_info->setrow) (cb, &lcl_setrow, setrow_params, miblib_obj_info[args->i_tbl_id], FALSE);
...
uns32 miblib_process_mib_op_req(NCSCONTEXT cb, NCSMIB_ARG *args)
{
	...
	if ((args->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) != NCSMIB_POLICY_PSS_BELIEVE_ME) {
		/* Validate the tbl_info & var_info, if the request is not from
		   PSSv. */
		if ((ret_code = miblib_validate_mib_obj(miblib_obj_info[args->i_tbl_id], args))
		    != NCSCC_RC_SUCCESS) {
			goto CALL_RESP;
		}
	}
	...
	case NCSMIB_OP_REQ_GET:
		...
		if ((ret_code = (tbl_info->get) (cb, args, &data)) != NCSCC_RC_SUCCESS) {
			break;
		}

		/* Get the object value from the subsystem data-structure */
		ret_code = (tbl_info->extract) (&(io_get_rsp->i_param_val),
						&var_info[i_get_req->i_param_id - 1], data, buffer);

		break;

	case NCSMIB_OP_REQ_SET:
		io_set_rsp->i_param_val = i_set_req->i_param_val;
		ret_code = (tbl_info->set) (cb, args, &var_info[i_set_req->i_param_val.i_param_id - 1], FALSE);
		break;

	case NCSMIB_OP_REQ_TEST:
		io_test_rsp->i_param_val = i_test_req->i_param_val;
		ret_code = (tbl_info->set) (cb, args, &var_info[i_test_req->i_param_val.i_param_id - 1], TRUE);
		break;
	...
	case NCSMIB_OP_REQ_NEXT:
		...
		if (data != NULL)
			ret_code = (tbl_info->extract) (&(io_next_rsp->i_param_val),
							&var_info[i_next_req->i_param_id - 1], data, buffer);

		break;

	case NCSMIB_OP_REQ_NEXTROW:
		ret_code = miblib_process_getrow_nextrow(cb, args, next_inst_id, &next_inst_id_len);
		/* Set the next inst_id pointer to the array on the stack */
		if (ret_code == NCSCC_RC_SUCCESS) {
...
uns32 ncsmiblib_process_req(NCSMIBLIB_REQ_INFO *req_info)
{
	...
	case NCSMIBLIB_REQ_INIT_OP:
		memset(miblib_obj_info, 0, (MIB_UD_TBL_ID_END * sizeof(NCSMIB_OBJ_INFO *)));
		ret_code = NCSCC_RC_SUCCESS;
		break;
