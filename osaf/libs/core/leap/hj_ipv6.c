...
void ncs_mask_prefix_addr(NCS_IPV6_ADDR * addr, uns32 pfxlen, NCS_IPV6_ADDR * o_masked_addr)
{
	...
	if ((15 - byte_to_be_masked) > 0) {
		 ...
		 */
		memset(&o_masked_addr->ipv6.ipv6_addr8[byte_to_be_masked + 1], 0, 15 - byte_to_be_masked);
	}
...
{
	if ((addr->ipv6.ipv6_addr8[0] == 0xfe) && ((uns8)(addr->ipv6.ipv6_addr8[1] & 0xC0) == (uns8)0x80)) {
		return TRUE;
	}
	return FALSE;
}
...
NCS_BOOL ncs_is_valid_ipv6_uc(NCS_IPV6_ADDR * addr, uns8 bits)
{
	...
	if ((addr->ipv6.ipv6_addr32[0] == 0) && (addr->ipv6.ipv6_addr32[1] == 0) &&
	    (addr->ipv6.ipv6_addr32[2] == 0) && (addr->ipv6.ipv6_addr16[6] == 0) && (addr->ipv6.ipv6_addr8[14] == 0)) {
...
void ncs_ipv6_str_from_ipv6addr(NCS_IPV6_ADDR * i_v6addr, int8 *o_str)
{
	...
	for (i = 0; i < NCS_IPV6_ADDR_UNS16_CNT; i++) {
		temp_addr.ipv6.ipv6_addr16[i] = (uns16)m_NCS_OS_NTOHS((uns16)temp_addr.ipv6.ipv6_addr16[i]);
	}
	...
	sprintf(o_str, "%x:%x:%x:%x:%x:%x:%x:%x", temp_addr.ipv6.ipv6_addr16[0],
		...
		temp_addr.ipv6.ipv6_addr16[5], temp_addr.ipv6.ipv6_addr16[6], temp_addr.ipv6.ipv6_addr16[7]);
