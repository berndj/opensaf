...
LEAPDLL_API void ncs_mem_dump_entry(NCS_MPOOL_ENTRY *me,
				    uns32 i,
				    unsigned int ignore_count,
				    char *pBuf, LEAP_PRINT_CALLBACK ptr_custom, void *ucontext);

LEAPDLL_API uns32 ncs_mem_whatsout(NCS_MPOOL_ENTRY_DBG *entry, uns32 slots);

LEAPDLL_API uns32 ncs_mem_fetch_stat(uns32 size, NCS_MPOOL_STAT *storage);
...
uns32 ncs_mem_create()
{
  ...
  /** Allocate and zero out the array of pools. **/
	...
	for (mdex = 0, mp = mmgr.mpools, size = 16; mdex <= MMGR_NUM_POOLS; mdex++, mp++, size *= 2) {
...
#if ((NCS_MMGR_DEBUG == 1) || (NCSSYSM_MEM_DBG_ENABLE == 1))
		if (mp->inuse != NULL) {	/* Just to put a breakpoint */
			temp = 1;
		}
#endif
...
void *ncs_mem_alloc(unsigned int nbytes,
		    void *mem_region, NCS_SERVICE_ID service_id, unsigned int sub_id, unsigned int line, char *file)
{
	NCS_MPOOL_ENTRY *me;
	NCS_MPOOL *mp;
	unsigned int len;
		...
		for (index = 0; index < NCSSYSM_STKTRACE_INFO_MAX; index++) {
			if (me->line == mmgr.stktrace_info[index].line) {
				if (0 == strcmp(pfile, mmgr.stktrace_info[index].file)) {
					me->stkentry =
					    m_NCS_OS_MEMALLOC(sizeof(NCS_OS_STKTRACE_ENTRY), NCS_MEM_REGION_PERSISTENT);
	     ...
	     void *mem_region, NCS_SERVICE_ID service_id, unsigned int sub_id, unsigned int line, char *file)
{
  ...
  *
  ****************************************************************************/
void *ncs_mem_realloc(void *payload,
		      unsigned int nbytes, void *mem_region, NCS_SERVICE_ID service_id, unsigned int sub_id)
{
	NCS_MPOOL_ENTRY *me;
	NCS_MPOOL *mp;
	char *temp_p = NULL;
...
*
****************************************************************************/
uns32 ncs_mem_stats_dump(char *filename)
{
#if (NCS_MMGR_DEBUG == 1)
	...
	fh == NULL ? printf("%s", output_string) : fprintf(fh, output_string);
	sprintf(output_string,
		"|   size |  in-use|   avail|  hwater|  allocs|   frees|  errors|  falloc| freed to os\n");
	fh == NULL ? printf("%s", output_string) : fprintf(fh, output_string);
	sprintf(output_string,
		"|--------+--------+--------+--------+--------+--------+--------+--------+-------------|\n");
	fh == NULL ? printf("%s", output_string) : fprintf(fh, output_string);
	...
	for (mp = mmgr.mpools;; mp++) {
		...
		sprintf(output_string, "%9d%9d%9d%9d%9d%9d%9d%9d%9d\n",
			...
			mp->stat.highwater,
			mp->stat.allocs,
			mp->stat.frees, mp->stat.errors, mp->stat.failed_alloc, mp->stat.num_freed_to_os);
		fh == NULL ? printf("%s", output_string) : fprintf(fh, output_string);
...
uns32 ncs_mem_fetch_stat(uns32 size, NCS_MPOOL_STAT *storage)
{
#if (NCS_MMGR_DEBUG == 1)
	NCS_MPOOL *mp;
...
uns32 ncs_mem_whatsout_dump_custom(LEAP_PRINT_CALLBACK customptr, void *ucontext)
{
#if (NCS_MMGR_DEBUG != 0)
	...
	(*customptr) (ucontext,
		      "|---|----+----+-----------+-----+-----------+--------+---+---+----------------+----|\n");
	(*customptr) (ucontext,
		      "|         N O N - I G N O R E D   O U T S T A N D I N G   M E M O R Y              |\n");
	(*customptr) (ucontext,
		      "|---|----+----+-----------+-----+-----------+--------+---+---+----------------+----|\n");
	(*customptr) (ucontext,
		      "|  #|show|Real|   alloc'ed|Owner|Ownr claims| User ID|Svc|Sub|                |bloc|\n");
	(*customptr) (ucontext,
		      "|  #| Cnt|line|    in file| line|    in file|| marker| ID| ID|    Ptr value   |size|\n");
	(*customptr) (ucontext,
		      "|---|----+----+-----------+-----+-----------+--------+---+---+----------------+----|\n");
...
uns32 ncs_mem_whatsout(NCS_MPOOL_ENTRY_DBG *entry, uns32 slots)
{
...
void *ncs_next_mpool_entry(void *arg)
{
...
void
ncs_mem_dump_entry(NCS_MPOOL_ENTRY *me, uns32 i, unsigned int ignore_count, char *pBuf, LEAP_PRINT_CALLBACK ptr_custom,
		   void *ucontext)
{
#if (NCS_MMGR_DEBUG == 1)
...
uns32 ncs_mem_whatsout_summary(char *mem_file)
{
		...
		for (me = mp->inuse; me != NULL; me = me->next) {
			...
			for (test = hash[me->service_id]; test != NULL; test = test->next) {
				if ((test->me->line == me->line) &&
				    ...
				    (test->me->count == me->count) && (strcmp(me->file, test->me->file) == 0)) {
					test->hit_cnt++;
					found = TRUE;
					break;
				}
	...
	for (itr = 0; itr < NCS_SERVICE_ID_MAX; itr++) {
		for (test = hash[itr]; test != NULL; test = test->next) {
			sprintf(pBuf, "|%6d|%5d|%13s|%6d|%4d|%4d|%5d|\n", test->hit_cnt,	/* hits  */
				...
				test->me->sub_id, test->me->real_size);
...
void ncs_mem_ignore(NCS_BOOL yes_r_no)
{
#if (NCS_MMGR_DEBUG != 0)
...
void ncs_mem_ignore_subsystem(unsigned int zero_or_one, unsigned int service_id)
{
#if (NCS_MMGR_DEBUG != 0)
...
void ncs_mem_ignore_timestamp(unsigned int zero_or_one)
{
