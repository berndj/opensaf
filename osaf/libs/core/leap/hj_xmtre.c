 ...
 *******************************************************************/
static void m_xmtree_set_bit(char *pKey, uns32 keyLen, uns32 maxKeyLen)
{
 ...
 *******************************************************************/
static int m_xmtree_is_key_zero(char *pKey, uns32 maxKeyLen)
{
	uns32 byteCount = 0;
...
static void m_xmtree_mask_key(char *pDestKey, char *pSrcKey, int maskLen, uns32 maxKeyLen)
{
	uns32 i;
...
static void m_xmtree_invert_mask_and_or_key(char *pDestKey, char *pSrcKey, int maskLen, uns32 maxKeyLen)
{
	uns32 i;
	int j = 0xff;
 ...
 *******************************************************************/
static void m_xmtree_gen_mask(char *pMask, uns32 maskLen, uns32 maxKeyLen)
{
	uns32 byteCount = 0x0;
	uns32 tempCount = 0x0;
	...
	if (maskLen > 0) {
		...
		if ((maskLen % 8) != 0)
			pMask[m_XMTREE_BYTE_POSITION(byteCount, maxKeyLen)] = xmtreeBitMasks[(8 - (maskLen % 8))];
	}
	return;
}
 ...
 *******************************************************************/
static void m_xmtree_and_keys(char *pDestKey, char *pSrcKey1, char *pSrcKey2, uns32 maxKeyLen)
{
	uns32 tempCount;
 ...
 *******************************************************************/
static void m_xmtree_increment_key(char *pDestKey, char *pSrcKey, uns32 maxKeyLen)
{
	uns32 tempKeyLen = 0;
	uns32 bytePosition;
 ...
 *******************************************************************/
static int m_xtree_compare_with_mask(char *pKey1, char *pKey2, int keyLength, uns32 maxKeyLen)
{
	int bitCount;
	int byteCount;
	int tempByteCount;
 ...
 * Purpose:   Finds the first bit possition where the given sets 
              of keys differs
 *******************************************************************/
static int m_xmtree_find_first_diff(char *pKey1, char *pKey2, uns32 keyMaxsize)
{
	uns32 byteCounter = 0;
	uns32 bitCounter = 0;
	uns32 tempByteCount;
 ...
 ***************************************************************/
unsigned int xmtree_init(NCS_MTREE *const pTree)
{
...
/* Initialize the root node, which is actually part of the tree structure. */
	...
	pTree->Style.Pat.Root.Style.Pat.pLeft = pTree->Style.Pat.Root.Style.Pat.pRight = &pTree->Style.Pat.Root;
	pTree->Style.Pat.Root.Style.Pat.pDownToMe =
	    pTree->Style.Pat.Root.Style.Pat.pMoreSpecific =
	    pTree->Style.Pat.Root.Style.Pat.pLessSpecific = NCS_MTREE_NODE_NULL;
 ...
 *****************************************************************/
void xmtree_dest(NCS_MTREE *const pTree)
{
}
							 ...
							 short KeyBitSize, void *Cookie), void *const Cookie)
{
	...
	if (m_XMTREE_KEY_CMP(pKey, pSrch->pKey, pTree->Cfg.KeyOctetSize) == 0) {
		...
		if (pSrch->KeyBitSize < KeyBitSize) {
			...
			if ((pNode = (*pFunc) (pKey, KeyBitSize, Cookie)) != NCS_MTREE_NODE_NULL) {
				...
				pSrch->Style.Pat.pLeft =
				    pSrch->Style.Pat.pRight = pSrch->Style.Pat.pDownToMe = NCS_MTREE_NODE_NULL;
		...
		if ((pNode = (*pFunc) (pKey, KeyBitSize, Cookie)) != NCS_MTREE_NODE_NULL) {
			...
			pNode->Style.Pat.pRight =
			    pNode->Style.Pat.pLeft = pNode->Style.Pat.pDownToMe = NCS_MTREE_NODE_NULL;
	...
	if ((pNode = (*pFunc) (pKey, KeyBitSize, Cookie)) != NCS_MTREE_NODE_NULL) {
		...
		pNode->Style.Pat.pLessSpecific = pNode->Style.Pat.pMoreSpecific = NCS_MTREE_NODE_NULL;
 ...
 ******************************************************************/
void xmtree_del(NCS_MTREE *const pTree, NCS_MTREE_NODE *const pNode)
{
	...
	if ((pTemp = pNode->Style.Pat.pMoreSpecific) != NCS_MTREE_NODE_NULL) {
		...
		if ((pTemp->Style.Pat.pLessSpecific = pNode->Style.Pat.pLessSpecific)
		    != NCS_MTREE_NODE_NULL) {
			pNode->Style.Pat.pLessSpecific->Style.Pat.pMoreSpecific = pNode->Style.Pat.pMoreSpecific;
		}
		return;
	}
	...
	pLegDownToNode = ((pPrev->Style.Pat.pLeft == pNode) ? &pPrev->Style.Pat.pLeft : &pPrev->Style.Pat.pRight);
	pPrevLeg = pLegDownToNode;
	pNext = pNode;

	/* Keep going 'down' until we find the leg which points back to pNode. */
	do {
		UpWentRight = m_XMTREE_BIT_IS_SET(pNode->pKey, pNext->Style.Pat.Bit, pTree->Cfg.KeyOctetSize);
		pNextLeg = ((UpWentRight) ? &pNext->Style.Pat.pRight : &pNext->Style.Pat.pLeft);
		pTemp = *pNextLeg;
	...
	*pPrevLeg = ((UpWentRight) ? pNext->Style.Pat.pLeft : pNext->Style.Pat.pRight);
 ...
 start_searching_best:
	...
	*pMoreSpecificExists = FALSE;	/* Assume not. */
	...
	do {
		...
		if (pSrch->Style.Pat.Bit <= pPrev->Style.Pat.Bit) {
			if ((pSrch->KeyBitSize > KeyBitSize) &&
			    (!m_XMTREE_KEY_MASK_CMP(pTarget, pSrch->pKey, KeyBitSize, pTree->Cfg.KeyOctetSize))) {
		...
		if (m_XMTREE_KEY_MASK_CMP(pTarget, pSrch->pKey, pSrch->Style.Pat.Bit, pTree->Cfg.KeyOctetSize)) {
	...
	while (pSrch->KeyBitSize >= pPrev->Style.Pat.Bit) {
		if ((pSrch->KeyBitSize <= pSrch->Style.Pat.Bit) &&
		    ...
		    (!m_XMTREE_KEY_MASK_CMP(pTarget, pSrch->pKey, pSrch->KeyBitSize, pTree->Cfg.KeyOctetSize))
		    ) {
			if (Dummy != 0)
				*pMoreSpecificExists = firstMoreSpecExist;
			return pSrch;
 ...
 Lookback:
	...
	while (pPrev != NCS_MTREE_NODE_NULL) {
		...
		while (KeyBitSize > pPrev->Style.Pat.Bit) {
			if (!m_XMTREE_KEY_MASK_CMP(pTarget, pSrch->pKey, KeyBitSize, pTree->Cfg.KeyOctetSize)) {
 ...
 ***********************************************************************/
NCS_MTREE_NODE *xmtree_next(NCS_MTREE *const pTree, const uns32 *const pTarget, short KeyBitSize)
{
 ...
 backup:
			...
			do {
				...
				if (pSrch == pPrev->Style.Pat.pLeft) {
					...
					m_XMTREE_MASK_KEY(Target, pSrch->pKey, pSrch->Style.Pat.Bit,
							  pTree->Cfg.KeyOctetSize);

					/*generating the mask for the given number of bits */
					/*set the particular bit */
					m_XMTREE_SET_BIT(Target, (1 + pSrch->Style.Pat.Bit), pTree->Cfg.KeyOctetSize);
			 ...
			 * too far. */
			if (m_XMTREE_KEY_MASK_CMP(Target, pSrch->pKey, pSrch->Style.Pat.Bit, pTree->Cfg.KeyOctetSize)) {
 ...
 * Purpose:   Finds 'next' key.
 ***********************************************************************/
NCS_MTREE_NODE *xmtree_get_next_key(NCS_MTREE *const pTree, const uns32 *const pKey, uns32 maskLen, short KeyBitSize)
{
	...
	while (pSearchNode != NULL) {
		m_XMTREE_MASK_KEY(pSearchKey, pSearchNode->pKey, maskLen, pTree->Cfg.KeyOctetSize);
		m_XMTREE_MASK_KEY(pInputKey, pKey, maskLen, pTree->Cfg.KeyOctetSize);
		if (m_XMTREE_KEY_CMP(pSearchKey, pInputKey, pTree->Cfg.KeyOctetSize) == 0) {
			return pSearchNode;
		}

		pSearchNode = xmtree_next(pTree, pSearchNode->pKey, pSearchNode->KeyBitSize);
 ...
 ***************************************************************/
unsigned int ncs_xmtree_init(NCS_MTREE *const pTree, const NCS_MTREE_CFG *const pCfg)
{
...
void ncs_xmtree_destroy(NCS_MTREE *const pTree)
{
 ...
 ***********************************************************************/
static NCS_MTREE_NODE *XmtreePrivateCallback(const void *pKey, short KeyBitSize, void *pNode)
{
...
unsigned int ncs_xmtree_add(NCS_MTREE *const pTree, NCS_MTREE_NODE *const pNode)
{
	NCS_MTREE_NODE *pRetNode;

	pRetNode = ncs_xmtree_find_add(pTree, pNode->pKey, pNode->KeyBitSize, XmtreePrivateCallback, pNode);
	if (pRetNode == pNode) {
		return NCSCC_RC_SUCCESS;
	} else {
		return NCSCC_RC_FAILURE;
 ...
 *******************************************************************/
NCS_MTREE_NODE *ncs_xmtree_find_add(NCS_MTREE *const pTree,
 ...
 * Returns:  nothing.
 ******************************************************************/
void ncs_xmtree_del(NCS_MTREE *const pTree, NCS_MTREE_NODE *const pNode)
{
 ...
 *******************************************************************/
NCS_MTREE_NODE *ncs_xmtree_find_nextbest(NCS_MTREE *const pTree, const NCS_MTREE_NODE *const pBetterNode)
{
 ...
 *******************************************************************/
NCS_MTREE_NODE *ncs_xmtree_find_best(NCS_MTREE *const pTree,
				     const void *const pKey, uns16 KeyBitSize, int *const pMoreSpecificExists)
{
	return ((*XmtreeActions.pBestFunc) (pTree, pKey, KeyBitSize, pMoreSpecificExists));
}
 ...
 ***********************************************************************/
NCS_MTREE_NODE *ncs_xmtree_next(NCS_MTREE *const pTree, const void *const pKey, short KeyBitSize)
{
