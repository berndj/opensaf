...
static uns32 oaclib_oac_create(NCS_LIB_REQ_INFO *req_info);

/* to create the mailbox and thread for OAA */
static uns32 oaclib_oac_create_ipc_task();

/* instantiate OAA in a particular PWE */
static uns32 oaclib_oac_instantiate(NCS_LIB_REQ_INFO *req_info);

/* un-instantiate OAA in a particular PWE */
static uns32 oaclib_oac_uninstantiate(PW_ENV_ID i_env_id, SaNameT i_inst_name, uns32 i_oac_hdl);

/* to add to the global list of instances */
static uns32 oac_inst_list_add(PW_ENV_ID env_id, uns32 i_oac_hdl, SaNameT i_inst_name);

/* to delete and release a particular OAA instance from the list */
static void oac_inst_list_release(PW_ENV_ID env_id);
...
uns32 oaclib_request(NCS_LIB_REQ_INFO *req_info)
{
	...
	case NCS_LIB_REQ_UNINSTANTIATE:
		status = oaclib_oac_uninstantiate(req_info->info.uninst.i_env_id,
						  req_info->info.uninst.i_inst_name, req_info->info.uninst.i_inst_hdl);
		break;
...
static uns32 oaclib_oac_create(NCS_LIB_REQ_INFO *req_info)
{
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR, MAB_OAC_ERR_SPLR_REG_FAILED, status, 0);
		/* unbind with DTS */
		mab_log_unbind();
		return status;
	}
...
static uns32 oaclib_oac_create_ipc_task()
{
	...
	if (gl_oacmbx.oac_mbx == (SYSF_MBX)((long)NULL)) {	/* (To avoid the sysf_ipc.c mem leak) */
		...
		if (status != NCSCC_RC_SUCCESS) {
			/* log the failure */
			m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_OAC_ERR_IPC_CREATE_FAILED, status);
			/* unbind with DTS */
			mab_log_unbind();
			return status;
		}
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_OAC_ERR_IPC_ATTACH_FAILED, status);
	...
	status = m_NCS_TASK_CREATE((NCS_OS_CB)oac_do_evts,
				   ...
				   NCS_MAB_OAC_PRIORITY, NCS_MAB_OAC_STACKSIZE, &gl_oacmbx.oac_mbx_hdl);
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_OAC_ERR_TASK_CREATE_FAILED, status);
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_OAC_ERR_TASK_START_FAILED, status);
...
static uns32 oaclib_oac_instantiate(NCS_LIB_REQ_INFO *req_info)
{
	...
	memcpy(&oac_lmarg.info.create.i_inst_name, &req_info->info.inst.i_inst_name, sizeof(SaNameT));
	...
	if (status != NCSCC_RC_SUCCESS) {
		...
		m_LOG_MAB_ERROR_II(NCSFL_LC_FUNC_RET_FAIL, NCSFL_SEV_ERROR,
				   MAB_OAC_ERR_CREATE_FAILED, status, req_info->info.inst.i_env_id);
		return status;
	}
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR,
				   MAB_OAC_ERR_INST_LIST_ADD_FAILED, status, req_info->info.inst.i_env_id);
		return status;
	}
...
static uns32 oaclib_oac_uninstantiate(PW_ENV_ID i_env_id, SaNameT i_inst_name, uns32 i_oac_hdl)
{
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_II(NCSFL_LC_FUNC_RET_FAIL, NCSFL_SEV_ERROR,
				   MAB_OAC_ERR_DESTROY_FAILED, status, i_env_id);
		return status;
	}
	...
	m_LOG_MAB_HDLN_I(NCSFL_SEV_INFO, MAB_HDLN_OAC_UNINSTANTIATE_SUCCESS, i_env_id);
...
*                 OAA in a prcoess                                           * 
\****************************************************************************/
uns32 oaclib_oac_destroy(NCS_LIB_REQ_INFO *req_info)
{
	uns32 status = NCSCC_RC_FAILURE;
	NCS_SPLR_REQ_INFO splr_info;
	...
	while (this_oac_inst) {
		...
		if (status != NCSCC_RC_SUCCESS) {
			/* log the failure */
			m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR,
					   MAB_OAC_ERR_SPIR_RMV_INST_FAILED, status, this_oac_inst->i_env_id);
		}
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR, MAB_OAC_ERR_SPLR_DEREG_FAILED, status, 0);
		return status;
	}
...
static uns32 oac_inst_list_add(PW_ENV_ID env_id, uns32 i_oac_hdl, SaNameT i_inst_name)
{
	...
	if (add_me == NULL) {
		...
		m_LOG_MAB_MEMFAIL(NCSFL_SEV_ERROR, MAB_MF_MAB_INST_NODE_ALLOC_FAILED, "oac_inst_list_add()");
		return NCSCC_RC_FAILURE;
	}
