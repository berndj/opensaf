...

{
   ...
   *ckptHandle = 0;
   ...
   if(cb == NULL)
   {
      m_LOG_CPA_CCL(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR,"CkptInit:HDL_TAKE", __FILE__ ,__LINE__);
      return SA_AIS_ERR_LIBRARY;
   }
   ...
   if(cl_node == NULL)
   {
      m_LOG_CPA_CCL(CPA_MEM_ALLOC_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR,"CkptInit:CPA_CLIENT_NODE", __FILE__ ,__LINE__);
   ...
   if( FALSE == cb->is_cpnd_up)
   {
       ...
       m_LOG_CPA_CCL(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_INFO,"CkptInit:CPND_DOWN", __FILE__ ,__LINE__);
       goto mds_fail;
   }
   ...
   if(out_evt)
   {
      ...
      if(rc != SA_AIS_OK)
      {
         m_LOG_CPA_CCLL(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR, "CkptInit:CPND_ERR", __FILE__ ,__LINE__, rc);
         goto rsp_not_ok;
      }
   ...
   {
      m_LOG_CPA_CCLL(CPA_PROC_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR, "CkptInit", __FILE__ ,__LINE__,rc);
                                     ...
                                     SaSelectionObjectT *selectionObject)
{
   ...
   if (m_NCS_LOCK(&cb->cb_lock, NCS_LOCK_WRITE) != NCSCC_RC_SUCCESS)
   {
      m_LOG_CPA_CCLLF(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR, "SelObjGet:LOCK", __FILE__ ,__LINE__,0,ckptHandle);
      rc = SA_AIS_ERR_LIBRARY;
      goto lock_fail;
   }
...

{
   ...
   if(!cb)
   {
      m_LOG_CPA_CCLLF(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR, "CkptDispatch:HDL_TAKE", __FILE__ ,__LINE__,0,ckptHandle);
      rc = SA_AIS_ERR_BAD_HANDLE;
      return rc;
   }
  ...
  /* get the client_info*/
   if (m_NCS_LOCK(&cb->cb_lock, NCS_LOCK_WRITE) != NCSCC_RC_SUCCESS)
   {
      m_LOG_CPA_CCLLF(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR, "CkptDispatch:LOCK", __FILE__ ,__LINE__,0,ckptHandle);
      rc = SA_AIS_ERR_LIBRARY;
      goto lock_fail;
   }
                                 ...
                                 SaCkptCheckpointHandleT *checkpointHandle)
{
  ...
  /* get the CB Lock*/
   ...
   if(checkpointCreationAttributes)
   {
      ...
      evt.info.cpnd.info.openReq.ckpt_attrib.retentionDuration = checkpointCreationAttributes->retentionDuration;
...
/****************************************************************************
  ...
  Notes         :
******************************************************************************/
SaAisErrorT saCkptCheckpointOpenAsync(SaCkptHandleT ckptHandle,
                                              ...
                                              checkpointOpenFlags)
{
  ...
  /* get the CB Lock*/
   ...
   if(proc_rc != NCSCC_RC_SUCCESS)
   {
      rc = SA_AIS_ERR_LIBRARY;
      m_LOG_CPA_CCLLFF(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR,
                           "CkptOpenAsync:lcl_ckpt_node_add", __FILE__ ,__LINE__, rc , ckptHandle, lc_node->lcl_ckpt_hdl);
      goto lc_node_add_fail;
   }
   ...
   if(lc_node->async_req_tmr.uarg)
   {
      ...
      if (!(lc_node->async_req_tmr.uarg = ncshm_create_hdl(cb->pool_id, 
                       NCS_SERVICE_ID_CPA, (NCSCONTEXT)&lc_node->async_req_tmr)))
      {
         m_LOG_CPA_CCLLFF(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR,
                           "CkptOpenAsync:HDL_CREATE", __FILE__ ,__LINE__, rc , ckptHandle, lc_node->lcl_ckpt_hdl);
         rc = SA_AIS_ERR_LIBRARY;
         goto hm_create_fail;
      }
   }
...
/****************************************************************************
  ...
  Notes         :
******************************************************************************/
SaAisErrorT  saCkptCheckpointRetentionDurationSet(SaCkptCheckpointHandleT checkpointHandle,
                                ...
                                SaUint32T *erroneousVectorIndex)
{
   ...
   proc_rc=cpa_proc_build_data_access_evt(ioVector,numberOfElements, \
                            CPSV_CKPT_ACCESS_WRITE,&ckpt_data); 
                               ...
                               SaUint32T *erroneousVectorIndex)
{
  ...
  while(counter < numberOfElements)
  {
   ...
   if (proc_rc != NCSCC_RC_SUCCESS)
   {
      rc=SA_AIS_ERR_INVALID_PARAM;
      m_LOG_CPA_CCLLF(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR,
                             "CkptRead:check_iovector", __FILE__ ,__LINE__, rc, checkpointHandle);
      goto ckpt_node_get_fail;
   }
  ...
  if(is_local_read == FALSE)
  {
   ...
   proc_rc=cpa_proc_build_data_access_evt(ioVector,numberOfElements, \
                            CPSV_CKPT_ACCESS_READ,&ckpt_data); 
   if ( proc_rc == NCSCC_RC_FAILURE)
   {
      rc=SA_AIS_ERR_INVALID_PARAM;
      m_LOG_CPA_CCLLF(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR,
...
/****************************************************************************
  ...
  Notes         :
******************************************************************************/
SaAisErrorT  saCkptCheckpointSynchronize(SaCkptCheckpointHandleT checkpointHandle,
                                      SaTimeT timeout)
{
   SaAisErrorT  rc = SA_AIS_OK;
   CPSV_EVT     evt;
...
/****************************************************************************
  ...
  Notes         :
******************************************************************************/
SaAisErrorT saCkptCheckpointSynchronizeAsync(SaCkptCheckpointHandleT checkpointHandle,
                                             SaInvocationT invocation)
{
   SaAisErrorT  rc = SA_AIS_OK;
   CPSV_EVT     evt;
   ...
   switch (proc_rc)
   {
       ...
       case NCSCC_RC_REQ_TIMOUT:
           rc = SA_AIS_ERR_TIMEOUT;
           m_LOG_CPA_CCLLFF(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR,
                             "CkptSynschronizeAsync:MDS", __FILE__ ,__LINE__, proc_rc , checkpointHandle, cb->cpnd_mds_dest);
           goto fail1;
       default:
           rc = SA_AIS_ERR_NO_RESOURCES;
           m_LOG_CPA_CCLLFF(CPA_API_FAILED, NCSFL_LC_CKPT_MGMT, NCSFL_SEV_ERROR,
                             "CkptSynchronizeAsync:MDS", __FILE__ ,__LINE__, proc_rc , checkpointHandle, cb->cpnd_mds_dest);
           goto fail1;
   }
