...
SaAisErrorT saAmfComponentRegister(SaAmfHandleT hdl, const SaNameT *comp_name, const SaNameT *proxy_comp_name)
{
	...
	if (!proxy_comp_name && (comp_name->length != m_NCS_OS_NTOHS(cb->comp_name_net.length))) {
		rc = SA_AIS_ERR_INVALID_PARAM;
		goto done;
	}

	/* proxy component, Length part of proxy component name should be Ok */
	if (proxy_comp_name && (proxy_comp_name->length != m_NCS_OS_NTOHS(cb->comp_name_net.length))) {
		rc = SA_AIS_ERR_INVALID_PARAM;
		goto done;
	}

	/* non-proxied component should not forge its name while registering */
	if (!proxy_comp_name && strncmp(comp_name->value, cb->comp_name_net.value, comp_name->length)) {
		rc = SA_AIS_ERR_BAD_OPERATION;
		goto done;
	}

	/* proxy component should not forge its name while registering its proxied */
	if (proxy_comp_name && strncmp(proxy_comp_name->value, cb->comp_name_net.value, proxy_comp_name->length)) {
		rc = SA_AIS_ERR_BAD_OPERATION;
		goto done;
	}
	...
	if (NCSCC_RC_SUCCESS == rc) {
		m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
		m_AVSV_ASSERT(AVSV_AMF_COMP_REG == msg_rsp->info.api_resp_info.type);
...
SaAisErrorT saAmfComponentUnregister(SaAmfHandleT hdl, const SaNameT *comp_name, const SaNameT *proxy_comp_name)
{
	...
	if (NCSCC_RC_SUCCESS == rc) {
		m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
		m_AVSV_ASSERT(AVSV_AMF_COMP_UNREG == msg_rsp->info.api_resp_info.type);
		rc = msg_rsp->info.api_resp_info.rc;
	} else if (NCSCC_RC_FAILURE == rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
	else if (NCSCC_RC_REQ_TIMOUT == rc)
				  ...
				  SaAmfHealthcheckInvocationT inv, SaAmfRecommendedRecoveryT rec_rcvr)
{
	...
	m_AVA_HC_START_MSG_FILL(msg, cb->ava_dest, hdl, *comp_name, cb->comp_name_net, *hc_key, inv, rec_rcvr);
	rc = ava_mds_send(cb, &msg, &msg_rsp);
	if (NCSCC_RC_SUCCESS == rc) {
		m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
		m_AVSV_ASSERT(AVSV_AMF_HC_START == msg_rsp->info.api_resp_info.type);
		rc = msg_rsp->info.api_resp_info.rc;
	} else if (NCSCC_RC_FAILURE == rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
	else if (NCSCC_RC_REQ_TIMOUT == rc)
...
SaAisErrorT saAmfHealthcheckStop(SaAmfHandleT hdl, const SaNameT *comp_name, const SaAmfHealthcheckKeyT *hc_key)
{
	...
	m_AVA_HC_STOP_MSG_FILL(msg, cb->ava_dest, hdl, *comp_name, cb->comp_name_net, *hc_key);
	rc = ava_mds_send(cb, &msg, &msg_rsp);
	if (NCSCC_RC_SUCCESS == rc) {
		m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
		m_AVSV_ASSERT(AVSV_AMF_HC_STOP == msg_rsp->info.api_resp_info.type);
		rc = msg_rsp->info.api_resp_info.rc;
	} else if (NCSCC_RC_FAILURE == rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
	else if (NCSCC_RC_REQ_TIMOUT == rc)
				    ...
				    const SaNameT *comp_name, const SaAmfHealthcheckKeyT *hc_key, SaAisErrorT hc_result)
{
	...
	m_AVA_HC_CONFIRM_MSG_FILL(msg, cb->ava_dest, hdl, *comp_name, cb->comp_name_net, *hc_key, hc_result);
	rc = ava_mds_send(cb, &msg, &msg_rsp);
	if (NCSCC_RC_SUCCESS == rc) {
		m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
		m_AVSV_ASSERT(AVSV_AMF_HC_CONFIRM == msg_rsp->info.api_resp_info.type);
		rc = msg_rsp->info.api_resp_info.rc;
	} else if (NCSCC_RC_FAILURE == rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
	else if (NCSCC_RC_REQ_TIMOUT == rc)
			 ...
			 SaInt32T desc_TreeDepth, SaAmfPmErrorsT pmErr, SaAmfRecommendedRecoveryT rec_Recovery)
{
	...
	if (NCSCC_RC_SUCCESS == rc) {
		m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
		m_AVSV_ASSERT(AVSV_AMF_PM_START == msg_rsp->info.api_resp_info.type);
		rc = msg_rsp->info.api_resp_info.rc;
	} else if (NCSCC_RC_FAILURE == rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
	else if (NCSCC_RC_REQ_TIMOUT == rc)
			...
			SaAmfPmStopQualifierT stopQual, SaInt64T processId, SaAmfPmErrorsT pmErr)
{
	...
	if (NCSCC_RC_SUCCESS == rc) {
		m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
		m_AVSV_ASSERT(AVSV_AMF_PM_STOP == msg_rsp->info.api_resp_info.type);
		rc = msg_rsp->info.api_resp_info.rc;
	} else if (NCSCC_RC_FAILURE == rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
	else if (NCSCC_RC_REQ_TIMOUT == rc)
...
SaAisErrorT saAmfComponentNameGet(SaAmfHandleT hdl, SaNameT *o_comp_name)
{
	...
	/* fetch the comp name */
	if (m_AVA_FLAG_IS_COMP_NAME(cb)) {
		*o_comp_name = cb->comp_name_net;
		o_comp_name->length = m_NCS_OS_NTOHS(cb->comp_name_net.length);
	} else
...
SaAisErrorT saAmfCSIQuiescingComplete(SaAmfHandleT hdl, SaInvocationT inv, SaAisErrorT error)
{
	...
	m_AVA_CSI_QUIESCING_COMPL_MSG_FILL(msg, cb->ava_dest, hdl, inv, error, cb->comp_name_net);
	rc = ava_mds_send(cb, &msg, 0);
	if (NCSCC_RC_SUCCESS != rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
...
SaAisErrorT saAmfHAStateGet(SaAmfHandleT hdl, const SaNameT *comp_name, const SaNameT *csi_name, SaAmfHAStateT *o_ha)
{
	...
	if (NCSCC_RC_SUCCESS == rc) {
		m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
		m_AVSV_ASSERT(AVSV_AMF_HA_STATE_GET == msg_rsp->info.api_resp_info.type);
		rc = msg_rsp->info.api_resp_info.rc;
	} else if (NCSCC_RC_FAILURE == rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
	else if (NCSCC_RC_REQ_TIMOUT == rc)
				      ...
				      SaUint8T flags, SaAmfProtectionGroupNotificationBufferT *buf)
{
	...
	if (NCSCC_RC_SUCCESS == rc) {
		...
		if (AVSV_AVND_AMF_CBK_MSG == msg_rsp->type) {
			...
			m_AVSV_ASSERT(AVSV_AMF_PG_TRACK == msg_rsp->info.cbk_info->type);
			...
			m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
			m_AVSV_ASSERT(AVSV_AMF_PG_START == msg_rsp->info.api_resp_info.type);
			rc = msg_rsp->info.api_resp_info.rc;
		}
	} else if (NCSCC_RC_FAILURE == rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
...
SaAisErrorT saAmfProtectionGroupTrackStop(SaAmfHandleT hdl, const SaNameT *csi_name)
{
	...
	if (NCSCC_RC_SUCCESS == rc) {
		m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
		m_AVSV_ASSERT(AVSV_AMF_PG_STOP == msg_rsp->info.api_resp_info.type);
		rc = msg_rsp->info.api_resp_info.rc;
	} else if (NCSCC_RC_FAILURE == rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
	else if (NCSCC_RC_REQ_TIMOUT == rc)
				      ...
				      SaTimeT err_time, SaAmfRecommendedRecoveryT rec_rcvr, SaNtfIdentifierT ntf_id)
{
	...
	if (NCSCC_RC_SUCCESS == rc) {
		m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
		m_AVSV_ASSERT(AVSV_AMF_ERR_REP == msg_rsp->info.api_resp_info.type);
		rc = msg_rsp->info.api_resp_info.rc;
	} else if (NCSCC_RC_FAILURE == rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
	else if (NCSCC_RC_REQ_TIMOUT == rc)
...
SaAisErrorT saAmfComponentErrorClear(SaAmfHandleT hdl, const SaNameT *comp_name, SaNtfIdentifierT ntf_id)
{
	...
	if (NCSCC_RC_SUCCESS == rc) {
		m_AVSV_ASSERT(AVSV_AVND_AMF_API_RESP_MSG == msg_rsp->type);
		m_AVSV_ASSERT(AVSV_AMF_ERR_CLEAR == msg_rsp->info.api_resp_info.type);
		rc = msg_rsp->info.api_resp_info.rc;
	} else if (NCSCC_RC_FAILURE == rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
	else if (NCSCC_RC_REQ_TIMOUT == rc)
...
SaAisErrorT saAmfResponse(SaAmfHandleT hdl, SaInvocationT inv, SaAisErrorT error)
{
	...
	m_AVA_AMF_RESP_MSG_FILL(msg, cb->ava_dest, hdl, inv, error, cb->comp_name_net);
	rc = ava_mds_send(cb, &msg, 0);
	if (NCSCC_RC_SUCCESS != rc)
		rc = SA_AIS_ERR_TRY_AGAIN;
