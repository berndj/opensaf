...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
SaAisErrorT saClmInitialize(SaClmHandleT *o_hdl, const SaClmCallbacksT *reg_cbks, SaVersionT *ver)
{
	CLA_CB *cb = 0;
	CLA_HDL_DB *hdl_db = 0;
	...
	m_CLA_LOG_API(CLA_LOG_API_INITIALIZE, rc + CLA_LOG_API_DISPATCH, NCSFL_SEV_INFO);
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
SaAisErrorT saClmSelectionObjectGet(SaClmHandleT hdl, SaSelectionObjectT *o_sel_obj)
{
	CLA_CB *cb = 0;
	CLA_HDL_REC *hdl_rec = 0;
	SaAisErrorT rc = SA_AIS_OK;
	...
	*o_sel_obj = (SaSelectionObjectT)m_GET_FD_FROM_SEL_OBJ(hdl_rec->sel_obj);
	...
	/* log the status of api processing */
	m_CLA_LOG_API(CLA_LOG_API_SEL_OBJ_GET, rc + CLA_LOG_API_DISPATCH, NCSFL_SEV_INFO);
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
SaAisErrorT saClmDispatch(SaClmHandleT hdl, SaDispatchFlagsT flags)
{
	CLA_CB *cb = 0;
	CLA_HDL_REC *hdl_rec = 0;
	SaAisErrorT rc = SA_AIS_OK;
	...
	/* log the status of api processing */
	m_CLA_LOG_API(CLA_LOG_API_DISPATCH, rc + CLA_LOG_API_DISPATCH, NCSFL_SEV_INFO);
...
SaAisErrorT saClmFinalize(SaClmHandleT hdl)
{
	...
	/* log the status of api processing */
	m_CLA_LOG_API(CLA_LOG_API_FINALIZE, rc + CLA_LOG_API_DISPATCH, NCSFL_SEV_INFO);
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
SaAisErrorT saClmClusterTrack(SaClmHandleT hdl, SaUint8T flags, SaClmClusterNotificationBufferT *buf)
{
	CLA_CB *cb = 0;
	CLA_HDL_DB *hdl_db = 0;
	CLA_HDL_REC *hdl_rec = 0;
	...
	/* validate the falgs */
	if (!((flags & SA_TRACK_CURRENT) || (flags & SA_TRACK_CHANGES) || (flags & SA_TRACK_CHANGES_ONLY))) {
	...
	if (msg_rsp)
		avsv_nda_cla_msg_free(msg_rsp);

	/* log the status of api processing */
	m_CLA_LOG_API(CLA_LOG_API_TRACK_START, rc + CLA_LOG_API_DISPATCH, NCSFL_SEV_INFO);
...
SaAisErrorT saClmClusterTrackStop(SaClmHandleT hdl)
{
	...
	/* log the status of api processing */
	m_CLA_LOG_API(CLA_LOG_API_TRACK_STOP, rc + CLA_LOG_API_DISPATCH, NCSFL_SEV_INFO);
				...
				SaClmNodeIdT node_id, SaTimeT timeout, SaClmClusterNodeT *cluster_node)
{
	...
	if (cluster_node) {
		...
		if (rc == NCSCC_RC_SUCCESS) {
			...
			if (rc == SA_AIS_OK) {
				memcpy(cluster_node,
				       &msg_rsp->info.api_resp_info.param.node_get, sizeof(SaClmClusterNodeT));
			}
		} else if (rc == NCSCC_RC_FAILURE)	/* MDS send failure */
			rc = SA_AIS_ERR_TRY_AGAIN;
	...
	/* log the status of api processing */
	m_CLA_LOG_API(CLA_LOG_API_NODE_GET, rc + CLA_LOG_API_DISPATCH, NCSFL_SEV_INFO);
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
SaAisErrorT saClmClusterNodeGetAsync(SaClmHandleT hdl, SaInvocationT inv, SaClmNodeIdT node_id)
{
	CLA_CB *cb = 0;
	CLA_HDL_DB *hdl_db = 0;
	CLA_HDL_REC *hdl_rec = 0;
	...
	/* log the status of api processing */
	m_CLA_LOG_API(CLA_LOG_API_NODE_ASYNC_GET, rc + CLA_LOG_API_DISPATCH, NCSFL_SEV_INFO);
