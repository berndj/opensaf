...
static uns32 cla_mds_msg_syn_send (CLA_CB *, NCSMDS_INFO *, 
...
uns32 cla_mds_cbk (NCSMDS_CALLBACK_INFO *info)
{
   ...
   /* retrieve cla cb */
   if( 0 == (cb = (CLA_CB *)ncshm_take_hdl(NCS_SERVICE_ID_CLA, 
   ...
   case MDS_CALLBACK_RECEIVE:
      {
         ...
         if (NCSCC_RC_SUCCESS != rc)
            m_CLA_LOG_MDS(AVSV_LOG_MDS_RCV_CBK, AVSV_LOG_MDS_FAILURE, 
                          NCSFL_SEV_CRITICAL);
         else
            m_CLA_LOG_MDS(AVSV_LOG_MDS_RCV_CBK, AVSV_LOG_MDS_SUCCESS, 
                          NCSFL_SEV_INFO);
      }
      break;

   case MDS_CALLBACK_COPY:
      assert(0); /* CLA never resides with AvND */
   ...
   case MDS_CALLBACK_SVC_EVENT:
      {
         ...
         if (NCSCC_RC_SUCCESS != rc)
            m_CLA_LOG_MDS(AVSV_LOG_MDS_SVEVT_CBK, AVSV_LOG_MDS_FAILURE, 
                          NCSFL_SEV_CRITICAL);
         else
            m_CLA_LOG_MDS(AVSV_LOG_MDS_SVEVT_CBK, AVSV_LOG_MDS_SUCCESS, 
                          NCSFL_SEV_INFO);
      }
      break;
   ...
   case MDS_CALLBACK_ENC_FLAT:
      {
         ...
         {
            /* log the problem */
            m_CLA_LOG_MDS(AVSV_LOG_MDS_FLAT_ENC_CBK, AVSV_LOG_MDS_FAILURE,
                       NCSFL_SEV_CRITICAL);
         ...
         if (NCSCC_RC_SUCCESS != rc)
            ...
            m_CLA_LOG_MDS(AVSV_LOG_MDS_FLAT_ENC_CBK, AVSV_LOG_MDS_SUCCESS, 
                          NCSFL_SEV_INFO);
      }
      break;   
   ...
   case MDS_CALLBACK_DEC_FLAT:
      {
         ...
         {
            /* log the problem */
            m_CLA_LOG_MDS(AVSV_LOG_MDS_DEC_CBK, AVSV_LOG_MDS_FAILURE,
                          NCSFL_SEV_CRITICAL);
         ...
         if (NCSCC_RC_SUCCESS != rc)
            m_CLA_LOG_MDS(AVSV_LOG_MDS_FLAT_DEC_CBK, AVSV_LOG_MDS_FAILURE, 
                          NCSFL_SEV_CRITICAL);
         else
            m_CLA_LOG_MDS(AVSV_LOG_MDS_FLAT_DEC_CBK, AVSV_LOG_MDS_SUCCESS, 
                          NCSFL_SEV_INFO);
      }
      break;
   ...
   case MDS_CALLBACK_DEC:
      {
         ...
         if (NCSCC_RC_SUCCESS != rc)
            ...
            m_CLA_LOG_MDS(AVSV_LOG_MDS_DEC_CBK, AVSV_LOG_MDS_SUCCESS, 
                          NCSFL_SEV_INFO);
      }
      break;
   ...
   case MDS_CALLBACK_ENC:
      {
         ...
         {
            /* log the problem */
            m_CLA_LOG_MDS(AVSV_LOG_MDS_ENC_CBK, AVSV_LOG_MDS_FAILURE,
                       NCSFL_SEV_CRITICAL);
         ...
         if (NCSCC_RC_SUCCESS != rc)
            ...
            m_CLA_LOG_MDS(AVSV_LOG_MDS_ENC_CBK, AVSV_LOG_MDS_SUCCESS, 
                          NCSFL_SEV_INFO);
      }
      break;   
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 cla_mds_send (CLA_CB *cb, AVSV_NDA_CLA_MSG *i_msg, 
                    AVSV_NDA_CLA_MSG **o_msg, uns32 timeout)
{
   NCSMDS_INFO mds_info;
   uns32       rc = NCSCC_RC_SUCCESS;

   if (!i_msg) return NCSCC_RC_FAILURE; /* nothing to send */

   if((cb->avnd_intf.avnd_up == FALSE) || 
...
uns32 cla_mds_dec_flat (CLA_CB *cb, MDS_CALLBACK_DEC_INFO *dec_info)
{
   ...
   /* allocate the msg */
   msg = calloc(1,sizeof(AVSV_NDA_CLA_MSG));
   if (!msg)
   {
      rc = NCSCC_RC_FAILURE;
      goto err;
   ...
   case AVSV_AVND_CLM_CBK_MSG:
      {
         if(msg->info.cbk_info.type == AVSV_CLM_CBK_TRACK)
         {
            ...
            msg->info.cbk_info.param.track.notify.notification = 
                   (SaClmClusterNotificationT *)calloc(1, size); 
            ...
            rc = ncs_decode_n_octets_from_uba(dec_info->io_uba, 
                 (uns8 *)msg->info.cbk_info.param.track.notify.notification, size);
   ...
   case AVSV_AVND_CLM_API_RESP_MSG:
      {
         if(msg->info.api_resp_info.type == AVSV_CLM_TRACK_START)
         {
            ...
            size = (msg->info.api_resp_info.param.track.num) * 
                   (sizeof (SaClmClusterNotificationT));
            ...
            /* decode cbk-info */
            rc = ncs_decode_n_octets_from_uba(dec_info->io_uba, 
                 (uns8 *)msg->info.api_resp_info.param.track.notify, size);
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 cla_mds_msg_normal_send (CLA_CB *cb, 
                               NCSMDS_INFO *mds_info, 
                               AVSV_NDA_CLA_MSG *i_msg)
{
   ...
   if ( (AVSV_CLA_API_MSG == i_msg->type) &&
        ((AVSV_CLM_INITIALIZE == i_msg->info.api_info.type) ||
         (AVSV_CLM_FINALIZE == i_msg->info.api_info.type)) )
      m_NCS_UNLOCK(&cb->lock, NCS_LOCK_WRITE);
   else
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 cla_mds_msg_syn_send (CLA_CB *cb, 
                            NCSMDS_INFO *mds_info, 
                            AVSV_NDA_CLA_MSG *i_msg,
                            uns32  timeout)
{
...
uns32 cla_mds_dec (CLA_CB *cb, MDS_CALLBACK_DEC_INFO *dec_info)
{
   ...
   case NCSMDS_SVC_ID_AVND:
      ...
      rc = m_NCS_EDU_VER_EXEC(&cb->edu_hdl, avsv_edp_nd_cla_msg, dec_info->io_uba,
                          EDP_OP_TYPE_DEC, (AVSV_NDA_CLA_MSG**)&dec_info->o_msg, &ederror, dec_info->i_msg_fmt_ver);
