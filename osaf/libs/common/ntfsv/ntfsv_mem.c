...
SaAisErrorT ntfsv_alloc_ntf_header(
    ...
    SaUint16T numAdditionalInfo)
{
	...
	malloc(sizeof (SaNtfIdentifierT));
    if(notificationHeader->notificationId == NULL)
    {
                                  ...
                                  SaUint16T numProposedRepairActions)
{
    ...
    /* Trend */
    alarmNotification->trend =
	malloc(sizeof (SaNtfSeverityTrendT));
    if (alarmNotification->trend ==
	NULL) {
	...
	malloc(sizeof (SaNtfProbableCauseT));
	...
	alarmNotification->proposedRepairActions = (SaNtfProposedRepairActionT *)
	    malloc(numProposedRepairActions * \
                           ...
                           const SaNtfNotificationHeaderT *src)
{
    ...
    *(dest->notifyingObject) = *(src->notifyingObject);
    ...
    dest->notificationClassId->majorId =
        src->notificationClassId->majorId;
    dest->notificationClassId->minorId =
        src->notificationClassId->minorId;

    *(dest->notificationId) =
        *src->notificationId;

    dest->lengthAdditionalText =
        src->lengthAdditionalText;
    /* Set additional text and additional info */
    (void)strncpy(dest->additionalText,
                  src->additionalText,
                  src->lengthAdditionalText);
    TRACE_LEAVE();
}
                                        ...
                                        const SaNtfStateChangeNotificationT *src)
{
    ...
    if (0 < dest->numStateChanges) {
        memcpy(dest->changedStates,
               src->changedStates,
               dest->numStateChanges
...
void ntfsv_alloc_and_copy_not(ntfsv_send_not_req_t *dest,
                              const ntfsv_send_not_req_t *src)
{
    TRACE_ENTER2("ntfsv_send_not_req_t ptr = %p"
                 "notificationType = %x",
                 src,
                 (int)src->notificationType);
