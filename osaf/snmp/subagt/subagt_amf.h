...
uns32 snmpsubagt_amf_initialize(struct ncsSa_cb *cb);

/* Unregistration and Finalization with AMF Library */
uns32 snmpsubagt_amf_finalize(struct ncsSa_cb *cb);

/* Following functions may be moved to another common file -- NCS 2.0 ??? */

/* to register the MIBs supported in NCS of all the services */
EXTERN_C uns32 snmpsubagt_pending_regs_cmp(uns8 *key1, uns8 *key2);

EXTERN_C uns32 snmpsubagt_pending_regs_free(NCS_DB_LINK_LIST_NODE *free_node);

/* process the pending registrations */
EXTERN_C uns32 snmpsubagt_pending_registrations_do(NCS_DB_LINK_LIST *pending_registrations);

EXTERN_C void snmpsubagt_pending_registrations_free(NCS_DB_LINK_LIST *pending_registrations);

/* to start the health check */
EXTERN_C uns32 snmpsubagt_amf_healthcheck_start_msg_post(struct ncsSa_cb *cb);

/* to register the MIBs supported in NCS of all the services */
EXTERN_C uns32 snmpsubagt_appl_mibs_register(void);

/* to start the timer to retry amf initialisation. */
EXTERN_C uns32 snmpsubagt_amf_init_timer_start(struct ncsSa_cb *cb);

/* to read the configuration file and to load/unload the 
 * application spec MIBs 
 */
EXTERN_C uns32 snmpsubagt_spa_job(uns8 *file_name, uns32 what_to_do);

/* to reload the libraries in subagent configuration file */
EXTERN_C uns32 snmpsubagt_mibs_reload(struct ncsSa_cb *cb);

/* unload the subagent integration libraries */
EXTERN_C uns32 snmpsubagt_mibs_unload(void);

/* Go to ACTIVE State */
EXTERN_C uns32 snmpsubagt_ACTIVE_process(struct ncsSa_cb *cb, SaInvocationT invocation);

/* Go to STANDBY State */
EXTERN_C uns32 snmpsubagt_STANDBY_process(struct ncsSa_cb *cb, SaInvocationT invocation);
