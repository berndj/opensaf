 ...
 *  NOTE: 
 *****************************************************************************/
uns32 snmpsubagt_edu_edp_initialize(EDU_HDL *edu_hdl)
{
 ...
 *****************************************************************************/
uns32 subagt_edu_ncs_trap_decode(EDU_HDL *edu_hdl, uns8 *evt_data, uns32 evt_data_size, NCS_TRAP *o_ncs_trap)
{
	uns32 ret_code = NCSCC_RC_SUCCESS;
	EDU_ERR ed_error = 0;

	/* validate the input */
	if ((evt_data == NULL) || (evt_data_size == 0) || (edu_hdl == NULL) || (o_ncs_trap == NULL)) {
		/* log the error */
	...
	ret_code = m_NCS_EDU_TLV_EXEC(edu_hdl, ncs_edp_ncs_trap,
				      evt_data, evt_data_size, EDP_OP_TYPE_DEC, &o_ncs_trap, &ed_error);
