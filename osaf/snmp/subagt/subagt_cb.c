 ...
 *  NOTE: 
 *****************************************************************************/
NCSSA_CB *snmpsubagt_cb_init(void)
{
	...
	cb->evtCallback.saEvtEventDeliverCallback = snmpsubagt_eda_callback;
...
uns32 snmpsubagt_cb_finalize(NCSSA_CB *cb)
{
	uns32 status = NCSCC_RC_FAILURE;
 ...
 *  NOTE: 
 *****************************************************************************/
uns32 snmpsubagt_cb_update(NCSSA_CB *cb, int32 argc, uns8 **argv)
{
	uns32 status = NCSCC_RC_FAILURE;
...
void snmpsubagt_timer_cb(void *arg)
{
	...
	status = m_NCS_IPC_SEND(&pSacb->subagt_mbx, (NCS_IPC_MSG *)post_me, NCS_IPC_PRIORITY_HIGH);
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
...
uns32 snmpsubagt_timer_start(NCSSA_CB *pSacb, SNMPSUBAGT_TMR_MSG_TYPE msgType)
{
	...
	if (timer_data->timer_id == TMR_T_NULL) {
		/* create the timer. */
		m_NCS_TMR_CREATE(timer_data->timer_id, timer_data->period, snmpsubagt_timer_cb, (void *)(timer_data));
	}
	/* start the timer.  */
	m_NCS_TMR_START(timer_data->timer_id, timer_data->period, snmpsubagt_timer_cb, (void *)(timer_data));
