...
typedef uns32
 (*SnmpSaReadinessAndHAStateProcess) (NCSSA_CB *cb, SaInvocationT invocation);

static uns32 snmpsubagt_invalid_state_process(NCSSA_CB *cb, SaInvocationT invocation);

/* Go to ACTIVE State from ACTIVE state*/
static uns32 snmpsubagt_ACTIVE_ACTIVE_process(NCSSA_CB *cb, SaInvocationT invocation);

/* Go to QUIESCED State */
static uns32 snmpsubagt_QUIESCED_process(NCSSA_CB *cb, SaInvocationT invocation);

/* Go to QUIESCING State */
static uns32 snmpsubagt_QUIESCING_process(NCSSA_CB *cb, SaInvocationT invocation);

/* Go to QUIESCED from QUIESCING State */
static uns32 snmpsubagt_QUIESCING_QUIESCED_process(NCSSA_CB *cb, SaInvocationT invocation);

static uns32 snmpsubagt_STANDBY_STANDBY_process(NCSSA_CB *cb, SaInvocationT invocation);

/* to unregister the MIBs supported in NCS of all the services */
static uns32 snmpsubagt_appl_mibs_unregister(void);

static uns32 subagt_amf_componentize(NCSSA_CB *cb);

/* Initialize the HA state machine */
const SnmpSaReadinessAndHAStateProcess snmpsubagt_ReadinessAndHAStateDispatch[SNMPSUBAGT_HA_STATE_MAX]
...
static uns32 snmpsubagt_amf_callbacks_init(SaAmfCallbacksT *amfCallbacks);

/* Describes the health check of the NCS SubAgent */
static void
snmpsubagt_amf_health_check_callback(SaInvocationT invocation, const SaNameT *compName, SaAmfHealthcheckKeyT *checkKey);
...
static void snmpsubagt_amf_component_terminate_callback(SaInvocationT invocation, const SaNameT *compName);
 ...
 *  NOTE: 
 ******************************************************************************/
SaAisErrorT snmpsubagt_amf_initialize(NCSSA_CB *cb)
{
	...
	if (ret_code != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_AMF_CALLBACKS_INIT_FAILED, ret_code, 0, 0);
		return SA_AIS_ERR_INVALID_PARAM;
	}
	...
	if (status != SA_AIS_OK) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_AMF_INIT_FAILED, cb->amfHandle, status, 0);
		return status;
	}

	/* get the communication Handle */
	status = saAmfSelectionObjectGet(cb->amfHandle, &cb->amfSelectionObject);
	if (status != SA_AIS_OK) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_AMF_SEL_OBJ_GET_FAILED, cb->amfHandle, status, 0);
	...
	if (status != SA_AIS_OK) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_AMF_COMP_NAME_GET_FAILED, cb->amfHandle, status, 0);
		saAmfFinalize(cb->amfHandle);
		cb->amfHandle = 0;
		return status;
	}
...
uns32 snmpsubagt_amf_finalize(NCSSA_CB *cb)
{
	SaAisErrorT status = SA_AIS_OK;
	...
	status = saAmfHealthcheckStop(cb->amfHandle, &cb->compName, &cb->healthCheckKey);
	if (status != SA_AIS_OK) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_AMF_HLTH_CHK_STOP_FAILED, status, 0, 0);
		/* continue finalization */
	}

	/* Unregister with AMF */
	status = saAmfComponentUnregister(cb->amfHandle, &cb->compName, NULL);
	if (status != SA_AIS_OK) {
		/* log the error */
...
static uns32 snmpsubagt_amf_callbacks_init(SaAmfCallbacksT *amfCallbacks)
{
	...
	amfCallbacks->saAmfHealthcheckCallback = snmpsubagt_amf_health_check_callback;

	/* Component Terminate Callback */
	amfCallbacks->saAmfComponentTerminateCallback = snmpsubagt_amf_component_terminate_callback;
...
snmpsubagt_amf_health_check_callback(SaInvocationT invocation,
				     const SaNameT *compName, SaAmfHealthcheckKeyT *healthCheckKey)
{
	SaAisErrorT status = SA_AIS_OK;
			       ...
			       const SaNameT *compName, SaAmfHAStateT haState, SaAmfCSIDescriptorT csiDescriptor)
{
	...
	status = snmpsubagt_ReadinessAndHAStateDispatch[cb->haCstate][haState] (cb, invocation);
...
/* Go to ACTIVE State */
uns32 snmpsubagt_ACTIVE_process(NCSSA_CB *cb, SaInvocationT invocation)
{
...
/* Go to STANDBY State */
uns32 snmpsubagt_STANDBY_process(NCSSA_CB *cb, SaInvocationT invocation)
{
...
static uns32 snmpsubagt_QUIESCED_process(NCSSA_CB *cb, SaInvocationT invocation)
{
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_NETSNMP_LIB_DEINIT_FAILED, status, 0, 0);
		saAmfResponse(cb->amfHandle, invocation, SA_AIS_ERR_FAILED_OPERATION);
		return status;
	}
 ...
 *  NOTE: 
 *****************************************************************************/
uns32 snmpsubagt_amf_healthcheck_start_msg_post(NCSSA_CB *cb)
{
	uns32 status = NCSCC_RC_FAILURE;
	...
	status = m_NCS_IPC_SEND(&cb->subagt_mbx, (NCS_IPC_MSG *)post_me, NCS_IPC_PRIORITY_HIGH);
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
 ...
 *                 NCSCC_RC_FAILURE   -  failure                              *
\*****************************************************************************/
uns32 snmpsubagt_appl_mibs_register()
{
	uns32 status = NCSCC_RC_FAILURE;
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_STR_LOG(SNMPSUBAGT_SUBSYS_REG_FAILED, "subagt_register_avsv_subsys()", 0, 0);
	}
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_STR_LOG(SNMPSUBAGT_SUBSYS_REG_FAILED, "subagt_register_avm_subsys()", 0, 0);
	}
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_STR_LOG(SNMPSUBAGT_SUBSYS_REG_FAILED, "subagt_register_dtsv_subsys()", 0, 0);
	}
 ...
 *                 NCSCC_RC_FAILURE   -  failure                              *
\*****************************************************************************/
static uns32 snmpsubagt_appl_mibs_unregister()
{
	uns32 status = NCSCC_RC_FAILURE;
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_STR_LOG(SNMPSUBAGT_SUBSYS_UNREG_FAILED, "subagt_unregister_avsv_subsys()", 0, 0);
	}
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_STR_LOG(SNMPSUBAGT_SUBSYS_UNREG_FAILED, "subagt_unregister_avm_subsys()", 0, 0);
	}
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_STR_LOG(SNMPSUBAGT_SUBSYS_UNREG_FAILED, "subagt_unregister_dtsv_subsys()", 0, 0);
	}
 ...
 *                 NCSCC_RC_FAILURE   -  failure                              *
\*****************************************************************************/
uns32 snmpsubagt_spa_job(uns8 *file_name, uns32 what_to_do)
{
	FILE *fp = NULL;
	int8 lib_name[255] = { 0 };
	...
	while (((nargs = fscanf(fp, "%s %s", lib_name, appl_name)) == 2) && (nargs != EOF)) {
 ...
 *  Returns:       Nothing                                                    *
\*****************************************************************************/
void snmpsubagt_pending_registrations_free(NCS_DB_LINK_LIST *registrations)
{
	...
	while (temp) {
		...
		if (status != NCSCC_RC_SUCCESS) {
			/* ERROR LOG: free failed */
			m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_PEND_DEL_FAILED, status, 0, 0);
			/* continue to the next node */
		}
...
*                                    cb->haCstate                            *
*****************************************************************************/
EXTERN_C uns32 subagt_rda_init_role_get(struct ncsSa_cb *cb)
{
	uns32 rc;
	uns32 status = NCSCC_RC_SUCCESS;
...
static uns32 subagt_amf_componentize(NCSSA_CB *cb)
{
	...
	if ((cb->amfInitStatus == m_SNMPSUBAGT_AMF_INIT_COMPLETED)
...
EXTERN_C void subagt_process_sig_usr1_signal(NCSSA_CB *cb)
{
