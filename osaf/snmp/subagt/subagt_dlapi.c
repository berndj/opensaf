...
static uns32 snmpsubagt_svcs_init(NCSSA_CB *cb);

/* start_routine for the SubAgent thread */
static void snmpsubagt_start(SYSF_MBX *subagt_mbx);

/* For cleanup during the initialization */
static void snmpsubagt_cleanup(NCSSA_CB *cb);

/* callback to free the message in subagt's mailbox. */
static NCS_BOOL snmpsubagt_leave_on_queue_cb(void *arg1, void *arg2);

/* SIGHUP Handler */
static void snmpsubagt_sighup_handler(int i_sig_num);

/* SIGUSR1 Handler */
static void snmpsubagt_amf_sigusr1_handler(int i_sig_num);

/* SIGUSR2 Handler */
static void snmpsubagt_sigusr2_handler(int i_sig_num);
...
*  Note:
******************************************************************************/
uns32 ncs_snmpsubagt_lib_req(NCS_LIB_REQ_INFO *io_lib_req)
{
	...
	case NCS_LIB_REQ_CREATE:
		...
		status = ncs_snmpsubagt_create(io_lib_req->info.create.argc, (uns8 **)io_lib_req->info.create.argv);
...
uns32 ncs_snmpsubagt_create(int32 argc, uns8 **argv)
{
	FILE *fp;
	uns32 status = NCSCC_RC_FAILURE;
	NCSSA_CB *cb = NULL;
	...
	status = m_NCS_TASK_CREATE((NCS_OS_CB)snmpsubagt_start,
				   &cb->subagt_mbx,
				   "NCS_SNMP_SUBAGT",
				   NCS_SNMPSUBAGT_PRIORITY, NCS_SNMPSUBAGT_STACK_SIZE, &cb->subagt_task);
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
...
static void snmpsubagt_start(SYSF_MBX *subagt_mbx)
{
	...
	while (1) {
		...
		if (status != NCSCC_RC_SUCCESS) {
			...
			if (status == NCSCC_RC_INVALID_PING_TIMEOUT)
				return;
...
*  Note:
******************************************************************************/
uns32 snmpsubagt_destroy(NCSSA_CB *cb, NCS_BOOL comp_unreg)
{
	uns32 status = NCSCC_RC_FAILURE;
...
uns32 ncs_snmpsubagt_init_deinit_msg_post(uns8 *init_deinit_routine)
{
	...
	strcpy(post_me->init_or_deinit_routine, init_deinit_routine);
