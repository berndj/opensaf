...
static netsnmp_variable_list *snmpsubagt_eda_varbind_compose(NCSSA_OID_DATABASE_NODE *i_oid_db_node,
...
snmpsubagt_eda_trap_varbind_value_populate(netsnmp_variable_list *var_bind,
					   struct variable *object_details, NCSMIB_PARAM_VAL *i_param_val);

/* to get the complete OID (base_oid + param_id + instance) */
static oid *snmpsubagt_eda_full_oid_populate(oid *base_oid,
					     uns32 base_oid_len,
					     NCSMIB_PARAM_ID i_param_id,
					     uns32 *i_instids, uns32 i_inst_len, size_t *io_name_length);

/* to compose the Varbind for the snmpTrap.0 */
static netsnmp_variable_list *snmpsubagt_eda_snmpTrapId_vb_populate(NCS_PATRICIA_TREE *i_oid_db,
								    uns32 i_trap_tbl_id, uns32 i_trap_id);

/* to append to the list of varbinds */
static uns32 snmpsubagt_eda_varbindlist_append(netsnmp_variable_list **add_to_me, netsnmp_variable_list *to_be_added);

static uns32 snmpsubagt_eda_varbinds_free(netsnmp_variable_list *to_be_freed);

static int32
subagt_eda_object_get(struct variable *object_details, uns32 number_of_objects, NCSMIB_PARAM_ID i_param_id);
uns32 subagt_send_v2trap(NCS_PATRICIA_TREE *oid_db, NCS_TRAP *trap_header);
...
******************************************************************************/
SaAisErrorT snmpsubagt_eda_initialize(NCSSA_CB *pSacb)
{
	...
	if (status != SA_AIS_OK) {
		/* log the failure */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_EVT_INIT_FAIL, status, 0, 0);
		return status;
	}
	 ...
	 */
	channelFlags = SA_EVT_CHANNEL_CREATE | SA_EVT_CHANNEL_PUBLISHER | SA_EVT_CHANNEL_SUBSCRIBER;

	status = saEvtChannelOpen(pSacb->evtHandle,	/* input */
				  &pSacb->evtChannelName,	/* TBD input  */
				  channelFlags,	/* input */
				  timeout_in_ns, &pSacb->evtChannelHandle);	/* i/o */
	if (status != SA_AIS_OK) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_EVT_CHNL_OPEN_FAIL, status, pSacb->evtHandle, channelFlags);
	...
	if (status != SA_AIS_OK) {
		...
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_EDA_SEL_OBJ_GET_FAILED,
				       status, pSacb->evtHandle, (uns32)pSacb->evtSelectionObject);
 ...
 *  NOTE: 
 *****************************************************************************/
uns32 snmpsubagt_eda_finalize(NCSSA_CB *pSacb)
{
	SaAisErrorT status = SA_AIS_OK;
	...
	if (status != SA_AIS_OK) {
		...
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_EVT_UNSUBSCRIBE_FAIL,
				       status, pSacb->evtSubscriptionId, pSacb->evtChannelHandle);
	...
	if (status != SA_AIS_OK) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_EVT_CHNL_CLOSE_FAIL, status, pSacb->evtChannelHandle, 0);
		return NCSCC_RC_FAILURE;
	}
	...
	if (status != SA_AIS_OK) {
		...
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_EDA_FINALIZE_FAILED, status, pSacb->evtHandle, 0);
 ...
 *  NOTE: 
 *****************************************************************************/
void snmpsubagt_eda_callback(SaEvtSubscriptionIdT subscriptionid,	/* input */
...
{				/* input */
	...
	if (subscriptionid != cb->evtSubscriptionId) {	/* mismatch in subscription */
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_EVT_SUBSCRIP_MISMATCH, subscriptionid, cb->evtSubscriptionId, 0);
		return;
	}
	...
	if (status != SA_AIS_OK) {
		 ...
		 */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_EDA_EVT_DATA_GET_FAIL, status, 0, 0);
		/* free the eventData */
		free(eventData);
		return;
	}
	...
	ret_code = snmpsubagt_eda_trapevt_to_agentxtrap_populate(&cb->edu_hdl,
								 &cb->oidDatabase, eventData, eventDataSize);
	...
	if (status != SA_AIS_OK) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_EVT_FREE_FAIL, status, 0, 0);
	}
 ...
 *  NOTE: 
 *****************************************************************************/
uns32 snmpsubagt_eda_trapevt_to_agentxtrap_populate(EDU_HDL *edu_hdl, NCS_PATRICIA_TREE *oid_db,	/*in */
						    uns8 *evtData,	/* in */
						    uns32 evtDataSize)
{				/* in */
	...
	status = subagt_edu_ncs_trap_decode(edu_hdl, evtData, evtDataSize, &trap_header);
	...
	if (status != NCSCC_RC_SUCCESS) {
		...
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_TRAP_SEND_FAIL, status, trap_header.i_trap_tbl_id,
...
uns32 subagt_send_v2trap(NCS_PATRICIA_TREE *oid_db, NCS_TRAP *trap_header)
{
	...
	if ((oid_db == NULL) || (trap_header == NULL)) {
		/* log the error */
	...
	var_trap_elem = snmpsubagt_eda_snmpTrapId_vb_populate(oid_db,
							      trap_header->i_trap_tbl_id, trap_header->i_trap_id);
	...
	while (trap_varbind) {
		...
		oid_db_node = (NCSSA_OID_DATABASE_NODE *)ncs_patricia_tree_get(oid_db, (uns8 *)&oid_db_key);
		if (oid_db_node == NULL) {
		...
		var_trap_elem = snmpsubagt_eda_varbind_compose(oid_db_node, trap_varbind);
		if (var_trap_elem == NULL) {
			/* there is an error */
							     ...
							     NCS_TRAP_VARBIND *i_trap_varbind)
{
	...
	var_bind->name = snmpsubagt_eda_full_oid_populate(i_oid_db_node->base_oid,
							  ...
							  i_trap_varbind->i_idx.i_inst_len, &var_bind->name_length);
...
static oid *snmpsubagt_eda_full_oid_populate(oid *base_oid,
					     uns32 base_oid_len,
					     NCSMIB_PARAM_ID i_param_id,
					     uns32 *i_inst_ids, uns32 i_inst_len, size_t *io_name_length)
{
	uns32 i = 0;
								    ...
								    uns32 i_trap_tbl_id, uns32 i_trap_id)
{
	...
	/* Get the base oid based on the trap_table_id */
	oid_db_node = (NCSSA_OID_DATABASE_NODE *)ncs_patricia_tree_get(i_oid_db, (uns8 *)&oid_db_key);
	if (oid_db_node == NULL) {
		m_SNMPSUBAGT_HEADLINE_LOG(SNMPSUBAGT_OID_DB_NODE_NULL);
		return NULL;
	}
	...
	/* get the complete oid */
	memcpy(var_bind->name, objid_snmptrap, (var_bind->name_length) * sizeof(oid));

	/* assign the value */
	var_bind->type = ASN_OBJECT_ID;
	var_bind->val.objid = snmpsubagt_eda_full_oid_populate(oid_db_node->base_oid,
							       oid_db_node->base_oid_len,
							       i_trap_id, 0, 0, &var_bind->val_len);
 ...
 *  NOTE: 
 *****************************************************************************/
static int32 subagt_eda_object_get(struct variable *object_details, uns32 number_of_objects, NCSMIB_PARAM_ID i_param_id)
{
	int32 param_location = -1;
	uns32 i = 0;
...
static void subagt_log_ncs_trap_vb(NCS_TRAP_VARBIND *trap_vb)
{
	...
	m_SNMPSUBAGT_MEMDUMP_LOG(SNMPSUBAGT_NCSMIB_ARG_PARAM_INFO_DUMP, log_trap_data);
	...
	if (trap_vb->i_param_val.i_fmat_id == NCSMIB_FMAT_INT) {
		...
		m_SNMPSUBAGT_MEMDUMP_LOG(SNMPSUBAGT_NCSMIB_ARG_INT_INFO_DUMP, log_trap_data);
	}
