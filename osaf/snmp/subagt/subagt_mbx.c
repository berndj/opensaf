...
static uns32 snmpsubagt_mbx_destroy_process(NCSSA_CB *cb);

/* to handle the register/deregister message */
static uns32 snmpsubagt_mbx_init_deinit_process(NCSSA_CB *cb, uns8 *init_deinit_routine);

/* to start the health check */
static uns32 snmpsubagt_mbx_healthcheck_start(NCSSA_CB *cb);
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
uns32 snmpsubagt_mbx_process(NCSSA_CB *cb)
{
	...
	if ((process_me = (SNMPSUBAGT_MBX_MSG *)m_NCS_IPC_NON_BLK_RECEIVE(&cb->subagt_mbx, NULL)) != NULL) {
		...
		case SNMPSUBAGT_MBX_MSG_MIB_INIT_OR_DEINIT:
			status = snmpsubagt_mbx_init_deinit_process(cb, process_me->init_or_deinit_routine);
			if (status != NCSCC_RC_SUCCESS) {
				/* log the failure */
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
static uns32 snmpsubagt_mbx_init_deinit_process(NCSSA_CB *cb, uns8 *reg_dereg_routine)
{
	uns32 status = NCSCC_RC_FAILURE;
	...
	init_deinit_routine = m_NCS_OS_DLIB_SYMBOL(lib_hdl, reg_dereg_routine);
	if ((dl_error = m_NCS_OS_DLIB_ERROR()) != NULL) {
		/* log the error returned from dlopen() */
...
static uns32 snmpsubagt_mbx_healthcheck_start(NCSSA_CB *cb)
{
	...
	saf_status = saAmfHealthcheckStart(cb->amfHandle,
					   &cb->compName,
					   &cb->healthCheckKey, cb->invocationType, cb->recommendedRecovery);
	if (saf_status != SA_AIS_OK) {
		/* log the error */
		m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_AMF_HLTH_CHK_STRT_FAILED, cb->amfHandle, saf_status, 0);
		return NCSCC_RC_FAILURE;
	}
...
static void snmpsubagt_check_snmpd_and_subscribe_edsv(NCSSA_CB *cb)
{
	...
	if (main_session) {
		...
		if (saf_status != SA_AIS_OK) {
			/* log the error */
			m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_EDA_EVT_SUBSCRIBE_FAIL, saf_status, cb->evtSubscriptionId, 0);
	...
	if (cb->eda_timer.timer_id != NULL) {
		...
		if (status != SA_AIS_OK) {
			...
			m_SNMPSUBAGT_ERROR_LOG(SNMPSUBAGT_EVT_UNSUBSCRIBE_FAIL,
					       status, cb->evtSubscriptionId, cb->evtChannelHandle);
		}
