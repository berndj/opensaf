...
uns32 cli_stricmp(int8 *i_str1, int8 *i_str2)
{
	...
	for (i = 0; i < strlen(i_str1); i++) {
		if (i_str1[i] == CLI_CONS_BLANK_SPACE)
			break;
	}
...
void cli_set_cmd_into_history(CLI_CB *pCli, int8 *i_cmdstr)
{
	...
	if (0 == pCli->ctree_cb.cmdHtry) {
		...
		if (!pCli->ctree_cb.cmdHtry->pCmdStr)
			return;
...
{
	if (!pCli->ctree_cb.htryMrkr.cmd_node)
		return;
	...
	if (CLI_HIS_BWD_MVMT == i_direction) {
		...
		if (0 != pCli->ctree_cb.htryMrkr.cmd_node->prev) {
			pCli->ctree_cb.htryMrkr.cmd_node = pCli->ctree_cb.htryMrkr.cmd_node->prev;
		}
...
{
	while (i_count)
		i_count = cli_move_char_backward(i_count);
	return i_count;
}
...
uns32 cli_move_end_of_line(int8 *i_buffer, uns32 i_count)
{
	...
	while (i_count < data_len)
		i_count = cli_move_char_forward(i_buffer, i_count);
	return i_count;
}
...
void cli_read_input(CLI_CB *pCli, NCS_VRID vr_id)
{
	...
	uns32 cmd_len = 0;
	NCS_BOOL display_flag = FALSE, passwd_flag = FALSE, cmd_help = FALSE;
	int8 error_pos, display_str[256];
...
/*   
   ...
   */
	...
	pCli->ctree_cb.modeStack[++pCli->ctree_cb.modeStackPtr] = pCli->ctree_cb.trvMrkr;
 ...
 tcinitset:
		...
		case CONTROL(CLI_CONS_REDISPLAY_CMD):
			...
			if (0 != chCnt)
				chCnt = cli_delete_beginning_of_line(buffer, chCnt);
		...
		case CONTROL(CLI_CONS_DELETE_REST):
			...
			while (chCnt != index)
				chCnt = cli_delete_char(buffer, chCnt);
		...
		case CONTROL(CLI_CONS_DELETE_WORD):
			...
			if (0 != chCnt)
				chCnt = cli_delete_word(buffer, chCnt);
			else
				putchar(CLI_CONS_BEEP);
		...
		case CONTROL(CLI_CONS_START_OF_LINE):
			...
			if (0 != chCnt)
				chCnt = cli_move_beginning_of_line(chCnt);
			else
				putchar(CLI_CONS_BEEP);
...
/* quit command is valid only for pcode. For other cases it is invalidated .  */
			...
			if (0 == cli_stricmp(buffer, CLI_HELP_DESC)) {
				...
				if ((0 == cli_stricmp(buffer, CLI_EXIT)) && (NCSCC_RC_CALL_PEND != pCli->cefStatus)) {
					cli_current_mode_exit(pCli, &session_info, &display_flag);
					memset(buffer, '\0', CLI_BUFFER_SIZE);
					chCnt = 0;
					break;
				...
				case CLI_SUCCESSFULL_MATCH:
					if (CLI_HELP_NEXT_LEVEL == param.i_cmdtype) {
						switch (param.o_status) {
						case CLI_NO_MATCH:
							error_pos = (int8)(strlen(session_info.prompt_string) +
									   param.o_errpos + 1);
		...
		case CLI_CONS_HELP:
			...
			/*Set the current prompt */
			if (CLI_PARTIAL_MATCH == param.o_status || CLI_SUCCESSFULL_MATCH == param.o_status) {
		...
		case CLI_CONS_TAB:
			...
			param.i_cmdbuf = buffer;
			cli_execute_command(pCli, &param, &session_info);
			if (param.o_status == CLI_SUCCESSFULL_MATCH || param.o_status == CLI_PARTIAL_MATCH) {
		...
		case CLI_CONS_LEFT_ARROW:
			...
			if (0 != chCnt)
				chCnt = cli_move_char_backward(chCnt);
			else
				putchar(CLI_CONS_BEEP);
...
NOTES           :        
*****************************************************************************/
void cli_current_mode_exit(CLI_CB *pCli, CLI_SESSION_INFO *io_session_info, NCS_BOOL *io_display_flag)
{
...
uns32 cli_clean_cmds(CLI_CB *pCli, NCSCLI_DEREG_CMD_LIST *list)
{
	...
	for (cmdcnt = 0; cmdcnt < list->i_cmd_count; cmdcnt++) {
		...
		while (pCli->ctree_cb.dregStackMrkr >= 0) {
			...
			if (!node->node_ptr.pChild && !node->node_ptr.pDataPtr && !node->node_ptr.pSibling) {
...
*****************************************************************************/
void cli_exec_cmd_from_file(CLI_CB *pCli, CLI_EXECUTE_PARAM *io_param, CLI_SESSION_INFO *o_session)
{
	...
	for (;;) {
		memset(cmdStr, 0, sizeof(cmdStr));
		if (!sysf_readln_from_file(cmdStr))
			continue;
		m_GET_TIME_STAMP(pCli->cli_last_active_time);	/*Added to fix bug 58609 */
		...
		if ((0 == cli_stricmp(cmdStr, CLI_EXIT)) && (NCSCC_RC_CALL_PEND != pCli->cefStatus)) {
