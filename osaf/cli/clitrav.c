...
static uns32 cli_get_ipv6_dlmt_cnt(int8 *i_str, uns8 *o_dlimt_cnt, NCS_BOOL *o_dbl_dlimt);
#endif
		...
		if (i_str[i] == ':') {
			...
			if (dlimt_flag == FALSE) {
				...
				if (dbl_dlimt == FALSE)
					...
					return NCSCC_RC_FAILURE;	/* Don't allow more that one double delimiter */
			}
		} else if (((i_str[i] >= '0') && (i_str[i] <= '9')) ||
			   ((i_str[i] >= 'A') && (i_str[i] <= 'F')) || ((i_str[i] >= 'a') && (i_str[i] <= 'f'))) {
...
*****************************************************************************/
#if(NCS_IPV6 == 1)
uns32 cli_check_ipv6addr(int8 *i_str)
{
	uns8 dlimt_cnt = 0;
...
*****************************************************************************/
#if(NCS_IPV6 == 1)
uns32 cli_check_ipv6prefix(int8 *i_str)
{
	NCS_BOOL dbl_dlimt = FALSE;
	...
	strncpy(buffer, i_str, sizeof(buffer) - 1);
	...
	if (dlimt_cnt >= NCS_IPV6_ADDR_UNS16_CNT)
		return CLI_NO_MATCH;
...
*****************************************************************************/
uns32 cli_check_macaddr(int8 *i_str)
{
	int8 *delimiter = ":";
	int8 buffer[CLI_BUFFER_SIZE];
	int8 *token = 0;
	...
	if (NCSCLI_MAC_ADDR_CNT == addr_cnt)
		ret_val = CLI_SUCCESSFULL_MATCH;
	return ret_val;
}
...
*****************************************************************************/
uns32 cli_check_community(int8 *i_str)
{
	int8 *delimiter = ":";
	int8 buffer[CLI_BUFFER_SIZE];
	int8 *token = 0;
	uns32 ret_val = CLI_NO_MATCH, ret = CLI_NO_MATCH;
...
*****************************************************************************/
uns32 cli_check_ipv4addr(int8 *i_str)
{
	int8 *ptr = 0;
	uns32 dots = 0, digit = 0;
...
uns32 cli_check_ipv4prefix(int8 *i_cmdstring)
{
	...
	if (cli_check_ipv4addr(iptoken) != CLI_SUCCESSFULL_MATCH)
		return CLI_NO_MATCH;
	str++;

	/* If exceeds range then return error */
	if (atoi(str) > 32 || atoi(str) < 0)
		return CLI_NO_MATCH;
	return CLI_SUCCESSFULL_MATCH;
}
...
uns32 cli_ipv4_to_int(int8 *i_string)
{
	...
	strncpy(str_addr, i_string, sizeof(str_addr) - 1);
...
*****************************************************************************/
uns32 cli_matchtoken(int8 *i_str1, int8 *i_str2, CLI_CMD_ERROR *o_cli_cmd_error)
{
	int8 *s1 = 0, *s2 = 0;
	uns16 pos = 0;
	 ...
	 */
	if (pos > o_cli_cmd_error->errorpos)
		o_cli_cmd_error->errorpos = pos;

	/* both strings are not equal, copy error string */
	strncpy(o_cli_cmd_error->errstring, i_str2, sizeof(o_cli_cmd_error->errstring) - 1);
	return CLI_NO_MATCH;
}
...
*****************************************************************************/
uns32 cli_get_tokentype(int8 *i_token)
{
	...
	while (0 != *i_token) {
		...
		if (!isdigit((uns32)*i_token))
			type = NCSCLI_STRING;
		i_token++;
	}

	/* If type is NCSCLI_NUMBER then return */
	if (NCSCLI_NUMBER == type)
		return NCSCLI_NUMBER;
...
uns32 cli_set_arg(int8 *i_arg, NCSCLI_ARG_SET *o_cmd_tokens, CLI_CB *pCli)
{				/* Fxi for the bug 59854 , CLI_CB* extra argument passed */
	...
	if (NCSCLI_NUMBER == o_cmd_tokens->i_arg_record[count].i_arg_type) {
		...
		if (longnum > hvalue) {
			...
			strncpy(pCli->outOfBoundaryToken, i_arg, strlen(i_arg));
		}
	} else {
		...
		strncpy((int8 *)o_cmd_tokens->i_arg_record[count].cmd.strval, i_arg, len - 1);
	}
	return NCSCC_RC_SUCCESS;
}
...
uns32 cli_getcommand_tokens(int8 *i_cmdstring, NCSCLI_ARG_SET *ctok, CLI_CB *pCli)
{				/* CLI_CB arg ment added extra to fix the bug 59854 */
	...
	for (;;) {
		...
		*(token + slen) = '\0';
		...
		if (NCSCC_RC_SUCCESS != cli_set_arg(token, ctok, pCli))
			return NCSCC_RC_FAILURE;
		while (((uns32)*curptr == CLI_CONS_BLANK_SPACE) && '\0' != *curptr)
			curptr++;
...
*****************************************************************************/
uns32 cli_check_token(CLI_CB *pCli, NCSCLI_ARG_VAL *i_argval, CLI_CMD_ERROR *o_cli_cmd_error)
{
	...
	if (NCSCLI_OPTIONAL == pCli->ctree_cb.cmdElement->tokType) {
		...
		if (NCSCLI_MACADDR == i_argval->i_arg_type) {
			...
			strncpy(o_cli_cmd_error->errstring, i_argval->cmd.strval,
				sizeof(o_cli_cmd_error->errstring) - 1);
			ret = CLI_NO_MATCH;
		}
		...
		if (NCSCLI_CIDRv4 == i_argval->i_arg_type) {
			 ...
			 */
			if (NCSCLI_CIDRv4 == i_argval->i_arg_type) {
				strncpy(ipmask, (int8 *)i_argval->cmd.strval, sizeof(ipmask) - 1);
				...
				if (ret == CLI_SUCCESSFULL_MATCH) {
					...
					if (iptoken)
						i_argval->cmd.ip_pfx.len = (uns8)atoi(iptoken);
					else
						i_argval->cmd.ip_pfx.len = 0;
				}
			}
		} else {
			/* Arg var is not a string, it can be a IP6 address or Int or .. */
...
CLI_CMD_NODE *cli_node_change(CLI_CB *pCli)
{
	...
	strncpy(nodePath, pCli->ctree_cb.cmdElement->nodePath, sizeof(nodePath) - 1);
...
*****************************************************************************/
uns32 cli_check_range(CLI_CB *pCli, NCSCLI_ARG_VAL *i_arg_record, CLI_CMD_ERROR *o_cli_cmd_error)
{
...
void cli_add_default_values(CLI_CMD_ELEMENT *i_cmdElement, NCSCLI_ARG_SET *o_cmd_tokens, uns32 i_index)
{
	...
	if (NCSCLI_NUMBER == o_cmd_tokens->i_arg_record[i_index].i_arg_type) {
		...
		strncpy((int8 *)o_cmd_tokens->i_arg_record[i_index].cmd.strval, (int8 *)i_cmdElement->defVal, len - 1);
	}
...
uns32 cli_check_syntax(CLI_CB *pCli,
		       NCSCLI_ARG_SET *io_cmd_tokens, CLI_EXECUTE_PARAM *io_param, CLI_CMD_ERROR *cli_cmd_error)
{
	uns32 ret = CLI_NO_MATCH,
	    i = 0, traverseCount = 0,
	    cmd_pos = io_cmd_tokens->i_pos_value,
	    default_pos = 0, lowerLimit = 0, upperLimit = 0, optStack_ptr = 0, len = 0, prev_optStack_ptr = 0;

	NCS_BOOL partialmatch = FALSE,
	    loopflag = FALSE,
	    mandatoryelement = FALSE,
	    defaultmatch = FALSE, command_matched = FALSE, endofcmd = FALSE, help_str = FALSE, exit_str = FALSE;

	CLI_CMD_ELEMENT *partialmatchpointer = 0, *tabnode = 0, *defaultnode = 0, *recursivemarker = 0;
		...
		if (CLI_NO_MATCH == ret || CLI_PARTIAL_MATCH == ret) {
			...
			while (optStack_ptr) {
				...
				if (!command_matched || partialmatch) {
					...
					if (pCli->ctree_cb.cmdElement->node_ptr.pSibling) {
						pCli->ctree_cb.cmdElement =
						    pCli->ctree_cb.cmdElement->node_ptr.pSibling;
						command_matched = FALSE;
						loopflag = TRUE;
						prev_optStack_ptr = optStack_ptr;
						break;
				...
				if (pCli->ctree_cb.cmdElement->node_ptr.pChild) {
					...
					if (cmd_pos & 1) {
						...
						m_CHECK_IF_NUMBER(io_cmd_tokens->i_arg_record[i].i_arg_type,
								  io_cmd_tokens->i_arg_record[i].cmd,
								  cli_cmd_error->errstring);
					...
					if ((CLI_SUCCESSFULL_MATCH == ret || CLI_PASSWORD_MATCHED == ret) &&
						...
						if (help_str) {
							...
							if (io_cmd_tokens->i_arg_record[0].cmd.strval)
								m_MMGR_FREE_CLI_DEFAULT_VAL(io_cmd_tokens->
											    i_arg_record[0].cmd.strval);

							/* Alloc new chunk of memeory */
							len = strlen("?") + 1;
							io_cmd_tokens->i_arg_record[0].cmd.strval =
							    m_MMGR_ALLOC_CLI_DEFAULT_VAL(len);
							if (!io_cmd_tokens->i_arg_record[0].cmd.strval)
								return ret;
							memset(io_cmd_tokens->i_arg_record[0].cmd.strval, '\0', len);
							strncpy(io_cmd_tokens->i_arg_record[0].cmd.strval, "?",
								len - 1);
							return (io_param->o_status = CLI_SUCCESSFULL_MATCH);
						}

						if (exit_str)
							strncpy(io_param->i_cmdbuf, CLI_EXIT, strlen(CLI_EXIT));

						io_param->cmd_exec_func = pCli->ctree_cb.cmdElement->cmd_exec_func;
						io_param->cmd_access_level =
						    pCli->ctree_cb.cmdElement->cmd_access_level;
						io_param->bindery = pCli->ctree_cb.cmdElement->bindery;
					}
					break;
				}
			...
			if (NCSCLI_PASSWORD == pCli->ctree_cb.cmdElement->tokType) {
				...
				while (pCli->ctree_cb.cmdElement) {
					...
					if (!is_any_parm_wildcard) {
						...
						is_any_param_optional =
						    is_any_optional_node(&pCli->ctree_cb.cmdElement);
						if (!is_any_param_optional)
							ret = CLI_PARTIAL_MATCH;
					}
...
void cli_modify_cmdargs(NCSCLI_ARG_SET *io_cmd_tokens)
{
	...
	while (cmd_pos & 1) {
		...
		if (NCSCLI_KEYWORD == io_cmd_tokens->i_arg_record[i].i_arg_type &&
			...
			/* Set bit to 0 */
			io_cmd_tokens->i_pos_value = io_cmd_tokens->i_pos_value ^ cli_to_power(2, i);
		}
...
*****************************************************************************/
uns32 cli_execute_command(CLI_CB *pCli, CLI_EXECUTE_PARAM *io_param, CLI_SESSION_INFO *o_session)
{
	uns32 status = 0;
	NCSCLI_ARG_SET ctok;
	...
	memset(&cli_cmd_error, 0, sizeof(CLI_CMD_ERROR));
	...
	if (CLI_EXECUTE == io_param->i_cmdtype) {
		...
		if ((CLI_SUCCESSFULL_MATCH == io_param->o_status) || (CLI_PASSWORD_MATCHED == io_param->o_status)) {
			...
			if ((0 != io_param->cmd_exec_func)) {
				...
				if ((NCSCC_RC_SUCCESS == pCli->cefStatus) || (NCSCC_RC_CALL_PEND == pCli->cefStatus)) {
					...
					if (pCli->ctree_cb.currNode) {
						...
						if (CLI_PASSWORD_MATCHED != io_param->o_status) {
							if (strcmp(pCli->ctree_cb.trvMrkr->mode, "exec") != 0) {	/*For suppressing passwd for enable cmd */
								strcat(o_session->prompt_string, "-");
								strcat(o_session->prompt_string,
								       pCli->ctree_cb.trvMrkr->mode);

								/* Set Mode Information */
								pCli->ctree_cb.trvMrkr->pData =
								    pCli->subsys_cb.i_cef_mode;
...
cli_get_help_desc(CLI_CB *pCli,
		  NCSCLI_ARG_SET *i_cmd_tokens, CLI_EXECUTE_PARAM *io_param, CLI_CMD_ERROR *o_cli_cmd_error)
{
	uns32 ret = CLI_NO_MATCH, i = 0, helpcount = 0;
	NCS_BOOL loopflag = FALSE;
	uns32 cmd_pos = i_cmd_tokens->i_pos_value, optStack_ptr = 0;	/* copy token bit pos locally */
	CLI_CMD_ELEMENT *optnode_stack[CLI_DUMMY_STACK_SIZE];
	...
	while (cmd_pos & 1) {
		...
		if (CLI_NO_MATCH == ret || CLI_PARTIAL_MATCH == ret || CLI_SUCCESSFULL_MATCH == ret) {
			...
			while (optStack_ptr) {
				...
				if (!pCli->ctree_cb.cmdElement->isMand && pCli->ctree_cb.cmdElement->node_ptr.pChild) {
					pCli->ctree_cb.cmdElement = pCli->ctree_cb.cmdElement->node_ptr.pChild;
					loopflag = TRUE;
					break;
				}
			...
			break;
		} else if (CLI_INVALID_PASSWD == ret || CLI_PASSWORD_MATCHED == ret)
			break;
	}
	return ret;
}
...
*****************************************************************************/
void
cli_get_helpstr(CLI_CB *pCli, NCSCLI_ARG_SET *io_cmd_tokens, CLI_EXECUTE_PARAM *io_param, CLI_SESSION_INFO *io_session)
{
	...
	strncpy(tmpbuf, io_cmd_tokens->i_arg_record[count].cmd.strval, sizeof(tmpbuf) - 1);
	token = tmpbuf;
	...
	if (CLI_AMBIGUOUS_MATCH == io_param->o_status)
		return;

	if (CLI_SUCCESSFULL_MATCH == io_param->o_status) {
		strcpy(io_param->o_hotkey.hlpstr.helpargs[io_param->o_hotkey.hlpstr.count].cmdstring, CLI_CR);
		io_param->o_hotkey.hlpstr.count++;
		return;
	}

	if ((io_cmd_tokens->i_pos_value != bit_pos) && (CLI_NO_MATCH == io_param->o_status)) {
		/* Error occured in command */
		io_param->o_errpos = cli_find_errpos(io_param->i_cmdbuf,
						     cli_cmd_error.errstring, cli_cmd_error.errorpos);
		return;
	} else
		io_param->o_status = CLI_SUCCESSFULL_MATCH;
	...
	if (CLI_HELP_PARTIAL_MATCH == io_param->i_cmdtype) {
		...
		if (!io_cmd_tokens->i_arg_record[0].cmd.strval)
			return;
		memset(io_cmd_tokens->i_arg_record[0].cmd.strval, '\0', len);
		strncpy(io_cmd_tokens->i_arg_record[0].cmd.strval, tmpbuf, len - 1);
	}
...
void cli_complete_cmd(CLI_CB *pCli,
		      NCSCLI_ARG_SET *io_cmd_tokens, CLI_EXECUTE_PARAM *io_param, CLI_SESSION_INFO *io_session)
{
	int32 i = 0, index;
	CLI_CMD_ERROR cli_cmd_error;
	int32 cmdlen = 0;
	...
	/* check if command matched and if token type is keyword, 
	   ...
	   tokens 
	 */
	if ((CLI_SUCCESSFULL_MATCH == io_param->o_status || CLI_PARTIAL_MATCH == io_param->o_status)) {
		if (io_cmd_tokens->i_arg_record[i - 1].i_arg_type == NCSCLI_KEYWORD) {
			if (ncscli_user_access_level_authenticate(pCli)) {
				...
				strncpy(io_param->o_hotkey.tabstring,
					&io_cmd_tokens->i_arg_record[i - 1].cmd.strval[cmdlen], index - cmdlen + 1);
			}
		}
	}
	 ...
	 */
	if (io_param->o_hotkey.tabstring[0] == '\0')
		io_param->o_status = CLI_NO_MATCH;
}
	...
	while (0 != optionalptr) {
		if (NCSCLI_OPTIONAL == optionalptr->tokType) {
			*ptr = optionalptr;
			...
			optionalptr = optionalptr->node_ptr.pSibling;
	}
	return FALSE;
}
	...
	while (0 != wildcardptr) {
		if (NCSCLI_WILDCARD == wildcardptr->tokType) {
			*ptr = wildcardptr;
			...
			wildcardptr = wildcardptr->node_ptr.pSibling;
	}
	return FALSE;
}
...
static void convert_to_community(NCSCLI_ARG_VAL *arg_val, int8 *str)
{
	...
	if (token) {
		/* Get firt part of community */
		strncpy(ipbuff, token, sizeof(ipbuff) - 1);
	...
	while (0 != token) {
		token = strtok(0, delimiter);
		if (token)
			arg_val->cmd.community.num = (uns32)atoi(token);
	}
...
void cli_update_cmd_arg(NCSCLI_ARG_SET *ctok, CLI_EXECUTE_PARAM *io_param)
{
	int8 *delimiter = " ";
	int8 *token = 0;
	uns32 pos = 0, i = 0, cnt = io_param->o_tokprocs, len = 0, cmd_pos = ctok->i_pos_value;
	...
	if (!io_param->o_tokprocs)
		return;

	strncpy(buffer, io_param->i_cmdbuf, sizeof(buffer) - 1);
	...
	strncpy(wldcbuff, io_param->i_cmdbuf + pos, sizeof(wldcbuff) - 1);
	while (cmd_pos & 1) {
		cmd_pos = cmd_pos >> 1;
		...
		if (i == io_param->o_tokprocs) {
			...
			if (NCSCLI_NUMBER != ctok->i_arg_record[i].i_arg_type && ctok->i_arg_record[i].cmd.strval) {
			...
			if (!ctok->i_arg_record[i].cmd.strval)
				return;
			memset(ctok->i_arg_record[i].cmd.strval, '\0', len);
			strncpy(ctok->i_arg_record[i].cmd.strval, wldcbuff, len - 1);
		}
	...
	for (i = 0; i < NCSCLI_ARG_COUNT; i++) {
		...
		default:
			if ((FALSE == arg->i_arg_record[i].sym_valid) &&
			    (NCSCLI_NUMBER != arg->i_arg_record[i].i_arg_type) && (arg->i_arg_record[i].cmd.strval)) {
				m_MMGR_FREE_CLI_DEFAULT_VAL(arg->i_arg_record[i].cmd.strval);
			}
			break;
		}
...
static void cli_help_str_fill(CLI_CB *pCli, CLI_CMD_HELP *buf)
{
	...
	if ((NCSCLI_NUMBER == pCli->ctree_cb.cmdElement->tokType) && pCli->ctree_cb.cmdElement->range) {
		lvalue = *(uns32 *)pCli->ctree_cb.cmdElement->range->lLimit;
		hvalue = *(uns32 *)pCli->ctree_cb.cmdElement->range->uLimit;
		if (!lvalue && !hvalue)
			strncpy(buf->cmdstring, pCli->ctree_cb.cmdElement->tokName, sizeof(buf->cmdstring) - 1);
		else {
			sprintf(str, "<%u-%u>", lvalue, hvalue);
			strncpy(buf->cmdstring, str, sizeof(buf->cmdstring) - 1);
		}
	} else
		strncpy(buf->cmdstring, pCli->ctree_cb.cmdElement->tokName, sizeof(buf->cmdstring) - 1);

	if (pCli->ctree_cb.cmdElement->helpStr)
		strncpy(buf->helpstr, pCli->ctree_cb.cmdElement->helpStr, sizeof(buf->helpstr) - 1);
}
#else
extern int dummy;
