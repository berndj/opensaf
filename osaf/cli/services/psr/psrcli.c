...
uns32 ncspss_cli_register(NCSCLI_BINDERY *i_bindery)
{
	...
	data.i_command_list[cmd_count].i_cmdstr = "save !Save Profile to Current Config or a named profile! \
                     profile !Keyword!\
                     [NCSCLI_STRING !Profile Name!]";
	data.i_command_list[cmd_count].i_cmd_access_level = NCSCLI_ADMIN_ACCESS;
	...
	data.i_command_list[cmd_count].i_cmdstr = "set !Set!  playback-option-from-xml-config !Keyword!\
                              NCSCLI_STRING !PCN name, upto 255 characters!";
	data.i_command_list[cmd_count].i_cmd_access_level = NCSCLI_ADMIN_ACCESS;
	data.i_command_list[cmd_count++].i_cmd_exec_func = pss_cef_set_playback_option_from_xml_config;
	...
	data.i_command_list[cmd_count].i_cmdstr =
	    "dump!Dump MIB entries of a specified client in the profile!  profile !Keyword!\
                    ...
                    NCSCLI_STRING !PCN-name, upto 255 characters!";
	data.i_command_list[cmd_count].i_cmd_access_level = NCSCLI_VIEWER_ACCESS;
	data.i_command_list[cmd_count++].i_cmd_exec_func = pss_cef_dump_profile;
	...
	data.i_command_list[cmd_count].i_cmdstr =
	    "show!Show registered clients in the profile!  profile-clients !Keyword!\
                              NCSCLI_STRING !profile-name!";
	data.i_command_list[cmd_count].i_cmd_access_level = NCSCLI_VIEWER_ACCESS;
	data.i_command_list[cmd_count++].i_cmd_exec_func = pss_cef_show_profile_clients;

	/* reload pssv_lib.conf config file command */
	data.i_command_list[cmd_count].i_cmdstr = "reload!Reload the config file!  pssv_lib_conf-config !Keyword!";
	data.i_command_list[cmd_count].i_cmd_access_level = NCSCLI_ADMIN_ACCESS;
	data.i_command_list[cmd_count++].i_cmd_exec_func = pss_cef_reload_pssv_lib_conf;

	/* reload pssv_spcn.list config file command */
	data.i_command_list[cmd_count].i_cmdstr = "reload!Reload the config file!  pssv_spcn_list-config !Keyword!";
	data.i_command_list[cmd_count].i_cmd_access_level = NCSCLI_ADMIN_ACCESS;
	data.i_command_list[cmd_count++].i_cmd_exec_func = pss_cef_reload_pssv_spcn_list;
...
*****************************************************************************/
uns32 pss_cef_set_playback_option_from_xml_config(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
	NCSMIB_ARG mib_arg;
	NCS_UBAID lcl_uba;
	uns8 *buff_ptr = NULL;
...
*****************************************************************************/
uns32 pss_cef_dump_profile(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
	NCSMIB_ARG mib_arg;
	FILE *fh = NULL;
	NCS_UBAID lcl_uba;
...
*****************************************************************************/
uns32 pss_cef_show_profile_clients(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
	NCSMIB_ARG mib_arg;
	NCS_UBAID lcl_uba;
	uns8 *buff_ptr = NULL;
...
*****************************************************************************/
uns32 pss_cef_reload_pssv_lib_conf(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
...
*****************************************************************************/
uns32 pss_cef_reload_pssv_spcn_list(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
	NCSMIB_ARG ncsmib_arg;
	uns8 space[2048];
	NCSMEM_AID ma;
...
uns32 pss_cli_cmd_mib_resp_show_profile_clients(NCSMIB_ARG *resp)
{
	...
	if ((resp->i_op == NCSMIB_OP_RSP_CLI_DONE) ||
	    ((resp->i_op == NCSMIB_OP_RSP_CLI) && (resp->rsp.info.cli_rsp.o_partial == FALSE)))
		rsp_completed = TRUE;
	...
	case NCSCC_RC_SUCCESS:
		{
			...
			if (resp->rsp.info.cli_rsp.i_cmnd_id == PSS_CMD_TBL_CMD_NUM_DISPLAY_PROFILE) {
				...
				for (j = 0; j < pwe_cnt; j++) {
					...
					for (k = 0; k < pcn_cnt; k++) {
						buff_ptr =
						    ncs_dec_flatten_space(&lcl_uba, (uns8 *)&str_len, sizeof(uns16));
						if (buff_ptr == NULL) {
							printf("\npayload decode failed...");
							pss_cli_done(resp->i_usr_key, NCSCC_RC_FAILURE);
							return NCSCC_RC_FAILURE;
						...
						ncs_dec_skip_space(&lcl_uba, sizeof(uns16));
						if (ncs_decode_n_octets_from_uba(&lcl_uba, (char *)&pcn, str_len) !=
						    NCSCC_RC_SUCCESS) {
							printf("\npayload decode failed...");
							pss_cli_done(resp->i_usr_key, NCSCC_RC_FAILURE);
							return NCSCC_RC_FAILURE;
...
uns32 pss_cef_list_profiles(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
	...
	if (strcmp(arg_list->i_arg_record[0].cmd.strval, "no")) {
		...
		while (TRUE) {
			...
			memcpy(inst_ids, ncsmib_arg.rsp.info.next_rsp.i_next.i_inst_ids, num_inst_ids * sizeof(uns32));
...
uns32 pss_cef_copy_profile(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
	...
	if (strcmp(arg_list->i_arg_record[0].cmd.strval, "no")) {
		...
		ncsmib_arg.req.info.set_req.i_param_val.i_length =
		    (uns16)(strlen(arg_list->i_arg_record[2].cmd.strval) + 1);
		...
		ncsmib_arg.req.info.set_req.i_param_val.i_fmat_id = NCSMIB_FMAT_OCT;
		ncsmib_arg.req.info.set_req.i_param_val.i_length =
		    (uns16)(strlen(arg_list->i_arg_record[3].cmd.strval) + 1);
...
*****************************************************************************/
uns32 pss_cef_describe_profile(NCSCLI_ARG_SET *arg_list, NCSCLI_CEF_DATA *cef_data)
{
	NCSMIB_ARG ncsmib_arg;
	uns8 space[2048];
	NCSMEM_AID ma;
