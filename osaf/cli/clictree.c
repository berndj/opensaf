...
*****************************************************************************/
void cli_add_cmd_node(CLI_CB *pCli, CLI_CMD_ELEMENT *i_node, CLI_TOKEN_RELATION i_relationship)
{
		...
		if (CLI_START_CMD == i_node->tokLvl) {
			...
			if (!pCli->ctree_cb.cmdMrkr) {
				...
				if (FALSE == pCli->ctree_cb.nodeFound) {
					if (CLI_CMD_ADDED_PARTIALL ==
					    cli_append_node(pCli->ctree_cb.ctxtMrkr->pCmdl, pCli->ctree_cb.cmdMrkr,
							    &i_node)) {
						pCli->ctree_cb.ctxtMrkr->pCmdl = i_node;
						i_node->node_ptr.pParent = pCli->ctree_cb.ctxtMrkr->pCmdl;
					}
					pCli->ctree_cb.cmdMrkr = i_node;
					i_node->cmd_access_level =
					    i_node->cmd_access_level | pCli->ctree_cb.cmd_access_level;
				} else {
					pCli->ctree_cb.lvlMrkr = pCli->ctree_cb.cmdMrkr;
					pCli->ctree_cb.cmdMrkr->cmd_access_level =
					    pCli->ctree_cb.cmdMrkr->cmd_access_level | pCli->ctree_cb.cmd_access_level;
					cli_free_cmd_element(&i_node);
					i_node = pCli->ctree_cb.cmdMrkr;
				}
			}

			m_LOG_NCSCLI_COMMENTS(i_node->tokName, NCSCLI_REL_CHILDOF, pCli->ctree_cb.ctxtMrkr->name);
		} else {
			...
			if (0 == pCli->ctree_cb.lvlMrkr->node_ptr.pChild)
				...
				pCli->ctree_cb.nodeFound =
				    (uns8)cli_cmd_ispresent(&pCli->ctree_cb.lvlMrkr, i_node->tokName);
			...
			if (FALSE == pCli->ctree_cb.nodeFound) {
				...
				pCli->ctree_cb.lvlMrkr->cmd_access_level =
				    pCli->ctree_cb.lvlMrkr->cmd_access_level | pCli->ctree_cb.cmd_access_level;
				cli_free_cmd_element(&i_node);
	...
	case CLI_SIBLING_TOKEN:
		...
		pCli->ctree_cb.nodeFound = (uns8)cli_cmd_ispresent(&pCli->ctree_cb.orMrkr, i_node->tokName);

		if (FALSE == pCli->ctree_cb.nodeFound) {
			pCli->ctree_cb.orMrkr->node_ptr.pSibling = i_node;
			m_LOG_NCSCLI_COMMENTS(i_node->tokName, NCSCLI_REL_SIBLINGOF, pCli->ctree_cb.orMrkr->tokName);
			...
			pCli->ctree_cb.orMrkr->cmd_access_level =
			    pCli->ctree_cb.orMrkr->cmd_access_level | pCli->ctree_cb.cmd_access_level;
			cli_free_cmd_element(&i_node);
	...
	case CLI_GRP_CHILD_TOKEN:
		...
		if (((NCSCLI_OPTIONAL == pCli->ctree_cb.optMrkr->tokType) ||
			...
			if ((NCSCLI_GROUP == pCli->ctree_cb.optMrkr->tokType) && FALSE == i_node->isMand) {
			...
			if (pCli->ctree_cb.optMrkr->tokLvl != i_node->tokLvl) {
				...
				pCli->ctree_cb.nodeFound =
				    (uns8)cli_cmd_ispresent(&pCli->ctree_cb.optMrkr, i_node->tokName);
		...
		if (FALSE == pCli->ctree_cb.nodeFound) {
			...
			pCli->ctree_cb.optMrkr->cmd_access_level =
			    pCli->ctree_cb.optMrkr->cmd_access_level | pCli->ctree_cb.cmd_access_level;
			cli_free_cmd_element(&i_node);
			i_node = pCli->ctree_cb.optMrkr;
		}

		if (TRUE == i_node->isCont)
	...
	case CLI_GRP_SIBLING_TOKEN:
		{
			...
			if (TRUE == i_node->isCont)
				tempMarker = pCli->ctree_cb.contMrkr;
			else {
				tempMarker = pCli->ctree_cb.optMrkr;
				tempMarker = tempMarker->node_ptr.pDataPtr;
			}

			pCli->ctree_cb.nodeFound = (uns8)cli_cmd_ispresent(&tempMarker, i_node->tokName);

			if (FALSE == pCli->ctree_cb.nodeFound)
				tempMarker->node_ptr.pSibling = i_node;

			if (FALSE == i_node->isMand) {
				m_LOG_NCSCLI_COMMENTS(i_node->tokName, NCSCLI_REL_OPTSIBLINGOF, tempMarker->tokName);
			} else {
				m_LOG_NCSCLI_COMMENTS(i_node->tokName, NCSCLI_REL_SIBLINGOF, tempMarker->tokName);
			}
			...
			if (FALSE == pCli->ctree_cb.nodeFound) {
				...
				tempMarker->cmd_access_level =
				    tempMarker->cmd_access_level | pCli->ctree_cb.cmd_access_level;
				cli_free_cmd_element(&i_node);
				i_node = tempMarker;
			}
		}
...
void cli_set_cmd_to_parse(CLI_CB *pCli, int8 *src, CLI_BUFFER_TYPE buffType)
{
	...
	case CLI_CMD_BUFFER:
		if (src) {
			...
			strncat(pCli->par_cb.ipbuffer, CLI_TOK_EOC,
				sizeof(pCli->par_cb.ipbuffer) - strlen(pCli->par_cb.ipbuffer) - 1);
		} else
...
void cli_find_add_node(CLI_CB *pCli, int8 *tok, NCS_BOOL cookie)
{
	...
	if (0 != pCli->ctree_cb.ctxtMrkr->pChild) {
		...
		if (FALSE == pCli->ctree_cb.nodeFound) {
			...
			if (!pCli->ctree_cb.ctxtMrkr->pSibling)
				return;

			if (!cookie)
				return;	/* Do not create the node */

			memset(pCli->ctree_cb.ctxtMrkr->pSibling, 0, sizeof(CLI_CMD_NODE));
			strncpy(pCli->ctree_cb.ctxtMrkr->pSibling->name, tok,
				sizeof(pCli->ctree_cb.ctxtMrkr->pSibling->name) - 1);
		...
		if (!pCli->ctree_cb.ctxtMrkr->pChild)
			return;
...
*****************************************************************************/
uns32 cli_append_node(CLI_CMD_ELEMENT *root, CLI_CMD_ELEMENT *cnode, CLI_CMD_ELEMENT **i_node)
{
	NCS_BOOL done = FALSE;
...
void cli_scan_token(CLI_CB *pCli, CLI_CMD_ELEMENT *node, CLI_TOKEN_RELATION rel)
{
	if (NCSCLI_BRACE_END == node->tokType) {
		...
		cli_add_cmd_node(pCli, node, rel);
}
...
void cli_clean_cmd_element(CLI_CMD_ELEMENT *node)
{
	...
	while (ptr) {
		...
		if (ptr->node_ptr.pChild)
			ptr = ptr->node_ptr.pChild;
		else if (ptr->node_ptr.pDataPtr)
			ptr = ptr->node_ptr.pDataPtr;
		else if (ptr->node_ptr.pSibling)
			ptr = ptr->node_ptr.pSibling;
		else {
			...
			if (ptr->node_ptr.pChild == curr)
				ptr->node_ptr.pChild = 0;
			else if (ptr->node_ptr.pDataPtr == curr)
				ptr->node_ptr.pDataPtr = 0;
			else if (ptr->node_ptr.pSibling == curr)
				ptr->node_ptr.pSibling = 0;

			/* Push the bindery structre in list for freeing */
			if (curr->bindery)
				cli_push_bindery(&bdryl, curr->bindery);

			/*cli_free_cmd_element(&curr); */
			if (curr != ptr)
				cli_free_cmd_element(&curr);

			if (!ptr->node_ptr.pChild && !ptr->node_ptr.pDataPtr && !ptr->node_ptr.pSibling) {
...
{
	if ((*node)->defVal)
		m_MMGR_FREE_CLI_DEFAULT_VAL((*node)->defVal);
	...
	if ((*node)->tokName)
		m_MMGR_FREE_CLI_DEFAULT_VAL((*node)->tokName);
	if ((*node)->helpStr)
		m_MMGR_FREE_CLI_DEFAULT_VAL((*node)->helpStr);
	if ((*node)->nodePath)
		m_MMGR_FREE_CLI_DEFAULT_VAL((*node)->nodePath);
