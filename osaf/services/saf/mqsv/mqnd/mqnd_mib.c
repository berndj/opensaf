...
static uns32 mqnd_mib_tbl_req(struct ncsmib_arg *args)
{
	...
	if (status == NCSCC_RC_SUCCESS)
		m_LOG_MQSV_ND(MQND_MIBLIB_TBL_REQ_SUCCESS, NCSFL_LC_MQSV_INIT, NCSFL_SEV_INFO, status, __FILE__,
			      __LINE__);
	else
		m_LOG_MQSV_ND(MQND_MIBLIB_TBL_REQ_FAILED, NCSFL_LC_MQSV_INIT, NCSFL_SEV_ERROR, status, __FILE__,
			      __LINE__);
...
uns32 mqnd_reg_with_mab(MQND_CB *cb)
{
	...
	if (rc != NCSCC_RC_SUCCESS) {
		m_LOG_MQSV_ND(MQND_MIB_MSGQ_TBL_REG_WITH_MAB_FAILED, NCSFL_LC_MQSV_INIT, NCSFL_SEV_ERROR, rc, __FILE__,
			      __LINE__);
		return NCSCC_RC_FAILURE;
	} else
		m_LOG_MQSV_ND(MQND_MIB_MSGQ_TBL_REG_WITH_MAB_SUCCESS, NCSFL_LC_MQSV_INIT, NCSFL_SEV_NOTICE, rc,
			      __FILE__, __LINE__);
	...
	rc = ncsoac_ss(&mab_arg);
	if (rc != NCSCC_RC_SUCCESS) {
		m_LOG_MQSV_ND(MQND_MIB_MSGQPR_TBL_REG_WITH_MAB_FAILED, NCSFL_LC_MQSV_INIT, NCSFL_SEV_ERROR, rc,
			      __FILE__, __LINE__);
		return rc;
	} else
		m_LOG_MQSV_ND(MQND_MIB_MSGQPR_TBL_REG_WITH_MAB_SUCCESS, NCSFL_LC_MQSV_INIT, NCSFL_SEV_NOTICE, rc,
			      __FILE__, __LINE__);
...
uns32 mqnd_reg_with_miblib(void)
{
	...
	if (rc != NCSCC_RC_SUCCESS) {
		m_LOG_MQSV_ND(MQND_MSGQTBL_REG_WITH_MIBLIB_FAILED, NCSFL_LC_MQSV_INIT, NCSFL_SEV_ERROR, rc, __FILE__,
			      __LINE__);
		return rc;
	} else
		m_LOG_MQSV_ND(MQND_MSGQTBL_REG_WITH_MIBLIB_SUCCESS, NCSFL_LC_MQSV_INIT, NCSFL_SEV_NOTICE, rc, __FILE__,
			      __LINE__);

	rc = samsgqueuepriorityentry_tbl_reg();
	if (rc != NCSCC_RC_SUCCESS) {
		m_LOG_MQSV_ND(MQND_MSGQPRTBL_REG_WITH_MIBLIB_FAILED, NCSFL_LC_MQSV_INIT, NCSFL_SEV_ERROR, rc, __FILE__,
			      __LINE__);
		return rc;
	} else
		m_LOG_MQSV_ND(MQND_MSGQPRTBL_REG_WITH_MIBLIB_SUCCESS, NCSFL_LC_MQSV_INIT, NCSFL_SEV_NOTICE, rc,
			      __FILE__, __LINE__);
