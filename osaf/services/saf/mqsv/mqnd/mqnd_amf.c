 ...
 *****************************************************************************/
static void
mqnd_saf_health_chk_callback (SaInvocationT invocation,
                             const SaNameT *compName,
                             SaAmfHealthcheckKeyT *checkType)
{
   MQND_CB  *mqnd_cb;
...
uns32 mqnd_amf_init (MQND_CB *mqnd_cb)
{
   ...
   if (error != SA_AIS_OK)
   {
      ...
      m_LOG_MQSV_ND(MQND_AMF_INITIALISATION_SUCCESS,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,error,__FILE__,__LINE__);
   error = saAmfComponentNameGet(mqnd_cb->amf_hdl,&mqnd_cb->comp_name);
   if (error != SA_AIS_OK)
   {
      res = NCSCC_RC_FAILURE;
      m_LOG_MQSV_ND(MQND_AMF_COMP_NAME_GET_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,error,__FILE__,__LINE__);
   }
...
uns32  mqnd_amf_register (MQND_CB *mqnd_cb)
{
   ...
   {
      m_LOG_MQSV_ND(MQND_AMF_REGISTRATION_SUCCESS,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,error,__FILE__,__LINE__);
...
uns32  mqnd_amf_deregister (MQND_CB *mqnd_cb)
{
   ...
   {
      m_LOG_MQSV_ND(MQND_AMF_COMP_UNREGISTER_SUCCESS,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,error,__FILE__,__LINE__);
      return NCSCC_RC_SUCCESS;
   }
   else
   {
      m_LOG_MQSV_ND(MQND_AMF_COMP_UNREGISTER_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,error,__FILE__,__LINE__);
      return NCSCC_RC_FAILURE;
   }
}

static void 
mqnd_amf_comp_terminate_callback(SaInvocationT invocation,
                                 const SaNameT *compName)
{
   MQND_CB     *mqnd_cb=0;
   ...
   if(haState == SA_AMF_HA_ACTIVE)
   {
     /* In integration CPSV code is not getting called 60265*/
      if(!mqnd_cb->is_restart_done)   
      {
        ...
        m_LOG_MQSV_ND(MQND_MQA_TMR_STARTED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_NOTICE,saErr,__FILE__,__LINE__);
      }
   }
