...
static uns32 mqd_mds_enc(MQD_CB * pMqd, MDS_CALLBACK_ENC_INFO *enc)
{
  ...
  /* Get the Msg Format version from the SERVICE_ID & RMT_SVC_PVT_SUBPART_VERSION */
   ...
   if(enc->o_msg_fmt_ver)
   {
    ...
    if(rc != NCSCC_RC_SUCCESS)
      m_LOG_MQSV_D(MQD_MDS_ENCODE_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
    return rc;
   }
   else
   {
...
static uns32 mqd_mds_dec(MQD_CB *pMqd, MDS_CALLBACK_DEC_INFO *dec)
{
   ...
   if(is_valid_msg_fmt && (dec->i_msg_fmt_ver!=1))
   {
    ...
    if(NCSCC_RC_SUCCESS != rc) {
      m_LOG_MQSV_D(MQD_MDS_DECODE_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
         ...
         */
         ...
         if((svc->i_svc_id == NCSMDS_SVC_ID_MQA))
         {
            ...
            if(pEvt)
            {
               ...
               if(NCSCC_RC_SUCCESS != rc) 
               {               
                 m_LOG_MQSV_D(MQD_MDS_SVC_EVT_MQA_DOWN_EVT_SEND_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
                 m_MMGR_FREE_MQSV_EVT(pEvt, pMqd->my_svc_id);
               }
            }
           else 
         ...
         if(svc->i_svc_id == NCSMDS_SVC_ID_MQND)
         {
                  ...
                  if(pNdEvent)
                  {
                     ...
                     if(NCSCC_RC_SUCCESS != rc)
                     {
                        ...
                        m_LOG_MQSV_D(MQD_MDS_SVC_EVT_MQND_DOWN_EVT_SEND_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
                     }
                  }
                  else
                  {
    ...
    case NCSMDS_UP:            
     {
         if(svc->i_svc_id == NCSMDS_SVC_ID_MQND)
         {     
              uns16 to_dest_slotid,o_msg_fmt_ver;
              ...
              if(pNdEvent)
              {
                 ...
                 if(NCSCC_RC_SUCCESS != rc)
                 {
                    m_LOG_MQSV_D(MQD_MDS_SVC_EVT_MQND_UP_EVT_SEND_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
                    m_MMGR_FREE_MQSV_EVT(pNdEvent, pMqd->my_svc_id);
                 }
              }
              else
...
static uns32 mqd_mds_quiesced_process(MQD_CB* pMqd)
{
   ...
   if(pMqd->is_quisced_set)
   {
      ...
      if(rc != NCSCC_RC_SUCCESS)
      {
         m_LOG_MQSV_D(MQD_MDS_QUISCED_EVT_SEND_FAILED,NCSFL_LC_MQSV_INIT,NCSFL_SEV_ERROR,rc,__FILE__,__LINE__);
         m_MMGR_FREE_MQSV_EVT(pEvt, pMqd->my_svc_id);
      }
   }
   return rc;
