...
void mqd_get_node_from_group_for_getnext(MQD_CB *cb, MQD_OBJ_NODE **grpNode, MQD_OBJ_INFO **qNode,
					 SaNameT memberQueueName)
{
		...
		while (front != NCS_QELEM_NULL) {
			...
			if ((memcmp(&frontptr->pObject->name, &memberQueueName, sizeof(SaNameT))) > 0) {
				if (!localQueuePtr) {
				    ...
				    if ((memcmp
					 (&frontptr->pObject->name, &localQueuePtr->pObject->name,
					  sizeof(SaNameT))) < 0) {
			...
			*grpNode = pNode;
			...
			while (pNode) {
				if (pNode->oinfo.type == MQSV_OBJ_QUEUE) {
					localQueueName = pNode->oinfo.name;
					pNode =
					    (MQD_OBJ_NODE *)ncs_patricia_tree_getnext(&cb->qdb,
										      (uns8 *)&localQueueName);
				} else
					break;
			}
