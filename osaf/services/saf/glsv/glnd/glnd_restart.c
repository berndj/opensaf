...
uns32 glnd_restart_build_database( GLND_CB *glnd_cb )
{
  ...
  if( ret_val == NCSCC_RC_SUCCESS ) 
  {
    ...
    if( ret_val == NCSCC_RC_SUCCESS) 
    {
      ...
      if( ret_val == NCSCC_RC_SUCCESS) 
      {
         m_LOG_GLND(GLND_RESTART_BUILD_DATABASE_SUCCESS,NCSFL_LC_HEADLINE, NCSFL_SEV_INFO,ret_val,__FILE__,__LINE__, 0, 0,0);
         return NCSCC_RC_SUCCESS;
      }
      else
      {
         m_LOG_GLND(GLND_RESTART_BUILD_DATABASE_FAILURE,NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR,ret_val,__FILE__,__LINE__, 0, 0,0);
...
static uns32 glnd_restart_build_resource_tree(GLND_CB   *glnd_cb )
{
   ...
   for(i=0;i < GLND_RESOURCE_INFO_CKPT_MAX_SECTIONS ;i++)
   {
      if( shm_base_addr[i].valid == GLND_SHM_INFO_VALID )
      {
        ...
        {
           m_LOG_GLND(GLND_RESTART_RESOURCE_TREE_BUILD_FAILURE,NCSFL_LC_HEADLINE, NCSFL_SEV_INFO,rc,__FILE__,__LINE__, 0, 0,0);
           return NCSCC_RC_FAILURE;
        }
      }
   }
   m_LOG_GLND(GLND_RESTART_RESOURCE_TREE_BUILD_SUCCESS,NCSFL_LC_HEADLINE, NCSFL_SEV_INFO,NCSCC_RC_SUCCESS,__FILE__,__LINE__, 0, 0,0);
  ...
  uns32                              i;
   ...
   for(i=0;i < GLND_RES_LOCK_INFO_CKPT_MAX_SECTIONS ;i++)
   {
       if( shm_base_addr[i].valid == GLND_SHM_INFO_VALID )
       {
         ...
         {
            m_LOG_GLND(GLND_RESTART_LCK_LIST_BUILD_FAILURE,NCSFL_LC_HEADLINE, NCSFL_SEV_INFO,rc,__FILE__,__LINE__, 0, 0,0);
            return NCSCC_RC_FAILURE;
         }

       }
   }
   m_LOG_GLND(GLND_RESTART_LCK_LIST_BUILD_SUCCESS,NCSFL_LC_HEADLINE, NCSFL_SEV_INFO,NCSCC_RC_SUCCESS,__FILE__,__LINE__, 0, 0,0);
   return NCSCC_RC_SUCCESS;
}
                                         ...
                                         GLND_RESTART_RES_LOCK_LIST_INFO restart_res_lock_list_info )
{
  ...
  uns32                     node_id;
   ...
   client_info = (GLND_CLIENT_INFO *)ncs_patricia_tree_get(&glnd_cb->glnd_client_tree,(uns8 *)&restart_res_lock_list_info.lock_info.handleId);
   ...
   if(res_info)
   {
     ...
     if(restart_res_lock_list_info.to_which_list == 2)
     {
...
static uns32 glnd_restart_event_add(GLND_CB *glnd_cb, GLSV_RESTART_BACKUP_EVT_INFO *evt_info)
{
   ...
   {
     for(lck_list_info= res_node->lcl_lck_req_info;lck_list_info!=NULL;lck_list_info= lck_list_info->next)
     {
...
static uns32 glnd_restart_build_backup_event_tree(GLND_CB *glnd_cb)
{
  ...
  for(i=0;i < GLND_BACKUP_EVT_CKPT_MAX_SECTIONS;i++)
  {
      if( shm_base_address[i].valid == GLND_SHM_INFO_VALID )
      {
         ...
         {
            m_LOG_GLND(GLND_RESTART_EVT_LIST_BUILD_FAILURE,NCSFL_LC_HEADLINE, NCSFL_SEV_INFO,rc,__FILE__,__LINE__, 0, 0,0);
            return NCSCC_RC_FAILURE;
         }

      }
   }
   m_LOG_GLND(GLND_RESTART_EVT_LIST_BUILD_SUCCESS,NCSFL_LC_HEADLINE, NCSFL_SEV_INFO,NCSCC_RC_SUCCESS,__FILE__,__LINE__, 0, 0,0);
   return NCSCC_RC_SUCCESS;
}
