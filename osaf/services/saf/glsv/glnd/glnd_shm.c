...
static uns32 glnd_shm_open(GLND_CB *cb, char shm_name[] )
{
  ...
  if( rc == NCSCC_RC_FAILURE)
  {
      ...
      if( rc == NCSCC_RC_FAILURE)
      {
         m_LOG_GLND(GLND_SHM_CREATE_FAILURE,NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR,rc,__FILE__,__LINE__, 0,0,0);
         return rc;
      }
      else
      {
...
uns32 glnd_shm_create(GLND_CB *cb)
{
  ...
  if( cb->node_state == GLND_CLIENT_INFO_GET_STATE )
  {
     ...
     glnd_start_tmr(cb,&cb->agent_info_get_tmr,
                     ...
                     GLND_AGENT_INFO_GET_TIMEOUT,
                     (uns32)cb->cb_hdl_id);
     m_LOG_GLND(GLND_RESTARTED,NCSFL_LC_HEADLINE, NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__, 0, 0, 0);
  }
  else
     m_LOG_GLND(GLND_COMING_UP_FIRST_TIME,NCSFL_LC_HEADLINE, NCSFL_SEV_NOTICE,rc,__FILE__,__LINE__, 0, 0, 0);
