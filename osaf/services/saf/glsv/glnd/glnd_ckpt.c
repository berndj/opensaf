...
uns32 glnd_restart_res_lock_list_ckpt_write( GLND_CB *glnd_cb, GLND_RES_LOCK_LIST_INFO *res_lock_list,SaLckResourceIdT res_id, SaLckHandleT app_handle_id,uns8 to_which_list);

uns32 glnd_restart_res_lock_list_ckpt_overwrite( GLND_CB *glnd_cb, GLND_RES_LOCK_LIST_INFO *res_lock_list,SaLckResourceIdT res_id, SaLckHandleT app_handle_id,uns8 to_which_list);

uns32 glnd_restart_res_lock_ckpt_read( GLND_CB *glnd_cb, GLND_RESTART_RES_LOCK_LIST_INFO *restart_res_lock_info, uns32 offset );
...
uns32 glnd_restart_resource_info_ckpt_write( GLND_CB *glnd_cb, GLND_RESOURCE_INFO *res_info)
{
  ...
  restart_resource_info.pr_orphan_req_count = res_info->lck_master_info.
                                               pr_orphan_req_count;
  restart_resource_info.ex_orphan_req_count = res_info->lck_master_info.
                                               ex_orphan_req_count;
  restart_resource_info.pr_orphaned     = res_info->lck_master_info.
                                               pr_orphaned;
  restart_resource_info.ex_orphaned     = res_info->lck_master_info.
                                               ex_orphaned; 
  ...
  {
      m_LOG_GLND(GLND_RESOURCE_SHM_WRITE_FAILURE,NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__, 0, res_info->resource_id, 0);
      return rc;
  }
  return rc;
...
uns32 glnd_restart_lock_event_info_ckpt_write( GLND_CB *glnd_cb, GLSV_RESTART_BACKUP_EVT_INFO    restart_backup_evt)
{
  ...
  if(rc != NCSCC_RC_SUCCESS)
  {
      m_LOG_GLND(GLND_EVT_LIST_SHM_WRITE_FAILURE,NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__, 0 , 0, 0);
      return rc;
  }
  return rc;
...
uns32 glnd_restart_res_lock_list_ckpt_write( GLND_CB *glnd_cb, GLND_RES_LOCK_LIST_INFO *res_lock_list,SaLckResourceIdT res_id, SaLckHandleT app_handle_id,uns8 to_which_list)
{
  ...
  restart_res_lock_list_info.glnd_res_lock_mds_ctxt  = res_lock_list->
  ...
  if(rc != NCSCC_RC_SUCCESS)
  {
      m_LOG_GLND(GLND_LCK_LIST_SHM_WRITE_FAILURE,NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__, app_handle_id, res_id,
                                                 res_lock_list->lock_info.lcl_lockid );
      return rc;
  }
  return rc;
...
{ 
  ...
  if(rc != NCSCC_RC_SUCCESS)
     m_LOG_GLND(GLND_LCK_LIST_SHM_READ_FAILURE,NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__, 0, 0,0);
...
uns32 glnd_restart_resource_ckpt_read( GLND_CB *glnd_cb, GLND_RESTART_RES_INFO   *restart_resource_info, uns32 offset)
{
  ...
  rc =ncs_os_posix_shm(&read_req);
  if(rc != NCSCC_RC_SUCCESS)
     m_LOG_GLND(GLND_RESOURCE_SHM_READ_FAILURE,NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__, 0, 0,0);
...
uns32 glnd_restart_backup_event_read( GLND_CB *glnd_cb, GLSV_RESTART_BACKUP_EVT_INFO     *restart_backup_evt, uns32 offset)
{
  ...
  rc =ncs_os_posix_shm(&read_req);
  if(rc != NCSCC_RC_SUCCESS)
     m_LOG_GLND(GLND_EVT_LIST_SHM_READ_FAILURE,NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR,NCSCC_RC_FAILURE,__FILE__,__LINE__, 0, 0,0);
