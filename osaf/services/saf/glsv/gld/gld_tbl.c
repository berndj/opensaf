...
static uns32 glsv_gld_map_from_instid(const uns32 *i_inst_ids, uns32 i_inst_len, GLSV_GLD_RSC_MAP_INFO *rsc_map_info,
				      GLSV_GLD_CB *gld_cb);
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 glsv_gld_map_from_instid(const uns32 *i_inst_ids, uns32 i_inst_len, GLSV_GLD_RSC_MAP_INFO *rsc_map_info,
			       GLSV_GLD_CB *gld_cb)
{
	SaNameT rsc_name;
	uns32 counter = 0;
	...
	tmp_rsc_map_info = (GLSV_GLD_RSC_MAP_INFO *)ncs_patricia_tree_get(&gld_cb->rsc_map_info, (uns8 *)&rsc_name);
	if (tmp_rsc_map_info)
...
uns32 salckresourceentry_extract(NCSMIB_PARAM_VAL *param, NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
	...
	if (var_info->param_id == saLckResourceCreationTimeStamp_ID) {
		...
		return NCSCC_RC_SUCCESS;
	} else if (var_info->param_id == saLckResourceIsOrphaned_ID) {
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 salckresourceentry_next(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data, uns32 *next_inst_id,
