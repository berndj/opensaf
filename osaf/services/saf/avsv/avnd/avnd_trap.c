...
/*****************************************************************************
  ...
  Notes         :
*****************************************************************************/
uns32 avnd_gen_comp_inst_failed_trap(AVND_CB *avnd_cb, AVND_COMP *comp)
{
	NCS_TRAP trap;
...
/*****************************************************************************
  ...
  Notes         :
*****************************************************************************/
uns32 avnd_gen_comp_term_failed_trap(AVND_CB *avnd_cb, AVND_COMP *comp)
{
	NCS_TRAP trap;
...
/*****************************************************************************
  ...
  Notes         :
*****************************************************************************/
uns32 avnd_gen_comp_clean_failed_trap(AVND_CB *avnd_cb, AVND_COMP *comp)
{
	NCS_TRAP trap;
...
/*****************************************************************************
  ...
  Notes         :
*****************************************************************************/
uns32 avnd_gen_comp_proxied_orphaned_trap(AVND_CB *avnd_cb, AVND_COMP *comp)
{
	NCS_TRAP trap;
...
*****************************************************************************/
uns32 avnd_populate_comp_traps(AVND_CB *avnd_cb, AVND_COMP *comp, NCS_TRAP *trap, NCS_BOOL isProxTrap)
{
...
/*****************************************************************************
  ...
  Notes         :
*****************************************************************************/
uns32 avnd_gen_su_oper_state_chg_trap(AVND_CB *avnd_cb, AVND_SU *su)
{
	NCS_TRAP trap;
...
/*****************************************************************************
  ...
  Notes         :
*****************************************************************************/
uns32 avnd_gen_su_pres_state_chg_trap(AVND_CB *avnd_cb, AVND_SU *su)
{
	NCS_TRAP trap;
...
*****************************************************************************/
uns32 avnd_populate_su_traps(AVND_CB *avnd_cb, AVND_SU *su, NCS_TRAP *trap, NCS_BOOL isOper)
{
	NCS_TRAP_VARBIND *i_trap_varbind = NULL;
...
/*****************************************************************************
  ...
  Notes         :
*****************************************************************************/
uns32 avnd_gen_comp_fail_on_node_trap(AVND_CB *avnd_cb, AVND_ERR_SRC errSrc, AVND_COMP *comp)
{
	NCS_TRAP trap;
	NCS_TRAP_VARBIND *i_trap_varbind = NULL;
	NCS_TRAP_VARBIND *s_trap_varbind = NULL;
...
/*****************************************************************************
  ...
  Notes         :
*****************************************************************************/
uns32 avnd_create_and_send_trap(AVND_CB *avnd_cb, NCS_TRAP *trap, SaEvtEventPatternT *pattern)
{
	uns32 status = NCSCC_RC_FAILURE;
	uns32 tlv_size = 0;
	...
	saStatus = saEvtEventAttributesSet(eventHandle,
					   &patternArray, priority, retentionTime, &avnd_cb->publisherName);
	if (saStatus != SA_AIS_OK) {
		m_MMGR_FREE_AVND_EDA_TLV_MSG(encoded_buffer);
...
uns32 avnd_eda_initialize(AVND_CB *avnd_cb)
{
	...
	if (!avnd_cb->evtHandle) {
		...
		avnd_cb->publisherName.length = strlen(avnd_cb->publisherName.value);

		/* update the Channel Name */
		strcpy(avnd_cb->evtChannelName.value, m_SNMP_EDA_EVT_CHANNEL_NAME);
		avnd_cb->evtChannelName.length = strlen(avnd_cb->evtChannelName.value);

		/* initialize the eda  */
		status = saEvtInitialize(&avnd_cb->evtHandle, &callbacks, &saf_version);
	...
	if (!avnd_cb->evtChannelHandle) {
		...
		status = saEvtChannelOpen(avnd_cb->evtHandle,
					  &avnd_cb->evtChannelName, channelFlags, timeout, &avnd_cb->evtChannelHandle);
		if (status != SA_AIS_OK) {
			/* log the error code here */
