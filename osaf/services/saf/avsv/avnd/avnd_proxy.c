...
uns32 avnd_evt_ava_comp_val_req(AVND_CB *cb, AVND_EVT *evt)
{
	...
	if (0 != (msg.info.avd = m_MMGR_ALLOC_AVSV_DND_MSG)) {
		...
		if ((0 != (rec = avnd_diq_rec_add(cb, &msg)))) {
			...
			rec->msg.info.avd->msg_info.n2d_comp_valid_info.proxy_comp_name_net = reg->proxy_comp_name_net;
			...
			if ((NCSCC_RC_SUCCESS != rc) && rec) {
				m_AVND_AVND_ERR_LOG("avnd_diq_rec_send:failed:Comp,Type and Hdl are",
						    &reg->comp_name_net, api_info->type, reg->hdl, 0, 0);
				/* pop & delete */
			...
			rc = NCSCC_RC_FAILURE;
			m_AVND_AVND_ERR_LOG("avnd_diq_rec_add failed:Comp,Type and Hdl are",
					    &reg->comp_name_net, api_info->type, reg->hdl, 0, 0);
		}
	} else
		rc = NCSCC_RC_FAILURE;
	...
	if (NCSCC_RC_FAILURE == rc) {
		m_AVND_AVND_ERR_LOG("avnd_evt_ava_comp_val_req :failure:Comp,Type and Hdl are",
				    &reg->comp_name_net, api_info->type, reg->hdl, 0, 0);
	}
...
uns32 avnd_evt_avd_comp_validation_resp_msg(AVND_CB *cb, AVND_EVT *evt)
{
	...
	m_AVND_AVND_ENTRY_LOG("avnd_evt_avd_comp_validation_resp_msg():Comp,MsgId,NodeId and result are",
	...
	if ((NULL == rec) || (memcmp(&info->comp_name_net,
				     &rec->msg.info.avd->msg_info.n2d_comp_valid_info.comp_name_net,
				     sizeof(SaNameT)) != 0)) {
		/* Seems the rec was deleted, some problem. */
		m_AVND_AVND_ERR_LOG("Valid Rep:Rec is NULL or Name Mismatch:Comp,MsgId,NodeId and result are",
				    &info->comp_name_net, info->msg_id, info->node_id, info->result, 0);
		rc = NCSCC_RC_FAILURE;
		goto done;
	}
	...
	if (AVSV_VALID_FAILURE == info->result) {
		...
		if ((comp) && (SA_AIS_OK == rc)) {
			...
			if (0 == (pxy_comp = m_AVND_COMPDB_REC_GET(cb->compdb, comp_valid_info.proxy_comp_name_net))) {
				avnd_internode_comp_del(cb, &(cb->internode_avail_comp_db), &(info->comp_name_net));
				rc = NCSCC_RC_FAILURE;
				amf_rc = SA_AIS_ERR_INVALID_PARAM;
				goto send_resp;
			}
			...
			if (rc != NCSCC_RC_SUCCESS) {
				m_AVND_AVND_ERR_LOG("avnd_avnd_msg_send failed:Comp,MsgId,NodeId and result are",
...
uns32 avnd_avnd_msg_send(AVND_CB *cb, uns8 *msg_info, AVSV_AMF_API_TYPE type, MDS_SYNC_SND_CTXT *ctxt, NODE_ID node_id)
{
	...
	m_AVND_AVND_ENTRY_LOG("avnd_avnd_msg_send():Type and NodeID are", NULL, type, node_id, 0, 0);
			     ...
			     MDS_SYNC_SND_CTXT *ctxt, SaAisErrorT *o_amf_rc, NCS_BOOL *int_ext_comp)
{
	...
	*int_ext_comp = FALSE;
	...
	m_AVND_AVND_ENTRY_LOG("avnd_int_ext_comp_hdlr():Comp and Type are", &comp_name, api_info->type, 0, 0, 0);
...
*******************************************************************************/
			...
			if (!cbk_rec) {
				m_AVND_AVND_ERR_LOG
				    ("avnd_int_ext_comp_hdlr:Couldn't get cbk_rec: Comp, Type and Mds are", &comp_name,
				     api_info->type, api_info->dest, 0, 0);
				rc = NCSCC_RC_FAILURE;
				goto done;
			}
...
/************************* Section 2 Ends *********************************/
		...
		if (NCSCC_RC_SUCCESS != rc) {
			...
			*o_amf_rc = SA_AIS_ERR_TRY_AGAIN;
			m_AVND_AVND_ERR_LOG("avnd_int_ext_comp_hdlr:Msg Send Failed: Comp, Type and Mds are",
...
uns32 avnd_int_ext_comp_val(AVND_CB *cb, SaNameT *comp_name, AVND_COMP **o_comp, SaAisErrorT *o_amf_rc)
{
	...
	*o_amf_rc = SA_AIS_OK;
	...
	m_AVND_AVND_ENTRY_LOG("avnd_int_ext_comp_val(): Comp Name is", comp_name, 0, 0, 0, 0);
