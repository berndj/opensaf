...
uns32 avnd_evt_avnd_avnd_msg (AVND_CB *cb, AVND_EVT *evt)
{
   ...
   m_AVND_AVND_ENTRY_LOG("avnd_evt_avnd_avnd_msg(): Msg Type is ",
                         NULL,avnd_avnd_msg->type,0,0,0);
...
uns32 avnd_evt_avnd_avnd_api_msg_hdl (AVND_CB *cb, AVND_EVT *evt)
{
   ...
   m_AVND_AVND_ENTRY_LOG("avnd_evt_avnd_avnd_api_msg_hdl(): evt_type is",
                        NULL,evt_type,0,0,0);
   ...
   if(AVND_EVT_AVA_COMP_REG == evt_type)
   {
     if(AVND_LED_STATE_GREEN != cb->led_state)
     {
       ...
       if(NULL == comp) return NCSCC_RC_FAILURE;
...
uns32 avnd_evt_avnd_avnd_cbk_msg_hdl (AVND_CB *cb, AVND_EVT *evt)
{
   ...
   if(0 == (comp = m_AVND_INT_EXT_COMPDB_REC_GET(cb->internode_avail_comp_db,
            cbk_info->param.hc.comp_name_net)))
   {
     /* 
     ...
     */
     ...
     m_AVND_AVND_ERR_LOG("Couldn't find comp in Inter/Ext Comp DB:Comp,Type,Hdl and Inv are",
                    &cbk_info->param.hc.comp_name_net,cbk_info->type,
                    cbk_info->hdl,cbk_info->inv,0);
   ...
   /* add the record */
   if ( (0 != (rec = avnd_comp_cbq_rec_add(cb, comp, cbk_rec, 
                                           &comp->reg_dest, 0))) )
   {
