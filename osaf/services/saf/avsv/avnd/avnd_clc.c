...
uns32 avnd_comp_clc_st_chng_prc(AVND_CB *, AVND_COMP *, NCS_PRES_STATE, NCS_PRES_STATE);
...
static AVND_COMP_CLC_FSM_FN avnd_comp_clc_fsm[NCS_PRES_MAX - 1][AVND_COMP_CLC_PRES_FSM_EV_MAX - 1] = {
...
uns32 avnd_evt_clc_resp(AVND_CB *cb, AVND_EVT *evt)
{
	...
	if (clc_evt->exec_ctxt) {
		...
		for (comp = (AVND_COMP *)ncs_patricia_tree_getnext(&cb->compdb,
								   (uns8 *)0); comp;
		     comp = (AVND_COMP *)ncs_patricia_tree_getnext(&cb->compdb, (uns8 *)&comp->name_net)) {
...
uns32 avnd_comp_clc_fsm_run(AVND_CB *cb, AVND_COMP *comp, AVND_COMP_CLC_PRES_FSM_EV ev)
{
	...
	m_AVND_LOG_COMP_FSM(AVND_LOG_COMP_FSM_ENTER, prv_st, ev, &comp->name_net, NCSFL_SEV_NOTICE);
	...
	m_AVND_LOG_COMP_FSM(AVND_LOG_COMP_FSM_EXIT, final_st, 0, &comp->name_net, NCSFL_SEV_NOTICE);
...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 avnd_comp_clc_st_chng_prc(AVND_CB *cb, AVND_COMP *comp, NCS_PRES_STATE prv_st, NCS_PRES_STATE final_st)
{
	...
	if ((NCS_PRES_INSTANTIATED == prv_st) && (NCS_PRES_UNINSTANTIATED != final_st)) {
	...
	if ((NCS_PRES_RESTARTING == prv_st) && (NCS_PRES_INSTANTIATIONFAILED == final_st)) {
	...
	if ((NCS_PRES_INSTANTIATIONFAILED == final_st)
		...
		syslog(LOG_ERR, "NCS_AvSv: %s got Inst failed", comp->name_net.value);
	}
	...
	if (m_AVND_SU_IS_PREINSTANTIABLE(comp->su) && m_AVND_COMP_TYPE_IS_PREINSTANTIABLE(comp)) {
		...
		if ((NCS_PRES_INSTANTIATED == prv_st) && (NCS_PRES_TERMINATING == final_st)) {
		...
		if ((NCS_PRES_TERMINATING == prv_st) && (NCS_PRES_TERMINATIONFAILED == final_st)) {
	...
	*((uns32 *)param.value) = m_NCS_OS_HTONL(comp->pres);
	...
	/* if enabled - Start AM */
	if (comp->is_am_en && NCS_PRES_INSTANTIATED == final_st && NCS_PRES_ORPHANED != prv_st)
...
uns32 avnd_comp_clc_cmd_execute(AVND_CB *cb, AVND_COMP *comp, AVND_COMP_CLC_CMD_TYPE cmd_type)
{
	...
	strncpy(env_val_name.value, comp->name_net.value, m_NCS_OS_NTOHS(comp->name_net.length));
