...
AVD_HLT *avd_hlt_struc_crt(AVD_CL_CB *cb, AVSV_HLT_KEY lhlt_chk, NCS_BOOL ckpt)
{
	...
	memcpy(hlth->key_name.comp_name_net.value, lhlt_chk.comp_name_net.value,
	       m_NCS_OS_NTOHS(lhlt_chk.comp_name_net.length));
...
{
	AVD_HLT *hlth = AVD_HLT_NULL;
	hlth = (AVD_HLT *)ncs_patricia_tree_getnext(&cb->hlt_anchor, (uns8 *)&lhlt_key);
...
uns32 saamfhealthchecktableentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data)
{
...
uns32 saamfhealthchecktableentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL test_flag)
{
	...
	if (hlt_chk == AVD_HLT_NULL) {
		...
		if (arg->req.info.set_req.i_param_val.i_param_id == saAmfHealthCheckRowStatus_ID) {
			...
			case NCS_ROW_ACTIVE:
				...
				if ((hlt_chk->max_duration == (SaTimeT)0) || (hlt_chk->period == (SaTimeT)0)) {
	...
	if (arg->req.info.set_req.i_param_val.i_param_id == saAmfHealthCheckRowStatus_ID) {
		...
		old_time = *time;
		*time = m_NCS_OS_NTOHLL_P(arg->req.info.set_req.i_param_val.info.i_oct);
