...
static uns32 avd_rcv_mab_req(struct ncsmib_arg *req)
{
	...
	if ((cb = (AVD_CL_CB *)ncshm_take_hdl(NCS_SERVICE_ID_AVD, (uns32)(req->i_mib_key))) == AVD_CL_CB_NULL) {
...
static uns32 avd_mab_reg_tbls(AVD_CL_CB *cb)
{
	...
	for (tbl_id = NCSMIB_TBL_AVSV_BASE; tbl_id <= NCSMIB_TBL_AVSV_AVD_END; tbl_id++) {
		avd_oac_arg.info.tbl_owned.is_persistent = TRUE;

		if (((tbl_id >= NCSMIB_TBL_AVSV_AMF_SG_SI_RANK) &&
		     (tbl_id <= NCSMIB_TBL_AVSV_AMF_SG_SU_RANK)) || (tbl_id == NCSMIB_TBL_AVSV_AMF_SU_SI)) {
...
uns32 avd_mab_reg_rows(AVD_CL_CB *cb)
{
	...
	for (tbl_id = NCSMIB_TBL_AVSV_BASE; tbl_id <= NCSMIB_TBL_AVSV_AVD_END; tbl_id++) {
		...
		cb->mab_row_hdl_list[tbl_id - NCSMIB_TBL_AVSV_BASE] = avd_oac_arg.info.row_owned.o_row_hdl;
...
uns32 avd_mab_unreg_rows(AVD_CL_CB *cb)
{
	...
	for (tbl_id = NCSMIB_TBL_AVSV_BASE; tbl_id <= NCSMIB_TBL_AVSV_AVD_END; tbl_id++) {
		...
		avd_oac_arg.i_tbl_id = tbl_id;
		avd_oac_arg.info.row_gone.i_row_hdl = cb->mab_row_hdl_list[tbl_id - NCSMIB_TBL_AVSV_BASE];
