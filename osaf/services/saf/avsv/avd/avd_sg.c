...
uns32 ncssgtableentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data)
{
...
uns32 ncssgtableentry_extract(NCSMIB_PARAM_VAL *param, NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
...
uns32 ncssgtableentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL test_flag)
{
	AVD_CL_CB *avd_cb = (AVD_CL_CB *)cb;
	AVD_SG *sg;
	SaNameT sg_name;
	...
	if (arg->req.info.set_req.i_param_val.i_param_id == ncsSGIsNcs_ID) {
		...
		if ((sg->row_status != NCS_ROW_ACTIVE) ||
		    ...
		    (sg->sg_ncs_spec == SA_TRUE) || (avd_cb->init_state != AVD_APP_STATE))
			return NCSCC_RC_INV_VAL;
		else {
...
uns32 saamfsgtableentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data)
{
	AVD_CL_CB *avd_cb = (AVD_CL_CB *)cb;
	AVD_SG *sg;
	SaNameT sg_name;
...
uns32 saamfsgtableentry_extract(NCSMIB_PARAM_VAL *param, NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
	AVD_SG *sg = (AVD_SG *)data;
...
uns32 saamfsgtableentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL test_flag)
{
	AVD_CL_CB *avd_cb = (AVD_CL_CB *)cb;
	AVD_SG *sg;
	SaNameT sg_name;
		...
		if (arg->req.info.set_req.i_param_val.i_param_id == saAmfSGRowStatus_ID) {
			...
			case NCS_ROW_DESTROY:
				...
				if (sg->row_status == NCS_ROW_ACTIVE) {
					 ...
					 * of this node.
					 */
					if ((sg->list_of_su != AVD_SU_NULL) || (sg->list_of_si != AVD_SI_NULL)) {
						/* log information error */
						return NCSCC_RC_INV_VAL;
					}
