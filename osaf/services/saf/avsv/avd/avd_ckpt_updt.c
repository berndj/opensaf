 ...
 * 
\**************************************************************************/
uns32 avsv_ckpt_add_rmv_updt_avnd(AVD_CL_CB *cb, AVD_AVND *avnd, NCS_MBCSV_ACT_TYPE action)
{
 ...
 * 
\**************************************************************************/
uns32 avsv_ckpt_add_rmv_updt_sg_data(AVD_CL_CB *cb, AVD_SG *sg, NCS_MBCSV_ACT_TYPE action)
{
	uns32 status = NCSCC_RC_SUCCESS;
	AVD_SG *sg_ptr;
	...
	case NCS_MBCSV_ACT_ADD:
		{
			if (NULL == sg_ptr) {
				 ...
				 */
				if (AVD_SG_NULL == (sg_ptr = avd_sg_struc_crt(cb, sg->name_net, TRUE))) {
					m_AVD_LOG_INVALID_VAL_FATAL(action);
					return NCSCC_RC_FAILURE;
				}
			}
 ...
 * 
\**************************************************************************/
uns32 avsv_ckpt_add_rmv_updt_su_data(AVD_CL_CB *cb, AVD_SU *su, NCS_MBCSV_ACT_TYPE action)
{
	uns32 status = NCSCC_RC_SUCCESS;
	AVD_SU *su_ptr;
	SaNameT temp_name;
	...
	case NCS_MBCSV_ACT_ADD:
		{
			if (NULL == su_ptr) {
				 ...
				 */
				if (AVD_SU_NULL == (su_ptr = avd_su_struc_crt(cb, su->name_net, TRUE))) {
					m_AVD_LOG_INVALID_VAL_FATAL(action);
					return NCSCC_RC_FAILURE;
				}
			}
			...
			if (su_ptr->row_status != NCS_ROW_ACTIVE) {
				...
				memcpy(su_ptr->sg_name.value, su->sg_name.value, su->sg_name.length);

				/* Add to SG list */
				if ((su_ptr->sg_of_su = avd_sg_struc_find(cb, su_ptr->sg_name, TRUE)) == AVD_SG_NULL) {
				...
				if (FALSE == su_ptr->su_is_external) {
					...
					if (NULL == cb->ext_comp_info.ext_comp_hlt_check) {
						/* This is an external SU and we need to create the
						   supporting info. */
						cb->ext_comp_info.ext_comp_hlt_check = m_MMGR_ALLOC_AVD_AVND;
						if (NULL == cb->ext_comp_info.ext_comp_hlt_check) {
							avd_su_del_sg_list(cb, su_ptr);
							return NCSCC_RC_INV_VAL;
						}
						memset(cb->ext_comp_info.ext_comp_hlt_check, 0, sizeof(AVD_AVND));
					}
					/* if(NULL == cb->ext_comp_info.ext_comp_hlt_check) */
 ...
 * 
\**************************************************************************/
uns32 avsv_ckpt_add_rmv_updt_si_dep_data(AVD_CL_CB *cb, AVD_SI_SI_DEP *si_dep, NCS_MBCSV_ACT_TYPE action)
{
	uns32 status = NCSCC_RC_SUCCESS;
	...
	case NCS_MBCSV_ACT_ADD:
		{
			if (NULL == si_ptr_up) {
				...
				if (NULL == (si_ptr_up = avd_si_si_dep_struc_crt(cb, &si_dep->indx_mib))) {
					m_AVD_LOG_INVALID_VAL_FATAL(action);
					return NCSCC_RC_FAILURE;
				}
			}
 ...
 * 
\**************************************************************************/
uns32 avsv_ckpt_add_rmv_updt_si_data(AVD_CL_CB *cb, AVD_SI *si, NCS_MBCSV_ACT_TYPE action)
{
	uns32 status = NCSCC_RC_SUCCESS;
	AVD_SI *si_ptr_up;
	...
	case NCS_MBCSV_ACT_ADD:
		{
			if (NULL == si_ptr_up) {
				 ...
				 * add new SI entry into pat tree.
				 */
				if (AVD_SI_NULL == (si_ptr_up = avd_si_struc_crt(cb, si->name_net, TRUE))) {
					m_AVD_LOG_INVALID_VAL_FATAL(action);
					return NCSCC_RC_FAILURE;
				}
			}
			...
			if (si_ptr_up->row_status != NCS_ROW_ACTIVE) {
				...
				memcpy(si_ptr_up->sg_name.value, si->sg_name.value, si->sg_name.length);
 ...
 * 
\**************************************************************************/
uns32 avsv_decode_add_rmv_su_oper_list(AVD_CL_CB *cb, AVD_SU *su_ptr, NCS_MBCSV_ACT_TYPE action)
{
	uns32 status = NCSCC_RC_SUCCESS;
	AVD_SU *su_struct;
 ...
 * 
\**************************************************************************/
uns32 avsv_update_sg_admin_si(AVD_CL_CB *cb, NCS_UBAID *uba, NCS_MBCSV_ACT_TYPE action)
{
	uns32 status = NCSCC_RC_SUCCESS;
	AVD_SI *si, *si_ptr_up;
	AVD_SI dec_si;
 ...
 * 
\**************************************************************************/
uns32 avsv_updt_su_si_rel(AVD_CL_CB *cb, AVSV_SU_SI_REL_CKPT_MSG *su_si_ckpt, NCS_MBCSV_ACT_TYPE action)
{
	uns32 status = NCSCC_RC_SUCCESS;
	...
	su_si_rel_ptr = avd_susi_struc_find(cb, su_si_ckpt->su_name_net, su_si_ckpt->si_name_net, FALSE);
 ...
 * 
\**************************************************************************/
uns32 avsv_ckpt_add_rmv_updt_comp_data(AVD_CL_CB *cb, AVD_COMP *comp, NCS_MBCSV_ACT_TYPE action)
{
	uns32 status = NCSCC_RC_SUCCESS;
	AVD_COMP *comp_ptr;
	SaNameT temp_name;
	...
	case NCS_MBCSV_ACT_UPDATE:
		{
			if (NULL != comp_ptr) {
				...
				memcpy(comp_ptr->comp_info.init_info,
				       comp->comp_info.init_info, AVSV_MISC_STR_MAX_SIZE);
				...
				memcpy(comp_ptr->comp_info.term_info,
				       comp->comp_info.term_info, AVSV_MISC_STR_MAX_SIZE);
				...
				memcpy(comp_ptr->comp_info.clean_info,
				       comp->comp_info.clean_info, AVSV_MISC_STR_MAX_SIZE);
				...
				memcpy(comp_ptr->comp_info.amstart_info,
				       comp->comp_info.amstart_info, AVSV_MISC_STR_MAX_SIZE);
				...
				memcpy(comp_ptr->comp_info.amstop_info,
				       comp->comp_info.amstop_info, AVSV_MISC_STR_MAX_SIZE);
				...
				comp_ptr->comp_info.terminate_callback_timeout =
				...
				comp_ptr->comp_info.quiescing_complete_timeout =
				...
				comp_ptr->comp_info.proxied_inst_callback_timeout =
				    comp->comp_info.proxied_inst_callback_timeout;
				comp_ptr->comp_info.proxied_clean_callback_timeout =
 ...
 * 
\**************************************************************************/
uns32 avsv_ckpt_add_rmv_updt_csi_data(AVD_CL_CB *cb, AVD_CSI *csi, NCS_MBCSV_ACT_TYPE action)
{
	uns32 status = NCSCC_RC_SUCCESS;
	AVD_CSI *csi_ptr;
	SaNameT temp_name;
	...
	case NCS_MBCSV_ACT_ADD:
		{
			if (NULL == csi_ptr) {
				 ...
				 */
				if (AVD_CSI_NULL == (csi_ptr = avd_csi_struc_crt(cb, csi->name_net, TRUE))) {
					m_AVD_LOG_INVALID_VAL_FATAL(action);
					return NCSCC_RC_FAILURE;
				}
			} else {
			...
			if (csi_ptr->row_status != NCS_ROW_ACTIVE) {
				...
				memcpy(csi_ptr->csi_type.value, csi->csi_type.value, csi->csi_type.length);
 ...
 * 
\**************************************************************************/
uns32 avsv_ckpt_add_rmv_updt_hlt_data(AVD_CL_CB *cb, AVD_HLT *hlt, NCS_MBCSV_ACT_TYPE action)
{
	uns32 status = NCSCC_RC_SUCCESS;
	...
	case NCS_MBCSV_ACT_UPDATE:
		{
			if (NULL == hlt_ptr) {
				 ...
				 */
				if (AVD_HLT_NULL == (hlt_ptr = avd_hlt_struc_crt(cb, hlt->key_name, TRUE))) {
					m_AVD_LOG_INVALID_VAL_FATAL(action);
					return NCSCC_RC_FAILURE;
				}
			}
	...
	case NCS_MBCSV_ACT_UPDATE:
		{
			...
			if (su_si->row_status != NCS_ROW_ACTIVE) {
				...
				memcpy(su_si->su_name.value, su_si_rank->su_name.value, su_si->su_name.length);
			}
 ...
 *
\**************************************************************************/
uns32 avsv_ckpt_add_rmv_updt_comp_cs_type_data(AVD_CL_CB *cb, AVD_COMP_CS_TYPE *comp_cs_type, NCS_MBCSV_ACT_TYPE action)
{
	uns32 status = NCSCC_RC_SUCCESS;
	AVD_COMP_CS_TYPE *comp_cs = AVD_COMP_CS_TYPE_NULL;
