...
AVD_SU *avd_su_struc_find(AVD_CL_CB *cb, SaNameT su_name, NCS_BOOL host_order)
{
	...
	lsu_name.length = (host_order == FALSE) ? su_name.length : m_HTON_SANAMET_LEN(su_name.length);
...
uns32 saamfsutableentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data)
{
...
uns32 saamfsutableentry_extract(NCSMIB_PARAM_VAL *param, NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
	...
	case saAmfSUParentSGName_ID:
		m_AVSV_OCTVAL_TO_PARAM(param, buffer, su->sg_name.length, su->sg_name.value);
		break;
	default:
...
uns32 saamfsutableentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL test_flag)
{
	AVD_CL_CB *avd_cb = (AVD_CL_CB *)cb;
	AVD_SU *su;
	SaNameT su_name;
				...
				if (FALSE == su->su_is_external) {
					...
					if (NULL == avd_cb->ext_comp_info.ext_comp_hlt_check) {
						/* This is an external SU and we need to create the 
						   supporting info. */
						avd_cb->ext_comp_info.ext_comp_hlt_check = m_MMGR_ALLOC_AVD_AVND;
						if (NULL == avd_cb->ext_comp_info.ext_comp_hlt_check) {
							avd_su_del_sg_list(avd_cb, su);
							return NCSCC_RC_INV_VAL;
						}
				...
				if (su->num_of_comp == su->curr_num_comp) {
					...
					if (m_AVD_APP_SU_IS_INSVC(su, su_node_ptr)) {
						...
						case AVSV_SG_RED_MODL_NWAYACTV:
							if (avd_sg_nacvred_su_insvc_func(avd_cb, su) !=
							    NCSCC_RC_SUCCESS) {
								m_AVD_SET_SU_REDINESS(cb, su, NCS_OUT_OF_SERVICE);
								m_AVD_SET_SU_ADMIN(cb, su, NCS_ADMIN_STATE_LOCK);
								return NCSCC_RC_INV_VAL;
	...
	case saAmfSUParentSGName_ID:
		...
		memcpy(su->sg_name.value, arg->req.info.set_req.i_param_val.info.i_oct, su->sg_name.length);
		break;
	case saAmfSUFailOver_ID:
...
uns32 ncssutableentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data)
{
	AVD_CL_CB *avd_cb = (AVD_CL_CB *)cb;
	AVD_SU *su;
	SaNameT su_name;
...
uns32 ncssutableentry_extract(NCSMIB_PARAM_VAL *param, NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
	AVD_SU *su = (AVD_SU *)data;
...
uns32 ncssutableentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL test_flag)
{
	AVD_CL_CB *avd_cb = (AVD_CL_CB *)cb;
	AVD_SU *su;
	SaNameT su_name;
...
uns32 saamfsgsurankentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data)
{
	AVD_CL_CB *avd_cb = (AVD_CL_CB *)cb;
	AVD_SG_SU_RANK_INDX indx;
	uns16 len;
...
uns32 saamfsgsurankentry_extract(NCSMIB_PARAM_VAL *param, NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
...
uns32 saamfsgsurankentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL test_flag)
{
	return NCSCC_RC_FAILURE;
}
