...
extern AVM_ENT_INFO_T *avm_add_ent_info(AVM_CB_T *avm_cb, SaHpiEntityPathT *entity_path)
{
	...
	if (ncs_patricia_tree_add(&avm_cb->db.ent_info_anchor, &ent_info->tree_node) != NCSCC_RC_SUCCESS) {
		m_AVM_LOG_PATRICIA_OP(AVM_LOG_PAT_ADD, entity_path->Entry, sizeof(SaHpiEntityPathT),
				      AVM_LOG_PAT_FAILURE, NCSFL_SEV_CRITICAL);
		m_MMGR_FREE_AVM_ENT_INFO(ent_info);
		return AVM_ENT_INFO_NULL;
	}

	if ((ent_info->ent_hdl = ncshm_create_hdl(NCS_HM_POOL_ID_COMMON, NCS_SERVICE_ID_AVM, ent_info)) == 0) {
	...
	ent_info->dhcp_serv_conf.label1.other_label = &ent_info->dhcp_serv_conf.label2;

	ent_info->dhcp_serv_conf.label2.other_label = &ent_info->dhcp_serv_conf.label1;
	...
	m_AVM_LOG_PATRICIA_OP(AVM_LOG_PAT_ADD, entity_path->Entry, sizeof(SaHpiEntityPathT), AVM_LOG_PAT_SUCCESS,
			      NCSFL_SEV_INFO);
...
extern AVM_ENT_INFO_T *avm_find_first_ent_info(AVM_CB_T *avm_cb)
{
	...
	ent_info = (AVM_ENT_INFO_T *)
	    ncs_patricia_tree_getnext(&avm_cb->db.ent_info_anchor, (uns8 *)AVM_ENT_INFO_NULL);
...
extern AVM_NODE_INFO_T *avm_add_node_name_info(AVM_CB_T *avm_cb, SaNameT node_name)
{
	...
	if (ncs_patricia_tree_add(&avm_cb->db.node_name_anchor, &node_info->tree_node_name) != NCSCC_RC_SUCCESS) {
		m_AVM_LOG_PATRICIA_OP(AVM_LOG_PAT_ADD, node_name.value, SA_MAX_NAME_LENGTH, AVM_LOG_PAT_FAILURE,
				      NCSFL_SEV_CRITICAL);
...
extern uns32 avm_populate_node_info(AVM_CB_T *cb, SaHpiEntityPathT *entity_path, AVM_NODE_INFO_T *node_info)
{
	...
	if (AVM_ENT_INFO_NULL == ent_info) {
		m_AVM_LOG_GEN("No entity with Entity Path:", entity_path->Entry, sizeof(SaHpiEntityPathT),
			      NCSFL_SEV_CRITICAL);
		return NCSCC_RC_FAILURE;
	}
...
extern uns32 find_parent_entity_path(SaHpiEntityPathT *child_entity_path, SaHpiEntityPathT *parent_entity_path)
{
	uns32 i;
...
extern AVM_VALID_INFO_T *avm_find_next_valid_info(AVM_CB_T *avm_cb, AVM_ENT_DESC_NAME_T *desc_name)
{
	...
	valid_info =
	    (AVM_VALID_INFO_T *)ncs_patricia_tree_getnext(&avm_cb->db.valid_info_anchor, (uns8 *)desc_name->name);
