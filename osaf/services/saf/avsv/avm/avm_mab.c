 ...
 **********************************************************************/
static AVM_MIBLIB_REG_FUNC avm_miblib_reg_func[NCSMIB_TBL_AVM_END - NCSMIB_TBL_AVM_BASE + 1] = {
...
static uns32 avm_rcv_mab_req(struct ncsmib_arg *req)
{
...
static uns32 avm_mab_reg_tbls(AVM_CB_T *cb)
{
	...
	for (tbl_id = NCSMIB_TBL_AVM_BASE; tbl_id <= NCSMIB_TBL_AVM_END; tbl_id++) {
...
static uns32 avm_mab_reg_rows(AVM_CB_T *cb)
{
	...
	for (tbl_id = NCSMIB_TBL_AVM_BASE; tbl_id <= NCSMIB_TBL_AVM_END; tbl_id++) {
		...
		cb->mab_row_hdls[tbl_id - NCSMIB_TBL_AVM_BASE] = avm_oac_arg.info.row_owned.o_row_hdl;
...
static uns32 avm_mab_unreg_rows(AVM_CB_T *cb)
{
	...
	for (tbl_id = NCSMIB_TBL_AVM_BASE; tbl_id <= NCSMIB_TBL_AVM_END; tbl_id++) {
		...
		avm_oac_arg.i_tbl_id = tbl_id;
		avm_oac_arg.info.row_gone.i_row_hdl = cb->mab_row_hdls[tbl_id - NCSMIB_TBL_AVM_BASE];
...
uns32 avm_miblib_init(AVM_CB_T *cb)
{
	...
	if (NCSCC_RC_SUCCESS != reg_func()) {
		m_AVM_LOG_MAS_VAL(NCSMIB_TBL_AVM_UPGRADE, AVM_LOG_MIBLIB_REGISTER, AVM_LOG_MAS_SKIP,
				  NCSFL_SEV_CRITICAL);
		rc = NCSCC_RC_FAILURE;
	} else {
