...
static uns32 avm_encode_cold_sync_rsp_validation_info(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);
static uns32 avm_encode_cold_sync_rsp_ent_state(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);
static uns32 avm_encode_cold_sync_rsp_async_updt_cnt(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);

static uns32 avm_encode_ckpt_ent_cfg(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);

static uns32 avm_encode_ckpt_evt_id(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);

static uns32 avm_encode_ckpt_ent_state(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);

static uns32 avm_encode_ckpt_ent_state_sensor(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);

static uns32 avm_encode_ckpt_ent_add_dependent(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);

static uns32 avm_encode_ckpt_evt_id(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);

static uns32 avm_encode_ckpt_ent_adm_op(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);

static uns32 avm_encode_ckpt_dummy(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);
static uns32 avm_complete_db(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc, NCS_BOOL cold_sync);

/* to update entity DHCP configuration */
static uns32 avm_encode_ckpt_ent_dhcp_conf_chg(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);

/* to update entity DHCP conf state */
static uns32 avm_encode_ckpt_ent_dhcp_state_chg(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);

/* to update entity upgrade state */
static uns32 avm_encode_ckpt_ent_upgd_state_chg(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc);
...
const AVM_ENCODE_CKPT_DATA_FUNC_PTR avm_enc_ckpt_data_func_list[AVM_CKPT_MSG_MAX] = {
 ...
 */
const AVM_ENCODE_COLD_SYNC_RSP_DATA_FUNC_PTR avm_enc_cold_sync_rsp_data_func_list[AVM_CKPT_COLD_SYNC_MSG_MAX] = {
	...
	case NCS_MBCSV_ACT_UPDATE:
		{
			...
			status = m_NCS_EDU_VER_EXEC(&cb->edu_hdl, avm_edp_ckpt_msg_ent, &enc->io_uba, EDP_OP_TYPE_ENC,
						    (AVM_ENT_INFO_T *)(NCS_INT64_TO_PTR_CAST(enc->io_reo_hdl)),
...
/*         cb->edu_hdl.to_version = enc->i_peer_version; */
			status =
			    m_NCS_EDU_SEL_VER_EXEC(&cb->edu_hdl, avm_edp_ckpt_msg_ent, &enc->io_uba, EDP_OP_TYPE_ENC,
						   (AVM_ENT_INFO_T *)(NCS_INT64_TO_PTR_CAST(enc->io_reo_hdl)), &ederror,
						   enc->i_peer_version, 1, 2);
		}
		break;
...
static uns32 avm_encode_ckpt_ent_state_sensor(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc)
{
	...
	status = m_NCS_EDU_SEL_VER_EXEC(&cb->edu_hdl, avm_edp_ckpt_msg_ent, &enc->io_uba, EDP_OP_TYPE_ENC,
					(AVM_ENT_INFO_T *)(NCS_INT64_TO_PTR_CAST(enc->io_reo_hdl)), &ederror,
					enc->i_peer_version, 5, 2, 9, 12, 13, 14);
...
static uns32 avm_encode_ckpt_ent_adm_op(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc)
{
	...
	status = m_NCS_EDU_SEL_VER_EXEC(&cb->edu_hdl, avm_edp_ckpt_msg_ent, &enc->io_uba, EDP_OP_TYPE_ENC,
					(AVM_ENT_INFO_T *)(NCS_INT64_TO_PTR_CAST(enc->io_reo_hdl)), &ederror,
					enc->i_peer_version, 4, 2, 18, 19, 20);
...
static uns32 avm_encode_ckpt_evt_id(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc)
{
	...
	status = m_NCS_EDU_VER_EXEC(&cb->edu_hdl, avm_edp_ckpt_msg_evt_id, &enc->io_uba, EDP_OP_TYPE_ENC,
				    (AVM_EVT_ID_T *)(NCS_INT64_TO_PTR_CAST(enc->io_reo_hdl)), &ederror,
				    enc->i_peer_version);
...
static uns32 avm_encode_ckpt_ent_dhcp_conf_chg(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc)
{
	...
	status = m_NCS_EDU_SEL_VER_EXEC(&cb->edu_hdl, avm_edp_ckpt_msg_ent, &enc->io_uba,
					EDP_OP_TYPE_ENC, (AVM_ENT_INFO_T *)(NCS_INT64_TO_PTR_CAST(enc->io_reo_hdl)),
					&ederror, enc->i_peer_version, 2, 2, 22);
...
static uns32 avm_encode_ckpt_ent_dhcp_state_chg(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc)
{
	...
	status = m_NCS_EDU_SEL_VER_EXEC(&cb->edu_hdl, avm_edp_ckpt_msg_ent, &enc->io_uba,
					EDP_OP_TYPE_ENC, (AVM_ENT_INFO_T *)(NCS_INT64_TO_PTR_CAST(enc->io_reo_hdl)),
					&ederror, enc->i_peer_version, 2, 2, 23);
...
static uns32 avm_encode_ckpt_ent_upgd_state_chg(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc)
{
   ...
   ***************************************************************/
	...
	status = ncs_edu_exec(&cb->edu_hdl, avm_edp_ckpt_msg_ent, &enc->io_uba,
			      EDP_OP_TYPE_ENC, (AVM_ENT_INFO_T *)(NCS_INT64_TO_PTR_CAST(enc->io_reo_hdl)), &ederror, 2,
			      2, 26);
...
static uns32 avm_encode_cold_sync_rsp_validation_info(AVM_CB_T *cb, NCS_MBCSV_CB_ENC *enc)
{
	...
	for (valid_info =
	     (AVM_VALID_INFO_T *)ncs_patricia_tree_getnext(&cb->db.valid_info_anchor, (uns8 *)desc_name.name);
	     valid_info != AVM_VALID_INFO_NULL;
	     valid_info =
	     (AVM_VALID_INFO_T *)ncs_patricia_tree_getnext(&cb->db.valid_info_anchor, (uns8 *)desc_name.name)) {
		status =
		    m_NCS_EDU_VER_EXEC(&cb->edu_hdl, avm_edp_ckpt_msg_validation_info, &enc->io_uba, EDP_OP_TYPE_ENC,
