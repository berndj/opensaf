...
static uns32 avm_assign_state(AVM_ENT_INFO_T *ent_info, AVM_FSM_STATES_T next_state);
...
static uns32 avm_comp_text_buffers(SaHpiTextBufferT *src, SaHpiTextBufferT *dst)
{
...
static uns32 avm_validate_fru(AVM_ENT_INFO_T *ent_info, HPI_EVT_T *fsm_evt)
{
	...
	if (NCSCC_RC_SUCCESS ==
	    avm_comp_text_buffers(&ent_info->valid_info->inv_data.product_name, &fsm_evt->inv_data.product_name)) {
		if (NCSCC_RC_SUCCESS ==
		    avm_comp_text_buffers(&ent_info->valid_info->inv_data.product_version,
					  &fsm_evt->inv_data.product_version)) {
			return NCSCC_RC_SUCCESS;
		} else {
 ...
 ***********************************************************************
******/
static uns32 avm_list_nodes_chgd(AVM_ENT_INFO_T *ent_info, AVM_LIST_HEAD_T *head)
{
...
static uns32 avm_ins_pend(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
   ...
   *****************************************************************/
	...
	if (ent_info->ipmc_tmr.status == AVM_TMR_RUNNING) {
		...
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
			 "AVM-SSU: Payload blade %s: Unexpected Event received, while upgrading IPMC",
	...
	if ((strcmp(arch_type, "HP_CCLASS") != 0) && (strcmp(arch_type, "HP_PROLIANT") != 0)) {
	...
	if (TRUE == ent_info->depends_on_valid) {
		...
		if (AVM_ENT_INFO_NULL == dep_ent_info) {
			m_AVM_LOG_GEN("Depends On: No Entity existing ", ent_info->depends_on.Entry,
				      sizeof(SaHpiEntityPathT), NCSFL_SEV_WARNING);
			avm_hisv_api_cmd(ent_info, HS_RESOURCE_INACTIVE_SET, 0);
			return NCSCC_RC_SUCCESS;
		} else {
			if (dep_ent_info->row_status != NCS_ROW_ACTIVE) {
				m_AVM_LOG_GEN_EP_STR("Depends On: Row Status Not Active", dep_ent_info->ep_str.name,
						     NCSFL_SEV_WARNING);
				avm_hisv_api_cmd(ent_info, HS_RESOURCE_INACTIVE_SET, 0);
				return NCSCC_RC_FAILURE;
			}

			if ((dep_ent_info->current_state == AVM_ENT_ACTIVE)
			    || (dep_ent_info->current_state == AVM_ENT_RESET_REQ)) {
				m_AVM_LOG_GEN_EP_STR("Depends On: Activate Entity", ent_info->ep_str.name,
						     NCSFL_SEV_INFO);
				api_cmd = HS_RESOURCE_ACTIVE_SET;
			} else {
				m_AVM_LOG_GEN_EP_STR("Depends On: Dependee Entity not Yet Active",
						     ent_info->ep_str.name, NCSFL_SEV_INFO);

				rc = avm_validate_fru(ent_info, hpi_evt);
				if (NCSCC_RC_SUCCESS != rc) {
					m_AVM_LOG_GEN_EP_STR("FRU validation Failure:", ent_info->ep_str.name,
							     NCSFL_SEV_ERROR);
...
************************************************************************
*****/
extern uns32 avm_active(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	...
	for (child = ent_info->child; child != NULL; child = child->next) {
		if ((child->ent_info->is_fru) && (AVM_ADM_LOCK != child->ent_info->adm_lock)) {
			m_AVM_LOG_GEN_EP_STR("Child Entity insertion Requested", child->ent_info->ep_str.name,
					     NCSFL_SEV_INFO);
			rc = avm_hisv_api_cmd(child->ent_info, HS_ACTION_REQUEST, SAHPI_HS_ACTION_INSERTION);
			if (NCSCC_RC_SUCCESS != rc)
				m_AVM_LOG_GEN_EP_STR("Child Entity insertion Request Failed",
						     child->ent_info->ep_str.name, NCSFL_SEV_INFO);
...
************************************************************************
*****/
static uns32 avm_not_pres_active(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_not_pres_ext_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	...
	for (child = ent_info->child; child != AVM_ENT_INFO_LIST_NULL; child = child->next)
		if ((AVM_ENT_INACTIVE < (child->ent_info->current_state))
		    && (AVM_ENT_INVALID > (child->ent_info->current_state)))
...
************************************************************************
*****/
static uns32 avm_not_pres_avd_reset_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
************************************************************************
*****/
static uns32 avm_not_pres_adm_shutdown(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
************************************************************************
*****/
static uns32 avm_not_pres_adm_lock(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
************************************************************************
*****/
static uns32 avm_not_pres_adm_unlock(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_not_pres_resource_rest(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
...
************************************************************************
*****/
static uns32 avm_inactive_adm_shutdown(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
************************************************************************
*****/
static uns32 avm_inactive_adm_lock(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
************************************************************************
*****/
static uns32 avm_inactive_adm_unlock(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	uns8 str[AVM_LOG_STR_MAX_LEN];
...
************************************************************************
*****/
static uns32 avm_inactive_active(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	uns8 str[AVM_LOG_STR_MAX_LEN];
	...
	if (ent_info->previous_state == AVM_ENT_EXTRACT_REQ) {
		m_AVM_LOG_GEN_EP_STR("Latch Opened and closed without AvM resp", ent_info->ep_str.name,
				     NCSFL_SEV_ERROR);
		return NCSCC_RC_SUCCESS;
	}
...
static uns32 avm_inactive_inactive(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_LIST_HEAD_T head;
	AVM_LIST_NODE_T *node;
...
************************************************************************
*****/
static uns32 avm_inactive_extracted(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	uns8 str[AVM_LOG_STR_MAX_LEN];
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
...
static uns32 avm_inactive_resource_fail(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
...
static uns32 avm_inactive_resource_rest(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_inactive_sensor_assert(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_inactive_sensor_deassert(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
************************************************************************
*****/
static uns32 avm_active_adm_shutdown(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	...
	if ((AVM_ADM_LOCK != ent_info->adm_lock) && (!ent_info->adm_shutdown)) {
		...
		if (NCSCC_RC_SUCCESS != rc) {
			...
			m_AVM_LOG_GEN_EP_STR("SAHPI_HS_ACTION_EXTRACTION failed", ent_info->ep_str.name,
					     NCSFL_SEV_ERROR);
		} else {
...
************************************************************************
*****/
static uns32 avm_active_adm_lock(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_EVT_T *evt = (AVM_EVT_T *)fsm_evt;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
...
************************************************************************
*****/
static uns32 avm_active_adm_unlock(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
...
************************************************************************
*****/
static uns32 avm_active_extract_pend(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
...
static uns32 avm_active_adm_soft_reset_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
...
/*  Soft reset case , Child will not be reseted , Parent-Child in different fault domain  */
	...
	uns8 *cli_msg =
	    "Either lock or shutdown of this entity is in progress as well as a locked entity can not be reset";
...
static uns32 avm_active_adm_hard_reset_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	...
	if ((!ent_info->adm_shutdown) && (AVM_ADM_LOCK != ent_info->adm_lock)) {
		...
		if (NCSCC_RC_SUCCESS != rc) {
			if (avm_is_this_entity_self(avm_cb, ent_info->entity_path) == TRUE) {
				...
				evt->evt.mib_req->rsp.add_info =
				    m_MMGR_ALLOC_MIB_OCT(evt->evt.mib_req->rsp.add_info_len + 1);

				memset(evt->evt.mib_req->rsp.add_info, 0, evt->evt.mib_req->rsp.add_info_len + 1);

				memcpy(evt->evt.mib_req->rsp.add_info, cli_msg3, evt->evt.mib_req->rsp.add_info_len);

				m_AVM_LOG_GEN_EP_STR
				    ("AvM failed to reset using HISV but AvD will attempt NCS Reboot of entity:",
				...
				m_AVM_LOG_GEN_EP_STR("Cold/Adm Hard Reset Failure", ent_info->ep_str.name,
						     NCSFL_SEV_ERROR);
			}
		} else {
...
static uns32 avm_active_resource_fail(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
	AVM_LIST_HEAD_T head;
...
************************************************************************
*****/
static uns32 avm_active_sensor_assert(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
************************************************************************
*****/
static uns32 avm_active_sensor_deassert(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_active_node_shutdown_resp(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
...
static uns32 avm_active_avd_reset_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
************************************************************************
*****/
static uns32 avm_active_surp_extract(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
************************************************************************
*****/
static uns32 avm_active_extracted(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
...
************************************************************************
*****/
static uns32 avm_active_inactive(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
...
static uns32 avm_faulty_node_failover_resp(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	AVM_LIST_HEAD_T head;
	uns32 rc = NCSCC_RC_SUCCESS;
	...
	if (NCSCC_RC_SUCCESS == ent_info->sensor_assert) {
		m_AVM_LOG_GEN("Node is not yet deasserted ", ent_info->node_name.value, SA_MAX_NAME_LENGTH,
			      NCSFL_SEV_INFO);
	} else {
		head.node = m_MMGR_ALLOC_AVM_AVD_LIST_NODE;
		if (AVM_LIST_NODE_NULL == head.node) {
...
static uns32 avm_faulty_sensor_deassert(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_faulty_adm_reset_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_faulty_avd_reset_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_NODE_RESET_RESP_T resp;
...
static uns32 avm_faulty_sensor_assert(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	m_AVM_LOG_GEN("Entity is already under failover", ent_info->entity_path.Entry, sizeof(SaHpiEntityPathT),
		      NCSFL_SEV_INFO);
	return NCSCC_RC_SUCCESS;
}

static uns32 avm_faulty_extract_pend(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_faulty_surp_extract(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_faulty_adm_shutdown(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	return NCSCC_RC_SUCCESS;
}

static uns32 avm_faulty_adm_lock(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	return NCSCC_RC_SUCCESS;
}
...
static uns32 avm_reset_req_extract_pend(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
...
************************************************************************
*****/
static uns32 avm_reset_req_node_shutdown_resp(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_NODE_RESET_RESP_T resp;
	AVM_EVT_T *avd_avm;
	...
	if (AVM_ADM_LOCK == ent_info->adm_lock) {
		m_AVM_LOG_GEN_EP_STR("Entity Locked, Ignore the Soft Reset by admin", ent_info->ep_str.name,
				     NCSFL_SEV_WARNING);
		rc = avm_hisv_api_cmd(ent_info, HS_RESOURCE_INACTIVE_SET, 0);
		return rc;
	}
	...
	if (NCSCC_RC_FAILURE == avd_avm->evt.avd_evt->avd_avm_msg.shutdown_resp.recovery_status) {
		m_AVM_LOG_GEN("AvD rejected Reset request for the node", ent_info->node_name.value, SA_MAX_NAME_LENGTH,
			      NCSFL_SEV_ERROR);
		avm_assign_state(ent_info, AVM_ENT_ACTIVE);
		return NCSCC_RC_SUCCESS;
	}
...
************************************************************************
*****/
static uns32 avm_reset_req_node_failover_resp(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
...
************************************************************************
*****/
static uns32 avm_reset_req_surp_extract(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
************************************************************************
*****/
static uns32 avm_reset_req_avd_reset_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	...
	if (TRUE == ent_info->is_node) {
		...
		if (NCSCC_RC_SUCCESS != rc) {
			...
			m_AVM_LOG_GEN_EP_STR("AvD Issued Reset/Cold Successful", ent_info->ep_str.name,
					     NCSFL_SEV_NOTICE);
		}
		avm_avd_node_reset_resp(avm_cb, resp, ent_info->node_name);
...
static uns32 avm_reset_req_resource_fail(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
	AVM_LIST_HEAD_T head;
...
************************************************************************
*****/
static uns32 avm_reset_req_sensor_assert(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc;
...
************************************************************************
*****/
static uns32 avm_reset_req_sensor_deassert(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc;
...
************************************************************************
*****/
static uns32 avm_reset_req_adm_hard_reset_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_NODE_RESET_RESP_T resp;
	AVM_EVT_T *evt = (AVM_EVT_T *)fsm_evt;
	...
	if ((AVM_ADM_LOCK == ent_info->adm_lock) || (ent_info->adm_shutdown)) {
		...
		m_AVM_LOG_GEN_EP_STR("Entity Locked, Ignore the Hard Reset by admin", ent_info->ep_str.name,
				     NCSFL_SEV_WARNING);
		return NCSCC_RC_FAILURE;
	}
...
************************************************************************
*****/
static uns32 avm_reset_req_adm_shutdown(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_FAILURE;
	AVM_EVT_T *evt = (AVM_EVT_T *)fsm_evt;
...
************************************************************************
*****/
static uns32 avm_reset_req_adm_lock(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_EVT_T *evt = (AVM_EVT_T *)fsm_evt;
...
************************************************************************
*****/
static uns32 avm_reset_req_adm_unlock(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_FAILURE;
...
************************************************************************
*****/
static uns32 avm_reset_req_extracted(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	AVM_LIST_HEAD_T head;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
	uns32 rc = NCSCC_RC_SUCCESS;
...
************************************************************************
*****/
static uns32 avm_reset_req_inactive(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	AVM_LIST_HEAD_T head;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_ext_req_node_shutdown_resp(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_EVT_T *avd_avm;
	...
	if (NCSCC_RC_FAILURE == avd_avm->evt.avd_evt->avd_avm_msg.shutdown_resp.recovery_status) {
		m_AVM_LOG_GEN("AvD rejected shutdown request for the node", ent_info->node_name.value,
			      SA_MAX_NAME_LENGTH, NCSFL_SEV_ERROR);
	...
	if (ent_info->is_fru) {	/* Managed FRU  */
		for (child = ent_info->child; child != AVM_ENT_INFO_LIST_NULL; child = child->next) {
			if (AVM_ENT_EXTRACT_REQ == child->ent_info->current_state) {
				rc = avm_hisv_api_cmd(child->ent_info, HS_RESOURCE_INACTIVE_SET, 0);
				if (NCSCC_RC_SUCCESS != rc) {
					m_AVM_LOG_GEN_EP_STR("Entity Inactivation failure received  ",
							     child->ent_info->ep_str.name, NCSFL_SEV_NOTICE);
		...
		rc = avm_hisv_api_cmd(ent_info, HS_RESOURCE_INACTIVE_SET, 0);
		if (NCSCC_RC_SUCCESS != rc) {
			m_AVM_LOG_GEN_EP_STR("Entity Inactivation failure received  ", ent_info->ep_str.name,
					     NCSFL_SEV_NOTICE);
			/* If the above call failed, generate a trap to notify this */
 ...
 *****************************************************************************
 ******/
static uns32 avm_ext_req_active(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
 ...
 *****************************************************************************
 ******/
static uns32 avm_ext_req_inactive(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
	AVM_LIST_HEAD_T head;
 ...
 *****************************************************************************
 ******/
static uns32 avm_ext_req_avd_reset_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_NODE_RESET_RESP_T resp;
...
static uns32 avm_ext_req_adm_soft_reset_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_FAILURE;
...
static uns32 avm_ext_req_adm_hard_reset_req(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc;
	AVM_NODE_RESET_RESP_T resp;
	AVM_EVT_T *evt = (AVM_EVT_T *)fsm_evt;
...
static uns32 avm_ext_req_adm_shutdown(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_ext_req_adm_lock(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc;
	AVM_EVT_T *evt = (AVM_EVT_T *)fsm_evt;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
...
static uns32 avm_ext_req_adm_unlock(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_FAILURE;
 ...
 ***************************************************************************
 ******/
static uns32 avm_ext_req_extracted(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
	AVM_LIST_HEAD_T head;
...
static uns32 avm_ext_req_resource_fail(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	AVM_ENT_INFO_LIST_T *child = AVM_ENT_INFO_LIST_NULL;
	AVM_LIST_HEAD_T head;
...
************************************************************************
*****/
static uns32 avm_handle_fw_progress_event(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	...
	if (evt_state == HPI_EVT_FWPROG_PROG) {
		...
		if (evt_type == HPI_FWPROG_NODE_INIT_SUCCESS) {
			if (NULL != ent_info->dhcp_serv_conf.curr_act_label->name.name) {
				logbuf[0] = '\0';
				snprintf(logbuf, sizeof(logbuf) - 1, "AVM-SSU: Payload blade %s : Came up with %s",
			...
			if (ent_info->dhcp_serv_conf.upgd_prgs == TRUE) {
				...
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s : Upgraded successfully with %s",
					 ent_info->ep_str.name, ent_info->dhcp_serv_conf.curr_act_label->name.name);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
			}
		}
		...
		/* check if the new bios boots up successfully               */
			...
			if (avm_check_config(ent_info->ep_str.name, &flag) == NCSCC_RC_FAILURE) {
				snprintf(logbuf, sizeof(logbuf) - 1, "AVM-SSU: ssuHepler.conf file open has failed %s ",
					 "ssuHelper.conf");
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
				return NCSCC_RC_FAILURE;
			} else {
				if (flag == 1) {
					...
					snprintf(logbuf, sizeof(logbuf) - 1,
						 "AVM-SSU: Payload blade %s : Bios Upgraded successfully",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
		...
		/* timer                                                   */
			...
			snprintf(logbuf, sizeof(logbuf) - 1, "AVM-SSU: Payload blade %s : Bios Boot Success",
				 ent_info->ep_str.name);
			m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
...
/* functional                                                         */
				...
				/* async update */
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload %s : Upgrade Failed : BIOS UPGRADE TIMER RUNNING ",
					 ent_info->ep_str.name);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
			...
			snprintf(logbuf, sizeof(logbuf) - 1,
				 "AVM-SSU: Payload blade %s : Upgrade Failed with %s, BootProgressStatus=%x",
				 ent_info->ep_str.name, ent_info->dhcp_serv_conf.curr_act_label->name.name,
				 ent_info->boot_prg_status);
			m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
			...
			if (ent_info->bios_upgrade_tmr.status == AVM_TMR_RUNNING) {
				...
				logbuf[0] = '\0';
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload %s : BIOS UPGRADE TIMER RUNNING : Rollback Failed",
					 ent_info->ep_str.name);
...
{
	m_AVM_LOG_EVT_IGN(AVM_LOG_EVT_IGNORE, ((AVM_EVT_T *)fsm_evt)->fsm_evt_type, ent_info->current_state,
			  NCSFL_SEV_INFO);
	return NCSCC_RC_SUCCESS;
}
...
extern uns32 avm_handle_invalid_event(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt)
{
	...
	m_AVM_LOG_EVT_INVALID(AVM_LOG_EVT_INVALID, ((AVM_EVT_T *)fsm_evt)->fsm_evt_type, ent_info->current_state,
			      NCSFL_SEV_WARNING);

	return rc;
}

extern uns32 avm_handle_avd_error(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, AVM_EVT_T *evt)
{
