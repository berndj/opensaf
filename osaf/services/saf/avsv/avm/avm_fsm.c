...
static uns32 avm_proc_hpi(AVM_EVT_T *hpi_evt, AVM_CB_T *avm_cb);

static uns32 avm_proc_avd(AVM_EVT_T *avd_avm, AVM_CB_T *avm_cb);

static uns32 avm_proc_bam(AVM_EVT_T *bam_avm, AVM_CB_T *avm_cb);

static uns32 avm_proc_mib(AVM_EVT_T *mib_req, AVM_CB_T *avm_cb);

static uns32 avm_proc_tmr(AVM_EVT_T *tmr, AVM_CB_T *avm_cb);

static uns32 avm_proc_rde(AVM_EVT_T *rde_evt, AVM_CB_T *avm_cb);

static uns32 avm_proc_mds(AVM_EVT_T *mds_evt, AVM_CB_T *avm_cb);

static uns32 avm_proc_ssu(AVM_EVT_T *hpi_evt, AVM_CB_T *avm_cb);

static uns32 avm_proc_ssu_tmr_exp(AVM_EVT_T *hpi_evt, AVM_CB_T *avm_cb);

static uns32 avm_proc_upgd_succ_tmr_exp(AVM_EVT_T *hpi_evt, AVM_CB_T *avm_cb);

static uns32 avm_proc_boot_succ_tmr_exp(AVM_EVT_T *hpi_evt, AVM_CB_T *avm_cb);

static uns32 avm_proc_dhcp_fail_tmr_exp(AVM_EVT_T *hpi_evt, AVM_CB_T *avm_cb);

static uns32 avm_proc_bios_upgrade_tmr_exp(AVM_EVT_T *my_evt, AVM_CB_T *avm_cb);
 ...
 ************************************************************************
 ******/
extern uns32 avm_msg_handler(AVM_CB_T *avm_cb, AVM_EVT_T *evt)
{
...
static uns32 avm_proc_hpi(AVM_EVT_T *hpi_evt, AVM_CB_T *avm_cb)
{
	...
	if (AVM_ENT_INFO_NULL == ent_info) {
		m_AVM_LOG_GEN("No Entity in the DB with Entity Path ", hpi_evt->evt.hpi_evt->entity_path.Entry,
			      sizeof(SaHpiEntityPathT), NCSFL_SEV_WARNING);

		memcpy(temp_ent_info.entity_path.Entry, hpi_evt->evt.hpi_evt->entity_path.Entry,
   ...
   **************************************************************/
	if ((hpi_evt->evt.hpi_evt->hpi_event.EventDataUnion.HotSwapEvent.HotSwapState == SAHPI_HS_STATE_INACTIVE) &&
		...
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
			 "AVM-SSU: Payload blade %s: IPMC UPGRADE TRIGGERED AS BLADE GOT LOCKED",
...
static uns32 avm_proc_avd(AVM_EVT_T *avd_avm, AVM_CB_T *avm_cb)
{
	...
	if (AVM_NODE_INFO_NULL == node_info) {
		m_AVM_LOG_GEN("No Entity in the DB with Node Name ", node_name.value, SA_MAX_NAME_LENGTH,
			      NCSFL_SEV_WARNING);
 ...
 * Notes         : None.
 ************************************************************************/
static uns32 avm_proc_mds(AVM_EVT_T *mds, AVM_CB_T *avm_cb)
{
...
extern uns32 avm_fsm_handler(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, AVM_EVT_T *fsm_evt)
{
	...
	if (((ent_info->current_state >= AVM_ENT_NOT_PRESENT) && (ent_info->current_state <= AVM_ENT_INVALID)) &&
		...
		if ((avm_fsm_table_g[ent_info->current_state][fsm_evt->fsm_evt_type].current_state
		    ...
		    (avm_fsm_table_g[ent_info->current_state][fsm_evt->fsm_evt_type].evt == fsm_evt->fsm_evt_type)) {
			rc = avm_fsm_table_g[ent_info->current_state][fsm_evt->fsm_evt_type].f_ptr(avm_cb, ent_info,
												   fsm_evt);
		} else {
 ...
 ************************************************************************
 ******/
static uns32 avm_proc_upgd_succ_tmr_exp(AVM_EVT_T *my_evt, AVM_CB_T *avm_cb)
{
	AVM_ENT_INFO_T *ent_info;
	uns8 logbuf[500];
	...
	snprintf(logbuf, sizeof(logbuf) - 1,
		 "AVM-SSU: Payload blade %s : Upgrade Failed with %s, BootProgressStatus=%x", ent_info->ep_str.name,
		 ent_info->dhcp_serv_conf.curr_act_label->name.name, ent_info->boot_prg_status);
 ...
 ************************************************************************
 ******/
static uns32 avm_proc_boot_succ_tmr_exp(AVM_EVT_T *my_evt, AVM_CB_T *avm_cb)
{
	AVM_ENT_INFO_T *ent_info;
	uns8 logbuf[500];
	uns32 chassis;
	...
	snprintf(logbuf, sizeof(logbuf) - 1, "AVM-SSU: Payload blade %s : BootFailed for %s",
	...
	syslog(LOG_NOTICE,
	       "avm_proc_boot_succ_tmr_exp(): AVM issued cold reset for payload - %s, return value of API: %d, sent reset indication to FMwith return value of API: %d\n",
 ...
 ************************************************************************
 ******/
static uns32 avm_proc_dhcp_fail_tmr_exp(AVM_EVT_T *my_evt, AVM_CB_T *avm_cb)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
static uns32 avm_proc_bios_upgrade_tmr_exp(AVM_EVT_T *my_evt, AVM_CB_T *avm_cb)
{
	...
	if (ent_info->dhcp_serv_conf.upgd_prgs == TRUE) {
		...
		snprintf(logbuf, sizeof(logbuf) - 1, "AVM-SSU: Payload  %s :Bios Upgrade Tmr Exp :Switch Bios Bank",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
		...
		if (rc != NCSCC_RC_SUCCESS) {
			...
			snprintf(logbuf, sizeof(logbuf) - 1,
				 "AVM-SSU: Payload %s :Bios Upgrade Tmr Exp :Failed to switch Bios Bank",
				 ent_info->ep_str.name);
		...
		snprintf(logbuf, sizeof(logbuf) - 1, "AVM-SSU: Payload %s :Bios Upgrade Tmr Exp :Rollback Failed",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_CRITICAL);
		ncshm_give_hdl(my_evt->evt.tmr.ent_hdl);
		return NCSCC_RC_FAILURE;
...
static uns32 avm_proc_bios_failover_tmr_exp(AVM_EVT_T *my_evt, AVM_CB_T *avm_cb)
{
	...
	if (ent_info->dhcp_serv_conf.bios_upgd_state) {
		...
		snprintf(logbuf, sizeof(logbuf) - 1, "AVM-SSU: Payload %s : Bios Failover Tmr Exp: Bios State=%d",
		...
		case BIOS_TMR_EXPIRED:
			{
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s : Going to switch Bios Bank",
					 ent_info->ep_str.name);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
				rc = avm_hisv_api_cmd(ent_info, HISV_PAYLOAD_BIOS_SWITCH, 0);
				...
				if (rc != NCSCC_RC_SUCCESS) {
					...
					snprintf(logbuf, sizeof(logbuf) - 1,
						 "AVM-SSU: Payload blade %s : Failed to switch Bios Bank",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_CRITICAL);
					ncshm_give_hdl(my_evt->evt.tmr.ent_hdl);
					return NCSCC_RC_FAILURE;
		...
		case BIOS_EXP_BANK_0:
			{
				...
				if (rc != NCSCC_RC_SUCCESS) {
					snprintf(logbuf, sizeof(logbuf) - 1,
						 "AVM-SSU: Payload %s : Bios Failover Tmr Exp: Get Bios Bank",
						 ent_info->ep_str.name);
		...
		default:
			{
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s : Invalid bios_upgd_state:%d",
					 ent_info->ep_str.name, ent_info->dhcp_serv_conf.bios_upgd_state);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_CRITICAL);
			}
			break;
