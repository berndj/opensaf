...
static uns32 avm_validate_loc_range(AVM_ENT_INFO_T *ent_info, SaHpiEntityT entity, AVM_VALID_INFO_T *valid_info);
static uns32 avm_rmv_node_info(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info);

static uns32 avm_rmv_frm_parent(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info);

static uns32 avm_rmv_frm_dependee(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info);

static uns32 ncs_avm_build_mib_idx(NCSMIB_IDX *mib_idx, AVM_ENT_PATH_STR_T *idx_val);
...
extern uns32 avm_map_hpi2fsm(AVM_CB_T *cb, HPI_EVT_T *evt, AVM_FSM_EVT_TYPE_T *fsm_evt_type, AVM_ENT_INFO_T *ent_info)
{
	...
	case SAHPI_ET_RESOURCE:
		{
			...
			m_AVM_LOG_HPI_RES_EVT(AVM_LOG_EVT_RESOURCE,
					      hpi_event.EventDataUnion.ResourceEvent.ResourceEventType,
			...
			} else if (SAHPI_RESE_RESOURCE_RESTORED ==
				   hpi_event.EventDataUnion.ResourceEvent.ResourceEventType) {
	...
	case SAHPI_ET_HOTSWAP:
		{
			m_AVM_LOG_HPI_HS_EVT(AVM_LOG_EVT_HOTSWAP, hpi_event.EventDataUnion.HotSwapEvent.HotSwapState,
					     hpi_event.EventDataUnion.HotSwapEvent.PreviousHotSwapState,
					     ent_info->ep_str.name);
			...
			if (hpi_event.EventDataUnion.HotSwapEvent.HotSwapState == SAHPI_HS_STATE_NOT_PRESENT) {
				if ((hpi_event.EventDataUnion.HotSwapEvent.PreviousHotSwapState ==
				     SAHPI_HS_STATE_INACTIVE) || (ent_info->current_state == AVM_ENT_INACTIVE)) {
	...
	case SAHPI_ET_SENSOR:
		{
			m_AVM_LOG_HPI_HS_EVT(AVM_LOG_EVT_SENSOR, hpi_event.EventDataUnion.SensorEvent.Assertion,
					     hpi_event.Severity, ent_info->ep_str.name);
			m_AVM_LOG_HPI_HS_EVT(AVM_LOG_EVT_SENSOR, hpi_event.EventType,
					     hpi_event.EventDataUnion.SensorEvent.SensorType, ent_info->ep_str.name);
	...
	default:
		{
			m_AVM_LOG_GEN_EP_STR("AvM doesnt support the event from HPI for entity", ent_info->ep_str.name,
					     NCSFL_SEV_INFO);
		...
		if (entity_path.Entry[i].EntityType == SAHPI_ENT_ROOT) {
			...
			if ((SAHPI_ENT_UNSPECIFIED <= entity_path.Entry[i].EntityType) &&
				...
				if ((SAHPI_ENT_ROOT <= entity_path.Entry[i].EntityType) &&
				    (SAHPI_ENT_SUBBOARD_CARRIER_BLADE >= entity_path.Entry[i].EntityType)) {
					index =
					    SAHPI_ENT_BATTERY + entity_path.Entry[i].EntityType - SAHPI_ENT_ROOT + 4;
				} else {
					if (SAHPI_ENT_CHASSIS_SPECIFIC == entity_path.Entry[i].EntityType) {
						index = SAHPI_ENT_BATTERY + 1;
					} else if (SAHPI_ENT_BOARD_SET_SPECIFIC == entity_path.Entry[i].EntityType) {
						index = SAHPI_ENT_BATTERY + 2;
					} else if (SAHPI_ENT_OEM_SYSINT_SPECIFIC == entity_path.Entry[i].EntityType) {
...
extern uns32 avm_scope_active_criteria(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, uns32 **child_impact)
{
	uns32 node_insert = NCSCC_RC_FAILURE;
	uns32 rc = NCSCC_RC_SUCCESS;
...
extern uns32
avm_scope_fault_criteria(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, AVM_SCOPE_EVENTS_T fault_type, uns32 *impact)
{
	uns32 rc;
...
extern uns32 avm_scope_fault(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, AVM_SCOPE_EVENTS_T fault_type)
{
	...
	for (fault_node = fault_domain; fault_node != NULL; fault_node = fault_node->next) {
		...
		if (AVM_ENT_INFO_NULL == fault_ent_info) {
			m_AVM_LOG_GEN("Entity not found:", fault_node->ent_path.Entry, sizeof(SaHpiEntityPathT),
				      NCSFL_SEV_ERROR);
		} else {
	...
	if (dependee->dependents == AVM_ENT_INFO_LIST_NULL) {
		...
		for (temp = dependee->dependents; temp != AVM_ENT_INFO_LIST_NULL; temp = temp->next) {
			if (!memcmp(temp->ent_info->ep_str.name, dependent->ep_str.name, AVM_MAX_INDEX_LEN)) {
				m_AVM_LOG_GEN_EP_STR("Entity in the list", dependee->dependents->ent_info->ep_str.name,
						     NCSFL_SEV_INFO);
				return NCSCC_RC_SUCCESS;
			}
		}
			...
			if (strcmp(arch_type, "HP_PROLIANT") != 0) {
				...
				if (api_cmd == HS_RESOURCE_ACTIVE_SET) {
					...
					if (api_cmd == HS_RESOURCE_INACTIVE_SET) {
						rc = hpl_resource_power_set(chassis_id, entity_path,
									    HISV_RES_POWER_OFF);
					} else {
	...
	case HISV_PAYLOAD_BIOS_SWITCH:
		{
			...
			if (bootbank_number == 0) {
				ent_info->dhcp_serv_conf.bios_upgd_state =
				    (ent_info->dhcp_serv_conf.bios_upgd_state ==
				     BIOS_TMR_STOPPED) ? BIOS_STOP_BANK_0 : BIOS_EXP_BANK_0;
				...
				ent_info->dhcp_serv_conf.bios_upgd_state =
				    (ent_info->dhcp_serv_conf.bios_upgd_state ==
				     BIOS_TMR_STOPPED) ? BIOS_STOP_BANK_1 : BIOS_EXP_BANK_1;
				m_AVM_SEND_CKPT_UPDT_SYNC_UPDT(avm_cb, ent_info, AVM_CKPT_ENT_UPGD_STATE_CHG);
				ncshm_give_hdl(g_avm_hdl);
...
EXTERN_C uns32 avm_get_hpi_hs_state(AVM_ENT_INFO_T *ent_info, uns32 *arg)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	uns32 chassis_id;
	uns8 *entity_path = NULL;
...
extern uns32 avm_bld_validation_info(AVM_CB_T *cb, NCS_HW_ENT_TYPE_DESC *ent_desc_info)
{
	...
	for (ptr = ent_desc_info; NCS_HW_ENT_TYPE_DESC_NULL != ptr; ptr = ptr->next) {
		...
		memcpy(valid_info->inv_data.product_name.Data, ptr->fru_product_name,
		       valid_info->inv_data.product_name.DataLength);

		valid_info->inv_data.product_version.DataLength = strlen(ptr->fru_product_version);
		memcpy(valid_info->inv_data.product_version.Data, ptr->fru_product_version,
		       valid_info->inv_data.product_version.DataLength);
		...
		for (i = 0; i < ptr->num_possible_parents; i++) {
			...
			memcpy(valid_info->location[i].parent.name, ptr->location_range[i].parent_ent,
			       valid_info->location[i].parent.length);
...
extern uns32 avm_bld_valid_info_parent_child_relation(AVM_CB_T *cb)
{
	...
	for (valid_info = (AVM_VALID_INFO_T *)ncs_patricia_tree_getnext(&cb->db.valid_info_anchor, desc_name.name);
		...
		for (i = 0; i < valid_info->parent_cnt; i++) {
			...
			parent_valid_info =
			    (AVM_VALID_INFO_T *)ncs_patricia_tree_get(&cb->db.valid_info_anchor, parent_desc_name.name);
...
extern uns32 avm_check_deployment(AVM_CB_T *cb)
{
	...
	for (ent_info = (AVM_ENT_INFO_T *)avm_find_next_ent_str_info(cb, &ep_str);
	     ent_info != AVM_ENT_INFO_NULL; ent_info = (AVM_ENT_INFO_T *)avm_find_next_ent_str_info(cb, &ep_str)
	    ) {
		...
		ent_info->valid_info =
		    (AVM_VALID_INFO_T *)ncs_patricia_tree_get(&cb->db.valid_info_anchor, ent_info->desc_name.name);
		ent_info->parent_valid_info =
		...
		if (AVM_NODE_INFO_NULL != node_info) {
			...
			m_AVM_LOG_GEN(" Node already in DB", ent_info->node_name.value, SA_MAX_NAME_LENGTH,
				      NCSFL_SEV_WARNING);
			status = NCSCC_RC_FAILURE;
			avm_rmv_frm_parent(cb, ent_info);
			continue;
		...
		if (ent_info->parent_valid_info->entity_type == ent_info->entity_path.Entry[1].EntityType) {
			for (i = 0; i < MAX_POSSIBLE_PARENTS; i++) {
				if (ent_info->parent_valid_info == ent_info->valid_info->parents[i]) {
					if (ent_info->parent->row_status == NCS_ROW_ACTIVE) {
						m_AVM_LOG_GEN_EP_STR("Row Made Active", ent_info->ep_str.name,
								     NCSFL_SEV_INFO);
						ent_info->row_status = NCS_ROW_ACTIVE;
						m_AVM_ADM_LOCK_ENT(cb, ent_info);
						m_AVM_ADM_UNLOCK_ENT(cb, ent_info);
...
extern uns32 avm_check_dynamic_ent(AVM_CB_T *cb, AVM_ENT_INFO_T *ent_info)
{
	...
	ent_info->valid_info =
	    (AVM_VALID_INFO_T *)ncs_patricia_tree_get(&cb->db.valid_info_anchor, ent_info->desc_name.name);
	ent_info->parent_valid_info =
	    (AVM_VALID_INFO_T *)ncs_patricia_tree_get(&cb->db.valid_info_anchor, ent_info->parent_desc_name.name);
...
static uns32 avm_rmv_frm_parent(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info)
{
	...
	if (AVM_ENT_INFO_NULL != ent_info->parent) {
		if (ent_info == ent_info->parent->child->ent_info) {
			...
			for (temp = ent_info->parent->child;
			     (temp->next != AVM_ENT_INFO_LIST_NULL) && (ent_info != temp->next->ent_info);
			     temp = temp->next) ;
...
uns32 avm_push_admin_mib_set_to_psr(AVM_CB_T *cb, AVM_ENT_INFO_T *ent_info, AVM_ADM_OP_T adm_req)
{
	NCSMIB_ARG mib_arg;
	uns32 status = NCSCC_RC_SUCCESS;
 ...
 ******/
extern
uns32 avm_standby_boot_succ_tmr_handler(AVM_CB_T *avm_cb, AVM_EVT_T *hpi_evt, AVM_ENT_INFO_T *ent_info,
