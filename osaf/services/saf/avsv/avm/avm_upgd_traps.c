...
uns32 avm_send_boot_upgd_trap(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, uns32 trap_id)
{
	...
	if (trap_id == ncsAvmBootFailure_ID) {
	...
	} else if ((trap_id == ncsAvmUpgradeSuccess_ID) || (trap_id == ncsAvmCurrActiveLabelChange_ID)) {
		/* Allocate Memory */
		i_trap_varbind = m_MMGR_ALLOC_AVM_TRAP_VB;
		if (i_trap_varbind == NULL) {
			return NCSCC_RC_FAILURE;
		...
		i_trap_varbind->i_param_val.i_length = ent_info->dhcp_serv_conf.curr_act_label->name.length;
		i_trap_varbind->i_param_val.info.i_oct = ent_info->dhcp_serv_conf.curr_act_label->name.name;
		...
		/* Add to the trap */
		snmptm_avm_trap.i_trap_vb = i_trap_varbind;
	} else if ((trap_id == ncsAvmSwFwUpgradeFailure_ID) || (trap_id == ncsAvmUpgradeProgress_ID)) {
		/* Allocate Memory */
		i_trap_varbind = m_MMGR_ALLOC_AVM_TRAP_VB;
		if (i_trap_varbind == NULL) {
			return NCSCC_RC_FAILURE;
		...
		i_trap_varbind->i_param_val.i_length = sizeof(int);
		i_trap_varbind->i_param_val.info.i_int = avm_cb->upgrade_module;
		/* add to the trap */
		...
		if (trap_id == ncsAvmSwFwUpgradeFailure_ID) {
			...
			i_trap_varbind->i_param_val.i_length = sizeof(int);
			i_trap_varbind->i_param_val.info.i_int = avm_cb->upgrade_error_type;
		} else {
			...
			i_trap_varbind->i_param_val.i_length = sizeof(int);
			i_trap_varbind->i_param_val.info.i_int = avm_cb->upgrade_prg_evt;
		}
		/* add to the trap */
		temp_trap_varbind->next_trap_varbind = i_trap_varbind;
	}
	...
	saStatus = saEvtEventAttributesSet(eventHandle, &patternArray, priority, retentionTime, &avm_cb->publisherName);
	if (saStatus != SA_AIS_OK) {
 ...
 done1:
	if ((trap_id == ncsAvmUpgradeSuccess_ID) ||
	    ...
	    (trap_id == ncsAvmBootFailure_ID) || (trap_id == ncsAvmCurrActiveLabelChange_ID)) {
		m_MMGR_FREE_AVM_TRAP_VB(temp_trap_varbind);
	}

	if ((trap_id == ncsAvmUpgradeFailure_ID) || (trap_id == ncsAvmBootProgress_ID)) {
...
extern uns32 avm_open_trap_channel(AVM_CB_T *avm_cb)
{
	...
	status = saEvtChannelOpen(avm_cb->eda_hdl,
				  &avm_cb->trap_chan_name, channelFlags, timeout, &avm_cb->trap_chan_hdl);
