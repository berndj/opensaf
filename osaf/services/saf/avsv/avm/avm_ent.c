...
static uns32 avm_octet_string_2_entity_path(uns8 *ep, uns32 length, SaHpiEntityPathT *entity_path)
{
	uns8 i;
	uns32 entity_type;
...
ncsavmentdeploytableentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL test_flag)
{
	...
	if (AVM_ENT_INFO_NULL == ent_info) {
		...
		m_AVM_LOG_GEN("Entity Path Converted from String :", entity_path.Entry, sizeof(SaHpiEntityPathT),
			      NCSFL_SEV_INFO);
						...
						if (ent_info->adm_lock)
						{
							fsm_evt.fsm_evt_type =
							    (arg->req.info.set_req.i_param_val.info.i_int +
							     AVM_EVT_ADM_OP_BASE - 1);
							avm_fsm_handler(avm_cb, ent_info, &fsm_evt);
						}

						m_AVM_LOG_GEN_EP_STR("Row made active now:", ent_info->ep_str.name,
								     NCSFL_SEV_INFO);
					} else {
			...
			case NCS_ROW_NOT_IN_SERVICE:
				{
					if (AVM_ENT_NOT_PRESENT != ent_info->current_state) {
						m_AVM_LOG_GEN_EP_STR("Entity in Chassis", ent_info->ep_str.name,
								     NCSFL_SEV_INFO);
						return NCSCC_RC_INV_VAL;
					}
					...
					if (AVM_ENT_INFO_LIST_NULL != ent_info->child) {
						m_AVM_LOG_GEN_EP_STR
						    ("Child Entities Existing , Cant Make Row Status Not in Service",
	...
	if (((arg->req.info.set_req.i_param_val.i_param_id < ncsAvmEntNetBoot_ID) ||
		...
		if (avm_cb->config_state == AVM_CONFIG_NOT_DONE) {
			...
			case ncsAvmEntAdmReq_ID:
				{
					...
					fsm_evt.fsm_evt_type =
					    (arg->req.info.set_req.i_param_val.info.i_int + AVM_EVT_ADM_OP_BASE - 1);
			...
			case ncsAvmEntAdmReset_ID:
				{
					...
					fsm_evt.fsm_evt_type =
					    (arg->req.info.set_req.i_param_val.info.i_int + AVM_EVT_ADM_RESET_REQ_BASE -
					     1);
			...
			if (AVM_ENT_NOT_PRESENT != ent_info->current_state) {
				m_AVM_LOG_GEN_EP_STR("Entity In Chassis, no Mib sets allowed", ent_info->ep_str.name,
						     NCSFL_SEV_INFO);
				return NCSCC_RC_INV_VAL;
			}

			m_AVM_LOG_GEN_EP_STR("Row already active, no Mib sets allowed", ent_info->ep_str.name,
					     NCSFL_SEV_INFO);
			return NCSCC_RC_INV_VAL;
		}
	}
	...
	case ncsAvmEntNodeName_ID:
		{
			...
			memcpy(ent_info->node_name.value, arg->req.info.set_req.i_param_val.info.i_oct,
			       ent_info->node_name.length);
		}
		break;
	...
	case ncsAvmEntDependsOn_ID:
		{
			...
			memcpy(ent_info->dep_ep_str.name, arg->req.info.set_req.i_param_val.info.i_oct,
			       ent_info->dep_ep_str.length);

			if (ent_info->dep_ep_str.length) {
				avm_octet_string_2_entity_path(ent_info->dep_ep_str.name, ent_info->dep_ep_str.length,
							       &ent_info->depends_on);
	...
	case ncsAvmEntAdmReset_ID:
		{
			m_AVM_LOG_GEN_EP_STR("Cant issue Adm Reset, Row not yet active", ent_info->ep_str.name,
					     NCSFL_SEV_INFO);
			return NCSCC_RC_INV_VAL;
		}
		break;
	...
	case ncsAvmEntAdmReq_ID:
		{
			if ((AVM_ADM_SHUTDOWN == arg->req.info.set_req.i_param_val.info.i_int)
			    || (AVM_ADM_LOCK == arg->req.info.set_req.i_param_val.info.i_int)) {
	...
	case ncsAvmEntDescName_ID:
		{
			...
			memcpy(ent_info->desc_name.name, arg->req.info.set_req.i_param_val.info.i_oct,
			       ent_info->desc_name.length);
		}
		break;
	...
	case ncsAvmEntParentDescName_ID:
		{
			...
			memcpy(ent_info->parent_desc_name.name, arg->req.info.set_req.i_param_val.info.i_oct,
			       ent_info->parent_desc_name.length);
		}
		break;
	...
	case ncsAvmEntNetBoot_ID:
		{
			...
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				...
				ent_info->dhcp_serv_conf.net_boot = arg->req.info.set_req.i_param_val.info.i_int;

				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s: ncsAvmEntNetBoot_ID played back to: %d ",
					 ent_info->ep_str.name, ent_info->dhcp_serv_conf.net_boot);
			...
			if ((netboot_prev == NCS_SNMP_TRUE) && (ent_info->dhcp_serv_conf.net_boot == NCS_SNMP_FALSE))
	...
	case ncsAvmEntDHCPConfTFTPServerIp_ID:
		{
			...
			memcpy(ent_info->dhcp_serv_conf.tftp_serve_ip, arg->req.info.set_req.i_param_val.info.i_oct, 4);
	...
	case ncsAvmEntDHCPConfPrefLabel_ID:

		{
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				 ...
				 */
				if (!AVM_DHCONF_MEMCMP_LEN
				    (arg->req.info.set_req.i_param_val.info.i_oct,
				     ent_info->dhcp_serv_conf.label1.name.name,
				    ...
				    if (!AVM_DHCONF_MEMCMP_LEN
					(arg->req.info.set_req.i_param_val.info.i_oct,
					 ent_info->dhcp_serv_conf.label2.name.name,
					...
					snprintf(logbuf, sizeof(logbuf) - 1,
						 "AVM-SSU: Payload blade %s : ncsAvmEntDHCPConfPrefLabel_ID playback failed.. not matching to any label",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_ERROR);
					return NCSCC_RC_FAILURE;
				}
				m_AVM_SET_NAME(ent_info->dhcp_serv_conf.pref_label, arg->req.info.set_req.i_param_val);
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfPrefLabel_ID played back to: %s ",
				 ...
				 */
				if (!AVM_DHCONF_MEMCMP_LEN
				    (ent_info->dhcp_serv_conf.pss_curr_label.name,
				     ent_info->dhcp_serv_conf.label1.name.name,
				     ent_info->dhcp_serv_conf.label1.name.length,
				     ent_info->dhcp_serv_conf.pss_curr_label.length)) {
					...
					snprintf(logbuf, sizeof(logbuf) - 1,
						 "AVM-SSU: Payload blade %s : pss_curr_label:%s not matching to any label",
			...
			if (ent_info->dhcp_serv_conf.default_chg == TRUE) {
				...
				ckpt_dhconf = TRUE;
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s : ncsAvmEntDHCPConfPrefLabel SET to: %s",
					 ent_info->ep_str.name, ent_info->dhcp_serv_conf.pref_label.name);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
			}
		}
	...
	case ncsAvmEntDHCPConfLabel1Name_ID:
		{
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				/* If it is playback from PSS, just store it. Don't act on it  */
				m_AVM_SET_NAME(ent_info->dhcp_serv_conf.label1.name, arg->req.info.set_req.i_param_val);

				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfLabel1Name playedback to: %s ",
			...
			if (FALSE ==
			    avm_validate_state_for_dhcp_conf_change(ent_info, &ent_info->dhcp_serv_conf.label1,
								    &ent_info->dhcp_serv_conf.label2))
				goto failure;

			if ((0 != ent_info->dhcp_serv_conf.pref_label.length) &&
			    (!AVM_DHCONF_MEMCMP_LEN
			     (ent_info->dhcp_serv_conf.pref_label.name, ent_info->dhcp_serv_conf.label1.name.name,
			      ent_info->dhcp_serv_conf.label1.name.length,
				...
				m_AVM_SSU_PSSV_PUSH_STR(cb, ent_info->dhcp_serv_conf.pref_label.name,
			...
			ckpt_dhconf = TRUE;
			snprintf(logbuf, sizeof(logbuf) - 1,
				 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfLabel1Name SET to %s",
				 ent_info->ep_str.name, ent_info->dhcp_serv_conf.label1.name.name);
			m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
	...
	case ncsAvmEntDHCPConfFileNameLabel1_ID:
		{
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				...
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfFileNameLabel1 played back to: %s ",
			...
			if (NCSCC_RC_SUCCESS !=
			    avm_dhcp_file_validation(cb, ent_info, arg->req.info.set_req.i_param_val,
						     &ent_info->dhcp_serv_conf.label1, LABEL1)) {
			...
			/* Copy the file name */
			m_AVM_SET_NAME(ent_info->dhcp_serv_conf.label1.file_name, arg->req.info.set_req.i_param_val);
			ckpt_dhconf = TRUE;
			snprintf(logbuf, sizeof(logbuf) - 1,
				 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfFileNameLabel1 SET to %s",
				 ent_info->ep_str.name, ent_info->dhcp_serv_conf.label1.file_name.name);
			m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
	...
	case ncsAvmEntDHCPConfLabel2Name_ID:
		{
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				/* If it is playback from PSS, just store it. Don't act on it  */
				m_AVM_SET_NAME(ent_info->dhcp_serv_conf.label2.name, arg->req.info.set_req.i_param_val);

				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfLabel2Name played back to: %s ",
...
/* Should be reversed */
			if (FALSE ==
			    avm_validate_state_for_dhcp_conf_change(ent_info, &ent_info->dhcp_serv_conf.label2,
								    &ent_info->dhcp_serv_conf.label1))
				goto failure;

			if ((0 != ent_info->dhcp_serv_conf.pref_label.length) &&
			    (!AVM_DHCONF_MEMCMP_LEN
			     (ent_info->dhcp_serv_conf.pref_label.name, ent_info->dhcp_serv_conf.label2.name.name,
			      ent_info->dhcp_serv_conf.label2.name.length,
			      ent_info->dhcp_serv_conf.pref_label.length))) {
				memset(ent_info->dhcp_serv_conf.pref_label.name, '\0', AVM_NAME_STR_LENGTH);
				m_AVM_SET_NAME(ent_info->dhcp_serv_conf.pref_label, arg->req.info.set_req.i_param_val);
			...
			ckpt_dhconf = TRUE;
			snprintf(logbuf, sizeof(logbuf) - 1,
				 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfLabel2Name SET to %s",
				 ent_info->ep_str.name, ent_info->dhcp_serv_conf.label2.name.name);
			m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
	...
	case ncsAvmEntDHCPConfFileNameLabel2_ID:
		{
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				...
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfFileNameLabel2 played back to: %s ",
...
/* Should be reversed */
			...
			/* Copy the file name */
			m_AVM_SET_NAME(ent_info->dhcp_serv_conf.label2.file_name, arg->req.info.set_req.i_param_val);
			ckpt_dhconf = TRUE;
			snprintf(logbuf, sizeof(logbuf) - 1,
				 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfFileNameLabel2 SET to %s",
				 ent_info->ep_str.name, ent_info->dhcp_serv_conf.label2.file_name.name);
			m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
	...
	case ncsAvmEntDHCPConfLabel1Status_ID:
		{
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				...
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfLabel1Status_ID played back to: %d ",
					 ent_info->ep_str.name, ent_info->dhcp_serv_conf.label1.status);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
	...
	case ncsAvmEntDHCPConfLabel2Status_ID:
		{
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				...
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfLabel2Status_ID played back to: %d ",
					 ent_info->ep_str.name, ent_info->dhcp_serv_conf.label2.status);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
	...
	case ncsAvmEntDHCPConfInstallTimeLabel1_ID:
		{
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				/* If it is playback from PSS, just store it. Don't act on it  */
				memcpy(ent_info->dhcp_serv_conf.label1.install_time.install_time,
				       arg->req.info.set_req.i_param_val.info.i_oct, 8);
	...
	case ncsAvmEntDHCPConfLabel1SwVersion_ID:
		{
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				...
				ent_info->dhcp_serv_conf.label1.sw_version.length =
				    arg->req.info.set_req.i_param_val.i_length;
				memcpy(ent_info->dhcp_serv_conf.label1.sw_version.name,
				       arg->req.info.set_req.i_param_val.info.i_oct,
				       ent_info->dhcp_serv_conf.label1.sw_version.length);

				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfLabel1SwVersion_ID played back to: %s ",
					 ent_info->ep_str.name, ent_info->dhcp_serv_conf.label1.sw_version.name);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
	...
	case ncsAvmEntDHCPConfInstallTimeLabel2_ID:
		{
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				/* If it is playback from PSS, just store it. Don't act on it  */
				memcpy(ent_info->dhcp_serv_conf.label2.install_time.install_time,
				       arg->req.info.set_req.i_param_val.info.i_oct, 8);
	...
	case ncsAvmEntDHCPConfLabel2SwVersion_ID:
		{
			if (arg->i_policy & NCSMIB_POLICY_PSS_BELIEVE_ME) {
				...
				ent_info->dhcp_serv_conf.label2.sw_version.length =
				    arg->req.info.set_req.i_param_val.i_length;
				memcpy(ent_info->dhcp_serv_conf.label2.sw_version.name,
				       arg->req.info.set_req.i_param_val.info.i_oct,
				       ent_info->dhcp_serv_conf.label2.sw_version.length);

				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s: ncsAvmEntDHCPConfLabel2SwVersion_ID played back to: %s ",
					 ent_info->ep_str.name, ent_info->dhcp_serv_conf.label2.sw_version.name);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
	...
	case ncsAvmEntDHCPConfCurrActiveLabel_ID:
	    ...
	    /*****************************************************************************************
            ...
            *****************************************************************************************/
				memset(ent_info->dhcp_serv_conf.label2.file_name.name, '\0', AVM_NAME_STR_LENGTH);
				m_AVM_SET_NAME(ent_info->dhcp_serv_conf.pss_curr_label,
					       arg->req.info.set_req.i_param_val);
			}
		}
		break;
	...
	case ncsAvmEntProductName_ID:
		{
			...
			if (ent_info->valid_info) {
				...
				memcpy((uns8 *)buffer, ent_info->valid_info->inv_data.product_name.Data,
				       param->i_length);
				param->info.i_oct = (uns8 *)buffer;
			}
	...
	case ncsAvmEntProductVer_ID:
		{
			...
			if (AVM_VALID_INFO_NULL != ent_info->valid_info) {
				...
				memcpy((uns8 *)buffer, ent_info->valid_info->inv_data.product_version.Data,
				       param->i_length);
				param->info.i_oct = (uns8 *)buffer;
			}
		}
	...
	case ncsAvmEntHWState_ID:
		{
			...
			} else if (AVM_ENT_EXTRACT_REQ == ent_info->current_state) {
				param->info.i_int = 4;
			} else {
	...
	case ncsAvmEntDHCPConfCurrActiveLabel_ID:
		{
			...
			if (NULL != ent_info->dhcp_serv_conf.curr_act_label) {
				...
				memcpy((uns8 *)buffer, ent_info->dhcp_serv_conf.curr_act_label->name.name,
				       param->i_length);
			} else
				param->i_length = 0;
			param->info.i_oct = (uns8 *)buffer;
	...
	case ncsAvmEntDHCPConfInstallTimeLabel1_ID:
		{
			param->i_fmat_id = NCSMIB_FMAT_OCT;
			param->i_length = 8;
			memcpy((uns8 *)buffer, ent_info->dhcp_serv_conf.label1.install_time.install_time,
			       param->i_length);
			param->info.i_oct = (uns8 *)buffer;
		}
		break;
	...
	case ncsAvmEntDHCPConfInstallTimeLabel2_ID:
		{
			param->i_fmat_id = NCSMIB_FMAT_OCT;
			param->i_length = 8;
			memcpy((uns8 *)buffer, ent_info->dhcp_serv_conf.label2.install_time.install_time,
			       param->i_length);
			param->info.i_oct = (uns8 *)buffer;
		}
		break;
	...
	case ncsAvmEntHelperPayload_ID:
		{
			...
			if (NCSCC_RC_SUCCESS !=
			    avm_convert_nodeid_to_entpath(arg->req.info.set_req.i_param_val, &helper_ep)) {
				m_AVM_LOG_DEBUG("AVM-SSU: not a valid node_id", NCSFL_SEV_ERROR);
				goto failure;
			}

			if ((ent_info->ep_str.length == helper_ep.length)
			    && (!memcmp(ent_info->ep_str.name, helper_ep.name, helper_ep.length))) {
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s : Helper blade same as target payload blade. IPMC upgrade failed",
					 ent_info->ep_str.name);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_ERROR);
				return NCSCC_RC_INV_VAL;
			}
			...
			m_AVM_SET_NAME(ent_info->dhcp_serv_conf.ipmc_helper_node, arg->req.info.set_req.i_param_val);

			snprintf(logbuf, sizeof(logbuf) - 1,
				 "AVM-SSU: Payload blade %s: ncsAvmEntHelperPayload_ID SET to %s",
	...
	case ncsAvmEntAdminOper_ID:
		{
			...
			if (AVM_DHCP_COMMIT == arg->req.info.set_req.i_param_val.info.i_int) {
				if (SSU_COMMIT_PENDING == ent_info->dhcp_serv_conf.label1.status) {
					...
					ckpt_dhstate = TRUE;
					snprintf(logbuf, sizeof(logbuf) - 1,
						 "AVM-SSU: Payload blade %s: %s's state SSU_COMMITTED",
						 ent_info->ep_str.name, ent_info->dhcp_serv_conf.label1.name.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
					...
					snprintf(logbuf, sizeof(logbuf) - 1,
						 "AVM-SSU: Payload blade %s: Can't COMMIT the LABEL, it is not in COMMIT_PENDING State",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_ERROR);
					goto failure;
				}
				...
				ent_info->dhcp_serv_conf.pref_label.length =
				    ent_info->dhcp_serv_conf.default_label->name.length;
				memcpy(ent_info->dhcp_serv_conf.pref_label.name,
				       ent_info->dhcp_serv_conf.default_label->name.name,
				       ent_info->dhcp_serv_conf.pref_label.length);
				...
				if ((ent_info->dhcp_serv_conf.ipmc_upgd_state != 0)
				    || (ent_info->dhcp_serv_conf.upgd_prgs == TRUE)) {
					/* Upgrade is already in progress */
					snprintf(logbuf, sizeof(logbuf) - 1,
						 "AVM-SSU: Payload %s: Upgrade is already in progress. Upgrade not allowed",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
					goto failure;
				}
				if (ent_info->dhcp_serv_conf.curr_act_label == NULL) {
					snprintf(logbuf, sizeof(logbuf) - 1,
						 "AVM-SSU: Payload blade %s: No current active label. Upgrade not allowed",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
					goto failure;
				}
				...
				if ((IPMC == ent_info->dhcp_serv_conf.upgrade_type)
				    || (INTEG == ent_info->dhcp_serv_conf.upgrade_type)) {
					flag = 0;
					if (avm_check_config(ent_info->ep_str.name, &flag) == NCSCC_RC_FAILURE) {
						goto failure;
					} else {
				...
				if ((IPMC == ent_info->dhcp_serv_conf.upgrade_type)) {
					...
					if (rc == NCSCC_RC_FAILURE) {
						snprintf(logbuf, sizeof(logbuf) - 1,
							 "AVM-SSU: Payload %s: IPMC upgrade trigger failed",
							 ent_info->ep_str.name);
						m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_ERROR);
					...
					if ((ent_info->dhcp_serv_conf.pref_label.length ==
					    ...
					    (!memcmp(ent_info->dhcp_serv_conf.pref_label.name,
						     ...
						     ent_info->dhcp_serv_conf.pref_label.length))) {
						snprintf(logbuf, sizeof(logbuf) - 1,
							 "AVM-SSU: Payload blade %s: Preferred label is the current active label. No Upgrade",
							 ent_info->ep_str.name);
						m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
					} else
					    if ((ent_info->dhcp_serv_conf.pref_label.length ==
						 ent_info->dhcp_serv_conf.curr_act_label->other_label->name.length)
						&&
						(!memcmp
						...
						if (rc == NCSCC_RC_FAILURE) {
							snprintf(logbuf, sizeof(logbuf) - 1,
								 "AVM-SSU: Payload %s: INTEG upgrade trigger failed",
								 ent_info->ep_str.name);
							m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_ERROR);
						...
						snprintf(logbuf, sizeof(logbuf) - 1,
							 "AVM-SSU: Payload blade %s: Not a valid Preferred label.",
							 ent_info->ep_str.name);
						m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_ERROR);
						goto failure;
					}
					...
					snprintf(logbuf, sizeof(logbuf) - 1,
						 "AVM-SSU: Payload blade %s: Invalid Operation", ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_ERROR);
					goto failure;
				}
			} else {
