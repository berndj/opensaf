...
avm_dhcp_file_validation(AVM_CB_T *cb, AVM_ENT_INFO_T *ent_info,
			 NCSMIB_PARAM_VAL param_val, AVM_PER_LABEL_CONF *label, AVM_LABEL_NUM label_no)
{
	AVM_DHCP_CONF_NAME_TYPE file;
	char *tmp_str, ver_str[AVM_NAME_STR_LENGTH];
	char file_name[AVM_NAME_STR_LENGTH + AVM_DHCPD_SW_IPMC_PLD_BLD_FILE_LEN];
	FILE *fp;
	...
	snprintf(file_name, AVM_NAME_STR_LENGTH - 1, "/%s", file.name);
	...
	memset(&fileStat, '\0', sizeof(struct stat));
	strncat(file_name, AVM_DHCPD_SW_PXE_FILE, AVM_DHCPD_SW_PXE_FILE_LEN);
	if (-1 == stat(file_name, &fileStat)) {
		perror(file_name);
		return NCSCC_RC_FAILURE;
	}
	...
	label->install_time.dttm.year = m_NCS_OS_HTONS(label->install_time.dttm.year);

	tmp_str = strrchr(file_name, '/');
	*(++tmp_str) = '\0';

	/* read version file */
	strncat(file_name, AVM_DHCPD_SW_VER_FILE, AVM_DHCPD_SW_VER_FILE_LEN);
	...
	*(++tmp_str) = '\0';
	...
	strncat(file_name, AVM_DHCPD_SW_IPMC_PLD_BLD_FILE, AVM_DHCPD_SW_IPMC_PLD_BLD_FILE_LEN);
	if (-1 == access(file_name, F_OK)) {
		snprintf(str, sizeof(str) - 1,
			 "AVM-SSU: Payload blade %s:AVM_DHCPD_SW_IPMC_PLD_BLD_FILE is missing in the file path",
	...
	*(++tmp_str) = '\0';
	...
	strncat(file_name, AVM_DHCPD_SW_IPMC_PLD_RTM_FILE1, AVM_DHCPD_SW_IPMC_PLD_RTM_FILE1_LEN);
	if (-1 == access(file_name, F_OK)) {
		snprintf(str, sizeof(str) - 1,
			 "AVM-SSU: Payload blade %s:AVM_DHCPD_SW_IPMC_PLD_RTM_FILE1 is missing in the file path",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_WARNING);
	}
	...
	*(++tmp_str) = '\0';
	...
	strncat(file_name, AVM_DHCPD_SW_IPMC_PLD_RTM_FILE2, AVM_DHCPD_SW_IPMC_PLD_RTM_FILE2_LEN);
	if (-1 == access(file_name, F_OK)) {
		snprintf(str, sizeof(str) - 1,
			 "AVM-SSU: Payload blade %s:AVM_DHCPD_SW_IPMC_PLD_RTM_FILE2 is missing in the file path",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_WARNING);
	}
...
extern uns32
avm_set_preferred_label(AVM_CB_T *cb, AVM_ENT_INFO_T *ent_info, AVM_ENT_DHCP_CONF *dhcp_conf,
			NCSMIB_PARAM_VAL param_val)
{
	...
	if (!AVM_DHCONF_MEMCMP_LEN(param_val.info.i_oct, dhcp_conf->label1.name.name,
		...
		if ((SSU_NO_CONFIG == dhcp_conf->label1.status) || (SSU_COMMIT_PENDING == dhcp_conf->label1.status))
			return NCSCC_RC_FAILURE;
		else {
		...
		dhcp_conf->pref_label.length = param_val.i_length;
		memcpy(dhcp_conf->pref_label.name, param_val.info.i_oct, param_val.i_length);
		/* Push the preferred label into pssv */
		m_AVM_SSU_PSSV_PUSH_STR(cb, dhcp_conf->pref_label.name, ncsAvmEntDHCPConfPrefLabel_ID, ent_info,
		...
		m_AVM_SSU_PSSV_PUSH_INT(cb, ent_info->dhcp_serv_conf.label1.status, ncsAvmEntDHCPConfLabel1Status_ID,
					ent_info);
		...
		if ((SSU_NO_CONFIG == dhcp_conf->label2.status) || (SSU_COMMIT_PENDING == dhcp_conf->label2.status))
			return NCSCC_RC_FAILURE;
		else {
		...
		m_AVM_SSU_PSSV_PUSH_INT(cb, ent_info->dhcp_serv_conf.label2.status, ncsAvmEntDHCPConfLabel2Status_ID,
					ent_info);
	} else
		return NCSCC_RC_FAILURE;
 ...
 *****/
extern void
avm_set_label_state_to_install(AVM_CB_T *cb, AVM_ENT_INFO_T *ent_info, AVM_ENT_DHCP_CONF *dhcp_conf,
			       AVM_PER_LABEL_CONF *label_cnf, AVM_LABEL_NUM label_no)
{
...
avm_ssu_dhconf_set(AVM_CB_T *avm_cb, AVM_ENT_DHCP_CONF *dhcp_conf, AVM_PER_LABEL_CONF *dhcp_label, uns8 dhcp_restart)
{
	...
	uns32 rc;
	char pxe_file_name[AVM_NAME_STR_LENGTH + AVM_DHCPD_SW_PXE_FILE_LEN];
	char *pStrstr = NULL;
	...
	snprintf(pxe_file_name, AVM_NAME_STR_LENGTH - 1, "%s", dhcp_label->file_name.name);
	...
	if (pStrstr == NULL) {
		strncat(pxe_file_name, AVM_DHCPD_SW_PXE_FILE, AVM_DHCPD_SW_PXE_FILE_LEN);
	}

	filename = pxe_file_name;

	snprintf(tftpserver, AVM_IP_ADDR_STR_LEN - 1, "%d.%d.%d.%d", dhcp_conf->tftp_serve_ip[0] & 0xff,
		 dhcp_conf->tftp_serve_ip[1] & 0xff,
		 dhcp_conf->tftp_serve_ip[2] & 0xff, dhcp_conf->tftp_serve_ip[3] & 0xff);

	snprintf(script_buf, AVM_SCRIPT_BUF_LEN - 1,
		 "%s %s %s %d %02x:%02x:%02x:%02x:%02x:%02x %02x:%02x:%02x:%02x:%02x:%02x %s %s %d ",
		 AVM_SSU_DHCONF_SCRIPT, host1, host2, dhcp_conf->net_boot, mac1[0], mac1[1], mac1[2], mac1[3], mac1[4],
		 mac1[5], mac2[0], mac2[1], mac2[2], mac2[3], mac2[4], mac2[5], filename, tftpserver, dhcp_restart);

	snprintf(log_str, sizeof(log_str) - 1, "AVM-SSU: %s %s %s ", "DHCP script buffer = ", script_buf, "\n");
	...
	if (rc) {
		...
		if (1 == ret_val) {
			...
			m_AVM_LOG_DEBUG
			    ("AVM-SSU: execution of AVM_SSU_DHCONF_SCRIPT failed because of dhcp stop failed",
			     NCSFL_SEV_NOTICE);
			m_AVM_LOG_DEBUG("AVM-SSU: DHCP_FAIL_TMR started", NCSFL_SEV_NOTICE);
		} else {
			m_AVM_LOG_DEBUG
			    ("AVM-SSU: execution of AVM_SSU_DHCONF_SCRIPT failed due to some unknown reason. DEBUG IT",
			     NCSFL_SEV_ERROR);
		}
		return NCSCC_RC_FAILURE;
	} else {
...
extern uns32 avm_ssu_dhconf(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info, void *fsm_evt, uns8 dhcp_restart)
{
	...
	if (AVM_ENT_INFO_NULL == ent_info) {
		m_AVM_LOG_GEN("AVM-SSU: avm_ssu_dhconf: No Entity existing ", ent_info->depends_on.Entry,
    ...
    ********************************************************************************************/
	if ((dhcp_conf->upgrade_type != INTEG) && (dhcp_conf->ipmc_upgd_state == IPMC_UPGD_DONE)) {
		snprintf(logbuf, sizeof(logbuf) - 1,
			 "AVM-SSU: Payload blade %s : dhconf set is skipped, as the IPMC upgrade is in progress",
			 ent_info->ep_str.name);
	...
	if ((dhcp_conf->net_boot == NCS_SNMP_FALSE) || (dhcp_conf->default_label == NULL)) {
		...
		if (avm_ssu_clear_mac(avm_cb, ent_info)) {
			...
			m_AVM_LOG_DEBUG
			    ("AVM-SSU: Dummy label passed to avm_ssu_dhconf_set to remove the mac address from the dhcpd.conf file \n",
			     NCSFL_SEV_NOTICE);
	...
	if ((FALSE == dhcp_conf->default_chg) &&
	    (FALSE == dhcp_conf->default_label->conf_chg) && (0 == access(AVM_DHCPD_CONF, F_OK))) {
		...
		case SSU_INSTALLED:
			...
			if ((dhcp_conf->default_label->other_label->status == SSU_NO_CONFIG) ||
				...
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s : CurrActiveLabel SET to:%s", ent_info->ep_str.name,
					 dhcp_conf->curr_act_label->name.name);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
				/* Push the current active label into pssv */
				m_AVM_SSU_PSSV_PUSH_STR(avm_cb, dhcp_conf->curr_act_label->name.name,
							ncsAvmEntDHCPConfCurrActiveLabel_ID, ent_info,
							dhcp_conf->curr_act_label->name.length);
			...
			if (dhcp_conf->default_label->other_label->status == SSU_COMMITTED) {
				...
				memcpy(dhcp_conf->pref_label.name, dhcp_conf->default_label->name.name,
				       dhcp_conf->pref_label.length);
				/* Push the preferred label into pssv */
				m_AVM_SSU_PSSV_PUSH_STR(avm_cb, dhcp_conf->pref_label.name,
							ncsAvmEntDHCPConfPrefLabel_ID, ent_info,
							dhcp_conf->pref_label.length);
				dhcp_conf->default_chg = TRUE;
				avm_send_boot_upgd_trap(avm_cb, ent_info, ncsAvmCurrActiveLabelChange_ID);
				snprintf(logbuf, sizeof(logbuf) - 1,
					 "AVM-SSU: Payload blade %s : CurrActiveLabel SET to : %s",
					 ent_info->ep_str.name, dhcp_conf->curr_act_label->name.name);
				m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_NOTICE);
				/* Push the current active label into pssv */
				m_AVM_SSU_PSSV_PUSH_STR(avm_cb, dhcp_conf->curr_act_label->name.name,
							ncsAvmEntDHCPConfCurrActiveLabel_ID, ent_info,
		...
		case SSU_COMMITTED:
			...
			if (dhcp_conf->default_label->other_label->status == SSU_COMMIT_PENDING) {
				...
				if (memcmp
				    (dhcp_conf->curr_act_label->name.name, dhcp_conf->default_label->name.name,
	...
	if (hpi_evt != NULL) {
		...
		if ((conf_chg == FALSE) &&
		    (!memcmp
		     (dhcp_conf->mac_address[0], hpi_evt->inv_data.oem_inv_data.interface_mac_addr[0],
		      AVM_MAC_DATA_LEN))
		    &&
		    (!memcmp
		     (dhcp_conf->mac_address[1], hpi_evt->inv_data.oem_inv_data.interface_mac_addr[1],
		      AVM_MAC_DATA_LEN))) {
	...
	if (rc == AVM_DHCP_CONF_CHANGE) {
		...
		snprintf(logbuf, sizeof(logbuf) - 1,
			 "AVM-SSU: DHCP CONF file updation FAILED, rc =%d, dhcp_conf->default_chg =%d, dhcp_conf->default_label->conf_chg =%d \n",
...
extern uns32 avm_ssu_dhcp_tmr_reconfig(AVM_CB_T *avm_cb, AVM_EVT_T *hpi_evt)
{
	...
	if (hpi_evt->evt.hpi_evt->hpi_event.EventDataUnion.HotSwapEvent.HotSwapState !=
	...
	if (AVM_ENT_INFO_NULL == ent_info) {
		m_AVM_LOG_GEN("AVM-SSU: No Entity in the DB with Entity Path ", hpi_evt->evt.hpi_evt->entity_path.Entry,
			      sizeof(SaHpiEntityPathT), NCSFL_SEV_WARNING);
		return NCSCC_RC_FAILURE;
	}
...
/*****************************************************************************
  ...
  NOTES         : None
*****************************************************************************/
extern void avm_ssu_dhcp_rollback(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info)
{
	...
	if ((dhcp_conf->curr_act_label->status == SSU_COMMIT_PENDING) &&
		...
		if (dhcp_conf->default_label == &dhcp_conf->label1) {
			...
			memcpy(dhcp_conf->pref_label.name, dhcp_conf->label1.name.name, dhcp_conf->label1.name.length);
			/* Push the preferred label into pssv */
			m_AVM_SSU_PSSV_PUSH_STR(avm_cb, dhcp_conf->pref_label.name, ncsAvmEntDHCPConfPrefLabel_ID,
						ent_info, dhcp_conf->pref_label.length);
			...
			memcpy(dhcp_conf->pref_label.name, dhcp_conf->label2.name.name, dhcp_conf->label2.name.length);
			/* Push the preferred label into pssv */
			m_AVM_SSU_PSSV_PUSH_STR(avm_cb, dhcp_conf->pref_label.name, ncsAvmEntDHCPConfPrefLabel_ID,
						ent_info, dhcp_conf->pref_label.length);
...
/*****************************************************************************
  ...
  NOTES         : None
*****************************************************************************/
extern void avm_ssu_dhcp_integ_rollback(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info)
{
	AVM_ENT_DHCP_CONF *dhcp_conf = &ent_info->dhcp_serv_conf;
	uns8 str[AVM_LOG_STR_MAX_LEN];
	AVM_EVT_T fsm_evt;
	...
	if (dhcp_conf->upgrade_type == INTEG) {
		...
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1, "AVM-SSU: Payload blade %s: INTEG: Rollback triggered ",
			 ent_info->ep_str.name);
		...
		if ((dhcp_conf->pld_bld_ipmc_status) || (dhcp_conf->pld_rtm_ipmc_status)) {
			...
			snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
				 "AVM-SSU: Payload blade %s: INTEG: IPMC Rollback triggered ", ent_info->ep_str.name);
			m_AVM_LOG_DEBUG(str, NCSFL_SEV_NOTICE);
			...
			snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
				 "AVM-SSU: Payload blade %s: INTEG: SW And BIOS Rollback triggered ",
				 ent_info->ep_str.name);
			m_AVM_LOG_DEBUG(str, NCSFL_SEV_NOTICE);
		...
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1, "AVM-SSU: Payload blade %s: IPMC Rollback triggered ",
			 ent_info->ep_str.name);
		...
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1, "AVM-SSU: Payload blade %s: SW_BIOS Rollback triggered ",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_NOTICE);
		avm_send_boot_upgd_trap(avm_cb, ent_info, ncsAvmUpgradeProgress_ID);
	}
...
/*****************************************************************************
  ...
  NOTES         : None
*****************************************************************************/
static uns32 avm_ssu_clear_mac(AVM_CB_T *cb, AVM_ENT_INFO_T *ent_info_in)
{
	...
	for (ent_info = avm_find_next_ent_str_info(cb, &ep);
	     ent_info != AVM_ENT_INFO_NULL; ent_info = (AVM_ENT_INFO_T *)avm_find_next_ent_str_info(cb, &ep)) {
		mac_change = 0;
...
extern uns32 extract_slot_shelf_subslot(uns8 *node_name, struct slot_info *slot_info)
{
	uns8 *param;
	uns8 *str = node_name;
...
extern uns32 avm_prepare_entity_path(uns8 *str, const struct slot_info sInfo)
{
	uns32 no_entries = 0;
	uns32 i = 0;
	uns32 len = 0;
...
extern uns32 avm_convert_nodeid_to_entpath(NCSMIB_PARAM_VAL param_val, AVM_ENT_PATH_STR_T *ep)
{
	uns32 rc = NCSCC_RC_FAILURE;
...
extern uns32 avm_compute_ipmb_address(AVM_ENT_INFO_T *ent_info)
{
	...
	if (ent_info->ep_str.name[4] == '1') {
		if (ent_info->ep_str.name[5] == '}')
			...
			snprintf(str, sizeof(str) - 1, "AVM-SSU: Payload blade %s : Unknown physical slot number ",
				 ent_info->ep_str.name);
			m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
			return NCSCC_RC_FAILURE;
		}
	} else if ((ent_info->ep_str.name[4] >= '2' && ent_info->ep_str.name[4] <= '9')
		   && (ent_info->ep_str.name[5] == '}')) {
		physical_slot_id = ent_info->ep_str.name[4] - '0';
	} else {
		snprintf(str, sizeof(str) - 1, "AVM-SSU: Payload blade %s : Unknown physical slot number ",
			 ent_info->ep_str.name);
	...
	if (!memcmp(chassis_type, AVM_CHASSIS_TYPE_1405, len)) {
		if (physical_slot_id < 7)
			...
			snprintf(str, sizeof(str) - 1, "AVM-SSU: Blade %s : is not a payload blade",
				 ent_info->ep_str.name);
			m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
			return NCSCC_RC_FAILURE;
		}
		...
		snprintf(str, sizeof(str) - 1, "AVM-SSU: Payload blade %s : Unknown chassis type ",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
		return NCSCC_RC_FAILURE;
	}
...
extern uns32 avm_upgrade_ipmc_trigger(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info)
{
	NCSMIB_ARG ssu_dummy_mib_obj;
	AVM_EVT_T fsm_evt;
	uns32 rc = NCSCC_RC_FAILURE;
	...
	if (((ent_info->current_state != AVM_ENT_INACTIVE) && (ent_info->current_state != AVM_ENT_ACTIVE)) &&
		...
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
			 "AVM-SSU: Payload blade %s :Target Payload State Not Good. IPMC upgrade failed",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
		...
		flag_upgrade_sanity = NCSCC_RC_FAILURE;
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
			 "AVM-SSU: Payload blade %s : Helper blade not present. IPMC upgrade failed",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
	}
	/* Sanity test failed */
	if (flag_upgrade_sanity == NCSCC_RC_FAILURE) {
		...
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1, "AVM-SSU: Payload blade %s : IPMC upgrade sanity check failed",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
		return NCSCC_RC_FAILURE;
	}
	...
	if ((ent_info->adm_lock == AVM_ADM_LOCK) && (ent_info->current_state == AVM_ENT_INACTIVE)) {
		...
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1, "AVM-SSU: Payload blade %s : Blade Already In Lock State",
			 ent_info->ep_str.name);
		...
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1, "AVM-SSU: Payload blade %s : Lock the blade",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_DEBUG);
...
extern uns32 avm_upgrade_ipmc(AVM_CB_T *avm_cb, AVM_ENT_INFO_T *ent_info)
{
	uns32 rc = NCSCC_RC_FAILURE;
	uns8 str[AVM_LOG_STR_MAX_LEN];
   ...
   *************************************************************************************************/
	...
	if (INTEG == ent_info->dhcp_serv_conf.upgrade_type) {
		if (ent_info->dhcp_serv_conf.pld_bld_ipmc_status && ent_info->dhcp_serv_conf.pld_rtm_ipmc_status) {
			...
			snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
				 "AVM-SSU: Payload blade %s : INTEG : IPMC for PAYLOAD AND RTM requires ROLLBACK",
				 ent_info->ep_str.name);
			m_AVM_LOG_DEBUG(str, NCSFL_SEV_DEBUG);
			...
			snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
				 "AVM-SSU: Payload blade %s : INTEG : IPMC for PAYLOAD requires ROLLBACK",
				 ent_info->ep_str.name);
			m_AVM_LOG_DEBUG(str, NCSFL_SEV_DEBUG);
			...
			snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
				 "AVM-SSU: Payload blade %s : INTEG : IPMC for PAYLOAD RTM requires ROLLBACK",
				 ent_info->ep_str.name);
			m_AVM_LOG_DEBUG(str, NCSFL_SEV_DEBUG);
			...
			snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
				 "AVM-SSU: Payload blade %s : INTEG : IPMC Upgrade In Prg", ent_info->ep_str.name);
			m_AVM_LOG_DEBUG(str, NCSFL_SEV_DEBUG);
			...
			if (rc != NCSCC_RC_SUCCESS) {
				...
				snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
					 "AVM-SSU: Payload blade %s : INTEG : IPMC Upgrade FUND Mib Set Failed ",
					 ent_info->ep_str.name);
				m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
		...
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1, "AVM-SSU: Payload blade %s : PAYLOAD IPMC Upgrade In PRG",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_DEBUG);
		...
		if (rc != NCSCC_RC_SUCCESS) {
			snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
				 "AVM-SSU: Payload blade %s : IPMC Upgrade FUND Mib Set Failed ",
				 ent_info->ep_str.name);
			m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
...
extern NCS_BOOL avm_is_the_helper_payload_present(AVM_CB_T *avm_cb, AVM_ENT_PATH_STR_T helper_ent_path)
{
...
extern NCS_BOOL
avm_validate_state_for_dhcp_conf_change(AVM_ENT_INFO_T *ent_info, AVM_PER_LABEL_CONF *self_label,
					AVM_PER_LABEL_CONF *other_label)
{
	uns8 str[AVM_LOG_STR_MAX_LEN];
	...
	if (self_label == ent_info->dhcp_serv_conf.curr_act_label) {
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
			 "AVM-SSU: Payload blade %s : DHCP Configuration can't be changed for the current active label",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
		return FALSE;
	} else if (other_label->status == SSU_COMMIT_PENDING) {
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
			 "AVM-SSU: Payload blade %s : DHCP Configuration can't be changed, when the other label is in COMMIT PENDING state",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
		return FALSE;
	} else if (ent_info->dhcp_serv_conf.upgd_prgs == TRUE) {
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
			 "AVM-SSU: Payload blade %s : Upgrade is in progress, DHCP Configuration can't be changed",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
		return FALSE;
	} else if (ent_info->dhcp_serv_conf.ipmc_upgd_state) {
		snprintf(str, AVM_LOG_STR_MAX_LEN - 1,
			 "AVM-SSU: Payload blade %s : IPMC Upgrade is in progress, DHCP Configuration can't be changed",
			 ent_info->ep_str.name);
		m_AVM_LOG_DEBUG(str, NCSFL_SEV_ERROR);
		return FALSE;
	} else
...
void avm_role_change_check_pld_upgd_prg(AVM_CB_T *avm_cb)
{
	AVM_ENT_INFO_T *ent_info;
	SaHpiEntityPathT entity_path;
	...
	for (ent_info = avm_find_next_ent_info(avm_cb, &entity_path);
		...
		if (ent_info->dhcp_serv_conf.upgd_prgs == TRUE) {
			snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
				 "StandbyToActive: Payload blade %s : Upgrade Timer Start ", ent_info->ep_str.name);
			...
			case IPMC_UPGD_TRIGGERED:
				{
					snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
						 "StandbyToActive: Payload blade %s : IPMC State=IPMC_UPGD_TRIGGERED",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);
					...
					if (rc != NCSCC_RC_SUCCESS) {
						if (IPMC == ent_info->dhcp_serv_conf.upgrade_type) {
							snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
								 "StandbyToActive: Payload blade %s : IPMC IPMC_UPGD_TRIGGERED Failed",
								 ent_info->ep_str.name);
							m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);

							ent_info->dhcp_serv_conf.ipmc_upgd_state = 0;
							m_AVM_SEND_CKPT_UPDT_SYNC_UPDT(avm_cb, ent_info,
										       AVM_CKPT_ENT_UPGD_STATE_CHG);
						} else if (INTEG == ent_info->dhcp_serv_conf.upgrade_type) {
							snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
								 "StandbyToActive: Payload blade %s : INTEG IPMC_UPGD_TRIGGERED Failed",
								 ent_info->ep_str.name);
							m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);
							...
							ent_info->dhcp_serv_conf.curr_act_label->other_label->status =
							    SSU_UPGD_FAILED;
							m_AVM_SSU_PSSV_PUSH_INT(avm_cb,
										ent_info->dhcp_serv_conf.label1.status,
										ncsAvmEntDHCPConfLabel1Status_ID,
										ent_info);
							m_AVM_SSU_PSSV_PUSH_INT(avm_cb,
										ent_info->dhcp_serv_conf.label2.status,
										ncsAvmEntDHCPConfLabel2Status_ID,
										ent_info);
							...
							m_AVM_SSU_PSSV_PUSH_STR(avm_cb,
										ent_info->dhcp_serv_conf.pref_label.
										name, ncsAvmEntDHCPConfPrefLabel_ID,
										ent_info,
										ent_info->dhcp_serv_conf.pref_label.
										length);

							dhcp_conf->default_chg = FALSE;
							m_AVM_SEND_CKPT_UPDT_ASYNC_UPDT(avm_cb, ent_info,
			...
			case IPMC_BLD_LOCKED:
				{
					...
					snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
						 "StandbyToActive: Payload blade %s : IPMC State=IPMC_BLD_LOCKED",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);

					if ((ent_info->adm_lock == AVM_ADM_LOCK)
					    && (ent_info->current_state == AVM_ENT_INACTIVE)) {
						/* Entity is already in required state */
						snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
							 "StandbyToActive: Payload blade %s : Blade Already Locked",
							 ent_info->ep_str.name);
						m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);
						...
						snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
							 "StandbyToActive: Payload blade %s : Blade Lock Invoked",
							 ent_info->ep_str.name);
						m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);
						fsm_evt.evt.mib_req = &ssu_dummy_mib_obj;
						fsm_evt.fsm_evt_type = AVM_ADM_LOCK + AVM_EVT_ADM_OP_BASE - 1;
	       ...
	       /*************************************************************************
               ...
               *************************************************************************/
					...
					snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
						 "StandbyToActive: Payload blade %s : IPMC State=IPMC_UPGD_IN_PRG",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);
			...
			case IPMC_ROLLBACK_IN_PRG:
				{
					...
					snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
						 "StandbyToActive: Payload blade %s : IPMC State=IPMC_ROLLBACK_IN_PRG",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);
			...
			case IPMC_PLD_BLD_ROLLBACK_IN_PRG:
				{
					...
					snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
						 "StandbyToActive: Payload blade %s : IPMC State=IPMC_PLD_BLD_ROLLBACK_IN_PRG",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);

					m_AVM_ROLE_CHG_WAIT_TMR_START(avm_cb, ent_info,
								      AVM_UPGD_IPMC_PLD_MOD_TMR_INTVL);
				}
				break;
			...
			case IPMC_PLD_RTM_ROLLBACK_IN_PRG:
				{
					...
					snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
						 "StandbyToActive: Payload blade %s : IPMC State=IPMC_PLD_RTM_ROLLBACK_IN_PRG",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);
			...
			case IPMC_UPGD_DONE:
				{
					...
					snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
						 "StandbyToActive: Payload blade %s : IPMC State=IPMC_UPGD_DONE",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);
		...
		if (ent_info->dhcp_serv_conf.bios_upgd_state) {
			...
			if (NCSCC_RC_SUCCESS != rc) {
				m_AVM_LOG_GEN("Cant convert the entity path to string:", ent_info->entity_path.Entry,
					      sizeof(SaHpiEntityPathT), NCSFL_SEV_ERROR);
				return;
			}
			...
			case BIOS_TMR_STARTED:
				{
					snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
						 "StandbyToActive: Payload blade %s : BIOS State=BIOS_TMR_STARTED",
						 ent_info->ep_str.name);
					m_AVM_LOG_DEBUG(logbuf, NCSFL_SEV_DEBUG);
			...
			case BIOS_EXP_BANK_1:
				{
					snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
						 "StandbyToActive:: Payload blade %s : BIOS State = :%d",
			...
			default:
				{
					snprintf(logbuf, AVM_LOG_STR_MAX_LEN - 1,
						 "StandbyToActive:: Payload blade %s : Invalid bios_upgd_state:%d",
						 ent_info->ep_str.name, ent_info->dhcp_serv_conf.bios_upgd_state);
