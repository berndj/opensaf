...
static void avm_chan_open_callback(SaInvocationT inv, SaEvtChannelHandleT chan_hdl, SaAisErrorT rc);

static void avm_evt_callback(SaEvtSubscriptionIdT sub_id, SaEvtEventHandleT event_hdl, SaSizeT evt_data_size);
static uns32 avm_alloc_pattern_array(SaEvtEventPatternArrayT **pattern_array, uns32 num_patterns, uns32 pattern_size);

static uns32 avm_free_pattern_array(SaEvtEventPatternArrayT *pattern_array);
 ...
 ***********************************************************************
 *****/
uns32 avm_eda_initialize(AVM_CB_T *cb)
{
...
static void avm_evt_callback(SaEvtSubscriptionIdT sub_id, SaEvtEventHandleT event_hdl, SaSizeT evt_data_size)
{
	...
	if ((rc =
	     hpl_decode_hisv_evt((HPI_HISV_EVT_T *)hpi_event, p_data, data_len,
				 (HISV_SW_VERSION | HISV_EDS_INF_VERSION))) != NCSCC_RC_SUCCESS) {
		m_AVM_LOG_INVALID_VAL_ERROR(rc);
		/* Free the event */
		rc = saEvtEventFree(event_hdl);
	...
	if (AVM_CB_NULL == avm_cb) {
		...
		if ((avm_cb->ha_state == SA_AMF_HA_STANDBY) || (avm_cb->ha_state == SA_AMF_HA_QUIESCING)
		    || (avm_cb->ha_state == SA_AMF_HA_QUIESCED) || (TRUE == avm_cb->adm_switch)) {
			   ...
			   Expiry of this timer. */
			{
				...
				if (ent_info) {
					...
					if ((fsm_evt_type == AVM_EVT_INSERTION_PENDING)
					    || (fsm_evt_type == AVM_EVT_SENSOR_FW_PROGRESS)) {
						avm_standby_boot_succ_tmr_handler(avm_cb, avm_evt, ent_info,
										  fsm_evt_type);
					}
				}
			}	/* End of 91104 fix */
