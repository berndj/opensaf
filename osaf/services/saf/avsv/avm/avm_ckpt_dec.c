...
static uns32 avm_decode_cold_sync_rsp_validation_info(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);
static uns32 avm_decode_cold_sync_rsp_ent_cfg(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);
static uns32 avm_decode_cold_sync_rsp_async_updt_cnt(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);

static uns32 avm_decode_ckpt_ent_cfg(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);

static uns32 avm_decode_ckpt_ent_dhcp_conf_chg(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);

static uns32 avm_decode_ckpt_ent_dhcp_state_chg(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);

static uns32 avm_decode_ckpt_ent_state(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);

static uns32 avm_decode_ckpt_ent_state_sensor(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);

static uns32 avm_decode_ckpt_ent_add_dependent(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);
static uns32 avm_decode_ckpt_ent_adm_op(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);

static uns32 avm_decode_ckpt_evt_id(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);

static uns32 avm_decode_ckpt_dummy(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);

static uns32 avm_decode_cold_dummy(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);

static uns32 avm_decode_ckpt_ent_upgd_state_chg(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec);
...
const AVM_DECODE_CKPT_DATA_FUNC_PTR avm_dec_ckpt_data_func_list[AVM_CKPT_MSG_MAX] = {
...
const AVM_DECODE_COLD_SYNC_RSP_DATA_FUNC_PTR avm_dec_cold_sync_rsp_data_func_list[] = {
	...
	case NCS_MBCSV_ACT_RMV:
		{
			/* Send Key */
			status =
			    ncs_edu_exec(&cb->edu_hdl, avm_edp_ckpt_msg_ent, &dec->i_uba, EDP_OP_TYPE_DEC,
					 (AVM_ENT_INFO_T **)&ent_info, &ederror, 1, 2);
		}
		break;
...
static uns32 avm_decode_ckpt_ent_state(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec)
{
	...
	status =
	    ncs_edu_exec(&cb->edu_hdl, avm_edp_ckpt_msg_ent, &dec->i_uba, EDP_OP_TYPE_DEC, &temp_ent_info, &ederror, 2,
			 2, 10);
	...
	if ((AVM_ENT_ACTIVE == ent_info->current_state) && (AVM_ENT_INACTIVE == ent_info->previous_state)) {
...
static uns32 avm_decode_ckpt_ent_state_sensor(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec)
{
	...
	status =
	    ncs_edu_exec(&cb->edu_hdl, avm_edp_ckpt_msg_ent, &dec->i_uba, EDP_OP_TYPE_DEC, &temp_ent_info, &ederror, 5,
			 2, 12, 15, 16, 17);
...
static uns32 avm_decode_ckpt_ent_adm_op(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec)
{
	...
	status =
	    ncs_edu_exec(&cb->edu_hdl, avm_edp_ckpt_msg_ent, &dec->i_uba, EDP_OP_TYPE_DEC, &temp_ent_info, &ederror, 4,
			 2, 18, 19, 20);
...
static uns32 avm_decode_cold_sync_rsp_validation_info(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec)
{
	...
	for (count = 0; count < num_of_inst; count++) {
		status =
		    m_NCS_EDU_VER_EXEC(&cb->edu_hdl, avm_edp_ckpt_msg_validation_info, &dec->i_uba, EDP_OP_TYPE_DEC,
 ...
 *
\**************************************************************************/
uns32 avm_decode_cold_sync_rsp(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec)
{
...
static uns32 avm_decode_cold_sync_rsp_ent_cfg(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec)
{
	...
	for (count = 0; count < num_of_inst; count++) {
		...
		for (temp_ent_info_list = temp_ent_info->dependents;
		     temp_ent_info_list != AVM_ENT_INFO_LIST_NULL; temp_ent_info_list = temp_ent_info_list->next) {
		...
		for (temp_ent_info_list = ent_info->dependents;
			...
			m_AVM_ENT_IS_VALID(temp_ent_info_list->ent_info, rc)
				...
				if (AVM_ENT_INFO_NULL == t_ent_info) {
					for (t_ent_info_list = ent_info->dependents;
					     t_ent_info_list != AVM_ENT_INFO_LIST_NULL;) {
...
extern uns32 avm_decode_warm_sync_rsp(AVM_CB_T *cb, NCS_MBCSV_CB_DEC *dec)
{
	...
	snprintf(sprbuf, sizeof(sprbuf) - 1,
		 " Standby ent_updt = %d ent_cfg_updt = %d adm_op_updt = %d evt_id_updt = %d dhconf_updt = %d dhstate_updt = %d \n Active  ent_updt = %d ent_cfg_updt = %d adm_op_updt = %d evt_id_updt = %d dhconf_updt = %d dhstate_updt = %d upgd_state_updt = %d\n",
		 cb->async_updt_cnt.ent_updt, cb->async_updt_cnt.ent_cfg_updt, cb->async_updt_cnt.ent_adm_op_updt,
		 cb->async_updt_cnt.evt_id_updt, cb->async_updt_cnt.ent_dhconf_updt,
		 cb->async_updt_cnt.ent_dhstate_updt, updt_cnt->ent_updt, updt_cnt->ent_cfg_updt,
		 updt_cnt->ent_adm_op_updt, updt_cnt->evt_id_updt, updt_cnt->ent_dhconf_updt,
		 updt_cnt->ent_dhstate_updt, updt_cnt->ent_upgd_state_updt);
