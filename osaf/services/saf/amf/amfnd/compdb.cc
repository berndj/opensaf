...
static amf_comp_type_t *avnd_comptype_create(SaImmHandleT immOmHandle, const SaNameT *dn)
{
	...
	if ((compt = static_cast<amf_comp_type_t*>(calloc(1, sizeof(amf_comp_type_t)))) == NULL) {
	...
	if (IS_COMP_LOCAL(compt->saAmfCtCompCategory) && 
	    immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfCtSwBundle"), attributes, 0, &compt->saAmfCtSwBundle) != SA_AIS_OK) {
		osafassert(0);
		return NULL;	
	}

        immutil_getAttrValuesNumber(const_cast<SaImmAttrNameT>("saAmfCtDefCmdEnv"), attributes, &j);
	compt->saAmfCtDefCmdEnv = static_cast<SaStringT*>(calloc(j + 1, sizeof(SaStringT)));
	...
	compt->saAmfCtDefInstantiateCmdArgv = static_cast<SaStringT*>(calloc(j + 1, sizeof(SaStringT)));
	...
	compt->saAmfCtDefTerminateCmdArgv = static_cast<SaStringT*>(calloc(j + 1, sizeof(SaStringT)));
	...
	compt->saAmfCtDefCleanupCmdArgv = static_cast<SaStringT*>(calloc(j + 1, sizeof(SaStringT)));
	...
	compt->saAmfCtDefAmStartCmdArgv = static_cast<SaStringT*>(calloc(j + 1, sizeof(SaStringT)));
	...
	compt->saAmfCtDefAmStopCmdArgv = static_cast<SaStringT*>(calloc(j + 1, sizeof(SaStringT)));
	...
	compt->osafAmfCtDefHcCmdArgv = static_cast<SaStringT*>(calloc(j + 1, sizeof(SaStringT)));
	...
	bool bundle_missing_is_error)
{
	...
	res = get_string_attr_from_imm(immOmHandle, const_cast<SaImmAttrNameT>("saAmfNodeSwBundlePathPrefix"),
	...
	cmd = &comp->clc_info.cmds[AVND_COMP_CLC_CMD_TYPE_INSTANTIATE - 1];
	if (immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfCompInstantiateTimeout"), attributes, 0, &cmd->timeout) != SA_AIS_OK) {
	...
	cmd = &comp->clc_info.cmds[AVND_COMP_CLC_CMD_TYPE_CLEANUP - 1];
	if (immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfCompCleanupTimeout"), attributes, 0, &cmd->timeout) != SA_AIS_OK) {
		cmd->timeout = comptype->saAmfCtDefClcCliTimeout;
	...
	cmd = &comp->clc_info.cmds[AVND_COMP_CLC_CMD_TYPE_AMSTART - 1];
	if (immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfCompAmStartTimeout"), attributes, 0, &cmd->timeout) != SA_AIS_OK)
		cmd->timeout = comptype->saAmfCtDefClcCliTimeout;
	...
	cmd = &comp->clc_info.cmds[AVND_COMP_CLC_CMD_TYPE_AMSTOP - 1];
	if (immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfCompAmStopTimeout"), attributes, 0, &cmd->timeout) != SA_AIS_OK)
		cmd->timeout = comptype->saAmfCtDefClcCliTimeout;
	...
	cmd = &comp->clc_info.cmds[AVND_COMP_CLC_CMD_TYPE_TERMINATE - 1];
	if (immutil_getAttr(const_cast<SaImmAttrNameT>("saAmfCompTerminateTimeout"), attributes, 0, &cmd->timeout) != SA_AIS_OK) {
		if (m_AVND_COMP_TYPE_IS_PREINSTANTIABLE(comp)) {
	...
	comp->saAmfCompCmdEnv = static_cast<SaStringT*>(calloc(comp->numOfCompCmdEnv + 1, sizeof(SaStringT)));
...
static AVND_COMP *avnd_comp_create(const SaNameT *comp_name, const SaImmAttrValuesT_2 **attributes, AVND_SU *su)
{
	...
	if ((comp = static_cast<AVND_COMP*>(calloc(1, sizeof(*comp)))) == NULL) {
