...
static void clms_plm_readiness_track_callback (
                        SaPlmEntityGroupHandleT entityGrpHdl,
	...
	/*Check for the expectedReadinessStatus of the entityName*/
        if (step == SA_PLM_CHANGE_VALIDATE || step == SA_PLM_CHANGE_START || step == SA_PLM_CHANGE_ABORTED){
			...
			clms_clear_node_dep_list(node); 
                        ...
                        if (node->nodeup &&
                                  trackedEntities->entities[i].expectedReadinessStatus.readinessState == SA_PLM_READINESS_OUT_OF_SERVICE) {
		...
		node->plm_invid = invocation;
			...
			TRACE("Entity Readiness State %d",trackedEntities->entities[i].expectedReadinessStatus.readinessState);
			...
			node->admin_op = PLM;
				  ...
				  (trackedEntities->entities[i].currentReadinessStatus.readinessState == SA_PLM_READINESS_IN_SERVICE)) {
...
SaAisErrorT clms_plm_init(CLMS_CB *cb)
{
	...
	rc = saPlmReadinessTrack(cb->ent_group_hdl,
				(SA_TRACK_CURRENT | SA_TRACK_CHANGES | SA_TRACK_START_STEP | SA_TRACK_VALIDATE_STEP),
				 1,
				 trackedEntities); /* trackCookie TBD */
