...
uns32 edsv_mab_register(EDS_CB *cb)
{
	...
	for (tbl_id = NCSMIB_TBL_EDSV_BASE; tbl_id <= NCSMIB_TBL_EDSV_CHAN_TBL; tbl_id++) {
		...
		if ((rc = ncsoac_ss(&edsv_oac_arg)) != NCSCC_RC_SUCCESS) {
			/* Log Error */
			m_LOG_EDSV_S(EDS_MIB_FAILURE, NCSFL_LC_EDSV_INIT, NCSFL_SEV_ERROR, rc, __FILE__, __LINE__,
				     tbl_id);
			printf("oac register failed \n");
			return NCSCC_RC_FAILURE;
		} else {
			/* Log Success */
			m_LOG_EDSV_S(EDS_MIB_SUCCESS, NCSFL_LC_EDSV_INIT, NCSFL_SEV_INFO, rc, __FILE__, __LINE__,
				     tbl_id);
		...
		case NCSMIB_TBL_EDSV_SCALAR:
			...
			cb->row_handle.scalar_tbl_row_hdl = (uns32)edsv_oac_arg.info.row_owned.o_row_hdl;
			m_LOG_EDSV_S(EDS_MIB_SUCCESS, NCSFL_LC_EDSV_INIT, NCSFL_SEV_INFO,
				     cb->row_handle.scalar_tbl_row_hdl, __FILE__, __LINE__, tbl_id);

			break;

		case NCSMIB_TBL_EDSV_CHAN_TBL:

			cb->row_handle.chan_tbl_row_hdl = (uns32)edsv_oac_arg.info.row_owned.o_row_hdl;
			m_LOG_EDSV_S(EDS_MIB_SUCCESS, NCSFL_LC_EDSV_INIT, NCSFL_SEV_INFO,
				     cb->row_handle.chan_tbl_row_hdl, __FILE__, __LINE__, tbl_id);
	...
	for (tbl_id = NCSMIB_TBL_EDSV_BASE; tbl_id <= NCSMIB_TBL_EDSV_CHAN_TBL; tbl_id++) {
		...
		case NCSMIB_TBL_EDSV_SCALAR:
			if (cb->row_handle.scalar_tbl_row_hdl) {
				edsv_oac_arg.info.row_gone.i_row_hdl = cb->row_handle.scalar_tbl_row_hdl;

				if ((rc = ncsoac_ss(&edsv_oac_arg)) != NCSCC_RC_SUCCESS) {
					/* Log Error */
					m_LOG_EDSV_S(EDS_MIB_FAILURE, NCSFL_LC_EDSV_INIT, NCSFL_SEV_ERROR, rc, __FILE__,
						     __LINE__, cb->row_handle.scalar_tbl_row_hdl);
					return NCSCC_RC_FAILURE;
				} else
					m_LOG_EDSV_S(EDS_MIB_SUCCESS, NCSFL_LC_EDSV_INIT, NCSFL_SEV_INFO, rc, __FILE__,
						     __LINE__, cb->row_handle.scalar_tbl_row_hdl);
		...
		case NCSMIB_TBL_EDSV_CHAN_TBL:
			if (cb->row_handle.chan_tbl_row_hdl) {
				edsv_oac_arg.info.row_gone.i_row_hdl = cb->row_handle.chan_tbl_row_hdl;

				if ((rc = ncsoac_ss(&edsv_oac_arg)) != NCSCC_RC_SUCCESS) {
					m_LOG_EDSV_S(EDS_MIB_FAILURE, NCSFL_LC_EDSV_INIT, NCSFL_SEV_ERROR, rc, __FILE__,
						     __LINE__, cb->row_handle.chan_tbl_row_hdl);
					return NCSCC_RC_FAILURE;
				} else
					m_LOG_EDSV_S(EDS_MIB_SUCCESS, NCSFL_LC_EDSV_INIT, NCSFL_SEV_INFO, rc, __FILE__,
						     __LINE__, cb->row_handle.chan_tbl_row_hdl);
...
uns32 safevtscalarobject_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL test_flag)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	EDS_CB *eds_cb = NULL;
	uns32 svc_state = 0;
...
uns32 safevtscalarobject_extract(NCSMIB_PARAM_VAL *param, NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
uns32 saevtchannelentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL test_flag)
{
...
uns32 saevtchannelentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data)
{
	EDS_CB *eds_cb = NULL;
	SaNameT chan_name;
	uns32 i = 0;
...
uns32 saevtchannelentry_extract(NCSMIB_PARAM_VAL *param, NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
			     ...
			     NCSCONTEXT *data, uns32 *next_inst_id, uns32 *next_inst_id_len)
{
	...
	if (arg->i_idx.i_inst_len != 0) {
		...
		cn = (EDS_CNAME_REC *)ncs_patricia_tree_getnext(&eds_cb->eds_cname_list, (uns8 *)&chan_name);
