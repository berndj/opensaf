...
static uns32 immd_evt_proc_immnd_intro(IMMD_CB *cb, 
                                       IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo);
static uns32 immd_evt_proc_immnd_req_sync(IMMD_CB *cb, 
                                          IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo);
static uns32 immd_evt_proc_immnd_announce_load(IMMD_CB *cb, 
                                               IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo);
...
static uns32 immd_evt_proc_adminit_req(IMMD_CB *cb,    
                                       IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo);
static uns32 immd_evt_proc_impl_set_req(IMMD_CB *cb,    
                                        IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo);
static uns32 immd_evt_proc_ccbinit_req(IMMD_CB *cb,    
                                       IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo);
static uns32 immd_evt_proc_rt_modify_req(IMMD_CB *cb,    
                                         IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo);
static uns32 immd_evt_proc_discard_impl(IMMD_CB *cb,    
                                        IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo);
static uns32 immd_evt_proc_abort_ccb(IMMD_CB *cb,    
                                     IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo);
static uns32 immd_evt_proc_admo_hard_finalize(IMMD_CB *cb,    
                                              IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo);

static uns32 immd_evt_proc_mds_evt(IMMD_CB *cb, IMMD_EVT *evt);

static uns32 immd_evt_mds_quiesced_ack_rsp(IMMD_CB *cb,IMMD_EVT *evt, 
                                           IMMSV_SEND_INFO *sinfo);
...
void immd_process_evt(void)
{
    ...
    {
        LOG_WA("Error returned from processing message err:%u msg-type:%u",
    ...
    IMMD_IMMND_INFO_NODE *node_info)
{
    ...
    if (proc_rc != NCSCC_RC_SUCCESS)
    {
        LOG_ER("failed to replicate dump_ok message to stdby err:%u", 
               proc_rc);
        TRACE_LEAVE();
        return 0;
    }
...
static void immd_req_sync(IMMD_CB *cb, IMMD_IMMND_INFO_NODE *node_info)
{
    ...
    TRACE_5("Send-1 SYNC_REQ back to requesting IMMND  to dest:%llu", 
    ...
    {/*Coord immnd is local, i.e. at active SC. */
        ...
        TRACE_5("Send-2 SYNC_REQ to local coord IMMND at active SC, dest: %llu", 
            cb->loc_immnd_dest);
        TRACE_5("nodeid:%x epoch:%u isOnController:%u",
            node_info->immnd_key, cb->mRulingEpoch, node_info->isOnController);
    ...
    else /*Coord immnd is at remote, i.e. at standby SC.*/
    {
        ...
        TRACE_5("Send-3 SYNC_REQ to remote coord IMMND at standby SC, dest:%llu",
            cb->rem_immnd_dest);
        proc_rc = immd_mds_msg_send(cb, NCSMDS_SVC_ID_IMMND, 
                             ...
                             NCS_BOOL doReply)
{
    ...
    if (proc_rc != NCSCC_RC_SUCCESS)
    {
        LOG_ER("failed to replicate node accept message to stdby err:%u", 
               proc_rc);
        goto done;
    }

    if (doReply)
    { /*If doReply is false then this was only an epoch refresh
        from an IMMND. */
 ...
 *
 *****************************************************************************/
static uns32 immd_evt_proc_immnd_announce_dump(IMMD_CB *cb, IMMD_EVT *evt, 
                                               IMMSV_SEND_INFO *sinfo)
{
    uns32                proc_rc = NCSCC_RC_SUCCESS;
    IMMD_IMMND_INFO_NODE *node_info=NULL;
    ...
    if (node_info)
    {
        ...
        {
            LOG_ER("Wrong PID %u != %u", node_info->immnd_execPid,
        ...
        {
            LOG_ER("Wrong Epoch %u != %u", node_info->epoch,
                   cb->mRulingEpoch);
            proc_rc = NCSCC_RC_FAILURE;
        }
                                               ...
                                               IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo)
{
    ...
    if (node_info)
    {
        ...
        {
            LOG_ER("Wrong new Epoch %u!=%u +1", evt->info.ctrl_msg.epoch, 
                   cb->mRulingEpoch);
            /* TODO: Return error to coord, remove coord designation ?? */
            proc_rc = NCSCC_RC_FAILURE;
        }
        ...
        if (proc_rc == NCSCC_RC_SUCCESS)
        {
          ...
          Loop through all nodes*/
            ...
            LOG_IN("Successfully announced sync. New ruling epoch:%u",
                   cb->mRulingEpoch);
        }
    }
    else
    ...
    IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo)
{
    ...
    if (node_info)
    {
        ...
        if (node_info->immnd_key != cb->immnd_coord)
        {
            LOG_ER("Not Coord! %x != %x", node_info->immnd_key, 
                cb->immnd_coord);
            proc_rc = NCSCC_RC_FAILURE;
            goto fail;
            /*TODO Return error to coord, remove coord designation ?? */
        ...
        LOG_IN("Successfully announced loading. New ruling epoch:%u",
               cb->mRulingEpoch);
    }
    else
    {
    ...
    IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo)
{
    ...
    if (node_info)
    {
        ...
        if (oldPid != newPid || oldEpoch != newEpoch)
        {
            ...
            {
                LOG_IN("Detected new IMMND process at node %x "
                    "old epoch: %u  new epoch:%u", node_info->immnd_key, 
                    oldEpoch, newEpoch);
            }
        }
 ...
 * @return uns32
 */
static uns32 immd_evt_mds_quiesced_ack_rsp (IMMD_CB *cb,
    IMMD_EVT *evt, IMMSV_SEND_INFO *sinfo)
{
    TRACE_ENTER();
    ...
    if (cb->is_quiesced_set)
    {
        ...
        LOG_ER("Received IMMD_EVT_MDS_QUIESCED_ACK_RSP message but "
