...
uns32 cpsv_map_from_instid(const uns32 *i_inst_ids, uns32 i_inst_len, CPD_CKPT_MAP_INFO *map_info, CPD_CB *cb,
			   int32 type);
...
uns32 sackptcheckpointentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data)
{
	...
	if (rc != NCSCC_RC_SUCCESS) {
		m_LOG_CPD_FCL(CPD_CKPT_INFO_NODE_GET_FAILED, CPD_FC_HDLN, NCSFL_SEV_ERROR, map_info.ckpt_id, __FILE__,
			      __LINE__);
		ncshm_give_hdl(cpd_cb->cpd_hdl);
		return rc;
	}
...
/*   uns64 *ckpt_size = NULL; */
	...
	case saCkptCheckpointUsedSize_ID:
		...
		if (ckpt_node->active_dest == 0) {
			...
			rc = cpd_mds_msg_sync_send(cpd_cb, NCSMDS_SVC_ID_CPND, ckpt_node->active_dest, &send_evt,
						   &out_evt, CPSV_WAIT_TIME);
			if (rc != NCSCC_RC_SUCCESS) {
				m_LOG_CPD_FCL(CPD_MDS_SEND_FAIL, CPD_FC_HDLN, NCSFL_SEV_ERROR, ckpt_node->active_dest,
					      __FILE__, __LINE__);
			}
		...
		if (ckpt_node->active_dest == 0) {
			...
			if (out_evt == NULL) {
				...
				ckpt_node->num_sections =
				    out_evt->info.cpd.info.ckpt_created_sections.ckpt_num_sections;
				m_MMGR_FREE_CPSV_EVT(out_evt, NCS_SERVICE_ID_CPD);
			}
		}
