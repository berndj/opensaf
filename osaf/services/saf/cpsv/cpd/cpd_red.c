...
void  cpd_a2s_ckpt_unlink_set(CPD_CB *cb,CPD_CKPT_INFO_NODE *ckpt_node)
{
   ...
   rc = cpd_mbcsv_async_update(cb,&cpd_msg);
   if(rc != SA_AIS_OK)
      m_LOG_CPD_CCL(CPD_A2S_CKPT_UNLINK_ASYNC_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,ckpt_node->ckpt_name.value,__FILE__,__LINE__);
   else
...
void cpd_a2s_ckpt_rdset(CPD_CB *cb,CPD_CKPT_INFO_NODE *ckpt_node)
{
   ...
   rc = cpd_mbcsv_async_update(cb,&cpd_msg);
   if(rc != SA_AIS_OK)
      m_LOG_CPD_FCL(CPD_A2S_CKPT_RDSET_ASYNC_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,ckpt_node->ckpt_id,__FILE__,__LINE__);   
   else
      m_LOG_CPD_FCL(CPD_A2S_CKPT_RDSET_ASYNC_SUCCESS,CPD_FC_MBCSV,NCSFL_SEV_INFO,ckpt_node->ckpt_id,__FILE__,__LINE__);
}
...
void cpd_a2s_ckpt_arep_set(CPD_CB *cb,CPD_CKPT_INFO_NODE *ckpt_node)
{
   ...
   rc = cpd_mbcsv_async_update(cb,&cpd_msg);
   if(rc != SA_AIS_OK)
     m_LOG_CPD_FFCL(CPD_A2S_CKPT_AREP_SET_ASYNC_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,ckpt_node->ckpt_id,ckpt_node->active_dest,__FILE__,__LINE__);
   else
     m_LOG_CPD_FFCL(CPD_A2S_CKPT_AREP_SET_ASYNC_SUCCESS,CPD_FC_MBCSV,NCSFL_SEV_INFO,ckpt_node->ckpt_id,ckpt_node->active_dest,__FILE__,__LINE__); 
}
...
void cpd_a2s_ckpt_dest_add(CPD_CB *cb,CPD_CKPT_INFO_NODE *ckpt_node,MDS_DEST *dest)
{
  ...
  /* send it to MBCSv */
   rc = cpd_mbcsv_async_update(cb,&cpd_msg);
   if(rc != SA_AIS_OK)
     m_LOG_CPD_FFCL(CPD_A2S_CKPT_DESTADD_ASYNC_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,ckpt_node->ckpt_id,*dest,__FILE__,__LINE__);
   else
     m_LOG_CPD_FFCL(CPD_A2S_CKPT_DESTADD_ASYNC_SUCCESS,CPD_FC_MBCSV,NCSFL_SEV_INFO,ckpt_node->ckpt_id,*dest,__FILE__,__LINE__);
}
...
void  cpd_a2s_ckpt_dest_del(CPD_CB *cb,SaCkptCheckpointHandleT ckpt_hdl,MDS_DEST *cpnd_dest,NCS_BOOL ckptid_flag)
{
   ...
   rc = cpd_mbcsv_async_update(cb,&cpd_msg);
   if ( rc != SA_AIS_OK)
     m_LOG_CPD_FFCL(CPD_A2S_CKPT_DESTDEL_ASYNC_FAILED,CPD_FC_MBCSV,NCSFL_SEV_ERROR,cpd_msg.info.dest_del.ckpt_id,*cpnd_dest,__FILE__,__LINE__);     
   else
     m_LOG_CPD_FFCL(CPD_A2S_CKPT_DESTDEL_ASYNC_SUCCESS,CPD_FC_MBCSV,NCSFL_SEV_INFO,cpd_msg.info.dest_del.ckpt_id,*cpnd_dest,__FILE__,__LINE__); 
} 
