 ...
 * Notes         : None.
 *****************************************************************************/
uns32 ncs_hisv_hcd_lib_req(NCS_LIB_REQ_INFO *req_info)
{
...
uns32 hisv_hcd_init(NCS_LIB_REQ_INFO *req_info)
{
	...
	hcd_cb->my_hdl = ncshm_create_hdl(hcd_cb->hm_poolid, NCS_SERVICE_ID_HCD, (NCSCONTEXT)hcd_cb);
	...
	while (retry++ < HPI_INIT_MAX_RETRY) {
		...
		if ((SA_OK != err) && (err != SA_ERR_HPI_DUPLICATE)) {
			...
			printf
			    ("hisv_hcd_init: saHpiInitialize Error %d: May need to do E-keying of interface to shelf manager\n",
			     err);
	...
	if (retry >= HPI_INIT_MAX_RETRY) {
		...
		printf
		    ("hisv_hcd_init: saHpiInitialize Error %d: May need to do E-keying of interface to shelf manager\n",
		     err);
		m_LOG_HISV_DTS_CONS
		    ("hsm_rediscover: This is expected if standby SCXB does not have connectivity to shelf manager\n");
	...
	if (NCSCC_RC_SUCCESS != (rc = m_NCS_TASK_CREATE((NCS_OS_CB)hcd_hsm,
							...
							(NCS_STACKSIZE_HUGE * 2), &hsm_cb->task_hdl))) {
	...
	if (NCSCC_RC_SUCCESS != (rc = m_NCS_TASK_CREATE((NCS_OS_CB)hcd_sim,
							...
							NCS_STACKSIZE_HUGE, &sim_cb->task_hdl))) {
	...
	if (NCSCC_RC_SUCCESS != (rc = m_NCS_TASK_CREATE((NCS_OS_CB)hcd_ham,
							...
							NCS_STACKSIZE_HUGEX2, &ham_cb->task_hdl))) {
	...
	amf_error = saAmfComponentRegister(hcd_cb->amf_hdl, &hcd_cb->comp_name, (SaNameT *)NULL);
...
uns32 hisv_hcd_destroy(NCS_LIB_REQ_INFO *req_info)
{
	...
	if ((hcd_cb = (NCSCONTEXT)ncshm_take_hdl(NCS_SERVICE_ID_HCD, gl_hcd_hdl))
		...
		saAmfComponentUnregister(hcd_cb->amf_hdl, &hcd_cb->comp_name, (SaNameT *)NULL);
		saAmfFinalize(hcd_cb->amf_hdl);
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 hcd_cb_init(HCD_CB *hcd_cb)
{
	uns32 rc = NCSCC_RC_FAILURE;
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 hcd_cb_destroy(HCD_CB *hcd_cb)
{
	uns32 rc = NCSCC_RC_FAILURE;
 ...
 * Notes         : None.
 *****************************************************************************/
NCS_BOOL hcd_clear_mbx(NCSCONTEXT arg, NCSCONTEXT msg)
{
