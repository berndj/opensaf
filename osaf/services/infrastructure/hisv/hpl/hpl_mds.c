...
static uns32 hpl_msg_proc(HPL_CB *hpl_cb, HISV_MSG *hisv_msg, MDS_SEND_PRIORITY_TYPE prio);
...
static uns32 hpl_mds_cb_dec(struct ncsmds_callback_info *info)
{
	...
	if ((info->i_yr_svc_id == NCSMDS_SVC_ID_HPL)
		...
		if (!m_NCS_MSG_FORMAT_IS_VALID(info->info.dec.i_msg_fmt_ver,
					       HPL_HCD_SUB_PART_VER_MIN,
					       HPL_HCD_SUB_PART_VER_MAX, hpl_hcd_msg_fmt_map_table)) {
...
static uns32 hpl_mds_cb_rcv(struct ncsmds_callback_info *mds_cb_info)
{
	...
	/* retrieve HPL CB */
	if (NULL == (hpl_cb = (HPL_CB *)ncshm_take_hdl(NCS_SERVICE_ID_HPL, (uns32)mds_cb_info->i_yr_svc_hdl))) {
    ...
    **/
	rc = hpl_msg_proc(hpl_cb, hisv_msg, mds_cb_info->info.receive.i_priority);
...
static uns32 hpl_msg_proc(HPL_CB *hpl_cb, HISV_MSG *hisv_msg, MDS_SEND_PRIORITY_TYPE prio)
{
	...
	case HAM_MDS_UP:
	 ...
	 /** check if the table already contain the chassis-id information
		...
		memcpy(&ham_info->ham_dest, &hisv_msg->info.cbk_info.hpl_ret.h_vdest.ham_dest, (int32)sizeof(MDS_DEST));
...
uns32 hpl_mds_msg_async_send(HPL_CB *cb, HISV_MSG *i_msg, MDS_DEST *ham_dest, uns32 prio)
{
	...
	memcpy(&mds_info.info.svc_send.info.snd.i_to_dest, ham_dest, (int32)sizeof(MDS_DEST));
...
HISV_MSG *hpl_mds_msg_sync_send(HPL_CB *cb, HISV_MSG *i_msg, MDS_DEST *ham_dest, uns32 prio, uns32 timeout)
{
	NCSMDS_INFO mds_info;
	int32 rc;
	...
	/* fill the sub send strcuture */
	memcpy(&mds_info.info.svc_send.info.sndrsp.i_to_dest, ham_dest, (int32)sizeof(MDS_DEST));
