...
EXTERN_C uns32 ncs_bam_destroy();

EXTERN_C uns32 ncs_bam_initialize(NCS_LIB_REQ_INFO *);

EXTERN_C uns32 parse_and_build_DOMDocument(char *xmlFilename, BAM_PARSE_SUB_TREE);

EXTERN_C uns32 ncs_bam_dl_func(NCS_LIB_REQ_INFO *req_info);

EXTERN_C uns32 ncs_bam_mds_reg(NCS_BAM_CB *);

EXTERN_C uns32 ncs_bam_rda_reg(NCS_BAM_CB *);

EXTERN_C void ncs_bam_send_cfg_done_msg(void);
EXTERN_C void pss_bam_send_cfg_done_msg(BAM_PARSE_SUB_TREE);

EXTERN_C uns32 bam_avd_rcv_msg(NCS_BAM_CB *bam_cb, AVD_BAM_MSG *msg);

EXTERN_C uns32 bam_psr_rcv_msg(NCS_BAM_CB *bam_cb, PSS_BAM_MSG *msg);
...
EXTERN_C uns32 ncs_bam_build_and_generate_mibsets(NCSMIB_TBL_ID, uns32, NCSMIB_IDX *, char *, NCSMIB_FMAT_ID);

EXTERN_C uns32 ncs_bam_generate_counter64_mibset(NCSMIB_TBL_ID, uns32, NCSMIB_IDX *, char *);

EXTERN_C SaAisErrorT
ncs_bam_search_table_for_oid(TAG_OID_MAP_NODE *, uns32, char *, char *, NCSMIB_TBL_ID *, uns32 *, NCSMIB_FMAT_ID *);

EXTERN_C uns32 ncs_bam_build_mib_idx(NCSMIB_IDX *, char *, NCSMIB_FMAT_ID);

EXTERN_C uns32 ncs_bam_add_sec_mib_idx(NCSMIB_IDX *mib_idx, char *idx_val, NCSMIB_FMAT_ID format);
