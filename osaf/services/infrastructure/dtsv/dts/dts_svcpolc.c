...
static uns32 dts_service_row_status_set(DTS_CB *inst, DTS_SVC_REG_TBL *node);
...
* Notes:  
**************************************************************************/
uns32 ncsdtsvservicelogpolicyentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL flag)
{
	...
	case ncsDtsvServiceLogDevice_ID:
		...
		if (set_req->i_param_val.info.i_oct == NULL)
			return m_DTS_DBG_SINK(NCSCC_RC_FAILURE,
					      "ncsdtsvservicelogpolicyentry_set: NULL pointer for i_oct in SET request received");
...
* Notes:  
**************************************************************************/
uns32 ncsdtsvservicelogpolicyentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data)
{
	DTS_CB *inst = (DTS_CB *)cb;
	SVC_KEY nt_key;
	DTS_SVC_REG_TBL *service;
		...
		if (params[paramid - 1].set_flag) {
			...
			case ncsDtsvServiceLogDevice_ID:
				...
				if (params[paramid - 1].param.info.i_oct == NULL)
					return m_DTS_DBG_SINK(NCSCC_RC_FAILURE,
							      "ncsdtsvservicelogpolicyentry_setrow: NULL pointer for i_oct in SETROW request received");
				else if (*(uns8 *)params[paramid - 1].param.info.i_oct < DTS_LOG_DEV_VAL_MAX) {
					if ((*(uns8 *)params[paramid - 1].param.info.i_oct == 0) &&
					    (old_log_device != 0))
						m_LOG_DTS_EVT(DTS_LOG_DEV_NONE, service->my_key.ss_svc_id,
							      service->my_key.node, 0);
					else if (*(uns8 *)params[paramid - 1].param.info.i_oct != 0)
						return m_DTS_DBG_SINK(NCSCC_RC_FAILURE,
								      "dtsv_setrow_svc_policy_obj: Log Device bit map beyond permissible values");
				}
			...
			/* Do processing specific to the param objects after set */
			rc = dts_handle_service_param_set(inst, service, paramid, old_log_device, old_buff_size);
		}
	}
	return rc;
}
...
/**************************************************************************
 ...
 Notes:  
**************************************************************************/
static uns32 dts_service_row_status_set(DTS_CB *inst, DTS_SVC_REG_TBL *service)
{
...
dts_handle_service_param_set(DTS_CB *inst,
			     DTS_SVC_REG_TBL *service,
			     NCSMIB_PARAM_ID paramid, uns8 old_log_device, uns32 old_buff_size)
{
	uns32 rc = NCSCC_RC_SUCCESS;
...
{
	if ((param_val->i_param_id == ncsDtsvServiceLogDevice_ID)
	    || (param_val->i_param_id == ncsDtsvServiceCategoryBitMap_ID)
	    || (param_val->i_param_id == ncsDtsvServiceSeverityBitMap_ID)) {
...
* Notes:
**************************************************************************/
uns32 dtsv_svc_policy_set_oct(DTS_CB *cb, NCSMIB_ARG *arg, POLICY *policy)
{
...
* Notes:
**************************************************************************/
uns32 dtsv_svc_extract_oct(NCSMIB_PARAM_VAL *param_val, NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
	uns32 rc = NCSCC_RC_SUCCESS;
