      ...
      case ncsDtsvGlobalLogDevice_ID:
         ...
         else if(*(uns8*)set_req->i_param_val.info.i_oct < DTS_LOG_DEV_VAL_MAX)
         {
            if((*(uns8*)set_req->i_param_val.info.i_oct == 0) && 
               ...
               return m_DTS_DBG_SINK(NCSCC_RC_FAILURE, "dtsv_global_policy_set_obj: Log Device bit map beyond permissible values");
         }
...
post_set:
   ...
   */
   ...
   case ncsDtsvGlobalMessageLogging_ID:
       ...
       if (NCSCC_RC_SUCCESS != dtsv_global_filtering_policy_change(inst, 
                               set_req->i_param_val.i_param_id))
   ...
   case ncsDtsvGlobalLogDevice_ID:
       ...
       rc = dts_log_device_set(&inst->g_policy.g_policy,
           &inst->g_policy.device, old_log_device);
      break;
   ...
   case ncsDtsvGlobalCircularBuffSize_ID:
       if(sameval)
          return rc;   
       rc = dts_buff_size_set(&inst->g_policy.g_policy,
           &inst->g_policy.device, old_buff_size);
      break;
   ...
   {  
      ...
      while(m_DTS_NUM_LOG_FILES(device) > inst->g_policy.g_num_log_files)
      {
         ...
         m_DTSV_SEND_CKPT_UPDT_ASYNC(&dts_cb, NCS_MBCSV_ACT_RMV, NCS_PTR_TO_UNS64_CAST(&data), DTSV_CKPT_DTS_LOG_FILE_CONFIG);
      }
      ...
      while(node != NULL)
      {
         ...
         while(m_DTS_NUM_LOG_FILES(device) > inst->g_policy.g_num_log_files)
         {
            ...
            m_DTSV_SEND_CKPT_UPDT_ASYNC(&dts_cb, NCS_MBCSV_ACT_RMV, (long)&data, DTSV_CKPT_DTS_LOG_FILE_CONFIG);
         } 

         node = (DTS_SVC_REG_TBL *)ncs_patricia_tree_getnext(&inst->svc_tbl, (const uns8*)&node->ntwk_key);
      }
   }
      break;
   ...
   if(rc != NCSCC_RC_FAILURE)
   {
      ...
      m_DTSV_SEND_CKPT_UPDT_ASYNC(inst, NCS_MBCSV_ACT_UPDATE, (long)&inst->g_policy, DTSV_CKPT_GLOBAL_POLICY_CONFIG);
   }
...
**************************************************************************/
static uns32
dtsv_global_filtering_policy_change(DTS_CB *inst, NCSMIB_PARAM_ID param_id)
{
    SVC_KEY               nt_key;
    ...
    while (service != NULL)
    {
        ...
        if ((service->my_key.ss_svc_id != 0) &&
            (param_id != ncsDtsvGlobalMessageLogging_ID))
        {
...
* Notes:
**************************************************************************/
uns32 ncsdtsvscalars_extract(NCSMIB_PARAM_VAL *param_val, 
                             NCSMIB_VAR_INFO *var_info, 
                             NCSCONTEXT data, 
                             NCSCONTEXT buffer)
{
    if((param_val->i_param_id == ncsDtsvGlobalLogDevice_ID) || (param_val->i_param_id ==ncsDtsvGlobalCategoryBitMap_ID) || (param_val->i_param_id == ncsDtsvGlobalSeverityBitMap_ID))
    {
...
* Notes:
**************************************************************************/
uns32 dtsv_global_policy_set_oct(DTS_CB *cb, NCSMIB_ARG *arg,
                               POLICY *policy)
{
