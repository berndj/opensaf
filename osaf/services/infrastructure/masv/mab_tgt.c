...
void mab_dbg_dump_fltr_op(MAS_TBL *inst, MAS_FLTR *fltr, uns32 tbl_id, MAB_FMODE mode)
{
	uns8 *role_name = NULL;

	printf("\n\n===============================================");
	printf("\n [MAS_FLTR] MODE:%s.", mode == MFM_CREATE ? "CREATE" : mode == MFM_MODIFY ? "MODIFY" : "DESTROY");
	printf("\n================================================");

#if (NCS_RMS == 1)
	role_name =
	    (uns8 *)(inst->re.role == NCSFT_ROLE_PRIMARY ? "PRIMARY" : inst->re.role ==
		     NCSFT_ROLE_BACKUP ? "BACKUP" : "OTHER");
#else
	role_name = (uns8 *)("NONE");
#endif
...
static uns32 get_full_path_of_file(uns8 *root, uns8 *profile, char *n_pcn,
				   uns16 pwe, uns32 tbl, uns8 *buf, uns32 buf_len)
{
...
static uns32 pssts_open_file(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_OPEN_FILE *open)
{
	...
	retval = create_directories(inst->root_dir, open->i_profile_name, open->i_pcn, open->i_pwe_id);
	if (retval != NCSCC_RC_SUCCESS)
		return NCSCC_RC_FAILURE;
	...
	case NCS_PSSTS_FILE_PERM_READ:	/* Open the existing file for reading */
		{
			retval = get_full_path_of_file(inst->root_dir,
						       open->i_profile_name,
						       open->i_pcn, open->i_pwe_id, open->i_tbl_id, buf, sizeof(buf));
			if (retval != NCSCC_RC_SUCCESS)
				return NCSCC_RC_FAILURE;
...
static uns32 pssts_file_exists(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_FILE_EXISTS *file_exists)
{
	...
	retval = get_full_path_of_file(inst->root_dir,
				       ...
				       file_exists->i_pcn,
				       file_exists->i_pwe_id, file_exists->i_tbl_id, buf, sizeof(buf));
	if (retval != NCSCC_RC_SUCCESS)
		return NCSCC_RC_FAILURE;
...
static uns32 pssts_file_size(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_FILE_SIZE *file_size)
{
	...
	retval = get_full_path_of_file(inst->root_dir,
				       ...
				       file_size->i_pcn, file_size->i_pwe_id, file_size->i_tbl_id, buf, sizeof(buf));
	if (retval != NCSCC_RC_SUCCESS)
		return NCSCC_RC_FAILURE;
...
static uns32 pssts_create_profile(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_CREATE_PROFILE *create_profile)
{
	NCS_OS_FILE file;
	uns32 retval;
	uns8 buf[NCS_PSSTS_MAX_PATH_LEN];
...
static uns32 pssts_copy_temp_file(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_COPY_TEMP_FILE *copy_tfile)
{
	...
	retval = get_full_path_of_file(inst->root_dir,
				       ...
				       copy_tfile->i_pcn,
				       copy_tfile->i_pwe_id, copy_tfile->i_tbl_id, buf2, sizeof(buf2));
	if (retval != NCSCC_RC_SUCCESS)
		return NCSCC_RC_FAILURE;
...
static uns32 pssts_get_next_profile(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_GET_NEXT_PROFILE *get_next)
{
	NCS_OS_FILE file;
...
static uns32 pssts_profile_exists(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_PROFILE_EXISTS *profile_exists)
{
	NCS_OS_FILE file;
	uns32 retval;
	uns8 buf[NCS_PSSTS_MAX_PATH_LEN];
...
static uns32 pssts_pwe_exists(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_PWE_EXISTS *pwe_exists)
{
	NCS_OS_FILE file;
	uns32 retval;
	uns8 buf[NCS_PSSTS_MAX_PATH_LEN];
...
static uns32 pssts_pcn_exists(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_PCN_EXISTS *pcn_exists)
{
	NCS_OS_FILE file;
	uns32 retval;
	uns8 buf[NCS_PSSTS_MAX_PATH_LEN];
...
static uns32 pssts_delete_file(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_DELETE_FILE *delete_file)
{
	...
	retval = get_full_path_of_file(inst->root_dir,
				       ...
				       delete_file->i_pcn, delete_file->i_pwe, delete_file->i_tbl_id, buf, sizeof(buf));
	if (retval != NCSCC_RC_SUCCESS)
		return NCSCC_RC_FAILURE;
...
static uns32 pssts_get_clients(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_GET_CLIENTS *get_clients)
{
	...
	while ((retval == NCSCC_RC_SUCCESS) && (next_pwe[0] != '\0')) {
		if ((strcmp((char *)&next_pwe, ".") == 0) ||
		    (strcmp((char *)&next_pwe, "..") == 0) || (strcmp((char *)&next_pwe, "ProDesc.txt") == 0)) {
		...
		while ((retval == NCSCC_RC_SUCCESS) && (next_pcn[0] != '\0')) {
			if ((strcmp((char *)&next_pcn, ".") == 0) || (strcmp((char *)&next_pcn, "..") == 0)) {
...
static uns32 pssts_move_tbl_details_file(NCS_PSSTS_CB *inst, NCS_PSSTS_ARG_MOVE_TBL_DETAILS_FILE *move_detailsfile)
{
	...
	retval = get_full_path_of_file(inst->root_dir,
				       ...
				       move_detailsfile->i_pwe_id, move_detailsfile->i_tbl_id, buf2, sizeof(buf2));
	if (retval != NCSCC_RC_SUCCESS)
		return NCSCC_RC_FAILURE;
...
static uns32 ncspssts_lm_create(NCS_PSSTS_LM_CREATE *create)
{
	...
	inst = (NCS_PSSTS_CB *)m_NCS_MEM_ALLOC(sizeof(NCS_PSSTS_CB), NCS_MEM_REGION_TRANSIENT, NCS_SERVICE_ID_PSSTS, 0);
	if (inst == NULL)
		return NCSCC_RC_FAILURE;
	...
	inst->hm_hdl = ncshm_create_hdl(inst->hmpool_id, NCS_SERVICE_ID_PSSTS, (NCSCONTEXT)inst);
...
static uns32 ncspssts_lm_destroy(NCS_PSSTS_LM_DESTROY *destroy)
{
	...
	m_NCS_MEM_FREE(inst, NCS_MEM_REGION_TRANSIENT, NCS_SERVICE_ID_PSSTS, 0);
