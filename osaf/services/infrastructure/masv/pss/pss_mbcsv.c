...
uns32 pss_ckpt_encode_cbk_handler(NCS_MBCSV_CB_ARG *cbk_arg)
{
	...
	mbcsv_version = m_NCS_MBCSV_FMT_GET(cbk_arg->info.encode.i_peer_version,
					    PSS_MBCSV_VERSION, PSS_MBCSV_VERSION_MIN);
	if (0 == mbcsv_version) {
	...
	case NCS_MBCSV_MSG_COLD_SYNC_RESP_COMPLETE:
		if ((pwe_cb->p_pss_cb->ha_state == SA_AMF_HA_ACTIVE) ||
			...
			rc = pss_ckpt_enc_coldsync_resp(pwe_cb, &cbk_arg->info.encode.io_uba,
							cbk_arg->info.encode.i_peer_version);
			if (rc != NCSCC_RC_SUCCESS) {
				m_LOG_PSS_HDLN_II(NCSFL_SEV_ERROR, PSS_HDLN_COLD_SYNC_RESP_ENC_FAIL, pwe_cb->pwe_id,
						  rc);
				ncshm_give_hdl(cbk_arg->i_client_hdl);
				return rc;
			} else {
			...
			m_LOG_PSS_HDLN_I(NCSFL_SEV_ERROR, PSS_HDLN_COLD_SYNC_RESP_ENC_IN_NON_ACTIVE_ROLE,
					 pwe_cb->pwe_id);
			ncshm_give_hdl(cbk_arg->i_client_hdl);
			return NCSCC_RC_FAILURE;
		}
	...
	case NCS_MBCSV_MSG_WARM_SYNC_RESP_COMPLETE:
		if ((pwe_cb->p_pss_cb->ha_state == SA_AMF_HA_ACTIVE) ||
			...
			if (rc != NCSCC_RC_SUCCESS) {
				m_LOG_PSS_HDLN_II(NCSFL_SEV_ERROR, PSS_HDLN_WARM_SYNC_RESP_ENC_FAIL, pwe_cb->pwe_id,
						  rc);
				ncshm_give_hdl(cbk_arg->i_client_hdl);
				return rc;
			} else {
			...
			/* Do nothing */ ;;
			/* Log it. Ideally, this shouldn't occur. */
			m_LOG_PSS_HDLN_I(NCSFL_SEV_ERROR, PSS_HDLN_WARM_SYNC_RESP_ENC_IN_NON_ACTIVE_ROLE,
					 pwe_cb->pwe_id);
			ncshm_give_hdl(cbk_arg->i_client_hdl);
			return NCSCC_RC_FAILURE;
		}
	...
	case NCS_MBCSV_MSG_DATA_RESP:
		if ((pwe_cb->p_pss_cb->ha_state == SA_AMF_HA_ACTIVE) ||
			...
			rc = pss_ckpt_enc_data_resp(pwe_cb, &cbk_arg->info.encode.io_uba,
						    cbk_arg->info.encode.i_peer_version);
			if (rc != NCSCC_RC_SUCCESS)
...
uns32 pss_ckpt_enc_coldsync_resp(PSS_PWE_CB *pwe_cb, NCS_UBAID *io_uba, uns16 peer_mbcsv_version)
{
	...
	if (pss_enc_pwe_data_for_sync_with_standby(pwe_cb, io_uba, enc_lib_conf, peer_mbcsv_version) !=
	    NCSCC_RC_SUCCESS) {
...
uns32 pss_ckpt_enc_warmsync_resp(PSS_PWE_CB *pwe_cb, NCS_UBAID *io_uba)
{
	...
	if ((data = ncs_enc_reserve_space(io_uba, sizeof(uns32))) == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_ENC_RESERVE_SPACE_FAIL, "pss_ckpt_enc_warmsync_resp()");
		return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
	}
...
uns32 pss_ckpt_dec_data_resp(PSS_PWE_CB *pwe_cb, NCS_UBAID *io_uba, uns16 peer_mbcsv_version)
{
	...
	if (pss_dec_pwe_data_for_sync_with_standby(pwe_cb, io_uba, dec_lib_conf, peer_mbcsv_version) !=
	    NCSCC_RC_SUCCESS) {
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 pss_enc_pwe_data_for_sync_with_standby(PSS_PWE_CB *pwe_cb, NCS_UBAID *io_uba, NCS_BOOL enc_lib_conf,
					     uns16 peer_mbcsv_version)
{
	...
	while ((pNode = ncs_patricia_tree_getnext(&pwe_cb->client_table, (const uns8 *)&client_key)) != NULL) {
		...
		for (i = 0; i < MAB_MIB_ID_HASH_TBL_SIZE; i++) {
			for (rec = client_node->hash[i], ederror = EDU_NORMAL; rec != NULL;
			     rec = rec->next, ++tbl_entry_cnt) {
				rc = m_NCS_EDU_VER_EXEC(&pwe_cb->p_pss_cb->edu_hdl, ncs_edp_mds_dest, io_uba,
							EDP_OP_TYPE_ENC, &rec->p_oaa_entry->key.mds_dest, &ederror,
							peer_mbcsv_version);
				if (rc != NCSCC_RC_SUCCESS) {
					m_LOG_PSS_HDLN_I(NCSFL_SEV_ERROR, PSS_HDLN_MBCSV_MDEST_ENC_FAIL,
							 pwe_cb->pwe_id);
					return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
				}
				...
				ncs_enc_claim_space(io_uba, sizeof(uns32));
				m_LOG_PSS_HDLN_II(NCSFL_SEV_DEBUG, PSS_HDLN_MBCSV_TBL_ID_ENC_SUCCESS, pwe_cb->pwe_id,
						  rec->tbl_id);
			}
		}
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 pss_dec_pwe_data_for_sync_with_standby(PSS_PWE_CB *pwe_cb, NCS_UBAID *io_uba, NCS_BOOL dec_lib_conf,
					     uns16 peer_mbcsv_version)
{
	...
	for (i = 0; i < re_data_cnt; i++) {
		...
		for (j = 0; j < tbl_cnt; j++) {
			...
			if (pss_find_table_tree(pwe_cb, client_node, oaa_node, tbl_id, TRUE, &prc) == NULL) {
				if (prc != PSS_RET_MIB_DESC_NULL) {
					m_LOG_PSS_HDLN_II(NCSFL_SEV_ERROR, PSS_HDLN_RE_TBL_REC_ADD_FAIL, pwe_cb->pwe_id,
							  tbl_id);
					m_MMGR_FREE_MAB_PCN_STRING(p_pcn);
					pss_destroy_pwe_oaa_tree(&pwe_cb->oaa_tree, FALSE);
					pss_destroy_pwe_client_table(&pwe_cb->client_table, FALSE);
				...
				m_LOG_PSS_HDLN2_II(NCSFL_SEV_NOTICE, PSS_HDLN2_RE_TBL_REC_ADD_FAIL_MIB_DESC_NULL,
						   pwe_cb->pwe_id, tbl_id);
			} else {
				m_LOG_PSS_HDLN_II(NCSFL_SEV_DEBUG, PSS_HDLN_RE_TBL_REC_ADD_SUCCESS, pwe_cb->pwe_id,
						  tbl_id);
			}
		}
		m_MMGR_FREE_MAB_PCN_STRING(p_pcn);
...
uns32 pss_ckpt_encode_async_update(NCS_MBCSV_CB_ARG *cbk_arg)
{
	...
	default:
		rc = NCSCC_RC_FAILURE;
		m_LOG_PSS_HDLN_II(NCSFL_SEV_ERROR, PSS_HDLN_MBCSV_ASYNC_ENC_CALLBACK_INVALID_EVT, pwe_cb->pwe_id,
				  data->ckpt_data_type);
		break;
	}
	m_LOG_PSS_HDLN_III(NCSFL_SEV_DEBUG, PSS_HDLN_MBCSV_ASYNC_ENC_CALLBACK_EVT_STATUS, pwe_cb->pwe_id,
			   data->ckpt_data_type, rc);
...
uns32 pss_ckpt_decode_async_update(NCS_MBCSV_CB_ARG *cbk_arg)
{
	...
	case PSS_CKPT_DATA_TBL_BIND:
		rc = pss_ckpt_dec_tbl_bind(pwe_cb, &cbk_arg->info.decode.i_uba,
					   (PSS_CKPT_TBL_BIND_MSG *)&data.ckpt_data.tbl_bind,
					   cbk_arg->info.decode.i_peer_version);
		break;
...
uns32 pss_ckpt_decode_cbk_handler(NCS_MBCSV_CB_ARG *cbk_arg, NCS_BOOL *is_in_sync)
{
	...
	case NCS_MBCSV_MSG_ASYNC_UPDATE:
		...
		if (rc != NCSCC_RC_SUCCESS) {
			...
			if (gl_pss_amf_attribs.amf_attribs.amfHandle != 0) {
				saf_status = saAmfComponentErrorReport(gl_pss_amf_attribs.amf_attribs.amfHandle, &gl_pss_amf_attribs.amf_attribs.compName, 0,	/* errorDetectionTime; AvNd will provide this value */
								       SA_AMF_COMPONENT_FAILOVER /* recovery */ ,
								       0 /*NTF Id */ );
	...
	case NCS_MBCSV_MSG_COLD_SYNC_RESP_COMPLETE:
		if (pwe_cb->ckpt_state != PSS_COLD_SYNC_COMPLETE) {
			...
			rc = pss_ckpt_dec_coldsync_resp(pwe_cb, &cbk_arg->info.decode.i_uba,
							cbk_arg->info.decode.i_peer_version);
			if (rc != NCSCC_RC_SUCCESS) {
				m_LOG_PSS_HDLN_II(NCSFL_SEV_ERROR, PSS_HDLN_COLD_SYNC_RESP_DEC_FAIL, pwe_cb->pwe_id,
						  rc);
				pwe_cb->ckpt_state = PSS_COLD_SYNC_FAIL;
				sprintf(addrstr, "PWE-id:%d, CHK PT STATE: %s ", pwe_cb->pwe_id,
					chk_pt_states[pwe_cb->ckpt_state]);
				ncs_logmsg(NCS_SERVICE_ID_PSS, PSS_LID_HDLN_C, PSS_FC_HDLN, NCSFL_LC_HEADLINE,
					   NCSFL_SEV_ERROR, NCSFL_TYPE_TIC, PSS_HDLN_CKPT_STATE, addrstr);
			} else {
				...
				sprintf(addrstr, "PWE-id:%d, CHK PT STATE: %s ", pwe_cb->pwe_id,
					chk_pt_states[pwe_cb->ckpt_state]);
				ncs_logmsg(NCS_SERVICE_ID_PSS, PSS_LID_HDLN_C, PSS_FC_HDLN, NCSFL_LC_HEADLINE,
					   NCSFL_SEV_NOTICE, NCSFL_TYPE_TIC, PSS_HDLN_CKPT_STATE, addrstr);
				if (pwe_cb->is_amfresponse_for_active_pending) {
					...
					m_LOG_PSS_HDLN_I(NCSFL_SEV_NOTICE, PSS_HDLN_INVKD_PENDING_AMF_RESP,
							 pwe_cb->pwe_id);
				}
			}
		} else {
			/* Do nothing */ ;;
			/* Log it. Ideally, this shouldn't occur. */
			m_LOG_PSS_HDLN_II(NCSFL_SEV_ERROR, PSS_HDLN_COLD_SYNC_RESP_DEC_IN_CKPT_STATE, pwe_cb->pwe_id,
					  pwe_cb->ckpt_state);
		}
		break;
	...
	case NCS_MBCSV_MSG_WARM_SYNC_RESP_COMPLETE:
		if (pwe_cb->p_pss_cb->ha_state == SA_AMF_HA_STANDBY) {
			...
			if (rc != NCSCC_RC_SUCCESS) {
				m_LOG_PSS_HDLN_II(NCSFL_SEV_ERROR, PSS_HDLN_WARM_SYNC_RESP_DEC_FAIL, pwe_cb->pwe_id,
						  rc);
				...
				if (*is_in_sync == TRUE) {
					...
					if (pss_send_data_req(pwe_cb->ckpt_hdl)
					    != NCSCC_RC_SUCCESS) {
						m_LOG_PSS_HDLN_I(NCSFL_SEV_ERROR, PSS_HDLN_SEND_DATA_REQ_FAIL,
								 pwe_cb->pwe_id);
						pwe_cb->ckpt_state = PSS_WARM_SYNC_FAIL;
						memset(addrstr, '\0', 255);
						sprintf(addrstr, "PWE-id:%d, CHK PT STATE: %s ", pwe_cb->pwe_id,
							chk_pt_states[pwe_cb->ckpt_state]);
						ncs_logmsg(NCS_SERVICE_ID_PSS, PSS_LID_HDLN_C, PSS_FC_HDLN,
							   NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR, NCSFL_TYPE_TIC,
							   PSS_HDLN_CKPT_STATE, addrstr);
	...
	case NCS_MBCSV_MSG_DATA_RESP_COMPLETE:
		if (pwe_cb->p_pss_cb->ha_state == SA_AMF_HA_STANDBY) {
			...
			rc = pss_ckpt_dec_data_resp(pwe_cb, &cbk_arg->info.decode.i_uba,
						    cbk_arg->info.decode.i_peer_version);
			if (rc != NCSCC_RC_SUCCESS) {
...
uns32 pss_ckpt_dec_warmsync_resp(PSS_PWE_CB *pwe_cb, NCS_UBAID *io_uba, NCS_BOOL *is_in_sync)
{
	...
	if ((data = ncs_dec_flatten_space(io_uba, (uns8 *)&async_cnt, sizeof(uns32))) == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_DEC_FLATTEN_SPACE_FAIL, "pss_ckpt_dec_warmsync_resp()");
		return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
	}
	async_cnt = ncs_decode_32bit(&data);
	ncs_dec_skip_space(io_uba, sizeof(uns32));
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 pss_ckpt_convert_n_process_event(PSS_PWE_CB *pwe_cb, PSS_CKPT_MSG *data, NCS_MBCSV_CB_ARG *cbk_arg)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	...
	default:
		rc = NCSCC_RC_FAILURE;
		m_LOG_PSS_HDLN_II(NCSFL_SEV_ERROR, PSS_HDLN_MBCSV_ASYNC_EVT_HANLDING_EVT_INVALID, pwe_cb->pwe_id,
				  data->ckpt_data_type);
		break;
	}
...
uns32 pss_process_re_plbck_ssn_info(PSS_PWE_CB *pwe_cb, PSS_CKPT_MSG *data)
{
	...
	case TRUE:
		...
		case TRUE:
			...
			m_LOG_PSS_HDLN2_I(NCSFL_SEV_DEBUG, PSS_HDLN_MBCSV_RE_NEW_PLBCK_SSN_DETAILS_MODIFIED,
					  pwe_cb->pwe_id);
			break;
		...
		default:	/* FALSE */
			...
			m_LOG_PSS_HDLN2_I(NCSFL_SEV_DEBUG, PSS_HDLN_MBCSV_RE_NEW_PLBCK_SSN_DETAILS_COPIED,
					  pwe_cb->pwe_id);
			break;
		}
		break;
	...
	default:		/* FALSE */
		...
		default:	/* FALSE */
			...
			m_LOG_PSS_HDLN2_I(NCSFL_SEV_ERROR,
...
uns32 pss_mdfy_plbck_info(PSS_PWE_CB *pwe_cb, PSS_CKPT_MSG *data)
{
	...
	pwe_cb->curr_plbck_ssn_info.info.info2.partial_delete_done =
	    data->ckpt_data.plbck_ssn_info.info.info2.partial_delete_done;
...
uns32 pss_re_sync(PSS_PWE_CB *pwe_cb, MAB_MSG *mab_msg, PSS_CKPT_DATA_TYPE ckpt_type)
{
	...
	case PSS_CKPT_DATA_TBL_BIND:
		...
		if (rc != NCSCC_RC_SUCCESS) {
			m_LOG_PSS_HDLN2_II(NCSFL_SEV_ERROR, PSS_HDLN_RE_SYNC_CKPT_EVT_POPULATE_FAIL, pwe_cb->pwe_id,
					   ckpt_type);
			return rc;
		}
		break;
	...
	case PSS_CKPT_DATA_BAM_CONF_DONE:
		...
		if ((msg.ckpt_data.bam_conf_done.pcn_list.pcn = m_MMGR_ALLOC_MAB_PCN_STRING(len + 1)) == NULL) {
			m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_PCN_STRING_ALLOC_FAIL, "pss_re_sync()");
			m_LOG_PSS_HDLN2_II(NCSFL_SEV_ERROR, PSS_HDLN_RE_SYNC_CKPT_EVT_POPULATE_FAIL, pwe_cb->pwe_id,
					   ckpt_type);
			return NCSCC_RC_FAILURE;
		}
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 pss_re_direct_sync(PSS_PWE_CB *pwe_cb, NCSCONTEXT i_msg, PSS_CKPT_DATA_TYPE ckpt_type,
			 NCS_MBCSV_ACT_TYPE act_type)
{
	PSS_CKPT_MSG msg;
	NCS_MBCSV_ARG mbcsv_arg;
	uns32 rc = NCSCC_RC_SUCCESS;
	...
	case PSS_CKPT_DATA_PLBCK_SSN_INFO:
		{
			...
			if (rc != NCSCC_RC_SUCCESS) {
				m_LOG_PSS_HDLN2_II(NCSFL_SEV_ERROR, PSS_HDLN_RE_SYNC_CKPT_EVT_POPULATE_FAIL,
						   pwe_cb->pwe_id, ckpt_type);
				return rc;
			}
		}
		break;
...
uns32 pss_duplicate_pcn_list(MAB_PSS_CLIENT_LIST *src, MAB_PSS_CLIENT_LIST *dest)
{
	...
	if ((dest->pcn = m_MMGR_ALLOC_MAB_PCN_STRING(len + 1)) == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_PCN_STRING_ALLOC_FAIL, "pss_duplicate_pcn_list()");
		return NCSCC_RC_FAILURE;
	}
	...
	for (p_src_tbl = src->tbl_list, pp_dst_tbl = &dest->tbl_list;
	     p_src_tbl != NULL; p_src_tbl = p_src_tbl->next, pp_dst_tbl = &(*pp_dst_tbl)->next) {
   ...
   /**dst = *src;*/
	...
	if (src->pcn != NULL) {
		...
		if (src->is_warmboot_ssn) {
			...
			strcpy((char *)&dst->info.alt_profile, (char *)&src->info.alt_profile);
		}
	}
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 pss_ckpt_enc_tbl_unbind(PSS_PWE_CB *pwe_cb, NCS_UBAID *uba, PSS_CKPT_TBL_UNBIND_MSG *unbind,
			      uns16 peer_mbcsv_version)
{
	EDU_ERR ederror = EDU_NORMAL;
	uns32 rc = NCSCC_RC_SUCCESS;
	...
	if (p8 == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_ENC_RESERVE_SPACE_FAIL, "pss_ckpt_enc_tbl_unbind()");
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 pss_ckpt_dec_tbl_unbind(PSS_PWE_CB *pwe_cb, NCS_UBAID *uba, PSS_CKPT_TBL_UNBIND_MSG *unbind,
			      uns16 peer_mbcsv_version)
{
	EDU_ERR ederror = EDU_NORMAL;
	MDS_DEST **pp_mdest = NULL, *p_mdest = NULL;
	...
	if (p8 == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_DEC_FLATTEN_SPACE_FAIL, "pss_ckpt_dec_tbl_unbind()");
		m_LOG_PSS_HDLN_I(NCSFL_SEV_ERROR, PSS_HDLN_MBCSV_TBL_ID_DEC_FAIL, pwe_cb->pwe_id);
		return NCSCC_RC_FAILURE;
	}
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 pss_ckpt_enc_oaa_down(PSS_PWE_CB *pwe_cb, NCS_UBAID *uba, PSS_CKPT_OAA_DOWN_MSG *oaa_down,
			    uns16 peer_mbcsv_version)
{
	EDU_ERR ederror = EDU_NORMAL;
	uns32 rc = NCSCC_RC_SUCCESS;
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 pss_ckpt_dec_oaa_down(PSS_PWE_CB *pwe_cb, NCS_UBAID *uba, PSS_CKPT_OAA_DOWN_MSG *oaa_down,
			    uns16 peer_mbcsv_version)
{
	EDU_ERR ederror = EDU_NORMAL;
	MDS_DEST **pp_mdest = NULL, *p_mdest = NULL;
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 pss_ckpt_enc_plbck_ssn_info(PSS_PWE_CB *pwe_cb, NCS_UBAID *uba, PSS_CKPT_PLBCK_SSN_INFO_MSG * plbck_ssn,
				  uns16 peer_mbcsv_version)
{
	uns32 rc = NCSCC_RC_SUCCESS;
	...
	if (p8 == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_ENC_RESERVE_SPACE_FAIL, "pss_ckpt_enc_plbck_ssn_info()");
	...
	ncs_enc_claim_space(uba, 4);
	m_LOG_PSS_HDLN_II(NCSFL_SEV_DEBUG, PSS_HDLN_MBCSV_MIB_IDX_LEN_ENC_SUCCESS, pwe_cb->pwe_id,
			  plbck_ssn->mib_idx.i_inst_len);
	...
	if (plbck_ssn->is_warmboot_ssn) {
		...
		if (plbck_ssn->pcn == NULL) {
			...
			/* Encode MDS_DEST */
			rc = m_NCS_EDU_VER_EXEC(&pwe_cb->p_pss_cb->edu_hdl, ncs_edp_mds_dest, uba,
						EDP_OP_TYPE_ENC, &plbck_ssn->info.info2.mds_dest, &ederror,
						peer_mbcsv_version);
			if (rc != NCSCC_RC_SUCCESS) {
				m_LOG_PSS_HDLN_I(NCSFL_SEV_ERROR, PSS_HDLN_MBCSV_MDEST_ENC_FAIL, pwe_cb->pwe_id);
				return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
			...
			for (wbreq = &plbck_ssn->info.info2.wb_req; wbreq != NULL; wbreq = wbreq->next, ++wb_req_cnt) {
				p8 = ncs_enc_reserve_space(uba, 1);
				if (p8 == NULL) {
					m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_ENC_RESERVE_SPACE_FAIL,
							  "pss_ckpt_enc_plbck_ssn_info()");
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 pss_ckpt_dec_plbck_ssn_info(PSS_PWE_CB *pwe_cb, NCS_UBAID *uba, PSS_CKPT_PLBCK_SSN_INFO_MSG * plbck_ssn,
				  uns16 peer_mbcsv_version)
{
	uns32 rc = NCSCC_RC_SUCCESS, i = 0;
	...
	if (p8 == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_DEC_FLATTEN_SPACE_FAIL, "pss_ckpt_dec_plbck_ssn_info()");
	...
	ncs_dec_skip_space(uba, 4);
	m_LOG_PSS_HDLN_II(NCSFL_SEV_DEBUG, PSS_HDLN_MBCSV_MIB_IDX_LEN_DEC_SUCCESS, pwe_cb->pwe_id,
			  plbck_ssn->mib_idx.i_inst_len);
	...
	if (plbck_ssn->is_warmboot_ssn) {
		...
		if (plbck_ssn->pcn == NULL) {
			...
			ncs_dec_skip_space(uba, 4);
			m_LOG_PSS_HDLN_II(NCSFL_SEV_DEBUG, PSS_HDLN_MBCSV_DEC_SEQ_NUM_DONE, pwe_cb->pwe_id,
					  plbck_ssn->info.info2.seq_num);
			...
			for (i = 0; i < wb_req_cnt; i++, pp_wbreq = &(*pp_wbreq)->next) {
				if (i != 0) {
					if ((tmp = m_MMGR_ALLOC_MAB_PSS_WARMBOOT_REQ) == NULL) {
						m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR,
...
uns32 pss_ckpt_enc_reload_pssvlibconf(NCS_UBAID *uba, char *file_name)
{
	...
	while (((read = fscanf(fh, "%s %s", (char *)&libname, (char *)&appname)) == 2) && (read != EOF)) {
		if (p_line_cnt == NULL) {
...
uns32 pss_ckpt_dec_n_updt_reload_pssvlibconf(PSS_PWE_CB *pwe_cb, NCS_UBAID *uba)
{
	...
	char file_libname[16][256] = { "\0" }, file_appname[16][64] = {
	"\0"};
	int loop = 0, read = 0;
...
uns32 pss_encode_pcn_list(NCS_UBAID *uba, MAB_PSS_CLIENT_LIST *pcn_list)
{
	...
	if (p_tbl_cnt == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_ENC_RESERVE_SPACE_FAIL, "pss_encode_pcn_list()");
		return NCSCC_RC_FAILURE;
	}
	ncs_enc_claim_space(uba, 2);
...
uns32 pss_decode_pcn_list(NCS_UBAID *uba, MAB_PSS_CLIENT_LIST *pcn_list)
{
	...
	if (data == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_DEC_FLATTEN_SPACE_FAIL, "pss_decode_pcn_list()");
		return NCSCC_RC_FAILURE;
	}
	tbl_cnt = ncs_decode_16bit(&data);
	ncs_dec_skip_space(uba, 2);
	...
	for (i = 0; i < tbl_cnt; i++, pp_tbl = &(*pp_tbl)->next) {
		if ((p_tbl = m_MMGR_ALLOC_MAB_PSS_TBL_LIST) == NULL) {
			m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_MAB_PSS_TBL_LIST_ALLOC_FAIL, "pss_decode_pcn_list()");
...
uns32 pss_resume_active_role_activity(MAB_MSG *msg)
{
	...
	if (pwe_cb->curr_plbck_ssn_info.plbck_ssn_in_progress) {
		...
		if (pwe_cb->curr_plbck_ssn_info.is_warmboot_ssn) {
			...
			memset((char *)&pwe_cb->p_pss_cb->existing_profile, '\0', NCS_PSS_MAX_PROFILE_NAME);
...
uns32 pss_re_update_pend_wbreq_info(PSS_PWE_CB *pwe_cb,
				    PSS_CKPT_PEND_WBREQ_TO_BAM_MSG *pend_wbreq_to_bam, NCS_MBCSV_ACT_TYPE action)
{
	if (action == NCS_MBCSV_ACT_RMV) {
		pss_del_entry_frm_spcn_wbreq_pend_list(pwe_cb, pend_wbreq_to_bam->pcn_list.pcn);
	} else {
		pss_add_entry_to_spcn_wbreq_pend_list(pwe_cb, pend_wbreq_to_bam->pcn_list.pcn);
	}
	return NCSCC_RC_SUCCESS;
}
...
uns32 pss_encode_str(NCS_UBAID *uba, char *str_ptr)
{
	...
	if (data == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_ENC_RESERVE_SPACE_FAIL, "pss_encode_str()");
		return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
	}
	...
	if (len != 0) {
		if (ncs_encode_n_octets_in_uba(uba, str_ptr, len) != NCSCC_RC_SUCCESS) {
			m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_ENC_OCTETS_FAIL, "pss_encode_str()");
			return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
		}
	}
...
uns32 pss_decode_str(NCS_UBAID *uba, char *p_str)
{
	...
	if (data == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_DEC_FLATTEN_SPACE_FAIL, "pss_decode_str()");
		return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
	}
...
uns32 pss_encode_pcn(NCS_UBAID *uba, char *pcn)
{
	...
	data = ncs_enc_reserve_space(uba, 2);
	if (data == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_ENC_RESERVE_SPACE_FAIL, "pss_encode_pcn()");
		return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
	}
	ncs_encode_16bit(&data, len);
	ncs_enc_claim_space(uba, 2);
	...
	if (len != 0) {
		if (ncs_encode_n_octets_in_uba(uba, pcn, len) != NCSCC_RC_SUCCESS) {
			m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_ENC_OCTETS_FAIL, "pss_encode_pcn()");
...
uns32 pss_decode_pcn(NCS_UBAID *uba, char **pcn)
{
	...
	data = ncs_dec_flatten_space(uba, (uns8 *)&len, 2);
	if (data == NULL) {
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_UBA_DEC_FLATTEN_SPACE_FAIL, "pss_decode_pcn()");
		return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
	}
	len = ncs_decode_16bit(&data);
	ncs_dec_skip_space(uba, 2);
	...
	if (len != 0) {
		if (*pcn == NULL) {
			if ((*pcn = m_MMGR_ALLOC_MAB_PCN_STRING(len + 1)) == NULL) {
				m_LOG_PSS_MEMFAIL(NCSFL_SEV_ERROR, PSS_MF_PCN_STRING_ALLOC_FAIL, "pss_decode_pcn()");
				return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
			}
		}
		memset(*pcn, '\0', len + 1);
		if (ncs_decode_n_octets_from_uba(uba, *pcn, len) != NCSCC_RC_SUCCESS) {
			m_LOG_PSS_HDLN_STR(NCSFL_SEV_ERROR, PSS_HDLN_MBCSV_DEC_PCN_FAIL, "pss_decode_pcn()");
			m_MMGR_FREE_MAB_PCN_STRING(*pcn);
			return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
		}
		m_LOG_PSS_HDLN_STR(NCSFL_SEV_DEBUG, PSS_HDLN_MBCSV_DEC_PCN_SUCCESS, (*pcn));
	} else {
		if (*pcn != NULL) {
...
static uns32 pss_red_err_ind_handle(NCS_MBCSV_CB_ARG *cbk_arg)
{
	...
	if ((pwe_cb->p_pss_cb->ha_state == SA_AMF_HA_ACTIVE) || (pwe_cb->p_pss_cb->ha_state == SA_AMF_HA_QUIESCING)) {
