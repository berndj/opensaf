...
static uns32 pss_store_reformatting_higher_to_2(PSS_CB *inst, uns32 ts_hdl, PSS_TABLE_PATH_RECORD *rec);
...
uns32 pss_tbl_details_header_write(PSS_CB *inst, uns32 ts_hdl, long tfile_hdl, PSS_TABLE_PATH_RECORD *rec)
{
	uns32 retval = NCSCC_RC_SUCCESS;
	...
	m_NCS_PSSTS_FILE_WRITE(inst->pssts_api, ts_hdl, retval, tfile_hdl, PSS_TABLE_DETAILS_HEADER_LEN, (char *)&hdr);
...
uns32 pss_log_pss_return_codes(PSS_RETURN_CODES pss_retval, char *src_file_name, char *dest_file_name)
{
	switch (pss_retval) {
...
uns32 pss_store_reformatting_1ext_to_higher(PSS_CB *inst, uns32 ts_hdl, PSS_TABLE_PATH_RECORD *rec)
{
	...
	if (inst_file.info.file_exists.o_file_exists == TRUE) {
		...
		snprintf(source_file_path, sizeof(source_file_path) - 1, "%s%d/%s/%d/%s/%d",
			 NCS_PSS_DEF_PSSV_ROOT_PATH, rec->ps_format_version, rec->profile, rec->pwe_id, rec->pcn,
			 rec->tbl_id);
...
/*       m_NCS_PSSTS_FILE_DELETE(inst->pssts_api, ts_hdl, retval, rec->profile, rec->pwe_id, rec->pcn, rec->tbl_id); */
		...
		m_NCS_PSSTS_FILE_DELETE(inst->pssts_api, ts_hdl, retval, rec->profile, rec->pwe_id, rec->pcn,
					rec->tbl_id);
...
uns32 pss_store_reformatting_higher_to_2(PSS_CB *inst, uns32 ts_hdl, PSS_TABLE_PATH_RECORD *rec)
{
	uns32 retval = NCSCC_RC_SUCCESS;
	uns32 final_retval = NCSCC_RC_SUCCESS;
	PSS_RETURN_CODES pss_retval = PSSRC_SUCCESS;
	...
	m_LOG_PSS_HDLN_I(NCSFL_SEV_DEBUG, PSS_HDLN_STORE_REFORMATTING_FROM_HIGHER_TO_2_START, rec->tbl_id);
	...
	m_NCS_PSSTS_FILE_OPEN(inst->pssts_api, ts_hdl, retval, rec->profile,
			      rec->pwe_id, rec->pcn, rec->tbl_id, NCS_PSSTS_FILE_PERM_READ, file_hdl);
	if (NCSCC_RC_SUCCESS != retval) {
		m_LOG_PSS_HDLN_I(NCSFL_SEV_ERROR, PSS_HDLN_ERR_TABLE_FOUND, rec->tbl_id);
		m_LOG_PSS_STORE(NCSFL_SEV_ERROR, PSS_MIB_OPEN_FAIL, rec->pwe_id, rec->pcn, rec->tbl_id);
		return NCSCC_RC_FAILURE;
	}
...
uns32 pss_store_reformatting_higher_to_1ext(PSS_CB *inst, uns32 ts_hdl, PSS_TABLE_PATH_RECORD *rec)
{
	uns32 retval = NCSCC_RC_SUCCESS;
	uns32 final_retval = NCSCC_RC_SUCCESS;
	PSS_RETURN_CODES pss_retval = PSSRC_SUCCESS;
	...
	m_LOG_PSS_HDLN_I(NCSFL_SEV_DEBUG, PSS_HDLN_STORE_REFORMATTING_FROM_HIGHER_TO_1EXT_START, rec->tbl_id);
...
uns32 pss_verify_n_mib_reformat(PSS_CB *inst, uns32 ts_hdl, PSS_TABLE_PATH_RECORD *rec)
{
	uns32 retval = NCSCC_RC_SUCCESS;
	uns32 final_retval = NCSCC_RC_SUCCESS;
	long file_hdl = 0;
	...
	if (inst_file.info.file_exists.o_file_exists == FALSE) {
		...
		if (NCSCC_RC_SUCCESS != retval) {
			m_LOG_PSS_HDLN_I(NCSFL_SEV_ERROR, PSS_HDLN_ERR_TABLE_FOUND, rec->tbl_id);
			m_LOG_PSS_STORE(NCSFL_SEV_ERROR, PSS_MIB_FILE_SIZE_OP_FAIL, rec->pwe_id, rec->pcn, rec->tbl_id);
			if (file_hdl != 0)
				m_NCS_PSSTS_FILE_CLOSE(inst->pssts_api, ts_hdl, retval, file_hdl);
		...
		if (hdr.table_version == tbl_info->i_tbl_version) {
			if ((hdr.max_row_length == tbl_info->max_row_length) &&
				...
				printf("\n No change in the table_version and the table details are consistent: %d",
				       rec->tbl_id);
				if (file_hdl != 0)
					m_NCS_PSSTS_FILE_CLOSE(inst->pssts_api, ts_hdl, retval, file_hdl);
				return NCSCC_RC_SUCCESS;
				...
				m_LOG_PSS_TBL_DTLS(NCSFL_SEV_ERROR, PSS_HDLN_PROPERTIES_CHANGED_BUT_NOT_TABLE_VERSION,
						   hdr.table_version, hdr.max_row_length, hdr.max_key_length,
						   hdr.bitmap_length, tbl_info->i_tbl_version, tbl_info->max_row_length,
						   tbl_info->max_key_length, tbl_info->bitmap_length);
				printf("\nINCONSISTENT: Table_version is not changed but table details are changed: %d",
				       rec->tbl_id);
				if (file_hdl != 0)
					m_NCS_PSSTS_FILE_CLOSE(inst->pssts_api, ts_hdl, retval, file_hdl);
				return NCSCC_RC_FAILURE;
		       ...
		       PSS_TABLE_PATH_RECORD *rec, PSS_TABLE_DETAILS_HEADER *hdr)
{
	...
	m_NCS_PSSTS_FILE_OPEN(inst->pssts_api, ts_hdl, retval, "_ISU",
			      rec->pwe_id, rec->pcn, rec->tbl_id, NCS_PSSTS_FILE_PERM_WRITE, tfile_hdl);
	if (NCSCC_RC_SUCCESS != retval) {
		m_LOG_PSS_STORE(NCSFL_SEV_ERROR, PSS_MIB_OPEN_FAIL, rec->pwe_id, rec->pcn, rec->tbl_id);
		return NCSCC_RC_FAILURE;
	}
...
/* Framing the new bitmap */
	...
	if (NULL == new_bitmap) {
		m_LOG_PSS_HDLN_I(NCSFL_SEV_ERROR, PSS_HDLN_ERR_TABLE_FOUND, rec->tbl_id);
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_OCT_ALLOC_FAIL, "pss_mib_reformat(): new_bitmap");
		m_NCS_PSSTS_FILE_CLOSE(inst->pssts_api, ts_hdl, retval, tfile_hdl);
		return NCSCC_RC_FAILURE;
	}
...
/* Required for being compatible with higher versions 
   where there could be extensions in Table details header */
	if (hdr->header_len > PSS_TABLE_DETAILS_HEADER_LEN) {
		printf("\nhdr->header_len: %d > PSS_TABLE_DETAILS_HEADER_LEN: %d ", hdr->header_len,
		       PSS_TABLE_DETAILS_HEADER_LEN);
	...
	if (NULL == (read_buff = m_MMGR_ALLOC_PSS_OCT(read_buf_size))) {
		m_LOG_PSS_HDLN_I(NCSFL_SEV_ERROR, PSS_HDLN_ERR_TABLE_FOUND, rec->tbl_id);
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_OCT_ALLOC_FAIL, "pss_mib_reformat(): read_buff");
		m_MMGR_FREE_PSS_OCT(new_bitmap);
		m_NCS_PSSTS_FILE_CLOSE(inst->pssts_api, ts_hdl, retval, tfile_hdl);
		return NCSCC_RC_FAILURE;
	}
	if (NULL == (write_buff = m_MMGR_ALLOC_PSS_OCT(write_buf_size))) {
		m_LOG_PSS_HDLN_I(NCSFL_SEV_ERROR, PSS_HDLN_ERR_TABLE_FOUND, rec->tbl_id);
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_OCT_ALLOC_FAIL, "pss_mib_reformat(): write_buff");
		m_MMGR_FREE_PSS_OCT(new_bitmap);
	...
	while (rem_num_rows != 0) {
		...
		if (NCSCC_RC_SUCCESS != retval || bytes_read != read_size) {
			...
			m_MMGR_FREE_PSS_OCT(new_bitmap);
			m_NCS_PSSTS_FILE_CLOSE(inst->pssts_api, ts_hdl, retval, tfile_hdl);
			m_LOG_PSS_STORE(NCSFL_SEV_ERROR, PSS_MIB_READ_FAIL, rec->pwe_id, rec->pcn, rec->tbl_id);
			return NCSCC_RC_FAILURE;
		}
		...
		m_NCS_PSSTS_FILE_WRITE(inst->pssts_api, ts_hdl, retval, tfile_hdl, write_size, (char *)write_buff);
		if (NCSCC_RC_SUCCESS != retval) {
			m_MMGR_FREE_PSS_OCT(read_buff);
			m_MMGR_FREE_PSS_OCT(write_buff);
			m_MMGR_FREE_PSS_OCT(new_bitmap);
...
*                                                                            *
\****************************************************************************/
uns32 pss_reformat(PSS_CB *inst, PSS_REFORMAT_TYPE reformat_type, uns32 ts_hdl, uns32 cur_ps_format)
{
	...
	do {			/* Loop for all profiles in the cur_ps_format */
		...
		if (NCSCC_RC_SUCCESS == get_profile_retval) {
			...
			if (buff_len == 0) {
				...
				m_LOG_PSS_INFO_C(NCSFL_SEV_NOTICE, PSS_INFO_NO_CLIENTS_FOUND_IN_PROFILE, profile_next);
				break;
			}
			...
			for (j = 0; j < pwe_cnt; j++) {	/* Loop for all PWEs */
				...
				for (k = 0; k < pcn_cnt; k++) {	/* Loop for all PCNs */
					...
					if (NCSCC_RC_SUCCESS !=
					    ncs_decode_n_octets_from_uba(&lcl_uba, (char *)&pcn, str_len)) {
					...
					if (table_cnt > 0) {
						while (table_cnt--) {
							...
							if (NULL != (tbl_info = inst->mib_tbl_desc[tbl_id])) {
								...
								memcpy(&ps_file_record.profile, &profile_next,
								       sizeof(profile_next));
								...
								case PSS_REFORMAT_TYPE_STORE_1_TO_1EXT:
									if (NCSCC_RC_SUCCESS !=
									    pss_store_reformatting_1_to_1ext(inst,
													     &ps_file_record))
									{
										final_retval = NCSCC_RC_FAILURE;
									...
									m_LOG_PSS_HDLN_I(NCSFL_SEV_DEBUG,
								...
								case PSS_REFORMAT_TYPE_STORE_HIGHER_TO_1EXT:
									if (NCSCC_RC_SUCCESS !=
									    pss_store_reformatting_higher_to_1ext(inst,
														  ts_hdl,
														  &ps_file_record))
									{
										final_retval = NCSCC_RC_FAILURE;
								...
								case PSS_REFORMAT_TYPE_STORE_1EXT_TO_HIGHER:
									if (NCSCC_RC_SUCCESS !=
									    pss_store_reformatting_1ext_to_higher(inst,
														  ts_hdl,
														  &ps_file_record))
									{
										final_retval = NCSCC_RC_FAILURE;
								...
								case PSS_REFORMAT_TYPE_STORE_HIGHER_TO_2:
									if (NCSCC_RC_SUCCESS !=
									    pss_store_reformatting_higher_to_2(inst,
													       ts_hdl,
													       &ps_file_record))
									{
										final_retval = NCSCC_RC_FAILURE;
								...
								case PSS_REFORMAT_TYPE_MIB:
									if (NCSCC_RC_SUCCESS !=
									    pss_verify_n_mib_reformat(inst, ts_hdl,
												      &ps_file_record))
									{
										m_NCS_PSSTS_FILE_DELETE(inst->pssts_api,
													ts_hdl, retval,
													"_ISU", pwe_id,
													pcn, tbl_id);
										final_retval = NCSCC_RC_FAILURE;
								...
								m_NCS_PSSTS_FILE_DELETE(inst->pssts_api, ts_hdl, retval,
											profile_next, pwe_id, pcn,
											tbl_id);
							}
	...
	if (NCSCC_RC_NO_INSTANCE == get_profile_retval) {
		...
		if (PSS_REFORMAT_TYPE_STORE_1_TO_1EXT != reformat_type && PSS_REFORMAT_TYPE_MIB != reformat_type) {
			...
			snprintf(source_file_path, sizeof(source_file_path) - 1, "%s%d", root_dir, cur_ps_format);
			printf("\n source_file_path: %s", source_file_path);
...
uns32 pss_check_n_reformat(PSS_CB *inst, uns32 ps_format_version)
{
	/*
	   For 3.0.b:
	...
	if (ps_format_version == 1) {
		...
		if (NCSCC_RC_SUCCESS !=
		    pss_reformat(inst, PSS_REFORMAT_TYPE_STORE_1EXT_TO_HIGHER, pwe.i_pssts_hdl, ps_format_version))
			return NCSCC_RC_FAILURE;
		...
		if (NCSCC_RC_SUCCESS !=
		    pss_reformat(inst, PSS_REFORMAT_TYPE_MIB, inst->pssts_hdl, PSS_PS_FORMAT_VERSION))
			return NCSCC_RC_FAILURE;
