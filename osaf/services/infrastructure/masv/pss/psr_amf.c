...
/* Stethascope of AMF */
void pss_amf_health_check(SaInvocationT, const SaNameT *, SaAmfHealthcheckKeyT *);

/* pest for Suicide */
void pss_amf_comp_terminate(SaInvocationT, const SaNameT *);

/* to create a new CSI or to change the state of an existing CSI */
void pss_amf_csi_set(SaInvocationT, const SaNameT *, SaAmfHAStateT, SaAmfCSIDescriptorT);

/* to remove one or more CSIs from PSS */
void pss_amf_csi_remove(SaInvocationT, const SaNameT *, const SaNameT *, SaAmfCSIFlagsT);

/* to create a CSI in ACTIVE state */
uns32 pss_amf_INIT_ACTIVE_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to move to ACTIVE state */
uns32 pss_amf_ACTIVE_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to create a CSI in STANDBY state */
uns32 pss_amf_INIT_STANDBY_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to STANDBY state */
uns32 pss_amf_STANDBY_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to QUIESCED state */
uns32 pss_amf_QUIESCED_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to QUIESCING state */
uns32 pss_amf_QUIESCING_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* from QUIESCING to QUIESCED */
uns32 pss_amf_QUIESCING_QUIESCED_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to create a new CSI */
void pss_amf_csi_new(SaInvocationT, const SaNameT *, SaAmfHAStateT, SaAmfCSIDescriptorT);

/* to do the role change for all the CSIs */
void pss_amf_csi_set_all(SaInvocationT, const SaNameT *, SaAmfHAStateT, SaAmfCSIDescriptorT);

/* do a role change for a specific CSI */
void pss_amf_csi_set_one(SaInvocationT, const SaNameT *, SaAmfHAStateT, SaAmfCSIDescriptorT);

/* to remove a single CSI */
SaAisErrorT pss_amf_csi_remove_one(const PSS_CSI_NODE *);

/* to create a new Environment */
uns32 pss_amf_csi_create_and_install_env(PW_ENV_ID, SaAmfHAStateT, SaAmfCSIDescriptorT);

/* to change the role of the VDEST */
uns32 pss_amf_csi_vdest_role_change(NCS_APP_AMF_CSI_ATTRIBS *, SaAmfHAStateT);

/* serach by the env-id in the global CSI list */
PSS_CSI_NODE *pss_amf_csi_list_find_envid(PW_ENV_ID);

/* search by CSI name in the global CSI list */
PSS_CSI_NODE *pss_amf_csi_list_find_csiname(const SaNameT *);

/* delink this node from the global CSI list */
PSS_CSI_NODE *pss_amf_csi_list_delink(const SaNameT *);

/* decodes the CSI attributes and gives the Environment ID */
PW_ENV_ID pss_amf_csi_attrib_decode(SaAmfCSIAttributeListT);

uns32 pss_amf_component_name_set(void);

static void pss_amf_sigusr2_handler(int i_sig_num);
...
*                   - gl_pss_amf_attribs                                     *
\****************************************************************************/
void pss_mbx_amf_process(SYSF_MBX *pss_mbx)
{
	...
	while (1) {
		...
		if (gl_pss_amf_attribs.handles.mbcsv_hdl != 0) {
			/* Use the following macro to set the saAmfSelectionObject in NCS_SEL_OBJ. */
			m_SET_FD_IN_SEL_OBJ((uns32)gl_pss_amf_attribs.handles.mbcsv_sel_obj, ncs_mbcsv_sel_obj);
		...
		if (count > 0) {
			...
			if ((gl_pss_amf_attribs.amf_attribs.amfHandle == 0) &&
				...
				if (saf_status != SA_AIS_ERR_TRY_AGAIN) {
					...
					m_NCS_SEL_OBJ_RMV_IND(gl_pss_amf_attribs.amf_attribs.sighdlr_sel_obj, TRUE,
							      TRUE);
			...
			/* AMF FD is selected */
				...
				if (saf_status != SA_AIS_OK) {
					m_LOG_PSS_ERROR_I(NCSFL_SEV_ERROR, PSS_ERR_AMF_DISPATCH_FAILED, saf_status);
			...
			/* MBCSv Interface requests */
				...
				if (status != NCSCC_RC_SUCCESS) {
					m_LOG_PSS_ERROR_I(NCSFL_SEV_ERROR, PSS_ERR_MBCSV_DISPATCH_FAILED, status);
					m_LOG_PSS_HEADLINE2(NCSFL_SEV_ERROR, PSS_HDLN_RETURN_FROM_THREAD_HANDLER);
					return;
				}
...
*/
				{
					...
					if (mmsg != NULL) {
						if ((NCS_APP_AMF_HA_STATE_STANDBY == gl_pss_amf_attribs.ha_state) &&
							...
							char addr_str[255] = { 0 };
							status =
							    pss_stdby_oaa_down_list_update(mmsg->fr_card, mmsg->yr_hdl,
											   PSS_STDBY_OAA_DOWN_BUFFER_ADD);
							if (status != NCSCC_RC_SUCCESS) {
								if (m_NCS_NODE_ID_FROM_MDS_DEST(mmsg->fr_card) == 0)
									sprintf(addr_str, "VDEST:%d",
										m_MDS_GET_VDEST_ID_FROM_MDS_DEST(mmsg->
														 fr_card));
								else
									sprintf(addr_str,
										"ADEST:node_id:%d, v1.pad16:%d, v1.vcard:%d",
										m_NCS_NODE_ID_FROM_MDS_DEST(mmsg->
													    fr_card), 0,
										(uns32)(mmsg->fr_card));
								ncs_logmsg(NCS_SERVICE_ID_PSS, PSS_LID_HDLN_C,
									   PSS_FC_HDLN, NCSFL_LC_HEADLINE,
							...
							if (status != NCSCC_RC_SUCCESS) {
								...
								m_LOG_PSS_ERROR_I(NCSFL_SEV_NOTICE,
										  PSS_ERR_DO_EVT_FAILED, status);
							} else
								m_LOG_PSS_HDLN2_I(NCSFL_SEV_DEBUG,
...
*/
			...
			/* log the error */
			m_LOG_PSS_ERROR_I(NCSFL_SEV_ERROR, PSS_ERR_SELECT_FAILED, count);
		}
	}			/* while(1) */
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
uns32 pss_amf_componentize(NCS_APP_AMF_ATTRIBS *amf_attribs, SaAisErrorT *o_saf_status)
{
	...
	memset(&ha_hdls, 0, sizeof(NCS_APP_AMF_HA_STATE_HANDLERS));
	...
	memset(&amf_clbks, 0, sizeof(SaAmfCallbacksT));
	...
	status = ncs_app_amf_attribs_init(amf_attribs,
					  ...
					  m_PSS_DISPATCH,
					  m_PSS_HB_INVOCATION_TYPE, m_PSS_RECOVERY, &ha_hdls, &amf_clbks);
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_LOG_PSS_ERROR_I(NCSFL_SEV_ERROR, PSS_ERR_AMF_ATTRIBS_INIT_FAILED, status);
		/* reset the attribs */
...
\*****************************************************************************/
void pss_amf_health_check(SaInvocationT invocation, const SaNameT *compName, SaAmfHealthcheckKeyT *healthcheckKey)
{
	SaAisErrorT saf_status = SA_AIS_OK;
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
uns32 pss_amf_prepare_will(void)
{
	uns32 status;
	...
	if (status != NCSCC_RC_SUCCESS) {
		...
		m_LOG_PSS_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR, PSS_ERR_VDEST_DESTROY_FAILED, status);
		return status;
	}
 ...
 *  NOTE:                                                                     * 
 \****************************************************************************/
void pss_amf_comp_terminate(SaInvocationT invocation, const SaNameT *compName)
{
	SaAisErrorT saf_status = SA_AIS_OK;
	PSS_CB *inst = NULL;
		...
		const SaNameT *compName, SaAmfHAStateT haState, SaAmfCSIDescriptorT csiDescriptor)
{
	...
	m_LOG_PSS_CSI_DETAILS(NCSFL_SEV_NOTICE,
			      PSS_HDLN_AMF_CSI_DETAILS, csiDescriptor.csiFlags, tCompName, tCsiName, haState);
	...
	default:
		/* log the error code */
		m_LOG_PSS_ERROR_I(NCSFL_SEV_ERROR, PSS_ERR_AMF_CSI_FLAGS_ILLEGAL, csiDescriptor.csiFlags);
		saAmfResponse(gl_pss_amf_attribs.amf_attribs.amfHandle, invocation, SA_AIS_ERR_FAILED_OPERATION);
		break;
	}
	return;
}
		...
		const SaNameT *compName, SaAmfHAStateT haState, SaAmfCSIDescriptorT csiDescriptor)
{
	...
	if ((env_id == m_PSS_DEFAULT_ENV_ID) && (csi != NULL)) {
		...
		memcpy(&csi->csi_info.work_desc.value, &csiDescriptor.csiName.value, csi->csi_info.work_desc.length);
		saf_status = SA_AIS_OK;
		goto enough;
	}
	...
	if (status == NCSCC_RC_SUCCESS) {
		...
		m_LOG_PSS_STATE_CHG(NCSFL_SEV_NOTICE,
				    PSS_HDLN_AMF_CSI_STATE_CHG_SUCCESS, NCS_APP_AMF_HA_STATE_NULL, haState);
		saf_status = SA_AIS_OK;
	} else {
		saf_status = SA_AIS_ERR_FAILED_OPERATION;

		/* log the environment ID for which CSI creation failed */
		m_LOG_PSS_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL, PSS_ERR_CSI_ADD_ONE_FAILED, status, env_id);

		/* log the state change failure */
		m_LOG_PSS_STATE_CHG(NCSFL_SEV_ERROR,
				    PSS_HDLN_AMF_CSI_STATE_CHG_FAILED, NCS_APP_AMF_HA_STATE_NULL, haState);
	}
 ...
 *  NOTE:                                                                     * 
 \****************************************************************************/
uns32 pss_amf_csi_create_and_install_env(PW_ENV_ID envid, SaAmfHAStateT haState, SaAmfCSIDescriptorT csiDescriptor)
{
	PSS_CSI_NODE *csi = NULL;
		    ...
		    const SaNameT *compName, SaAmfHAStateT haState, SaAmfCSIDescriptorT csiDescriptor)
{
	...
	while (csi_node) {	/* for all the CSIs in PSS */
		...
		status =
		    gl_pss_amf_attribs.amf_attribs.ncs_app_amf_hastate_dispatch[csi_node->csi_info.
										ha_state][haState] ((void *)csi_node,
												    csi_node->csi_info.
												    env_id,
												    csiDescriptor);
		/* update the state */
	...
	if (saf_status == SA_AIS_OK) {
		...
		m_LOG_PSS_STATE_CHG(NCSFL_SEV_NOTICE, PSS_HDLN_AMF_CSI_STATE_CHG_SUCCESS, prevState, haState);
	} else {
		/* log the state change failure */
		m_LOG_PSS_STATE_CHG(NCSFL_SEV_ERROR, PSS_HDLN_AMF_CSI_STATE_CHG_FAILED, prevState, haState);
	}
	...
	if (saf_status == SA_AIS_OK && prevState == SA_AMF_HA_STANDBY && haState == SA_AMF_HA_ACTIVE) {
		...
		if (ps_cnt > 2) {
			...
			if (tmp_ps0 == PSS_PS_FORMAT_VERSION) {
				...
				/* if none of the formats returned by scandir is PSS_PS_FORMAT_VERSION, return FAILURE */
				m_LOG_PSS_STR(NCSFL_SEV_CRITICAL,
					      "pss_check_n_reformat(): Invalid formats of persistent store found");
			}
		}
		for (; ps_cnt > 0; ps_cnt--)
			free(ps_list[ps_cnt - 1]);
		...
		if (ps_format_version != 0) {
			if (NCSCC_RC_SUCCESS != pss_check_n_reformat(gl_pss_amf_attribs.pss_cb, ps_format_version)) {
				saAmfComponentErrorReport(gl_pss_amf_attribs.amf_attribs.amfHandle, &gl_pss_amf_attribs.amf_attribs.compName, 0,	/* errorDetectionTime; AvNd will provide this value */
		    ...
		    const SaNameT *compName, SaAmfHAStateT haState, SaAmfCSIDescriptorT csiDescriptor)
{
	...
	status =
	    gl_pss_amf_attribs.amf_attribs.ncs_app_amf_hastate_dispatch[csi->csi_info.ha_state][haState] ((void *)csi,
													  csi->csi_info.
													  env_id,
													  csiDescriptor);
	if (status == NCSCC_RC_SUCCESS) {
...
\*****************************************************************************/
void
pss_amf_csi_remove(SaInvocationT invocation, const SaNameT *compName, const SaNameT *csiName, SaAmfCSIFlagsT csiFlags)
{
	SaAisErrorT saf_status = SA_AIS_ERR_FAILED_OPERATION;
	PSS_CSI_NODE *csi_node = NULL;
...
uns32 pss_amf_INIT_ACTIVE_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	return pss_amf_csi_create_and_install_env(envid, SA_AMF_HA_ACTIVE, csiDescriptor);
}
...
\*****************************************************************************/
/* Go to ACTIVE State */
uns32 pss_amf_ACTIVE_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	...
	if (csi_node->pwe_cb->pss_stdby_oaa_down_buffer != NULL) {
		...
		m_LOG_PSS_STR(NCSFL_SEV_NOTICE,
			      "pss_amf_ACTIVE_process:Cleaning up the stale pointers before change role to active");
		while (buffer_node) {
			char addr_str[255] = { 0 };
			if (pss_handle_oaa_down_event(csi_node->pwe_cb, &buffer_node->oaa_addr) != NCSCC_RC_SUCCESS) {
				if (m_NCS_NODE_ID_FROM_MDS_DEST(buffer_node->oaa_addr) == 0)
					sprintf(addr_str, "VDEST:%d",
						m_MDS_GET_VDEST_ID_FROM_MDS_DEST(buffer_node->oaa_addr));
				else
					sprintf(addr_str, "ADEST:node_id:%d, v1.pad16:%d, v1.vcard:%d",
						m_NCS_NODE_ID_FROM_MDS_DEST(buffer_node->oaa_addr), 0,
						(uns32)(buffer_node->oaa_addr));
				ncs_logmsg(NCS_SERVICE_ID_PSS, PSS_LID_HDLN_C, PSS_FC_HDLN, NCSFL_LC_HEADLINE,
					   NCSFL_SEV_NOTICE, NCSFL_TYPE_TIC, PSS_HDLN_OAA_DOWN_EVENT_FAILED, addr_str);
			} else {
				if (m_NCS_NODE_ID_FROM_MDS_DEST(buffer_node->oaa_addr) == 0)
					sprintf(addr_str, "VDEST:%d",
						m_MDS_GET_VDEST_ID_FROM_MDS_DEST(buffer_node->oaa_addr));
					...
					sprintf(addr_str, "ADEST:node_id:%d, v1.pad16:%d, v1.vcard:%d",
						...
						(uns32)(buffer_node->oaa_addr));
				ncs_logmsg(NCS_SERVICE_ID_PSS, PSS_LID_HDLN_C, PSS_FC_HDLN, NCSFL_LC_HEADLINE,
					   NCSFL_SEV_NOTICE, NCSFL_TYPE_TIC, PSS_HDLN_OAA_DOWN_EVENT_SUCCESS, addr_str);
			}
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
uns32 pss_amf_INIT_STANDBY_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	return pss_amf_csi_create_and_install_env(envid, SA_AMF_HA_STANDBY, csiDescriptor);
}
...
\*****************************************************************************/
/* Go to STANDBY State */
uns32 pss_amf_STANDBY_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	PSS_CSI_NODE *csi_node = (PSS_CSI_NODE *)data;
	uns32 retval = NCSCC_RC_SUCCESS;
...
uns32 pss_amf_QUIESCED_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	PSS_CSI_NODE *csi_node = (PSS_CSI_NODE *)data;
	uns32 retval = NCSCC_RC_SUCCESS;
...
uns32 pss_amf_QUIESCING_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	PSS_CSI_NODE *csi_node = (PSS_CSI_NODE *)data;
	uns32 retval = NCSCC_RC_SUCCESS;
...
uns32 pss_amf_QUIESCING_QUIESCED_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	/* nothing to do */
	return NCSCC_RC_SUCCESS;
}
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
SaAisErrorT pss_amf_csi_remove_one(const PSS_CSI_NODE *csi_node)
{
	NCSMDS_INFO info;
	uns32 status;
	...
	if (status != NCSCC_RC_SUCCESS) {
		...
		m_LOG_PSS_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   PSS_ERR_MDS_UNINSTALL_FAILED, status, csi_node->csi_info.env_id);
		goto enough;
	}
	...
	if (status != NCSCC_RC_SUCCESS) {
		...
		m_LOG_PSS_ERROR_II(NCSFL_LC_FUNC_RET_FAIL, NCSFL_SEV_CRITICAL,
				   PSS_ERR_PSSTBL_DESTROY_FAILED, status, csi_node->csi_info.env_id);
		goto enough;
	}
	...
	if (del_me == NULL) {
		m_LOG_PSS_ERROR_I(NCSFL_SEV_ERROR, PSS_ERR_CSI_DELINK_FAILED, env_id);
		goto enough;
	}
 ...
 *  NOTE:                                                                     * 
 \****************************************************************************/
SaAisErrorT pss_amf_csi_remove_all(void)
{
 ...
 *  NOTE:                                                                     * 
 \****************************************************************************/
PSS_CSI_NODE *pss_amf_csi_install(PW_ENV_ID envid, SaAmfHAStateT haState)
{
	uns32 status;
	...
	if ((envid == 0) || (envid > NCSMDS_MAX_PWES)) {
		/* log the error */
		m_LOG_PSS_ERROR_NO_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_HEADLINE, PSS_ERR_AMF_INVALID_PWE_ID);
		return NULL;
	}
	...
	if (gl_pss_amf_attribs.handles.pss_cb_hdl == 0) {
		...
		m_LOG_PSS_ERROR_NO_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_HEADLINE, PSS_ERR_CREATE_LM_API_NOT_YET_CALLED);
		return NULL;
	}
	...
	if (csi == NULL) {
		...
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_CSI_NODE_ALLOC_FAILED, "pss_amf_csi_install()");
		return csi;
	}
	...
	if ((pwe_cb = m_MMGR_ALLOC_PSS_PWE_CB) == NULL) {
		...
		m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_PWE_CB_ALLOC_FAIL, "pss_amf_csi_install()");
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_PSS_ERROR_II(NCSFL_LC_FUNC_RET_FAIL, NCSFL_SEV_ERROR, PSS_ERR_PWE_CB_INIT_FAILED, status, envid);
		m_MMGR_FREE_PSS_CSI_NODE(csi);
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_PSS_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR, PSS_ERR_VDEST_ROLE_CHANGE_FAILED, status);
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_PSS_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR, PSS_ERR_MDS_PWE_INSTALL_FAILED, status);
		m_MMGR_FREE_PSS_CSI_NODE(csi);
		m_MMGR_FREE_PSS_PWE_CB(pwe_cb);
		return NULL;
	}
	...
	status = ncsmds_api(&info);
	if (status != NCSCC_RC_SUCCESS) {
		m_LOG_PSS_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR, PSS_ERR_MDS_SUBSCRIBE_FAILED, status);
		m_MMGR_FREE_PSS_CSI_NODE(csi);
		if (envid == m_PSS_DEFAULT_ENV_ID) {
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
uns32 pss_amf_csi_vdest_role_change(NCS_APP_AMF_CSI_ATTRIBS *csi_info, SaAmfHAStateT haState)
{
	uns32 status;
	NCSMDS_INFO info;
	NCSVDA_INFO vda_info;
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_PSS_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR, PSS_ERR_MDS_PWE_QUERY_FAILED, status);
		return status;
	}
 ...
 *  NOTE:                                                                     * 
 \****************************************************************************/
PSS_CSI_NODE *pss_amf_csi_list_find_envid(PW_ENV_ID env_id)
{
	PSS_CSI_NODE *csi_node = NULL;
 ...
 *  NOTE:                                                                     * 
 \****************************************************************************/
PSS_CSI_NODE *pss_amf_csi_list_find_csiname(const SaNameT *find_me)
{
	SaNameT t_find_me;
...
*  NOTE:                                                                     * 
\****************************************************************************/
PSS_CSI_NODE *pss_amf_csi_list_delink(const SaNameT *csiName)
{
	SaNameT t_del_me;
	PSS_CSI_NODE *del_me = NULL;
...
*  NOTE:                                                                     * 
\****************************************************************************/
void pss_amf_csi_list_add_front(PSS_CSI_NODE *new_csi)
{
	/* add in the front */
 ...
 *  NOTE:                                                                     * 
 \****************************************************************************/
PW_ENV_ID pss_amf_csi_attrib_decode(SaAmfCSIAttributeListT csiAttr)
{
	uns32 decoded_env = 0;
	...
	if (csiAttr.attr == NULL) {
		...
		m_LOG_PSS_ERROR_NO_DATA(NCSFL_SEV_ERROR, NCSFL_LC_HEADLINE, PSS_ERR_AMF_CSI_ATTRIBS_NULL);
		return 0;
	}
	...
	if (strcmp(csiAttr.attr->attrName, "Env-Id") != 0) {
		/* log that CSI Attribute Name/Value is empty */
		m_LOG_PSS_ERROR_NO_DATA(NCSFL_SEV_ERROR, NCSFL_LC_HEADLINE, PSS_ERR_AMF_CSI_ATTRIBS_NAME_INVALID);
		return 0;
	}

	/* good amount of input validation, now get to the work */
	/* log the received attribute and value pair */
	sprintf(attrName, "%s:%s\n", "pss_amf_csi_attrib_decode():attribute name", csiAttr.attr->attrName);
	sprintf(attrValue, "%s:%s\n", "pss_amf_csi_attrib_decode():attribute value", csiAttr.attr->attrValue);
	...
	if (num_args == 0) {
		m_LOG_PSS_ERROR_NO_DATA(NCSFL_SEV_ERROR, NCSFL_LC_HEADLINE, PSS_ERR_AMF_CSI_SSCANF_FAILED);
		return 0;
	}
	...
	if ((decoded_env <= 0) || (decoded_env > NCSMDS_MAX_PWES)) {
		/* log the error */
		m_LOG_PSS_ERROR_I(NCSFL_SEV_ERROR, PSS_ERR_AMF_CSI_DECODED_ENV_ID_OUTOF_REACH, decoded_env);
		return 0;
	}
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
void pss_amf_sigusr1_handler(int i_sig_num)
{
...
uns32 pss_amf_component_name_set(void)
{
	...
	if (setenv("SA_AMF_COMPONENT_NAME", (char *)&comp_name, 1) == -1) {
		m_LOG_PSS_ERROR_NO_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_HEADLINE, PSS_ERR_COMP_NAME_SETENV_FAILED);
		return NCSCC_RC_FAILURE;
	}
...
uns32 pss_amf_initialize(NCS_APP_AMF_ATTRIBS *amf_attribs, SaAisErrorT *o_saf_status)
{
	...
	if (saf_status != SA_AIS_OK) {
		/* log the failure */
		m_LOG_PSS_ERROR_I(NCSFL_SEV_ERROR, PSS_ERR_AMF_INITIALIZE_FAILED, saf_status);
		...
		if (saf_status == SA_AIS_ERR_TRY_AGAIN) {
			...
			if (status == NCSCC_RC_FAILURE) {
				...
				m_LOG_PSS_ERROR_I(NCSFL_SEV_ERROR, PSS_ERR_AMF_TIMER_FAILED, status);
			}
			return status;
		}
...
uns32 pss_confirm_ha_role(PW_ENV_ID pwe_id, SaAmfHAStateT ha_role)
{
	...
	/* send the response to AMF */
	saAmfResponse(gl_pss_amf_attribs.amf_attribs.amfHandle, csi_node->pwe_cb->amf_invocation_id, SA_AIS_OK);
