...
*****************************************************************************/
uns32 ncspssvprofiletableentry_get(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSCONTEXT *data)
{
	...
	if (arg->i_idx.i_inst_len != 0) {
		pss_get_profile_from_inst_ids(profile_name, arg->i_idx.i_inst_len, arg->i_idx.i_inst_ids);
	} else {
				       ...
				       NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
	...
	if ((param->i_param_id != ncsPSSvProfileTableStatus_ID) && (param->i_param_id != ncsPSSvProfileDesc_ID)) {
		return NCSCC_RC_NO_INSTANCE;
	}

	memset(profile_name, '\0', sizeof(profile_name));
	memset(profile_desc, '\0', sizeof(profile_desc));
	m_NCS_PSSTS_PROFILE_EXISTS(inst->pssts_api, inst->pssts_hdl, retval, inst->profile_extract, profile_exists);
...
*****************************************************************************/
uns32 ncspssvprofiletableentry_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL test_flag)
{
	...
	case ncsPSSvProfileDesc_ID:
		{
			...
			m_NCS_PSSTS_SET_DESC(inst->pssts_api, inst->pssts_hdl, retval, profile_name,
					     (char *)&desc_text);
			if (retval != NCSCC_RC_SUCCESS) {
				return NCSCC_RC_INV_VAL;
			}
			...
			memcpy(&arg->rsp.info.set_rsp.i_param_val,
			       &arg->req.info.set_req.i_param_val, sizeof(NCSMIB_PARAM_VAL));
		}
		break;
...
/****************************************************************************
  ...
  Notes         : 
****************************************************************************/
uns32 ncspssvscalars_set(NCSCONTEXT cb, NCSMIB_ARG *arg, NCSMIB_VAR_INFO *var_info, NCS_BOOL test_flag)
{
	PSS_CB *inst = (PSS_CB *)cb;
	uns32 retval = NCSCC_RC_SUCCESS;
		...
		if (test_flag) {
			...
			if (arg->req.info.set_req.i_param_val.i_length < sizeof(inst->existing_profile))
				memset(((char *)inst->existing_profile + arg->req.info.set_req.i_param_val.i_length),
				       '\0',
				       (sizeof(inst->existing_profile) - arg->req.info.set_req.i_param_val.i_length));
	...
	case ncsPSSvTriggerOperation_ID:
		if (test_flag) {
			...
			retval = pss_process_trigger_op(inst, arg->req.info.set_req.i_param_val.info.i_int);
			memcpy(&arg->rsp.info.set_rsp.i_param_val,
			       &arg->req.info.set_req.i_param_val, sizeof(NCSMIB_PARAM_VAL));
		}
		break;
	...
	case ncsPSSvCurrentProfile_ID:
		if (test_flag) {
			...
			memcpy((char *)inst->current_profile,
			       (char *)arg->req.info.set_req.i_param_val.info.i_oct,
			       arg->req.info.set_req.i_param_val.i_length);
			m_NCS_PSSTS_SET_PSS_CONFIG(inst->pssts_api, inst->pssts_hdl, retval, inst->current_profile);
			memcpy(&arg->rsp.info.set_rsp.i_param_val,
			       &arg->req.info.set_req.i_param_val, sizeof(NCSMIB_PARAM_VAL));
		}
		break;
...
/****************************************************************************
  ...
  Notes         :
****************************************************************************/
uns32 ncspssvscalars_extract(NCSMIB_PARAM_VAL *param, NCSMIB_VAR_INFO *var_info, NCSCONTEXT data, NCSCONTEXT buffer)
{
	PSS_CB *inst = (PSS_CB *)data;
...
/****************************************************************************
  ...
  Notes         :
****************************************************************************/
uns32 ncspssvscalars_get(NCSCONTEXT inst, NCSMIB_ARG *arg, NCSCONTEXT *data)
{
...
uns32 pss_process_trigger_op(PSS_CB *inst, PSR_TRIGGER_VALUES val)
{
	...
	case ncsPSSvTriggerLoad:
...
#if (NCS_PSS_RED == 1)
		{
			...
			strcpy(&pwe_cb->curr_plbck_ssn_info.info.alt_profile, inst->existing_profile);
	...
	case ncsPSSvTriggerCopy:
		...
		m_NCS_PSSTS_PROFILE_EXISTS(inst->pssts_api, inst->pssts_hdl, retval, inst->new_profile, profile_exists);
		if (retval != NCSCC_RC_SUCCESS)
			return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
		if (profile_exists == TRUE)
			return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
