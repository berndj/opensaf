...
*/
	...
	if (strlen(tbl_owned->tbl_info.mib_tbl_name) >= (PSS_MIB_TBL_NAME_LEN_MAX - 1)) {
		/* Truncate and store */
		strncpy((char *)&tbl_info->ptbl_info->mib_tbl_name, (char *)tbl_owned->tbl_info.mib_tbl_name,
			(PSS_MIB_TBL_NAME_LEN_MAX - 1));
	} else {
	...
	if (tbl_arg_info->objects_local_to_tbl != NULL) {
		...
		tbl_info->ptbl_info->objects_local_to_tbl =
		    (uns16 *)m_MMGR_ALLOC_PSS_OCT(sizeof(uns16) * (tbl_arg_info->objects_local_to_tbl[0] + 1));
		...
		memset(tbl_info->ptbl_info->objects_local_to_tbl, '\0',
		       (sizeof(uns16) * (tbl_arg_info->objects_local_to_tbl[0] + 1)));
		memcpy(tbl_info->ptbl_info->objects_local_to_tbl, tbl_arg_info->objects_local_to_tbl,
		       (sizeof(uns16) * (tbl_arg_info->objects_local_to_tbl[0] + 1)));
	} else {
		...
		tbl_info->ptbl_info->objects_local_to_tbl =
		    (uns16 *)m_MMGR_ALLOC_PSS_OCT(sizeof(uns16) * (tbl_owned->tbl_info.num_objects + 1));
		memset(tbl_info->ptbl_info->objects_local_to_tbl, '\0',
		       (sizeof(uns16) * (tbl_owned->tbl_info.num_objects + 1)));
	...
	for (i = 0, index_loc[0] = 0, k = 1; i < tbl_info->ptbl_info->num_objects; i++) {
		if ((tbl_owned->var_info[i].mib_obj_name == NULL) || (tbl_owned->var_info[i].param_id == 0))
			continue;
		...
		strncpy((char *)&var_info[i].var_name, (char *)tbl_owned->var_info[i].mib_obj_name,
			PSS_VAR_NAME_LEN_MAX - 1);
		...
		if (tbl_owned->var_info[i].obj_type == NCSMIB_OCT_OBJ_TYPE) {
			...
			if (tbl_owned->var_info[i].obj_spec.stream_spec.max_len >
			    NCS_PSS_UNBOUNDED_OCTET_SOFT_LIMITED_SIZE) {
				row_length += NCS_PSS_UNBOUNDED_OCTET_SOFT_LIMITED_SIZE;
				if (tbl_owned->var_info[i].is_index_id == TRUE)
					key_length += NCS_PSS_UNBOUNDED_OCTET_SOFT_LIMITED_SIZE;
				var_info[i].var_info.obj_spec.stream_spec.max_len =
				    NCS_PSS_UNBOUNDED_OCTET_SOFT_LIMITED_SIZE;
			} else {
			...
			key_length += sizeof(uns16);
			lcl_len = ncsmiblib_get_max_len_of_discrete_octet_string(&tbl_owned->var_info[i]);
			row_length += lcl_len;
			if (tbl_owned->var_info[i].is_index_id == TRUE) {
				key_length += lcl_len;
			}
			...
			if (tbl_owned->var_info[i].obj_type == NCSMIB_INT_DISCRETE_OBJ_TYPE) {
				if ((var_info[i].var_info.obj_spec.value_spec.values =
				     m_MMGR_ALLOC_PSS_NCSMIB_INT_OBJ_RANGE(var_info[i].var_info.obj_spec.value_spec.
									   num_values))
				    == NULL) {
					pss_free_individual_var_info_pointers(tbl_info);
					if (tbl_info->ptbl_info->objects_local_to_tbl != NULL)
				...
				memset(var_info[i].var_info.obj_spec.value_spec.values, '\0',
				       (sizeof(NCSMIB_INT_OBJ_RANGE) *
					var_info[i].var_info.obj_spec.value_spec.num_values));
	...
	if (index_loc[0] != 0) {
		...
		if (tbl_info->ptbl_info->idx_data == NULL) {
			m_LOG_PSS_MEMFAIL(NCSFL_SEV_CRITICAL, PSS_MF_NCSMIB_TBL_INFO_ALLOC_FAIL,
					  "pss_ss_tbl_reg() - idx_data ");
	...
	for (i = 0; i < tbl_info->ptbl_info->num_objects; i++) {
		if (tbl_info->pfields[i].var_info.obj_type == NCSMIB_OCT_DISCRETE_OBJ_TYPE) {
			if (tbl_info->pfields[i].var_info.obj_spec.disc_stream_spec.values != NULL) {
				m_MMGR_FREE_PSS_NCSMIB_OCT_OBJ(tbl_info->pfields[i].var_info.obj_spec.disc_stream_spec.
							       values);
			...
			if (tbl_info->pfields[i].var_info.obj_spec.value_spec.values != NULL) {
				m_MMGR_FREE_PSS_NCSMIB_INT_OBJ_RANGE(tbl_info->pfields[i].var_info.obj_spec.value_spec.
								     values);
...
uns32 pss_svc_create(NCSPSS_CREATE *create)
{
	...
	strncpy((char *)inst->current_profile, pssts_arg.info.pss_config.current_profile_name,
		NCS_PSS_MAX_PROFILE_NAME - 1);
	...
	if (retval != NCSCC_RC_SUCCESS) {
		...
		m_LOG_PSS_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR,
				   PSS_ERR_SPIR_OAA_LOOKUP_CREATE_FAILED, retval, m_PSS_DEFAULT_ENV_ID);
	...
	/* update the handler */
	inst->hm_hdl = ncshm_create_hdl(inst->hmpool_id, NCS_SERVICE_ID_PSS, (NCSCONTEXT)inst);
...
uns32 pss_pwe_cb_init(uns32 pss_cb_handle, PSS_PWE_CB *pwe_cb, PW_ENV_ID envid)
{
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_PSS_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR,
				   PSS_ERR_SPIR_MDS_LOOKUP_CREATE_FAILED, status, envid);
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_PSS_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR,
				   PSS_ERR_SPIR_MAA_LOOKUP_CREATE_FAILED, status, envid);
	...
	pwe_cb->hm_hdl = ncshm_create_hdl(inst->hmpool_id, NCS_SERVICE_ID_PSS, (NCSCONTEXT)pwe_cb);
...
uns32 pss_read_create_spcn_config_file(PSS_CB *inst)
{
	...
	while (((read = fscanf(fh, "%s %s", (char *)&pcn, (char *)&source)) == 2) && (read != EOF)) {
...
uns32 pss_pwe_cb_destroy(PSS_PWE_CB *pwe_cb)
{
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_PSS_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR,
				   PSS_ERR_SPIR_MAA_REL_INST_FAILED, status, pwe_cb->pwe_id);
	}
	m_LOG_PSS_HDLN_I(NCSFL_SEV_INFO, PSS_HDLN_MAA_REL_INST_SUCCESS, pwe_cb->pwe_id);
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_PSS_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR,
				   PSS_ERR_SPIR_MDS_REL_INST_FAILED, status, pwe_cb->pwe_id);
	}
	m_LOG_PSS_HDLN_I(NCSFL_SEV_INFO, PSS_HDLN_MDS_REL_INST_SUCCESS, pwe_cb->pwe_id);

	/* Free the OAA_tree first */
	status = pss_destroy_pwe_oaa_tree(&pwe_cb->oaa_tree, TRUE);
	if (status != NCSCC_RC_SUCCESS) {
		m_LOG_PSS_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR,
				   PSS_ERR_OAA_TREE_DESTROY_FAILED, status, pwe_cb->pwe_id);
	} else {
		m_LOG_PSS_HDLN_I(NCSFL_SEV_INFO, PSS_HDLN_PWE_CB_OAA_TREE_DESTROY_SUCCESS, pwe_cb->pwe_id);
	}
	...
	if (status != NCSCC_RC_SUCCESS) {
		m_LOG_PSS_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR,
				   PSS_ERR_CLIENT_TABLE_TREE_DESTROY_FAILED, status, pwe_cb->pwe_id);
	} else {
		m_LOG_PSS_HDLN_I(NCSFL_SEV_INFO, PSS_HDLN_PWE_CB_CLNT_TREE_DESTROY_SUCCESS, pwe_cb->pwe_id);
	}
...
void pss_cb_data_dump()
{
	...
	for (list = gl_pss_amf_attribs.pss_cb->spcn_list; list != NULL; list = list->next) {
		...
		fprintf(fh, "\tClient:%s, Source=%s\n", list->pcn, (list->plbck_frm_bam ? "XML" : "PSS"));
		fflush(fh);
	}
		...
		while ((pNode = ncs_patricia_tree_getnext(&pwe_cb->oaa_tree, (const uns8 *)&oaa_key)) != NULL) {
			...
			for (i = 0; i < MAB_MIB_ID_HASH_TBL_SIZE; i++) {
				...
				for (j = 0, oaa_clt_entry = oaa_entry->hash[i];
				     oaa_clt_entry != NULL; oaa_clt_entry = oaa_clt_entry->next, ++j) {
					fprintf(fh, "\t\t - [%d] - tbl_rec = %lx, tbl_id = %d, tbl_name = %s\n",
						j, (long)oaa_clt_entry->tbl_rec, oaa_clt_entry->tbl_rec->tbl_id,
						gl_pss_amf_attribs.pss_cb->mib_tbl_desc[oaa_clt_entry->tbl_rec->
											tbl_id]->ptbl_info->
						mib_tbl_name);
					fflush(fh);
				}
				...
				for (j = 0, trec = clt_node->hash[i]; trec != NULL; trec = trec->next, ++j) {
					if (gl_pss_amf_attribs.pss_cb->mib_tbl_desc[trec->tbl_id] != NULL) {
						fprintf(fh, "\t\t - [%d] - tbl_rec = %lx, tbl_id = %d, tbl_name = %s\n",
							...
							gl_pss_amf_attribs.pss_cb->mib_tbl_desc[trec->tbl_id]->
							ptbl_info->mib_tbl_name);
						...
						fprintf(fh,
							"\t\t\t\t -  Related OAA-MDS_DEST:%x:%x:%x:%x:%x:%x:%x:%x:\n",
						...
						fprintf(fh,
							"\t\t - [%d] - tbl_rec = %lx, tbl_id = %d, tbl_description = NULL!!!\n",
							j, (long)trec, trec->tbl_id);
						fflush(fh);
					}
				}
