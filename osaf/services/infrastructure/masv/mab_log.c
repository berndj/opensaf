...
static uns32 log_mab_oaa_and_fltr_data(uns8 sev, uns32 env_id, uns32 table_id, uns8 exist, MAS_FLTR *fltr);
/*****************************************************************************
...
#if (MAB_LOG == 1)
uns32 log_mab_svc_prvdr_evt(uns8 sev, uns32 id, uns32 svc_id, MDS_DEST addr, uns32 new_state, uns32 anchor)
{
	char addr_str[255] = { 0 };
...
uns32 log_mab_svc_prvdr_msg(uns8 sev, uns32 id, uns32 svc_id, MDS_DEST addr, uns32 msg_type)
{
	char addr_str[255] = { 0 };
...
uns32 log_mab_fltr_data(uns8 sev, uns32 env_id, uns32 tbl_id, uns32 fltr_id, NCSMAB_FLTR *fltr_data)
{
	NCSFL_MEM index;
	...
	case NCSMAB_FLTR_RANGE:
		ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_FLTR_RANGE,
			   ...
			   env_id, tbl_id, fltr_id, fltr_data->fltr.range.i_idx_len, fltr_data->fltr.range.i_bgn_idx);
		...
		ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_FLTR_RANGE_INDEX,
			   MAB_FC_TBL_DET, NCSFL_LC_DATA, sev, "TID", MAB_MAS_RCV_RNG_FLTR_MIN_IDX, index);

		index.addr = index.dump = (char *)fltr_data->fltr.range.i_max_idx_fltr;
		ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_FLTR_RANGE_INDEX,
			   MAB_FC_TBL_DET, NCSFL_LC_DATA, sev, "TID", MAB_MAS_RCV_RNG_FLTR_MAX_IDX, index);
		break;
	case NCSMAB_FLTR_EXACT:
		ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_FLTR_EXACT,
			   MAB_FC_TBL_DET, NCSFL_LC_DATA, sev,
			   "TILLLLL", MAB_MAS_RCV_EXACT_FLTR_REG_REQ,
			   env_id, tbl_id, fltr_id, fltr_data->fltr.exact.i_idx_len, fltr_data->fltr.exact.i_bgn_idx);
		...
		ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_FLTR_EXACT_INDEX,
			   MAB_FC_TBL_DET, NCSFL_LC_DATA, sev, "TID", MAB_MAS_RCV_EXACT_FLTR_IDX, index);
		break;
	case NCSMAB_FLTR_ANY:
		return ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_FLTR_ANY,
				  MAB_FC_TBL_DET, NCSFL_LC_DATA, sev,
				  NCSFL_TYPE_TILLL, MAB_MAS_RCV_ANY_FLTR_REG_REQ, env_id, tbl_id, fltr_id);
		break;
	case NCSMAB_FLTR_SAME_AS:
		return ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_FLTR_SA,
				  ...
				  env_id, tbl_id, fltr_id, fltr_data->fltr.same_as.i_table_id);
		break;
	case NCSMAB_FLTR_DEFAULT:
		return ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_FLTR_DEF,
				  MAB_FC_TBL_DET, NCSFL_LC_DATA, sev,
				  NCSFL_TYPE_TILLL, MAB_MAS_RCV_DEF_FLTR_REG_REQ, env_id, tbl_id, fltr_id);
		break;
...
static uns32 log_mab_oaa_and_fltr_data(uns8 sev, uns32 env_id, uns32 table_id, uns8 exist, MAS_FLTR *fltr)
{
	...
	if (m_NCS_NODE_ID_FROM_MDS_DEST(fltr->vcard) == 0)
		...
		snprintf(addr_str, sizeof(addr_str) - 1,
			 exist ==
			 1 ? "Existing ADEST:node_id:%d, v1.pad16:%d, v1.vcard:%llu" :
			 "New ADEST:node_id:%d, v1.pad16:%d, v1.vcard:%llu", m_NCS_NODE_ID_FROM_MDS_DEST(fltr->vcard),
			 0, (fltr->vcard));

	ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_NO_CB, MAB_FC_HDLN,
		   NCSFL_LC_HEADLINE, sev, NCSFL_TYPE_TIC, MAB_HDLN_MAS_OAA_ADDR, addr_str);

	/* log the filter data */
	log_mab_fltr_data(sev, env_id, table_id,
			  (fltr->fltr_ids.active_fltr != NULL) ? fltr->fltr_ids.active_fltr->fltr_id : 0, &fltr->fltr);

	if (!fltr->fltr_ids.fltr_id_list) {
		snprintf(addr_str1, sizeof(addr_str1) - 1, "Other Filter-Id Anchor Mapping list: Empty");
		ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_NO_CB, MAB_FC_HDLN,
			   NCSFL_LC_HEADLINE, sev, NCSFL_TYPE_TIC, MAB_HDLN_MAS_OAA_ADDR, addr_str1);
	} else {
		...
		while (fltr->fltr_ids.fltr_id_list) {
			snprintf(fltr_anchr, sizeof(fltr_anchr) - 1, "%llu:%u ",
				 fltr->fltr_ids.fltr_id_list->anchor, fltr->fltr_ids.fltr_id_list->fltr_id);

			ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_NO_CB, MAB_FC_HDLN,
				   NCSFL_LC_HEADLINE, sev, NCSFL_TYPE_TIC, MAB_HDLN_MAS_ANCHR_FLTR_PAIR, fltr_anchr);
...
uns32 log_overlapping_fltrs(uns8 sev, uns32 env_id, uns32 table_id, MAS_FLTR *exst_fltr, MAS_FLTR *new_fltr)
{
