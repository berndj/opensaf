...
const NCSFL_STR mab_hdln_set[] = {
	...
	{MAB_HDLN_MAS_FRWD_MOVEROW_MIB_REQ_TO_OAC,
	 "MAS Successfully forwaded the MOVEROW request to OAC (for table-id)"},
	...
	{MAB_HDLN_MAS_POP_FISE_FAILED,
	 "MAS mas_info_response(), Could not get the Filter-Id from the NCSMIB_ARG stack"},
	...
	{MAB_HDLN_MBCSV_ERR_IND_AMF_ERR_REPORT,
	 "MAS-MBCSv-SBY: Reported an Error report to AMF (MBCSv error code, saf_status)"},
	...
	{MAB_HDLN_MAS_MBCSV_ASYNC_UPDT_SUCCESS,
	 "MAS-MBCSv-ACT: mas_red_sync(): asyn update send success (type, status)"},
	...
	{MAB_HDLN_AMF_RESPONSE_POSTPONED,
	 "MAS mas_amf_csi_new() AMF RESP for SBY is postponed because COLD SYNC not completed (invocation-id, env-id)"},
	{MAB_HDLN_AMF_SBY_RESPONSE_SENT,
	 "MAS-MBCSv-SBY COLD SYNC completed, sending the pending response to AMF (invocation-id, env-id)"},
	...
	{MAB_HDLN_OAC_CONVRT_WBREQ_TO_MAB_MSG_FAIL,
	 "OAA send warmboot-req, oac_convert_input_wbreq_to_mab_request() returned FAIL"},
	...
	{MAB_HDLN_OAC_PCN_OF_WARMBOOT_REQ_NOT_IN_PCN_LIST,
	 "OAA PCN in WARMBOOT REQ API not found in pcn_list. So, it's invalid PCN."},
	...
	{MAB_HDLN_OAC_WARMBOOT_REQ_REJECTED_DUE_TO_INVALID_TBLS_SPCFD,
	 "OAA WARMBOOT REQ REJECTED as all specified Tables are either Invalid or non-persistent"},
	...
	{MAB_HDLN_OAC_PSS_MDS_NEW_ACTIVE_AS_UP_EVT,
	 "OAA received PSS MDS_NEW_ACTIVE event, which is being treated as MDS_UP"},
	{MAB_HDLN_OAA_PCN_NULL_IN_OAC_FINDADD_PCN_IN_LIST_FUNC,
	 "OAA error - Input PCN NULL in oac_findadd_pcn_in_list() function"},
...
const NCSFL_STR mab_error_set[] = {
	...
	{MAB_MAC_ERR_SPIR_LOOKUP_CREATE_FAILED,
	 "MAA ncs_spir_api() for NCSMAC_LM_OP_CREATE failed for NCS_SPIR_REQ_LOOKUP_CREATE_INST_INFO operation"},
	...
	{MAB_MAS_ERR_DEF_ENV_INIT_FAILED,
	 "MAS Default Environment Intialization failed for Env-Id and Initial HAPS state respectively "},
	...
	{MAB_MAS_ERR_DEF_FLTR_NOT_REGISTERED,
	 "MAS moverow operation is requested for a table with no staging area/no default filter registered"},
	...
	{MAB_MAS_ERR_AMF_CSI_ATT_COUNT_INVD,
	 "MAS mas_amf_csi_attrib_decode(): Invalid number of CSI attributes received "},
	...
	{MAB_MAS_ERR_AMF_CSI_ATTS_NAME_OR_ATTRIB_VAL_NULL,
	 "MAS mas_amf_csi_attrib_decode(): Either CSI attribute name or value is NULL"},
	...
	{MAB_MAS_ERR_MOVEROW_OAC_SND_FAILED,
	 "MAS mas_forward_msg() failed to send MOVEROW request to OAC(for tble-id)"},
	{MAB_MAS_ERR_INFORM,
	 "MAS mas_exact_fltr_almighty_inform(): EXACT Filter processing failed (reason, data follows)"},
	...
	{MAB_MAS_ERR_MDS_TO_MBX_SND_FAILED,
	 "MAS failed to send the MAB_MSG from MDS thread to its mailbox (status, MSG type) "},
	...
	{MAB_MAS_ERR_MBCSV_SEL_OBJ_GET_FAILED,
	 "MAS-MBCSv NCS_MBCSV_OP_SEL_OBJ_GET operation failed (mbcsv_hdl, error)"},
	...
	{MAB_MAS_ERR_MBCSV_ENC_UNSUP_MSG_TYPE,
	 "MAS-MBCSv Interface received invalid message type during encode (msg_type)"},
	{MAB_MAS_ERR_MBCSV_DEC_UNSUP_MSG_TYPE,
	 "MAS-MBCSv Interface received invalid message type during decode (msg_type)"},
	...
	{MAB_MAS_ERR_COLD_SYNC_DEC_BKT_NON_NULL,
	 "MAS-MBCSv Cold_sync response decode success, but add failed because of a non-null pointer (bucket-id)"},
	...
	{MAB_MAS_ERR_MBCSV_OP_NOT_SUPPORTED,
	 "MAS-MBCSV mas_mbcsv_cb() received an unsupported mbcsv operation (mbcsv op code)"},
	{MAB_MAS_ERR_AMF_SBY_TO_ACT_FAILED_COLD_SYNC,
	 "MAS-AMF-SBY: Standby ==> Active transition failed because Cold Sync is not complete"},
	{MAB_MAS_ERR_AMF_SBY_TO_ACT_FAILED_WARM_SYNC,
	 "MAS-AMF-SBY: Standby ==> Active transition failed because Warm Sync is not complete"},
	{MAB_MAS_ERR_MBCSV_API_SND_USR_ASYNC_FAILED,
	 "MAS-MBCSv-ACT: mas_red_sync() failed for NCS_MBCSV_ACT_UPDATE operation (type of updt, error)"},
	{MAB_MAS_ERR_MBCSV_ENC_FAILED,
	 "MAS-MBCSv: mas_red_msg_enc(), Encode of a message failed (enc->io_msg_type, rc)"},
	{MAB_MAS_ERR_MBCSV_DEC_FAILED,
	 "MAS-MBCSv: mas_red_msg_dec(), Decode of a message failed (dec->i_msg_type, rc)"},
	{MAB_MAS_ERR_MBCSV_WARM_SYNC_BKT_CNT_ZERO,
	 "MAS-MBCSv-ACT: mas_red_data_rsp_enc_cb(), data_response encode called for zero out of sync buckets"},
	{MAB_MAS_ERR_MBCSV_DATA_RSP_ENCODE_FAILED_FOR_BKT,
	 "MAS-MBCSv-ACT: mas_red_data_rsp_enc_cb() data response encode failed for bucket (to_be_sent_bkt, inst->hash[to_be_sent_bkt])"},
	{MAB_MAS_ERR_MBCSV_API_DATA_REQ_FAILED,
	 "MAS-MBCSv-SBY: mas_red_warm_sync_resp_dec() failed for NCS_MBCSV_OP_SEND_DATA_REQ op(bkt_count, status)"},
	{MAB_MAS_ERR_MBCSV_DATA_REQ_DEC_BKTCNT_ZERO,
	 "MAS-MBCSv-ACT: mas_red_data_req_dec() out of sync bucket count is zero"},
	{MAB_MAS_ERR_PREPARE_WILL_FAILED, "MAS mas_svc_destroy(): mas_amf_prepare_will() failed (status)"},
	{MAB_MAS_ERR_MBCSV_FINALIZE_FAILED,
	 "MAS-MBCSv mas_svc_destroy(): ncs_mbcsv_svc() NCS_MBCSV_OP_FINALIZE failed (status)"},
	...
	{MAB_MAS_ERR_AMF_STATE_CHG_FAILED,
	 "MAS mas_amf_csi_new(): mas_amf_csi_state_change_process() failed (asked state, curr state)"},
	...
	{MAB_MAS_ERR_GET_ROLE_FAILED,
	 "MAS mas_get_crd_role(): ncsmds_api(): MDS_QUERY_DEST failed (MDS DEST and FILTER info follows)(error): "},
	...
	{MAB_OAC_ERR_SPIR_LOOKUP_CREATE_FAILED,
	 "OAA ncs_spir_api() for NCSOAC_LM_OP_CREATE failed for NCS_SPIR_REQ_LOOKUP_CREATE_INST_INFO operation"},
	...
	{MAB_OAC_ERR_MDS_TO_MBX_SND_FAILED,
	 "OAA failed to send the MAB_MSG from MDS thread to its mailbox (status, msg type)"},
...
NCSFL_FMAT mab_fmat_set[] = {
	...
	{MAB_LID_SVC_PRVDR_EVT, NCSFL_TYPE_TICLLL,
	 "MAB %s SVC PRVDR: %s\n\t\tFrom:%s, \n\t\tFrom Svc-id: %ld, NewState: %ld, Anchor:%ld\n\n"},
	{MAB_LID_SVC_PRVDR_MSG, NCSFL_TYPE_TICLL,
	 "MAB %s SVC PRVDR: %s\n\t\tFrom:%s,\n\t\tFrom Svc-id: %ld, MsgType: %ld\n\n"},
	{MAB_LID_FLTR_RANGE, "TILLLLL",
	...
	{MAB_LID_FLTR_SA, NCSFL_TYPE_TILLLL,
	...
	{MAB_LID_OAA_WARMBOOTREQ_INFO_II, "TICLL",
	 "OAA %s WARMBOOT REQ INFO: %s\n\t\t PCN=%s, is_system_client=%ld, TBL_ID=%ld \n"},
	{MAB_LID_OAA_WARMBOOTREQ_INFO_III, "TICLLL",
 ...
 * Notes         : None.
 *****************************************************************************/
uns32 mab_log_str_lib_req(NCS_LIB_REQ_INFO *req_info)
{
