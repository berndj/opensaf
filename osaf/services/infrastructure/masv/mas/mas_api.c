...
uns32 mas_svc_create(NCSMAS_CREATE *create)
{
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_I(NCSFL_SEV_CRITICAL, MAB_MAS_ERR_RDA_INIT_ROLE_GET_FAILED, status);
	...
	if (csi == NULL) {
		...
		m_LOG_MAB_ERROR_II(NCSFL_LC_FUNC_RET_FAIL, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_DEF_ENV_INIT_FAILED, m_MAS_DEFAULT_ENV_ID, init_ha_role);
	...
	m_LOG_MAB_HDLN_II(NCSFL_SEV_INFO, MAB_HDLN_MAS_DEF_ENV_INIT_SUCCESS, m_MAS_DEFAULT_ENV_ID, init_ha_role);
...
uns32 mas_svc_destroy(NCSMAS_DESTROY *destroy)
{
	...
	if (status != NCSCC_RC_SUCCESS) {
		m_LOG_MAB_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR, MAB_MAS_ERR_PREPARE_WILL_FAILED, status);
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR, MAB_MAS_ERR_MBCSV_FINALIZE_FAILED, status);
		m_MAB_DBG_TRACE("\nmas_svc_destroy():left.");
	}
#endif
