...
static uns32 dummy;
static uns32 mas_get_crd_role(MDS_HDL mds_hdl, MDS_DEST dest, MAB_ANCHOR anc, V_DEST_RL *role);
...
mas_exact_fltr_process(MAB_MSG *msg,
		       MAS_TBL *inst, MAS_ROW_REC *tbl_rec, MAS_FLTR *new_fltr, uns32 i_fltr_id, V_DEST_RL oaa_role);

static uns32 mas_exact_fltr_almighty_inform(MAS_TBL *inst, MAS_ROW_REC *tbl_rec, MAS_FLTR *new_fltr, uns32 event_type);

/* to get the filter-id for a given anchor value */
static uns32 mab_fltrid_list_get(MAS_FLTR_IDS *fltr_list, MAB_ANCHOR i_anchor, uns32 *o_fltr_id);

/* to add this filtr_id and anchor value pair */
static uns32 mab_fltrid_list_add(MAS_FLTR_IDS *i_list, MAB_ANCHOR i_anchor, V_DEST_RL oaa_role, uns32 i_fltr_id);

/* to add this filtr_id and anchor value pair */
static uns32 mab_fltrid_list_del(MAS_FLTR_IDS *fltr_list, MAB_ANCHOR i_anchor);

/* inform the MAA with the given MIB_ARG and error_code */
static uns32 mas_inform_maa(MAB_MSG *msg, MAS_TBL *inst, NCSMIB_ARG *mib_req, uns32 error_code);

/* to remove the filters of a particular OAA */
static uns32 mas_flush_fltrs(MAS_TBL *inst, MDS_DEST vcard, MAB_ANCHOR anc, uns32 ss_id);

/* to process the OAA down event */
static uns32 mas_oaa_down_process(MAB_MSG *msg);

/* removes a default fitler from the list od tables */
static NCS_BOOL
mas_def_flter_del(MAS_TBL *inst, MAS_ROW_REC *tbl_rec, MDS_DEST *vcard, MAB_ANCHOR anc, MAS_ROW_REC **next_tbl_rec);

/* to process the OAA state change */
static uns32 mas_oaa_role_chg_process(MAB_MSG *msg);

/* adjust the filter-id/anchor value mapping list */
static uns32 mas_fltr_ids_adjust(MAS_FLTR_IDS *fltr_list, MAB_ANCHOR i_anchor, V_DEST_RL i_role);
...
uns32 mas_do_evts(SYSF_MBX *mbx)
{
	MAB_MSG *msg;
...
uns32 mas_get_crd_role(MDS_HDL mds_hdl, MDS_DEST dest, MAB_ANCHOR anc, V_DEST_RL *role)
{
	...
	if (m_NCS_NODE_ID_FROM_MDS_DEST(dest) != 0) {	/* Destination is ADEST */
		*role = V_DEST_RL_ACTIVE;
	} else {		/* Destination is VDEST */
		...
		if (status != NCSCC_RC_SUCCESS) {
			...
			if (m_NCS_NODE_ID_FROM_MDS_DEST(dest) == 0)
				snprintf(addr_str, sizeof(addr_str) - 1, "VDEST:%d",
					 m_MDS_GET_VDEST_ID_FROM_MDS_DEST(dest));
			else
...
MAS_FLTR *mas_fltr_find(MAS_TBL *inst, MAS_FLTR **head, uns32 fltr_id, MAB_ANCHOR anc, MDS_DEST vcard)
{
	...
	if (fltr != NULL) {
		...
		while (fltr) {
			...
			if ((status == NCSCC_RC_SUCCESS) && (exist_fltr_id == fltr_id) &&	/* compare the filter id */
...
MAS_FLTR *mas_fltr_create(MAS_TBL *inst,
			  ...
			  MDS_DEST vcard, V_DEST_RL fr_oaa_role, MAB_ANCHOR anc, uns32 fltr_id, MAS_FLTR_FNC fltr_fnc)
{
	MAS_FLTR *ret = NULL;
	uns32 status = NCSCC_RC_FAILURE;
	...
	if (ret == NULL) {
		m_LOG_MAB_MEMFAIL(NCSFL_SEV_CRITICAL, MAB_MF_MAS_FLTR_CREATE_FAILED, "mas_fltr_create()");
		return (MAS_FLTR *)m_MAB_DBG_SINK((long)NULL);
	}
...
void *mas_classify_range(MAS_TBL *inst, NCSMAB_FLTR *fltr, uns32 *inst_ids, uns32 inst_len, NCS_BOOL is_next_req)
{
	...
	m_MAB_FLTR_CMP(min_res,
		       inst_ids + fltr->fltr.range.i_bgn_idx, fltr->fltr.range.i_min_idx_fltr, to_be_used_inst_len);
	if (min_res == 0) {
		return fltr;
	} else if (min_res > 0) {
		m_MAB_FLTR_CMP(max_res,
			       inst_ids + fltr->fltr.range.i_bgn_idx,
			       fltr->fltr.range.i_max_idx_fltr, to_be_used_inst_len);
...
void *mas_classify_same_as(MAS_TBL *inst, NCSMAB_FLTR *fltr, uns32 *inst_ids, uns32 inst_len, NCS_BOOL is_next_req)
{
	...
	if ((tbl_rec = mas_row_rec_find(inst, fltr->fltr.same_as.i_table_id)) == NULL) {
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_FLTR_SA_INVALID_TBL, fltr->fltr.same_as.i_table_id);
		return NULL;
	}

	if ((match_fltr = mas_locate_dst_oac(inst, tbl_rec->fltr_list, inst_ids, inst_len, is_next_req)) == NULL) {
	...
	if (is_next_req == FALSE) {
		if (inst_len == fltr->fltr.exact.i_idx_len) {
			m_MAB_FLTR_CMP(res,
				       ...
				       fltr->fltr.exact.i_exact_idx, fltr->fltr.exact.i_idx_len);
			if (res == 0) {
				return fltr;
			}
		}
		...
		m_MAB_FLTR_CMP(res, fltr->fltr.exact.i_exact_idx, inst_ids + fltr->fltr.exact.i_bgn_idx, size);
		if (res == 0) {
...
MAS_FLTR *mas_locate_dst_oac(MAS_TBL *inst, MAS_FLTR *head, uns32 *inst_ids, uns32 inst_len, NCS_BOOL is_next_req)
{
	...
	for (fltr = head; fltr != NULL; fltr = fltr->next) {
		if (fltr->fltr.type == NCSMAB_FLTR_SAME_AS) {
			if ((ret =
			     (MAS_FLTR *)fltr->test(inst, &(fltr->fltr), inst_ids, inst_len, is_next_req)) != NULL) {
				break;
			}
		} else {
...
mas_def_flter_del(MAS_TBL *inst, MAS_ROW_REC *tbl_rec, MDS_DEST *vcard, MAB_ANCHOR anc, MAS_ROW_REC **next_tbl_rec)
{
	...
	if (memcmp(&tbl_rec->dfltr.vcard, vcard, sizeof(MDS_DEST)) == 0) {
		...
		if ((tbl_rec->dfltr.fltr_ids.fltr_id_list == NULL) && (tbl_rec->dfltr.fltr_ids.active_fltr == NULL)) {
...
uns32 mas_flush_fltrs(MAS_TBL *inst, MDS_DEST vcard, MAB_ANCHOR anc, uns32 ss_id)
{
	...
	if (m_NCS_NODE_ID_FROM_MDS_DEST(vcard) == 0)
		snprintf(addr_str, sizeof(addr_str) - 1, "VDEST:%d", m_MDS_GET_VDEST_ID_FROM_MDS_DEST(vcard));
	else
		snprintf(addr_str, sizeof(addr_str) - 1, "ADEST:node_id:%d, v1.pad16:%d, v1.vcard:%lu",
			 m_NCS_NODE_ID_FROM_MDS_DEST(vcard), 0, (long)(vcard));

	/* now log the OAA address whose filters are getting wiped off */
	ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_NO_CB, MAB_FC_HDLN,
		   NCSFL_LC_HEADLINE, NCSFL_SEV_NOTICE, NCSFL_TYPE_TIC, MAB_HDLN_MAS_FLTRS_FLUSH, addr_str);
			...
			if (tbl_rec->ss_id == ss_id) {
				...
				for (fltr = tbl_rec->fltr_list; fltr != NULL;) {
					if (memcmp(&fltr->vcard, &vcard, sizeof(vcard)) == 0) {
						...
						if ((fltr->fltr_ids.fltr_id_list == NULL)
						    && (fltr->fltr_ids.active_fltr == NULL)) {
							del_fltr = fltr;
					...
					if (tbl_rec->fltr_list == NULL) {
						...
						if ((del_tbl_rec = mas_row_rec_rmv(inst, tbl_rec->tbl_id)) == NULL) {
							m_LOG_MAB_TBL_DETAILS(NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR,
									      MAB_MAS_RMV_ROW_REC_FAILED, inst->vrid,
									      tbl_rec->tbl_id);
						...
						m_LOG_MAB_TBL_DETAILS(NCSFL_LC_DATA, NCSFL_SEV_DEBUG,
								      MAB_MAS_RMV_ROW_REC_SUCCESS, inst->vrid,
								      del_tbl_rec->tbl_id);
						m_MAB_DBG_TRACE2_MAS_TR_DALLOC(del_tbl_rec);
...
uns32 mas_info_request(MAB_MSG *msg)
{
	...
	if ((tbl_rec = mas_row_rec_find(inst, mib_req->i_tbl_id)) == NULL) {
		...
		if (mas_relay_msg_to_mac(msg, inst, TRUE) != NCSCC_RC_SUCCESS) {
			m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_SNT_NO_TBL_RSP_FAILED, mib_req->i_tbl_id);
			return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
		}
	...
	/* if(move_case == TRUE) */
	/* completed MOVEROW request handling */
	...
	if (match_fltr == NULL) {
		...
		if (tbl_rec->dfltr_regd == TRUE) {
			...
			if (mas_forward_msg(msg, MAB_OAC_DEF_REQ_HDLR, tx_mds_hdl, to_vcard, NCSMDS_SVC_ID_OAC, TRUE) ==
			    NCSCC_RC_FAILURE) {
#if (NCS_MDS == 1)
		...
		case NCSMIB_OP_REQ_TEST:
			...
			mib_req->rsp.info.set_rsp.i_param_val.i_param_id =
			    mib_req->req.info.set_req.i_param_val.i_param_id;
			break;
	...
	/* forward the message to OAC */
	{
		...
		if (mas_forward_msg(msg, MAB_OAC_REQ_HDLR, tx_mds_hdl, to_vcard, NCSMDS_SVC_ID_OAC, TRUE) !=
		    NCSCC_RC_SUCCESS) {
#if (NCS_MDS == 1)
			if (match_fltr->fltr_ids.active_fltr != NULL) {
				MAB_LM_EVT mle;
			...
			if (m_NCS_NODE_ID_FROM_MDS_DEST(match_fltr->vcard) == 0)
				snprintf(addr_str, sizeof(addr_str) - 1, "VDEST:%d",
					 m_MDS_GET_VDEST_ID_FROM_MDS_DEST(match_fltr->vcard));
			else
				snprintf(addr_str, sizeof(addr_str) - 1, "ADEST:node_id:%d, v1.pad16:%d, v1.vcard:%lu",
...
uns32 mas_info_response(MAB_MSG *msg)
{
	...
	if (mib_rsp->i_op == NCSMIB_OP_RSP_MOVEROW) {
		if (mib_rsp->rsp.i_status != NCSCC_RC_SUCCESS) {
			...
			m_LOG_MAB_HDLN_I(NCSFL_SEV_ERROR, MAB_HDLN_MAS_MOVEROW_RSP_FAILED, mib_rsp->rsp.i_status);
			failed = TRUE;
	...
	if ((tbl_rec = mas_row_rec_find(inst, mib_rsp->i_tbl_id)) == NULL) {
		m_LOG_MAB_TBL_DETAILS(NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR,
				      MAB_MAS_FIND_REQ_TBL_FAILED, inst->vrid, mib_rsp->i_tbl_id);
	...
	if ((match_fltr = mas_fltr_find(inst, &(tbl_rec->fltr_list), fltr_id, msg->fr_anc, msg->fr_card)) == NULL) {
		...
		if (mib_rsp->rsp.info.cli_rsp.o_partial != TRUE) {
			...
			if (match_fltr == NULL) {
				...
				if (mas_relay_msg_to_mac(msg, inst, FALSE) != NCSCC_RC_SUCCESS) {
					m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_NO_NI_FRWD_TO_MAC,
							  NCSCC_RC_FAILURE);
					return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
				}
				...
				if (mas_next_op_stack_push(inst, &mib_rsp->stack,
							   match_fltr->fltr_ids.active_fltr->fltr_id) !=
				    NCSCC_RC_SUCCESS) {
					m_LOG_MAB_HEADLINE(NCSFL_SEV_ERROR, MAB_HDLN_MAS_PUSH_FISE_FAILED);
					return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
				}
		...
		case NCSCC_RC_NO_INSTANCE:
			...
			if (match_fltr->next == NULL) {
				...
				case NCSMIB_OP_RSP_NEXT:
					...
					mib_rsp->req.info.next_req.i_param_id =
					    mib_rsp->rsp.info.next_rsp.i_param_val.i_param_id;
					break;
...
uns32 mas_info_register(MAB_MSG *msg)
{
	...
	if (status != NCSCC_RC_SUCCESS) {
		...
		ncs_logmsg(NCS_SERVICE_ID_MAB, MAB_LID_ANC, MAB_FC_HDLN,
			   NCSFL_LC_HEADLINE, NCSFL_SEV_NOTICE, "TF", (double)msg->fr_anc);
	...
	if ((tbl_rec = mas_row_rec_find(inst, reg_req->tbl_id)) == NULL) {
		...
		if (tbl_rec == NULL) {
			m_LOG_MAB_MEMFAIL(NCSFL_SEV_CRITICAL, MAB_MF_MAS_ROW_REC_ALLOC_FAILED, "mas_info_register()");
			m_MAS_UNLK(&inst->lock);
			m_LOG_MAB_LOCK(MAB_LK_MAS_UNLOCKED, &inst->lock);
			ncshm_give_hdl(NCS_PTR_TO_INT32_CAST(msg->yr_hdl));
			mas_mab_fltr_indices_cleanup(&msg->data.data.reg.fltr);
	...
	if (reg_req->fltr.type == NCSMAB_FLTR_DEFAULT) {
		...
		if ((tbl_rec->dfltr_regd == TRUE) &&
			...
			snprintf(addr_str, sizeof(addr_str) - 1, " For Env-Id: %d, Table-Id :%d, ", inst->vrid,
				 tbl_rec->tbl_id);
			...
			if (m_NCS_NODE_ID_FROM_MDS_DEST(tbl_rec->dfltr.vcard) == 0)
				...
				snprintf(str_ptr, sizeof(addr_str) - 1,
					 "Existing ADEST:node_id:%d, v1.pad16:%d, v1.vcard:%lu",
			...
			if (m_NCS_NODE_ID_FROM_MDS_DEST(msg->fr_card) == 0)
				...
				snprintf(str_ptr, sizeof(addr_str) - 1,
					 "Request From ADEST:node_id:%d, v1.pad16:%d, v1.vcard:%lu",
					 m_NCS_NODE_ID_FROM_MDS_DEST(msg->fr_card), 0, (long)(msg->fr_card));
				...
				for (mas_fltr = tbl_rec->fltr_list; mas_fltr != NULL; mas_fltr = mas_fltr->next) {
					...
					if (min_res == 0) {
						m_MAB_FLTR_CMP(max_res, mas_fltr->fltr.fltr.range.i_max_idx_fltr,
							       new_fltr->fltr.fltr.range.i_max_idx_fltr,
							       new_fltr->fltr.fltr.range.i_idx_len);
						if (max_res == 0) {
							if (new_fltr->fltr.is_move_row_fltr !=
							    mas_fltr->fltr.is_move_row_fltr) {
								MAB_LM_EVT mle;
								...
								m_LOG_MAB_OVERLAPPING_FLTRS(NCSFL_SEV_NOTICE,
											    inst->vrid, tbl_rec->tbl_id,
											    mas_fltr, new_fltr);
								inst->lm_cbfnc(&mle);
							...
							status =
							    mab_fltrid_list_add(&mas_fltr->fltr_ids, msg->fr_anc,
							...
							if (status != NCSCC_RC_SUCCESS) {
								...
								mle.i_vrid = inst->vrid;
								mle.i_which_mab = NCSMAB_MAS;
								m_LOG_MAB_EVT(NCSFL_SEV_NOTICE,
							...
							m_MAS_FLTR_FREE(new_fltr);
							m_MAB_DBG_TRACE2_MAS_MF_OP(inst, mas_fltr, tbl_rec->tbl_id,
										   MFM_MODIFY);
					...
					if (min_res == 0) {
						if (new_fltr->fltr.is_move_row_fltr == TRUE) {
							if (mas_fltr->fltr.is_move_row_fltr == TRUE) {
								mas_fltr->vcard = new_fltr->vcard;
								status =
								    mab_fltrid_list_add(&mas_fltr->fltr_ids,
											msg->fr_anc, fr_oaa_role,
											reg_req->fltr_id);
								if (status != NCSCC_RC_SUCCESS) {
									m_MAS_FLTR_FREE(new_fltr);

									ncshm_give_hdl(NCS_PTR_TO_INT32_CAST
										       (msg->yr_hdl));
									mas_mab_fltr_indices_cleanup(&msg->data.data.
												     reg.fltr);
									m_MMGR_FREE_MAB_MSG(msg);

									m_MAS_UNLK(&inst->lock);
									m_LOG_MAB_LOCK(MAB_LK_MAS_UNLOCKED,
										       &inst->lock);
									return NCSCC_RC_FAILURE;
								}

								m_MAS_FLTR_FREE(new_fltr);
								m_MAB_DBG_TRACE2_MAS_MF_OP(inst, mas_fltr,
											   tbl_rec->tbl_id, MFM_MODIFY);

								/* Sync Done message to the Standby */
								ncshm_give_hdl(NCS_PTR_TO_INT32_CAST(msg->yr_hdl));
								m_MAS_RE_REG_UNREG_SYNC_DONE(inst, msg,
											     tbl_rec->tbl_id);
						...
						if (mas_fltr == tbl_rec->fltr_list) {
							...
							if (max_res >= 0) {
								...
								m_MAB_DBG_TRACE2_MAS_MF_OP(inst, new_fltr,
											   tbl_rec->tbl_id, MFM_CREATE);
						...
						if (mas_fltr->next != NULL) {
							...
							if (max_res == 0) {
								if (new_fltr->fltr.is_move_row_fltr == TRUE) {
									if (mas_fltr->next->fltr.is_move_row_fltr ==
									    TRUE) {
										mas_fltr->next->vcard = new_fltr->vcard;
										status =
										    mab_fltrid_list_add(&mas_fltr->
													next->fltr_ids,
													msg->fr_anc,
													fr_oaa_role,
													reg_req->
										...
										if (status != NCSCC_RC_SUCCESS) {
											...
											ncshm_give_hdl
											    (NCS_PTR_TO_INT32_CAST
											     (msg->yr_hdl));
											mas_mab_fltr_indices_cleanup
											    (&msg->data.data.reg.fltr);
											m_MMGR_FREE_MAB_MSG(msg);
										...
										m_MAB_DBG_TRACE2_MAS_MF_OP(inst,
													   mas_fltr->
													   next,
													   tbl_rec->
													   tbl_id,
							...
							m_MAB_DBG_TRACE2_MAS_MF_OP(inst, new_fltr, tbl_rec->tbl_id,
										   MFM_CREATE);
	    ...
	    /*******[ FREE STUFF FROM RANGE HERE ]*************/
			...
			/* process the exact filter */
			status = mas_exact_fltr_process(msg, inst, tbl_rec, new_fltr, reg_req->fltr_id, fr_oaa_role);
			if (status != NCSCC_RC_SUCCESS) {
				/* log the error */
				if (status == NCSCC_RC_DUPLICATE_FLTR) {
					m_LOG_MAB_EVT(NCSFL_SEV_NOTICE, MAB_EV_LM_MAS_FLTR_REG_EXIST);
					m_LOG_MAB_OVERLAPPING_FLTRS(NCSFL_SEV_NOTICE, inst->vrid, tbl_rec->tbl_id,
			...
			/* do a check for identical filters first */
			{
				...
				if (cur_fltr != NULL) {
					if (reg_req->fltr.type == cur_fltr->fltr.type) {
						switch (reg_req->fltr.type) {
						case NCSMAB_FLTR_SAME_AS:
							if (cur_fltr->fltr.fltr.same_as.i_table_id ==
							    new_fltr->fltr.fltr.same_as.i_table_id)
								fltr_matched = TRUE;
							break;
						case NCSMAB_FLTR_ANY:
							fltr_matched = TRUE;
						...
						if (fltr_matched == TRUE) {
							if (memcmp
							    (&cur_fltr->vcard, &new_fltr->vcard,
							     sizeof(new_fltr->vcard)) == 0) {
#if(NCS_MAS_RED == 1)
								o_fltr_id = 0;
								status =
								    mab_fltrid_list_get(&cur_fltr->fltr_ids,
											msg->fr_anc, &o_fltr_id);
								if ((status == NCSCC_RC_SUCCESS)
								    && (o_fltr_id == reg_req->fltr_id)
								    && (inst->red.ha_state ==
									NCS_APP_AMF_HA_STATE_STANDBY)) {
									m_MAS_FLTR_FREE(new_fltr);
								...
								status =
								    mab_fltrid_list_add(&cur_fltr->fltr_ids,
											msg->fr_anc, fr_oaa_role,
											reg_req->fltr_id);
								if (status != NCSCC_RC_SUCCESS) {
									/* log the failure */
									if (status == NCSCC_RC_DUPLICATE_FLTR) {
										m_LOG_MAB_EVT(NCSFL_SEV_NOTICE,
											      MAB_EV_LM_MAS_FLTR_REG_EXIST);
										m_LOG_MAB_OVERLAPPING_FLTRS
										    (NCSFL_SEV_NOTICE, inst->vrid,
										     tbl_rec->tbl_id,
										     tbl_rec->fltr_list, new_fltr);
									}
									m_MAS_FLTR_FREE(new_fltr);
								...
								m_MAB_DBG_TRACE2_MAS_MF_OP(inst, cur_fltr,
											   tbl_rec->tbl_id, MFM_MODIFY);
			...
			if ((tbl_rec->fltr_list != NULL) && (reg_req->fltr.type != NCSMAB_FLTR_SAME_AS)) {
				...
				if (reg_req->fltr.type == NCSMAB_FLTR_SAME_AS) {
					if ((same_as_tbl_rec =
					     mas_row_rec_find(inst, reg_req->fltr.fltr.same_as.i_table_id)) != NULL) {
						if (same_as_tbl_rec->fltr_list != NULL) {
							 ...
							 */
							if (same_as_tbl_rec->fltr_list->fltr.type ==
							    NCSMAB_FLTR_SAME_AS) {
								MAB_LM_EVT mle;
								...
								mle.i_vrid = inst->vrid;
								mle.i_which_mab = NCSMAB_MAS;
								m_LOG_MAB_EVT(NCSFL_SEV_ERROR,
									      MAB_EV_LM_MAS_FLTR_REG_INV_SA);
								inst->lm_cbfnc(&mle);
								ncshm_give_hdl(NCS_PTR_TO_INT32_CAST(msg->yr_hdl));
								m_MAS_FLTR_FREE(new_fltr);
								m_MMGR_FREE_MAB_MSG(msg);
								m_LOG_MAB_TBL_DETAILS(NCSFL_LC_HEADLINE,
										      NCSFL_SEV_ERROR,
										      MAB_MAS_FLTR_SA_BAD_DST_TBL,
										      inst->vrid,
										      reg_req->fltr.fltr.same_as.
										      i_table_id);
								m_MAS_UNLK(&inst->lock);
								m_LOG_MAB_LOCK(MAB_LK_MAS_UNLOCKED, &inst->lock);
								return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
					...
					if (tbl_rec->fltr_list != NULL) {
						if (tbl_rec->fltr_list->fltr.fltr.same_as.i_table_id ==
						    new_fltr->fltr.fltr.same_as.i_table_id) {
							...
							m_LOG_MAB_EVT(NCSFL_SEV_NOTICE, MAB_EV_LM_MAS_FLTR_REG_EXIST);
							m_LOG_MAB_OVERLAPPING_FLTRS(NCSFL_SEV_NOTICE, inst->vrid,
										    tbl_rec->tbl_id, tbl_rec->fltr_list,
										    new_fltr);
	...
	if (unreg_req->fltr_id == 1) {
		if (tbl_rec->dfltr_regd == FALSE) {
			...
			m_LOG_MAB_FLTR_DETAILS(NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR,
					       MAB_MAS_FLTR_RMV_FAILED, unreg_req->tbl_id, unreg_req->fltr_id, -1);
		...
		if ((tbl_rec->fltr_list->fltr.type == NCSMAB_FLTR_RANGE) ||
			...
			if (tmp == NULL) {
				...
				m_LOG_MAB_FLTR_DETAILS(NCSFL_LC_HEADLINE, NCSFL_SEV_ERROR, MAB_MAS_FLTR_RMV_FAILED,
						       unreg_req->tbl_id, unreg_req->fltr_id,
						       tbl_rec->fltr_list->fltr.type);
		...
		if ((rem_fltr_ids->fltr_ids.active_fltr != NULL) && (rem_fltr_ids->fltr_ids.fltr_id_list != NULL)) {
		...
		if (do_del_fltr == TRUE) {
			...
			if ((tbl_rec->fltr_list == NULL) && (tbl_rec->dfltr_regd != TRUE)) {
				...
				m_LOG_MAB_TBL_DETAILS(NCSFL_LC_DATA, NCSFL_SEV_DEBUG,
						      MAB_MAS_RMV_ROW_REC_SUCCESS, inst->vrid, unreg_req->tbl_id);
				m_MAB_DBG_TRACE2_MAS_TR_DALLOC(tbl_rec);
...
uns32 mas_cli_msg_to_mac(MAB_MSG *msg, MAS_TBL *inst)
{
	 ...
	 */
	{
		...
		if ((se = ncsstack_push(&mib_rsp->stack, NCS_SE_TYPE_BACKTO, (uns8)sizeof(NCS_SE_BACKTO))) == NULL) {
			m_LOG_MAB_HEADLINE(NCSFL_SEV_ERROR, MAB_HDLN_MAS_PUSH_FISE_FAILED);
			m_MAS_UNLK(&inst->lock);
			m_LOG_MAB_LOCK(MAB_LK_MAS_UNLOCKED, &inst->lock);
			return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
...
uns32 mas_relay_msg_to_mac(MAB_MSG *msg, MAS_TBL *inst, NCS_BOOL is_req)
{
	...
	/* KCQ: for now all messages going to a MAC are
	   responses. It won't be so when traps are implemented
	 */
	if (mas_forward_msg(msg, mso, inst->mds_hdl, dst_vcard, (SS_SVC_ID)dst_svc_id, is_req) == NCSCC_RC_FAILURE) {
		return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
	} else {
...
uns32 mas_next_op_stack_push(MAS_TBL *inst, NCS_STACK *stack, uns32 fltr_id)
{
	...
	if ((se = ncsstack_push(stack, NCS_SE_TYPE_FILTER_ID, (uns8)sizeof(NCS_SE_FILTER_ID))) == NULL) {
		return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
	}
	...
	if ((se = ncsstack_push(stack, NCS_SE_TYPE_BACKTO, (uns8)sizeof(NCS_SE_BACKTO))) == NULL) {
		return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
	}
		       ...
		       MAS_TBL *inst, MAS_ROW_REC *tbl_rec, MAS_FLTR *new_fltr, uns32 i_fltr_id, V_DEST_RL oaa_role)
{
	...
	mf = &tbl_rec->fltr_list->fltr;
	if ((mf->type != new_fltr->fltr.type) || (mf->fltr.exact.i_bgn_idx != new_fltr->fltr.fltr.exact.i_bgn_idx)) {
	...
	/* see if there's already a filter with the given fltr_id/vcard pair registered */
	fltr = mas_fltr_find(inst, &tbl_rec->fltr_list, i_fltr_id, msg->fr_anc, new_fltr->vcard);
	if ((fltr != NULL)
#if (NCS_MAS_RED == 1)
	    && (inst->red.ha_state != NCS_APP_AMF_HA_STATE_STANDBY)
#endif
...
/* Following is a cut and paste, I am not sure why this is required...  */
	...
	for (mas_fltr = tbl_rec->fltr_list; mas_fltr != NULL; mas_fltr = mas_fltr->next) {
		...
		m_MAB_FLTR_CMP(res, mas_fltr->fltr.fltr.exact.i_exact_idx, new_fltr->fltr.fltr.exact.i_exact_idx, size);
	...
	if ((res == 0) && (duplicate == TRUE)) {
		if ((new_fltr->fltr.is_move_row_fltr != mas_fltr->fltr.is_move_row_fltr) ||
			...
			/* log the error */
			m_LOG_MAB_EVT(NCSFL_SEV_NOTICE, MAB_EV_LM_MAS_FLTR_REG_EXIST);
			m_LOG_MAB_OVERLAPPING_FLTRS(NCSFL_SEV_NOTICE, inst->vrid, tbl_rec->tbl_id, mas_fltr, new_fltr);
			return NCSCC_RC_FAILURE;
		}
...
{
	/* log the error */
	m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_INFORM, event_type);
...
/* to add this filtr_id and anchor value pair */
static uns32 mab_fltrid_list_add(MAS_FLTR_IDS *fltr_ids, MAB_ANCHOR i_anchor, V_DEST_RL oaa_role, uns32 i_fltr_id)
{
	...
	if (oaa_role == V_DEST_RL_ACTIVE) {
		...
		if (((status == NCSCC_RC_SUCCESS) && (dummy != i_fltr_id)) || (status != NCSCC_RC_SUCCESS)) {
			/* allocate the memory for the new node */
			...
			if (tbl_rec->ss_id == msg->fr_svc) {
				...
				if (tbl_rec->dfltr_regd == TRUE) {
					...
					if (memcmp(&tbl_rec->dfltr.vcard, &msg->fr_card, sizeof(msg->fr_card)) == 0) {
						...
						status =
						    mas_fltr_ids_adjust(&tbl_rec->dfltr.fltr_ids, msg->fr_anc,
									msg->fr_role);
				...
				for (fltr = tbl_rec->fltr_list; fltr != NULL;) {
					if (memcmp(&fltr->vcard, &msg->fr_card, sizeof(fltr->vcard)) == 0) {
						status =
						    mas_fltr_ids_adjust(&fltr->fltr_ids, msg->fr_anc, msg->fr_role);
						if (status != NCSCC_RC_FAILURE) {
							/* log the error */
							/* continue cleaning up the other fitlers of the OAA in other tables */
						}
