...
static uns32 mas_red_msg_enc(MAS_TBL *inst, NCS_MBCSV_CB_ENC *enc);

/* to decode the request from MBCSv */
static uns32 mas_red_msg_dec(MAS_TBL *inst, NCS_MBCSV_CB_DEC *dec);

/* encode the async count of all the buckets into a user buf */
static uns32 mas_red_async_count_enc(uns32 *async_count, NCS_UBAID *uba);

/* to decode the async count array */
static uns32 mas_red_async_count_dec(uns32 *async_count, NCS_UBAID *uba);

/* MBCSv COLD Sync Resp encode callback */
/* Encodes one bucket of hash table */
static uns32 mas_red_cold_sync_enc(MAS_TBL *inst, NCS_MBCSV_CB_ENC *enc);

/* MBCSv COLD Sync Resp decode callback */
/* Decodes one bucket of hash table */
static uns32 mas_red_cold_sync_dec(MAS_TBL *inst, NCS_MBCSV_CB_DEC *dec);

static uns32 mas_red_warm_sync_resp_dec(MAS_TBL *inst, NCS_MBCSV_CB_DEC *dec);

/* Active MAS decodes the Datareq and sets the context for Data Resp */
static uns32 mas_red_data_req_dec(NCS_MBCSV_CB_DEC *dec);

/* Active MAS sends DATA reposne to Standby MAS */
static uns32 mas_red_data_rsp_enc_cb(MAS_TBL *inst, NCS_MBCSV_CB_ENC *enc);

/* Standby MAS received the data response */
static uns32 mas_red_data_resp_dec_cb(MAS_TBL *inst, NCS_MBCSV_CB_DEC *dec);

/* Encode all the table-records in a bucket and the associated async count */
static uns32 mas_red_bucket_enc(uns8 bucket_id, MAS_ROW_REC *tbl_list, uns32 async_count, NCS_MBCSV_CB_ENC *enc);

/* Decode all the table-records in a bucket */
static uns32 mas_red_bucket_dec(uns8 *bucket_id, MAS_ROW_REC **tbl_list, uns32 *async_count, NCS_MBCSV_CB_DEC *dec);

/* encode filter list */
static uns32 mas_red_mas_fltr_enc(MAS_FLTR *fltr_list, NCS_UBAID *data_uba);

/* decode the filter list */
static uns32 mas_red_mas_fltr_dec(MAS_FLTR **fltr_list, NCS_UBAID *data_uba);

/* encode the default filter attributes */
static uns32 mas_red_mas_dfltr_enc(MAS_DFLTR *dfltr, NCS_UBAID *data_uba);

/* decode the default filter attributes */
static uns32 mas_red_mas_dfltr_dec(MAS_DFLTR *dfltr, NCS_UBAID *data_uba);

/* encode the filter-ids linked list */
static uns32 mas_red_mas_fltr_ids_enc(MAB_FLTR_ANCHOR_NODE *fltr_ids, NCS_UBAID *data_uba);

/* decode the filter-ids linked list */
static uns32 mas_red_mas_fltr_ids_dec(MAB_FLTR_ANCHOR_NODE **fltr_ids, NCS_UBAID *data_uba);

/* Encode the Async update to be sent to Standby MAS */
static uns32 mas_red_updt_enc(NCS_MBCSV_CB_ENC *enc);

/* Decode the Async update of Active at Standby MAS */
static uns32 mas_red_updt_dec(MAS_TBL *inst, NCS_MBCSV_CB_DEC *dec);

static uns32 mas_red_process_mab_msg(MAS_TBL *inst, MAS_ASYNC_UPDATE_TYPE async_type);

static uns32 mas_red_err_ind_handle(MAS_TBL *inst, NCS_MBCSV_CB_ARG *arg);
...
uns32 mas_mbcsv_cb(NCS_MBCSV_CB_ARG *arg)
{
	...
	if (arg == NULL) {
		/* log the failure */
		m_LOG_MAB_ERROR_NO_DATA(NCSFL_SEV_ERROR, NCSFL_LC_SVC_PRVDR, MAB_MAS_ERR_MBCSV_CB_NULL_ARG);
	...
	if ((inst = (MAS_TBL *)m_MAS_VALIDATE_HDL(hm_hdl)) == NULL) {
		/* log the failure */
		m_LOG_MAB_ERROR_NO_DATA(NCSFL_SEV_ERROR, NCSFL_LC_SVC_PRVDR, MAB_MAS_ERR_MBCSV_CB_INVALID_HDL);
		return m_MAB_DBG_SINK(NCSCC_RC_FAILURE);
	}
    ...
    **/
	...
	case NCS_MBCSV_CBOP_ERR_IND:
		...
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_ERROR,
				   MAB_MAS_ERR_MBCSV_CB_ERR_IND, arg->info.error.i_code, arg->info.error.i_err);
	...
	default:
		m_LOG_MAB_ERROR_I(NCSFL_SEV_INFO, MAB_MAS_ERR_MBCSV_OP_NOT_SUPPORTED, arg->i_op);
		rc = NCSCC_RC_SUCCESS;
		break;
	}			/* switch(arg->i_op) */
...
static uns32 mas_red_msg_enc(MAS_TBL *inst, NCS_MBCSV_CB_ENC *enc)
{
	...
	m_LOG_MAB_HDLN_I(NCSFL_SEV_DEBUG, MAB_HDLN_MAS_MBCSV_ENC, enc->io_msg_type);
	...
	case NCS_MBCSV_MSG_COLD_SYNC_REQ:
		...
		memset(inst->red.async_count, 0, sizeof(uns32) * MAB_MIB_ID_HASH_TBL_SIZE);
	...
	default:
		...
		m_LOG_MAB_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR,
				     MAB_MAS_ERR_MBCSV_ENC_UNSUP_MSG_TYPE, enc->io_msg_type);
		break;
	}
...
static uns32 mas_red_msg_dec(MAS_TBL *inst, NCS_MBCSV_CB_DEC *dec)
{
	...
	/* log the msg_type */
	m_LOG_MAB_HDLN_I(NCSFL_SEV_DEBUG, MAB_HDLN_MAS_MBCSV_DEC, dec->i_msg_type);
	...
	default:
		rc = NCSCC_RC_FAILURE;
		m_LOG_MAB_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR,
				     MAB_MAS_ERR_MBCSV_DEC_UNSUP_MSG_TYPE, dec->i_msg_type);
		break;
	}
...
static uns32 mas_red_cold_sync_enc(MAS_TBL *inst, NCS_MBCSV_CB_ENC *enc)
{
	...
	/* encode this bucket */
	status = mas_red_bucket_enc(bucket_id, inst->hash[bucket_id], inst->red.async_count[bucket_id], enc);
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_COLD_SYNC_ENC_BKT_FAILED, bucket_id);
...
static uns32 mas_red_cold_sync_dec(MAS_TBL *inst, NCS_MBCSV_CB_DEC *dec)
{
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_COLD_SYNC_DEC_BKT_FAILED, dec_bucket_id);
		return status;
	}
	...
	if (inst->hash[dec_bucket_id] == NULL) {
		...
		m_LOG_MAB_HDLN_I(NCSFL_SEV_DEBUG, MAB_HDLN_MAS_COLD_SYNC_DEC_SUCCESS, dec_bucket_id);
	} else {

		/* log the failure */
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_COLD_SYNC_DEC_BKT_NON_NULL, dec_bucket_id);
...
static uns32 mas_red_data_rsp_enc_cb(MAS_TBL *inst, NCS_MBCSV_CB_ENC *enc)
{
	...
	if ((warm_sync_cntxt == NULL) || (warm_sync_cntxt->bkt_count == 0)) {
		m_LOG_MAB_ERROR_NO_DATA(NCSFL_SEV_ERROR, NCSFL_LC_HEADLINE, MAB_MAS_ERR_MBCSV_WARM_SYNC_BKT_CNT_ZERO);
		return NCSCC_RC_FAILURE;
	}
...
static uns32 mas_red_bucket_dec(uns8 *bucket_id, MAS_ROW_REC **tbl_list, uns32 *async_count, NCS_MBCSV_CB_DEC *dec)
{
	...
	*tbl_list = NULL;
	...
	while (tbl_cnt) {
		...
		if (tbl_rec == NULL) {
			/* log the failure */
			m_LOG_MAB_MEMFAIL(NCSFL_SEV_CRITICAL, MAB_MF_MAS_ROW_REC_ALLOC_FAILED, "mas_red_bucket_dec()");
...
static uns32 mas_red_mas_fltr_dec(MAS_FLTR **fltr_list, NCS_UBAID *data_uba)
{
	...
	*fltr_list = NULL;
	while (fltr_cnt) {
		...
		if (new_fltr == NULL) {
			/* log the failure */
			m_LOG_MAB_MEMFAIL(NCSFL_SEV_CRITICAL, MAB_MF_MAS_FLTR_CREATE_FAILED, "mas_red_mas_fltr_dec()");
			return NCSCC_RC_OUT_OF_MEM;
		}
...
\****************************************************************************/
/* encode the filter-ids linked list */
static uns32 mas_red_mas_fltr_ids_enc(MAB_FLTR_ANCHOR_NODE *fltr_anc_ids, NCS_UBAID *data_uba)
{
	uns8 *data, *p_anc_cnt;
	uns32 anc_count;
...
static uns32 mas_red_warm_sync_resp_dec(MAS_TBL *inst, NCS_MBCSV_CB_DEC *dec)
{
	...
	if (status != NCSCC_RC_SUCCESS) {
		...
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_MBCSV_API_DATA_REQ_FAILED, bkt_count, status);
		return status;
	}
...
static uns32 mas_red_data_req_dec(NCS_MBCSV_CB_DEC *dec)
{
	...
	if (out_of_sync_buckets == 0) {
		...
		m_LOG_MAB_ERROR_NO_DATA(NCSFL_SEV_ERROR, NCSFL_LC_HEADLINE, MAB_MAS_ERR_MBCSV_DATA_REQ_DEC_BKTCNT_ZERO);
		return NCSCC_RC_FAILURE;
	}
...
uns32 mas_red_sync_done(MAS_TBL *inst, MAS_ASYNC_UPDATE_TYPE type)
{
	uns32 status;
	MAB_MSG *new_msg;
	...
	if (new_msg == NULL) {
		...
		m_LOG_MAB_MEMFAIL(NCSFL_SEV_CRITICAL, MAB_MF_MABMSG_CREATE, "mas_red_sync_done()");
		return NCSCC_RC_OUT_OF_MEM;
	}
...
uns32 mas_red_sync(MAS_TBL *inst, MAB_MSG *msg, MAS_ASYNC_UPDATE_TYPE type)
{
	uns32 status;
	NCS_MBCSV_ARG mbcsv_arg;
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_MBCSV_API_SND_USR_ASYNC_FAILED, type, status);
		return status;
	}

	/* log the event of successful sending the update */
	m_LOG_MAB_HDLN_II(NCSFL_SEV_INFO, MAB_HDLN_MAS_MBCSV_ASYNC_UPDT_SUCCESS, type, status);
...
static uns32 mas_red_updt_enc(NCS_MBCSV_CB_ENC *enc)
{
	...
	mas_mbcsv_msg_fmt_ver = m_NCS_MBCSV_FMT_GET(enc->i_peer_version,
						    m_NCS_MAS_MBCSV_VERSION, m_NCS_MAS_MBCSV_VERSION_MIN);
...
static uns32 mas_red_process_mab_msg(MAS_TBL *inst, MAS_ASYNC_UPDATE_TYPE async_type)
{
	...
	case MAS_ASYNC_MIB_ARG:
		...
		ncsmib_arg_free_resources(inst->red.process_msg->data.data.snmp,
					  (m_NCSMIB_ISIT_A_REQ(inst->red.process_msg->data.data.snmp->i_op) ? TRUE :
					   FALSE));
		m_MMGR_FREE_MAB_MSG(inst->red.process_msg);
		break;
	default:
...
*                               for details.                                    *
\*******************************************************************************/
uns32 mas_mbcsv_interface_initialize(MAS_MBCSV_ATTRIBS *mbcsv_attribs)
{
	uns32 status;
	NCS_MBCSV_ARG mbcsv_arg;
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_MBCSV_INIT_FAILED, NCS_SERVICE_ID_MAB, status);
		return status;
	}
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_MBCSV_SEL_OBJ_GET_FAILED, mbcsv_attribs->mbcsv_hdl, status);
	...
	if (inst->red.ha_state == SA_AMF_HA_STANDBY) {
		...
		case NCS_MBCSV_DATA_RSP_CMPLT_TMR_EXP:
			...
			if (gl_mas_amf_attribs.amf_attribs.amfHandle != 0) {
				saf_status = saAmfComponentErrorReport(gl_mas_amf_attribs.amf_attribs.amfHandle, &gl_mas_amf_attribs.amf_attribs.compName, 0,	/* errorDetectionTime; AvNd will provide this value */
...
static void mas_dump_tbl_rec(FILE *fp, MAS_ROW_REC *tbl_rec)
{
	...
	while (curr_fltr) {
		...
		case NCSMAB_FLTR_ANY:
			...
			break;
		case NCSMAB_FLTR_SAME_AS:
			fprintf(fp, "\n\t\t\tFilter Type: SAME_AS; SameAs Table-ID: %d",
				curr_fltr->fltr.fltr.same_as.i_table_id);
			break;
		case NCSMAB_FLTR_EXACT:
			fprintf(fp, "\n\t\t\tFilter Type: EXACT");
		...
		fprintf(fp, "\n\t\t\tIs MoveRow Filter: %s", (curr_fltr->fltr.is_move_row_fltr) ? "TRUE" : "FALSE");
...
static void mas_dump_vcard(FILE *fp, MDS_DEST mds_dest)
{
	...
	if (m_NCS_NODE_ID_FROM_MDS_DEST(mds_dest) == 0)
		fprintf(fp, "\n\t\t\t\tVDEST:%d", m_MDS_GET_VDEST_ID_FROM_MDS_DEST(mds_dest));
	else
		fprintf(fp, "\n\t\t\t\tADEST:node_id:%d, v1.pad16:%d, v1.vcard:%lu",
			m_NCS_NODE_ID_FROM_MDS_DEST(mds_dest), 0, (long)(mds_dest));
	return;
}
...
static void mas_dump_fltr_ids_list(FILE *fp, MAS_FLTR_IDS *fltr_ids)
{
	...
	if (list != NULL) {
		fprintf(fp, "\n\t\t\t\t Anchor Vaue: %llu, Filter-ID: %d", list->anchor, list->fltr_id);
	}
