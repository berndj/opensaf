...
static uns32 mas_amf_componentize(NCS_APP_AMF_ATTRIBS *amf_attribs);

/* Stethascope of AMF */
static void mas_amf_health_check(SaInvocationT, const SaNameT *, SaAmfHealthcheckKeyT *);

/* pest for Suicide */
static void mas_amf_comp_terminate(SaInvocationT, const SaNameT *);

static void mas_amf_comp_terminate_complete(void);

/* to create a new CSI or to change the state of an existing CSI */
static void mas_amf_csi_set(SaInvocationT, const SaNameT *, SaAmfHAStateT, SaAmfCSIDescriptorT);

/* to remove one or more CSIs from MAS */
static void mas_amf_csi_remove(SaInvocationT, const SaNameT *, const SaNameT *, SaAmfCSIFlagsT);

/* to create a CSI in ACTIVE state */
static uns32 mas_amf_INIT_ACTIVE_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to move to ACTIVE state */
static uns32 mas_amf_ACTIVE_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to create a CSI in STANDBY state */
static uns32 mas_amf_INIT_STANDBY_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to STANDBY state */
static uns32 mas_amf_STANDBY_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to QUIESCED state */
static uns32 mas_amf_QUIESCED_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to QUIESCING state */
static uns32 mas_amf_QUIESCING_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* from QUIESCING to QUIESCED */
static uns32 mas_amf_QUIESCING_QUIESCED_process(void *, PW_ENV_ID, SaAmfCSIDescriptorT);

/* to create a new CSI */
static void mas_amf_csi_new(SaInvocationT, const SaNameT *, SaAmfHAStateT, SaAmfCSIDescriptorT);

/* to do the role change for all the CSIs */
static void mas_amf_csi_set_all(SaInvocationT, const SaNameT *, SaAmfHAStateT, SaAmfCSIDescriptorT);

/* do a role change for a specific CSI */
static void mas_amf_csi_set_one(SaInvocationT, const SaNameT *, SaAmfHAStateT, SaAmfCSIDescriptorT);

/* to remove a single CSI */
static SaAisErrorT mas_amf_csi_remove_one(const MAS_CSI_NODE *);

/* to remove all the CSIs in MAS */
static SaAisErrorT mas_amf_csi_remove_all(void);

/* to create a new Environment */
static uns32 mas_amf_csi_create_and_install_env(PW_ENV_ID, SaAmfHAStateT, SaAmfCSIDescriptorT);

/* to change the role of the VDEST */
static uns32 mas_amf_csi_vdest_role_change(MAS_CSI_NODE *csi, SaAmfHAStateT);

/* to destroy the table information of an environment */
static uns32 mas_amf_csi_mas_tbl_destroy(MAS_TBL *inst);

/* serach by the env-id in the global CSI list */
static MAS_CSI_NODE *mas_amf_csi_list_find_envid(PW_ENV_ID);

/* search by CSI name in the global CSI list */
static MAS_CSI_NODE *mas_amf_csi_list_find_csiname(const SaNameT *);

/* delink this node from the global CSI list */
static MAS_CSI_NODE *mas_amf_csi_list_delink(const SaNameT *);

/* decodes the CSI attributes and gives the Environment ID */
static PW_ENV_ID mas_amf_csi_attrib_decode(SaAmfCSIAttributeListT csiAttr);

static void mas_process_sig_usr1_signal(void);

/* signal handler to dump the MAS data structures */
static void mas_amf_sigusr2_handler(int i_sig_num);

extern NCS_BOOL gl_inited;

/* to change the state */
static uns32 mas_amf_csi_state_change_process(MAS_CSI_NODE *csi_node, SaAmfHAStateT haState);
 ...
 *                   - gl_mas_amf_attribs                                     *
\*****************************************************************************/
void mas_mbx_amf_process(SYSF_MBX *mas_mbx)
{
	...
	if ((ncs_app_signal_install(SIGUSR2, mas_amf_sigusr2_handler)) == -1) {
		m_LOG_MAB_ERROR_NO_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_FUNC_RET_FAIL, MAB_MAS_ERR_SIGUSR2_INSTALL_FAILED);
		return;
	}
#if (NCS_MAS_RED == 1)
		...
		if (count > 0) {
			if (gl_mas_amf_attribs.amf_attribs.amfHandle == 0) {
				if (m_NCS_SEL_OBJ_ISSET(gl_mas_amf_attribs.amf_attribs.sighdlr_sel_obj, &readfds)) {
					...
					m_NCS_SEL_OBJ_RMV_IND(gl_mas_amf_attribs.amf_attribs.sighdlr_sel_obj, TRUE,
							      TRUE);
				...
				if (m_NCS_SEL_OBJ_ISSET(ncs_amf_sel_obj, &readfds)) {
					...
					if (saf_status != SA_AIS_OK) {
						...
						m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR,
								  MAB_MAS_ERR_AMF_DISPATCH_FAILED, saf_status);
						m_MAB_DBG_TRACE("\nmas_mbx_amf_process():left.");
						return;
					}
			...
			if (m_NCS_SEL_OBJ_ISSET(mbx_fd, &readfds)) {
				...
				if (status != NCSCC_RC_SUCCESS) {
					/* log the error */
					m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_DO_EVTS_FAILED, status);
					m_MAB_DBG_TRACE("\nmas_mbx_amf_process():left.");
					return;
				}
...
static uns32 mas_amf_componentize(NCS_APP_AMF_ATTRIBS *amf_attribs)
{
	...
	status = ncs_app_amf_attribs_init(amf_attribs,
					  ...
					  m_MAS_DISPATCH,
					  m_MAS_HB_INVOCATION_TYPE, m_MAS_RECOVERY, &ha_hdls, &amf_clbks);
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_AMF_ATTRIBS_INIT_FAILED, status);
		/* reset the attribs */
...
\*****************************************************************************/
static void
mas_amf_health_check(SaInvocationT invocation, const SaNameT *compName, SaAmfHealthcheckKeyT *healthcheckKey)
{
	SaAisErrorT saf_status = SA_AIS_OK;
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
uns32 mas_amf_prepare_will(void)
{
	uns32 status;
	...
	if (status != NCSCC_RC_SUCCESS) {
		...
		m_LOG_MAB_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR, MAB_MAS_ERR_VDEST_DESTROY_FAILED, status);
		m_MAB_DBG_TRACE("\nmas_amf_prepare_will():left.");
		return status;
	}
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
static void mas_amf_comp_terminate(SaInvocationT invocation, const SaNameT *compName)
{
	SaAisErrorT saf_status = SA_AIS_OK;
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
static void mas_amf_comp_terminate_complete()
{
		...
		const SaNameT *compName, SaAmfHAStateT haState, SaAmfCSIDescriptorT csiDescriptor)
{
	...
	m_LOG_MAB_CSI_DETAILS(NCSFL_SEV_NOTICE,
			      MAB_HDLN_MAS_AMF_CSI_DETAILS, csiDescriptor.csiFlags, tCompName, tCsiName, haState);
	...
	default:
		/* log the error code */
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_AMF_CSI_FLAGS_ILLEGAL, csiDescriptor.csiFlags);
		saAmfResponse(gl_mas_amf_attribs.amf_attribs.amfHandle, invocation, SA_AIS_ERR_FAILED_OPERATION);
		...
		const SaNameT *compName, SaAmfHAStateT haState, SaAmfCSIDescriptorT csiDescriptor)
{
		...
		if (csi->csi_info.ha_state != haState) {
			...
			if (status != NCSCC_RC_SUCCESS) {
				m_LOG_MAB_ERROR_II(NCSFL_LC_STATE_MCH, NCSFL_SEV_ERROR,
						   MAB_MAS_ERR_AMF_STATE_CHG_FAILED, haState, csi->csi_info.ha_state);
	...
	if (status == NCSCC_RC_SUCCESS) {
		...
		m_LOG_MAB_STATE_CHG(NCSFL_SEV_NOTICE,
				    MAB_HDLN_MAS_AMF_CSI_STATE_CHG_SUCCESS, NCS_APP_AMF_HA_STATE_NULL, haState);
		saf_status = SA_AIS_OK;
	} else {
		saf_status = SA_AIS_ERR_FAILED_OPERATION;
		...
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_CSI_ADD_ONE_FAILED, status, env_id);

		/* log the state change failure */
		m_LOG_MAB_STATE_CHG(NCSFL_SEV_ERROR,
				    MAB_HDLN_MAS_AMF_CSI_STATE_CHG_FAILED, NCS_APP_AMF_HA_STATE_NULL, haState);
	}
...
\*****************************************************************************/
static uns32
mas_amf_csi_create_and_install_env(PW_ENV_ID envid, SaAmfHAStateT haState, SaAmfCSIDescriptorT csiDescriptor)
{
	MAS_CSI_NODE *csi = NULL;
		    ...
		    const SaNameT *compName, SaAmfHAStateT haState, SaAmfCSIDescriptorT csiDescriptor)
{
	...
	while (csi_node) {	/* for all the CSIs in MAS */
		...
		status =
		    gl_mas_amf_attribs.amf_attribs.ncs_app_amf_hastate_dispatch[csi_node->csi_info.
	...
	if (saf_status == SA_AIS_OK) {
		...
		m_LOG_MAB_STATE_CHG(NCSFL_SEV_NOTICE, MAB_HDLN_MAS_AMF_CSI_STATE_CHG_SUCCESS, prevState, haState);
	} else {
		/* log the state change failure */
		m_LOG_MAB_STATE_CHG(NCSFL_SEV_ERROR, MAB_HDLN_MAS_AMF_CSI_STATE_CHG_FAILED, prevState, haState);
	}
		    ...
		    const SaNameT *compName, SaAmfHAStateT haState, SaAmfCSIDescriptorT csiDescriptor)
{
	...
	status =
	    gl_mas_amf_attribs.amf_attribs.ncs_app_amf_hastate_dispatch[csi->csi_info.ha_state][haState] ((void *)csi,
													  csi->csi_info.
													  env_id,
													  csiDescriptor);
	if (status == NCSCC_RC_SUCCESS) {
...
\*****************************************************************************/
static void
mas_amf_csi_remove(SaInvocationT invocation, const SaNameT *compName, const SaNameT *csiName, SaAmfCSIFlagsT csiFlags)
{
	SaAisErrorT saf_status = SA_AIS_ERR_FAILED_OPERATION;
	MAS_CSI_NODE *csi_node = NULL;
...
static uns32 mas_amf_INIT_ACTIVE_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	return mas_amf_csi_create_and_install_env(envid, SA_AMF_HA_ACTIVE, csiDescriptor);
}
...
\*****************************************************************************/
/* Go to ACTIVE State */
static uns32 mas_amf_ACTIVE_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
#if (NCS_MAS_RED==1)
...
#endif
	return (mas_amf_csi_state_change_process((MAS_CSI_NODE *)data, SA_AMF_HA_ACTIVE));
}
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
static uns32 mas_amf_INIT_STANDBY_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	return mas_amf_csi_create_and_install_env(envid, SA_AMF_HA_STANDBY, csiDescriptor);
}
...
\*****************************************************************************/
/* Go to STANDBY State */
static uns32 mas_amf_STANDBY_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	return (mas_amf_csi_state_change_process((MAS_CSI_NODE *)data, SA_AMF_HA_STANDBY));
}
...
static uns32 mas_amf_QUIESCED_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	/* no data has been identified to do the checkpoint at this point of time */
	return (mas_amf_csi_state_change_process((MAS_CSI_NODE *)data, SA_AMF_HA_QUIESCED));
}
...
static uns32 mas_amf_QUIESCING_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
...
static uns32 mas_amf_QUIESCING_QUIESCED_process(void *data, PW_ENV_ID envid, SaAmfCSIDescriptorT csiDescriptor)
{
	/* nothing to do */
	return NCSCC_RC_SUCCESS;
}
...
static uns32 mas_amf_csi_state_change_process(MAS_CSI_NODE *csi_node, SaAmfHAStateT haState)
{
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_MBCSV_CKPT_CHG_ROLE_FAILED, haState, status);
	}
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
static SaAisErrorT mas_amf_csi_remove_one(const MAS_CSI_NODE *csi_node)
{
	NCSVDA_INFO vda_info;
	NCSMDS_INFO info;
	uns32 status;
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_MBCSV_CKPT_CLOSE_FAILED, csi_node->csi_info.env_id, status);
		goto enough;
	}
#endif   /* #if (NCS_MAS_RED == 1) */
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the error */
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_MDS_UNINSTALL_FAILED, status, csi_node->csi_info.env_id);
		goto enough;
	}
	...
	if ((gl_inited == TRUE) && (csi_node->csi_info.env_id != m_MAS_DEFAULT_ENV_ID)) {
		...
		if (status != NCSCC_RC_SUCCESS) {
			...
			/* log the error */
			m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
					   MAB_MAS_ERR_MDS_PWE_DESTROY_FAILED, status, csi_node->csi_info.env_id);
			goto enough;
		}
	}
	...
	if (status != NCSCC_RC_SUCCESS) {
		...
		m_LOG_MAB_ERROR_II(NCSFL_LC_FUNC_RET_FAIL, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_MASTBL_DESTROY_FAILED, status, csi_node->csi_info.env_id);
		goto enough;
	}
	...
	if (del_me == NULL) {
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_CSI_DELINK_FAILED, env_id);
		goto enough;
	}
 ...
 *  NOTE:                                                                     * 
 \****************************************************************************/
MAS_CSI_NODE *mas_amf_csi_install(PW_ENV_ID envid, SaAmfHAStateT haState)
{
	uns32 status;
	MAS_TBL *inst = NULL;
	MAS_CSI_NODE *csi = NULL;
	...
	if (csi == NULL) {
		...
		m_LOG_MAB_MEMFAIL(NCSFL_SEV_CRITICAL, MAB_MF_MAS_CSI_NODE_ALLOC_FAILED, "mas_amf_csi_install()");
	...
	if ((inst = m_MMGR_ALLOC_MAS_TBL) == NULL) {
		...
		m_LOG_MAB_MEMFAIL(NCSFL_SEV_CRITICAL, MAB_MF_MAS_TBL_ALLOC_FAILED, "mas_amf_csi_install()");
	...
	/* create the new PWE */
	if ((gl_inited == TRUE) && ((envid != m_MAS_DEFAULT_ENV_ID) ||	/* For multiple-CSI support */
				    ((envid == m_MAS_DEFAULT_ENV_ID) && (gl_mas_amf_attribs.amf_attribs.amfHandle != 0)))) {	/* For CSI-1 creation after CSI-remove-all */
		/* create the new PWE */
		memset(&vda_info, 0, sizeof(NCSVDA_INFO));
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_MBCSV_CKPT_OPEN_FAILED, envid, status);
		m_MAS_UNLK(&inst->lock);
		mas_amf_csi_mas_tbl_destroy(inst);
		m_MMGR_FREE_MAS_CSI_NODE(csi);
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_MBCSV_CKPT_CHG_ROLE_FAILED, envid, status);
		m_MAS_UNLK(&inst->lock);
		mas_amf_csi_mas_tbl_destroy(inst);
		m_MMGR_FREE_MAS_CSI_NODE(csi);
	...
	status = ncsmds_api(&info);
	if (status != NCSCC_RC_SUCCESS) {
		m_LOG_MAB_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR, MAB_MAS_ERR_MDS_SUBSCRIBE_FAILED, status);
		m_MAS_UNLK(&inst->lock);
		m_MMGR_FREE_MAS_TBL(inst);
		m_MMGR_FREE_MAS_CSI_NODE(csi);
 ...
 *  NOTE:                                                                     * 
\*****************************************************************************/
static uns32 mas_amf_csi_vdest_role_change(MAS_CSI_NODE *csi, SaAmfHAStateT haState)
{
	uns32 status;
	NCSMDS_INFO info;
	NCSVDA_INFO vda_info;
	...
	if (status != NCSCC_RC_SUCCESS) {
		/* log the failure */
		m_LOG_MAB_ERROR_DATA(NCSFL_SEV_CRITICAL, NCSFL_LC_SVC_PRVDR, MAB_MAS_ERR_MDS_PWE_QUERY_FAILED, status);
 ...
 *  NOTE:                                                                     * 
 \****************************************************************************/
static MAS_CSI_NODE *mas_amf_csi_list_find_envid(PW_ENV_ID env_id)
{
	MAS_CSI_NODE *csi_node = NULL;
 ...
 *  NOTE:                                                                     * 
 \****************************************************************************/
static MAS_CSI_NODE *mas_amf_csi_list_find_csiname(const SaNameT *find_me)
{
	SaNameT t_find_me;
...
*  NOTE:                                                                     * 
\****************************************************************************/
static MAS_CSI_NODE *mas_amf_csi_list_delink(const SaNameT *csiName)
{
	SaNameT t_del_me;
	MAS_CSI_NODE *del_me = NULL;
...
*  NOTE:                                                                     * 
\****************************************************************************/
void mas_amf_csi_list_add_front(MAS_CSI_NODE *new_csi)
{
	/* validate the input */
	if (new_csi == NULL)
		return;
...
static PW_ENV_ID mas_amf_csi_attrib_decode(SaAmfCSIAttributeListT csiAttr)
{
	...
	if (csiAttr.attr == NULL) {
		...
		m_LOG_MAB_ERROR_NO_DATA(NCSFL_SEV_ERROR, NCSFL_LC_HEADLINE, MAB_MAS_ERR_AMF_CSI_ATTRIBS_NULL);
		return 0;
	}
	...
	if (strcmp(csiAttr.attr->attrName, "Env-Id") != 0) {
		/* log that CSI Attribute Name/Value is empty */
		m_LOG_MAB_ERROR_NO_DATA(NCSFL_SEV_ERROR, NCSFL_LC_HEADLINE, MAB_MAS_ERR_AMF_CSI_ATTRIBS_NAME_INVALID);
		return 0;
	}
	...
	if (num_args == 0) {
		m_LOG_MAB_ERROR_NO_DATA(NCSFL_SEV_ERROR, NCSFL_LC_HEADLINE, MAB_MAS_ERR_AMF_CSI_SSCANF_FAILED);
		return 0;
	}
	...
	if ((decoded_env <= 0) || (decoded_env > NCSMDS_MAX_PWES)) {
		/* log the error */
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_AMF_CSI_DECODED_ENV_ID_OUTOF_REACH, decoded_env);
		return 0;
	}
...
void mas_amf_sigusr1_handler(int i_sig_num)
{
	/* ignore the signal */
...
uns32 mas_amf_initialize(NCS_APP_AMF_ATTRIBS *amf_attribs)
{
	...
	if (saf_status != SA_AIS_OK) {
		/* log the failure */
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_AMF_INITIALIZE_FAILED, saf_status);
		if (saf_status == SA_AIS_ERR_TRY_AGAIN) {
			...
			if (status == NCSCC_RC_FAILURE) {
				...
				m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_AMF_TIMER_FAILED, status);
...
\****************************************************************************/
uns32 mas_table_rec_list_free(MAS_ROW_REC *tbl_list)
{
	...
	for (tbl_rec = tbl_list; tbl_rec != NULL; is_next_tbl_set = FALSE) {
		...
		for (fltr = tbl_rec->fltr_list; fltr != NULL;) {
			...
			m_LOG_MAB_FLTR_DETAILS(NCSFL_LC_DATA, NCSFL_SEV_DEBUG,
					       MAB_MAS_FLTR_RMV_SUCCESS, tbl_rec->tbl_id, 0, del_fltr->fltr.type);
			/* free this filter */
...
uns32 mas_amf_mds_quiesced_process(NCSCONTEXT mas_hdl)
{
	...
	saAmfResponse(gl_mas_amf_attribs.amf_attribs.amfHandle, inst->amf_invocation_id, SA_AIS_OK);
