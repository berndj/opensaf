...
static uns32 mas_mds_evt_cb(NCSMDS_CALLBACK_INFO *cbinfo);

/* posts message to MAS mailbox */
static uns32 mas_mds_mbx_post(NCSMDS_CALLBACK_INFO *cbinfo, MAB_SVC_OP evt);
...
mas_mds_rcv(MDS_CLIENT_HDL yr_svc_hdl, NCSCONTEXT msg,
	    MDS_DEST fr_card, SS_SVC_ID fr_svc, V_DEST_QA fr_anc, MDS_DEST to_card, SS_SVC_ID to_svc)
{
	uns32 status;
	...
	if (inst) {
		m_LOG_MAB_SVC_PRVDR_MSG(NCSFL_SEV_INFO, MAB_SP_MAS_MDS_RCV_MSG, fr_svc, fr_card, ((MAB_MSG *)msg)->op);
	}
	...
	if (status != NCSCC_RC_SUCCESS) {
		m_LOG_MAB_ERROR_II(NCSFL_LC_SVC_PRVDR, NCSFL_SEV_CRITICAL,
				   MAB_MAS_ERR_MDS_TO_MBX_SND_FAILED, status, ((MAB_MSG *)msg)->op);
...
uns32 mas_mds_cb(NCSMDS_CALLBACK_INFO *cbinfo)
{
	...
	default:
		m_LOG_MAB_ERROR_I(NCSFL_SEV_ERROR, MAB_MAS_ERR_MDS_CB_INVALID_OP, cbinfo->i_op);
