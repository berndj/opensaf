...
typedef void *(*MAS_FLTR_FNC)(struct mas_tbl * inst, NCSMAB_FLTR *fltr, uns32 *inst_ids, uns32 inst_len,
...
*****************************************************************************/
EXTERN_C void mas_fltr_put(MAS_FLTR **insert_after, MAS_FLTR *new_fltr);

EXTERN_C MAS_FLTR *mas_fltr_rmv(MAS_FLTR **head, MAS_FLTR *del_me);

EXTERN_C MAS_FLTR *mas_fltr_find(MAS_TBL *inst, MAS_FLTR **head, uns32 fltr_id, MAB_ANCHOR anc, MDS_DEST vcard);

EXTERN_C MAS_FLTR *mas_fltr_create(MAS_TBL *inst,
				   NCSMAB_FLTR *mab_fltr,
				   MDS_DEST vcard,
				   V_DEST_RL fr_oaa_role, MAB_ANCHOR anc, uns32 fltr_id, MAS_FLTR_FNC fltr_fnc);

EXTERN_C void *mas_classify_range(MAS_TBL *inst,
				  NCSMAB_FLTR *fltr, uns32 *inst_ids, uns32 inst_len, NCS_BOOL is_next_req);
...
EXTERN_C void *mas_classify_exact(MAS_TBL *inst, NCSMAB_FLTR *fltr,
				  uns32 *inst_ids, uns32 inst_len, NCS_BOOL is_next_req);

EXTERN_C MAS_FLTR *mas_locate_dst_oac(MAS_TBL *inst,
				      MAS_FLTR *head, uns32 *inst_ids, uns32 inst_len, NCS_BOOL is_next_req);
...
EXTERN_C uns32 mas_forward_msg(MAB_MSG *msg,
			       MAB_SVC_OP msg_op,
			       MDS_HDL mds_hdl, MDS_DEST to_vcard, SS_SVC_ID to_svc_id, NCS_BOOL is_req);

EXTERN_C uns32 mas_relay_msg_to_mac(MAB_MSG *msg, MAS_TBL *inst, NCS_BOOL is_req);

EXTERN_C uns32 mas_next_op_stack_push(MAS_TBL *inst, NCS_STACK *stack, uns32 fltr_id);
