...
EXTERN_C void pss_mbx_amf_process(SYSF_MBX *pss_mbx);

EXTERN_C uns32 pss_amf_componentize(struct ncs_app_amf_attribs *amf_attribs, SaAisErrorT *o_saf_status);

/* add the env details to the front of the global CSI list */
EXTERN_C void pss_amf_csi_list_add_front(PSS_CSI_NODE *);

EXTERN_C PSS_CSI_NODE *pss_amf_csi_install(PW_ENV_ID, SaAmfHAStateT);

/* to remove all the CSIs in PSS */
EXTERN_C SaAisErrorT pss_amf_csi_remove_all(void);

EXTERN_C uns32 pss_confirm_ha_role(PW_ENV_ID pwe_id, SaAmfHAStateT ha_role);

/* Cleanup function of PSS */
EXTERN_C uns32 pss_amf_prepare_will(void);

/* USR1 signal handler of PSS */
EXTERN_C void pss_amf_sigusr1_handler(int i_sig_num);

/* Decodes the CSI attribute list */
EXTERN_C PW_ENV_ID pss_amf_csi_attrib_decode(SaAmfCSIAttributeListT);

EXTERN_C uns32 pss_amf_vdest_role_chg_api_callback(MDS_CLIENT_HDL handle);
/* Function which initialises interface with AMF */
EXTERN_C uns32 pss_amf_initialize(NCS_APP_AMF_ATTRIBS *amf_attribs, SaAisErrorT *o_saf_status);
