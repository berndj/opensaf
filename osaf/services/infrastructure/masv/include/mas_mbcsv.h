...
EXTERN_C uns32 mas_mbcsv_interface_initialize(MAS_MBCSV_ATTRIBS *mbcsv_attribs);

/* MAS - MBCSv callback function */
EXTERN_C uns32 mas_mbcsv_cb(NCS_MBCSV_CB_ARG *arg);

/* Active MAS, Async Update API */
EXTERN_C uns32 mas_red_sync(struct mas_tbl *inst, MAB_MSG *msg, MAS_ASYNC_UPDATE_TYPE type);

/* send the async done message to the standby MAS */
EXTERN_C uns32 mas_red_sync_done(struct mas_tbl *inst, MAS_ASYNC_UPDATE_TYPE type);

/* dump masv dictionary */
EXTERN_C void mas_dictionary_dump(void);
