...
EXTERN_C void mas_mbx_amf_process(SYSF_MBX *mas_mbx);

/* add the env details to the front of the global CSI list */
EXTERN_C void mas_amf_csi_list_add_front(MAS_CSI_NODE *);

/* Function which creates CSI either in ACTIVE or STANDBY state */
EXTERN_C MAS_CSI_NODE *mas_amf_csi_install(PW_ENV_ID, SaAmfHAStateT);

/* Cleanup function of MAS */
EXTERN_C uns32 mas_amf_prepare_will(void);

/* USR1 signal handler function */
EXTERN_C void mas_amf_sigusr1_handler(int i_sig_num);

/* Function which initialises interface with AMF */
EXTERN_C uns32 mas_amf_initialize(NCS_APP_AMF_ATTRIBS *amf_attribs);

/* frees the associated filters */
EXTERN_C uns32 mas_table_rec_list_free(MAS_ROW_REC *tbl_list);

/* sends the AMF response for QUIESCED state transition */
EXTERN_C uns32 mas_amf_mds_quiesced_process(NCSCONTEXT mas_hdl);
