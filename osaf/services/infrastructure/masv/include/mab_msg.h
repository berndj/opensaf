...
EXTERN_C uns32 mas_mab_range_fltr_clone(NCSMAB_RANGE *src_range, NCSMAB_RANGE *dst_range);

/* clones a exact filter indices */
EXTERN_C uns32 mas_mab_exact_fltr_clone(NCSMAB_EXACT *src_exact, NCSMAB_EXACT *dst_exact);

/* frees the indices information based on the type of the filter, and frees the 
 * message 
 */
EXTERN_C void mas_mab_fltr_indices_cleanup(NCSMAB_FLTR *fltr);

/* to free the messages in the mail-box of MAA, OAA, and MAS */
EXTERN_C NCS_BOOL mab_leave_on_queue_cb(void *arg1, void *arg2);

EXTERN_C uns32 mab_fltr_encode(NCSMAB_FLTR *mf, NCS_UBAID *uba);

EXTERN_C uns32 mab_fltr_decode(NCSMAB_FLTR *mf, NCS_UBAID *uba);
