...
#
#  /etc/drbd.conf & /opt/opensaf/controller/scripts/*drbd*
#  /etc/opt/opensaf/
#  /opt/opensaf/controller/scripts/*drbd*
#  /var/crash/core*ncs*
#  /var/log/messages
#  /var/log/snmpd.log
#  /var/log/ncsSnmpSubagt.log
#  /var/opt/opensaf/
#  
...
declare LOG_DIR[$MAX_LOG_DIR]=(drbd_config /etc/opt/opensaf/ coredumps /var/log/messages /var/log/snmpd.log /var/log/ncsSnmpSubagt.log /var/opt/opensaf/) 
...
echo Usage :
echo
echo "1 /etc/drbd.conf & /opt/opensaf/controller/scripts/*drbd*"
echo 2  /etc/opt/opensaf/
echo "3 coredumps /var/crash/core*ncs*"
echo 4  /var/log/messages
echo 5  /var/log/snmpd.log
echo 6  /var/log/ncsSnmpSubagt.log
echo 7  /var/opt/opensaf/
...
echo "      2. If you are using DRBD, mount partition manually"
echo =====================================================================
echo
exit
fi
...
zip_current_object()
{
    ...
    elif [ $CURRENT_OBJECT = "drbd-config" ]
        then	
	    DISK_SPACE_REQUIRED=`du -sck /etc/drbd.conf /opt/opensaf/controller/scripts/*drbd* | grep total | cut -f1`
        else
    ...
    then
        ...
        then
            nice -n 17 tar -h -zcf /var/log/$SUB_TAR_NAME  /opt/opensaf/controller/scripts/*drbd* /etc/drbd.conf 2>/dev/null
        else
...
MASTER_TAR_NAME=collected_logs_controller_${HOSTNAME}_`date +%F_%H-%M-%S`.tar
SEPERATE_FILES=0
CURRENT_OBJECT=""
